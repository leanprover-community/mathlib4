...
example {α : Type _} [CommRing α] {a b c d : α} :
  ...
  simp? [Matrix.det_succ_row_zero, Fin.sum_univ_succ] says
    simp only [det_succ_row_zero, Nat.reduceSucc, Fin.isValue, of_apply,
      cons_val', empty_val', cons_val_fin_one, cons_val_zero, det_unique, Fin.default_eq_zero,
      submatrix_apply, Fin.succ_zero_eq_one, cons_val_one, head_fin_const, Fin.sum_univ_succ,
      Fin.val_zero, pow_zero, one_mul, Fin.zero_succAbove, head_cons, Finset.univ_unique,
