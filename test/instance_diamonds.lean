...
example : RestrictScalars.algebra ℝ ℂ ℂ = Complex.instAlgebraOfReal_mathlib := by
  rfl
...
example :
    @EuclideanDomain.toCommRing _ (@Field.toEuclideanDomain _ (ZMod.instField_mathlib p)) =
      ZMod.commRing p := by
  with_reducible_and_instances rfl
...
example : (Algebra.id ℂ).complexToReal = Complex.instAlgebraOfReal_mathlib := rfl
