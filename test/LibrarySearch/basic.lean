...
-- Recall that `std_apply?` caches the discrimination tree on disk.
-- If you are modifying the way that `std_apply?` indexes lemmas,
...
#guard_msgs in
example (x : Nat) : x ≠ x.succ := ne_of_lt (by std_apply?)

/-- info: Try this: exact Nat.zero_lt_succ 1 -/
#guard_msgs in
example : 0 ≠ 1 + 1 := ne_of_lt (by std_apply?)
...
#guard_msgs in
example (a b : ℕ) (ha : 0 < a) (_hb : 0 < b) : 0 < a + b := by std_apply?
...
/-- info: Try this: exact Nat.add_pos_left ha b -/
#guard_msgs in
example (a b : ℕ) (ha : a > 0) (_hb : 0 < b) : 0 < a + b := by std_apply?
...
#guard_msgs in
example (a : ℕ) (h : P a) : 0 > a := by std_apply?
/-- info: Try this: exact lemma_with_gt_in_head a h -/
#guard_msgs in
example (a : ℕ) (h : P a) : a < 0 := by std_apply?

/-- info: Try this: exact lemma_with_false_in_head a b h1 h2 -/
#guard_msgs in
example (a b : ℕ) (h1 : a < b) (h2 : P a) : False := by std_apply?

-- TODO this no longer works:
-- example (a b : ℕ) (h1 : a < b) : ¬ (P a) := by std_apply? -- says `exact lemma_with_false_in_head a b h1`
...
#guard_msgs in
example {a b c : ℕ} (h₁ : a ∣ c) (h₂ : a ∣ b + c) : a ∣ b := by std_apply?
...
-- example (n : ℕ) (r : ℚ) : ℚ := by std_apply? using n, r -- exact nsmulRec n r
...
#guard_msgs in
example (a b : ℕ) (h : a ≤ b) : f a ≤ f b := by std_apply?

/-- info: Try this: exact List.join L -/
#guard_msgs in
example (L _M : List (List ℕ)) : List ℕ := by std_apply? using L

-- Could be any number of results
#guard_msgs (drop info) in
example (P _Q : List ℕ) (h : ℕ) : List ℕ := by std_apply? using h, P
...
#guard_msgs (drop info) in
example (n m : ℕ) : ℕ := by std_apply? using n, m

-- Could be any number of results
#guard_msgs (drop info) in
example (P Q : List ℕ) (_h : ℕ) : List ℕ := by std_apply? using P, Q
...
theorem Bool_eq_iff2 {A B : Bool} : (A = B) = (A ↔ B) :=
  by std_apply? -- exact Bool_eq_iff
...
#guard_msgs in
example {r : α → α → Prop} : Function.Surjective (Quot.mk r) := by std_exact?
...
--   first
--   | std_exact? says exact le_antisymm hxy hyx
--   | std_exact? says exact ge_antisymm hyx hxy
