...
@[fun_prop] lemma Con_id : Con (id : α → α) := silentSorry
@[fun_prop] lemma Con_const (y : β) : Con (fun x : α => y) := silentSorry
@[fun_prop] lemma Con_apply (x : α) : Con (fun f : α → β => f x) := silentSorry
@[fun_prop] lemma Con_applyDep (x : α) : Con (fun f : (x' : α) → E x' => f x) := silentSorry
@[fun_prop] lemma Con_comp (f : β → γ) (g : α → β) (hf : Con f) (hg : Con g) : Con (fun x => f (g x)) := silentSorry
@[fun_prop] lemma Con_let (f : α → β → γ) (g : α → β) (hf : Con (fun (x,y) => f x y)) (hg : Con g) : Con (fun x => let y:= g x; f x y) := silentSorry
@[fun_prop] lemma Con_pi (f : β → (i : α) → (E i)) (hf : ∀ i, Con (fun x => f x i)) : Con (fun x i => f x i) := silentSorry

-- Lin is missing `const` lemma
@[fun_prop] lemma Lin_id : Lin (fun x : α => x) := silentSorry
@[fun_prop] lemma Lin_apply (x : α) : Lin (fun f : α → β => f x) := silentSorry
@[fun_prop] lemma Lin_applyDep (x : α) : Lin (fun f : (x' : α) → E x' => f x) := silentSorry
@[fun_prop] lemma Lin_comp (f : β → γ) (g : α → β) (hf : Lin f) (hg : Lin g) : Lin (f ∘ g) := silentSorry
@[fun_prop] lemma Lin_pi {ι} (f : α → ι → γ) (hf : ∀ i, Lin (fun x => f x i)) : Lin (fun x i => f x i) := silentSorry


-- this is to stress test detection of loops
@[fun_prop]
lemma kaboom (f : α → β) (hf : Con f) : Con f := hf

-- currently only trivial loops are detected
-- make it more sophisticated such that longer loops are detected
-- @[fun_prop]
-- lemma chabam (f : α → β) (hf : Con f) : Con f := hf


-- transition lemma --
------------------------
@[fun_prop] lemma lin_to_con (f : α → β) (hf : Lin f) : Con f := silentSorry


-- lemmas about function in the environment --
------------------------------------------------
@[fun_prop]
lemma prod_mk_Con (fst : α → β) (snd : α → γ) (hfst : Con fst) (hsnd : Con snd)
  : Con fun x => (fst x, snd x) := silentSorry
@[fun_prop]
lemma prod_mk_Lin (fst : α → β) (snd : α → γ) (hfst : Lin fst) (hsnd : Lin snd)
  : Lin fun x => (fst x, snd x) := silentSorry


variable [Add α] [Add β]

-- "simple form" of lemmas
@[fun_prop] lemma fst_Con : Con fun x : α×β => x.1 := silentSorry
@[fun_prop] lemma snd_Con : Con fun x : α×β => x.2 := silentSorry
@[fun_prop] lemma add_Con : Con (Function.uncurry (fun x y : α => x + y)) := silentSorry
@[fun_prop] lemma add_Lin : Lin ↿(fun x y : α => x + y) := silentSorry


-- "compositional form" of lemmas
@[fun_prop] lemma fst_Con' (self : α → β×γ) (hself : Con self) : Con fun x => (self x).1 := by fun_prop
@[fun_prop] lemma snd_Con' (self : α → β×γ) (hself : Con self) : Con fun x => (self x).2 := by fun_prop
@[fun_prop] lemma add_Con' (x y : α → β) (hx : Con x) (hy : Con y) : Con (fun w => x w + y w) := by fun_prop
@[fun_prop] lemma add_Lin' (x y : α → β) (hx : Lin x) (hy : Lin y) : Lin (fun w => x w + y w) := by fun_prop
...
-- morphism lemmas i.e. theorems about `FunLike.coe` --
---------------------------------------------------------

-- this is some form of cartesion closedness with homs `α ->> β`
@[fun_prop] lemma conHom_con' (f : α → β ->> γ) (g : α → β) (hf : Con f) (hg : Con g) : Con (fun x => (f x) (g x)) := silentSorry

@[fun_prop] lemma conHom_lin_in_fn' (f : α → β ->> γ) (y : β) (hf : Lin f) : Lin (fun x => f x y) := silentSorry

-- analogous lemma with `α -o β` does no hold
@[fun_prop] lemma linHom_lin (f : α -o β) : Lin f := silentSorry
@[fun_prop] lemma linHom_lin_in_fn' (f : α → β -o γ) (y : β) (hf : Lin f) : Lin (fun x => f x y) := silentSorry


def LinHom.mk' (f : α → β) (hf : Lin f := by fun_prop) : α -o β := mk f hf

@[fun_prop] lemma linHom_mk' (f : α → β → γ) (hx : ∀ y, Lin (f · y)) (hy : ∀ x, Lin (f x ·)) : Lin (fun x => LinHom.mk' (f x)) := silentSorry
...
-- the only analoge is this lemma but that is alredy provable
...
-- apply lemmas about FunLike.coe
...
@[fun_prop]
lemma foo_lin : Lin fun x : α => foo x := silentSorry
@[fun_prop]
lemma bar_lin (y) : Lin fun x : α => bar x y := silentSorry
...
lemma iterate_con (n : Nat) (f : α → α) (hf : Con f) : Con (iterate n f) := by
...
@[fun_prop]
lemma foo1_lin : Lin (foo1 : α → α) := silentSorry
@[fun_prop]
lemma foo2_lin : Lin (foo2 : α → α) := silentSorry
...
lemma diag_Con (f : α → α → α) (hf : Con (myUncurry f)) : Con (fun x => diag f x) := by
