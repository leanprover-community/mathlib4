...
example (p q r : Prop) : True :=
  by
  have : p ∨ ¬p := by fail_if_success itauto; sorry; clear this
  have : ¬(p ↔ q) → ¬p → q := by fail_if_success itauto; sorry; clear this
  have : ¬(p ↔ q) → (r ↔ q) → (p ↔ ¬r) := by fail_if_success itauto; sorry; clear this
  trivial

example (P : ℕ → Prop) (n : ℕ) (h : ¬(n = 7 ∨ n = 0) ∧ P n) : ¬(P n → n = 7 ∨ n = 0) := by itauto
