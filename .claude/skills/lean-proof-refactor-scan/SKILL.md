...
你的任务是扫描当前 Lean 源文件，进行如下检查：

1. 检查当前文件的证明中有没有重复造轮子，推荐用 lean-lsp-mcp 进行检索。对于重复造轮子的代码进行重构，希望重构完行数减少。

2. 注意，希望重构的地方证明有本质性改变，单纯调整缩进、或者语法糖/风格的压行是没有必要的。

   不要为了压行调整缩进，也不要嵌套 `simpa` 丧失可读性。
   ...
   + have : ∀ᶠ y in 𝓝 (0 : E), f (x + y) - f x ∈ Metric.eball 0 q.radius :=
   +   ((hf.continuousAt.comp_of_eq (by fun_prop) (by simp)).sub continuousAt_const).preimage_mem_nhds
   +     (by simpa using Metric.eball_mem_nhds (0 : F) hq)
   ```
