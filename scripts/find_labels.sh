...
prs=$(gh pr list --repo "$repo_owner/$repo_name" --state closed --search "closedAt:>$start_date closedAt:<$end_date" --json number,labels,title,closedAt --limit "$((commits_in_range * 2))")

# Print PR numbers and their labels
echo "$prs" | jq -r '.[] | select(.title | startswith("[Merged by Bors]")) | "PR #\(.number) - Labels: \((.labels | map(.name) | join(", ")) // "No labels") - ClosedAt: \(.closedAt) - Title: \(.title)"'
