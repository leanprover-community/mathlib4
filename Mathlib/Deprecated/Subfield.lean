...
theorem IsSubfield.mul_mem {s : Set F} (h : IsSubfield s) {a b : F} : a ∈ s → b ∈ s → a * b ∈ s :=
  h.mul_mem'

theorem IsSubfield.one_mem {s : Set F} (h : IsSubfield s) : 1 ∈ s :=
  h.one_mem'

theorem IsSubfield.inv_mem {s : Set F} (h : IsSubfield s) {a : F} : a ∈ s → a⁻¹ ∈ s :=
  h.inv_mem' _

theorem IsSubfield.add_mem {s : Set F} (h : IsSubfield s) {a b : F} : a ∈ s → b ∈ s → a + b ∈ s :=
  h.add_mem'

theorem IsSubfield.zero_mem {s : Set F} (h : IsSubfield s) : 0 ∈ s :=
  h.zero_mem'

theorem IsSubfield.neg_mem {s : Set F} (h : IsSubfield s) {a : F} : a ∈ s → -a ∈ s :=
  h.neg_mem'
...
theorem Image.isSubfield {K : Type*} [Field K] (f : F →+* K) {s : Set F} (hs : IsSubfield s) :
  ...
  { f.isSubring_image hs.toIsSubring with
    inv_mem' := fun _ ⟨x, xmem, ha⟩ ↦ ⟨x⁻¹, hs.inv_mem xmem, ha ▸ map_inv₀ f x⟩ }
...
theorem closure.isSubfield : IsSubfield (closure S) :=
  { closure.isSubmonoid with
    ...
    zero_mem' := ring_closure_subset Ring.closure.isSubring.zero_mem
    neg_mem' := by
...
theorem IsSubfield.inter {S₁ S₂ : Set F} (hS₁ : IsSubfield S₁) (hS₂ : IsSubfield S₂) :
  ...
  { IsSubring.inter hS₁.toIsSubring hS₂.toIsSubring with
    inv_mem' := fun _ hx ↦ ⟨hS₁.inv_mem hx.1, hS₂.inv_mem hx.2⟩ }
...
theorem IsSubfield.iInter {ι : Sort*} {S : ι → Set F} (h : ∀ y : ι, IsSubfield (S y)) :
  ...
  { IsSubring.iInter fun y ↦ (h y).toIsSubring with
    inv_mem' := fun _ hx ↦ Set.mem_iInter.2 fun y ↦ (h y).inv_mem <| Set.mem_iInter.1 hx y }
