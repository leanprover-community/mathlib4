...
lemma comp (hf : IsSemiringHom f) {γ} [Semiring γ] {g : β → γ} (hg : IsSemiringHom g) :
    IsSemiringHom (g ∘ f) :=
...
lemma to_isAddMonoidHom (hf : IsSemiringHom f) : IsAddMonoidHom f :=
...
lemma to_isMonoidHom (hf : IsSemiringHom f) : IsMonoidHom f :=
...
lemma map_zero (hf : IsRingHom f) : f 0 = 0 :=
  calc
...
lemma map_neg (hf : IsRingHom f) : f (-x) = -f x :=
  calc
...
lemma map_sub (hf : IsRingHom f) : f (x - y) = f x - f y := by
...
lemma comp (hf : IsRingHom f) {γ} [Ring γ] {g : β → γ} (hg : IsRingHom g) : IsRingHom (g ∘ f) :=
...
lemma to_isSemiringHom (hf : IsRingHom f) : IsSemiringHom f :=
