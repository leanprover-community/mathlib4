...
theorem splits_zmod_X_pow_sub_X : Splits (Polynomial.map (RingHom.id (ZMod p)) (X ^ p - X)) := by
  have hp : 1 < p := h_prime.out.one_lt
...
theorem _root_.FiniteField.splits_X_pow_card_sub_X :
    Splits (Polynomial.map (algebraMap (ZMod p) K) (X ^ Fintype.card K - X)) :=
...
theorem _root_.FiniteField.splits_X_pow_nat_card_sub_X [Finite K] :
    Splits (Polynomial.map (algebraMap (ZMod p) K) (X ^ Nat.card K - X)) := by
