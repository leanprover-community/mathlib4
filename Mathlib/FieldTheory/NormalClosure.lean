...
lemma isSplittingField_iff_isNormalClosure {x : K} :
    IsSplittingField F L (minpoly F x) ↔ IsNormalClosure F F⟮x⟯ L := by
  by_cases ix : IsIntegral F x
  · refine ⟨fun h ↦ ⟨fun y ↦ ?_, ?_⟩, fun h ↦ isSplittingField_iff_intermediateField.mpr ?_⟩
    · rw [← minpoly.algHom_eq _ F⟮x⟯.val.injective]
      refine splits_of_mem_adjoin F _ (fun x h ↦ ?_) y.2
      cases h; exact ⟨ix, h.splits⟩
    · refine top_le_iff.mp (le_trans ?_ <| le_iSup _ ⟨x, subset_adjoin _ _ rfl⟩)
      rw [← minpoly.algHom_eq _ F⟮x⟯.val.injective]
      exact (Eq.ge h.adjoin_rootSet).trans (algebra_adjoin_le_adjoin F _)
    · have := h.1 ⟨x, subset_adjoin _ _ rfl⟩
      rw [← minpoly.algHom_eq _ F⟮x⟯.val.injective] at this
      refine ⟨this, ?_⟩
      have := h.2

theorem IsNormalClosure.nonempty_algHom [IsNormalClosure F K L] :
    Nonempty (integralClosure F K →ₐ[F] L) :=

theorem isNormalClosure_iff_of_adjoin_eq {S : Set K}
    (hS : (adjoin F S).toSubalgebra = integralClosure F K) :
    IsNormalClosure F K L ↔ (∀ s ∈ S, (minpoly F s).Splits (algebraMap F L)) ∧
      ⨆ s ∈ S, adjoin F ((minpoly F s).rootSet L) = ⊤ := by
...
instance isNormalClosure_normalClosure : IsNormalClosure F K (normalClosure F K L) := by
...
noncomputable def IsNormalClosure.lift [h : IsNormalClosure F K L] {L'} [Field L'] [Algebra F L']
  ...
  · rw [← minpoly.eq_of_irreducible_of_monic
        (minpoly.irreducible iy) (mem_rootSet.mp hx).2 (minpoly.monic iy)]
    exact splits y
...
lemma isNormalClosure_normalClosure [ne : Nonempty (K →ₐ[F] L)] [h : Normal F L] :
