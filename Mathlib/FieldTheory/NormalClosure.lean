...
variable (F : Type*) (K : Type w) (L : Type u)
...
open Cardinal in
theorem Algebra.IsAlgebraic.cardinal_mk_algHom_le_of_splits (alg : IsAlgebraic F K)
    (h : ∀ x : K, (minpoly F x).Splits (algebraMap F L)) (L' : Type v) [Field L'] [Algebra F L'] :
    lift.{u} #(K →ₐ[F] L') ≤ lift.{v} #(K →ₐ[F] L) := by
  obtain ⟨φ⟩ : Nonempty (↑(⨆ x : K, IntermediateField.adjoin F ((minpoly F x).rootSet L')) →ₐ[F] L)
  · rw [← gc.l_iSup]
    ...
    · simp [minpoly.eq_zero iy] at hx
  rw [← lift_le.{w}, lift_lift, lift_lift]
  let φ := φ.comp (inclusion <| normalClosure_le_iSup_adjoin alg)
  refine Cardinal.lift_mk_le_lift_mk_of_injective
    (f := φ.comp ∘ (normalClosure.algHomEquiv F K L').symm) fun ψ₁ ψ₂ h ↦ ?_
  apply (normalClosure.algHomEquiv F K L').symm.injective
