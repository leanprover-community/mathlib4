...
theorem splits [IsGalois F E] (x : E) : (Polynomial.map (algebraMap F E) (minpoly F x)).Splits :=
  Normal.splits' x
...
theorem IntermediateField.AdjoinSimple.card_aut_eq_finrank [FiniteDimensional F E] {α : E}
    ...
    (h_splits : (Polynomial.map (algebraMap F F⟮α⟯) (minpoly F α)).Splits) :
...
theorem card_aut_eq_finrank [FiniteDimensional F E] [IsGalois F E] :
  ...
  have h_splits : (Polynomial.map (algebraMap F E) (minpoly F α)).Splits := IsGalois.splits F α
  replace h_splits : Polynomial.Splits (Polynomial.map (algebraMap F F⟮α⟯) (minpoly F α)) := by
    simpa using
