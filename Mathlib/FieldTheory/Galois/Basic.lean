...
protected theorem mem_fixingSubgroup_iff (f : E ≃ₐ[F] E) :
    f ∈ K.fixingSubgroup ↔ ∀ x ∈ K, f x = x := by
  simp only [fixingSubgroup, _root_.mem_fixingSubgroup_iff, SetLike.mem_coe, AlgEquiv.smul_def]
