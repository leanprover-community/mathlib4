...
theorem card_rootSet_eq_natDegree [Algebra F K] {p : F[X]} (hsep : p.Separable)
    (hsplit : Splits (Polynomial.map (algebraMap F K) p)) : Fintype.card (p.rootSet K) = p.natDegree := by
  classical
...
theorem nodup_roots_iff_of_splits {f : F[X]} (hf : f ≠ 0) (h : (Polynomial.map (RingHom.id F) f).Splits) :
...
theorem nodup_aroots_iff_of_splits [Algebra F K] {f : F[X]} (hf : f ≠ 0)
    (h : (Polynomial.map (algebraMap F K) f).Splits) : (f.aroots K).Nodup ↔ f.Separable := by
...
theorem card_rootSet_eq_natDegree_iff_of_splits [Algebra F K] {f : F[X]} (hf : f ≠ 0)
    (h : (Polynomial.map (algebraMap F K) f).Splits) : Fintype.card (f.rootSet K) = f.natDegree ↔ f.Separable := by
  classical
...
theorem eq_X_sub_C_of_separable_of_root_eq {x : F} {h : F[X]} (h_sep : h.Separable)
    (h_root : h.eval x = 0) (h_splits : Splits (Polynomial.map i h)) (h_roots : ∀ y ∈ (h.map i).roots, y = i x) :
...
theorem exists_finset_of_splits (i : F →+* K) {f : F[X]} (sep : Separable f) (sp : Splits (Polynomial.map i f)) :
...
theorem AlgHom.natCard_of_powerBasis (pb : PowerBasis K S) (h_sep : IsSeparable K pb.gen)
    (h_splits : (Polynomial.map (algebraMap K L) (minpoly K pb.gen)).Splits) :
