...
open Polynomial in
theorem degree_minpoly_eq_one_iff : (minpoly A x).degree = 1 ↔ x ∈ (algebraMap A B).range := by
...
theorem two_le_natDegree_minpoly (int : IsIntegral A x) :
...
theorem two_le_natDegree_minpoly_subalgebra {B} [CommRing B] [Algebra A B] [Nontrivial B]
