...
theorem natDegree_le_spanFinrank [Module.Finite A B] :
...
variable (x) in
theorem degree_le [Nontrivial A] [Module.Finite A B] [Module.Free A B] :
    (minpoly A x).degree ≤ Module.finrank A B := degree_le_of_natDegree_le <| natDegree_le x
