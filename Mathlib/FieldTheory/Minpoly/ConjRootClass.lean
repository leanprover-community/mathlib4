...
theorem zero_def : (0 : ConjRootClass K L) = mk K 0 :=
  rfl
...
theorem exist_mem_carrier_add_eq_zero (x y : ConjRootClass K L) :
    (∃ a b : L, (a ∈ x.carrier ∧ b ∈ y.carrier) ∧ a + b = 0) ↔ x = -y := by
  simp_rw [mem_carrier]
  constructor
  ...
  · rintro rfl
    ...
    refine ⟨-y, y, ?_⟩
    simp_rw [mk_neg, neg_add_cancel, and_self]
...
theorem separable_minpoly (c : ConjRootClass K L) :
    Separable c.minpoly := by
  induction c
