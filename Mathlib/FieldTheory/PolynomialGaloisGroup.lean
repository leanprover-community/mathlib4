...
instance (priority := 10000) instGroup : Group (Gal p) :=
  inferInstanceAs (Group (p.SplittingField ≃ₐ[F] p.SplittingField))
instance (priority := 10000) instFintype : Fintype (Gal p) :=
  inferInstanceAs (Fintype (p.SplittingField ≃ₐ[F] p.SplittingField))

instance (priority := 10000) : CoeFun p.Gal fun _ => p.SplittingField → p.SplittingField :=
  -- Porting note: was AlgEquiv.hasCoeToFun
  inferInstanceAs (CoeFun (p.SplittingField ≃ₐ[F] p.SplittingField) _)

instance (priority := 10000) applyMulSemiringAction : MulSemiringAction p.Gal p.SplittingField :=
...
instance (priority := 10000) galActionAux : MulAction p.Gal (rootSet p p.SplittingField) where
...
instance (priority := 10000) smul [Fact (p.Splits (algebraMap F E))] : SMul p.Gal (rootSet p E) where
...
instance (priority := 10000) galAction [Fact (p.Splits (algebraMap F E))] : MulAction p.Gal (rootSet p E) where
