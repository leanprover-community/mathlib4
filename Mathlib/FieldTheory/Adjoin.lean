...
- `F⟮α₁,...,αₙ⟯`: adjoin the elements `α₁`, ..., `αₙ` to `F`.
- `F⟮S⟯`: adjoin the set of elements `S` to `F`.
-/
...
/-- `maybe_singleton% t` elaborates `t` and, if it is not a `Set`, coerces it to
be a `Set` using `singleton`. -/
syntax (name := maybeSingletonTerm) "maybe_singleton% " term : term

@[term_elab maybeSingletonTerm]
def elabMaybeSingletonTerm : Elab.Term.TermElab := fun stx expectedType? =>
  match stx with
  | `(maybe_singleton% $t) => do
    ...
    if ty.isAppOfArity' ``Set 1 then
      ...
      Elab.Term.elabTerm e' expectedType?
  | _ => Elab.throwUnsupportedSyntax

/-- If `x₁ x₂ ... xₙ : E` then `F⟮x₁,x₂,...,xₙ⟯` is the `IntermediateField F E`
generated by these elements, given by `adjoin F {x₁,x₂,...,xₙ}`.

If `S : Set E`, then `F⟮S⟯` is the `IntermediateField F E` generated by
this set of elements, given by `adjoin F S`. -/
macro:max K:term "⟮" xs:term,* "⟯" : term => do
  let xs := xs.getElems
  if xs.size == 1 then
    ``(adjoin $K (maybe_singleton% $(xs[0]!)))
  else
    ``(adjoin $K $(← mkInsertTerm xs))

@[delab app.IntermediateField.adjoin]
partial def delabAdjoinNotation : Delab := whenPPOption getPPNotation do
  guard <| (← getExpr).isAppOfArity ``adjoin 6
  let F ← withNaryArg 0 delab
  notation1 F <|> notation2 F
where
  delabInsertArray : DelabM (List Term) := do
    ...
    if e.isAppOfArity ``EmptyCollection.emptyCollection 2 then
      ...
      -- Do this second so that it's not run if this isn't ultimately `{x,y,z,...}` notation
      let x ← withNaryArg 3 delab
      return x :: xs
    else failure
