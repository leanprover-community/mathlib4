...
def elabMaybeSingletonTerm : Elab.Term.TermElab := fun stx expectedType? =>
  ...
  | `(maybe_singleton% $t) => do
    let res? ← Elab.Term.observing do
      let e ← Elab.Term.elabTerm t none
      let e ← instantiateMVars e
      let ty ← instantiateMVars (← Meta.inferType e)
      guard <| (ty.isAppOfArity' ``Set 1
               || e.isAppOfArity' ``EmptyCollection.emptyCollection 2
               || e.isAppOfArity' ``insert 5)
      return e
    if let .ok .. := res? then
      Elab.Term.applyResult res?
    else
      let t' ← `((singleton $t : Set _))
...
/-
variables [Field F] [Field E] [Algebra F E]
...
-- example (α : E) : F⟮{}⟯ ≤ F⟮α⟯ := sorry
-- Not supported:
-- example (α β : E) : F⟮α, β⟯ = F⟮{α, β}⟯ := sorry
-/
end AdjoinNotation
...
@[simp]
theorem adjoin_le_iff {S : Set E} {T : IntermediateField F E} : F⟮S⟯ ≤ T ↔ S ≤ T :=
...
theorem adjoin_eq_range_algebraMap_adjoin :
    (F⟮S⟯ : Set E) = Set.range (algebraMap F⟮S⟯ E) :=
...
theorem adjoin.algebraMap_mem (x : F) : algebraMap F E x ∈ F⟮S⟯ :=
...
theorem adjoin.range_algebraMap_subset : Set.range (algebraMap F E) ⊆ F⟮S⟯ := by
  intro x hx
  cases' hx with f hf
  rw [← hf]
...
instance adjoin.fieldCoe : CoeTC F F⟮S⟯
...
instance adjoin.setCoe : CoeTC S F⟮S⟯ where coe x := ⟨x, subset_adjoin F S (Subtype.mem x)⟩
#align intermediate_field.adjoin.set_coe IntermediateField.adjoin.setCoe

@[mono]
theorem adjoin.mono (T : Set E) (h : S ⊆ T) : F⟮S⟯ ≤ F⟮T⟯ :=
  GaloisConnection.monotone_l gc h
#align intermediate_field.adjoin.mono IntermediateField.adjoin.mono

theorem adjoin_contains_field_as_subfield (F : Subfield E) : (F : Set E) ⊆ F⟮S⟯ := fun x hx =>
...
theorem subset_adjoin_of_subset_left {F : Subfield E} {T : Set E} (HT : T ⊆ F) : T ⊆ F⟮S⟯ :=
  fun x hx => F⟮S⟯.algebraMap_mem ⟨x, HT hx⟩
#align intermediate_field.subset_adjoin_of_subset_left IntermediateField.subset_adjoin_of_subset_left

theorem subset_adjoin_of_subset_right {T : Set E} (H : T ⊆ S) : T ⊆ F⟮S⟯ := fun _ hx =>
...
@[simp]
theorem adjoin_empty (F E : Type _) [Field F] [Field E] [Algebra F E] : F⟮(∅ : Set E)⟯ = ⊥ :=
