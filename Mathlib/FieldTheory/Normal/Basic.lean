...
theorem splits_of_mem_adjoin {L} [Field L] [Algebra F L] {S : Set K}
    (splits : ∀ x ∈ S, IsIntegral F x ∧ (Polynomial.map (algebraMap F L) (minpoly F x)).Splits) {x : K}
    (hx : x ∈ adjoin F S) : (Polynomial.map (algebraMap F L) (minpoly F x)).Splits := by
  ...
  have : ∀ x ∈ S, (Polynomial.map (algebraMap F E) (minpoly F x)).Splits := fun x hx ↦ splits_of_splits
...
instance normal_iInf {ι : Type*} [hι : Nonempty ι]
  ...
  · have hx : ∀ i, Splits (Polynomial.map (algebraMap F (t i)) (minpoly F x)) := by
      intro i
