...
variable (splits : ∀ x : K, (Polynomial.map (algebraMap F L) (minpoly F x)).Splits)
...
lemma isNormalClosure_iff : IsNormalClosure F K L ↔
    (∀ x : K, (Polynomial.map (algebraMap F L) (minpoly F x)).Splits) ∧ normalClosure F K L = ⊤ := by
...
noncomputable def IsNormalClosure.lift [h : IsNormalClosure F K L] {L'} [Field L'] [Algebra F L']
    (splits : ∀ x : K, (Polynomial.map (algebraMap F L') (minpoly F x)).Splits) : L →ₐ[F] L' := by
...
noncomputable def Algebra.IsAlgebraic.algHomEmbeddingOfSplits [Algebra.IsAlgebraic F K]
    (h : ∀ x : K, (Polynomial.map (algebraMap F L) (minpoly F x)).Splits) (L' : Type*) [Field L'] [Algebra F L'] :
    (K →ₐ[F] L') ↪ (K →ₐ[F] L) :=
