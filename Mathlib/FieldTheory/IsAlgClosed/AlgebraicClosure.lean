...
def finEquivRoots {K} [Field K] [DecidableEq K] {i : k →+* K} {f : Monics k} (hf : (Polynomial.map i f.1).Splits) :
...
lemma Monics.splits_finsetProd {s : Finset (Monics k)} {f : Monics k} (hf : f ∈ s) :
    (Polynomial.map (algebraMap k (SplittingField (∏ f ∈ s, f.1))) f.1).Splits :=
