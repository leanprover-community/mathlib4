  ...
  ⟨Quot.lift
      (fun x : ℕ × K =>
        Quot.lift
          ...
          (private_decl% (mul_aux_right K p x)))
      fun x1 x2 (H : R K p x1 x2) =>
      funext fun e => Quot.inductionOn e fun y => private_decl% (mul_aux_left K p x1 x2 y H)⟩
  ...
  ⟨Quot.lift
      (fun x : ℕ × K =>
        Quot.lift
          (fun y : ℕ × K =>
            mk K p (x.1 + y.1, (frobenius K p)^[y.1] x.2 + (frobenius K p)^[x.1] y.2))
          (private_decl% (add_aux_right K p x)))
      fun x1 x2 (H : R K p x1 x2) =>
      funext fun e => Quot.inductionOn e fun y => private_decl% (add_aux_left K p x1 x2 y H)⟩
