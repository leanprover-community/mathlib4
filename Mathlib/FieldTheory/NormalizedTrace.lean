...
theorem normalizedTrace_trans_apply [Algebra.IsIntegral E K] [CharZero E] (a : K) :
  ...
  let E₀ := adjoin F S
  haveI : S.Finite := by simp_all only [Finset.finite_toSet, S]
  haveI : FiniteDimensional F E₀ := IntermediateField.finiteDimensional_adjoin
    fun x _ ↦ Algebra.IsIntegral.isIntegral x
  haveI : Algebra.IsIntegral E₀ K := Algebra.IsIntegral.trans E
  have hsub : S ⊆ (algebraMap E₀ E).range :=
    Subalgebra.range_algebraMap E₀.toSubalgebra ▸ subset_adjoin F S
