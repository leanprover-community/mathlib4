...
class IsSplittingField (f : K[X]) : Prop where
  splits' : Splits (Polynomial.map (algebraMap K L) f)
...
theorem splits (f : K[X]) [IsSplittingField K L f] : Splits (Polynomial.map (algebraMap K L) f) :=
  splits'
...
theorem splits_iff (f : K[X]) [IsSplittingField K L f] :
    Splits (Polynomial.map (RingHom.id K) f) ↔ (⊤ : Subalgebra K L) = ⊥ :=
  ⟨fun h => by
...
theorem IsScalarTower.splits (f : F[X]) [IsSplittingField K L (mapAlg F K f)] :
    Splits (Polynomial.map (RingHom.id L) (mapAlg F L f)) := by
...
def lift [Algebra K F] (f : K[X]) [IsSplittingField K L f]
    (hf : Splits (Polynomial.map (algebraMap K F) f)) : L →ₐ[K] F :=
  if hf0 : f = 0 then
    (Algebra.ofId K F).comp <|
      (Algebra.botEquiv K L : (⊥ : Subalgebra K L) →ₐ[K] K).comp <| by
        rw [← (splits_iff L f).1 (show (Polynomial.map (RingHom.id K) f).Splits from hf0.symm ▸ splits_zero _)]
        exact Algebra.toTop
  else AlgHom.comp (by
...
theorem IntermediateField.splits_of_splits (h : (Polynomial.map (algebraMap K L) p).Splits)
    (hF : ∀ x ∈ p.rootSet L, x ∈ F) : (Polynomial.map (algebraMap K F) p).Splits := by
  classical
...
theorem IntermediateField.splits_iff_mem (h : (Polynomial.map (algebraMap K L) p).Splits) :
    (Polynomial.map (algebraMap K F) p).Splits ↔ ∀ x ∈ p.rootSet L, x ∈ F := by
...
theorem IsIntegral.mem_intermediateField_of_minpoly_splits {x : L} (int : IsIntegral K x)
    {F : IntermediateField K L} (h : Splits (Polynomial.map (algebraMap K F) (minpoly K x))) : x ∈ F := by
...
theorem isSplittingField_iff_intermediateField : p.IsSplittingField K L ↔
    (Polynomial.map (algebraMap K L) p).Splits ∧ IntermediateField.adjoin K (p.rootSet L) = ⊤ := by
...
theorem IntermediateField.isSplittingField_iff :
    p.IsSplittingField K F ↔ (Polynomial.map (algebraMap K F) p).Splits ∧ F = adjoin K (p.rootSet L) := by
...
theorem IntermediateField.adjoin_rootSet_isSplittingField (hp : (Polynomial.map (algebraMap K L) p).Splits) :
