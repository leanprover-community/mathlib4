...
instance commRing : CommRing (SplittingField f) :=
  inferInstanceAs (CommRing (IntermediateField.adjoin K (f.rootSet (AlgebraicClosure K))))
...
instance algebra : Algebra K (SplittingField f) :=
  inferInstanceAs (Algebra K (IntermediateField.adjoin K (f.rootSet (AlgebraicClosure K))))

instance algebra' {R : Type*} [CommSemiring R] [Algebra R K] : Algebra R (SplittingField f) :=
  inferInstanceAs (Algebra R (IntermediateField.adjoin K (f.rootSet (AlgebraicClosure K))))

instance isScalarTower {R : Type*} [CommSemiring R] [Algebra R K] :
    IsScalarTower R K (SplittingField f) :=
  inferInstanceAs (IsScalarTower R K (IntermediateField.adjoin K (f.rootSet (AlgebraicClosure K))))

instance instGroupWithZero : GroupWithZero (SplittingField f) :=
  inferInstanceAs (GroupWithZero (IntermediateField.adjoin K (f.rootSet (AlgebraicClosure K))))

instance instField : Field (SplittingField f) :=
  inferInstanceAs (Field (IntermediateField.adjoin K (f.rootSet (AlgebraicClosure K))))
