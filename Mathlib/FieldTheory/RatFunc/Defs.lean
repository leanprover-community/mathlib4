...
-/
abbrev RatFunc [CommRing K] : Type u := FractionRing K[X]
...
/-! ### Induction principles for `RatFunc`s -/
...
theorem liftOn_nonZeroDivisors_mk {P : Sort v} (n : K[X]) (d : K[X]⁰) (f : K[X] → K[X] → P)
    (H : ∀ {p q p' q'} (_hq : q ∈ K[X]⁰) (_hq' : q' ∈ K[X]⁰), q' * p = q * p' → f p q = f p' q') :
    RatFunc.liftOn (Localization.mk n d) f @H = f n d := by
  rw [RatFunc.liftOn]
...
protected irreducible_def mk (p q : K[X]) : RatFunc K :=
  algebraMap _ _ p / algebraMap _ _ q

theorem mk_eq_div' (p q : K[X]) :
    RatFunc.mk p q = algebraMap _ _ p / algebraMap _ _ q := by rw [RatFunc.mk]

theorem mk_zero (p : K[X]) : RatFunc.mk p 0 = 0 := by
  rw [mk_eq_div', RingHom.map_zero, div_zero]

theorem mk_coe_def (p : K[X]) (q : K[X]⁰) : RatFunc.mk p q = IsLocalization.mk' _ p q := by
  simp only [mk_eq_div', ← Localization.mk_eq_mk', FractionRing.mk_eq_div]

theorem mk_def_of_mem (p : K[X]) {q} (hq : q ∈ K[X]⁰) :
    RatFunc.mk p q = IsLocalization.mk' (FractionRing K[X]) p ⟨q, hq⟩ := by
  simp only [← mk_coe_def]

theorem mk_def_of_ne (p : K[X]) {q : K[X]} (hq : q ≠ 0) :
    RatFunc.mk p q =
      IsLocalization.mk' (FractionRing K[X]) p ⟨q, mem_nonZeroDivisors_iff_ne_zero.mpr hq⟩ :=
  mk_def_of_mem p _

theorem mk_eq_localization_mk (p : K[X]) {q : K[X]} (hq : q ≠ 0) :
    RatFunc.mk p q = Localization.mk p ⟨q, mem_nonZeroDivisors_iff_ne_zero.mpr hq⟩ := by
  rw [mk_def_of_ne _ hq, Localization.mk_eq_mk']

theorem mk_one' (p : K[X]) :
    RatFunc.mk p 1 = algebraMap _ _ p := by
...
theorem mk_eq_mk {p q p' q' : K[X]} (hq : q ≠ 0) (hq' : q' ≠ 0) :
  ...
  rw [mk_def_of_ne _ hq, mk_def_of_ne _ hq', IsLocalization.mk'_eq_iff_eq',
...
theorem liftOn_mk {P : Sort v} (p q : K[X]) (f : K[X] → K[X] → P) (f0 : ∀ p, f p 0 = f 0 1)
  ...
  by_cases hq : q = 0
  · subst hq
    simp only [mk_zero, f0, ← Localization.mk_zero 1, liftOn_nonZeroDivisors_mk, Submonoid.coe_one]
  · simp only [mk_eq_localization_mk _ hq, liftOn_nonZeroDivisors_mk]
