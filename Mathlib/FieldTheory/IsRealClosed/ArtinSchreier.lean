...
lemma exists_root_or_all_irreducible_divisors {p : ℕ} (f : F[X]) (hr : finrank F K ∣ p)
    (hp : p.Prime) :
...
lemma exists_root_or_all_irreducible_divisors' {p : ℕ} (f : F[X]) (hr : finrank F K ∣ p)
  ...
  (exists_root_or_all_irreducible_divisors f hr hp).imp_right (fun h ↦
    dvd_natDegree_of_monic_of_irreducible f (fun d _ h1 h2 ↦ (h d h1 h2).symm.dvd))

/- A field admitting a finite extension which is algebraically closed is perfect. -/
lemma perfect_of_finite_dimensional_algebraic_closure [hf : FiniteDimensional F K] :
    PerfectField F := by
...
lemma not_charP_of_finrank {p : ℕ} (hp : p.Prime) (hr : finrank F K = p) : ¬CharP F p := by
  ...
  have := perfect_of_finite_dimensional_algebraic_closure (hf := hf)
  have : IsAlgClosure F K := ⟨Hac, inferInstance⟩
  have h : IsGalois F K := inferInstance
  by_contra
  obtain ⟨a, x, ha⟩ := ((isCyclic_charP_tfae hp rfl).out 1 5).mp h
  have h := (degree_eq_iff_natDegree_eq_of_pos one_pos).mp
    (Hac.degree_eq_one_of_irreducible K
    (irreducible_artinSchreierPoly_tower hp rfl a x ha))
...
lemma isSquare_mul_add_or_isSquare_neg (h : finrank F K ∣ 2) (a b : F) :
  ...
  rcases exists_root_or_all_irreducible_divisors g h prime_two with ⟨x, hx⟩ | h
  · right
    use x^2 - a
...
lemma isAlgClosed_of_isSquare_neg_one [hf : FiniteDimensional F K] (hm : IsSquare (-1 : F)) :
  ...
  have h : finrank F K = 1 := by
    ...
    have h := ((isCyclic_tfae (fixedField (zpowers g)) K ?_).out 1 2).mp
    · rw [hr] at h
      ...
      · subst h1
        ...
        refine (isSquare_mul_add_or_isSquare_neg hr.dvd 0 a).elim (by simp) (fun h ↦ ?_)
        have := (hm.map (algebraMap F _)).mul h
        aesop
      · have h := ((X_pow_sub_C_irreducible_iff_of_prime_pow_of_ne_two hp h1 (n := 2)
            (by simp)).mpr h3).natDegree_dvd_finrank (Hac.splits _)
        simp [hr, not_pos_pow_dvd hp.one_lt] at h
    rw [hr]
    rcases exists_root_or_all_irreducible_divisors' _ hr.dvd hp with ⟨z, hz⟩ | hz
    · have := neZero_iff.mpr ((CharP.charP_iff_prime_eq_zero hp).mpr.mt
        (not_charP_of_finrank hp hr))
...
lemma isRealClosed_of_isAlgClosed_adjoin (h1 : ¬IsSquare (-1 : F)) (h2 : ∃ i : K, i ^ 2 = -1 ∧
  ...
  have hssq : ∀ x : F, IsSumSq x → IsSquare x := by
    ...
    · rintro a b ⟨y, rfl⟩ - h
      ...
      · contrapose h1
        rw [←neg_div_self h0]
        exact ((isSquare_mul_add_or_isSquare_neg hr y b).resolve_left h1).div h
  have := isSemireal_iff_not_isSumSq_neg_one.mpr ((hssq _).mt h1)
  constructor
  · have := isSquare_mul_add_or_isSquare_neg hr 0
    simp_all
  · intro f _
    exact (exists_root_or_all_irreducible_divisors' f hr prime_two).resolve_right (by grind)
...
public theorem isAlgClosed_or_isRealClosed (F : Type u) (K : Type u) [Field F] [Field K]
  ...
  · left
    exact isAlgClosed_of_isSquare_neg_one hF (K := K)
  · right
    have ⟨i, hi⟩ := ‹IsAlgClosed K›.exists_pow_nat_eq (-1) two_pos
    refine isRealClosed_of_isAlgClosed_adjoin hF ⟨i, ⟨hi,
        isAlgClosed_of_isSquare_neg_one ?_ (K := K)⟩⟩
