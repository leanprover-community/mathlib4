...
/-!
# Irreducibility of Kummer Polynomials
...
open AdjoinRoot IntermediateField Polynomial

section Splits

lemma root_X_pow_sub_C_pow (n : ℕ) (a : K) :
    (AdjoinRoot.root (X ^ n - C a)) ^ n = AdjoinRoot.of (X ^ n - C a) a := by
...
theorem X_pow_sub_C_irreducible_of_odd
  ...
  | h p n hp IH =>
    ...
    rw [← map_pow, hb, ← adjoin.powerBasis_gen this,
      Algebra.norm_eq_matrix_det (adjoin.powerBasis this).basis,
      Matrix.det_eq_sign_charpoly_coeff, charpoly_leftMulMatrix]
