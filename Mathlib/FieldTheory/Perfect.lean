...
-/
lemma mkOfReduced [IsReduced R] (h : Surjective $ frobenius R p) : PerfectRing R p :=
  ⟨frobenius_inj R p, h⟩

instance ofFiniteOfIsReduced [Finite R] [IsReduced R] : PerfectRing R p :=
...
lemma PerfectRing.toPerfectField (K : Type _) (p : ℕ)
  ...
  refine' PerfectField.mk $ fun {f} hf ↦ _
...
instance PerfectField.toPerfectRing (p : ℕ) [Fact p.Prime] [CharP K p] : PerfectRing K p := by
