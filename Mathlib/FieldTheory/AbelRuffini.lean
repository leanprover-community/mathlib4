...
theorem gal_isSolvable_of_splits {p q : F[X]}
    (_ : Fact ((Polynomial.map (algebraMap F q.SplittingField) p).Splits)) (hq : IsSolvable q.Gal) :
    IsSolvable p.Gal :=
...
theorem gal_isSolvable_tower (p q : F[X]) (hpq : (Polynomial.map (algebraMap F q.SplittingField) p).Splits)
    (hp : IsSolvable p.Gal) (hq : IsSolvable (q.map (algebraMap F p.SplittingField)).Gal) :
    IsSolvable q.Gal := by
  let K := p.SplittingField
  let L := q.SplittingField
  haveI : Fact ((Polynomial.map (algebraMap F L) p).Splits) := ⟨hpq⟩
...
theorem gal_X_pow_sub_C_isSolvable_aux (n : ℕ) (a : F)
    (h : (Polynomial.map (RingHom.id F) (X ^ n - 1 : F[X])).Splits) : IsSolvable (X ^ n - C a).Gal := by
  by_cases ha : a = 0
  · rw [ha, C_0, sub_zero]
...
theorem splits_X_pow_sub_one_of_X_pow_sub_C {F : Type*} [Field F] {E : Type*} [Field E]
    (i : F →+* E) (n : ℕ) {a : F} (ha : a ≠ 0) (h : (Polynomial.map i (X ^ n - C a)).Splits) :
    (Polynomial.map i (X ^ n - 1 : F[X])).Splits := by
