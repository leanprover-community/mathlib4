...
theorem exists_lift_of_splits' (x : Lifts F E K) {s : E} (h1 : IsIntegral x.carrier s)
    (h2 : (Polynomial.map x.emb.toRingHom (minpoly x.carrier s)).Splits) : ∃ y, x ≤ y ∧ s ∈ y.carrier :=
...
theorem exists_lift_of_splits (x : Lifts F E K) {s : E} (h1 : IsIntegral F s)
    (h2 : (Polynomial.map (algebraMap F K) (minpoly F s)).Splits) : ∃ y, x ≤ y ∧ s ∈ y.carrier :=
...
private theorem exists_algHom_adjoin_of_splits'' {L : IntermediateField F E}
    (f : L →ₐ[F] K) (hK : ∀ s ∈ S, IsIntegral L s ∧ (Polynomial.map f.toRingHom (minpoly L s)).Splits) :
...
theorem exists_algHom_of_splits' (hK : ∀ s : E, IsIntegral L s ∧ (Polynomial.map f.toRingHom (minpoly L s)).Splits) :
    ∃ φ : E →ₐ[F] K, φ.restrictDomain L = f :=
  exists_algHom_of_adjoin_splits' f (fun x _ ↦ hK x) (adjoin_univ L E)

end

variable (hK : ∀ s ∈ S, IsIntegral F s ∧ (Polynomial.map (algebraMap F K) (minpoly F s)).Splits)
  (hK' : ∀ s : E, IsIntegral F s ∧ (Polynomial.map (algebraMap F K) (minpoly F s)).Splits)
...
theorem Algebra.IsAlgebraic.range_eval_eq_rootSet_minpoly_of_splits {F K : Type*} (L : Type*)
    ...
    (hA : ∀ x : K, (Polynomial.map (algebraMap F L) (minpoly F x)).Splits)
