...
variable {ι F E : Type*} {x : ι → E} [Field F] [Field E] [Algebra F E]
...
axiom aevalEquivField (hx : AlgebraicIndependent F x) :
    FractionRing (MvPolynomial ι F) ≃ₐ[F] ↥(IntermediateField.adjoin F (Set.range x))

/-- TODO: remove this when PR #18648 is merged -/
@[simp]
axiom aevalEquivField_apply_coe (hx : AlgebraicIndependent F x)
    (a : FractionRing (MvPolynomial ι F)) :
    hx.aevalEquivField a =
      IsFractionRing.lift (algebraicIndependent_iff_injective_aeval.2 hx) a

theorem aevalEquivField_algebraMap_apply_coe (hx : AlgebraicIndependent F x)
    (a : MvPolynomial ι F) :
    hx.aevalEquivField (algebraMap _ _ a) = MvPolynomial.aeval x a := by
  simp

end AlgebraicIndependent

/-- TODO: remove this when PR #18648 is merged -/
axiom IsTranscendenceBasis.isAlgebraic_field {ι F E : Type*} {x : ι → E}
