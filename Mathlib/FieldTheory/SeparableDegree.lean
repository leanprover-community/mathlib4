...
def embEquivOfAdjoinSplits {S : Set E} (hS : adjoin F S = ⊤)
    (hK : ∀ s ∈ S, IsIntegral F s ∧ Splits (Polynomial.map (algebraMap F K) (minpoly F s))) :
    Emb F E ≃ (E →ₐ[F] K) :=
...
theorem natSepDegree_eq_of_splits [DecidableEq E] (h : (Polynomial.map (algebraMap F E) f).Splits) :
...
theorem eq_X_sub_C_pow_of_natSepDegree_eq_one_of_splits (hm : f.Monic)
    (hs : (Polynomial.map (RingHom.id F) f).Splits)
...
theorem PerfectField.splits_of_natSepDegree_eq_one [PerfectField K] {f : E[X]}
    (i : E →+* K) (hf : f.natSepDegree = 1) : (Polynomial.map i f).Splits :=
