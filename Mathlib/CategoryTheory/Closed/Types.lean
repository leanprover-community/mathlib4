...
instance (priority := 10000) (X : Type v₁) : IsLeftAdjoint (Types.binaryProductFunctor.obj X) where
  right :=
    { obj := fun Y => X ⟶ Y
      map := fun f g => g ≫ f }
  adj :=
...
instance (priority := 10000) : HasFiniteProducts (Type v₁) :=
  hasFiniteProducts_of_hasProducts.{v₁} _

instance (priority := 10000) : CartesianClosed (Type v₁) :=
...
instance (priority := 10000) {C : Type u₁} [Category.{v₁} C] : HasFiniteProducts (C ⥤ Type u₂) :=
  hasFiniteProducts_of_hasProducts _

instance (priority := 10000) {C : Type v₁} [SmallCategory C] : CartesianClosed (C ⥤ Type v₁) :=
  CartesianClosed.mk _
    (fun F =>
