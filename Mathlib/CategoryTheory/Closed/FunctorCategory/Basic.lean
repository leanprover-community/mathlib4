...
noncomputable def homEquiv : (F₁ ⊗ F₂ ⟶ F₃) ≃ (F₂ ⟶ functorEnrichedHom C F₁ F₃) where
  ...
  invFun g :=
    { app := fun j ↦ uncurry (g.app j ≫ enrichedHomπ C _ _ (Under.mk (𝟙 j)) )
      naturality := fun j j' φ ↦ by
        ...
        simpa using (enrichedHom_condition C (Under.forget j ⋙ F₁)
          (Under.forget j ⋙ F₃) α).symm }
  left_inv f := by
    dsimp
    ext j
    dsimp
  ...
  right_inv g := by
    ...
    rw [end_.lift_π]
    dsimp
    simp only [enrichedOrdinaryCategorySelf_eHomWhiskerRight, pre_id, NatTrans.id_app,
...
lemma homEquiv_naturality_three [∀ (F₁ F₂ : J ⥤ C), HasEnrichedHom C F₁ F₂]
  ...
  rw [Category.assoc, end_.lift_π]
  dsimp
  simp only [enrichedOrdinaryCategorySelf_eHomWhiskerRight, pre_id, NatTrans.id_app,
    enrichedOrdinaryCategorySelf_eHomWhiskerLeft, Functor.map_id, Category.comp_id,
    Category.comp_id, homEquiv_apply_π, enrichedCategorySelf_hom, curry_natural_right]
  congr 2
  symm
