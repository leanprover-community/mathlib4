...
def pre (f : B ⟶ A) : ihom A ⟶ ihom B :=
  Conjugates (ihom.adjunction _) (ihom.adjunction _) ((tensoringLeft C).map f)
...
theorem id_tensor_pre_app_comp_ev (f : B ⟶ A) (X : C) :
  ...
  Conjugates_counit _ _ ((tensoringLeft C).map f) X
...
theorem pre_map {A₁ A₂ A₃ : C} [Closed A₁] [Closed A₂] [Closed A₃] (f : A₁ ⟶ A₂) (g : A₂ ⟶ A₃) :
  ...
  rw [pre, pre, pre, Conjugates_comp, (tensoringLeft C).map_comp]
