...
the Yoneda embedding as a fully faithful functor `yoneda : C ⥤ Cᵒᵖ ⥤ TypeCat.{v₁}`,
In addition to `yoneda`, we also define `uliftYoneda : C ⥤ Cᵒᵖ ⥤ TypeCat.{max w v₁}`
with the additional universe parameter `w`. When `C` is locally `w`-small,
one may also use `shrinkYoneda.{w} : C ⥤ Cᵒᵖ ⥤ TypeCat.{w}` from the file
...
@[pp_with_univ, simps!]
def uliftYoneda : C ⥤ Cᵒᵖ ⥤ TypeCat.{max w v₁} :=
...
-/
abbrev coyoneda : Cᵒᵖ ⥤ C ⥤ TypeCat.{v₁} := yoneda.flip
...
theorem naturality {X Y : C} (α : yoneda.obj X ⟶ yoneda.obj Y) {Z Z' : C} (f : Z ⟶ Z')
  ...
  (NatTrans.naturality_apply α f.op h).symm
...
def fullyFaithful : (yoneda (C := C)).FullyFaithful where
  ...
  map_preimage := by -- this was automatic
    intro Z W f
    ext X x
    simp only [yoneda_obj_obj_carrier, yoneda_map_app, ConcreteCategory.hom_ofHom, hom_as_apply]
    rw [naturality] -- simp still doesn't find this
    simp
...
@[stacks 001P]
instance yoneda_full : (yoneda : C ⥤ Cᵒᵖ ⥤ TypeCat.{v₁}).Full :=
  fullyFaithful.full

/-- The Yoneda embedding is faithful. -/
@[stacks 001P]
instance yoneda_faithful : (yoneda : C ⥤ Cᵒᵖ ⥤ TypeCat.{v₁}).Faithful :=
...
the functor `uliftYoneda.{w} : C ⥤ Cᵒᵖ ⥤ TypeCat.{max w v₁}` is fully faithful. -/
...
def fullyFaithful : (coyoneda (C := C)).FullyFaithful where
  ...
  map_preimage := by
    intro Z W f
    ext X x
    simp only [flip_obj_obj, yoneda_obj_obj_carrier, flip_map_app, yoneda_obj_map,
      Quiver.Hom.unop_op, ConcreteCategory.hom_ofHom, hom_as_apply]
    rw [naturality] -- simp still doesn't find this
    simp
...
instance coyoneda_full : (coyoneda : Cᵒᵖ ⥤ C ⥤ TypeCat.{v₁}).Full :=
  fullyFaithful.full

instance coyoneda_faithful : (coyoneda : Cᵒᵖ ⥤ C ⥤ TypeCat.{v₁}).Faithful :=
  fullyFaithful.faithful
...
def punitIso : coyoneda.obj (Opposite.op (of PUnit)) ≅ 𝟭 (TypeCat.{v₁}) :=
  NatIso.ofComponents fun X =>
    { hom := ofHom ⟨fun f => f ⟨⟩⟩
      inv := ofHom ⟨fun x => ofHom ⟨fun _ => x⟩⟩ }
...
/-- When `C` is a category such that `Category.{v₁} C`, then
the functor `uliftCoyoneda.{w} : C ⥤ Cᵒᵖ ⥤ TypeCat.{max w v₁}` is fully faithful. -/
...
/-- The data which expresses that a functor `F : Cᵒᵖ ⥤ TypeCat.{v}` is representable by `Y : C`. -/
structure RepresentableBy (F : Cᵒᵖ ⥤ TypeCat.{v}) (Y : C) where
...
lemma RepresentableBy.comp_homEquiv_symm {F : Cᵒᵖ ⥤ TypeCat.{v}} {Y : C}
...
lemma RepresentableBy.homEquiv_unop_comp {F : Cᵒᵖ ⥤ TypeCat} {Y : C}
...
def RepresentableBy.ofIso {F F' : Cᵒᵖ ⥤ TypeCat.{v}} {Y : C} (e : F.RepresentableBy Y)
...
/-- The data which expresses that a functor `F : C ⥤ TypeCat.{v}` is corepresentable by `X : C`. -/
structure CorepresentableBy (F : C ⥤ TypeCat.{v}) (X : C) where
...
lemma CorepresentableBy.homEquiv_symm_comp {F : C ⥤ TypeCat.{v}} {X : C}
...
def CorepresentableBy.ofIso {F F' : C ⥤ TypeCat.{v}} {X : C} (e : F.CorepresentableBy X)
    (e' : F ≅ F') :
...
lemma RepresentableBy.homEquiv_eq {F : Cᵒᵖ ⥤ TypeCat.{v}} {Y : C} (e : F.RepresentableBy Y)
    {X : C} (f : X ⟶ Y) :
    e.homEquiv f = F.map f.op (e.homEquiv (𝟙 Y)) := by
  conv_lhs => rw [← Category.comp_id f, e.homEquiv_comp]

lemma CorepresentableBy.homEquiv_eq {F : C ⥤ TypeCat.{v}} {X : C} (e : F.CorepresentableBy X)
    {Y : C} (f : X ⟶ Y) :
...
@[simps!]
def RepresentableBy.uniqueUpToIso {F : Cᵒᵖ ⥤ TypeCat.{v}} {Y Y' : C} (e : F.RepresentableBy Y)
...
@[simps!]
def CorepresentableBy.uniqueUpToIso {F : C ⥤ TypeCat.{v}} {X X' : C} (e : F.CorepresentableBy X)
...
@[ext]
lemma RepresentableBy.ext {F : Cᵒᵖ ⥤ TypeCat.{v}} {Y : C} {e e' : F.RepresentableBy Y}
...
@[ext]
lemma CorepresentableBy.ext {F : C ⥤ TypeCat.{v}} {X : C} {e e' : F.CorepresentableBy X}
...
when `F : Cᵒᵖ ⥤ TypeCat.{v₁}` and `[Category.{v₁} C]`. -/
def representableByEquiv {F : Cᵒᵖ ⥤ TypeCat.{v₁}} {Y : C} :
...
def RepresentableBy.toIso {F : Cᵒᵖ ⥤ TypeCat.{v₁}} {Y : C} (e : F.RepresentableBy Y) :
    yoneda.obj Y ≅ F :=
  representableByEquiv e

/-- The obvious bijection `F.CorepresentableBy X ≃ (yoneda.obj Y ≅ F)`
when `F : C ⥤ TypeCat.{v₁}` and `[Category.{v₁} C]`. -/
def corepresentableByEquiv {F : C ⥤ TypeCat.{v₁}} {X : C} :
...
def CorepresentableBy.toIso {F : C ⥤ TypeCat.{v₁}} {X : C} (e : F.CorepresentableBy X) :
    coyoneda.obj (op X) ≅ F :=
  corepresentableByEquiv e

/-- Transport `RepresentableBy` along an isomorphism of the object. -/
@[simps]
def RepresentableBy.ofIsoObj {F : Cᵒᵖ ⥤ TypeCat.{w}} {X Y : C} (R : F.RepresentableBy X)
    (e : Y ≅ X) :
...
/-- Transport `RepresentableBy` along an isomorphism of the object. -/
@[simps]
def CorepresentableBy.ofIsoObj {F : C ⥤ TypeCat.{w}} {X Y : C} (R : F.CorepresentableBy X)
    (e : Y ≅ X) :
...
@[simps]
def RepresentableBy.equivOfIsoObj {F : Cᵒᵖ ⥤ TypeCat.{w}} {X Y : C} (e : Y ≅ X) :
...
@[simps]
def CorepresentableBy.equivOfIsoObj {F : C ⥤ TypeCat.{w}} {X Y : C} (e : Y ≅ X) :
...
@[simps]
def RepresentableBy.equivUliftYonedaIso (F : Cᵒᵖ ⥤ TypeCat.{max w v₁}) (X : C) :
...
@[simps]
def CorepresentableBy.equivUliftCoyonedaIso (F : C ⥤ TypeCat.{max w v₁}) (X : C) :
...
/-- A functor `F : Cᵒᵖ ⥤ TypeCat.{v}` is representable if there is an object `Y` with a structure
...
@[stacks 001Q]
class IsRepresentable (F : Cᵒᵖ ⥤ TypeCat.{v}) : Prop where
  has_representation : ∃ (Y : C), Nonempty (F.RepresentableBy Y)

lemma RepresentableBy.isRepresentable {F : Cᵒᵖ ⥤ TypeCat.{v}} {Y : C} (e : F.RepresentableBy Y) :
...
lemma IsRepresentable.mk' {F : Cᵒᵖ ⥤ TypeCat.{v₁}} {X : C} (e : yoneda.obj X ≅ F) :
    F.IsRepresentable :=
...
lemma CorepresentableBy.isCorepresentable {F : C ⥤ TypeCat.{v}} {X : C}
...
lemma IsCorepresentable.mk' {F : C ⥤ TypeCat.{v₁}} {X : C} (e : coyoneda.obj (op X) ≅ F) :
    F.IsCorepresentable :=
...
variable (F : Cᵒᵖ ⥤ TypeCat.{v}) [hF : F.IsRepresentable]
...
-/
noncomputable def reprW (F : Cᵒᵖ ⥤ TypeCat.{v₁}) [F.IsRepresentable] :
    yoneda.obj F.reprX ≅ F := F.representableBy.toIso

theorem reprW_hom_app (F : Cᵒᵖ ⥤ TypeCat.{v₁}) [F.IsRepresentable]
...
noncomputable def uliftYonedaReprXIso (F : Cᵒᵖ ⥤ TypeCat.{max v v₁}) [F.IsRepresentable] :
    uliftYoneda.{v}.obj F.reprX ≅ F :=
  (RepresentableBy.equivUliftYonedaIso F _) F.representableBy

lemma uliftYonedaReprXIso_hom_app (F : Cᵒᵖ ⥤ TypeCat.{max v v₁}) [F.IsRepresentable]
...
variable (F : C ⥤ TypeCat.{v}) [hF : F.IsCorepresentable]
...
-/
noncomputable def coreprW (F : C ⥤ TypeCat.{v₁}) [F.IsCorepresentable] :
    coyoneda.obj (op F.coreprX) ≅ F :=
  F.corepresentableBy.toIso

theorem coreprW_hom_app (F : C ⥤ TypeCat.{v₁}) [F.IsCorepresentable] (X : C) (f : F.coreprX ⟶ X) :
...
noncomputable def uliftCoyonedaCoreprXIso (F : C ⥤ TypeCat.{max v v₁}) [F.IsCorepresentable] :
    uliftCoyoneda.{v}.obj (op F.coreprX) ≅ F :=
  (CorepresentableBy.equivUliftCoyonedaIso F _) F.corepresentableBy

lemma uliftCoyonedaCoreprXIso_hom_app (F : C ⥤ TypeCat.{max v v₁}) [F.IsCorepresentable]
...
lemma isRepresentable_comp_uliftFunctor_iff {F : Cᵒᵖ ⥤ TypeCat.{v}} :
...
lemma isCorepresentable_comp_uliftFunctor_iff {F : C ⥤ TypeCat.{v}} :
...
instance (F : Cᵒᵖ ⥤ TypeCat.{v}) [F.IsRepresentable] : (F ⋙ uliftFunctor.{w}).IsRepresentable :=
  isRepresentable_comp_uliftFunctor_iff.mpr ‹_›

instance (F : C ⥤ TypeCat.{v}) [F.IsCorepresentable] : (F ⋙ uliftFunctor.{w}).IsCorepresentable :=
  isCorepresentable_comp_uliftFunctor_iff.mpr ‹_›

end Functor

theorem isRepresentable_of_natIso (F : Cᵒᵖ ⥤ TypeCat.{v₁}) {G} (i : F ≅ G) [F.IsRepresentable] :
    G.IsRepresentable :=
  (F.representableBy.ofIso i).isRepresentable

theorem corepresentable_of_natIso (F : C ⥤ TypeCat.{v₁}) {G} (i : F ≅ G) [F.IsCorepresentable] :
    G.IsCorepresentable :=
...
instance prodCategoryInstance1 : Category ((Cᵒᵖ ⥤ TypeCat.{v₁}) × Cᵒᵖ) :=
  CategoryTheory.prod'.{max u₁ v₁, v₁} (Cᵒᵖ ⥤ TypeCat.{v₁}) Cᵒᵖ

instance prodCategoryInstance2 : Category (Cᵒᵖ × (Cᵒᵖ ⥤ TypeCat.{v₁})) :=
  CategoryTheory.prod'.{v₁, max u₁ v₁} Cᵒᵖ (Cᵒᵖ ⥤ TypeCat.{v₁})
...
-/
def yonedaEquiv {X : C} {F : Cᵒᵖ ⥤ TypeCat.{v₁}} : (yoneda.obj X ⟶ F) ≃ F.obj (op X) where
  toFun η := η.app (op X) (𝟙 X)
  invFun ξ := { app _ := ofHom ⟨fun f ↦ F.map f.op ξ⟩ }
  left_inv := by
    intro η
    ext Y f
...
theorem yonedaEquiv_apply {X : C} {F : Cᵒᵖ ⥤ TypeCat.{v₁}} (f : yoneda.obj X ⟶ F) :
    yonedaEquiv f = f.app (op X) (𝟙 X) :=
  rfl

@[simp]
theorem yonedaEquiv_symm_app_apply {X : C} {F : Cᵒᵖ ⥤ TypeCat.{v₁}} (x : F.obj (op X)) (Y : Cᵒᵖ)
    (f : Y.unop ⟶ X) : (yonedaEquiv.symm x).app Y f = F.map f.op x :=
  rfl

/-- See also `yonedaEquiv_naturality'` for a more general version. -/
lemma yonedaEquiv_naturality {X Y : C} {F : Cᵒᵖ ⥤ TypeCat.{v₁}} (f : yoneda.obj X ⟶ F)
...
lemma yonedaEquiv_naturality' {X Y : Cᵒᵖ} {F : Cᵒᵖ ⥤ TypeCat.{v₁}} (f : yoneda.obj (unop X) ⟶ F)
    (g : X ⟶ Y) : F.map g (yonedaEquiv f) = yonedaEquiv (yoneda.map g.unop ≫ f) :=
  yonedaEquiv_naturality _ _

lemma yonedaEquiv_comp {X : C} {F G : Cᵒᵖ ⥤ TypeCat.{v₁}} (α : yoneda.obj X ⟶ F) (β : F ⟶ G) :
...
lemma yonedaEquiv_symm_naturality_left {X X' : C} (f : X' ⟶ X) (F : Cᵒᵖ ⥤ TypeCat.{v₁})
    (x : F.obj ⟨X⟩) : yoneda.map f ≫ yonedaEquiv.symm x = yonedaEquiv.symm ((F.map f.op) x) := by
  apply yonedaEquiv.injective
  simp only [yonedaEquiv_comp, yoneda_obj_obj_carrier, Equiv.apply_symm_apply]
...
lemma map_yonedaEquiv {X Y : C} {F : Cᵒᵖ ⥤ TypeCat.{v₁}} (f : yoneda.obj X ⟶ F)
...
/-- Variant of `map_yonedaEquiv` with general `g`. This is technically strictly more general
    ...
    can avoid the "motive is not type correct" error. -/
lemma map_yonedaEquiv' {X Y : Cᵒᵖ} {F : Cᵒᵖ ⥤ TypeCat.{v₁}} (f : yoneda.obj (unop X) ⟶ F)
    (g : X ⟶ Y) : F.map g (yonedaEquiv f) = f.app Y g.unop := by
  rw [yonedaEquiv_naturality', yonedaEquiv_comp, yonedaEquiv_yoneda_map]

lemma yonedaEquiv_symm_map {X Y : Cᵒᵖ} (f : X ⟶ Y) {F : Cᵒᵖ ⥤ TypeCat.{v₁}} (t : F.obj X) :
...
lemma hom_ext_yoneda {P Q : Cᵒᵖ ⥤ TypeCat.{v₁}} {f g : P ⟶ Q}
...
-/
def yonedaEvaluation : Cᵒᵖ × (Cᵒᵖ ⥤ TypeCat.{v₁}) ⥤ TypeCat.{max u₁ v₁} :=
  evaluationUncurried Cᵒᵖ (TypeCat.{v₁}) ⋙ uliftFunctor

@[simp]
theorem yonedaEvaluation_map_down (P Q : Cᵒᵖ × (Cᵒᵖ ⥤ TypeCat.{v₁})) (α : P ⟶ Q)
...
-/
def yonedaPairing : Cᵒᵖ × (Cᵒᵖ ⥤ TypeCat.{v₁}) ⥤ TypeCat.{max u₁ v₁} :=
  Functor.prod yoneda.op (𝟭 (Cᵒᵖ ⥤ TypeCat.{v₁})) ⋙ Functor.hom (Cᵒᵖ ⥤ TypeCat.{v₁})

@[ext]
lemma yonedaPairingExt {X : Cᵒᵖ × (Cᵒᵖ ⥤ TypeCat.{v₁})} {x y : (yonedaPairing C).obj X}
    (w : ∀ Y, x.app Y = y.app Y) : x = y :=
  NatTrans.ext (funext w)

@[simp]
theorem yonedaPairing_map (P Q : Cᵒᵖ × (Cᵒᵖ ⥤ TypeCat.{v₁})) (α : P ⟶ Q)
...
def yonedaLemma : yonedaPairing C ≅ yonedaEvaluation C :=
  NatIso.ofComponents
    ...
    (by intro (X, F) (Y, G) f
        ...
        erw [types_comp_apply, types_comp_apply] -- Regression: `erw`
        simp [← NatTrans.naturality_apply])
...
def yonedaOpCompYonedaObj {C : Type u₁} [Category.{v₁} C] (P : Cᵒᵖ ⥤ TypeCat.{v₁}) :
...
for any presheaf of type `F : Cᵒᵖ ⥤ TypeCat.{max w v₁}` for some
auxiliary universe `w`. -/
@[simps! -isSimp apply symm_apply_app]
def uliftYonedaEquiv {X : C} {F : Cᵒᵖ ⥤ TypeCat.{max w v₁}} :
    (uliftYoneda.{w}.obj X ⟶ F) ≃ F.obj (op X) where
  toFun τ := τ.app (op X) (ULift.up (𝟙 _))
  invFun x := {
      app Y := ofHom ⟨fun y ↦ F.map y.down.op x⟩
      naturality := by -- Regression: was automatic
        intros
        ext
        simp only [uliftYoneda_obj_obj_carrier, yoneda_obj_obj_carrier, op_unop, hom_as_apply]
        erw [types_comp_apply, types_comp_apply]
        simp [uliftYoneda] }
  left_inv τ := by
    ext ⟨Y⟩ ⟨y⟩
    simp only [uliftYoneda, comp_obj, whiskeringRight_obj_obj, uliftFunctor_obj_carrier,
      yoneda_obj_obj_carrier, ConcreteCategory.hom_ofHom, hom_as_apply]
    erw [← NatTrans.naturality_apply] -- Regression: erw
    simp
  right_inv x := by simp
...
lemma uliftYonedaEquiv_naturality {X Y : Cᵒᵖ} {F : Cᵒᵖ ⥤ TypeCat.{max w v₁}}
  ...
  erw [← f.naturality_apply g (ULift.up (𝟙 _))] -- Regression: `erw`
  conv_rhs => erw [types_comp_apply] -- Regression: `erw`
  simp [uliftYoneda]

lemma uliftYonedaEquiv_comp {X : C} {F G : Cᵒᵖ ⥤ TypeCat.{max w v₁}}
    (α : uliftYoneda.{w}.obj X ⟶ F) (β : F ⟶ G) :
    uliftYonedaEquiv.{w} (α ≫ β) = β.app _ (uliftYonedaEquiv α) :=
  rfl

@[reassoc]
lemma uliftYonedaEquiv_symm_map {X Y : Cᵒᵖ} (f : X ⟶ Y) {F : Cᵒᵖ ⥤ TypeCat.{max w v₁}}
    (t : F.obj X) :
...
lemma hom_ext_uliftYoneda {P Q : Cᵒᵖ ⥤ TypeCat.{max w v₁}} {f g : P ⟶ Q}
    (h : ∀ (X : C) (p : uliftYoneda.{w}.obj X ⟶ P), p ≫ f = p ≫ g) :
    f = g := by -- was `ext X x; simpa [uliftYonedaEquiv] using congr_arg uliftYonedaEquiv.{w} (h _ (uliftYonedaEquiv.symm x))`
  ext X x
  have := congr_arg uliftYonedaEquiv.{w} (h _ (uliftYonedaEquiv.symm x))
  simp only [op_unop, uliftYonedaEquiv, uliftYoneda_obj_obj_carrier, yoneda_obj_obj_carrier,
    Equiv.coe_fn_symm_mk, Equiv.coe_fn_mk, NatTrans.comp_app] at this
  erw [types_comp_apply, types_comp_apply] at this
  simp only [uliftYoneda_obj_obj_carrier, ConcreteCategory.hom_ofHom, Fun.as_apply, op_id, op_unop,
    map_id, id_apply] at this
  simp only [hom_as_apply, this]
  erw [types_comp_apply]
  simp
...
-/
def coyonedaEquiv {X : C} {F : C ⥤ TypeCat.{v₁}} : (coyoneda.obj (op X) ⟶ F) ≃ F.obj X where
  toFun η := η.app X (𝟙 X)
...
theorem coyonedaEquiv_apply {X : C} {F : C ⥤ TypeCat.{v₁}} (f : coyoneda.obj (op X) ⟶ F) :
    coyonedaEquiv f = f.app X (𝟙 X) :=
  rfl

@[simp]
theorem coyonedaEquiv_symm_app_apply {X : C} {F : C ⥤ TypeCat.{v₁}} (x : F.obj X) (Y : C)
    (f : X ⟶ Y) : (coyonedaEquiv.symm x).app Y f = F.map f x :=
  rfl

lemma coyonedaEquiv_naturality {X Y : C} {F : C ⥤ TypeCat.{v₁}} (f : coyoneda.obj (op X) ⟶ F)
    (g : X ⟶ Y) : F.map g (coyonedaEquiv f) = coyonedaEquiv (coyoneda.map g.op ≫ f) := by
  change (f.app X ≫ F.map g) (𝟙 X) = f.app Y (g ≫ 𝟙 Y)
  rw [← f.naturality]
  simp

lemma coyonedaEquiv_comp {X : C} {F G : C ⥤ TypeCat.{v₁}} (α : coyoneda.obj (op X) ⟶ F) (β : F ⟶ G) :
...
lemma map_coyonedaEquiv {X Y : C} {F : C ⥤ TypeCat.{v₁}} (f : coyoneda.obj (op X) ⟶ F)
    (g : X ⟶ Y) : F.map g (coyonedaEquiv f) = f.app Y g := by
  rw [coyonedaEquiv_naturality, coyonedaEquiv_comp, coyonedaEquiv_coyoneda_map]

lemma coyonedaEquiv_symm_map {X Y : C} (f : X ⟶ Y) {F : C ⥤ TypeCat.{v₁}} (t : F.obj X) :
...
to `F.obj X`, functorially in both `X` and `F`.
-/
def coyonedaEvaluation : C × (C ⥤ TypeCat.{v₁}) ⥤ TypeCat.{max u₁ v₁} :=
  evaluationUncurried C (Type v₁) ⋙ uliftFunctor

@[simp]
theorem coyonedaEvaluation_map_down (P Q : C × (C ⥤ TypeCat.{v₁})) (α : P ⟶ Q)
...
-/
def coyonedaPairing : C × (C ⥤ TypeCat.{v₁}) ⥤ TypeCat.{max u₁ v₁} :=
  Functor.prod coyoneda.rightOp (𝟭 (C ⥤ TypeCat.{v₁})) ⋙ Functor.hom (C ⥤ TypeCat.{v₁})

@[ext]
lemma coyonedaPairingExt {X : C × (C ⥤ TypeCat.{v₁})} {x y : (coyonedaPairing C).obj X}
    (w : ∀ Y, x.app Y = y.app Y) : x = y :=
  NatTrans.ext (funext w)

@[simp]
theorem coyonedaPairing_map (P Q : C × (C ⥤ TypeCat.{v₁})) (α : P ⟶ Q) (β : (coyonedaPairing C).obj P) :
...
def coyonedaCompYonedaObj {C : Type u₁} [Category.{v₁} C] (P : C ⥤ TypeCat.{v₁}) :
    coyoneda.rightOp ⋙ yoneda.obj P ≅ P ⋙ uliftFunctor.{u₁} :=
  isoWhiskerRight largeCurriedCoyonedaLemma ((evaluation _ _).obj P)

/-- The curried version of coyoneda lemma when `C` is small. -/
def curriedCoyonedaLemma' {C : Type u₁} [SmallCategory C] :
    yoneda ⋙ (whiskeringLeft C (C ⥤ TypeCat.{u₁})ᵒᵖ (Type u₁)).obj coyoneda.rightOp
      ≅ 𝟭 (C ⥤ TypeCat.{u₁}) :=
...
auxiliary universe `w`. -/
@[simps! -isSimp]
def uliftCoyonedaEquiv {X : Cᵒᵖ} {F : C ⥤ TypeCat.{max w v₁}} :
...
@[reassoc]
lemma uliftCoyonedaEquiv_symm_map {X Y : C} (f : X ⟶ Y) {F : C ⥤ TypeCat.{max w v₁}}
    (t : F.obj X) :
...
lemma hom_ext_uliftCoyoneda {P Q : C ⥤ TypeCat.{max w v₁}} {f g : P ⟶ Q}
...
functor. -/
@[simps apply_app]
def Functor.sectionsEquivHom (F : C ⥤ TypeCat.{u₂}) (X : Type u₂) [Unique X] :
...
lemma Functor.sectionsEquivHom_naturality {F G : C ⥤ TypeCat.{u₂}} (f : F ⟶ G) (X : Type u₂) [Unique X]
    (x : F.sections) :
    (G.sectionsEquivHom X) ((sectionsFunctor C).map f x) = (F.sectionsEquivHom X) x ≫ f := by
  rfl

lemma Functor.sectionsEquivHom_naturality_symm {F G : C ⥤ TypeCat.{u₂}} (f : F ⟶ G) (X : Type u₂)
