...
* `CategoryTheory.B ⥤ᴼ C` : an oplax functor between bicategories `B` and `C`
...
@[simps]
def id (B : Type u₁) [Bicategory.{w₁, v₁} B] : B ⥤ᴼ B where
  toPrelaxFunctor := PrelaxFunctor.id B
  mapId := fun a => 𝟙 (𝟙 a)
  mapComp := fun f g => 𝟙 (f ≫ g)

instance : Inhabited (B ⥤ᴼ B) :=
  ⟨id B⟩
...
--@[simps]
def comp (F : B ⥤ᴼ C) (G : C ⥤ᴼ D) : B ⥤ᴼ D where
...
structure PseudoCore (F : B ⥤ᴼ C) where
