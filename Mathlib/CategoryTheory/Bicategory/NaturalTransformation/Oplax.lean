...
Using these, we define two `CategoryStruct` (scoped) instances on `B ⥤ᴼ C`, one in the
...
-/
structure OplaxTrans (F G : B ⥤ᴼ C) where
...
variable {F : B ⥤ᴼ C} {G H : B ⥤ᴼ C} (η : OplaxTrans F G) (θ : OplaxTrans G H)
...
/-- `CategoryStruct` on `B ⥤ᴼ C` where the (1-)morphisms are given by oplax
transformations. -/
@[simps! id_app id_naturality comp_app comp_naturality]
scoped instance : CategoryStruct (B ⥤ᴼ C) where
  Hom := OplaxTrans
  id := OplaxTrans.id
...
-/
structure StrongTrans (F G : B ⥤ᴼ C) where
  app (a : B) : F.obj a ⟶ G.obj a
  naturality {a b : B} (f : a ⟶ b) : F.map f ≫ app b ≅ app a ≫ G.map f
  naturality_naturality {a b : B} {f g : a ⟶ b} (η : f ⟶ g) :
...
structure OplaxTrans.StrongCore {F G : B ⥤ᴼ C} (η : F ⟶ G) where
...
@[simps]
def toOplax {F G : B ⥤ᴼ C} (η : StrongTrans F G) : OplaxTrans F G where
  app := η.app
...
def mkOfOplax {F G : B ⥤ᴼ C} (η : OplaxTrans F G) (η' : OplaxTrans.StrongCore η) :
...
/-- Construct a strong natural transformation from an oplax natural transformation whose
naturality 2-morphism is an isomorphism. -/
noncomputable def mkOfOplax' {F G : B ⥤ᴼ C} (η : OplaxTrans F G)
...
variable {F} {G H : B ⥤ᴼ C} (η : StrongTrans F G) (θ : StrongTrans G H)
...
/-- `CategoryStruct` on `B ⥤ᴼ C` where the (1-)morphisms are given by strong
transformations. -/
@[simps! id_app id_naturality comp_app comp_naturality]
scoped instance OplaxFunctor.instCategoryStruct : CategoryStruct (B ⥤ᴼ C) where
  Hom := StrongTrans
  id := StrongTrans.id
