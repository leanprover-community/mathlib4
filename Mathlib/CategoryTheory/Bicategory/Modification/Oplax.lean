...
@[simps]
scoped instance category (F G : B ⥤ᴼ C) : Category (F ⟶ G) where
  Hom := Modification
  id := Modification.id
  comp := Modification.vcomp

@[ext]
lemma ext {F G : B ⥤ᴼ C} {α β : F ⟶ G} {m n : α ⟶ β} (w : ∀ b, m.app b = n.app b) :
    m = n := by
...
@[simp]
lemma Modification.id_app' {X : B} {F G : B ⥤ᴼ C} (α : F ⟶ G) :
    Modification.app (𝟙 α) X = 𝟙 (α.app X) := rfl

/-- Version of `Modification.comp_app` using category notation -/
@[simp]
lemma Modification.comp_app' {X : B} {F G : B ⥤ᴼ C} {α β γ : F ⟶ G}
