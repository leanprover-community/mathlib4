...
@[simps -isSimp]
def ofMonoidAction (G : Type*) (H : Type u) [Monoid G] [MonoidAction G H] :
    Action (Type u) G where
  V := H
  ρ := (TypeCat.endEquiv _).toMonoidHom.comp (@MonoidAction.toEndHom _ _ _ (by assumption))
...
@[simp]
theorem ofMonoidAction_apply {G : Type*} {H : Type*} [Monoid G] [MonoidAction G H] (g : G) (x : H) :
...
theorem ofMonoidAction_apply {G : Type*} {H : FintypeCat.{u}} [Monoid G] [MonoidAction G H]
    (g : G) (x : H) : ConcreteCategory.hom ((FintypeCat.ofMonoidAction G H).ρ g) x = (g • x : H) :=
  rfl
