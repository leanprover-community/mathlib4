...
Authors: Sina Hazratpour, Gaëtan Serré
-/
module
...
/-!
# Wide Subcategories and Stability Structures

A wide subcategory of a category `C` is a subcategory containing all the objects of `C`.

This file also contains stability typeclasses for morphism properties with respect to
monoidal, braided, and comonoid structures, together with induced structure instances
on `WideSubcategory P`.
...
In the `CategoryTheory.MorphismProperty` namespace, this file defines:

`IsAssocStable` expresses stability under associator isomorphisms.
`IsUnitorStable` expresses stability under left and right unitor isomorphisms.
`IsMonoidalStable` combines tensor stability with associator and unitor stability.
`IsBraidedStable` extends monoidal stability with stability under braiding isomorphisms.
`IsComonStable` extends braided stability with stability under counit and comultiplication.
-/
