...
instance [W.RespectsIso] : (W.costructuredArrowObj L (X := X)).IsClosedUnderIsomorphisms :=
  inferInstaceAs% ((W.commaObj _ _).IsClosedUnderIsomorphisms)
...
instance : (Over.forget P ⊤ X).Faithful := inferInstaceAs% ((Comma.forget _ _ _ _ _).Faithful)
instance : (Over.forget P ⊤ X).Full := inferInstaceAs% ((Comma.forget _ _ _ _ _).Full)
...
instance : (Under.forget P ⊤ X).Faithful := inferInstaceAs% ((Comma.forget _ _ _ _ _).Faithful)
instance : (Under.forget P ⊤ X).Full := inferInstaceAs% ((Comma.forget _ _ _ _ _).Full)
