...
-/
class Reflective (R : D ⥤ C) extends R.IsRightAdjoint, R.Full, R.Faithful
...
theorem unit_obj_eq_map_unit [Reflective i] (X : C) :
    (ofIsRightAdjoint i).unit.app (i.obj (i.leftAdjoint.obj X)) =
      i.map ((i.leftAdjoint).map ((ofIsRightAdjoint i).unit.app X)) := by
  rw [← cancel_mono (i.map ((ofIsRightAdjoint i).counit.app (i.leftAdjoint.obj X))),
    ← i.map_comp]
  simp
...
-/
example [Reflective i] {B : D} : IsIso ((ofIsRightAdjoint i).unit.app (i.obj B)) :=
  inferInstance
...
theorem Functor.essImage.unit_isIso [Reflective i] {A : C} (h : A ∈ i.essImage) :
    IsIso ((ofIsRightAdjoint i).unit.app A) := by
...
theorem mem_essImage_of_unit_isIso [i.IsRightAdjoint] (A : C)
    [IsIso ((ofIsRightAdjoint i).unit.app A)] : A ∈ i.essImage :=
  ⟨i.leftAdjoint.obj A, ⟨(asIso ((ofIsRightAdjoint i).unit.app A)).symm⟩⟩
...
theorem mem_essImage_of_unit_isSplitMono [Reflective i] {A : C}
    [IsSplitMono ((ofIsRightAdjoint i).unit.app A)] : A ∈ i.essImage := by
  let η : 𝟭 C ⟶ i.leftAdjoint ⋙ i := (ofIsRightAdjoint i).unit
  haveI : IsIso (η.app (i.obj (i.leftAdjoint.obj A))) :=
    Functor.essImage.unit_isIso ((i.obj_mem_essImage _))
  have : Epi (η.app A) := by
    refine @epi_of_epi _ _ _ _ _ (retraction (η.app A)) (η.app A) ?_
    rw [show retraction _ ≫ η.app A = _ from η.naturality (retraction (η.app A))]
    apply epi_comp (η.app (i.obj (i.leftAdjoint.obj A)))
...
def unitCompPartialBijectiveAux [Reflective i] (A : C) (B : D) :
    (A ⟶ i.obj B) ≃ (i.obj (i.leftAdjoint.obj A) ⟶ i.obj B) :=
...
theorem unitCompPartialBijectiveAux_symm_apply [Reflective i] {A : C} {B : D}
    (f : i.obj (i.leftAdjoint.obj A) ⟶ i.obj B) :
...
def unitCompPartialBijective [Reflective i] (A : C) {B : C} (hB : B ∈ i.essImage) :
    (A ⟶ B) ≃ (i.obj (i.leftAdjoint.obj A) ⟶ B) :=
  calc
...
theorem unitCompPartialBijective_symm_natural [Reflective i] (A : C) {B B' : C} (h : B ⟶ B')
    (hB : B ∈ i.essImage) (hB' : B' ∈ i.essImage) (f : i.obj (i.leftAdjoint.obj A) ⟶ B) :
...
instance [Reflective i] (X : Functor.EssImageSubcategory i) :
  IsIso (NatTrans.app (ofIsRightAdjoint i).unit X.obj) :=
...
def equivEssImageOfReflective_counitIso_app [Reflective i] (X : Functor.EssImageSubcategory i) :
    ((Functor.essImageInclusion i ⋙ i.leftAdjoint) ⋙ Functor.toEssImage i).obj X ≅ X := by
  refine Iso.symm (@asIso _ _ X _ ((ofIsRightAdjoint i).unit.app X.obj) ?_)
...
lemma equivEssImageOfReflective_map_counitIso_app_hom [Reflective i]
  ...
  (Functor.essImageInclusion i).map (equivEssImageOfReflective_counitIso_app X).hom =
    inv (NatTrans.app (ofIsRightAdjoint i).unit X.obj) := by
...
lemma equivEssImageOfReflective_map_counitIso_app_inv [Reflective i]
  ...
  (Functor.essImageInclusion i).map (equivEssImageOfReflective_counitIso_app X).inv =
    (NatTrans.app (ofIsRightAdjoint i).unit X.obj) := rfl
...
def equivEssImageOfReflective [Reflective i] : D ≌ i.EssImageSubcategory
  ...
  inverse := i.essImageInclusion ⋙ (i.leftAdjoint : _)
  unitIso :=
