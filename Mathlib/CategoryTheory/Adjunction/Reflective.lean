...
/-- The reflector `D ⥤ C` when `R : D ⥤ C` is reflective. -/
...
/-- For a reflective functor `i` (with left adjoint `L`), with unit `η`, we have `η_iL = iL η`.
-/
theorem counit_obj_eq_map_counit [Coreflective j] (X : D) :
...
(For any `B` in the reflective subcategory, we automatically have that `ε_B` is an iso.)
-/
theorem Functor.essImage.counit_isIso [Coreflective j] {A : D} (h : A ∈ j.essImage) :
    IsIso ((coreflectorAdjunction j).counit.app A) := by
  rwa [isIso_counit_app_iff_mem_essImage]

/-- If `η_A` is an isomorphism, then `A` is in the essential image of `i`. -/
theorem mem_essImage_of_counit_isIso {R : D ⥤ C} (adj : j ⊣ R) (A : D)
...
theorem mem_essImage_of_counit_isSplitEpi [Coreflective j] {A : D}
  ...
  exact mem_essImage_of_counit_isIso (coreflectorAdjunction j) A
