...
instance (priority := 10000) : IsFilteredOrEmpty (FullSubcategory (FilteredClosure f)) where
  cocone_objs j j' :=
...
instance (priority := 10000) : EssentiallySmall.{max v w} (FullSubcategory (FilteredClosure f)) :=
...
noncomputable instance (priority := 10000) : SmallCategory (SmallFilteredIntermediate F) :=
...
instance (priority := 10000) : Faithful (inclusion F) :=
  show Faithful ((equivSmallModel _).inverse ⋙ fullSubcategoryInclusion _) from Faithful.comp _ _

noncomputable instance (priority := 10000) : Full (inclusion F) :=
...
instance (priority := 10000) : IsFilteredOrEmpty (SmallFilteredIntermediate F) :=
  IsFilteredOrEmpty.of_equivalence (equivSmallModel _)

instance (priority := 10000) [Nonempty D] : IsFiltered (SmallFilteredIntermediate F) :=
...
instance (priority := 10000) : IsCofilteredOrEmpty (FullSubcategory (CofilteredClosure f)) where
  cone_objs j j' :=
...
instance (priority := 10000) : EssentiallySmall.{max v w} (FullSubcategory (CofilteredClosure f)) :=
...
noncomputable instance (priority := 10000) : SmallCategory (SmallCofilteredIntermediate F) :=
...
instance (priority := 10000) : IsCofilteredOrEmpty (SmallCofilteredIntermediate F) :=
  IsCofilteredOrEmpty.of_equivalence (equivSmallModel _)

instance (priority := 10000) [Nonempty D] : IsCofiltered (SmallCofilteredIntermediate F) :=
