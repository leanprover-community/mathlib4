...
lemma Presheaf.comp_χ_eq {F G : Cᵒᵖ ⥤ Type (max u v)} (m : F ⟶ G) :
    m ≫ Presheaf.χ m = (Functor.isTerminalConst _ Types.isTerminalPUnit).from F ≫
      Presheaf.truth C := by
  ext
  apply Sieve.ext
...
lemma Presheaf.isPullback_χ_truth {F G : Cᵒᵖ ⥤ Type (max u v)} (m : F ⟶ G) [Mono m] :
    IsPullback m ((Functor.isTerminalConst _ Types.isTerminalPUnit).from F)
      (χ m) (truth C) := by
...
lemma Presheaf.isClosed_χ_app_apply_of_isSheaf_of_isSeparated (J : GrothendieckTopology C)
  ...
  intro Y f hf
  simp only [χ_app, Opposite.op_unop] at hf ⊢
