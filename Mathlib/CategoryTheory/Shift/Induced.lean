import Mathlib.CategoryTheory.Functor.CommShift
...
variable {C D : Type _} [Category C] [Category D]
  ...
  [Full ((whiskeringLeft C D D).obj F)]
  [Faithful ((whiskeringLeft C D D).obj F)]
...
noncomputable def induced : HasShift D A :=
  hasShiftMk D A
    { F := s
      zero := induced_zero F s i
      add := induced_add F s i
      zero_add_hom_app := fun n => by
        suffices (induced_add F s i 0 n).hom =
          eqToHom (by rw [zero_add]; rfl) ≫ whiskerRight (induced_zero F s i).inv (s n) by
          intro X
      ...
      add_zero_hom_app := fun n => by
        suffices (induced_add F s i n 0).hom =
            eqToHom (by rw [add_zero]; rfl) ≫ whiskerLeft (s n) (induced_zero F s i).inv  by
          intro X
          simpa using NatTrans.congr_app this X
        apply ((whiskeringLeft C D D).obj F).map_injective
        ext X
      ...
      assoc_hom_app := fun m₁ m₂ m₃ => by
        suffices (induced_add F s i (m₁ + m₂) m₃).hom ≫
            whiskerRight (induced_add F s i m₁ m₂).hom (s m₃) =
            eqToHom (by rw [add_assoc]) ≫ (induced_add F s i m₁ (m₂ + m₃)).hom ≫
              whiskerLeft (s m₁) (induced_add F s i m₂ m₃).hom by
          intro X
          simpa using NatTrans.congr_app this X
        apply ((whiskeringLeft C D D).obj F).map_injective
        ext X
