...
lemma opEquiv_symm_comp {a b : ℤ}
    ...
    (opEquiv _).symm (f.comp g h) =
      ((opEquiv _).symm g).comp ((opEquiv _).symm f) (by grind) := by
  rw [opEquiv_symm_apply, opEquiv_symm_apply,
    opShiftFunctorEquivalence_unitIso_inv_app_eq _ _ _ _ (show a + b = c by grind), comp, comp]
  dsimp
...
noncomputable def opEquiv' (n a a' : ℤ) (h : n + a = a') :
  ...
  ((shiftFunctorAdd' C a n a' (by grind)).symm.app Y).homToEquiv.symm.trans (opEquiv n)

lemma opEquiv'_symm_apply {n a : ℤ} (f : Opposite.op (Y⟦a⟧) ⟶ (Opposite.op X)⟦n⟧)
    (a' : ℤ) (h : n + a = a') :
    (opEquiv' n a a' h).symm f =
      (opEquiv n).symm f ≫ (shiftFunctorAdd' C a n a' (by grind)).inv.app _ :=
  rfl

lemma opEquiv'_apply {a' : ℤ} (f : ShiftedHom X Y a') (n a : ℤ) (h : n + a = a') :
    opEquiv' n a a' h f =
      opEquiv n (f ≫ (shiftFunctorAdd' C a n a' (by grind)).hom.app Y) := by
  rfl
...
lemma opEquiv'_add_symm (n m a a' a'' : ℤ) (ha' : n + a = a') (ha'' : m + a' = a'')
    ...
    (opEquiv' (m + n) a a'' (by grind)).symm x =
  ...
  simp only [assoc, Functor.map_comp, ← shiftFunctorAdd'_eq_shiftFunctorAdd,
    ...
    shiftFunctorAdd'_assoc_inv_app a n m a' (m + n) a'' (by grind) (by grind) (by grind)]
  rfl
