...
def tensor_eComp_eq (c c' c'' : C) (d d' d'': D) : eComp V (C := C × D) ⟨c, d⟩ ⟨c', d'⟩ ⟨c'', d''⟩ =
    tensorμ _ _ _ _ ≫ (tensorHom (eComp V _ _ _) (eComp V _ _ _)) := rfl
...
def enrichedBifunctor.mk {E : Type u₄} [EnrichedCategory V E]
    ...
    (F_comp_left : (c c' c'' : C) → (d : D) →
      ...
      ((F_map_left c c' d) ⊗ (F_map_left c' c'' d)) ≫ eComp V _ _ _)
    (F_comp_right : (c : C) → (d d' d'' : D) →
      eComp V d d' d'' ≫ F_map_right c d d'' =
      ((F_map_right c d d') ⊗ (F_map_right c d' d'')) ≫ eComp V _ _ _)
    (F_left_right_naturality : (c c' : C) → (d d' : D) →
      ((F_map_left c c' d) ⊗ (F_map_right c' d d')) ≫ eComp V _ _ _ =
      ((F_map_left c c' d') ⊗ (F_map_right c d d')) ≫ (β_ _ _).inv ≫ eComp V _ _ _)
    : EnrichedFunctor V (C × D) E where
  obj p := F_obj p.1 p.2
  map p q := ((F_map_left p.1 q.1 p.2) ⊗ (F_map_right q.1 p.2 q.2)) ≫ eComp V _ _ _
  map_id p := by
    have : eId V p = (λ_ _).inv ≫ ((eId V p.1) ⊗ (eId V p.2)) := rfl
  ...
  map_comp p q r := by
    have : eComp V p q r = tensorμ _ _ _ _ ≫
      (tensorHom (eComp V p.1 q.1 r.1) (eComp V p.2 q.2 r.2)) := rfl
    simp only [this, Category.assoc]
...
structure EnrichedBifunctor where
  ...
  comp_left : (c c' c'' : C) → (d : D) → eComp V c c' c'' ≫ map_left c c'' d =
    ((map_left c c' d) ⊗ (map_left c' c'' d)) ≫ eComp V _ _ _
  comp_right : (c : C) → (d d' d'' : D) → eComp V d d' d'' ≫ map_right c d d'' =
    ((map_right c d d') ⊗ (map_right c d' d'')) ≫ eComp V _ _ _
  left_right_naturality : (c c' : C) → (d d' : D) → ((map_left c c' d) ⊗ (map_right c' d d')) ≫
    eComp V _ _ _ = ((map_left c c' d') ⊗ (map_right c d d')) ≫ (β_ _ _).inv ≫ eComp V _ _ _
...
lemma braiding_tensorμ (a b c d: V) : (β_ (c ⊗ d) (a ⊗ b)).inv ≫ tensorμ c d a b =
  ...
  rw [tensorHom_def]
  simp only [BraidedCategory.braiding_inv_tensor_right, BraidedCategory.braiding_inv_tensor_left,
...
def enrichedBifunctorEquiv.from (F : EnrichedFunctor V (C × D) E) : EnrichedBifunctor V C D E where
  ...
  id_left c d := by
    have : ((λ_ (𝟙_ V)).inv ≫ (eId V c ⊗ eId V d)) = eId V (C := C × D) ⟨c, d⟩ := rfl
  ...
  left_right_naturality c c' d d' := by
    ...
    simp only [braiding_inv_tensorUnit_left, braiding_inv_tensorUnit_right, tensor_comp,
      Category.assoc, tensor_inv_hom_id, tensorHom_id, leftUnitor_inv_whiskerRight,
      leftUnitor_whiskerRight, Iso.inv_hom_id_assoc, Iso.inv_hom_id, Category.comp_id]
    rw [tensorμ_unit_unit]

lemma enrichedFunctorEq (F G : EnrichedFunctor V C D) (obj_eq : F.obj = G.obj) (map_eq : HEq F.map G.map)
    : F = G := by
  induction F with
...
def enrichedBifunctorEquiv : (EnrichedBifunctor V C D E) ≃ (EnrichedFunctor V (C × D) E) where
  ...
  left_inv := fun F => by
    ...
    have : (enrichedBifunctorEquiv.to V C D E F).map ⟨c, d⟩ ⟨c', d⟩
      = ((F.map_left c c' d) ⊗ (F.map_right c' d d)) ≫ eComp V _ _ _ := rfl
    rw [this]
    rw [← id_tensorHom]
    rw [← tensor_comp_assoc]
    rw [F.id_right]
  ...
  right_inv := fun F => by
    ...
    have : (enrichedBifunctorEquiv.to V C D E F').map p q = ((F'.map_left p.1 q.1 p.2) ⊗ (F'.map_right q.1 p.2 q.2)) ≫ eComp V _ _ _ := rfl
    rw [this]
    have : F'.map_left p.1 q.1 p.2 = (ρ_ _).inv ≫ (_ ◁ eId V p.2) ≫ F.map ⟨p.1, p.2⟩ ⟨q.1, p.2⟩ := rfl
    rw [this]
    have : F'.map_right q.1 p.2 q.2 = (λ_ _).inv ≫ (eId V q.1 ▷ _) ≫ F.map ⟨q.1, p.2⟩ ⟨q.1, q.2⟩ := rfl
    rw [this]
    have : eComp V (F'.obj p.1 p.2) (F'.obj q.1 p.2) (F'.obj q.1 q.2) = eComp V (F.obj ⟨p.1, p.2⟩) (F.obj ⟨q.1, p.2⟩) (F.obj ⟨q.1, q.2⟩) := rfl
    rw [this]
    ...
    rw [← tensorHom_id, ← id_tensorHom]
    rw [tensorμ_natural_assoc]
    rw [tensorHom_id, id_tensorHom, ← tensor_comp_assoc]
    ...
    unfold tensorμ
    simp only [Category.assoc]
    have : (β_ (𝟙_ V) (𝟙_ V)).hom = 𝟙 _ := by simp only [braiding_tensorUnit_right]; rw [← unitors_equal]; exact (λ_ _).hom_inv_id
    rw [this]
    simp only [id_whiskerRight, MonoidalCategory.whiskerLeft_id, Category.id_comp,
