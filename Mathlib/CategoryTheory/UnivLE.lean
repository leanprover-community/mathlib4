...
instance (priority := 10000) EssSurj.ofUnivLE [UnivLE.{max u v, v}] :
...
instance (priority := 10000) [UnivLE.{max u v, v}] : IsEquivalence uliftFunctor.{u, v} :=
...
instance (priority := 10000) [UnivLE.{max u v, v}] : Faithful UnivLE.witness.{u, v} :=
  inferInstanceAs <| Faithful (_ ⋙ _)

instance (priority := 10000) [UnivLE.{max u v, v}] : Full UnivLE.witness.{u, v} :=
