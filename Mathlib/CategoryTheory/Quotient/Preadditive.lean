...
def preadditive
  ...
  homGroup P Q :=
    ...
    { add_assoc := by rintro ⟨_⟩ ⟨_⟩ ⟨_⟩; exact congr((functor r).map $(add_assoc _ _ _))
      zero_add := by rintro ⟨_⟩; exact congr((functor r).map $(zero_add _))
      add_zero := by rintro ⟨_⟩; exact congr((functor r).map $(add_zero _))
      add_comm := by rintro ⟨_⟩ ⟨_⟩; exact congr((functor r).map $(add_comm _ _))
      neg_add_cancel := by rintro ⟨_⟩; exact congr((functor r).map $(neg_add_cancel _))
      -- todo: use a better defeq
      nsmul := nsmulRec
      zsmul := zsmulRec }
  add_comp := by
    rintro _ _ _ ⟨_⟩ ⟨_⟩ ⟨_⟩
    exact congr((functor r).map $(by apply Preadditive.add_comp))
  comp_add := by
    rintro _ _ _ ⟨_⟩ ⟨_⟩ ⟨_⟩
    exact congr((functor r).map $(by apply Preadditive.comp_add))
