...
A typeclass `IsCofiltered.IsEventuallyConstant` is also introduced, and
...
/-- Given `h : F.IsEventuallyConstantFrom i₀`, this is the (colimit) cocone for `F` whose
point is `F.obj i₀`. -/
@[simps]
...
`c.ι.app i₀` is an isomorphism, `c` is a colimit cocone. -/
