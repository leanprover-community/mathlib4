...
@[simps!]
noncomputable def liftToFinsetObj (F : Discrete α ⥤ C) : Finset (Discrete α) ⥤ C where
...
@[simps!]
noncomputable def liftToFinsetColimitCocone [HasColimitsOfShape (Finset (Discrete α)) C]
...
coproducts. -/
@[simps!]
noncomputable def liftToFinset : (Discrete α ⥤ C) ⥤ (Finset (Discrete α) ⥤ C) where
  obj := liftToFinsetObj
...
@[simps!]
noncomputable def finiteSubcoproductsCocone (f : α → C) [HasCoproduct f] :
...
functor. -/
noncomputable def liftToFinsetColimIso : liftToFinset C α ⋙ colim ≅ colim :=
  NatIso.ofComponents
...
with `colim`. -/
noncomputable def liftToFinsetEvaluationIso [HasFiniteCoproducts C] (I : Finset (Discrete α)) :
...
`Finset (Discrete α) ⥤ C` by taking coproducts. -/
@[simps!]
noncomputable def liftToFinsetObj (F : Discrete α ⥤ C) : (Finset (Discrete α))ᵒᵖ ⥤ C where
...
taking the colimit of the diagram formed by the coproducts of finite sets over the indexing type. -/
@[simps!]
noncomputable def liftToFinsetLimitCone [HasLimitsOfShape (Finset (Discrete α))ᵒᵖ C]
...
@[simps!]
noncomputable def finiteSubproductsCone (f : α → C) [HasProduct f] :
...
coproducts. -/
@[simps!]
noncomputable def liftToFinset : (Discrete α ⥤ C) ⥤ ((Finset (Discrete α))ᵒᵖ ⥤ C) where
  obj := liftToFinsetObj
...
/-- The `liftToFinset` functor, precomposed with forming a colimit, is a coproduct on the original
functor. -/
noncomputable def liftToFinsetLimIso [HasLimitsOfShape (Finset (Discrete α))ᵒᵖ C]
