...
instance (priority := 10000) {B : C} [HasPullbacks C] : HasPullbacks (Over B) := by
...
instance (priority := 10000) {B : C} [HasEqualizers C] : HasEqualizers (Over B) := by
...
instance (priority := 10000) hasFiniteLimits {B : C} [HasFiniteWidePullbacks C] : HasFiniteLimits (Over B) := by
...
instance (priority := 10000) hasLimits {B : C} [HasWidePullbacks.{w} C] : HasLimitsOfSize.{w} (Over B) := by
