...
is the quotient of `Y` by the relation generated by `f x ~ g x` for `x : X`.
...
/-- The categorical coequalizer in `Type u` is the quotient by the relation generated by `f x ~ g x`. -/
