...
Let `C` be a concrete category and `F : J ⥤ C` a diagram in `C`. We discuss some results about
`colimit F` when objects and morphisms in `C` has some algebraic structures.
