...
theorem to_product_injective_of_isLimit {D : Cone F} (hD : IsLimit D) :
    Function.Injective fun (x : CC D.pt) (j : J) => D.π.app j x := by
...
theorem isLimit_ext {D : Cone F} (hD : IsLimit D) (x y : CC D.pt) :
    (∀ j, D.π.app j x = D.π.app j y) → x = y := fun h =>
  Concrete.to_product_injective_of_isLimit _ hD (funext h)

theorem limit_ext [HasLimit F] (x y : CC ↑(limit F)) :
...
theorem from_union_surjective_of_isColimit {D : Cocone F} (hD : IsColimit D) :
    let ff : (Σj : J, CC (F.obj j)) → CC D.pt := fun a => D.ι.app a.1 a.2
...
theorem isColimit_exists_rep {D : Cocone F} (hD : IsColimit D) (x : CC D.pt) :
    ∃ (j : J) (y : CC (F.obj j)), D.ι.app j y = x := by
  obtain ⟨a, rfl⟩ := Concrete.from_union_surjective_of_isColimit F hD x
  exact ⟨a.1, a.2, rfl⟩

theorem colimit_exists_rep [HasColimit F] (x : CC (colimit F)) :
    ∃ (j : J) (y : CC (F.obj j)), colimit.ι F j y = x :=
  Concrete.isColimit_exists_rep F (colimit.isColimit _) x

end

theorem isColimit_rep_eq_of_exists {D : Cocone F} {i j : J} (x : CC (F.obj i))
    (y : CC (F.obj j)) (h : ∃ (k : _) (f : i ⟶ k) (g : j ⟶ k), F.map f x = F.map g y) :
...
theorem colimit_rep_eq_of_exists [HasColimit F] {i j : J} (x : CC (F.obj i))
    (y : CC (F.obj j))
...
theorem isColimit_exists_of_rep_eq {D : Cocone F} {i j : J} (hD : IsColimit D)
    (x : CC (F.obj i)) (y : CC (F.obj j)) (h : D.ι.app _ x = D.ι.app _ y) :
...
theorem colimit_exists_of_rep_eq [HasColimit F] {i j : J} (x : CC (F.obj i))
    (y : CC (F.obj j))
...
theorem colimit_rep_eq_iff_exists [HasColimit F] {i j : J} (x : CC (F.obj i))
    (y : CC (F.obj j)) :
