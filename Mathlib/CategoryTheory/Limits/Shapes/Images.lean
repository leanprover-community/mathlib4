...
-- ought to exist...
instance [Mono f] : Inhabited (MonoFactorisation f) := fast_instance% ⟨self f⟩
...
instance [Mono f] : Inhabited (IsImage (MonoFactorisation.self f)) := fast_instance%
  ⟨self f⟩
...
instance [Mono f] : Inhabited (ImageFactorisation f) := fast_instance%
  ⟨⟨_, IsImage.self f⟩⟩
...
instance image.lift_mono (F' : MonoFactorisation f) : Mono (image.lift F') := fast_instance% by
...
instance (h : f = f') : IsIso (image.eqToHom h) := fast_instance%
...
-/
instance image.preComp_mono [HasImage g] [HasImage (f ≫ g)] : Mono (image.preComp f g) := fast_instance% by
...
instance hasImage_iso_comp [IsIso f] [HasImage g] : HasImage (f ≫ g) := fast_instance%
  HasImage.mk
...
(we need `C` to have equalizers to prove this).
-/
instance image.isIso_precomp_iso (f : X ⟶ Y) [IsIso f] [HasImage g] : IsIso (image.preComp f g) := fast_instance%
  ⟨⟨image.lift
        { I := image (f ≫ g)
          m := image.ι (f ≫ g)
...
-- `image f ⟶ image (f ≫ g)`.
instance hasImage_comp_iso [HasImage f] [IsIso g] : HasImage (f ≫ g) := fast_instance%
  HasImage.mk
...
instance {X Y : C} (f : X ⟶ Y) [HasImage f] : HasImage (Arrow.mk f).hom := fast_instance%
...
instance inhabitedImageMap {f : Arrow C} [HasImage f.hom] : Inhabited (ImageMap (𝟙 f)) := fast_instance%
  ⟨⟨𝟙 _, by aesop⟩⟩
...
instance : Subsingleton (ImageMap sq) := fast_instance%
