...
instance (priority := 10000) hasZeroObject_pUnit : HasZeroObject (Discrete PUnit) where zero :=
  ⟨⟨⟨⟩⟩,
    { unique_to := fun ⟨⟨⟩⟩ =>
      ⟨{ default := 𝟙 _,
...
instance (priority := 10000) hasZeroObject_op : HasZeroObject Cᵒᵖ :=
...
instance (priority := 10000) (X : C) : Subsingleton (X ≅ 0) := ⟨fun f g => by ext⟩

instance (priority := 10000) {X : C} (f : 0 ⟶ X) : Mono f where right_cancellation g h _ := by ext

instance (priority := 10000) {X : C} (f : X ⟶ 0) : Epi f where left_cancellation g h _ := by ext

instance (priority := 10000) zero_to_zero_isIso (f : (0 : C) ⟶ 0) : IsIso f := by
