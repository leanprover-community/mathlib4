...
inductive Pushout.Rel' : Sum X₁ X₂ → Sum X₁ X₂ → Prop
  | refl (x : Sum X₁ X₂) : Rel' x x
...
lemma Rel'.symm {x y : Sum X₁ X₂} (h : Rel' f g x y) :
    Rel' f g y x := by
...
lemma quot_mk_eq_iff [Mono f] (a b : Sum X₁ X₂) :
...
lemma pushoutCocone_inl_eq_inr_iff_of_iso {c c' : PushoutCocone i₁ i₂} (e : c ≅ c') (x₁ : X₁) (x₂ : X₂) :
...
lemma pushoutCocone_inl_eq_inr_iff_of_isColimit {c : PushoutCocone i₁ i₂} (hc : IsColimit c)
  ...
  rw [pushoutCocone_inl_eq_inr_iff_of_iso
    (Cocones.ext (IsColimit.coconePointUniqueUpToIso hc (Pushout.isColimitCocone i₁ i₂)) (by aesop_cat))]
