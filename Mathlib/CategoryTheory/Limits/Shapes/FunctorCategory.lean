...
instance (priority := 10000) functor_category_hasFiniteLimits [HasFiniteLimits D] : HasFiniteLimits (C ⥤ D) where
...
instance (priority := 10000) functor_category_hasFiniteColimits [HasFiniteColimits D] : HasFiniteColimits (C ⥤ D) where
  out _ _ _ := inferInstance
