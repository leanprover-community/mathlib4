...
/-- The property of a functor between arrow categories which preserves pushouts. -/
class PreservesCobaseChange (F : Arrow C₁ ⥤ Arrow C₂) : Prop where
  preservesPushout {A B K L : C₁} {f : A ⟶ B} {s : K ⟶ A} {g : K ⟶ L} {t : L ⟶ B}
    (h : IsPushout s g f t) :
    IsPushout
      (F.map (Arrow.homMk' s t h.w)).left
      (F.obj (Arrow.mk g)).hom
