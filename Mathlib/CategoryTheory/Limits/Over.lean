...
noncomputable instance createsColimitsOfSize : CreatesColimitsOfSize.{w, w'} (forget X) :=
...
noncomputable instance createsColimitsOfSizeMapCompForget {Y : C} (f : X ⟶ Y) :
...
noncomputable def _root_.CategoryTheory.Limits.colimit.isColimitToOver (F : J ⥤ C) [HasColimit F] :
...
noncomputable instance createsLimitsOfSize : CreatesLimitsOfSize.{w, w'} (forget X) :=
...
noncomputable instance createLimitsOfSizeMapCompForget {Y : C} (f : X ⟶ Y) :
...
noncomputable def isLimitToUnder {F : J ⥤ C} {c : Cone F} (hc : IsLimit c) : IsLimit c.toUnder :=
...
/-- If `F` has a limit, then the cone `limit.toUnder F` with cone point `𝟙 (limit F)` is
    also a limit cone. -/
noncomputable def _root_.CategoryTheory.Limits.limit.isLimitToOver (F : J ⥤ C) [HasLimit F] :
