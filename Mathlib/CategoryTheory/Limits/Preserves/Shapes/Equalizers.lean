...
instance : IsIso (equalizerComparison f g G) := fast_instance% by
...
instance : IsIso (coequalizerComparison f g G) := fast_instance% by
  rw [← PreservesCoequalizer.iso_hom]
  infer_instance

instance map_π_epi : Epi (G.map (coequalizer.π f g)) := fast_instance%
  ⟨fun {W} h k => by
