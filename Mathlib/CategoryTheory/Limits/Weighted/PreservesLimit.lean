...
public instance preservesLimit : PreservesLimit G (weightedLimObj.{w} W (C := C)) where
  preserves hc := ⟨preservesLimit.isLimitMapCone _ hc⟩

end weightedLimObj

public instance [HasColimitsOfShape Kᵒᵖ (Type w)]
...
public instance [HasColimitsOfSize.{v'', u''} (Type w)]
    (F : J ⥤ C) [HasWeightedLimFlipObj.{w} F] :
...
public instance [HasLimitsOfShape K C] (W : J ⥤ Type w) [HasWeightedLimObj.{w} W C] :
    PreservesLimitsOfShape K (W.weightedLimObj.{w} (C := C)) where

public instance [HasLimitsOfSize.{v'', u''} C] (W : J ⥤ Type w) [HasWeightedLimObj.{w} W C] :
    PreservesLimitsOfSize.{v'', u''} (W.weightedLimObj (C := C)) where

public instance [HasColimitsOfShape Kᵒᵖ (Type w)] [∀ (W : J ⥤ Type w), W.HasWeightedLimObj C] :
...
public instance [HasLimitsOfShape K C] [∀ (W : J ⥤ Type w), W.HasWeightedLimObj C] :
...
public instance [HasLimitsOfSize.{v'', u''} C] [∀ (W : J ⥤ Type w), W.HasWeightedLimObj C] :
