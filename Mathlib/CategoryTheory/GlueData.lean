...
instance t_isIso (i j : D.J) : IsIso (D.t i j) := fast_instance%
  ⟨⟨D.t j i, D.t_inv _ _, D.t_inv _ _⟩⟩
#align category_theory.glue_data.t_is_iso CategoryTheory.GlueData.t_isIso

instance t'_isIso (i j k : D.J) : IsIso (D.t' i j k) := fast_instance%
...
instance π_epi : Epi D.π := fast_instance% by
  unfold π
  infer_instance
...
instance (i j k : D.J) : HasPullback (F.map (D.f i j)) (F.map (D.f i k)) := fast_instance%
