...
instance (priority := 10000) [HasZeroObject C] {X Y : C} (f : X ⟶ Y) [Mono f]
    [IsIso (Abelian.coimageImageComparison f)] : IsIso (imageMonoFactorisation f).e := by
  rw [imageMonoFactorisation_e']
  exact IsIso.comp_isIso

instance (priority := 10000) [HasZeroObject C] {X Y : C} (f : X ⟶ Y) [Epi f] : IsIso (imageMonoFactorisation f).m := by
  dsimp
  infer_instance
...
instance (priority := 10000) : Epi (Abelian.factorThruImage f) := by infer_instance

instance (priority := 10000) isIso_factorThruImage [Mono f] : IsIso (Abelian.factorThruImage f) := by infer_instance
#align category_theory.abelian.is_iso_factor_thru_image CategoryTheory.Abelian.isIso_factorThruImage

/-- The canonical morphism `i : coimage f ⟶ Q` is a monomorphism -/
instance (priority := 10000) : Mono (Abelian.factorThruCoimage f) := by infer_instance

instance (priority := 10000) isIso_factorThruCoimage [Epi f] : IsIso (Abelian.factorThruCoimage f) := by infer_instance
...
-/
instance (priority := 10000) : IsIso (coimageImageComparison f) := by
  convert
    IsIso.of_iso
...
instance (priority := 10000) epi_pullback_of_epi_f [Epi f] : Epi (pullback.snd : pullback f g ⟶ Y) :=
...
/-- In an abelian category, the pullback of an epimorphism is an epimorphism. -/
instance (priority := 10000) epi_pullback_of_epi_g [Epi g] : Epi (pullback.fst : pullback f g ⟶ X) :=
...
instance (priority := 10000) mono_pushout_of_mono_f [Mono f] : Mono (pushout.inr : Z ⟶ pushout f g) :=
...
instance (priority := 10000) mono_pushout_of_mono_g [Mono g] : Mono (pushout.inl : Y ⟶ pushout f g) :=
  mono_of_cancel_zero _ fun {R} e h => by
