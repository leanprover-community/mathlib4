...
lemma exists_transfiniteCompositionOfShape :
  ...
  have : Nonempty (Shrink (Set.Iio o)) := ⟨j⟩
  have : OrderBot (Shrink (Set.Iio o)) := WellFoundedLT.toOrderBot _
...
instance : HasSmallObjectArgument.{w} (generatingMonomorphisms G) := by
  ...
  have : Nonempty (Shrink (Set.Iio κ.ord)) := by simpa using hκ'.ne_zero
  have := WellFoundedLT.toOrderBot (Shrink (Set.Iio κ.ord))
  exact ⟨κ, inferInstance, inferInstance,
    { preservesColimit {A B X Y} i hi f hf := by
        let hf' : (monomorphisms C).TransfiniteCompositionOfShape (Shrink (Set.Iio κ.ord)) f :=
        ...
        have (j j' : Shrink (Set.Iio κ.ord)) (φ : j ⟶ j') : Mono (hf'.F.map φ) := hf'.mem_map φ
