...
lifting `𝟙 R`, then `Cartesian.map f φ φ'` is the morphism `a' ⟶ a` obtained from the universal
property of `φ`. -/
protected noncomputable def map : a' ⟶ a :=
  Classical.choose <| IsCartesian.universal_property (p:=p) (f:=f) (φ:=φ) φ'

instance map_isHomLift : IsHomLift p (𝟙 R) (Cartesian.map p f φ φ') :=
  (Classical.choose_spec <| IsCartesian.universal_property (p:=p) (f:=f) (φ:=φ) φ').1.1

@[reassoc (attr := simp)]
lemma fac : Cartesian.map p f φ φ' ≫ φ = φ' :=
...
@[simp]
lemma map_self : Cartesian.map p f φ φ = 𝟙 a := by
...
noncomputable def domainUniqueUpToIso {a' : 𝒳} (φ' : a' ⟶ b) [IsCartesian p f φ'] : a' ≅ a where
  hom := Cartesian.map p f φ φ'
  inv := Cartesian.map p f φ' φ
  hom_inv_id := by
...
instance of_iso_comp {a' : 𝒳} (φ' : a' ≅ a) [IsHomLift p (𝟙 R) φ'.hom] :
  ...
  universal_property := by
    intro c ψ hψ
    use Cartesian.map p f φ ψ ≫ φ'.inv
