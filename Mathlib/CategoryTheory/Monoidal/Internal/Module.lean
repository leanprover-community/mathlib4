...
-/
@[simps!]
def functor : Mon_ (ModuleCat.{u} R) ⥤ Alg R where
  obj A := Alg.of R A.X
  map {_ _} f := Alg.ofHom
...
-/
@[simps]
def inverseObj (A : Alg.{u} R) : Mon_ (ModuleCat.{u} R) where
  X := ModuleCat.of R A
  ...
  one_mul := by
    ...
    -- dsimp [Alg.id_apply, TensorProduct.mk_apply, Algebra.linearMap_apply,
    --    LinearMap.compr₂_apply, Function.comp_apply, RingHom.map_one,
    --    ModuleCat.MonoidalCategory.tensorHom_tmul, Alg.hom_comp,
  ...
  mul_one := by
    ...
    -- Porting note: this `dsimp` does nothing
    -- dsimp only [Alg.id_apply, TensorProduct.mk_apply, Algebra.linearMap_apply,
...
-/
@[simps]
def inverse : Alg.{u} R ⥤ Mon_ (ModuleCat.{u} R) where
  obj := inverseObj
  map f :=
...
is equivalent to the category of "native" bundled `R`-algebras.
-/
def monModuleEquivalenceAlgebra : Mon_ (ModuleCat.{u} R) ≌ Alg R where
  functor := functor
  inverse := inverse
  unitIso :=
    NatIso.ofComponents
