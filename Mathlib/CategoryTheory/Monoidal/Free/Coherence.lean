...
instance (priority := 10000) (x y : N C) : Subsingleton (x ⟶ y) := Discrete.instSubsingletonDiscreteHom _ _
...
/-- The monoidal coherence theorem. -/
instance (priority := 10000) subsingleton_hom : Quiver.IsThin (F C) := fun X Y =>
  ⟨fun f g => by
...
instance (priority := 10000) : Groupoid.{u} (F C) :=
