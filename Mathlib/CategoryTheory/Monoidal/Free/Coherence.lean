...
@[simp]
lemma inclusion_obj (X : N C) :
    inclusion.obj X = inclusionObj X.as :=
  rfl

@[simp]
lemma inclusion_map {X Y : N C} (f : X ⟶ Y) :
...
lemma tensorFunc_map_app {X Y : F C} (f : X ⟶ Y) (n) : ((tensorFunc C).map f).app n = _ ◁ f :=
  rfl
...
lemma normalizeObj_congr (n : NormalMonoidalObject C) {X Y : F C} (f : X ⟶ Y) :
...
lemma normalize_naturality (n : NormalMonoidalObject C) {X Y : F C} (f : X ⟶ Y) :
