...
@[to_additive instIsAddModHomId]
instance instIsModHomId : IsModHom A (𝟙 M) where

instance instIsAddModHomComp {A₁ : C} [AddMonObj A₁] {M₁ N₁ O₁ : D}
    [AddModObj A₁ M₁] [AddModObj A₁ N₁] [AddModObj A₁ O₁]
    (f : M₁ ⟶ N₁) (g : N₁ ⟶ O₁) [IsAddModHom A₁ f] [IsAddModHom A₁ g] :
    IsAddModHom A₁ (f ≫ g) where

@[to_additive existing instIsAddModHomComp]
instance instIsModHomComp (f : M ⟶ N) (g : N ⟶ O) [IsModHom A f] [IsModHom A g] :
    IsModHom A (f ≫ g) where

@[to_additive instIsAddModHomIso]
...
open AddMonObj in
theorem assoc_flip : A ⊴ₗ δ ≫ δ = (αₗ A A M.X).inv ≫ σ ⊵ₗ M.X ≫ δ := by simp
...
taking a morphism without an `[IsAddModHom]` instance, as well as the relevant
equality to put such an instance. -/
@[simps!]
def Hom.mk' {M N : AddMod D A} (f : M.X ⟶ N.X)
    (vadd_hom : δ[M.X] ≫ f = A ⊴ₗ f ≫ δ[N.X] := by cat_disch) : Hom M N :=
  letI : IsAddModHom A f := ⟨vadd_hom⟩
  ⟨f⟩

/-- An alternative constructor for `Hom`,
taking a morphism without an `[IsAddModHom]` instance, between objects with
an `AddModObj` instance (rather than bundled as `AddMod`),
as well as the relevant equality to put such an instance. -/
@[simps!]
def Hom.mk'' {M N : D} [AddModObj A M] [AddModObj A N] (f : M ⟶ N)
    (vadd_hom : δ[M] ≫ f = A ⊴ₗ f ≫ δ[N] := by cat_disch) :
...
@[simps!]
def id (M : AddMod D A) : Hom M M where hom := 𝟙 M.X

instance homInhabited (M : AddMod D A) : Inhabited (Hom M M) :=
  ⟨id M⟩

/-- Composition of additive module object morphisms. -/
@[simps!]
def comp {M N O : AddMod D A} (f : Hom M N) (g : Hom N O) :
    Hom M O where
  hom := f.hom ≫ g.hom

instance : Category (AddMod D A) where
  Hom M N := Hom M N
  id := id
  comp f g := comp f g

@[ext]
lemma hom_ext {M N : AddMod D A} (f₁ f₂ : M ⟶ N) (h : f₁.hom = f₂.hom) :
    f₁ = f₂ :=
  Hom.ext h

@[simp]
theorem id_hom' (M : AddMod D A) : (𝟙 M : M ⟶ M).hom = 𝟙 M.X := by
  rfl

@[simp]
theorem comp_hom' {M N K : AddMod D A} (f : M ⟶ N) (g : N ⟶ K) :
    (f ≫ g).hom = f.hom ≫ g.hom :=
  rfl
...
@[simps]
def regular : AddMod C A :=
  letI : AddModObj A A := .regular A
  ⟨A⟩

instance : Inhabited (AddMod C A) :=
  ⟨regular A⟩

/-- The forgetful functor from additive module objects to the ambient category. -/
@[simps]
def forget : AddMod D A ⥤ D where
  obj A := A.X
  map f := f.hom

section comap

variable {A B : C} [AddMonObj A] [AddMonObj B] (f : A ⟶ B) [IsAddMonHom f]
...
def scalarRestriction (M : D) [AddModObj B M] : AddModObj A M where
  vadd := f ⊵ₗ M ≫ δ[B,M]
  zero_vadd := by
    rw [← comp_actionHomLeft_assoc]
    rw [IsAddMonHom.zero_hom, AddModObj.zero_vadd]
  add_vadd := by
    slice_rhs 2 3 => rw [action_exchange]
    simp only [actionHomLeft_action_assoc, Category.assoc, Iso.hom_inv_id_assoc,
      actionHomRight_comp]
    slice_rhs 4 6 => rw [AddModObj.assoc_flip]
    ...
    rw [← comp_actionHomLeft, Category.assoc, ← comp_actionHomLeft_assoc,
      IsAddMonHom.add_hom, tensorHom_def, Category.assoc]

open MonoidalLeftAction in
/-- If `g : M ⟶ N` is a `B`-linear morphism of `B`-additive modules, then it induces an
`A`-linear morphism when `M` and `N` have an `A`-additive module structure obtained
by restricting scalars along an additive monoid morphism `A ⟶ B`. -/
lemma scalarRestriction_hom
    (M N : D) [AddModObj B M] [AddModObj B N] (g : M ⟶ N) [IsAddModHom B g] :
  ...
  letI := scalarRestriction f N
  { vadd_hom := by
      simpa using (action_exchange_assoc f g δ).symm }
...
-/
@[simps]
def comap {A B : C} [AddMonObj A] [AddMonObj B] (f : A ⟶ B) [IsAddMonHom f] :
    AddMod D B ⥤ AddMod D A where
  obj M :=
...
@[simps!, implicit_reducible, to_additive existing]
