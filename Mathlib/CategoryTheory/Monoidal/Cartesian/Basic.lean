...
theorem prodComparison_comp {E : Type u₂} [Category.{v₂} E] [CartesianMonoidalCategory E] (G : D ⥤ E) :
...
lemma η_of_CartesianMonoidalCategory : η F = terminalComparison F := toUnit_unique ..

lemma δ_of_CartesianMonoidalCategory (X Y : C) : δ F X Y = prodComparison F X Y := by
  ext
...
lemma ε_of_CartesianMonoidalCategory : ε F = (preservesTerminalIso F).inv := by
  change (εIso F).symm.inv = _; congr; ext; simpa using η_of_CartesianMonoidalCategory F
...
instance [F.Monoidal] : PreservesFiniteProducts F :=
  have (A B) : IsIso (prodComparison F A B) := δ_of_CartesianMonoidalCategory F A B ▸ inferInstance
  have : IsIso (terminalComparison F) := η_of_CartesianMonoidalCategory F ▸ inferInstance
