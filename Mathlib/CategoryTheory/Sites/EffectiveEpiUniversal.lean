...
--     {π : (a : α) → X a ⟶ B} (c : Cofan X) (hc : IsUniversalColimit c)
--     [∀ {Z : C} (g : Z ⟶ c.pt) (a : α), HasPullback g (c.ι.app ⟨a⟩)]
--     {W : C} {e : (a : α) → X a ⟶ W} (h : ∀ {Z : C} (a₁ a₂ : α) (g₁ : Z ⟶ X a₁) (g₂ : Z ⟶ X a₂),
--       g₁ ≫ π a₁ = g₂ ≫ π a₂ → g₁ ≫ e a₁ = g₂ ≫ e a₂) {Z : C}
--     {g₁ g₂ : Z ⟶ c.pt}
--     (hg : g₁ ≫ (hc.isColimit.desc (Cofan.mk B π)) = g₂ ≫ (hc.isColimit.desc (Cofan.mk B π))) :
--     g₁ ≫ (hc.isColimit.desc (Cofan.mk _ e)) = g₂ ≫ (hc.isColimit.desc (Cofan.mk _ e)) := by
--   apply_fun ((hc.isColimit.desc (Cofan.mk _ (fun a ↦ pullback.fst (f := g₁) (g := (c.ι.app ⟨a⟩))))) ≫ ·) using
...
--     (π : (a : α) → (X a ⟶ B)) (c : Cofan X) (hc : IsUniversalColimit c)
--     [∀ {Z : C} (g : Z ⟶ c.pt) (a : α), HasPullback g (c.ι.app ⟨a⟩)]
--     [EffectiveEpi (hc.isColimit.desc (Cofan.mk B π))] :
--     EffectiveEpiFamilyStruct X π where
--   desc e h := by
--     refine EffectiveEpi.desc (hc.isColimit.desc (Cofan.mk B π)) (hc.isColimit.desc (Cofan.mk _ e))
--       fun g₁ g₂ hg ↦ effectiveEpiFamilyStructOfEffectiveEpiDesc_aux' c hc h hg
