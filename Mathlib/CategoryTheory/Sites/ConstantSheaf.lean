...
lemma constantCommuteCompose_hom_app_val (X : D) : ((constantCommuteCompose J U).hom.app X).hom =
...
lemma constantSheafAdj_counit_w {T : C} (hT : IsTerminal T) :
  ...
  rw [InducedCategory.comp_hom, constantCommuteCompose_hom_app_val, assoc, Iso.inv_comp_eq]
