...
instance : CompleteLattice (Sieve X) where
  ...
  isLUB_sSup_of_exists_isLUB _ _ :=
    ⟨fun S hS _ _ hf ↦ ⟨S, hS, hf⟩, fun _ ha _ _ ⟨b, hb, hf⟩ ↦ ha hb _ hf⟩
  isGLB_sInf_of_exists_isGLB _ _ :=
    ⟨fun S hS _ _ h ↦ h _ hS, fun _ hS _ _ hf _ hR ↦ hS hR _ hf⟩
  exists_isLUB _ := ⟨.sup _, fun S hS _ _ hf ↦ ⟨S, hS, hf⟩, fun _ ha _ _ ⟨b, hb, hf⟩ ↦ ha hb _ hf⟩
  exists_isGLB _ := ⟨.inf _, fun S hS _ _ h ↦ h _ hS, fun _ hS _ _ hf _ hR ↦ hS hR _ hf⟩
