...
lemma Sheaf.ΓObjEquivSections_naturality_symm [HasWeakSheafify J TypeCat.{w}]
    [HasGlobalSectionsFunctor J TypeCat.{w}] {F G : Sheaf J TypeCat.{w}} (f : F ⟶ G)
...
noncomputable def Sheaf.ΓNatIsoSectionsFunctor :
    Γ J (TypeCat.{max u v}) ≅ sheafToPresheaf J _ ⋙ Functor.sectionsFunctor _ :=
...
noncomputable def Sheaf.ΓObjEquivHom [HasWeakSheafify J TypeCat.{w}]
    [HasGlobalSectionsFunctor J TypeCat.{w}] (F : Sheaf J TypeCat.{w}) (X : TypeCat.{w})
    [Unique X] : (Γ J TypeCat.{w}).obj F ≃ ((constantSheaf J TypeCat.{w}).obj X ⟶ F) :=
  ((Equiv.funUnique X _).symm.trans TypeCat.homEquiv.symm).trans
    ((constantSheafΓAdj J TypeCat.{w}).homEquiv _ _).symm

set_option backward.isDefEq.respectTransparency false in
lemma Sheaf.ΓObjEquivHom_naturality [HasWeakSheafify J TypeCat.{w}]
    [HasGlobalSectionsFunctor J TypeCat.{w}] (X : TypeCat.{w}) [Unique X]
    {F G : Sheaf J TypeCat.{w}} (f : F ⟶ G) (x : (Γ J TypeCat.{w}).obj F) :
    (ΓObjEquivHom J G X) ((Γ J TypeCat.{w}).map f x) = (ΓObjEquivHom J F X) x ≫ f :=
  (constantSheafΓAdj J TypeCat.{w}).homEquiv_naturality_right_symm
    (TypeCat.ofHom ⟨uniqueElim x⟩) f

lemma Sheaf.ΓObjEquivHom_naturality_symm [HasWeakSheafify J TypeCat.{w}]
    [HasGlobalSectionsFunctor J TypeCat.{w}] {X : TypeCat.{w}} [Unique X]
    {F G : Sheaf J TypeCat.{w}} (f : F ⟶ G) (x : (constantSheaf J _).obj X ⟶ F) :
...
noncomputable def Sheaf.ΓNatIsoCoyoneda (X : TypeCat.{max u v}) [Unique X] :
    Γ J TypeCat.{max u v} ≅ coyoneda.obj (op ((constantSheaf J TypeCat.{max u v}).obj X)) :=
