...
lemma FamilyOfElements.Compatible.restrict {R₁ R₂ : Presieve X} (h : R₁ ≤ R₂)
...
lemma FamilyOfElements.Compatible.sieveExtend {x : FamilyOfElements P R} (hx : x.Compatible) :
...
lemma extend_agrees {x : FamilyOfElements P R} (t : x.Compatible) {f : Y ⟶ X} (hf : R f) :
...
@[simp]
lemma restrict_extend {x : FamilyOfElements P R} (t : x.Compatible) :
...
-/
@[simp]
lemma extend_restrict {x : FamilyOfElements P (generate R)} (t : x.Compatible) :
...
-/
lemma restrict_inj {x₁ x₂ : FamilyOfElements P (generate R)} (t₁ : x₁.Compatible)
...
C2.1.4 of [Elephant].
-/
lemma isSheafFor_iff_yonedaSheafCondition {P : Cᵒᵖ ⥤ Type v₁} :
...
-/
@[reassoc (attr := simp)]
lemma IsSheafFor.functorInclusion_comp_extend {P : Cᵒᵖ ⥤ Type v₁} (h : IsSheafFor P S)
...
lemma IsSheafFor.unique_extend {P : Cᵒᵖ ⥤ Type v₁} (h : IsSheafFor P S) {f : S.functor ⟶ P}
...
-/
lemma IsSheafFor.hom_ext {P : Cᵒᵖ ⥤ Type v₁} (h : IsSheafFor P (S : Presieve X))
...
lemma isSeparatedFor_and_exists_isAmalgamation_iff_isSheafFor :
...
-/
lemma IsSeparatedFor.isSheafFor (t : IsSeparatedFor P R) :
...
lemma IsSheafFor.isSeparatedFor : IsSheafFor P R → IsSeparatedFor P R := fun q =>
...
/-- C2.1.3 in [Elephant] -/
lemma isSheafFor_iff_generate (R : Presieve X) :
...
[Elephant] C2.1.5(i)
-/
lemma isSheafFor_singleton_iso (P : Cᵒᵖ ⥤ Type w) : IsSheafFor P (Presieve.singleton (𝟙 X)) := by
  intro x _
...
[Elephant] C2.1.5(ii)
-/
lemma isSheafFor_top_sieve (P : Cᵒᵖ ⥤ Type w) : IsSheafFor P ((⊤ : Sieve X) : Presieve X) := by
...
-/
lemma isSheafFor_iso {P' : Cᵒᵖ ⥤ Type w} (i : P ≅ P') : IsSheafFor P R → IsSheafFor P' R := by
  intro h x hx
...
-/
lemma isSheafFor_subsieve_aux (P : Cᵒᵖ ⥤ Type w) {S : Sieve X} {R : Presieve X}
...
-/
lemma isSheafFor_subsieve (P : Cᵒᵖ ⥤ Type w) {S : Sieve X} {R : Presieve X}
...
lemma IsSheaf.isSheafFor {P : Cᵒᵖ ⥤ Type w} (hp : IsSheaf J P) (R : Presieve X)
...
lemma isSheaf_of_le (P : Cᵒᵖ ⥤ Type w) {J₁ J₂ : GrothendieckTopology C} :
...
lemma isSeparated_of_isSheaf (P : Cᵒᵖ ⥤ Type w) (h : IsSheaf J P) : IsSeparated J P :=
...
presieves only.
-/
lemma isSheaf_pretopology [HasPullbacks C] (K : Pretopology C) :
...
lemma isSheaf_bot : IsSheaf (⊥ : GrothendieckTopology C) P := fun X => by
...
lemma yonedaFamily_fromCocone_compatible (S : Sieve X) (s : Cocone (diagram S.arrows)) :
...
-/
lemma forallYonedaIsSheaf_iff_colimit (S : Sieve X) :
...
-/
lemma compatible_iff (x : FirstObj P S) :
...
map it to the same point.
-/
lemma compatible_iff (x : FirstObj P R) :
...
-/
lemma sheaf_condition : R.IsSheafFor P ↔ Nonempty (IsLimit (Fork.ofι _ (w P R))) := by
  rw [Types.type_equalizer_iff_unique]
...
@[ext]
lemma Hom.ext' {X Y : SheafOfTypes J} (f g : X ⟶ Y) (w : f.val = g.val) : f = g :=
  Hom.ext f g w
