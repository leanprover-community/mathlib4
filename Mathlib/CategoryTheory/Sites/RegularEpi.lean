...
lemma isRegularEpiCategory_sheaf (J : GrothendieckTopology C)
  ...
  regularEpiOfEpi {F G} f _ := by
    ...
    have h₂ : f = inv ((sheafificationAdjunction J D).counit.app F) ≫
      ...
      simp [h₁]
    -- The sheafification of `f.hom` is still an epimorphism
    have : Epi ((presheafToSheaf J D).map f.hom) := by
      rw [h₁]
      infer_instance
    -- The sheafification of `i` is an epimorphism, because the sheafification of `p ≫ i = f.hom`
    -- is an epimorphism.
    ...
    exact ⟨⟨{
      ...
      w := by
        ...
        congr 1
        rw [← cancel_mono i]
        simp [hpi, ← InducedCategory.comp_hom, pullback.condition]
      isColimit := by
