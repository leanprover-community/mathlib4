...
instance (J : GrothendieckTopology C) [HasSheafify J TypeCat.{u}] :
    IsRegularEpiCategory (Sheaf J TypeCat.{u}) := isRegularEpiCategory_sheaf J fun f hf ↦
...
example {C : Type u} [Category.{v} C] (J : GrothendieckTopology C) :
    IsRegularEpiCategory (Sheaf J TypeCat.{max u v}) :=
  inferInstance
