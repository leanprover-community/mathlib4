...
theorem getSection_isAmalgamation (Y : StructuredArrow (op U) G.op) :
  ...
  IsSheafFor.isAmalgamation
    -- porting note: in Lean 3 these next two inputs were just `_ _` and they were
    -- coming from `get_section._proof_5` and `get_section._proof_6`. For some reason
    -- in Lean 4 we have to explicitly give the terms (thanks Jujian!) and it's not clear
    -- to me that the relevant proofs are being generated by Lean 4's `definition` mechanism
    -- (I couldn't find them with `whatsnew in`)
    (ℱ.2 X _ (hu.cover_lift (K.pullback_stable Y.hom.unop hS)))
    (((hx.pullback Y.3.unop).functorPullback G).compPresheafMap $ _)
set_option linter.uppercaseLean3 false in
