...
class HasCoherentCoverage : Prop where
  condition {B₁ B₂ : C} (f : B₂ ⟶ B₁) :
...
def CoherentCoverage [HasCoherentCoverage C] : Coverage C where
  ...
  pullback := by
    ...
    obtain ⟨β,_,X₂,π₂,h,i,ι,hh⟩ := HasCoherentCoverage.condition f α X₁ π₁ hS
    refine ⟨Presieve.ofArrows X₂ π₂, ⟨β, inferInstance, X₂, π₂, rfl, h⟩, ?_⟩
    rintro _ _ ⟨b⟩
    refine ⟨(X₁ (i b)), ι _, π₁ _, ⟨_⟩, hh _⟩

def CoherentTopology [HasCoherentCoverage C] : GrothendieckTopology C :=
  Coverage.toGrothendieck _ <| CoherentCoverage C

lemma isSheaf_coherent [HasCoherentCoverage C] (P : Cᵒᵖ ⥤ Type w) :
