...
theorem naturality_apply [G.IsLocallyFull K] {X Y : C} (i : G.obj X ⟶ G.obj Y) (x) :
    ℱ'.obj.map i.op (α.app _ x) = α.app _ (ℱ.map i.op x) := by
  have {X Y} (i : X ⟶ Y) (x) :
      ℱ'.obj.map (G.map i).op (α.app _ x) = α.app _ (ℱ.map (G.map i).op x) := by
...
theorem naturality [G.IsLocallyFull K] {X Y : C} (i : G.obj X ⟶ G.obj Y) :
    α.app _ ≫ ℱ'.obj.map i.op = ℱ.map i.op ≫ α.app _ := types_ext _ _ (naturality_apply α i)
...
theorem pushforwardFamily_compatible {X} (x : ℱ.obj (op X)) :
  ...
  suffices ∀ {Z W₁ W₂} (iWX₁ : G.obj W₁ ⟶ X) (iWX₂ : G.obj W₂ ⟶ X) (iZW₁ : Z ⟶ G.obj W₁)
      ...
      ℱ'.obj.map iZW₁.op (α.app _ (ℱ.map iWX₁.op x)) =
        ℱ'.obj.map iZW₂.op (α.app _ (ℱ.map iWX₂.op x)) by
...
instance full_sheafPushforwardContinuous [G.IsContinuous J K] :
  ...
  map_surjective α := ⟨ObjectProperty.homMk (sheafHom α.hom),
    InducedCategory.hom_ext <| sheafHom_restrict_eq α.hom⟩
...
`F.obj.obj (op Y) ⟶ F.obj.obj (op X)` induced by a morphism
...
lemma mapPreimage_map_of_fac {X Y Z : C} (f : G.obj X ⟶ G.obj Y)
  ...
  Presheaf.IsSheaf.hom_ext F.property
    ⟨_, J.pullback_stable p (imageSieve_mem J K G f)⟩ _ _ (by
      ...
      rw [F.property.amalgamate_map ⟨_, imageSieve_mem J K G f⟩
