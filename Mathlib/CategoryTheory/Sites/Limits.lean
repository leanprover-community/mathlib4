...
def multiforkEvaluationCone (F : K ⥤ Sheaf J D) (E : Cone (F ⋙ sheafToPresheaf J D)) (X : C)
  ...
  pt := S.pt
  π :=
    { app := fun k => (Presheaf.isLimitOfIsSheaf J (F.obj k).obj W (F.obj k).property).lift <|
      ...
      naturality := by
        ...
        rw [Category.id_comp]
        apply Presheaf.IsSheaf.hom_ext (F.obj j).property W
        intro ii
...
def isLimitMultiforkOfIsLimit (F : K ⥤ Sheaf J D) (E : Cone (F ⋙ sheafToPresheaf J D))
  ...
  Multifork.IsLimit.mk _
    ...
    (by
      ...
      dsimp
      erw [(isLimitOfPreserves ((evaluation Cᵒᵖ D).obj (op X)) hE).fac]
      apply Presheaf.IsSheaf.hom_ext (F.obj k).property W
      intro i
...
instance (F : K ⥤ Sheaf J D) : CreatesLimit F (sheafToPresheaf J D) :=
  createsLimitOfReflectsIso fun E hE =>
    { liftedCone := ⟨⟨E.pt, isSheaf_of_isLimit _ _ hE⟩,
        ⟨fun _ => ObjectProperty.homMk (E.π.app _),
          fun _ _ _ => InducedCategory.hom_ext <| E.π.naturality _⟩⟩
      validLift := Cones.ext (eqToIso rfl) fun j => by simp
      makesLimit :=
        { lift := fun S => ObjectProperty.homMk (hE.lift ((sheafToPresheaf J D).mapCone S))
          fac := fun S j => by
            ext1
            apply hE.fac ((sheafToPresheaf J D).mapCone S) j
          uniq := fun S m hm => by
            ext1
            exact hE.uniq ((sheafToPresheaf J D).mapCone S) m.hom fun j =>
              congr_arg InducedCategory.Hom.hom (hm j) } }
...
lemma sheafifyCocone_ι_app_val
  ...
  rw [← InducedCategory.comp_hom_assoc, ← NatTrans.comp_app, IsIso.hom_inv_id, NatTrans.id_app]
  dsimp
...
def createsColimitOfIsSheaf (F : K ⥤ Sheaf J D)
  ...
  createsColimitOfReflectsIso fun E hE =>
    { liftedCocone := ⟨⟨E.pt, h _ hE⟩,
        ⟨fun _ => ObjectProperty.homMk (E.ι.app _),
          fun _ _ _ => InducedCategory.hom_ext <| E.ι.naturality _⟩⟩
      validLift := Cocones.ext (eqToIso rfl) fun j => by simp
      makesColimit :=
        { desc := fun S => ObjectProperty.homMk (hE.desc ((sheafToPresheaf J D).mapCocone S))
