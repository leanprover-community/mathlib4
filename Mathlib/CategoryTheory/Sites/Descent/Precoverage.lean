...
lemma isEquivalence_toDescentData_of_sieve_le
  ...
  obtain ⟨H, h, ⟨e⟩⟩ :
    ...
    · refine F.fullyFaithfulPullFunctor hp h₁
  ...
  isSheaf {S} M N :=
    (isSheaf_iff_isSheaf_of_type ..).2 (by
      rw [over_toGrothendieck_eq_toGrothendieck_comap_forget,
        ← Precoverage.toGrothendieck_toPretopology,
      ...
      obtain ⟨ι, Z, f, hf⟩ := Presieve.exists_eq_ofArrows R'.arrows
      have := (bijective_toDescentData_map_iff' (M := M) (N := N) (p := X.hom) f).1 (by
        ...
        refine (nonempty_fullyFaithful_toDescentData_iff_of_sieve_eq _ _ _ ?_).1 ⟨hF _ _ hR⟩
        refine le_antisymm ?_ ?_
        · simp only [Sieve.generate_le_iff]
          rintro _ _ ⟨⟨W, hW⟩⟩
          obtain ⟨W, g, rfl⟩ := W.mk_surjective
          obtain ⟨W', hW, b, hb, fac⟩ := hW.exists
          obtain ⟨W', g', rfl⟩ := W'.mk_surjective
          obtain rfl : W = W' := hW
        ...
        · simp only [Sieve.generate_le_iff]
          rintro _ _ ⟨i⟩
          let g : Over.mk (f i ≫ X.hom) ⟶ X := Over.homMk (f i)
          have hg : Sieve.generate R g := by
            simp only [← hR', Functor.const_obj_obj, Sieve.overEquiv_symm_iff, Over.mk_left, hf]
            exact ⟨i⟩
          obtain ⟨T, p, q, hq, fac⟩ := hg
          replace fac := (Over.forget _).congr_map fac
          let a : (Presieve.map (Over.forget S) R).category := ⟨Over.mk q.left, ⟨hq⟩⟩
          exact ⟨_, p.left, _, ⟨a⟩, fac⟩)
      ...
      · rintro W g hg
        ...
        obtain ⟨W, p, rfl⟩ := W.mk_surjective
        obtain ⟨g, hg', rfl⟩ := Over.homMk_surjective g
        rw [hf] at hg
        dsimp at hg
        obtain ⟨i⟩ := hg
        exact ⟨Over.mk (f i ≫ X.hom), Over.homMk (𝟙 _), _, ⟨i⟩, by cat_disch⟩
...
lemma IsStack.of_precoverage
  ...
  have : F.IsPrestack J.toGrothendieck := .of_precoverage (fun S R hR ↦ by
    have := hF S R hR
    exact .ofFullyFaithful _)
  rw [← J.toGrothendieck_toPretopology] at this ⊢
  constructor
  rintro S R ⟨R', hR', h⟩
  have := hF S R' hR'
  have h₁ : Sieve.ofArrows _ (fun (f : R'.category) ↦ f.obj.hom) ∈
      J.toPretopology.toGrothendieck _ :=
    ⟨_, hR', fun _ g hg ↦ ⟨_, 𝟙 _, g, .mk (ι := R'.category) ⟨Over.mk g, hg⟩, by simp⟩⟩
  have h₂ : Sieve.ofArrows _ (fun (f : R'.category) ↦ f.obj.hom) ≤
    Sieve.ofArrows _ (fun (f : R.arrows.category) ↦ f.obj.hom) := by
