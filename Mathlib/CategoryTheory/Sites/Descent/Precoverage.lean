...
this is the sieve of `Over.mk (𝟙 (X i)) : Over (X i)` which corresponds
...
lemma IsStackFor.of_le {S : C} {R : Presieve S}
  ...
  exact isEquivalence_toDescentData_of_sieve_le _ hR₂ (Sieve.generate_mono h)
...
lemma IsStack.of_precoverage
  ...
  rw [← J.toGrothendieck_toPretopology] at hR
  obtain ⟨R', hR', h⟩ := hR
  dsimp at hR'
