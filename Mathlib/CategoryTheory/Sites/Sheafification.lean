...
-/
noncomputable
def constantSheaf : D ⥤ Sheaf J D := Functor.const Cᵒᵖ ⋙ (presheafToSheaf J D)

/--
The functor which maps a `D`-valued presheaf on `C` to its evaluation at the terminal object of `C`.
-/
@[simps]
noncomputable
def presheafToUnderlying [HasTerminal C] : (Cᵒᵖ ⥤ D) ⥤ D where
  obj X := X.obj (op (⊤_ C))
  map f := f.app _

/--
An auxiliary definition, the counit in the presheaf part of the adjunction `constantSheaf.adj`
below.
-/
@[simps]
noncomputable
def constantSheaf.adj_aux_couinit [HasTerminal C] :
    presheafToUnderlying D ⋙ Functor.const Cᵒᵖ ⟶ 𝟭 _ where
  app F := {
    app := fun ⟨X⟩ => F.map (IsTerminal.from terminalIsTerminal X).op
...
/--
The functor which maps a `D`-valued sheaf on `C` to its evaluation at the terminal object of `C`.
-/
@[simps]
noncomputable
def sheafToUnderlying [HasTerminal C] : Sheaf J D ⥤ D where
  obj X := X.val.obj (op (⊤_ C))
  map f := f.val.app _

/--
The constant sheaf functor is left adjoint to evaluation at the terminal object.
-/
noncomputable
def constantSheaf.adj [HasTerminal C] : constantSheaf J D ⊣ sheafToUnderlying J D := by
  change (Functor.const Cᵒᵖ ⋙ (presheafToSheaf J D)) ⊣
    (sheafToPresheaf J D ⋙ presheafToUnderlying (C := C) D)
  exact Adjunction.comp (Adjunction.mkOfUnitCounit ⟨(Functor.constCompEvaluationObj _
    (op (⊤_ C))).hom, adj_aux_couinit D, by aesop, by aesop⟩) (sheafificationAdjunction J D)
