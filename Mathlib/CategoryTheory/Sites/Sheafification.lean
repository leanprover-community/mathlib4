...
theorem sheafifyLift_unique {P Q : Cᵒᵖ ⥤ D} (η : P ⟶ Q) (hQ : Presheaf.IsSheaf J Q)
  ...
  rw [toSheafify] at h
  rw [sheafifyLift]
  let γ' : (presheafToSheaf J D).obj P ⟶ ⟨Q, hQ⟩ := ObjectProperty.homMk γ
  change γ'.hom = _
  rw [← InducedCategory.Hom.ext_iff, ← Adjunction.homEquiv_apply_eq, Adjunction.homEquiv_unit]
  exact h
...
noncomputable def sheafificationIso (P : Sheaf J D) : P ≅ (presheafToSheaf J D).obj P.obj where
  hom := ObjectProperty.homMk (isoSheafify J P.property).hom
  inv := ObjectProperty.homMk (isoSheafify J P.property).inv
  hom_inv_id := by
    ext1
    apply (isoSheafify J P.property).hom_inv_id
  inv_hom_id := by
    ext1
    apply (isoSheafify J P.property).inv_hom_id
