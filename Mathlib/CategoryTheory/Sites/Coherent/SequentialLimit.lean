...
private structure struct (F : ℕᵒᵖ ⥤ Sheaf (coherentTopology C) (Type v)) where
  ...
  w (n : ℕ) : (F.map (homOfLE (n.le_add_right 1)).op).hom.app (op (X (n + 1))) (x (n + 1)) =
      (F.obj (op n)).obj.map (map n).op (x n)

include hF in
private lemma exists_effectiveEpi (n : ℕ) (X : C) (y : (F.obj ⟨n⟩).obj.obj ⟨X⟩) :
    ∃ (X' : C) (φ : X' ⟶ X) (_ : EffectiveEpi φ) (x : (F.obj ⟨n + 1⟩).obj.obj ⟨X'⟩),
...
private noncomputable def preimageDiagram (X : C) (y : (F.obj ⟨0⟩).obj.obj ⟨X⟩) : ℕᵒᵖ ⥤ C :=
...
lemma isLocallySurjective_π_app_zero_of_isLocallySurjective_map :
  ...
  simp only [← (coherentTopology C).yonedaEquiv_comp, cone, IsLimit.fac, NatTrans.ofOpSequence_app,
    (coherentTopology C).yonedaEquiv_comp, (coherentTopology C).yonedaEquiv_yoneda_map]
  rfl
