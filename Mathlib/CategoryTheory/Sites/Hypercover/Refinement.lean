...
noncomputable
def interOver [IsStableUnderBaseChange.{w} J] [IsStableUnderComposition.{w, w} J] {X Y S : C}
    (E : ZeroHypercover.{w} J X) (F : ZeroHypercover.{w'} J Y) (f : X ⟶ S) (g : Y ⟶ S)
    [Limits.HasPullbacks C] :
    ZeroHypercover J (pullback f g) where
  __ := E.toPreZeroHypercover.interOver F.toPreZeroHypercover f g
  mem₀ := sorry
...
variable {J}
variable [HasPullbacks C] {E F G : OneHypercover.{w} J S}
  [HasMultiequalizer (E.multicospanIndex P)]
...
include f g in
noncomputable
abbrev refinePullback (i : E.I₀) (j : F.I₀) :
    J.toPrecoverage.ZeroHypercover (Limits.pullback (E.f i) (F.f j)) :=
  letI K := J.toPrecoverage
  letI W := (G.pullback₁ (pullback.fst (E.f i) (F.f j) ≫ E.f i)).toZeroHypercover
  letI E' (k : G.I₀) : K.ZeroHypercover (Limits.pullback (E.f i) (G.f k)) :=
  ...
  letI F' (k : G.I₀) : K.ZeroHypercover (Limits.pullback (F.f j) (G.f k)) :=
    ((F.cover₀ j (g.s₀ k)).pullback₁ (pullback.fst (pullback.snd _ _) (g.h₀ k))).pushforwardIso
      (pullbackLeftPullbackSndIso _ _ _ ≪≫ pullback.congrHom rfl (by simp))
  letI A (k : G.I₀) : K.ZeroHypercover (W.X k) :=
...
noncomputable
def refinePullbackHom {i : E.I₀} {j : F.I₀} (l : (refinePullback f g i j).I₀) :
      (refinePullback f g i j).X l ⟶ Limits.pullback (pullback.fst (E.f i) (F.f j) ≫ E.f i) (G.f l.fst) := by
  ...
  · simp [refinePullback]
    ...
    congr 1
    simp only [Precoverage.ZeroHypercover.pushforwardIso_toPreZeroHypercover,
      Precoverage.ZeroHypercover.pullback₁_toPreZeroHypercover, cover₀_toPreZeroHypercover,
      PreZeroHypercover.pushforwardIso_I₀, PreZeroHypercover.pullback₁_I₀, PreOneHypercover.cover₀_I₀,
      PreZeroHypercover.pushforwardIso_X, PreZeroHypercover.pullback₁_X, PreOneHypercover.cover₀_X,
      PreOneHypercover.cover₀_f, PreZeroHypercover.pushforwardIso_f, PreZeroHypercover.pullback₁_f, Iso.trans_hom,
      pullback.congrHom_hom, Category.assoc, limit.lift_π, PullbackCone.mk_pt, PullbackCone.mk_π_app, Category.comp_id,
      pullbackLeftPullbackSndIso_hom_snd, PreZeroHypercover.Hom.w₀, F']
    congr 1
    rw [pullback.condition, pullback.condition_assoc]
    simp
  · simp only [cover₀_toPreZeroHypercover, PreOneHypercover.cover₀_X, PreOneHypercover.cover₀_f,
      limit.lift_π_assoc, PullbackCone.mk_pt, cospan_left, PullbackCone.mk_π_app, Category.assoc]
    congr 2
    rw [pullback.condition, pullback.condition_assoc]
    simp
-/
...
lemma exists_zeroHypercover_of_mapMultifork_eq [HasPullbacks C] (E F G : OneHypercover.{w} J S)
  ...
  let K := J.toPrecoverage
  let W := (G.pullback₁ (pullback.fst (E.f i) (F.f j) ≫ E.f i)).toZeroHypercover
  let E' (k : G.I₀) : K.ZeroHypercover (Limits.pullback (E.f i) (G.f k)) :=
    ((E.cover₀ i (f.s₀ k)).pullback₁ (pullback.fst (pullback.snd _ _) (f.h₀ k))).pushforwardIso
      (pullbackLeftPullbackSndIso _ _ _ ≪≫ pullback.congrHom rfl (by simp))
  let F' (k : G.I₀) : K.ZeroHypercover (Limits.pullback (F.f j) (G.f k)) :=
    ((F.cover₀ j (g.s₀ k)).pullback₁ (pullback.fst (pullback.snd _ _) (g.h₀ k))).pushforwardIso
      (pullbackLeftPullbackSndIso _ _ _ ≪≫ pullback.congrHom rfl (by simp))
  let A (k : G.I₀) : K.ZeroHypercover (W.X k) :=
    ((E' k).interOver (F' k) (pullback.snd _ _) (pullback.snd _ _)).pushforwardIso <|
      pullbackLeftPullbackSndIso _ _ _ ≪≫
        pullback.congrHom rfl pullback.condition.symm ≪≫
        (pullbackAssoc _ _ _ _).symm ≪≫
        pullback.congrHom pullback.condition.symm rfl
  let W' : K.ZeroHypercover (Limits.pullback (E.f i) (F.f j)) := W.bind A
  refine ⟨W', fun l ↦ ?_⟩
  ...
  · calc
    ...
    · rw [pullbackMultiequalizer_ι_assoc]
      ...
      have hl : W'.f l ≫ pullback.fst (E.f i) (F.f j) =
        ...
        obtain ⟨a, b⟩ := l
        simp only [toZeroHypercover_toPreZeroHypercover, pullback₁_toPreOneHypercover,
          PreOneHypercover.pullback₁_toPreZeroHypercover, PreZeroHypercover.pullback₁_X,
          Precoverage.ZeroHypercover.bind_toPreZeroHypercover, PreZeroHypercover.pullback₁_I₀,
          Precoverage.ZeroHypercover.pushforwardIso_toPreZeroHypercover,
          Precoverage.ZeroHypercover.interOver_toPreZeroHypercover,
          PreZeroHypercover.pushforwardIso_I₀, PreZeroHypercover.interOver_I₀,
          PreZeroHypercover.bind_X, PreZeroHypercover.pushforwardIso_X,
          PreZeroHypercover.interOver_X, PreZeroHypercover.bind_f,
          PreZeroHypercover.pushforwardIso_f, PreZeroHypercover.interOver_f, Iso.trans_hom,
          pullback.congrHom_hom, Iso.symm_hom, PreZeroHypercover.pullback₁_f, Category.assoc,
          ...
          cospan_left, cover₀_toPreZeroHypercover, PreOneHypercover.cover₀_f, W', W, A]
        congr 1
        simp only [Precoverage.ZeroHypercover.pushforwardIso_toPreZeroHypercover,
          Precoverage.ZeroHypercover.pullback₁_toPreZeroHypercover, cover₀_toPreZeroHypercover,
          PreZeroHypercover.pushforwardIso_I₀, PreZeroHypercover.pullback₁_I₀,
      ...
      have hr : pW' ≫ pullback.snd (pullback.fst (E.f i) (F.f j) ≫ E.f i) (G.f l.fst) ≫
        ...
        simp only [cover₀_toPreZeroHypercover, PreOneHypercover.cover₀_X, PreOneHypercover.cover₀_f,
          limit.lift_π_assoc, PullbackCone.mk_pt, cospan_right, PullbackCone.mk_π_app,
          Category.assoc, pW']
        congr 1
        rw [← pullback.condition]
        rw [pullback.condition_assoc]
    ...
    · rw [pullbackMultiequalizer_ι_assoc]
      ...
      have hl : W'.f l ≫ pullback.snd (E.f i) (F.f j) =
        ...
        simp only [toZeroHypercover_toPreZeroHypercover, pullback₁_toPreOneHypercover,
          ...
          pullbackLeftPullbackSndIso_hom_snd_assoc, cover₀_toPreZeroHypercover,
          PreOneHypercover.cover₀_f, W', W, A]
        congr 1
        simp only [Precoverage.ZeroHypercover.pushforwardIso_toPreZeroHypercover,
          Precoverage.ZeroHypercover.pullback₁_toPreZeroHypercover, cover₀_toPreZeroHypercover,
          PreZeroHypercover.pushforwardIso_I₀, PreZeroHypercover.pullback₁_I₀,
      ...
      simp_rw [Category.assoc, hl, hr, op_comp, Functor.map_comp, Category.assoc]
      rw [PreOneHypercover.multiequalizer_condition_assoc]
  · simp only [cover₀_toPreZeroHypercover, PreOneHypercover.cover₀_X, PreOneHypercover.cover₀_f, Category.assoc]
    rw [pullback.condition]
    have : (E' l.fst).f l.snd.1 ≫ pullback.snd (E.f i) (G.f l.fst) =
        pullback.fst (pullback.fst (pullback.snd (E.f i) (E.f (f.s₀ l.fst))) (f.h₀ l.fst))
          (E.toPullback l.snd.1) ≫
