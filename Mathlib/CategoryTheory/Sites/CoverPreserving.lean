...
/-- If `C` is generated by `1`-hypercovers in universe `w` and `F : C ⥤ D` preserves
`1`-hypercovers in universe `w`, then `F` is cover preserving. -/
lemma CoverPreserving.of_preservesOneHypercovers {C : Type u₁} [Category.{v₁} C] {D : Type u₂}
    [Category.{v₂} D]
