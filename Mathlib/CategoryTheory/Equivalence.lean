...
lemma asEquivalence_inverse (F : C ⥤ D) [IsEquivalence F] : (asEquivalence F).inverse = F.inv := by
  rfl
...
isomorphism to the original `e.inverse`. -/
noncomputable def Equivalence.asEquivalenceInverse (e : C ≌ D) :
  ...
  · intros Y Y' g
    ...
    by
      simp [asEquivalence_inverse, Functor.inv_map, ← map_comp]
      congr 1
      rw [map_preimage F]
      aesop_cat
    rw [this]
