...
zero := by
  simp only
  rw [← shiftFunctorComm_eq, shiftFunctorComm_eq_shift₂FunctorComm]

  -- Functor.CommShift.isoZero]
  --change shiftFunctorComm C 0 n = _
/-  change shiftFunctorComm C (0, 0) (0, n) =
    isoWhiskerRight (shiftFunctorZero C (ℤ × ℤ)) (shiftFunctor C (n, (0 : ℤ))) ≪≫
    (shiftFunctor C (n, (0 : ℤ))).leftUnitor ≪≫
      (shiftFunctor C (n, (0 : ℤ))).rightUnitor.symm ≪≫
      isoWhiskerLeft (shiftFunctor C (n. (0 : ℤ)))
      (shiftFunctorZero C (ℤ × ℤ)).symm-/
  ext A
  simp only [Functor.comp_obj, Functor.CommShift.isoZero_hom_app]
