...
lemma zero {X Y : C} (f : X ⟶ Y) (n₀ n₁ : ℤ) (h : n₀ < n₁)
  ...
  apply t.zero'
  · change (t.le 0).shift n₀ X
    rw [t.shift_le n₀ 0 n₀ (add_zero n₀)]
    exact t.le_of_isLE X n₀
  · change (t.ge 1).shift n₀ Y
    rw [t.shift_ge n₀ 1 (n₀ + 1) rfl]
    apply t.ge_antitone (a := n₀ + 1) (b := n₁) (Order.succ_le_of_lt h)
    exact t.ge_of_isGE Y n₁
