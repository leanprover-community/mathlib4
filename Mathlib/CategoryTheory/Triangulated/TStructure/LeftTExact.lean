...
variable {H₁ H₂} in
def homologyLeftTExact (n : ℕ) : H₁ ⥤ H₂ := t₁.ιHeart (H := H₁) ⋙ F ⋙ t₂.homology (H := H₂) n

instance (n : ℕ) : (F.homologyLeftTExact t₁ t₂ (H₁ := H₁) (H₂ := H₂) n).Additive := by
  dsimp [homologyLeftTExact]
  infer_instance
...
variable {H₁ H₂} in
noncomputable def homologyLeftTExactδ :
    (F.homologyLeftTExact t₁ t₂ (H₂ := H₂) n₀).obj S.X₃ ⟶
      (F.homologyLeftTExact t₁ t₂ n₁).obj S.X₁ :=
  t₂.homologyδ (H := H₂) (F.mapTriangle.obj (t₁.heartShortExactTriangle S hS)) n₀ n₁
    (by simp [← hn₁])

variable {H₁ H₂} in
@[reassoc (attr := simp)]
lemma homologyLeftTExactδ_comp :
    F.homologyLeftTExactδ t₁ t₂ (H₁ := H₁) (H₂ := H₂) S hS n₀ n₁ hn₁ ≫
      (F.homologyLeftTExact t₁ t₂ n₁).map S.f = 0 :=
  t₂.homologyδ_comp _ (F.map_distinguished _ (t₁.heartShortExactTriangle_distinguished S hS)) _ _ _

variable {H₁ H₂} in
@[reassoc (attr := simp)]
lemma homologyLeftTExact_comp_δ :
     (F.homologyLeftTExact t₁ t₂ n₀).map S.g ≫
      F.homologyLeftTExactδ t₁ t₂ (H₂ := H₂) S hS n₀ n₁ hn₁ = 0 :=
  t₂.comp_homologyδ _ (F.map_distinguished _ (t₁.heartShortExactTriangle_distinguished S hS)) _ _ _

variable {H₁ H₂} in
lemma homologyLeftTExact_exact₁ :
    (ShortComplex.mk _ _ (F.homologyLeftTExactδ_comp t₁ t₂ (H₂ := H₂) S hS n₀ n₁ hn₁)).Exact :=
  t₂.homology_exact₁ _ (F.map_distinguished _ (t₁.heartShortExactTriangle_distinguished S hS)) _ _ _

include hS in
variable {H₁ H₂} in
lemma homologyLeftTExact_exact₂ (n : ℕ) :
    (S.map (F.homologyLeftTExact t₁ t₂ (H₂ := H₂) n)).Exact :=
  t₂.homology_exact₂ _ (F.map_distinguished _ (t₁.heartShortExactTriangle_distinguished S hS)) _

variable {H₁ H₂} in
lemma homologyLeftTExact_exact₃ :
    (ShortComplex.mk _ _ (F.homologyLeftTExact_comp_δ t₁ t₂ (H₂ := H₂) S hS n₀ n₁ hn₁)).Exact :=
  t₂.homology_exact₃ _ (F.map_distinguished _ (t₁.heartShortExactTriangle_distinguished S hS)) _ _ _

variable [Functor.LeftTExact F t₁ t₂]
...
instance :
    (F.homologyLeftTExact t₁ t₂ (H₁ := H₁) (H₂ := H₂) 0).PreservesMonomorphisms where
...
set_option backward.defeqAttrib.useBackward true in
variable {H₁ H₂} in
lemma homologyLeftTExact₀_map_exact (h : S.Exact) [hf : Mono S.f] :
    (S.map (F.homologyLeftTExact t₁ t₂ (H₁ := H₁) (H₂ := H₂) 0)).Exact := by
  ...
  let ψ := (F.homologyLeftTExact t₁ t₂ (H₁ := H₁) (H₂ := H₂) 0).mapShortComplex.map φ
  ...
  exact F.homologyLeftTExact_exact₂ t₁ t₂ S' hS' 0
