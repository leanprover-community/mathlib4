...
import Mathlib.CategoryTheory.Shift.Adjoint
import Mathlib.CategoryTheory.Triangulated.Yoneda

/-!
# The (left or right) adjoint of a triangulated functor is triangulated
...
lemma isTriangulated_of_left_adjoint_triangulated_aux (adj : F ⊣ G)
    [adj.compatCommShift ℤ] [F.IsTriangulated] (T : Triangle D)
    (dT : T ∈ distinguishedTriangles) (X : C) :
    (homologySequenceComposableArrows₅ (preadditiveCoyoneda.obj (Opposite.op X))
    (G.mapTriangle.obj T) 0 1 (by simp)).Exact := by
  apply Exact.exact_of_comp_exact (AddCommGrp.uliftFunctor.{v₁, max v₁ v₂})
  set e : homologySequenceComposableArrows₅ (preadditiveCoyoneda.obj (Opposite.op (F.obj X)))
      T 0 1 (by simp) ⋙ AddCommGrp.uliftFunctor.{v₂, max v₁ v₂} ≅ homologySequenceComposableArrows₅
      (preadditiveCoyoneda.obj (Opposite.op X)) (G.mapTriangle.obj T) 0 1 (by simp) ⋙
    ...
    refine ?_ ≪≫ (@homologySequenceComposableArrows₅_comp_iso _ _ _ _ _
      (G ⋙ preadditiveCoyoneda.obj (Opposite.op X)) (ShiftSequence.comp_left _ ℤ G)
      T 0 1 (by simp) _ _ _).symm
    apply @homologySequenceComposableArrows₅_iso_of_natIso _ _ _ _ _
      (preadditiveCoyoneda.obj (Opposite.op (F.obj X)) ⋙ AddCommGrp.uliftFunctor)
      (ShiftSequence.comp_right _ _ _) T 0 1 (by simp) ((G ⋙ preadditiveCoyoneda.obj
      (Opposite.op X)) ⋙ AddCommGrp.uliftFunctor) (@ShiftSequence.comp_right _ _ _ _
      (G ⋙ preadditiveCoyoneda.obj (Opposite.op X)) ℤ _ _ (ShiftSequence.comp_left _ _ _) _ _ _)
    refine @ShiftSequenceIso.mk  _ _ _ _
      (preadditiveCoyoneda.obj (Opposite.op (F.obj X)) ⋙ AddCommGrp.uliftFunctor.{v₂, max v₁ v₂})
      ℤ _ _ ((G ⋙ preadditiveCoyoneda.obj (Opposite.op X)) ⋙ AddCommGrp.uliftFunctor)
      (ShiftSequence.comp_right _ _ _) (@ShiftSequence.comp_right _ _ _ _ (G ⋙
      preadditiveCoyoneda.obj (Opposite.op X)) ℤ _ _ (ShiftSequence.comp_left _ _ _) _ _ _) ?_ ?_
    · intro n
      change (shiftFunctor D n ⋙ preadditiveCoyoneda.obj (Opposite.op (F.obj X))) ⋙
        AddCommGrp.uliftFunctor ≅ (G ⋙ (shiftFunctor C n ⋙ preadditiveCoyoneda.obj
        (Opposite.op X))) ⋙ AddCommGrp.uliftFunctor
      refine NatIso.ofComponents (fun Y ↦ ?_) ?_
      · refine adj.homAddEquiv_of_left_adjoint_additive_ulift X (Y⟦n⟧) ≪≫ ?_
        change AddCommGrp.uliftFunctor.obj ((preadditiveCoyoneda.obj (Opposite.op X)).obj _) ≅
          AddCommGrp.uliftFunctor.obj ((preadditiveCoyoneda.obj (Opposite.op X)).obj _)
        apply AddCommGrp.uliftFunctor.mapIso
        apply (preadditiveCoyoneda.obj (Opposite.op X)).mapIso
  ...
  exact (homologySequenceComposableArrows₅_exact (preadditiveCoyoneda.obj (Opposite.op (F.obj X))) _
    dT 0 1 (by simp)).comp_exact _

def isTriangulated_of_left_adjoint_triangulated (adj : F ⊣ G) [adj.compatCommShift ℤ]
  ...
  have hφ₂ : h' = φ ≫ G.map T.mor₃ ≫ (G.commShiftIso 1).hom.app T.obj₁ := by
    ...
    erw [compatCommShift_right_triangle, comp_id]
  have hφ : IsIso φ := by
    rw [isIso_iff_isIso_yoneda_map]
    intro X
    suffices h' : IsIso ((preadditiveCoyoneda.obj (Opposite.op X)).map φ) by
      have : ((yoneda.map φ).app (Opposite.op X)) = (coyoneda.obj (Opposite.op X)).map φ := by
        simp [yoneda, coyoneda]
      rw [this]
      have : (coyoneda.obj (Opposite.op X)).map φ = (forget AddCommGrp).map
        ((preadditiveCoyoneda.obj (Opposite.op X)).map φ) := by aesop
      rw [this]
      apply Functor.map_isIso
    suffices h'' : IsIso (((preadditiveCoyoneda.obj (Opposite.op X)).shift (0 : ℤ)).map φ) by
      have : (preadditiveCoyoneda.obj (Opposite.op X)).map φ =
          ((preadditiveCoyoneda.obj (Opposite.op X)).isoShiftZero ℤ).inv.app _ ≫
          ((preadditiveCoyoneda.obj (Opposite.op X)).shift (0 : ℤ)).map φ ≫
          ((preadditiveCoyoneda.obj (Opposite.op X)).isoShiftZero ℤ).hom.app _ := by
        rw [((preadditiveCoyoneda.obj (Opposite.op X)).isoShiftZero ℤ).hom.naturality, ← assoc,
    ...
    have hR₁ : R₁.Exact := (homologySequenceComposableArrows₅_exact (preadditiveCoyoneda.obj
      (Opposite.op X)) _ dT' 0 1 (by simp)).exact_truncation 4 (by linarith)
    set R₂ : ComposableArrows AddCommGrp 4 :=
      Monotone.functor (f := Fin.castLE (n := 4 + 1) (m := 5 + 1) (by simp)) (fun ⦃a b⦄ h ↦ h) ⋙
      homologySequenceComposableArrows₅ (preadditiveCoyoneda.obj (Opposite.op X))
    ...
    set Φ : R₁ ⟶ R₂ := by
      refine whiskerLeft (Monotone.functor (f := Fin.castLE (n := 4 + 1) (m := 5 + 1) (by simp))
        ...
        (Opposite.op X)).homologySequenceComposableArrows₅_map_of_triangle_map ?_ 0 1 (by simp))
