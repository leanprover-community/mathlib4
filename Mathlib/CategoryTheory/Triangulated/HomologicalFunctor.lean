...
lemma homologySequence_exact₃ :
  ...
  exact ShortComplex.isoMk (Iso.refl _) (Iso.refl _)
    ((F.shiftIso 1 n₀ n₁ (by grind)).app _) (by simp) (by simp [homologySequenceδ, shiftMap])
...
lemma homologySequence_exact₁ :
  ...
  refine ShortComplex.isoMk (-((F.shiftIso (-1) n₁ n₀ (by grind)).app _))
    (Iso.refl _) (Iso.refl _) ?_ (by simp)
  dsimp
  simp only [homologySequenceδ, neg_comp, map_neg, comp_id,
    F.shiftIso_hom_app_comp_shiftMap_of_add_eq_zero T.mor₃ (-1) (neg_add_cancel 1) n₀ n₁ (by grind)]
