...
def obj (P : Karoubi (HomologicalComplex C c)) : HomologicalComplex (Karoubi C) c where
  ...
  shape i j hij := by simp only [hom_eq_zero_iff, P.X.shape i j hij, Limits.comp_zero]; aesop_cat
                      -- ⊢ HomologicalComplex.Hom.f P.p i ≫ HomologicalComplex.d P.X i j = 0
                                                                                        -- 🎉 no goals
...
def unitIso : 𝟭 (Karoubi (HomologicalComplex C c)) ≅ functor ⋙ inverse where
    ...
    { app := fun P =>
        { f :=
            { f := fun n => P.p.f n
              comm' := fun i j _ => by
                dsimp
                -- ⊢ HomologicalComplex.Hom.f P.p i ≫ HomologicalComplex.Hom.f P.p i ≫ Homologica …
                simp only [HomologicalComplex.Hom.comm, HomologicalComplex.Hom.comm_assoc,
                  HomologicalComplex.p_idem] }
          comm := by
            ext n
            -- ⊢ HomologicalComplex.Hom.f (HomologicalComplex.Hom.mk fun n => HomologicalComp …
            dsimp
            -- ⊢ HomologicalComplex.Hom.f P.p n = HomologicalComplex.Hom.f P.p n ≫ Homologica …
            simp only [HomologicalComplex.p_idem] }
            -- 🎉 no goals
      naturality := fun P Q φ => by
        ext
        -- ⊢ HomologicalComplex.Hom.f ((𝟭 (Karoubi (HomologicalComplex C c))).map φ ≫ (fu …
        dsimp
        -- ⊢ HomologicalComplex.Hom.f φ.f i✝ ≫ HomologicalComplex.Hom.f Q.p i✝ = Homologi …
    ...
    { app := fun P =>
        { f :=
            { f := fun n => P.p.f n
              comm' := fun i j _ => by
                dsimp
                -- ⊢ HomologicalComplex.Hom.f P.p i ≫ HomologicalComplex.d P.X i j = (Homological …
      ...
      naturality := fun P Q φ => by
        ext
        -- ⊢ HomologicalComplex.Hom.f ((functor ⋙ inverse).map φ ≫ (fun P => Hom.mk (Homo …
        dsimp
        -- ⊢ HomologicalComplex.Hom.f φ.f i✝ ≫ HomologicalComplex.Hom.f Q.p i✝ = Homologi …
  ...
  hom_inv_id := by
    ext
    -- ⊢ HomologicalComplex.Hom.f (NatTrans.app ((NatTrans.mk fun P => Hom.mk (Homolo …
    dsimp
    -- ⊢ HomologicalComplex.Hom.f x✝.p i✝ ≫ HomologicalComplex.Hom.f x✝.p i✝ = Homolo …
