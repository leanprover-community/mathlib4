...
theorem sInf_isNormal (s : Set <| Subgroupoid C) (sn : ∀ S ∈ s, IsNormal S) : IsNormal (sInf s) :=
  { wide c := by rw [mem_sInf_arrows]; exact fun S Ss => (sn S Ss).wide c
    conj p γ := by simp_rw [mem_sInf_arrows]; exact fun hγ S Ss => (sn S Ss).conj p (hγ S Ss) }
...
theorem subset_generated (c d : C) : X c d ⊆ (generated X).arrows c d := by
  intro f fS
  simp only [generated, mem_sInf_arrows]
  exact fun S hS => hS _ _ fS
