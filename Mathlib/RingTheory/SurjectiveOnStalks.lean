...
lemma SurjectiveOnStalks.of_surjective (h : Function.Surjective f) :
...
variable (S) in
lemma SurjectiveOnStalks.of_isLocalization [IsLocalization M S] :
    SurjectiveOnStalks (algebraMap R S) := by
  intro P hP
  rw [surjective_localRingHom_iff]
  intro s
  obtain ⟨x, s, rfl⟩ := IsLocalization.mk'_surjective M s
  exact ⟨x, s, 1, P.primeCompl.one_mem, fun e ↦ hP.ne_top <|
    P.eq_top_of_isUnit_mem e (IsLocalization.map_units S s), by simp⟩
...
lemma SurjectiveOnStalks.baseChange
  ...
  rw [surjective_localRingHom_iff]
  intro x
  obtain ⟨t, r, a, ht, e⟩ := exists_mul_eq_tmul_of_surjectiveOnStalks hf x (J.comap g)
    inferInstance
...
lemma surjectiveOnStalks_iff_of_isLocalRingHom [LocalRing R] [LocalRing S] [IsLocalRingHom f] :
    f.SurjectiveOnStalks ↔ Function.Surjective f := by
  refine ⟨fun H x ↦ ?_, fun h ↦ SurjectiveOnStalks.of_surjective h⟩
