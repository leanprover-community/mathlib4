...
theorem mem_iSup_of_predirected {ι} [hι : Nonempty ι] {S : ι → NonUnitalSubring R}
  ...
  let U : NonUnitalSubring R :=
    NonUnitalSubring.mk' (⋃ i, (S i : Set R)) (⨆ i, (S i).toSubsemigroup) (⨆ i, (S i).toAddSubgroup)
      (Subsemigroup.coe_iSup_of_predirected hS) (AddSubgroup.coe_iSup_of_predirected hS)
  suffices ⨆ i, S i ≤ U by simpa [U] using @this x
  exact iSup_le fun i x hx ↦ Set.mem_iUnion.2 ⟨i, hx⟩

theorem coe_iSup_of_predirected {ι} [Nonempty ι] {S : ι → NonUnitalSubring R}
    (hS : Predirected (· ≤ ·) S) : ((⨆ i, S i : NonUnitalSubring R) : Set R) = ⋃ i, S i :=
  Set.ext fun x ↦ by simp [mem_iSup_of_predirected hS]

theorem mem_sSup_of_predirectedOn {S : Set (NonUnitalSubring R)} (Sne : S.Nonempty)
    (hS : PredirectedOn (· ≤ ·) S) {x : R} : x ∈ sSup S ↔ ∃ s ∈ S, x ∈ s := by
  haveI : Nonempty S := Sne.to_subtype
  simp only [sSup_eq_iSup', mem_iSup_of_predirected hS.predirected_val, SetCoe.exists,
    exists_prop]

theorem coe_sSup_of_predirectedOn {S : Set (NonUnitalSubring R)} (Sne : S.Nonempty)
