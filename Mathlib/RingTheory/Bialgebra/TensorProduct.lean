...
/-- `lTensor A f : A ⊗ B →ₐc A ⊗ C` is the natural coalgebra morphism induced by `f : N →ₐc P`. -/
noncomputable abbrev lTensor (f : B →ₐc[R] C) : A ⊗[R] B →ₐc[R] A ⊗[R] C :=
  Bialgebra.TensorProduct.map (BialgHom.id R A) f

/-- `rTensor A f : B ⊗ A →ₐc C ⊗ A` is the natural coalgebra morphism induced by `f : B →ₐc C`. -/
