...
noncomputable def ofBijective (f : A →ₐc[R] B) (hf : Bijective f) : A ≃ₐc[R] B :=
  .ofAlgEquiv (.ofBijective (f : A →ₐ[R] B) hf) (by ext; simp?) (by
    ext
    simp only [AlgHom.coe_comp, comp_apply, comulAlgHom_apply, AlgEquiv.coe_algHom,
      AlgEquiv.coe_ofBijective, BialgHom.coe_toAlgHom]
    sorry) --ext; simp)
