...
/-- The comultiplication is the unique algebra map satisfying
`comul (ι R m) = ι R m ⊗ 1 + 1 ⊗ ι R m` for all `m : M`. -/
@[simp]
lemma comul_ι (m : M) :
    Coalgebra.comul (ι R m) = ι R m ⊗ₜ[R] 1 + 1 ⊗ₜ[R] ι R m := by
  simp [CoalgebraStruct.comul, comul]

@[simp]
lemma counit_ι (m : M) : Coalgebra.counit (R := R) (ι R m) = 0 := by
