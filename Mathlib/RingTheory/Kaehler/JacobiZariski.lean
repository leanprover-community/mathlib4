...
def kerCompPreimage (Q : Generators S T) (P : Generators R S) (x : Q.ker) :
  ...
  · simp only [ker, Submodule.restrictScalars_mem, RingHom.mem_ker,
    ...
    simp only [_root_.map_one, Finsupp.prod_mapDomain_index_inj Sum.inl_injective, Sum.elim_inl,
      self_val, id_eq, one_mul, aeval_rename, Sum.elim_comp_inr, Function.comp, comp_val,
      Sum.elim_inr]
    conv_lhs =>
      enter [2, a, b]
      tactic =>
...
attribute [local instance 999999]
  TensorProduct.leftModule KaehlerDifferential.module' TensorProduct.isScalarTower_left
      ...
      have : (Q.toComp P).toAlgHom (monomial j (coeff (e.symm (i, j)) x)) =
        ...
        ext (i₁ | i₂) <;>
          simp only [Finsupp.sumFinsuppAddEquivProdFinsupp_symm_apply, comp_vars,
            ...
            Finsupp.mapDomain_apply Sum.inr_injective]
...
lemma Cotangent.exact (Q : Generators S T) (P : Generators R S) :
  ...
  · rw [LinearMap.liftBaseChange_comp, ← map_comp, AddEquivClass.map_eq_zero_iff]
    ext x
  ...
  · intro x hx
    ...
    simp only [AlgHom.toRingHom_eq_coe, Ideal.mem_comap, RingHom.coe_coe,
      Submodule.mem_map, Submodule.mem_comap, Submodule.restrictScalars_mem, Submodule.coeSubtype,
...
lemma CotangentSpace.compCotangentSpace_symm_inr
  (Q : Generators.{w} S T) (P : Generators.{w'} R S) :
    (compCotangentSpace Q P).symm.toLinearMap ∘ₗ
      LinearMap.inr T Q.CotangentSpace (T ⊗[S] P.CotangentSpace) =
        (map (Q.toComp P)).liftBaseChange T := by
  classical
  ...
  simp only [compCotangentSpace, LinearEquiv.trans_symm, LinearEquiv.symm_symm,
    ...
    Function.comp_apply, LinearEquiv.trans_apply, Basis.repr_symm_apply, Basis.repr_total,
    LinearMap.liftBaseChangeEquiv_tmul, one_smul, repr_map, toComp_val, pderiv_X]
  obtain (j | j) := j <;>
...
lemma CotangentSpace.compCotangentSpace_symm_zero
  (Q : Generators.{w} S T) (P : Generators.{w'} R S) (x) :
    (compCotangentSpace Q P).symm (0, x) =
        (map (Q.toComp P)).liftBaseChange T x :=
...
lemma CotangentSpace.fst_compCotangentSpace
  (Q : Generators.{w} S T) (P : Generators.{w'} R S) :
    LinearMap.fst T Q.CotangentSpace (T ⊗[S] P.CotangentSpace) ∘ₗ
      (compCotangentSpace Q P).toLinearMap = map (Q.ofComp P) := by
  classical
  ...
  obtain (i | i) := i <;>
    simp only [comp_vars, Basis.repr_symm_apply, Finsupp.total_single, Basis.prod_apply,
...
lemma CotangentSpace.fst_compCotangentSpace_apply
  (Q : Generators.{w} S T) (P : Generators.{w'} R S) (x) :
    (compCotangentSpace Q P x).1 = map (Q.ofComp P) x :=
...
variable {T' : Type*} [CommRing T'] [Algebra R T'] [Algebra T T'] [IsScalarTower R T T']
variable [Algebra R' T'] [Algebra S' T'] [IsScalarTower R' S' T'] [IsScalarTower R R' T']
variable [Algebra S T'] [IsScalarTower S S' T'] [IsScalarTower S T T']
...
def H1Cotangent.δAux :
  ...
  Finsupp.lsum R (R := R)
    (fun f ↦ (TensorProduct.mk S T _ (f.prod (Q.val · ^ ·))).restrictScalars R ∘ₗ (D R S).toLinearMap)
...
lemma H1Cotangent.δAux_toAlgHom {Q : Generators.{u₁} S T}
    ...
    δAux R Q' (f.toAlgHom x) = δAux R Q x + Finsupp.total _ _ _ (δAux R Q' ∘ f.val)
  ...
  · simp only [map_add, hx₁, hx₂, tmul_add]
    rw [add_add_add_comm]
  · simp only [AlgHom.map_mul, Hom.toAlgHom_X, δAux_mul, algebraMap_apply, Hom.algebraMap_toAlgHom,
      ...
      LinearMap.map_smul, Finsupp.total_single, Function.comp_apply, ← cotangentSpaceBasis_apply]
    rw [add_left_comm]
    rfl
...
lemma H1Cotangent.δAux_ofComp (x : (Q.comp P).Ring) :
  ...
  · simp only [AlgHom.map_mul, Hom.toAlgHom_X, ofComp_val, δAux_mul,
    ...
    · simp only [comp_vars, Sum.elim_inl, δAux_X, smul_zero, aeval_X,
        ...
        Basis.repr_self, Finsupp.total_single, Basis.prod_apply, LinearMap.coe_inl,
    ...
    · simp only [comp_vars, Sum.elim_inr, Function.comp_apply, algHom_C, δAux_C,
        ...
        algebraMap_smul, Basis.repr_self, Finsupp.total_single, Basis.prod_apply, LinearMap.coe_inl,
        LinearMap.coe_inr, Basis.baseChange_apply, one_smul, LinearMap.baseChange_tmul,
        toKaehler_cotangentSpaceBasis, add_left_inj]
      rfl
...
lemma H1Cotangent.exact_δ_map :
  ...
  · apply (P.cotangentSpaceBasis.baseChange T).ext
    ...
    simp only [Basis.baseChange_apply, LinearMap.coe_comp, Function.comp_apply,
      ...
      one_smul, comp_vars, LinearMap.liftBaseChangeEquiv_tmul]
...
lemma H1Cotangent.δ_eq_δAux (Q : Generators.{u₁} S T) (P : Generators.{u₂} R S)
  ...
  have : IsScalarTower (Q.comp P).Ring
    (Q.comp P).Ring T := IsScalarTower.left _
...
noncomputable
def Generators.ofSurjectiveAlgebraMap (h : Function.Surjective (algebraMap R S)) :
    Generators.{w} R S :=
  Generators.ofSurjective PEmpty.elim (fun x ↦ by
    obtain ⟨x, rfl⟩ := h x
    exact ⟨C x, aeval_C _ _⟩)

noncomputable
def Generators.ofSurjectiveAlgebraMapRing (h : Function.Surjective (algebraMap R S)) :
    R ≃ₐ[R] (Generators.ofSurjectiveAlgebraMap R S h).Ring where
  __ := Algebra.ofId _ _
  invFun := coeff 0
  left_inv x := by simp [ofId]
  right_inv x := by
    ext m
    obtain rfl : m = 0 := by ext ⟨⟩
    simp [ofId]

def Ideal.equivCotangent {R} [CommRing R] {A B} [CommRing A] [CommRing B]
    [Algebra R A] [Algebra R B] (I₁ : Ideal A) (I₂ : Ideal B) (f : A ≃ₐ[R] B)
    (h : I₁ = Ideal.comap f I₂) : I₁.Cotangent ≃ₗ[R] I₂.Cotangent where
  __ := Ideal.mapCotangent _ _ f.toAlgHom h.le
  invFun := Ideal.mapCotangent _ _ f.symm.toAlgHom (by rw [h]; intro x hx; aesop)
  left_inv x := by
...
noncomputable
def Generators.ofSurjectiveAlgebraMapCotangent (h : Function.Surjective (algebraMap R S)) :
    letI : Module S (RingHom.ker (algebraMap R S)).Cotangent := Module.compHom _
      ((algebraMap R S).quotientKerEquivOfSurjective h).symm.toRingHom
    (Generators.ofSurjectiveAlgebraMap R S h).Cotangent ≃ₗ[S]
      (RingHom.ker (algebraMap R S)).Cotangent :=
  letI : Module S (RingHom.ker (algebraMap R S)).Cotangent := Module.compHom _
    ((algebraMap R S).quotientKerEquivOfSurjective h).symm.toRingHom
  LinearEquiv.symm
  { __ := Ideal.equivCotangent _ _ (ofSurjectiveAlgebraMapRing R S h)
      (by ext; simp [ker, RingHom.mem_ker, ofSurjectiveAlgebraMapRing, ofId])
    map_smul' := by
      intro r x
      obtain ⟨r, rfl⟩ := h r
      ext
      simp only [algebraMap_smul, RingHom.id_apply, Cotangent.val_smul'']
      exact (Ideal.equivCotangent _ _ _ _).map_smul r x }

noncomputable
def Generators.ofSurjectiveAlgebraMapH1Cotangent (h : Function.Surjective (algebraMap R S)) :
    letI : Module S (RingHom.ker (algebraMap R S)).Cotangent := Module.compHom _
      ((algebraMap R S).quotientKerEquivOfSurjective h).symm.toRingHom
    (Generators.ofSurjectiveAlgebraMap R S h).H1Cotangent ≃ₗ[S]
      (RingHom.ker (algebraMap R S)).Cotangent := by
  refine (LinearEquiv.ofTop _ ?_).trans (Generators.ofSurjectiveAlgebraMapCotangent R S h)
  rw [← top_le_iff]
  intro x _
  simp only [LinearMap.mem_ker]
  apply (Generators.cotangentSpaceBasis _).ext_elem
  rintro ⟨⟩

noncomputable
def H1Cotangent.ofSurjective (h : Function.Surjective (algebraMap R S)) :
    letI : Module S (RingHom.ker (algebraMap R S)).Cotangent := Module.compHom _
      ((algebraMap R S).quotientKerEquivOfSurjective h).symm.toRingHom
    H1Cotangent R S ≃ₗ[S] (RingHom.ker (algebraMap R S)).Cotangent :=
  (Generators.H1Cotangent.equiv _ (Generators.ofSurjectiveAlgebraMap.{0} R S h)).trans
    (Generators.ofSurjectiveAlgebraMapH1Cotangent R S h)
