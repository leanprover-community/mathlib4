...
lemma ofComp_kerCompPreimage (x : Q.ker) :
  ...
  simp only [AlgHom.toLinearMap_apply, _root_.map_mul]
  -- simp does not apply this anymore after #21050
  rw [Hom.toAlgHom_monomial]
...
lemma Cotangent.exact :
  ...
  · rw [LinearMap.liftBaseChange_comp, ← Extension.Cotangent.map_comp,
    ...
    obtain ⟨⟨x, hx⟩, rfl⟩ := Extension.Cotangent.mk_surjective x
    rw [map_mk]
    simp only [LinearMap.zero_apply, val_zero]
    rw [val_mk]
...
noncomputable instance : Algebra (Q.comp P).Ring T := (Q.comp P).algebra
