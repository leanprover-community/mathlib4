...
theorem closure_subsemigroup_closure (s : Set R) : closure ↑(Subsemigroup.closure s) = closure s :=
  le_antisymm
    (closure_le.mpr fun _ hy =>
      (SetLike.mem_closure.mp hy) (closure s).toSubsemigroup subset_closure)
    (closure_mono (SetLike.subset_closure _))
...
theorem closure_addSubmonoid_closure {s : Set R} :
  ...
  refine (SetLike.mem_closure.mp hy) H.toSubsemigroup fun z hz => ?_
