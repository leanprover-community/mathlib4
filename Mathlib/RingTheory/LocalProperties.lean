...
theorem ideal_eq_bot_of_localization (I : Ideal R)
  ...
  ideal_eq_bot_of_localization' _ fun P hP =>
    (Ideal.map_eq_bot_iff_le_ker _).mpr fun x hx => by
      erw [RingHom.mem_ker, ← Submodule.mem_bot R, ← h P hP, IsLocalization.mem_coeSubmodule]
      exact ⟨x, hx, rfl⟩
#align ideal_eq_bot_of_localization ideal_eq_bot_of_localization
...
theorem localization_finite : RingHom.LocalizationPreserves @RingHom.Finite := by
  ...
  erw [IsLocalization.mk'_eq_mul_mk'_one, mul_comm, Finset.coe_image]
  ...
  erw [Submodule.map_span fₐ.toLinearMap T] at hy
  ...
  have := (Submodule.span R' (algebraMap S S' '' T)).smul_mem
    (IsLocalization.mk' R' (1 : R) ⟨r, hr⟩) (H hy)
  rw [Algebra.smul_def] at this
  erw [IsLocalization.map_mk' M.le_comap_map] at this
  erw [map_one] at this
  apply this
  -- have :
...
theorem localization_finiteType : RingHom.LocalizationPreserves @RingHom.FiniteType := by
  ...
  have := (Algebra.adjoin R' (algebraMap S S' '' T)).smul_mem (H hy)
    (IsLocalization.mk' R' (1 : R) ⟨r, hr⟩)
  rw [Algebra.smul_def] at this
  erw [IsLocalization.map_mk' M.le_comap_map] at this
  rw [map_one] at this
  apply this
