...
theorem Bivariate.equivAdjoinOfTranscendental_swap_eq_aeval (x : A) (p : R[X][Y])
    ...
    equivAdjoinOfTranscendental x hx (swap p) =
      aeval (C ⟨x, Algebra.self_mem_adjoin_singleton R x⟩) p := by
  induction p using Polynomial.induction_on' with
  | add => simp_all only [map_add]
  | monomial n a =>
    induction a using Polynomial.induction_on' with
    | add p q => simp_all only [map_add]
    | monomial => simp_all [C_mul_X_pow_eq_monomial, Bivariate.equivAdjoinOfTranscendental]
...
theorem Bivariate.aeval_aeval_eq_aeval_equivAdjoinOfTranscendental (x : A) (y : B)
    (hx : Transcendental R x) (p : R[X][Y]) :
    aeval (algebraMap A B x) (aeval (C (⟨y, Algebra.self_mem_adjoin_singleton R y⟩ :
...
theorem _root_.IsAlgebraic.adjoin_singleton {x : A} {y : B} (hx : Transcendental R x)
    ...
    IsAlgebraic (Algebra.adjoin R {y}) (algebraMap A B x) := by
  ...
  rw [equivAdjoinOfTranscendental_swap_eq_aeval y _ hy]
  simpa [aeval_aeval_eq_aeval_equivAdjoinOfTranscendental x y hx]
