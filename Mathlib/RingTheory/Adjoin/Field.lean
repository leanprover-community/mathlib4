...
theorem Polynomial.lift_of_splits {F K L : Type*} [Field F] [Field K] [Field L] [Algebra F K]
    [Algebra F L] (s : Finset K) : (∀ x ∈ s, IsIntegral F x ∧
      Splits (Polynomial.map (algebraMap F L) (minpoly F x))) → Nonempty (Algebra.adjoin F (s : Set K) →ₐ[F] L) := by
  classical
    ...
    have H6 : (Polynomial.map (algebraMap Ks L) (minpoly Ks a)).Splits := by
...
theorem IsIntegral.mem_range_algHom_of_minpoly_splits
    (int : IsIntegral R x) (h : Splits (Polynomial.map (algebraMap R K) (minpoly R x))) (f : K →ₐ[R] L) :
    x ∈ f.range :=
...
theorem IsIntegral.mem_range_algebraMap_of_minpoly_splits [Algebra K L] [IsScalarTower R K L]
    (int : IsIntegral R x) (h : Splits (Polynomial.map (algebraMap R K) (minpoly R x))) :
    x ∈ (algebraMap K L).range :=
  int.mem_range_algHom_of_minpoly_splits h (IsScalarTower.toAlgHom R K L)

theorem minpoly_neg_splits [Algebra K L] {x : L} (g : (Polynomial.map (algebraMap K L) (minpoly K x)).Splits) :
    (Polynomial.map (algebraMap K L) (minpoly K (-x))).Splits := by
  rw [minpoly.neg]
...
theorem minpoly_add_algebraMap_splits [Algebra K L] {x : L} (r : K)
    (g : (Polynomial.map (algebraMap K L) (minpoly K x)).Splits) :
    (Polynomial.map (algebraMap K L) (minpoly K (x + algebraMap K L r))).Splits := by
...
theorem minpoly_algebraMap_add_splits [Algebra K L] {x : L} (r : K)
    (g : (Polynomial.map (algebraMap K L) (minpoly K x)).Splits) :
    (Polynomial.map (algebraMap K L) (minpoly K (algebraMap K L r + x))).Splits := by
...
theorem IsIntegral.minpoly_splits_tower_top' (int : IsIntegral R x) {f : K →+* L}
    (h : Splits (Polynomial.map (f.comp <| algebraMap R K) (minpoly R x))) :
    Splits (Polynomial.map f (minpoly K x)) :=
...
theorem IsIntegral.minpoly_splits_tower_top [Algebra K L] [Algebra R L] [IsScalarTower R K L]
    (int : IsIntegral R x) (h : Splits (Polynomial.map (algebraMap R L) (minpoly R x))) :
    Splits (Polynomial.map (algebraMap K L) (minpoly K x)) := by
