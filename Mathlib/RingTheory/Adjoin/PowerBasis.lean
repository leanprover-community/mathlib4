...
noncomputable def adjoin.powerBasisAux {x : S} (hx : IsIntegral K x) :
  ...
  have hx' :
    IsIntegral K (⟨x, subset_adjoin (mem_singleton_self x)⟩ : K[(x : S)]) := by
    apply (isIntegral_algebraMap_iff hST).mp
    convert! hx
  apply Basis.mk (v := fun i : Fin _ ↦ ⟨x, subset_adjoin (mem_singleton_self x)⟩ ^ (i : ℕ))
...
noncomputable def adjoin.powerBasis {x : S} (hx : IsIntegral K x) :
  ...
  gen := ⟨x, subset_adjoin (mem_singleton_self x)⟩
