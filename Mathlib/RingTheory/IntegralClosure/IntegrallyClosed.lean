...
theorem IsIntegrallyClosed.of_localization_maximal {R : Type*} [CommRing R] [IsDomain R]
  ...
  have h1 : 1 ∈ I := by
    ...
    have hxy :
        mk' _ x.1 x.2 = algebraMap (Localization.AtPrime p) (FractionRing R) (mk' _ y.1 y.2) := by
      rw [← mk_eq_mk', ← mk_eq_mk', ← hy]
      rfl
    ...
    have hyi : y.2.1 ∈ I := by
      ...
      exact congrArg (HMul.hMul b) <| Eq.trans (mul_comm y.1 x.2.1) <|
        NoZeroSMulDivisors.algebraMap_injective R (Localization R⁰) <|
          IsLocalization.mk'_eq_iff_eq.mp <| Eq.trans
            (IsLocalization.mk'_eq_algebraMap_mk'_of_submonoid_le (Localization.AtPrime p)
              (Localization R⁰) (primeCompl_le_nonZeroDivisors p) y.1 y.2) hxy.symm
    exact y.2.2 (hpi hyi)
