...
theorem LinearMap.lTensor_mul'_comp_assoc_comp_rTensor_comul_of
    (h : lTensor A μ ∘ₗ α ∘ₗ rTensor A δ = rTensor A μ[R] ∘ₗ α⁻¹ ∘ₗ lTensor A δ) :
    lTensor A μ ∘ₗ α ∘ₗ rTensor A δ = δ ∘ₗ μ[R] :=
  calc _ = rTensor A μ ∘ₗ α⁻¹ ∘ₗ lTensor A δ := h
    _ = rTensor _ μ ∘ₗ α⁻¹ ∘ₗ ((λ ∘ₗ rTensor _ ε ∘ₗ δ) ⊗ₘ δ) := by ext; simp
    _ = rTensor _ μ ∘ₗ α⁻¹ ∘ₗ rTensor _ λ ∘ₗ rTensor _ (rTensor _ ε) ∘ₗ (δ ⊗ₘ δ) := by
      simp only [rTensor_comp_map]
    _ = rTensor _ (μ[R] ∘ₗ λ ∘ₗ rTensor _ ε ∘ₗ α) ∘ₗ α⁻¹ ∘ₗ (δ ⊗ₘ δ) := by
      simp_rw [← LinearMap.comp_assoc]
      congr 1; ext; simp [smul_mul_assoc]
    _ = rTensor _ (λ ∘ₗ (lTensor _ μ[R] ∘ₗ rTensor _ ε) ∘ₗ α) ∘ₗ α⁻¹ ∘ₗ (δ ⊗ₘ δ) := by
      simp_rw [← LinearMap.comp_assoc]
      congr 5; ext; simp
    _ = rTensor _ (λ ∘ₗ (rTensor _ ε ∘ₗ lTensor _ μ[R]) ∘ₗ α) ∘ₗ α⁻¹ ∘ₗ (δ ⊗ₘ δ) := by
      congr; ext; simp
    _ = rTensor _ (λ ∘ₗ (rTensor _ ε ∘ₗ lTensor _ μ[R]) ∘ₗ α) ∘ₗ
        (α⁻¹ ∘ₗ rTensor _ δ) ∘ₗ lTensor _ δ := by
      simp only [comp_assoc]
      congr; ext; simp
    _ = rTensor _ (λ ∘ₗ (rTensor _ ε ∘ₗ lTensor _ μ[R]) ∘ₗ α) ∘ₗ
        rTensor _ (rTensor _ δ) ∘ₗ α⁻¹ ∘ₗ lTensor _ δ := by
      simp only [rTensor_tensor, comp_assoc]
      simp only [← comp_assoc _ _ α⁻¹, LinearEquiv.symm_comp, id_comp]
    _ = (λ ∘ₗ rTensor _ ε) ∘ₗ α ∘ₗ (rTensor _ (rTensor _ μ[R] ∘ₗ α⁻¹ ∘ₗ lTensor _ δ) ∘ₗ α⁻¹) ∘ₗ
        lTensor A δ := by
      rw [← h]
      simp_rw [← comp_assoc]
      congr 2
    ...
    _ = λ ∘ₗ rTensor _ ε ∘ₗ α ∘ₗ rTensor _ (rTensor _ μ[R]) ∘ₗ rTensor A α⁻¹ ∘ₗ
        α⁻¹ ∘ₗ lTensor _ (rTensor A δ ∘ₗ δ) := by
      simp_rw [comp_assoc]
      congr 3
      simp_rw [← comp_assoc]
      rw [← rTensor_comp]
    ...
    _ = λ ∘ₗ rTensor _ ε ∘ₗ α ∘ₗ rTensor _ (rTensor _ μ[R]) ∘ₗ rTensor A α⁻¹ ∘ₗ
        α⁻¹ ∘ₗ lTensor _ (α⁻¹ ∘ₗ lTensor A δ ∘ₗ δ) := by
      rw [Coalgebra.coassoc_symm]
    _ = λ ∘ₗ rTensor _ ε ∘ₗ α ∘ₗ rTensor _ (rTensor _ μ[R]) ∘ₗ rTensor A α⁻¹ ∘ₗ α⁻¹ ∘ₗ
        lTensor _ α⁻¹ ∘ₗ lTensor _ (lTensor A δ) ∘ₗ lTensor _ δ := by
      simp_rw [← lTensor_comp]
    _ = λ ∘ₗ rTensor _ ε ∘ₗ (rTensor _ μ[R] ∘ₗ α) ∘ₗ rTensor A α⁻¹ ∘ₗ α⁻¹ ∘ₗ lTensor _ α⁻¹ ∘ₗ
        lTensor _ (lTensor A δ) ∘ₗ lTensor _ δ := by
      simp_rw [rTensor_tensor, comp_assoc]
      simp only [← comp_assoc _ _ α⁻¹, LinearEquiv.symm_comp, id_comp]
    _ = λ ∘ₗ rTensor _ ε ∘ₗ rTensor _ μ[R] ∘ₗ (α⁻¹ ∘ₗ lTensor _ (lTensor A δ)) ∘ₗ lTensor _ δ := by
      rw [assoc_tensor'']
      simp_rw [LinearEquiv.trans_symm, ← LinearEquiv.comp_coe, LinearEquiv.symm_symm, comp_assoc]
      rfl
    _ = λ ∘ₗ rTensor _ ε ∘ₗ (rTensor _ μ[R] ∘ₗ lTensor _ δ) ∘ₗ α⁻¹ ∘ₗ lTensor _ δ := by
      simp_rw [lTensor_tensor, comp_assoc]
      simp only [← comp_assoc _ _ α, LinearEquiv.comp_symm, id_comp]
    _ = λ ∘ₗ rTensor _ ε ∘ₗ lTensor _ δ ∘ₗ (lTensor _ μ[R] ∘ₗ α ∘ₗ rTensor _ δ) := by
      rw [rTensor_comp_lTensor, ← lTensor_comp_rTensor, h]
      simp_rw [comp_assoc]
    _ = λ ∘ₗ (lTensor _ (δ ∘ₗ μ[R]) ∘ₗ rTensor _ ε) ∘ₗ α ∘ₗ rTensor _ δ := by
      rw [lTensor_comp_rTensor, ← rTensor_comp_lTensor, lTensor_comp]
      simp_rw [comp_assoc]
    _ = λ ∘ₗ lTensor _ (δ ∘ₗ μ[R]) ∘ₗ (rTensor _ ε ∘ₗ α) ∘ₗ rTensor _ δ := by
      simp_rw [comp_assoc]
    _ = λ ∘ₗ lTensor _ (δ ∘ₗ μ[R]) ∘ₗ (α ∘ₗ rTensor _ (rTensor _ ε)) ∘ₗ rTensor _ δ := by
      simp_rw [rTensor_tensor, comp_assoc]
      simp only [← comp_assoc _ _ α⁻¹, LinearEquiv.symm_comp, id_comp]
    _ = λ ∘ₗ lTensor _ (δ ∘ₗ μ[R]) ∘ₗ α ∘ₗ rTensor _ λ⁻¹ := by
      rw [(by rfl : λ⁻¹ = (TensorProduct.mk R R A) 1),
...
theorem lTensor_mul'_comp_assoc_comp_rTensor_comul_eq_comul_comp_mul' :
    lTensor A μ[R] ∘ₗ α ∘ₗ rTensor A δ = δ ∘ₗ μ[R] :=
...
theorem rTensor_mul'_comp_assoc_symm_comp_lTensor_comul_eq_comul_comp_mul :
    rTensor A μ[R] ∘ₗ α⁻¹ ∘ₗ lTensor A δ = δ ∘ₗ μ[R] :=
...
theorem rTensor_counit_comp_mul'_assoc_symm_comp_lTensor_comul_comp_algebraLinearMap :
    rTensor A (ε ∘ₗ μ[R]) ∘ₗ α⁻¹ ∘ₗ lTensor A (δ ∘ₗ η[R]) =
...
theorem lTensor_counit_comp_mul_comp_assoc_comp_rTensor_comul_comp_algebraLinearMap :
    lTensor A (ε ∘ₗ μ[R]) ∘ₗ α ∘ₗ rTensor A (δ ∘ₗ η[R]) =
      (TensorProduct.comm _ _ _).toLinearMap := by
