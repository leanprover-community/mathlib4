...
lemma adicValuation_deriv_nonneg_of_nonneg (p : F) [Fact (Prime p)] (a : K)
  ...
  simp only [emultiplicity_eq_zero.mpr ha, map_zero, WithTop.map_zero, CharP.cast_eq_zero,
    WithTop.coe_zero, smul_zero, neg_zero, zero_add]
...
lemma adicValuation_deriv_lt_neg_one_of_neg (p : F) [Fact (Prime p)] (hp : ¬p ∣ p′) (a : K)
  ...
  · simp only [← md, mf.emultiplicity_eq_multiplicity, map_add, map_natCast, map_one, two_smul,
    neg_add_rev, emultiplicity_mul Fact.out, ha2, zero_add, WithTop.map_natCast, Nat.cast_id,
    WithTop.coe_natCast, gt_iff_lt]
