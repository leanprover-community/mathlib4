...
variable {R A M : Type*} [CommSemiring R] [CommRing A] [CommRing M] [Algebra R A] [Algebra R M]

def liftOfRightInverse (f : A →ₐ[R] M) (f_inv : M → A) (hf : Function.RightInverse f_inv f)
    (d : Derivation R A A) (hd : ∀ x, f x = 0 → f (d x) = 0) : Derivation R M M where
  toFun x := f (d (f_inv x))
  map_add' x y := by
    simp only [← map_add]
...
@[simp]
lemma liftOfRightInverse_apply (f : A →ₐ[R] M) (f_inv : M → A) (hf : Function.RightInverse f_inv f)
    (d : Derivation R A A) (hd : ∀ x, f x = 0 → f (d x) = 0) (x : A) :
...
noncomputable abbrev liftOfSurjective (f : A →ₐ[R] M) (hf : Function.Surjective f)
    (d : Derivation R A A) (hd : ∀ x, f x = 0 → f (d x) = 0) : Derivation R M M :=
  d.liftOfRightInverse f (Function.surjInv hf) (Function.rightInverse_surjInv hf) hd

lemma liftOfSurjective_apply (f : A →ₐ[R] M) (hf : Function.Surjective f)
    (d : Derivation R A A) (hd : ∀ x, f x = 0 → f (d x) = 0) (x : A) :
