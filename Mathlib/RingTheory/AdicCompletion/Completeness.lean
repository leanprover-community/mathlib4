...
open scoped Pointwise in
theorem restrictScalars_image_smul_eq_smul_restrictScalars {S : Type*} [CommSemiring S] [Module S M]
    [Algebra S R] [IsScalarTower S R M] (s : Set S) (N : Submodule R M) :
  ...
  · rw [restrictScalars_mem] at x_in
    ...
    · intro r y h h'
      ...
      refine mem_set_smul_of_mem_mem u_in' ?_
      rw [restrictScalars_mem]
      exact coe_mem (c ((algebraMap S R) u))
  · rw [restrictScalars_mem] at x_in
