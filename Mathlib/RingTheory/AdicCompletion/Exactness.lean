...
theorem map_injective {f : M →ₗ[R] N} (hf : Function.Injective f) :
  ...
  intro hx
  refine AdicCompletion.mk_zero_of _ _ _ ⟨42, fun n _ ↦ ⟨n + k, by grind, n, by grind, ?_⟩⟩
  ...
  nth_rw 1 [show n = n + k - k by grind]
  rw [← hk (n + k) (show n + k ≥ k by grind)]
  ...
  have h : f (y - yₙ) ∈ (I ^ (k + n) • ⊤ : Submodule R N) := by
    ...
    · refine Submodule.sub_mem _ (Submodule.sub_mem _ ?_ ?_) hyₙ
      · rw [← Submodule.Quotient.eq]
        exact AdicCauchySequence.mk_eq_mk (by grind) _
      · exact (Submodule.smul_mono_left (Ideal.pow_le_pow_right (by grind))) hdmem
  ...
  · apply AdicCompletion.induction_on I N y (fun b ↦ ?_)
    ...
    · ext n
      suffices h : Submodule.Quotient.mk (p := (I ^ n • ⊤ : Submodule R N)) (f (a n)) =
        ...
        simp [h, AdicCauchySequence.mk_eq_mk (show n ≤ k + n by grind)]
      rw [Submodule.Quotient.eq]
      have hle : (I ^ (k + n) • ⊤ : Submodule R N) ≤ (I ^ n • ⊤ : Submodule R N) :=
        Submodule.smul_mono_left (Ideal.pow_le_pow_right (by grind))
