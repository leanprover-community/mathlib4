...
@[simp]
lemma of_apply (x : M) (n : ℕ) : (of I M x).1 n = mkQ (I ^ n • ⊤ : Submodule R M) x :=
  rfl
...
lemma eval_of (n : ℕ) (x : M) : eval I M n (of I M x) = mkQ (I ^ n • ⊤ : Submodule R M) x :=
  rfl
...
lemma eval_surjective (n : ℕ) : Function.Surjective (eval I M n) := fun x ↦
...
@[simp]
lemma val_zero (n : ℕ) : (0 : AdicCompletion I M).val n = 0 :=
  rfl

variable {I M}

@[simp]
lemma val_add (n : ℕ) (f g : AdicCompletion I M) : (f + g).val n = f.val n + g.val n :=
  rfl

@[simp]
lemma val_sub (n : ℕ) (f g : AdicCompletion I M) : (f - g).val n = f.val n - g.val n :=
  rfl

@[simp]
lemma val_smul (n : ℕ) (r : R) (f : AdicCompletion I M) : (r • f).val n = r • f.val n :=
  rfl

@[ext]
lemma ext {x y : AdicCompletion I M} (h : ∀ n, x.val n = y.val n) : x = y :=
  Subtype.eq <| funext h
#align adic_completion.ext AdicCompletion.ext

lemma ext_iff {x y : AdicCompletion I M} : x = y ↔ ∀ n, x.val n = y.val n :=
...
@[simp]
lemma transitionMap_mk {m n : ℕ} (hmn : m ≤ n) (x : M) :
...
@[simp]
lemma transitionMap_eq (n : ℕ) : transitionMap I M (Nat.le_refl n) = LinearMap.id := by
  ext
  simp

@[simp]
lemma transitionMap_comp {m n k : ℕ} (hmn : m ≤ n) (hnk : n ≤ k) :
    transitionMap I M hmn ∘ₗ transitionMap I M hnk = transitionMap I M (hmn.trans hnk) := by
  ext
  simp

@[simp]
lemma transitionMap_comp_apply {m n k : ℕ} (hmn : m ≤ n) (hnk : n ≤ k)
...
@[simp]
lemma transitionMap_comp_eval_apply {m n : ℕ} (hmn : m ≤ n) (x : AdicCompletion I M) :
    transitionMap I M hmn (x.val n) = x.val m :=
  x.property hmn

@[simp]
lemma transitionMap_comp_eval {m n : ℕ} (hmn : m ≤ n) :
...
@[simp]
lemma zero_apply (n : ℕ) : (0 : AdicCauchySequence I M) n = 0 :=
  rfl

variable {I M}

@[simp]
lemma add_apply (n : ℕ) (f g : AdicCauchySequence I M) : (f + g) n = f n + g n :=
  rfl

@[simp]
lemma sub_apply (n : ℕ) (f g : AdicCauchySequence I M) : (f - g) n = f n - g n :=
  rfl

@[simp]
lemma smul_apply (n : ℕ) (r : R) (f : AdicCauchySequence I M) : (r • f) n = r • f n :=
  rfl

@[ext]
lemma ext {x y : AdicCauchySequence I M} (h : ∀ n, x n = y n) : x = y :=
  Subtype.eq <| funext h

lemma ext_iff {x y : AdicCauchySequence I M} : x = y ↔ ∀ n, x n = y n :=
