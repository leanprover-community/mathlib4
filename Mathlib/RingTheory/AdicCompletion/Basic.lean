...
theorem mk_zero_of (f : AdicCauchySequence I M)
  ...
  obtain ⟨k, h⟩ := h
  ext n
  obtain ⟨m, hnm, l, hnl, hl⟩ := h (n + k) (by grind)
  rw [mk_apply_coe, Submodule.mkQ_apply, val_zero,
    ← AdicCauchySequence.mk_eq_mk (show n ≤ m by grind)]
  simpa using (Submodule.smul_mono_left (Ideal.pow_le_pow_right (by grind))) hl
