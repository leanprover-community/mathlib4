...
instance range [Finite R M] (f : M →ₗ[R] N) : Finite R (LinearMap.range f) := fast_instance%
...
instance map (p : Submodule R M) [Finite R p] (f : M →ₗ[R] N) : Finite R (p.map f) := fast_instance%
...
instance self : Finite R R := fast_instance%
...
instance prod [hM : Finite R M] [hN : Finite R N] : Finite R (M × N) := fast_instance%
  ⟨by
...
instance ulift [Finite R M] : Finite R (ULift M) := fast_instance% equiv ULift.moduleEquiv.symm

theorem iff_fg {N : Submodule R M} : Module.Finite R N ↔ N.FG := Module.finite_def.trans (fg_top _)

variable (R M)

instance bot : Module.Finite R (⊥ : Submodule R M) := fast_instance% iff_fg.mpr fg_bot

instance top [Finite R M] : Module.Finite R (⊤ : Submodule R M) := fast_instance% iff_fg.mpr out
...
instance span_singleton (x : M) : Module.Finite R (R ∙ x) := fast_instance%
  Module.Finite.span_of_finite R <| Set.finite_singleton _

/-- The submodule generated by a finset is `R`-finite. -/
instance span_finset (s : Finset M) : Module.Finite R (span R (s : Set M)) := fast_instance%
