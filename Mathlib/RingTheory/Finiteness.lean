...
lemma fg_iSup {ι : Sort*} [Finite ι] (N : ι → Submodule R M) (h : ∀ i, (N i).FG) :
    (iSup N).FG := by
...
lemma FG.stabilizes_of_iSup_eq {M' : Submodule R M} (hM' : M'.FG) (N : ℕ →o Submodule R M)
...
lemma equiv_iff (e : M ≃ₗ[R] N) : Finite R M ↔ Finite R N :=
  ⟨fun _ ↦ equiv e, fun _ ↦ equiv e.symm⟩

instance ulift [Finite R M] : Finite R (ULift M) := equiv ULift.moduleEquiv.symm

lemma iff_fg {N : Submodule R M} : Module.Finite R N ↔ N.FG := Module.finite_def.trans (fg_top _)
...
lemma Module.End.isNilpotent_iff_of_finite {R M : Type*} [CommSemiring R] [AddCommMonoid M]
...
lemma trans {R : Type*} (A M : Type*) [Semiring R] [Semiring A] [Module R A]
