...
instance (priority := 10000) quotient (R) {A M} [Semiring R] [AddCommGroup M] [Ring A] [Module A M] [Module R M]
...
instance (priority := 10000) range [Finite R M] (f : M →ₗ[R] N) : Finite R (LinearMap.range f) :=
...
instance (priority := 10000) map (p : Submodule R M) [Finite R p] (f : M →ₗ[R] N) : Finite R (p.map f) :=
...
instance (priority := 10000) self : Finite R R :=
...
instance (priority := 10000) prod [hM : Finite R M] [hN : Finite R N] : Finite R (M × N) :=
  ⟨by
    rw [← Submodule.prod_top]
    exact hM.1.prod hN.1⟩
#align module.finite.prod Module.Finite.prod

instance (priority := 10000) pi {ι : Type*} {M : ι → Type*} [_root_.Finite ι] [∀ i, AddCommMonoid (M i)]
...
instance (priority := 10000) ulift [Finite R M] : Finite R (ULift M) := equiv ULift.moduleEquiv.symm

theorem iff_fg {N : Submodule R M} : Module.Finite R N ↔ N.FG := Module.finite_def.trans (fg_top _)

variable (R M)

instance (priority := 10000) bot : Module.Finite R (⊥ : Submodule R M) := iff_fg.mpr fg_bot

instance (priority := 10000) top [Finite R M] : Module.Finite R (⊤ : Submodule R M) := iff_fg.mpr out
...
instance (priority := 10000) span_singleton (x : M) : Module.Finite R (R ∙ x) :=
  Module.Finite.span_of_finite R <| Set.finite_singleton _

/-- The submodule generated by a finset is `R`-finite. -/
instance (priority := 10000) span_finset (s : Finset M) : Module.Finite R (span R (s : Set M)) :=
...
instance (priority := 10000) Module.Finite.base_change [CommSemiring R] [Semiring A] [Algebra R A] [AddCommMonoid M]
...
instance (priority := 10000) Module.Finite.tensorProduct [CommSemiring R] [AddCommMonoid M] [Module R M]
...
instance (priority := 10000) finite_sup (S₁ S₂ : Submodule R V) [h₁ : Module.Finite R S₁]
...
instance (priority := 10000) finite_finset_sup {ι : Type*} (s : Finset ι) (S : ι → Submodule R V)
...
instance (priority := 10000) finite_iSup {ι : Sort*} [Finite ι] (S : ι → Submodule R V)
...
instance (priority := 10000) Module.Finite.finsupp {ι : Type*} [_root_.Finite ι] [Module.Finite R V] :
    Module.Finite R (ι →₀ V) :=
