...
theorem exists_lift {B : Type u} [CommRing B] [SMul R B] [_RB : Algebra R B]
...
variable {C : Type u} [CommRing C] [SMul R C] [Algebra R C]
...
variable {P A : Type u} [CommRing A] [SMul R A] [Algebra R A] [CommRing P] [SMul R P] [Algebra R P]
...
theorem localization_base [FormallySmooth R Sₘ] : FormallySmooth Rₘ Sₘ := by
  constructor
  intro Q _ _ I e f
  letI := ((algebraMap Rₘ Q).comp (algebraMap R Rₘ)).toSMul
...
theorem localization_map [FormallySmooth R S] : FormallySmooth Rₘ Sₘ := by
  haveI : FormallySmooth S Sₘ := sorry -- FormallySmooth.of_isLocalization (M.map (algebraMap R S))
...
class Smooth [CommSemiring R] (A : Type u) [Semiring A] [SMul R A] [Algebra R A] : Prop where
...
theorem of_isLocalization_Away (r : R) [IsLocalization.Away r A] : Smooth R A where
  formallySmooth := sorry -- Algebra.FormallySmooth.of_isLocalization (Submonoid.powers r)
