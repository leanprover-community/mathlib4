...
lemma height_eq_height_add_one (p : Ideal R)
  ...
  have disj : Disjoint (p.primeCompl.map (C (R := R) : R →* R[X]) : Set R[X]) P := by
    refine Set.disjoint_left.mpr fun a ⟨b, hb⟩ ha ↦ hb.1 ?_
    simp only [MonoidHom.coe_ofClass] at hb
  ...
  have : P'.IsMaximal :=
    IsLocalization.isMaximal_of_isMaximal_under (p.primeCompl.map (C (R := R) : R →* R[X])) Rₚ[X] P'
  ...
  have eq2 : P.height = P'.height := by
    rw [height_map_of_disjoint (Submonoid.map (C (R := R) : R →* R[X]) <| p.primeCompl) _ disj]
  rw [eq1, eq2]
