...
public import Mathlib.RingTheory.LocalRing.ResidueField.Spectrum
...
theorem Polynomial.ringKrullDim_le {R : Type*} [CommRing R] :
  ...
  · rw [show C = (algebraMap R (Polynomial R)) from rfl, Order.krullDim_eq_of_orderIso
      ...
      ← ringKrullDim_eq_of_ringEquiv ((polyEquivTensor R (p.asIdeal.ResidueField)).trans
        ((Ideal.Fiber.algEquivTensor p.1 (Polynomial R)).symm.restrictScalars R)).toRingEquiv,
      ← Ring.krullDimLE_iff]
    infer_instance
