...
private lemma quadratic_roots_of_unity [Invertible (2 : K)] (z : K) :
    z ^ 2 = 1 ↔ z = 1 ∨ z = -1 := by
...
private lemma quadratic_roots_of_minus_one (s : K) (hs : -1 = s * s) (z : K) :
  -1 = z * z ↔ z = s ∨ z = -s := by
  constructor
  · intro hz
    sorry
...
lemma cubic_cyclotomic_polynomial_roots_of_sq [Invertible (2 : K)] {z s : K}
  ...
  suffices 1 * (z * z) + 1 * z + 1 = 0 ↔ z = (s - 1) * ⅟2 ∨ z = (-s - 1) * ⅟2 by
    grind
...
lemma cubic_roots_of_unity_of_sq_eq [Invertible (2 : K)] {s : K} (hs : s * s = -3) :
  ...
  grind [cubic_cyclotomic_polynomial_roots_of_sq hs,
...
lemma cubic_roots_of_unity_of_sq_ne (h : ¬ IsSquare (-3 : K)) : {z : K | z^3 = 1} = {1} := by
  ...
  · intro x
    ...
    grind [cubic_cyclotomic_polynomial_ne_zero_of_sq_ne h]
...
example : {z : ℂ | z ^ 3 = 1} = {1, -(1 / 2) - √3 / 2 * I, -(1 / 2) + √3 / 2 * I} := by
  ...
  rw [cubic_roots_of_unity_of_sq_eq hs, (show ⅟(2 : ℂ) = 1 / 2 by simp)]
  ring_nf
...
lemma quartic_roots_of_unity_of_sq_eq [Invertible (2 : K)] {s : K} (hs : s * s = -1) :
  ...
  simp only [Set.ext_iff, Set.mem_setOf_eq, Set.mem_insert_iff, Set.mem_singleton_iff]
  intro z
  have : z ^ 2 = -1 ↔ z = s ∨ z = -s := by grind only [pow_two, quadratic_roots_of_minus_one]
...
lemma quintic_factorize_cyclotomic_polynomial [Invertible (4 : K)] {s t₁ t₂ : K} (hs : s * s = 5)
    ...
    z ^ 4 + z ^ 3 + z ^ 2 + z + 1 = (z - ( (s - 1) * ⅟4 + t₁ * ⅟4))
                                  ...
                                  * (z - (-(s + 1) * ⅟4 + t₂ * ⅟4))
                                  * (z - (-(s + 1) * ⅟4 - t₂ * ⅟4)) := by
  have s4 : s ^ 4 = 25 := by grind
  ring_nf
  rw [sq t₁, ht₁, sq t₂, ht₂, sq s, hs, s4]
  ring_nf
  rw [sq s, hs]
  have h1 : ⅟(4 : K) * 4 = 1 := invOf_mul_self _
  have h2 : ⅟(4 : K) ^ 2 * 16 = 1 := by grind only [(show (16 : K) = 4 * 4 by norm_num)]
  have h3 : ⅟(4 : K) ^ 3 * 64 = 1 := by grind only [(show (64 : K) = 16 * 4 by norm_num)]
  have h4 : ⅟(4 : K) ^ 4 * 256 = 1 := by grind only [(show (256 : K) = 64 * 4 by norm_num)]
  grind
