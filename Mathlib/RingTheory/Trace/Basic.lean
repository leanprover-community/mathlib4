...
theorem PowerBasis.trace_gen_eq_sum_roots [Nontrivial S] (pb : PowerBasis K S)
    (hf : (Polynomial.map (algebraMap K F) (minpoly K pb.gen)).Splits) :
...
theorem trace_gen_eq_sum_roots (x : L) (hf : (Polynomial.map (algebraMap K F) (minpoly K x)).Splits) :
...
theorem trace_eq_sum_embeddings_gen (pb : PowerBasis K L)
    (hE : (Polynomial.map (algebraMap K E) (minpoly K pb.gen)).Splits) (hfx : IsSeparable K pb.gen) :
