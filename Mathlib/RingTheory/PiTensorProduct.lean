...
lemma smul_tprod_mul_smul_tprod (r s : R) (x y : Π i, A i) :
  ...
  simp only [mul_def, map_smul, _root_.smul_apply, mul_tprod_tprod, mul_comm r s, mul_smul]
...
instance instAlgebra : Algebra R' (⨂[R] i, A i) where
  ...
  { toFun := (· • 1)
    ...
    map_mul' r s := show (r * s) • 1 = mul (r • 1) (s • 1) by
      rw [LinearMap.map_smul_of_tower, LinearMap.map_smul_of_tower, _root_.smul_apply, mul_comm,
        mul_smul]
      congr
  ...
  commutes' r x := by
    ...
    rw [LinearMap.map_smul_of_tower, LinearMap.map_smul_of_tower, _root_.smul_apply]
    change r • (1 * x) = r • (x * 1)
    rw [mul_one, one_mul]
  smul_def' r x := by
    simp only [RingHom.coe_mk, MonoidHom.coe_mk, OneHom.coe_mk]
    change _ = mul _ _
    rw [LinearMap.map_smul_of_tower, _root_.smul_apply]
    change _ = r • (1 * x)
    rw [one_mul]
