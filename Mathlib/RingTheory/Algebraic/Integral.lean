...
/- This does not hold if only `R` is assumed to be a domain but `S` is not: for example, let
`S = R[X] ⧸ (X² - X)` and let `A` be the subalgebra of `S[Y]` generated by `XY`.
`A` is algebraic over `S` because any element `∑ᵢ aᵢ(XY)ⁱ` is a root of the polynomial
`(X - 1)(Z - a₀)` in `S[Z]`, because `X(X - 1) = X² - X = 0` in `S`.
However, `XY` is a transcendental element in `A` over `R`, because `∑ᵢ aᵢ (XY)ⁱ = 0` in `S[Y]`
implies all `aᵢXⁱ = 0` in `S`, which implies `aᵢ = 0` in `R`.
This example is inspired by the comment
...
algebraic. This may not hold when `R` is not a domain: see https://mathoverflow.net/a/132192/ for
an example. -/
