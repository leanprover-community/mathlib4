...
theorem iff_map_piEvalRingHom [Finite ι] :
    ...
    M.map (MonoidHom.ofClass (Pi.evalRingHom R i))) S' :=
...
variable [∀ i, IsLocalization (M.map (MonoidHom.ofClass (Pi.evalRingHom R i))) (S i)]
...
lemma isUnit_piRingHom_algebraMap_comp_piEvalRingHom (y : M) :
  ...
  Pi.isUnit_iff.mpr fun i ↦ map_units _
    (⟨y.1 i, y, y.2, rfl⟩ : M.map (MonoidHom.ofClass (Pi.evalRingHom R i)))
...
lemma surjective_piRingHom_algebraMap_comp_piEvalRingHom
  ...
  by_cases h₀ : (0 : R i) ∈ (M.map (MonoidHom.ofClass (Pi.evalRingHom R i)))
...
lemma algebraMap_pi_surjective_of_isLocalization [∀ i, Ring.KrullDimLE 0 (R i)]
  ...
  intro s
  set S := fun (i : ι) => Localization (M.map (MonoidHom.ofClass (Pi.evalRingHom R i)))
  obtain ⟨r, hr⟩ :=
