...
variable [∀ i, IsLocalization (M.map (Pi.evalRingHom R i : ((i : ι) → R i) →* R i)) (S i)]
...
lemma isUnit_piRingHom_algebraMap_comp_piEvalRingHom (y : M) :
  ...
  Pi.isUnit_iff.mpr fun i ↦ map_units _
    (⟨y.1 i, y, y.2, rfl⟩ : M.map (Pi.evalRingHom R i : ((i : ι) → R i) →* R i))
...
lemma surjective_piRingHom_algebraMap_comp_piEvalRingHom
  ...
  by_cases h₀ : (0 : R i) ∈ (M.map (Pi.evalRingHom R i : ((i : ι) → R i) →* R i))
...
lemma algebraMap_pi_surjective_of_isLocalization [∀ i, Ring.KrullDimLE 0 (R i)]
  ...
  intro s
  set S := fun (i : ι) => Localization (M.map (Pi.evalRingHom R i : ((i : ι) → R i) →* R i))
  obtain ⟨r, hr⟩ :=
