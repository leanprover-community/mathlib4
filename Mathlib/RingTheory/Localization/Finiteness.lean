...
theorem IsLocalization.smul_mem_finsetIntegerMultiple_span [Algebra R S] [Algebra R S']
    [IsScalarTower R S S'] [IsLocalization (M.map (algebraMap R S : R →* S)) S'] (x : S)
    (s : Finset S') (hx : algebraMap S S' x ∈ Submodule.span R (s : Set S')) :
    ∃ m : M, m • x ∈ Submodule.span R
    (IsLocalization.finsetIntegerMultiple (M.map (algebraMap R S : R →* S)) s : Set S) := by
  let g : S →ₐ[R] S' :=
  ...
  obtain ⟨⟨_, a, ha₁, rfl⟩, ha₂⟩ :=
    (IsLocalization.eq_iff_exists (M.map (algebraMap R S : R →* S)) S').mp hx''
  use (⟨a, ha₁⟩ : M) * (⟨y', hy'⟩ : M)
  convert!
    (Submodule.span R
        (IsLocalization.finsetIntegerMultiple (M.map (algebraMap R S : R →* S)) s :
            Set S)).smul_mem
      a hx' using 1
