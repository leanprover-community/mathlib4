...
private def map_ideal (I : Ideal R) : Ideal S where
  ...
  add_mem' := by
    ...
    use ⟨Z, a'.2 * b'.2⟩
    simp +zetaDelta only [RingHom.map_add, Submodule.coe_mk, Submonoid.coe_mul, RingHom.map_mul]
  ...
  smul_mem' := by
    ...
    use ⟨Z, c'.2 * x'.2⟩
    simp +zetaDelta only [← hx, ← hc, smul_eq_mul, Submodule.coe_mk, Submonoid.coe_mul,
      RingHom.map_mul]
    ring
