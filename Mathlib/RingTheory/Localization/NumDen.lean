...
theorem exists_reduced_fraction (x : K) :
  ...
  -- ⊢ ↑(algebraMap A K) (c' * b') * mk' K a' { val := b', property := b'_nonzero } …
  simp only [Subtype.coe_mk, RingHom.map_mul, Algebra.smul_def] at *
  -- ⊢ ↑(algebraMap A K) c' * ↑(algebraMap A K) b' * mk' K a' { val := b', property …
...
theorem isInteger_of_isUnit_den {x : K} (h : IsUnit (den A x : A)) : IsInteger A x := by
  ...
  -- ⊢ (↑(algebraMap A K) ↑(den A x))⁻¹ * ↑(algebraMap A K) (num A x) = mk' K (num  …
  apply mul_left_cancel₀ d_ne_zero
  -- ⊢ ↑(algebraMap A K) ↑(den A x) * ((↑(algebraMap A K) ↑(den A x))⁻¹ * ↑(algebra …
