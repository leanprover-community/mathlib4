...
theorem selfZPow_sub_natCast {n m : ℕ} :
  ...
  · rw [← neg_sub, ← Int.ofNat_sub (ge_of_not_le h), selfZPow_neg_natCast,
      IsLocalization.mk'_eq_iff_eq]
    simp [Submonoid.pow_apply, ← pow_add, Nat.sub_add_cancel (ge_of_not_le h)]
...
theorem selfZPow_mul_neg (d : ℤ) : selfZPow x B d * selfZPow x B (-d) = 1 := by
  ...
  · rw [selfZPow_of_nonpos x B hd, selfZPow_of_nonneg, ← map_pow, Int.natAbs_neg,
    ...
    rwa [neg_neg]
  · rw [selfZPow_of_nonneg x B (ge_of_not_le hd), selfZPow_of_nonpos, ← map_pow, Int.natAbs_neg,
