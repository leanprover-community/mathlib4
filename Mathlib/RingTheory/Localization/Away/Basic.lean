...
noncomputable def mapₐ (f : A →ₐ[R] B) (a : A) [Away a Aₚ]
    [Away (f a) Bₚ] : Aₚ →ₐ[R] Bₚ :=
  ⟨map Aₚ Bₚ f.toRingHom a, by
    intro r
    simp
    rw [IsScalarTower.algebraMap_eq R A Aₚ]
    simp
    rw [← IsLocalization.mk'_one (M := Submonoid.powers a) Aₚ]
