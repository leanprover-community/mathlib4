...
* `IsLocalization.Away.prime_of_prime_in_localization`: Nagata's criterion — if `p` is prime,
  `x` is irreducible, and `algebraMap R (Away p) x` is prime, then `x` is prime.
