...
abbrev Away (S : Type*) [CommSemiring S] [SMul R S] [Algebra R S] :=
...
noncomputable def awayToAwayRight (y : R) [SMul R P] [Algebra R P] [IsLocalization.Away (x * y) P] : S →+* P :=
  lift x <|
...
variable (S) (Q : Type*) [CommSemiring Q] [SMul P Q] [Algebra P Q]
...
variable (x : R)
variable (B : Type*) [CommRing B] [SMul R B] [Algebra R B] [IsLocalization.Away x B]
...
theorem selfZPow_mul_neg (d : ℤ) : selfZPow x B d * selfZPow x B (-d) = 1 := by
  ...
  · erw [selfZPow_of_nonneg x B (le_of_not_le hd), selfZPow_of_nonpos, ← map_pow, Int.natAbs_neg,
      @IsLocalization.mk'_spec' R _ (Submonoid.powers x) B _ _ _ _  1 (Submonoid.pow x d.natAbs),
      map_one]
