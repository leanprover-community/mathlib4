...
/-- `IsFractionRing R K` states `K` is the ring of fractions of a ring `R`. -/
...
variable (A K) in
/-- If `A` is a ring with fraction field `K`, then the subfield of `K` generated by the image of
`algebraMap A K` is equal to the whole field `K`. -/
...
/-- If `A` is a ring with fraction field `K`, `L` is a field, `g : A →+* L` lifts to `f : K →+* L`,
then the image of `f` is the subfield generated by the image of `g`. -/
...
/-- Given a ring `A` with field of fractions `K`,
...
/-- Given rings `A, B` where `B` is an integral domain, with fraction rings `K`, `L`
