...
then the image of `f` is the field generated by `s`. -/
...
theorem lift_unique'
    ...
    {f1 f2 : K →+* L}
    (hf1 : ∀ x, f1 (algebraMap A K x) = g x)
    (hf2 : ∀ x, f2 (algebraMap A K x) = g x) : f1 = f2 :=
  Eq.trans (lift_unique hg hf1).symm (lift_unique hg hf2)
