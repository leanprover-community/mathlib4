...
* `IsFractionRing R K` expresses that `K` is a total ring of fractions of `R`, as an abbreviation of
  `IsLocalization (.regulars R) K`
...
variable {A : Type*} [CommSemiring A] (K : Type*)
...
instance {R : Type*} [Semifield R] : IsFractionRing R R :=
  IsLocalization.at_units _ (fun _ ↦ isUnit_of_mem_nonZeroDivisors ∘ IsRegular.mem_nonZeroDivisors)

theorem IsFractionRing.of_semifield [Semifield K] [Algebra R K] [FaithfulSMul R K]
    (surj : ∀ z : K, ∃ x y, z = algebraMap R K x / algebraMap R K y) :
    IsFractionRing R K :=
  have := (FaithfulSMul.algebraMap_injective R K).isDomain
  IsLocalization.of_semifield (fun h ↦ one_ne_zero (h.1 (a₁ := 1) (a₂ := 0) (by simp))) fun z ↦ by
    have ⟨x, y, eq⟩ := surj z
    obtain rfl | hy := eq_or_ne y 0
    · obtain rfl : z = 0 := by simpa using eq
      exact ⟨0, 1, by simp⟩
    · exact ⟨x, y, isRegular_iff_ne_zero.mpr hy, eq⟩

/-- The cast from `Int` to `Rat` as a `FractionRing`. -/
instance Rat.isFractionRing : IsFractionRing ℤ ℚ :=
  .of_semifield _ _ fun q ↦ ⟨_, _, q.num_div_den.symm⟩

/-- As a corollary, `Rat` is also a localization at only positive integers. -/
instance : IsLocalization (Submonoid.pos ℤ) ℚ := .of_semifield
  (by simp) fun q ↦ ⟨_, _, show (0 : ℤ) < _ from mod_cast q.den_pos, q.num_div_den.symm⟩

/-- `NNRat` is the ring of fractions of `Nat`. -/
instance NNRat.isFractionRing : IsFractionRing ℕ ℚ≥0 :=
  .of_semifield _ _ fun q ↦ ⟨_, _, q.num_div_den.symm⟩
...
@[deprecated (since := "2026-01-27")] alias of_field := of_semifield

variable {R K}

section CommSemiring

variable [CommSemiring K] [Algebra R K] [IsFractionRing R K] [Algebra A K] [IsFractionRing A K]

theorem to_map_eq_zero_iff {x : R} : algebraMap R K x = 0 ↔ x = 0 :=
  IsLocalization.to_map_eq_zero_iff _ (regulars_le_nonZeroDivisors R)

variable (R K)

protected theorem injective : Function.Injective (algebraMap R K) :=
  IsLocalization.injectiveₛ _ le_rfl

include R in
theorem regulars_eq_isUnit : .regulars K = IsUnit.submonoid K := by
  refine le_antisymm (fun x hx ↦ ?_) (isUnit_le_regulars K)
  have ⟨r, eq⟩ := surj (.regulars R) x
  let r' : Submonoid.regulars R := ⟨r.1, .of_injective (IsFractionRing.injective R K)
    (eq ▸ mul_mem hx (map_units ..).isRegular)⟩
...
noncomputable def algEquiv (L) [CommSemiring L] [Algebra K L] [IsFractionRing K L] : K ≃ₐ[K] L :=
  atUnits K _ (regulars_eq_isUnit R K).le

include R in
theorem idem : IsFractionRing K K := IsLocalization.self (regulars_eq_isUnit R K).le

/-- Taking fraction ring is idempotent: a fraction ring of a fraction ring of `R` is
itself a fraction ring of `R`. -/
theorem trans (L) [CommSemiring L] [Algebra K L] [IsFractionRing K L] [Algebra R L]
...
theorem self_iff_regulars_eq_isUnit : IsFractionRing R R ↔ .regulars R = IsUnit.submonoid R where
  mp _ := regulars_eq_isUnit R R
  mpr h := IsLocalization.self h.le

theorem self_iff_regulars_le_isUnit : IsFractionRing R R ↔ .regulars R ≤ IsUnit.submonoid R := by
  rw [self_iff_regulars_eq_isUnit, le_antisymm_iff,
    and_iff_left (isUnit_le_regulars R)]

theorem self_iff_bijective : IsFractionRing R R ↔ Function.Bijective (algebraMap R K) where
  mp h := (atUnits R _ <| self_iff_regulars_le_isUnit.mp h).bijective
...
protected theorem to_map_ne_zero_of_mem_nonZeroDivisors [Nontrivial R] {x : R}
  ...
  IsLocalization.to_map_ne_zero_of_mem_nonZeroDivisors _ (regulars_le_nonZeroDivisors R) hx

variable (A) [IsDomain A]

include A in
/-- A `CommSemiring` `K` which is the localization of an integral domain `R` at `R - {0}` is an
integral domain. -/
protected theorem isDomain : IsDomain K :=
  isDomain_of_le_nonZeroDivisors _ (regulars_le_nonZeroDivisors A)

/-- The inverse of an element in the field of fractions of an integral domain. -/
protected noncomputable irreducible_def inv (z : K) : K := open scoped Classical in
  if h : z = 0 then 0
  else
    mk' (M := .regulars A) K (sec (.regulars A) z).2
      ⟨(sec (.regulars A) z).1,
        isRegular_iff_ne_zero.2 fun h0 =>
          h <| eq_zero_of_fst_eq_zero (sec_spec _ z) h0⟩

protected theorem mul_inv_cancel (x : K) (hx : x ≠ 0) : x * IsFractionRing.inv A x = 1 := by
  rw [IsFractionRing.inv, dif_neg hx, ←
    IsUnit.mul_left_inj
      (map_units (M := .regulars A) K
        ⟨(sec _ x).1,
          isRegular_iff_ne_zero.2 fun h0 =>
            hx <| eq_zero_of_fst_eq_zero (sec_spec (.regulars A) x) h0⟩),
    one_mul, mul_assoc]
  rw [mk'_spec, ← eq_mk'_iff_mul_eq]
  exact (mk'_sec _ x).symm

/-- A `CommSemiring` `K` which is the localization of an integral domain `R` at `R - {0}` is a
semifield. See note [reducible non-instances]. -/
noncomputable abbrev toSemifield : Semifield K where
...
lemma surjective_iff_isField [IsDomain R] : Function.Surjective (algebraMap R K) ↔ IsField R where
  mp h := (RingEquiv.ofBijective (algebraMap R K)
      ⟨IsFractionRing.injective R K, h⟩).toMulEquiv.isField (IsFractionRing.toSemifield R).toIsField
  mpr h :=
    letI := h.toSemifield
    (IsLocalization.atUnits R (.regulars R) (S := K)
      fun _ hx ↦ Ne.isUnit (isRegular_iff_ne_zero.mp hx)).surjective

end CommSemiring

variable {B : Type*} [CommSemiring B] [IsDomain B] [Semifield K] {L : Type*} [Semifield L]
  [Algebra A K] [IsFractionRing A K] {g : A →+* L}

open Submonoid (regulars)

theorem mk'_mk_eq_div {r s} (hs : s ∈ regulars A) :
    mk' K r ⟨s, hs⟩ = algebraMap A K r / algebraMap A K s :=
  haveI := (algebraMap A K).domain_nontrivial
  mk'_eq_iff_eq_mul.2 <|
    (div_mul_cancel₀ (algebraMap A K r)
        (IsFractionRing.to_map_ne_zero_of_mem_nonZeroDivisors hs.mem_nonZeroDivisors)).symm

@[simp]
theorem mk'_eq_div {r} (s : Submonoid.regulars A) :
    mk' K r s = algebraMap A K r / algebraMap A K s :=
  mk'_mk_eq_div s.2

theorem div_surjective (z : K) :
    ∃ x y : A, y ∈ Submonoid.regulars A ∧ algebraMap _ _ x / algebraMap _ _ y = z :=
  let ⟨x, ⟨y, hy⟩, h⟩ := exists_mk'_eq (.regulars A) z
...
theorem mk'_eq_zero_iff_eq_zero [Algebra R K] [IsFractionRing R K] {x : R} {y : regulars R} :
    mk' K x y = 0 ↔ x = 0 := by
  haveI := (algebraMap R K).domain_nontrivial
  simp [y.2.ne_zero]

theorem mk'_eq_one_iff_eq {x : A} {y : regulars A} : mk' K x y = 1 ↔ x = y := by
  haveI := (algebraMap A K).domain_nontrivial
  refine ⟨?_, fun hxy => by rw [hxy, mk'_self']⟩
  intro hxy
  have hy : (algebraMap A K) ↑y ≠ (0 : K) :=
    IsFractionRing.to_map_ne_zero_of_mem_nonZeroDivisors y.2.mem_nonZeroDivisors
  rw [IsFractionRing.mk'_eq_div, div_eq_one_iff_eq hy] at hxy
  exact IsFractionRing.injective A K hxy

section commutes

variable [Algebra A B] {K₁ K₂ : Type*} [Semifield K₁] [Semifield K₂] [Algebra A K₁] [Algebra A K₂]
  [IsFractionRing A K₁] {L₁ L₂ : Type*} [Semifield L₁] [Semifield L₂] [Algebra B L₁] [Algebra B L₂]
...
field hom sending `z : K` to `g x * (g y)⁻¹`, where `(x, y) : A × (regulars A)` are
such that `z = f x * (f y)⁻¹`. -/
noncomputable def lift (hg : Injective g) : K →+* L :=
  IsLocalization.lift fun y : regulars A => isUnit_map_of_injective hg ⟨y, y.2.mem_nonZeroDivisors⟩
...
noncomputable def liftAlgHom : K →ₐ[R] L :=
  IsLocalization.liftAlgHom fun y : regulars A =>
    isUnit_map_of_injective hg ⟨y, y.2.mem_nonZeroDivisors⟩
...
variable {A K L : Type*} [CommRing A] [Field K] [Field L] [Algebra A K] [IsFractionRing A K]
variable {g : A →+* L}
...
`x : A, y ∈ regulars A`. -/
theorem lift_mk' (hg : Injective g) (x) (y : regulars A) :
...
sending `z : K` to `g (j x) * (g (j y))⁻¹`, where `(x, y) : A × (regulars A)` are
such that `z = f x * (f y)⁻¹`. -/
noncomputable def map {A B K L : Type*} [CommRing A] [CommRing B] [IsCancelMulZero B] [CommRing K]
    [Algebra A K] [IsFractionRing A K] [CommRing L] [Algebra B L] [IsFractionRing B L] {j : A →+* B}
    (hj : Injective j) : K →+* L :=
  IsLocalization.map L j
    (show regulars A ≤ (regulars B).comap j by
      nontriviality A
      exact fun x hx ↦ .of_ne_zero <| (map_ne_zero_iff j hj).mpr hx.ne_zero)

section ringEquivOfRingEquiv

variable {A K B L : Type*} [CommSemiring A] [CommSemiring B] [CommSemiring K] [CommSemiring L]
...
noncomputable def ringEquivOfRingEquiv : K ≃+* L :=
  IsLocalization.ringEquivOfRingEquiv K L h (MulEquivClass.map_regulars h)
...
variable {R A K B L : Type*}
  [CommSemiring R] [CommSemiring A] [CommSemiring B] [CommSemiring K] [CommSemiring L]
...
noncomputable def algEquivOfAlgEquiv : K ≃ₐ[R] L :=
  IsLocalization.algEquivOfAlgEquiv K L h (MulEquivClass.map_regulars h)
...
variable {A B C D : Type*}
  [CommSemiring A] [CommSemiring B] [CommSemiring C] [CommSemiring D]
  [Algebra A B] [Algebra A C] [Algebra A D]
  (FA FB FC FD : Type*) [Semifield FA] [Semifield FB] [Semifield FC] [Semifield FD]
...
variable {A B : Type*} [CommSemiring A] [CommSemiring B] [Algebra A B]
  (K L : Type*) [Semifield K] [Semifield L]
...
theorem isFractionRing_iff_of_base_ringEquiv (h : R ≃+* P) :
  ...
  convert isLocalization_iff_of_base_ringEquiv (regulars R) S h
  exact (MulEquivClass.map_regulars h).symm

variable (R S : Type*) [CommSemiring R] [CommSemiring S] [Algebra R S] [IsFractionRing R S]

theorem nontrivial_iff_nontrivial : Nontrivial R ↔ Nontrivial S where
  mp _ := (IsFractionRing.injective R S).nontrivial
  mpr _ := Module.nontrivial R S

protected theorem nontrivial [Nontrivial R] : Nontrivial S :=
  (nontrivial_iff_nontrivial R S).mp ‹_›
...
instance [Nontrivial R] : Nontrivial (FractionRing R) := IsFractionRing.nontrivial R _

variable [IsDomain A]

noncomputable instance : Semifield (FractionRing A) := IsFractionRing.toSemifield A
...
variable [Semifield K] [Algebra R K] [FaithfulSMul R K]
...
type and `K`. -/
noncomputable def algEquiv (K : Type*) [CommSemiring K] [Algebra A K] [IsFractionRing A K] :
    FractionRing A ≃ₐ[A] K :=
  Localization.algEquiv (.regulars A) K
...
instance (k K : Type*) [Semifield k] [Semifield K] [Algebra A k] [Algebra A K] [Algebra k K]
...
theorem nonZeroDivisors_eq_isUnit : K⁰ = IsUnit.submonoid K := by
  rw [← regulars_eq_isUnit R K, regulars_eq_nonZeroDivisors]

variable {R}

theorem self_iff_nonZeroDivisors_eq_isUnit : IsFractionRing R R ↔ R⁰ = IsUnit.submonoid R := by
  rw [self_iff_regulars_eq_isUnit, regulars_eq_nonZeroDivisors]

theorem self_iff_nonZeroDivisors_le_isUnit : IsFractionRing R R ↔ R⁰ ≤ IsUnit.submonoid R := by
  rw [self_iff_regulars_le_isUnit, regulars_eq_nonZeroDivisors]
...
See note [reducible non-instances]. -/
@[stacks 09FJ]
noncomputable abbrev toField [IsDomain R] : Field K where
  __ := ‹CommRing K›
  __ := toSemifield R
  qsmul := _
  qsmul_def _ _ := rfl
...
noncomputable instance FractionRing.field (A) [CommRing A] [IsDomain A] : Field (FractionRing A) :=
