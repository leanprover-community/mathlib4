...
variable (S : Type*) [CommRing S] {ι : Type*} (R' S' : ι → Type*) [∀ i, CommRing (R' i)]
  ...
  {M' : ∀ i, Submonoid (R' i)} [isloc : ∀ i, IsLocalization (M' i) (S' i)]
...
theorem injective_of_product_of_localizations [inst : Fintype ι] [DecidableEq ι]
  ...
  set R := (∀i, R' i)
  set P := (∀i, S' i)
  set f := sloc.lift (isUnit_of_product_of_localizations R' S' h ) -- S to P
  set f' : ∀i, R →+* S' i := fun i ↦ RingHom.comp (algebraMap (R' i) (S' i)) (Pi.evalRingHom R' i)
  set f'' :  R →+* P := Pi.ringHom f' -- R to P
  refine (injective_iff_map_eq_zero _ ).mpr ?_
  intro s s₀
  rw [← sloc.mk'_sec S s] at s₀ ⊢
  have h₁ (y : M) (i : ι) : y.1 i ∈ M' i := by
    rw [h i]
    exact Submonoid.mem_map_of_mem (Pi.evalRingHom R' i) y.2
  have (x : R × M) : f (mk' S x.1 x.2) = fun i ↦ (isloc i).mk' (S' i) (x.1 i) ⟨x.2.1 i, h₁ x.2 i⟩
    := by
