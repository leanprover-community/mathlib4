...
theorem nilpotent_iff_mem_maximal {x : _} :
  ...
  exact ⟨(· (LocalRing.maximalIdeal _) (Ideal.IsMaximal.isPrime' _)), fun _ J _ =>
    by simpa [prime_unique hMin J]⟩

theorem nilpotent_iff_not_unit {x : Localization I.primeCompl} :
    IsNilpotent x ↔ x ∈ nonunits _ := by
  simpa only [← LocalRing.mem_maximalIdeal] using AtMinimalPrime.nilpotent_iff_mem_maximal hMin
