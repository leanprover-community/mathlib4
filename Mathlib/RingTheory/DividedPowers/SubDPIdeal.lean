...
/- We now provide the divided power structure on a quotient ring in two settings:
* The case of a surjective `RingHom`.
* The specific construction for `Ideal.Quotient.mk`. -/
...
/-In this namespace we define divided powers on a quotient map via a surjective morphism. -/
...
/-- The definition of divided powers on the codomain `B` of a surjective ring homomorphism
  ...
  `IsSubDPIdeal hI (RingHom.ker f ⊓ I)`, `dpow_quot_eq` proves that no choices are involved. -/
noncomputable def dpow : ℕ → B → B := fun n ↦
  Function.extend (fun a ↦ f a : I → B) (fun a ↦ f (hI.dpow n a) : I → B) 0

variable {f} (hf : Function.Surjective f) (hIf : IsSubDPIdeal hI (RingHom.ker f ⊓ I))
...
theorem dpow_apply' (hIf : IsSubDPIdeal hI (RingHom.ker f ⊓ I)) {n : ℕ} {a : A} (ha : a ∈ I) :
  ...
  apply hIf (by simp only [Submodule.coe_mem]) ha
...
theorem dividedPowers_unique (hquot : DividedPowers (I.map f))
    (hm : DividedPowers.IsDPMorphism hI hquot f) :
