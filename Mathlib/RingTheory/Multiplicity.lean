...
lemma multiplicity_le_multiplicity_iff {a b c d : α} :
...
lemma multiplicity_eq_multiplicity_iff {a b c d : α} :
    multiplicity a b = multiplicity c d ↔ ∀ n : ℕ, a ^ n ∣ b ↔ c ^ n ∣ d :=
  ⟨fun h n =>
...
@[simp]
lemma addValuation_apply {hp : Prime p} {r : R} : addValuation hp r = multiplicity p r :=
  rfl
