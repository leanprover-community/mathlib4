...
/-!
# The property of exactness om AssociatedGradedAddGroupHom

In this file, we define the concept of exhaustive filtrations.

We also prove a AssociatedGradedAddGroupHom sequence is exact iff each GradedPieceHom is exact.

And when a sequence is strict exact, the corresponding AssociatedGradedAddGroupHom sequence is also
exact.
...
theorem AssociatedGradedAddGroupHom_eq_zero_iff_GradedPieceHom_eq_zero
    (x : AssociatedGraded FS FS_lt) : Gr[g] x = 0 ↔ ∀ i : ι, Gr(i)[g] (x i) = 0 :=
  mem_map_ker_iff (fun i ↦ Gr(i)[g]) x

theorem GradedPieceHom_eq_zero_iff_AssociatedGradedAddGroupHom_of_eq_zero {i : ι} [DecidableEq ι]
    (u : GradedPiece FS FS_lt i): Gr(i)[g] u = 0 ↔ Gr[g] (of (GradedPiece FS FS_lt) i u) = 0 :=
  of_mem_map_ker_iff (fun i ↦ Gr(i)[g]) i u

theorem AssociatedGradedAddGroupHom_in_range_iff_GradedPieceHom_in_range
    (m : AssociatedGraded FS FS_lt) : m ∈ Gr[f].range ↔ ∀ i : ι, m i ∈ Set.range Gr(i)[f] :=
  mem_map_range_iff (fun i ↦ Gr(i)[f]) m

theorem GradedPieceHom_in_range_iff_AssociatedGradedAddGroupHom_of_in_range {i : ι} [DecidableEq ι]
...
theorem GradedPieceHom_ker_eq_GradedPieceHom_range (i : ι) (Gexact : Function.Exact Gr[f] Gr[g])
    [DecidableEq ι] : Gr(i)[g].ker = Gr(i)[f].range := by
  ext u
  apply Iff.trans (GradedPieceHom_eq_zero_iff_AssociatedGradedAddGroupHom_of_eq_zero g u)
  rw [Gexact ((of (GradedPiece FS FS_lt) i) u)]
  exact (GradedPieceHom_in_range_iff_AssociatedGradedAddGroupHom_of_in_range f u).symm

theorem GradedPieceHom_exact_of_AssociatedGradedAddGroupHom_exact [DecidableEq ι] :
...
lemma exact_component_of_strict_exact_component (fstrict : f.IsStrict) (gstrict : g.IsStrict)
  ...
  · simp only [← GradedPiece.mk_eq, GradedPieceHom_apply_mk_eq_mk_piece_wise_hom]
...
theorem exact_of_strict_exact (fstrict : f.IsStrict) (gstrict : g.IsStrict)
  ...
  rw [AssociatedGradedAddGroupHom_eq_zero_iff_GradedPieceHom_eq_zero, this,
      AssociatedGradedAddGroupHom_in_range_iff_GradedPieceHom_in_range]
  exact forall_congr'
