...
lemma eval_zero_comp_det :
    eval 0 (comp m m n n R[X] <| f.polyToMatrix.mapMatrix <| cornerAddX M k).det =
      (comp m m n n R <| f.mapMatrix M).det := by
  simp_rw [← coe_evalRingHom, RingHom.map_det, ← compRingEquiv_apply, ← RingEquiv.coe_toRingHom,
    ← RingHom.comp_apply, ← RingHom.comp_assoc, evalRingHom_mapMatrix_comp_compRingEquiv,
    RingHom.comp_assoc, RingHom.mapMatrix_comp, evalRingHom_mapMatrix_comp_polyToMatrix,
    ← RingHom.mapMatrix_comp, RingHom.comp_apply]
  congr with i j
...
variable {M f} in
lemma det_det_aux
    (ih : ∀ M, (f M.det).det = ((f.mapMatrix M).comp {a // (a = k) = False} _ n n R).det) :
...
theorem Matrix.det_det [Fintype m] [Fintype n] (f : S →+* Matrix n n R) :
    (f M.det).det = ((f.mapMatrix M).comp m m n n R).det := by
  ...
  have : (f' M'.det).det = ((f'.mapMatrix M').comp m m n n R[X]).det := by
