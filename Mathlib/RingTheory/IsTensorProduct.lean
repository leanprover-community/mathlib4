...
theorem Algebra.IsPushout.symm (h : Algebra.IsPushout R S R' S') : Algebra.IsPushout R R' S S' := by
  ...
  let e : R' ⊗[R] S ≃ₗ[R'] S' := by
    ...
    · intro x y
      simp only [smul_tmul', smul_eq_mul, TensorProduct.comm_tmul, smul_def, TensorProduct.algebraMap_apply,
        id.map_eq_id, RingHom.id_apply, TensorProduct.tmul_mul_tmul, one_mul, h.1.equiv_tmul, AlgHom.toLinearMap_apply,
        _root_.map_mul, IsScalarTower.coe_toAlgHom']
      ring
    · intro x y hx hy
