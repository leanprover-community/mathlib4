...
* `isJacobson_iff_Inf_maximal` is the equivalence between conditions 1 and 2 above.
* `isJacobson_of_surjective` says that if `R` is a Jacobson ring and `f : R →+* S` is surjective,
  then `S` is also a Jacobson ring
* `isJacobson_mv_polynomial` says that multi-variate polynomials over a Jacobson ring are Jacobson.
## Tags
...
theorem isJacobson_localization [H : IsJacobson R] : IsJacobson S := by
  ...
  have :
    sInf { I : Ideal R | comap (algebraMap R S) P' ≤ I ∧ I.IsMaximal ∧ y ∉ I } ≤
        ...
        RingHom.isIntegral_trans _ _
          (RingHom.isIntegral_of_surjective _
            (IsLocalization.surjective_quotientMap_of_maximal_of_localization (Submonoid.powers x)
              Rₘ (by rwa [comap_comap, hcomm, ← bot_quotient_isMaximal_iff])))
  ...
  · rw [jacobson_eq_iff_jacobson_quotient_eq_bot]
    ...
    refine'
      @jacobson_bot_of_integral_localization
        (R[X] ⧸ P) _ _
        (R ⧸ P') _ _ hR'
        Rₘ -- (Localization.Away x)
        Sₘ -- (Localization ((Submonoid.powers x).map φ : Submonoid (R[X] ⧸ P)))
        _ _
        φ hφ x hx _ _ _ _
        _
    -- -- `convert` is noticeably faster than `exact` here:
    -- convert isIntegral_is_localization_polynomial_quotient P p pP
...
-- #align ideal.polynomial.is_jacobson_polynomial_of_domain Ideal.Polynomial.isJacobson_polynomial_of_domain
...
theorem isJacobson_polynomial_iff_isJacobson : IsJacobson R[X] ↔ IsJacobson R := by
  ...
  exact
    isJacobson_of_surjective
      ⟨eval₂RingHom (RingHom.id _) 1, fun x =>
...
lemma isMaximal_comap_C_of_isJacobson' {P : Ideal R[X]} (hP : IsMaximal P) : (P.comap (C : R →+* R[X])).IsMaximal := by
  haveI := hP
...
theorem comp_C_integral_of_surjective_of_jacobson {S : Type _} [Field S] (f : R[X] →+* S)
  ...
  rw [← hfg] at hf
  norm_num at hf -- Porting note: added
...
private lemma aux_IH₃ {R S T: Type _} [CommRing R] [CommRing S] [CommRing T]
  ...
  (P : Ideal T) :
  RingHom.IsIntegral (Ideal.quotientMap P u.toAlgHom.toRingHom le_rfl) := by
    rw [isIntegral_quotientMap_iff]
    apply RingHom.isIntegral_of_surjective
    simp only [AlgEquiv.toAlgHom_eq_coe, AlgHom.toRingHom_eq_coe, coe_comp, coe_coe,
      AlgHom.coe_coe, EquivLike.surjective_comp]
    exact Quotient.mk_surjective

private lemma aux_IH₂ {R : Type _} [CommRing R] [IsJacobson R] (n : ℕ)
  (P : Ideal (MvPolynomial (Fin (n.succ)) R)) (hP : IsMaximal P) :
  RingHom.IsIntegral (Quotient.mk (comap (MvPolynomial.finSuccEquiv R n).symm.toAlgHom.toRingHom P) ∘ᵣ (Polynomial.C : MvPolynomial (Fin n) R
    →+* (MvPolynomial (Fin n) R) [X])) := by
    haveI isJac : IsJacobson (MvPolynomial (Fin n) R) := by infer_instance
    let P' := comap (MvPolynomial.finSuccEquiv R n).symm.toAlgHom.toRingHom P
    haveI hP' : IsMaximal P' := by
