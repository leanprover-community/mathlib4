...
lemma quadratic_vieta_aroots {T S : Type*} [CommRing T] [CommRing S] [IsDomain S] [Algebra T S]
...
private lemma roots_of_ne_zero_of_vieta {R : Type*} [CommRing R] [IsDomain R] {a b c x1 x2 : R}
...
/-- **Vieta's formula** for quadratic in term of `Polynomial.roots`. -/
lemma quadratic_vieta_iff_of_ne_zero {R : Type*} [CommRing R] [IsDomain R] {a b c x1 x2 : R}
    (ha : a ≠ 0) : (C a * X ^ 2 + C b * X + C c).roots = {x1, x2} ↔
      b = -a * (x1 + x2) ∧ c = a * x1 * x2 := by
  refine ⟨quadratic_vieta, roots_of_ne_zero_of_vieta ha⟩

lemma quadratic_vieta_aroots_iff_of_ne_zero {T S : Type*} [CommRing T] [CommRing S] [IsDomain S]
...
/-- **Vieta's formula** for quadratic in term of `Polynomial.roots`. -/
lemma quadratic_vieta_iff_of_ne_zero' {R : Type*} [Field R] {a b c x1 x2 : R} (ha : a ≠ 0) :
