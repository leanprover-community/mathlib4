...
The following results generalize the binary coprime kernel decomposition above to finite products
of pairwise coprime polynomials, using `smeval` instead of `aeval`.
-/

variable {ι : Type*}

/-- If `i ∈ s`, then `ker(P i |>.smeval f) ≤ ker(s.prod P |>.smeval f)`. -/
theorem ker_smeval_le_ker_smeval_prod (f : Module.End R M)
...
theorem ker_smeval_prod_le_iSup_ker_smeval (f : Module.End R M)
    (s : Finset ι) (P : ι → R[X]) (hs : s.Nonempty)
  ...
  have h' : Pairwise (Function.onFun IsCoprime fun i : s ↦ P i) := by
    intro ⟨i, hi⟩ ⟨j, hj⟩ hij
  ...
  have hx_eq : x = ∑ i ∈ s, ((Q i * ∏ j ∈ s \ {i}, P j).smeval f) x := by
    rw [← LinearMap.sum_apply, ← Polynomial.smeval_finset_sum_apply, hQ_smeval,
      Module.End.one_eq_id, LinearMap.id_apply]
  rw [hx_eq]
  -- Each summand belongs to the corresponding kernel
  refine Submodule.sum_mem_biSup fun i hi => ?_
  rw [LinearMap.mem_ker, ← Module.End.mul_apply, ← Polynomial.smeval_mul]
...
theorem ker_smeval_prod_eq_iSup_ker_smeval (f : Module.End R M)
    (s : Finset ι) (P : ι → R[X])
    (h : (s : Set ι).Pairwise <| IsCoprime.onFun P) :
    (s.prod P |>.smeval f).ker = ⨆ i ∈ s, (P i |>.smeval f).ker := by
  classical
