...
lemma swap_mem_stabilizer {α : Type*} [DecidableEq α] (S : Set α) (a b : α) :
    swap a b ∈ MulAction.stabilizer (Perm α) S ↔ (a ∈ S ↔ b ∈ S) := by
  rw [MulAction.mem_stabilizer_iff, Set.ext_iff, ← swap_inv]
  simp_rw [Set.mem_inv_smul_set_iff, Perm.smul_def, swap_apply_def]
  constructor
  · intro h
    specialize h a
    rwa [if_pos rfl, Iff.comm] at h
  · intros; split_ifs <;> simp [*]

/- A transitive permutation group generated by transpositions must be the entire symmetric group -/
lemma closure_of_isSwap_of_isPretransitive {α : Type*} [DecidableEq α] [Finite α]
    (S : Set (Perm α)) (hS : ∀ σ ∈ S, σ.IsSwap) [MulAction.IsPretransitive (Subgroup.closure S) α] :
  ...
  · rw [Subgroup.closure_le, Set.not_subset] at key0
    ...
    by_cases hx : x ∈ T
    · exact ⟨x, y, Subgroup.subset_closure hτ, hx, fun hy ↦ hτ'.mpr hy hx⟩
    · exact ⟨y, x, swap_comm x y ▸ Subgroup.subset_closure hτ, hτ'.mp hx, hx⟩
...
lemma keylemma' {α β : Type*} [Group α] [MulAction α β] [DecidableEq β] [Finite β]
    [h : MulAction.IsPretransitive α β] (S : Set α)
  ...
  have := MulAction.IsPretransitive.of_compHom (α := β) (MulAction.toPermHom α β).rangeRestrict
...
theorem X_pow_sub_X_sub_one_gal :
  ...
  let R := 𝓞 K
  let S0 : Set f.Gal := ⋃ (q : Ideal R) (hq : q.IsMaximal), {σ | ∀ x : R, res σ.toRingEquiv x - x ∈ q}
  ...
  · rintro σ ⟨hσ, hσ1 : σ ≠ 1⟩
    ...
    obtain ⟨q, hσ⟩ := hσ
    rw [Set.mem_iUnion] at hσ
    obtain ⟨hq, hσ : ∀ x : R, res σ.toRingEquiv x - x ∈ q⟩ := hσ
    let F := R ⧸ q
