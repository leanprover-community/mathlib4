...
lemma keylemma1 {α : Type*} [Finite α] (P : Set α → Prop) (S0 : Set α)
    (h0 : P S0) (ih : ∀ S ≠ Set.univ, P S → ∃ a ∉ S, P ({a} ∪ S)) : P Set.univ := by
  refine' Finite.to_wellFoundedGT.wf.induction_bot' _ h0
  intro T hT hT'
  obtain ⟨a, ha, ha'⟩ := ih T hT hT'
  refine' ⟨({a} ∪ T : Set α), _, ha'⟩
  exact Set.ssubset_insert ha

open Pointwise

/- A transitive group generated by transpositions must be all of the symmetric group. -/
lemma keylemma {α : Type*} [DecidableEq α] [Finite α] (S : Set (Equiv.Perm α)) (hS : ∀ σ ∈ S, σ.IsSwap)
    [MulAction.IsPretransitive (Subgroup.closure S) α] : Subgroup.closure S = ⊤ := by
  rcases isEmpty_or_nonempty α with _ | ⟨⟨a0⟩⟩
  · apply Subsingleton.elim
  let P : Set α → Prop := fun T ↦ ∀ x ∈ T, ∀ y ∈ T, x ≠ y → Equiv.swap x y ∈ Subgroup.closure S
  suffices : P Set.univ
  · rw [eq_top_iff, ← Equiv.Perm.closure_isSwap, Subgroup.closure_le]
    rintro - ⟨x, y, hxy, rfl⟩
    exact this x (Set.mem_univ x) y (Set.mem_univ y) hxy
  apply keylemma1 P ∅ (by simp)
  intro T hT hT'
  ...
  have key : ∃ a b, Equiv.swap a b ∈ Subgroup.closure S ∧ a ∈ T ∧ b ∉ T
  · obtain ⟨⟨σ, hσ⟩, rfl⟩ := MulAction.exists_smul_eq (Subgroup.closure S) a b
    let H : Subgroup (Equiv.Perm α) := MulAction.stabilizer (Equiv.Perm α) T
    have h : ¬ Subgroup.closure S ≤ H :=
      fun h ↦ ne_of_mem_of_not_mem' (Set.smul_mem_smul_set ha) hb (h hσ)
    rw [Subgroup.closure_le, Set.not_subset] at h
    obtain ⟨τ, hτ, hτ'⟩ := h
    obtain ⟨x, y, hxy, rfl⟩ := hS τ hτ
    by_cases hx : x ∈ T <;> by_cases hy : y ∈ T
    · absurd hτ'
      ext z
      rw [Set.mem_smul_set_iff_inv_smul_mem, Equiv.swap_inv, Equiv.Perm.smul_def, Equiv.swap_apply_def]
      split_ifs <;> simp only [*]
    · exact ⟨x, y, Subgroup.subset_closure hτ, hx, hy⟩
    · exact ⟨y, x, Equiv.swap_comm x y ▸ Subgroup.subset_closure hτ, hy, hx⟩
    · absurd hτ'
      ext z
      rw [Set.mem_smul_set_iff_inv_smul_mem, Equiv.swap_inv, Equiv.Perm.smul_def, Equiv.swap_apply_def]
      split_ifs <;> simp only [*]
  ...
  · rcases eq_or_ne y a with rfl | h
    · exact Equiv.swap_comm x y ▸ hab
    rw [← Equiv.swap_mul_swap_mul_swap h hxy.symm]
    exact Subgroup.mul_mem _ (Subgroup.mul_mem _ hab (hT' y hy a ha h)) hab
  · rcases eq_or_ne x a with rfl | h
    · exact Equiv.swap_comm x y ▸ hab
    rw [Equiv.swap_comm, ← Equiv.swap_mul_swap_mul_swap h hxy]
    exact Subgroup.mul_mem _ (Subgroup.mul_mem _ hab (hT' x hx a ha h)) hab
  · exact hT' x hx y hy hxy

/- A transitive group generated by transpositions must be all of the symmetric group. -/
lemma keylemma' {α β : Type*} [Group α] [MulAction α β] [DecidableEq β] [Finite β] [h : MulAction.IsPretransitive α β]
    (S : Set α) (hS1 : ∀ σ ∈ S, Equiv.Perm.IsSwap (MulAction.toPermHom α β σ))
    (hS2 : Subgroup.closure S = ⊤) : Function.Surjective (MulAction.toPermHom α β) := by
...
theorem X_pow_sub_X_sub_one_gal :
  ...
  · -- from irreducible
    sorry
  let S : Set f.Gal := ⋃ (p : Nat.Primes), sorry
  have hS1 : ∀ σ ∈ S, Equiv.Perm.IsSwap (MulAction.toPermHom f.Gal (f.rootSet ℂ) σ) := sorry
  have hS2 : Subgroup.closure S = ⊤ := sorry
