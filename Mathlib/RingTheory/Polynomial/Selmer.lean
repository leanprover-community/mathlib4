...
theorem tada' {R S : Type*} [CommRing R] [Field S] [Algebra R S] [NoZeroSMulDivisors R S] (f : R[X])
    (hf : f.Monic) (hf' : f.Splits (algebraMap R S)) [DecidableEq (f.rootSet S)]
    (G : Type*) [Group G] [MulSemiringAction G S] [SMulCommClass G R S]
  ...
  have hS2 : ∀ σ ∈ X, (MulAction.toPermHom G (f.rootSet S) σ).IsSwap := by
    ...
    obtain ⟨m, hm, hσ⟩ := hσ
    apply (tada f hf G m ?_ ?_ σ hm).resolve_left
    · rwa [map_eq_one_iff _ hinj]
    · rw [Polynomial.splits_iff_card_roots.mp ?_, hf.natDegree_map]
      rwa [Polynomial.splits_id_iff_splits]
    · grw [← h]
