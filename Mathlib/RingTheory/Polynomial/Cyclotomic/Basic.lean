...
theorem int_coeff_of_cyclotomic' {K : Type*} [CommRing K] [IsDomain K] {ζ : K} {n : ℕ}
  ...
  induction n using Nat.strong_induction_on generalizing ζ with | ind k ihk
...
theorem cyclotomic_eq_prod_X_sub_primitiveRoots {K : Type*} [CommRing K] [IsDomain K] {ζ : K}
  ...
  rw [← cyclotomic']
  induction n using Nat.strong_induction_on generalizing ζ with | ind k hk
...
theorem cyclotomic_coeff_zero (R : Type*) [CommRing R] {n : ℕ} (hn : 1 < n) :
  ...
  induction n using Nat.strong_induction_on with | ind n hi
