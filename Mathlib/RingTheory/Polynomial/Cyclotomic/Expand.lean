...
theorem cyclotomic_expand_eq_cyclotomic_mul {p n : ℕ} (hp : Nat.Prime p) (hdiv : ¬p ∣ n)
  ...
  · refine (IsPrimitive.Int.dvd_iff_map_cast_dvd_map_cast _ _
    ...
    · have hpos : 0 < n * p := mul_pos hnpos hp.pos
      ...
      rw [← eval_map_algebraMap, map_expand, map_cyclotomic, expand_eval, ← IsRoot,
    ...
    · have hprim := Complex.isPrimitiveRoot_exp _ hnpos.ne.symm
      ...
      refine minpoly.dvd ℚ _ ?_
      rw [← eval_map_algebraMap, map_expand, expand_eval, ← IsRoot, ←
...
theorem isRoot_cyclotomic_prime_pow_mul_iff_of_charP {m k p : ℕ} {R : Type*} [CommRing R]
  ...
  · rw [IsRoot, cyclotomic_mul_prime_pow_eq R (NeZero.not_char_dvd R p m) hk, eval_pow]
      at h
    replace h := eq_zero_of_pow_eq_zero h
    rwa [← IsRoot, isRoot_cyclotomic_iff] at h
  · rw [← isRoot_cyclotomic_iff, IsRoot] at h
    rw [cyclotomic_mul_prime_pow_eq R (NeZero.not_char_dvd R p m) hk, IsRoot, eval_pow,
      h, zero_pow]
