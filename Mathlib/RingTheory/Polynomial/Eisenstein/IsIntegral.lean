...
theorem dvd_coeff_zero_of_aeval_eq_prime_smul_of_minpoly_isEisensteinAt {B : PowerBasis K L}
  ...
  have hintsum :
    IsIntegral R
      (z * B.gen ^ n - ∑ x : ℕ ∈ (range (Q.natDegree + 1)).erase 0, Q.coeff x • f (x + n)) := by
  ...
  calc
    ...
    _ = norm K (p • (z * B.gen ^ n) -
          ∑ x : ℕ ∈ (range (Q.natDegree + 1)).erase 0, p • Q.coeff x • f (x + n)) :=
  ...
  calc
    _ = (Q.coeff 0 • ↑1 + ∑ x : ℕ ∈ (range (Q.natDegree + 1)).erase 0, Q.coeff x • B.gen ^ x) *
          B.gen ^ n := ?_
    _ = (Q.coeff 0 • B.gen ^ 0 +
        ∑ x : ℕ ∈ (range (Q.natDegree + 1)).erase 0, Q.coeff x • B.gen ^ x) * B.gen ^ n :=
...
theorem mem_adjoin_of_smul_prime_smul_of_minpoly_isEisensteinAt {B : PowerBasis K L}
  ...
  · intro hj
    ...
    have hintsum : IsIntegral R (z * B.gen ^ (P.natDegree - (j + 2)) -
        (∑ x : ℕ ∈ (range (Q.natDegree - j)).erase 0,
          Q.coeff (j + 1 + x) • f (x + P.natDegree - 1) +
            ∑ x : ℕ ∈ range (j + 1), g x • B.gen ^ x * B.gen ^ (P.natDegree - (j + 2)))) := by
