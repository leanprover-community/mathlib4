...
theorem dvd_coeff_zero_of_aeval_eq_prime_smul_of_minpoly_is_eiseinstein_at {B : PowerBasis K L}
  ...
  have aux : ∀ i ∈ (range (Q.nat_degree + 1)).eraseₓ 0, B.dim ≤ i + n := by
    intro i hi
  ...
  have hintsum :
    IsIntegral R
      (z * B.gen ^ n - ∑ x : ℕ in (range (Q.nat_degree + 1)).eraseₓ 0, Q.coeff x • f (x + n)) := by
    refine'
  ...
  calc
    _ =
        (Q.coeff 0 • 1 + ∑ x : ℕ in (range (Q.nat_degree + 1)).eraseₓ 0, Q.coeff x • B.gen ^ x) *
          B.gen ^ n :=
      _
    _ =
        (Q.coeff 0 • B.gen ^ 0 +
            ∑ x : ℕ in (range (Q.nat_degree + 1)).eraseₓ 0, Q.coeff x • B.gen ^ x) *
          B.gen ^ n :=
      by rw [pow_zero]
...
theorem mem_adjoin_of_smul_prime_smul_of_minpoly_is_eiseinstein_at {B : PowerBasis K L}
  ...
  · intro hj
    ...
    have hf₁ :
      ∀ k ∈ (range (Q.nat_degree - j)).eraseₓ 0,
        Q.coeff (j + 1 + k) • B.gen ^ (j + 1 + k) * B.gen ^ (P.nat_degree - (j + 2)) =
          (algebraMap R L) p * Q.coeff (j + 1 + k) • f (k + P.nat_degree - 1) := by
      intro k hk
      ...
      IsIntegral R
        (z * B.gen ^ (P.nat_degree - (j + 2)) -
          (∑ x : ℕ in (range (Q.nat_degree - j)).eraseₓ 0,
            ...
            ∑ x : ℕ in range (j + 1), g x • B.gen ^ x * B.gen ^ (P.nat_degree - (j + 2)))) := by
      refine'
        isIntegral_sub (isIntegral_mul hzint (IsIntegral.pow hBint _))
