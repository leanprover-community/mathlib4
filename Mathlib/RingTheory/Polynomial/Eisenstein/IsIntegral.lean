...
theorem cyclotomic_comp_X_add_one_isEisensteinAt [hp : Fact p.Prime] :
  ...
  · rw [show (X + 1 : ℤ[X]) = X + C 1 by simp]
    -- ⊢ Monic (comp (cyclotomic p ℤ) (X + ↑C 1))
  ...
  · rw [cyclotomic_prime, geom_sum_X_comp_X_add_one_eq_sum, ← lcoeff_apply, LinearMap.map_sum]
    -- ⊢ ∑ i_1 in range p, ↑(lcoeff ℤ i) (↑(Nat.choose p (i_1 + 1)) * X ^ i_1) ∈ Subm …
    conv =>
      congr
      congr
      next => skip
...
theorem cyclotomic_prime_pow_comp_X_add_one_isEisensteinAt [hp : Fact p.Prime] (n : ℕ) :
  ...
  · rw [show (X + 1 : ℤ[X]) = X + C 1 by simp]
    -- ⊢ Monic (comp (cyclotomic (p ^ (n + 1)) ℤ) (X + ↑C 1))
  ...
  · induction' n with n hn
    -- ⊢ ∀ {n : ℕ}, n < natDegree (comp (cyclotomic (p ^ (Nat.zero + 1)) ℤ) (X + 1))  …
    · intro i hi
      -- ⊢ coeff (comp (cyclotomic (p ^ (Nat.zero + 1)) ℤ) (X + 1)) i ∈ Submodule.span  …
      rw [Nat.zero_add, pow_one] at hi ⊢
      -- ⊢ coeff (comp (cyclotomic p ℤ) (X + 1)) i ∈ Submodule.span ℤ {↑p}
      exact (cyclotomic_comp_X_add_one_isEisensteinAt p).mem hi
      -- 🎉 no goals
    · intro i hi
      -- ⊢ coeff (comp (cyclotomic (p ^ (Nat.succ n + 1)) ℤ) (X + 1)) i ∈ Submodule.spa …
      ...
      · simp only [ite_eq_right_iff]
        -- ⊢ p ∣ i → coeff (comp (cyclotomic (p ^ Nat.succ n) (ZMod p)) (X + 1)) (i / p)  …
        rintro ⟨k, hk⟩
        -- ⊢ coeff (comp (cyclotomic (p ^ Nat.succ n) (ZMod p)) (X + 1)) (i / p) = 0
        rw [natDegree_comp, show (X + 1 : ℤ[X]) = X + C 1 by simp, natDegree_X_add_C, mul_one,
        ...
        -- ⊢ coeff (comp (cyclotomic (p ^ Nat.succ n) (ZMod p)) (X + 1)) k = 0
...
theorem dvd_coeff_zero_of_aeval_eq_prime_smul_of_minpoly_isEisensteinAt {B : PowerBasis K L}
  ...
  -- ⊢ p ^ succ n ∣ coeff Q 0 ^ succ n * ((-1) ^ (succ n * n) * coeff (minpoly R B. …
  use r
  -- ⊢ coeff Q 0 ^ succ n * ((-1) ^ (succ n * n) * coeff (minpoly R B.gen) 0 ^ n) = …

  -- Do the computation in `K` so we can work in terms of `z` instead of `r`.
  apply IsFractionRing.injective R K
  -- ⊢ ↑(algebraMap R K) (coeff Q 0 ^ succ n * ((-1) ^ (succ n * n) * coeff (minpol …
  simp only [_root_.map_mul, _root_.map_pow, _root_.map_neg, _root_.map_one]
  -- ⊢ ↑(algebraMap R K) (coeff Q 0) ^ succ n * ((-1) ^ (succ n * n) * ↑(algebraMap …
  ...
  · simp only [Algebra.smul_def, algebraMap_apply R K L, Algebra.norm_algebraMap, _root_.map_mul,
    ...
    -- 🎉 no goals
  swap
  -- ⊢ ↑(Algebra.norm K) (p • (z * B.gen ^ n) - ∑ x in Finset.erase (range (natDegr …
...
theorem mem_adjoin_of_dvd_coeff_of_dvd_aeval {A B : Type*} [CommSemiring A] [CommRing B]
  ...
  -- ⊢ ∑ x_1 : Fin (natDegree Q + 1), f ↑x_1 • x ^ ↑x_1 ∈ adjoin A {x}
  exact
...
theorem mem_adjoin_of_smul_prime_smul_of_minpoly_isEisensteinAt {B : PowerBasis K L}
  ...
  -- ⊢ i ∈ range (natDegree Q + 1) → p ∣ coeff Q i
  induction' i using Nat.case_strong_induction_on with j hind
  -- ⊢ 0 ∈ range (natDegree Q + 1) → p ∣ coeff Q 0
  · intro _
    -- ⊢ p ∣ coeff Q 0
  ...
  · intro hj
    ...
    -- ⊢ ℕ
    exact n
    -- ⊢ p ^ succ n ∣ coeff Q (succ j) ^ succ n * coeff P 0 ^ n
    ...
    -- ⊢ ∀ (k : ℕ), k ∈ range (j + 1) → coeff Q k • B.gen ^ k = ↑(algebraMap R L) p * …
    · intro k hk
      -- ⊢ coeff Q k • B.gen ^ k = ↑(algebraMap R L) p * g k • B.gen ^ k
    ...
    -- ⊢ p ^ succ n ∣ coeff Q (succ j) ^ succ n * coeff (minpoly R B.gen) 0 ^ (succ j …
