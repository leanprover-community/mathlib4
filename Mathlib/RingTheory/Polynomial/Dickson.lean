...
theorem dickson_one_one_zmod_p (p : ℕ) [Fact p.Prime] : dickson 1 (1 : ZMod p) p = X ^ p := by
  ...
  · rintro _ ⟨x, rfl, hx⟩
    simp only [eval_X, eval_pow, Set.mem_ofPred, ZMod.cast_one', add_pow_char,
  ...
  · intro h
    ...
    suffices (Set.univ : Set K) =
      ...
      classical
        ...
        ext1 y
        simp only [φ, Multiset.mem_toFinset, Set.mem_ofPred, Finset.mem_coe, Multiset.mem_union,
    ...
    intro x
    simp only [exists_prop, Set.mem_iUnion, Ne, Set.mem_ofPred]
    by_cases hx : x = 0
