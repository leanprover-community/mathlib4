...
@[simp]
lemma aeval_map_algebraMap (x : B) (p : R[X]) : aeval x (map (algebraMap R A) p) = aeval x p := by
...
lemma aeval_algebraMap_apply (x : A) (p : R[X]) :
...
@[simp]
lemma aeval_coe (S : Subalgebra R A) (x : S) (p : R[X]) : aeval (x : A) p = aeval x p :=
