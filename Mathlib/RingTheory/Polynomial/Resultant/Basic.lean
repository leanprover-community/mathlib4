  ...
  have (i : ℕ) (hi : i ≤ m) : (M i).det = M₂.det := by
    ...
    | succ i IH =>
      rw [← IH (by cutsat), ← Matrix.det_updateCol_add_smul_self (i := ⟨i, by cutsat⟩)
        (j := ⟨i + k + m, by cutsat⟩) (c := -r) (M (i + 1)) (by simp; cutsat)]
      congr 1
      ext j₁ j₂
      ...
      · simp only [↓reduceIte, sylvester, Set.mem_Icc, coeff_add, Fin.eta, Matrix.of_apply,
        ...
        | right i =>
          ...
          have : Fin.mk (n := m + n) (m + ↑i + (k + m)) (by cutsat) =
              Fin.natAdd m ⟨↑i + (k + m), by cutsat⟩ := Fin.ext (by simp; cutsat)
          simp only [Fin.addCases_right, Fin.val_natAdd, sub_eq_self, this]
          rw [if_neg, mul_zero]
          cutsat
      split_ifs with h₁ h₂ h₃ h₄ h₅ <;> cutsat
  ...
  · rw [resultant, ← Matrix.det_reindex_self (finCongr (show (m + 1).add n = m + (n + 1) by grind))]
    ...
    · ext i j
      ...
      | right j =>
        ...
        split_ifs with h₁ h₂ h₃ h₃ <;> try first | cutsat | rfl
...
lemma resultant_deriv {f : R[X]} (hf : 0 < f.degree) :
  ...
  ring_nf
  rw [Nat.div_mul_cancel (by convert Nat.two_dvd_mul_add_one (f.natDegree - 1) using 2; cutsat)]
