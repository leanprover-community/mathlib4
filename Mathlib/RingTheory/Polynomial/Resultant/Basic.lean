...
sylvesterDeriv (f : R[X]) :
  ...
  letI n := f.natDegree
  if hn : n = 0 then 0
  else (f.sylvester f.derivative n (n - 1)).updateRow ⟨2 * n - 2, by grind⟩
...
lemma sylvesterDeriv_updateRow (f : R[X]) (hf : 0 < f.natDegree) :
    (sylvesterDeriv f).updateRow ⟨2 * f.natDegree - 2, by grind⟩
      (f.leadingCoeff • (sylvesterDeriv f ⟨2 * f.natDegree - 2, by grind⟩)) =
  ...
  · -- Bottom row
    ...
    split_ifs
    on_goal 2 => rw [show f.natDegree = 1 by grind]
    on_goal 3 =>
      rw [← Nat.cast_one (R := R), ← Nat.cast_add, show f.natDegree = 1 by grind]
      norm_num
...
lemma disc_of_degree_eq_one {f : R[X]} (hf : f.degree = 1) : disc f = 1 := by
  ...
  let e : Fin (f.natDegree - 1 + f.natDegree) ≃ Fin 1 := finCongr (by grind)
  have : f.sylvesterDeriv.reindex e e = !![1] := by
    have : NeZero (f.natDegree - 1 + f.natDegree) := ⟨by grind⟩
    ext ⟨i, hi⟩ ⟨j, hj⟩
...
lemma disc_of_degree_eq_two {f : R[X]} (hf : f.degree = 2) :
  ...
  let e : Fin (f.natDegree - 1 + f.natDegree) ≃ Fin 3 := finCongr (by grind)
