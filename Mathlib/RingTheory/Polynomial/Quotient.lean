...
lemma polynomialQuotientEquivQuotientPolynomial_eval₂RingHom_leftInverse (I : Ideal R) :
  ...
  · rintro n ⟨x⟩
    erw [← smul_X_eq_monomial, smul_eq_C_mul, _root_.map_mul, coe_eval₂RingHom, eval₂_C,
      eval₂_X_pow, Quotient.lift_mk, coe_eval₂RingHom, eval₂_mul, eval₂_C, RingHom.comp_apply,
      eval₂_X_pow]
...
def polynomialQuotientEquivQuotientPolynomial (I : Ideal R) :
  ...
  { e with
    invFun := Quotient.lift (map C I : Ideal R[X]) (eval₂RingHom (C.comp (Quotient.mk I)) X)
        eval₂_C_mk_eq_zero
    -- porting note: need to split this definition into two sub-lemmas to prevent time out
