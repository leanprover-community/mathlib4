...
variable (p : Polynomial R) (hp : p ≠ 0) (d : ℕ)

/--
Given a polynomial `p`, the greatest factor of `p` that is not divided by `1 - X`.
-/
noncomputable def greatestFactorOneSubNotDvd : Polynomial R :=
...
theorem pow_rootMultiplicity_mul_greatestFactorOneSubNotDvd_eq :
    ((1 - (X : R[X])) ^ p.rootMultiplicity 1) *
    greatestFactorOneSubNotDvd p hp = p := by
...
theorem greatestFactorOneSubNotDvd_ne_zero : greatestFactorOneSubNotDvd p hp ≠ 0 := λ h0 ↦ by
...
theorem natDegree_greatestFactorOneSubNotDvd_le :
  ...
  have : p.natDegree = (((1 - (X : R[X])) ^ p.rootMultiplicity 1) *
      greatestFactorOneSubNotDvd p hp).natDegree := by
    rw [pow_rootMultiplicity_mul_greatestFactorOneSubNotDvd_eq]
  rw [this, natDegree_mul]
  exact (greatestFactorOneSubNotDvd p hp).natDegree.le_add_left
    (natDegree ((1 - X) ^ p.rootMultiplicity 1))
  exact pow_ne_zero _ <| λ h0 ↦ by
    let this : Polynomial.coeff (1 - (X : R[X])) 0 = 0 := by rw [h0]; simp only [coeff_zero]
    simp only [coeff_sub, coeff_one_zero, coeff_X_zero, sub_zero] at this
...
theorem natDegree_pow_rootMultiplicity_sub_mul_greatestFactorOneSubNotDvd_le
    ...
    ((1 - X) ^ (rootMultiplicity 1 p - (d + 1)) * greatestFactorOneSubNotDvd p hp).natDegree
    ≤ p.natDegree := by
  let this := pow_ne_zero (p.rootMultiplicity 1 - (d + 1)) <| λ (h0 : 1 - (X : R[X]) = 0) ↦ by
  ...
  rw [show p.natDegree = (((1 - X : R[X]) ^ (p.rootMultiplicity 1 - (d + 1) + (d + 1))) *
    greatestFactorOneSubNotDvd p hp).natDegree by rw [← Nat.eq_add_of_sub_eq hp1 rfl,
    pow_rootMultiplicity_mul_greatestFactorOneSubNotDvd_eq], pow_add, mul_assoc, mul_comm
    ((1 - Polynomial.X) ^ (d + 1)), ← mul_assoc, natDegree_mul, natDegree_mul, natDegree_mul]
  ...
  · simp only [h, ↓reduceDite, zsmul_eq_mul]
    have coe_one_sub : (1 - (X : ℤ[X])).ToPowerSeries = 1 - (PowerSeries.X : ℤ⟦X⟧) := by
      rw [PowerSeries.ext_iff]; exact λ i ↦ by_cases (λ (hi : i = 0) ↦ by
        ...
        map_sub, PowerSeries.coeff_one, ↓reduceIte, coeff_zero_X]) (λ hi ↦ by
        ...
        simp only [hi]) (λ hi ↦ by simp only [hi]; exact if_neg <| Ne.symm hi))
    ...
    · simp only [h1, ↓reduceIte, eval_zero, Int.cast_eq_zero]
      rw [← pow_rootMultiplicity_mul_greatestFactorOneSubNotDvd_eq p h, mul_comm, coe_mul,
        ← mul_assoc, coe_pow, coe_one_sub, ← @Nat.sub_add_cancel (p.rootMultiplicity 1) (d + 1) h1,
        mul_comm (invOneSubPow d).val, pow_add, mul_assoc ((1 - PowerSeries.X) ^
    ...
    · simp only [h1, ↓reduceIte]
      rw [Polynomial.coe_inj.2 (pow_rootMultiplicity_mul_greatestFactorOneSubNotDvd_eq p h).symm,
        coe_mul, mul_comm ((1 - X : ℤ[X]) ^ p.rootMultiplicity 1).ToPowerSeries, mul_assoc,
        invOneSubPow_eq_inv_one_sub_pow, show d + 1 = (p.rootMultiplicity 1) + (d -
        (p.rootMultiplicity 1) + 1) by rw [← Nat.sub_add_comm <| Nat.not_lt.mp h1,
        Nat.add_sub_of_le <| Nat.le_of_not_ge h1], pow_add, Units.val_mul, ← mul_assoc ((1 - X :
        ℤ[X]) ^ rootMultiplicity 1 p).ToPowerSeries, coe_pow, coe_one_sub,
        ← invOneSubPow_eq_inv_one_sub_pow]
      simp only [Units.inv_mk, Units.val_pow_eq_pow_val]
      rw [← mul_pow, PowerSeries.mul_invOfUnit _ _ <| by simp only [map_sub, map_one,
        constantCoeff_X, sub_zero, Units.val_one], one_pow, one_mul, show
        (greatestFactorOneSubNotDvd p h).ToPowerSeries = (Finset.sum (Finset.range
        ((greatestFactorOneSubNotDvd p h).natDegree + 1)) (fun (i : ℕ) =>
        ((greatestFactorOneSubNotDvd p h).coeff i) • (X ^ i)) : Polynomial ℤ).ToPowerSeries by
        simp only [zsmul_eq_mul, coe_inj]; exact as_sum_range_C_mul_X_pow
          (greatestFactorOneSubNotDvd p h), invOneSubPow]
      ...
      rw [(Finset.sum_eq_sum_iff_of_le (λ i hi ↦ by
        ...
        (d - rootMultiplicity 1 p) i n <| Nat.le_trans (Nat.le_of_lt_succ hi) (le_trans
        (natDegree_greatestFactorOneSubNotDvd_le p h) (le_of_lt hn))])).2 <| λ i hi ↦ by
        simp only [Subtype.forall, Finset.mem_range, mul_eq_mul_left_iff, Int.cast_eq_zero] at *;
        exact Or.intro_left _ <| preHilbert_eq_choose_sub_add (d - rootMultiplicity 1 p) i n <|
      ...
      rw [Finset.Nat.sum_antidiagonal_eq_sum_range_succ_mk,
        show Nat.succ n = (natDegree (greatestFactorOneSubNotDvd p h) + 1) +
        ((Nat.succ n) - (natDegree (greatestFactorOneSubNotDvd p h) + 1)) by
        ...
        exact Nat.succ_eq_one_add n, Finset.sum_range_add]
...
theorem exists_unique_hilbert (p : Polynomial ℤ) (d : ℕ) :
  ...
  λ q ⟨N, hqN⟩ ↦ eq_of_infinite_eval_eq q (hilbert p d) <| λ hfin ↦ Set.Infinite.image
  (Set.injOn_of_injective Nat.cast_injective _) (Set.Ioi_infinite (max N (natDegree p))) <|
  Set.Finite.subset hfin <| show @Nat.cast ℚ _ '' (Set.Ioi (max N (natDegree p))) ⊆
  (@setOf ℚ fun x ↦ eval x q = eval x (hilbert p d)) by
...
lemma prod_sub_rootMultiplicity_coeff_eq_one (p : Polynomial ℤ) (d x : ℕ) :
    coeff (∏ x1 in Finset.attach (Finset.range (d - rootMultiplicity 1 p)),
    (Polynomial.X - (x : ℚ[X]) + ↑↑x1 + 1)) (d - rootMultiplicity 1 p) = 1 := by
  let hcoeff := @coeff_prod_of_natDegree_le ℚ ({ x // x ∈ Finset.range (d -
    rootMultiplicity 1 p) }) (Finset.attach (Finset.range (d - rootMultiplicity 1 p))) _ (fun x1 ↦
    Polynomial.X - (x : ℚ[X]) + ↑↑x1 + 1) 1 <| show ∀ x1 ∈ Finset.attach (Finset.range (d -
    rootMultiplicity 1 p)), natDegree (Polynomial.X - (x : ℚ[X]) + ↑↑x1 + 1) ≤ 1 by
...
-/
theorem natDegree_hilbert (p : Polynomial ℤ) (d : ℕ) (hh : hilbert p d ≠ 0) :
  ...
  · by_cases h1 : d + 1 ≤ p.rootMultiplicity 1
    ...
    · refine' natDegree_eq_of_le_of_coeff_ne_zero _ _
      · rw [hilbert]; simp only [h, ↓reduceDite, h1, ↓reduceIte, zsmul_eq_mul]
        refine' @natDegree_sum_le_of_forall_le ℕ (Finset.range (natDegree
          (greatestFactorOneSubNotDvd p h) + 1)) ℚ _ (d - rootMultiplicity 1 p) (fun x ↦
          (@Int.cast ℚ[X] Ring.toIntCast (coeff (greatestFactorOneSubNotDvd p h) x)) * preHilbert
          (d - rootMultiplicity 1 p) x) _
        · intro i _
          refine' le_trans (@natDegree_mul_le ℚ _ (@Int.cast ℚ[X] _ (coeff
            (greatestFactorOneSubNotDvd p h) i)) (preHilbert (d - rootMultiplicity 1 p) i)) _
          · simp only [natDegree_int_cast, zero_add]; rw [preHilbert]
            simp only [Finset.univ_eq_attach, map_natCast]
            refine' le_trans (natDegree_smul_le (@Inv.inv ℚ _ (Nat.factorial (d -
              rootMultiplicity 1 p))) _) _
            · refine' le_trans (natDegree_prod_le (@Finset.attach ℕ (Finset.range (d -
                rootMultiplicity 1 p))) _) _
              · have : ∀ x ∈ Finset.attach (Finset.range (d - rootMultiplicity 1 p)), natDegree
                  (Polynomial.X - (i : ℚ[X]) + ↑↑x + 1) ≤ 1 :=
      ...
      · rw [hilbert]
        ...
        constructor
        · rw [show ∑ i in Finset.range (natDegree (greatestFactorOneSubNotDvd p h) + 1),
            (@Int.cast ℚ Ring.toIntCast (coeff (greatestFactorOneSubNotDvd p h) i)) = eval 1
            (greatestFactorOneSubNotDvd p h) by rw [eval_eq_sum_range]; simp only [one_pow,
