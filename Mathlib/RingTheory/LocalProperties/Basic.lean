...
lemma RingHom.HoldsForLocalization.isLocalizationMap
  ...
  have hle : Submonoid.map (f : R →* S) M ≤ T := by
    simpa [Submonoid.map_le_iff_le_comap]
  let : Algebra (Localization (M.map (f : R →* S))) S' :=
    IsLocalization.localizationAlgebraOfSubmonoidLe _ _ (M.map (f : R →* S)) T hle
  have : IsScalarTower S (Localization (Submonoid.map (f : R →* S) M)) S' :=
    IsLocalization.localization_isScalarTower_of_submonoid_le _ _ _ _ _
  have : IsLocalization (T.map (algebraMap S
      (Localization (M.map (f : R →* S)))).toMonoidHom) S' :=
    IsLocalization.isLocalization_of_submonoid_le _ _ (M.map (f : R →* S)) T hle
  have heq : IsLocalization.map (S := R') S' f hy =
      (algebraMap (Localization (Submonoid.map ((f : R →* S)) M)) _).comp
        (IsLocalization.map (M := M) (T := M.map (f : R →* S)) (S := R')
          (Localization (M.map (f : R →* S))) f
          (M.le_comap_map)) := by
  ...
  exact hPc _ _ (hPp _ _ _ _ hf)
    (hPl _ (T.map (algebraMap S (Localization (M.map (f : R →* S)))).toMonoidHom))
...
lemma RingHom.IsStableUnderBaseChange.isLocalization_map (M : Submonoid R) [IsLocalization M Rᵣ]
    (f : R →+* S) [IsLocalization (M.map (f : R →* S)) Sᵣ] (hf : P f) :
  ...
  have : IsLocalization (Algebra.algebraMapSubmonoid S M) Sᵣ :=
    inferInstanceAs <| IsLocalization (M.map (f : R →* S)) Sᵣ
