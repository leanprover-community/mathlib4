...
def RingHom.LocalizationPreserves :=
  ∀ ⦃R S : Type u⦄ [CommRing R] [CommRing S] (f : R →+* S) (M : Submonoid R) (R' S' : Type u)
    ...
    [IsLocalization (M.map (MonoidHom.ofClass f)) S'],
...
lemma RingHom.HoldsForLocalization.isLocalizationMap
  ...
  have hle : Submonoid.map (MonoidHom.ofClass f) M ≤ T := by
    simpa [Submonoid.map_le_iff_le_comap]
  let : Algebra (Localization (M.map (MonoidHom.ofClass f))) S' :=
    IsLocalization.localizationAlgebraOfSubmonoidLe _ _ (M.map (MonoidHom.ofClass f)) T hle
  have : IsScalarTower S (Localization (Submonoid.map (MonoidHom.ofClass f) M)) S' :=
    IsLocalization.localization_isScalarTower_of_submonoid_le _ _ _ _ _
  have : IsLocalization (T.map (algebraMap S
      (Localization (M.map (MonoidHom.ofClass f)))).toMonoidHom) S' :=
    IsLocalization.isLocalization_of_submonoid_le _ _ (M.map (MonoidHom.ofClass f)) T hle
  have heq : IsLocalization.map (S := R') S' f hy =
      (algebraMap (Localization (Submonoid.map ((MonoidHom.ofClass f)) M)) _).comp
        (IsLocalization.map (M := M) (T := M.map (MonoidHom.ofClass f)) (S := R')
          (Localization (M.map (MonoidHom.ofClass f))) f
          (M.le_comap_map)) := by
  ...
  exact hPc _ _ (hPp _ _ _ _ hf)
    (hPl _ (T.map (algebraMap S (Localization (M.map (MonoidHom.ofClass f)))).toMonoidHom))
...
theorem RingHom.LocalizationPreserves.away (H : RingHom.LocalizationPreserves @P) :
  ...
  have : IsLocalization ((Submonoid.powers r).map (MonoidHom.ofClass f)) S' := by
...
lemma RingHom.IsStableUnderBaseChange.isLocalization_map (M : Submonoid R) [IsLocalization M Rᵣ]
    (f : R →+* S) [IsLocalization (M.map (MonoidHom.ofClass f)) Sᵣ] (hf : P f) :
    P (IsLocalization.map Sᵣ f (M.le_comap_map
    (f := (MonoidHom.ofClass f))) : Rᵣ →+* Sᵣ) := by
  ...
  have : IsLocalization (Algebra.algebraMapSubmonoid S M) Sᵣ :=
    inferInstanceAs <| IsLocalization (M.map (MonoidHom.ofClass f)) Sᵣ
