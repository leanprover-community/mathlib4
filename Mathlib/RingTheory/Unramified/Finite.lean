...
theorem iff_exists_tensorProduct [EssFiniteType R S] :
  ...
  · rintro ⟨t, ht₁, ht₂⟩
    ...
    · suffices e ∈ (Submodule.span (S ⊗[R] S) {1 - e}).annihilator by
      ...
      suffices Ideal.span (Set.range fun s ↦ 1 ⊗ₜ[R] s - s ⊗ₜ[R] 1) ≤
          (Submodule.span (S ⊗[R] S) {1 - e}).annihilator by
        exact this ht₂
      simpa only [Ideal.span_le, Set.range_subset_iff,
        Submodule.mem_annihilator_span_singleton, SetLike.mem_coe]
