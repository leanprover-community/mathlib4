...
instance linearOrderOverring : LinearOrder {S | A ≤ S} :=
  {
    (inferInstance :
      PartialOrder
        _) with
    le_total :=
      let i : IsTotal (PrimeSpectrum A) (· ≤ ·) := ⟨fun ⟨x, _⟩ ⟨y, _⟩ => LE.le.isTotal.Total x y⟩
...
variable {K} {Γ Γ₁ Γ₂ : Type _} [LinearOrderedCommGroupWithZero Γ]
...
def principalUnitGroupEquiv :
    A.principalUnitGroup ≃* (Units.map (LocalRing.residue A).toMonoidHom).ker
    where
  toFun x :=
