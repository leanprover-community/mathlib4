...
lemma comap_vlt (φ : A →+* B) (v : Spv B) {a₁ a₂ : A} :
    (comap φ v).vlt a₁ a₂ = v.vlt (φ a₁) (φ a₂) := rfl
...
lemma not_vle_zero_of_isUnit {f : A} (hf : IsUnit f) (v : Spv A) : ¬ v.vle f 0 :=
  letI : ValuativeRel A := v.toValuativeRel; ValuativeRel.not_vle_zero_of_isUnit hf
