...
lemma le_supp_comap_quotientMk (w : Spv (A ⧸ 𝔞)) :
...
/-- `quotientLift 𝔞 (le_supp_comap_quotientMk 𝔞 w) = w`. -/
@[simp]
lemma quotientLift_comap (w : Spv (A ⧸ 𝔞)) :
    quotientLift 𝔞 (le_supp_comap_quotientMk 𝔞 w) = w := by
...
lemma range_comap_quotientMk :
    Set.range (comap (Ideal.Quotient.mk 𝔞)) = {v : Spv A | 𝔞 ≤ v.supp} :=
  Set.ext fun _ ↦ ⟨fun ⟨w, hw⟩ ↦ hw ▸ le_supp_comap_quotientMk 𝔞 w,
