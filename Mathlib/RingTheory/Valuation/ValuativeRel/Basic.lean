...
def embed [v.Compatible] : ValueGroupWithZero R →*₀ valueGroup₀ (.ofClass v) where
  toFun := ValueGroupWithZero.lift
    (fun r s ↦ (rangeRestrict (.ofClass v) r / (rangeRestrict (.ofClass v) s))) <| by
    intro x y r s
...
lemma embed_mk [v.Compatible] (x : R) (s : posSubmonoid R) :
    embed v (.mk x s) = (rangeRestrict (.ofClass v) x / (rangeRestrict (.ofClass v) s)) :=
  rfl
...
lemma coe_embed_valuation (x : ValueGroupWithZero R) :
  ...
  | mk r s =>
    rw [embed_mk, ValueGroupWithZero.mk_eq_div]
    rfl
...
-/
@[simp]
lemma embed_valuation_eq_rangeRestrict [v.Compatible] (x : R) :
    embed v (valuation R x) = rangeRestrict (.ofClass v) x := by
...
lemma embed_strictMono [v.Compatible] : StrictMono (embed v) := by
  ...
  rw [← (SubmonoidWithZeroClass.subtype_strictMono _).lt_iff_lt]
  simp only [map_div₀]
  ...
  · rw [← map_mul, ← map_mul, (isEquiv (valuation R) v).lt_iff_lt] at h
    simp only [embed, MonoidWithZeroHom.coe_mk, ZeroHom.coe_mk, lift_valuation,
      OneMemClass.coe_one, map_one, div_one, SubmonoidWithZeroClass.subtype_apply]
    rw [coe_rangeRestrict a, coe_rangeRestrict b, coe_rangeRestrict r.1,
      coe_rangeRestrict s.1]
    simpa using h
...
def orderMonoidIso [v.Compatible] : ValueGroupWithZero R ≃*o valueGroup₀ (.ofClass v) where
  __ := embed v
  invFun x := (((isEquiv v (valuation R)).orderMonoidIso x : valueGroup₀ _) : ValueGroupWithZero R)
  left_inv x := by simp
  right_inv := Function.rightInverse_of_injective_of_leftInverse
      (fun _ _ h ↦ (isEquiv v (valuation R)).orderMonoidIso.injective (Subtype.val_injective h))
      (fun x ↦ by simp)
...
lemma orderMonoidIso_mk [v.Compatible] (x : R) (s : posSubmonoid R) :
    orderMonoidIso v (.mk x s) = rangeRestrict (.ofClass v) x / (rangeRestrict (.ofClass v) s) :=
  rfl

/-- This is the same as `ValuativeRel.ValueGroupWithZero.embed_valuation_eq_rangeRestrict`,
where `embed` is upgraded to `orderMonoidIso`. -/
@[simp]
lemma orderMonoidIso_valuation_eq_rangeRestrict [v.Compatible] (x : R) :
    orderMonoidIso v (valuation R x) = rangeRestrict (.ofClass v) x :=
  embed_valuation_eq_rangeRestrict v x

/-- This is the same as `ValuativeRel.ValueGroupWithZero.coe_embed_valuation`, where
...
/-- Coercing back out of `valueGroup₀` undoes `orderMonoidIso (valuation R)`. -/
lemma leftInverse_coe_orderMonoidIso :
    Function.LeftInverse (Subtype.val : valueGroup₀ (.ofClass (valuation R)) → _)
      (orderMonoidIso (valuation R)) :=
  coe_orderMonoidIso_valuation
...
def mapValueGroupWithZero : ValueGroupWithZero A →*₀ ValueGroupWithZero B :=
  ...
  (SubmonoidWithZeroClass.subtype _).comp
    (orderMonoidIso ((valuation B).comap (algebraMap A B))).toMonoidWithZeroHom
...
lemma mapValueGroupWithZero_strictMono : StrictMono (mapValueGroupWithZero A B) :=
  (SubmonoidWithZeroClass.subtype_strictMono _).comp (embed_strictMono _)
