...
instance le_total_ideal : IsTotalLE (Ideal A) := by
...
theorem _root_.PreValuationRing.iff_ideal_total [CommRing R] :
    PreValuationRing R ↔ IsTotalLE (Ideal R) := by
  classical
...
theorem iff_ideal_total : ValuationRing R ↔ IsTotalLE (Ideal R) :=
  Iff.trans (⟨fun inst ↦ inst.toPreValuationRing, fun _ ↦ .mk⟩)
...
protected theorem TFAE (R : Type u) [CommRing R] [IsDomain R] :
    List.TFAE
      [ValuationRing R,
        ...
        @Std.Total R (· ∣ ·), IsTotalLE (Ideal R), IsLocalRing R ∧ IsBezout R] := by
