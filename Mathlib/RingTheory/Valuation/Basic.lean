...
/-- The subgroup generated by the image of the valuation -/
def rangeGroup : Subgroup Γ₀ˣ := Subgroup.closure (Units.val ⁻¹' Set.range v)

-- instance : LinearOrderedCommGroup v.rangeGroup := by
--   apply @SubgroupClass.toLinearOrderedCommGroup Γ₀ˣ (Subgroup Γ₀ˣ) _ ?_ ?_ v.rangeGroup
--   · let Γ₀
--     have := @Injective.linearOrderedCommGroup
--   · sorry-- exact @Subgroup.instSubgroupClass Γ₀ˣ _
...
theorem injective_withZero_inclusion : Injective (withZero_inclusion v) := by
  intro a b hab
  by_cases h : a = 0
  · rwa [h, Eq.comm, withZero_inclusion_zero, withZero_inclusion_eq_zero_iff, ← h, Eq.comm] at hab
...
instance : LinearOrderedCommMonoidWithZero v.rangeGroupWithZero := by
  have : LinearOrderedCommMonoidWithZero v.rangeGroupWithZero := by
    apply Function.Injective.linearOrderedCommMonoidWithZero (withZero_inclusion v)
      (injective_withZero_inclusion v) (withZero_inclusion_zero v)
      (by rfl) _ _ _ _ --(by sorry) (by sorry) (by simp) (by sorry)
    · intro x y
      match x with
      | 0 => simp
      | some a =>
        match y with
        | 0 => simp
        | some b => sorry--simp [withZero.some withZero_inclusion_some]
    · sorry
    · sorry
    · sorry
  exact this

-- instance : LinearOrderedCommGroupWithZero v.rangeGroupWithZero where
--   inv_zero := sorry
