...
def units_rangeGroup₀_equiv_rangeGroup :
    v.rangeGroup₀ˣ ≃* v.rangeGroup where
  toFun u := by
    refine ⟨Units.mk0 u.val (fun h ↦ u.ne_zero ?_),
      (mem_rangeGroup_iff_mem_rangeGroup₀ v).mpr u.val.prop⟩
    rw [← Subtype.coe_inj, h, MonoidHomWithZero.range₀_coe_zero]
  invFun u := by
    apply Units.mk0 ⟨(u : Γ₀ˣ).val, (mem_rangeGroup_iff_mem_rangeGroup₀ v).mp u.prop⟩
