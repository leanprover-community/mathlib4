...
instance : LinearOrderedCommGroup v.rangeGroup where
  ...
  min := fun ⟨a, ha⟩ ⟨b, hb⟩ ↦ if a ≤ b then ⟨a, ha⟩ else ⟨b, hb⟩
  max := fun ⟨a, ha⟩ ⟨b, hb⟩ ↦ if a ≤ b then ⟨b, hb⟩ else ⟨a, ha⟩
  min_def := fun ⟨a, ha⟩ ⟨b, hb⟩ ↦ by simp only [Subtype.mk_le_mk]
  max_def := fun ⟨a, ha⟩ ⟨b, hb⟩ ↦ by simp only [Subtype.mk_le_mk]
