...
/- The above construction of the range of a valuation is restricted to the case
  where the ground ring is a division ring, and we wish to generalize this construction.
  There are two options:
    - getting a monoid with zero, and this would be the range.
    - getting a group with zero when the target of the valuation is already a group with zero,
    and then, this would be the subgroup with zero generated by the range.
  For the definition of the topology associated with a valuation,
  Wedhorn seems that one to needs to consider the second one, but does it make a difference?
  there are a priori more open sets for the second one, so the topology is a priori finer.
...
instance : LinearOrder Γ₀ˣ := LinearOrder.lift' (fun x : Γ₀ˣ ↦ (↑x : Γ₀)) Units.ext

section

variable {α β : Type*} [LinearOrder β]
  {f : α → β} {inj : Injective f}

theorem LinearOrder.lift'_le_iff {a b : α} :
    (LinearOrder.lift' f inj).le a b ↔ f a ≤ f b :=
  Iff.rfl

theorem LinearOrder.lift'_lt_iff {a b : α} :
    (LinearOrder.lift' f inj).lt a b ↔ f a < f b :=
  Iff.rfl

variable [Max α] (hsup : ∀ x y, f (x ⊔ y) = max (f x) (f y))
    [Min α] (hinf : ∀ x y, f (x ⊓ y) = min (f x) (f y))

theorem LinearOrder.lift_le_iff {a b : α} :
    (LinearOrder.lift f inj hsup hinf).le a b ↔ f a ≤ f b :=
  Iff.rfl

theorem LinearOrder.lift_lt_iff {a b : α} :
    (LinearOrder.lift f inj hsup hinf).lt a b ↔ f a < f b :=
  Iff.rfl

variable [Ord α] (compare_f : ∀ a b : α, compare a b = compare (f a) (f b))

theorem LinearOrder.liftWithOrd_le_iff {a b : α} :
    (LinearOrder.liftWithOrd f inj hsup hinf compare_f).le a b ↔ f a ≤ f b :=
  Iff.rfl

theorem LinearOrder.liftWithOrd_lt_iff {a b : α} :
    (LinearOrder.liftWithOrd f inj hsup hinf compare_f).lt a b ↔ f a < f b :=
  Iff.rfl
...
instance: LinearOrderedCommMonoid v.rangeGroup₀ where
  toOrderedCommMonoid := inferInstance
  ...
  min a b := ⟨min (a : Γ₀) b, by simp [min_def, Subtype.coe_le_coe, ite_mem, a.prop, b.prop]⟩
  max a b := ⟨max (a : Γ₀) b, by simp [max_def, Subtype.coe_le_coe, ite_mem, a.prop, b.prop]⟩
  ...
  compare_eq_compareOfLessAndEq a b := by
    ...
    simp only [compareOfLessAndEq, Subtype.coe_lt_coe, SetLike.coe_eq_coe]
