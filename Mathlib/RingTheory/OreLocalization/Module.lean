...
instance (priority := 950) {A : Type*} [Semiring A] [Algebra R A] :
  ...
  toFun := Quotient.map (fun p ↦ (algebraMap R A p.1, p.2)) <| by
    rintro ⟨r₁, s₁⟩ ⟨r₂, s₂⟩ ⟨s, r, e₁, e₂⟩
    refine ⟨s, r, ?_, ?_⟩
    · simp only [smul_eq_mul, Submonoid.smul_def] at e₁
      simp only [Algebra.smul_def, ← map_mul, ← e₁, Submonoid.smul_def]
    · simp only [smul_eq_mul, Submonoid.smul_def] at e₂
      simp only [Algebra.smul_def, ← map_mul, ← e₂, Submonoid.smul_def]
  map_one' := show _ /ₒ 1 = _ /ₒ 1 by rw [map_one]
  map_mul' x y := by
    induction' x with x
    induction' y with y
  ...
  map_add' x y := by
    induction' x with x
    induction' y with y
    show algebraMap R A (_ * _ + _ * _) /ₒ _ = _ /ₒ _
    rw [map_add, map_mul, map_mul, ← Algebra.smul_def, ← Algebra.smul_def]
    rfl
  commutes' x y := by
    induction' x with x
    induction' y using OreLocalization.ind with y s
    show _ /ₒ (s * x.2) = _ /ₒ _
    rw [Algebra.commutes, mul_comm s x.2]
  smul_def' x y := by
    induction' x with x
    induction' y using OreLocalization.ind with y s
    show _ /ₒ _ = algebraMap R A _ * _ /ₒ _
    rw [← Algebra.smul_def]
    rfl

lemma instAlgebraLocalization_eq_id :
  (LocalizedModule.instAlgebraLocalization : Algebra (Localization S) (Localization S)) =
    Algebra.id _ := by ext; rfl

theorem algebraMap_mk {A : Type*} [Semiring A] [Algebra R A] (a : R) (s : S) :
    algebraMap _ _ (Localization.mk a s) = mk (algebraMap R A a) s := rfl
