...
* `koszulComplex.X_isZero_of_card_generators_lt`: if `M` is generated by `k` elements then
  `koszulComplex φ` vanishes in degrees `> k`; in particular
  (`koszulComplex.ofList_X_isZero_of_length_lt`) the Koszul complex of a list of length `r`
