...
* `koszulComplexAuxAlternating φ n`: the alternating map `M [⋀^Fin (n + 1)]→ₗ[R] ⋀[R]^n M`
  displayed above, defined as the alternatisation of `x ↦ φ x • ιMulti`.
* `koszulComplexAux φ n`: the induced linear map `⋀[R]^(n + 1) M →ₗ[R] ⋀[R]^n M`, the Koszul
  differential.
...
* `koszulComplexAux_comp_eq_zero`: the Koszul differential squares to zero.
