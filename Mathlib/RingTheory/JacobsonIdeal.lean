    ...
    by_cases
      (fun hxy : I ⊔ span {y * x + 1} = ⊤ =>
        ...
        ⟨r, by
          ...
          rw [mul_assoc, ← mul_add_one r (y * x), hr, ← hpq, ← neg_sub, add_sub_eq_left]
          exact I.neg_mem hpi⟩)
      ...
      fun hxm => hm1.1.1 <| (eq_top_iff_one _).2 <| add_sub_eq_right (y * x) 1 ▸
...
theorem IsLocal.mem_jacobson_or_exists_inv {I : Ideal R} (hi : IsLocal I) (x : R) :
  ...
  by_cases
    (fun h : I ⊔ span {x} = ⊤ =>
      ...
      let ⟨r, hr⟩ := mem_span_singleton.1 hq
      Or.inr ⟨r, by rw [← hpq, mul_comm, ← hr, ← neg_sub, add_sub_eq_left]; exact I.neg_mem hpi⟩)
    fun h : I ⊔ span {x} ≠ ⊤ =>
    Or.inl <|
