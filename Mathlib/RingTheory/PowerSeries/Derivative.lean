...
/- In the next lemma, we use `smul_right_inj`, which requires not only `Module.IsTorsionFree ℕ R`, but
...
theorem derivative.ext {R} [CommRing R] [Module.IsTorsionFree ℕ R] {f g} (hD : d⁄dX R f = d⁄dX R g)
