...
`R⟦X⟧` is noetherian if `R` is. The proof of `eq_of_le_of_X_notmem_of_fg_of_prime` is inspired by
[zbMATH05171613].


## Main results

* `PowerSeries.eq_span_insert_X_of_Xmem_of_span_eq` : given an ideal `I` of `R⟦X⟧` such that
...
* `PowerSeries.IsPrime.fg_iff` : a prime ideal `P` of `R⟦X⟧` is finitely generated if and only
...
theorem exist_eq_span_eq_ncard_of_X_notMem (hI : X ∉ I) {S : Set R}
  ...
  refine ⟨T, eq_of_le_of_X_notmem_of_fg_of_prime (span_le.2  hTI) hI (Submodule.fg_def.2
