...
`f ∈ I` is generated by `n` elements, then `I` is generated by at most `n + 1` elements (actually it
is generated by `n` elements if `X ∉ I` and `n + 1` elements otherwise). This implies immediately
that `R⟦X⟧` is noetherian if `R` is. We follow the proof given in [zbMATH05171613].
