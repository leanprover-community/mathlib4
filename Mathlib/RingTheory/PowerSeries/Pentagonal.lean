...
This file proves pentagonal number theorem for power series

$$ \prod_{n = 0}^{\infty} 1 - x^{n + 1} = \sum_{k=-\infty}^{\infty} (-1)^k x^{a_k} $$

where $a_k = k(3k - 1)/2$ are pentagonal numbers.
...
* `PowerSeries.WithPiTopology.tprod_one_sub_X_pow_eq_tsum_nat` pentagonal number
  theorem for summing over natural numbers by grouping pairs of terms
  `$\sum_{k=0}^{\infty} (-1)^k (x^{-a_k} - a^{k+1})$.
...
/-- **Pentagonal number theorem** for power series, summing over integers with exponent $a_{-k}$
for pentagonal numbers $a_k$
...
/-- **Pentagonal number theorem** for power series, summing over integers with exponent $a_k$
for pentagonal numbers $a_k$
