...
lemma submodulesBasis : SubmodulesBasis (basis σ α) where
  inter d e := by
    simp_rw [le_inf_iff]
...
lemma has_submodules_basis_topology [DiscreteTopology α] :
    MvPowerSeries.WithPiTopology.topologicalSpace σ α = (submodulesBasis σ α).topology := by
  rw [TopologicalAddGroup.ext_iff inferInstance inferInstance]
  ext s
  rw [((ringSubgroupsBasis σ α).hasBasis_nhds  0).mem_iff]
  simp only [sub_zero, Submodule.mem_toAddSubgroup, true_and]
  exact mem_nhds_zero_iff σ α

/-- If the coefficient ring `α` is endowed with the discrete topology, then the pointwise
  topology on `MvPowerSeries σ α)` agrees with the topology generated by `MvPowerSeries.basis`. -/
theorem topology_eq_ideals_basis_topology :
    MvPowerSeries.WithPiTopology.topologicalSpace σ α = (idealIsBasis σ α).topology := by
  exact has_submodules_basis_topology σ α
