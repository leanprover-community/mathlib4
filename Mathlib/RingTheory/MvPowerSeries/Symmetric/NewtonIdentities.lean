...
This file defines `MvPowerSeries` power sums as a means of implementing Newton's identities. The
...
* `MvPowerSeries.mul_esymm_eq_sum`: a recurrence relation for the `k`th elementary
  symmetric polynomial in terms of lower-degree elementary symmetric polynomials and power sums.

* `MvPowerSeries.psum_eq_mul_esymm_sub_sum`: a recurrence relation for the degree-`k` power sum
...
open BigOperators MvPowerSeries
...
private def weight (k : ℕ) (t : Finset σ × σ) : MvPowerSeries σ R :=
...
private theorem weight_add_weight_pairMap {k : ℕ} (t : Finset σ × σ) (h : t ∈ pairs σ k) :
  ...
  rw [weight, weight]
  rw [mem_pairs] at h
  have h2 (n : ℕ) : -(-1 : MvPowerSeries σ R) ^ n = (-1) ^ (n + 1) := by
    rw [← neg_one_mul ((-1 : MvPowerSeries σ R) ^ n), pow_add, pow_one, mul_comm]
  rcases (em (t.snd ∈ t.fst)) with h1 | h1
  · rw [pairMap_of_snd_mem_fst σ h1]
    simp only [← prod_erase_mul t.fst (fun j ↦ (X j : MvPowerSeries σ R)) h1,
    ...
    rw [← tsub_tsub_assoc h.left h3, ← neg_neg ((-1 : MvPowerSeries σ R) ^ (card t.fst - 1)),
  ...
  · rw [pairMap_of_snd_nmem_fst σ h1]
    ...
    rw [← neg_neg ((-1 : MvPowerSeries σ R) ^ card t.fst), h2]
    simp
...
private theorem sum_filter_pairs_eq_sum_powersetCard_sum (k : ℕ)
    (f : Finset σ × σ → MvPowerSeries σ R) :
...
private theorem sum_filter_pairs_eq_sum_powersetCard_mem_filter_antidiagonal_sum (k : ℕ) (a : ℕ × ℕ)
    (ha : a ∈ (antidiagonal k).filter (fun a ↦ a.fst < k)) (f : Finset σ × σ → MvPowerSeries σ R) :
...
private theorem esymm_summand_to_weight (k : ℕ) (A : Finset σ) (h : A ∈ powersetCard k univ) :
    ∑ j in A, weight σ R k (A, j) = k * (-1) ^ k * (∏ i in A, X i : MvPowerSeries σ R) := by
...
private theorem esymm_to_weight (k : ℕ) : k * esymm σ R k =
  ...
  --rw [esymm, sum_filter_pairs_eq_sum_powersetCard_sum σ R k (fun t ↦ weight σ R k t),
    -- sum_congr rfl (esymm_summand_to_weight σ R k), mul_comm (k : MvPowerSeries σ R) ((-1) ^ k),
...
theorem mul_esymm_eq_sum (k : ℕ) : k * esymm σ R k =
  ...
  rw [NewtonIdentities.esymm_to_weight σ R k, NewtonIdentities.esymm_mul_psum_to_weight σ R k,
    ...
    neg_eq_neg_one_mul ((-1 : MvPowerSeries σ R) ^ k)]
  nth_rw 2 [← pow_one (-1 : MvPowerSeries σ R)]
...
theorem sum_antidiagonal_card_esymm_psum_eq_zero :
  ...
  suffices (-1 : MvPowerSeries σ R) ^ (k + 1) *
...
theorem psum_eq_mul_esymm_sub_sum (k : ℕ) (h : 0 < k) : psum σ R k =
  ...
  have sub_both_sides := congrArg (· - (-1 : MvPowerSeries σ R) ^ (k + 1) *
  ...
  have sub_both_sides := congrArg ((-1 : MvPowerSeries σ R) ^ (k + 1) * ·) sub_both_sides
