...
In a ring of power series in countably many variables over a commutative ring, the subalgebra of
symmetric functions is freely generated by the elementary symmetric functions (excluding
the 0th, which is simply 1). This is expressed as an isomorphism
`MvPowerSeries.equiv_SymmetricFunctions` between `MvPowerSeries ℕ R` and the symmetric subalgebra
of any ring of power series `MvPowerSeries σ R` with `σ` countable. The forward map is called
`MvPowerSeries.esymmAlgHom`.
