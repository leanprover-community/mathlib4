...
lemma monomial_finite_of_bounded_degree (k : ℕ) :
    {p | ∃ (hp1 : p.support ⊆ (HomogeneousGeneratingSetOf.irrelevant 𝒜).toFinset),
      (degreeMonomial p fun a ha ↦ (HomogeneousGeneratingSetOf.irrelevant 𝒜).deg (hp1 ha)) ≤ k}.Finite := by
...
lemma generatingSet_is_finite (k : ℕ) :
    {x : ℳ k |
      ∃ (ω : M) (_ : ω ∈ generatingSetModule A ℳ)
        (p : A →₀ ℕ) (hp1 : p.support ⊆ (HomogeneousGeneratingSetOf.irrelevant 𝒜).toFinset),
      degreeMonomial p (fun a ha ↦ (HomogeneousGeneratingSetOf.irrelevant 𝒜).deg (hp1 ha)) ≤ k ∧
  ...
  let S := {x : ℳ k |
      ∃ (ω : M) (_ : ω ∈ generatingSetModule A ℳ)
        (p : A →₀ ℕ) (hp1 : p.support ⊆ (HomogeneousGeneratingSetOf.irrelevant 𝒜).toFinset),
      degreeMonomial p (fun a ha ↦ (HomogeneousGeneratingSetOf.irrelevant 𝒜).deg (hp1 ha)) ≤ k ∧
      (x : M) = evalMonomial p • ω }
  change S.Finite
  have eq1 := calc
      S = ⋃ (ω ∈ generatingSetModule A ℳ),
            {x : ℳ k | ∃ (p : A →₀ ℕ) (hp1 : p.support ⊆ (HomogeneousGeneratingSetOf.irrelevant 𝒜).toFinset),
              degreeMonomial p (fun a ha ↦ (HomogeneousGeneratingSetOf.irrelevant 𝒜).deg (hp1 ha)) ≤ k ∧
              (x : M) = evalMonomial p • ω} := by ext s; simp
      _ = ⋃ (ω ∈ generatingSetModule A ℳ),
          ⋃ (p ∈ {p : A →₀ ℕ |
                ∃ (hp1 : p.support ⊆ (HomogeneousGeneratingSetOf.irrelevant 𝒜).toFinset),
                    degreeMonomial p (fun a ha ↦ (HomogeneousGeneratingSetOf.irrelevant 𝒜).deg (hp1 ha)) ≤ k}),
...
lemma kth_degree_eq_span (k : ℕ) :
  ...
  replace hr1 (a : A) : ∀ j ∈ (r a).support, ∃ f, f.support ⊆ (HomogeneousGeneratingSetOf.irrelevant 𝒜).toFinset ∧ j = evalMonomial f
  · specialize hr1 a
    exact hr1
  ...
  replace eq0 := calc
    ...
    _ = ∑ i in f.support.attach.filter fun i : f.support ↦ degM _ (f_support_le i.2) ≤ k,
        (∑ j in (r (f i)).support.attach,
          if degreeMonomial (p _ _ j.2) (fun a ha ↦ (HomogeneousGeneratingSetOf.irrelevant 𝒜).deg (hp1 _ _ j.2 ha)) ≤
              k - degM _ (f_support_le i.2)
          then
            GradedRing.proj 𝒜
              ((k - degM _ (f_support_le i.2)) -
                degreeMonomial (p _ _ j.2) fun a ha ↦ (HomogeneousGeneratingSetOf.irrelevant 𝒜).deg (hp1 _ _ j.2 ha))
        ...
        pick_goal 2
        · apply evalMonomial_mem 𝒜 (p _ _ hj) (fun a ha ↦ (HomogeneousGeneratingSetOf.irrelevant 𝒜).deg (hp1 _ _ hj ha))
          rintro a ha
          apply (HomogeneousGeneratingSetOf.irrelevant 𝒜).mem_deg (hp1 _ _ hj ha)
        rfl
    _ = ∑ i in f.support.attach.filter fun i : f.support ↦ degM _ (f_support_le i.2) ≤ k,
    ...
    ∑ i in (f.support.attach.filter fun i : f.support ↦ degM _ (f_support_le i.2) ≤ k).attach,
        ∑ j in ((r (f i)).support.attach.filter
          ...
          r (f i) j • (⟨(evalMonomial (p _ _ j.1.2) : A) • (i : M), by
            have mem1 := evalMonomial_mem 𝒜 (p _ _ j.1.2) (fun a ha ↦ (HomogeneousGeneratingSetOf.irrelevant 𝒜).deg (hp1 _ _ j.1.2 ha))
              (fun a ha ↦ (HomogeneousGeneratingSetOf.irrelevant 𝒜).mem_deg (hp1 _ _ j.1.2 ha))
