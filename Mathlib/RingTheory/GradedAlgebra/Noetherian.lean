...
-- lemma homogeneous_of_mem_generatingSetModule (m : M) (hm : m ∈ HomogeneousGeneratingSetOf.top A ℳ) :
--     Homogeneous ℳ m :=
--   Submodule.fg_iff_homogeneously_fg (A := A) (ℳ := ℳ) (p := ⊤) |>.mp finite_module.out
--   |>.choose_spec.1 m hm |>.1

-- lemma generatingSetModule_span :
--     (⊤ : Submodule A M) = Submodule.span A (HomogeneousGeneratingSetOf.top A ℳ) :=
--   Submodule.fg_iff_homogeneously_fg (A := A) (ℳ := ℳ) (p := ⊤) |>.mp finite_module.out
--   |>.choose_spec.2


-- open finite_module_over_degree_zero_subring

-- variable {ℳ} in
-- /--
-- Since each element in the generating set is homogeneous, we can pick a degree for each element.
-- -/
-- noncomputable def degM : (m : M) → m ∈ HomogeneousGeneratingSetOf.top A ℳ → ℕ :=
--   fun m hm ↦ homogeneous_of_mem_generatingSetModule (A := A) ℳ m hm |>.choose

-- variable {ℳ} in
-- lemma h_degM : ∀ (m : M) (hm : m ∈ HomogeneousGeneratingSetOf.top A ℳ), m ∈ ℳ (degM m hm) :=
