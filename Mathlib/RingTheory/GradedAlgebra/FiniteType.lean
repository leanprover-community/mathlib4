...
-- move to:
-- SetLike.GradeZero.instAlgebraSubtypeMemSubmoduleOfNat in Mathlib.Algebra.DirectSum.Internal
-- might cause diamonds if I generalise it too much
instance {ι A σ : Type*} [CommSemiring A]
    [SetLike σ A] [AddSubmonoidClass σ A]
    [AddCommMonoid ι] (𝒜 : ι → σ) [SetLike.GradedMonoid 𝒜] :
    Algebra (𝒜 0) A :=
  Algebra.ofSubsemiring <| SetLike.GradeZero.subsemiring 𝒜

@[simp] lemma to_be_moved {ι A σ : Type*} [CommSemiring A]
    [SetLike σ A] [AddSubmonoidClass σ A]
    [AddCommMonoid ι] (𝒜 : ι → σ) [SetLike.GradedMonoid 𝒜] (x : 𝒜 0) :
    algebraMap (𝒜 0) A x = x :=
  rfl
