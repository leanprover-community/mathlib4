...
* `Rel.IsHomogeneous`: a relation is `Homogeneous` iff
...
variable {M ι σ : Type*} [AddCommMonoid M] [SetLike σ M]
    [AddSubmonoidClass σ M] (ℳ : ι → σ)
    (r : M → M → Prop)
...
`r a b` implies that the components of `a` and `b` are related by `r` -/
def IsHomogeneous [DecidableEq ι] [DirectSum.Decomposition ℳ] : Prop :=
  ∀ {a b : M} (_ : r a b), ∀ i, r (decompose ℳ a i) (decompose ℳ b i)

variable {ℳ r} [DecidableEq ι] [DirectSum.Decomposition ℳ]

lemma IsPureHomogeneous.isHomogeneous (hr0 : r 0 0) (hr : IsPureHomogeneous ℳ r) :
    IsHomogeneous ℳ r := by
  intro a b h i
  obtain ⟨j, ha, hb⟩ := hr h
  by_cases hij : j = i
  · rw [← hij]
...
theorem IsHomogeneous.eqvGenIsHomogeneous (hr : Rel.IsHomogeneous ℳ r) :
...
theorem DirectSum.eq_of_of_mem
    {M : Type*} [AddCommMonoid M] {ι σ : Type*} {ℳ : ι → σ} {m : M} {i : ι}
    [SetLike σ M] [AddSubmonoidClass σ M] [DecidableEq ι] [DirectSum.Decomposition ℳ]
    (hm : m ∈ ℳ i) :
    decompose ℳ m = of (fun i ↦ ℳ i) i (⟨m, hm⟩ : ℳ i) := by
  conv_lhs => rw [← Subtype.coe_mk m hm]
  rw [decompose_coe]

theorem DirectSum.mem_iff_forall_ne_decompose_eq_zero
    {M : Type*} [AddCommMonoid M] {ι σ : Type*} {ℳ : ι → σ} {m : M} {i : ι}
    [SetLike σ M] [AddSubmonoidClass σ M] [DecidableEq ι] [DirectSum.Decomposition ℳ] :
  ...
  · intro hm
    ...
    by_cases hj : j = i
    · subst hj; simp
    · rw [SetLike.coe_eq_coe, hm j hj, eq_comm, of_eq_of_ne]
      exact hj
...
theorem RingConGen.Rel.isHomogeneous_of [GradedRing 𝒜] (hr : Rel.IsHomogeneous 𝒜 r) :
  ...
  | @mul a b c d _ _ k k' =>
    ...
    apply DFinsupp.rel_sum_sum (RingConGen.Rel.refl 0) (RingConGen.Rel.add)
      (gMulRight 𝒜 n (decompose 𝒜 c)) (gMulRight 𝒜 n (decompose 𝒜 d))
    intro i
    apply DFinsupp.rel_sum_sum (RingConGen.Rel.refl 0) (RingConGen.Rel.add)
    intro j
    by_cases hn : i + j = n
...
open DirectSum Function
/-
variable {A ι σ : Type*} [Semiring A]
    [SetLike σ A] [AddSubmonoidClass σ A]
    [DecidableEq ι] (𝒜 : ι → σ) (rel : A → A → Prop)


/-- The graded pieces of `RingQuot rel` with respect to `𝒜`.

This is only relevant when the relation `rel` is homogeneous. -/
def quotAddSubmonoids (i : ι) : AddSubmonoid (RingQuot rel) :=
  AddSubmonoid.map (RingQuot.mkRingHom rel).toAddMonoidHom (AddSubmonoid.ofClass (𝒜 i))

/-- The canonical additive map from the graded pieces of `A` to that of RingQuot rel. -/
def quotAddSubmonoidsMap (i : ι) :
    𝒜 i →+ quotAddSubmonoids 𝒜 rel i :=
  (RingQuot.mkRingHom rel).toAddMonoidHom.addSubmonoidMap (AddSubmonoid.ofClass (𝒜 i))

/-- The canonical additive map from the graded pieces of `A` to that of RingQuot rel. -/
def quotCompMap (i : ι) : 𝒜 i →+ quotAddSubmonoids 𝒜 rel i where
  toFun u := ⟨RingQuot.mkRingHom rel ↑u, by
      rw [quotAddSubmonoids, AddSubmonoid.mem_map]
      exact ⟨↑u, u.prop, rfl⟩⟩
  map_zero' := by simp
  map_add' u v := by simp
-/
...
@[deprecated coeLinearMap "replace with `coeLinearMap (quotSubmodules _ _)`" (since := "today")]
def quotCoeLinearMap :
    DirectSum ι (fun i ↦ quotSubmodules 𝒜 rel i) →ₗ[R] RingQuot rel :=
  toModule R ι _ fun i ↦ (quotSubmodules 𝒜 rel i).subtype
...
alias quotDecompose_left_inv'_aux := coeLinearMap_quotSubmodules_comp_lmap_quotSubmodulesMap
...
theorem coeLinearMap_quotSubmodules_injective (hrel : IsHomogeneous 𝒜 rel) :
...
lemma coeLinearMap_quotSubmodules_bijective (hrel : IsHomogeneous 𝒜 rel) :
...
lemma quotDecomposition_IsInternal (hrel : IsHomogeneous 𝒜 rel) :
...
@[reducible]
noncomputable def quotDecomposition [GradedAlgebra 𝒜] (hrel : IsHomogeneous 𝒜 rel) :
...
@[reducible]
noncomputable def quotGradedAlgebra [GradedAlgebra 𝒜] (hrel : IsHomogeneous 𝒜 rel) :
...
theorem Ideal.quotDecomposition_left_inv' :
    LeftInverse (coeLinearMap (I.quotSubmodules 𝒜)) (Ideal.quotDecompose 𝒜 I hI) := by
