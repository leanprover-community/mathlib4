...
example (R : Type*) [Semiring R] (r : R) : R →+ R where
  toFun := Mul.mul r
  map_zero' := mul_zero r
  map_add' := mul_add r

example [GradedRing 𝒜] : Mul (⨁ i, 𝒜 i) := by
  exact instMul fun i ↦ ↥(𝒜 i)

example {α β M : Type*} [CommMonoid M] (f : α × β → A)

example : ((⨁ i, 𝒜 i) →+ A) := by
  exact DirectSum.coeAddMonoidHom 𝒜

example : (⨁ i, 𝒜 i) →+ A := by
  exact DirectSum.coeAddMonoidHom 𝒜

example (i : ι) : 𝒜 i →+ (⨁ j, 𝒜 j) := by
  exact DirectSum.of (fun j ↦ 𝒜 j) i
...
#find_home! RingConGen.Rel.isHomogeneous_of
