...
/- -- Unfinished attempt to a more sensible definition
def gradedMul [SetLike.GradedMul 𝒜] (i j : ι) : 𝒜 i →+ 𝒜 j →+ 𝒜 (i + j) where
  toFun x := {
    toFun y := ⟨x * y, SetLike.GradedMul.mul_mem x.prop y.prop⟩
    map_add' y y' := by rw [← Subtype.coe_inj]; simp [mul_add]
    map_zero' := by rw [← Subtype.coe_inj]; simp }
  map_add' x x' := by ext; simp [add_mul]
  map_zero' := by ext; simp

def gradedProj [DirectSum.Decomposition 𝒜] (i : ι) : A →+ 𝒜 i where
  toFun x := decompose 𝒜 x i
  map_add' x x' := by simp
  map_zero' := by simp

def gradedMulRight [GradedRing 𝒜] (i j : ι) (y : A) : 𝒜 i →+ 𝒜 j :=
   (gradedProj 𝒜 j).comp ((AddMonoidHom.mul.flip y).comp (AddSubmonoidClass.subtype (𝒜 i)))

def gradedMulRight' [GradedRing 𝒜] (j : ι) (y : A) (i : ι) : 𝒜 i →+ A :=
   (GradedRing.proj 𝒜 j).comp ((AddMonoidHom.mul.flip y).comp (AddSubmonoidClass.subtype (𝒜 i)))

theorem gradedMulRight'_apply [GradedRing 𝒜] {j : ι} {y : A} {i : ι} {x : 𝒜 i} :
    gradedMulRight' 𝒜 j y i x = ((of (fun i ↦ ↥(𝒜 i)) i) x * (decompose 𝒜) y) j := by
  simp [gradedMulRight']

private theorem gradedMulRight'_apply'
    [DecidableEq A] [GradedRing 𝒜] {j : ι} {y : A} {i : ι} {x : 𝒜 i} :
    gradedMulRight' 𝒜 j y i x = gMulRight 𝒜 j (decompose 𝒜 y) i x := by
  simp [gradedMulRight'_apply, gMulRight, gMul]
  sorry
-/
...
theorem RingConGen.Rel.isHomogeneous_of [GradedRing 𝒜] (hr : Rel.IsHomogeneous 𝒜 r) :
  ...
  | @mul a b c d _ _ k k' =>
    ...
    apply DFinsupp.addCon_sum (r := (ringConGen r).toAddCon)
      (gMulRight 𝒜 n _) (gMulRight 𝒜 _ _)
    intro i
    apply DFinsupp.addCon_sum
    intro j
    by_cases hn : i + j = n
