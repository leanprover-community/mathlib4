...
theorem Rel.IsPureHomogeneous.ideal_ofRel_isHomogeneous [GradedRing 𝒜]
  ...
  suffices decompose 𝒜 x = DirectSum.of (fun j ↦ 𝒜 j) i (decompose 𝒜 x i) by
    ...
    rw [this]
    exact SetLike.coe_mem (((decompose 𝒜) x) i)
...
instance SetLike.GradedMonoid_RingQuot [AddMonoid ι] [SetLike.GradedMonoid 𝒜] :
...
theorem quotDecompose_left_inv'_aux :
    (coeLinearMap (quotSubmodules 𝒜 rel)).comp (lmap (quotCompMap 𝒜 rel)) =
...
theorem quotDecompose_left_inv'_aux_apply (x) :
    coeLinearMap (quotSubmodules 𝒜 rel) (lmap (quotCompMap 𝒜 rel) x) =
...
theorem quotDecompose_injective [h𝒜 : GradedAlgebra 𝒜]
    (hrel : IsHomogeneous 𝒜 rel) {x y : A}
...
theorem quotDecompose'_injective [h𝒜 : GradedAlgebra 𝒜]
    (hrel : IsHomogeneous 𝒜 rel) : Injective (quotDecompose' 𝒜 rel) := by
...
theorem quotDecompose_injective' [h𝒜 : GradedAlgebra 𝒜] (hrel : IsHomogeneous 𝒜 rel) :
...
lemma quotDecompose'_bijective [GradedAlgebra 𝒜] (hrel : IsHomogeneous 𝒜 rel) :
    Bijective (quotDecompose' 𝒜 rel) :=
...
lemma quotDecomposition_IsInternal [GradedAlgebra 𝒜] (hrel : IsHomogeneous 𝒜 rel) :
