...
lemma of_forall_exists_mul_mem_range [QuasiFinite R S] (f : S →ₐ[R] T)
    ...
    QuasiFinite R T := by
  let φ : Localization ((IsUnit.submonoid T).comap (MonoidHom.ofClass f)) →ₐ[R] T :=
    IsLocalization.liftAlgHom (M := (IsUnit.submonoid T).comap (MonoidHom.ofClass f))
      (f := f) (by simp [IsUnit.mem_submonoid_iff])
  suffices Function.Surjective φ from .of_surjective_algHom φ this
  intro x
  obtain ⟨s, hs, t, ht⟩ := H x
  refine ⟨IsLocalization.mk' (M := (IsUnit.submonoid T).comap (MonoidHom.ofClass f)) _ t
    ⟨s, hs⟩, ?_⟩
