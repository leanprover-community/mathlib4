...
variable [IsLocalization M Rₘ] [IsLocalization (M.map (algebraMap R S : R →* S)) Sₘ]
include M
...
theorem localization_map [FormallyEtale R S] : FormallyEtale Rₘ Sₘ := by
  have : FormallyEtale S Sₘ := FormallyEtale.of_isLocalization (M.map (algebraMap R S : R →* S))
