...
theorem Ideal.eq_prime_pow_mul_coprime [DecidableEq (Ideal T)] {P I : Ideal T}
    [hpp : P.IsPrime] (hp0 : P ≠ ⊥) (hI : I ≠ ⊥) :
  ...
  · refine P.sup_multiset_prod_eq_top (fun p hpi ↦ ?_)
    ...
    have hpm : p.IsMaximal := (isPrime_of_prime hp).isMaximal hp.ne_zero
    exact (hpp.isMaximal hp0).coprime_of_ne hpm (of_mem_filter hpi)
