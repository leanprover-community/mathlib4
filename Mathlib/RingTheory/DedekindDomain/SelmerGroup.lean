...
def valuation : K⟮S,n⟯ →* S → Multiplicative (ZMod n) where
  ...
  map_mul' x y := by simp only [Submonoid.coe_mul, Subgroup.coe_toSubmonoid, map_mul]; sorry -- rfl
...
def fromUnit {n : ℕ} : Rˣ →* K⟮(∅ : Set <| HeightOneSpectrum R),n⟯ where
  ...
  map_mul' _ _ := sorry -- by simp only [RingHom.toMonoidHom_eq_coe, map_mul, MonoidHom.mem_range,
