...
lemma foo (k : ℕ) (n : ℤ) (hn : 0 ≤ n) : (k ! : ℤ) ∣ ∏ i ∈ Finset.range k, (n + i) := by
...
lemma stuff (k : ℕ) (n : ℤ) : (k ! : ℤ) ∣ ∏ i ∈ Finset.range k, (n + i) := by
  by_cases hn : 0 ≤ n
  · exact foo k n hn
  · simp at hn
    by_cases hnk : 0 < n + k
    · have hnnk : -n < k := by linarith
      have hnnnk : 0 ≤ -n := by linarith
      · have : ∏ i ∈ Finset.range k, (n + ↑i) = 0 := by
          apply Finset.prod_eq_zero_iff.mpr
          simp_all only [Finset.mem_range]
          obtain ⟨x, hx⟩ := Int.eq_ofNat_of_zero_le hnnnk
          exact ⟨x, by omega⟩
        apply Int.modEq_zero_iff_dvd.mp (congrFun (congrArg HMod.hMod this) ↑k !)
    · simp at hnk
      rw [← dvd_abs, Finset.abs_prod]
      have := foo k (-n) (by linarith)
...
def natPOrdering : (Set.univ : Set ℤ).pOrdering p where
  ...
  -- k is our step (kpos says it's at least 1)
  -- s is an arbitrary other candidate
  emultiplicity_le := fun k ⟨s, hs⟩ kpos ↦ by
    ...
    use n
    constructor
    · have := k.descFactorial_eq_prod_range k
      zify at this
      zify at this
      erw [emultiplicity_eq_coe]
      constructor
