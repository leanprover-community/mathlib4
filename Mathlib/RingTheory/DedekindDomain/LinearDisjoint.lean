...
/-!
# Different ideal in disjoint extensions

Let `A ⊆ B` be a finite extension of Dedekind domains and assume that `A ⊆ R₁, R₂ ⊆ B` are two
subrings such that `R₁ ⊔ R₂ = B`, their fractions fields are linearly disjoint
(over the fraction field of `A`) and that `𝓓(R₁/A)` and `𝓓(R₂/A)` are coprime,
where `𝓓` denote the different ideal.

# Main results

* `FractionalIdeal.differentIdeal_eq_map_differentIdeal_of_linearDisjoint`: `𝓓(B/R₁) = 𝓓(R₂/A)`
* `FractionalIdeal.differentIdeal_eq_differentIdeal_mul_differentIdeal_of_linearDisjoint`:
  `𝓓(B/A) = 𝓓(R₁/A) * 𝓓(R₂/A)`.
...
theorem traceDual_le_span_map_traceDual_of_linearDisjoint [Module.Free A R₂]
...
theorem differentIdeal_dvd_map_differentIdeal_of_linearDisjoint [Algebra.IsIntegral R₂ B]
    [Module.Free A R₂] [Algebra.IsSeparable (FractionRing A) (FractionRing R₂)]
    [IsLocalization (Algebra.algebraMapSubmonoid R₂ A⁰) F₂] (h₁ : F₁.LinearDisjoint F₂)
    (h₂ : F₁ ⊔ F₂ = ⊤) :
  ...
  · have := Submodule.span_mono (R := B) <|
      traceDual_le_span_map_traceDual_of_linearDisjoint A K L B R₁ R₂ h₁ h₂
...
include K L in
theorem differentIdeal_eq_map_differentIdeal_of_linearDisjoint [Module.Free A R₂]
    [Algebra.IsSeparable (FractionRing A) (FractionRing R₂)]
    (h₁ : F₁.LinearDisjoint F₂) (h₂ : F₁ ⊔ F₂ = ⊤)
    (h₃ : IsCoprime ((differentIdeal A R₁).map (algebraMap R₁ B))
      ((differentIdeal A R₂).map (algebraMap R₂ B))) :
    differentIdeal R₁ B = Ideal.map (algebraMap R₂ B) (differentIdeal A R₂) := by
  apply dvd_antisymm
  · exact differentIdeal_dvd_map_differentIdeal_of_linearDisjoint A K L B R₁ R₂ h₁ h₂
  · exact map_differentIdeal_dvd_differentIdeal A B R₁ R₂ h₃

theorem differentIdeal_eq_differentIdeal_mul_differentIdeal_of_linearDisjoint
    [Module.Free A R₂] [Algebra.IsSeparable (FractionRing A) (FractionRing R₂)]
    (h₁ : F₁.LinearDisjoint F₂) (h₂ : F₁ ⊔ F₂ = ⊤)
    (h₃ : IsCoprime ((differentIdeal A R₁).map (algebraMap R₁ B))
      ((differentIdeal A R₂).map (algebraMap R₂ B))) :
  ...
  rwa [← differentIdeal_eq_map_differentIdeal_of_linearDisjoint A K L B R₁ R₂ h₁ h₂ h₃,
    mul_comm] at this
