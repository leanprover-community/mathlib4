...
-/
lemma Ideal.quotMulExact {a : R} (I : Ideal R) :
...
theorem CommRing.ord_mul {a b : R} (hb : b ∈ nonZeroDivisors R) :
  ...
  have :=  Module.length_eq_add_of_exact (Ideal.mulQuot b (Ideal.span {a}))
          ...
          (Ideal.quotOfMulSurjective (Ideal.span {a})) (Ideal.quotMulExact (Ideal.span {a}))
...
def CommRing.ordMonoidWithZeroHom [Nontrivial R] : R →*₀ ℕₘ₀ where
  ...
  map_mul' := by
    intro x y
    split_ifs
    · exact CommRing.ord_mul R (by expose_names; exact h_2)
