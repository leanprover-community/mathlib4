...
-/
lemma Ideal.addExact_mulQuot_quotOfMul {a : R} (I : Ideal R) :
    Function.AddExact (Ideal.mulQuot a I) (Ideal.quotOfMul a I) := by
  simp only [addExact_iff]
...
alias Ideal.exact_mulQuot_quotOfMul := Ideal.addExact_mulQuot_quotOfMul
...
theorem ord_mul {a b : R} (hb : b ∈ nonZeroDivisors R) :
  ...
  have := Module.length_eq_add_of_addExact (Ideal.mulQuot b (Ideal.span {a}))
          (Ideal.quotOfMul b (Ideal.span {a})) (Ideal.mulQuot_injective (Ideal.span {a}) hb)
          (Ideal.quotOfMul_surjective (Ideal.span {a}))
          (Ideal.addExact_mulQuot_quotOfMul (Ideal.span {a}))
