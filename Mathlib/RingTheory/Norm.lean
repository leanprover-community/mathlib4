...
theorem norm_eq_prod_roots [IsSeparable K L] [FiniteDimensional K L] {x : L}
    ...
    algebraMap K F (norm K x) =
      (minpoly K x).[F]-roots.prod ^ finrank K⟮x⟯ L := by
