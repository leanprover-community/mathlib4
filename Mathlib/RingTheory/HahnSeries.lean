...
@[simp]
lemma single_eq_zero_iff {a : Γ} {r : R} : single a r = 0 ↔ r = 0 := by
  constructor
  · contrapose!
...
lemma min_order_le_order_add {Γ} [LinearOrderedCancelAddCommMonoid Γ] {x y : HahnSeries Γ R}
...
@[simp]
lemma ofPowerSeries_X_pow {R} [CommSemiring R] (n : ℕ) :
