...
import Mathlib.RingTheory.LocalRing.EmbeddingDimension
import Mathlib.RingTheory.LocalRing.LocalRingDimension
...
/-- If `(R,m)` is a regular local ring and `x ∈ m \ m²`, then `R ⧸ x` is regular. -/
...
/-- If `R` is a noetherian local ring and `x` is a prime element not contained in any
...
/-- Inductive statement:  If `R` is a regular local ring of dimension `n`, then `R` is a domain. -/
...
/-- Regular local rings are integral domains. Eisenbud, Cor. 10.14 -/
@[stacks 00NP]
