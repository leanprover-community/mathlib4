...
/--
If `M` is a finitely presented module over a local ring `(R, 𝔪)` such that `m ⊗[R] M → M` is
injective, then `M` is free.
-/
theorem Module.free_of_maximalIdeal_rTensor_injective [Module.FinitePresentation R P]
