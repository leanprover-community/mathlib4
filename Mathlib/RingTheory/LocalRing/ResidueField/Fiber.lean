...
lemma Ideal.ResidueField.exists_smul_eq_one_tmul
...
noncomputable def PrimeSpectrum.preimageEquivTensorResidueField (p : PrimeSpectrum R) :
  ...
  right_inv q := by
    ext x
    obtain ⟨r, hr, s, e⟩ := Ideal.ResidueField.exists_smul_eq_one_tmul _ x
