...
/-- If `R` is a ring with finite Krull dimension, then `finRingKrullDim R = ringKrullDim R`. -/
...
/-- If `R` is a ring, `I ⊆ R` is a prime ideal, and `P` is a minimal prime ideal of `R`, then
...
/-- If `(R,m)` is noetherian local ring, then there exists a subset `s ⊆ R` of cardinality
...
/-- If `(R,m)` is a noetherian local ring and `x ∈ m` then
...
theorem IsLocalRing.quotient_singleton_ringKrullDim [IsLocalRing R] [IsNoetherianRing R] (x : R)
  ...
  have smspan : span S' = span (S ∪ {x}) := by simp_all only [Finset.card_map, Finset.coe_map,
    Function.Embedding.coeFn_mk, Set.union_singleton, Set.Finite.coe_toFinset, q, S, S']
  ...
  have maxmin : (maxl R) ∈ (span S').minimalPrimes := by
    ...
    have : (maxl R) ∈ comap (Ideal.Quotient.mk (span {x})) '' (span ↑s).minimalPrimes := by
      simp_all only [Finset.card_map, Finset.coe_map, Function.Embedding.coeFn_mk,
        Set.union_singleton, Set.Finite.coe_toFinset, Set.mem_image, S', S, q]
      apply Exists.intro
      · apply And.intro
        · exact hs2
        · rfl
...
/-- If `(R,m)` is a noetherian local ring and `x ∈ m \ m^2`, then
...
/-- If `(R,m)` is a noetherian local ring, then `ringKrullDim R ≤ spanFinrank(m)`. -/
