...
/-- If `σ : M → N` is an injective `R`-module map and `p ⊆ M` is a submodule, then
...
/-- if `M` is an `R`-module and `N ⊆ M` is an `R`-submodule, then
...
/-- For a finite dimensional vector space `V` over a field, `finrank(V) = spanFinrank(V)`. -/
...
/-- If `(R,m)` is a noetherian local ring, then `embDim R = spanFinrank(m)`. -/
...
/-- If `x ∈ I/I^2` and `y ∈ I` is a choice of representative for `x` (i.e.,
...
/-- If `R` is a noetherian local ring and `x ∈ m \ m^2`, then there exists a
  ...
  have scard : #s = #s'' + 1 := by
    ...
    have d : x' ∉ s'' := by
      ...
      have c : ((maxl R).toCotangent x') ∉ S \ {(maxl R).toCotangent x'} := by
        exact Set.notMem_diff_of_mem rfl
      exact c a
...
/-- If `(R,m)` is a noetherian local ring and `R/x` is nontrivial, then
...
/-- If `(R,m)` is a noetherian local ring such that `R/x` is nontrivial and `x ∈ m \ m^2`,
...
/-- If `R` is a noetherian ring and `I ⊆ R` is an ideal, then `spanRank(I) = spanFinrank(I)`. -/
...
/-- If `(R,m)` is a noetherian local ring and `x ∈ m \ m²`,
