...
theorem finite_stableUnderBaseChange : StableUnderBaseChange @Finite := by
  ...
  replace h : Module.Finite R T := by
    rw [RingHom.Finite] at h; convert h; ext; sorry; sorry-- simp_rw [Algebra.smul_def]; rfl
  suffices Module.Finite S (S ⊗[R] T) by
    rw [RingHom.Finite]; convert this; congr; sorry; sorry; sorry -- ext; simp_rw [Algebra.smul_def]; rfl
  exact inferInstance
