...
theorem finitePresentation_localizationPreserves : LocalizationPreserves @FinitePresentation := by
  ...
  let g' : MvPolynomial (Fin n) R' →ₐ[R'] S' := IsLocalization.mapₐ M g
  let k : RingHom.ker g →ₗ[MvPolynomial (Fin n) R] RingHom.ker g' := AlgHom.toKerIsLocalization M g
  have : IsLocalizedModule MX k := AlgHom.toKerIsLocalization_isLocalizedModule M g
  have : Module.Finite (MvPolynomial (Fin n) R) (ker g) := Module.Finite.iff_fg.mpr hgker
  exact ⟨n, g', IsLocalization.mapₐ_surjective_of_surjective M g hgsurj,
