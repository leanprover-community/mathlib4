...
@[simp]
lemma rel_mk {s : Con R} {h a b} : RingCon.mk s h a b ↔ s a b :=
  Iff.rfl
...
@[simp, norm_cast]
lemma coe_natCast (n : ℕ) : (↑(n : R) : c.Quotient) = n :=
  rfl
...
@[simp, norm_cast]
lemma coe_intCast (n : ℕ) : (↑(n : R) : c.Quotient) = n :=
  rfl
...
@[simp, norm_cast]
lemma coe_iInf {ι : Sort*} (f : ι → RingCon R) : ⇑(iInf f) = ⨅ i, ⇑(f i) := by
