...
lemma IsAssociatedPrime.isPrime (h : IsAssociatedPrime I M) : I.IsPrime := h.1
#align is_associated_prime.is_prime IsAssociatedPrime.isPrime
lemma IsAssociatedPrime.map_of_injective (h : IsAssociatedPrime I M) (hf : Function.Injective f) :
...
lemma LinearEquiv.isAssociatedPrime_iff (l : M ≃ₗ[R] M') :
...
lemma associatedPrimes.subset_of_injective (hf : Function.Injective f) :
...
lemma LinearEquiv.AssociatedPrimes.eq (l : M ≃ₗ[R] M') :
...
lemma IsAssociatedPrime.annihilator_le (h : IsAssociatedPrime I M) :
...
lemma IsAssociatedPrime.eq_radical (hI : I.IsPrimary) (h : IsAssociatedPrime J (R ⧸ I)) :
    J = I.radical := by
  obtain ⟨hJ, x, e⟩ := h
  have : x ≠ 0 := by
    rintro rfl
