...
lemma bot_prime {R : Type*} [Ring R] [IsDomain R] : (⊥ : Ideal R).IsPrime :=
...
lemma exists_maximal [Nontrivial α] : ∃ M : Ideal α, M.IsMaximal :=
...
lemma span_singleton_lt_span_singleton [CommRing β] [IsDomain β] {x y : β} :
...
lemma factors_decreasing [CommRing β] [IsDomain β] (b₁ b₂ : β) (h₁ : b₁ ≠ 0) (h₂ : ¬IsUnit b₂) :
...
lemma isField_iff_isSimpleOrder_ideal : IsField R ↔ IsSimpleOrder (Ideal R) := by
...
lemma ne_bot_of_isMaximal_of_not_isField [Nontrivial R] {M : Ideal R} (max : M.IsMaximal)
