...
/--
If `x₁ x₂ ... xₙ : R` then `⟪x₁,x₂,...,xₙ : R⟫` is the `Ideal R` generated by these elements. -/
macro_rules
  | `(⟪ $xs,* ⟫) => do
      ``(Ideal.span $(← mkInsertTerm xs.getElems))
  | `(⟪ $xs,* : $R ⟫) => do
      ``((Ideal.span $(← mkInsertTerm xs.getElems) : Ideal $R))
...
@[simp]
theorem span_singleton_one : ⟪ 1 : α  ⟫  = ⊤ :=
...
theorem mem_span_singleton' {x y : α} : x ∈ ⟪ y ⟫ ↔ ∃ a, a * y = x :=
...
@[simp]
theorem span_singleton_le_iff_mem {x : α} : ⟪ x ⟫ ≤ I ↔ x ∈ I :=
...
theorem span_pair_comm {x y : α} : ⟪ x, y ⟫ = ⟪ y, x ⟫ := by
...
@[simp]
theorem span_pair_zero (x : α) : ⟪ x, 0 ⟫ = ⟪ x ⟫ := by
...
@[simp]
theorem span_pair_eq_span_left_iff_dvd {x y : α} : ⟪ x, y ⟫ = ⟪ x ⟫ ↔ x ∣ y := by
  rw [Ideal.span_insert, sup_eq_left, span_singleton_le_span_singleton]

@[simp]
theorem span_pair_eq_span_right_iff_dvd {x y : α} : ⟪ x, y ⟫ = ⟪ y ⟫ ↔ y ∣ x := by
...
theorem span_singleton_mul_right_unit {a : α} (h2 : IsUnit a) (x : α) :
    span ({x * a} : Set α) = ⟪ x ⟫ := by rw [mul_comm, span_singleton_mul_left_unit h2]
...
@[simp]
theorem span_singleton_neg : ⟪ -x ⟫ = ⟪ x ⟫ := by
  ext
...
@[simp]
theorem span_pair_neg : ⟪ x, -y ⟫ = ⟪ x, y ⟫ := by
  rw [span_pair_comm, span_insert_neg, span_pair_comm]

@[simp]
theorem span_pair_add_right : ⟪ x + y, y ⟫ = ⟪ x, y ⟫ := by
  ext
...
@[simp]
theorem span_pair_add_left : ⟪ x, y + x ⟫ = ⟪ x, y ⟫ := by
  rw [span_pair_comm, span_pair_add_right, span_pair_comm]

@[simp]
theorem span_pair_right_add : ⟪ y + x, y ⟫ = ⟪ x, y ⟫ := by
  rw [add_comm, span_pair_add_right]

@[simp]
theorem span_pair_left_add : ⟪ x, x + y ⟫ = ⟪ x, y ⟫ := by
  rw [add_comm, span_pair_add_left]

@[simp]
theorem span_pair_sub_right : ⟪ x - y, y ⟫ = ⟪ x, y ⟫ := by
  rw [← span_pair_add_right, sub_add_cancel]

@[simp]
theorem span_pair_sub_left : ⟪ x, y - x ⟫ = ⟪ x, y ⟫ := by
  rw [← span_pair_add_left, sub_add_cancel]

@[simp]
theorem span_pair_sub_right' : ⟪ x, x - y ⟫ = ⟪ x, y ⟫ := by
  rw [← span_pair_sub_left, sub_sub_cancel_left, span_pair_neg]

@[simp]
theorem span_pair_sub_left' : ⟪ y - x, y ⟫ = ⟪ x, y ⟫ := by
  rw [← span_pair_sub_right, sub_sub_cancel_left, span_insert_neg]

@[simp]
theorem span_singleton_abs [LinearOrder α] : ⟪ |x| ⟫ = ⟪ x ⟫ := by
...
@[simp]
theorem span_pair_abs [LinearOrder α] : ⟪ x, |y| ⟫ = ⟪ x, y ⟫ := by
...
@[simp]
theorem span_pair_add_right_mul : ⟪ x + y * z, y ⟫ = ⟪ x, y ⟫ := by
  ext
  simp_rw [mem_span_pair]
...
@[simp]
theorem span_pair_add_left_mul : ⟪ x, y + x * z ⟫ = ⟪ x, y ⟫ := by
  rw [span_pair_comm, span_pair_add_right_mul, span_pair_comm]

@[simp]
theorem span_pair_right_mul_add : ⟪ y * z + x, y ⟫ = ⟪ x, y ⟫ := by
  rw [add_comm, span_pair_add_right_mul]

@[simp]
theorem span_pair_left_mul_add : ⟪ x, x * z + y ⟫ = ⟪ x, y ⟫ := by
  rw [add_comm, span_pair_add_left_mul]

@[simp]
theorem span_pair_add_mul_right : ⟪ x + z * y, y ⟫ = ⟪ x, y ⟫ := by
  rw [mul_comm, span_pair_add_right_mul]

@[simp]
theorem span_pair_add_mul_left : ⟪ x, y + z * x ⟫ = ⟪ x, y ⟫ := by
  rw [mul_comm, span_pair_add_left_mul]

@[simp]
theorem span_pair_mul_right_add : ⟪ z * y + x, y ⟫ = ⟪ x, y ⟫ := by
  rw [mul_comm, span_pair_right_mul_add]

@[simp]
theorem span_pair_mul_left_add : ⟪ x, z * x + y ⟫ = ⟪ x, y ⟫ := by
  rw [mul_comm, span_pair_left_mul_add]

@[simp]
theorem span_pair_sub_right_mul : ⟪ x - y * z, y ⟫ = ⟪ x, y ⟫ := by
  rw [← span_pair_add_right_mul, sub_add_cancel]

@[simp]
theorem span_pair_sub_left_mul : ⟪ x, y - x * z ⟫ = ⟪ x, y ⟫ := by
  rw [← span_pair_add_left_mul, sub_add_cancel]

@[simp]
theorem span_pair_sub_mul_right : ⟪ x - z * y, y ⟫ = ⟪ x, y ⟫ := by
  rw [mul_comm, span_pair_sub_right_mul]

@[simp]
theorem span_pair_sub_mul_left : ⟪ x, y - z * x ⟫ = ⟪ x, y ⟫ := by
