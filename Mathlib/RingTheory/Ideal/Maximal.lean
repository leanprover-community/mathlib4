...
theorem irreducible_of_isMaximal_of_eq_span_singleton_of_not_isIdempotentElem {a : α}
    (max : I.IsMaximal) (hI : I = Ideal.span {a})
    (idem : ∀ x, I = Ideal.span {x} → ¬IsIdempotentElem x) :
    Irreducible a := by
  subst hI
  constructor
  · intro ha
...
theorem irreducible_of_isMaximal_span_singleton [IsDomain α] {a : α}
    ...
    Irreducible a := by
  apply irreducible_of_isMaximal_of_eq_span_singleton_of_not_isIdempotentElem _ max rfl
  intro x hx
