...
/-- In a Noetherian integral domain, the ideal generated by a prime element has height one. -/
theorem Ideal.height_span_singleton_eq_one_of_prime [IsDomain R] {p : R} (hp : Prime p) :
  ...
  · exact height_le_one_of_isPrincipal_of_mem_minimalPrimes _ _
      ((Ideal.span {p}).minimalPrimes_eq_subsingleton_self ▸ Set.mem_singleton _)
  · simp only [height_eq_primeHeight, ENat.one_le_iff_ne_zero, ne_eq, primeHeight_eq_zero_iff,
