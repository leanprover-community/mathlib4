...
instance (priority := low) (P : Submodule R M) : (N.colon P).IsTwoSided where
  mul_mem_of_left {r} s hr p hp := by
    obtain ⟨p, hp, rfl⟩ := hp
    exact hr ⟨_, P.smul_mem _ hp, (mul_smul ..).symm⟩

@[simp]
--theorem colon_top {I : Ideal R} [I.IsTwoSided] : I.colon (⊤ : Ideal R) = I := by
...
theorem annihilator_quotient : Module.annihilator R (M ⧸ N) = N.colon (Set.univ : Set M) := by
  ...
  rw [← annihilator_top (R := R) (M := M ⧸ N), htop,
    annihilator_map_mkQ_eq_colon (N := N) (P := ⊤), top_coe]
