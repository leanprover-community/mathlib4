...
instance (priority := 1) [K.IsTwoSided] : (comap f K).IsTwoSided :=
...
instance (priority := 1) (f : R →+* S) [RingHomSurjective f] (I : Ideal R) [I.IsTwoSided] :
...
instance (priority := 1) : (ker f).IsTwoSided := inferInstanceAs (Ideal.comap f ⊥).IsTwoSided
...
instance (priority := 1) : (Module.annihilator R M).IsTwoSided :=
