...
theorem mem_of_span_eq_top_of_smul_pow_mem (M' : Submodule R M) (s : Set R) (hs : Ideal.span s = ⊤)
    (x : M) (H : ∀ r : s, ∃ n : ℕ, (Pow.pow r n : R) • x ∈ M') : x ∈ M' := by
  obtain ⟨s', hs₁, hs₂⟩ := (Ideal.span_eq_top_iff_finite _).mp hs
  replace H : ∀ r : s', ∃ n : ℕ, (Pow.pow r n  : R) • x ∈ M' := fun r => H ⟨_, hs₁ r.2⟩
  choose n₁ n₂ using H
...
def radical  (I : Ideal R) : Ideal R where
  ...
  smul_mem' := sorry
  -- fun {r s} ⟨n, hsni⟩ => ⟨n, (mul_pow r s n).symm ▸ I.mul_mem_left (r ^ n) hsni⟩
...
theorem subset_union {R : Type u} [Ring R] {I J K : Ideal R} :
  ...
  ⟨fun h =>
    ...
    fun h =>
    Or.casesOn h (fun h => Set.Subset.trans h <| Set.subset_union_left J K) fun h =>
...
theorem range_finsuppTotal : (finsuppTotal ι M I v).range = I • Submodule.span R (Set.range v) := by
  ext
