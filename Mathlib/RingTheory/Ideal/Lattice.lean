...
instance (priority := 1) : IsTwoSided (⊥ : Ideal α) :=
  ⟨fun _ h ↦ by rw [h, zero_mul]; exact zero_mem _⟩

instance (priority := 1) : IsTwoSided (⊤ : Ideal α) := ⟨fun _ _ ↦ trivial⟩

instance (priority := 1) {ι} (I : ι → Ideal α) [∀ i, (I i).IsTwoSided] : (⨅ i, I i).IsTwoSided :=
