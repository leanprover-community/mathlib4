...
theorem trace_eq_sum_roots [FiniteDimensional K L] {x : L}
    ...
    algebraMap K F (Algebra.trace K L x) =
      finrank K⟮x⟯ L • (minpoly K x).[F]-roots.sum := by
