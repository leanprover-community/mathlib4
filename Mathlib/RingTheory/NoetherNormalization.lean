...
/-- Let `I` be an ideal containing `f`. `hom1` is a homomorphism from `k[X_0,...X_{n-1}]`
to `k[X_1,...X_n][X]/φ(T(I))`, where `φ` is the isomorphism from `k[X_0,...X_{n-1}]`
to `k[X_1,...X_n][X]`. And it is integral by the lemma above. -/
...
/-- Use induction to prove there is an injective map from `k[X_0,...X_{r-1}]`
to `k[X_0,...X_n]/I`. -/
