...
lemma truncateFun_nsmul (m : ℕ) (x : 𝕎 R) : truncateFun n (m • x) = m • truncateFun n x := by
  witt_truncateFun_tac
#align witt_vector.truncate_fun_nsmul WittVector.truncateFun_nsmul

lemma truncateFun_zsmul (m : ℤ) (x : 𝕎 R) : truncateFun n (m • x) = m • truncateFun n x := by
  witt_truncateFun_tac
...
lemma truncateFun_natCast (m : ℕ) : truncateFun n (m : 𝕎 R) = m := rfl
#align witt_vector.truncate_fun_nat_cast WittVector.truncateFun_natCast

lemma truncateFun_intCast (m : ℤ) : truncateFun n (m : 𝕎 R) = m := rfl
