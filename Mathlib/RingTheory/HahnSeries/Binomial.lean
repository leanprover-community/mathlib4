...
theorem support_one_sub_single_pow {g : Γ} (hg : 0 < g) {r : R} {n : ℤ} :
  ...
  cases n with
  | ofNat n =>
    simp_all only [Int.ofNat_eq_coe, zpow_natCast, Units.val_pow_eq_pow_val, IsUnit.unit_spec,
      ne_eq, SetLike.mem_coe]
    refine Subset.trans ?_ ?_
    refine support_pow_subset_closure (1 - (single g) r) n
    sorry
  | negSucc _ => sorry -- use support_pow_subset_closure
