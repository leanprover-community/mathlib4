...
/-! When `Γ` has a linear order, any nonzero Hahn series on `Γ` has a unique lowest-order term, and
all remaining terms have strictly higher order. -/
...
theorem add_single_order_of_ne_order (hy : y ≠ 0) : x.order ≠ y.order := by
  by_contra h
  exact (Eq.mpr (congrArg (fun g ↦ y.coeff g ≠ 0) h) (coeff_order_ne_zero hy))
...
theorem support_subset_add_single_support : y.support ⊆ x.support :=
  ...
  · exact ((fun _ ↦ hg (Eq.mpr (congrArg (fun g ↦ y.coeff g = 0) hgx)
