...
* `HahnSeries.archimedeanClass₀_orderIso`: `ArchimedeanClass₀` of `Lex (HahnSeries Γ R)` can be
  decomposed by `Γ`.
  ...
  · -- general case: x and y are not zero
    ...
    · -- mk x ≤ mk y → mk x.leadingCoeff ≤ mk y.leadingCoeff
      ...
      · -- impossible case: x and y differ before their leading coefficients
        have hjlt' : WithTop.some j < (ofLex |y|).orderTop := h'.symm ▸ hjlt
...
def archimedeanClass₀_orderHom :
...
private noncomputable
def archimedeanClass₀_orderHom_inv :
...
def archimedeanClass₀_orderIso :
  ...
  apply OrderIso.ofHomInv (archimedeanClass₀_orderHom Γ R) (archimedeanClass₀_orderHom_inv Γ R)
  · ext x
    cases x with | h x
    obtain ⟨order, coeff⟩ := x
    induction coeff using ArchimedeanClass₀.ind with | mk a ha
    simp [archimedeanClass₀_orderHom, archimedeanClass₀_orderHom_inv, ha]
  · ext x
...
noncomputable
def archimedeanClass₀_orderIso_order : ArchimedeanClass₀ (Lex (HahnSeries Γ R)) ≃o Γ :=
  have : Unique (ArchimedeanClass₀ R) := (nonempty_unique _).some
  (archimedeanClass₀_orderIso Γ R).trans (Prod.Lex.prodUnique _ _)

theorem archimedeanClass₀_orderIso_order_apply {x : Lex (HahnSeries Γ R)} (h : x ≠ 0) :
    WithTop.some (archimedeanClass₀_orderIso_order Γ R (ArchimedeanClass₀.mk h)) =
    (ofLex x).orderTop := by
  simp [archimedeanClass₀_orderIso_order]
...
def archimedeanClass_orderIso : ArchimedeanClass (Lex (HahnSeries Γ R)) ≃o WithTop Γ :=
  ...
  (OrderIso.withTopCongr (archimedeanClass₀_orderIso_order _ _))
...
theorem archimedeanClass_orderIso_apply (x : Lex (HahnSeries Γ R)) :
  ...
  · rw [OrderIso.trans_apply, ArchimedeanClass₀.withTop_orderIso_symm_apply h]
    rw [OrderIso.withTopCongr_apply_some, archimedeanClass₀_orderIso_order_apply h]
