...
`WithConv (A →ₗ[R] A)`; on `𝒜 N` it computes the antipode. -/
...
section
variable [SetLike.GradedComul 𝒜]

/-- If `f` vanishes on the degree-zero part, then `f ^ k` vanishes on `𝒜 m` for `m < k`. -/
lemma convPow_apply_eq_zero_of_lt {f : WithConv (A →ₗ[R] A)} (hf : ∀ x ∈ 𝒜 0, f.ofConv x = 0)
    {m k : ℕ} (hmk : m < k) {x : A} (hx : x ∈ 𝒜 m) : (f ^ k).ofConv x = 0 := by
...
lemma takeuchiSeries_summand_apply_eq_zero_of_lt {m k : ℕ} (hmk : m < k) {x : A}
    (hx : x ∈ 𝒜 m) :
    (((1 - toConv LinearMap.id : WithConv (A →ₗ[R] A))) ^ k).ofConv x = 0 :=
  convPow_apply_eq_zero_of_lt 𝒜 (fun a ha ↦ by
    change algebraMap R A (counit a) - a = 0
    rw [Algebra.algebraMap_eq_smul_one,
      ← GradedAlgebra.IsConnected.eq_counit_smul_one (𝒜 := 𝒜) ha, sub_self]) hmk hx
...
noncomputable def takeuchiAntipode : A →ₗ[R] A :=
  (toModule R ℕ A
    (fun n => ((takeuchiSeries n).ofConv).comp (𝒜 n).subtype)).comp
...
lemma takeuchiAntipode_apply_of_le {m N : ℕ} (hmN : m ≤ N) {a : A} (ha : a ∈ 𝒜 m) :
