...
def Hom.subToKer (f g : Hom P P') : P.Ring →ₗ[R] P'.ker := by
  ...
  intro x
  simp only [_root_.sub_apply, AlgHom.toLinearMap_apply, ker,
...
lemma CotangentSpace.map_sub_map (f g : Hom P P') :
  ...
  | tmul x y =>
    ...
    | add => simp only [map_add, tmul_add, LinearMap.coe_comp, LinearMap.coe_restrictScalars,
      Function.comp_apply, *]
    | tmul =>
      simp only [Derivation.tensorProductTo_tmul, tmul_smul, smul_tmul', _root_.sub_apply,
...
lemma Cotangent.map_sub_map (f g : Hom P P') :
  ...
  simp only [_root_.sub_apply, map_mk, LinearMap.coe_comp, Function.comp_apply,
...
lemma H1Cotangent.map_eq (f g : Hom P P') : map f = map g := by
  ...
  rw [← sub_eq_zero, ← Cotangent.val_sub, ← _root_.sub_apply, Cotangent.map_sub_map]
