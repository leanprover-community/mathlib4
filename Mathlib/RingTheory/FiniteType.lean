...
theorem mem_adjoin_support (f : R[M]) : f ∈ adjoin R (of' R M '' f.coeff.support) := by
  suffices span R (of' R M '' f.coeff.support) ≤
      Subalgebra.toSubmodule (adjoin R (of' R M '' f.coeff.support)) by
