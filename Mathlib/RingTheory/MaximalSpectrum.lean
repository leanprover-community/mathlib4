...
theorem iInf_localization_eq_bot : (⨅ v : MaximalSpectrum R,
  ...
  · contrapose
    ...
    apply hd (hle ⟨n, _⟩)
    simp [denom, Algebra.smul_def, mul_left_comm, mul_inv_cancel₀ <| (map_ne_zero_iff _ <|
      NoZeroSMulDivisors.algebraMap_injective R K).mpr fun h ↦ hd (by apply h ▸ max.zero_mem)]
  · rintro ⟨y, rfl⟩ ⟨v, hv⟩
