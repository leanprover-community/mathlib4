...
theorem degree_sub_of_ne {f g : MvPolynomial σ R}
    (h : m.degree f ≠ m.degree g) :
    m.toSyn (m.degree (f - g)) = m.toSyn (m.degree f) ⊔ m.toSyn (m.degree g) := by
  rw [sub_eq_add_neg, degree_add_of_ne ?_, degree_neg]
  rw [degree_neg]
  exact h
...
theorem monomialOrderDiv {ι : Type*} (b : ι → MvPolynomial σ R) (hb : ∀ i, IsUnit (m.lCoeff (b i)))
    (f : MvPolynomial σ R) :
  ...
  · obtain ⟨i, hf⟩ := hf
    have deg_reduce : m.degree (m.reduce (hb i) f) ≺[m] m.degree f := by
      ...
      apply hb' i
      simpa [hf0'] using hf
    obtain ⟨g', r', H'⟩ := monomialOrderDiv b hb (m.reduce (hb i) f)
    use g' +
...
protected def Finsupp.DegLex (r : α → α → Prop) (s : ℕ → ℕ → Prop) :
    (α →₀ ℕ) → (α →₀ ℕ) → Prop :=
  (Prod.Lex s (Finsupp.Lex r s)) on (fun x ↦ (x.degree, x))

theorem Finsupp.degLex_def {r : α → α → Prop} {s : ℕ → ℕ → Prop} {a b : α →₀ ℕ} :
...
theorem DegLex.lt_iff [LT α] {a b : DegLex (α →₀ ℕ)} :
  ...
  simp only [DegLex.lt_def, Prod.Lex.lt_iff]
...
theorem _root_.Finsupp.degree_add {α : Type*} (a b : α →₀ ℕ) :
  ...
  Finsupp.sum_add_index' (h := fun _ ↦ id) (congrFun rfl) fun _ _ ↦ congrFun rfl
...
noncomputable instance : OrderedCancelAddCommMonoid (DegLex (α →₀ ℕ)) where
  ...
  le_of_add_le_add_left a b c h := by
    ...
    simpa only [ofDegLex_add, Finsupp.degree_add, add_lt_add_iff_left, add_right_inj, toLex_add,
      add_le_add_iff_left] using h
  add_le_add_left a b h c := by
    rw [DegLex.le_iff] at h ⊢
    simpa [ofDegLex_add, Finsupp.degree_add] using h
...
instance DegLex.orderBot : OrderBot (DegLex (α →₀ ℕ)) where
  ...
  bot_le x := by
    ...
    · simp only [Finsupp.degree_zero, h, lt_self_iff_false, true_and, false_or]
      exact bot_le
    · simp [h]
...
noncomputable def MonomialOrder.degLex (σ : Type*) [LinearOrder σ] [WellFoundedGT σ] :
  ...
  toSyn_monotone a b h := by
    ...
    · refine Or.inr ⟨le_antisymm ?_ (not_lt.mp ha), toLex_monotone h⟩
      rw [← add_tsub_cancel_of_le h, Finsupp.degree_add]
      exact Nat.le_add_right a.degree (b - a).degree

theorem degLex_lt_iff {σ : Type*} [LinearOrder σ] [WellFoundedGT σ] {a b : σ →₀ ℕ} :
