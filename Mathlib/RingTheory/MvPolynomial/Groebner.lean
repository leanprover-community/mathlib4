...
theorem degree_reduce_lt {f b : MvPolynomial σ R} (hb : IsUnit (m.lCoeff b))
  ...
  have H : m.degree f =
      m.degree ((monomial (m.degree f - m.degree b)) (hb.unit⁻¹ * m.lCoeff f)) + m.degree b := by
    classical
  ...
  have H' : coeff (m.degree f) (m.reduce hb f) = 0 := by
    ...
    nth_rewrite 2 [H]
    rw [coeff_mul_of_degree_add (m := m), lCoeff_monomial, mul_comm, ← mul_assoc,
      IsUnit.mul_val_inv, one_mul, ← lCoeff]
  rw [lt_iff_le_and_ne]
  constructor
  · classical
    apply le_trans degree_sub_le
  ...
  · intro K
    ...
    rw [← lCoeff, lCoeff_eq_zero_iff] at H'
