...
noncomputable def determinantalMinorFinset (m n t : ℕ) :
...
theorem mem_determinantalMinorFinset {m n t : ℕ}
...
noncomputable def determinantalIdeal (m n t : ℕ) :
...
theorem mvPolynomialMinor_mem_determinantalIdeal {m n t : ℕ}
    (row : Fin t ↪o Fin m) (col : Fin t ↪o Fin n) :
...
theorem determinantalIdeal_le_iff {m n t : ℕ}
