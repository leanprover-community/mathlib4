...
theorem aeval_map_algebraMap (x : σ → B) (p : MvPolynomial σ R) :
    aeval (R := A) x (map (algebraMap R A) p) = aeval (R := R) x p := by
...
variable [CommSemiring R] [CommSemiring A] [CommSemiring B]
variable [SMul R A] [Algebra R A] [SMul A B] [Algebra A B] [SMul R B] [Algebra R B] [IsScalarTower R A B]
variable {R A}

theorem aeval_algebraMap_apply (x : σ → A) (p : MvPolynomial σ R) :
    aeval (R := R) (algebraMap A B ∘ x) p = algebraMap A B (MvPolynomial.aeval (R := R) x p) := by
...
theorem aeval_algebraMap_eq_zero_iff [NoZeroSMulDivisors A B] [Nontrivial B] (x : σ → A)
    (p : MvPolynomial σ R) : aeval (R := R) (algebraMap A B ∘ x) p = 0 ↔ aeval (R := R) x p = 0 := by
...
theorem aeval_algebraMap_eq_zero_iff_of_injective {x : σ → A} {p : MvPolynomial σ R}
    ...
    aeval (R := R) (algebraMap A B ∘ x) p = 0 ↔ aeval (R := R) x p = 0 :=
...
theorem mvPolynomial_aeval_coe (S : Subalgebra R A) (x : σ → S) (p : MvPolynomial σ R) :
    aeval (R := R) (fun i => (x i : A)) p = aeval (R := R) x p := by convert aeval_algebraMap_apply A x p
