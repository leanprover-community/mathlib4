...
lemma MeasurableSpace.comap_prodMk {α : Type*} (X : α → β) (Y : α → γ) :
    MeasurableSpace.comap (fun ω ↦ (X ω, Y ω)) inferInstance = mβ.comap X ⊔ mγ.comap Y := by
  rw [← generateFrom_prod, comap_generateFrom, comap_eq_generateFrom, comap_eq_generateFrom,
  ...
  · rintro _ ⟨a, ha, b, hb, rfl⟩
    ...
    · exact measurableSet_generateFrom <| .inl ⟨a, ha, rfl⟩
    · exact measurableSet_generateFrom <| .inr ⟨b, hb, rfl⟩
  · refine fun t ht ↦ measurableSet_generateFrom ?_
    cases ht with
    | inl h => obtain ⟨s, hs, rfl⟩ := h; exact ⟨s, hs, .univ, .univ, by simp⟩
    | inr h => obtain ⟨t, ht, rfl⟩ := h; exact ⟨.univ, .univ, t, ht, by simp⟩
