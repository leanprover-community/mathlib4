...
-/
lemma cardinal_generateMeasurableRec_le (s : Set (Set α)) (i : ω₁) :
...
lemma generateMeasurable_eq_rec (s : Set (Set α)) :
...
most `(max #s 2) ^ ℵ₀`. -/
lemma cardinal_generateMeasurable_le (s : Set (Set α)) :
...
lemma cardinalMeasurableSet_le (s : Set (Set α)) :
...
lemma cardinal_generateMeasurable_le_continuum {s : Set (Set α)} (hs : #s ≤ 𝔠) :
...
/-- If a sigma-algebra is generated by a set of sets `s` with cardinality at most the continuum,
then the sigma algebra has the same cardinality bound. -/
lemma cardinal_measurableSet_le_continuum {s : Set (Set α)} :
