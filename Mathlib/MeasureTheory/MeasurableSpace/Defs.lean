...
instance (priority := 10000) [h : MeasurableSpace α] : MeasurableSpace αᵒᵈ := h
...
instance (priority := 10000) : LE (MeasurableSpace α) where le m₁ m₂ := ∀ s, MeasurableSet[m₁] s → MeasurableSet[m₂] s
...
instance (priority := 10000) : PartialOrder (MeasurableSpace α) :=
...
instance (priority := 10000) : CompleteLattice (MeasurableSpace α) :=
  giGenerateFrom.liftCompleteLattice

instance (priority := 10000) : Inhabited (MeasurableSpace α) := ⟨⊤⟩
...
instance (priority := 10000) : @DiscreteMeasurableSpace α ⊤ :=
