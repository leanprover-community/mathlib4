...
theorem measurableEquiv_nat_bool_of_countablyGenerated [MeasurableSpace α]
  ...
  rw [← generateFrom_natGeneratingSequence α] at ‹SeparatesPoints α›
  let f : α → ℕ → Bool := (· ∈  natGeneratingSequence α ·)
  have : Injective f := by
    intro x y hxy
  ...
  use range f, Equiv.ofInjective _ this
  ...
  rw [← generateFrom_natGeneratingSequence α]
