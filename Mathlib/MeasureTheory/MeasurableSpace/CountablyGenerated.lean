...
def generatingSet (α : Type*) [MeasurableSpace α] [h : CountablyGenerated α] : Set (Set α) :=
  insert ∅ h.isCountablyGenerated.choose

lemma countable_generatingSet [MeasurableSpace α] [h : CountablyGenerated α] :
    Countable (generatingSet α) :=
  Countable.insert _ h.isCountablyGenerated.choose_spec.1

lemma generateFrom_generatingSet [m : MeasurableSpace α] [h : CountablyGenerated α] :
    generateFrom (generatingSet α) = m :=
  (generateFrom_insert_empty _).trans <| h.isCountablyGenerated.choose_spec.2.symm

lemma nonempty_generatingSet [MeasurableSpace α] [CountablyGenerated α] :
    Set.Nonempty (generatingSet α) :=
  ⟨∅, mem_insert _ _⟩
...
the union of all those partions is the measurable space `α`. -/
...
lemma finite_countablePartition (α : Type*) [MeasurableSpace α] [CountablyGenerated α] (n : ℕ) :
  ...
  memPartition_finite _ n

instance instFinite_countablePartition (n : ℕ) : Finite (countablePartition α n) :=
  Set.finite_coe_iff.mp (MeasurableSpace.finite_countablePartition _ _)
...
lemma measurableSet_succ_countablePartition (n : ℕ) {s : Set α} (hs : s ∈ countablePartition α n) :
    MeasurableSet[generateFrom (countablePartition α (n + 1))] s := by
  let t : ℕ → Set α := enumerateCountable countable_generatingSet ∅
  rw [← diff_union_inter s (t (n + 1))]
  refine MeasurableSet.union ?_ ?_ <;>
    · refine measurableSet_generateFrom ?_
      rw [countablePartition, memPartition_succ]
      exact ⟨s, hs, by simp⟩
...
lemma generateFrom_iUnion_countablePartition (α : Type*) [m : MeasurableSpace α]
  ...
  let t : ℕ → Set α := enumerateCountable countable_generatingSet ∅
  have ht_mem : ∀ n, t n ∈ generatingSet α :=
    enumerateCountable_mem countable_generatingSet (mem_insert _ _)
  conv_rhs => rw [← generateFrom_generatingSet (α := α)]
  ...
  · simp only [mem_iUnion] at hu
    ...
    | succ n ih =>
      simp only [countablePartition, memPartition_succ, mem_setOf_eq] at hun
      obtain ⟨v, hv, huv⟩ := hun
      rcases huv with rfl | rfl
      · exact (ih v hv).inter (measurableSet_generateFrom (ht_mem _))
      · exact (ih v hv).diff (measurableSet_generateFrom (ht_mem _))
  · obtain ⟨n, rfl⟩ : ∃ n, u = t n := by
      have h := subset_range_enumerate countable_generatingSet ∅ hu
      obtain ⟨n, hn⟩ := mem_range.mpr h
      exact ⟨n, hn.symm⟩
    suffices MeasurableSet[generateFrom (memPartition t n)] (t n) by
      suffices h_le : generateFrom (memPartition t n) ≤ generateFrom (⋃ n, memPartition t n) by
        exact h_le _ this
      exact generateFrom_mono (subset_iUnion _ _)
    cases n with
    | zero =>
      simp only [Nat.zero_eq, memPartition_zero]
      refine measurableSet_generateFrom ?_
      simp
    | succ n =>
      have : t (n + 1) = ⋃ u ∈ memPartition t n, u ∩ t (n + 1) := by
        simp_rw [← iUnion_inter, ← sUnion_eq_biUnion, sUnion_memPartition, univ_inter]
      rw [this]
      refine MeasurableSet.biUnion (Finite.countable (memPartition_finite _ _)) (fun v hv ↦ ?_)
      refine measurableSet_generateFrom ?_
...
lemma exists_countablePartition_mem (n : ℕ) (t : α) : ∃ s, s ∈ countablePartition α n ∧ t ∈ s := by
...
def partitionSet (n : ℕ) (t : α) : Set α := (exists_countablePartition_mem n t).choose

lemma partitionSet_mem (n : ℕ) (t : α) : partitionSet n t ∈ countablePartition α n :=
  (exists_countablePartition_mem n t).choose_spec.1

lemma mem_partitionSet (n : ℕ) (t : α) : t ∈ partitionSet n t :=
  (exists_countablePartition_mem n t).choose_spec.2
