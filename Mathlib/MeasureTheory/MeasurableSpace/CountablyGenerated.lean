...
lemma exists_countablePartition :
  ...
  let b' := insert ∅ b
  have hb'_count : Set.Countable b' := Countable.insert _ hb_count
  have hb'_gen : m = generateFrom b' := by rwa [generateFrom_insert_empty]
  let t : ℕ → Set α := enumerateCountable hb'_count ∅
  -- `ht_mem` is the reason we inserted `∅` into `b` to make `b'`
  have ht_mem : ∀ n, t n ∈ b' := enumerateCountable_mem hb'_count (mem_insert _ _)
  refine ⟨memPartition t, memPartition_finite t, disjoint_memPartition t,
    sUnion_memPartition t, ?_, ?_⟩
  · intro n u hun
  ...
  · rw [hb'_gen]
    ...
    · obtain ⟨n, rfl⟩ : ∃ n, u = t n := by
        have h := subset_range_enumerate hb'_count ∅ hu
...
def countablePartition (α : Type*) [MeasurableSpace α] [CountablyGenerated α] : ℕ → Set (Set α) :=
  exists_countablePartition.choose
...
lemma generateFrom_iUnion_countablePartition (α : Type*) [m : MeasurableSpace α]
  ...
  exists_countablePartition.choose_spec.2.2.2.2.symm
