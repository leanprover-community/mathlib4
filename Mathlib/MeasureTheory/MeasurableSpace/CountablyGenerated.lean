...
lemma measurableSet_generateFrom_memPartition (t : ℕ → Set α) (n : ℕ) :
    MeasurableSet[generateFrom (memPartition t n)] (t n) := by
  cases n with
  | zero => exact measurableSet_generateFrom (by simp)
  | succ n =>
...
lemma generateFrom_iUnion_countablePartition (α : Type*) [m : MeasurableSpace α]
  ...
  · obtain ⟨n, hn⟩ := mem_range.mpr (subset_range_enumerate countable_generatingSet ∅ hu)
    rw [← hn]
...
lemma partitionSet_eq_iff {n : ℕ} (t : α) {s : Set α} (hs : s ∈ countablePartition α n) :
    partitionSet n t = s ↔ t ∈ s := by
  refine ⟨fun h ↦ h ▸ mem_partitionSet n t, fun h ↦ ?_⟩
  by_contra h_ne
  have h_disj : Disjoint s (partitionSet n t) :=
    disjoint_countablePartition hs (partitionSet_mem n t) (Ne.symm h_ne)
...
lemma partitionSet_of_mem {n : ℕ} {t : α} {s : Set α} (hs : s ∈ countablePartition α n)
    (ht : t ∈ s) :
    partitionSet n t = s :=
  (partitionSet_eq_iff t hs).mpr ht
