...
instance (priority := 100) instCountablyGeneratedOfFinite [MeasurableSpace α] [Finite α] :
    CountablyGenerated α where
  isCountablyGenerated :=
    ⟨{s | MeasurableSet s}, Set.to_countable _, generateFrom_measurableSet.symm⟩

instance instCountablyGeneratedSubtype [MeasurableSpace α] [CountablyGenerated α] {p : α → Prop} :
    CountablyGenerated { x // p x } := .comap _

instance instCountablyGeneratedProd [MeasurableSpace α] [CountablyGenerated α]
    [MeasurableSpace β] [CountablyGenerated β] :
...
/-! ### Countably generated spaces and separating sets -/
...
This definition gives such a space. -/
...
lemma injective_mapNatBool (m : MeasurableSpace α) [CountablyGenerated α] [SeparatesPoints α] :
    Injective (mapNatBool α m) := by
  intro x y hxy
...
noncomputable
def equivNatBool [MeasurableSpace α] [CountablyGenerated α] [SeparatesPoints α] :
