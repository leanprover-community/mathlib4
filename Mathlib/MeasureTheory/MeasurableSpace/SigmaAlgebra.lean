...
/-- The σ-algebra generated by a family of sets `F`. -/
@[simp]
def generateFrom (F : Set (Set α)) : Set (Set α) :=
  (MeasurableSpace.generateFrom F).SigmaAlgebra

/-- `IsSigmaAlgebra F` holds iff F equals the σ-algebra it generates. -/
def IsSigmaAlgebra (F : Set (Set α)) : Prop := (MeasurableSpace.generateFrom F).SigmaAlgebra = F
...
lemma IsSigmaAlgebra_of_measurableSpace :
    SigmaAlgebra.IsSigmaAlgebra m.SigmaAlgebra :=
