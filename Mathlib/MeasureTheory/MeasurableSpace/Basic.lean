...
class SeparatesPoints [m : MeasurableSpace α] : Prop where
...
theorem exists_countablyGenerated_le_of_hasCountableSeparatingOn [m : MeasurableSpace α]
  ...
  · use b
  exact @SeparatesPoints.mk _ (generateFrom b) fun x y hxy ↦ hb _ trivial _ trivial
    fun s hs ↦ hxy _ $ measurableSet_generateFrom hs

open Classical

/-- If a measurable space is countably generated and separates points, it is measure equivalent
to some some subset of the Cantor spacd `ℕ → Bool` (equipped with the product sigma algebra). -/
...
theorem measurable_injection_nat_bool_of_countablyGenerated [MeasurableSpace α]
  ...
  rcases @measurableEquiv_nat_bool_of_countablyGenerated _ m' _ _ with ⟨s, ⟨f⟩⟩
  refine' ⟨Subtype.val ∘ f, _, Subtype.coe_injective.comp (MeasurableEquiv.injective _)⟩
  apply measurable_subtype_coe.comp
  exact f.measurable_toFun.mono m'le (by rfl)
