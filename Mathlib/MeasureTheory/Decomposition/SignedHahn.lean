...
private theorem some_exists_one_div_lt_spec (hi : ¬s ≤[i] 0) :
    some_exists_one_div_lt s i ⊆ i ∧
      MeasurableSet (some_exists_one_div_lt s i) ∧
        (1 / (find_exists_one_div_lt s i + 1) : ℝ) < s (some_exists_one_div_lt s i) := by
    ...
    some_exists_one_div_lt s
      (i \
        ⋃ k ≤ n, by
          ...
          exact restrict_nonpos_seq s i k)
...
private theorem restrict_nonpos_seq_lt (n : ℕ) (hn : ¬s ≤[i \ ⋃ k ≤ n, restrict_nonpos_seq s i k] 0) :
    (1 / (find_exists_one_div_lt s (i \ ⋃ k ≤ n, restrict_nonpos_seq s i k) + 1) : ℝ) <
      s (restrict_nonpos_seq s i n.succ) := by
...
private theorem measure_of_restrict_nonpos_seq (hi₂ : ¬s ≤[i] 0) (n : ℕ)
  ...
  · rw [restrict_nonpos_seq_succ]
    have h₁ : ¬s ≤[i \ ⋃ (k : ℕ) (_H : k ≤ n), restrict_nonpos_seq s i k] 0 := by
      refine' mt (restrict_le_zero_subset _ _ (by simp [Nat.lt_succ_iff, subset_rfl])) hn
...
private theorem restrict_nonpos_seq_measurable_set (n : ℕ) :
    MeasurableSet (restrict_nonpos_seq s i n) := by
  cases n <;>
    · rw [restrict_nonpos_seq]
...
private theorem exists_subset_restrict_nonpos' (hi₁ : MeasurableSet i) (hi₂ : s i < 0)
  ...
  have hmeas : MeasurableSet (⋃ (l : ℕ) (_H : l < k), restrict_nonpos_seq s i l) :=
  ...
  · have h₁ : ∀ l < k, 0 ≤ s (restrict_nonpos_seq s i l) := by
    ...
    suffices 0 ≤ ∑' l : ℕ, s (⋃ _H : l < k, restrict_nonpos_seq s i l) by
      rw [sub_neg]
...
theorem exists_subset_restrict_nonpos (hi : s i < 0) :
  ...
  have h₃' : Summable fun n => (1 / (bdd n + 1) : ℝ) := by
    have : Summable fun l => s (restrict_nonpos_seq s i l) :=
      HasSum.summable
        (s.m_iUnion (fun _ => restrict_nonpos_seq_measurable_set _) restrict_nonpos_seq_disjoint)
    refine'
  ...
  have : ∃ k, 1 ≤ bdd k ∧ 1 / (bdd k : ℝ) < s E := by
    ...
    refine' ⟨k, _, _⟩
    · exact_mod_cast le_of_max_le_right (hk k le_rfl)
    · have : 1 / s E < bdd k := by
  ...
  have hA' : A ⊆ i \ ⋃ l ≤ k, restrict_nonpos_seq s i l := by
    ...
    exact ⟨n, hn₂⟩
  refine'
    find_exists_one_div_lt_min (hn' k) (Nat.sub_lt hk₁ (Nat.succ_pos 0))
