...
theorem eq_withDensity_rnDeriv {s : Measure α} {f : α → ℝ≥0∞} (hf : Measurable f) (hs : s ⟂ₘ ν)
  ...
  have hνinter : ν ((S ∩ T)ᶜ) = 0 := by
    ...
    exact le_rfl
  have heq :
    (ν.with_density f).restrict (S ∩ T) = (ν.with_density (μ.rn_deriv ν)).restrict (S ∩ T) :=
    by
    ext1 A hA
  ...
  have heq' :
    ∀ A : Set α, MeasurableSet A → ν.with_density f A = (ν.with_density f).restrict (S ∩ T) A :=
    by
    intro A hA
...
theorem exists_positive_of_not_mutuallySingular (μ ν : Measure α) [IsFiniteMeasure μ]
    ...
    ∃ ε : ℝ≥0,
      0 < ε ∧
        ∃ E : Set α,
          MeasurableSet E ∧ 0 < ν E ∧ 0 ≤[E] μ.toSignedMeasure - (ε • ν).toSignedMeasure :=
  by
