...
inductive MemGeneratePiSystem (S : Set (Set α)) : Set α → Prop
  | base {s : Set α} (h_s : s ∈ S) : MemGeneratePiSystem S s
  | inter {s t : Set α} (h_s : MemGeneratePiSystem S s) (h_t : MemGeneratePiSystem S t)
    (h_nonempty : (s ∩ t).Nonempty) : MemGeneratePiSystem S (s ∩ t)

def generatePiSystem (S : Set (Set α)) : Set (Set α) :=
  {s | MemGeneratePiSystem S s}

theorem isPiSystem_generatePiSystem (S : Set (Set α)) : IsPiSystem (generatePiSystem S) :=
  fun _ h_s _ h_t h_nonempty => MemGeneratePiSystem.inter h_s h_t h_nonempty

theorem subset_generatePiSystem_self (S : Set (Set α)) : S ⊆ generatePiSystem S := fun _ =>
  MemGeneratePiSystem.base
...
theorem generatePiSystem_mono {S T : Set (Set α)} (hST : S ⊆ T) :
  ...
  induction ht with
  | base h_s => exact MemGeneratePiSystem.base (Set.mem_of_subset_of_mem hST h_s)
...
theorem generateFrom_generatePiSystem_eq {g : Set (Set α)} :
  ...
  · exact fun t h_t => measurableSet_generateFrom (MemGeneratePiSystem.base h_t)
...
def toMeasurableSpace (h_inter : ∀ s₁ s₂, d.Has s₁ → d.Has s₂ → d.Has (s₁ ∩ s₂)) :
  ...
  measurableSet_iUnion f hf := by
    ...
    refine d.has_iUnion (disjoint_disjointed _) fun n => disjointedRec (fun t i h ↦ ?_) (hf n)
    exact h_inter _ _ h <| d.has_compl <| hf i
