...
* `piUnionInter` defines a new π-system from a family of π-systems `π : ι → Set (Set α)` and a
  set of indices `S : Set ι`. `piUnionInter π S` is the set of sets that can be written
...
theorem piUnionInter_singleton_left (s : ι → Set α) (S : Set ι) :
    piUnionInter (fun i => ({s i} : Set (Set α))) S =
      { s' : Set α | ∃ (t : Finset ι)(htS : ↑t ⊆ S), s' = ⋂ i ∈ t, s i } := by
  ext1 s'
...
/-- If `π` is a family of π-systems, then `piUnionInter π S` is a π-system. -/
