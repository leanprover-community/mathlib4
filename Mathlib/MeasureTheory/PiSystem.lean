...
instance (priority := 10000) instLEDynkinSystem : LE (DynkinSystem α) where le m₁ m₂ := m₁.Has ≤ m₂.Has
...
instance (priority := 10000) : PartialOrder (DynkinSystem α) :=
...
instance (priority := 10000) : Inhabited (DynkinSystem α) :=
  ⟨generate univ⟩
