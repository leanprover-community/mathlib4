...
We also define the algebra of sets generated by a family of sets ang give itd basic properties,
...
* `MeasureTheory.mem_generateSetAlgebra_elim`: If a sets `s` belongs to the algebra of sets
  generated by `𝒜`, then it can be written as finite union of finite intersections of sets which
...
theorem isSetAlgebra_generateSetAlgebra :
  ...
  compl_mem := fun _ ↦ generateSetAlgebra.compl
  union_mem := fun _ _ ↦ generateSetAlgebra.union

/-- The algebra of sets generated by `𝒜` contains `𝒜`. -/
theorem self_subset_generateSetAlgebra : 𝒜 ⊆ generateSetAlgebra 𝒜 :=
  fun _ ↦ generateSetAlgebra.base

/-- The measurable space generated by a family of subsets `𝒜` is the same as the one generated
by the algebra of sets generated by `𝒜`. -/
theorem generateFrom_generateSetAlgebra_eq :
    generateFrom 𝒜 = generateFrom (generateSetAlgebra 𝒜) := by
  refine' le_antisymm (generateFrom_mono self_subset_generateSetAlgebra) (fun s ms ↦ _)
  refine' @generateFrom_induction _ _ (generateSetAlgebra 𝒜) (fun t ht ↦ _)
    ...
    s ms
  induction' ht with u u_mem u _ mu u v _ _ mu mv
  · exact measurableSet_generateFrom u_mem
  · exact @MeasurableSet.empty _ (generateFrom 𝒜)
  · exact MeasurableSet.compl mu
  · exact MeasurableSet.union mu mv
...
theorem generateSetAlgebra_mono {ℬ : Set (Set α)} (h : 𝒜 ⊆ ℬ) :
  ...
  intro s hs
  induction' hs with _ t_mem t _ t_mem t u _ _ t_mem u_mem
  · exact self_subset_generateSetAlgebra (h t_mem)
  · exact isSetAlgebra_generateSetAlgebra.empty_mem
  · exact isSetAlgebra_generateSetAlgebra.compl_mem t_mem
  · exact isSetAlgebra_generateSetAlgebra.union_mem t_mem u_mem
...
theorem generateSetAlgebra_subset {ℬ : Set (Set α)} (h : 𝒜 ⊆ ℬ)
  ...
  · exact h t_mem
  · exact hℬ.empty_mem
  · exact hℬ.compl_mem t_mem
  · exact hℬ.union_mem t_mem u_mem
...
/-- If a set is part of the algebra of sets generated by `𝒜` then it can be written as a finite
union of finite intersections of sets in which are in `𝒜` or have their complement in `𝒜`. -/
theorem mem_generateSetAlgebra_elim (s_mem : s ∈ generateSetAlgebra 𝒜) :
  ...
  induction' s_mem with u u_mem u _ u_ind u v _ _ u_ind v_ind
  · refine' ⟨{{u}}, finite_singleton {u}, fun a ha ↦ eq_of_mem_singleton ha ▸ finite_singleton u,
      fun a ha t ht ↦ _, by simp⟩
    rw [eq_of_mem_singleton ha, ha, eq_of_mem_singleton ht, ht] at *
    exact Or.inl u_mem
  · exact ⟨∅, finite_empty, fun _ h ↦ (not_mem_empty _ h).elim,
  ...
  · rcases u_ind with ⟨A, A_fin, mem_A, hA, u_eq⟩
    ...
    · ext x
      simp only [u_eq, compl_iUnion, compl_iInter, mem_iInter, mem_iUnion, mem_compl_iff,
        exists_prop, Subtype.exists, mem_setOf_eq, iUnion_exists, iUnion_iUnion_eq', iInter_exists]
...
theorem countable_generateSetAlgebra (h : 𝒜.Countable) :
  ...
  have count_ℬ : ℬ.Countable := by
    ...
    have : compl '' 𝒜 = {s | sᶜ ∈ 𝒜} := by
      ext s; simp
      refine' ⟨fun ⟨x, x_mem, hx⟩ ↦ by simp [← hx, x_mem], fun hs ↦ ⟨sᶜ, hs, by simp⟩⟩
