...
theorem ae_tendsto_average_norm_sub {f : α → E} (hf : LocallyIntegrable f μ) :
  ...
  have A : IntegrableOn (fun y => (‖f y - f x‖₊ : ℝ)) a μ := by
    simp_rw [coe_nnnorm]
    exact (h''a.sub ((integrableOn_const enorm_ne_top).2 (Or.inr h'a))).norm
  dsimp [enorm]
...
theorem ae_tendsto_average [NormedSpace ℝ E] [CompleteSpace E] {f : α → E}
  ...
  · exact (integrable_inv_smul_measure ha.ne' h'a.ne).2
      ((integrableOn_const enorm_ne_top).2 (Or.inr h'a))
