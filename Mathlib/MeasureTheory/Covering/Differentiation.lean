...
#exit
  ...
  have L0 : Tendsto (fun i ↦ ∫ y, ‖g i y‖ * ‖f y - c‖ ∂μ) l (𝓝 0) := by sorry
  have L1 : Tendsto (fun i ↦ ∫ y, g i y • (f y - c) ∂μ) l (𝓝 0) := by
    refine' squeeze_zero_norm (fun n ↦ _) L0
    apply (norm_integral_le_integral_norm _).trans
    simp only [norm_smul, Real.norm_eq_abs, le_refl]
  have := L1.add (hg.smul_const c)
