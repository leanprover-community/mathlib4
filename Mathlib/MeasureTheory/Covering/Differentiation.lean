...
theorem withDensity_le_mul {s : Set α} (hs : MeasurableSet s) {t : ℝ≥0} (ht : 1 < t) :
  ...
  have A : ν (s ∩ f ⁻¹' {0}) ≤ ((t : ℝ≥0∞) ^ 2 • ρ :) (s ∩ f ⁻¹' {0}) := by
    ...
    simp +zetaDelta only [ν, nonpos_iff_eq_zero, M, withDensity_apply, lintegral_eq_zero_iff f_meas]
  ...
  have C :
    ...
    simp +zetaDelta only [ν, M, withDensity_apply, coe_nnreal_smul_apply]
    calc
...
theorem ae_tendsto_measure_inter_div (s : Set α) :
  ...
  have B : ∀ᵐ x ∂μ.restrict s, t.indicator 1 x = (1 : ℝ≥0∞) := by
    ...
    simp +zetaDelta only [hx, Pi.one_apply, indicator_of_mem]
