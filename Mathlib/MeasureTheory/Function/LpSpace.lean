...
lemma mem_Lp_iff_snorm_lt_top {f : α →ₘ[μ] E} : f ∈ Lp E p μ ↔ snorm f p μ < ∞ := Iff.rfl
...
@[simp, norm_cast]
lemma nnnorm_coe_ennreal (f : Lp E p μ) : (‖f‖₊ : ℝ≥0∞) = snorm f p μ :=
...
lemma dist_eq_norm (f g : Lp E p μ) : dist f g = ‖f - g‖ := rfl
