...
theorem support_eq [MeasurableSpace α] [Zero β] (f : α →ₛ β) :
  ...
  Set.ext fun x => by
    simp only [mem_support, Set.mem_preimage, Finset.mem_filter, mem_range_self, true_and_iff,
      exists_prop, mem_iUnion, Set.mem_range, mem_singleton_iff, exists_eq_right']
