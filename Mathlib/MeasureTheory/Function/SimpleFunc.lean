...
def IndexedPartition.piecewise {ι : Type*} [Finite ι] {s : ι → Set α} (hs : IndexedPartition s)
    (hms : ∀ i, MeasurableSet (s i)) (f : ι → α →ₛ β) : α →ₛ β :=
  ⟨hs.piecewise (fun i => f i), fun _ =>
    letI : MeasurableSpace β := ⊤
    Measurable.IndexedPartition.piecewise hms hs (fun i => (f i).measurable) trivial,
    (Set.finite_iUnion (fun i => (f i).finite_range)).subset (hs.range_subset)⟩
