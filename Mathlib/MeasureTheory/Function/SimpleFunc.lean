...
protected theorem induction' {α γ} [MeasurableSpace α] [Nonempty γ] {P : SimpleFunc α γ → Prop}
    (h_ind : ∀ (c), P (SimpleFunc.const _ c))
    (h_add : ∀ ⦃f g : SimpleFunc α γ⦄ {s} (hs : MeasurableSet s), P f → P g →
  ...
  | @insert x s hxs ih =>
    ...
    · rw [Finset.coe_empty, insert_emptyc_eq] at h
      have : f = SimpleFunc.const _ x := by
        ext y
    ...
    let g := SimpleFunc.piecewise (f ⁻¹' {x}) mx (SimpleFunc.const _ x) F
    have PF : P F := by
      apply ih F
      simp only [coe_piecewise, coe_const, range_piecewise, Function.const_apply, F]
      apply Subset.antisymm (union_subset _ _) _ <;> intro y
      ...
      · simp only [mem_image, mem_compl_iff, mem_preimage, mem_singleton_iff, Finset.mem_coe,
        ...
        · rw [← Finset.mem_coe]
          ...
          · rw [← h, Set.mem_range]
            exact ⟨z, z_y⟩
          · rwa [← z_y]
      · simp only [mem_union, mem_image, mem_preimage, mem_singleton_iff, mem_compl_iff]
    ...
    have f_g : f = g := by
      ...
      · simp only [coe_piecewise, coe_const, y_s, piecewise_eq_of_mem, Function.const_apply, g]
        simp only [mem_preimage, mem_singleton_iff, g] at y_s
        exact y_s
      · simp only [coe_piecewise, coe_const, y_s, not_false_eq_true, piecewise_eq_of_not_mem, g, F]
    rw [f_g]
    exact h_add mx (h_ind x) PF
