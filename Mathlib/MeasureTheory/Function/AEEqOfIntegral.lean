...
-- TODO: put globally and test if anything breaks
attribute [to_dual_do_translate] Real
-- -- TODO put to relevant places
-- attribute [to_dual self (reorder := f g, hf hg)] nullMeasurableSet_le
-- attribute [to_dual self (reorder := f g, hf hg)] setIntegral_mono_ae_restrict
...
-- NB: this is not generated by `to_dual` because it uses `mul_neg_of_neg_of_pos` which is
-- contravariant in two arguments, so cannot be dualized.
@[to_dual existing ae_nonneg_of_forall_setIntegral_nonneg]
theorem ae_nonpos_of_forall_setIntegral_nonpos (hf : Integrable f μ)
    (hf_zero : ∀ s, MeasurableSet s → μ s < ∞ → ∫ x in s, f x ∂μ ≤ 0) : f ≤ᵐ[μ] 0 := by
  filter_upwards [ae_nonneg_of_forall_setIntegral_nonneg hf.neg (by simpa [integral_neg])]
  simp
