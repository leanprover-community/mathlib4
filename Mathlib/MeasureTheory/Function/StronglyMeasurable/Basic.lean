...
@[nontriviality]
lemma StronglyMeasurable.of_finite [Finite α] {_ : MeasurableSpace α}
...
protected lemma pow [Monoid β] [ContinuousMul β] (hf : StronglyMeasurable f) (n : ℕ) :
...
@[to_additive (attr := aesop safe 20 apply (rule_sets := [Measurable])) const_nsmul]
protected lemma pow [Monoid β] [ContinuousMul β] (hf : AEStronglyMeasurable f μ) (n : ℕ) :
...
lemma _root_.aestronglyMeasurable_iff_nullMeasurable_separable [PseudoMetrizableSpace β]
    [MeasurableSpace β] [BorelSpace β] :
    AEStronglyMeasurable f μ ↔
...
lemma piecewise {s : Set α} [DecidablePred (· ∈ s)]
