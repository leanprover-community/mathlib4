...
theorem tendsto_approxBounded_of_norm_le {β} {f : α → β} [AddCommGroup β] [NormedAddCommGroup β]
    [NormedSpace ℝ β] {m : MeasurableSpace α} (hf : StronglyMeasurable[m] f) {c : ℝ} {x : α}
    (hfx : ‖f x‖ ≤ c) :
...
theorem tendsto_approxBounded_ae {β} {f : α → β} [AddCommGroup β] [NormedAddCommGroup β]
    [NormedSpace ℝ β] {m m0 : MeasurableSpace α} {μ : Measure α} (hf : StronglyMeasurable[m] f)
...
theorem norm_approxBounded_le {β} {f : α → β} [AddCommGroup β] [SeminormedAddCommGroup β]
    [NormedSpace ℝ β] {m : MeasurableSpace α} {c : ℝ} (hf : StronglyMeasurable[m] f) (hc : 0 ≤ c)
    (n : ℕ) (x : α) :
...
@[fun_prop]
protected theorem norm {_ : MeasurableSpace α} {β : Type*} [AddCommGroup β]
    [SeminormedAddCommGroup β] {f : α → β} (hf : StronglyMeasurable f) :
    StronglyMeasurable fun x => ‖f x‖ :=
  continuous_norm.comp_stronglyMeasurable hf

@[fun_prop]
protected theorem nnnorm {_ : MeasurableSpace α} {β : Type*} [AddCommGroup β]
    [SeminormedAddCommGroup β] {f : α → β} (hf : StronglyMeasurable f) :
    StronglyMeasurable fun x => ‖f x‖₊ :=
...
theorem exists_spanning_measurableSet_norm_le [AddCommGroup β] [SeminormedAddCommGroup β]
    {m m0 : MeasurableSpace α} (hm : m ≤ m0) (hf : StronglyMeasurable[m] f) (μ : Measure α)
