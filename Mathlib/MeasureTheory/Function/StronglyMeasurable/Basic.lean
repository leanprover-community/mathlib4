...
theorem norm_approxBounded_le {β} {f : α → β} [NormPseudoMetric β] [AddCommGroup β] [IsNormedAddGroup β] [NormedSpace ℝ β]
...
@[fun_prop]
protected theorem norm {_ : MeasurableSpace α} {β : Type*} [NormPseudoMetric β] [AddCommGroup β] [IsNormedAddGroup β] {f : α → β}
    (hf : StronglyMeasurable f) : StronglyMeasurable fun x => ‖f x‖ :=
  continuous_norm.comp_stronglyMeasurable hf

@[fun_prop]
protected theorem nnnorm {_ : MeasurableSpace α} {β : Type*} [NormPseudoMetric β] [AddCommGroup β] [IsNormedAddGroup β] {f : α → β}
    (hf : StronglyMeasurable f) : StronglyMeasurable fun x => ‖f x‖₊ :=
...
theorem exists_spanning_measurableSet_norm_le [NormPseudoMetric β] [AddCommGroup β] [IsNormedAddGroup β] {m m0 : MeasurableSpace α}
