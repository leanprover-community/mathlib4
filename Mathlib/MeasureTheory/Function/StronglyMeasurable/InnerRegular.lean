...
private theorem Measure.exists_minimal_disjoint_null_cover_aux {i : Type u}
  ...
  let M : R → Set i := fun r ↦ B r \ ⋃ q : Iio r, B q.1
  ...
  have hM_union : (⋃ r, M r) = ⋃ r, B r := by
    apply Subset.antisymm
    · exact iUnion_mono fun _ ↦ sdiff_subset
    · intro x hx
  ...
  have hMsub (r : R) : M r ⊆ s := sdiff_subset.trans (hBs r)
  have hMnull (r : R) : nu (M r) = 0 := measure_mono_null sdiff_subset (hBnull r)
...
private theorem Measure.exists_cardinalInterUltrafilter_aux {i R : Type u}
  ...
  have hrho_atom (t : Set R) : rho t = 0 ∨ rho tᶜ = 0 := by
    have ht_sub : (⋃ r ∈ t, M r) ⊆ s := iUnion_subset fun r ↦
  ...
  have hUinter : CardinalInterFilter (U : Filter R) (Cardinal.mk R) := by
    ...
    rw [show (⋃ r ∈ (⋂₀ S)ᶜ, M r) = ⋃ t : S, C t by
      ...
      · intro hx
        obtain ⟨t, hx⟩ := mem_iUnion.1 hx
        obtain ⟨r, hrt, hxr⟩ := mem_iUnion₂.1 hx
        apply mem_iUnion₂.2
        refine ⟨r, ?_, hxr⟩
        change r ∉ ⋂₀ S
        intro hr
        exact hrt (mem_sInter.1 hr t t.2)]
    exact hnull
...
private theorem ultrafilter_exists_cardinal_homogeneous_set {R C : Type u} {κ : Cardinal.{u}}
  ...
  let O := κ.ord.ToType
  have hstage (a : O) (previous : ∀ b, b < a → R) :
    ...
    have hpred : Cardinal.mk (Iio a) < κ := by
      simpa only [O, Cardinal.mk_ord_toType] using Cardinal.mk_Iio_lt a (by simp [O])
    ...
    have hindex : Cardinal.mk (List (Iio a) × ℕ) < κ := by
      ...
      apply Cardinal.mul_lt_of_lt hκ.le
      · simpa using hlist
      · exact hκ
    have hprior : Cardinal.mk prior < κ := Cardinal.mk_range_le.trans_lt hpred
    have hgood (p : List (Iio a) × ℕ) : good p ∈ U := hlarge p.2 _
    have hinter : (⋂ p, good p) ∈ U := (Filter.cardinal_iInter_mem hindex).2 hgood
    have hprior_compl : priorᶜ ∈ U := ultrafilter_compl_mem_of_mk_lt U hfree hprior
    obtain ⟨x, hxprior, hxgood⟩ := U.nonempty_of_mem (inter_mem hprior_compl hinter)
    refine ⟨x, hxprior, fun l n ↦ ?_⟩
    exact mem_iInter.1 hxgood (l, n)
  choose next hnext_prior hnext_good using hstage
  let selected : O → R := wellFounded_lt.fix fun a previous ↦ next a previous
  have selected_eq (a : O) : selected a = next a fun b _ ↦ selected b := by
    exact WellFounded.fix_eq wellFounded_lt (fun a previous ↦ next a previous) a
  ...
  have hinvariant (G : Finset O) :
    ...
    | insert a G hmax ih =>
      ...
      have hl : (l.map fun b ↦ selected b.1).toFinset = G.image selected := by
        ext x
        simp [l]
      have hstep := selected_good a l (m - (insert a G).card)
      rw [hl] at hstep
      have hcard : (insert a G).card = G.card + 1 := by simp [haG]
      have hsub_succ : m - (insert a G).card + 1 = m - G.card := by omega
      calc
  ...
  have hGimage : G.image selected = F := by
    ...
    ext x
    simp
  have h := hinvariant G m (by omega)
...
private theorem Measure.exists_subset_measure_eq_of_splits_aux {i : Type*} [MeasurableSpace i]
  ...
  have hu_mono : Monotone (fun n : ℕ ↦ (u n).1) := by
    apply monotone_nat_of_le_succ
    intro n
    change (u n).1 ⊆ (u (n + 1)).1
    rw [u_succ]
    exact hnext_sub n (u n)
  let U : Set i := ⋃ n, (u n).1
  have hUs : U ⊆ s := iUnion_subset fun n ↦ (u n).2.1
  have hU_le : nu U ≤ a := by
    change nu (⋃ n, (u n).1) ≤ a
    rw [hu_mono.measure_iUnion]
    exact iSup_le fun n ↦ (u n).2.2
  by_cases hUa : nu U = a
  · exact ⟨U, hUs, hUa⟩
  have hU_lt : nu U < a := lt_of_le_of_ne hU_le hUa
  ...
  have huU : (u n).1 ⊆ U := subset_iUnion (fun n ↦ (u n).1) n
  have hUw : U ⊆ w := by
    intro x hx
    exact Or.inl hx
  have happ := hnext_approx n (u n) w (huU.trans hUw) hw_s hw_le
  have hunU : nu.real (u (n + 1)).1 ≤ nu.real U :=
    measureReal_mono (subset_iUnion (fun n ↦ (u n).1) (n + 1))
  rw [hw_real] at happ
  have : nu.real t ≤ 1 / (n + 1) := by
    rw [u_succ] at hunU
    change nu.real U + nu.real t ≤ nu.real (next n (u n)) + 1 / (n + 1) at happ
    linarith
...
private theorem Measure.exists_nullFiber_map_of_splits_aux {i : Type*} [MeasurableSpace i]
  ...
  have hnode_measure (l : List Bool) :
      nu (node l) = nu s * (2⁻¹ : ENNReal) ^ l.length := by
    induction l with
    | nil => simp [node]
    | cons b l ih =>
      cases b
      · change nu (half (node l)) = nu s * (2⁻¹ : ENNReal) ^ (false :: l).length
        rw [hhalf_measure, ih, div_eq_mul_inv]
        simp [pow_succ, mul_assoc]
      · change nu (node l \ half (node l)) = nu s * (2⁻¹ : ENNReal) ^ (true :: l).length
  ...
  have hxnode {x : i} (hxs : x ∈ s) : ∀ n, x ∈ node (path x n) := by
    ...
    | succ n ih =>
      rw [path_succ]
      by_cases hx : x ∈ half (node (path x n))
      · have hc : code x n = false := by simp [code, hx]
        rw [hc]
        change x ∈ half (node (path x n))
        exact hx
      · have hc : code x n = true := by simp [code, hx]
        rw [hc]
        change x ∈ node (path x n) \ half (node (path x n))
        exact ⟨ih, hx⟩
...
private theorem UncountableCompactUnitInterval.mk_carrier
  ...
  apply le_antisymm
  · exact (Cardinal.mk_subtype_le _).trans_eq Cardinal.mk_real
...
private theorem exists_pair_avoiding_prior [LinearOrder UncountableCompactUnitInterval]
  ...
  have hs_nonempty : (K.1 \ bad).Nonempty := by
    by_contra h
    have hsub : K.1 ⊆ bad := fun x hx ↦ by
      by_contra hxbad
      exact h ⟨x, hx, hxbad⟩
    have hle : Cardinal.mk K.1 ≤ Cardinal.mk bad := Cardinal.mk_le_mk_of_subset hsub
    exact (not_le_of_gt hbad) (K.mk_carrier ▸ hle)
  obtain ⟨s, hsK, hsbad⟩ := hs_nonempty
  let bad' := bad ∪ {s}
  have hbad' : Cardinal.mk bad' < 𝔠 := by
    apply (Cardinal.mk_union_le bad {s}).trans_lt
  ...
  have hr_nonempty : (Icc (0 : ℝ) 1 \ bad').Nonempty := by
    by_contra h
    have hsub : Icc (0 : ℝ) 1 ⊆ bad' := fun x hx ↦ by
      by_contra hxbad
      exact h ⟨x, hx, hxbad⟩
    have hle : Cardinal.mk (Icc (0 : ℝ) 1) ≤ Cardinal.mk bad' :=
      Cardinal.mk_le_mk_of_subset hsub
    rw [Cardinal.mk_Icc_real zero_lt_one] at hle
...
private theorem exists_bernstein_unitInterval :
  ...
  have chosen_eq (K : UncountableCompactUnitInterval) :
      chosen K = step K fun J _ ↦ chosen J := by
    exact WellFounded.fix_eq wellFounded_lt step K
  ...
  have hR_inj : Function.Injective R := by
    ...
    · exact (hprior_chosen h).1 hRK.symm
    · exact (hprior_chosen h).1 hRK
  ...
  have hBcard : Cardinal.mk B = 𝔠 := by
    ...
    · exact (Cardinal.mk_subtype_le _).trans_eq Cardinal.mk_real
    rw [← mk_uncountableCompactUnitInterval]
    let R' : UncountableCompactUnitInterval → B := fun K ↦ ⟨R K, mem_range_self K⟩
    exact Cardinal.mk_le_of_injective (f := R') fun _ _ h ↦ hR_inj (congr_arg Subtype.val h)
...
private theorem Measure.measure_iUnion_eq_zero_of_pairwiseDisjoint_of_splits_aux
  ...
  have hlabel_zero (r : ℝ) : nu {j | label j = r} = 0 := by
    by_cases hr : ∃ j, label j = r
    · obtain ⟨j, hj⟩ := hr
      apply measure_mono_null (t := code ⁻¹' {code j}) _ (hcode_zero (code j))
      intro k hk
      have hek : e (code k) = e (code j) := Subtype.ext (hk.trans hj.symm)
      exact e.injective hek
    · rw [show {j | label j = r} = ∅ by ext j; simp only [mem_setOf_eq,
        mem_empty_iff_false, iff_false]; exact fun h ↦ hr ⟨j, h⟩]
      simp
  have hfiber_zero (r : ℝ) : μ (g ⁻¹' {r} ∩ U) = 0 := by
    rw [show g ⁻¹' {r} ∩ U = ⋃ j ∈ {j | label j = r}, A j by
      ext x
      constructor
      · rintro ⟨hgxr, hxU⟩
        obtain ⟨j, hxj⟩ := mem_iUnion.1 hxU
        apply mem_iUnion.2 ⟨j, ?_⟩
        exact mem_iUnion.2 ⟨(hgA j hxj).symm.trans hgxr, hxj⟩
      · intro hx
        obtain ⟨j, hx⟩ := mem_iUnion.1 hx
        obtain ⟨hjr, hxj⟩ := mem_iUnion.1 hx
  ...
  letI : Countable (g '' K) := hKimage_count.to_subtype
  have hKzero : μ K = 0 := by
    ...
    have hsub : K ∩ g ⁻¹' {(r : ℝ)} ⊆ g ⁻¹' {(r : ℝ)} ∩ U := fun _ hx ↦
      ⟨hx.2, hKU hx.1⟩
    exact measure_mono_null hsub (hfiber_zero r)
  exact hKpos.ne' hKzero
...
theorem Measure.exists_finset_iInter_nonempty_of_uncountable [IsFiniteMeasure μ]
  ...
  obtain ⟨n, hn⟩ :
    ...
    by_contra! h
    apply Set.not_countable_univ (α := ι)
    apply (countable_iUnion h).mono
    intro i _
...
private theorem Measure.measure_iUnion_eq_zero_of_pairwiseDisjoint_of_atom_aux
  ...
  have htail_pos (r : R) : 0 < nu (tail r) := by
    let initial : Set i := ⋃ q : Iio r, M q.1
    have hpred : Cardinal.mk (Iio r) < Cardinal.mk R := Cardinal.mk_Iio_lt r hord
    have hinitial : nu initial = 0 := hsmall (fun q : Iio r ↦ M q.1)
      (fun q ↦ hMsub q.1) (fun q ↦ hMnull q.1) hpred
    by_contra htail
    have hcover : (⋃ q, M q) ⊆ initial ∪ tail r := by
      intro x hx
    ...
    exact hMpos.ne' (measure_mono_null hcover <|
      measure_union_null hinitial (nonpos_iff_eq_zero.1 (not_lt.1 htail)))
  ...
  let color : Finset R → ULift.{u} Bool := fun F ↦
    ULift.up (if (⋂ r ∈ F, K r).Nonempty then true else false)
  have hBool : Cardinal.mk (ULift.{u} Bool) < Cardinal.mk R := by
    apply lt_of_lt_of_le _ hR.le
    simp
  obtain ⟨Q, hQcard, hhom⟩ :=
    ultrafilter_exists_cardinal_homogeneous_set U hUfree hR hBool color
  ...
  have hQ_nonempty : Q.Nonempty := by
    ...
    exact ⟨q, q.2⟩
  have hcolor_eq_true {F : Finset R} (hFQ : (F : Set R) ⊆ Q) (hFpos : 0 < F.card) :
      color F = ULift.up true := by
    obtain ⟨c, hc⟩ := hhom F.card
    let sQ : Q → Set X := fun q ↦ K q.1
    obtain ⟨G, hGcard, hGnonempty⟩ :=
      Measure.exists_finset_iInter_nonempty_of_uncountable sQ
        (fun q ↦ hKmeas q.1) (fun q ↦ hKpos q.1) hFpos
    ...
    have hG'Q : (G' : Set R) ⊆ Q := by
      rintro q hq
      obtain ⟨q', -, rfl⟩ := Finset.mem_image.1 hq
      exact q'.2
    ...
    have hG'color : color G' = ULift.up true := by
      simp only [color, if_pos hG'nonempty]
    exact (hc F hFQ rfl).trans ((hc G' hG'Q hG'card).symm.trans hG'color)
  have hfinite_inter (F : Finset Q) : (⋂ q ∈ F, K q.1).Nonempty := by
    by_cases hF : F = ∅
    ...
    · let F' : Finset R := F.image ((↑) : Q → R)
      ...
      have hc := hcolor_eq_true hF'Q hF'pos
      have hF'nonempty : (⋂ r ∈ F', K r).Nonempty := by
        by_contra h
        simp only [color, if_neg h] at hc
        exact Bool.false_ne_true (congrArg ULift.down hc)
      obtain ⟨x, hx⟩ := hF'nonempty
      refine ⟨x, mem_iInter₂.2 fun q hq ↦ ?_⟩
      exact mem_iInter₂.1 hx q.1 (Finset.mem_image.2 ⟨q, hq, rfl⟩)
  obtain ⟨q₀, hq₀⟩ := hQ_nonempty
  have htotal : (K q₀ ∩ ⋂ q : Q, K q.1).Nonempty :=
    (hKcompact q₀).inter_iInter_nonempty (fun q : Q ↦ K q.1)
      (fun q ↦ (hKcompact q.1).isClosed) fun F ↦ by
        obtain ⟨x, hx⟩ := hfinite_inter (insert ⟨q₀, hq₀⟩ F)
  ...
  obtain ⟨x, hxK₀, hxall⟩ := htotal
  have hQ_unbounded (r : R) : ∃ q : Q, r < q.1 := by
    by_contra h
    push Not at h
    have hsub : Q ⊆ Iic r := fun q hq ↦ h ⟨q, hq⟩
    have hle := Cardinal.mk_le_mk_of_subset hsub
    rw [hQcard] at hle
    exact (not_le_of_gt (Cardinal.mk_Iic_lt r hord hR.le)) hle
  have hx₀ : x ∈ sourceTail q₀ := hKsub q₀ hxK₀
  change x ∈ ⋃ j ∈ tail q₀, A j at hx₀
  obtain ⟨i₀, hi₀tail, hxi₀⟩ := mem_iUnion₂.1 hx₀
  change i₀ ∈ ⋃ r ∈ Ici q₀, M r at hi₀tail
  obtain ⟨r, hq₀r, hi₀r⟩ := mem_iUnion₂.1 hi₀tail
  obtain ⟨q, hrq⟩ := hQ_unbounded r
  have hxq : x ∈ sourceTail q.1 := hKsub q.1 (mem_iInter.1 hxall q)
  change x ∈ ⋃ j ∈ tail q.1, A j at hxq
  obtain ⟨j, hjtail, hxj⟩ := mem_iUnion₂.1 hxq
  change j ∈ ⋃ t ∈ Ici q.1, M t at hjtail
  ...
  exact ((not_lt_of_ge (hrt ▸ hqt)) hrq).elim
