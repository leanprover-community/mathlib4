...
@[fun_prop]
protected theorem norm {β : Type*} [NormPseudoMetric β] [AddCommGroup β] [IsNormedAddGroup β] {f : α → β}
    (hf : AEStronglyMeasurable f μ) : AEStronglyMeasurable (fun x => ‖f x‖) μ :=
  continuous_norm.comp_aestronglyMeasurable hf

@[fun_prop]
protected theorem nnnorm {β : Type*} [NormPseudoMetric β] [AddCommGroup β] [IsNormedAddGroup β] {f : α → β}
    (hf : AEStronglyMeasurable f μ) : AEStronglyMeasurable (fun x => ‖f x‖₊) μ :=
...
variable {G : Type*} [NormPseudoMetric G] [AddCommGroup G] [IsNormedAddGroup G] [MeasurableSpace G] [BorelSpace G]
