...
protected theorem inner {𝕜 : Type*} {E : Type*} [ROrCLike 𝕜] [NormedAddCommGroup E]
...
variable {m : MeasurableSpace α} {μ : Measure α} {𝕜 : Type*} {E : Type*} [ROrCLike 𝕜]
  [NormedAddCommGroup E] [InnerProductSpace 𝕜 E]

local notation "⟪" x ", " y "⟫" => @inner 𝕜 _ _ x y

protected theorem re {f : α → 𝕜} (hf : AEStronglyMeasurable f μ) :
    AEStronglyMeasurable (fun x => ROrCLike.re (f x)) μ :=
  ROrCLike.continuous_re.comp_aestronglyMeasurable hf
...
protected theorem im {f : α → 𝕜} (hf : AEStronglyMeasurable f μ) :
    AEStronglyMeasurable (fun x => ROrCLike.im (f x)) μ :=
  ROrCLike.continuous_im.comp_aestronglyMeasurable hf
