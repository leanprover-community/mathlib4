...
theorem _root_.Measurable.dependsOn [MeasurableSpace Z] [MeasurableSingletonClass Z]
    (hf : Measurable[piLE X i] f) : DependsOn f (Iic i) :=
...
first coordinates, then it only depends on those first coordinates. -/
theorem StronglyMeasurable.dependsOn [TopologicalSpace Z] [PseudoMetrizableSpace Z] [T1Space Z]
    (hf : StronglyMeasurable[piLE X i] f) : DependsOn f (Iic i) :=
  dependsOn_iff_factorsThrough.2 hf.factorsThrough
