...
-- XXX: better naming suggestions welcome
...
theorem memℒp_congr_ae [TopologicalSpace ε] {f g : α → ε} (hfg : f =ᵐ[μ] g) : Memℒp f p μ ↔ Memℒp g p μ := by
  simp only [Memℒp, eLpNorm_congr_ae hfg, aestronglyMeasurable_congr hfg]

theorem Memℒp.ae_eq [TopologicalSpace ε] {f g : α → ε} (hfg : f =ᵐ[μ] g) (hf_Lp : Memℒp f p μ) : Memℒp g p μ :=
