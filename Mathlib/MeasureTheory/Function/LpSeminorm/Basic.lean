...
/-- It seems we are re-declaring `E` here. We need to think about this. -/
...
/-- Trivial check to ensure the previous two instances are defeq when μ is finite and p = ∞. -/
example [IsFiniteMeasure μ] : instConstOfIsFiniteMeasure (μ := μ) = instConstTopENNReal (E := E)
  := rfl

theorem memLp_const_enorm' {c : E} (hc : ‖c‖ₑ ≠ ⊤) [MemLp.Const E p μ] :
    MemLp (fun _ : α ↦ c) p μ :=
...
also `Lp.const` and related results in the `LpSpace.Indicator` file. We should change the
`[IsFiniteMeasure μ]` instance at the beginning of the `const` section to a `[MemLp.Const]`
instance.
