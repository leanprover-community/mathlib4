...
theorem eLpNorm_add_le (hf : AEStronglyMeasurable f μ) (hg : AEStronglyMeasurable g μ)
    (hfg : AEStronglyMeasurable (f + g) μ) (hp1 : 1 ≤ p) :
...
theorem eLpNorm_add_le' (hf : AEStronglyMeasurable f μ) (hg : AEStronglyMeasurable g μ)
    (hfg : AEStronglyMeasurable (f + g) μ) (p : ℝ≥0∞) :
  ...
  · simpa [LpAddConst_of_one_le h'p] using eLpNorm_add_le hf hg hfg h'p
...
theorem exists_Lp_half (p : ℝ≥0∞) {δ : ℝ≥0∞} (hδ : δ ≠ 0) :
  ...
  refine ⟨η, ηpos, fun f g hf hg hfg Hf Hg => ?_⟩
  calc
    eLpNorm (f + g) p μ ≤ LpAddConst p * (eLpNorm f p μ + eLpNorm g p μ) :=
      eLpNorm_add_le' hf hg hfg p
...
theorem eLpNorm_sub_le' {f g : α → E}
    ...
    (p : ℝ≥0∞) :
    eLpNorm (f - g) p μ ≤ LpAddConst p * (eLpNorm f p μ + eLpNorm g p μ) := by
  simpa only [sub_eq_add_neg, eLpNorm_neg _ _ _ hg] using
    eLpNorm_add_le' hf hg.neg (by simpa only [sub_eq_add_neg] using hf.sub hg) p
...
theorem eLpNorm_sum_le [ContinuousAdd ε'] {ι} {f : ι → α → ε'} {s : Finset ι}
  ...
  Finset.le_sum_of_subadditive_on_pred (fun f : α → ε' => eLpNorm f p μ)
    ...
    (fun _f _g hf hg => eLpNorm_add_le hf hg (hf.add hg) hp1)
