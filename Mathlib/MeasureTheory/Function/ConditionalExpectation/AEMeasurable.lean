...
theorem Lp.induction_stronglyMeasurable (hm : m ≤ m0) (hp_ne_top : p ≠ ∞) (P : Lp F p μ → Prop)
  ...
  have hs_f_eq : s_f =ᵐˢ[μ] Function.support f := hfm.ae_eq_mk.symm.support
  let s_g : Set α := Function.support (hgm.mk g)
  have hs_g : MeasurableSet[m] s_g := hgm.stronglyMeasurable_mk.measurableSet_support
  have hs_g_eq : s_g =ᵐˢ[μ] Function.support g := hgm.ae_eq_mk.symm.support
  have h_inter_empty : (s_f ∩ s_g : Set α) =ᵐˢ[μ] (∅ : Set α) := by
  ...
  have hff' : f =ᵐ[μ] f' := by
    have : s_f \ s_g =ᵐˢ[μ] s_f := by
      rw [← Set.diff_inter_self_eq_diff, Set.inter_comm]
      grw [h_inter_empty, Set.diff_empty]
