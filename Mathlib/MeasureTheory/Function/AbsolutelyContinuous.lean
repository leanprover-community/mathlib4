...
theorem boundedVariationOn (hf : AbsolutelyContinuousOnInterval f a b) :
  ...
  apply v_each
  · convert h_mono (show 0 ≤ i by cutsat); simp
  · convert h_mono (show i ≤ i + 1 by cutsat); norm_cast
  · rw [add_mul, ← add_assoc]; simpa
  · convert h_mono (show i + 1 ≤ n + 1 by cutsat)
    · norm_cast
    · simp only [Nat.cast_add, Nat.cast_one, δ']; field
