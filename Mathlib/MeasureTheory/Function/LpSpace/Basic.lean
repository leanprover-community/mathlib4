...
theorem eLpNorm_aeeqFun {α E : Type*} [MeasurableSpace α] {μ : Measure α} [AddCommGroup E]
    [NormedAddCommGroup E] {p : ℝ≥0∞} {f : α → E} (hf : AEStronglyMeasurable f μ) :
    eLpNorm (AEEqFun.mk f hf) p μ = eLpNorm f p μ :=
  eLpNorm_congr_ae (AEEqFun.coeFn_mk _ _)

theorem MemLp.eLpNorm_mk_lt_top {α E : Type*} [MeasurableSpace α] {μ : Measure α} [AddCommGroup E]
    [NormedAddCommGroup E] {p : ℝ≥0∞} {f : α → E} (hfp : MemLp f p μ) :
    eLpNorm (AEEqFun.mk f hfp.1) p μ < ∞ := by simp [hfp.2]

/-- Lp space -/
def Lp {α} (E : Type*) {m : MeasurableSpace α} [AddCommGroup E] [NormedAddCommGroup E] (p : ℝ≥0∞)
...
theorem LipschitzWith.comp_memLp {α E F} {K} [MeasurableSpace α] {μ : Measure α} [AddCommGroup E]
    [NormedAddCommGroup E] [AddCommGroup F] [NormedAddCommGroup F] {f : α → E} {g : E → F}
    (hg : LipschitzWith K g) (g0 : g 0 = 0) (hL : MemLp f p μ) : MemLp (g ∘ f) p μ :=
...
theorem MeasureTheory.MemLp.of_comp_antilipschitzWith {α E F} {K'} [MeasurableSpace α]
    ...
    {f : α → E} {g : E → F} (hL : MemLp (g ∘ f) p μ) (hg : UniformContinuous g)
    (hg' : AntilipschitzWith K' g) (g0 : g 0 = 0) : MemLp f p μ := by
...
theorem memLp_comp_iff_of_antilipschitz {α E F} {K K'} [MeasurableSpace α] {μ : Measure α}
    [AddCommGroup E] [NormedAddCommGroup E] [AddCommGroup F] [NormedAddCommGroup F] {f : α → E}
    {g : E → F} (hg : LipschitzWith K g) (hg' : AntilipschitzWith K' g) (g0 : g 0 = 0) :
    MemLp (g ∘ f) p μ ↔ MemLp f p μ :=
