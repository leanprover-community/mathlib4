...
theorem tendsto_approxOn_Lp_snorm [OpensMeasurableSpace E] {f : β → E} (hf : Measurable f)
    ...
    Tendsto (fun n => snorm ((approxOn f hf s y₀ h₀ n : β → E) - f) p μ) atTop (𝓝 0) := by
  ...
  -- bounded, at any given point, by `λ x, ‖f x - y₀‖ ^ p.toReal`
  have h_bound :
    ∀ n,
      (fun x => (‖approxOn f hf s y₀ h₀ n x - f x‖₊ : ℝ≥0∞) ^ p.toReal) ≤ᵐ[μ] fun x =>
        ‖f x - y₀‖₊ ^ p.toReal :=
    fun n =>
    eventually_of_forall fun x => by
      dsimp only
      exact rpow_le_rpow (coe_mono (nnnorm_approxOn_le hf h₀ x n)) toReal_nonneg
  -- (3) The bounding function `λ x, ‖f x - y₀‖ ^ p.toReal` has finite integral
  have h_fin : (∫⁻ a : β, ‖f a - y₀‖₊ ^ p.toReal ∂μ) ≠ ⊤ :=
    (lintegral_rpow_nnnorm_lt_top_of_snorm_lt_top hp_zero hp_ne_top hi).ne
  -- (4) The functions "`p`-th power of distance between `f` and the approximation" tend pointwise
  -- to zero
  have h_lim :
    ∀ᵐ a : β ∂μ,
      Tendsto (fun n => (‖approxOn f hf s y₀ h₀ n a - f a‖₊ : ℝ≥0∞) ^ p.to_real) atTop (𝓝 0) := by
...
theorem tendsto_approxOn_range_Lp_snorm [BorelSpace E] {f : β → E} (hp_ne_top : p ≠ ∞)
    ...
    (hf : snorm f p μ < ∞) :
    Tendsto (fun n => snorm ((approxOn f fmeas (range f ∪ {0}) 0 (by simp) n : β → E) - f) p μ) atTop (𝓝 0) :=
  by
...
theorem MeasureTheory.Memℒp.exists_simpleFunc_snorm_sub_lt {E : Type _} [NormedAddCommGroup E]
    ...
    ∃ g : β →ₛ E, snorm (f - (g : β → E)) p μ < ε ∧ Memℒp g p μ := by
  borelize E
  let f' := hf.1.mk f
  suffices H : ∃ g : β →ₛ E, snorm (f' - (g : β → E)) p μ < ε ∧ Memℒp g p μ
  ...
  exact ⟨_, hn, memℒp_approx_on_range f'meas hf' _⟩
...
def simpleFunc : AddSubgroup (Lp E p μ) where
  ...
  neg_mem' := fun {f} ⟨s, hs⟩ =>
    ⟨-s, by
      simp only [← hs, AEEqFun.neg_mk, SimpleFunc.coe_neg, AEEqFun.mk_eq_mk,
        AddSubgroup.coe_neg]⟩
...
protected def instSMul : SMul 𝕜 (Lp.simpleFunc E p μ) :=
  ⟨fun k f =>
    ⟨k • (f : Lp E p μ), by
      ...
      rw [hs]
      rfl⟩⟩
#align measure_theory.Lp.simple_func.has_smul MeasureTheory.Lp.simpleFunc.instSMul

attribute [local instance] simpleFunc.instSMul
...
theorem toSimpleFunc_eq_to_fun (f : Lp.simpleFunc E p μ) : toSimpleFunc f =ᵐ[μ] f :=
  show ⇑(toSimpleFunc f) =ᵐ[μ] ⇑(f : α →ₘ[μ] E) by
    convert(AEEqFun.coe_fn_mk (toSimpleFunc f)
          (toSimpleFunc f).aestronglyMeasurable).symm using
      2
...
theorem zero_toSimpleFunc : toSimpleFunc (0 : Lp.simpleFunc E p μ) =ᵐ[μ] 0 := by
  filter_upwards [toSimpleFunc_eq_to_fun (0 : Lp.simpleFunc E p μ),
...
theorem add_toSimpleFunc (f g : Lp.simpleFunc E p μ) :
  ...
  filter_upwards [toSimpleFunc_eq_to_fun (f + g), toSimpleFunc_eq_to_fun f,
    toSimpleFunc_eq_to_fun g, Lp.coe_fn_add (f : Lp E p μ) g]with _
...
theorem sub_toSimpleFunc (f g : Lp.simpleFunc E p μ) :
    toSimpleFunc (f - g) =ᵐ[μ] toSimpleFunc f - toSimpleFunc g := by
  filter_upwards [toSimpleFunc_eq_to_fun (f - g), toSimpleFunc_eq_to_fun f,
    toSimpleFunc_eq_to_fun g, Lp.coe_fn_sub (f : Lp E p μ) g]with _
...
theorem norm_toSimpleFunc [Fact (1 ≤ p)] (f : Lp.simpleFunc E p μ) :
  ...
  simpa [toLp_toSimpleFunc] using norm_toLp (toSimpleFunc f) (simpleFunc.mem_ℒp f)
...
protected theorem denseEmbedding (hp_ne_top : p ≠ ∞) :
  ...
  refine'
    ⟨fun n =>
      ↑(toLp
          (simpleFunc.approxOn f (Lp.stronglyMeasurable f).measurable (range f ∪ {0}) 0 (by simp)
            n)
          (simpleFunc.mem_ℒp_approxOn_range (Lp.stronglyMeasurable f).measurable hfi' n)),
      fun n => mem_range_self _, _⟩
  convert simpleFunc.tendsto_approxOn_range_Lp hp_ne_top (Lp.stronglyMeasurable f).measurable
      hfi'
...
theorem coeFn_nonneg (f : Lp.simpleFunc G p μ) : 0 ≤ᵐ[μ] f ↔ 0 ≤ f := by
...
theorem denseRange_coeSimpleFuncNonnegToLpNonneg [hp : Fact (1 ≤ p)] (hp_ne_top : p ≠ ∞) :
  ...
  have hx_nonneg_Lp : ∀ n, 0 ≤ to_Lp (x n) (hx_mem_ℒp n) := by
    ...
    rw [ha0, ha_to_Lp]
    exact hx_nonneg n a
  have hx_tendsto : Tendsto (fun n : ℕ => snorm (x n - g) p μ) atTop (𝓝 0) := by
