...
theorem integrableOn_Ici_iff_integrableAtFilter_atTop {f : ℝ → E} {μ : Measure ℝ} {a : ℝ} :
  ...
  all_goals obtain ⟨a', ha'⟩ := mem_atTop_sets.mp hs_top; replace hs := hs.mono ha' le_rfl
  ...
  · refine ⟨⟨Iio a, Iio_mem_atBot a, ?_⟩, ⟨Ici a', Ici_mem_atTop a', hs.restrict measurableSet_Ici⟩⟩
...
theorem integrableOn_Iic_iff_integrableAtFilter_atBot {f : ℝ → E} {μ : Measure ℝ} {a : ℝ} :
  ...
  all_goals obtain ⟨a', ha'⟩ := mem_atBot_sets.mp hs_bot; replace hs := hs.mono ha' le_rfl
  · rewrite [IntegrableOn, Measure.restrict_restrict, ← IntegrableOn] at hs
    · exact ⟨(Iic a') ∩ (Iic a), inter_mem (Iic_mem_atBot a') (Iic_mem_atBot a), hs⟩
    · exact measurableSet_Iic
  · refine ⟨⟨Iic a', Iic_mem_atBot a', hs.restrict measurableSet_Iic⟩, ⟨Ioi a, Ioi_mem_atTop a, ?_⟩⟩
