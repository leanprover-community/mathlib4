...
lemma HasFiniteIntegral.of_finite [Finite α] [IsFiniteMeasure μ] {f : α → β} :
...
@[simp]
lemma Integrable.of_finite [Finite α] [MeasurableSpace α] [MeasurableSingletonClass α]
...
lemma Integrable.smul_measure_nnreal {f : α → β} (h : Integrable f μ) {c : ℝ≥0} :
    Integrable f (c • μ) := by
...
lemma Integrable.re (hf : Integrable f μ) : Integrable (fun x => RCLike.re (f x)) μ := by
  rw [← memℒp_one_iff_integrable] at hf ⊢
  exact hf.re
#align measure_theory.integrable.re MeasureTheory.Integrable.re

lemma Integrable.im (hf : Integrable f μ) : Integrable (fun x => RCLike.im (f x)) μ := by
  rw [← memℒp_one_iff_integrable] at hf ⊢
  exact hf.im
...
lemma nnnorm_toL1 {f : α → β} (hf : Integrable f μ) :
...
@[simp]
lemma ContinuousLinearEquiv.integrable_comp_iff {φ : α → H} (L : H ≃L[𝕜] E) :
...
@[simp]
lemma LinearIsometryEquiv.integrable_comp_iff {φ : α → H} (L : H ≃ₗᵢ[𝕜] E) :
    Integrable (fun a : α ↦ L (φ a)) μ ↔ Integrable φ μ :=
