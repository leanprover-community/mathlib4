...
/-!
# Action of `DomAct` and `DomAct` on `α →ₘ[μ] β`

If `M` acts on `α` by measure-preserving transformations, then `Mᵈᵃ` acts on `α →ₘ[μ] β` by sending
each function `f` to `f ∘ (DomAct.mk.symm c • ·)`. We define this action and basic instances
about it.
...
@[to_additive]
instance : SMul Mᵈᵃ (α →ₘ[μ] β) where
  smul c f := f.compMeasurePreserving (mk.symm c • ·) (measurePreserving_smul _ _)

@[to_additive]
theorem smul_aeeqFun_aeeq (c : Mᵈᵃ) (f : α →ₘ[μ] β) :
    c • f =ᵐ[μ] (f <| mk.symm c • ·) :=
...
@[to_additive (attr := simp)]
theorem smul_aeeqFun_const (c : Mᵈᵃ) (b : β) :
    c • (AEEqFun.const α b : α →ₘ[μ] β) = AEEqFun.const α b :=
  rfl

instance [SMul N β] [ContinuousConstSMul N β] : SMulCommClass Mᵈᵃ N (α →ₘ[μ] β) where
  smul_comm := by rintro _ _ ⟨_⟩; rfl

instance [SMul N β] [ContinuousConstSMul N β] : SMulCommClass N Mᵈᵃ (α →ₘ[μ] β) :=
  .symm _ _ _

@[to_additive]
instance [SMul N α] [MeasurableSMul N α] [SMulInvariantMeasure N α μ] [SMulCommClass M N α] :
    SMulCommClass Mᵈᵃ Nᵈᵃ (α →ₘ[μ] β) where
...
instance [Zero β] : SMulZeroClass Mᵈᵃ (α →ₘ[μ] β) where
  smul_zero _ := rfl

-- TODO: add `AEEqFun.addZeroClass`
instance [AddMonoid β] [ContinuousAdd β] : DistribSMul Mᵈᵃ (α →ₘ[μ] β) where
...
@[to_additive]
instance : MulAction Mᵈᵃ (α →ₘ[μ] β) where
...
instance [Monoid β] [ContinuousMul β] : MulDistribMulAction Mᵈᵃ (α →ₘ[μ] β) where
  smul_one _ := rfl
  smul_mul := by rintro _ ⟨⟩ ⟨⟩; rfl

instance [AddMonoid β] [ContinuousAdd β] : DistribMulAction Mᵈᵃ (α →ₘ[μ] β) where
  smul_zero := smul_zero
  smul_add := smul_add
