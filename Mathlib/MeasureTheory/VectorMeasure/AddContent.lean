...
theorem exists_extension_of_isSetSemiring_of_le_measure [NormedSpace ℝ E]
  ...
  -- next line is to make sure that the default instance is picked below.
  ...
  · simp only [h'C s hs, ↓reduceIte, m'']
    have : ∫ᵛ (x : α), μ[s.indicator 1 | M] x ∂•m' = ∫ᵛ (x : α), s.indicator 1 x ∂•m' := by
      ...
      · apply (integrable_const 1).indicator (h'C s hs)
  ...
  · by_cases hs : MeasurableSet s; swap
    ...
    apply enorm_integral_le_lintegral_enorm.trans
    apply (lintegral_mono' m'_le le_rfl).trans_eq
    rw [hμ', lintegral_trim _ (by fun_prop), lintegral_enorm_condExp_indicator _ hs]
