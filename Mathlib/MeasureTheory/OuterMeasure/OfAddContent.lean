...
Let `C : Set (Set α)` be a `SetSemiring` and `m : AddContent C`, which is
sigma-finite and sigma-additive. Consider the `inducedOuterMeasure` of `m`
and the sigma-algebra `F` generated from the Carathéodory measurable sets. Then,
all sets in `MeasurableSet[MeasurableSpace.generateFrom C]` are `isCaratheodory`, i.e.\ are
Carathéodory measurable, and the inducedOuterMeasure` coincides with `m` on `C`.
...
* `MeasureTheory.Measure.ofAddContent`: Construct a measure from a sigma-subadditive content
...
A sigma-additive content on a semiring induces an outer measure.
* `MeasureTheory.isCaratheodory_AddContentSemiring`: The Caratheodory measurable sets
  are at least all members of the `SetSemiring`.
* `MeasureTheory.isCaratheodory_inducedOuterMeasure`: The Caratheodory measurable sets
  are at least all members of the measurable structure generated by the `SetSemiring`.
* `MeasureTheory.Measure.ofAddContentCaratheodory_eq_inducedOuterMeasure`: The
  `MeasureTheory.Measure.ofAddContentCaratheodory` generated from an `m : AddContent C`
  on a `IsSetSemiring C` coincides with the `MeasureTheory.inducedOuterMeasure`.
...
/-- For `m : AddContent C` sigma-sub-aditive, finite on `C`, the `inducedOuterMeasure` given by `m`
coincides with `m` on `C`. -/
...
theorem isCaratheodory_AddContentSemiring (hC : IsSetSemiring C) (m : AddContent C)
    {s : Set α} (hs : s ∈ C) :
...
theorem isCaratheodory_inducedOuterMeasure (hC : IsSetSemiring C) (m : AddContent C)
  ...
  induction hs with
  | basic u hu => exact isCaratheodory_AddContentSemiring hC m hu
...
/-- Construct a measure from a sigma-subadditive function on a semiring. This
measure is defined on the associated Carathéodory sigma-algebra. -/
noncomputable def ofAddContentCaratheodory (hC : IsSetSemiring C)
    (m : AddContent C)
    (m_sigma_subadd : ∀ ⦃f : ℕ → Set α⦄ (_hf : ∀ i, f i ∈ C) (_hf_Union : (⋃ i, f i) ∈ C),
      m (⋃ i, f i) ≤ ∑' i, m (f i)) :
  ...
  { inducedOuterMeasure (fun x _ ↦ m x) hC.empty_mem addContent_empty with
    ...
    trim_le := by
      ...
      have hs_meas : MeasurableSet[(inducedOuterMeasure (fun x _ ↦ m x) hC.empty_mem
        ...
        exact isCaratheodory_AddContentSemiring hC m hs
      rw [OuterMeasure.trim_eq _ hs_meas,
        inducedOuterMeasure_addContent_of_subadditive hC m m_sigma_subadd hs] }

/-- The measure `MeasureTheory.Measure.ofAddContentCaratheodory` generated from an
`m : AddContent C` on a `IsSetSemiring C` coincides with the `MeasureTheory.inducedOuterMeasure`. -/
theorem ofAddContentCaratheodory_eq_inducedOuterMeasure (hC : IsSetSemiring C)
    (m : AddContent C)
    (m_sigma_subadd : ∀ ⦃f : ℕ → Set α⦄ (_hf : ∀ i, f i ∈ C) (_hf_Union : (⋃ i, f i) ∈ C),
        m (⋃ i, f i) ≤ ∑' i, m (f i))
    (s : Set α) :
    ofAddContentCaratheodory hC m m_sigma_subadd s
      = inducedOuterMeasure (fun x _ ↦ m x) hC.empty_mem addContent_empty s := rfl

theorem ofAddContentCaratheodory_eq (hC : IsSetSemiring C) (m : AddContent C)
    (m_sigma_subadd : ∀ ⦃f : ℕ → Set α⦄ (_hf : ∀ i, f i ∈ C) (_hf_Union : (⋃ i, f i) ∈ C),
        m (⋃ i, f i)≤ ∑' i, m (f i))
    {s : Set α} (hs : s ∈ C) :
    ofAddContentCaratheodory hC m m_sigma_subadd s = m s :=
  inducedOuterMeasure_addContent_of_subadditive hC m  m_sigma_subadd hs
...
noncomputable def ofAddContent [mα : MeasurableSpace α] (hC : IsSetSemiring C)
    (hC_gen : MeasurableSpace.generateFrom C = mα) (m : AddContent C)
    (m_sigma_subadd : ∀ ⦃f : ℕ → Set α⦄ (_hf : ∀ i, f i ∈ C) (_hf_Union : (⋃ i, f i) ∈ C),
      m (⋃ i, f i) ≤ ∑' i, m (f i)) :
    Measure α :=
  (ofAddContentCaratheodory hC m m_sigma_subadd).trim
    (by rw [← hC_gen]; exact isCaratheodory_inducedOuterMeasure hC m)

theorem ofAddContent_eq [mα : MeasurableSpace α] (hC : IsSetSemiring C)
    (hC_gen : MeasurableSpace.generateFrom C = mα) (m : AddContent C)
    (m_sigma_subadd : ∀ ⦃f : ℕ → Set α⦄ (_hf : ∀ i, f i ∈ C) (_hf_Union : (⋃ i, f i) ∈ C),
        m (⋃ i, f i) ≤ ∑' i, m (f i))
    {s : Set α} (hs : s ∈ C) :
    ofAddContent hC hC_gen m m_sigma_subadd s = m s := by
  rw [ofAddContent, trim_measurableSet_eq]
  · exact ofAddContentCaratheodory_eq hC m m_sigma_subadd hs
