...
## Main results
* `MeasureTheory.inducedOuterMeasure_addContent_eq`: The outer measure induced by
  a sigma-subadditive content on a semiring is equal to the content on the sets of the semiring.
* `MeasureTheory.isCaratheodory_inducedOuterMeasure_of_mem`: all sets of the semiring are
  Carathéodory measurable with respect to the outer measure induced by a sigma-subadditive content.
* `MeasureTheory.isCaratheodory_inducedOuterMeasure`: all sets of the sigma-algebra generated by
  the semiring are Carathéodory measurable with respect to the outer measure induced by a
  sigma-subadditive content.
...
coincides with `m` on `C`. -/
theorem inducedOuterMeasure_addContent_eq (hC : IsSetSemiring C) (m : AddContent C)
    (m_sigma_subadd : ∀ ⦃f : ℕ → Set α⦄ (_hf : ∀ i, f i ∈ C) (_hf_Union : (⋃ i, f i) ∈ C),
      m (⋃ i, f i) ≤ ∑' i, m (f i))
    (hs : s ∈ C) :
  ...
  { inducedOuterMeasure (fun x _ ↦ m x) hC.empty_mem addContent_empty with
    ...
    trim_le := by
      ...
      rw [OuterMeasure.trim_eq _ hs_meas,
        inducedOuterMeasure_addContent_eq hC m m_sigma_subadd hs] }
