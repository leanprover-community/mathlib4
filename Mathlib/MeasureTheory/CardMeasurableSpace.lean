...
-- porting note: fails with quotPrecheck enabled.
set_option quotPrecheck false in
-- mathport name: exprω₁
local notation "ω₁" => ((aleph 1 : Cardinal.{u}).ord.out.α)

-- porting note: TODO: derive this somehow. In mathlib3 it was inferred automatically?!,
-- also `Inhabited` which seems to be needed for `generateMeasurable_eq_rec`
variable [Preorder ω₁] [Inhabited ω₁]
...
def generateMeasurableRec (s : Set (Set α)) : ω₁ → Set (Set α)
  | i =>
    let S := ⋃ j : Iio i, generateMeasurableRec (j.1)
