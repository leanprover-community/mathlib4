...
instance (priority := 10000) : CoeSort MeasCat (Type*) :=
  Bundled.coeSort

instance (priority := 10000) (X : MeasCat) : MeasurableSpace X :=
  X.str
...
instance (priority := 10000) unbundledHom : UnbundledHom @Measurable :=
...
instance (priority := 10000) : ConcreteCategory MeasCat := by
  unfold MeasCat
  infer_instance

instance (priority := 10000) : Inhabited MeasCat :=
  ⟨MeasCat.of Empty⟩
...
instance (priority := 10000) TopCat.hasForgetToMeasCat : HasForget₂ TopCat.{u} MeasCat.{u} :=
