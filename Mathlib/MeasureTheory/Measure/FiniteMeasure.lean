...
theorem tendsto_lintegral_nn_of_le_const (μ : FiniteMeasure Ω) {fs : ℕ → Ω →ᵇ ℝ≥0} {c : ℝ≥0}
  ...
  tendsto_lintegral_nn_filter_of_le_const μ
    (Eventually.of_forall fun n => Eventually.of_forall (fs_le_const n))
    (Eventually.of_forall fs_lim)
...
theorem tendsto_of_forall_integral_tendsto {γ : Type*} {F : Filter γ} {μs : γ → FiniteMeasure Ω}
  ...
  have f₀_ae_nn : 0 ≤ᵐ[(μ : Measure Ω)] ⇑f₀ := Eventually.of_forall f₀_nn
  have f₀_ae_nns : ∀ i, 0 ≤ᵐ[(μs i : Measure Ω)] ⇑f₀ := fun i => Eventually.of_forall f₀_nn
  have aux :=
