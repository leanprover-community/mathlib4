...
lemma prod_apply (s : Set (α × β)) (s_mble : MeasurableSet s) :
  ...
  simp [coeFn_eq_toNNReal_coeFn_toMeasure, Measure.prod_apply s_mble]

lemma prod_apply_symm (s : Set (α × β)) (s_mble : MeasurableSet s) :
    μ.prod ν s = ENNReal.toNNReal (∫⁻ y, μ.toMeasure ((fun x ↦ ⟨x, y⟩) ⁻¹' s) ∂ν) := by
  simp [coeFn_eq_toNNReal_coeFn_toMeasure, Measure.prod_apply_symm s_mble]

lemma prod_prod (s : Set α) (t : Set β) : μ.prod ν (s ×ˢ t) = μ s * ν t := by
  simp [coeFn_eq_toNNReal_coeFn_toMeasure]

@[simp] lemma mass_prod : (μ.prod ν).mass = μ.mass * ν.mass := by
  simp only [mass, univ_prod_univ.symm, coeFn_eq_toNNReal_coeFn_toMeasure, toMeasure_prod]
...
@[simp] lemma map_fst_prod : (μ.prod ν).map Prod.fst = ν univ • μ := by
  ...
  ext s _
  simp only [Measure.smul_apply, smul_eq_mul, coe_smul, Measure.nnreal_smul_coe_apply,
