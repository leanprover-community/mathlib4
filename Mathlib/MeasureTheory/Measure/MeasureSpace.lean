...
theorem measure_eq_measure_of_null_diff {s t : Set α} (hst : s ⊆ t) (h_nulldiff : μ (t \ s) = 0) :
    μ s = μ t := measure_congr <| hst.eventually.antisymm (ae_le_set.mpr h_nulldiff)
...
theorem union_ae_eq_left_iff_ae_subset : (s ∪ t : Set α) =ᵐˢ[μ] s ↔ t ⊆ᵐ[μ] s := by
  ...
  refine
    ⟨fun h => by simpa only [union_diff_left] using (ae_eq_set.mp h).1, fun h =>
      EventuallySubset.antisymm (by rwa [ae_le_set, union_diff_left]) subset_union_left.eventually⟩
...
theorem ae_eq_of_subset_of_measure_ge (h₁ : s ⊆ t) (h₂ : μ t ≤ μ s) (hsm : NullMeasurableSet s μ)
    (ht : μ t ≠ ∞) : s =ᵐˢ[μ] t :=
  ae_eq_of_ae_subset_of_measure_ge h₁.eventually h₂ hsm ht
...
theorem measure_iUnion_congr_of_subset {ι : Sort*} [Countable ι] {s : ι → Set α} {t : ι → Set α}
  ...
  calc
    ...
    _ = μ (⋃ b, M (t b) ∩ M (⋃ b, s b)) := measure_congr (.symm <| .countable_iUnion H)
...
theorem measure_iInter_eq_iInf_measure_iInter_le {α ι : Type*} {_ : MeasurableSpace α}
  ...
  · exact fun i ↦ .iInter fun _ ↦ .iInter fun _ ↦ h _
...
theorem exists_measure_iInter_lt {α ι : Type*} {_ : MeasurableSpace α} {μ : Measure α}
  ...
  let F m := μ (⋂ n ∈ Iic m, f n)
  have hFAnti : Antitone F := fun i j hij =>
    measure_mono (biInter_subset_biInter_left fun k hki => le_trans hki hij)
...
theorem Ioo_ae_eq_Ioc' (hb : μ {b} = 0) : Ioo a b =ᵐˢ[μ] Ioc a b := by
  rw [← Ioi_inter_Iio, ← Ioi_inter_Iic]
  exact .inter .rfl (Iio_ae_eq_Iic' hb)

theorem Ioc_ae_eq_Icc' (ha : μ {a} = 0) : Ioc a b =ᵐˢ[μ] Icc a b := by
  rw [← Ioi_inter_Iic, ← Ici_inter_Iic]
  exact (Ioi_ae_eq_Ici' ha).inter .rfl

theorem Ioo_ae_eq_Ico' (ha : μ {a} = 0) : Ioo a b =ᵐˢ[μ] Ico a b := by
...
theorem Ico_ae_eq_Icc' (hb : μ {b} = 0) : Ico a b =ᵐˢ[μ] Icc a b := by
