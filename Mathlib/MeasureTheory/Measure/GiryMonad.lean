...
lemma measurable_coe {s : Set α} (hs : MeasurableSet s) : Measurable fun μ : Measure α => μ s :=
...
lemma measurable_dirac : Measurable (Measure.dirac : α → Measure α) := by
  refine' measurable_of_measurable_coe _ fun s hs => _
...
@[simp]
lemma join_apply {m : Measure (Measure α)} {s : Set α} (hs : MeasurableSet s) :
    join m s = ∫⁻ μ, μ s ∂m :=
  Measure.ofMeasurable_apply s hs
#align measure_theory.measure.join_apply MeasureTheory.Measure.join_apply

@[simp]
lemma join_zero : (0 : Measure (Measure α)).join = 0 := by
  ext1 s hs
...
@[simp]
lemma bind_apply {m : Measure α} {f : α → Measure β} {s : Set β} (hs : MeasurableSet s)
...
lemma measurable_bind' {g : α → Measure β} (hg : Measurable g) : Measurable fun m => bind m g :=
...
lemma lintegral_bind {m : Measure α} {μ : α → Measure β} {f : β → ℝ≥0∞} (hμ : Measurable μ)
...
lemma bind_bind {γ} [MeasurableSpace γ] {m : Measure α} {f : α → Measure β} {g : β → Measure γ}
...
lemma bind_dirac {f : α → Measure β} (hf : Measurable f) (a : α) : bind (dirac a) f = f a := by
  ext1 s hs
...
lemma dirac_bind {m : Measure α} : bind m dirac = m := by
  ext1 s hs
