...
theorem unionᵢ_null_iff' (m : OuterMeasure α) {ι : Prop} {s : ι → Set α} :
    m (⋃ i, s i) = 0 ↔ ∀ i, m (s i) = 0 :=
  ⟨ fun h _ => mono_null m (subset_unionᵢ _ _) h,
    by by_cases i : ι <;> simp [i]; exact fun h => h _ ⟩
#align measure_theory.outer_measure.Union_null_iff' MeasureTheory.OuterMeasure.unionᵢ_null_iff'
...
theorem null_of_locally_null [TopologicalSpace α] [SecondCountableTopology α] (m : OuterMeasure α)
  ...
  obtain ⟨t, ts, t_count, ht⟩ := TopologicalSpace.countable_cover_nhdsWithin hxu
...
theorem unionᵢ_nat_of_monotone_of_tsum_ne_top (m : OuterMeasure α) {s : ℕ → Set α}
  ...
  intro i x hx
  rcases Nat.findX (by exists i : ∃i, x ∈ s i) with ⟨j, hj, hlt⟩
  clear hx i
...
instance OuterMeasure.orderBot : OrderBot (OuterMeasure α) where
      ...
      ENNReal.le_of_forall_pos_le_add <| by
        ...
        exact
          unionᵢ_subset fun i =>
            Subset.trans (hf i).1 <|
              unionᵢ_subset fun j =>
                Nat.pairEquiv_symm_apply ▸ unionᵢ_unpair f ▸ subset_unionᵢ₂ _ _ (s := f) }
...
theorem comap_ofFunction {β} (f : β → α) (h : Monotone m ∨ Surjective f) :
    comap f (OuterMeasure.ofFunction m m_empty) =
      OuterMeasure.ofFunction (fun s => m (f '' s)) (by simp only [image_empty, m_empty]) := by
  ...
  · rw [comap_apply, ofFunction_apply, ofFunction_apply]
    ...
    exact h.elim
      ...
      fun h => (congr_arg m (h.image_preimage (t n))).le
...
theorem restrict_ofFunction (s : Set α) (hm : Monotone m) :
    restrict s (OuterMeasure.ofFunction m m_empty) =
      OuterMeasure.ofFunction (fun t => m (t ∩ s)) (by simp only [empty_inter, m_empty]) := by
      rw [restrict]
...
theorem isCaratheodory_unionᵢ_nat {s : ℕ → Set α} (h : ∀ i, IsCaratheodory m (s i))
  ...
  (isCaratheodory_iff_le' m).2 fun t => by
...
protected def caratheodory : MeasurableSpace α :=
  (caratheodoryDynkin m).toMeasurableSpace fun _s₁ _s₂ => isCaratheodory_inter m
...
theorem le_smul_caratheodory (a : ℝ≥0∞) (m : OuterMeasure α) :
  ...
  simp [smul_apply]; rw [(isCaratheodory_iff m).mp h t]; simp [mul_add]
...
theorem infₛ_eq_boundedBy_infₛGen (m : Set (OuterMeasure α)) :
  ...
  · refine' le_boundedBy.2 fun s => le_infᵢ₂ fun μ hμ => _
    exact (show infₛ m ≤ μ from infₛ_le hμ) s
  · refine' le_infₛ _
    intro μ hμ t
...
theorem le_trim : m ≤ m.trim :=
  (le_ofFunction (m_empty := extend_empty MeasurableSet.empty (empty' _))).mpr
    fun _ => le_infᵢ fun _ => le_rfl
...
theorem le_trim_iff {m₁ m₂ : OuterMeasure α} : m₁ ≤ m₂.trim ↔ ∀ s, MeasurableSet s → m₁ s ≤ m₂ s :=
  (le_ofFunction (m_empty := extend_empty MeasurableSet.empty (empty' _))).trans <|
    forall_congr' fun _ => le_infᵢ_iff
...
theorem exists_measurable_superset_eq_trim (m : OuterMeasure α) (s : Set α) :
  ...
  · have : ∀ r > ms, ∃ t, MeasurableSet t ∧ s ⊆ t ∧ m t < r := by
      intro r hs
      simpa [infᵢ_lt_iff] using hs
    have : ∀ n : ℕ, ∃ t, MeasurableSet t ∧ s ⊆ t ∧ m t < ms + (n : ℝ≥0∞)⁻¹ := by
      intro n
