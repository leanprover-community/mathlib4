...
* `MeasureTheory.MeasureDense μ 𝒜`: `𝒜` is a measure-dense family if it only contains measurable
  sets and if the following condition is satisfied: if `s` is measurable with finite measure,
  then for any `ε > 0` then there exists `t ∈ 𝒜` such that `μ (s ∆ t) < ε `.
...
to the usual notion of density in the metric space made by constant indicators of measurable sets
equipped with the `L¹` norm. -/
structure MeasureDense (μ : Measure X) (𝒜 : Set (Set X)) : Prop :=
...
theorem measurable_measureDense : MeasureDense μ {s | MeasurableSet s} where
...
lemma fin_meas_approx_of_measureDense (h𝒜 : MeasureDense μ 𝒜) {s : Set X}
...
with finite measure. -/
theorem fin_meas_measureDense_of_measureDense (h𝒜 : MeasureDense μ 𝒜) :
    MeasureDense μ {s | s ∈ 𝒜 ∧ μ s ≠ ∞} where
...
class SeparableMeasureSpace (μ : Measure X) : Prop :=
  exists_countable_measureDense : ∃ 𝒜, 𝒜.Countable ∧ MeasureDense μ 𝒜
...
theorem exists_countable_measureDense [SeparableMeasureSpace μ] :
    ∃ 𝒜, 𝒜.Countable ∧ MeasureDense μ 𝒜 :=
