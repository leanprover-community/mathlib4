...
lemma rnDeriv_withDensity_left_of_absolutelyContinuous {f : α → ℝ≥0∞}
    {ν : Measure α} [SigmaFinite μ] [SigmaFinite ν] (hμν : μ ≪ ν) (hf : AEMeasurable f ν) :
...
lemma absolutelyContinuous_tilted [IsFiniteMeasure μ] (hf : AEMeasurable f μ) :
  ...
  | inr h0 =>
    have : μ = (μ Set.univ) • (μ.tilted f).tilted (-f) := by
      ...
      · exact measure_ne_top _ _
    nth_rw 1 [this]
    exact (tilted_absolutelyContinuous _ _).smul (μ Set.univ)
...
lemma rnDeriv_tilted_right_of_absolutelyContinuous' {ν : Measure α} [SigmaFinite μ]
    [IsFiniteMeasure ν] (hμν : μ ≪ ν) (h_int : Integrable (fun x ↦ exp (f x)) ν) :
...
lemma rnDeriv_tilted_right_of_absolutelyContinuous (μ ν : Measure α) [SigmaFinite μ]
  ...
  suffices μ.rnDeriv (ν.tilted f)
    ...
    filter_upwards [this] with x hx
    rw [hx]
  exact rnDeriv_tilted_right_of_absolutelyContinuous' hμν h_int

lemma todo (μ ν : Measure α) [IsFiniteMeasure μ] [IsFiniteMeasure ν]
    (h_int : Integrable (fun x ↦ exp (f x)) ν) :
    μ.rnDeriv (ν.tilted f) =ᵐ[ν] (ν.withDensity (μ.rnDeriv ν)).rnDeriv (ν.tilted f) := by
  have hf : AEMeasurable f ν :=
    aemeasurable_of_aemeasurable_exp (AEStronglyMeasurable.aemeasurable h_int.1)
  have : IsFiniteMeasure (ν.tilted f) := isFiniteMeasure_tilted h_int
  ...
  have h_add : (μ.singularPart ν + ν.withDensity (μ.rnDeriv ν)).rnDeriv (ν.tilted f)
    ...
    hν_ac.ae_le (Measure.rnDeriv_add (μ.singularPart ν) (ν.withDensity (μ.rnDeriv ν)) (ν.tilted f))
  ...
  exact Measure.MutuallySingular.mono_ac (μ.mutuallySingular_singularPart ν)
    Measure.AbsolutelyContinuous.rfl (tilted_absolutelyContinuous _ _)
...
lemma rnDeriv_tilted_left_of_absolutelyContinuous'
    {ν : Measure α} [SigmaFinite μ] [SigmaFinite ν] (hμν : μ ≪ ν) (hf : AEMeasurable f ν) :
    (μ.tilted f).rnDeriv ν
      =ᵐ[ν] fun x ↦ ENNReal.ofReal (exp (f x - logIntegralExp μ f)) * μ.rnDeriv ν x :=
  rnDeriv_tilted_left' (hf.mono_ac hμν) hf

lemma rnDeriv_tilted_left_of_absolutelyContinuous
    {ν : Measure α} [SigmaFinite μ] [SigmaFinite ν] (hμν : μ ≪ ν) (hf : AEMeasurable f ν) :
    (fun x ↦ ((μ.tilted f).rnDeriv ν x).toReal)
      =ᵐ[ν] fun x ↦ exp (f x - logIntegralExp μ f) * (μ.rnDeriv ν x).toReal := by
  filter_upwards [rnDeriv_tilted_left_of_absolutelyContinuous' hμν hf] with x hx
  rw [hx]
  simp only [ENNReal.toReal_mul, mul_eq_mul_right_iff, ENNReal.toReal_ofReal_eq_iff]
  exact Or.inl (exp_pos _).le
