...
theorem tendsto_pre (m : Set X → ℝ≥0∞) (s : Set X) :
  ...
  -- ⊢ Tendsto ((fun r => ↑(pre m r) s) ∘ Subtype.val) atBot (𝓝 (⨆ (i : { i // i >  …
...
theorem mkMetric'_isMetric (m : Set X → ℝ≥0∞) : (mkMetric' m).IsMetric := by
  ...
  -- ⊢ (fun r => ↑(mkMetric'.pre m r) (s ∪ t)) =ᶠ[𝓝[Ioi 0] 0] fun x => ↑(mkMetric'. …
  filter_upwards [Ioo_mem_nhdsWithin_Ioi ⟨le_rfl, r0⟩]
  -- ⊢ ∀ (a : ℝ≥0∞), a ∈ Ioo 0 r → ↑(mkMetric'.pre m a) (s ∪ t) = ↑(mkMetric'.pre m …
  rintro ε ⟨_, εr⟩
  -- ⊢ ↑(mkMetric'.pre m ε) (s ∪ t) = ↑(mkMetric'.pre m ε) s + ↑(mkMetric'.pre m ε) t
  refine' boundedBy_union_of_top_of_nonempty_inter _
  -- ⊢ ∀ (u : Set X), Set.Nonempty (s ∩ u) → Set.Nonempty (t ∩ u) → extend (fun s x …
...
theorem isometry_comap_mkMetric (m : ℝ≥0∞ → ℝ≥0∞) {f : X → Y} (hf : Isometry f)
  ...
  -- ⊢ ↑(comap f) (boundedBy (extend fun s x => m (diam s))) = boundedBy (extend fu …
  rw [comap_boundedBy _ (H.imp _ id)]
  -- ⊢ (boundedBy fun s => extend (fun s x => m (diam s)) (f '' s)) = boundedBy (ex …
  · congr with s : 1
  ...
  · intro h_mono s t hst
    -- ⊢ (fun s => extend (fun s x => m (diam s)) ↑s) s ≤ (fun s => extend (fun s x = …
...
theorem mkMetric_smul (m : ℝ≥0∞ → ℝ≥0∞) {c : ℝ≥0∞} (hc : c ≠ ∞) (hc' : c ≠ 0) :
  ...
  -- ⊢ ⨆ (r : ℝ≥0∞) (_ : r > 0), boundedBy (extend fun s x => (c • m) (diam s)) = c …
...
theorem trim_mkMetric [MeasurableSpace X] [BorelSpace X] (m : ℝ≥0∞ → ℝ≥0∞) :
  ...
  -- ⊢ ⨆ (i : ℕ), trim (mkMetric'.pre (fun s => m (diam s)) (↑i)⁻¹) = ⨆ (n : ℕ), mk …
  congr 1 with n : 1
  -- ⊢ trim (mkMetric'.pre (fun s => m (diam s)) (↑n)⁻¹) = mkMetric'.pre (fun s =>  …
...
theorem mkMetric_le_liminf_tsum {β : Type*} {ι : β → Type*} [∀ n, Countable (ι n)] (s : Set X)
  ...
  -- ⊢ ⨆ (r : ℝ≥0∞) (_ : 0 < r), ⨅ (t : ℕ → Set X) (_ : s ⊆ iUnion t) (_ : ∀ (n : ℕ …
  refine' iSup₂_le fun ε hε => _
  -- ⊢ ⨅ (t : ℕ → Set X) (_ : s ⊆ iUnion t) (_ : ∀ (n : ℕ), diam (t n) ≤ ε), ∑' (n  …
  ...
  -- ⊢ ⨅ (_ : ∀ (n : ℕ), diam (u n) ≤ ε), ∑' (n : ℕ), ⨆ (_ : Set.Nonempty (u n)), m …
...
theorem noAtoms_hausdorff {d : ℝ} (hd : 0 < d) : NoAtoms (hausdorffMeasure d : Measure X) := by
  ...
  -- ⊢ ⨆ (r : ℝ≥0∞) (_ : 0 < r), ⨅ (t : ℕ → Set X) (_ : {x} ⊆ ⋃ (n : ℕ), t n) (_ :  …
...
theorem hausdorffMeasure_zero_singleton (x : X) : μH[0] ({x} : Set X) = 1 := by
  ...
  · rw [hausdorffMeasure_apply]
    -- ⊢ 1 ≤ ⨆ (r : ℝ≥0∞) (_ : 0 < r), ⨅ (t : ℕ → Set X) (_ : {x} ⊆ ⋃ (n : ℕ), t n) ( …
    suffices
      (1 : ℝ≥0∞) ≤
    ...
    -- ⊢ ∀ (i : ℕ → Set X), {x} ⊆ ⋃ (n : ℕ), i n → (∀ (n : ℕ), diam (i n) ≤ 1) → 1 ≤  …
    intro t hst _
...
theorem hausdorffMeasure_image_le (h : HolderOnWith C r f s) (hr : 0 < r) {d : ℝ} (hd : 0 ≤ d) :
  ...
  · have hCd0 : (C : ℝ≥0∞) ^ d ≠ 0 := by simp [hC0.ne']
    ...
    -- ⊢ ⨅ (t : ℕ → Set Y) (_ : f '' s ⊆ ⋃ (n : ℕ), t n) (_ : ∀ (n : ℕ), diam (t n) ≤ …
    ...
    · refine' ENNReal.tsum_le_tsum fun n => _
      -- ⊢ ⨆ (_ : Set.Nonempty ((fun n => f '' (t n ∩ s)) n)), diam ((fun n => f '' (t  …
      simp only [iSup_le_iff, nonempty_image_iff]
      -- ⊢ Set.Nonempty (t n ∩ s) → diam (f '' (t n ∩ s)) ^ d ≤ ⨆ (_ : Set.Nonempty (t  …
      intro hft
      -- ⊢ diam (f '' (t n ∩ s)) ^ d ≤ ⨆ (_ : Set.Nonempty (t n)), ↑C ^ d * diam (t n)  …
...
theorem hausdorffMeasure_preimage_le (hf : AntilipschitzWith K f) (hd : 0 ≤ d) (s : Set Y) :
  ...
  -- ⊢ ⨅ (t : ℕ → Set X) (_ : f ⁻¹' s ⊆ ⋃ (n : ℕ), t n) (_ : ∀ (n : ℕ), diam (t n)  …
...
theorem hausdorffMeasure_image (hf : Isometry f) (hd : 0 ≤ d ∨ Surjective f) (s : Set X) :
  ...
  -- ⊢ ↑(↑(OuterMeasure.comap f) ↑(mkMetric fun r => r ^ d)) s = ↑(OuterMeasure.mkM …
  -- porting note: this proof was slightly nicer before the port
  simp only [mkMetric_toOuterMeasure]
  -- ⊢ ↑(↑(OuterMeasure.comap f) (OuterMeasure.mkMetric fun r => r ^ d)) s = ↑(Oute …
...
theorem hausdorffMeasure_homothety_image {d : ℝ} (hd : 0 ≤ d) (x : P) {c : 𝕜} (hc : c ≠ 0)
  ...
  borelize E
  -- ⊢ ↑↑μH[d] (↑(IsometryEquiv.vaddConst x) '' ((fun x x_1 => x • x_1) c '' (↑(Iso …
...
theorem hausdorffMeasure_homothety_preimage {d : ℝ} (hd : 0 ≤ d) (x : P) {c : 𝕜} (hc : c ≠ 0)
  ...
  -- ⊢ ↑↑μH[d] (↑(↑(AffineEquiv.homothetyUnitsMulHom x) (Units.mk0 c hc)) ⁻¹' s) =  …
...
theorem hausdorffMeasure_lineMap_image (x y : P) (s : Set ℝ) :
  ...
  suffices μH[1] (IsometryEquiv.vaddConst x '' ((· • (y -ᵥ x)) '' s)) = nndist x y • μH[1] s by
    simpa only [Set.image_image]
  borelize E
  -- ⊢ ↑↑μH[1] (↑(IsometryEquiv.vaddConst x) '' ((fun x_1 => x_1 • (y -ᵥ x)) '' s)) …
