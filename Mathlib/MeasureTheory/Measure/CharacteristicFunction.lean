...
* `charFunDual μ L`: the characteristic function of a measure `μ` at `L : E →L[ℝ] ℝ` in
...
noncomputable
def probCharDual (L : F →L[ℝ] ℝ) : F →ᵇ ℂ :=
...
lemma probCharDual_apply (L : F →L[ℝ] ℝ) (x : F) : probCharDual L x = exp (L x * I) := rfl

@[simp]
lemma probCharDual_zero : probCharDual (0 : F →L[ℝ] ℝ) = 1 := by simp [probCharDual]
...
noncomputable
def charFunDual (μ : Measure E) (L : E →L[ℝ] ℝ) : ℂ := ∫ v, probCharDual L v ∂μ

lemma charFunDual_apply (L : E →L[ℝ] ℝ) : charFunDual μ L = ∫ v, exp (L v * I) ∂μ := rfl

lemma charFunDual_eq_charFun_map_one [OpensMeasurableSpace E] (L : E →L[ℝ] ℝ) :
...
lemma charFun_map_eq_charFunDual_smul [OpensMeasurableSpace E] (L : E →L[ℝ] ℝ) (u : ℝ) :
...
lemma charFunDual_map [OpensMeasurableSpace E] [BorelSpace F] (L : E →L[ℝ] F) (L' : F →L[ℝ] ℝ) :
...
@[simp]
lemma charFunDual_dirac [OpensMeasurableSpace E] {x : E} (L : E →L[ℝ] ℝ) :
...
lemma charFunDual_prod [SFinite μ] [SFinite ν] (L : E × F →L[ℝ] ℝ) :
