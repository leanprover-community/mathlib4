...
lemma star_mem_range_probCharAlgHom (he : Continuous e) (hL : Continuous fun p : V × W ↦ L p.1 p.2)
...
def probCharPoly (he : Continuous e) (hL : Continuous fun p : V × W ↦ L p.1 p.2) :
  ...
  star_mem' := by
    intro x hx
    exact star_mem_range_probCharAlgHom he hL hx
...
lemma separatesPoints_probCharPoly (he : Continuous e) (he' : e ≠ 1)
...
theorem ext_of_charFun_eq (he : Continuous e) (he' : e ≠ 1)
  ...
  apply ext_of_forall_mem_subalgebra_integral_eq_of_pseudoEMetric_complete_countable
      (separatesPoints_probCharPoly he he' hL hL')
  intro _ hg
  simp only [mem_probCharPoly] at hg
  obtain ⟨w, hw⟩ := hg
  rw [hw]
