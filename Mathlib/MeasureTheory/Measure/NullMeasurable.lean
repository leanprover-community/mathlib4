...
lemma measure_of_measure_compl_eq_zero₀ (hs : NullMeasurableSet s μ) (h : μ sᶜ = 0) :
    μ s = μ univ := by simpa [h] using measure_add_measure_compl₀ hs

lemma measure_of_measure_compl_eq_zero (hs : MeasurableSet s) (h : μ sᶜ = 0) : μ s = μ univ :=
  measure_of_measure_compl_eq_zero₀ hs.nullMeasurableSet h
