...
lemma measurableAtom_eq_of_mem [Countable α] {x y : α} (hx : x ∈ measurableAtom y) :
    measurableAtom x = measurableAtom y := by
  have h_meas (x : α) : MeasurableSet (measurableAtom x) := .measurableAtom_of_countable x
  ext z
  refine ⟨fun hz ↦ mem_of_mem_measurableAtom hz (h_meas _) hx, fun hz ↦ ?_⟩
  by_cases hy : y ∈ measurableAtom x
  · exact mem_of_mem_measurableAtom hz (h_meas _) hy
  let s := measurableAtom y \ measurableAtom x
  refine absurd (?_ : x ∈ s) (by simp [s])
  have hs : MeasurableSet s := (h_meas _).diff (h_meas _)
  exact mem_of_mem_measurableAtom hx hs (by simp [s, hy])

lemma disjoint_measurableAtom_of_notMem [Countable α] {x y : α} (hx : x ∉ measurableAtom y) :
