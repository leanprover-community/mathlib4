...
theorem withDensity_apply_eq_zero' {f : α → ℝ≥0∞} {s : Set α} (hf : AEMeasurable f μ) :
  ...
  · intro hs
    ...
    convert! A using 2
    ext x
    simp only [and_comm, exists_prop, mem_inter_iff, mem_ofPred,
      not_forall]
  · intro hs
    ...
    have A : s ⊆ t ∪ { x | f x = 0 } := by
      ...
      · simp only [fx, mem_union, mem_ofPred, or_true]
      · left
...
theorem ae_withDensity_iff' {p : α → Prop} {f : α → ℝ≥0∞} (hf : AEMeasurable f μ) :
  ...
  congr
  ext x
  simp only [exists_prop, mem_inter_iff, mem_ofPred, not_forall]
