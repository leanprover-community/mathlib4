...
theorem mulVec_stdBasisMatrix [Semiring R] [Fintype n] [DecidableEq n] [Fintype m] [DecidableEq m]
    (i : n) (j : m) (c : R) (x : m → R) :
    mulVec (stdBasisMatrix i j c) x = fun i' ↦ if i' = i then c * x j else 0 := by
  ext i'
...
theorem volume_preserving_transvectionStruct [DecidableEq ι] (t : TransvectionStruct ι ℝ) :
  ...
  refine ⟨sorry /- unused -/, ?_⟩
  refine (pi_eq fun s hs ↦ ?_).symm
  rw [← pi_pi, ← lintegral_indicator_one sorry, ← lintegral_indicator_one sorry]
  rw [lintegral_map sorry sorry, volume_pi]
  apply integral_eq_of_marginal_eq {t.i} sorry sorry
