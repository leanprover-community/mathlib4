...
theorem smul_map_volume_mul_left {a : ℝ} (h : a ≠ 0) :
  ...
  · simp only [Real.volume_Ioo, Measure.smul_apply, ← ENNReal.ofReal_mul (neg_pos.2 h).le,
  ...
  · simp only [Real.volume_Ioo, Measure.smul_apply, ← ENNReal.ofReal_mul h.le,
      Measure.map_apply (measurable_const_mul a) measurableSet_Ioo, preimage_const_mul_Ioo₀ _ _ h,
      abs_of_pos h, mul_sub, mul_div_cancel₀ _ h.ne', smul_eq_mul]
