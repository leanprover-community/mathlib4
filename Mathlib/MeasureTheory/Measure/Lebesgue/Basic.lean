...
theorem smul_map_volume_mul_left {a : ℝ} (h : a ≠ 0) :
  ...
  -- ⊢ ↑↑volume (Ioo ↑p ↑q) = ↑↑(ofReal |a| • Measure.map (fun x => a * x) volume)  …
...
theorem smul_map_diagonal_volume_pi [DecidableEq ι] {D : ι → ℝ} (h : det (diagonal D) ≠ 0) :
  ...
  -- ⊢ ↑↑(ofReal |det (Matrix.diagonal D)| • Measure.map (↑(↑toLin' (Matrix.diagona …
  simp only [det_diagonal, Measure.coe_smul, Algebra.id.smul_eq_mul, Pi.smul_apply]
  -- ⊢ ofReal |∏ i : ι, D i| * ↑↑(Measure.map (↑(↑toLin' (Matrix.diagonal D))) volu …
  rw [Measure.map_apply _ (MeasurableSet.univ_pi hs)]
  -- ⊢ ofReal |∏ i : ι, D i| * ↑↑volume (↑(↑toLin' (Matrix.diagonal D)) ⁻¹' Set.pi  …
...
theorem map_matrix_volume_pi_eq_smul_volume_pi [DecidableEq ι] {M : Matrix ι ι ℝ} (hM : det M ≠ 0) :
  ...
  · intro D hD
    -- ⊢ Measure.map (↑(↑toLin' (Matrix.diagonal D))) volume = ofReal |(det (Matrix.d …
  ...
  · intro t
    -- ⊢ Measure.map (↑(↑toLin' (TransvectionStruct.toMatrix t))) volume = ofReal |(d …
...
theorem measurableSet_regionBetween (hf : Measurable f) (hg : Measurable g) (hs : MeasurableSet s) :
  ...
  -- ⊢ MeasurableSet ({a | a.fst ∈ s} ∩ {a | a.snd ∈ {a_1 | f a.fst < a_1} ∩ {a_1 | …
  refine'
...
theorem volume_regionBetween_eq_lintegral [SigmaFinite μ] (hf : AEMeasurable f (μ.restrict s))
  ...
  convert h₂ using 1
  -- ⊢ ↑↑(Measure.prod μ volume) (regionBetween f g s) = ↑↑(Measure.prod (Measure.r …
  · rw [Measure.restrict_prod_eq_prod_univ]
    -- ⊢ ↑↑(Measure.prod μ volume) (regionBetween f g s) = ↑↑(Measure.restrict (Measu …
    exact (Measure.restrict_eq_self _ (regionBetween_subset f g s)).symm
    -- 🎉 no goals
  · rw [Measure.restrict_prod_eq_prod_univ]
    -- ⊢ ↑↑(Measure.prod μ volume) (regionBetween (AEMeasurable.mk f hf) (AEMeasurabl …
    exact
...
theorem ae_restrict_of_ae_restrict_inter_Ioo {μ : Measure ℝ} [NoAtoms μ] {s : Set ℝ} {p : ℝ → Prop}
  ...
  -- ⊢ ∀ᵐ (x : ℝ) ∂Measure.restrict μ (s \ u ∪ ⋃ (p : ↑s × ↑s) (_ : p ∈ A), s ∩ T p …
  rw [ae_restrict_union_iff, ae_restrict_biUnion_iff _ A_count]
  -- ⊢ (∀ᵐ (x : ℝ) ∂Measure.restrict μ (s \ u), p x) ∧ ∀ (i : ↑s × ↑s), i ∈ A → ∀ᵐ  …
  constructor
  ...
  · have : μ.restrict (s \ u) = 0 := by simp only [restrict_eq_zero, hfinite.measure_zero]
    ...
    -- 🎉 no goals
  · rintro ⟨⟨a, as⟩, ⟨b, bs⟩⟩ -
    -- ⊢ ∀ᵐ (x : ℝ) ∂Measure.restrict μ (s ∩ T ({ val := a, property := as }, { val : …
    dsimp
