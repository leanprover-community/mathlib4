...
theorem measure_le_eq_lt {r : ℝ} (hr : 0 < r) :
    μ {x : E | g x ≤ r} = μ {x : E | g x < r} := by
  obtain hE | hE := subsingleton_or_nontrivial E
  · have : Set.Nonempty {x | g x ≤ r} := ⟨0, by simp [h1, le_of_lt hr]⟩
    rw [this.eq_zero]
    have : Set.Nonempty {x | g x < r} := ⟨0, by simp [h1, hr]⟩
    rw [this.eq_zero]
  · -- We copy `E` to a new type `F` on which we will put the norm defined by `g`
    letI F : Type _ := E
    letI : NormedAddCommGroup F :=
...
theorem volume_sum_rpow_lt {r p : ℝ} (hp : 1 ≤ p) (hr : 0 < r) :
    volume {x : ι → ℝ | (∑ i, |x i| ^ p) ^ (1 / p) < r} =
      .ofReal ((r * 2 * Real.Gamma (1 / p + 1)) ^ card ι / Real.Gamma (card ι / p + 1)) := by
  have h₁ : 0 ≤ r ^ (card ι) := pow_nonneg (le_of_lt hr) _
  have h₂ : ∀ x : ι → ℝ, 0 ≤ ∑ i, |x i| ^ p := by
    refine fun _ => Finset.sum_nonneg' ?_
    exact fun i => (fun _ => rpow_nonneg_of_nonneg (abs_nonneg _) _) _
  rw [mul_assoc, mul_pow, mul_div_assoc, ofReal_mul h₁, ← volume_sum_rpow_lt_one _ hp,
    ← abs_eq_self.mpr h₁, ← finrank_pi ℝ]
...
theorem volume_sum_rpow_le {r p : ℝ} (hp : 1 ≤ p) (hr : 0 < r) :
    volume {x : ι → ℝ | (∑ i, |x i| ^ p) ^ (1 / p) ≤ r} =
      .ofReal ((r * 2 * Real.Gamma (1 / p + 1)) ^ card ι / Real.Gamma (card ι / p + 1)) := by
  have h₁ : 0 < p := by linarith
  ...
  rw [measure_le_eq_lt _ nm_zero (fun x ↦ nm_neg x) (fun x y ↦ nm_add x y) (eq_zero _).mp
    (fun r x => nm_smul r x) hr, volume_sum_rpow_lt _ hp hr]
...
theorem _root_.Complex.volume_sum_rpow_lt {r p : ℝ} (hp : 1 ≤ p) (hr : 0 < r) :
    ...
    .ofReal ((r ^ 2 * π * Real.Gamma (2 / p + 1)) ^ card ι / Real.Gamma (2 * card ι / p + 1)) := by
  have h₁ : 0 ≤ r ^ (2 * card ι) := pow_nonneg (le_of_lt hr) _
  have h₂ : ∀ x : ι → ℂ, 0 ≤ ∑ i, ‖x i‖ ^ p := by
    refine fun _ => Finset.sum_nonneg' ?_
    exact fun i => (fun _ => rpow_nonneg_of_nonneg (norm_nonneg _) _) _
  rw [mul_assoc, mul_pow, mul_div_assoc, ← pow_mul, ofReal_mul h₁, ← Complex.volume_sum_rpow_lt_one
    _ hp, ← abs_eq_self.mpr h₁, ← finrank_pi ℝ]
  convert addHaar_smul volume r  {x : ι → ℂ |  ∑ i, ‖x i‖ ^ p < 1} using 2
...
theorem _root_.Complex.volume_sum_rpow_le {r p : ℝ} (hp : 1 ≤ p) (hr : 0 < r) :
    volume {x : ι → ℂ | (∑ i, ‖x i‖ ^ p) ^ (1 / p) ≤ r} =
    .ofReal ((r ^ 2 * π * Real.Gamma (2 / p + 1)) ^ card ι / Real.Gamma (2 * card ι / p + 1)) := by
  have h₁ : 0 < p := by linarith
  -- We collect facts about `Lp` norms that will be used in `measure_lt_one_eq_integral_div_gamma`
  ...
  simp_rw [eq_norm, norm_eq_abs] at nm_smul
  rw [measure_le_eq_lt _ nm_zero (fun x ↦ nm_neg x) (fun x y ↦ nm_add x y) (eq_zero _).mp
    (fun r x => nm_smul r x) hr, Complex.volume_sum_rpow_lt _ hp hr]
...
variable {E : Type*} [NormedAddCommGroup E] [InnerProductSpace ℝ E] [FiniteDimensional ℝ E]
  [MeasurableSpace E] [BorelSpace E] [Nontrivial E] (x : E) {r : ℝ} (hr : 0 ≤ r)
...
theorem InnerProductSpace.volume_closedBall :
    volume (Metric.closedBall x r) =
      .ofReal ((r * Real.sqrt π ) ^ finrank ℝ E / Real.Gamma (finrank ℝ E / 2 + 1)) := by
  rw [addHaar_closedBall_eq_addHaar_ball, InnerProductSpace.volume_ball _ hr]
...
@[simp]
theorem Euclidean_space.volume_closedBall (x : EuclideanSpace ℝ (Fin 2)) (r : ℝ) :
    volume (Metric.closedBall x r) = NNReal.pi * (.ofReal r) ^ 2 := by
