...
instance [h : SigmaFinite (μ.restrict s)] : SigmaFinite (μ.restrict (s ∩ t)) := by
  convert sigmaFinite_of_le _ (restrict_mono_ae (ae_of_all _ (Set.inter_subset_left _ _)))
  exact h

instance [h : SigmaFinite (μ.restrict t)] : SigmaFinite (μ.restrict (s ∩ t)) := by
  convert sigmaFinite_of_le _ (restrict_mono_ae (ae_of_all _ (Set.inter_subset_right _ _)))
  exact h
