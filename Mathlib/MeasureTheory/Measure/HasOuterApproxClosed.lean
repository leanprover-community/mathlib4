...
lemma omg (h : ∀ (f : X →ᵇ ℝ≥0) (g : Y →ᵇ ℝ≥0),
  ...
  have hπ2 : generateFrom π = (inferInstance : MeasurableSpace (X × Y)) := by
    rw [Prod.instMeasurableSpace, BorelSpace.measurable_eq (α := X),
      BorelSpace.measurable_eq (α := Y)]
    apply le_antisymm
  ...
  · rintro - ⟨s₁, s₂, hs₁, hs₂, rfl⟩
    ...
    · have obs_μ := HasOuterApproxClosed.tendsto_lintegral_apprSeq hs₁ μ
      ...
      have (n : ℕ) (ω : X × Y) : hs₁.apprSeq n ω.1 * hs₂.apprSeq n ω.2 =
          (((hs₁.apprSeq n).compContinuous ⟨Prod.fst (β := Y), continuous_fst⟩) *
          ((hs₂.apprSeq n).compContinuous ⟨Prod.snd (α := X), continuous_snd⟩)) ω := by
        simp
      simp_rw [this]
