...
lemma charFun_map_smul' {X : Type*} {mX : MeasurableSpace X} {μ : Measure X} [IsFiniteMeasure μ]
    [BorelSpace E] {f : X → E} (hf : AEMeasurable f μ) (r : ℝ) (t : E) :
...
lemma charFun_map_mul' {X : Type*} {mX : MeasurableSpace X} {μ : Measure X} [IsFiniteMeasure μ]
    {f : X → ℝ} (hf : AEMeasurable f μ) (r t : ℝ) :
    charFun (μ.map (fun x ↦ r * (f x))) t = charFun (μ.map f) (r * t) :=
  charFun_map_smul' hf r t
