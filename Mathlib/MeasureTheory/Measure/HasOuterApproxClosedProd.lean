...
lemma ext_of_lintegral_prod_mul_prod_boundedContinuousFunction
  ...
  have hπ1 : IsPiSystem π := by
    ...
    · simp only [Set.mem_pi, mem_univ, mem_ofPred, forall_const] at hs₁ hs₂ ⊢
      exact fun i ↦ (hs₁ i).inter (hs₂ i)
    · simp only [Set.mem_pi, mem_univ, mem_ofPred, forall_const] at ht₁ ht₂ ⊢
  ...
  rintro - ⟨-, ⟨s, hs, rfl⟩, -, ⟨t, ht, rfl⟩, rfl⟩
  simp only [Set.mem_pi, mem_univ, mem_ofPred, forall_const] at hs ht
