...
lemma chaar_sup_eq {K₀ : PositiveCompacts G}
...
@[to_additive]
lemma haarContent_outerMeasure_closure_pos (K₀ : PositiveCompacts G) :
...
@[to_additive]
lemma haarMeasure_closure_self {K₀ : PositiveCompacts G} : haarMeasure K₀ (closure K₀) = 1 := by
