...
lemma ae_ae_add_linearMap_mem_iff [LocallyCompactSpace F] {s : Set F} (hs : MeasurableSet s) :
  ...
  -- Note: #8386 had to change `range_eq_top_iff_surjective` into
  -- `range_eq_top_iff_surjective (f := _)`
  have hM : Function.Surjective M := by
    simp [M, ← LinearMap.range_eq_top_iff_surjective (f := _), LinearMap.range_coprod]
