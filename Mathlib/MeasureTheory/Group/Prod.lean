...
lemma measurePreserving_prod_mul [IsMulLeftInvariant ν] :
...
lemma measurePreserving_prod_mul_swap [IsMulLeftInvariant μ] :
...
"The map `(x, y) ↦ (x, - x + y)` is measure-preserving."]
lemma measurePreserving_prod_inv_mul [IsMulLeftInvariant ν] :
    MeasurePreserving (fun z : G × G => (z.1, z.1⁻¹ * z.2)) (μ.prod ν) (μ.prod ν) :=
...
"The map `(x, y) ↦ (y, - y + x)` sends `μ × ν` to `ν × μ`."]
lemma measurePreserving_prod_inv_mul_swap :
    MeasurePreserving (fun z : G × G => (z.2, z.2⁻¹ * z.1)) (μ.prod ν) (ν.prod μ) :=
...
lemma measurePreserving_mul_prod_inv [IsMulLeftInvariant ν] :
...
@[to_additive]
lemma quasiMeasurePreserving_inv : QuasiMeasurePreserving (Inv.inv : G → G) μ μ := by
...
@[to_additive]
lemma measure_inv_null : μ s⁻¹ = 0 ↔ μ s = 0 := by
...
@[to_additive]
lemma inv_absolutelyContinuous : μ.inv ≪ μ :=
...
@[to_additive]
lemma absolutelyContinuous_inv : μ ≪ μ.inv := by
...
@[to_additive]
lemma lintegral_lintegral_mul_inv [IsMulLeftInvariant ν] (f : G → G → ℝ≥0∞)
...
lemma measure_mul_lintegral_eq [IsMulLeftInvariant ν] (sm : MeasurableSet s) (f : G → ℝ≥0∞)
...
@[to_additive
" Any two nonzero left-invariant measures are absolutely continuous w.r.t. each other. "]
lemma absolutelyContinuous_of_isMulLeftInvariant [IsMulLeftInvariant ν] (hν : ν ≠ 0) : μ ≪ ν := by
...
@[to_additive]
lemma ae_measure_preimage_mul_right_lt_top [IsMulLeftInvariant ν] (sm : MeasurableSet s)
...
@[to_additive]
lemma ae_measure_preimage_mul_right_lt_top_of_ne_zero [IsMulLeftInvariant ν]
...
lemma measure_lintegral_div_measure [IsMulLeftInvariant ν] (sm : MeasurableSet s) (h2s : ν s ≠ 0)
...
@[to_additive]
lemma measure_mul_measure_eq [IsMulLeftInvariant ν] {s t : Set G} (hs : MeasurableSet s)
...
lemma measure_eq_div_smul [IsMulLeftInvariant ν] (hs : MeasurableSet s) (h2s : ν s ≠ 0)
...
lemma measurePreserving_prod_mul_right [IsMulRightInvariant ν] :
...
lemma measurePreserving_prod_mul_swap_right [IsMulRightInvariant μ] :
...
lemma measurePreserving_mul_prod [IsMulRightInvariant μ] :
...
lemma measurePreserving_prod_div [IsMulRightInvariant ν] :
    MeasurePreserving (fun z : G × G => (z.1, z.2 / z.1)) (μ.prod ν) (μ.prod ν) :=
...
@[to_additive measurePreserving_prod_sub_swap
      "The map `(x, y) ↦ (y, x - y)` sends `μ × ν` to `ν × μ`."]
lemma measurePreserving_prod_div_swap [IsMulRightInvariant μ] :
    MeasurePreserving (fun z : G × G => (z.2, z.1 / z.2)) (μ.prod ν) (ν.prod μ) :=
...
" The map `(x, y) ↦ (x - y, y)` preserves the measure `μ × ν`. "]
lemma measurePreserving_div_prod [IsMulRightInvariant μ] :
    MeasurePreserving (fun z : G × G => (z.1 / z.2, z.2)) (μ.prod ν) (μ.prod ν) :=
...
"The map `(x, y) ↦ (x + y, - x)` is measure-preserving."]
lemma measurePreserving_mul_prod_inv_right [IsMulRightInvariant μ] [IsMulRightInvariant ν] :
...
@[to_additive]
lemma quasiMeasurePreserving_inv_of_right_invariant [IsMulRightInvariant μ] :
...
@[to_additive]
lemma quasiMeasurePreserving_div_left [IsMulLeftInvariant μ] (g : G) :
...
@[to_additive]
lemma quasiMeasurePreserving_div_left_of_right_invariant [IsMulRightInvariant μ] (g : G) :
    QuasiMeasurePreserving (fun h : G => g / h) μ μ := by
  rw [← μ.inv_inv]
  exact
...
@[to_additive]
lemma quasiMeasurePreserving_div_of_right_invariant [IsMulRightInvariant μ] :
...
@[to_additive]
lemma quasiMeasurePreserving_div [IsMulLeftInvariant μ] :
...
lemma quasiMeasurePreserving_mul_right [IsMulLeftInvariant μ] (g : G) :
...
lemma quasiMeasurePreserving_mul_left [IsMulRightInvariant μ] (g : G) :
    QuasiMeasurePreserving (fun h : G => g * h) μ μ := by
  have :=
