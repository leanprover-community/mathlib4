...
theorem squareCylinders_eq_iUnion_image (C : ∀ i, Set (Set (α i))) :
  ...
  ext1 f
  simp only [squareCylinders, mem_iUnion, mem_image, mem_univ_pi, mem_ofPred,
    eq_comm (a := f)]
...
theorem comap_eval_le_generateFrom_squareCylinders_singleton
  ...
  simp only [mem_ofPred, mem_image, mem_univ_pi, forall_exists_index, and_imp]
  intro t ht h
  classical
...
theorem generateFrom_squareCylinders [∀ i, MeasurableSpace (α i)] :
  ...
  · rw [MeasurableSpace.generateFrom_le_iff]
    rintro S ⟨s, t, h, rfl⟩
    simp only [mem_univ_pi, mem_ofPred] at h
...
theorem generateFrom_measurableCylinders :
  ...
  · refine iSup_le fun i ↦ ?_
    ...
    simp only [singleton_pi, mem_image, mem_pi, mem_univ, mem_ofPred,
    ...
    ext1 x
    simp only [mem_preimage, Function.eval, mem_cylinder, mem_ofPred, Finset.restrict]
