...
theorem Integrable.prod_mul {L : Type*} [ROrCLike L] {f : α → L} {g : β → L} (hf : Integrable f μ)
...
theorem integral_prod_smul {𝕜 : Type*} [ROrCLike 𝕜] [NormedSpace 𝕜 E] (f : α → 𝕜) (g : β → E) :
...
theorem integral_prod_mul {L : Type*} [ROrCLike L] (f : α → L) (g : β → L) :
...
theorem set_integral_prod_mul {L : Type*} [ROrCLike L] (f : α → L) (g : β → L) (s : Set α)
    (t : Set β) :
