...
lemma IsPiSystem.pi {C : ∀ i, Set (Set (α i))} (hC : ∀ i, IsPiSystem (C i)) :
...
lemma isPiSystem_pi [∀ i, MeasurableSpace (α i)] :
...
lemma IsCountablySpanning.pi {C : ∀ i, Set (Set (α i))} (hC : ∀ i, IsCountablySpanning (C i)) :
...
lemma generateFrom_pi_eq {C : ∀ i, Set (Set (α i))} (hC : ∀ i, IsCountablySpanning (C i)) :
...
lemma generateFrom_eq_pi [h : ∀ i, MeasurableSpace (α i)] {C : ∀ i, Set (Set (α i))}
...
/-- The product σ-algebra is generated from boxes, i.e. `s ×ˢ t` for sets `s : set α` and
  `t : set β`. -/
lemma generateFrom_pi [∀ i, MeasurableSpace (α i)] :
...
lemma pi_eq_generateFrom {C : ∀ i, Set (Set (α i))}
...
lemma pi_of_empty {α : Type*} [IsEmpty α] {β : α → Type*} {m : ∀ a, MeasurableSpace (β a)}
...
lemma measurePreserving_piEquivPiSubtypeProd {ι : Type u} {α : ι → Type v} [Fintype ι]
...
lemma volume_preserving_piEquivPiSubtypeProd {ι : Type*} (α : ι → Type*) [Fintype ι]
...
lemma measurePreserving_piFinSuccAboveEquiv {n : ℕ} {α : Fin (n + 1) → Type u}
    {m : ∀ i, MeasurableSpace (α i)} (μ : ∀ i, Measure (α i)) [∀ i, SigmaFinite (μ i)]
    (i : Fin (n + 1)) :
...
lemma volume_preserving_piFinSuccAboveEquiv {n : ℕ} (α : Fin (n + 1) → Type u)
...
lemma measurePreserving_funUnique {β : Type u} {m : MeasurableSpace β} (μ : Measure β)
    (α : Type v) [Unique α] :
...
lemma measurePreserving_pi_empty {ι : Type u} {α : ι → Type v} [IsEmpty ι]
...
lemma volume_preserving_pi_empty {ι : Type u} (α : ι → Type v) [IsEmpty ι]
    [∀ i, MeasureSpace (α i)] :
