...
nonrec theorem IsCountablySpanning.pi {C : ∀ i, Set (Set (α i))}
  ...
  let e : ℕ → ι → ℕ := fun n => (decode (α := ι → ℕ) n).iget
  ...
  · refine' iSup_le _; intro i; rw [comap_generateFrom]
    ...
    have :
      pi univ (update (fun i' : ι => Union (t i')) i (⋃ i' : ℕ, s)) =
        pi univ fun k => ⋃ j : ℕ, @update ι (fun i' => Set (α i')) _ (fun i' => t i' j) i s k := by
...
theorem pi_caratheodory :
  ...
  · rintro j - -; apply mono'; apply image_subset; apply inter_subset_left
  · rintro j - -; apply mono'; apply image_subset; apply diff_subset
...
nonrec def FiniteSpanningSetsIn.pi {C : ∀ i, Set (Set (α i))}
  ...
  refine'
      ⟨fun n => pi univ fun i => (hμ i).set ((decode (α := ι → ℕ) n).iget i), fun n => _, fun n => _,
  ...
  · calc
      Measure.pi μ (pi univ fun i => (hμ i).set (e n i)) ≤
          Measure.pi μ (pi univ fun i => toMeasurable (μ i) ((hμ i).set (e n i))) :=
        measure_mono (pi_mono fun i hi => subset_toMeasurable _ _)
...
@[to_additive isAddLeftInvariant]
...
@[to_additive isAddRightInvariant]
...
@[to_additive
      "We intentionally restrict this only to the nondependent function space, since type-class
      inference cannot find an instance for `ι → ℝ` when this is stated for dependent function
      spaces."]
