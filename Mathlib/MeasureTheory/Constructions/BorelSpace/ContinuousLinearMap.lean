...
@[measurability]
lemma measurable_apply [MeasurableSpace F] [BorelSpace F] (x : E) :
...
@[measurability]
lemma measurable_apply' [MeasurableSpace E] [OpensMeasurableSpace E] [MeasurableSpace F]
    [BorelSpace F] : Measurable fun (x : E) (f : E →L[𝕜] F) => f x :=
  measurable_pi_lambda _ fun f => f.measurable
#align continuous_linear_map.measurable_apply' ContinuousLinearMap.measurable_apply'

@[measurability]
lemma measurable_coe [MeasurableSpace F] [BorelSpace F] :
...
@[measurability]
lemma Measurable.apply_continuousLinearMap {φ : α → F →L[𝕜] E} (hφ : Measurable φ) (v : F) :
...
@[measurability]
lemma AEMeasurable.apply_continuousLinearMap {φ : α → F →L[𝕜] E} {μ : Measure α}
...
lemma aemeasurable_smul_const {f : α → 𝕜} {μ : Measure α} {c : E} (hc : c ≠ 0) :
