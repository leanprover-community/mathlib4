...
theorem setToSimpleFunc_congr (T : Set α → E →L[ℝ] F)
  ...
  simp_rw [Set.mem_inter_iff, Set.mem_ofPred_eq, Set.mem_preimage, Set.mem_singleton]
  intro h
  rwa [h.1, h.2]
...
theorem setToSimpleFunc_indicator (T : Set α → F →L[ℝ] F') (hT_empty : T ∅ = 0)
  ...
  swap; · exact Set.mem_singleton_self x
  rw [← Function.const_zero, ← Function.const_def, preimage_const_of_notMem]
  swap; · rw [Set.mem_singleton]; exact Ne.symm hx0
  simp
...
theorem setToSimpleFunc_const' [Nonempty α] (T : Set α → F →L[ℝ] F') (x : F)
  ...
  simp only [setToSimpleFunc, range_const, Set.mem_singleton_self, preimage_const_of_mem,
