...
lemma IntegrableOn.inter_of_restrict (h : IntegrableOn f s (μ.restrict t)) :
...
lemma _root_.MeasurableEmbedding.integrableOn_iff_comap [MeasurableSpace β] {e : α → β}
...
lemma _root_.MeasurableEmbedding.integrableAtFilter_map_iff [MeasurableSpace β] {e : α → β}
...
lemma _root_.MeasurableEmbedding.integrableAtFilter_iff_comap [MeasurableSpace β] {e : α → β}
...
protected lemma IntegrableAtFilter.norm (hf : IntegrableAtFilter f l μ) :
...
@[simp]
lemma integrableAtFilter_top : IntegrableAtFilter f ⊤ μ ↔ Integrable f μ := by
  refine ⟨fun h ↦ ?_, fun h ↦ h.integrableAtFilter ⊤⟩
  obtain ⟨s, hsf, hs⟩ := h
  exact (integrableOn_iff_integrable_of_support_subset fun _ _ ↦ hsf _).mp hs

lemma IntegrableAtFilter.sup_iff {l l' : Filter α} :
