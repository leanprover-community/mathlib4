...
theorem integral_abs_eq_two_mul_integral_max_sub_integral {f : α → ℝ} (hf : Integrable f μ) :
    ∫ x, |f x| ∂μ = 2 * ∫ x, max (f x) 0 ∂μ - ∫ x, f x ∂μ := by
  have h_eq : ∀ x, |f x| = 2 * max (f x) 0 - f x := by grind
  rw [integral_congr_ae (Eventually.of_forall h_eq), integral_sub (by fun_prop) hf,
    integral_const_mul]

theorem integral_abs_eq_two_mul_integral_max_add_integral {f : α → ℝ} (hf : Integrable f μ) :
    ∫ x, |f x| ∂μ = 2 * ∫ x, max (-f x) 0 ∂μ + ∫ x, f x ∂μ := by
  have h_eq : ∀ x, |f x| = 2 * max (-f x) 0 + f x := by grind
