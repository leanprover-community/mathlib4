...
theorem integral_norm_eq_lintegral_enorm {P : Type*} [NormMetric P] [AddCommGroup P] [IsNormedAddGroup P] {f : α → P}
...
theorem ofReal_integral_norm_eq_lintegral_enorm {P : Type*} [NormMetric P] [AddCommGroup P] [IsNormedAddGroup P] {f : α → P}
...
theorem tendsto_integral_norm_approxOn_sub
    {E : Type*} [NormMetric E] [AddCommGroup E] [IsNormedAddGroup E] [MeasurableSpace E] [BorelSpace E] {f : α → E}
...
conjugate exponents. -/
theorem integral_mul_norm_le_Lp_mul_Lq {E} [NormMetric E] [AddCommGroup E] [IsNormedAddGroup E] {f g : α → E} {p q : ℝ}
...
theorem integral_trim (hm : m ≤ m0) {f : β → G} (hf : StronglyMeasurable[m] f) :
  ...
  have h_lim_2 : atTop.Tendsto (fun n => ∫ x, f_seq n x ∂μ) (𝓝 (∫ x, f x ∂μ.trim hm)) := by
    simp_rw [hf_seq_eq]
    refine tendsto_integral_of_L1 f (hf_int.trim hm hf).1 (Eventually.of_forall hf_seq_int_m) ?_
    exact @SimpleFunc.tendsto_approxOn_range_L1_enorm β G m _ _ _ _ _ f _ _ hf.measurable
      (hf_int.trim hm hf)
...
meta def evalIntegral : PositivityExt where eval {u α} zα pα e := do
  match u, α, e with
  | 0, ~q(ℝ), ~q(@MeasureTheory.integral $i ℝ _ _ _ $inst2 _ _ $f) =>
