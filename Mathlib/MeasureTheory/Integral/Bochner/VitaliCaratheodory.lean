...
open MeasureTheory MeasureTheory.Measure CompleteLinearOrderedAddCommMonoidWithTop
...
theorem exists_le_lowerSemicontinuous_lintegral_ge (f : α → ℝ≥0∞) (hf : Measurable f) {ε : ℝ≥0∞}
  ...
  · rw [← tsum_eapproxDiff f hf]
    exact tsum_le_tsum fun n => ENNReal.coe_le_coe.2 (f_le_g n x)
  ...
  · calc
      ...
      _ ≤ ∑' n, ((∫⁻ x, eapproxDiff f n x ∂μ) + δ n) := tsum_le_tsum hg
      _ = ∑' n, ∫⁻ x, eapproxDiff f n x ∂μ + ∑' n, δ n := tsum_add
      _ ≤ (∫⁻ x : α, f x ∂μ) + ε := by
