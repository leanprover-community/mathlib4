...
variable {X Y E F : Type*} [MeasurableSpace X] {μ : Measure X} {𝕜 : Type*} [RCLike 𝕜]
  [NormedAddCommGroup E] [NormedSpace 𝕜 E] [NormedAddCommGroup F] [NormedSpace 𝕜 F] {p : ℝ≥0∞}
...
lemma integral_const_mul {A : Type*} [NonUnitalNormedRing A] [CompleteSpace A] [NormedSpace ℝ A]
...
lemma integral_mul_const {A : Type*} [NonUnitalNormedRing A] [CompleteSpace A] [NormedSpace ℝ A]
    [IsScalarTower ℝ A A] [SMulCommClass ℝ A A] {f : X → A} (hf : Integrable f μ) {c : A} :
    ∫ x, f x * c ∂μ = (∫ x, f x ∂μ) * c := by
