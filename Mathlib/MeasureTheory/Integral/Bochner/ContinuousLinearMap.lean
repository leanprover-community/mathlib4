...
variable {X Y E F Fₗ : Type*} [MeasurableSpace X] {μ : Measure X} {𝕜 𝕜' : Type*} [RCLike 𝕜]
  [RCLike 𝕜'] [AddCommGroup E] [NormedAddCommGroup E] [NormedSpace 𝕜 E] [AddCommGroup F]
  [NormedAddCommGroup F] [NormedSpace 𝕜' F] [AddCommGroup Fₗ]
...
theorem integral_apply {H : Type*} [AddCommGroup H] [NormedAddCommGroup H] [NormedSpace 𝕜 H]
    {φ : X → H →L[𝕜] E} (φ_int : Integrable φ μ) (v : H) : (∫ x, φ x ∂μ) v = ∫ x, φ x v ∂μ := by
