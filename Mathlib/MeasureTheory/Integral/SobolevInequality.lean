...
variable {π : ι → Type*} [∀ i, MeasurableSpace (π i)]
...
For a finite dependent product `Π i : ι, π i` of sigma-finite measure spaces, a finite set `s` of
indices from `ι`, and a (later assumed nonnegative) real number `p`, this operation acts on a
function `f` from `Π i, π i` into the extended nonnegative reals.  The operation is to partially
integrate, in the `s` co-ordinates, the function whose value at `x : Π i, π i` is obtained by
...
function on the "lower-dimensional" space `Π i : s, π i`. -/
def T (p : ℝ) (f : (∀ i, π i) → ℝ≥0∞) (s : Finset ι) : (∀ i, π i) → ℝ≥0∞ :=
  ∫⋯∫_s, f ^ (1 - (s.card - 1 : ℝ) * p) * ∏ i in s, (∫⋯∫_{i}, f ∂μ) ^ p ∂μ

variable {p : ℝ}

@[simp] lemma T_univ (f : (∀ i, π i) → ℝ≥0∞) (x : ∀ i, π i) :
    T μ p f univ x
    = ∫⁻ (x : ∀ i, π i), (f x ^ (1 - (#ι - 1 : ℝ) * p)
      * ∏ i : ι, (∫⁻ t : π i, f (update x i t) ∂(μ i)) ^ p) ∂(.pi μ) := by
  simp [T, marginal_univ, marginal_singleton, card_univ]

@[simp] lemma T_empty (f : (∀ i, π i) → ℝ≥0∞) (x : ∀ i, π i) :
...
theorem T_insert_le_T_marginal_singleton (hp1 : 0 ≤ p) (u : Finset ι) (hp2 : (u.card : ℝ) * p ≤ 1)
    (i : ι) (hi : i ∉ u) {f : (∀ i, π i) → ℝ≥0∞} (hf : Measurable f) :
    T μ p f (insert i u) ≤ T μ p (∫⋯∫_{i}, f ∂μ) u := by
  calc T μ p f (insert i u)
      = ∫⋯∫_insert i u,
            f ^ (1 - (u.card : ℝ) * p) * ∏ j in (insert i u), (∫⋯∫_{j}, f ∂μ) ^ p ∂μ := by
          simp_rw [T, card_insert_of_not_mem hi]
          congr!
          push_cast
          ring
    _ = ∫⋯∫_u, (fun x ↦ ∫⁻ (t : π i),
            (f (update x i t) ^ (1 - (u.card : ℝ) * p)
            * ∏ j in (insert i u), (∫⋯∫_{j}, f ∂μ) (update x i t) ^ p)  ∂ (μ i)) ∂μ := by
...
increases. -/
theorem T_marginal_antitone (hp1 : 0 ≤ p) (hp2 : (#ι - 1 : ℝ) * p ≤ 1)
  ...
  convert T_insert_le_T_marginal_singleton μ hp1 s ?_ i hi (hf.marginal μ) using 2
  · rw [← marginal_union μ f hf]
...
For any finite dependent product `Π i : ι, π i` of sigma-finite measure spaces, for any
nonnegative real number `p` such that `(#ι - 1) * p ≤ 1`, for any function `f` from `Π i, π i` into
the extended nonnegative reals, we consider an associated "grid-lines quantity", the integral of an
associated function from `Π i, π i` into the extended nonnegative reals.  The value of this function
at `x : Π i, π i` is obtained by multiplying a certain power of `f` with the product, for each
...
integral of `f`. -/
theorem lintegral_mul_prod_lintegral_pow_le {p : ℝ} (hp1 : 0 ≤ p)
  ...
  simpa [marginal_univ] using GridLines.T_marginal_antitone μ hp1 hp2 hf H default

/-- The "grid-lines lemma" (not a standard name), stated with the extremal exponent
`p = (#ι - 1)⁻¹`. Compare with `lintegral_mul_prod_lintegral_pow_le`.

For any finite dependent product `Π i : ι, π i` of at least two sigma-finite measure spaces, for any
function `f` from `Π i, π i` into the extended nonnegative reals, we consider an associated
"grid-lines quantity", the integral of an associated function from `Π i, π i` into the extended
nonnegative reals.  The value of this function at `x : Π i, π i` is the product, for each
co-ordinate `i`, of the `(#ι - 1)⁻¹`-th power of the integral of `f` along the "grid line" in the
`i` direction through `x`.

This lemma bounds the Lebesgue integral of the grid-lines quantity by the `#ι / (#ι - 1)`-th power
of the Lebesgue integral of `f`. -/
theorem lintegral_prod_lintegral_pow_le [Nontrivial ι] {f} (hf : Measurable f) :
  ...
  convert lintegral_mul_prod_lintegral_pow_le μ hp1 hp2 hf using 2 <;> field_simp
