...
lemma HomClass.map_constants {F M N} [L.Structure M] [L.Structure N] [FunLike F M N]
...
@[simp]
lemma comp_id (f : M →[L] N) : f.comp (id L M) = f :=
  rfl

@[simp]
lemma id_comp (f : M →[L] N) : (id L N).comp f = f :=
  rfl
...
lemma toHom_injective : @Function.Injective (M ↪[L] N) (M →[L] N) (·.toHom) := by
  intro f f' h
  ext
  exact congr_fun (congr_arg (↑) h) _

@[simp]
lemma toHom_inj {f g : M ↪[L] N} : f.toHom = g.toHom ↔ f = g :=
...
lemma comp_injective (h : N ↪[L] P) :
    Function.Injective (h.comp : (M ↪[L] N) →  (M ↪[L] P)) := by
  intro f g hfg
  ext x; exact h.injective (DFunLike.congr_fun hfg x)

@[simp]
lemma comp_inj (h : N ↪[L] P) (f g : M ↪[L] N) : h.comp f = h.comp g ↔ f = g :=
  ⟨fun eq ↦ h.comp_injective eq, congr_arg h.comp⟩

lemma toHom_comp_injective (h : N ↪[L] P) :
    Function.Injective (h.toHom.comp : (M →[L] N) →  (M →[L] P)) := by
  intro f g hfg
  ext x; exact h.injective (DFunLike.congr_fun hfg x)

@[simp]
lemma toHom_comp_inj (h : N ↪[L] P) (f g : M →[L] N) : h.toHom.comp f = h.toHom.comp g ↔ f = g :=
...
@[simp]
lemma comp_refl (f : M ↪[L] N) : f.comp (refl L M) = f := DFunLike.coe_injective rfl

@[simp]
lemma refl_comp (f : M ↪[L] N) : (refl L N).comp f = f := DFunLike.coe_injective rfl

@[simp]
lemma refl_toHom : (refl L M).toHom = Hom.id L M :=
  rfl
...
lemma injective_toEmbedding : Function.Injective (toEmbedding : (M ≃[L] N) → M ↪[L] N) := by
...
@[simp]
lemma comp_refl (g : M ≃[L] N) : g.comp (refl L M) = g :=
  rfl

@[simp]
lemma refl_comp (g : M ≃[L] N) : (refl L N).comp g = g :=
  rfl

@[simp]
lemma refl_toEmbedding : (refl L M).toEmbedding = Embedding.refl L M :=
  rfl
...
@[simp]
lemma comp_right_inj (h : M ≃[L] N) (f g : N ≃[L] P) : f.comp h = g.comp h ↔ f = g :=
