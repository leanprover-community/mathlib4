...
of terms. -/
private theorem realize_equal_iff_of_agree_qf
...
applied to terms. -/
private theorem realize_rel_iff_of_agree_qf
    {α : Type*} {M N : Type*} [L.Structure M] [L.Structure N] {v : α → M} {w : α → N}
...
private theorem relabel_realize_eq_iff_of_agree_qf
  ...
  exact (Formula.realize_equal.symm.trans
    (realize_equal_iff_of_agree_qf hQF (t.relabel idx) (u.relabel idx)).symm).trans
...
private theorem exists_embedding_of_agree_qf
  ...
  have hvar : ∀ {i j : α}, v i = v j → w i = w j := fun {i j} hij => by
    ...
    simpa [Formula.realize_equal] using (realize_equal_iff_of_agree_qf hQF _ _).mp hv
  ...
  refine ⟨{
    ...
    map_rel' := by
      ...
      simpa [hterm_realize, hrepr, Function.comp_def] using
        (realize_rel_iff_of_agree_qf hQF R fun i => (repr (x i)).relabel idx).symm
  }, ?_⟩
