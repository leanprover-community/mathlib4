...
/-- Realizing `θ.toFormula` after putting the unique bound variable in the right summand. -/
private theorem realize_toFormula_snoc {α : Type*} {M : Type*} [L.Structure M]
    (θ : L.BoundedFormula α 1) (v : α → M) (b : M) :
    θ.toFormula.Realize (Sum.elim v (Fin.snoc default b)) ↔
      θ.Realize v (Fin.snoc default b) := by
...
private theorem realize_relabel_id_snoc {α : Type*} {M : Type*} [L.Structure M]
    (φ : L.Formula (α ⊕ Fin 1)) (v : α → M) (b : M) :
...
theorem hasQuantifierElimination_of_exists_realize_of_embeddings {T : L.Theory} :
  ...
  · obtain ⟨c, hc⟩ := h φ hφQF f g a ⟨b, (realize_toFormula_snoc θ (f ∘ a) b).mpr hb⟩
    exact ⟨c, (realize_toFormula_snoc θ (g ∘ a) c).mp hc⟩
  · obtain ⟨b, hb⟩ := h φ hφQF g f a ⟨c, (realize_toFormula_snoc θ (g ∘ a) c).mpr hc⟩
    exact ⟨b, (realize_toFormula_snoc θ (f ∘ a) b).mp hb⟩
...
/-- Build a `< κ`-generated partial equivalence between `M` and `N` from a pair of embeddings
`f : A ↪[L] M` and `g : A ↪[L] N` together with a finite tuple `a : α → A`. The domain is the
substructure generated by the range of `f ∘ a`, and the equivalence agrees with `g ∘ a` on the
images of the `a i`. -/
private theorem cardinalLTEquiv_of_embeddings
    {α : Type} [Finite α] {κ : Cardinal} (hκ : Cardinal.aleph0 ≤ κ)
    {M N A : Type max u v} [L.Structure M] [L.Structure N] [L.Structure A]
    (f : A ↪[L] M) (g : A ↪[L] N) (a : α → A) :
    ∃ (p₀ : L.CardinalLTEquiv M N κ) (hp_dom : ∀ i, f (a i) ∈ (p₀ : M ≃ₚ[L] N).dom),
      ∀ i, ((p₀ : M ≃ₚ[L] N).toEquiv ⟨f (a i), hp_dom i⟩ : N) = g (a i) := by
  ...
  refine ⟨⟨{ dom := S, cod := k.toHom.range, toEquiv := k.equivRange }, hS_cardinal⟩,
    fun i => Substructure.subset_closure ⟨i, rfl⟩, fun i => ?_⟩
  change g (f.equivRange.symm (Substructure.inclusion hSrange _)) = g (a i)
  congr 1
...
private theorem exists_realize_descent_through_elementary
  ...
  have hθN' : θ.ex.Realize (e.toEmbedding ∘ ga) default :=
    BoundedFormula.realize_ex.mpr
      ⟨b', (realize_relabel_id_snoc φ (e.toEmbedding ∘ ga) b').mpr htarget⟩
  ...
  exact ⟨c, (realize_relabel_id_snoc φ ga c).mp hc⟩
...
theorem hasQuantifierElimination_of_isElementaryExtensionPairCardinalLTGenerated
  ...
  obtain ⟨b, hb⟩ := hM
  obtain ⟨p₀, hp_dom, hp_apply⟩ := cardinalLTEquiv_of_embeddings hκ f g a
