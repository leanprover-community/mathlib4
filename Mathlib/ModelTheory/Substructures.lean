...
@[simp]
lemma subtype_substructureEquivMap (f : M ↪[L] N) (s : L.Substructure M) :
...
@[simp]
lemma subtype_equivRange (f : M ↪[L] N) : (subtype _).comp f.equivRange.toEmbedding = f := by
  ext; rfl
...
@[simp]
lemma inclusion_self (S : L.Substructure M) : inclusion (le_refl S) = Embedding.refl L S := rfl
