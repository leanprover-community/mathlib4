...
@[simp]
lemma closedUnder_univ : ClosedUnder f (univ : Set M) := fun _ _ => mem_univ _
...
lemma inter (hs : ClosedUnder f s) (ht : ClosedUnder f t) : ClosedUnder f (s ∩ t) := fun x h =>
...
lemma inf (hs : ClosedUnder f s) (ht : ClosedUnder f t) : ClosedUnder f (s ⊓ t) :=
  hs.inter ht
#align first_order.language.closed_under.inf FirstOrder.Language.ClosedUnder.inf

variable {S : Set (Set M)}

lemma sInf (hS : ∀ s, s ∈ S → ClosedUnder f s) : ClosedUnder f (sInf S) := fun x h s hs =>
...
@[simp]
lemma mem_carrier {s : L.Substructure M} {x : M} : x ∈ s.carrier ↔ x ∈ s :=
  Iff.rfl
...
@[ext]
lemma ext {S T : L.Substructure M} (h : ∀ x, x ∈ S ↔ x ∈ T) : S = T :=
  SetLike.ext h
...
lemma constants_mem (c : L.Constants) : (c : M) ∈ S :=
...
@[simp]
lemma coe_inf (p p' : L.Substructure M) :
...
@[simp]
lemma mem_inf {p p' : L.Substructure M} {x : M} : x ∈ p ⊓ p' ↔ x ∈ p ∧ x ∈ p' :=
  Iff.rfl
...
@[simp, norm_cast]
lemma coe_sInf (S : Set (L.Substructure M)) :
...
lemma mem_sInf {S : Set (L.Substructure M)} {x : M} : x ∈ sInf S ↔ ∀ p ∈ S, x ∈ p :=
  Set.mem_iInter₂
#align first_order.language.substructure.mem_Inf FirstOrder.Language.Substructure.mem_sInf

lemma mem_iInf {ι : Sort*} {S : ι → L.Substructure M} {x : M} : (x ∈ ⨅ i, S i) ↔ ∀ i, x ∈ S i :=
...
@[simp, norm_cast]
lemma coe_iInf {ι : Sort*} {S : ι → L.Substructure M} :
...
@[simp]
lemma subset_closure : s ⊆ closure L s :=
...
lemma not_mem_of_not_mem_closure {P : M} (hP : P ∉ closure L s) : P ∉ s := fun h =>
  hP (subset_closure h)
...
lemma lift_card_closure_le_card_term : Cardinal.lift.{max u w} #(closure L s) ≤ #(L.Term s) := by
...
@[elab_as_elim]
lemma closure_induction {p : M → Prop} {x} (h : x ∈ closure L s) (Hs : ∀ x ∈ s, p x)
...
@[elab_as_elim]
lemma dense_induction {p : M → Prop} (x : M) {s : Set M} (hs : closure L s = ⊤)
...
@[simp]
lemma closure_eq : closure L (S : Set M) = S :=
...
@[simp]
lemma closure_empty : closure L (∅ : Set M) = ⊥ :=
...
@[simp]
lemma closure_univ : closure L (univ : Set M) = ⊤ :=
...
lemma closure_unionᵢ {ι} (s : ι → Set M) : closure L (⋃ i, s i) = ⨆ i, closure L (s i) :=
...
@[simp]
lemma mem_comap {S : L.Substructure N} {f : M →[L] N} {x : M} : x ∈ S.comap f ↔ f x ∈ S :=
  Iff.rfl
...
lemma gc_map_comap (f : M →[L] N) : GaloisConnection (map f) (comap f) := fun _ _ =>
  map_le_iff_le_comap
...
lemma map_closure (f : M →[L] N) (s : Set M) : (closure L s).map f = closure L (f '' s) :=
  Eq.symm <|
...
@[simp]
lemma closure_image (f : M →[L] N) : closure L (f '' s) = map f (closure L s) :=
...
lemma comap_iInf_map_of_injective (S : ι → L.Substructure M) :
...
lemma comap_iSup_map_of_injective (S : ι → L.Substructure M) :
...
lemma map_iInf_comap_of_surjective (S : ι → L.Substructure N) :
...
lemma map_iSup_comap_of_surjective (S : ι → L.Substructure N) :
...
@[simp]
lemma coeSubtype : ⇑S.subtype = ((↑) : S → M) :=
  rfl
...
@[elab_as_elim]
lemma closure_induction' (s : Set M) {p : ∀ x, x ∈ closure L s → Prop}
...
@[simp]
lemma mem_substructureReduct {x : M} {S : L'.Substructure M} :
...
@[simp]
lemma coe_substructureReduct {S : L'.Substructure M} : (φ.substructureReduct S : Set M) = ↑S :=
  rfl
...
lemma subset_closure_withConstants : A ⊆ closure (L[[A]]) s := by
  intro a ha
...
lemma closure_withConstants_eq :
    closure (L[[A]]) s =
...
@[simp]
lemma comp_codRestrict (f : M →[L] N) (g : N →[L] P) (p : L.Substructure P) (h : ∀ b, g b ∈ p) :
...
@[simp]
lemma subtype_comp_codRestrict (f : M →[L] N) (p : L.Substructure N) (h : ∀ b, f b ∈ p) :
...
lemma range_le_iff_comap {f : M →[L] N} {p : L.Substructure N} : range f ≤ p ↔ comap f p = ⊤ := by
...
lemma map_le_range {f : M →[L] N} {p : L.Substructure M} : map f p ≤ range f :=
...
lemma eqOn_closure {f g : M →[L] N} {s : Set M} (h : Set.EqOn f g s) :
...
lemma eq_of_eqOn_top {f g : M →[L] N} (h : Set.EqOn f g (⊤ : Substructure L M)) : f = g :=
  ext fun _ => h trivial
#align first_order.language.hom.eq_of_eq_on_top FirstOrder.Language.Hom.eq_of_eqOn_top

variable {s : Set M}

lemma eq_of_eqOn_dense (hs : closure L s = ⊤) {f g : M →[L] N} (h : s.EqOn f g) : f = g :=
...
@[simp]
lemma domRestrict_apply (f : M ↪[L] N) (p : L.Substructure M) (x : p) : f.domRestrict p x = f x :=
  rfl
...
@[simp]
lemma codRestrict_apply (p : L.Substructure N) (f : M ↪[L] N) {h} (x : M) :
...
@[simp]
lemma substructureEquivMap_apply (f : M ↪[L] N) (p : L.Substructure M) (x : p) :
...
@[simp]
lemma equivRange_apply (f : M ↪[L] N) (x : M) : (f.equivRange x : N) = f x :=
  rfl
...
lemma toHom_range (f : M ≃[L] N) : f.toHom.range = ⊤ := by
  ext n
...
@[simp]
lemma coe_inclusion {S T : L.Substructure M} (h : S ≤ T) :
...
lemma range_subtype (S : L.Substructure M) : S.subtype.toHom.range = S := by
  ext x
