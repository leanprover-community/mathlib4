...
- `FirstOrder.Language.Theory.IsElementaryExtensionPair` and
  `FirstOrder.Language.Theory.IsElementaryExtensionPairFG` are elementary-extension variants for
  partial equivalences between models of a theory.
...
- `FirstOrder.Language.Theory.IsElementaryExtensionPair.FG` shows that the general elementary
  extension-pair property implies the finitely generated variant.
...
/-- The type of equivalences between Kappa small substructures. -/
abbrev LTKappaEquiv (κ : Cardinal) := {f : M ≃ₚ[L] N // f.dom.LTKappaGenerated κ}
...
def IsElementaryExtensionPairLTKappaGenerated (T : L.Theory) (κ : Cardinal) : Prop :=
  IsElementaryExtensionPairCore (fun M N _ _ => L.LTKappaEquiv M N κ) (fun f => f.1) T
...
-/
theorem IsElementaryExtensionPair.LTKappaGenerated {T : L.Theory} (κ : Cardinal) (h : κ >= ℵ₀)
    (h : T.IsElementaryExtensionPair) : T.IsElementaryExtensionPairLTKappaGenerated κ := by
  sorry
