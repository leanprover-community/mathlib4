...
- `FirstOrder.Language.CardinalLTEquiv` is the type of partial equivalences `M ≃ₚ[L] N` with
  domain generated by fewer than `κ` elements.
...
## Main Results
- `FirstOrder.Language.exists_cardinalLTEquiv_of_embeddings` builds a `< κ`-generated partial
  equivalence from two embeddings out of a common structure and a finite tuple.
...
/-- The type of equivalences between substructures generated by fewer than `κ` elements. -/
abbrev CardinalLTEquiv (κ : Cardinal) := {f : M ≃ₚ[L] N // f.dom.CardinalLTGenerated κ}
...
images of the `a i`. -/
theorem exists_cardinalLTEquiv_of_embeddings
    {α : Type*} [Finite α] {κ : Cardinal} (hκ : Cardinal.aleph0 ≤ κ)
    {A : Type*} [L.Structure A] (f : A ↪[L] M) (g : A ↪[L] N) (a : α → A) :
