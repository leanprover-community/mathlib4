...
theorem Linear.singleton (v) : ({v} : Set α).Linear :=
  ⟨v, ∅, by simp⟩
...
theorem Semilinear.singleton (v) : ({v} : Set α).Semilinear :=
...
theorem Linear.span (hs : s.Linear) : (span ℕ s : Set α).Semilinear := by
  classical
