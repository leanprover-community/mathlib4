...
via `finFree_isColimit_at` and `Types.jointly_surjective_of_isColimit`.
...
private lemma pTC_inj (Y : Profinite.{u}) (F : CondensedSet.{u})
    ...
    (h : e1.hom.app (.op (cH' Y)) (ULift.up (𝟭 (cH' Y))) =
         e2.hom.app (.op (cH' Y)) (ULift.up (𝟭 (cH' Y)))) : e1 = e2 := by
  ext S s
  have key : ∀ e : profiniteToCondensed.obj Y ⟶ F,
      e.hom.app S s = F.obj.map s.down.op (e.hom.app (.op (cH' Y)) (ULift.up (𝟭 (cH' Y)))) := by
    intro e
    ...
    rw [show s = (profiniteToCondensed.obj Y).obj.map s.down.op (ULift.up (𝟭 (cH' Y))) from rfl,
        show e.hom.app S ((profiniteToCondensed.obj Y).obj.map s.down.op (ULift.up (𝟭 (cH' Y)))) =
...
lemma surj_factor (T : FintypeCat.{u}) (X : Profinite.{u})
  ...
  let elem : F_set.obj.obj (.op (cH' X)) :=
    eta_adj.hom.app (.op (cH' X)) (ULift.up (𝟭 (cH' X)))
  ...
  have eta_eq : eta_adj = profiniteToCondensed.map q_j ≫ eta0 :=
    pTC_inj _ _ _ _ <| by
      have lhs : (ConcreteCategory.hom (eta_adj.hom.app (.op (cH' X)))) { down := 𝟭 (cH' X) } =
                 elem := rfl
      have rhs : (ConcreteCategory.hom
            ((profiniteToCondensed.map q_j ≫ eta0).hom.app (.op (cH' X))))
            { down := 𝟭 (cH' X) } = elem :=
        (show (ConcreteCategory.hom ((profiniteToCondensed.map q_j ≫ eta0).hom.app
                (.op (cH' X)))) { down := 𝟭 (cH' X) } =
...
lemma isSolid_of_isLimit_gen {J : Type*} [Category J] {F : J ⥤ CondensedMod.{u} R}
  ...
  have bijFun : ∀ j, Function.Bijective ((yoneda.obj (F.obj j)).map sol.op) := by
    intro j
    exact isIso_iff_bijective.mp ((hj j).isIso_solidification_map X)
  constructor
  · intro f g hfg
    apply hc.hom_ext; intro j; exact (bijFun j).1 (congrArg (· ≫ c.π.app j) hfg)
  · intro h_map
    choose g_j hg_j using fun j => (bijFun j).2 (h_map ≫ c.π.app j)
    have compat : ∀ {j k : J} (phi : j ⟶ k), g_j j ≫ F.map phi = g_j k := fun j k phi =>
