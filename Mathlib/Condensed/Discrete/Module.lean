...
def functorToPresheaves : ModuleCat.{max u w} R ⥤ ((CompHausLike.{u} P)ᵒᵖ ⥤ ModuleCat R) where
  obj X := {
    ...
    map := fun f ↦ ModuleCat.asHom (comapₗ R f.unop) }
  map f := { app := fun S ↦ ModuleCat.asHom (mapₗ R f.hom) }
...
noncomputable def functorIsoDiscreteAux₁ (M : ModuleCat.{u+1} R) :
  ...
  hom := ModuleCat.asHom (constₗ R)
  inv := ModuleCat.asHom (evalₗ R PUnit.unit)
