...
variable (X : CompHaus.{u}ᵒᵖ ⥤ Type max u w) [PreservesFiniteProducts X]
...
-/
def sigmaComparison : X.obj ⟨(of ((a : α) × σ a))⟩ ⟶ ((a : α) → X.obj ⟨of (σ a)⟩) :=
...
theorem sigmaComparison_eq_comp_isos : sigmaComparison X σ =
    (X.mapIso (opCoproductIsoProduct' (finiteCoproduct.isColimit.{u, u} fun a ↦ of (σ a))
      (productIsProduct fun x ↦ Opposite.op (of (σ x))))).hom ≫
    (PreservesProduct.iso X fun a ↦ ⟨of (σ a)⟩).hom ≫
    (Types.productIso.{u, max u w} fun a ↦ X.obj ⟨of (σ a)⟩).hom := by
  ext x a
  ...
  have := congrFun (piComparison_comp_π X (fun a ↦ ⟨of (σ a)⟩) a)
...
-/
@[simps]
def functorToPresheaves : Type (max u w) ⥤ (CompHaus.{u}ᵒᵖ ⥤ Type max u w) where
  obj X := {
...
variable {S T : CompHaus.{u}} (g : T ⟶ S) {Y : CompHaus.{u}ᵒᵖ ⥤ Type max u w}
