...
def functor :
  ...
  map f := ObjectProperty.homMk (functorToPresheaves.{u, w}.map f)
...
noncomputable def counit [HasExplicitFiniteCoproducts.{u} P] : haveI := CompHausLike.preregular hs
  ...
  naturality X Y g := by
    ...
    simp only [functor, Functor.comp_obj, Functor.flip_obj_obj,
      ...
      sheafToPresheaf_map, ObjectProperty.FullSubcategory.comp_hom, Functor.id_map,
...
set_option backward.isDefEq.respectTransparency false in
@[simp]
lemma counit_app_hom [HasExplicitFiniteCoproducts.{u} P] :
    haveI := CompHausLike.preregular hs
    ∀ (X : Sheaf (coherentTopology (CompHausLike.{u} P)) (Type (max u w))),
    ((counit P hs).app X).hom = counitApp X.obj := fun _ => rfl
...
noncomputable def adjunction [HasExplicitFiniteCoproducts.{u} P] :
  ...
  left_triangle_components := by
    ...
    simp only [Functor.comp_obj, Functor.id_obj, Functor.flip_obj_obj, sheafToPresheaf_obj,
      functor_obj_obj, functorToPresheaves_obj_obj]
    apply InducedCategory.hom_ext
    rw [ObjectProperty.FullSubcategory.comp_hom, ObjectProperty.FullSubcategory.id_hom]
    exact adjunction_left_triangle P hs X
  right_triangle_components := by
    intro X
    ext (x : X.obj.obj _)
    simp only [Functor.comp_obj, Functor.id_obj, Functor.flip_obj_obj, sheafToPresheaf_obj,
      functor_obj_obj, functorToPresheaves_obj_obj, types_id_apply,
      Functor.flip_obj_map, sheafToPresheaf_map, counit_app_hom]
    have := CompHausLike.preregular hs
