...
instance : Epi (Limits.Pi.map.{0} f) := by
  have : Limits.Pi.map.{0} f = (cone f).π.app ⟨0⟩ := rfl
  rw [this]
  refine epi_π_app_zero_of_epi R (isLimit f) (fun n ↦ by simpa using by infer_instance)
