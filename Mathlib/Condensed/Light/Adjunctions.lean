...
-- TODO: add to file defining `HasSheafCompose`
instance {A B C : Type (u+1)} [LargeCategory C] [LargeCategory A] [LargeCategory B]
    [EssentiallySmall C] (F : A ⥤ B)
    (J : GrothendieckTopology C)
    [((equivSmallModel C).locallyCoverDense J).inducedTopology.HasSheafCompose F] :
    J.HasSheafCompose F := sorry
