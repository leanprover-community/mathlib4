...
`coindHom φ A` and prove the two representations are isomorphic.
...
/-- The submodule of functions `H → A` underlying the coinduction of `ρ` along `φ`. -/
@[simps]
...
/-- The coinduction of `ρ` along `φ`, defined by sending `h : H` and a function `f : H → A` to the
function sending `h₁` to `f(h₁ * h)`. -/
@[simps]
...
/-- The coinduction of `A` along `φ`, defined by sending `h : H` and a function `f : H → A` to the
function `H → A` sending `h₁` to `f(h₁ * h)`. -/
...
to `coind φ A`, with action on maps given by postcomposition. -/
@[simps obj map]
...
/-- The coinduction of `A` along `φ`, defined by sending `h : H` and a `G`-representation morphism
`f : k[H] ⟶ A` to the `G`-representation morphism `k[H] ⟶ A` sending `r • h₁` to
`r • f(h₁ * h)`. -/
@[simps]
noncomputable def _root_.Representation.coindHom :
...
`f : k[H] ⟶ A` to the `G`-representation morphism `k[H] ⟶ A` sending `r • h₁` to
`r • f(h₁ * h)`. -/
noncomputable abbrev coindHom : Rep k H := Rep.of (Representation.coindHom φ A)

variable {A} in
@[ext]
lemma coindHom_ext {f g : coindHom φ A}
    (hfg : ∀ h, f.hom (single h 1) = g.hom (single h 1)) : f = g :=
  Action.Hom.ext <| ModuleCat.hom_ext <| lhom_ext' fun h => LinearMap.ext_ring <| hfg h

/-- Given a monoid morphism `φ : G →* H` and a morphism of `G`-representations `f : A ⟶ B`, there
is a natural `H`-representation morphism `coindHom φ A ⟶ coindHom φ B`, given by postcomposition
by `f`. -/
@[simps]
noncomputable def coindHomMap {A B : Rep k G} (f : A ⟶ B) : coindHom φ A ⟶ coindHom φ B where
  hom := ModuleCat.ofHom <| Linear.rightComp k _ f
  comm h := by ext; simp [ModuleCat.endRingEquiv]

variable (k) in
/-- Given a monoid homomorphism `φ : G →* H`, this is the functor sending a `G`-representation `A`
to `coindHom φ A`, with action on maps given by postcomposition. -/
@[simps obj map]
noncomputable def coindHomFunctor : Rep k G ⥤ Rep k H where
  obj A := coindHom φ A
...
/-- The `k`-submodule of functions `f : H → A` such that for all `g : G`, `h : H`,
`f (φ g * h) = A.ρ g (f h)`, is `k`-linearly equivalent to the `G`-representation morphisms
`k[H] ⟶ A`. -/
@[simps]
noncomputable def coindVLEquiv :
  ...
  map_add' _ _ := coindHom_ext φ fun _ => by simp
  map_smul' _ _ := coindHom_ext φ fun _ => by simp
  invFun f := {
    val h := f.hom (single h 1)
    property g h := by have := (hom_comm_apply f g (single h 1)).symm; simp_all [Rep.res_obj_ρ φ] }
  left_inv x := by simp
  right_inv x := coindHom_ext φ fun _ => by simp

/-- `coind φ A` and `coindHom φ A` are isomorphic representations, with the underlying
`k`-linear equivalence given by `coindVLEquiv`. -/
@[simps! hom_hom_hom inv_hom_hom]
noncomputable def coindIso : coind φ A ≅ coindHom φ A :=
...
`coindFunctor k φ` and `coindHomFunctor k φ` are naturally isomorphic, with isomorphism on objects
given by `coindIso φ`. -/
@[simps!]
noncomputable def coindFunctorIso : coindFunctor k φ ≅ coindHomFunctor k φ :=
  NatIso.ofComponents (coindIso φ) fun _ => by
    simp only [coindFunctor_obj, coindHomFunctor_obj]
    ext
    simp
