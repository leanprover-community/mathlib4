...
def ofMulAction (G : Type v) (H : Type u) [Monoid G] [MulAction G H] : Action (Type u) G where
  V := H
...
@[simp]
theorem ofMulAction_apply {G : Type v} {H : Type v} [Monoid G] [MulAction G H] (g : G) (x : H) :
...
def ofMulActionLimitCone {ι : Type w} (G : Type v) [Monoid G] (F : ι → Type max w u)
    [∀ i : ι, MulAction G (F i)] :
...
instance {G : Type u} [Monoid G] (X : Action FintypeCat.{v} G) : MulAction G X.V :=
  Action.instMulAction X
instance {G : Type u} [Group G] (X : Action FintypeCat.{v} G) : MulAction G X.V :=
  Action.instMulAction X
