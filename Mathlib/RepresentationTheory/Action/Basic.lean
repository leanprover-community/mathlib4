...
def actionPunitEquivalence : Action V PUnit ≌ V where
  ...
  unitIso :=
    NatIso.ofComponents fun X => mkIso (Iso.refl _) fun ⟨⟩ => by
      simp only [Functor.id_obj, MonoidHom.one_apply, id_eq, Functor.comp_obj, forget_obj,
        Iso.refl_hom, Category.comp_id, End.one_def]
      exact ρ_one X
...
-/
@[simps]
def res {G : Type v} {H : Type w} [Monoid G] [Monoid H] (f : G →* H) :
...
-/
@[simps!]
def resId {G : Type v} [Monoid G] : res V (MonoidHom.id G) ≅ 𝟭 (Action V G) :=
  NatIso.ofComponents fun M => mkIso (Iso.refl _)
set_option linter.uppercaseLean3 false in
...
@[simps]
def mapAction (F : V ⥤ W) (G : Type v) [Monoid G] : Action V G ⥤ Action W G where
  obj M :=
    { V := F.obj M.V
      ρ :=
