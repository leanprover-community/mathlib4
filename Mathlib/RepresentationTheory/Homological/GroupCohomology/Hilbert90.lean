...
* `NumberField.exists_eq_did_gen_apply_of_norm_eq_one`: the classical formulation of Hilbert's
...
lemma exists_eq_div_gen_apply_of_norm_eq_one [IsGalois K L] : ∃ ε : L, η = ε / σ ε := by
...
lemma exists_eq_div_gen_apply_of_norm_eq_one_integral [IsGalois K L] {η : B}
  ...
  obtain ⟨ε, hε⟩ := exists_eq_div_gen_apply_of_norm_eq_one hσ hη
