...
In `RepresentationTheory.Homological.Group(Co)homology.Basic`, we then use `Rep.barResolution` to
define the inhomogeneous (co)chains of a representation, useful for computing group (co)homology.
