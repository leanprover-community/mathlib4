...
theorem δ₀_apply (z : G →₀ X.X₃) (hz : dZero X.X₃ z = 0) (y : G →₀ X.X₂)
  ...
  have δ_1_0 := congr((isoH0 X.X₁).hom $((map_chainsFunctor_shortExact hX).δ_apply 1 0 rfl
...
theorem epi_δ₀_of_isZero (h0 : IsZero (H0 X.X₂)) : Epi (δ₀ hX) := by
  letI : Epi ((map_chainsFunctor_shortExact hX).δ 1 0 rfl) := (map_chainsFunctor_shortExact hX).epi_δ _ _ rfl
    (h0.of_iso (isoH0 X.X₂))
  exact epi_comp _ _
...
theorem δ₁_apply_aux (y : G × G →₀ X.X₂) (x : G →₀ X.X₁)
    ...
    dZero X.X₁ x = 0 := by
  have h1 :=  (map_chainsFunctor_shortExact hX).δ_apply_aux 2 1 ((twoChainsLequiv X.X₂).symm y)
...
theorem epi_δ₁_of_isZero (h1 : IsZero (H1 X.X₂)) :
    Epi (δ₁ hX) := by
  letI : Epi ((map_chainsFunctor_shortExact hX).δ 2 1 rfl) := (map_chainsFunctor_shortExact hX).epi_δ _ _ rfl
    (h1.of_iso (isoH1 X.X₂))
  exact epi_comp _ _
...
noncomputable abbrev H0ShortComplex₁ :=
  ShortComplex.mk (δ₀ hX) (H0Map (MonoidHom.id G) X.f) <| by
    simpa [δ₀, ← map_comp_isoH0_hom] using (map_chainsFunctor_shortExact hX).δ_comp_assoc 1 0 rfl _
...
noncomputable abbrev H1ShortComplex₃ :=
  ShortComplex.mk (H1Map (MonoidHom.id G) X.g) (δ₀ hX) <| by
    ...
    have h := (map_chainsFunctor_shortExact hX).comp_δ 1 0 rfl
...
noncomputable abbrev H1ShortComplex₁ :=
  ShortComplex.mk (δ₁ hX) (H1Map (MonoidHom.id G) X.f) <| by
    simpa [δ₁, ← map_comp_isoH1_hom] using (map_chainsFunctor_shortExact hX).δ_comp_assoc 2 1 rfl _
...
noncomputable abbrev H2ShortComplex₃ :=
  ShortComplex.mk (H2Map (MonoidHom.id G) X.g) (δ₁ hX) <| by
    ...
    have h := (map_chainsFunctor_shortExact hX).comp_δ 2 1 rfl
    simp_all only [δ₁, ← Category.assoc, Preadditive.IsIso.comp_right_eq_zero]
    simp_all
...
theorem δ₀_apply (z : G →₀ X.X₃) (hz : dZero X.X₃ z = 0) (y : G →₀ X.X₂)
  ...
  have δ_1_0 := congr((isoH0 X.X₁).hom $((map_chainsFunctor_shortExact hX).δ_apply 1 0 rfl
    ...
    ((zeroChainsLequiv X.X₁).symm x) (by simpa using hxy) 0 (by simp)))
  · convert δ_1_0
    · show ConcreteCategory.hom (H1π X.X₃ ≫ δ₀ hX) ⟨z, hz⟩ = _
      rw [δ₀, ← Category.assoc]
      simp [(CommSq.vert_inv ⟨groupHomologyπ_comp_isoH1_hom X.X₃⟩).w,
        isoOneCycles_inv_apply_eq_cyclesMk X.X₃ ⟨z, hz⟩, groupHomology, groupHomologyπ,
        -groupHomologyπ_comp_isoH0_hom]
    · simp [(Iso.eq_inv_comp _).2 (groupHomologyπ_comp_isoH0_hom X.X₁).symm, groupHomology,
        -groupHomologyπ_comp_isoH0_hom, isoZeroCycles_inv_apply_eq_cyclesMk X.X₁ x]

open Limits

theorem epi_δ₀_of_isZero (h0 : IsZero (H0 X.X₂)) : Epi (δ₀ hX) := by
  letI : Epi ((map_chainsFunctor_shortExact hX).δ 1 0 rfl) :=
    (map_chainsFunctor_shortExact hX).epi_δ _ _ rfl (h0.of_iso (isoH0 X.X₂))
  exact epi_comp _ _
...
theorem δ₁_apply (z : G × G →₀ X.X₃) (hz : dOne X.X₃ z = 0) (y : G × G →₀ X.X₂)
  ...
  have δ_2_1 := congr((isoH1 X.X₁).hom $(δ_apply hX _ _ 0 rfl (by simp)
    ...
    ((oneChainsLequiv X.X₁).symm x) hxy))
  · convert δ_2_1
    · show ConcreteCategory.hom (H2π X.X₃ ≫ δ₁ hX) ⟨z, hz⟩ = _
      rw [moduleCatCyclesIso_inv_apply, δ₁, ← Category.assoc]
      simp [(CommSq.vert_inv ⟨groupHomologyπ_comp_isoH2_hom X.X₃⟩).w,
        isoTwoCycles_inv_apply_eq_cyclesMk X.X₃ ⟨z, hz⟩, HomologicalComplex.cyclesMk,
        groupHomology, groupHomologyπ,
        -groupHomologyπ_comp_isoH1_hom]
    · rw [moduleCatCyclesIso_inv_apply]
      simp [(Iso.eq_inv_comp _).2 (groupHomologyπ_comp_isoH1_hom X.X₁).symm,
        -groupHomologyπ_comp_isoH1_hom, isoOneCycles_inv_apply_eq_cyclesMk X.X₁ ⟨x, _⟩,
        HomologicalComplex.cyclesMk]
...
theorem epi_δ₁_of_isZero (h1 : IsZero (H1 X.X₂)) :
    Epi (δ₁ hX) := by
  letI : Epi ((map_chainsFunctor_shortExact hX).δ 2 1 rfl) :=
    (map_chainsFunctor_shortExact hX).epi_δ _ _ rfl (h1.of_iso (isoH1 X.X₂))
  exact epi_comp _ _
...
noncomputable abbrev H0ShortComplex₂ :=
  ShortComplex.mk (H0Map (MonoidHom.id G) X.f) (H0Map (MonoidHom.id G) X.g) <|
    ModuleCat.hom_ext <| Submodule.linearMap_qext _ <| by
      ext x
      have := congr(Action.Hom.hom $(X.zero) x)
      simp_all [-ShortComplex.zero, H0Map, LinearMap.zero_apply (M₂ := X.X₃) x, H0, shortComplexH0]
...
noncomputable def isoH0ShortComplex₁ :
    mapShortComplex₁ hX (i := 1) rfl ≅ H0ShortComplex₁ hX :=
  isoMk (isoH1 _) (isoH0 _) (isoH0 _) (by simp [δ₀]) (map_comp_isoH0_hom (MonoidHom.id G) _).symm
