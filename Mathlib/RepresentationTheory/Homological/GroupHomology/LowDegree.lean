...
on `A` is trivial. -/
def oneCyclesIsoOfIsTrivial [A.IsTrivial] :
    ModuleCat.of k (oneCycles A) ≅ ModuleCat.of k (G →₀ A) :=
  (LinearEquiv.ofTop _ (oneCycles_eq_top_of_isTrivial A)).toModuleIso

@[simp]
lemma oneCyclesIsoOfIsTrivial_hom_apply [A.IsTrivial] (x : oneCycles A) :
    (oneCyclesIsoOfIsTrivial A).hom x = x.1 := rfl

@[simp]
lemma oneCyclesIsoOfIsTrivial_inv_apply [A.IsTrivial] (x : G →₀ A) :
    ((oneCyclesIsoOfIsTrivial A).inv x).1 = x := rfl
...
def H0Iso : H0 A ≅ (coinvariantsFunctor k G).obj A :=
  (ChainComplex.isoHomologyι₀ _) ≪≫ zeroOpcyclesIso A

/-- The quotient map from `A` to `H₀(G, A)`. -/
def H0π : A.V ⟶ H0 A := (zeroCyclesIso A).inv ≫ π A 0

instance : Epi (H0π A) := by unfold H0π; infer_instance

@[reassoc (attr := simp), elementwise (attr := simp)]
lemma π_comp_H0Iso_hom  :
    π A 0 ≫ (H0Iso A).hom = (zeroCyclesIso A).hom ≫ (coinvariantsMk k G).app A := by
  simp [H0Iso, zeroCyclesIso]
...
@[reassoc (attr := simp), elementwise (attr := simp)]
lemma zeroCyclesIso_comp_H0π :
    (zeroCyclesIso A).hom ≫ H0π A = π A 0 := by
  simp [H0π]

@[elab_as_elim]
theorem H0_induction_on {C : H0 A → Prop} (x : H0 A)
    (h : ∀ x : A, C (H0π A x)) : C x :=
  groupHomology_induction_on x fun y => by simpa using h ((zeroCyclesIso A).hom y)
...
theorem π_comp_H0IsoOfIsTrivial_hom :
    π A 0 ≫ (H0IsoOfIsTrivial A).hom = (zeroCyclesIso A).hom := by
...
def H1π : ModuleCat.of k (oneCycles A) ⟶ H1 A :=
  (isoOneCycles A).inv ≫ π A 1
...
lemma H1π_eq_zero_iff (x : oneCycles A) : H1π A x = 0 ↔ x.1 ∈ oneBoundaries A := by
  ...
  simp only [H1π, isoOneCycles, π, HomologicalComplex.homologyπ, homologyπ,
  ...
  simp [LinearMap.range_codRestrict, oneBoundaries, shortComplexH1, oneCycles]
...
@[elab_as_elim]
theorem H1_induction_on {C : H1 A → Prop} (x : H1 A) (h : ∀ x : oneCycles A, C (H1π A x)) :
    C x :=
  groupHomology_induction_on x fun y => by simpa [H1π] using h ((isoOneCycles A).hom y)
...
lemma π_comp_H1Iso_hom  :
    π A 1 ≫ (H1Iso A).hom = (isoOneCycles A).hom ≫
      (shortComplexH1 A).moduleCatLeftHomologyData.π := by
  simp [H1Iso, isoOneCycles, π, HomologicalComplex.homologyπ, leftHomologyπ]
...
def mkH1OfIsTrivial : Additive (Abelianization G) →ₗ[ℤ] A →ₗ[ℤ] H1 A :=
  AddMonoidHom.toIntLinearMap <| AddMonoidHom.toMultiplicative'.symm <| Abelianization.lift {
    toFun g := Multiplicative.ofAdd (AddMonoidHom.toIntLinearMap (AddMonoidHomClass.toAddMonoidHom
      ((H1π A).hom ∘ₗ (oneCyclesIsoOfIsTrivial A).inv.hom ∘ₗ lsingle g)))
    ...
    map_mul' g h := Multiplicative.toAdd.bijective.1 <| LinearMap.ext fun a => by
      simpa [← map_add] using ((H1π_eq_iff _ _).2 ⟨single (g, h) a, by
        simp [oneCyclesIsoOfIsTrivial, sub_add_eq_add_sub, add_comm (single h a),
          dOne_single (A := A)]⟩).symm }

variable {A} in
@[simp]
lemma mkH1OfIsTrivial_apply (g : G) (a : A) :
    mkH1OfIsTrivial A (Additive.ofMul (Abelianization.of g)) a =
      H1π A ((oneCyclesIsoOfIsTrivial A).inv (single g a)) := rfl
...
def H1ToTensorOfIsTrivial : H1 A →ₗ[ℤ] (Additive <| Abelianization G) ⊗[ℤ] A :=
  ((QuotientAddGroup.lift _ ((Finsupp.liftAddHom fun g => AddMonoidHomClass.toAddMonoidHom
    (TensorProduct.mk ℤ _ _ (Additive.ofMul (Abelianization.of g)))).comp
      (oneCycles A).toAddSubgroup.subtype) fun ⟨y, hy⟩ ⟨z, hz⟩ => AddMonoidHom.mem_ker.2 <| by
      simp [← hz, dOne, sum_sum_index, sum_add_index, tmul_add, sum_sub_index, tmul_sub,
...
lemma H1ToTensorOfIsTrivial_H1π_single (g : G) (a : A) :
    H1ToTensorOfIsTrivial A (H1π A <| (oneCyclesIsoOfIsTrivial A).inv (single g a)) =
  ...
  show QuotientAddGroup.lift _ _ _ ((H1Iso A).hom _) = _
  simp [π_comp_H1Iso_hom_apply, Submodule.Quotient.mk, QuotientAddGroup.lift, AddCon.lift,
    AddCon.liftOn, AddSubgroup.subtype, oneCyclesIsoOfIsTrivial]
...
def H1AddEquivOfIsTrivial :
  ...
  LinearEquiv.toAddEquiv <| LinearEquiv.ofLinear
    ...
    (H1Iso A).symm.toLinearEquiv.toAddEquiv.comp_left_injective <|
      ...
      (oneCyclesIsoOfIsTrivial A).symm.toLinearEquiv.toAddEquiv.comp_left_injective <|
      Finsupp.addHom_ext fun _ _ => by
        ...
        show TensorProduct.lift _ (QuotientAddGroup.lift _ _ _ ((H1Iso A).hom _)) = _
        simpa [AddSubgroup.subtype, oneCyclesIsoOfIsTrivial_inv_apply (A := A),
          -π_comp_H1Iso_inv_apply] using (π_comp_H1Iso_inv_apply A _).symm

@[simp]
lemma H1AddEquivOfIsTrivial_single (g : G) (a : A) :
    H1AddEquivOfIsTrivial A (H1π A <| (oneCyclesIsoOfIsTrivial A).inv (single g a)) =
      Additive.ofMul (Abelianization.of g) ⊗ₜ[ℤ] a := by
...
lemma H1AddEquivOfIsTrivial_symm_tmul (g : G) (a : A) :
    (H1AddEquivOfIsTrivial A).symm (Additive.ofMul (Abelianization.of g) ⊗ₜ[ℤ] a) =
      H1π A ((oneCyclesIsoOfIsTrivial A).inv <| single g a) := by
  rfl
...
def H2π : ModuleCat.of k (twoCycles A) ⟶ H2 A :=
  (isoTwoCycles A).inv ≫ π A 2
...
lemma H2π_eq_zero_iff (x : twoCycles A) : H2π A x = 0 ↔ x.1 ∈ twoBoundaries A := by
  ...
  simp only [H2π, isoTwoCycles, π, HomologicalComplex.homologyπ, homologyπ,
    cyclesMapIso'_inv, leftHomologyπ, ← h, ← leftHomologyMapIso'_inv, ModuleCat.hom_comp,
    LinearMap.coe_comp, Function.comp_apply, map_eq_zero_iff _
    ((ModuleCat.mono_iff_injective <|  _).1 inferInstance)]
  simp [LinearMap.range_codRestrict, twoBoundaries, shortComplexH2, twoCycles]
...
@[elab_as_elim]
theorem H2_induction_on {C : H2 A → Prop} (x : H2 A) (h : ∀ x : twoCycles A, C (H2π A x)) :
    C x :=
  groupHomology_induction_on x (fun y => by simpa [H2π] using h ((isoTwoCycles A).hom y))
...
lemma π_comp_H2Iso_hom  :
    π A 2 ≫ (H2Iso A).hom = (isoTwoCycles A).hom ≫
      (shortComplexH2 A).moduleCatLeftHomologyData.π := by
  simp [H2Iso, isoTwoCycles, π, HomologicalComplex.homologyπ, leftHomologyπ]
