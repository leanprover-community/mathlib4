...
theorem ofModule_asAlgebraHom_apply_apply (r : k[G])
  ...
  · intro f g fw gw
    simp only [fw, gw, map_add, add_smul, _root_.add_apply]
  · intro r f w
    simp only [w, map_smul, _root_.smul_apply, RestrictScalars.addEquiv_symm_map_smul_smul]
...
lemma single_smul (t : k) (g : G) (v : ρ.asModule) :
  ...
  rw [← _root_.smul_apply, ← asAlgebraHom_single, ← asModuleEquiv_map_smul]
  rfl
...
theorem smul_tprod_one_asModule (r : k[G]) (x : V) (y : W) :
  ...
  simp only [asAlgebraHom_def, MonoidAlgebra.lift_apply, tprod_apply, MonoidHom.one_apply,
    LinearMap.finsupp_sum_apply, _root_.smul_apply, TensorProduct.map_tmul, Module.End.one_apply]
