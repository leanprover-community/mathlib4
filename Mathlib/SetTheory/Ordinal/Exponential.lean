...
theorem opow_pos {a : Ordinal} (b : Ordinal) (a0 : 0 < a) : 0 < a ^ b := by
  ...
  | H₂ b IH =>
    ...
    exact mul_pos IH a0
  | H₃ b l _ =>
    exact (lt_opow_of_limit (pos_iff_ne_zero.1 a0) l).2 ⟨0, l.pos, h0⟩
    ...
    | H₃ c l IH =>
      exact
        (opow_le_of_limit a0 l).2 fun b' h =>
          (IH _ h).trans (opow_le_opow_right ((pos_iff_ne_zero.2 a0).trans_le ab) h.le)
...
theorem opow_add (a b c : Ordinal) : a ^ (b + c) = a ^ b * a ^ c := by
  ...
  · rcases eq_or_ne c 0 with (rfl | c0)
    · simp
    have : b + c ≠ 0 := ((pos_iff_ne_zero.2 c0).trans_le (le_add_left _ _)).ne'
...
theorem opow_mul_add_pos {b v : Ordinal} (hb : b ≠ 0) (u : Ordinal) (hv : v ≠ 0) (w : Ordinal) :
  ...
  (opow_pos u <| pos_iff_ne_zero.2 hb).trans_le <|
    (le_mul_left _ <| pos_iff_ne_zero.2 hv).trans <| le_add_right _ _
...
theorem log_zero_right (b : Ordinal) : log b 0 = 0 := by
  ...
  · rw [log_def hb, ← le_zero_iff_eq_zero, pred_le, succ_zero]
    apply csInf_le'
...
theorem opow_le_iff_le_log' {b x c : Ordinal} (hb : 1 < b) (hc : c ≠ 0) :
  ...
  · rw [log_zero_right, le_zero_iff_eq_zero, le_zero_iff_eq_zero, opow_eq_zero]
...
theorem le_log_of_opow_le {b x c : Ordinal} (hb : 1 < b) (h : b ^ c ≤ x) : c ≤ log b x := by
  obtain rfl | hx := eq_or_ne x 0
  · rw [le_zero_iff_eq_zero, opow_eq_zero] at h
...
theorem opow_le_of_le_log {b x c : Ordinal} (hc : c ≠ 0) (h : c ≤ log b x) : b ^ c ≤ x := by
  ...
  · rw [log_of_left_le_one hb] at h
    exact (h.not_lt (pos_iff_ne_zero.2 hc)).elim
...
theorem log_eq_zero {b o : Ordinal} (hbo : o < b) : log b o = 0 := by
  ...
  · rwa [← le_zero_iff_eq_zero, ← lt_succ_iff, succ_zero, ← lt_opow_iff_log_lt hb ho, opow_one]
