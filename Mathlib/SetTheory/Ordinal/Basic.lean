...
instance canonicallyOrderedAdd : CanonicallyOrderedAdd Ordinal where
  le_add_self a b := by simpa only [zero_add] using add_le_add_right (zero_le b) a
  le_self_add a b := by simpa only [add_zero] using add_le_add_left (zero_le b) a

@[deprecated le_self_add (since := "2025-11-21")]
protected theorem le_add_right (a b : Ordinal) : a ≤ a + b := by
  simpa only [add_zero] using add_le_add_left (zero_le b) a

@[deprecated le_add_self (since := "2025-11-21")]
protected theorem le_add_left (a b : Ordinal) : a ≤ b + a := by
  simpa only [zero_add] using add_le_add_right (zero_le b) a
