...
meta def _root_.Mathlib.Meta.Positivity.evalOmega0 : PositivityExt where
...
meta def _root_.Mathlib.Meta.Positivity.evalOPow : PositivityExt where
  eval {u α} zα pα e := do
    let some u' := u.dec | throwError "level is not succ"
    haveI' : u =QL u' + 1 := ⟨⟩
    match α, e with
  ...
  | oadd e n a hae he ha =>
    ...
    | oadd e' n' b hbe' =>
      ...
      rw [log_opow_mul_add one_lt_omega0 (by positivity) hae.repr_lt_repr,
        ...
        log_eq_zero (natCast_lt_omega0 _), log_eq_zero (natCast_lt_omega0 _),
...
lemma repr_lt_epsilon0 (o : NONote) : repr o < ε_ 0 := by
  ...
  | oadd e n a ha he ha₂ =>
    ...
    rw [lt_sub, ← nsmul_eq_mul, ← succ_nsmul, nsmul_eq_mul]
...
lemma range_repr : range repr = Iio (ε_ 0) := by
  ...
  case mpr =>
    ...
    | ind o hio =>
      ...
      case inl => exists 0
      rw [← not_le] at ho
      apply mt epsilon_zero_le_of_omega0_opow_le at ho
      rw [not_le] at ho
      replace hio : ∀ p ∈ CNF ω o, ∃ p' : NONote × ℕ+, p'.map repr (↑) = p := by
        rintro ⟨e, no⟩ hen
        obtain ⟨ne, rfl⟩ := hio _ ((CNF.fst_le_log hen).trans_lt (lt_log_of_lt_opow honn ho))
        obtain ⟨n, rfl⟩ := lt_omega0.mp (CNF.snd_lt one_lt_omega0 hen)
        lift n to PNat using by simpa only [Nat.cast_pos'] using CNF.snd_pos hen
        exists (ne, n)
