...
theorem exists_isFundamentalSeq (ha : o.cof.ord = a) : ∃ f : Iio a → Iio o, IsFundamentalSeq f := by
  subst ha
  obtain ⟨s, hs, hs'⟩ := exists_ord_cof_eq (Shrink (Iio o))
  rw [cof_shrink_Iio] at hs'
  let g := (OrderIso.setCongr _ _ (congrArg _ hs'.symm)).trans <|
    .ofRelIsoLT (enum (α := s) (· < ·))
  refine ⟨fun i ↦ (orderIsoShrink (Iio o)).symm (g i), le_rfl,
    fun _ _ h ↦ (orderIsoShrink (Iio o)).symm.lt_iff_lt.2
      (Subtype.coe_lt_coe.2 (g.lt_iff_lt.2 h)), ?_⟩
