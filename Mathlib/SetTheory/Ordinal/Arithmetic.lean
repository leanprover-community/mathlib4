...
@[simp]
lemma one_add_natCast (m : ℕ) : 1 + (m : Ordinal) = succ m := by
...
@[simp]
lemma one_add_ofNat (m : ℕ) [m.AtLeastTwo] :
    1 + (no_index (OfNat.ofNat m : Ordinal)) = Order.succ (OfNat.ofNat m : Ordinal) :=
  one_add_natCast m

@[simp, norm_cast]
lemma natCast_mul (m : ℕ) : ∀ n : ℕ, ((m * n : ℕ) : Ordinal) = m * n
  | 0 => by simp
...
@[simp, norm_cast]
lemma natCast_sub (m n : ℕ) : ((m - n : ℕ) : Ordinal) = m - n := by
...
@[simp, norm_cast]
lemma natCast_div (m n : ℕ) : ((m / n : ℕ) : Ordinal) = m / n := by
...
@[simp]
lemma sup_natCast : sup Nat.cast = ω :=
