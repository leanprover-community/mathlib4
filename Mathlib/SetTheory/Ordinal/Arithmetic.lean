...
theorem lt_limit {o} (h : IsLimit o) {a} : a < o ↔ ∃ x < o, a < x := by
  ...
  simpa only [not_forall_mem, not_le, bex_def] using not_congr (@limit_le _ h a)
...
theorem IsNormal.le_set {f o} (H : IsNormal f) (p : Set Ordinal) (p0 : p.Nonempty) (b)
  ...
  ⟨fun h a pa => (H.le_iff.2 ((H₂ _).1 le_rfl _ pa)).trans h, fun h => by
    ...
    | H₁ =>
      ...
      exact h _ px
    | H₂ S _ =>
      rcases not_forall_mem.1 (mt (H₂ S).2 <| (lt_succ S).not_le) with ⟨a, h₁, h₂⟩
      exact (H.le_iff.2 <| succ_le_of_lt <| not_le.1 h₂).trans (h _ h₁)
    | H₃ S L _ =>
      refine' (H.2 _ L _).2 fun a h' => _
      rcases not_forall_mem.1 (mt (H₂ a).2 h'.not_le) with ⟨b, h₁, h₂⟩
...
theorem lt_mul_of_limit {a b c : Ordinal} (h : IsLimit c) : a < b * c ↔ ∃ c' < c, a < b * c' := by
  -- Porting note: `bex_def` is required.
  simpa only [not_forall_mem, not_le, bex_def] using not_congr (@mul_le_of_limit b c a h)
