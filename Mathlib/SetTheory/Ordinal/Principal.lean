...
theorem principal_add_iff_add_left_eq_self {o : Ordinal} :
  ...
  · cases' lt_or_le 1 o with ho₁ ho₁
    · exact op_eq_self_of_principal hao (isNormal_add a) ho (principal_add_isLimit ho₁ ho)
    · rcases le_one_iff.1 ho₁ with (rfl | rfl)
      · exact (Ordinal.not_lt_zero a hao).elim
      · rw [lt_one_iff_zero] at hao
        rw [hao, zero_add]
  · rw [← h a hao]
    exact (isNormal_add a).strictMono hbo
...
theorem add_omega0_opow {a b : Ordinal} (h : a < ω ^ b) : a + ω ^ b = ω ^ b := by
  ...
  · rcases (lt_opow_of_limit omega0_ne_zero l).1 h with ⟨x, xb, ax⟩
    apply (((isNormal_add a).trans <| isNormal_opow one_lt_omega0).limit_le l).2
    intro y yb
...
theorem principal_mul_iff_mul_left_eq {o : Ordinal} :
  ...
  · cases' le_or_gt o 2 with ho ho
    ...
    · exact op_eq_self_of_principal hao (isNormal_mul ha₀) h (principal_mul_isLimit ho h)
  · rcases eq_or_ne a 0 with (rfl | ha)
    · dsimp only; rwa [zero_mul]
    rw [← Ordinal.pos_iff_ne_zero] at ha
    rw [← h a ha hao]
    exact (isNormal_mul ha).strictMono hbo
...
theorem mul_lt_omega0_opow {a b c : Ordinal} (c0 : 0 < c) (ha : a < ω ^ c) (hb : b < ω) :
  ...
  · rw [opow_succ] at ha
    rcases ((isNormal_mul <| opow_pos _ omega0_pos).limit_lt omega0_isLimit).1 ha with ⟨n, hn, an⟩
...
theorem mul_eq_opow_log_succ {a b : Ordinal.{u}} (ha : a ≠ 0) (hb : Principal (· * ·) b)
  ...
  · have hbl := principal_mul_isLimit hb₂ hb
    rw [← IsNormal.bsup_eq.{u, u} (isNormal_mul (Ordinal.pos_iff_ne_zero.2 ha)) hbl, bsup_le_iff]
    intro c hcb
