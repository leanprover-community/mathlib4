...
lemma principal_iff_principal_swap {op : Ordinal → Ordinal → Ordinal} {o : Ordinal} :
...
lemma principal_zero {op : Ordinal → Ordinal → Ordinal} : Principal op 0 := fun a _ h =>
  (Ordinal.not_lt_zero a h).elim
#align ordinal.principal_zero Ordinal.principal_zero

@[simp]
lemma principal_one_iff {op : Ordinal → Ordinal → Ordinal} : Principal op 1 ↔ op 0 0 = 0 := by
...
lemma Principal.iterate_lt {op : Ordinal → Ordinal → Ordinal} {a o : Ordinal} (hao : a < o)
...
lemma op_eq_self_of_principal {op : Ordinal → Ordinal → Ordinal} {a o : Ordinal.{u}} (hao : a < o)
...
lemma nfp_le_of_principal {op : Ordinal → Ordinal → Ordinal} {a o : Ordinal} (hao : a < o)
...
lemma principal_add_iff_add_left_eq_self {o : Ordinal} :
...
lemma exists_lt_add_of_not_principal_add {a} (ha : ¬Principal (· + ·) a) :
...
lemma principal_add_iff_add_lt_ne_self {a} :
...
lemma add_omega {a : Ordinal} (h : a < omega) : a + omega = omega := by
...
lemma principal_add_iff_zero_or_omega_opow {o : Ordinal} :
...
lemma opow_principal_add_of_principal_add {a} (ha : Principal (· + ·) a) (b : Ordinal) :
    Principal (· + ·) (a^b) := by
...
lemma add_absorp {a b c : Ordinal} (h₁ : a < (omega^b)) (h₂ : (omega^b) ≤ c) : a + c = c := by
...
lemma principal_mul_one : Principal (· * ·) 1 := by
  rw [principal_one_iff]
  exact zero_mul _
#align ordinal.principal_mul_one Ordinal.principal_mul_one

lemma principal_mul_two : Principal (· * ·) 2 := fun a b ha hb => by
...
lemma principal_mul_of_le_two {o : Ordinal} (ho : o ≤ 2) : Principal (· * ·) o := by
...
lemma principal_add_of_principal_mul {o : Ordinal} (ho : Principal (· * ·) o) (ho₂ : o ≠ 2) :
    Principal (· + ·) o := by
...
lemma principal_mul_isLimit {o : Ordinal.{u}} (ho₂ : 2 < o) (ho : Principal (· * ·) o) :
    o.IsLimit :=
...
lemma principal_mul_iff_mul_left_eq {o : Ordinal} :
...
lemma principal_mul_omega : Principal (· * ·) omega := fun a b ha hb =>
...
lemma mul_omega {a : Ordinal} (a0 : 0 < a) (ha : a < omega) : a * omega = omega :=
  principal_mul_iff_mul_left_eq.1 principal_mul_omega a a0 ha
#align ordinal.mul_omega Ordinal.mul_omega

lemma mul_lt_omega_opow {a b c : Ordinal} (c0 : 0 < c) (ha : a < (omega^c)) (hb : b < omega) :
    a * b < (omega^c) := by
...
lemma mul_omega_opow_opow {a b : Ordinal} (a0 : 0 < a) (h : a < (omega^omega^b)) :
...
lemma principal_add_of_principal_mul_opow {o b : Ordinal} (hb : 1 < b)
...
lemma principal_mul_iff_le_two_or_omega_opow_opow {o : Ordinal} :
...
lemma mul_omega_dvd {a : Ordinal} (a0 : 0 < a) (ha : a < omega) : ∀ {b}, omega ∣ b → a * b = b
  | _, ⟨b, rfl⟩ => by rw [← mul_assoc, mul_omega a0 ha]
#align ordinal.mul_omega_dvd Ordinal.mul_omega_dvd

lemma mul_eq_opow_log_succ {a b : Ordinal.{u}} (ha : a ≠ 0) (hb : Principal (· * ·) b)
...
lemma principal_opow_omega : Principal (·^·) omega := fun a b ha hb =>
  match a, b, lt_omega.1 ha, lt_omega.1 hb with
  | _, _, ⟨m, rfl⟩, ⟨n, rfl⟩ => by
...
lemma opow_omega {a : Ordinal} (a1 : 1 < a) (h : a < omega) : (a^omega) = omega :=
  le_antisymm
