...
theorem isPrincipal_add_omega0_opow (o : Ordinal) : IsPrincipal (· + ·) (ω ^ o) :=
  isPrincipal_add_iff_add_left_eq_self.2 fun _ => add_omega0_opow
...
theorem add_absorp_of_lt_omega0_opow_log {o o' : Ordinal} (hlt : o' < ω ^ log ω o) :
  ...
  · simpa using hlt
  exact add_absorp hlt <| opow_log_le_self ω h0
