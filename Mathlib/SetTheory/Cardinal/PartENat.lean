...
@[simp]
lemma partENatOfENat_toENat (c : Cardinal) : (toENat c : PartENat) = toPartENat c := rfl

@[simp]
lemma toPartENat_natCast (n : ℕ) : toPartENat n = n := by
...
lemma toPartENat_apply_of_lt_aleph0 {c : Cardinal} (h : c < ℵ₀) : toPartENat c = toNat c := by
  lift c to ℕ using h; simp
#align cardinal.to_part_enat_apply_of_lt_aleph_0 Cardinal.toPartENat_apply_of_lt_aleph0

lemma toPartENat_eq_top {c : Cardinal} :
    toPartENat c = ⊤ ↔ ℵ₀ ≤ c := by
...
lemma toPartENat_apply_of_aleph0_le {c : Cardinal} (h : ℵ₀ ≤ c) : toPartENat c = ⊤ :=
...
@[simp]
lemma mk_toPartENat_of_infinite [h : Infinite α] : toPartENat #α = ⊤ :=
...
lemma toPartENat_strictMonoOn : StrictMonoOn toPartENat (Set.Iic ℵ₀) :=
