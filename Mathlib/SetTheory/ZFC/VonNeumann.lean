...
lemma _root_.Ordinal.card_le_card_vonNeummann (o : Ordinal) : o.card ≤ card (V_ o) := by
  sorry
...
theorem card_vonNeumann (o : Ordinal.{u}) : card (V_ o) = preBeth o := by
  ...
  | limit o ho ih =>
    ...
    apply iSup_card_le_card_sUnion_range.antisymm'
    rw [← lift_le.{u + 1}]
    apply lift_card_sUnion_range_le_sum_card.trans
    ...
    · rw [Ordinal.mk_Iio_ordinal, lift_lift, lift_le]
      ...
      refine (⨆ i : Set.Iio o, (V_ ↑i).card).card_ord.not_lt <|
        (Ordinal.card_le_card_vonNeummann _).trans_lt <| (cantor _).trans_le ?_
