...
@[deprecated Sep.sep (since := "2026-09-05")]
protected def sep (p : ZFSet → Prop) (A : Class) : Class :=
  {y | A y ∧ p y}

@[ext, deprecated Set.ext (since := "2026-09-05")]
theorem ext {x y : Class.{u}} : (∀ z : ZFSet.{u}, x z ↔ y z) → x = y :=
  Set.ext

/-- Coerce a ZFC set into a class -/
@[coe, deprecated ZFClass.ofSet (since := "2026-09-05")]
def ofSet (x : ZFSet.{u}) : Class.{u} :=
  { y | y ∈ x }

@[deprecated inferInstance (since := "2026-09-05")]
instance : Coe ZFSet Class :=
  ⟨ofSet⟩

-- FIXME: `set_option linter.deprecatedCoercions false in` does not disable the warnings
...
@[simp, deprecated ZFClass.not_cmem_empty (since := "2026-09-05")]
theorem notMem_empty (x : Class.{u}) : x ∉ (∅ : Class.{u}) := fun ⟨_, _, h⟩ => h

@[simp, deprecated Set.notMem_empty (since := "2026-09-05")]
theorem not_empty_hom (x : ZFSet.{u}) : ¬(∅ : Class.{u}) x :=
  id

@[simp, deprecated ZFClass.cmem_univ (since := "2026-09-05")]
theorem mem_univ {A : Class.{u}} : A ∈ univ.{u} ↔ ∃ x : ZFSet.{u}, ↑x = A :=
  exists_congr fun _ => iff_of_eq (and_true _)

@[simp, deprecated Set.mem_univ (since := "2026-09-05")]
...
@[simp, deprecated ZFClass.congToClass_empty (since := "2026-09-05")]
...
@[simp, deprecated ZFClass.classToCong_empty (since := "2026-09-05")]
...
@[simp, norm_cast, deprecated ZFClass.ofSet_powerset (since := "2026-09-05")]
theorem coe_powerset (x : ZFSet.{u}) : ↑x.powerset = powerset.{u} x :=
  ext fun _ => ZFSet.mem_powerset

@[simp, deprecated ZFClass.mem_powerset (since := "2026-09-05")]
theorem powerset_apply {A : Class.{u}} {x : ZFSet.{u}} : powerset A x ↔ ↑x ⊆ A :=
  Iff.rfl

@[simp, deprecated ZFClass.mem_sUnion (since := "2026-09-05")]
...
open scoped ZFSet in
@[simp, norm_cast, deprecated ZFClass.ofSet_sUnion (since := "2026-09-05")]
theorem coe_sUnion (x : ZFSet.{u}) : ↑(⋃₀ x : ZFSet) = ⋃₀ (x : Class.{u}) :=
  ext fun y =>
    ZFSet.mem_sUnion.trans (sUnion_apply.trans <| by rfl).symm

@[simp, deprecated ZFClass.cmem_sUnion (since := "2026-09-05")]
...
open scoped ZFSet in
@[simp, norm_cast, deprecated ZFClass.ofSet_sInter (since := "2026-09-05")]
...
@[simp, deprecated ZFClass.sUnion_empty (since := "2026-09-05")]
theorem sUnion_empty : ⋃₀ (∅ : Class.{u}) = (∅ : Class.{u}) := by
  ext
  simp

@[simp, deprecated ZFClass.sInter_empty (since := "2026-09-05")]
