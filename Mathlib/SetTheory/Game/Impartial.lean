...
lemma impartialAux_def {G : PGame} :
    G.ImpartialAux ↔
...
lemma impartial_iff_aux {G : PGame} : G.Impartial ↔ G.ImpartialAux :=
  ⟨fun h => h.1, fun h => ⟨h⟩⟩
...
lemma impartial_congr : ∀ {G H : PGame} (_ : G ≡r H) [G.Impartial], H.Impartial
  | G, H => fun e => by
    intro h
...
lemma equiv_or_fuzzy_zero : (G ≈ 0) ∨ G ‖ 0 := by
...
@[simp]
lemma not_equiv_zero_iff : ¬(G ≈ 0) ↔ G ‖ 0 :=
...
@[simp]
lemma not_fuzzy_zero_iff : ¬G ‖ 0 ↔ (G ≈ 0) :=
...
-- Porting note: Changed `⟦G⟧` to `(⟦G⟧ : Quotient setoid)`
@[simp]
lemma mk'_add_self : (⟦G⟧ : Quotient setoid) + ⟦G⟧ = 0 :=
...
lemma le_zero_iff {G : PGame} [G.Impartial] : G ≤ 0 ↔ 0 ≤ G := by
...
lemma lf_zero_iff {G : PGame} [G.Impartial] : G ⧏ 0 ↔ 0 ⧏ G := by
...
lemma forall_rightMoves_fuzzy_iff_equiv_zero : (∀ j, G.moveRight j ‖ 0) ↔ (G ≈ 0) := by
  refine' ⟨fun hb => _, fun hp i => _⟩
...
lemma exists_left_move_equiv_iff_fuzzy_zero : (∃ i, G.moveLeft i ≈ 0) ↔ G ‖ 0 := by
...
lemma exists_right_move_equiv_iff_fuzzy_zero : (∃ j, G.moveRight j ≈ 0) ↔ G ‖ 0 := by
