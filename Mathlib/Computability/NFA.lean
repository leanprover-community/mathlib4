...
theorem acceptsFrom_iUnion {ι : Sort*} (s : ι → Set σ) :
  ...
  simp_rw [↑mem_iUnion, ↑mem_ofPred]; tauto
...
private theorem mem_acceptsFrom_sep_fact {S : Set σ} {p : Prop} {x : List α} :
  ...
  induction x generalizing S with
  | nil => simp only [nil_mem_acceptsFrom, mem_ofPred]; tauto
  | cons a x ih =>
    have h : M.stepSet {s ∈ S | p} a = {s ∈ M.stepSet S a | p} := by
      ext s; simp only [stepSet, mem_ofPred, mem_iUnion, exists_prop]; tauto
    simp [h, ih]
...
theorem disjoint_stepSet_reverse {a : α} {S S' : Set σ} :
  ...
  rw [← not_iff_not]
  simp only [Set.not_disjoint_iff, mem_stepSet, reverse_step, Set.mem_ofPred]
  tauto
