...
We restrict the nonterminal type of a context-free grammar to `Type` because universe polymorphism
would be cumbersome and unnecessary: one can always restrict attention to the finitely many
nonterminal symbols that actually appear in the grammar's finitely many rules.

## Main definitions
* `ContextFreeGrammar`: A context-free grammar.
* `ContextFreeGrammar.language`: the language generated by a given context-free grammar.
...
`r.Rewrites u v` means that `r` sends `u` to `v` (there may be multiple such strings `v`). -/
...
/-- Rule `r` rewrites string `u` to string `v` iff they share both a prefix `p` and a suffix `q`,
...
/-- Add an extra prefix to context-free rewriting. -/
...
/-- Add an extra suffix to context-free rewriting. -/
...
/-- The language (set of words) generated by a given context-free grammar `g`. -/
...
/-- Add an extra prefix to a context-free production. -/
...
/-- Add an extra suffix to a context-free derivation. -/
