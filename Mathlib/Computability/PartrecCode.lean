...
private theorem encode_ofNatCode : ∀ n, encodeCode (ofNatCode n) = n
  ...
  | n + 4 => by
    ...
    cases n.bodd <;> cases n.div2.bodd <;>
      simp +zetaDelta [encodeCode, ofNatCode, IH, IH1, IH2, Nat.bit_val]
...
theorem rec_prim' {α σ} [Primcodable α] [Primcodable σ] {c : α → Code} (hc : Primrec c) {z : α → σ}
  ...
  simp [G₁]; simp +zetaDelta [m, List.getElem?_map, List.getElem?_range, hm, m1, m2]
  ...
  have :
    ...
    Primrec.nat_strong_rec _ (hG.comp Primrec.snd).to₂ fun _ p => by
      ...
      · cases' encode_lt_comp cf cg with lf lg
        ...
        cases evaln k cg n
        · rfl
        simp +zetaDelta [hg (Nat.pair_lt_pair_right _ lf)]
      · cases' encode_lt_prec cf cg with lf lg
        ...
        cases evaln k' _ _
        · rfl
        simp +zetaDelta [hg (Nat.pair_lt_pair_right _ lg)]
  ...
  ⟨curry cg (encode cF),
    funext fun n =>
      show eval (f (curry cg (encode cF))) n = eval (curry cg (encode cF)) n by
        simp +zetaDelta [g, eg', eF', Part.map_id']⟩
