...
/-- Returns 1 if its two (unpaired) arguments are equal, 0 otherwise. -/
def kronecker : ℕ →. ℕ := fun p =>
  Part.some (if (Nat.unpair p).1 = (Nat.unpair p).2 then 1 else 0)

lemma kronecker_partrec : Nat.Partrec Partrec.kronecker :=
  Partrec.nat_iff.1 ((Computable.cond
    ((Primrec.beq.comp .fst .snd).to_comp.comp Computable.unpair)
    (.const 1) (.const 0)).partrec.of_eq fun p => by
      simp only [kronecker]
...
lemma kronecker_rfind_none :
    Nat.rfind (fun k => (fun m : ℕ => m = 0) <$>
        Part.map (1 - ·)
          (((Nat.pair <$> (Part.none : Part ℕ) <*> Part.some k) >>=
            Partrec.kronecker))) =
      Part.none :=
  Nat.rfind_zero_none (p := fun k => (fun m : ℕ => m = 0) <$>
    Part.map (1 - ·) (((Nat.pair <$> (Part.none : Part ℕ) <*>
      Part.some k) >>= kronecker))) (by simp [Seq.seq])
