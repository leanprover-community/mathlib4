...
theorem move_ok {p k₁ k₂ q s L₁ o L₂} {S : K' → List Γ'} (h₁ : k₁ ≠ k₂)
  ...
  · refine TransGen.head rfl _
    ...
    rw [e₃] at e
    cases e
    simp only [List.head_cons, e₂, List.tail_cons, ne_eq, cond_false]
...
theorem clear_ok {p k q s L₁ o L₂} {S : K' → List Γ'} (e : splitAtPred p (S k) = (L₁, o, L₂)) :
  ...
  · refine TransGen.head rfl _
    ...
    rw [e₃] at e
    cases e
    simp only [List.head_cons, e₂, List.tail_cons, cond_false]
...
theorem copy_ok (q s a b c d) :
  ...
  · refine TransGen.single _
    simp
  refine TransGen.head rfl _
  simp only [TM2.step, Option.mem_def, TM2.stepAux, elim_rev, List.head_cons, Option.isSome_some,
...
theorem pred_ok (q₁ q₂ s v) (c d : List Γ') : ∃ s',
  ...
  · simp [trPosNum, trNum, show Num.zero.succ' = PosNum.one from rfl]
    refine TransGen.head rfl _
    simp only [Option.mem_def, TM2.stepAux, elim_main, List.head_cons, Option.some.injEq,
