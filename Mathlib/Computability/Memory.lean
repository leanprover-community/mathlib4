...
This file contains the data structures used in Turing machine variants to
store data of arbitrary length. Throughout, we shall use `Γ` to denote
the alphabet of the data structure.

Primary definitions:

* `ListBlank` represents a half-tape.
* `Tape` represents a full tape, with a distinguished position under the head.
