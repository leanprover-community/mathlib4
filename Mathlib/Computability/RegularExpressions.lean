...
/-- The constructed NFA really corresponds to the original regular expression.
-/
theorem toNFA_spec (r : RegularExpression α) : r.toNFA.accepts = r.matches' := r.toNFAX.property
...
variable [DecidableEq α]
/--
Collecting the mutually-recursive instances in a product is easier than using the `mutual` feature.
-/
def decidableAux : (R : RegularExpression α) →
    (∀ p a, DecidablePred (R.toNFA.step p a)) ×
    DecidablePred R.toNFA.start ×
    DecidablePred R.toNFA.accept :=
  rec'
    (zeroDecidableStep, zeroDecidableStart, zeroDecidableAccept)
    (oneDecidableStep, oneDecidableStart, oneDecidableAccept)
    (fun a ↦ (charDecidableStep a, charDecidableStart a, charDecidableAccept a))
    (fun l r ⟨_, _, _⟩ ⟨_, _, _⟩ ↦ let L := l.toNFA; let R := r.toNFA
      (haddDecidableStep L R, haddDecidableStart L R, haddDecidableAccept L R))
    (fun l r ⟨_, _, _⟩ ⟨_, _, _⟩ ↦ let L := l.toNFA; let R := r.toNFA
      (hmulDecidableStep L R, hmulDecidableStart L R, hmulDecidableAccept L R))
    (fun i ⟨_, _, _⟩ ↦ let I := i.toNFA
      (kstarDecidableStep I, kstarDecidableStart I, kstarDecidableAccept I))
