...
This file contains the definition of a binary encoding, a map from a type to
strings of `Bool`s, used in defining computability by Turing machines.
-/
...
structure BinEncoding (α : Type u) where
...
theorem BinEncoding.encode_injective {α : Type u} (e : BinEncoding α) :
...
def encodingNatBool : BinEncoding ℕ where
  encode := encodeNat
...
def unaryBinEncodingNat : BinEncoding ℕ where
...
def binEncodingBool : BinEncoding Bool where
  encode := encodeBool
