...
lemma TuringReducible.partrec_of_const {s} (hf : f ≤ᵀ PFun.mk fun _ => s) : Partrec f :=
...
theorem partrec_iff_forall_turingReducible : Partrec f ↔ ∀ g, f ≤ᵀ g :=
  ⟨fun hf _ => hf.turingReducible, fun hf => hf (PFun.mk fun _ => Part.none) |>.partrec_of_const⟩
