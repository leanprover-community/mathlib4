...
theorem char_toNFA_correct {a : α} : (char a).matches' = (char a).toNFA.accepts :=
  ...
  · rintro ⟨⟨⟩⟩
    refine' ⟨tt, rfl, _⟩
    unfold NFA.eval NFA.evalFrom List.foldl
    rw [NFA.mem_stepSet]
    repeat' fconstructor
  · rintro ⟨q, accept, eval⟩
    cases q
...
theorem comp_toNFA_eval₁ {r₁ r₂ : RegularExpression α} {x : List α} (q : r₁.StateM) :
...
theorem comp_toNFA_eval₂ {r₁ r₂ : RegularExpression α} {x y : List α} (q : r₂.StateM)
      ...
      suffices
        ∀ x q,
          Sum.inr q ∈ (r₁.comp r₂).toNFA.eval x →
            ∃ y z, y ∈ r₁.to_NFA.accepts ∧ q ∈ r₂.to_NFA.eval z ∧ y ++ z = x
        by
