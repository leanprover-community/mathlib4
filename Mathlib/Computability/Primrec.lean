...
lemma casesOn1 {f} (m : ℕ) (hf : Nat.Primrec f) : Nat.Primrec (Nat.casesOn · m f) :=
...
lemma casesOn' {f g} (hf : Nat.Primrec f) (hg : Nat.Primrec g) :
...
lemma sub : Nat.Primrec (unpaired (· - ·)) :=
...
lemma mul : Nat.Primrec (unpaired (· * ·)) :=
...
lemma pow : Nat.Primrec (unpaired (· ^ ·)) :=
...
lemma nat_iff {f : ℕ → ℕ} : Primrec f ↔ Nat.Primrec f :=
  dom_denumerable
#align primrec.nat_iff Primrec.nat_iff

lemma encdec : Primrec fun n => encode (@decode α _ n) :=
  nat_iff.2 Primcodable.prim
#align primrec.encdec Primrec.encdec

lemma option_some : Primrec (@some α) :=
...
lemma of_eq {f g : α → σ} (hf : Primrec f) (H : ∀ n, f n = g n) : Primrec g :=
  (funext H : f = g) ▸ hf
...
protected lemma id : Primrec (@id α) :=
  (@Primcodable.prim α).of_eq <| by simp
#align primrec.id Primrec.id

lemma comp {f : β → σ} {g : α → β} (hf : Primrec f) (hg : Primrec g) : Primrec fun a => f (g a) :=
...
lemma of_equiv_iff {β} (e : β ≃ α) {f : σ → β} :
    haveI := Primcodable.ofEquiv α e
...
lemma of_equiv_symm_iff {β} (e : β ≃ α) {f : σ → α} :
    haveI := Primcodable.ofEquiv α e
...
lemma fst {α β} [Primcodable α] [Primcodable β] : Primrec (@Prod.fst α β) :=
  ((casesOn' zero
...
lemma snd {α β} [Primcodable α] [Primcodable β] : Primrec (@Prod.snd α β) :=
  ((casesOn' zero
...
lemma pair {α β γ} [Primcodable α] [Primcodable β] [Primcodable γ] {f : α → β} {g : α → γ}
...
lemma of_eq {f g : α → β → σ} (hg : Primrec₂ f) (H : ∀ a b, f a b = g a b) : Primrec₂ g :=
...
protected lemma pair : Primrec₂ (@Prod.mk α β) :=
  .pair .fst .snd
#align primrec₂.pair Primrec₂.pair

lemma left : Primrec₂ fun (a : α) (_ : β) => a :=
  .fst
#align primrec₂.left Primrec₂.left

lemma right : Primrec₂ fun (_ : α) (b : β) => b :=
  .snd
#align primrec₂.right Primrec₂.right

lemma natPair : Primrec₂ Nat.pair := by simp [Primrec₂, Primrec]; constructor
#align primrec₂.mkpair Primrec₂.natPair

lemma unpaired {f : ℕ → ℕ → α} : Primrec (Nat.unpaired f) ↔ Primrec₂ f :=
  ⟨fun h => by simpa using h.comp natPair, fun h => h.comp Primrec.unpair⟩
#align primrec₂.unpaired Primrec₂.unpaired

lemma unpaired' {f : ℕ → ℕ → ℕ} : Nat.Primrec (Nat.unpaired f) ↔ Primrec₂ f :=
  Primrec.nat_iff.symm.trans unpaired
#align primrec₂.unpaired' Primrec₂.unpaired'

lemma encode_iff {f : α → β → σ} : (Primrec₂ fun a b => encode (f a b)) ↔ Primrec₂ f :=
  Primrec.encode_iff
#align primrec₂.encode_iff Primrec₂.encode_iff

lemma option_some_iff {f : α → β → σ} : (Primrec₂ fun a b => some (f a b)) ↔ Primrec₂ f :=
  Primrec.option_some_iff
#align primrec₂.option_some_iff Primrec₂.option_some_iff

lemma ofNat_iff {α β σ} [Denumerable α] [Denumerable β] [Primcodable σ] {f : α → β → σ} :
    Primrec₂ f ↔ Primrec₂ fun m n : ℕ => f (ofNat α m) (ofNat β n) :=
  (Primrec.ofNat_iff.trans <| by simp).trans unpaired
#align primrec₂.of_nat_iff Primrec₂.ofNat_iff

lemma uncurry {f : α → β → σ} : Primrec (Function.uncurry f) ↔ Primrec₂ f := by
  rw [show Function.uncurry f = fun p : α × β => f p.1 p.2 from funext fun ⟨a, b⟩ => rfl]; rfl
#align primrec₂.uncurry Primrec₂.uncurry

lemma curry {f : α × β → σ} : Primrec₂ (Function.curry f) ↔ Primrec f := by
...
lemma Primrec.comp₂ {f : γ → σ} {g : α → β → γ} (hf : Primrec f) (hg : Primrec₂ g) :
    Primrec₂ fun a b => f (g a b) :=
  hf.comp hg
#align primrec.comp₂ Primrec.comp₂

lemma Primrec₂.comp {f : β → γ → σ} {g : α → β} {h : α → γ} (hf : Primrec₂ f) (hg : Primrec g)
    (hh : Primrec h) : Primrec fun a => f (g a) (h a) :=
  Primrec.comp hf (hg.pair hh)
#align primrec₂.comp Primrec₂.comp

lemma Primrec₂.comp₂ {f : γ → δ → σ} {g : α → β → γ} {h : α → β → δ} (hf : Primrec₂ f)
    (hg : Primrec₂ g) (hh : Primrec₂ h) : Primrec₂ fun a b => f (g a b) (h a b) :=
  hf.comp hg hh
#align primrec₂.comp₂ Primrec₂.comp₂

lemma PrimrecPred.comp {p : β → Prop} [DecidablePred p] {f : α → β} :
    PrimrecPred p → Primrec f → PrimrecPred fun a => p (f a) :=
  Primrec.comp
#align primrec_pred.comp PrimrecPred.comp

lemma PrimrecRel.comp {R : β → γ → Prop} [∀ a b, Decidable (R a b)] {f : α → β} {g : α → γ} :
    PrimrecRel R → Primrec f → Primrec g → PrimrecPred fun a => R (f a) (g a) :=
  Primrec₂.comp
#align primrec_rel.comp PrimrecRel.comp

lemma PrimrecRel.comp₂ {R : γ → δ → Prop} [∀ a b, Decidable (R a b)] {f : α → β → γ}
    {g : α → β → δ} :
...
lemma PrimrecPred.of_eq {α} [Primcodable α] {p q : α → Prop} [DecidablePred p] [DecidablePred q]
...
lemma PrimrecRel.of_eq {α β} [Primcodable α] [Primcodable β] {r s : α → β → Prop}
...
lemma swap {f : α → β → σ} (h : Primrec₂ f) : Primrec₂ (swap f) :=
  h.comp₂ Primrec₂.right Primrec₂.left
#align primrec₂.swap Primrec₂.swap

lemma nat_iff {f : α → β → σ} : Primrec₂ f ↔ Nat.Primrec
...
lemma to₂ {f : α × β → σ} (hf : Primrec f) : Primrec₂ fun a b => f (a, b) :=
  hf.of_eq fun _ => rfl
#align primrec.to₂ Primrec.to₂

lemma nat_rec {f : α → β} {g : α → ℕ × β → β} (hf : Primrec f) (hg : Primrec₂ g) :
...
lemma nat_rec₁ {f : ℕ → α → α} (a : α) (hf : Primrec₂ f) : Primrec (Nat.rec a f) :=
  nat_rec' .id (const a) <| comp₂ hf Primrec₂.right
#align primrec.nat_elim₁ Primrec.nat_rec₁

lemma nat_casesOn' {f : α → β} {g : α → ℕ → β} (hf : Primrec f) (hg : Primrec₂ g) :
...
lemma nat_casesOn {f : α → ℕ} {g : α → β} {h : α → ℕ → β} (hf : Primrec f) (hg : Primrec g)
    (hh : Primrec₂ h) : Primrec fun a => ((f a).casesOn (g a) (h a) : β) :=
  (nat_casesOn' hg hh).comp .id hf
#align primrec.nat_cases Primrec.nat_casesOn

lemma nat_casesOn₁ {f : ℕ → α} (a : α) (hf : Primrec f) :
    Primrec (fun (n : ℕ) => (n.casesOn a f : α)) :=
  nat_casesOn .id (const a) (comp₂ hf .right)
#align primrec.nat_cases₁ Primrec.nat_casesOn₁

lemma nat_iterate {f : α → ℕ} {g : α → β} {h : α → β → β} (hf : Primrec f) (hg : Primrec g)
...
lemma option_casesOn {o : α → Option β} {f : α → σ} {g : α → β → σ} (ho : Primrec o)
...
lemma option_bind {f : α → Option β} {g : α → β → Option σ} (hf : Primrec f) (hg : Primrec₂ g) :
...
lemma option_bind₁ {f : α → Option σ} (hf : Primrec f) : Primrec fun o => Option.bind o f :=
  option_bind .id (hf.comp snd).to₂
#align primrec.option_bind₁ Primrec.option_bind₁

lemma option_map {f : α → Option β} {g : α → β → σ} (hf : Primrec f) (hg : Primrec₂ g) :
...
lemma bind_decode_iff {f : α → β → Option σ} :
...
lemma ite {c : α → Prop} [DecidablePred c] {f : α → σ} {g : α → σ} (hc : PrimrecPred c)
...
lemma nat_le : PrimrecRel ((· ≤ ·) : ℕ → ℕ → Prop) :=
...
lemma nat_min : Primrec₂ (@min ℕ _) :=
  ite nat_le fst snd
#align primrec.nat_min Primrec.nat_min

lemma nat_max : Primrec₂ (@max ℕ _) :=
...
protected lemma not : Primrec not :=
  dom_bool _
#align primrec.bnot Primrec.not

protected lemma and : Primrec₂ and :=
  dom_bool₂ _
#align primrec.band Primrec.and

protected lemma or : Primrec₂ or :=
  dom_bool₂ _
#align primrec.bor Primrec.or

lemma _root_.PrimrecPred.not {p : α → Prop} [DecidablePred p] (hp : PrimrecPred p) :
    PrimrecPred fun a => ¬p a :=
  (Primrec.not.comp hp).of_eq fun n => by simp
#align primrec.not PrimrecPred.not

lemma _root_.PrimrecPred.and {p q : α → Prop} [DecidablePred p] [DecidablePred q]
...
lemma _root_.PrimrecPred.or {p q : α → Prop} [DecidablePred p] [DecidablePred q]
    (hp : PrimrecPred p) (hq : PrimrecPred q) : PrimrecPred fun a => p a ∨ q a :=
...
protected lemma beq [DecidableEq α] : Primrec₂ (@BEq.beq α _) :=
...
protected lemma eq [DecidableEq α] : PrimrecRel (@Eq α) := Primrec.beq
#align primrec.eq Primrec.eq

lemma nat_lt : PrimrecRel ((· < ·) : ℕ → ℕ → Prop) :=
  (nat_le.comp snd fst).not.of_eq fun p => by simp
#align primrec.nat_lt Primrec.nat_lt

lemma option_guard {p : α → β → Prop} [∀ a b, Decidable (p a b)] (hp : PrimrecRel p) {f : α → β}
...
lemma option_orElse : Primrec₂ ((· <|> ·) : Option α → Option α → Option α) :=
...
lemma list_indexOf₁ [DecidableEq α] (l : List α) : Primrec fun a => l.indexOf a :=
  list_findIdx₁ .beq l
#align primrec.list_index_of₁ Primrec.list_indexOf₁

lemma dom_fintype [Fintype α] (f : α → σ) : Primrec f :=
...
lemma nat_findGreatest {f : α → ℕ} {p : α → ℕ → Prop} [∀ x n, Decidable (p x n)]
...
lemma of_graph {f : α → ℕ} (h₁ : PrimrecBounded f)
...
lemma nat_div : Primrec₂ ((· / ·) : ℕ → ℕ → ℕ) := by
...
lemma nat_bodd : Primrec Nat.bodd :=
...
lemma nat_div2 : Primrec Nat.div2 :=
...
private lemma list_casesOn' {f : α → List β} {g : α → σ} {h : α → β × List β → σ}
...
private lemma list_foldl' {f : α → List β} {g : α → σ} {h : α → σ × β → σ}
    (hf : haveI := prim H; Primrec f) (hg : Primrec g) (hh : haveI := prim H; Primrec₂ h) :
...
private lemma list_cons' : (haveI := prim H; Primrec₂ (@List.cons β)) :=
  letI := prim H
  encode_iff.1 (succ.comp <| Primrec₂.natPair.comp (encode_iff.2 fst) (encode_iff.2 snd))

private lemma list_reverse' :
    haveI := prim H
...
lemma list_cons : Primrec₂ (@List.cons α) :=
  list_cons' Primcodable.prim
#align primrec.list_cons Primrec.list_cons

lemma list_casesOn {f : α → List β} {g : α → σ} {h : α → β × List β → σ} :
    Primrec f →
      Primrec g →
...
lemma list_foldl {f : α → List β} {g : α → σ} {h : α → σ × β → σ} :
    Primrec f →
...
lemma list_reverse : Primrec (@List.reverse α) :=
  list_reverse' Primcodable.prim
#align primrec.list_reverse Primrec.list_reverse

lemma list_foldr {f : α → List β} {g : α → σ} {h : α → β × σ → σ} (hf : Primrec f)
...
lemma list_get? : Primrec₂ (@List.get? α) :=
  let F (l : List α) (n : ℕ) :=
    l.foldl
      (fun (s : Sum ℕ α) (a : α) =>
...
lemma list_range : Primrec List.range :=
...
lemma list_join : Primrec (@List.join α) :=
...
lemma list_length : Primrec (@List.length α) :=
...
lemma list_findIdx {f : α → List β} {p : α → β → Bool}
...
lemma list_indexOf [DecidableEq α] : Primrec₂ (@List.indexOf α _) :=
...
lemma mem_range_encode : PrimrecPred (fun n => n ∈ Set.range (encode : α → ℕ)) :=
...
lemma subtype_val {p : α → Prop} [DecidablePred p] {hp : PrimrecPred p} :
...
lemma subtype_val_iff {p : β → Prop} [DecidablePred p] {hp : PrimrecPred p} {f : α → Subtype p} :
    haveI := Primcodable.subtype hp
...
lemma subtype_mk {p : β → Prop} [DecidablePred p] {hp : PrimrecPred p} {f : α → β}
...
lemma option_get {f : α → Option β} {h : ∀ a, (f a).isSome} :
...
lemma fin_val {n} : Primrec (fun (i : Fin n) => (i : ℕ)) :=
  fin_val_iff.2 .id
#align primrec.fin_val Primrec.fin_val

lemma fin_succ {n} : Primrec (@Fin.succ n) :=
  fin_val_iff.1 <| by simp [succ.comp fin_val]
#align primrec.fin_succ Primrec.fin_succ

lemma vector_toList {n} : Primrec (@Vector.toList α n) :=
  subtype_val
#align primrec.vector_to_list Primrec.vector_toList

lemma vector_toList_iff {n} {f : α → Vector β n} : (Primrec fun a => (f a).toList) ↔ Primrec f :=
  subtype_val_iff
#align primrec.vector_to_list_iff Primrec.vector_toList_iff

lemma vector_cons {n} : Primrec₂ (@Vector.cons α n) :=
...
lemma vector_get {n} : Primrec₂ (@Vector.get α n) :=
  option_some_iff.1 <|
...
lemma vector_ofFn {n} {f : Fin n → α → σ} (hf : ∀ i, Primrec (f i)) :
...
lemma vector_ofFn' {n} : Primrec (@Vector.ofFn α n) :=
  of_equiv
#align primrec.vector_of_fn' Primrec.vector_ofFn'

lemma fin_app {n} : Primrec₂ (@id (Fin n → σ)) :=
  (vector_get.comp (vector_ofFn'.comp fst) snd).of_eq fun ⟨v, i⟩ => by simp
#align primrec.fin_app Primrec.fin_app

lemma fin_curry₁ {n} {f : Fin n → α → σ} : Primrec₂ f ↔ ∀ i, Primrec (f i) :=
...
lemma fin_curry {n} {f : α → Fin n → σ} : Primrec f ↔ Primrec₂ f :=
...
lemma to_prim {n f} (pf : @Nat.Primrec' n f) : Primrec f := by
  induction pf
...
lemma of_eq {n} {f g : Vector ℕ n → ℕ} (hf : Primrec' f) (H : ∀ i, f i = g i) : Primrec' g :=
  (funext H : f = g) ▸ hf
#align nat.primrec'.of_eq Nat.Primrec'.of_eq

lemma const {n} : ∀ m, @Primrec' n fun _ => m
  | 0 => zero.comp Fin.elim0 fun i => i.elim0
  | m + 1 => succ.comp _ fun _ => const m
#align nat.primrec'.const Nat.Primrec'.const

lemma head {n : ℕ} : @Primrec' n.succ head :=
  (get 0).of_eq fun v => by simp [get_zero]
#align nat.primrec'.head Nat.Primrec'.head

lemma tail {n f} (hf : @Primrec' n f) : @Primrec' n.succ fun v => f v.tail :=
...
protected lemma cons {n m f g} (hf : @Primrec' n f) (hg : @Vec n m g) :
...
lemma comp' {n m f g} (hf : @Primrec' m f) (hg : @Vec n m g) : Primrec' fun v => f (g v) :=
...
lemma prec' {n f g h} (hf : @Primrec' n f) (hg : @Primrec' n g) (hh : @Primrec' (n + 2) h) :
...
lemma mul : @Primrec' 2 fun v => v.head * v.tail.head :=
  (prec (const 0) (tail (add.comp₂ _ (tail head) head))).of_eq fun v => by
    simp; induction v.head <;> simp [*, Nat.succ_mul]; rw [add_comm]
#align nat.primrec'.mul Nat.Primrec'.mul

lemma if_lt {n a b f g} (ha : @Primrec' n a) (hb : @Primrec' n b) (hf : @Primrec' n f)
...
lemma natPair : @Primrec' 2 fun v => v.head.pair v.tail.head :=
...
protected lemma encode : ∀ {n}, @Primrec' n encode
...
lemma sqrt : @Primrec' 1 fun v => v.head.sqrt := by
...
lemma unpair₁ {n f} (hf : @Primrec' n f) : @Primrec' n fun v => (f v).unpair.1 := by
...
lemma unpair₂ {n f} (hf : @Primrec' n f) : @Primrec' n fun v => (f v).unpair.2 := by
  have s := sqrt.comp₁ _ hf
  have fss := sub.comp₂ _ hf (mul.comp₂ _ s s)
...
lemma of_prim {n f} : Primrec f → @Primrec' n f :=
...
lemma prim_iff {n f} : @Primrec' n f ↔ Primrec f :=
  ⟨to_prim, of_prim⟩
#align nat.primrec'.prim_iff Nat.Primrec'.prim_iff

lemma prim_iff₁ {f : ℕ → ℕ} : (@Primrec' 1 fun v => f v.head) ↔ Primrec f :=
  prim_iff.trans
    ⟨fun h => (h.comp <| .vector_ofFn fun _ => .id).of_eq fun v => by simp, fun h =>
      h.comp .vector_head⟩
#align nat.primrec'.prim_iff₁ Nat.Primrec'.prim_iff₁

lemma prim_iff₂ {f : ℕ → ℕ → ℕ} : (@Primrec' 2 fun v => f v.head v.tail.head) ↔ Primrec₂ f :=
  prim_iff.trans
...
lemma vec_iff {m n f} : @Vec m n f ↔ Primrec f :=
...
lemma Primrec.nat_sqrt : Primrec Nat.sqrt :=
