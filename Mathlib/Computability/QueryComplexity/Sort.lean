...
lemma merge_eq (o : SOracle α) : (s t : List α) →
  ...
  | [], _ => by simp only [value', merge, value_pure, List.merge]
  | (_ :: _), [] => by simp only [value', merge, value_pure, List.merge]
  | (a :: l), (b :: r) => by
...
lemma sorts_mergeSort : Sorts (mergeSort (α := α)) := by
  intro o s t v pt
  simp only [value', mergeSort_eq _ ,  ne_eq, ite_eq_right_iff,
