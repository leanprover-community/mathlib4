...
@[simp]
lemma value_pure' (x : α) (o : I → Oracle ι ω) : (pure' x : Comp ι ω s α).value o = x := by
...
@[simp]
lemma query'_bind (o : I) (m : o ∈ s) (y : ι) (f : ω y → Comp ι ω s α)
    (g : α → Comp ι ω s β) : query' o m y f >>= g = .query' o m y fun b => f b >>= g := rfl

/-- `pure` has cost 0 -/
@[simp]
lemma cost_pure (x : α) (o : I → Oracle ι ω) (i : I) : (pure x : Comp ι ω s α).cost o i = 0 := by
  simp only [cost, run]

/-- `pure` has cost' 0 -/
@[simp]
lemma cost'_pure (x : α) (o : Oracle ι ω) (i : I) : (pure x : Comp ι ω s α).cost' o i = 0 := by
...
@[simp]
lemma cost_query' {i : I} (m : i ∈ s) (y : ι) (f : ω y → Comp ι ω s α) (o : I → Oracle ι ω)
    (j : I) : (query' i m y f).cost o j = (if j = i then 1 else 0) + (f (o i y)).cost o j := by
  simp [cost, run]
  exact Nat.add_comm (((f (o i y)).run o).snd j) _

@[simp]
lemma cost_query (i : I) (y : ι) (o : I → Oracle ι ω) :
...
lemma run_query {i : I} (m : i ∈ s) (y : ι) (f : ω y → Comp ι ω s α)
    (o : I → Oracle ι ω) : (query' i m y f).run o =
      let x := (o i) y
      let (z,c) := (f x).run o
...
@[simp]
lemma value_query' (i : I) (m : i ∈ s) (y : ι) (f : ω y → Comp ι ω s α) (o : I → Oracle ι ω) :
    (query' i m y f).value o = (f (o i y)).value o := by
  simp only [value, run_query]

@[simp]
lemma value_query (i : I) (y : ι) (o : I → Oracle ι ω) :
...
@[simp]
lemma value_bind (f : Comp ι ω s α) (g : α → Comp ι ω s β) (o : I → Oracle ι ω) :
...
@[simp]
lemma value_allow (f : Comp ι ω s α) (st : s ⊆ t) (o : I → Oracle ι ω) :
...
@[simp]
lemma value_allow_all (f : Comp ι ω s α) (o : I → Oracle ι ω) :
    f.allow_all.value o = f.value o := by
  apply value_allow

@[simp]
lemma cost_allow (f : Comp ι ω s α) (st : s ⊆ t) (o : I → Oracle ι ω) (i : I) :
...
@[simp]
lemma cost_allow_all (f : Comp ι ω s α) (o : I → Oracle ι ω) (i : I) :
...
@[simp]
lemma allow_all_pure (x : α) : (pure x : Comp ι ω s α).allow_all = pure x := by
...
@[simp]
lemma value_map (f : α → β) (g : Comp ι ω s α) (o : I → Oracle ι ω) :
    (f <$> g).value o = f (g.value o) := by
  simp only [map_eq, value_bind, value_pure]

@[simp]
lemma cost_map (f : α → β) (g : Comp ι ω s α) (o : I → Oracle ι ω) (i : I) :
