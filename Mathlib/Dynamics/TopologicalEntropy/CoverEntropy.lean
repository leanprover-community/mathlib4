...
of a point in `s`. -/
def IsDynCoverOf (T : X → X) (F : Set X) (U : Rel X X) (n : ℕ) (s : Set X) : Prop :=
  F ⊆ ⋃ x ∈ s, ball x (dynEntourage T U n)

lemma IsDynCoverOf.of_le {T : X → X} {F : Set X} {U : Rel X X} {m n : ℕ} (m_n : m ≤ n)
...
lemma IsDynCoverOf.of_entourage_subset {T : X → X} {F : Set X} {U V : Rel X X} (U_V : U ⊆ V)
...
@[simp]
lemma isDynCoverOf_empty {T : X → X} {U : Rel X X} {n : ℕ} {s : Set X} :
    IsDynCoverOf T ∅ U n s := by
  simp only [IsDynCoverOf, empty_subset]

lemma IsDynCoverOf.nonempty {T : X → X} {F : Set X} (h : F.Nonempty) {U : Rel X X} {n : ℕ}
    {s : Set X} (h' : IsDynCoverOf T F U n s) :
    s.Nonempty := by
  obtain ⟨x, x_s, _⟩ := nonempty_biUnion.1 (Nonempty.mono h' h)
  exact nonempty_of_mem x_s

lemma isDynCoverOf_zero (T : X → X) (F : Set X) (U : Rel X X) {s : Set X} (h : s.Nonempty) :
...
lemma IsDynCoverOf.nonempty_inter {T : X → X} {F : Set X} {U : Rel X X} {n : ℕ} {s : Finset X}
...
lemma IsDynCoverOf.iterate_le_pow {T : X → X} {F : Set X} (F_inv : MapsTo T F F) {U : Rel X X}
...
lemma exists_isDynCoverOf_of_isCompact_uniformContinuous [UniformSpace X] {T : X → X} {F : Set X}
    (F_comp : IsCompact F) (h : UniformContinuous T) {U : Rel X X} (U_uni : U ∈ 𝓤 X) (n : ℕ) :
...
lemma exists_isDynCoverOf_of_isCompact_invariant [UniformSpace X] {T : X → X} {F : Set X}
    (F_comp : IsCompact F) (F_inv : MapsTo T F F) {U : Rel X X} (U_uni : U ∈ 𝓤 X) (n : ℕ) :
    ∃ s : Finset X, IsDynCoverOf T F U n s := by
...
noncomputable def coverMincard (T : X → X) (F : Set X) (U : Rel X X) (n : ℕ) : ℕ∞ :=
  ⨅ (s : Finset X) (_ : IsDynCoverOf T F U n s), (s.card : ℕ∞)

lemma IsDynCoverOf.coverMincard_le_card {T : X → X} {F : Set X} {U : Rel X X} {n : ℕ}
    {s : Finset X} (h : IsDynCoverOf T F U n s) :
    coverMincard T F U n ≤ s.card :=
  iInf₂_le s h

lemma coverMincard_monotone_time (T : X → X) (F : Set X) (U : Rel X X) :
...
lemma coverMincard_antitone (T : X → X) (F : Set X) (n : ℕ) :
    Antitone fun U : Rel X X ↦ coverMincard T F U n :=
  fun _ _ U_V ↦ biInf_mono fun _ h ↦ h.of_entourage_subset U_V

lemma coverMincard_finite_iff (T : X → X) (F : Set X) (U : Rel X X) (n : ℕ) :
...
@[simp]
lemma coverMincard_empty {T : X → X} {U : Rel X X} {n : ℕ} : coverMincard T ∅ U n = 0 :=
  (sInf_le (by simp [IsDynCoverOf])).antisymm (zero_le (coverMincard T ∅ U n))

lemma coverMincard_eq_zero_iff (T : X → X) (F : Set X) (U : Rel X X) (n : ℕ) :
...
lemma one_le_coverMincard_iff (T : X → X) (F : Set X) (U : Rel X X) (n : ℕ) :
...
lemma coverMincard_zero (T : X → X) {F : Set X} (h : F.Nonempty) (U : Rel X X) :
...
lemma coverMincard_mul_le_pow {T : X → X} {F : Set X} (F_inv : MapsTo T F F) {U : Rel X X}
    [U.IsSymm] (m n : ℕ) :
...
lemma coverMincard_le_pow {T : X → X} {F : Set X} (F_inv : MapsTo T F F) {U : Rel X X}
...
lemma coverMincard_finite_of_isCompact_uniformContinuous [UniformSpace X] {T : X → X}
    {F : Set X} (F_comp : IsCompact F) (h : UniformContinuous T) {U : Rel X X} (U_uni : U ∈ 𝓤 X)
    (n : ℕ) :
...
lemma nonempty_inter_of_coverMincard {T : X → X} {F : Set X} {U : Rel X X} {n : ℕ}
...
lemma coverEntropyEntourage_antitone (T : X → X) (F : Set X) :
    Antitone fun U : Rel X X ↦ coverEntropyEntourage T F U :=
  fun _ _ U_V ↦ expGrowthSup_monotone fun n ↦ ENat.toENNReal_mono (coverMincard_antitone T F n U_V)

lemma coverEntropyInfEntourage_le_coverEntropyEntourage (T : X → X) (F : Set X) (U : Rel X X) :
    coverEntropyInfEntourage T F U ≤ coverEntropyEntourage T F U :=
  expGrowthInf_le_expGrowthSup

@[simp]
lemma coverEntropyEntourage_empty {T : X → X} {U : Rel X X} :
...
@[simp]
lemma coverEntropyInfEntourage_empty {T : X → X} {U : Rel X X} :
...
lemma coverEntropyInfEntourage_nonneg (T : X → X) {F : Set X} (h : F.Nonempty) (U : Rel X X) :
...
lemma coverEntropyEntourage_nonneg (T : X → X) {F : Set X} (h : F.Nonempty) (U : Rel X X) :
...
lemma coverEntropyEntourage_le_log_coverMincard_div {T : X → X} {F : Set X} (F_inv : MapsTo T F F)
    {U : Rel X X} [U.IsSymm] {n : ℕ} (n_pos : n ≠ 0) :
...
lemma IsDynCoverOf.coverEntropyEntourage_le_log_card_div {T : X → X} {F : Set X}
    (F_inv : MapsTo T F F) {U : Rel X X} [U.IsSymm] {n : ℕ} (n_pos : n ≠ 0)
    {s : Finset X} (h : IsDynCoverOf T F U n s) :
...
lemma coverEntropyEntourage_le_coverEntropyInfEntourage {T : X → X} {F : Set X}
    (F_inv : MapsTo T F F) {U : Rel X X} [U.IsSymm] :
...
lemma coverEntropyEntourage_finite_of_isCompact_invariant [UniformSpace X] {T : X → X} {F : Set X}
    (F_comp : IsCompact F) (F_inv : MapsTo T F F) {U : Rel X X} (U_uni : U ∈ 𝓤 X) :
...
lemma coverEntropyEntourage_le_coverEntropy (T : X → X) (F : Set X) {U : Rel X X}
    (h : U ∈ 𝓤 X) :
    coverEntropyEntourage T F U ≤ coverEntropy T F :=
  le_iSup₂ (f := fun (U : Rel X X) (_ : U ∈ 𝓤 X) ↦ coverEntropyEntourage T F U) U h

lemma coverEntropyInfEntourage_le_coverEntropyInf (T : X → X) (F : Set X) {U : Rel X X}
    (h : U ∈ 𝓤 X) :
    coverEntropyInfEntourage T F U ≤ coverEntropyInf T F :=
  le_iSup₂ (f := fun (U : Rel X X) (_ : U ∈ 𝓤 X) ↦ coverEntropyInfEntourage T F U) U h

lemma coverEntropy_eq_iSup_basis {ι : Sort*} {p : ι → Prop} {s : ι → Rel X X}
...
lemma coverEntropyInf_eq_iSup_basis {ι : Sort*} {p : ι → Prop} {s : ι → Rel X X}
    (h : (𝓤 X).HasBasis p s) (T : X → X) (F : Set X) :
...
lemma coverEntropyInf_le_coverEntropy (T : X → X) (F : Set X) :
  ...
  iSup₂_mono fun (U : Rel X X) (_ : U ∈ 𝓤 X) ↦
    coverEntropyInfEntourage_le_coverEntropyEntourage T F U
