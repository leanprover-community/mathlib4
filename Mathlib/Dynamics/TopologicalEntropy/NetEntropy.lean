...
lemma IsDynNetIn.of_le {T : X → X} {F : Set X} {U : SetRel X X} {m n : ℕ} (m_n : m ≤ n) {s : Set X}
    (h : IsDynNetIn T F U m s) :
    IsDynNetIn T F U n s :=
  ⟨h.1, PairwiseDisjoint.mono h.2 fun x ↦ ball_mono (dynEntourage_antitone T U m_n) x⟩

lemma IsDynNetIn.of_entourage_subset {T : X → X} {F : Set X} {U V : SetRel X X} (U_V : U ⊆ V)
    {n : ℕ} {s : Set X} (h : IsDynNetIn T F V n s) :
    IsDynNetIn T F U n s :=
  ⟨h.1, PairwiseDisjoint.mono h.2 fun x ↦ ball_mono (dynEntourage_monotone T n U_V) x⟩

lemma isDynNetIn_empty {T : X → X} {F : Set X} {U : SetRel X X} {n : ℕ} :
    IsDynNetIn T F U n ∅ :=
  ⟨empty_subset F, pairwise_empty _⟩

lemma isDynNetIn_singleton (T : X → X) {F : Set X} (U : SetRel X X) (n : ℕ) {x : X} (h : x ∈ F) :
...
lemma IsDynNetIn.card_le_card_of_isDynCoverOf {T : X → X} {F : Set X} {U : SetRel X X} [U.IsSymm]
...
lemma IsDynNetIn.card_le_netMaxcard {T : X → X} {F : Set X} {U : SetRel X X} {n : ℕ} {s : Finset X}
...
@[simp]
lemma netMaxcard_empty {T : X → X} {U : SetRel X X} {n : ℕ} : netMaxcard T ∅ U n = 0 := by
...
lemma netMaxcard_zero (T : X → X) {F : Set X} (h : F.Nonempty) (U : SetRel X X) :
...
lemma netMaxcard_le_coverMincard (T : X → X) (F : Set X) {U : SetRel X X}
    [U.IsSymm] (n : ℕ) :
...
lemma coverMincard_le_netMaxcard (T : X → X) (F : Set X) {U : SetRel X X} [U.IsRefl] [U.IsSymm]
    (n : ℕ) :
...
@[simp]
lemma netEntropyEntourage_empty {T : X → X} {U : SetRel X X} :
...
@[simp]
lemma netEntropyInfEntourage_empty {T : X → X} {U : SetRel X X} :
    netEntropyInfEntourage T ∅ U = ⊥ :=
  eq_bot_mono (netEntropyInfEntourage_le_netEntropyEntourage T ∅ U) netEntropyEntourage_empty

lemma netEntropyInfEntourage_nonneg (T : X → X) {F : Set X} (h : F.Nonempty) (U : SetRel X X) :
...
lemma netEntropyEntourage_nonneg (T : X → X) {F : Set X} (h : F.Nonempty) (U : SetRel X X) :
...
lemma netEntropyInfEntourage_le_coverEntropyInfEntourage (T : X → X) (F : Set X) {U : SetRel X X}
    [U.IsSymm] :
...
lemma coverEntropyInfEntourage_le_netEntropyInfEntourage (T : X → X) (F : Set X) {U : SetRel X X}
    [U.IsRefl] [U.IsSymm] :
...
lemma netEntropyEntourage_le_coverEntropyEntourage (T : X → X) (F : Set X) {U : SetRel X X}
    [U.IsSymm] :
...
lemma coverEntropyEntourage_le_netEntropyEntourage (T : X → X) (F : Set X) {U : SetRel X X}
    [U.IsRefl] [U.IsSymm] :
...
lemma netEntropyInfEntourage_le_coverEntropyInf {U : SetRel X X} (h : U ∈ 𝓤 X) :
...
lemma netEntropyEntourage_le_coverEntropy {U : SetRel X X} (h : U ∈ 𝓤 X) :
