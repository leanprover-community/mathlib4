...
lemma IsDynNetIn.card_le_card_of_isDynCoverOf {T : X → X} {F : Set X} {U : Rel X X} [U.IsSymm]
...
noncomputable def netMaxcard (T : X → X) (F : Set X) (U : Rel X X) (n : ℕ) : ℕ∞ :=
  ⨆ (s : Finset X) (_ : IsDynNetIn T F U n s), (s.card : ℕ∞)

lemma IsDynNetIn.card_le_netMaxcard {T : X → X} {F : Set X} {U : Rel X X} {n : ℕ} {s : Finset X}
    (h : IsDynNetIn T F U n s) :
    s.card ≤ netMaxcard T F U n :=
  le_iSup₂ (α := ℕ∞) s h

lemma netMaxcard_monotone_time (T : X → X) (F : Set X) (U : Rel X X) :
    Monotone fun n : ℕ ↦ netMaxcard T F U n :=
  fun _ _ m_n ↦ biSup_mono fun _ h ↦ h.of_le m_n

lemma netMaxcard_antitone (T : X → X) (F : Set X) (n : ℕ) :
    Antitone fun U : Rel X X ↦ netMaxcard T F U n :=
  fun _ _ U_V ↦ biSup_mono fun _ h ↦ h.of_entourage_subset U_V

lemma netMaxcard_finite_iff (T : X → X) (F : Set X) (U : Rel X X) (n : ℕ) :
    netMaxcard T F U n < ⊤ ↔
...
@[simp]
lemma netMaxcard_empty {T : X → X} {U : Rel X X} {n : ℕ} : netMaxcard T ∅ U n = 0 := by
...
lemma netMaxcard_eq_zero_iff (T : X → X) (F : Set X) (U : Rel X X) (n : ℕ) :
...
lemma one_le_netMaxcard_iff (T : X → X) (F : Set X) (U : Rel X X) (n : ℕ) :
...
lemma netMaxcard_zero (T : X → X) {F : Set X} (h : F.Nonempty) (U : Rel X X) :
...
lemma netMaxcard_infinite_iff (T : X → X) (F : Set X) (U : Rel X X) (n : ℕ) :
...
lemma netMaxcard_le_coverMincard (T : X → X) (F : Set X) {U : Rel X X}
    [U.IsSymm] (n : ℕ) :
...
lemma coverMincard_le_netMaxcard (T : X → X) (F : Set X) {U : Rel X X} [U.IsRefl] [U.IsSymm]
    (n : ℕ) :
  ...
  exact x_uncov x x_s (ball_mono (dynEntourage_monotone T n Rel.left_subset_comp) x <|
    Rel.rfl (dynEntourage T U n))
...
lemma netEntropyEntourage_nonneg (T : X → X) {F : Set X} (h : F.Nonempty) (U : Rel X X) :
...
lemma netEntropyInfEntourage_le_coverEntropyInfEntourage (T : X → X) (F : Set X) {U : Rel X X}
    [U.IsSymm] :
...
lemma coverEntropyInfEntourage_le_netEntropyInfEntourage (T : X → X) (F : Set X) {U : Rel X X}
    [U.IsRefl] [U.IsSymm] :
...
lemma netEntropyEntourage_le_coverEntropyEntourage (T : X → X) (F : Set X) {U : Rel X X}
    [U.IsSymm] :
...
lemma coverEntropyEntourage_le_netEntropyEntourage (T : X → X) (F : Set X) {U : Rel X X}
    [U.IsRefl] [U.IsSymm] :
...
theorem coverEntropyInf_eq_iSup_netEntropyInfEntourage :
  ...
  · obtain ⟨V, V_uni, V_symm, V_U⟩ := comp_symm_mem_uniformity_sets U_uni
    have := Rel.id_subset_iff.1 <| refl_le_uniformity V_uni
  ...
  · apply (netEntropyInfEntourage_antitone T F Rel.symmetrize_subset_self).trans
    apply (le_iSup₂ (Rel.symmetrize U) (symmetrize_mem_uniformity U_uni)).trans'
...
theorem coverEntropy_eq_iSup_netEntropyEntourage :
  ...
  · apply (netEntropyEntourage_antitone T F Rel.symmetrize_subset_self).trans
    apply (le_iSup₂ (Rel.symmetrize U) (symmetrize_mem_uniformity U_uni)).trans'
...
lemma coverEntropyInf_eq_iSup_basis_netEntropyInfEntourage {ι : Sort*} {p : ι → Prop}
    {s : ι → Rel X X} (h : (𝓤 X).HasBasis p s) (T : X → X) (F : Set X) :
...
lemma netEntropyInfEntourage_le_coverEntropyInf {U : Rel X X} (h : U ∈ 𝓤 X) :
    netEntropyInfEntourage T F U ≤ coverEntropyInf T F :=
  coverEntropyInf_eq_iSup_netEntropyInfEntourage T F ▸
    le_iSup₂ (f := fun (U : Rel X X) (_ : U ∈ 𝓤 X) ↦ netEntropyInfEntourage T F U) U h

lemma netEntropyEntourage_le_coverEntropy {U : Rel X X} (h : U ∈ 𝓤 X) :
    netEntropyEntourage T F U ≤ coverEntropy T F :=
  coverEntropy_eq_iSup_netEntropyEntourage T F ▸
    le_iSup₂ (f := fun (U : Rel X X) (_ : U ∈ 𝓤 X) ↦ netEntropyEntourage T F U) U h
