...
variable {X Y : Type*} {S : X → X} {T : Y → Y} {φ : X → Y} {V : SetRel Y Y}

lemma IsDynCoverOf.image (h : Semiconj φ S T) {F : Set X} [V.IsSymm]
...
lemma IsDynCoverOf.preimage (h : Semiconj φ S T) {F : Set X} [V.IsSymm] {n : ℕ}
