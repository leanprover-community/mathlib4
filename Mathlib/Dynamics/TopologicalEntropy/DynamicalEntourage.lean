...
lemma mem_dynEntourage {T : X → X} {U : SetRel X X} {n : ℕ} {x y : X} :
    (x, y) ∈ dynEntourage T U n ↔ ∀ k < n, (T^[k] x, T^[k] y) ∈ U := by
  simp [dynEntourage]

lemma mem_ball_dynEntourage {T : X → X} {U : SetRel X X} {n : ℕ} {x y : X} :
...
lemma ball_dynEntourage_mem_nhds [UniformSpace X] {T : X → X} (h : Continuous T) {U : SetRel X X}
    (U_uni : U ∈ 𝓤 X) (n : ℕ) (x : X) :
...
instance {T : X → X} {U : SetRel X X} [U.IsRefl] {n : ℕ} : (dynEntourage T U n).IsRefl := by
  simp [dynEntourage]; infer_instance

instance {T : X → X} {U : SetRel X X} [U.IsSymm] {n : ℕ} : (dynEntourage T U n).IsSymm := by
  simp [dynEntourage]; infer_instance
...
lemma _root_.isOpen.dynEntourage [TopologicalSpace X] {T : X → X} (T_cont : Continuous T)
    {U : SetRel X X} (U_open : IsOpen U) (n : ℕ) :
...
@[simp]
lemma dynEntourage_one {T : X → X} {U : SetRel X X} :
