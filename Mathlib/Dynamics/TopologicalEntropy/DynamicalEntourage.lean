...
open scoped Rel Topology Uniformity
...
lemma dynEntourage_eq_inter_Ico (T : X → X) (U : Rel X X) (n : ℕ) :
    dynEntourage T U n = ⋂ k : Ico 0 n, (map T T)^[k] ⁻¹' U := by
  simp [dynEntourage]

lemma mem_dynEntourage {T : X → X} {U : Rel X X} {n : ℕ} {x y : X} :
    (x, y) ∈ dynEntourage T U n ↔ ∀ k < n, (T^[k] x, T^[k] y) ∈ U := by
  simp [dynEntourage]

lemma mem_ball_dynEntourage {T : X → X} {U : Rel X X} {n : ℕ} {x y : X} :
...
lemma ball_dynEntourage_mem_nhds [UniformSpace X] {T : X → X} (h : Continuous T) {U : Rel X X}
    (U_uni : U ∈ 𝓤 X) (n : ℕ) (x : X) :
...
instance {T : X → X} {U : Rel X X} [U.IsRefl] {n : ℕ} : (dynEntourage T U n).IsRefl := by
  simp [dynEntourage]; infer_instance

instance {T : X → X} {U : Rel X X} [U.IsSymm] {n : ℕ} : (dynEntourage T U n).IsSymm := by
  simp [dynEntourage]; infer_instance

lemma dynEntourage_comp_subset (T : X → X) (U V : Rel X X) (n : ℕ) :
  ...
  simp only [Rel.comp, mem_iInter, mem_preimage, map_apply, mem_setOf_eq] at xy_comp ⊢
...
lemma mem_ball_dynEntourage_comp (T : X → X) (n : ℕ) {U : Rel X X} [U.IsSymm]
