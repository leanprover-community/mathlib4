...
theorem of_mem_extremePoints_measure_univ_eq (hfm : Measurable f) {c : ℝ≥0∞} (hc : c ≠ ∞)
    (h : μ ∈ extremePoints ℝ≥0∞ {ν | MeasurePreserving f ν ν ∧ ν univ = c}) : Ergodic f μ := by
  have hf : MeasurePreserving f μ μ := h.1.1
  rcases eq_or_ne c 0 with rfl | hc₀
  · convert! zero_measure
    rw [← measure_univ_eq_zero, h.1.2]
  · refine ⟨hf, .of_preimage_eq hfm hf.quasiMeasurePreserving ?_⟩
...
theorem of_mem_extremePoints (hfm : Measurable f)
    (h : μ ∈ extremePoints ℝ≥0∞ {ν | MeasurePreserving f ν ν ∧ IsProbabilityMeasure ν}) :
    Ergodic f μ :=
  .of_mem_extremePoints_measure_univ_eq hfm ENNReal.one_ne_top <| by
    simpa only [isProbabilityMeasure_iff] using h

-- TODO: do we need `IsFiniteMeasure ν` here?
theorem eq_smul_of_absolutelyContinuous [IsFiniteMeasure μ] [IsFiniteMeasure ν] (hfm : Measurable f)
    (hμ : Ergodic f μ) (hfν : MeasurePreserving f ν ν) (hνμ : ν ≪ μ) : ∃ c : ℝ≥0∞, ν = c • μ := by
  have := hfν.rnDeriv_comp_aeEq hfm hμ.toMeasurePreserving
...
theorem eq_of_absolutelyContinuous_measure_univ_eq [IsFiniteMeasure μ] [IsFiniteMeasure ν]
    (hfm : Measurable f) (hμ : Ergodic f μ) (hfν : MeasurePreserving f ν ν) (hνμ : ν ≪ μ)
...
theorem eq_of_absolutelyContinuous [IsProbabilityMeasure μ] [IsProbabilityMeasure ν]
    (hfm : Measurable f) (hμ : Ergodic f μ) (hfν : MeasurePreserving f ν ν) (hνμ : ν ≪ μ) : ν = μ :=
  eq_of_absolutelyContinuous_measure_univ_eq hfm hμ hfν hνμ <| by simp

theorem mem_extremePoints_measure_univ_eq [IsFiniteMeasure μ] (hfm : Measurable f)
    (hμ : Ergodic f μ) :
...
theorem mem_extremePoints [IsProbabilityMeasure μ] (hfm : Measurable f) (hμ : Ergodic f μ) :
    μ ∈ extremePoints ℝ≥0∞ {ν | MeasurePreserving f ν ν ∧ IsProbabilityMeasure ν} := by
  simpa only [isProbabilityMeasure_iff, measure_univ] using hμ.mem_extremePoints_measure_univ_eq hfm
...
theorem iff_mem_extremePoints_measure_univ_eq [IsFiniteMeasure μ] (hfm : Measurable f) :
  ...
  ⟨(mem_extremePoints_measure_univ_eq hfm ·),
    of_mem_extremePoints_measure_univ_eq hfm (measure_ne_top _ _)⟩
...
theorem iff_mem_extremePoints [IsProbabilityMeasure μ] (hfm : Measurable f) :
  ...
  ⟨(mem_extremePoints hfm ·), of_mem_extremePoints hfm⟩
