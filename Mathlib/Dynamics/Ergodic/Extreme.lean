...
theorem of_mem_extremePoints_measure_univ_eq {c : ℝ≥0∞} (hc : c ≠ ∞)
  ...
  · -- Replace `f` by an a.e. equal measurable map `g`.
    obtain ⟨g, hgm, hfg⟩ := hf.aemeasurable
    have hg : MeasurePreserving g μ μ := hf.congr hfg
    grw [hfg]
    refine .of_preimage_eq hgm hg ?_
    have : IsFiniteMeasure μ := by
      constructor
      rwa [h.1.2, lt_top_iff_ne_top]
    set S := {ν | MeasurePreserving f ν ν ∧ ν univ = c}
    have {s : Set X} (hsm : MeasurableSet s) (hgs : g ⁻¹' s = s) (hμs : μ s ≠ 0) :
        c • μ[|s] ∈ S := by
      -- The measures we feed to extremality are `≪ μ`, hence preserved by `f` iff by `g`.
      have hac : c • μ[|s] ≪ μ := smul_absolutelyContinuous.trans cond_absolutelyContinuous
      refine ⟨?_, ?_⟩
      · grw [hac.ae_eq hfg]
        refine .smul_measure (.smul_measure ?_ _) c
        convert! hg.restrict_preimage hsm
        exact hgs.symm
    ...
    have hcond : c • μ[|s] = μ := by
      apply h.2 (this hsm hgs hs) (this hsm.compl (by rw [preimage_compl, hgs]) hs')
...
theorem eq_smul_of_absolutelyContinuous [IsFiniteMeasure μ] [IsFiniteMeasure ν]
...
theorem eq_of_absolutelyContinuous_measure_univ_eq [IsFiniteMeasure μ] [IsFiniteMeasure ν]
    (hμ : Ergodic f μ) (hfν : MeasurePreserving f ν ν) (hνμ : ν ≪ μ)
