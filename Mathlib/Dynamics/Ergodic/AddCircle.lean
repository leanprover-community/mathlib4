...
-/
lemma ae_empty_or_univ_of_forall_vadd_ae_eq_self {s : Set <| AddCircle T}
...
lemma ergodic_zsmul {n : ℤ} (hn : 1 < |n|) : Ergodic fun y : AddCircle T => n • y :=
...
lemma ergodic_nsmul {n : ℕ} (hn : 1 < n) : Ergodic fun y : AddCircle T => n • y :=
