...
lemma iterate_injOn_Iio_minimalPeriod : (Iio <| minimalPeriod f x).InjOn (f^[·] x) :=
...
lemma iterate_eq_iterate_iff_of_lt_minimalPeriod {m n : ℕ} (hm : m < minimalPeriod f x)
...
@[simp] lemma minimalPeriod_id : minimalPeriod id x = 1 :=
...
lemma pow_mod_period_smul (n : ℕ) {m : M} {a : α} :
...
@[to_additive (attr := simp)]
lemma zpow_mod_period_smul (j : ℤ) {g : G} {a : α} :
