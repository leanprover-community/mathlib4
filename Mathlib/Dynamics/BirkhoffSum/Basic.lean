...
theorem birkhoffSum_apply_zero (f : α → α) (g : α → M) (x : α) : birkhoffSum f g 0 x = 0 :=
  sum_range_zero _
...
theorem birkhoffSum_succ (f : α → α) (g : α → M) (n : ℕ) :
  ...
  funext <| birkhoffSum_apply_succ f g n
...
theorem birkhoffSum_apply_add (f : α → α) (g g' : α → M) (n : ℕ) (x : α) :
...
theorem birkhoffSum_apply_neg (f : α → α) (g : α → G) (n : ℕ) (x : α) :
...
theorem birkhoffSum_apply_sub_birkhoffSum (f : α → α) (g : α → G) (n : ℕ) (x : α) :
  ...
  rw [← sub_eq_iff_eq_add.2 (birkhoffSum_apply_succ f g n x),
    ← sub_eq_iff_eq_add.2 (birkhoffSum_apply_succ' f g n x),
