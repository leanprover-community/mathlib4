...
theorem anisotropic_of_prod {R} [OrderedCommRing R] [Module R M₁] [Module R M₂]
  ...
  · intro x hx
    refine (h x 0 ?_).1
    rw [hx, zero_add, _root_.map_zero]
  · intro x hx
    refine (h 0 x ?_).2
    rw [hx, add_zero, _root_.map_zero]
...
theorem nonneg_prod_iff {R} [OrderedCommRing R] [Module R M₁] [Module R M₂]
  ...
  · intro h
    constructor
    · intro x; simpa only [add_zero, _root_.map_zero] using h x 0
    · intro x; simpa only [zero_add, _root_.map_zero] using h 0 x
  · rintro ⟨h₁, h₂⟩ x₁ x₂
...
theorem anisotropic_of_pi [Fintype ι] {R} [OrderedCommRing R] [∀ i, Module R (Mᵢ i)]
  ...
  · rw [Pi.single_eq_of_ne hji, _root_.map_zero]
