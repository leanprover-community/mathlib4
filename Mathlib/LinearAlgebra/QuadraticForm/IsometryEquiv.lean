...
instance (priority := 10000) : EquivLike (Q₁.IsometryEquiv Q₂) M₁ M₂ where
...
instance (priority := 10000) : LinearEquivClass (Q₁.IsometryEquiv Q₂) R M₁ M₂ where
  map_add f := map_add f.toLinearEquiv
  map_smulₛₗ f := map_smulₛₗ f.toLinearEquiv

-- Porting note: was `Coe`
instance (priority := 10000) : CoeOut (Q₁.IsometryEquiv Q₂) (M₁ ≃ₗ[R] M₂) :=
