...
Authors: Andrew Yang, Christopher Hoskin
-/
...
def dualSubmodule (N : Submodule R M) : Submodule R M where
  ...
  zero_mem' y _ := by rw [B.map_zero]; exact zero_mem _
  smul_mem' r a ha y hy := by
    convert (1 : Submodule R S).smul_mem r (ha y hy)
    rw [← IsScalarTower.algebraMap_smul S r a, map_smul, Algebra.smul_def]
    rfl
...
lemma le_flip_dualSubmodule {N₁ N₂ : Submodule R M} :
    N₁ ≤ dualSubmodule B.flip N₂ ↔ N₂ ≤ B.dualSubmodule N₁ := by
...
lemma dualSubmoduleToDual_injective (hB : B.SeparatingLeft) [NoZeroSMulDivisors R S]
    (N : Submodule R M) (hN : Submodule.span S (N : Set M) = ⊤) :
    Function.Injective (B.dualSubmoduleToDual N) := by
  intro x y e
  ext
  apply LinearMap.ker_eq_bot.mp (LinearMap.separatingLeft_iff_ker_eq_bot.mp hB)
...
lemma dualSubmodule_span_of_basis {ι} [Finite ι] [DecidableEq ι]
    (hB : B.SeparatingLeft) (b : Basis ι S M) :
...
lemma dualSubmodule_dualSubmodule_flip_of_basis {ι : Type*} [Finite ι]
    (hB : B.SeparatingLeft) (b : Basis ι S M) :
    B.dualSubmodule (dualSubmodule B.flip (Submodule.span R (Set.range b))) =
      Submodule.span R (Set.range b) := by
  classical
  letI := FiniteDimensional.of_fintype_basis b
  rw [dualSubmodule_span_of_basis _ (SeparatingLeft.flip hB), dualSubmodule_span_of_basis B hB,
    dualBasis_dualBasis_flip B hB]

lemma dualSubmodule_flip_dualSubmodule_of_basis {ι : Type*} [Finite ι]
    (hB : B.SeparatingLeft) (b : Basis ι S M) :
    dualSubmodule B.flip (B.dualSubmodule (Submodule.span R (Set.range b))) =
      Submodule.span R (Set.range b) := by
  classical
  letI := FiniteDimensional.of_fintype_basis b
  rw [dualSubmodule_span_of_basis B hB, dualSubmodule_span_of_basis _ (SeparatingLeft.flip hB),
    dualBasis_flip_dualBasis B hB]

lemma dualSubmodule_dualSubmodule_of_basis
    {ι} [Finite ι] (hB : B.SeparatingLeft) (hB' : B.IsSymm) (b : Basis ι S M) :
