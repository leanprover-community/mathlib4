...
-- Should be `@[simp]` but doesn't fire due to `https://github.com/leanprover/lean4/issues/3701`.
theorem span_image [RingHomSurjective σ₁₂] (f : F) :
    span R₂ (f '' s) = map f (span R s) :=
  (map_span f s).symm

@[simp] -- Should be replaced with `Submodule.span_image` when `https://github.com/leanprover/lean4/issues/3701` is fixed.
