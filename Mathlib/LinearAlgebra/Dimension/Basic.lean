...
theorem cardinal_lift_le_rank {ι : Type w} {v : ι → M}
  ...
  rw [Module.rank]
  refine le_trans ?_ (lift_le.mpr <| le_ciSup (bddAbove_of_small _) ⟨_, hv.coe_range⟩)
...
theorem lift_rank_le_of_injective_injective (i : R' → R) (j : M →+ M')
  ...
  simp_rw [Module.rank, lift_iSup (bddAbove_of_small _)]
  exact ciSup_mono' (bddAbove_of_small _) fun ⟨s, h⟩ ↦ ⟨⟨j '' s,
...
theorem lift_rank_range_le (f : M →ₗ[R] M') : Cardinal.lift.{v}
  ...
  rw [Cardinal.lift_iSup (Cardinal.bddAbove_of_small _)]
  apply ciSup_le'
  rintro ⟨s, li⟩
  apply le_trans
  swap
  · apply Cardinal.lift_le.mpr
    refine le_ciSup (Cardinal.bddAbove_of_small _) ⟨rangeSplitting f '' s, ?_⟩
