...
lemma coe_dualAnnihilator_span (s : Set M) :
  ...
  ext f
  simp only [SetLike.mem_coe, mem_dualAnnihilator, Set.mem_ofPred, ← LinearMap.mem_ker]
  exact span_le
...
lemma coe_dualCoannihilator_span (s : Set (Module.Dual R M)) :
  ...
  simp only [SetLike.mem_coe, mem_dualCoannihilator, Set.mem_ofPred, ← LinearMap.mem_ker, this]
  exact span_le
