...
lemma apply_eq_of_mem_genEigenspace_of_comm_of_isFinitelySemisimple_of_isNilpotent_sub
  ...
  suffices IsNilpotent ((g - algebraMap R (End R M) μ).restrict h₂) by
    replace this : g.restrict h₁ - algebraMap R (End R p) μ = 0 :=
      eq_zero_of_isNilpotent_of_isFinitelySemisimple this (by simpa using hss.restrict)
...
lemma IsFinitelySemisimple.genEigenspace_eq_eigenspace
  ...
  exact apply_eq_of_mem_genEigenspace_of_comm_of_isFinitelySemisimple_of_isNilpotent_sub hm rfl hf
    (by simp)
