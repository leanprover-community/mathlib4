...
theorem isCompl_range_inl_inr : IsCompl (inl R M M₂).range (inr R M M₂).range := by
  constructor
  · rw [disjoint_def]
...
theorem tailings_disjoint_tunnel (f : M × N →ₗ[R] M) (i : Injective f) (n : ℕ) :
    Disjoint (tailings f i n) (tunnel f i (n + 1)).ofDual := by
