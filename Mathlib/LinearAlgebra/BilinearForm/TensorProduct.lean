...
def tensorDistrib : LinearMap.BilinForm A M₁ ⊗[R] LinearMap.BilinForm R M₂ →ₗ[A]
    LinearMap.BilinForm A (M₁ ⊗[R] M₂) :=
...
@[simp]
theorem tensorDistrib_tmul (B₁ : LinearMap.BilinForm A M₁) (B₂ : LinearMap.BilinForm R M₂)
    (m₁ : M₁) (m₂ : M₂) (m₁' : M₁) (m₂' : M₂) :
...
@[reducible]
protected def _root_.LinearMap.tmul (B₁ : LinearMap.BilinForm A M₁)
    (B₂ : LinearMap.BilinForm R M₂) : LinearMap.BilinForm A (M₁ ⊗[R] M₂) :=
...
lemma _root_.LinearMap.IsSymm.tmul {B₁ : LinearMap.BilinForm A M₁} {B₂ : LinearMap.BilinForm R M₂}
...
protected def baseChange (B : LinearMap.BilinForm R M₂) : LinearMap.BilinForm A (A ⊗[R] M₂) :=
  LinearMap.tmul (R := R) (A := A) (M₁ := A) (M₂ := M₂) (LinearMap.mul A A) B

@[simp]
theorem baseChange_tmul (B₂ : LinearMap.BilinForm R M₂) (a : A) (m₂ : M₂)
    (a' : A) (m₂' : M₂) :
...
lemma IsSymm.baseChange {B₂ : LinearMap.BilinForm R M₂} (hB₂ : B₂.IsSymm) :
...
noncomputable def tensorDistribEquiv :
    LinearMap.BilinForm R M₁ ⊗[R] LinearMap.BilinForm R M₂ ≃ₗ[R]
    LinearMap.BilinForm R (M₁ ⊗[R] M₂) :=
...
@[simp, nolint simpNF]
theorem tensorDistribEquiv_tmul (B₁ : LinearMap.BilinForm R M₁) (B₂ : LinearMap.BilinForm R M₂)
    (m₁ : M₁) (m₂ : M₂) (m₁' : M₁) (m₂' : M₂) :
...
@[simp]
theorem tensorDistribEquiv_apply (B : LinearMap.BilinForm R M₁ ⊗ LinearMap.BilinForm R M₂) :
