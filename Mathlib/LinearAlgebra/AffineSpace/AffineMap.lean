...
@[simp]
theorem lineMap_eq_lineMap_iff [Module.IsTorsionFree k V1] {p₀ p₁ : P1} {c₁ c₂ : k} :
...
@[simp]
theorem lineMap_eq_left_iff [Module.IsTorsionFree k V1] {p₀ p₁ : P1} {c : k} :
    lineMap p₀ p₁ c = p₀ ↔ p₀ = p₁ ∨ c = 0 := by
  rw [← @lineMap_eq_lineMap_iff k V1, lineMap_apply_zero]

@[simp]
theorem lineMap_eq_right_iff [Module.IsTorsionFree k V1] {p₀ p₁ : P1} {c : k} :
    lineMap p₀ p₁ c = p₁ ↔ p₀ = p₁ ∨ c = 1 := by
  rw [← @lineMap_eq_lineMap_iff k V1, lineMap_apply_one]

variable (k) in
theorem lineMap_injective [Module.IsTorsionFree k V1] {p₀ p₁ : P1} (h : p₀ ≠ p₁) :
