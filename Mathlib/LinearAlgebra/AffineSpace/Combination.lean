...
@[simp (high)]
lemma weightedVSubOfPoint_apply_const (w : ι → k) (p : P) (b : P) :
...
lemma weightedVSubOfPoint_congr {w₁ w₂ : ι → k} (hw : ∀ i ∈ s, w₁ i = w₂ i) {p₁ p₂ : ι → P}
    (hp : ∀ i ∈ s, p₁ i = p₂ i) (b : P) :
...
lemma weightedVSubOfPoint_eq_of_weights_eq (p : ι → P) (j : ι) (w₁ w₂ : ι → k)
    (hw : ∀ i, i ≠ j → w₁ i = w₂ i) :
...
the weights is 0. -/
lemma weightedVSubOfPoint_eq_of_sum_eq_zero (w : ι → k) (p : ι → P) (h : ∑ i in s, w i = 0)
...
lemma weightedVSubOfPoint_vadd_eq_of_sum_eq_one (w : ι → k) (p : ι → P) (h : ∑ i in s, w i = 1)
...
@[simp (high)]
lemma weightedVSubOfPoint_erase [DecidableEq ι] (w : ι → k) (p : ι → P) (i : ι) :
...
@[simp (high)]
lemma weightedVSubOfPoint_insert [DecidableEq ι] (w : ι → k) (p : ι → P) (i : ι) :
...
lemma weightedVSubOfPoint_indicator_subset (w : ι → k) (p : ι → P) (b : P) {s₁ s₂ : Finset ι}
    (h : s₁ ⊆ s₂) :
...
`Finset`. -/
lemma weightedVSubOfPoint_map (e : ι₂ ↪ ι) (w : ι → k) (p : ι → P) (b : P) :
...
lemma sum_smul_vsub_eq_weightedVSubOfPoint_sub (w : ι → k) (p₁ p₂ : ι → P) (b : P) :
...
lemma sum_smul_vsub_const_eq_weightedVSubOfPoint_sub (w : ι → k) (p₁ : ι → P) (p₂ b : P) :
...
expressed as a subtraction involving a `weightedVSubOfPoint` expression. -/
lemma sum_smul_const_vsub_eq_sub_weightedVSubOfPoint (w : ι → k) (p₂ : ι → P) (p₁ b : P) :
...
lemma weightedVSubOfPoint_sdiff [DecidableEq ι] {s₂ : Finset ι} (h : s₂ ⊆ s) (w : ι → k)
    (p : ι → P) (b : P) :
...
lemma weightedVSubOfPoint_sdiff_sub [DecidableEq ι] {s₂ : Finset ι} (h : s₂ ⊆ s) (w : ι → k)
    (p : ι → P) (b : P) :
    (s \ s₂).weightedVSubOfPoint p b w - s₂.weightedVSubOfPoint p b (-w) =
      s.weightedVSubOfPoint p b w :=
...
lemma weightedVSubOfPoint_subtype_eq_filter (w : ι → k) (p : ι → P) (b : P) (pred : ι → Prop)
    [DecidablePred pred] :
...
lemma weightedVSubOfPoint_filter_of_ne (w : ι → k) (p : ι → P) (b : P) {pred : ι → Prop}
...
sum. -/
lemma weightedVSubOfPoint_const_smul (w : ι → k) (p : ι → P) (b : P) (c : k) :
...
lemma weightedVSub_apply (w : ι → k) (p : ι → P) :
...
lemma weightedVSub_eq_weightedVSubOfPoint_of_sum_eq_zero (w : ι → k) (p : ι → P)
...
is 0. -/
@[simp]
lemma weightedVSub_apply_const (w : ι → k) (p : P) (h : ∑ i in s, w i = 0) :
...
@[simp]
lemma weightedVSub_empty (w : ι → k) (p : ι → P) : (∅ : Finset ι).weightedVSub p w = (0 : V) := by
...
that are equal on `s`. -/
lemma weightedVSub_congr {w₁ w₂ : ι → k} (hw : ∀ i ∈ s, w₁ i = w₂ i) {p₁ p₂ : ι → P}
...
/-- The weighted sum is unaffected by changing the weights to the
corresponding indicator function and adding points to the set. -/
lemma weightedVSub_indicator_subset (w : ι → k) (p : ι → P) {s₁ s₂ : Finset ι} (h : s₁ ⊆ s₂) :
...
original `Finset`. -/
lemma weightedVSub_map (e : ι₂ ↪ ι) (w : ι → k) (p : ι → P) :
...
expressions. -/
lemma sum_smul_vsub_eq_weightedVSub_sub (w : ι → k) (p₁ p₂ : ι → P) :
...
lemma sum_smul_vsub_const_eq_weightedVSub (w : ι → k) (p₁ : ι → P) (p₂ : P)
...
sum of the weights is 0. -/
lemma sum_smul_const_vsub_eq_neg_weightedVSub (w : ι → k) (p₂ : ι → P) (p₁ : P)
...
/-- A weighted sum may be split into such sums over two subsets. -/
lemma weightedVSub_sdiff [DecidableEq ι] {s₂ : Finset ι} (h : s₂ ⊆ s) (w : ι → k) (p : ι → P) :
...
/-- A weighted sum may be split into a subtraction of such sums over two subsets. -/
lemma weightedVSub_sdiff_sub [DecidableEq ι] {s₂ : Finset ι} (h : s₂ ⊆ s) (w : ι → k)
...
/-- A weighted sum over `s.subtype pred` equals one over `s.filter pred`. -/
lemma weightedVSub_subtype_eq_filter (w : ι → k) (p : ι → P) (pred : ι → Prop)
    [DecidablePred pred] :
...
/-- A weighted sum over `s.filter pred` equals one over `s` if all the weights at indices in `s`
not satisfying `pred` are zero. -/
lemma weightedVSub_filter_of_ne (w : ι → k) (p : ι → P) {pred : ι → Prop} [DecidablePred pred]
...
lemma weightedVSub_const_smul (w : ι → k) (p : ι → P) (c : k) :
...
`weightedVSub`. -/
@[simp]
lemma affineCombination_linear (p : ι → P) :
...
lemma affineCombination_apply (w : ι → k) (p : ι → P) :
...
@[simp]
lemma affineCombination_apply_const (w : ι → k) (p : P) (h : ∑ i in s, w i = 1) :
...
points that are equal on `s`. -/
lemma affineCombination_congr {w₁ w₂ : ι → k} (hw : ∀ i ∈ s, w₁ i = w₂ i) {p₁ p₂ : ι → P}
...
lemma affineCombination_eq_weightedVSubOfPoint_vadd_of_sum_eq_one (w : ι → k) (p : ι → P)
    (h : ∑ i in s, w i = 1) (b : P) :
...
lemma weightedVSub_vadd_affineCombination (w₁ w₂ : ι → k) (p : ι → P) :
...
lemma affineCombination_vsub (w₁ w₂ : ι → k) (p : ι → P) :
...
combination. -/
@[simp]
lemma weightedVSub_eq_linear_combination {ι} (s : Finset ι) {w : ι → k} {p : ι → V}
...
combinations. -/
@[simp]
lemma affineCombination_eq_linear_combination (s : Finset ι) (p : ι → V) (w : ι → k)
...
@[simp]
lemma affineCombination_of_eq_one_of_eq_zero (w : ι → k) (p : ι → P) {i : ι} (his : i ∈ s)
...
corresponding indicator function and adding points to the set. -/
lemma affineCombination_indicator_subset (w : ι → k) (p : ι → P) {s₁ s₂ : Finset ι}
    (h : s₁ ⊆ s₂) :
...
`Finset`. -/
lemma affineCombination_map (e : ι₂ ↪ ι) (w : ι → k) (p : ι → P) :
...
expressions. -/
lemma sum_smul_vsub_eq_affineCombination_vsub (w : ι → k) (p₁ p₂ : ι → P) :
    (∑ i in s, w i • (p₁ i -ᵥ p₂ i)) =
...
/-- A weighted sum of pairwise subtractions, where the point on the right is constant and the
sum of the weights is 1. -/
lemma sum_smul_vsub_const_eq_affineCombination_vsub (w : ι → k) (p₁ : ι → P) (p₂ : P)
...
/-- A weighted sum of pairwise subtractions, where the point on the left is constant and the
sum of the weights is 1. -/
lemma sum_smul_const_vsub_eq_vsub_affineCombination (w : ι → k) (p₂ : ι → P) (p₁ : P)
...
lemma affineCombination_sdiff_sub [DecidableEq ι] {s₂ : Finset ι} (h : s₂ ⊆ s) (w : ι → k)
    (p : ι → P) :
...
lemma affineCombination_eq_of_weightedVSub_eq_zero_of_eq_neg_one {w : ι → k} {p : ι → P}
...
lemma affineCombination_subtype_eq_filter (w : ι → k) (p : ι → P) (pred : ι → Prop)
    [DecidablePred pred] :
...
lemma affineCombination_filter_of_ne (w : ι → k) (p : ι → P) {pred : ι → Prop}
    [DecidablePred pred] (h : ∀ i ∈ s, w i ≠ 0 → pred i) :
...
lemma eq_weightedVSubOfPoint_subset_iff_eq_weightedVSubOfPoint_subtype {v : V} {x : k} {s : Set ι}
    {p : ι → P} {b : P} :
...
corresponding to that subset. -/
lemma eq_weightedVSub_subset_iff_eq_weightedVSub_subtype {v : V} {s : Set ι} {p : ι → P} :
...
subset. -/
lemma eq_affineCombination_subset_iff_eq_affineCombination_subtype {p0 : P} {s : Set ι}
    {p : ι → P} :
...
lemma map_affineCombination {V₂ P₂ : Type*} [AddCommGroup V₂] [Module k V₂] [AffineSpace V₂ P₂]
...
@[simp]
lemma affineCombination_affineCombinationSingleWeights [DecidableEq ι] (p : ι → P) {i : ι}
...
specified points. -/
@[simp]
lemma weightedVSub_weightedVSubVSubWeights [DecidableEq ι] (p : ι → P) {i j : ι} (hi : i ∈ s)
...
`line_map`. -/
@[simp]
lemma affineCombination_affineCombinationLineMapWeights [DecidableEq ι] (p : ι → P) {i j : ι}
    (hi : i ∈ s) (hj : j ∈ s) (c : k) :
...
@[simp]
lemma centroidWeights_apply (i : ι) : s.centroidWeights k i = (card s : k)⁻¹ :=
  rfl
#align finset.centroid_weights_apply Finset.centroidWeights_apply

/-- `centroidWeights` equals a constant function. -/
lemma centroidWeights_eq_const : s.centroidWeights k = Function.const ι (card s : k)⁻¹ :=
  rfl
...
lemma sum_centroidWeights_eq_one_of_cast_card_ne_zero (h : (card s : k) ≠ 0) :
...
lemma sum_centroidWeights_eq_one_of_card_ne_zero [CharZero k] (h : card s ≠ 0) :
...
lemma sum_centroidWeights_eq_one_of_nonempty [CharZero k] (h : s.Nonempty) :
...
lemma sum_centroidWeights_eq_one_of_card_eq_add_one [CharZero k] {n : ℕ} (h : card s = n + 1) :
    ∑ i in s, s.centroidWeights k i = 1 :=
...
lemma centroid_def (p : ι → P) : s.centroid k p = s.affineCombination k p (s.centroidWeights k) :=
  rfl
...
@[simp]
lemma centroid_singleton (p : ι → P) (i : ι) : ({i} : Finset ι).centroid k p = p i := by
...
to a point. -/
lemma centroid_pair [DecidableEq ι] [Invertible (2 : k)] (p : ι → P) (i₁ i₂ : ι) :
...
lemma centroidWeightsIndicator_def :
...
lemma sum_centroidWeightsIndicator [Fintype ι] :
...
zero. -/
lemma sum_centroidWeightsIndicator_eq_one_of_card_ne_zero [CharZero k] [Fintype ι]
...
lemma sum_centroidWeightsIndicator_eq_one_of_nonempty [CharZero k] [Fintype ι] (h : s.Nonempty) :
...
lemma sum_centroidWeightsIndicator_eq_one_of_card_eq_add_one [CharZero k] [Fintype ι] {n : ℕ}
...
lemma centroid_eq_affineCombination_fintype [Fintype ι] (p : ι → P) :
...
image. -/
lemma centroid_eq_centroid_image_of_inj_on {p : ι → P}
...
lemma centroid_eq_of_inj_on_of_image_eq {p : ι → P}
...
an indexed family. -/
lemma weightedVSub_mem_vectorSpan {s : Finset ι} {w : ι → k} (h : ∑ i in s, w i = 0)
...
nontrivial. -/
lemma affineCombination_mem_affineSpan [Nontrivial k] {s : Finset ι} {w : ι → k}
...
lemma mem_vectorSpan_iff_eq_weightedVSub {v : V} {p : ι → P} :
...
lemma eq_affineCombination_of_mem_affineSpan {p1 : P} {p : ι → P}
...
lemma mem_affineSpan_iff_eq_affineCombination [Nontrivial k] {p1 : P} {p : ι → P} :
...
lemma mem_affineSpan_iff_eq_weightedVSubOfPoint_vadd [Nontrivial k] (p : ι → P) (j : ι) (q : P) :
...
unchanged. -/
lemma affineSpan_eq_affineSpan_lineMap_units [Nontrivial k] {s : Set P} {p : P} (hp : p ∈ s)
    (w : s → Units k) :
...
converted to `k`, is not zero. -/
lemma centroid_mem_affineSpan_of_cast_card_ne_zero {s : Finset ι} (p : ι → P)
...
lemma centroid_mem_affineSpan_of_card_ne_zero [CharZero k] {s : Finset ι} (p : ι → P)
...
lemma centroid_mem_affineSpan_of_nonempty [CharZero k] {s : Finset ι} (p : ι → P)
...
lemma centroid_mem_affineSpan_of_card_eq_add_one [CharZero k] {s : Finset ι} (p : ι → P) {n : ℕ}
