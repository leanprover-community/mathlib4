...
theorem coe_affineSpan_singleton (p : P) : (affineSpan k ({p} : Set P) : Set P) = {p} := by
  ext x
  rw [mem_coe, ← vsub_right_mem_direction_iff_mem (mem_affineSpan k (mem_singleton_self p)) _,
...
instance unique_affineSpan_singleton (p : P) : Unique (affineSpan k {p}) where
  default := ⟨p, mem_affineSpan _ (mem_singleton_self _)⟩
...
theorem vectorSpan_range_eq_span_range_vsub_left_ne (p : ι → P) (i₀ : ι) :
  ...
  congr with v
  simp only [Set.mem_range, Set.mem_image, Set.mem_sdiff, mem_singletone.exists]
  constructor
...
theorem affineSpan_singleton_union_vadd_eq_top_of_span_eq_top {s : Set V} (p : P)
  ...
  convert!
    ext_of_direction_eq _
      ⟨p, mem_affineSpan k (Set.mem_union_left _ (mem_singleton_self _)), mem_top k V p⟩
  rw [direction_affineSpan, direction_top,
    vectorSpan_eq_span_vsub_set_right k (Set.mem_union_left _ (mem_singleton_self _) : p ∈ _),
    eq_top_iff, ← h]
...
theorem direction_affineSpan_insert {s : AffineSubspace k P} {p₁ p₂ : P} (hp₁ : p₁ ∈ s) :
  ...
  rw [direction_sup hp₁ (mem_affineSpan k (mem_singleton_self _)), direction_affineSpan]
  simp
