...
`r = 0` to mean the output space passes through `c`, while `r = 1` means not moving the input space
at all. With this convention, this transformation is also equivalent to
`AffineSubspace.map (homothety c r)` when `r` is a unit.

## Main declarations
* `AffineSubspace.shift` defines the shift transformation.
* `AffineSubspace.shift_eq` shows the shift transform is equivalent to translation.
* `AffineSubspace.shift_eq_map_homothety` shows the shift transform is equivalent to homothety.
...
open Module Submodule Finset AffineMap
...
`s` is moved towards `c` with linear interpolation by `r`. When `r = 0`, that
point is moved onto `c`. When `r = 1`, that point stays at the original position.

We define `AffineSubspace.shift ⊥ c r = ⊥` -/
noncomputable
...
theorem AffineSubspace.shift_eq (s : AffineSubspace k P) (p : s) (c : P) (r : k) :
  ...
  · refine ⟨(1 - r) • (p.val -ᵥ h.some.val) +ᵥ x, ?_, ?_⟩
    · refine vadd_mem_of_mem_direction ?_ hx
      apply smul_mem
...
theorem AffineSubspace.shift_zero (s : AffineSubspace k P) [h : Nonempty s] (c : P) :
  ...
  apply ext_of_direction_eq (by simp)
  use c
...
theorem AffineSubspace.shift_eq_map_homothety (s : AffineSubspace k P) (c : P) {r : k}
  ...
  · refine ⟨t • (x -ᵥ h.some.val) +ᵥ h.some.val, ?_, ?_⟩
    ...
    · rw [vadd_vsub_assoc, smul_add, smul_smul, ht,
        sub_eq_add_neg, add_smul, one_smul, one_smul, neg_smul, ← smul_neg, neg_vsub_eq_vsub_rev]
...
theorem Affine.Simplex.closedInterior_inter_shift
  ...
  classical
  by_cases! hx0 : x = 0
  · rw [hx0, AffineSubspace.shift_zero,
      closedInterior_inter_affineSubspaceMk'_affineSpan_faceOpposite]
    have : Set.Nonempty (s.faceOpposite i).closedInterior :=
      Affine.Simplex.nonempty_closedInterior _
    aesop
  have hx0' : 0 < x := lt_of_le_of_ne hx.1 hx0.symm
  obtain ⟨q, hq⟩ := Classical.arbitrary (affineSpan k (Set.range (s.faceOpposite i).points))
  rw [AffineSubspace.shift_eq_map_homothety _ _ (IsUnit.mk0 _ hx0)]
  ext p
  ...
  · have hhom (q : P) : homothety (s.points i) x q = p ↔ homothety (s.points i) x⁻¹ p = q :=
    ...
    rw [homothety_affineCombination _ _ _ (by simp)]
    rw [Affine.Simplex.affineCombination_mem_closedInterior_iff hw1]
    ...
    rw [Affine.Simplex.affineCombination_mem_closedInterior_face_iff_mem_Icc _ _ hw']
    have h1 : (∀ j ∈ ({i} : Finset _)ᶜ,
      ...
      · specialize h j (by simpa using hj)
        rw [Pi.add_apply, Pi.smul_apply,
          affineCombinationSingleWeights_apply_of_ne _ hj] at h
        simpa using h
      · specialize h j (by simpa using hj)
        rw [Pi.add_apply, Pi.smul_apply,
    ...
    rw [h1, h2, affineCombination_eq_weightedVSubOfPoint_vadd_of_sum_eq_one _ _ _ hw' q,
      AffineSubspace.vadd_mem_iff_mem_direction _ hq, weightedVSubOfPoint_apply,
      ← sum_erase_add _ _ (show i ∈ univ by simp), Submodule.add_mem_iff_right _ (sum_mem
      fun j hj ↦ smul_mem _ _ <| AffineSubspace.vsub_mem_direction (by simpa using hj) hq)]
    ...
    · have hi : 1 - x⁻¹ + x⁻¹ * w i = 0 := by
        by_contra! h
        rw [smul_mem_iff _ h, AffineSubspace.vsub_right_mem_direction_iff_mem hq] at h2
        simp at h2
