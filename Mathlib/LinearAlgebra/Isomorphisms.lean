...
@[reducible]
def aux' (p p' : Submodule R M) := (comap (p ⊔ p').subtype p').mkQ.comp (Submodule.ofLe le_sup_left)

theorem aux (p p' : Submodule R M) : comap (Submodule.subtype p) (p ⊓ p') ≤ ker (aux' p p') := by
