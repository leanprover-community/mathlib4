...
theorem det_eq_prod_roots_charpoly_of_splits (hAps : (Polynomial.map (RingHom.id K) A.charpoly).Splits) :
...
theorem trace_eq_sum_roots_charpoly_of_splits (hAps : (Polynomial.map (RingHom.id K) A.charpoly).Splits) :
