...
theorem eigenvectorMatrixInv_apply (i j : n) :
  ...
  rw [eigenvectorMatrixInv, Basis.toMatrix_apply, OrthonormalBasis.coe_toBasis_repr_apply,
    ...
    EuclideanSpace.inner_single_right, one_mul, ROrCLike.star_def]
...
theorem eigenvalues_eq (i : n) :
    hA.eigenvalues i =
      ROrCLike.re (star (hA.eigenvectorMatrixᵀ i) ⬝ᵥ A *ᵥ hA.eigenvectorMatrixᵀ i) := by
  have := hA.spectral_theorem
  rw [← @Matrix.mul_inv_eq_iff_eq_mul_of_invertible (A := hA.eigenvectorMatrixInv)] at this
  have := congr_arg ROrCLike.re (congr_fun (congr_fun this i) i)
  rw [diagonal_apply_eq, Function.comp_apply, ROrCLike.ofReal_re,
...
lemma mulVec_eigenvectorBasis (i : n) :
  ...
  rw [mul_comm, Pi.smul_apply, ROrCLike.real_smul_eq_coe_mul, hA.eigenvectorMatrix_apply]
...
lemma exists_eigenvector_of_ne_zero (hA : IsHermitian A) (h_ne : A ≠ 0) :
  ...
  have : hA.eigenvalues ≠ 0 := by
    contrapose! h_ne
    have := hA.spectral_theorem'
    rwa [h_ne, Pi.comp_zero, ROrCLike.ofReal_zero, (by rfl : Function.const n (0 : 𝕜) = fun _ ↦ 0),
