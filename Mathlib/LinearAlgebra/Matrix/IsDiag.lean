...
theorem isDiag_iff [Zero α] {A : Matrix n n α} : IsDiag A ↔ ∀ ⦃i j⦄, i ≠ j → A i j = 0 := by
  rw [IsDiag, pairwise'_iff]

@[simp]
theorem isDiag_diagonal [Zero α] [DecidableEq n] (d : n → α) : (diagonal d).IsDiag := fun _ _ _ _ =>
...
theorem IsDiag.diagonal_diag [Zero α] [DecidableEq n] {A : Matrix n n α} (h : A.IsDiag) :
  ...
  ext fun i j => by
    ...
    · rw [diagonal_apply_ne _ hij, h (Set.mem_univ i) (Set.mem_univ j) hij]
...
theorem isDiag_of_subsingleton [Zero α] [Subsingleton n] (A : Matrix n n α) : A.IsDiag :=
  fun i _ j _ h => (h <| Subsingleton.elim i j).elim

/-- Every zero matrix is diagonal. -/
@[simp]
theorem isDiag_zero [Zero α] : (0 : Matrix n n α).IsDiag := fun _ _ _ _ _ => rfl

/-- Every identity matrix is diagonal. -/
@[simp]
theorem isDiag_one [DecidableEq n] [Zero α] [One α] : (1 : Matrix n n α).IsDiag := fun _ _ _ _=>
  one_apply_ne

theorem IsDiag.map [Zero α] [Zero β] {A : Matrix n n α} (ha : A.IsDiag) {f : α → β} (hf : f 0 = 0) :
    (A.map f).IsDiag := by
  intro i _ j _ h
  simp [isDiag_iff.mp ha h, hf]

theorem IsDiag.neg [SubtractionMonoid α] {A : Matrix n n α} (ha : A.IsDiag) : (-A).IsDiag := by
  intro i _ j _ h
  simp [isDiag_iff.mp ha h]

@[simp]
theorem isDiag_neg_iff [SubtractionMonoid α] {A : Matrix n n α} : (-A).IsDiag ↔ A.IsDiag := by
  refine ⟨?_ , IsDiag.neg⟩
  simp only [isDiag_iff]
  intro h i j hne
  simpa using h hne

theorem IsDiag.add [AddZeroClass α] {A B : Matrix n n α} (ha : A.IsDiag) (hb : B.IsDiag) :
    (A + B).IsDiag := by
  simp only [isDiag_iff] at *
  intro i j h
  simp [ha h, hb h]
...
theorem IsDiag.transpose [Zero α] {A : Matrix n n α} (ha : A.IsDiag) : Aᵀ.IsDiag := by
  simp only [isDiag_iff] at *
  exact fun _ _  h => ha h.symm
...
theorem IsDiag.submatrix [Zero α] {A : Matrix n n α} (ha : A.IsDiag) {f : m → n}
    (hf : Injective f) : (A.submatrix f f).IsDiag := by
  simp only [isDiag_iff] at *
  exact fun _ _ h => ha (hf.ne h)
...
theorem mul_transpose_self_isDiag_iff_hasOrthogonalRows [Fintype n] [Mul α] [AddCommMonoid α]
    {A : Matrix m n α} : (A * Aᵀ).IsDiag ↔ A.HasOrthogonalRows := by
  simp only [isDiag_iff] at *
  exact Iff.rfl

theorem transpose_mul_self_isDiag_iff_hasOrthogonalCols [Fintype m] [Mul α] [AddCommMonoid α]
    {A : Matrix m n α} : (Aᵀ * A).IsDiag ↔ A.HasOrthogonalCols :=by
  simp only [isDiag_iff] at *
  exact Iff.rfl
