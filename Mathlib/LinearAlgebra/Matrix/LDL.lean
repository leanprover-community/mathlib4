...
noncomputable def LDL.lowerInv : Matrix n n 𝕜 :=
  @gramSchmidt 𝕜 (n → 𝕜) _ (_ :) hS.transpose.innerProductSpace n _ _ _
    (Pi.basisFun 𝕜 n)

theorem LDL.lowerInv_eq_gramSchmidtBasis :
    LDL.lowerInv hS =
      ((Pi.basisFun 𝕜 n).toMatrix
          (@gramSchmidtBasis 𝕜 (n → 𝕜) _ (_ :) hS.transpose.innerProductSpace n _ _ _
...
theorem LDL.lowerInv_orthogonal {i j : n} (h₀ : i ≠ j) :
  ...
  @gramSchmidt_orthogonal 𝕜 _ _ (_ :) hS.transpose.innerProductSpace _ _ _ _ _ _ _ h₀
...
theorem LDL.lowerInv_triangular {i j : n} (hij : i < j) : LDL.lowerInv hS i j = 0 := by
  rw [←
    @gramSchmidt_triangular 𝕜 (n → 𝕜) _ (_ :) hS.transpose.innerProductSpace n _ _ _
