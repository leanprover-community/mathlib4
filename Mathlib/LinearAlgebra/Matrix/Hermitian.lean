...
lemma IsHermitian.fromBlocks {A : Matrix m m α} {B : Matrix m n α} {C : Matrix n m α}
...
lemma isHermitian_fromBlocks_iff {A : Matrix m m α} {B : Matrix m n α} {C : Matrix n m α}
    {D : Matrix n n α} :
...
lemma isHermitian_diagonal_of_self_adjoint [DecidableEq n] (v : n → α) (h : IsSelfAdjoint v) :
...
@[simp]
lemma isHermitian_diagonal [TrivialStar α] [DecidableEq n] (v : n → α) :
    (diagonal v).IsHermitian :=
...
lemma isHermitian_mul_conjTranspose_self [Fintype n] (A : Matrix m n α) : (A * Aᴴ).IsHermitian :=
...
lemma isHermitian_transpose_mul_self [Fintype m] (A : Matrix m n α) : (Aᴴ * A).IsHermitian := by
...
lemma isHermitian_conjTranspose_mul_mul [Fintype m] {A : Matrix m m α} (B : Matrix m n α)
...
/-- Note this is more general than `IsSelfAdjoint.conjugate` as `B` can be rectangular. -/
lemma isHermitian_mul_mul_conjTranspose [Fintype m] {A : Matrix m m α} (B : Matrix n m α)
...
@[simp]
lemma isHermitian_one [DecidableEq n] : (1 : Matrix n n α).IsHermitian :=
  conjTranspose_one
...
lemma IsHermitian.coe_re_apply_self {A : Matrix n n α} (h : A.IsHermitian) (i : n) :
...
/-- The diagonal elements of a complex hermitian matrix are real. -/
lemma IsHermitian.coe_re_diag {A : Matrix n n α} (h : A.IsHermitian) :
...
lemma isHermitian_iff_isSymmetric [Fintype n] [DecidableEq n] {A : Matrix n n α} :
