...
theorem isHermitian_iff_isSymmetric [Fintype n] [DecidableEq n] {A : Matrix n n α} :
  ...
  rw [LinearMap.IsSymmetric, (WithLp.toLp _).surjective.forall₂]
  simp only [toEuclideanLin_toLp, toLin'_apply, EuclideanSpace.inner_eq_star_dotProduct]
  simp_rw [← WithLp.symm_ofLp, Equiv.apply_symm_apply, star_mulVec]
  constructor
  · rintro (h : Aᴴ = A) x y
