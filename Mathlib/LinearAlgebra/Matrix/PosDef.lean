...
theorem re_dotProduct_nonneg {M : Matrix n n 𝕜} (hM : M.PosSemidef) (x : n → 𝕜) :
    0 ≤ ROrCLike.re (dotProduct (star x) (M *ᵥ x)) :=
  ROrCLike.nonneg_iff.mp (hM.2 _) |>.1
...
lemma posSemidef_sqrt : PosSemidef hA.sqrt := by
  ...
  rw [Function.comp_apply, ROrCLike.nonneg_iff]
  constructor
  · simp only [ROrCLike.ofReal_re]
    exact Real.sqrt_nonneg _
  · simp only [ROrCLike.ofReal_im]
...
lemma sq_sqrt : hA.sqrt ^ 2 = A := by
  ...
  have : E * E = diagonal ((↑) ∘ hA.1.eigenvalues) := by
    ...
    simp [← pow_two, ← ROrCLike.ofReal_pow, Real.sq_sqrt (hA.eigenvalues_nonneg v)]
  rw [this]
...
lemma eq_of_sq_eq_sq {B : Matrix n n 𝕜} (hB : PosSemidef B) (hAB : A ^ 2 = B ^ 2) : A = B := by
  ...
  have h_sum : 0 = t * (star v ⬝ᵥ A *ᵥ v + star v ⬝ᵥ B *ᵥ v) := calc
    ...
    _ = t * (star v ⬝ᵥ A *ᵥ v) + (star v) ᵥ* (A - B)ᴴ ⬝ᵥ B *ᵥ v := by
      rw [hv', mulVec_smul, dotProduct_smul, ROrCLike.real_smul_eq_coe_mul,
        dotProduct_mulVec _ (A - B), hA.1.sub hB.1]
    _ = t * (star v ⬝ᵥ A *ᵥ v + star v ⬝ᵥ B *ᵥ v) := by
      simp_rw [← star_mulVec, hv', mul_add, ← ROrCLike.real_smul_eq_coe_mul, ← smul_dotProduct]
      congr 2 with i
      simp only [Pi.star_apply, Pi.smul_apply, ROrCLike.real_smul_eq_coe_mul, star_mul',
        ROrCLike.star_def, ROrCLike.conj_ofReal]
  replace h_sum : star v ⬝ᵥ A *ᵥ v + star v ⬝ᵥ B *ᵥ v = 0 := by
    rw [eq_comm, ← mul_zero (t : 𝕜)] at h_sum
    exact mul_left_cancel₀ (ROrCLike.ofReal_ne_zero.mpr ht) h_sum
  ...
  rw [hv', dotProduct_smul, ROrCLike.real_smul_eq_coe_mul, ← mul_zero ↑t] at aux
  exact hv <| Matrix.dotProduct_star_self_eq_zero.mp <| mul_left_cancel₀
    (ROrCLike.ofReal_ne_zero.mpr ht) aux
...
theorem re_dotProduct_pos {M : Matrix n n 𝕜} (hM : M.PosDef) {x : n → 𝕜} (hx : x ≠ 0) :
    0 < ROrCLike.re (dotProduct (star x) (M *ᵥ x)) :=
  ROrCLike.pos_iff.mp (hM.2 _ hx) |>.1

theorem posSemidef {M : Matrix n n R} (hM : M.PosDef) : M.PosSemidef := by
  refine' ⟨hM.1, _⟩
  intro x
  by_cases hx : x = 0
  · simp only [hx, zero_dotProduct, star_zero, ROrCLike.zero_re']
    exact le_rfl
...
variable {𝕜 : Type*} [ROrCLike 𝕜] {n : Type*} [Fintype n]
