...
variable {ι R : Type*} [Fintype ι] [DecidableEq ι]
  ...
  (A : Matrix ι ι R) (d : ι → R) (hA : (A * diagonal d).PosDef) (hD : ∀ i, 0 < d i)
...
public lemma lt_two_mul_of_mul_diagonal_posDef_of_for_le_of_hasEigen
  ...
  obtain ⟨v, hv, hv₀⟩ : ∃ v, v ᵥ* N = (μ - ρ) • v ∧ v ≠ 0 := by
    replace hρ : HasEigenvalue (toLin' Nᵀ) (μ - ρ) := by
      simpa only [hasEigenvalue_toLin'_transpose_iff, N_def, map_sub, map_smul, toLin'_one,
        hasEigenvalue_sub'_iff, sub_sub_cancel]
    simpa [hasEigenvector_iff, mulVec_transpose] using hρ.exists_hasEigenvector
  set q := |v| ⬝ᵥ (N * diagonal d) *ᵥ |v| with q_def
  set Q := μ * ∑ i, d i * |v i| ^ 2 with Q_def
  have hq₀ : q < Q := by
    replace hv₀ : |v| ≠ 0 := by simpa
    have h₁ : 0 < ∑ j, ∑ i, |v i| * (A i j * d j) * |v j| := by
      simpa only [← mul_diagonal, ← dot_mulVec_eq_sum_sum, ← Pi.abs_apply, star_trivial] using
        hA.dotProduct_mulVec_pos hv₀
    have h₂ : Q = ∑ i, |v i| * (μ * d i) * |v i| := by
  ...
  have hq₁ : Q ≤ q := by
    ...
    have h₃ : (ρ - μ) * |v i| ≤ (|v| ᵥ* N) i := by
      calc (ρ - μ) * |v i|
          = |μ - ρ| * |v i| := by rw [← abs_eq_self.mpr h₀, abs_sub_comm]
        _ = |(v ᵥ* N) i| := by simp [hv]
        _ = |∑ j, v j * N j i| := by simp [vecMul_eq_sum]
        _ ≤ ∑ j, |v j * N j i| := Finset.abs_sum_le_sum_abs _ _
        _ = ∑ j, |v j| * N j i := by simp [abs_eq_self.mpr (hN _ i)]
        _ = (|v| ᵥ* N) i := by simp [vecMul_eq_sum]
    calc μ * (d i * |v i| ^ 2)
        = μ * |v i| * (d i * |v i|) := by ring
      _ ≤ (ρ - μ) * |v i| * (d i * |v i|) := by gcongr
      _ ≤ (|v| ᵥ* N) i * (d i * |v i|) := by gcongr
      _ = (|v| ᵥ* N) i * (diagonal d *ᵥ |v|) i := by simp [mulVec_eq_sum, diagonal_apply, mul_comm]
  rw [← not_lt] at hq₁
  contradiction
