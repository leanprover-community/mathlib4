...
theorem Matrix.toBilin'Aux_stdBasis [Fintype n] [DecidableEq n] (M : Matrix n n R₂) (i j : n) :
  ...
  -- ⊢ bilin { bilin := fun v w => ∑ i : n, ∑ j : n, v i * M i j * w j, bilin_add_l …
  dsimp only -- Porting note: had to add `dsimp only` to get rid of the projections
  -- ⊢ ∑ i_1 : n, ∑ j_1 : n, ↑(LinearMap.stdBasis R₂ (fun x => R₂) i) 1 i_1 * M i_1 …
  rw [sum_eq_single i, sum_eq_single j]
  · simp only [stdBasis_same, stdBasis_same, one_mul, mul_one]
    -- 🎉 no goals
  · rintro j' - hj'
    -- ⊢ ↑(LinearMap.stdBasis R₂ (fun x => R₂) i) 1 i * M i j' * ↑(LinearMap.stdBasis …
  ...
  · intros
    ...
    -- 🎉 no goals
  · rintro i' - hi'
    -- ⊢ ∑ j_1 : n, ↑(LinearMap.stdBasis R₂ (fun x => R₂) i) 1 i' * M i' j_1 * ↑(Line …
    refine' Finset.sum_eq_zero fun j _ => _
    -- ⊢ ↑(LinearMap.stdBasis R₂ (fun x => R₂) i) 1 i' * M i' j * ↑(LinearMap.stdBasi …
    ...
    -- 🎉 no goals
  · intros
    -- ⊢ ∑ j_1 : n, ↑(LinearMap.stdBasis R₂ (fun x => R₂) i) 1 i * M i j_1 * ↑(Linear …
...
theorem toBilin'Aux_toMatrixAux [DecidableEq n] (B₂ : BilinForm R₂ (n → R₂)) :
  ...
  -- ⊢ bilin (toBilin'Aux (↑(toMatrixAux fun j => ↑(LinearMap.stdBasis R₂ (fun x => …
...
theorem Matrix.toBilin'_apply' (M : Matrix n n R₂) (v w : n → R₂) :
  ...
  -- ⊢ ∑ i : n, ∑ j : n, v i * M i j * w j = ∑ x : n, v x * ∑ x_1 : n, M x x_1 * w  …
...
theorem BilinForm.toMatrix'_comp (B : BilinForm R₂ (n → R₂)) (l r : (o → R₂) →ₗ[R₂] n → R₂) :
  ...
  ext i j
  -- ⊢ ↑toMatrix' (comp B l r) i j = ((↑LinearMap.toMatrix' l)ᵀ * ↑toMatrix' B * ↑L …
  ...
  rw [sum_comm]
  -- ⊢ bilin B (↑l (↑(LinearMap.stdBasis R₂ (fun x => R₂) i) 1)) (↑r (↑(LinearMap.s …
  conv_lhs => rw [← BilinForm.sum_repr_mul_repr_mul (Pi.basisFun R₂ n) (l _) (r _)]
  -- ⊢ (Finsupp.sum (↑(Pi.basisFun R₂ n).repr (↑l (↑(LinearMap.stdBasis R₂ (fun x = …
  rw [Finsupp.sum_fintype]
  -- ⊢ (∑ i_1 : n, Finsupp.sum (↑(Pi.basisFun R₂ n).repr (↑r (↑(LinearMap.stdBasis  …
  · apply sum_congr rfl
    -- ⊢ ∀ (x : n), x ∈ univ → (Finsupp.sum (↑(Pi.basisFun R₂ n).repr (↑r (↑(LinearMa …
    rintro i' -
    -- ⊢ (Finsupp.sum (↑(Pi.basisFun R₂ n).repr (↑r (↑(LinearMap.stdBasis R₂ (fun x = …
    rw [Finsupp.sum_fintype]
    -- ⊢ ∑ i_1 : n, ↑(↑(Pi.basisFun R₂ n).repr (↑l (↑(LinearMap.stdBasis R₂ (fun x => …
    · apply sum_congr rfl
      -- ⊢ ∀ (x : n), x ∈ univ → ↑(↑(Pi.basisFun R₂ n).repr (↑l (↑(LinearMap.stdBasis R …
      rintro j' -
      -- ⊢ ↑(↑(Pi.basisFun R₂ n).repr (↑l (↑(LinearMap.stdBasis R₂ (fun x => R₂) i) 1)) …
...
theorem Matrix.toBilin_apply (M : Matrix n n R₂) (x y : M₂) :
  ...
  -- ⊢ bilin (↑(LinearEquiv.symm (BilinForm.congr (Basis.equivFun b))) (↑toBilin' M …
...
theorem BilinForm.toMatrix_comp (B : BilinForm R₂ M₂) (l r : M₂' →ₗ[R₂] M₂) :
  ...
  ext i j
  -- ⊢ ↑(toMatrix c) (comp B l r) i j = ((↑(LinearMap.toMatrix c b) l)ᵀ * ↑(toMatri …
  simp only [BilinForm.toMatrix_apply, BilinForm.comp_apply, transpose_apply, Matrix.mul_apply,
    LinearMap.toMatrix', LinearEquiv.coe_mk, sum_mul]
  rw [sum_comm]
  -- ⊢ bilin B (↑l (↑c i)) (↑r (↑c j)) = ∑ y : n, ∑ x : n, ↑(LinearMap.toMatrix c b …
  conv_lhs => rw [← BilinForm.sum_repr_mul_repr_mul b]
  -- ⊢ (Finsupp.sum (↑b.repr (↑l (↑c i))) fun i xi => Finsupp.sum (↑b.repr (↑r (↑c  …
  rw [Finsupp.sum_fintype]
  -- ⊢ (∑ i_1 : n, Finsupp.sum (↑b.repr (↑r (↑c j))) fun j yj => ↑(↑b.repr (↑l (↑c  …
  · apply sum_congr rfl
    -- ⊢ ∀ (x : n), x ∈ univ → (Finsupp.sum (↑b.repr (↑r (↑c j))) fun j yj => ↑(↑b.re …
    rintro i' -
    -- ⊢ (Finsupp.sum (↑b.repr (↑r (↑c j))) fun j yj => ↑(↑b.repr (↑l (↑c i))) i' • y …
    rw [Finsupp.sum_fintype]
    -- ⊢ ∑ i_1 : n, ↑(↑b.repr (↑l (↑c i))) i' • ↑(↑b.repr (↑r (↑c j))) i_1 • bilin B  …
    · apply sum_congr rfl
      -- ⊢ ∀ (x : n), x ∈ univ → ↑(↑b.repr (↑l (↑c i))) i' • ↑(↑b.repr (↑r (↑c j))) x • …
      rintro j' -
      -- ⊢ ↑(↑b.repr (↑l (↑c i))) i' • ↑(↑b.repr (↑r (↑c j))) j' • bilin B (↑b i') (↑b  …
    ...
    · intros
      ...
      -- 🎉 no goals
  · intros
    -- ⊢ (Finsupp.sum (↑b.repr (↑r (↑c j))) fun j yj => 0 • yj • bilin B (↑b i✝) (↑b  …
    simp only [zero_smul, Finsupp.sum_zero]
    -- 🎉 no goals
...
theorem isAdjointPair_toBilin' [DecidableEq n] :
  ...
  -- ⊢ BilinForm.compLeft (↑toBilin' J₃) (↑toLin' A) = BilinForm.compRight (↑toBili …
  have h :
...
theorem isAdjointPair_toBilin [DecidableEq n] :
    BilinForm.IsAdjointPair (Matrix.toBilin b J) (Matrix.toBilin b J₃) (Matrix.toLin b b A)
      ...
      Matrix.IsAdjointPair J J₃ A A' := by
  rw [BilinForm.isAdjointPair_iff_compLeft_eq_compRight]
  -- ⊢ BilinForm.compLeft (↑(toBilin b) J₃) (↑(toLin b b) A) = BilinForm.compRight  …
...
theorem Matrix.isAdjointPair_equiv' [DecidableEq n] (P : Matrix n n R₃) (h : IsUnit P) :
  ...
  -- ⊢ IsAdjointPair (Pᵀ * J * P) (Pᵀ * J * P) A A' ↔ IsAdjointPair J J (P * A * P⁻ …
  ...
  · rw [x_def, y_def, coe_u_inv, coe_v_inv]
    -- ⊢ P⁻¹ᵀ * (Aᵀ * Pᵀ * J) = J * P * A' * P⁻¹ ↔ (P * A * P⁻¹)ᵀ * J = J * (P * A' * …
