...
@[simp]
lemma BilinForm.toMatrixAux_apply (B : BilinForm R₂ M₂) (b : n → M₂) (i j : n) :
...
@[simp]
lemma BilinForm.toMatrixAux_stdBasis (B : BilinForm R₂ (n → R₂)) :
    -- porting note: had to hint the base ring even though it should be clear from context...
...
@[simp]
lemma BilinForm.toMatrix'_symm :
...
lemma nondegenerate_of_det_ne_zero (b : Basis ι A M₃) (h : (toMatrix b B₃).det ≠ 0) :
    B₃.Nondegenerate :=
