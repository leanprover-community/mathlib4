...
lemma Matrix.toBilin'Aux_stdBasis [Fintype n] [DecidableEq n] (M : Matrix n n R₂) (i j : n) :
...
@[simp]
lemma LinearMap.BilinForm.toMatrixAux_apply (B : BilinForm R₂ M₂) (b : n → M₂) (i j : n) :
...
lemma toBilin'Aux_toMatrixAux [DecidableEq n] (B₂ : BilinForm R₂ (n → R₂)) :
    -- Porting note: had to hint the base ring even though it should be clear from context...
...
@[simp]
lemma LinearMap.BilinForm.toMatrixAux_stdBasis (B : BilinForm R₂ (n → R₂)) :
    -- Porting note: had to hint the base ring even though it should be clear from context...
...
@[simp]
lemma LinearMap.BilinForm.toMatrix'_symm :
...
@[simp]
lemma Matrix.toBilin'_toMatrix' (B : BilinForm R₂ (n → R₂)) :
...
@[simp]
lemma BilinForm.toMatrix'_toBilin' (M : Matrix n n R₂) :
...
@[simp]
lemma BilinForm.toMatrix'_apply (B : BilinForm R₂ (n → R₂)) (i j : n) :
...
@[simp]
lemma BilinForm.toMatrix'_comp (B : BilinForm R₂ (n → R₂)) (l r : (o → R₂) →ₗ[R₂] n → R₂) :
...
lemma BilinForm.toMatrix'_compLeft (B : BilinForm R₂ (n → R₂)) (f : (n → R₂) →ₗ[R₂] n → R₂) :
...
lemma BilinForm.toMatrix'_compRight (B : BilinForm R₂ (n → R₂)) (f : (n → R₂) →ₗ[R₂] n → R₂) :
...
lemma BilinForm.mul_toMatrix'_mul (B : BilinForm R₂ (n → R₂)) (M : Matrix o n R₂)
...
lemma BilinForm.mul_toMatrix' (B : BilinForm R₂ (n → R₂)) (M : Matrix n n R₂) :
...
lemma BilinForm.toMatrix'_mul (B : BilinForm R₂ (n → R₂)) (M : Matrix n n R₂) :
...
lemma Matrix.toBilin'_comp (M : Matrix n n R₂) (P Q : Matrix n o R₂) :
...
@[simp]
lemma BilinForm.toMatrix_apply (B : BilinForm R₂ M₂) (i j : n) :
...
@[simp]
lemma Matrix.toBilin_apply (M : Matrix n n R₂) (x y : M₂) :
...
lemma BilinearForm.toMatrixAux_eq (B : BilinForm R₂ M₂) :
...
@[simp]
lemma BilinForm.toMatrix_symm : (BilinForm.toMatrix b).symm = Matrix.toBilin b :=
  rfl
#align bilin_form.to_matrix_symm BilinForm.toMatrix_symm

@[simp]
lemma Matrix.toBilin_symm : (Matrix.toBilin b).symm = BilinForm.toMatrix b :=
  (BilinForm.toMatrix b).symm_symm
#align matrix.to_bilin_symm Matrix.toBilin_symm

lemma Matrix.toBilin_basisFun : Matrix.toBilin (Pi.basisFun R₂ n) = Matrix.toBilin' := by
  ext M
...
lemma BilinForm.toMatrix_basisFun :
...
@[simp]
lemma Matrix.toBilin_toMatrix (B : BilinForm R₂ M₂) :
...
@[simp]
lemma BilinForm.toMatrix_toBilin (M : Matrix n n R₂) :
...
lemma BilinForm.toMatrix_comp (B : BilinForm R₂ M₂) (l r : M₂' →ₗ[R₂] M₂) :
...
lemma BilinForm.toMatrix_compLeft (B : BilinForm R₂ M₂) (f : M₂ →ₗ[R₂] M₂) :
...
lemma BilinForm.toMatrix_compRight (B : BilinForm R₂ M₂) (f : M₂ →ₗ[R₂] M₂) :
...
@[simp]
lemma BilinForm.toMatrix_mul_basis_toMatrix (c : Basis o R₂ M₂) (B : BilinForm R₂ M₂) :
...
lemma BilinForm.mul_toMatrix_mul (B : BilinForm R₂ M₂) (M : Matrix o n R₂) (N : Matrix n o R₂) :
...
lemma BilinForm.mul_toMatrix (B : BilinForm R₂ M₂) (M : Matrix n n R₂) :
...
lemma BilinForm.toMatrix_mul (B : BilinForm R₂ M₂) (M : Matrix n n R₂) :
...
lemma Matrix.toBilin_comp (M : Matrix n n R₂) (P Q : Matrix n o R₂) :
...
@[simp]
lemma isAdjointPair_toBilin' [DecidableEq n] :
...
lemma Matrix.isAdjointPair_equiv' [DecidableEq n] (P : Matrix n n R₃) (h : IsUnit P) :
...
lemma mem_pairSelfAdjointMatricesSubmodule' :
...
lemma mem_selfAdjointMatricesSubmodule' :
...
lemma mem_skewAdjointMatricesSubmodule' :
...
lemma _root_.Matrix.nondegenerate_toBilin'_iff_nondegenerate_toBilin {M : Matrix ι ι R₂}
...
lemma _root_.Matrix.Nondegenerate.toBilin' {M : Matrix ι ι R₃} (h : M.Nondegenerate) :
...
@[simp]
lemma _root_.Matrix.nondegenerate_toBilin'_iff {M : Matrix ι ι R₃} :
...
@[simp]
lemma _root_.Matrix.nondegenerate_toBilin_iff {M : Matrix ι ι R₃} (b : Basis ι R₃ M₃) :
...
@[simp]
lemma nondegenerate_toMatrix'_iff {B : BilinForm R₃ (ι → R₃)} :
...
lemma Nondegenerate.toMatrix' {B : BilinForm R₃ (ι → R₃)} (h : B.Nondegenerate) :
...
@[simp]
lemma nondegenerate_toMatrix_iff {B : BilinForm R₃ M₃} (b : Basis ι R₃ M₃) :
...
lemma Nondegenerate.toMatrix {B : BilinForm R₃ M₃} (h : B.Nondegenerate) (b : Basis ι R₃ M₃) :
...
lemma nondegenerate_toBilin'_iff_det_ne_zero {M : Matrix ι ι A} :
...
lemma nondegenerate_toBilin'_of_det_ne_zero' (M : Matrix ι ι A) (h : M.det ≠ 0) :
...
lemma nondegenerate_iff_det_ne_zero {B : BilinForm A M₃} (b : Basis ι A M₃) :
...
lemma nondegenerate_of_det_ne_zero (b : Basis ι A M₃) (h : (BilinForm.toMatrix b B₃).det ≠ 0) :
    B₃.Nondegenerate :=
