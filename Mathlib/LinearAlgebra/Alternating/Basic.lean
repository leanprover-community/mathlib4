...
structure AlternatingMap extends MultilinearMap (.id R) (fun _ : ι => M) N where
...
@[simp]
theorem coe_mk (f : MultilinearMap (.id R) (fun _ : ι => M) N) (h) :
    ⇑(⟨f, h⟩ : M [⋀^ι]→ₗ[R] N) = f :=
  rfl
...
instance instCoe : Coe (M [⋀^ι]→ₗ[R] N) (MultilinearMap (.id R) (fun _ : ι => M) N) :=
  ⟨fun x => x.toMultilinearMap⟩

@[simp, norm_cast]
theorem coe_multilinearMap : ⇑(f : MultilinearMap (.id R) (fun _ : ι => M) N) = f :=
  rfl

theorem coe_multilinearMap_injective :
    Function.Injective ((↑) : M [⋀^ι]→ₗ[R] N → MultilinearMap (.id R) (fun _ : ι => M) N) :=
  fun _ _ h => ext <| MultilinearMap.congr_fun h

theorem coe_multilinearMap_mk (f : (ι → M) → N) (h₁ h₂ h₃) :
    ((⟨⟨f, h₁, h₂⟩, h₃⟩ : M [⋀^ι]→ₗ[R] N) : MultilinearMap (.id R) (fun _ : ι => M) N) =
      ⟨f, @h₁, @h₂⟩ := by
  simp
...
instance instSMul : SMul S (M [⋀^ι]→ₗ[R] N) :=
  ⟨fun c f =>
    { c • (f : MultilinearMap (.id R) (fun _ : ι => M) N) with
...
@[norm_cast]
theorem coe_smul (c : S) : ↑(c • f) = c • (f : MultilinearMap (.id R) (fun _ : ι => M) N) :=
  rfl
...
theorem coe_prod (f : M [⋀^ι]→ₗ[R] N) (g : M [⋀^ι]→ₗ[R] P) :
    (f.prod g : MultilinearMap (.id R) (fun _ : ι => M) (N × P)) = MultilinearMap.prod f g :=
  rfl
...
theorem coe_pi {ι' : Type*} {N : ι' → Type*} [∀ i, AddCommMonoid (N i)] [∀ i, Module R (N i)]
    (f : ∀ i, M [⋀^ι]→ₗ[R] N i) :
    (pi f : MultilinearMap (.id R) (fun _ : ι => M) (∀ i, N i)) = MultilinearMap.pi fun a => f a :=
  rfl
...
theorem coe_smulRight {R M₁ M₂ ι : Type*} [CommSemiring R] [AddCommMonoid M₁] [AddCommMonoid M₂]
    ...
    (f.smulRight z : MultilinearMap (.id R) (fun _ : ι => M₁) M₂) = MultilinearMap.smulRight f z :=
  rfl

instance instAdd : Add (M [⋀^ι]→ₗ[R] N) where
  add a b :=
    { (a + b : MultilinearMap (.id R) (fun _ : ι => M) N) with
...
@[norm_cast]
theorem coe_add : (↑(f + f') : MultilinearMap (.id R) (fun _ : ι => M) N) = f + f' :=
  rfl

instance instZero : Zero (M [⋀^ι]→ₗ[R] N) :=
  ⟨{ (0 : MultilinearMap (.id R) (fun _ : ι => M) N) with
...
@[norm_cast]
theorem coe_zero : ((0 : M [⋀^ι]→ₗ[R] N) : MultilinearMap (.id R) (fun _ : ι => M) N) = 0 :=
  rfl

@[simp]
theorem mk_zero :
    mk (0 : MultilinearMap (.id R) (fun _ : ι ↦ M) N) (0 : M [⋀^ι]→ₗ[R] N).2 = 0 :=
  rfl
...
instance instNeg : Neg (M [⋀^ι]→ₗ[R] N') :=
  ⟨fun f =>
    { -(f : MultilinearMap (.id R) (fun _ : ι => M) N') with
      map_eq_zero_of_eq' := fun v i j h hij => by simp [f.map_eq_zero_of_eq v h hij] }⟩
...
@[norm_cast]
theorem coe_neg : ((-g : M [⋀^ι]→ₗ[R] N') : MultilinearMap (.id R) (fun _ : ι => M) N') = -g :=
  rfl

instance instSub : Sub (M [⋀^ι]→ₗ[R] N') :=
  ⟨fun f g =>
    { (f - g : MultilinearMap (.id R) (fun _ : ι => M) N') with
      map_eq_zero_of_eq' := fun v i j h hij => by
...
@[norm_cast]
theorem coe_sub : (↑(g - g₂) : MultilinearMap (.id R) (fun _ : ι => M) N') = g - g₂ :=
  rfl
...
@[simps]
def toMultilinearMapLM : (M [⋀^ι]→ₗ[R] N) →ₗ[S] MultilinearMap (.id R) (fun _ : ι ↦ M) N where
...
def ofSubsingleton [Subsingleton ι] (i : ι) : (M →ₗ[R] N) ≃ (M [⋀^ι]→ₗ[R] N) where
  toFun f := ⟨MultilinearMap.ofSubsingleton _ M N i f, fun _ _ _ _ ↦ absurd (Subsingleton.elim _ _)⟩
  invFun f := (MultilinearMap.ofSubsingleton _ M N i).symm f
  right_inv _ := coe_multilinearMap_injective <|
    (MultilinearMap.ofSubsingleton _ M N i).apply_symm_apply _
...
def constOfIsEmpty [IsEmpty ι] (m : N) : M [⋀^ι]→ₗ[R] N :=
  { MultilinearMap.constOfIsEmpty _ _ m with
...
def compAlternatingMap (g : N →ₗ[R] N₂) (f : M [⋀^ι]→ₗ[R] N) : M [⋀^ι]→ₗ[R] N₂ where
  __ := g.compMultilinearMap (f : MultilinearMap (.id R) (fun _ : ι => M) N)
...
def compLinearMap (f : M [⋀^ι]→ₗ[R] N) (g : M₂ →ₗ[R] M) : M₂ [⋀^ι]→ₗ[R] N :=
  { (f : MultilinearMap (.id R) (fun _ : ι => M) N).compLinearMap fun _ => g with
...
theorem coe_domDomCongr (σ : ι ≃ ι') :
    ↑(f.domDomCongr σ) = (f : MultilinearMap (.id R) (fun _ : ι => M) N).domDomCongr σ :=
...
private theorem alternization_map_eq_zero_of_eq_aux (m : MultilinearMap (.id R) (fun _ : ι => M) N')
...
permutations. -/
def alternatization : MultilinearMap (.id R) (fun _ : ι => M) N' →+ M [⋀^ι]→ₗ[R] N' where
  toFun m :=
...
theorem alternatization_def (m : MultilinearMap (.id R) (fun _ : ι => M) N') :
    ⇑(alternatization m) = (∑ σ : Perm ι, Equiv.Perm.sign σ • m.domDomCongr σ :) :=
  rfl

theorem alternatization_coe (m : MultilinearMap (.id R) (fun _ : ι => M) N') :
    ↑(alternatization m) = (∑ σ : Perm ι, Equiv.Perm.sign σ • m.domDomCongr σ :) :=
  coe_injective rfl

theorem alternatization_apply (m : MultilinearMap (.id R) (fun _ : ι => M) N') (v : ι → M) :
...
theorem coe_alternatization [DecidableEq ι] [Fintype ι] (a : M [⋀^ι]→ₗ[R] N') :
    MultilinearMap.alternatization (a : MultilinearMap (.id R) (fun _ => M) N')
