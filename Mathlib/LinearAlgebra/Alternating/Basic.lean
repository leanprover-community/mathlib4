...
structure AlternatingMap extends (fun _ : ι ↦ M) →ₗₘ[R] N where
...
@[simp]
theorem coe_mk (f : (fun _ : ι ↦ M) →ₗₘ[R] N) (h) :
    ⇑(⟨f, h⟩ : M [⋀^ι]→ₗ[R] N) = f :=
  rfl
...
instance instCoe : Coe (M [⋀^ι]→ₗ[R] N) ((fun _ : ι ↦ M) →ₗₘ[R] N) :=
  ⟨fun x => x.toMultilinearMap⟩

@[simp, norm_cast]
theorem coe_multilinearMap : ⇑(f : (fun _ : ι ↦ M) →ₗₘ[R] N) = f :=
  rfl

theorem coe_multilinearMap_injective :
    Function.Injective ((↑) : M [⋀^ι]→ₗ[R] N → (fun _ : ι ↦ M) →ₗₘ[R] N) :=
  fun _ _ h => ext <| MultilinearMap.congr_fun h

theorem coe_multilinearMap_mk (f : (ι → M) → N) (h₁ h₂ h₃) :
    ((⟨⟨f, h₁, h₂⟩, h₃⟩ : M [⋀^ι]→ₗ[R] N) : (fun _ : ι ↦ M) →ₗₘ[R] N) =
      ⟨f, @h₁, @h₂⟩ := by
  simp
...
@[norm_cast]
theorem coe_smul (c : S) : ↑(c • f) = c • (f : (fun _ : ι ↦ M) →ₗₘ[R] N) :=
  rfl
...
theorem coe_smulRight {R M₁ M₂ ι : Type*} [CommSemiring R] [AddCommMonoid M₁] [AddCommMonoid M₂]
    ...
    (f.smulRight z : (fun _ : ι ↦ M₁) →ₗₘ[R] M₂) = MultilinearMap.smulRight f z :=
  rfl

instance instAdd : Add (M [⋀^ι]→ₗ[R] N) where
  add a b :=
    { (a + b : (fun _ : ι ↦ M) →ₗₘ[R] N) with
...
@[norm_cast]
theorem coe_add : (↑(f + f') : (fun _ : ι ↦ M) →ₗₘ[R] N) = f + f' :=
  rfl

instance instZero : Zero (M [⋀^ι]→ₗ[R] N) :=
  ⟨{ (0 : (fun _ : ι ↦ M) →ₗₘ[R] N) with
...
@[norm_cast]
theorem coe_zero : ((0 : M [⋀^ι]→ₗ[R] N) : (fun _ : ι ↦ M) →ₗₘ[R] N) = 0 :=
  rfl

@[simp]
theorem mk_zero : mk (0 : (fun _ : ι ↦ M) →ₗₘ[R] N) (0 : M [⋀^ι]→ₗ[R] N).2 = 0 :=
  rfl
...
@[norm_cast]
theorem coe_neg : ((-g : M [⋀^ι]→ₗ[R] N') : (fun _ : ι ↦ M) →ₗₘ[R] N') = -g :=
  rfl

instance instSub : Sub (M [⋀^ι]→ₗ[R] N') :=
  ⟨fun f g =>
    { (f - g : (fun _ : ι ↦ M) →ₗₘ[R] N') with
      map_eq_zero_of_eq' := fun v i j h hij => by
...
@[norm_cast]
theorem coe_sub : (↑(g - g₂) : (fun _ : ι ↦ M) →ₗₘ[R] N') = g - g₂ :=
  rfl
...
@[simps]
def toMultilinearMapLM : (M [⋀^ι]→ₗ[R] N) →ₗ[S] (fun _ : ι ↦ M) →ₗₘ[R] N where
...
def compAlternatingMap (g : N →ₗ[R] N₂) (f : M [⋀^ι]→ₗ[R] N) : M [⋀^ι]→ₗ[R] N₂ where
  __ := g.compMultilinearMap (f : (fun _ : ι ↦ M) →ₗₘ[R] N)
...
def compLinearMap (f : M [⋀^ι]→ₗ[R] N) (g : M₂ →ₗ[R] M) : M₂ [⋀^ι]→ₗ[R] N :=
  { (f : (fun _ : ι ↦ M) →ₗₘ[R] N).compLinearMap fun _ => g with
...
theorem coe_domDomCongr (σ : ι ≃ ι') :
    ↑(f.domDomCongr σ) = (f : (fun _ : ι ↦ M) →ₗₘ[R] N).domDomCongr σ :=
...
theorem alternatization_def (m : (fun _ : ι ↦ M) →ₗₘ[R] N') :
    ⇑(alternatization m) = (∑ σ : Perm ι, Equiv.Perm.sign σ • m.domDomCongr σ :) :=
  rfl

theorem alternatization_coe (m : (fun _ : ι ↦ M) →ₗₘ[R] N') :
    ↑(alternatization m) = (∑ σ : Perm ι, Equiv.Perm.sign σ • m.domDomCongr σ :) :=
  coe_injective rfl

theorem alternatization_apply (m : (fun _ : ι ↦ M) →ₗₘ[R] N') (v : ι → M) :
    alternatization m v = ∑ σ : Perm ι, Equiv.Perm.sign σ • m.domDomCongr σ v := by
  simp only [alternatization_def, smul_apply, sum_apply]

@[simp]
theorem alternatization_compLinearMap (f : (fun _ : ι ↦ M') →ₗₘ[R] N') (g : M →ₗ[R] M') :
...
theorem coe_alternatization [DecidableEq ι] [Fintype ι] (a : M [⋀^ι]→ₗ[R] N') :
    MultilinearMap.alternatization (a : (fun _ : ι ↦ M) →ₗₘ[R] N')
...
theorem compMultilinearMap_alternatization (g : N' →ₗ[R] N'₂) (f : (fun _ : ι ↦ M) →ₗₘ[R] N') :
