...
structure AlternatingMap extends M [^ι]→ₗ[R] N where
...
instance instCoe : Coe (M [⋀^ι]→ₗ[R] N) (M [^ι]→ₗ[R] N) :=
  ⟨fun x => x.toMultilinearMap⟩

@[simp, norm_cast]
theorem coe_multilinearMap : ⇑(f : M [^ι]→ₗ[R] N) = f :=
  rfl

theorem coe_multilinearMap_injective :
    Function.Injective ((↑) : M [⋀^ι]→ₗ[R] N → M [^ι]→ₗ[R] N) :=
  fun _ _ h => ext <| MultilinearMap.congr_fun h

theorem coe_multilinearMap_mk (f : (ι → M) → N) (h₁ h₂ h₃) :
    ((⟨⟨f, h₁, h₂⟩, h₃⟩ : M [⋀^ι]→ₗ[R] N) : M [^ι]→ₗ[R] N) =
      ⟨f, @h₁, @h₂⟩ := by
  simp
...
@[norm_cast]
theorem coe_smul (c : S) : ↑(c • f) = c • (f : M [^ι]→ₗ[R] N) :=
  rfl
...
theorem coe_prod (f : M [⋀^ι]→ₗ[R] N) (g : M [⋀^ι]→ₗ[R] P) :
    (f.prod g : M [^ι]→ₗ[R] (N × P)) = MultilinearMap.prod f g :=
  rfl
...
theorem coe_pi {ι' : Type*} {N : ι' → Type*} [∀ i, AddCommMonoid (N i)] [∀ i, Module R (N i)]
    (f : ∀ i, M [⋀^ι]→ₗ[R] N i) :
    (pi f : M [^ι]→ₗ[R] (∀ i, N i)) = MultilinearMap.pi fun a => f a :=
  rfl
...
theorem coe_smulRight {R M₁ M₂ ι : Type*} [CommSemiring R] [AddCommMonoid M₁] [AddCommMonoid M₂]
    ...
    (f.smulRight z : M₁ [^ι]→ₗ[R] M₂) = MultilinearMap.smulRight f z :=
  rfl
...
@[norm_cast]
theorem coe_add : (↑(f + f') : M [^ι]→ₗ[R] N) = f + f' :=
  rfl
...
@[norm_cast]
theorem coe_zero : ((0 : M [⋀^ι]→ₗ[R] N) : M [^ι]→ₗ[R] N) = 0 :=
  rfl

@[simp]
theorem mk_zero : mk (0 : M [^ι]→ₗ[R] N) (0 : M [⋀^ι]→ₗ[R] N).2 = 0 :=
  rfl
...
@[norm_cast]
theorem coe_neg : ((-g : M [⋀^ι]→ₗ[R] N') : M [^ι]→ₗ[R] N') = -g :=
  rfl
...
@[norm_cast]
theorem coe_sub : (↑(g - g₂) : M [^ι]→ₗ[R] N') = g - g₂ :=
  rfl
...
@[simps]
def toMultilinearMapLM : (M [⋀^ι]→ₗ[R] N) →ₗ[S] M [^ι]→ₗ[R] N where
...
def compAlternatingMap (g : N →ₗ[R] N₂) (f : M [⋀^ι]→ₗ[R] N) : M [⋀^ι]→ₗ[R] N₂ where
  __ := g.compMultilinearMap (f : M [^ι]→ₗ[R] N)
...
def compLinearMap (f : M [⋀^ι]→ₗ[R] N) (g : M₂ →ₗ[R] M) : M₂ [⋀^ι]→ₗ[R] N :=
  { (f : M [^ι]→ₗ[R] N).compLinearMap fun _ => g with
...
theorem coe_domDomCongr (σ : ι ≃ ι') :
    ↑(f.domDomCongr σ) = (f : M [^ι]→ₗ[R] N).domDomCongr σ :=
...
private theorem alternization_map_eq_zero_of_eq_aux (m : M [^ι]→ₗ[R] N')
...
permutations. -/
def alternatization : M [^ι]→ₗ[R] N' →+ M [⋀^ι]→ₗ[R] N' where
  toFun m :=
...
theorem alternatization_def (m : M [^ι]→ₗ[R] N') :
    ⇑(alternatization m) = (∑ σ : Perm ι, Equiv.Perm.sign σ • m.domDomCongr σ :) :=
  rfl

theorem alternatization_coe (m : M [^ι]→ₗ[R] N') :
    ↑(alternatization m) = (∑ σ : Perm ι, Equiv.Perm.sign σ • m.domDomCongr σ :) :=
  coe_injective rfl

theorem alternatization_apply (m : M [^ι]→ₗ[R] N') (v : ι → M) :
...
theorem coe_alternatization [DecidableEq ι] [Fintype ι] (a : M [⋀^ι]→ₗ[R] N') :
    MultilinearMap.alternatization (a : M [^ι]→ₗ[R] N')
