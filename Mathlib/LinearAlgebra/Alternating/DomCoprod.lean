...
theorem domCoprod.summand_add_swap_smul_eq_zero (a : Mᵢ [⋀^ιa]→ₗ[R'] N₁)
  ...
  simp only [smul_apply, MultilinearMap.domDomCongr_apply, MultilinearMap.domCoprod_apply,
    coe_multilinearMap, neg_mul, one_mul, Units.neg_smul, _root_.neg_apply, Perm.coe_mul,
    Function.comp_apply]
...
theorem domCoprod.summand_eq_zero_of_smul_invariant (a : Mᵢ [⋀^ιa]→ₗ[R'] N₁)
  ...
  refine Quotient.inductionOn' σ fun σ => ?_
  dsimp only [Quotient.liftOn'_mk'', Quotient.map'_mk'', smul_apply,
...
def domCoprod' :
  ...
  TensorProduct.lift <| by
    ...
    · ext
      ...
      ext σ
      refine Quotient.inductionOn' σ fun σ => ?_
      simp only [Quotient.liftOn'_mk'', coe_add', coe_smul', smul_apply,
