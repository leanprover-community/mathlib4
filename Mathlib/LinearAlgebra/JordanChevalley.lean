...
theorem exists_isNilpotent_isSemisimple_of_separable_of_dvd_pow {P : K[X]} {n : ℕ}
    (sep : P.Separable) (nil : minpoly K f ∣ P ^ n) :
    ∃ s ∈ adjoin K {f}, IsNilpotent (f - s) ∧ s.IsSemisimple := by
  ...
  have sep' : IsUnit (aeval ff P') := by
    obtain ⟨a, b, h⟩ : IsCoprime (P ^ n) P' := sep.pow_left
    ...
    refine isUnit_of_mul_eq_one' (aeval ff b) _ (Subtype.ext_iff.mpr ?_)
    simpa [coe_aeval_mk_apply] using h
  obtain ⟨⟨s, mem⟩, ⟨⟨n, hn⟩, hss⟩, -⟩ := exists_unique_nilpotent_sub_and_aeval_eq_zero nil' sep'
  refine ⟨s, ⟨mem, ⟨n, ?_⟩, ?_⟩⟩
  · rw [Subtype.ext_iff] at hn
    simpa using hn
...
theorem exists_isNilpotent_isSemisimple [PerfectField K] :
    ∃ s ∈ adjoin K {f}, IsNilpotent (f - s) ∧ s.IsSemisimple := by
  obtain ⟨g, n, sep, -, nil⟩ := exists_squarefree_dvd_pow_of_ne_zero (minpoly.ne_zero_of_finite K f)
