...
instance : CoeFun (E →L.[R] F) fun f ↦ f.domain → F := ⟨fun f ↦ f.toFun⟩
...
@[simp]
theorem map_zero (f : E →L.[R] F) : f 0 = 0 := by
  exact LinearPMap.map_zero f

theorem map_add (f : E →L.[R] F) (x y : f.domain) : f (x + y) = f x + f y :=
  LinearPMap.map_add f.toLinearPMap x y
