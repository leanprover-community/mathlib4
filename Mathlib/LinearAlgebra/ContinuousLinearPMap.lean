...
/-- A `ContinuousLinearPMap R E F` or `E →L.[R] F` is
a continuous linear map from a submodule of `E` to `F`. -/
structure ContinuousLinearPMap (R : Type*) [Ring R]
  (E : Type*) [TopologicalSpace E] [AddCommGroup E] [Module R E]
  (F : Type*) [TopologicalSpace F] [AddCommGroup F] [Module R F] extends E →ₗ.[R] F where
  cont : Continuous toFun


@[inherit_doc] notation:25 E " →L.[" R:25 "] " F:0 => ContinuousLinearPMap R E F

variable {R : Type*} [Ring R] {E : Type*} [TopologicalSpace E] [AddCommGroup E] [Module R E]
...
attribute [coe] toLinearPMap
/-- Coerce partial continuous linear maps to partial linear maps. -/
instance LinearPMap.coe : Coe (E →L.[R] F) (E →ₗ.[R] F) := ⟨toLinearPMap⟩

@[coe]
def toFun' (f : E →L.[R] F) : f.domain → F := f.toFun

instance : CoeFun (E →L.[R] F) fun f ↦ f.domain → F := ⟨toFun'⟩

@[simp]
theorem toFun_eq_coe (f : E →L.[R] F) (x : f.domain) : f.toFun x = f x := rfl

@[ext (iff := false)]
theorem ext {f g : E →L.[R] F} (h : f.domain = g.domain)
    (h' : ∀ ⦃x : f.domain⦄ ⦃y : g.domain⦄ (_h : (x : E) = y), f x = g y) : f = g := by
  rcases f with ⟨f, _⟩
  rcases g with ⟨g, _⟩
  obtain rfl : f = g := LinearPMap.ext h h'
  rfl

@[simp]
theorem map_zero (f : E →L.[R] F) : f 0 = 0 := f.toFun.map_zero

theorem ext_iff {f g : E →L.[R] F} :
    f = g ↔
...
theorem ext' {s : Submodule R E} {f g : s →L[R] F} (h : f = g) :
    (mk ⟨s, f⟩ f.continuous : E →L.[R] F) = mk ⟨s, g⟩ g.continuous :=
  h ▸ rfl

theorem map_add (f : E →L.[R] F) (x y : f.domain) : f (x + y) = f x + f y :=
  f.toFun.map_add x y

theorem map_neg (f : E →L.[R] F) (x : f.domain) : f (-x) = -f x :=
  f.toFun.map_neg x

theorem map_sub (f : E →L.[R] F) (x y : f.domain) : f (x - y) = f x - f y :=
  f.toFun.map_sub x y

theorem map_smul (f : E →L.[R] F) (c : R) (x : f.domain) : f (c • x) = c • f x :=
  f.toFun.map_smul c x

@[simp]
theorem mk_apply (p : Submodule R E) (f : p →L[R] F) (x : p) : mk ⟨p, f⟩ f.continuous x = f x :=
  rfl

instance le : LE (E →L.[R] F) :=
  ⟨fun f g => f.domain ≤ g.domain ∧ ∀ ⦃x : f.domain⦄ ⦃y : g.domain⦄ (_h : (x : E) = y), f x = g y⟩

theorem apply_comp_inclusion {T S : E →L.[R] F} (h : T ≤ S) (x : T.domain) :
    T x = S (Submodule.inclusion h.1 x) :=
  h.2 rfl

theorem exists_of_le {T S : E →L.[R] F} (h : T ≤ S) (x : T.domain) :
    ∃ y : S.domain, (x : E) = y ∧ T x = S y :=
  ⟨⟨x.1, h.1 x.2⟩, ⟨rfl, h.2 rfl⟩⟩

theorem eq_of_le_of_domain_eq {f g : E →L.[R] F} (hle : f ≤ g) (heq : f.domain = g.domain) :
    f = g :=
  ext heq hle.2

/-- Given two partial linear maps `f`, `g`, the set of points `x` such that
both `f` and `g` are defined at `x` and `f x = g x` form a submodule. -/
def eqLocus (f g : E →L.[R] F) : Submodule R E := LinearPMap.eqLocus (F := F) f g

instance bot : Bot (E →L.[R] F) := ⟨⊥, continuous_const⟩

instance inhabited : Inhabited (E →L.[R] F) := ⟨⊥⟩

instance semilatticeInf : SemilatticeInf (E →L.[R] F) where
  le := (· ≤ ·)
  le_refl f := le_refl (α := E →ₗ.[R] F) f
  le_trans f g h := le_trans (α := E →ₗ.[R] F)
  le_antisymm f g h1 h2 := by rw [mk.injEq, le_antisymm (α := E →ₗ.[R] F) h1 h2]
  inf f g := ⟨f.toLinearPMap ⊓ g.toLinearPMap,
    f.cont.comp (continuous_inclusion (f.eqLocus_le_domain_left g))⟩
  le_inf f g h := le_inf (α := E →ₗ.[R] F)
  inf_le_left f g := inf_le_left (α := E →ₗ.[R] F)
  inf_le_right f g := inf_le_right (α := E →ₗ.[R] F)

instance orderBot : OrderBot (E →L.[R] F) where
  bot := ⊥
  bot_le _ := bot_le (α := E →ₗ.[R] F)

theorem le_of_eqLocus_ge {f g : E →L.[R] F} (H : f.domain ≤ f.eqLocus g) : f ≤ g :=
...
private theorem sup_aux (f g : E →L.[R] F)
    (h : ∀ (x : f.domain) (y : g.domain), (x : E) = y → f x = g y) :
    ∃ fg : ↥(f.domain ⊔ g.domain) →L[R] F,
...
with `f` and `g`. -/
protected noncomputable def sup (f g : E →L.[R] F)
    (h : ∀ (x : f.domain) (y : g.domain), (x : E) = y → f x = g y) : E →L.[R] F :=
  ⟨_, Classical.choose (sup_aux f g h)⟩

@[simp]
theorem domain_sup (f g : E →L.[R] F)
    (h : ∀ (x : f.domain) (y : g.domain), (x : E) = y → f x = g y) :
    (f.sup g h).domain = f.domain ⊔ g.domain :=
  rfl

theorem sup_apply {f g : E →L.[R] F} (H : ∀ (x : f.domain) (y : g.domain), (x : E) = y → f x = g y)
    (x : f.domain) (y : g.domain) (z : ↥(f.domain ⊔ g.domain)) (hz : (↑x : E) + ↑y = ↑z) :
    f.sup g H z = f x + g y :=
  Classical.choose_spec (sup_aux f g H) x y z hz

protected theorem left_le_sup (f g : E →L.[R] F)
...
protected theorem right_le_sup (f g : E →L.[R] F)
...
protected theorem sup_le {f g h : E →L.[R] F}
...
theorem sup_h_of_disjoint (f g : E →L.[R] F) (h : Disjoint f.domain g.domain) (x : f.domain)
...
instance instZero : Zero (E →L.[R] F) := ⟨⊤, 0⟩

@[simp]
theorem zero_domain : (0 : E →L.[R] F).domain = ⊤ := rfl

@[simp]
theorem zero_apply (x : (⊤ : Submodule R E)) : (0 : E →L.[R] F) x = 0 := rfl
...
instance instSMul : SMul M (E →L.[R] F) :=
  ⟨fun a f =>
    { domain := f.domain
      toFun := a • f.toFun }⟩

@[simp]
theorem smul_domain (a : M) (f : E →L.[R] F) : (a • f).domain = f.domain :=
  rfl

theorem smul_apply (a : M) (f : E →L.[R] F) (x : (a • f).domain) : (a • f) x = a • f x :=
  rfl

@[simp]
theorem coe_smul (a : M) (f : E →L.[R] F) : ⇑(a • f) = a • ⇑f :=
  rfl

instance instSMulCommClass [SMulCommClass M N F] : SMulCommClass M N (E →L.[R] F) :=
  ⟨fun a b f => ext' <| smul_comm a b f.toFun⟩

instance instIsScalarTower [SMul M N] [IsScalarTower M N F] : IsScalarTower M N (E →L.[R] F) :=
  ⟨fun a b f => ext' <| smul_assoc a b f.toFun⟩

instance instMulAction : MulAction M (E →L.[R] F) where
  smul := (· • ·)
...
instance instNeg : Neg (E →L.[R] F) :=
  ⟨fun f => ⟨f.domain, -f.toFun⟩⟩

@[simp]
theorem neg_domain (f : E →L.[R] F) : (-f).domain = f.domain := rfl

@[simp]
theorem neg_apply (f : E →L.[R] F) (x) : (-f) x = -f x :=
  rfl

instance instInvolutiveNeg : InvolutiveNeg (E →L.[R] F) :=
  ⟨fun f => by
    ext x y hxy
    · rfl
...
theorem add_domain (f g : E →L.[R] F) : (f + g).domain = f.domain ⊓ g.domain := rfl

theorem add_apply (f g : E →L.[R] F) (x : (f.domain ⊓ g.domain : Submodule R E)) :
    (f + g) x = f ⟨x, x.prop.1⟩ + g ⟨x, x.prop.2⟩ := rfl

instance instAddSemigroup : AddSemigroup (E →L.[R] F) :=
  ⟨fun f g h => by
    ext x y hxy
    · simp only [add_domain, inf_assoc]
    · simp only [add_apply, hxy, add_assoc]⟩

instance instAddZeroClass : AddZeroClass (E →L.[R] F) :=
  ⟨fun f => by
    ext x y hxy
    · simp [add_domain]
...
instance instAddMonoid : AddMonoid (E →L.[R] F) where
  zero_add f := by
    simp
  add_zero := by
    simp
...
instance instAddCommMonoid : AddCommMonoid (E →L.[R] F) :=
  ⟨fun f g => by
    ext x y hxy
...
instance instVAdd : VAdd (E →L[R] F) (E →L.[R] F) :=
  ⟨fun f g =>
    { domain := g.domain
      toFun := f.comp g.domain.subtype + g.toFun }⟩

@[simp]
theorem vadd_domain (f : E →L[R] F) (g : E →L.[R] F) : (f +ᵥ g).domain = g.domain :=
  rfl

theorem vadd_apply (f : E →L[R] F) (g : E →L.[R] F) (x : (f +ᵥ g).domain) :
    (f +ᵥ g) x = f x + g x :=
  rfl

@[simp]
theorem coe_vadd (f : E →L[R] F) (g : E →L.[R] F) : ⇑(f +ᵥ g) = ⇑(f.comp g.domain.subtype) + ⇑g :=
  rfl

instance instAddAction : AddAction (E →L[R] F) (E →L.[R] F) where
  vadd := (· +ᵥ ·)
...
instance instSub : Sub (E →L.[R] F) :=
  ⟨fun f g =>
    { domain := f.domain ⊓ g.domain
      toFun := f.toFun.comp (inclusion (inf_le_left : f.domain ⊓ g.domain ≤ _))
        - g.toFun.comp (inclusion (inf_le_right : f.domain ⊓ g.domain ≤ _)) }⟩

theorem sub_domain (f g : E →L.[R] F) : (f - g).domain = f.domain ⊓ g.domain := rfl

theorem sub_apply (f g : E →L.[R] F) (x : (f.domain ⊓ g.domain : Submodule R E)) :
    (f - g) x = f ⟨x, x.prop.1⟩ - g ⟨x, x.prop.2⟩ := rfl

instance instSubtractionCommMonoid : SubtractionCommMonoid (E →L.[R] F) where
  add_comm := add_comm
  ...
  neg_eq_of_add f g h' := by
    ext x y h
    · have : (0 : E →L.[R] F).domain = ⊤ := zero_domain
...
noncomputable def supSpanSingleton (f : E →L.[K] F) (x : E) (y : F) (hx : x ∉ f.domain) :
    E →L.[K] F :=
...
@[simp]
theorem domain_supSpanSingleton (f : E →L.[K] F) (x : E) (y : F) (hx : x ∉ f.domain) :
    (f.supSpanSingleton x y hx).domain = f.domain ⊔ K ∙ x :=
  rfl

@[simp]
theorem supSpanSingleton_apply_mk (f : E →L.[K] F) (x : E) (y : F) (hx : x ∉ f.domain) (x' : E)
    (hx' : x' ∈ f.domain) (c : K) :
...
private theorem sSup_aux (c : Set (E →L.[R] F)) (hc : DirectedOn (· ≤ ·) c) :
    ∃ f : ↥(sSup (domain '' c)) →L[R] F, (⟨_, f⟩ : E →L.[R] F) ∈ upperBounds c := by
...
protected noncomputable def sSup (c : Set (E →L.[R] F)) (hc : DirectedOn (· ≤ ·) c) : E →L.[R] F :=
  ⟨_, Classical.choose <| sSup_aux c hc⟩

protected theorem le_sSup {c : Set (E →L.[R] F)} (hc : DirectedOn (· ≤ ·) c) {f : E →L.[R] F}
    (hf : f ∈ c) : f ≤ LinearPMap.sSup c hc :=
  Classical.choose_spec (sSup_aux c hc) hf

protected theorem sSup_le {c : Set (E →L.[R] F)} (hc : DirectedOn (· ≤ ·) c) {g : E →L.[R] F}
...
protected theorem sSup_apply {c : Set (E →L.[R] F)} (hc : DirectedOn (· ≤ ·) c) {l : E →L.[R] F}
    (hl : l ∈ c) (x : l.domain) :
...
def toPMap (f : E →L[R] F) (p : Submodule R E) : E →L.[R] F :=
  ⟨p, f.comp p.subtype⟩

@[simp]
theorem toPMap_apply (f : E →L[R] F) (p : Submodule R E) (x : p) : f.toPMap p x = f x :=
  rfl

@[simp]
theorem toPMap_domain (f : E →L[R] F) (p : Submodule R E) : (f.toPMap p).domain = p :=
  rfl

/-- Compose a linear map with a `LinearPMap` -/
def compPMap (g : F →L[R] G) (f : E →L.[R] F) : E →L.[R] G where
  domain := f.domain
  toFun := g.comp f.toFun

@[simp]
theorem compPMap_apply (g : F →L[R] G) (f : E →L.[R] F) (x) : g.compPMap f x = g (f x) :=
  rfl
...
def codRestrict (f : E →L.[R] F) (p : Submodule R F) (H : ∀ x, f x ∈ p) : E →L.[R] p where
  domain := f.domain
  toFun := f.toFun.codRestrict p H

/-- Compose two `LinearPMap`s -/
def comp (g : F →L.[R] G) (f : E →L.[R] F) (H : ∀ x : f.domain, f x ∈ g.domain) : E →L.[R] G :=
...
def coprod (f : E →L.[R] G) (g : F →L.[R] G) : E × F →L.[R] G where
  ...
  toFun :=
    ...
    HAdd.hAdd
      (α := f.domain.prod g.domain →L[R] G)
      (β := f.domain.prod g.domain →L[R] G)
      (f.comp (LinearPMap.fst f.domain g.domain) fun x => x.2.1).toFun
      (g.comp (LinearPMap.snd f.domain g.domain) fun x => x.2.2).toFun

@[simp]
theorem coprod_apply (f : E →L.[R] G) (g : F →L.[R] G) (x) :
...
def domRestrict (f : E →L.[R] F) (S : Submodule R E) : E →L.[R] F :=
  ⟨S ⊓ f.domain, f.toFun.comp (Submodule.inclusion (by simp))⟩

@[simp]
theorem domRestrict_domain (f : E →L.[R] F) {S : Submodule R E} :
    (f.domRestrict S).domain = S ⊓ f.domain :=
  rfl

theorem domRestrict_apply {f : E →L.[R] F} {S : Submodule R E} ⦃x : ↥(S ⊓ f.domain)⦄ ⦃y : f.domain⦄
...
theorem domRestrict_le {f : E →L.[R] F} {S : Submodule R E} : f.domRestrict S ≤ f :=
...
def graph (f : E →L.[R] F) : Submodule R (E × F) :=
  f.toFun.graph.map (f.domain.subtype.prodMap (LinearMap.id : F →L[R] F))

theorem mem_graph_iff' (f : E →L.[R] F) {x : E × F} :
...
theorem mem_graph (f : E →L.[R] F) (x : domain f) : ((x : E), f x) ∈ f.graph := by simp

theorem graph_map_fst_eq_domain (f : E →L.[R] F) :
...
theorem graph_map_snd_eq_range (f : E →L.[R] F) :
...
theorem smul_graph (f : E →L.[R] F) (z : M) :
    (z • f).graph =
      f.graph.map ((LinearMap.id : E →L[R] E).prodMap (z • (LinearMap.id : F →L[R] F))) := by
...
theorem neg_graph (f : E →L.[R] F) :
    (-f).graph =
    f.graph.map ((LinearMap.id : E →L[R] E).prodMap (-(LinearMap.id : F →L[R] F))) := by
  ext x; cases' x with x_fst x_snd
  constructor <;> intro h
  · rw [mem_graph_iff] at h
    rcases h with ⟨y, hy, h⟩
...
theorem mem_graph_snd_inj (f : E →L.[R] F) {x y : E} {x' y' : F} (hx : (x, x') ∈ f.graph)
...
theorem mem_graph_snd_inj' (f : E →L.[R] F) {x y : E × F} (hx : x ∈ f.graph) (hy : y ∈ f.graph)
...
theorem graph_fst_eq_zero_snd (f : E →L.[R] F) {x : E} {x' : F} (h : (x, x') ∈ f.graph)
    (hx : x = 0) : x' = 0 :=
  f.mem_graph_snd_inj h f.graph.zero_mem hx

theorem mem_domain_iff {f : E →L.[R] F} {x : E} : x ∈ f.domain ↔ ∃ y : F, (x, y) ∈ f.graph := by
  constructor <;> intro h
  · use f ⟨x, h⟩
...
theorem mem_domain_of_mem_graph {f : E →L.[R] F} {x : E} {y : F} (h : (x, y) ∈ f.graph) :
    x ∈ f.domain := by
  rw [mem_domain_iff]
  exact ⟨y, h⟩

theorem image_iff {f : E →L.[R] F} {x : E} {y : F} (hx : x ∈ f.domain) :
...
theorem mem_range_iff {f : E →L.[R] F} {y : F} : y ∈ Set.range f ↔ ∃ x : E, (x, y) ∈ f.graph := by
  constructor <;> intro h
...
theorem mem_domain_iff_of_eq_graph {f g : E →L.[R] F} (h : f.graph = g.graph) {x : E} :
    x ∈ f.domain ↔ x ∈ g.domain := by simp_rw [mem_domain_iff, h]

theorem le_of_le_graph {f g : E →L.[R] F} (h : f.graph ≤ g.graph) : f ≤ g := by
  constructor
  · intro x hx
...
theorem le_graph_of_le {f g : E →L.[R] F} (h : f ≤ g) : f.graph ≤ g.graph := by
  intro x hx
  rw [mem_graph_iff] at hx ⊢
  cases' hx with y hx
  use ⟨y, h.1 y.2⟩
...
theorem le_graph_iff {f g : E →L.[R] F} : f.graph ≤ g.graph ↔ f ≤ g :=
  ⟨le_of_le_graph, le_graph_of_le⟩

theorem eq_of_eq_graph {f g : E →L.[R] F} (h : f.graph = g.graph) : f = g := by
  ext
...
noncomputable def toLinearPMapAux (g : Submodule R (E × F))
    ...
    g.map (LinearMap.fst R E F) →L[R] F where
...
noncomputable def toLinearPMap (g : Submodule R (E × F)) : E →L.[R] F where
...
noncomputable def inverse (f : E →L.[R] F) : F →L.[R] E :=
