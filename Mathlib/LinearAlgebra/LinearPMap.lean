...
@[ext]
structure twistedProductModule {R S : Type*} [Ring R] [Ring S] (σ : R →+* S) {E : Type*}
    [AddCommGroup E] (mr : Module R E) {F : Type*} [AddCommGroup F] (ms : Module S F) where
  left : E
  right : F

namespace twistedProductModule

variable {R S : Type*} [Ring R] [Ring S] (σ : R →+* S) {E : Type*} [AddCommGroup E]
  [mr : Module R E] {F : Type*} [AddCommGroup F] [ms : Module S F]

instance : Add (twistedProductModule σ mr ms) where
  add x y := mk (x.left + y.left) (x.right + y.right)

@[simp]
lemma add_left (x y : twistedProductModule σ mr ms) :
    (x + y).left = x.left + y.left := rfl

@[simp]
lemma add_right (x y : twistedProductModule σ mr ms) :
    (x + y).right = x.right + y.right := rfl

instance : Neg (twistedProductModule σ mr ms) where
  neg x := mk (-x.left) (-x.right)

@[simp]
lemma neg_left (x : twistedProductModule σ mr ms) :
    (-x).left = -x.left := rfl

@[simp]
lemma neg_right (x : twistedProductModule σ mr ms) :
    (-x).right = -x.right := rfl

instance : Zero (twistedProductModule σ mr ms) where
  zero := mk 0 0

@[simp]
lemma zero_left : (0 : twistedProductModule σ mr ms).left = 0 := rfl

@[simp]
lemma zero_right : (0 : twistedProductModule σ mr ms).right = 0 := rfl

instance : AddCommGroup (twistedProductModule σ mr ms) where
  add_assoc x y z := by
    ext
    · simpa using add_assoc _ _ _
    · simpa using add_assoc _ _ _
  zero_add x := by
    ext
    · simp
  ...
  zsmul_succ' n x := by
    ext
    · simp only [Nat.succ_eq_add_one, Nat.cast_add, Nat.cast_one, natCast_zsmul, add_left]
      rw [add_smul, one_smul]
      simp
    · simp only [Nat.succ_eq_add_one, Nat.cast_add, Nat.cast_one, natCast_zsmul, add_right]
      rw [add_smul, one_smul]
      simp
  nsmul_zero x := by
    ext
  ...
  zsmul_neg' n x := by
    ext
    · simp only [negSucc_zsmul, Nat.succ_eq_add_one, Nat.cast_add, Nat.cast_one, neg_left, neg_inj]
      rw [add_smul, one_smul, add_smul]
      simp
    · simp only [negSucc_zsmul, Nat.succ_eq_add_one, Nat.cast_add, Nat.cast_one, neg_right,
      neg_inj]
      rw [add_smul, one_smul, add_smul]
      simp

instance : SMul R (twistedProductModule σ mr ms) where
  smul s x := mk (s • x.left) (σ s • x.right)

@[simp]
lemma smul_left (s : R) (x : twistedProductModule σ mr ms) :
    (s • x).left = s • x.left := rfl

@[simp]
lemma smul_right (s : R) (x : twistedProductModule σ mr ms) :
    (s • x).right = σ s • x.right := rfl

instance : Module R (twistedProductModule σ mr ms) where
  mul_smul s t x := by
    ext
    · simp only [smul_left]
      exact mul_smul s t x.left
    · simp only [smul_right, map_mul]
      exact mul_smul (σ s) (σ t) x.right
  one_smul x := by
    ext
    · simp
    · simp
  ...
  add_smul s t x := by
    ext
    · simp only [smul_left, add_left]
      exact Module.add_smul s t x.left
    · simp only [smul_right, _root_.map_add, add_right]
      exact Module.add_smul (σ s) (σ t) x.right
  zero_smul x := by
    ext
    · simp
    · simp

end twistedProductModule
