...
variable {R : Type*} [CommRing R] {I : Ideal R} {p : ℕ} [Fact p.Prime] (hpI : (p : R) ∈ I)
include hpI
...
theorem pow_prime {x y : R} {m : ℕ} (hm : m ≠ 0) (h : x ≡ y [SMOD I ^ m]) :
  ...
  obtain ⟨r, hr⟩ := exists_add_pow_prime_eq' hp (x - y) y
  rw [sub_add_cancel] at hr
  rw [hr, add_right_comm, add_sub_cancel_right, mul_assoc _ y]
  refine add_mem ?_ ?_
  · have := Ideal.pow_mem_pow h p
    rw [← pow_mul] at this
    exact I.pow_le_pow_right (by grw [add_one_le_two_mul (by grind), hp.two_le, mul_comm]) this
  · rw [pow_succ']
...
theorem pow_prime_pow {x y : R} (h : x ≡ y [SMOD I]) {m : ℕ} :
  ...
  | succ m ih =>
    ...
    exact ih.pow_prime hpI m.succ_ne_zero
