...
lemma mem_lifts_iff (x : ⨂[R] i, s i) (p : FreeAddMonoid (R × Π i, s i)) :
  ...
  simp only [lifts, Set.mem_ofPred, FreeAddMonoid.toPiTensorProduct]
...
lemma lifts_add {x y : ⨂[R] i, s i} {p q : FreeAddMonoid (R × Π i, s i)}
  ...
  simp only [lifts, Set.mem_ofPred, AddCon.coe_add]
  rw [hp, hq]
...
theorem map_range_eq_span_tprod :
  ...
  simp only [Set.mem_range, comp_apply, map_tprod, Set.mem_ofPred]
