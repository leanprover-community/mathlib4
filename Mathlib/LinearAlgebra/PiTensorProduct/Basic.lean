...
* `lift φ` with `φ : s →ₗₘ[R] E` is the corresponding linear map
...
theorem liftAux_tprod (φ : s →ₗₘ[R] E) (f : Π i, s i) : liftAux φ (tprod R f) = φ f := by
...
theorem liftAux_tprodCoeff (φ : s →ₗₘ[R] E) (z : R) (f : Π i, s i) :
    liftAux φ (tprodCoeff R z f) = z • φ f := rfl

theorem liftAux.smul {φ : s →ₗₘ[R] E} (r : R) (x : ⨂[R] i, s i) :
...
@[simp]
theorem lift_tprod : lift (tprod R : s →ₗₘ[R] _) = LinearMap.id :=
...
theorem lift_comp_map (h : t →ₗₘ[R] E) : lift h ∘ₗ map f = lift (h.compLinearMap f) := by
  ext
...
theorem lift_comp_reindex (e : ι ≃ ι₂) (φ : (fun i ↦ s (e.symm i)) →ₗₘ[R] E) :
    lift φ ∘ₗ (reindex R s e) = lift ((domDomCongrLinearEquiv' R R s _ e).symm φ) := by
  ext; simp [reindex]

@[simp]
theorem lift_comp_reindex_symm (e : ι ≃ ι₂) (φ : s →ₗₘ[R] E) :
