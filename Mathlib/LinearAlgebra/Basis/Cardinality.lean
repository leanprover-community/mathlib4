...
lemma finite_of_span_finite_eq_top_finsupp [Nontrivial M] {ι : Type*}
    (w : Set (ι →₀ M)) (hw : w.Finite) (s : span R w = ⊤) : Finite ι :=
  suffices ⋃ i ∈ w, i.support.toSet = .univ from
    .of_finite_univ (this ▸ hw.biUnion fun _ _ ↦ by simp)
  have ⟨x, hx⟩ := exists_ne (0 : M)
  eq_univ_of_forall fun j ↦ (top_unique (s.ge.trans (span_le_supported_biUnion_support R w)) ▸
...
-/
lemma basis_finite_of_finite_spans [Nontrivial R] (w : Set M) (hw : w.Finite) (s : span R w = ⊤)
    {ι : Type w} (b : Basis ι R M) : Finite ι := by
  have := congr(($s).map b.repr)
  rw [← span_image, Submodule.map_top, LinearEquivClass.range] at this
  classical exact finite_of_span_finite_eq_top_finsupp _ (hw.image _) this
