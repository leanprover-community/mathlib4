...
lemma le_of_fin_injective [StrongRankCondition R] {n m : ℕ} (f : (Fin n → R) →ₗ[R] Fin m → R) :
    Injective f → n ≤ m :=
...
lemma strongRankCondition_iff_succ :
...
lemma card_le_of_injective [StrongRankCondition R] {α β : Type*} [Fintype α] [Fintype β]
...
lemma le_of_fin_surjective [RankCondition R] {n m : ℕ} (f : (Fin n → R) →ₗ[R] Fin m → R) :
    Surjective f → m ≤ n :=
  RankCondition.le_of_fin_surjective f
#align le_of_fin_surjective le_of_fin_surjective

lemma card_le_of_surjective [RankCondition R] {α β : Type*} [Fintype α] [Fintype β]
...
lemma card_eq_of_linearEquiv {α β : Type*} [Fintype α] [Fintype β] (f : (α → R) ≃ₗ[R] β → R) :
...
lemma nontrivial_of_invariantBasisNumber : Nontrivial R := by
  by_contra h
