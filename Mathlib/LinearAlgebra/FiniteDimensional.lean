...
variable {K : Type u} {R : Type u} {V : Type v}
...
lemma _root_.Module.rank_lt_alpeh0_iff [Module.Free R V] :
...
theorem finrank_of_infinite_dimensional [Module.Free R V] (h : ¬Module.Finite R V) :
    finrank R V = 0 :=
...
theorem _root_.Module.finite_of_finrank_pos [Module.Free R V] (h : 0 < finrank R V) :
...
theorem _root_.Module.finite_of_fact_finrank_eq_succ [Module.Free R V] (n : ℕ)
    [Fact (finrank R V = n + 1)] : Module.Finite R V :=
  Module.finite_of_finrank_pos <| by convert Nat.succ_pos n; apply Fact.out
#align finite_dimensional.fact_finite_dimensional_of_finrank_eq_succ Module.finite_of_fact_finrank_eq_succ
...
theorem finiteDimensional_iff_of_rank_eq_nsmul {W} [AddCommGroup W] [Module K W]
...
`finrank`. -/
theorem mk_finrank_eq_card_basis [Module.Finite R V] [Module.Free R V]
...
#align finite_dimensional.finrank_eq_card_basis' FiniteDimensional.mk_finrank_eq_card_basis
...
noncomputable def _root_.Basis.unique {ι : Type*} (b : Basis ι R R) : Unique ι := by
  have A : Cardinal.mk ι = ↑(FiniteDimensional.finrank R R) :=
    (FiniteDimensional.mk_finrank_eq_card_basis b).symm
...
noncomputable def finBasis [Module.Free R V] [Module.Finite R V] :
...
noncomputable def finBasisOfFinrankEq [Module.Free R V][Module.Finite R V] {n : ℕ}
    (hn : finrank R V = n) :
    Basis (Fin n) R V :=
...
noncomputable def basisUnique [Module.Free R V] (ι : Type*) [Unique ι] (h : finrank R V = 1) :
    Basis ι R V :=
...
@[simp]
theorem basisUnique.repr_eq_zero_iff [Module.Free R V] {ι : Type*} [Unique ι]
...
theorem _root_.Module.Finite.cardinal_mk_le_finrank_of_linearIndependent [Module.Finite R V]
...
theorem _root_.Module.Finite.fintype_card_le_finrank_of_linearIndependent [Module.Finite R V]
...
theorem _root_.Module.Finite.finset_card_le_finrank_of_linearIndependent [Module.Finite R V]
...
theorem _root_.Module.Finite.lt_aleph0_of_linearIndependent {ι : Type w}
...
#align finite_dimensional.lt_aleph_0_of_linear_independent Module.Finite.lt_aleph0_of_linearIndependent

theorem _root_.LinearIndependent.finite [Module.Finite R V] {b : Set V}
...
theorem _root_.Module.Finite.not_linearIndependent_of_infinite
...
theorem finrank_pos_iff_exists_ne_zero [Module.Finite R V] [NoZeroSMulDivisors R V] :
...
theorem finrank_pos_iff [Module.Finite R V] [NoZeroSMulDivisors R V] :
...
theorem finrank_pos [Module.Finite R V] [NoZeroSMulDivisors R V] [h : Nontrivial V] :
    0 < finrank R V :=
...
theorem finrank_eq_zero_iff_isTorsion {R} [CommRing R] [StrongRankCondition R]
...
theorem finrank_zero_iff [Module.Finite R V] [NoZeroSMulDivisors R V] :
...
instance finiteDimensional_self : FiniteDimensional K K := Module.Finite.self K
#align finite_dimensional.finite_dimensional_self FiniteDimensional.finiteDimensional_self

/-- The submodule generated by a finite set is finite-dimensional. -/
theorem _root_.Module.Finite.span_of_finite {A : Set V} (hA : Set.Finite A) :
...
instance _root_.Module.Finite.span_singleton (x : V) : Module.Finite R (R ∙ x) :=
...
instance _root_.Module.Finite.span_finset (s : Finset V) : Module.Finite R (span R (s : Set V)) :=
...
theorem _root_.CompleteLattice.Independent.subtype_ne_bot_le_finrank_aux [Module.Finite R V]
...
-/
theorem exists_nontrivial_relation_of_finrank_lt_card
...
#align finite_dimensional.exists_nontrivial_relation_of_rank_lt_card FiniteDimensional.exists_nontrivial_relation_of_finrank_lt_card

/-- If a finset has cardinality larger than `finrank + 1`,
then there is a nontrivial linear relation amongst its elements,
such that the coefficients of the relation sum to zero.
-/
theorem exists_nontrivial_relation_sum_zero_of_finrank_succ_lt_card [Module.Finite R V]
...
#align finite_dimensional.exists_nontrivial_relation_sum_zero_of_rank_succ_lt_card FiniteDimensional.exists_nontrivial_relation_sum_zero_of_finrank_succ_lt_card
...
/-- A slight strengthening of `exists_nontrivial_relation_sum_zero_of_finrank_succ_lt_card`
...
theorem exists_relation_sum_zero_pos_coefficient_of_finrank_succ_lt_card [FiniteDimensional L W]
  ...
  obtain ⟨f, sum, total, nonzero⟩ := exists_nontrivial_relation_sum_zero_of_finrank_succ_lt_card h
...
finite-dimensional. -/
instance _root_.Module.finite_sup (S₁ S₂ : Submodule R V) [h₁ : Module.Finite R S₁]
    [h₂ : Module.Finite R S₂] : Module.Finite R (S₁ ⊔ S₂ : Submodule R V) := by
  rw [finite_def] at *
  exact (fg_top _).2 (((fg_top S₁).1 h₁).sup ((fg_top S₂).1 h₂))
#align submodule.finite_dimensional_sup Module.finite_sup
...
instance _root_.Module.finite_finset_sup {ι : Type*} (s : Finset ι) (S : ι → Submodule R V)
  ...
  · intro S₁ hS₁ S₂ hS₂
    exact Module.finite_sup S₁ S₂
#align submodule.finite_dimensional_finset_sup Module.finite_finset_sup

/-- The submodule generated by a supremum of finite dimensional submodules, indexed by a finite
sort is finite-dimensional. -/
instance _root_.Module.finite_iSup {ι : Sort*} [Finite ι] (S : ι → Submodule R V)
    [∀ i, Module.Finite R (S i)] : Module.Finite R ↑(⨆ i, S i) := by
  cases nonempty_fintype (PLift ι)
  rw [← iSup_plift_down, ← Finset.sup_univ_eq_iSup]
  exact Module.finite_finset_sup _ _
#align submodule.finite_dimensional_supr Module.finite_iSup
...
lemma exists_smul_eq_of_finrank_eq_one
  ...
  have : Submodule.span K {x} = ⊤ := by
    have : FiniteDimensional K V := Module.finite_of_finrank_pos (zero_lt_one.trans_le h.symm.le)
    apply eq_top_of_finrank_eq
    rw [h]
...
theorem finrank_eq_one_iff (ι : Type*) [Unique ι] : finrank K V = 1 ↔ Nonempty (Basis ι K V) := by
  constructor
  · intro h
    haveI := Module.finite_of_finrank_pos (_root_.zero_lt_one.trans_le h.symm.le)
...
theorem is_simple_module_of_finrank_eq_one {A} [Semiring A] [Module A V] [SMul K A]
  ...
  haveI : FiniteDimensional _ _ := Module.finite_of_finrank_eq_succ h
...
instance Subalgebra.finiteDimensional_bot : FiniteDimensional F (⊥ : Subalgebra F E) := by
  nontriviality E
  exact Module.finite_of_rank_eq_one Subalgebra.rank_bot
...
theorem Subalgebra.eq_bot_of_rank_le_one {S : Subalgebra F E} (h : Module.rank F S ≤ 1) :
  ...
  haveI : FiniteDimensional F S := Module.finite_of_rank_eq_nat he
...
theorem Subalgebra.eq_bot_of_finrank_one {S : Subalgebra F E} (h : finrank F S = 1) : S = ⊥ :=
  Subalgebra.eq_bot_of_rank_le_one <| by
    -- porting note: fails without explicit type
    haveI : FiniteDimensional F S := Module.finite_of_finrank_eq_succ h
...
theorem Subalgebra.isSimpleOrder_of_finrank (hr : finrank F E = 2) :
  ...
  { toNontrivial :=
    ...
    eq_bot_or_eq_top := by
      intro S
      haveI : FiniteDimensional F E := Module.finite_of_finrank_eq_succ hr
