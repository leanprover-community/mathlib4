...
instance finiteDimensional_pi {ι : Type*} [Finite ι] : FiniteDimensional K (ι → K) := fast_instance%
  Finite.pi
...
/-- If a vector space is `FiniteDimensional`, `Basis.ofVectorSpace` is indexed by
  a finite type.-/
noncomputable instance [FiniteDimensional K V] : Fintype (Basis.ofVectorSpaceIndex K V) := fast_instance% by
...
instance finiteDimensional_self : FiniteDimensional K K := fast_instance% inferInstance
...
instance span_singleton (x : V) : FiniteDimensional K (K ∙ x) := fast_instance%
...
instance span_finset (s : Finset V) : FiniteDimensional K (span K (s : Set V)) := fast_instance%
...
instance finiteDimensional_bot : FiniteDimensional K (⊥ : Submodule K V) := fast_instance%
...
```lean4
instance (S : Subalgebra F E) : AddCommMonoid { x // x ∈ S } := fast_instance% inferInstance
instance (S : Subalgebra F E) : AddCommGroup { x // x ∈ S } := fast_instance% inferInstance
```
...
instance Subalgebra.finiteDimensional_bot : FiniteDimensional F (⊥ : Subalgebra F E) := fast_instance% by
  nontriviality E
