...
lemma PolarizationIn_eq (x : P.rootSpan S) :
  ...
  simp only [PolarizationIn, FunLike.coe_sum, LinearMap.coe_comp, Finset.sum_apply, comp_apply,
...
lemma rootFormIn_self_smul_coroot (i : ι) :
  ...
  simp only [FunLike.coe_sum, LinearMap.coe_smulRight, Finset.sum_apply,
    coroot'In_rootSpanMem_eq_pairingIn, _root_.smul_apply, smul_eq_mul, Finset.sum_smul,
...
lemma four_nsmul_coPolarization_compl_polarization_apply_root (i : ι) :
  ...
  rw [_root_.smul_apply, LinearMap.comp_apply, show 4 = 2 * 2 from rfl, mul_smul, ← map_nsmul,
