...
root positive form. -/
def rootFormRP [Fintype ι] : P.RootPositiveForm S where
  form := P.RootForm
...
/-- TODO Naming -/
theorem rootForm_self_non_neg' [Fintype ι] (x : M) (hx : x ∈ span S (range P.root)) :
    ∃ s ≥ 0, algebraMap S R s = P.RootForm x x := by
  refine ⟨(P.rootFormRP S).formIn ⟨x, hx⟩ ⟨x, hx⟩, IsSumSq.nonneg ?_, by simp [rootFormRP]⟩
  have (i : ι) : P.coroot' i x ∈ LinearMap.range (Algebra.linearMap S R) := by
    -- TODO Lemma in `Defs`?
  ...
  suffices (P.rootFormRP S).formIn ⟨x, hx⟩ ⟨x, hx⟩ = ∑ i, s i * s i from
    this ▸ IsSumSq.sum_mul_self Finset.univ s
  apply FaithfulSMul.injective_algebraMap S R
  simp only [rootFormRP, RootPositiveForm.algebraMap_formIn, map_sum, map_mul]
  simp [← Algebra.linearMap_apply, hs, PerfectPairing.flip_apply_apply, rootForm_apply_apply]

/-- SGA3 XXI Prop. 2.3.1 -/
lemma coxeterWeight_le_four [Finite ι] (i j : ι) :
  ...
  set li := (P.rootFormRP S).formIn ri ri
  set lj := (P.rootFormRP S).formIn rj rj
  set lij := (P.rootFormRP S).formIn ri rj
  obtain ⟨si, hsi, hsi'⟩ := (P.rootFormRP S).exists_pos_eq i
  obtain ⟨sj, hsj, hsj'⟩ := (P.rootFormRP S).exists_pos_eq j
  replace hsi' : si = li := FaithfulSMul.injective_algebraMap S R <| by simpa [li] using hsi'
  replace hsj' : sj = lj := FaithfulSMul.injective_algebraMap S R <| by simpa [lj] using hsj'
  rw [hsi'] at hsi
  rw [hsj'] at hsj
  have cs : 4 * lij ^ 2 ≤ 4 * (li * lj) := by
    rw [mul_le_mul_left four_pos]
    refine (P.rootFormRP S).formIn.apply_sq_le_of_symm ?_ (P.rootFormRP S).isSymm_formIn ri rj
    intro x
    obtain ⟨s, hs, hs'⟩ := P.rootForm_self_non_neg' S x x.property
    change _ = (P.rootFormRP S).form x x at hs'
    rw [(P.rootFormRP S).algebraMap_apply_eq_form_iff] at hs'
    rwa [← hs']
  have key : 4 • lij ^ 2 = P.coxeterWeightIn S i j • (li * lj) := by
    apply FaithfulSMul.injective_algebraMap S R
    simpa [map_ofNat, lij, rootFormRP, ri, rj, li, lj] using
...
lemma coxeterWeightIn_mem_set_of_isCrystallographic [Finite ι] [P.IsCrystallographic]
    [CharZero R] (i j : ι) :
    P.coxeterWeightIn ℤ i j ∈ ({0, 1, 2, 3, 4} : Set ℤ) := by
  have : Fintype ι := Fintype.ofFinite ι
  obtain ⟨n, hcn⟩ : ∃ n : ℕ, P.coxeterWeightIn ℤ i j = n := by
    have : 0 ≤ P.coxeterWeightIn ℤ i j := by
      simpa only [P.algebraMap_coxeterWeightIn] using P.coxeterWeight_non_neg (P.rootFormRP ℤ) i j
    obtain ⟨n, hn⟩ := Int.eq_ofNat_of_zero_le this
    exact ⟨n, by simp [← P.algebraMap_coxeterWeightIn ℤ, hn]⟩
  have : P.coxeterWeightIn ℤ i j ≤ 4 := P.coxeterWeight_le_four ℤ i j
  simp only [hcn, mem_insert_iff, mem_singleton_iff] at this ⊢
  norm_cast at this ⊢
  omega

lemma coxeterWeight_mem_set_of_isCrystallographic [Finite ι] [P.IsCrystallographic] [CharZero R]
    (i j : ι) :
    P.coxeterWeight i j ∈ ({0, 1, 2, 3, 4} : Set R) := by
  have := (FaithfulSMul.injective_algebraMap ℤ R).mem_set_image.mpr <|
