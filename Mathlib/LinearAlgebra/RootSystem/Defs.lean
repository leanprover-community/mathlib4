...
lemma root_coroot_zero_comm [NeZero (2 : R)] [NoZeroSMulDivisors R N]
    (h : P.toPerfectPairing (P.root j) (P.coroot i) = 0) :
      P.toPerfectPairing (P.root i) (P.coroot j) = 0 := by
  have root_i_j := P.reflection_perm_root i j
  rw [h, zero_smul, sub_zero, EmbeddingLike.apply_eq_iff_eq] at root_i_j
  have coroot_i_j := P.reflection_perm_coroot i j
  rw [← root_i_j, root_coroot_eq_pairing, sub_eq_self, smul_eq_zero] at coroot_i_j
  rcases coroot_i_j with h | h
  · exact h
  exact False.elim (P.ne_zero' i h)
