...
variable {P} in
lemma smul_coroot_eq_of_root_eq_smul [Finite ι] [Module.IsTorsionFree ℤ N] (i j : ι) (t : R)
...
variable {P} in
@[simp] lemma coroot_eq_smul_coroot_iff [Finite ι] [Module.IsTorsionFree ℤ M] [Module.IsTorsionFree ℤ N]
    {i j : ι} {t : R} :
...
lemma pairing_eq_zero_iff [NeZero (2 : R)] [Module.IsTorsionFree R M] :
...
lemma isOrthogonal_iff_pairing_eq_zero [NeZero (2 : R)] [Module.IsTorsionFree R M] :
    P.IsOrthogonal i j ↔ P.pairing i j = 0 :=
  ⟨fun h ↦ h.1, fun h ↦ ⟨h, pairing_eq_zero_iff.mp h⟩⟩

lemma isFixedPt_reflectionPerm_iff [NeZero (2 : R)] [Module.IsTorsionFree R M] :
