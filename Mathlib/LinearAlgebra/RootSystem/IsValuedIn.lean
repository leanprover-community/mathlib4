...
lemma rootSpan_dualAnnihilator_map_eq_iInf_ker_root' :
    (P.rootSpan R).dualAnnihilator.map P.flip.toPerfPair.symm = ⨅ i, LinearMap.ker (P.root' i) := by
  suffices (P.rootSpan R).dualAnnihilator.map P.flip.toPerfPair.symm = {x | ∀ i, P.root' i x = 0} from
