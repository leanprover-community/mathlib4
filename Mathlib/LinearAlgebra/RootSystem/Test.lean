...
def reflection_in (P : RootPairingOld ι R M N) (i : ι) : ι ≃ ι where
  ...
  right_inv j := by
    exact root_reflection_comp_self P i j
/-!
lemma reflection_in_flip (P : RootPairingOld ι R M N) (i : ι) :
    P.reflection_in i = P.flip.reflection_in i := by
  ext j
...
theorem coroot_coreflection (P : RootPairingOld ι R M N) (i j : ι) :
    P.coroot (P.reflection_in i j) = (P.coreflection i) (P.coroot j) := by
  simp only [reflection_in_apply]
  sorry
