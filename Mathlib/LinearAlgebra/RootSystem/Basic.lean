...
its roots. -/
protected lemma IsRootSystem.ext [CharZero R] [IsTorsionFree R M]
...
private lemma coroot_eq_coreflection_of_root_eq_of_span_eq_top [CharZero R] [IsTorsionFree R M]
    (p : M →ₗ[R] N →ₗ[R] R) [p.IsPerfPair]
    (root : ι ↪ M)
    (coroot : ι ↪ N)
    (hp : ∀ i, p (root i) (coroot i) = 2)
