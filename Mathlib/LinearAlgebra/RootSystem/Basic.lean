...
protected def equiv_of_mapsTo :
    ι ≃ ι where
  toFun j := (private_decl% (exist_eq_reflection_of_mapsTo p root coroot i j h)).choose
  invFun j := (private_decl% (exist_eq_reflection_of_mapsTo p root coroot i j h)).choose
