...
private lemma restrictScalar_field_aux
...
field. -/
instance restrictScalars.instIsPerfPair_of_field
    (hij : p.IsPerfectCompl (span L <| LinearMap.range i) (span L <| LinearMap.range j))
    (hp : ∀ m n, p (i m) (j n) ∈ (algebraMap K L).range) :
    (LinearMap.restrictScalarsRange₂ i j (Algebra.linearMap K L)
      (FaithfulSMul.algebraMap_injective K L) p hp).IsPerfPair := by
  have P : (p.compl₁₂ i j).IsPerfPair :=
    .restrict _ (Submodule.subtype <| span L (.range i)) (Submodule.subtype <| span L (.range j))
      (by simp) (by simp) (by simpa)
  exact p.restrictScalars_field_aux
