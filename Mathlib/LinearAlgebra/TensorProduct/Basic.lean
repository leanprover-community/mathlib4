...
protected theorem neg_add_self (x : M ⊗[R] N) : -x + x = 0 :=
  x.induction_on
    (by rw [add_zero]; apply (Neg.aux R).map_zero)
    (fun x y => by convert (add_tmul (R := R) (-x) x y).symm; rw [neg_add_self, zero_tmul])
    fun x y hx hy => by
...
instance addCommGroup : AddCommGroup (M ⊗[R] N) :=
  { TensorProduct.addCommMonoid with
    ...
    neg_add_self := fun x => TensorProduct.neg_add_self x
    zsmul := fun n v => n • v
    ...
    zsmul_neg' := fun n x => by
      ...
      rw [← zero_add (_ • x), ← TensorProduct.neg_add_self ((n.succ : ℤ) • x), add_assoc,
