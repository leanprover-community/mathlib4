...
lemma Algebra.TensorProduct.lTensor_ker (hg : Function.Surjective g) :
    (map (AlgHom.id R A) g).ker =
      g.ker.map (Algebra.TensorProduct.includeRight : C →ₐ[R] A ⊗[R] C) := by
  rw [← Submodule.restrictScalars_inj R]
  have : (map (AlgHom.id R A) g).ker.restrictScalars R =
...
lemma Algebra.TensorProduct.rTensor_ker (hf : Function.Surjective f) :
    (map f (AlgHom.id R C)).ker =
      f.ker.map (Algebra.TensorProduct.includeLeft : A →ₐ[R] A ⊗[R] C) := by
  rw [← Submodule.restrictScalars_inj R]
  have : (map f (AlgHom.id R C)).ker.restrictScalars R =
...
lemma AlgHom.comp_toRingHom' {R A B C : Type*} [CommSemiring R]
    [Semiring A] [Semiring B] [Semiring C] [Algebra R A] [Algebra R B]
    [Algebra R C] (f : B →ₐ[R] C) (g : A →ₐ[R] B) :
    (f.comp g).toRingHom = f.toRingHom.comp g.toRingHom := rfl
...
theorem Algebra.TensorProduct.map_ker (hf : Function.Surjective f) (hg : Function.Surjective g) :
    (map f g).ker =
      f.ker.map (Algebra.TensorProduct.includeLeft : A →ₐ[R] A ⊗[R] C) ⊔
        g.ker.map (Algebra.TensorProduct.includeRight : C →ₐ[R] A ⊗[R] C) := by
