...
def TensorProduct.flip : M ⊗[R] N →ₗ[R] N ⊗[R] M :=
  TensorProduct.lift (LinearMap.flip (TensorProduct.mk  R N M))

lemma TensorProduct.flip_flip_eq :
    (TensorProduct.flip : N ⊗[R] M →ₗ[R] M ⊗[R] N).comp TensorProduct.flip = LinearMap.id := by
  unfold TensorProduct.flip
  apply TensorProduct.ext'
  intro x y
  rfl

def TensorProduct.flipEquiv : M ⊗[R] N ≃ₗ[R] N ⊗[R] M := {
    TensorProduct.flip with
    invFun := TensorProduct.flip
    left_inv := fun p ↦ by
      simp only [AddHom.toFun_eq_coe, coe_toAddHom, ← LinearMap.comp_apply,TensorProduct.flip_flip_eq, id_coe, id_eq]
    right_inv := fun p ↦ by
      simp only [AddHom.toFun_eq_coe, coe_toAddHom, ← LinearMap.comp_apply,TensorProduct.flip_flip_eq, id_coe, id_eq] }

lemma TensorProduct.flip_tmul (m : M) (n: N) :
    TensorProduct.flip (m ⊗ₜ[R] n) = n ⊗ₜ[R] m := by
  rfl

lemma TensorProduct.lift_comp_flip_eq  {f : M →ₗ[R] N →ₗ[R] P} :
    (TensorProduct.lift f).comp TensorProduct.flip =
      TensorProduct.lift f.flip := by
  apply TensorProduct.ext'
  intro n m
  rfl

lemma TensorProduct.map_comp_flip_eq {f : M →ₗ[R] P} {g : N →ₗ[R] Q} :
    (TensorProduct.map f g).comp TensorProduct.flip =
      TensorProduct.flip.comp (TensorProduct.map g f) := by
  apply TensorProduct.ext'
  intro n m
  rfl

lemma LinearMap.flip_comp_rTensor_comp_flip_eq {g : N →ₗ[R] P} :
    (TensorProduct.flipEquiv.toLinearMap.comp (rTensor Q g)).comp TensorProduct.flipEquiv.toLinearMap = lTensor Q g := by
  apply TensorProduct.ext'
  intro n q
  rfl

lemma LinearMap.flipEquiv_comp_lTensor_comp_flipEquiv_eq {g : N →ₗ[R] P} :
    (TensorProduct.flipEquiv.toLinearMap.comp (lTensor Q g)).comp TensorProduct.flipEquiv.toLinearMap = rTensor Q g := by
  apply TensorProduct.ext'
  intro n q
  rfl

lemma LinearMap.flip_comp_lTensor_comp_flip_eq {g : N →ₗ[R] P} :
    (TensorProduct.flip.comp (lTensor Q g)).comp TensorProduct.flip = rTensor Q g := by
  apply TensorProduct.ext'
  intro n q
  rfl


lemma LinearMap.comp_flip_eq {g : N ⊗[R] M →ₗ[R] P} :
    g.comp TensorProduct.flip = g.comp TensorProduct.flipEquiv.toLinearMap := by
  rfl

lemma LinearMap.flip_comp_eq {g : P →ₗ[R] M ⊗[R] N} :
    TensorProduct.flip.comp g = TensorProduct.flipEquiv.toLinearMap.comp g :=
  rfl
...
/-- The inverse map in `rTensor.equiv_ofRightInverse`
...
/-- The inverse map in `lTensor.equiv_ofRightInverse` (computably, given a right inverse)-/
...
lemma lTensor.inverse_ofRightInverse_comp_rTensor
  ...
  simp only [coe_comp, Function.comp_apply, Submodule.mkQ_apply,
    lTensor.inverse_ofRightInverse_apply]
  ...
  smul_mem' := fun x y ↦ by
    ...
    | tmul a b =>
        ...
        | zero =>
            ...
            apply zero_mem
        | tmul x y =>
            simp only [LinearMap.lTensor_tmul, Submodule.coeSubtype, smul_eq_mul,
...
lemma Ideal.map_includeLeft_eq (I : Ideal A) :
  ...
  · rw [Ideal.map, Ideal.span_le]
    rintro x ⟨a, ha, rfl⟩
    simp only [Algebra.TensorProduct.includeRight_apply, Submodule.coe_set_mk,
      AddSubmonoid.coe_set_mk, AddSubsemigroup.coe_set_mk, SetLike.mem_coe, LinearMap.mem_range]
    use ⟨a, ha⟩ ⊗ₜ 1
    rfl
