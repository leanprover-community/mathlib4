...
-- Beforehttps://github.com/leanprover-community/mathlib4/issues/8386 this was `@[simp]` but it times out when we try to apply it.
...
theorem tmul_coe_mul_coe_tmul {j₁ i₂ : ι} (a₁ : A) (b₁ : ℬ j₁) (a₂ : 𝒜 i₂) (b₂ : B) :
  ...
  simp_rw [lof_eq_of R]
  -- Note:https://github.com/leanprover-community/mathlib4/issues/8386 had to specialize `map_smul` to `LinearEquiv.map_smul`
...
instance instAlgebra : Algebra R (𝒜 ᵍ⊗[R] ℬ) where
  ...
  smul_def' r x := by
    dsimp [mul_def, mulHom_apply, auxEquiv_tmul]
    simp_rw [DirectSum.decompose_algebraMap, DirectSum.decompose_one, algebraMap_gradedMul]
    -- Qualified `map_smul` to avoid a TC timeouthttps://github.com/leanprover-community/mathlib4/issues/8386
