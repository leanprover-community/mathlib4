...
theorem tmul_coe_mul_coe_tmul {j₁ i₂ : ι} (a₁ : A) (b₁ : ℬ j₁) (a₂ : 𝒜 i₂) (b₂ : B) :
  ...
  rw [@Units.smul_def _ _ (_) (_), ← intCast_smul_eq_zsmul R, LinearEquiv.map_smul, map_smul,
    intCast_smul_eq_zsmul R, ← @Units.smul_def _ _ (_) (_)]
  rw [congr_symm_tmul]
  dsimp
...
def lift (f : A →ₐ[R] C) (g : B →ₐ[R] C)
  ...
  AlgHom.ofLinearMap
    ...
    (by
      ...
      rw [@Units.smul_def _ _ (_) (_), ← intCast_smul_eq_zsmul R, map_smul, map_smul, map_smul]
      rw [intCast_smul_eq_zsmul R, ← @Units.smul_def _ _ (_) (_)]
...
@[simp] lemma comm_coe_tmul_coe {i j : ι} (a : 𝒜 i) (b : ℬ j) :
  ...
  (auxEquiv R ℬ 𝒜).injective <| by
    simp_rw [auxEquiv_comm, auxEquiv_tmul, decompose_coe, ← lof_eq_of R, gradedComm_of_tmul_of,
      @Units.smul_def _ _ (_) (_), ← intCast_smul_eq_zsmul R]
    -- Qualified `map_smul` to avoid a TC timeout #8386
