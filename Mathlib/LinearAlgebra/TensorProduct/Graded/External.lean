...
theorem gradedComm_of_tmul_of (i j : ι) (a : 𝒜 i) (b : ℬ j) :
  ...
  rw [TensorProduct.directSum_lof_tmul_lof, gradedCommAux_lof_tmul, Units.smul_def,
    ...
    ← intCast_smul_eq_zsmul R, LinearEquiv.map_smul, TensorProduct.directSum_symm_lof_tmul,
    intCast_smul_eq_zsmul, ← Units.smul_def]
...
theorem tmul_of_gradedMul_of_tmul (j₁ i₂ : ι)
  ...
  rw [Units.smul_def, Units.smul_def, ← intCast_smul_eq_zsmul R, ← intCast_smul_eq_zsmul R]
...
theorem gradedMul_assoc (x y z : DirectSum _ 𝒜 ⊗[R] DirectSum _ ℬ) :
  ...
  dsimp [mA]
  simp_rw [tmul_of_gradedMul_of_tmul, Units.smul_def, ← intCast_smul_eq_zsmul R,
  ...
  simp_rw [intCast_smul_eq_zsmul R, ← Units.smul_def, smul_smul, ← uzpow_add, add_mul, mul_add]
  congr 2
  abel
...
theorem gradedComm_gradedMul (x y : DirectSum _ 𝒜 ⊗[R] DirectSum _ ℬ) :
  ...
  -- Note: #8386 had to specialize `map_smul` to avoid timeouts.
  simp_rw [Units.smul_def, ← intCast_smul_eq_zsmul R, LinearEquiv.map_smul, LinearMap.map_smul,
    LinearMap.smul_apply]
  simp_rw [intCast_smul_eq_zsmul R, ← Units.smul_def, DirectSum.lof_eq_of, DirectSum.of_mul_of,
