...
## Tensor operations on submodules

* `Submodule.TensorProduct p q` : for `p : Submodule R M` and
`q : Submodule R N`, this is the submodule of `M ⊗[R] N`
generated by the image of `p ⊗[R] q`.
...
theorem tensorProduct_def :
    p.TensorProduct q = (TensorProduct.mapIncl p q).range := rfl
...
variable (q) in
lemma disjoint_left (hpp' : IsCompl p p') :
    Disjoint (TensorProduct p q) (TensorProduct p' q) := by
  rw [disjoint_def]
  rintro x ⟨y, h⟩ ⟨y', h'⟩
  rw [← id_apply x (R := R), ← rTensor_id, ← subtype_comp_projectionOnto_add_eq_id hpp']
  simp only [rTensor_add, rTensor_comp, add_apply, coe_comp, Function.comp_apply]
  nth_rewrite 2 [← h]
  nth_rewrite 1 [← h']
  simp [projectionOnto_comp_subtype_eq_zero]
...
lemma disjoint_right (hqq' : IsCompl q q') :
  ...
  rw [disjoint_def]
  rintro x ⟨y, h⟩ ⟨y', h'⟩
  rw [← id_apply x (R := R), ← lTensor_id, ← subtype_comp_projectionOnto_add_eq_id hqq']
...
variable (N) in
theorem isCompl_left_top (h : IsCompl p p') :
    IsCompl (TensorProduct p (⊤ : Submodule R N)) (TensorProduct p' ⊤) := by
  apply IsCompl.mk
  · exact disjoint_left ⊤ h
  · rw [codisjoint_iff, ← sup_left, codisjoint_iff.mp h.codisjoint]
    exact top_top R M N

variable (M) in
theorem isCompl_right_top (h : IsCompl q q') :
    IsCompl (TensorProduct (⊤ : Submodule R M) q) (TensorProduct ⊤ q') := by
  apply IsCompl.mk
  · exact disjoint_right ⊤ h
  · rw [codisjoint_iff, ← sup_right, codisjoint_iff.mp h.codisjoint]
    exact top_top R M N

theorem isCompl_left_left (hM : IsCompl p p') (hN : IsCompl q q') :
    IsCompl (TensorProduct p q) (TensorProduct ⊤ q' ⊔ TensorProduct p' ⊤) := by
  suffices TensorProduct p q' ⊔ TensorProduct p' ⊤
      = TensorProduct ⊤ q' ⊔ p'.TensorProduct ⊤ by
    rw [← this]
    apply Submodule.isCompl_assoc_of_disjoint_left
    · exact disjoint_right p hN
