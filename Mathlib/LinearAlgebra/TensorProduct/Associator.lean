...
lemma assoc_tensor :
    TensorProduct.assoc R (M ⊗[R] N) Q S = LinearEquiv.rTensor S (TensorProduct.assoc R M N Q)
    ≪≫ₗ TensorProduct.assoc R M (N ⊗[R] Q) S ≪≫ₗ LinearEquiv.lTensor M (TensorProduct.assoc R N Q S)
...
lemma assoc_tensor'' :
    ...
    ≪≫ₗ LinearEquiv.lTensor M (TensorProduct.assoc R N Q S) :=
  LinearEquiv.toLinearMap_inj.mp <| ext_fourfold' fun _ _ _ => congrFun rfl

lemma lid_tensor :
    TensorProduct.lid R (M ⊗[R] N)
    = (TensorProduct.assoc R R M N).symm ≪≫ₗ LinearEquiv.rTensor N (TensorProduct.lid R M) :=
