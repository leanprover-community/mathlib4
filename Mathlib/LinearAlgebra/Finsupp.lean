...
def lsum : (α → M →ₗ[R] N) ≃ₗ[S] (α →₀ M) →ₗ[R] N where
  ...
  left_inv F := by
  -- Porting note: first line of the following four proofs was `ext (x y)`, applied wrong ext lemmas
    apply Finsupp.lhom_ext'
    simp
  right_inv F := by
  -- Porting note: `repeat {Finsupp.lhom_ext'}` does not currently work
    apply Finsupp.lhom_ext'
    apply Finsupp.lhom_ext'
  map_add' F G := by
    apply Finsupp.lhom_ext'
...
theorem Span.finsupp_total_repr {w : Set M} (x : span R w) :
    Finsupp.total w M R coe (Span.repr R w x) = x := by
  rw [Span.repr]
