...
@[simp]
theorem lcongr_symm {ι κ : Sort*} (e₁ : ι ≃ κ) (e₂ : M ≃ₗ[R] N) :
...
theorem Submodule.exists_finset_of_mem_iSup {ι : Sort*} (p : ι → Submodule R M) {m : M}
...
theorem Submodule.mem_iSup_iff_exists_finset {ι : Sort*} {p : ι → Submodule R M} {m : M} :
