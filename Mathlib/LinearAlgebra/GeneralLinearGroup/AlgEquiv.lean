...
private def auxLinear [CommSemiring R] [Module R V] (f : End R V →ₗ[R] End R V)
    (y : V →ₗ[R] R) (z : V) : End R V :=
  applyₗ z ∘ₗ f ∘ₗ smulRightₗ y

@[simp] private theorem auxLinear_apply [CommSemiring R] [Module R V]
    (f : End R V →ₗ[R] End R V) (y : Dual R V) (x z : V) :
...
private theorem auxLinear_comp [CommSemiring R] [Module R V]
    (f : End R V →ₐ[R] End R V) (y : Dual R V) (z : V) (A : End R V) :
...
theorem AlgEquiv.coe_eq_linearEquiv_conjugate
  ...
  have surj : Function.Surjective T := by
    ...
    obtain ⟨d, hd⟩ := dual_exists_eq_one R this
