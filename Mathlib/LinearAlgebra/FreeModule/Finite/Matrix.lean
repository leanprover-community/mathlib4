...
instance (priority := 10000) Module.Free.linearMap [Module.Free S N] : Module.Free S (M →ₗ[R] N) :=
  Module.Free.of_equiv (linearMapEquivFun R S M N).symm
#align module.free.linear_map Module.Free.linearMap

instance (priority := 10000) Module.Finite.linearMap [Module.Finite S N] : Module.Finite S (M →ₗ[R] N) :=
...
instance (priority := 10000) Finite.algHom : Finite (M →ₐ[K] L) :=
...
instance (priority := 10000) Module.Finite.addMonoidHom [Module.Finite ℤ N] : Module.Finite ℤ (M →+ N) :=
  Module.Finite.equiv (addMonoidHomLequivInt ℤ).symm
#align module.finite.add_monoid_hom Module.Finite.addMonoidHom

instance (priority := 10000) Module.Free.addMonoidHom [Module.Free ℤ N] : Module.Free ℤ (M →+ N) :=
