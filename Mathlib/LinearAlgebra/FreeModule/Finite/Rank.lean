...
theorem finrank_quotient_eq_of_le_torsion {R M} [CommRing R] [AddCommGroup M] [Module R M]
    {N : Submodule R M} (hN : N ≤ torsion R M) : finrank R (M ⧸ N) = finrank R M := by
  rw [finrank, finrank, rank_quotient_eq_of_le_torsion hN]

lemma finrank_map_of_le_torsion {R M} [CommRing R] [AddCommGroup M] [Module R M]
    (N : Submodule R M) {M'} [AddCommGroup M'] [Module R M']
    (l : M →ₗ[R] M') (hN : N ⊓ LinearMap.ker l ≤ Submodule.torsion R M) :
    finrank R (N.map l) = finrank R N := by
  rw [finrank, finrank, ← toNat_lift, lift_rank_map_of_le_torsion N l hN, toNat_lift]

lemma finrank_of_surjective_of_le_torsion {R M} [CommRing R] [AddCommGroup M] [Module R M]
  ...
  rw [finrank, finrank, ← toNat_lift, lift_rank_of_surjective_of_le_torsion l hl hl', toNat_lift]

lemma FiniteDimensional.cast_finrank (R M) [Ring R] [StrongRankCondition R] [AddCommGroup M]
    [Module R M] [Module.Finite R M] : ↑(finrank R M) = Module.rank R M := by
  rw [finrank, cast_toNat_of_lt_aleph0]
  exact rank_lt_aleph0 R M

lemma Cardinal.cast_toNat_le {ω : Cardinal} : ↑(toNat ω) ≤ ω :=
  match lt_or_le ω ℵ₀ with
  | Or.inl h => (cast_toNat_of_lt_aleph0 h).le
  | Or.inr h => (cast_toNat_of_aleph0_le h).trans_le (zero_le _)

lemma FiniteDimensional.finrank_quotient_add_finrank_le {R M} [CommRing R] [AddCommGroup M]
    [StrongRankCondition R] [Module R M] [Module.Finite R M]
    (N : Submodule R M) :
    finrank R (M ⧸ N) + finrank R N ≤ finrank R M := by
  apply natCast_le.mp
  rw [Nat.cast_add, cast_finrank R M]
  exact (add_le_add cast_toNat_le cast_toNat_le).trans (rank_quotient_add_rank_le N)
