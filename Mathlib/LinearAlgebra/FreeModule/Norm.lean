...
theorem associated_norm_prod_smith [Fintype ι] (b : Basis ι R S) {f : S} (hf : f ≠ 0) :
  ...
  -- ⊢ Associated (↑(Algebra.norm R) f) (∏ i : ι, smithCoeffs b (span {f}) (_ : ¬sp …
...
theorem finrank_quotient_span_eq_natDegree_norm [Algebra F S] [IsScalarTower F F[X] S]
  ...
  -- ⊢ FiniteDimensional.finrank F (S ⧸ span {f}) = natDegree (↑(Algebra.norm F[X]) …
  ...
  -- ⊢ ∑ i : ι, FiniteDimensional.finrank F (F[X] ⧸ span {smithCoeffs b (span {f})  …
  -- finrank_quotient_eq_sum slow
  congr with i
  -- ⊢ FiniteDimensional.finrank F (F[X] ⧸ span {smithCoeffs b (span {f}) h i}) = n …
