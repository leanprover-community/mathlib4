...
theorem finrank_sup_span_singleton [Module.Finite K V] {p : Submodule K V} {v : V} (hv : v ∉ p) :
  ...
  rw [← Nat.add_left_inj, finrank_sup_add_finrank_inf_eq, add_assoc, Nat.add_left_cancel_iff,
    finrank_span_singleton (by aesop), Nat.left_eq_add, Submodule.finrank_eq_zero, eq_bot_iff]
  intro x
