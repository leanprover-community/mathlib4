...
private theorem free_and_finite_fin (n : ℕ) (N : Fin n → Type*) [∀ i, AddCommGroup (N i)]
    ...
    Module.Free R (MultilinearMap (.id R) N M₂) ∧
      Module.Finite R (MultilinearMap (.id R) N M₂) := by
  induction n with
  | zero =>
    have : IsEmpty (Fin Nat.zero) := inferInstanceAs (IsEmpty (Fin 0))
    exact
      ⟨Module.Free.of_equiv (constLinearEquivOfIsEmpty R N M₂ _),
        Module.Finite.equiv (constLinearEquivOfIsEmpty R N M₂ _)⟩
  | succ n ih =>
    suffices
      Module.Free R (N 0 →ₗ[R] MultilinearMap (.id R) (fun i : Fin n => N i.succ) M₂) ∧
        Module.Finite R (N 0 →ₗ[R] MultilinearMap (.id R) (fun i : Fin n => N i.succ) M₂) by
      cases this
      exact
        ⟨Module.Free.of_equiv (multilinearCurryLeftEquiv N M₂ _).symm,
          Module.Finite.equiv (multilinearCurryLeftEquiv N M₂ _).symm⟩
...
private theorem free_and_finite :
    Module.Free R (MultilinearMap (.id R) M₁ M₂) ∧
      Module.Finite R (MultilinearMap (.id R) M₁ M₂) := by
  cases nonempty_fintype ι
  have := @free_and_finite_fin R M₂ _ _ _ _ _ (Fintype.card ι)
    (fun x => M₁ ((Fintype.equivFin ι).symm x))
  obtain ⟨l, r⟩ := this
  have e := domDomCongrLinearEquiv' R M₁ M₂ (.id R) (Fintype.equivFin ι)
