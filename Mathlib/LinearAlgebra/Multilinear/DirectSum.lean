...
variable {R S ι M' : Type*} {κ : ι → Type*} {M : (i : ι) → κ i → Type*}
variable [Semiring R] [CommSemiring S] {σ : R →+* S}
variable [∀ i j, AddCommMonoid (M i j)] [∀ i j, Module R (M i j)] [AddCommMonoid M'] [Module S M']
...
theorem directSum_ext [Finite ι] [(i : ι) → DecidableEq (κ i)]
    ⦃f g : MultilinearMap σ (fun i ↦ ⨁ j : κ i, M i j) M'⦄
    (h : ∀ p : (i : ι) → κ i,
...
noncomputable def fromDirectSumEquiv [Finite ι] :
    ((p : (i : ι) → κ i) → MultilinearMap σ (fun i ↦ M i (p i)) M') ≃ₗ[S]
    MultilinearMap σ (fun i ↦ ⨁ j : κ i, M i j) M' :=
...
theorem fromDirectSumEquiv_lof [Finite ι] [(i : ι) → DecidableEq (κ i)]
    (f : (p : (i : ι) → κ i) → MultilinearMap σ (fun i ↦ M i (p i)) M')
...
theorem fromDirectSumEquiv_symm_apply [Finite ι] [(i : ι) → DecidableEq (κ i)]
    (f : MultilinearMap σ (fun i ↦ ⨁ j : κ i, M i j) M')
    (p : (i : ι) → κ i) :
