...
variable [Semiring R] [CommSemiring S] {σ : R →+* S}
  [∀ i, AddCommMonoid (M i)] [AddCommMonoid M'] [AddCommMonoid M₂]
  [∀ i, Module R (M i)] [Module R M'] [Module S M₂]
...
@[simp]
theorem coe_currySumEquiv : ⇑(currySumEquiv (σ := σ) (N := N) (M₂ := M₂)) = currySum :=
  rfl

@[simp]
theorem coe_currySumEquiv_symm : ⇑(currySumEquiv (σ := σ) (N := N) (M₂ := M₂)).symm = uncurrySum :=
  rfl
