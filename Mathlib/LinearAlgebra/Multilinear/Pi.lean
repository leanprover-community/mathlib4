...
-/
@[simps]
def piFamily (f : Π (p : Π i, κ i), MultilinearMap σ (fun i ↦ M i (p i)) (N p)) :
...
theorem piFamily_single_left_apply [Fintype ι] [∀ i, DecidableEq (κ i)]
    (p : Π i, κ i) (f : MultilinearMap σ (fun i ↦ M i (p i)) (N p)) (x : Π i j, M i j) :
...
theorem piFamily_single_left [Fintype ι] [∀ i, DecidableEq (κ i)]
    ...
    piFamily (Pi.single p f) =
      (LinearMap.single S _ p).compMultilinearMap (f.compLinearMap fun i => .proj (p i)) :=
...
theorem piFamily_zero :
    piFamily (0 : Π (p : Π i, κ i), MultilinearMap σ (fun i ↦ M i (p i)) (N p)) = 0 := by
  ext; simp

@[simp]
theorem piFamily_add (f g : Π (p : Π i, κ i), MultilinearMap σ (fun i ↦ M i (p i)) (N p)) :
