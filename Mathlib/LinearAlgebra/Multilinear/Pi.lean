...
theorem pi_ext [Finite ι] [∀ i, Finite (κ i)] [∀ i, DecidableEq (κ i)]
    ⦃f g : (fun i ↦ Π j : κ i, M i j) →ₘₗ[R] N⦄
    (h : ∀ p : Π i, κ i,
...
-/
@[simps]
def piFamily (f : Π (p : Π i, κ i), (fun i ↦ M i (p i)) →ₘₗ[R] N p) :
...
theorem piFamily_single_left_apply [Fintype ι] [∀ i, DecidableEq (κ i)]
    (p : Π i, κ i) (f : (fun i ↦ M i (p i)) →ₘₗ[R] N p) (x : Π i j, M i j) :
...
theorem piFamily_zero :
    piFamily (0 : Π (p : Π i, κ i), (fun i ↦ M i (p i)) →ₘₗ[R] N p) = 0 := by
  ext; simp

@[simp]
theorem piFamily_add (f g : Π (p : Π i, κ i), (fun i ↦ M i (p i)) →ₘₗ[R] N p) :
