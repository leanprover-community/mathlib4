...
structure SimultaneousDiagonalization (ι : Type*) (f : α → End S M) extends Basis ι S M where
  ...
  μ : α → ι → S
  /--
  We don't use `Module.End.HasEigenvector`, as it's slightly more convenient for all the
  degenerate bases over the trivial ring to be considered eigenbases.
  -/
...
variable {ι : Type*} {f : α → End S M} {b : Basis ι S M} {D D₁ D₂ : SimultaneousDiagonalization ι f}
...
-/
noncomputable def ofExists (exists_apply_toBasis : ∀ a i, ∃ μ : S, f a (b i) = μ • b i) :
...
noncomputable def ofIsDiagToMatrix [Fintype ι] [DecidableEq ι] {f : α → End C M'} {b : Basis ι C M'}
    (h : ∀ a, ((f a).toMatrix b b).IsDiag) : SimultaneousDiagonalization ι f where
  toBasis := b
...
def diagonalization (a : α) : (f a).Diagonalization ι := .mk (D.apply_toBasis a)

/-- Construct a simultaneous diagonalization from a family of diagonalizations. -/
def ofDiagonalization {D : (a : α) → (f a).Diagonalization ι} (h : ∀ a, (D a).toBasis = b) :
    SimultaneousDiagonalization ι f where
  toBasis := b
  μ a i := (D a).μ i
...
def smul {f : α → End C M'} (D : SimultaneousDiagonalization ι f) (c : α → C) :
...
def subSmulOne {f : α → End R G} (D : SimultaneousDiagonalization ι f) (c : α → R) :
...
def diagonalizationAdd (i j : α) : (f i + f j).Diagonalization ι :=
...
def diagonalizationSum [Fintype α] : (∑ a, f a).Diagonalization ι :=
  .mk (b := D.toBasis) (μ := ∑ a, D.μ a) <| fun k ↦ by simp [D.apply_toBasis, Finset.sum_smul]

variable (D) in
/-- Any simultaneous diagonalization of `f` also diagonalizes `f i * f j` for any `i` and `j`. -/
def diagonalizationMul (i j : α) : (f i * f j).Diagonalization ι :=
  .mk (b := D.toBasis) (μ := D.μ j * D.μ i) <| fun k ↦ by simp [D.apply_toBasis, smul_smul]

lemma commute {f : α → End C M'} (D : SimultaneousDiagonalization ι f) (i j : α) :
    Commute (f i) (f j) :=
...
variable {ι : Type*} {f : End S M} {b : Basis ι S M} {D D₁ D₂ : f.Diagonalization ι}

@[ext]
lemma ext (h : D₁.toBasis = D₂.toBasis) : D₁ = D₂ := SimultaneousDiagonalization.ext h

/-- Alternative constructor for `LinearMap.Diagonalization` from existence of eigenvalues. -/
noncomputable def ofExists (exists_apply_toBasis : ∀ i, ∃ μ : S, f (b i) = μ • b i) :
    f.Diagonalization ι :=
...
theorem hasEigenVector_μ [Nontrivial R] {f : End R G} (D : f.Diagonalization ι) (i : ι) :
...
lemma charpoly_eq [Nontrivial R] [Fintype ι] [DecidableEq ι] [Free R G] [Module.Finite R G]
    {f : End R G} (D : f.Diagonalization ι) :
...
lemma iSup_eigenspace {f : End R G} (D : f.Diagonalization ι) : ⨆ μ, f.eigenspace μ = ⊤ := by
  nontriviality R
