...
@[simp, norm_cast]
lemma coe_multilinearMap : ⇑(f : MultilinearMap R (fun _ : ι => M) N) = f :=
  rfl
#align alternating_map.coe_multilinear_map AlternatingMap.coe_multilinearMap

lemma coe_multilinearMap_injective :
...
-- @[simp]
lemma coe_multilinearMap_mk (f : (ι → M) → N) (h₁ h₂ h₃) :
...
@[simp]
lemma map_add [DecidableEq ι] (i : ι) (x y : M) :
...
@[simp]
lemma map_sub [DecidableEq ι] (i : ι) (x y : M') :
...
@[simp]
lemma map_neg [DecidableEq ι] (i : ι) (x : M') : g' (update v' i (-x)) = -g' (update v' i x) :=
  g'.toMultilinearMap.map_neg v' i x
#align alternating_map.map_neg AlternatingMap.map_neg

@[simp]
lemma map_smul [DecidableEq ι] (i : ι) (r : R) (x : M) :
...
@[simp]
lemma map_update_zero [DecidableEq ι] (m : ι → M) (i : ι) : f (update m i 0) = 0 :=
...
lemma map_eq_zero_of_not_injective (v : ι → M) (hv : ¬Function.Injective v) : f v = 0 := by
...
@[simp]
lemma coe_prod (f : AlternatingMap R M N ι) (g : AlternatingMap R M P ι) :
...
@[simp]
lemma coe_pi {ι' : Type*} {N : ι' → Type*} [∀ i, AddCommMonoid (N i)] [∀ i, Module R (N i)]
...
@[simp]
lemma coe_smulRight {R M₁ M₂ ι : Type*} [CommSemiring R] [AddCommMonoid M₁] [AddCommMonoid M₂]
...
@[simp]
lemma add_apply : (f + f') v = f v + f' v :=
  rfl
#align alternating_map.add_apply AlternatingMap.add_apply

@[norm_cast]
lemma coe_add : (↑(f + f') : MultilinearMap R (fun _ : ι => M) N) = f + f' :=
  rfl
...
@[simp]
lemma zero_apply : (0 : AlternatingMap R M N ι) v = 0 :=
  rfl
#align alternating_map.zero_apply AlternatingMap.zero_apply

@[norm_cast]
lemma coe_zero : ((0 : AlternatingMap R M N ι) : MultilinearMap R (fun _ : ι => M) N) = 0 :=
  rfl
...
@[simp]
lemma neg_apply (m : ι → M) : (-g) m = -g m :=
  rfl
#align alternating_map.neg_apply AlternatingMap.neg_apply

@[norm_cast]
lemma coe_neg : ((-g : AlternatingMap R M N' ι) : MultilinearMap R (fun _ : ι => M) N') = -g :=
  rfl
...
@[simp]
lemma sub_apply (m : ι → M) : (g - g₂) m = g m - g₂ m :=
  rfl
#align alternating_map.sub_apply AlternatingMap.sub_apply

@[norm_cast]
lemma coe_sub : (↑(g - g₂) : MultilinearMap R (fun _ : ι => M) N') = g - g₂ :=
  rfl
...
@[simp]
lemma subtype_compAlternatingMap_codRestrict (f : AlternatingMap R M N ι) (p : Submodule R N)
...
@[simp]
lemma compAlternatingMap_codRestrict (g : N →ₗ[R] N₂) (f : AlternatingMap R M N ι)
    (p : Submodule R N₂) (h) :
...
lemma coe_compLinearMap (f : AlternatingMap R M N ι) (g : M₂ →ₗ[R] M) :
    ⇑(f.compLinearMap g) = f ∘ (· ∘ ·) g :=
  rfl
#align alternating_map.coe_comp_linear_map AlternatingMap.coe_compLinearMap

@[simp]
lemma compLinearMap_apply (f : AlternatingMap R M N ι) (g : M₂ →ₗ[R] M) (v : ι → M₂) :
...
lemma compLinearMap_assoc (f : AlternatingMap R M N ι) (g₁ : M₂ →ₗ[R] M) (g₂ : M₃ →ₗ[R] M₂) :
...
@[simp]
lemma zero_compLinearMap (g : M₂ →ₗ[R] M) : (0 : AlternatingMap R M N ι).compLinearMap g = 0 := by
  ext
  simp only [compLinearMap_apply, zero_apply]
#align alternating_map.zero_comp_linear_map AlternatingMap.zero_compLinearMap

@[simp]
lemma add_compLinearMap (f₁ f₂ : AlternatingMap R M N ι) (g : M₂ →ₗ[R] M) :
...
@[simp]
lemma compLinearMap_zero [Nonempty ι] (f : AlternatingMap R M N ι) :
...
@[simp]
lemma compLinearMap_id (f : AlternatingMap R M N ι) : f.compLinearMap LinearMap.id = f :=
  ext fun _ => rfl
...
lemma compLinearMap_injective (f : M₂ →ₗ[R] M) (hf : Function.Surjective f) :
...
lemma compLinearMap_inj (f : M₂ →ₗ[R] M) (hf : Function.Surjective f)
...
@[simp]
lemma domLCongr_refl : domLCongr R N ι S (LinearEquiv.refl R M) = LinearEquiv.refl S _ :=
  LinearEquiv.ext fun _ => AlternatingMap.ext fun _ => rfl
#align alternating_map.dom_lcongr_refl AlternatingMap.domLCongr_refl

@[simp]
lemma domLCongr_symm (e : M ≃ₗ[R] M₂) : (domLCongr R N ι S e).symm = domLCongr R N ι S e.symm :=
  rfl
#align alternating_map.dom_lcongr_symm AlternatingMap.domLCongr_symm

lemma domLCongr_trans (e : M ≃ₗ[R] M₂) (f : M₂ ≃ₗ[R] M₃) :
...
@[simp]
lemma compLinearEquiv_eq_zero_iff (f : AlternatingMap R M N ι) (g : M₂ ≃ₗ[R] M) :
...
lemma map_update_sum {α : Type*} [DecidableEq ι] (t : Finset α) (i : ι) (g : α → M) (m : ι → M) :
...
lemma map_update_self [DecidableEq ι] {i j : ι} (hij : i ≠ j) :
...
lemma map_update_update [DecidableEq ι] {i j : ι} (hij : i ≠ j) (m : M) :
...
lemma map_swap_add [DecidableEq ι] {i j : ι} (hij : i ≠ j) :
...
lemma map_congr_perm [DecidableEq ι] [Fintype ι] (σ : Equiv.Perm ι) :
...
@[simp]
lemma domDomCongr_refl (f : AlternatingMap R M N ι) : f.domDomCongr (Equiv.refl ι) = f :=
  ext fun _ => rfl
#align alternating_map.dom_dom_congr_refl AlternatingMap.domDomCongr_refl

lemma domDomCongr_trans (σ₁ : ι ≃ ι') (σ₂ : ι' ≃ ι'') (f : AlternatingMap R M N ι) :
...
@[simp]
lemma domDomCongr_zero (σ : ι ≃ ι') : (0 : AlternatingMap R M N ι).domDomCongr σ = 0 :=
  rfl
#align alternating_map.dom_dom_congr_zero AlternatingMap.domDomCongr_zero

@[simp]
lemma domDomCongr_add (σ : ι ≃ ι') (f g : AlternatingMap R M N ι) :
    (f + g).domDomCongr σ = f.domDomCongr σ + g.domDomCongr σ :=
  rfl
#align alternating_map.dom_dom_congr_add AlternatingMap.domDomCongr_add

@[simp]
lemma domDomCongr_smul {S : Type*} [Monoid S] [DistribMulAction S N] [SMulCommClass R S N]
...
@[simp]
lemma domDomLcongr_toAddEquiv (σ : ι ≃ ι') :
...
@[simp]
lemma domDomCongr_eq_iff (σ : ι ≃ ι') (f g : AlternatingMap R M N ι) :
...
@[simp]
lemma domDomCongr_eq_zero_iff (σ : ι ≃ ι') (f : AlternatingMap R M N ι) :
...
lemma domDomCongr_perm [Fintype ι] [DecidableEq ι] (σ : Equiv.Perm ι) :
...
lemma map_linearDependent {K : Type*} [Ring K] {M : Type*} [AddCommGroup M] [Module K M]
...
lemma map_vecCons_add {n : ℕ} (f : AlternatingMap R M N (Fin n.succ)) (m : Fin n → M) (x y : M) :
...
lemma map_vecCons_smul {n : ℕ} (f : AlternatingMap R M N (Fin n.succ)) (m : Fin n → M) (c : R)
...
private lemma alternization_map_eq_zero_of_eq_aux (m : MultilinearMap R (fun _ : ι => M) N')
...
lemma alternatization_def (m : MultilinearMap R (fun _ : ι => M) N') :
...
lemma alternatization_coe (m : MultilinearMap R (fun _ : ι => M) N') :
    ↑(alternatization m) = (∑ σ : Perm ι, Equiv.Perm.sign σ • m.domDomCongr σ : _) :=
  coe_injective rfl
#align multilinear_map.alternatization_coe MultilinearMap.alternatization_coe

lemma alternatization_apply (m : MultilinearMap R (fun _ : ι => M) N') (v : ι → M) :
...
lemma coe_alternatization [DecidableEq ι] [Fintype ι] (a : AlternatingMap R M N' ι) :
...
lemma compMultilinearMap_alternatization (g : N' →ₗ[R] N'₂)
...
lemma ModSumCongr.swap_smul_involutive {α β : Type*} [DecidableEq (Sum α β)] (i j : Sum α β) :
...
lemma domCoprod.summand_mk'' (a : AlternatingMap R' Mᵢ N₁ ιa) (b : AlternatingMap R' Mᵢ N₂ ιb)
...
lemma domCoprod.summand_add_swap_smul_eq_zero (a : AlternatingMap R' Mᵢ N₁ ιa)
...
on the quotient. -/
lemma domCoprod.summand_eq_zero_of_smul_invariant (a : AlternatingMap R' Mᵢ N₁ ιa)
    (b : AlternatingMap R' Mᵢ N₂ ιb) (σ : Perm.ModSumCongr ιa ιb) {v : Sum ιa ιb → Mᵢ}
    {i j : Sum ιa ιb} (hv : v i = v j) (hij : i ≠ j) :
...
lemma domCoprod_coe (a : AlternatingMap R' Mᵢ N₁ ιa) (b : AlternatingMap R' Mᵢ N₂ ιb) :
...
@[simp]
lemma domCoprod'_apply (a : AlternatingMap R' Mᵢ N₁ ιa) (b : AlternatingMap R' Mᵢ N₂ ιb) :
...
lemma MultilinearMap.domCoprod_alternization_coe [DecidableEq ιa] [DecidableEq ιb]
...
-/
lemma MultilinearMap.domCoprod_alternization [DecidableEq ιa] [DecidableEq ιb]
    (a : MultilinearMap R' (fun _ : ιa => Mᵢ) N₁) (b : MultilinearMap R' (fun _ : ιb => Mᵢ) N₂) :
...
-/
lemma MultilinearMap.domCoprod_alternization_eq [DecidableEq ιa] [DecidableEq ιb]
...
lemma Basis.ext_alternating {f g : AlternatingMap R' N₁ N₂ ι} (e : Basis ι₁ R' N₁)
...
@[simp]
lemma curryLeft_zero {n : ℕ} : curryLeft (0 : AlternatingMap R' M'' N'' (Fin n.succ)) = 0 :=
  rfl
#align alternating_map.curry_left_zero AlternatingMap.curryLeft_zero

@[simp]
lemma curryLeft_add {n : ℕ} (f g : AlternatingMap R' M'' N'' (Fin n.succ)) :
    curryLeft (f + g) = curryLeft f + curryLeft g :=
  rfl
#align alternating_map.curry_left_add AlternatingMap.curryLeft_add

@[simp]
lemma curryLeft_smul {n : ℕ} (r : R') (f : AlternatingMap R' M'' N'' (Fin n.succ)) :
...
@[simp]
lemma curryLeft_same {n : ℕ} (f : AlternatingMap R' M'' N'' (Fin n.succ.succ)) (m : M'') :
...
@[simp]
lemma curryLeft_compAlternatingMap {n : ℕ} (g : N'' →ₗ[R'] N₂'')
...
@[simp]
lemma curryLeft_compLinearMap {n : ℕ} (g : M₂'' →ₗ[R'] M'')
    (f : AlternatingMap R' M'' N'' (Fin n.succ)) (m : M₂'') :
