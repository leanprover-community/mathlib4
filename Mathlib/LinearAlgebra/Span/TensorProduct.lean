...
variable {R : Type*} (A : Type*) {M : Type*} [CommSemiring R] [CommSemiring A] [Algebra R A]
    [AddCommMonoid M] [Module R M] [Module A M] [IsScalarTower R A M] (s : Set M)
...
noncomputable def tensorSpanEquivSpan : A ⊗[R] span R s ≃ₗ[A] span A s :=
