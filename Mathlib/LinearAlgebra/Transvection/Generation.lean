...
example : e ∈ dilatransvections K V ^ (finrank K (V ⧸ e.fixedSubmodule)) := by
  ...
  | succ n hind =>
    ...
    · simp only [mem_sup] at hv'
      ...
      simp_all
    · -- this case should run more smoothly, and `f` can be
      -- taken to be a transvection.
      -- Note : we are certain that this case can happen for
      -- an appropriate `v` iff `e.fixedReduce` is not a homothety
      suffices ∃ l : Dual K V, e.fixedSubmodule.map l = ⊥ ∧ l v = 1 ∧ l (e v) = 1 by
        obtain ⟨l, hle, hlv, hlev⟩ := this
        replace hlv : l (v - e v) = 0 := by simp_all
        replace hle (x : V) (hx : x ∈ e.fixedSubmodule) : l x = 0 := by
          rw [← Submodule.mem_bot K, ← hle]
          exact mem_map_of_mem hx
        refine ⟨LinearEquiv.transvection hlv, ?_, fun x hx ↦ ?_, ?_⟩
        · exact transvection_mem_dilatransvections hlv
        · rw [mem_fixedSubmodule_iff, transvection.apply, hle x hx, zero_smul, add_zero]
        · rw [transvection.apply]
          simp [hlev]
      sorry

example (W : Submodule K V) (l : Dual K V) (v : V) (hv : v ∉ W) (a : K) :
    ∃ l' : Dual K V, (∀ x ∈ W, l' x = l x) ∧ l' v = a := by
  suffices ∃ f : Dual K (V ⧸ W), f (W.mkQ v) = a - l v by
    obtain ⟨f, hf⟩ := this
    refine ⟨l + f ∘ₗ W.mkQ, fun x hx ↦ ?_, ?_⟩
    · suffices W.mkQ x = 0 by simp [this]
      rwa [← LinearMap.mem_ker, ker_mkQ]
    · simp_all
  obtain ⟨f, _, hfv⟩ := Submodule.exists_dual_map_eq_bot_of_notMem (p := ⊥) (x := W.mkQ v) (R := K) ?_ inferInstance
  sorry
  sorry
