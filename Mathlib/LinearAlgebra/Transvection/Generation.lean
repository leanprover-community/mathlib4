...
example : e ∈ dilatransvections K V ^ (finrank K (V ⧸ e.fixedSubmodule)) := by
  ...
  | succ n hind =>
    ...
    · simp only [mem_sup] at hv'
      ...
      let y := (c * l v)⁻¹ • (-x + (1 - c) • v)
      have hy : y ∈ e.fixedSubmodule ⊔ K ∙ v := sorry
      ...
      sorry
      -- f z = z + l z • w
      -- l nulle sur e.fixedSubmodule, l w  ≠ - 1
      -- f (e v) = f (x + c • v) = x + c • v + c • l v • w = v ?
      -- (c * l v) • w = (1 - c) • v - x
      -- (c * l v) ≠ (c - 1) * l v
      -- l v ≠ 0 ?
...
theorem Fin.snoc_apply_of_val_lt {n : ℕ} {α : Fin (n + 1) → Sort*}
    ...
    Fin.snoc g a i = g ⟨i, hi⟩ := by
  simp only [Fin.snoc, Fin.castSucc_castLT, cast_eq, dif_pos hi]
  rfl
  -- explicit proof:
