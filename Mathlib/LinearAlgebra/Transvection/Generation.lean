...
theorem mem_center_of_linearMap_apply_eq_smul
    {R V : Type*} [Semiring R] [AddCommMonoid V] [Module R V]
    [h : FaithfulSMul R V]
    {f : V →ₗ[R] V} {a : R} (hfa : ∀ x, f x = a • x) :
    a ∈ Set.center R := by
  rcases subsingleton_or_nontrivial V with hV | hV
  · convert Set.one_mem_center
    exact h.eq_of_smul_eq_smul (fun _ ↦ Subsingleton.allEq _ _)
  rw [Semigroup.mem_center_iff]
  intro b
  apply h.eq_of_smul_eq_smul
  intro x
  simp only [mul_smul, ← hfa, LinearMap.map_smul]

/- When `finrank K V = 1`, up to a linear equivalence `V ≃ₗ[K] K`,
then any `f` is *right*-multiplication by some `a : K`,
but not necessarily *left*-multiplication by an element of the center of `K`,
unless `K` is commutative, of course. -/
theorem LinearMap.exists_mem_center_apply_eq_smul
    {K V : Type*} [DivisionRing K] [AddCommGroup V] [Module K V]
    {f : V →ₗ[K] V}
    (hV1 : finrank K V ≠ 1)
    (h : ∀ v, ¬ LinearIndependent K ![v, f v]) :
    ∃ a ∈ Set.center K, ∀ x, f x = a • x := by
  rcases subsingleton_or_nontrivial V with hV | hV
  · use 1
    simp only [Set.one_mem_center, one_smul, true_and]
    intro x
    apply hV.allEq
  obtain ⟨u, hu⟩ := exists_ne (0 : V)
  have h' (v : V) (hv : v ≠ 0) : ∃ a : K, f v = a • v := by
  ...
  use φ ⟨u, hu⟩
  suffices _ by
    exact ⟨mem_center_of_linearMap_apply_eq_smul this, this⟩
  intro v
  by_cases hv : v = 0
  · simp [hv]
  ...
  · simp only [hφ ⟨v, hv⟩]
    suffices ∃ w, LinearIndependent K ![u, w] by
      obtain ⟨w, huw⟩ := this
      have hw : w ≠ 0 := huw.ne_zero (Fin.last _)
      rw [H ⟨u, hu⟩ ⟨w, hw⟩ huw]
      rw [H ⟨v, hv⟩ ⟨w, hw⟩ ?_]
      ...
      -- TODO : make a lemma from this!
      suffices s ≠ 0 by
        use - (1 / s) * t
        rw [← sub_eq_zero, neg_mul, neg_smul, sub_neg_eq_add]
        rw [← IsUnit.smul_left_cancel (Ne.isUnit this)]
        simp [← mul_smul, ← mul_assoc, mul_inv_cancel₀ this, hst]
      contrapose hu
      simpa [hu, hst' hu] using hst
    contrapose! hV1
    ...
    specialize hV1 w
    simp only [Nat.succ_eq_add_one, Nat.reduceAdd,
      LinearIndependent.pair_iff, not_forall] at hV1
    obtain ⟨x, y, ⟨hxy, hV⟩⟩ := hV1
    rw [and_comm, not_and] at hV
    -- TODO : the above lemma!
    suffices y ≠ 0 by
      use -(1/y) * x
      rw [eq_comm, ← sub_eq_zero, neg_mul, neg_smul, sub_neg_eq_add]
      rw [add_comm, ← IsUnit.smul_left_cancel (Ne.isUnit this)]
      simp [← mul_smul, ← mul_assoc, mul_inv_cancel₀ this, hxy]
    contrapose hu
    simpa [hu, hV hu] using hxy
...
@[simp]
theorem inv_mem_transvections {e : V ≃ₗ[R] V} :
...
theorem mem_dilatransvections_def {u : V ≃ₗ[R] V} :
...
theorem one_mem_dilatransvections : (1 : V ≃ₗ[R] V) ∈ dilatransvections R V := by
  simp only [mem_dilatransvections_def]
  nontriviality R
...
theorem inv_mem_dilatransvections_iff {u : V ≃ₗ[R] V} [IsNoetherian R V] [StrongRankCondition R] :
  ...
  suffices u1 - id (R := R) = v.comp (- u1) by
    rw [mem_dilatransvections_def, ← hu1, this]
    rw [mem_dilatransvections_def, ← hv] at hu
    apply le_trans _ hu
    apply finrank_mono
...
theorem fixedSubmodule_def (e : V ≃ₗ[R] V) :
    e.fixedSubmodule = LinearMap.ker ((e : V →ₗ[R] V) - id (R := R)) := rfl

theorem mem_dilatransvections_iff (e : V ≃ₗ[R] V) :
    e ∈ dilatransvections R V ↔ finrank R (V ⧸ e.fixedSubmodule) ≤ 1 := by
  rw [mem_dilatransvections_def, ← (quotKerEquivRange _).finrank_eq, ← fixedSubmodule_def]
...
theorem Submodule.le_comap_of_mem_stabilizer
    (W : Submodule R V) (u : V ≃ₗ[R] V) (hu : u ∈ stabilizer (V ≃ₗ[R] V) W) :
    W ≤ comap u W :=
  map_le_iff_le_comap.mp (le_of_eq hu)

section

variable [StrongRankCondition R] [HasRankNullity.{uV, uR} R]

theorem Module.finrank_quotient_le_finrank_quotient_iff
    {W W' : Submodule R V} [Module.Finite R V] :
    finrank R (V ⧸ W) ≤ finrank R (V ⧸ W') ↔ finrank R W' ≤ finrank R W := by
  rw [← Nat.add_le_add_iff_right, finrank_quotient_add_finrank,
    ← finrank_quotient_add_finrank W', add_le_add_iff_left]

theorem Module.finrank_quotient_eq_zero_iff
    {W W' : Submodule R V} [Module.Finite R V] :
    finrank R (V ⧸ W) ≤ finrank R (V ⧸ W') ↔ finrank R W' ≤ finrank R W := by
  rw [← Nat.add_le_add_iff_right, finrank_quotient_add_finrank,
    ← finrank_quotient_add_finrank W', add_le_add_iff_left]
...
variable {K : Type uK} [DivisionRing K] {V : Type uV} [AddCommGroup V] [Module K V]
...
-- The assumption that `V` is a finite `K`-module is useless.
theorem transvection_mem_dilatransvections [Module.Finite K V]
    {f : Dual K V} {v : V} (hfv : f v = 0) :
    transvection hfv ∈ dilatransvections K V := by
  rw [mem_dilatransvections_iff]
  rw [← Nat.add_le_add_iff_right, finrank_quotient_add_finrank,
  ...
  · apply le_trans (finrank_le _)
    simp
  · apply finrank_mono
    exact ker_le_fixedSubmodule_transvection hfv

-- The assumption that `V` is a finite `K`-module is useless.
theorem transvections_subset_dilatransvections [Module.Finite K V] :
    transvections K V ⊆ dilatransvections K V := fun x ↦ by
  rw [mem_transvections_iff]
  rintro ⟨f, v, hv, rfl⟩
  apply transvection_mem_dilatransvections
...
theorem mem_transvections_iff' [Module.Finite K V] (e : V ≃ₗ[K] V) :
  ...
  · by_cases he_one : e = 1
    ...
    rw [mem_dilatransvections_iff] at he
...
/-- If `e` is a product of `n` dilatransvections, then `V ⧸ e.fixedSubmodule` has rank `≤ n`. -/
...
theorem finrank_le_one_add_finrank_fixedSubmodule_of_mem_transvections
    {t : V ≃ₗ[K] V} (ht : t ∈ transvections K V) :
    finrank K V ≤ 1 + finrank K t.fixedSubmodule := by
  obtain ⟨f, v, hfv, rfl⟩ := ht
  have := Submodule.finrank_mono (ker_le_fixedSubmodule_transvection hfv)
...
abbrev IsExceptional (e : V ≃ₗ[K] V) : Prop :=
  fixedReduce e ≠ 1 ∧ ∃ a : K, ∀ x, fixedReduce e x = a • x

/-- If `e : V ≃ₗ[K] V` is such that `e.fixedReduce = 1`,
...
theorem mem_dilatransvections_pow_of_fixedReduce_eq_one
  ...
  | succ n hind =>
    match n with
    | 0 => simp [mem_dilatransvections_iff, h]
    | n + 1 =>
      ...
      have hv_notMem : v ∉ e.fixedSubmodule := by
        simpa only [v, smul_mem_iff _ (inv_ne_zero hfu)]
      ...
      · rw [inv_mem_dilatransvections_iff, ht]
        apply transvection_mem_dilatransvections
      set e' := t * e with he'
...
most `finrank K (V ⧸ e.fixedSubmodule)` dilatransvections.

This is the second non-exceptional case in Dieudonné's theorem. -/
theorem mem_dilatransvections_pow_of_fixedReduce_ne_smul_id
    {e : V ≃ₗ[K] V}
  ...
  | succ n hind =>
    ...
    | n + 1 =>
      ...
      have : ∃ v, LinearIndependent K ![v, e.fixedReduce v] := by
        contrapose! he
        obtain ⟨a, ha, he⟩ :=
          LinearMap.exists_mem_center_apply_eq_smul (by simp [h]) he
        exact ⟨a, he⟩
      obtain ⟨v, hu⟩ := this
      have hu_spare := id hu
      ...
      · -- this is the easy case where one knows that `e'` is
        ...
        rw [← e'_def, pow_succ', Set.mem_mul]
        refine ⟨(t hf)⁻¹, ?_, e', he', rfl⟩
        rw [inv_mem_dilatransvections_iff]
        apply transvection_mem_dilatransvections
      -- in the remaining case, one has `0 < n`
      have hn_pos : 0 < n := by
        contrapose! he'
        simp only [nonpos_iff_eq_zero] at he'
        simp only [he', zero_add, pow_one, mem_dilatransvections_iff]
        rw [he'_fixed hf hfv, he'_rank', he']
      ...
      -- TODO : do a lemma
      suffices e'' ∈ dilatransvections K V ^ (n + 1) by
        rw [← inv_mul_eq_iff_eq_mul] at e''_def
        rw [← e''_def, pow_succ', Set.mem_mul]
...
theorem mem_dilatransvections_pow_of_not_isExceptional
  ...
  · exact mem_dilatransvections_pow_of_fixedReduce_eq_one he
  · exact mem_dilatransvections_pow_of_fixedReduce_ne_smul_id he
...
theorem transvectionDegree_le_finrank_add_one_of_isExceptional (he : IsExceptional e) :
    e ∈ dilatransvections K V ^ (finrank K (V ⧸ e.fixedSubmodule) + 1) := by
  wlog finrank_le_add : finrank K e.fixedSubmodule + 2 ≤ finrank K V
  · rw [← Nat.add_le_add_iff_left, ← add_assoc,
      finrank_quotient_add_finrank (N := e.fixedSubmodule),
      add_comm, add_le_add_iff_right, not_le] at finrank_le_add
    apply dilatransvections_pow_mono (Nat.le_add_left 1 _)
    simp only [pow_one, mem_dilatransvections_iff]
  ...
  rw [he', pow_succ, Set.mem_mul]
  refine ⟨e', ?_, transvection hfv, transvection_mem_dilatransvections hfv, rfl⟩
  ...
  have he'_fixed : e'.fixedSubmodule = e.fixedSubmodule := by
    ...
    rw [← sub_smul, ← mul_smul, inv_mul_cancel₀ ha1, one_smul]
  rw [← he', ← he'_fixed]
...
theorem fixedSubmodule_transvection_le {f : Dual R V} {v : V} (hfv : f v = 0) :
