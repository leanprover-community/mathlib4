...
theorem trace_eq_contract' :
  ...
  simpa using trace_eq_contract_of_basis' (Module.Free.chooseBasis R M)
...
theorem trace_prodMap :
  ...
  refine (cancel_right h).1 ?_
  ext
  · simp only [LinearEquiv.coe_prodCongr,
      dualTensorHomEquiv_toLinearMap, AlgebraTensorModule.curry_apply,
