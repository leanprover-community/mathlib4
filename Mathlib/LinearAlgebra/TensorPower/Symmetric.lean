...
/-- The inductive closure of a relation `r` under reflexivity, symmetry, transitivity,
addition, and scalar multiplication — i.e., the smallest congruence relation on `M`
(compatible with its module structure) containing `r`. -/
...
/-- The `ModuleCon` generated by a module relation. -/
def moduleConGen (r : M → M → Prop) : ModuleCon R M :=
...
/-- The canonical linear map from `M` to its quotient `c.Quotient` by a `ModuleCon` `c`. -/
...
/-- The diagram describing the universal property for quotients of `Modules` commutes. -/
...
instance : AddCommMonoid (SymmetricPower R ι M) :=
  show AddCommMonoid (SymmetricPower.con R ι M).Quotient from inferInstance

instance : Module R (SymmetricPower R ι M) :=
  show Module R (SymmetricPower.con R ι M).Quotient from inferInstance
