...
(`t` is finer than `s`). -/
instance : PartialOrder (TopologicalSpace α) := fast_instance%
...
instance : CompleteLattice (TopologicalSpace α) := fast_instance% (gciGenerateFrom α).liftCompleteLattice
...
instance inhabitedTopologicalSpace {α : Type u} : Inhabited (TopologicalSpace α) := fast_instance%
  ⟨⊥⟩
...
instance : TopologicalSpace Empty := fast_instance% ⊥
instance : DiscreteTopology Empty := fast_instance% ⟨rfl⟩

instance : TopologicalSpace PEmpty := fast_instance% ⊥
instance : DiscreteTopology PEmpty := fast_instance% ⟨rfl⟩

instance : TopologicalSpace PUnit := fast_instance% ⊥
instance : DiscreteTopology PUnit := fast_instance% ⟨rfl⟩

instance : TopologicalSpace Bool := fast_instance% ⊥
instance : DiscreteTopology Bool := fast_instance% ⟨rfl⟩

instance : TopologicalSpace ℕ := fast_instance% ⊥
instance : DiscreteTopology ℕ := fast_instance% ⟨rfl⟩

instance : TopologicalSpace ℤ := fast_instance% ⊥
instance : DiscreteTopology ℤ := fast_instance% ⟨rfl⟩

instance {n} : TopologicalSpace (Fin n) := fast_instance% ⊥
instance {n} : DiscreteTopology (Fin n) := fast_instance% ⟨rfl⟩

instance sierpinskiSpace : TopologicalSpace Prop := fast_instance%
  generateFrom {{True}}
