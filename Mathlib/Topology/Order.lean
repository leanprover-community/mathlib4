...
lemma nhds_mkOfNhds_of_hasBasis {n : α → Filter α} {ι : α → Sort*} {p : ∀ a, ι a → Prop}
...
@[simp]
lemma denseRange_discrete {ι : Type*} {f : ι → α} : DenseRange f ↔ Surjective f := by
...
lemma nhds_nhdsAdjoint_same (a : α) (f : Filter α) :
...
lemma nhds_nhdsAdjoint_of_ne {a b : α} (f : Filter α) (h : b ≠ a) :
...
lemma closure_induced {f : α → β} {a : α} {s : Set α} :
...
lemma isClosed_induced_iff' {f : α → β} {s : Set α} :
