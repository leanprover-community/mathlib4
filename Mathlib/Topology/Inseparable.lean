...
lemma Specializes.not_disjoint (h : x ⤳ y) : ¬Disjoint (𝓝 x) (𝓝 y) := fun hd ↦
...
lemma Specializes.fst {a b : X × Y} (h : a ⤳ b) : a.1 ⤳ b.1 := (specializes_prod.1 h).1
lemma Specializes.snd {a b : X × Y} (h : a ⤳ b) : a.2 ⤳ b.2 := (specializes_prod.1 h).2
...
lemma IsOpen.continuous_piecewise_of_specializes [DecidablePred (· ∈ s)] (hs : IsOpen s)
...
lemma IsClosed.continuous_piecewise_of_specializes [DecidablePred (· ∈ s)] (hs : IsClosed s)
...
lemma map_prod_map_mk_nhds (x : X) (y : Y) :
...
@[simp]
lemma tendsto_lift_nhds_mk {f : X → α} {hf : ∀ x y, (x ~ᵢ y) → f x = f y} {l : Filter α} :
...
@[simp]
lemma tendsto_lift_nhdsWithin_mk {f : X → α} {hf : ∀ x y, (x ~ᵢ y) → f x = f y}
...
@[simp]
lemma continuousAt_lift {hf : ∀ x y, (x ~ᵢ y) → f x = f y}:
...
@[simp]
lemma continuousWithinAt_lift {hf : ∀ x y, (x ~ᵢ y) → f x = f y}
...
@[simp]
lemma continuousOn_lift {hf : ∀ x y, (x ~ᵢ y) → f x = f y} {s : Set (SeparationQuotient X)} :
...
@[simp]
lemma continuous_lift {hf : ∀ x y, (x ~ᵢ y) → f x = f y} :
...
lemma continuous_congr_of_inseparable (h : ∀ x, f x ~ᵢ g x) :
