...
lemma tendsto_extendFrom {A : Set X} {f : X → Y} {x : X} (h : ∃ y, Tendsto f (𝓝[A] x) (𝓝 y)) :
...
lemma continuousOn_extendFrom [RegularSpace Y] {f : X → Y} {A B : Set X} (hB : B ⊆ closure A)
...
lemma continuous_extendFrom [RegularSpace Y] {f : X → Y} {A : Set X} (hA : Dense A)
