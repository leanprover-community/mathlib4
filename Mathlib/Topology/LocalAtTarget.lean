...
theorem Set.restrictPreimage_openEmbedding (s : Set β) (h : OpenEmbedding f) :
  ...
  ⟨h.1.restrictPreimage s,
    (s.range_restrictPreimage (f := f)).symm ▸ continuous_subtype_val.isOpen_preimage _ h.isOpen_range⟩
...
theorem Set.restrictPreimage_closedEmbedding (s : Set β) (h : ClosedEmbedding f) :
  ...
  ⟨h.1.restrictPreimage s,
    (s.range_restrictPreimage (f := f)).symm ▸ inducing_subtype_val.isClosed_preimage _ h.isClosed_range⟩
