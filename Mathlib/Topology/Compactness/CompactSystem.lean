...
def closedCompactSquareCylinders' : Set (Set (Π i, α i)) :=
  ⋃ (s : Finset ι), s.toSet.pi '' s.toSet.pi
    (fun (i : ι) ↦ fun t : Set (α i) ↦ IsCompact t ∧ IsClosed t)

lemma supset :
  closedCompactSquareCylinders' α ⊆ (univ.pi '' univ.pi
    (fun (i : ι) ↦ (fun (s : Set (α i)) ↦ (IsCompact s ∧ IsClosed s) ∨ (s = univ))))  := by
  intro S
  simp [closedCompactSquareCylinders']
  intro s t hs hS
  classical
  ...
  · intro i
    by_cases hs' : i ∈ s
    · simp [hs']
      exact Or.inl (hs i hs')
    · simp [hs']
      exact Or.inr rfl
  · rw [← hS, ← univ_pi_ite s.toSet t]
    simp
...
theorem mem_closedCompactSquareCylinders' (S : Set (Π i, α i)) :
    S ∈ closedCompactSquareCylinders' α
      ↔ ∃ (s t : _) (_ : ∀ i ∈ s, IsCompact (t i) ∧ IsClosed (t i)),
        squareCylinder s t = S := by
  refine ⟨fun h ↦ ?_, fun h ↦ ?_⟩ <;>
  simp only [closedCompactSquareCylinders', squareCylinder,
    mem_iUnion, mem_image, mem_pi, Finset.mem_coe, exists_prop] at h ⊢ <;>
  · exact h
