...
/-- If `f : X → Y` is `IsInducing`, the image `f '' s` of a set `s` is σ-compact
...
protected lemma Topology.IsClosedEmbedding.sigmaCompactSpace {e : Y → X} (he : IsClosedEmbedding e) :
    SigmaCompactSpace Y :=
...
theorem IsClosed.sigmaCompactSpace {s : Set X} (hs : IsClosed s) : SigmaCompactSpace s :=
  hs.isClosedEmbedding_subtype_val.sigmaCompactSpace
