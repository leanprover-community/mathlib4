...
/-- A filter `f` with the countable intersection property that is finer than the principal filter on
a Lindelöf set `s` contains any open set that contains every cluster point of `f` lying in `s`. -/
