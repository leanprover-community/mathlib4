...
-/
def TopologicalSpace.compactlyGenerated (X : Type v) [TopologicalSpace X] : TopologicalSpace X :=
  let f : (Σ (i : (S : CompHaus.{u}) × C(S, X)), i.fst) → X := fun ⟨⟨_, i⟩, s⟩ ↦ i s
  coinduced f inferInstance

lemma continuous_from_compactlyGenerated {X : Type v} [TopologicalSpace X]
...
lemma eq_compactlyGenerated {X : Type v} [t : TopologicalSpace X] [UCompactlyGeneratedSpace.{u} X] :
...
lemma continuous_from_uCompactlyGeneratedSpace {X : Type v} [TopologicalSpace X]
...
lemma uCompactlyGeneratedSpace_of_continuous_maps {X : Type v} [t : TopologicalSpace X]
    (h : ∀ {Y : Type v} [tY : TopologicalSpace Y] (f : X → Y),
