...
instance (priority := 10000) [NoncompactSpace X] : NeBot (Filter.cocompact X) := by
...
instance (priority := 10000) [NoncompactSpace X] : NeBot (Filter.coclosedCompact X) :=
...
instance (priority := 10000) : NoncompactSpace ℤ :=
...
instance (priority := 10000) [CompactSpace X] [CompactSpace Y] : CompactSpace (X × Y) :=
...
instance (priority := 10000) {X : ι → Type*} [Finite ι] [∀ i, TopologicalSpace (X i)] [∀ i, CompactSpace (X i)] :
...
instance (priority := 10000) Pi.compactSpace [∀ i, CompactSpace (X i)] : CompactSpace (∀ i, X i) :=
  ⟨by rw [← pi_univ univ]; exact isCompact_univ_pi fun i => isCompact_univ⟩
#align pi.compact_space Pi.compactSpace

instance (priority := 10000) Function.compactSpace [CompactSpace Y] : CompactSpace (ι → Y) :=
  Pi.compactSpace
...
instance (priority := 10000) Quot.compactSpace {r : X → X → Prop} [CompactSpace X] : CompactSpace (Quot r) :=
  ⟨by
    rw [← range_quot_mk]
    exact isCompact_range continuous_quot_mk⟩
#align quot.compact_space Quot.compactSpace

instance (priority := 10000) Quotient.compactSpace {s : Setoid X} [CompactSpace X] : CompactSpace (Quotient s) :=
  Quot.compactSpace
