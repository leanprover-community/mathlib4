...
lemma Filter.Eventually.exists_lt {a : α} [NeBot (𝓝[<] a)] {p : α → Prop}
...
lemma Filter.Eventually.exists_gt {a : α} [NeBot (𝓝[>] a)] {p : α → Prop}
    (h : ∀ᶠ x in 𝓝 a, p x) : ∃ b > a, p b :=
  ((frequently_gt_nhds a).and_eventually h).exists
#align filter.eventually.exists_gt Filter.Eventually.exists_gt

lemma nhdsWithin_Ici_neBot {a b : α} (H₂ : a ≤ b) : NeBot (𝓝[Ici a] b) :=
...
lemma nhdsWithin_Iic_neBot {a b : α} (H : a ≤ b) : NeBot (𝓝[Iic b] a) :=
  nhdsWithin_neBot_of_mem H
...
lemma continuousWithinAt_Ioi_iff_Ici {a : α} {f : α → β} :
...
lemma continuousWithinAt_Iio_iff_Iic {a : α} {f : α → β} :
...
lemma continuousAt_iff_continuous_left_right {a : α} {f : α → β} :
...
lemma continuousAt_iff_continuous_left'_right' {a : α} {f : α → β} :
