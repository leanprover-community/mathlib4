...
element. -/
@[to_dual /-- The closure of the interval `(-∞, a)` is the closed interval `(∞, a]`,
unless `a` is a bot element. -/]
...
@[to_dual (attr := simp) /-- The closure of the interval `(-∞, a)` is the interval `(-∞, a]`. -/]
...
@[to_dual nhdsLT_neBot_of_exists_lt]
