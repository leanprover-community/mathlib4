...
The order topology on an ordered space is the topology generated by all open intervals
(or equivalently by those of the form `(-∞, a)` and `(b, +∞)`).
We define it as `TopologicalSpace.ofOrder α`.
However, we do *not* register it as an instance,
as many existing ordered types already have topologies,
which would be equal but not definitionally equal to the order topology.
Moreover, e.g., the product topology on `α × β` is not equal to the order topology.
We introduce a class `OrderTopology α` (which is a `Prop`, also known as a mixin)
saying that on the type `α` having already a topological space structure and a preorder structure,
the topological structure is equal to the order topology.
...
Indeed, on many such spaces, a topology has already been constructed in a different way
(think of the discrete spaces `ℕ` or `ℤ`, or `ℝ`
that could inherit a topology as the completion of `ℚ`),
and is in general not defeq to the one generated by the intervals.
We make it available as a definition `TopologicalSpace.ofOrder α` though,
that can be registered as an instance when necessary, or for specific types.
-/
...
def TopologicalSpace.ofOrder (α : Type*) [Preorder α] : TopologicalSpace α :=
  generateFrom (range Ioi ∪ range Iio)

@[deprecated (since := "2024-11-02")] alias Preorder.topology := TopologicalSpace.ofOrder

/-- The order topology on an ordered type is the topology generated by open intervals.
We register it on a preorder, but it is mostly interesting in linear orders,
where it is also order-closed.

Note that the product of spaces with order topologies is not a space with order topology.
A subtype has order topology, if the corresponding set is order-connected.

We define it as a mixin.
If you want to introduce the order topology on a preorder, use `TopologicalSpace.ofOrder`. -/
class OrderTopology (α : Type*) [t : TopologicalSpace α] [Preorder α] : Prop where
  ...
  topology_eq_ofOrder : t = .ofOrder α

@[deprecated (since := "2024-11-02")]
alias OrderTopology.topology_eq_generate_intervals := OrderTopology.topology_eq_ofOrder
...
instance : OrderTopology αᵒᵈ where
  topology_eq_ofOrder := by
    rw [ofOrder, union_comm]
    exact OrderTopology.topology_eq_ofOrder (α := α)

theorem isOpen_iff_generate_intervals {s : Set α} :
    IsOpen s ↔ GenerateOpen (range Ioi ∪ range Iio) s := by
  rw [OrderTopology.topology_eq_ofOrder (α := α)]; rfl

theorem nhds_eq_order (a : α) : 𝓝 a = (⨅ b < a, 𝓟 (Ioi b)) ⊓ ⨅ b > a, 𝓟 (Iio b) := by
  rw [OrderTopology.topology_eq_ofOrder (α := α), nhds_generateFrom]
  simp_rw [mem_setOf_eq, @and_comm (a ∈ _), iInf_and, iInf_union, iInf_range, mem_Ioi, mem_Iio]

theorem tendsto_order {f : β → α} {a : α} {x : Filter β} :
    Tendsto f x (𝓝 a) ↔ (∀ a' < a, ∀ᶠ b in x, a' < f b) ∧ ∀ a' > a, ∀ᶠ b in x, f b < a' := by
  simp [nhds_eq_order a, tendsto_iInf, tendsto_inf]

instance tendstoIccClassNhds (a : α) : TendstoIxxClass Icc (𝓝 a) (𝓝 a) := by
  simp only [nhds_eq_order, iInf_subtype']
  exact ((hasBasis_iInf_principal_finite _).inf (hasBasis_iInf_principal_finite _)).tendstoIxxClass
    fun _ _ ↦ OrdConnected.out inferInstance
...
/-- **Squeeze theorem** (also known as **sandwich theorem**).
This version assumes that inequalities hold eventually for the filter. -/
...
/-- **Squeeze theorem** (also known as **sandwich theorem**).
This version assumes that inequalities hold everywhere. -/
theorem tendsto_of_tendsto_of_tendsto_of_le_of_le {f g h : β → α} {b : Filter β} {a : α}
...
theorem tendsto_order_unbounded {f : β → α} {a : α} {x : Filter β}
...
theorem induced_topology_le_preorder [Preorder α] [Preorder β] [TopologicalSpace β]
    ...
    induced f ‹TopologicalSpace β› ≤ .ofOrder α := by
  simp only [OrderTopology.topology_eq_ofOrder (α := β), TopologicalSpace.ofOrder,
    induced_generateFrom_eq, image_union, generateFrom_union, induced_inf, ← range_comp]
  apply_rules [inf_le_inf, generateFrom_anti, range_subset_iff.mpr] <;>
    refine fun a ↦ ⟨f a, ?_⟩ <;> simp only [Ioi, Iio, preimage_setOf_eq, hf, comp_def]
...
theorem induced_topology_eq_preorder [Preorder α] [Preorder β] [TopologicalSpace β]
    ...
    induced f ‹TopologicalSpace β› = .ofOrder α := by
  let _ := TopologicalSpace.ofOrder α; have : OrderTopology α := ⟨rfl⟩
...
nonrec theorem StrictMono.induced_topology_eq_preorder {α β : Type*} [LinearOrder α]
    ...
    (hf : StrictMono f) (hc : OrdConnected (range f)) : t.induced f = .ofOrder α := by
...
theorem order_separated [OrderTopology α] {a₁ a₂ : α} (h : a₁ < a₂) :
    ∃ u v : Set α, IsOpen u ∧ IsOpen v ∧ a₁ ∈ u ∧ a₂ ∈ v ∧ ∀ b₁ ∈ u, ∀ b₂ ∈ v, b₁ < b₂ := by
  let ⟨x, hx, y, hy, h⟩ := h.exists_disjoint_Iio_Ioi
  obtain rfl := OrderTopology.topology_eq_ofOrder (α := α)
  exact ⟨Iio x, Ioi y, .basic _ <| .inr <| mem_range_self _,
    .basic _ <| .inl <| mem_range_self _, hx, hy, h⟩
...
theorem Dense.topology_eq_generateFrom [OrderTopology α] [DenselyOrdered α] {s : Set α}
  ...
  refine (OrderTopology.topology_eq_ofOrder (α := α)).trans ?_
  refine le_antisymm (generateFrom_anti ?_) (le_generateFrom ?_)
  · gcongr <;> apply image_subset_range
