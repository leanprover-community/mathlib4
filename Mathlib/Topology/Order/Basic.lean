...
(`OrderClosedTopology` vs `OrderTopology` etc)
...
It used to be registered on a preorder, but requires a `LinearOrder` now,
because this is not the right topology on partial orders like `ℝ × ℝ`. -/
...
instance tendstoIccClassNhdsPi {ι : Type*} {α : ι → Type*} [∀ i, LinearOrder (α i)]
...
theorem induced_topology_le_preorder [LinearOrder α] [LinearOrder β] [TopologicalSpace β]
...
theorem induced_topology_eq_preorder [LinearOrder α] [LinearOrder β] [TopologicalSpace β]
    [OrderTopology β] {f : α → β} (hf : ∀ {x y}, f x < f y ↔ x < y)
...
theorem induced_orderTopology' {α : Type u} {β : Type v} [LinearOrder α] [ta : TopologicalSpace β]
    [LinearOrder β] [OrderTopology β] (f : α → β) (hf : ∀ {x y}, f x < f y ↔ x < y)
...
theorem nhdsWithin_Ici_eq'' [TopologicalSpace α] [LinearOrder α] [OrderTopology α] (a : α) :
...
theorem nhdsWithin_Iic_eq'' [TopologicalSpace α] [LinearOrder α] [OrderTopology α] (a : α) :
...
theorem nhds_top_order [TopologicalSpace α] [LinearOrder α] [OrderTop α] [OrderTopology α] :
    𝓝 (⊤ : α) = ⨅ (l) (h₂ : l < ⊤), 𝓟 (Ioi l) := by simp [nhds_eq_order (⊤ : α)]

theorem nhds_bot_order [TopologicalSpace α] [LinearOrder α] [OrderBot α] [OrderTopology α] :
...
theorem tendsto_nhds_top_mono [TopologicalSpace β] [LinearOrder β] [OrderTop β] [OrderTopology β]
...
theorem tendsto_nhds_bot_mono [TopologicalSpace β] [LinearOrder β] [OrderBot β] [OrderTopology β]
