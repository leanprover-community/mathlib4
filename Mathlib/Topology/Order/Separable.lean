...
/-!
# Order-convex neighbourhoods, and hereditary separability of linear orders

A set in a preorder is *order-convex* when it contains `Set.Icc x y` for any two of its points
`x` and `y`; this is Mathlib's `Set.OrdConnected`.

In a linearly ordered topological space the open order-convex sets form a topological basis: the
order topology is generated by the rays `Set.Iio b` and `Set.Ioi a`, and a finite intersection of
rays is `Set.Ioo a b`, a ray, or the whole space, each of which is order-convex. We obtain such a
neighbourhood as the interior of `Set.ordConnectedComponent U x`.

The main application is that separability is hereditary for linearly ordered topological spaces:
every subset of a separable such space is separable in the subspace topology (`Set.separableSpace`).
This fails for general topological spaces, so the order is genuinely used.

## Main results

* `exists_isOpen_ordConnected_mem_subset`: every point of an open set has an open order-convex
...
`Set.OrdConnected.interior` already exists, but only for spaces with `HasUpperLowerClosure`
(ordered topological groups), which does not cover a general linear order. The order-convexity of
the interior is therefore established inline in `exists_isOpen_ordConnected_mem_subset`, using
that `Set.Ioo x y` is open.
-/
...
theorem Dense.mem_of_isOpen_singleton {s : Set X} {x : X} (hs : Dense s)
    (hx : IsOpen ({x} : Set X)) : x ∈ s := by
...
/-- Every point `x` of an open set `U` has an open order-convex neighbourhood contained in `U`.

We take the interior of the order-connected component of `x` in `U`. Its order-convexity is where
linearity is used: if `y` and `z` lie in the interior and `y < w < z`, then `w` lies in the open
set `Set.Ioo y z`, which is contained in `Set.Icc y z` and hence in the order-convex set. -/
theorem exists_isOpen_ordConnected_mem_subset {U : Set α} (hU : IsOpen U) {x : α} (hx : x ∈ U) :
  ...
  · exact hz
  exact mem_interior.2 ⟨Ioo y z, fun v hv ↦ Set.OrdConnected.out inferInstance
