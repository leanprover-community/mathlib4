...
theorem IsClosed.mem_of_ge_of_forall_exists_gt {a b : α} {s : Set α} (hs : IsClosed (s ∩ Icc a b))
  ...
  exact not_gt_of_le (le_csSup Sbd ⟨xs, le_trans (le_csSup Sbd ha) (le_of_lt cx), xb⟩) cx
...
theorem intermediate_value_Ico {a b : α} (hab : a ≤ b) {f : α → δ} (hf : ContinuousOn f (Icc a b)) :
  ...
  Or.elim (eq_or_lt_of_le hab) (fun he _ h => absurd h.2 (not_gt_of_le (he ▸ h.1))) fun hlt =>
...
theorem intermediate_value_Ico' {a b : α} (hab : a ≤ b) {f : α → δ}
  ...
  Or.elim (eq_or_lt_of_le hab) (fun he _ h => absurd h.1 (not_gt_of_le (he ▸ h.2))) fun hlt =>
...
theorem intermediate_value_Ioc {a b : α} (hab : a ≤ b) {f : α → δ} (hf : ContinuousOn f (Icc a b)) :
  ...
  Or.elim (eq_or_lt_of_le hab) (fun he _ h => absurd h.2 (not_ge_of_lt (he ▸ h.1))) fun hlt =>
...
theorem intermediate_value_Ioc' {a b : α} (hab : a ≤ b) {f : α → δ}
  ...
  Or.elim (eq_or_lt_of_le hab) (fun he _ h => absurd h.1 (not_ge_of_lt (he ▸ h.2))) fun hlt =>
