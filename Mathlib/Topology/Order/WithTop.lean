...
instance [ts : TopologicalSpace ι] [ht : OrderTopology ι] [SecondCountableTopology ι] :
  ...
  suffices H : instWithTopOfOrderTopology
    ...
    have : {s | ∃ a ∈ d, s = Ioi a ∨ s = Iio a} = Ioi '' d ∪ Iio '' d := by
      ...
      One way to solve this is via `grind (splits := 12)` or alternatively `ext; simp; grind`
      We squeeze here so that `grind?` also works. -/
      grind only [= mem_setOf_eq, = mem_union, = mem_image]
    rw [this]
  ...
  · apply le_generateFrom_iff_subset_isOpen.2
    grind only [= setOf_subset_setOf, isOpen_Ioi', isOpen_Iio']
