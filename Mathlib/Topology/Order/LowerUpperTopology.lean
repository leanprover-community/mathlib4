...
@[to_dual /-- Type synonym for a preorder equipped with the upper topology. -/]
...
def toLower : α ≃ WithLower α := Equiv.refl _
...
def ofLower : WithLower α ≃ α := Equiv.refl _

@[to_dual (attr := simp)] lemma toLower_symm : (@toLower α).symm = ofLower := rfl
@[to_dual (attr := simp)] lemma ofLower_symm : (@ofLower α).symm = toLower := rfl
@[to_dual (attr := simp)] lemma toLower_ofLower (a : WithLower α) : toLower (ofLower a) = a := rfl

@[to_dual (attr := simp)] lemma ofLower_toLower (a : α) : ofLower (toLower a) = a := rfl

@[to_dual] lemma toLower_inj {a b : α} : toLower a = toLower b ↔ a = b := Iff.rfl

@[to_dual] theorem ofLower_inj {a b : WithLower α} : ofLower a = ofLower b ↔ a = b :=
  Iff.rfl
...
@[to_dual] instance [Inhabited α] : Inhabited (WithLower α) := ‹Inhabited α›

section Preorder

variable [Preorder α] {s : Set α}

@[to_dual] instance : Preorder (WithLower α) := ‹Preorder α›
@[to_dual] instance : TopologicalSpace (WithLower α) := lower (WithLower α)
...
@[to_dual] instance [PartialOrder α] : PartialOrder (WithLower α) := ‹PartialOrder α›
@[to_dual] instance [LinearOrder α] : LinearOrder (WithLower α) := ‹LinearOrder α›
...
lemma toUpper_le_toUpper {x y : α} : toUpper x ≤ toUpper y ↔ x ≤ y := .rfl
@[to_dual existing, simp]
lemma toUpper_lt_toUpper {x y : α} : toUpper x < toUpper y ↔ x < y := .rfl
@[to_dual existing, simp]
lemma ofUpper_le_ofUpper {x y : WithUpper α} : ofUpper x ≤ ofUpper y ↔ x ≤ y := .rfl
@[to_dual existing, simp]
lemma ofUpper_lt_ofUpper {x y : WithUpper α} : ofUpper x < ofUpper y ↔ x < y := .rfl
...
attribute [to_dual existing] IsLower IsLower.topology_eq_lowerTopology

@[to_dual] instance [Preorder α] : IsLower (WithLower α) := ⟨rfl⟩
...
@[to_dual /-- The complements of the lower closures of finite sets are a collection of upper sets
