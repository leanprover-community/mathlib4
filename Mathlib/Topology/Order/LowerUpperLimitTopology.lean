...
A type synonym `WithLowerLimit` is introduced and for a preorder `α`, `WithLowerLimit α`
...
that `WithLowerLimit α` is an instance of `IsLowerLimit`.
...
-/
def lowerLimit (α : Type*) [Preorder α] : TopologicalSpace α :=
...
theorem nhds_lowerLimit_eq {α : Type*} [TopologicalSpace α] [Preorder α] (a : α) :
    @nhds α (lowerLimit α) a = 𝓝[≥] a := by
  sorry

theorem continuous_iff_rightContinuous {α β : Type*} [TopologicalSpace α] [TopologicalSpace β]
    [Preorder α] (f : α → β) :
    @Continuous α β (lowerLimit α) inferInstance f
    ↔ ∀ a, ContinuousWithinAt f (Set.Ici a) a := by
  sorry
...
theorem nhds_upperLimit_eq {α : Type*} [TopologicalSpace α] [Preorder α] (a : α) :
    @nhds α (upperLimit α) a = 𝓝[≤] a := by
  sorry

theorem continuous_iff_leftContinuous {α β : Type*} [TopologicalSpace α] [TopologicalSpace β]
    [Preorder α] (f : α → β) :
    @Continuous α β (upperLimit α) inferInstance f
    ↔ ∀ a, ContinuousWithinAt f (Set.Iic a) a := by
  sorry
...
instance _root_.OrderDual.instIsUpperLimit : IsUpperLimit αᵒᵈ where
...
instance : ClosedIciTopology α := by
  ...
  · grind
  · simp only [isOpen_iff_generate_Ico]
    exact GenerateOpen.basic _ ⟨x, a, rfl⟩
  · simp [notMem_Ici.1 hx]

instance [NoMaxOrder α] : OrderClosedTopology α := by
  constructor
  refine isOpen_compl_iff.1 (isOpen_iff_forall_mem_open.2 fun x hx => ?_)
  obtain ⟨c, hc⟩ := exists_gt x.1
  refine ⟨Ico x.1 c ×ˢ Ico x.2 x.1, fun y hy => ?_, ?_, ?_⟩
  · simpa using LT.lt.not_ge (LT.lt.trans_le hy.2.2 hy.1.1)
  · refine IsOpen.prod ?_ ?_
    · simp only [isOpen_iff_generate_Ico]
      exact GenerateOpen.basic _ ⟨x.1, c, rfl⟩
    · simp only [isOpen_iff_generate_Ico]
      exact GenerateOpen.basic _ ⟨x.2, x.1, rfl⟩
  · simp_all
...
variable (α)
variable [Preorder α] [TopologicalSpace α] [IsUpperLimit α] {s : Set α}

lemma topology_eq : ‹_› = upperLimit α := topology_eq_upperLimitTopology

variable {α}

/-- If `α` is equipped with the upper limit topology, then it is homeomorphic to `WithUpperLimit α`.
-/
def withUpperLimitHomeomorph : WithUpperLimit α ≃ₜ α :=
  WithUpperLimit.ofUpperLimit.toHomeomorphOfIsInducing ⟨topology_eq α ▸ induced_id.symm⟩

theorem isOpen_iff_generate_Ioc :
    IsOpen s ↔ GenerateOpen { t | ∃ i j, t = Set.Ioc i j } s := by
  simp [topology_eq α, upperLimit, generateFrom, IsOpen]

instance _root_.OrderDual.instIsLowerLimit : IsLowerLimit αᵒᵈ where
...
variable [LinearOrder α] [TopologicalSpace α] [IsUpperLimit α]

instance : ClosedIicTopology α := sorry

instance [NoMinOrder α] : OrderClosedTopology α := by sorry
