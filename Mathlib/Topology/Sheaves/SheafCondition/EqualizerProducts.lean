...
theorem w : res F U ≫ leftRes F U = res F U ≫ rightRes F U := by
  ...
  -- Porting note (https://github.com/leanprover-community/mathlib4/issues/11041): `ext` cannot see `limit.hom_ext` applies here:
...
def fork.isoOfIso (α : F ≅ G) :
  ...
  · -- Porting note (https://github.com/leanprover-community/mathlib4/issues/11041): `ext` cannot see `limit.hom_ext` applies here:
    refine limit.hom_ext (fun _ => ?_)
    dsimp only [Fork.ι]
    -- Ugh, `simp` cannot unfold abbreviations.
