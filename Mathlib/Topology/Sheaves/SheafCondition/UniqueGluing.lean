...
-/
def IsCompatible (sf : ∀ i : ι, CC (F.obj (op (U i)))) : Prop :=
...
for all `i`
-/
def IsGluing (sf : ∀ i : ι, CC (F.obj (op (U i)))) (s : CC (F.obj (op (iSup U)))) : Prop :=
...
def IsSheafUniqueGluing : Prop :=
  ∀ ⦃ι : Type x⦄ (U : ι → Opens X) (sf : ∀ i : ι, CC (F.obj (op (U i)))),
    IsCompatible F U sf → ∃! s : CC (F.obj (op (iSup U))), IsGluing F U sf s
...
variable [HasLimits C] [(CategoryTheory.forget (C := C)).ReflectsIsomorphisms]
variable [PreservesLimits (CategoryTheory.forget (C := C))]
variable {X : TopCat.{v}} (F : Sheaf C X) {ι : Type v} (U : ι → Opens X)

/-- A more convenient way of obtaining a unique gluing of sections for a sheaf.
-/
theorem existsUnique_gluing (sf : ∀ i : ι, CC (F.1.obj (op (U i)))) (h : IsCompatible F.1 U sf) :
    ∃! s : CC (F.1.obj (op (iSup U))), IsGluing F.1 U sf s :=
...
theorem existsUnique_gluing' (V : Opens X) (iUV : ∀ i : ι, U i ⟶ V) (hcover : V ≤ iSup U)
    (sf : ∀ i : ι, CC (F.1.obj (op (U i)))) (h : IsCompatible F.1 U sf) :
    ∃! s : CC (F.1.obj (op V)), ∀ i : ι, F.1.map (iUV i).op s = sf i := by
...
@[ext]
theorem eq_of_locally_eq (s t : CC (F.1.obj (op (iSup U))))
    (h : ∀ i, F.1.map (Opens.leSupr U i).op s = F.1.map (Opens.leSupr U i).op t) : s = t := by
  let sf : ∀ i : ι, CC (F.1.obj (op (U i))) := fun i => F.1.map (Opens.leSupr U i).op s
...
theorem eq_of_locally_eq' (V : Opens X) (iUV : ∀ i : ι, U i ⟶ V) (hcover : V ≤ iSup U)
    (s t : CC (F.1.obj (op V))) (h : ∀ i, F.1.map (iUV i).op s = F.1.map (iUV i).op t) : s = t := by
  have V_eq_supr_U : V = iSup U := le_antisymm hcover (iSup_le fun i => (iUV i).le)
...
theorem eq_of_locally_eq₂ {U₁ U₂ V : Opens X} (i₁ : U₁ ⟶ V) (i₂ : U₂ ⟶ V) (hcover : V ≤ U₁ ⊔ U₂)
    (s t : CC (F.1.obj (op V))) (h₁ : F.1.map i₁.op s = F.1.map i₁.op t)
