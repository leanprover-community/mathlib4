...
Any open cover `U : ι → Opens X` provides a functor `diagram U : Pairwise ι ⥤ Opens X`.

There is a canonical cocone over this functor, `cocone U`, whose cocone point is `iSup U`,
and in fact this is a colimit cocone.

A presheaf `F : Presheaf C X` is a sheaf precisely if it sends the opposite cone to a limit
cone. We express this in two equivalent ways, as
* `IsLimit (F.mapCone (cocone U).op)`, or
* `PreservesLimit (diagram U).op F`
