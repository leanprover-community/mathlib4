...
like `pairwiseIntersections` and `equalizerProducts`.
...
/-- `iSup U` as a cocone over the open sets contained in some element of the cover.
...
/-- Given a family of opens `U`, the cocone `opensLeCoverCocone U` is essentially the natural
cocone associated to the sieve generated by the presieve associated to `U`, with indexing
