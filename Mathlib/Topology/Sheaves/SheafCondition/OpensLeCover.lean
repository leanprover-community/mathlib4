...
-/
noncomputable def index (V : OpensLeCover U) : ι :=
  V.property.choose

/-- The morphism from `V` to `U i` for some `i`.
-/
noncomputable def homToIndex (V : OpensLeCover U) : V.obj ⟶ U (index V) :=
...
-/
noncomputable def opensLeCoverCocone : Cocone (ObjectProperty.ι _ : OpensLeCover U ⥤ Opens X) where
  pt := iSup U
...
@[simps]
noncomputable def whiskerIsoMapGenerateCocone (hY : Y = iSup U) :
...
noncomputable def isLimitOpensLeEquivGenerate₁ (hY : Y = iSup U) :
...
noncomputable def isLimitOpensLeEquivGenerate₂ (R : Presieve Y)
