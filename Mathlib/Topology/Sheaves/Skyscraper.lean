...
def skyscraperPresheaf : Presheaf C X where
  ...
  map_comp {U V W} iVU iWV := by
    ...
    · rw [dif_neg hW]; apply ((if_neg hW).symm.rec terminalIsTerminal).hom_ext
...
theorem SkyscraperPresheafFunctor.map'_id {a : C} : SkyscraperPresheafFunctor.map' p₀ (𝟙 a) = 𝟙 _ :=
  ...
  · apply ((if_neg h).symm.rec terminalIsTerminal).hom_ext
...
noncomputable def skyscraperPresheafCoconeIsColimitOfSpecializes {y : X} (h : p₀ ⤳ y) :
  ...
  fac c U := by
    ...
    rw [dif_pos]
    ·
      simpa only [skyscraperPresheafCoconeOfSpecializes_ι_app, eqToHom_trans_assoc,
    ...
    (Presheaf.isSheaf_iso_iff (eqToIso <| skyscraperPresheaf_eq_pushforward p₀ A)).mpr <|
      (Sheaf.pushforward_sheaf_of_sheaf _
        (Presheaf.isSheaf_on_pUnit_of_isTerminal _
          (by dsimp; rw [if_neg]; exact terminalIsTerminal; exact Set.not_mem_empty PUnit.unit)))
...
def toSkyscraperPresheaf {𝓕 : Presheaf C X} {c : C} (f : 𝓕.stalk p₀ ⟶ c) :
  ...
  naturality U V inc := by
    ...
    · have hU : p₀ ∈ U.unop := leOfHom inc.unop hV; split_ifs
      ...
      rfl
    · split_ifs; apply ((if_neg hV).symm.ndrec terminalIsTerminal).hom_ext
