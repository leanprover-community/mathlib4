...
lemma isClosed_closedBall : IsClosed (closedBall x ε) := isClosed_le (by fun_prop) .const

lemma isClosed_sphere : IsClosed (sphere x ε) := isClosed_eq (by fun_prop) .const
...
lemma frontier_ball_subset_sphere : frontier (ball x ε) ⊆ sphere x ε :=
  frontier_lt_subset_eq (by fun_prop) .const

lemma frontier_closedBall_subset_sphere : frontier (closedBall x ε) ⊆ sphere x ε :=
  frontier_le_subset_eq (by fun_prop) .const
