...
theorem isBounded_iff {s : Set α} :
  ...
  rw [isBounded_def, ← Filter.mem_sets, @PseudoMetricSpace.cobounded_sets α, mem_ofPred,
    compl_compl]
...
protected lemma uniformSpace_eq_bot :
  ...
  simp only [uniformity_basis_dist.uniformSpace_eq_bot, mem_ofPred, not_lt]
...
abbrev PseudoMetricSpace.replaceBornology {α} [B : Bornology α] (m : PseudoMetricSpace α)
  ...
  { m with
    ...
    cobounded_sets := Set.ext <| compl_surjective.forall.2 fun s =>
        (H s).trans <| by rw [isBounded_iff, mem_ofPred, compl_compl] }
