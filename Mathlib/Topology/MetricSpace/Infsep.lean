...
lemma einfsep_zero : s.einfsep = 0 ↔
...
lemma le_einfsep {d} (h : ∀ (x) (_ : x ∈ s) (y) (_ : y ∈ s) (_hxy : x ≠ y), d ≤ edist x y) :
    d ≤ s.einfsep :=
  le_einfsep_iff.2 h
...
lemma le_einfsep_of_forall_dist_le {d}
...
lemma Subsingleton.infsep_zero (hs : s.Subsingleton) : s.infsep = 0 := by
  rw [infsep_zero.mpr]
  right
  exact hs.einfsep
