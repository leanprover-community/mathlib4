...
instance emetricSpace : EMetricSpace (NonemptyCompacts α) where
  /- Since the topology on `NonemptyCompacts` is not defeq to `UniformSpace.hausdorff`, we replace
  the uniformity by `NonemptyCompacts.uniformSpace`, which has the right topology. -/
