...
instance : MetricSpace GHSpace where
  ...
  dist_comm x y := by
    have A :
      ...
      congr
      -- The next line had `singlePass := true` beforehttps://github.com/leanprover-community/mathlib4/issues/9928,
      -- then was changed to be `simp only [hausdorffDist_comm]`,
      -- then `singlePass := true` was readded inhttps://github.com/leanprover-community/mathlib4/issues/8386 because of timeouts.
