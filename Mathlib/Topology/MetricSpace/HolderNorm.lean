...
noncomputable
def eHoelderNorm (r : ℝ≥0) (f : X → Y) : ℝ≥0∞ := ⨅ (C) (_ : HolderWith C r f), C
...
c.f. `memHoelder_iff`. -/
def MemHoelder (r : ℝ≥0) (f : X → Y) : Prop := ∃ C, HolderWith C r f

lemma HolderWith.memHoelder {C : ℝ≥0} (hf : HolderWith C r f) : MemHoelder r f := ⟨C, hf⟩
...
lemma HolderWith.eHoelderNorm_le {C : ℝ≥0} (hf : HolderWith C r f) :
    eHoelderNorm r f ≤ C :=
  iInf₂_le C hf

@[simp]
lemma memHoelder_const {c : Y} : MemHoelder r (Function.const X c) :=
  (HolderWith.const (C := 0)).memHoelder
...
lemma MemHoelder.holderWith {r : ℝ≥0} {f : X → Y} (hf : MemHoelder r f) :
    HolderWith (nnHoelderNorm r f) r f := by
  intros x₁ x₂
  by_cases hx : x₁ = x₂
...
lemma memHoelder_iff_holderWith {r : ℝ≥0} {f : X → Y} :
    MemHoelder r f ↔ HolderWith (nnHoelderNorm r f) r f :=
  ⟨MemHoelder.holderWith, HolderWith.memHoelder⟩

lemma coe_nnHoelderNorm_le_eHoelderNorm {r : ℝ≥0} {f : X → Y} :
    (nnHoelderNorm r f : ℝ≥0∞) ≤ eHoelderNorm r f :=
  coe_toNNReal_le_self

lemma HolderWith.coe_nnHoelderNorm_eq_eHoelderNorm
...
lemma MemHoelder.coe_nnHoelderNorm_eq_eHoelderNorm
    ...
    (nnHoelderNorm r f : ℝ≥0∞) = eHoelderNorm r f :=
  hf.holderWith.coe_nnHoelderNorm_eq_eHoelderNorm

lemma HolderWith.nnhoelderNorm_le {C r : ℝ≥0} {f : X → Y} (hf : HolderWith C r f) :
...
lemma eHoelderNorm_add_le :
  ...
  · obtain ⟨hf, hg⟩ := hfg
    ...
    exact (hf.add hg).holderWith.eHoelderNorm_le.trans <|
      coe_le_coe.2 (hf.holderWith.add hg.holderWith).nnhoelderNorm_le
...
lemma eHoelderNorm_smul {α} [NormedDivisionRing α] [Module α Y] [BoundedSMul α Y] (c : α) :
  ...
  · refine le_antisymm ((hf.holderWith.smul c).eHoelderNorm_le.trans ?_) <| mul_le_of_le_div' ?_
    · rw [coe_mul, hf.coe_nnHoelderNorm_eq_eHoelderNorm, mul_comm]
    · rw [← (hf.holderWith.smul c).coe_nnHoelderNorm_eq_eHoelderNorm, ← coe_div hc]
      refine HolderWith.eHoelderNorm_le fun x₁ x₂ => ?_
