...
instance (priority := 100) instParacompactSpace [PseudoEMetricSpace α] : ParacompactSpace α := by
  ...
  set D : ℕ → ι → Set α := fun n =>
    Nat.strongRecOn' n fun n D' i =>
      ⋃ (x : α) (hxs : ind x = i) (hb : Metric.eball x (3 * 2⁻¹ ^ n) ⊆ s i) (hlt :
        ∀ (m : ℕ) (H : m < n), ∀ (j : ι), x ∉ D' m H j), Metric.eball x (2⁻¹ ^ n) with hD
  have Dn (n i) : D n i = ⋃ (x : α) (hxs : ind x = i) (hb : Metric.eball x (3 * 2⁻¹ ^ n) ⊆ s i)
      (hlt : ∀ m < n, ∀ (j : ι), x ∉ D m j), Metric.eball x (2⁻¹ ^ n) := by
    simp only [hD]
    rw [Nat.strongRecOn'_beta]
  have memD {n i y} :
      y ∈ D n i ↔ ∃ x : α, ind x = i ∧ Metric.eball x (3 * 2⁻¹ ^ n) ⊆ s i ∧
  ...
  · intro x
    ...
    rcases (nhds_basis_uniformity uniformity_basis_edist_inv_two_pow).mem_iff.1 this with
      ⟨k, -, hsub : Metric.eball x (2⁻¹ ^ k) ⊆ D n i⟩
    set B := Metric.eball x (2⁻¹ ^ (n + k + 1))
    ...
    have Hgt (m) (hm : n + k + 1 ≤ m) (i : ι) : Disjoint (D m i) B := by
      ...
      have : z ∉ Metric.eball x (2⁻¹ ^ k) := fun hz' => H n (by lia) i (hsub hz')
      apply this
      calc
