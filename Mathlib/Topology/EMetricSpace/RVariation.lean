...
theorem eVariationOn' (f : α → E) (s : Set α) :
  ...
  have : (1 : ℝ≥0∞) ≠ 0 := one_ne_zero
  have : (1 : ℝ≥0∞) ≠ ⊤ := ENNReal.one_ne_top
  simp only [rEVariationOn, eVariationOn, *, ↓reduceIte, ENNReal.toReal_one, inv_one,
    ENNReal.rpow_one]
...
theorem sum_le_of_monotoneOn_Icc (hr : r ≠ 0) (hr' : r ≠ ⊤) {f : α → E} {s : Set α}
  ...
  calc
    ...
    _ ≤ (∑ i ∈ Finset.range n, edist (f (v (i + 1))) (f (v i)) ^ r.toReal) ^ r.toReal ⁻¹ := by
      gcongr 1
      exact Finset.sum_mono_set _ (Nat.Iio_eq_range ▸ Finset.Ico_subset_Iio_self)
...
theorem eVariationOn_lt_top {f : α → E} {s : Set α} (h : r ≠ 0) (h' : r ≠ ∞)
  ...
  have : Inhabited (ℕ × { u // Monotone u ∧ ∀ (i : ℕ), u i ∈ s }) := by
    ...
    refine Classical.inhabited_of_nonempty instNonemptyProd
