...
class WeakPseudoEMetricSpace
  ...
  edist_comm : ∀ x y : α, edist x y = edist y x
  edist_triangle : ∀ x y z : α, edist x z ≤ edist x y + edist y z
  /-- The topology on `α` is at least as fine as the topology generated by the `edist`:
  every open ball in the `edist` is also open in the given topology on `α`. -/
  topology_le :
...
instance PseudoEMetricSpace.toWeakPseudoEMetricSpace (α : Type u) [inst : PseudoEMetricSpace α] :
  ...
  topology_eq_on_restrict := by
    intro _ _ h
    apply IsOpen.preimage_val
    have e: toUniformSpace.toTopologicalSpace = (uniformSpaceOfEDist
        inst.edist inst.edist_self inst.edist_comm inst.edist_triangle).toTopologicalSpace := by
      congr
      ext
