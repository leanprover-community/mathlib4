...
theorem edist_le_mul_of_le (h : LipschitzWith K f) (hr : edist x y ≤ r) :
    edist (f x) (f y) ≤ K * r :=
  (h x y).trans <| mul_right_monotone hr
...
protected theorem weaken (hf : LipschitzWith K f) {K' : ℝ≥0} (h : K ≤ K') : LipschitzWith K' f :=
  fun x y => le_trans (hf x y) <| mul_left_monotone (ENNReal.coe_le_coe.2 h)
...
protected theorem comp {Kf Kg : ℝ≥0} {f : β → γ} {g : α → β} (hf : LipschitzWith Kf f)
  ...
  calc
    ...
    _ ≤ Kf * (Kg * edist x y) := mul_right_monotone (hg _ _)
...
protected theorem uncurry {f : α → β → γ} {Kα Kβ : ℝ≥0} (hα : ∀ b, LipschitzWith Kα fun a => f a b)
  ...
  exact
    add_le_add (le_trans (hα _ _ _) <| mul_right_monotone <| le_max_left _ _)
      (le_trans (hβ _ _ _) <| mul_right_monotone <| le_max_right _ _)
...
theorem edist_le_mul_of_le (h : LipschitzOnWith K f s) {x y : α} (hx : x ∈ s) (hy : y ∈ s)
    ...
    edist (f x) (f y) ≤ K * r :=
  (h hx hy).trans <| mul_right_monotone hr
...
theorem ediam_image2_le (f : α → β → γ) {K₁ K₂ : ℝ≥0} (s : Set α) (t : Set β)
  ...
  exact
    add_le_add
      ((hf₁ b₁ hb₁ ha₁ ha₂).trans <| mul_right_monotone <| Metric.edist_le_ediam_of_mem ha₁ ha₂)
      ((hf₂ a₂ ha₂ hb₁ hb₂).trans <| mul_right_monotone <| Metric.edist_le_ediam_of_mem hb₁ hb₂)
