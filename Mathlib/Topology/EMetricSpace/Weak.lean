...
@[simp]
theorem edist_top_coe (a : ℝ≥0) : edist ∞ ↑a = ∞ :=
  rfl

@[simp]
theorem edist_coe_top (a : ℝ≥0) : edist ↑a ∞ = ∞ :=
  rfl
...
theorem edist_eq_max (a b : ℝ≥0∞) : edist a b = max (a - b) (b - a) := by
  cases a <;> cases b
  case coe.coe =>
    rw [edist_coe, ← coe_nnreal_ennreal_nndist, NNReal.nndist_eq]
    norm_cast
  all_goals simp
