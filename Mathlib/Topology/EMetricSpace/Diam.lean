...
theorem ediam_closedEBall_le {r : ℝ≥0∞} : ediam (Metric.closedEBall x r) ≤ 2 * r :=
...
theorem ediam_eball_le {r : ℝ≥0∞} : ediam (Metric.eball x r) ≤ 2 * r :=
  le_trans (ediam_mono Metric.eball_subset_closedEBall) ediam_closedEBall_le
