...
protected theorem UniformSpace.metrizable_uniformity (X : Type*) [UniformSpace X]
  ...
  /- Choose a fast decreasing antitone basis `U : ℕ → set (X × X)` of the uniformity filter `𝓤 X`.
    ...
    pseudometric space structure is Lipschitz equivalent to the `d`. Thus the uniformities
    generated by `d` and `dist` are equal. Since the former uniformity is equal to `𝓤 X`, the
    latter is equal to `𝓤 X` as well. -/
