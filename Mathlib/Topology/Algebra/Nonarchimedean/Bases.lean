...
def openAddSubgroup (i : ι) : @OpenAddSubgroup M _ hB.topology :=
  letI _ := hB.topology -- Porting note: failed to synthesize instance `TopologicalSpace A`
...
theorem RingFilterBasis.submodulesBasisIsBasis (BR : RingFilterBasis R) {B : ι → Submodule R M}
  ...
  letI _ := BR.topology -- Porting note: failed to synthesize instance `TopologicalSpace R`
  { inter := hB.inter
    smul := by
      letI := BR.topology
      intro m i
