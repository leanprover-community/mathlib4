...
lemma non_empty_intersection_compl_coset (x y : G) (U : Set G) (hx : x ∈ U)
    (A : Set G) (quot : (y⁻¹ * x) ∈ A ) (V : OpenSubgroup G) (dva : Disjoint (V : Set G) A) :
...
-- should be elsewhere
lemma _root_.foo {X : Type*} {U : Set X} {x : X} (hx : x ∈ U) (h : ∀ y : X, y ∈ U → y = x) :
    U = {x} := by
  ext
...
instance totally_disconnected_space : TotallyDisconnectedSpace G := by
      ...
      obtain ⟨y, hyu, hneq⟩ : ∃ y : G, (y ∈ connectedComponent x) ∧ (y ≠ x) := by
        by_contra! con2
        exact con <| foo mem_connectedComponent con2
      exact non_singleton_set_disconnected G x y (connectedComponent x) mem_connectedComponent hyu hneq isConnected_connectedComponent
