...
/-- The pullback `Spa(φ) : Spa(B, B⁺) → Spa(A, A⁺)` of a continuous morphism
is continuous (Wedhorn, Rem. 7.28: "defines via composition a continuous map
Spa(φ): Spa B → Spa A"). The Spa topology is generated by the rational
subsets; the preimage of each is rational (hence open) by
`preimage_comap_rationalSubset`. The openness of the pulled-back rational
subset `R(φ(T)/φ(s))` requires the image-ideal `φ(T)·B` to be open; that
ideal-level transfer is stated as the explicit premise `hTopen'` (an honest
surfacing of the gap — the openness of `span (T.image φ)` does not follow
from the openness of `span T` and the continuity of `φ` alone in this
generality). -/
theorem continuous_comap {B : Type*} [CommRing B] [TopologicalSpace B] [DecidableEq B]
    (Bplus : Subring B) (φ : A →+* B) (hφ : Continuous φ) (hAB : Aplus ≤ Bplus.comap φ)
    (hTopen' : ∀ T : Finset A,
      IsOpen ((Ideal.span (T : Set A)) : Set A) →
        IsOpen ((Ideal.span (T.image φ : Set B)) : Set B)) :
    Continuous (comap Aplus Bplus φ hφ hAB : SpaPoint B Bplus → SpaPoint A Aplus) := by
  rw [continuous_generateFrom_iff]
  rintro U ⟨s, T, hTopen, rfl⟩
