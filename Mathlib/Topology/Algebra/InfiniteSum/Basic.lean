...
@[to_additive]
lemma hasProd_empty [IsEmpty β] : HasProd f 1 := by
  convert @hasProd_one α β _ _
#align has_sum_empty hasSum_empty

@[to_additive]
lemma multipliable_one : Multipliable (fun _ ↦ 1 : β → α) :=
  hasProd_one.multipliable
#align summable_zero summable_zero

@[to_additive]
lemma multipliable_empty [IsEmpty β] : Multipliable f :=
  hasProd_empty.multipliable
#align summable_empty summable_empty

@[to_additive]
lemma multipliable_congr (hfg : ∀ b, f b = g b) : Multipliable f ↔ Multipliable g :=
  iff_of_eq (congr_arg Multipliable <| funext hfg)
#align summable_congr summable_congr

@[to_additive]
lemma Multipliable.congr (hf : Multipliable f) (hfg : ∀ b, f b = g b) : Multipliable g :=
...
@[to_additive]
lemma HasProd.hasProd_of_prod_eq {g : γ → α}
...
@[to_additive]
lemma hasProd_iff_hasProd {g : γ → α}
...
@[to_additive (attr := simp)] lemma hasProd_extend_one {g : β → γ} (hg : Injective g) :
    HasProd (extend g f 1) a ↔ HasProd f a := by
  rw [← hg.hasProd_iff, extend_comp hg]
  exact extend_apply' _ _

@[to_additive (attr := simp)] lemma multipliable_extend_one {g : β → γ} (hg : Injective g) :
    Multipliable (extend g f 1) ↔ Multipliable f :=
  exists_congr fun _ ↦ hasProd_extend_one hg

@[to_additive]
lemma hasProd_subtype_iff_mulIndicator {s : Set β} :
...
@[to_additive]
lemma multipliable_subtype_iff_mulIndicator {s : Set β} :
...
@[to_additive]
lemma multipliable_of_finite_mulSupport (h : (mulSupport f).Finite) : Multipliable f := by
  apply multipliable_of_ne_finset_one (s := h.toFinset); simp

@[to_additive]
lemma hasProd_single {f : β → α} (b : β) (hf : ∀ (b') (_ : b' ≠ b), f b' = 1) : HasProd f (f b) :=
...
@[to_additive]
lemma hasProd_ite_eq (b : β) [DecidablePred (· = b)] (a : α) :
...
@[to_additive]
lemma Equiv.hasProd_iff (e : γ ≃ β) : HasProd (f ∘ e) a ↔ HasProd f a :=
  e.injective.hasProd_iff <| by simp
#align equiv.has_sum_iff Equiv.hasSum_iff

@[to_additive]
lemma Function.Injective.hasProd_range_iff {g : γ → β} (hg : Injective g) :
...
@[to_additive]
lemma Equiv.multipliable_iff (e : γ ≃ β) : Multipliable (f ∘ e) ↔ Multipliable f :=
  exists_congr fun _ ↦ e.hasProd_iff
#align equiv.summable_iff Equiv.summable_iff

@[to_additive]
lemma Equiv.hasProd_iff_of_mulSupport {g : γ → α} (e : mulSupport f ≃ mulSupport g)
...
@[to_additive]
lemma hasProd_iff_hasProd_of_ne_one_bij {g : γ → α} (i : mulSupport g → β)
...
@[to_additive]
lemma Equiv.multipliable_iff_of_mulSupport {g : γ → α} (e : mulSupport f ≃ mulSupport g)
...
@[to_additive]
protected lemma HasProd.map [CommMonoid γ] [TopologicalSpace γ] (hf : HasProd f a) {G}
...
@[to_additive]
protected lemma Multipliable.map [CommMonoid γ] [TopologicalSpace γ] (hf : Multipliable f) {G}
...
@[to_additive]
protected lemma Multipliable.map_iff_of_leftInverse [CommMonoid γ] [TopologicalSpace γ] {G G'}
...
@[to_additive]
lemma Function.Surjective.multipliable_iff_of_hasProd_iff {α' : Type*} [CommMonoid α']
...
@[to_additive]
lemma HasProd.mul (hf : HasProd f a) (hg : HasProd g b) :
...
@[to_additive]
lemma hasProd_prod {f : γ → β → α} {a : γ → α} {s : Finset γ} :
...
@[to_additive]
lemma HasProd.mul_disjoint {s t : Set β} (hs : Disjoint s t) (ha : HasProd (f ∘ (↑) : s → α) a)
...
@[to_additive]
lemma hasProd_prod_disjoint {ι} (s : Finset ι) {t : ι → Set β} {a : ι → α}
...
@[to_additive]
lemma HasProd.mul_isCompl {s t : Set β} (hs : IsCompl s t) (ha : HasProd (f ∘ (↑) : s → α) a)
...
@[to_additive]
lemma HasProd.compl_mul {s : Set β} (ha : HasProd (f ∘ (↑) : (sᶜ : Set β) → α) a)
    (hb : HasProd (f ∘ (↑) : s → α) b) : HasProd f (a * b) :=
  ha.mul_isCompl isCompl_compl.symm hb
#align has_sum.compl_add HasSum.compl_add

@[to_additive]
lemma Multipliable.compl_add {s : Set β} (hs : Multipliable (f ∘ (↑) : (sᶜ : Set β) → α))
...
@[to_additive]
lemma tprod_bool (f : Bool → α) : ∏' i : Bool, f i = f false * f true := by
  rw [tprod_fintype, Fintype.prod_bool, mul_comm]
#align tsum_bool tsum_bool

@[to_additive]
lemma tprod_eq_mulSingle {f : β → α} (b : β) (hf : ∀ b' ≠ b, f b' = 1) :
    ∏' b, f b = f b := by
...
@[to_additive]
lemma tprod_tprod_eq_mulSingle (f : β → γ → α) (b : β) (c : γ) (hfb : ∀ b' ≠ b, f b' c = 1)
...
@[to_additive (attr := simp)]
lemma tprod_ite_eq (b : β) [DecidablePred (· = b)] (a : α) :
...
-- Porting note: Added nolint simpNF, simpNF falsely claims that lhs does not simplify under simp
@[to_additive (attr := simp, nolint simpNF)]
lemma tprod_singleton (b : β) (f : β → α) : ∏' x : ({b} : Set β), f x = f b := by
...
@[to_additive]
lemma Equiv.tprod_eq (e : γ ≃ β) (f : β → α) : ∏' c, f (e c) = ∏' b, f b :=
...
@[to_additive]
lemma tprod_subtype_eq_of_mulSupport_subset {f : β → α} {s : Set β} (hs : mulSupport f ⊆ s) :
    ∏' x : s, f x = ∏' x, f x :=
  Subtype.val_injective.tprod_eq <| by simpa
#align tsum_subtype_eq_of_support_subset tsum_subtype_eq_of_support_subset

@[to_additive]
lemma tprod_subtype_mulSupport (f : β → α) : ∏' x : mulSupport f, f x = ∏' x, f x :=
  tprod_subtype_eq_of_mulSupport_subset Set.Subset.rfl

@[to_additive]
lemma tprod_subtype (s : Set β) (f : β → α) : ∏' x : s, f x = ∏' x, s.mulIndicator f x := by
...
@[to_additive]
lemma tprod_image {g : γ → β} (f : β → α) {s : Set γ} (hg : Set.InjOn g s) :
    ∏' x : g '' s, f x = ∏' x : s, f (g x) :=
...
@[to_additive]
lemma tprod_eq_tprod_of_ne_one_bij {g : γ → α} (i : mulSupport g → β) (hi : Injective i)
...
@[to_additive]
lemma Equiv.tprod_eq_tprod_of_mulSupport {f : β → α} {g : γ → α}
...
@[to_additive]
lemma Function.Surjective.tprod_eq_tprod_of_hasProd_iff_hasProd {α' : Type*} [CommMonoid α']
...
@[to_additive]
lemma tprod_eq_tprod_of_hasProd_iff_hasProd {f : β → α} {g : γ → α}
...
lemma tprod_of_prod {f : γ → β → α} {s : Finset γ} (hf : ∀ i ∈ s, Multipliable (f i)) :
...
@[to_additive]
lemma tprod_mul_tprod_compl {s : Set β} (hs : Multipliable (f ∘ (↑) : s → α))
...
@[to_additive]
lemma tprod_union_disjoint {s t : Set β} (hd : Disjoint s t) (hs : Multipliable (f ∘ (↑) : s → α))
