...
lemma _root_.Summable.tendsto_zero_of_even_summable_symmetricIcc {F : Type*} [AddCommGroup F]
    [NormedAddCommGroup F] [NormSMulClass ℤ F] {f : ℤ → F} (hf : Summable f (symmetricIcc ℤ))
    (hs : f.Even) :
