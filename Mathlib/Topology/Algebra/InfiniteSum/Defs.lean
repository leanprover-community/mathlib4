...
@[to_additive (attr := informal "infinite sum") /-- `HasSum f a L` means that the (potentially
infinite) sum of the `f b` for `b : β` converges to `a` along the SummationFilter `L`.
