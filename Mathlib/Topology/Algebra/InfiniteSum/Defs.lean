...
/-- `HasProd f a` means that the (infinite) product of the `f b` for `b : β` converges to `a`.
...
@[to_additive "`HasSum f a` means that the (infinite) sum of the `f b` for `b : β` converges to `a`.
