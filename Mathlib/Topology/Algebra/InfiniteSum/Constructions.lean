...
@[to_additive]
lemma hasProd_pi_single [DecidableEq β] (b : β) (a : α) : HasProd (Pi.mulSingle b a) a := by
  convert hasProd_ite_eq b a
  simp [Pi.mulSingle_apply]
#align has_sum_pi_single hasSum_pi_single

@[to_additive (attr := simp)]
lemma tprod_pi_single [DecidableEq β] (b : β) (a : α) : ∏' b', Pi.mulSingle b a b' = a := by
...
lemma HasProd.prod_mk {f : β → α} {g : β → γ} {a : α} {b : γ}
...
@[to_additive]
lemma HasProd.sigma {γ : β → Type*} {f : (Σ b : β, γ b) → α} {g : β → α} {a : α}
...
@[to_additive]
lemma Multipliable.sigma' {γ : β → Type*} {f : (Σb : β, γ b) → α} (ha : Multipliable f)
...
@[to_additive]
lemma HasProd.sigma_of_hasProd {γ : β → Type*} {f : (Σb : β, γ b) → α} {g : β → α}
...
@[to_additive]
lemma tprod_sigma' {γ : β → Type*} {f : (Σb : β, γ b) → α}
...
lemma tprod_prod' {f : β × γ → α} (h : Multipliable f)
...
@[to_additive]
lemma tprod_comm' {f : β → γ → α} (h : Multipliable (Function.uncurry f))
...
@[to_additive]
lemma tprod_sigma {γ : β → Type*} {f : (Σb : β, γ b) → α} (ha : Multipliable f) :
    ∏' p, f p = ∏' (b) (c), f ⟨b, c⟩ :=
...
lemma tsum_unop [T2Space α] {f : β → αᵐᵒᵖ} :
...
lemma tsum_star [T2Space α] : star (∑' b, f b) = ∑' b, star (f b) := by
