...
/-!
# Infinite sums and products in non-Archimedean abelian groups

Let `G` be a complete non-Archimedean abelian group and let `f : α → G` be a function. We prove that
...
theorem cauchySeq_prod_of_tendsto_cofinite_one {f : α → G} (hf : Tendsto f cofinite (𝓝 1)) :
  ...
  intro U hU
  -- Since `G` is non-Archimedean, `U` contains an open subgroup `V`.
...
/-- Let `G` be a non-Archimedean abelian group, and let `f : ℕ → G` be a function
such that the quotients `f (n + 1) / f n` tend to one. Then the function is a Cauchy sequence. -/
@[to_additive /-- Let `G` be a non-Archimedean additive abelian group, and let `f : ℕ → G` be a
...
/-- Let `G` be a complete non-Archimedean multiplicative abelian group, and let `f : α → G` be a
function that tends to one on the filter of cofinite sets. Then `f` is unconditionally
multipliable. -/
@[to_additive /-- Let `G` be a complete non-Archimedean additive abelian group, and let `f : α → G`
...
/-- Let `G` be a complete non-Archimedean multiplicative abelian group. Then a function `f : α → G`
is unconditionally multipliable if and only if it tends to one on the filter of cofinite sets. -/
@[to_additive /-- Let `G` be a complete non-Archimedean additive abelian group. Then a function
...
/-- Let `R` be a non-Archimedean ring, let `f : α → R` be a function that sums to `a : R`,
...
/-- Let `R` be a non-Archimedean ring. If functions `f : α → R` and `g : β → R` are summable, then
