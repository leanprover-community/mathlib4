...
ring. -/
theorem TopologicalRing.of_norm {R 𝕜 : Type*}
    [NonUnitalNonAssocRing R] [Field 𝕜] [LinearOrderedField 𝕜]
...
variable {𝕜 α : Type*} [Field 𝕜] [LinearOrderedField 𝕜] [TopologicalSpace 𝕜] [OrderTopology 𝕜]
  {l : Filter α} {f g : α → 𝕜}
...
theorem tendsto_pow_neg_atTop {n : ℕ} (hn : n ≠ 0) :
  ...
  simpa only [zpow_neg, zpow_natCast] using (@tendsto_pow_atTop 𝕜 _ _ _ hn).inv_tendsto_atTop
...
instance (priority := 100) LinearOrderedSemifield.toHasContinuousInv₀ {𝕜}
    [Semifield 𝕜] [LinearOrderedSemifield 𝕜] [TopologicalSpace 𝕜] [OrderTopology 𝕜] [ContinuousMul 𝕜] :
