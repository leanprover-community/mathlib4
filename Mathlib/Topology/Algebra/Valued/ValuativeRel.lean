...
instance (priority := low) {R : Type*} [CommRing R] [ValuativeRel R] [UniformSpace R]
  ...
  is_topological_valuation := by
    ...
    refine ⟨fun ⟨γ, hγ⟩ ↦ ⟨Units.map (ValuativeRel.ValueGroupWithZero.embed _).symm.toMonoidHom γ,
      hγ⟩, fun ⟨γ, hγ⟩ ↦ ⟨Units.map (ValuativeRel.ValueGroupWithZero.embed _).toMonoidHom γ, ?_⟩⟩
    convert hγ
    simp [ValuativeRel.ValueGroupWithZero.embed]
