...
theorem IsEquiv.uniformContinuous_equiv [hval : Valued R Γ₀'] (hv : Valued.v = w)
  ...
  rw [← hr, equiv_apply, Set.mem_ofPred, lt_div_iff₀ ((restrict_pos_iff Valued.v s).mpr hs₀), hv,
  ...
  simp only [equiv_symm_apply, Units.val_mk0, Set.mem_ofPred, lt_div_iff₀ hs0'] at hx
...
theorem IsEquiv.uniformContinuous_equiv_symm [hval : Valued R Γ₀'] (hv : Valued.v = w)
  ...
  use .mk0 ((Valued.v.restrict ((WithVal.equiv v) r)) /
    ...
    exact ⟨hr₀.ne', hs₀.ne'⟩)
  intro x hx
  simp only [equiv_symm_apply, Set.mem_ofPred]
  simp only [equiv_apply, Units.val_mk0, Set.mem_ofPred] at hx
...
lemma IsEquiv.uniformContinuous (h : v.IsEquiv w) :
  ...
  simp only [Set.mem_ofPred, RingHom.id_apply]
