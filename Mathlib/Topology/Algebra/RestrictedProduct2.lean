...
variable (𝓕) in
def structureMap (x : Π i, A i) : Πᶠ i in 𝓕, [R i, A i] :=
  ⟨fun i ↦ x i, .of_forall fun i ↦ (x i).2⟩

def inclusion (h : 𝓕 ≤ 𝓖) (x : Πᶠ i in 𝓖, [R i, A i]) :
    Πᶠ i in 𝓕, [R i, A i] :=
  ⟨x, x.2.filter_mono h⟩
...
lemma exists_inclusion_eq_of_eventually (h : 𝓕 ≤ 𝓖) {x : Πᶠ i in 𝓕, [R i, A i]}
    (hx𝓖 : ∀ᶠ i in 𝓖, x i ∈ A i) :
    ∃ x' : Πᶠ i in 𝓖, [R i, A i], inclusion R A h x' = x :=
  ⟨⟨x.1, hx𝓖⟩, rfl⟩

lemma exists_structureMap_eq_of_forall {x : Πᶠ i in 𝓕, [R i, A i]}
    (hx : ∀ i, x.1 i ∈ A i) :
...
variable (R A 𝓕) in
instance topologicalSpace : TopologicalSpace (Πᶠ i in 𝓕, [R i, A i]) :=
  ⨆ (S : Set ι) (hS : 𝓕 ≤ 𝓟 S), .coinduced (inclusion R A hS)
    (.induced ((↑) : Πᶠ i in 𝓟 S, [R i, A i] → Π i, R i) inferInstance)

theorem continuous_coe :
    Continuous ((↑) : Πᶠ i in 𝓕, [R i, A i] → Π i, R i) :=
...
theorem topologicalSpace_eq_of_top :
    topologicalSpace R A ⊤ =
      .induced ((↑) : Πᶠ i in ⊤, [R i, A i] → Π i, R i) inferInstance :=
...
theorem isInducing_coe_of_principal :
    IsInducing ((↑) : Πᶠ i in 𝓟 S, [R i, A i] → Π i, R i) where
  eq_induced := topologicalSpace_eq_of_principal

theorem isInducing_coe_of_top :
    IsInducing ((↑) : Πᶠ i in ⊤, [R i, A i] → Π i, R i) :=
...
def homeoTop : (Π i, A i) ≃ₜ (Πᶠ i in ⊤, [R i, A i]) where
...
def homeoBot : (Π i, R i) ≃ₜ (Πᶠ i in ⊥, [R i, A i]) where
...
instance [hS : Fact (cofinite ≤ 𝓟 S)]
    ...
    WeaklyLocallyCompactSpace (Πᶠ i in 𝓟 S, [R i, A i]) where
  exists_compact_mem_nhds := fun x ↦ by
    ...
    have QU : (↑) ⁻¹' U ⊆ ((↑) ⁻¹' Q : Set (Πᶠ i in 𝓟 S, [R i, A i])) := fun y H i _ ↦ by
      dsimp only
      split_ifs with hi
      · exact y.2 hi
      · exact H i hi
...
theorem isOpen_forall_imp_mem_of_principal {S : Set ι} (hS : cofinite ≤ 𝓟 S) {p : ι → Prop} :
    IsOpen {f : Πᶠ i in 𝓟 S, [R i, A i] | ∀ i, p i → f.1 i ∈ A i} := by
...
theorem isOpen_forall_mem :
    IsOpen {f : Πᶠ i, [R i, A i] | ∀ i, f.1 i ∈ A i} := by
  simp_rw [topologicalSpace_eq_iSup cofinite, isOpen_iSup_iff, isOpen_coinduced]
...
instance [hAopen : Fact (∀ i, IsOpen (A i))] [∀ i, WeaklyLocallyCompactSpace (R i)]
    ...
    WeaklyLocallyCompactSpace (Πᶠ i, [R i, A i]) where
  exists_compact_mem_nhds := fun x ↦ by
    set S := {i | x i ∈ A i}
...
instance [Π i, Inv (R i)] [∀ i, InvMemClass (S i) (R i)] [∀ i, ContinuousInv (R i)] :
    ContinuousInv (Πᶠ i in 𝓕, [R i, A i]) where
  continuous_inv := by
    rw [continuous_dom]
    intro T hT
    haveI : ContinuousInv (Πᶠ i in 𝓟 T, [R i, A i]) :=
...
instance {G : Type*} [Π i, SMul G (R i)] [∀ i, SMulMemClass (S i) G (R i)]
    ...
    ContinuousConstSMul G (Πᶠ i in 𝓕, [R i, A i]) where
  continuous_const_smul g := by
    rw [continuous_dom]
    intro T hT
    haveI : ContinuousConstSMul G (Πᶠ i in 𝓟 T, [R i, A i]) :=
...
instance [Π i, Mul (R i)] [∀ i, MulMemClass (S i) (R i)] [∀ i, ContinuousMul (R i)] :
    ContinuousMul (Πᶠ i in 𝓟 T, [R i, A i]) :=
  let φ : Πᶠ i in 𝓟 T, [R i, A i] →ₙ* Π i, R i :=
  { toFun := (↑)
    map_mul' := fun _ _ ↦ rfl }
...
instance {G : Type*} [TopologicalSpace G] [Π i, SMul G (R i)] [∀ i, SMulMemClass (S i) G (R i)]
    ...
    ContinuousSMul G (Πᶠ i in 𝓟 T, [R i, A i]) :=
...
@[to_additive]
instance [Π i, Mul (R i)] [∀ i, MulMemClass (S i) (R i)] [∀ i, ContinuousMul (R i)] :
    ContinuousMul (Πᶠ i, [R i, A i]) where
  continuous_mul := by
...
instance {G : Type*} [TopologicalSpace G] [Π i, SMul G (R i)] [∀ i, SMulMemClass (S i) G (R i)]
    [∀ i, ContinuousSMul G (R i)] :
    ContinuousSMul G (Πᶠ i, [R i, A i]) where
  continuous_smul := by
