...
theorem coeff_bdd_of_roots_le {B : ℝ} {d : ℕ} (f : F →+* K) {p : F[X]} (h1 : p.Monic)
  ...
  · apply (coeff_le_of_roots_le i h1 h2 h4).trans
    calc
      _ ≤ max B 1 ^ (p.natDegree - i) * p.natDegree.choose i := by rel_congr; apply le_max_left
      _ ≤ max B 1 ^ d * p.natDegree.choose i := by
        rel_congr; apply le_max_right; exact le_trans (Nat.sub_le _ _) h3
      _ ≤ max B 1 ^ d * d.choose (d / 2) := by
        rel_congr; exact (i.choose_mono h3).trans (i.choose_le_middle d)
