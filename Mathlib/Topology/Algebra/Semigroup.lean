...
theorem exists_idempotent_of_compact_t2_of_continuous_mul_left {M} [Nonempty M] [Semigroup M]
  ...
  · obtain rfl | hcnemp := c.eq_empty_or_nonempty
    ...
    convert!
      @IsCompact.nonempty_iInter_of_predirected_nonempty_isCompact_isClosed _ _ _ hcnemp.coe_sort
        ((↑) : c → Set M) ?_ ?_ ?_ ?_
    · exact Set.sInter_eq_iInter
    · refine PredirectedOn.predirected_val (IsChain.predirectedOn hc.symm)
