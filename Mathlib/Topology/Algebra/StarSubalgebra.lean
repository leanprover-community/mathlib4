...
instance (priority := 10000) [TopologicalSemiring A] (s : StarSubalgebra R A) : TopologicalSemiring s :=
...
instance (priority := 10000) {A : Type*} [UniformSpace A] [CompleteSpace A] [Semiring A] [StarRing A]
...
instance (priority := 10000) [T2Space A] {x : A} [IsStarNormal x] : CommSemiring (elementalStarAlgebra R x) :=
  StarSubalgebra.commSemiringTopologicalClosure _ mul_comm

/-- The `elementalStarAlgebra` generated by a normal element is commutative. -/
instance (priority := 10000) {R A} [CommRing R] [StarRing R] [TopologicalSpace A] [Ring A] [Algebra R A] [StarRing A]
