...
@[to_additive (attr := implicit_reducible) /-- `AddGroupFilterBasis` constructor in the additive commutative group case. -/]
...
@[to_additive (attr := implicit_reducible) /-- The topological space structure coming from an additive group filter basis. -/]
