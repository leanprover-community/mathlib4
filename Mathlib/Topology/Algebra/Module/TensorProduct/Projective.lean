...
This file defines the projective topology on the tensor product of two topological vector
spaces over a nontrivially normed field.

Rather than using an abstract lattice-theoretic infimum over compatible topologies, the
projective topology is defined constructively. We construct a neighborhood filter basis at zero
consisting of the absolutely convex hulls of the tensor products of neighborhoods of zero from
each factor. This matches the classical neighborhood construction of the projective topology.

## Main definitions

* `ProjectiveTensorProduct 𝕜 E F` (notation: `E ⊗[𝕜]π F`): A type synonym for the tensor product
  `E ⊗[𝕜] F` equipped with the projective topology.
...
* `TensorProduct.instTopologicalSpaceProjectiveTensorProduct`: The projective topology on
  `E ⊗[𝕜]π F`, defined directly as the topology generated by `projectiveModuleFilterBasis`.

## Main statements

* `TensorProduct.locallyConvexSpace_projectiveModuleFilterBasis`: The projective tensor product
  space is locally convex.
* `TensorProduct.tendsto_tmul_nhds_zero_projectiveModuleFilterBasis`: The canonical bilinear map
  `tmul` is continuous at `(0, 0)`.
