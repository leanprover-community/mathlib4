...
* `Submodule.IsTopCompl`: we say that two submodules are *topological complements* if they are
  algebraic complements and the projection on `p` parallel to `q` is continuous. This is equivalent
...
* `Submodule.IsTopCompl.projectionOnto`: if `h : IsTopCompl p q`, `h.projectionOnto` is the
  continuous linear projection `M →L[R] p` parallel to `q`. This is the continuous version of
  `Submodule.linearProjOfIsCompl`.
* `Submodule.IsTopCompl.projection`: if `h : IsTopCompl p q`, `h.projection` is the continuous
  linear projection `M →L[R] M` onto `p` parallel to `q`. This is the continuous version of
...
on the left submdule parallel to the right one, because it is a simpler map to work with than the
map `M ≃ p × q`.
...
parallel to `q`. This is the continuous version of `Submodule.linearProjOfIsCompl`.
...
`p` parallel to `q`. This is the continuous version of `Submodule.IsCompl.projection`.
