...
structure ContinuousMultilinearMap (R : Type u) {ι : Type v} (M₁ : ι → Type w₁) (M₂ : Type w₂)
  ...
  [∀ i, TopologicalSpace (M₁ i)] [TopologicalSpace M₂] extends M₁ →ₗₘ[R] M₂ where
...
theorem toMultilinearMap_injective :
    Function.Injective
      (ContinuousMultilinearMap.toMultilinearMap : ContinuousMultilinearMap R M₁ M₂ → M₁ →ₗₘ[R] M₂)
...
instance : Zero (ContinuousMultilinearMap R M₁ M₂) :=
  ⟨{ (0 : M₁ →ₗₘ[R] M₂) with cont := continuous_const }⟩
...
@[simps]
def toMultilinearMapLinear : ContinuousMultilinearMap A M₁ M₂ →ₗ[R'] M₁ →ₗₘ[A] M₂ where
  toFun := toMultilinearMap
  map_add' := toMultilinearMap_add
