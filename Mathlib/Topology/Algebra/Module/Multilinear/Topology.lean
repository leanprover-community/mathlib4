...
theorem isUniformEmbedding_restrictScalars :
  ...
  letI : IsNontriviallyNormedField 𝕜 :=
    ⟨let ⟨x, hx⟩ := @IsNontriviallyNormedField.non_trivial 𝕜' _ _ _; ⟨algebraMap 𝕜' 𝕜 x, by simpa⟩⟩
  rw [← isUniformEmbedding_toUniformOnFun.of_comp_iff]
  convert! isUniformEmbedding_toUniformOnFun (𝕜 := 𝕜) using 4 with s
