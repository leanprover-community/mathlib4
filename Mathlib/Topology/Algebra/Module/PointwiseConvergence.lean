...
@[simp]
theorem neg_apply (f : E →SLₚₜ[σ] F) (x : E) : (-f) x = -f x :=
  rfl

-- This is not the right lemma
@[simp]
theorem smul_apply [ContinuousSMul 𝕜₂ F] (c : 𝕜₂) (f : E →SLₚₜ[σ] F) (x : E) :
    (c • f) x = c • f x :=
  rfl

instance [RingHomSurjective σ] [RingHomIsometric σ] [ContinuousSMul 𝕜₁ E] [TopologicalSpace F]
    [IsTopologicalAddGroup F] [ContinuousSMul 𝕜₂ F] :
    ContinuousSMul 𝕜₂ (E →SLₚₜ[σ] F) :=
  UniformConvergenceCLM.continuousSMul _ _ _ (fun _ ↦ Set.Finite.isVonNBounded)

lemma _root_.Set.finite_covers {X : Type*} : ⋃₀ {(s : Set X) | Set.Finite s} = Set.univ := by
...
instance [T2Space F] : T2Space (E →SLₚₜ[σ] F) :=
  UniformConvergenceCLM.t2Space _ _ _ Set.finite_covers
