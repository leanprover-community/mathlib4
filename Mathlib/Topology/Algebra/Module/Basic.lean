...
lemma continuousSMul_induced : @ContinuousSMul R M₁ _ u (t.induced f) :=
...
lemma Submodule.closure_subset_topologicalClosure_span (s : Set M) :
...
@[simp]
lemma natCast_apply [ContinuousAdd M₁] (n : ℕ) (m : M₁) : (↑n : M₁ →L[R₁] M₁) m = n • m :=
  rfl

@[simp]
lemma ofNat_apply [ContinuousAdd M₁] (n : ℕ) [n.AtLeastTwo] (m : M₁) :
...
lemma isClosed_ker [T1Space M₂] [FunLike F M₁ M₂] [ContinuousSemilinearMapClass F σ₁₂ M₁ M₂]
    (f : F) :
...
@[simp]
lemma coe_rangeRestrict [RingHomSurjective σ₁₂] (f : M₁ →SL[σ₁₂] M₂) :
...
@[simp]
lemma intCast_apply [TopologicalAddGroup M] (z : ℤ) (m : M) : (↑z : M →L[R] M) m = z • m :=
  rfl
...
@[simp] lemma algebraMap_apply (r : R) (m : M₂) : algebraMap R (M₂ →L[R] M₂) r m = r • m := rfl
...
lemma isOpenMap (e : M₁ ≃SL[σ₁₂] M₂) : IsOpenMap e :=
...
lemma ClosedComplemented.exists_isClosed_isCompl {p : Submodule R M} [T1Space p]
