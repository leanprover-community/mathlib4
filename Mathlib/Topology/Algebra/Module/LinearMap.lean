...
instance instIsOneApplyEqId : IsOneApplyEqId (M₁ →L[R₁] M₁) M₁ where
  one_apply_eq_id _ := rfl
...
@[deprecated (since := "2026-04-27")] alias one_apply := _root_.one_apply_eq_id
...
instance instIsMulApplyEqComp : IsMulApplyEqComp (M₁ →L[R₁] M₁) M₁ where
...
@[deprecated (since := "2026-04-27")] alias coe_mul := coe_mul'

@[deprecated (since := "2026-04-27")] alias mul_apply := _root_.mul_apply_eq_comp
...
@[deprecated (since := "2026-04-27")] alias natCast_apply := _root_.natCast_apply
...
def homeomorphOfUnit (T : (M₁ →L[R₁] M₁)ˣ) : M₁ ≃ₜ M₁ where
  toFun := T.1
  invFun := T⁻¹.1
  left_inv x := by rw [← mul_apply_eq_comp, Units.inv_mul, one_apply_eq_id]
  right_inv x := by rw [← mul_apply_eq_comp, Units.mul_inv, one_apply_eq_id]
...
instance instIsNegApply : IsNegApply (M →SL[σ₁₂] M₂) M M₂ where
  neg_apply _ _ := rfl

@[deprecated (since := "2026-04-27")] alias neg_apply := _root_.neg_apply

@[simp, norm_cast]
theorem coe_neg' (f : M →SL[σ₁₂] M₂) : (↑(-f) : M →ₛₗ[σ₁₂] M₂) = -f :=
  rfl

@[deprecated (since := "2026-04-27")] alias coe_neg := coe_neg'
...
instance instIsSubApply : IsSubApply (M →SL[σ₁₂] M₂) M M₂ where
  sub_apply _ _ _ := rfl

@[deprecated (since := "2026-04-27")] alias sub_apply := _root_.sub_apply

@[simp, norm_cast]
theorem coe_sub' (f g : M →SL[σ₁₂] M₂) : (↑(f - g) : M →ₛₗ[σ₁₂] M₂) = f - g :=
  rfl

@[deprecated (since := "2026-04-27")] alias coe_sub := coe_sub'
...
@[deprecated (since := "2026-04-27")] alias intCast_apply := _root_.intCast_apply
