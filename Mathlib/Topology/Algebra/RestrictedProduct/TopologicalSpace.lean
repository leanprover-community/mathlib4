...
theorem continuous_dom {X : Type*} [TopologicalSpace X]
  ...
  simp [topologicalSpace_eq_of_principal, continuous_iSup_dom, ← continuous_coinduced_dom]
...
theorem isOpen_forall_imp_mem {p : ι → Prop} :
  ...
  simp_rw -loopProtection [topologicalSpace_eq_iSup cofinite, isOpen_iSup_iff, isOpen_coinduced]
