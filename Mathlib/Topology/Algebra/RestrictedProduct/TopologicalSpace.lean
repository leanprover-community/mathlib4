...
theorem isOpen_forall_imp_mem_of_principal {S : Set ι} (hS : cofinite ≤ 𝓟 S) {p : ι → Prop} :
  ...
  suffices h : {f : Πʳ i, [R i, A i]_[𝓟 S] | ∀ i, p i → f.1 i ∈ A i} =
      ((↑) ⁻¹' (Sᶜ ∩ {i | p i}).pi A : Set (Πʳ i, [R i, A i]_[𝓟 S])) by
    rw [h]
    exact isOpen_set_pi (hS.inter_of_left {i | p i}) (fun i _ ↦ hAopen i) |>.preimage
      continuous_coe
  ext f
  simp only [mem_ofPred_eq, mem_preimage, Set.mem_pi, mem_inter_iff, mem_compl_iff]
