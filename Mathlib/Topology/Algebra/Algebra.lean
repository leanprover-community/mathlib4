...
theorem Algebra.self_mem_elementalAlgebra (x : A) : x ∈ elementalAlgebra R x :=
  SetLike.le_def.mp (Subalgebra.le_topologicalClosure (adjoin R ({x} : Set A))) <|
    self_mem_adjoin_singleton R x
#align algebra.self_mem_elemental_algebra Algebra.self_mem_elementalAlgebra

variable {R}

instance [T2Space A] {x : A} : CommRing (elementalAlgebra R x) :=
  Subalgebra.commRingTopologicalClosure _
    letI : CommRing (adjoin R ({x} : Set A)) :=
      adjoinCommRingOfComm R fun y hy z hz => by
...
instance DivisionRing.continuousConstSMul_rat {A} [DivisionRing A] [TopologicalSpace A]
  ...
  ⟨fun r => by simpa only [smul_def] using continuous_const.mul continuous_id⟩
