...
`Λ^ι⟮𝕜; F₁, E₁; F₂, E₂⟯` (notation for `Bundle.continuousAlternatingMap 𝕜 ι F₁ E₁ F₂ E₂ x`) to be
a type synonym for `fun x ↦ Λ^ι⟮𝕜; E₁ x; E₂ x⟯`, the sigma-type of continuous alternating maps
fibrewise from `E₁ x` to `E₂ x`. If the `E₁` and `E₂` are vector bundles with model fibers `F₁` and
`F₂`, then this will be a vector bundle with model fiber `F₁ [Λ^ι]→L[𝕜] F₂`.

The topology is constructed from the trivializations for `E₁` and `E₂` and the norm-topology on the
model fiber `F₁ [Λ^ι]→L[𝕜] F₂` using the `vector_prebundle` construction.  This
...
* `Bundle.continuousAlternatingMap.vectorBundle`: continuous alternating maps between
  vector bundles form a vector bundle.  (FIXME Notation `Λ^ι⟮𝕜; F₁, E₁; F₂, E₂⟯`.)
...
-- FIXME, notation more consistent with `F₁ [Λ^ι]→L[𝕜] F₂`?
notation3 "Λ^" ι "⟮" 𝕜 "; " F₁ ", " E₁ "; " F₂ ", " E₂ "⟯" => Bundle.continuousAlternatingMap 𝕜 ι F₁ E₁ F₂ E₂
...
instance (x : B) : AddCommMonoid (Λ^ι⟮𝕜; F₁, E₁; F₂, E₂⟯ x) := by
...
instance (x : B) : Module 𝕜 (Λ^ι⟮𝕜; F₁, E₁; F₂, E₂⟯ x) := by
  -- was delta_instance `Bundle.continuousAlternatingMap`, FIXME
  dsimp [Bundle.continuousAlternatingMap]
  infer_instance
...
def continuousAlternatingMapCoordChange
    ...
    (F₁ [Λ^ι]→L[𝕜] F₂) →L[𝕜] (F₁ [Λ^ι]→L[𝕜] F₂) :=
  ((e₁'.coordChangeL 𝕜 e₁ b).symm.continuousAlternatingMapCongrL (e₂.coordChangeL 𝕜 e₂' b) :
    (F₁ [Λ^ι]→L[𝕜] F₂) ≃L[𝕜] (F₁ [Λ^ι]→L[𝕜] F₂))
...
def continuousAlternatingMap : Pretrivialization (F₁ [Λ^ι]→L[𝕜] F₂)
    (π (F₁ [Λ^ι]→L[𝕜] F₂) Λ^ι⟮𝕜; F₁, E₁; F₂, E₂⟯) where
...
lemma continuousAlternatingMapCoordChange_apply (b : B)
  ...
  ext v
  have H : e₁'.coordChangeL 𝕜 e₁ b ∘ v = e₁.linearMapAt 𝕜 b ∘ e₁'.symm b ∘ v
  · ext i
    dsimp
...
instance (x : B) : TopologicalSpace (Λ^ι⟮𝕜; F₁, E₁; F₂, E₂⟯ x) :=
  TopologicalSpace.induced
    ((Pretrivialization.continuousAlternatingMap 𝕜 ι
      (trivializationAt F₁ E₁ x) (trivializationAt F₂ E₂ x)) ∘ TotalSpace.mk' (F₁ [Λ^ι]→L[𝕜] F₂) x)
    inferInstance
...
def _root_.Bundle.continuousAlternatingMap.vectorPrebundle :
    VectorPrebundle 𝕜 (F₁ [Λ^ι]→L[𝕜] F₂) Λ^ι⟮𝕜; F₁, E₁; F₂, E₂⟯ where
...
instance Bundle.continuousAlternatingMap.topologicalSpace_totalSpace :
    TopologicalSpace (TotalSpace (F₁ [Λ^ι]→L[𝕜] F₂) Λ^ι⟮𝕜; F₁, E₁; F₂, E₂⟯) :=
...
instance _root_.Bundle.continuousAlternatingMap.fiberBundle :
    FiberBundle (F₁ [Λ^ι]→L[𝕜] F₂) Λ^ι⟮𝕜; F₁, E₁; F₂, E₂⟯ :=
...
instance _root_.Bundle.continuousAlternatingMap.vectorBundle :
    VectorBundle 𝕜 (F₁ [Λ^ι]→L[𝕜] F₂) Λ^ι⟮𝕜; F₁, E₁; F₂, E₂⟯ :=
...
def Trivialization.continuousAlternatingMap :
    Trivialization (F₁ [Λ^ι]→L[𝕜] F₂) (π (F₁ [Λ^ι]→L[𝕜] F₂) Λ^ι⟮𝕜; F₁, E₁; F₂, E₂⟯) :=
