  ...
  { if hb : b ∈ e.baseSet ∩ e'.baseSet then
    ...
    continuous_toFun := by
      ...
      · simp_rw [dif_pos hb]
        ...
        exact
          e.continuousOn_symm.comp_continuous (Continuous.Prod.mk b) fun y =>
...
theorem coe_coordChangeL (e e' : Trivialization F (π E)) [e.IsLinear R] [e'.IsLinear R] {b : B}
  ...
  congr_arg FunLike.coe (dif_pos hb)
...
/-- Forward map of `continuousLinearEquivAt` (only propositionally equal),
...
/-- Backwards map of `continuousLinearEquivAt`, defined everywhere. -/
@[simps (config := { fullyApplied := false }) apply]
...
instance localTriv.isLinear (i : ι) : (Z.localTriv i).IsLinear R where
  linear x hx := by
    ...
    exact
      { map_add := fun v w => by simp only [map_add, mfld_simps]
        map_smul := fun r v => by simp only [map_smul, mfld_simps] }
...
instance vectorBundle : VectorBundle R F Z.Fiber where
  ...
  continuousOn_coord_change' := by
    ...
    refine' (Z.continuousOn_coord_change i i').congr fun b hb => _
    ext v
...
def toFiberPrebundle (a : VectorPrebundle R F E) : FiberPrebundle F E :=
  { a with
    continuous_triv_change := by
      intro e he e' he'
      have :=
        is_bounded_bilinear_map_apply.continuous.comp_continuousOn
          ((a.continuousOn_coord_change he' he).Prod_map continuousOn_id)
      have H :
        e'.to_local_equiv.target ∩ e'.to_local_equiv.symm ⁻¹' e.to_local_equiv.source =
          (e'.base_set ∩ e.base_set) ×ˢ univ := by
...
theorem to_vectorBundle :
  ...
  { trivialization_linear' := by
    ...
    continuousOn_coord_change' := by
      ...
      refine' (a.continuousOn_coord_change he he').congr _
      intro b hb
      ext v
...
protected theorem VectorBundleCore.inCoordinates_eq {ι ι'} (Z : VectorBundleCore 𝕜₁ B F ι)
  ...
  simp_rw [in_coordinates, Z'.trivialization_at_continuousLinearMapAt hy,
