...
open Classical in
/-- The restriction of a pretrivialization to a subset of the base. The argument `c` is for junk
values only, to exclude the case where `s` and `F` are nonempty but `proj ⁻¹' s` is empty. -/
...
/-- The restriction of a pretrivialization to a nonempty subset of the base. -/
noncomputable def restrictPreimage (e : Pretrivialization F proj) {s : Set B}
    (hs : (s ∩ e.baseSet).Nonempty) : Pretrivialization F (s.restrictPreimage proj) :=
  e.restrictPreimage' s <| fun ⟨_, f⟩ ↦ Nonempty.some <|
    have ⟨z, hzs, hzb⟩ := hs
...
/-- The restriction of a trivialization to a subset of the base. The argument `c` is for junk
values only, to exclude the case where `s` and `F` are nonempty but `proj ⁻¹' s` is empty. -/
...
/-- The restriction of a trivialization to a nonempty subset of the base. -/
noncomputable def restrictPreimage (e : Trivialization F proj) {s : Set B}
