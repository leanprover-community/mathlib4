...
@[simp, mfld_simps]
lemma coe_mk (e : LocalHomeomorph Z (B × F)) (i j k l m) (x : Z) :
...
@[simp, mfld_simps]
lemma symm_apply_mk_proj (ex : x ∈ e.source) : e.toLocalHomeomorph.symm (proj x, (e x).2) = x :=
...
lemma continuousAt_proj (ex : x ∈ e.source) : ContinuousAt proj x :=
