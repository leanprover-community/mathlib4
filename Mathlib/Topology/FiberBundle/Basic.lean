...
def trivChange (i j : ι) : PartialHomeomorph (B × F) (B × F) where
  ...
  left_inv' := by
    rintro ⟨x, v⟩ hx
    simp only [mk_mem_prod_iff, mem_inter_iff, and_true, mem_univ] at hx
    dsimp only
...
def localTrivAsPartialEquiv (i : ι) : PartialEquiv Z.TotalSpace (B × F) where
  ...
  map_source' p hp := by
    simpa only [Set.mem_preimage, and_true, Set.mem_univ, Set.mk_mem_prod_iff] using hp
  map_target' p hp := by
  ...
  right_inv' := by
    rintro ⟨x, v⟩ hx
    simp only [mk_mem_prod_iff, and_true, mem_univ] at hx
    dsimp only
    rw [Z.coordChange_comp, Z.coordChange_self]
...
theorem localTrivAsPartialEquiv_trans (i j : ι) :
  ...
  · rintro ⟨x, v⟩ hx
    simp only [trivChange, localTrivAsPartialEquiv, PartialEquiv.symm,
      Prod.mk_inj, mk_mem_prod_iff, PartialEquiv.trans_source, mem_inter_iff,
...
theorem open_source' (i : ι) : IsOpen (Z.localTrivAsPartialEquiv i).source := by
  ...
  ext p
  simp only [localTrivAsPartialEquiv_apply, mk_mem_prod_iff, mem_inter_iff, and_self_iff,
