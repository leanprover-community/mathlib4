...
instance (priority := 10000) CompletelyRegularSpace.instRegularSpace [CompletelyRegularSpace X] : RegularSpace X := by
  rw [regularSpace_iff]
  intro s a hs ha
...
instance (priority := 10000) NormalSpace.instCompletelyRegularSpace [NormalSpace X] : CompletelyRegularSpace X := by
...
instance (priority := 10000) T35Space.instT3space [T35Space X] : T3Space X := by
  have : T0Space X := T1Space.t0Space
  have : RegularSpace X := CompletelyRegularSpace.instRegularSpace
  exact {}

instance (priority := 10000) T4Space.instT35Space [T4Space X] : T35Space X := by
