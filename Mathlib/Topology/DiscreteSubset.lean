...
/-- In a `T1Space`, complements of singletons are codiscrete within any set. -/
@[simp] theorem compl_singleton_mem_codiscreteWithin
    {X : Type*} [TopologicalSpace X] [T1Space X] {s : Set X} {x : X} :
...
@[simp] theorem compl_finite_mem_codiscreteWithin
    {X : Type*} [TopologicalSpace X] [T1Space X] {s t : Set X} (h : t.Finite) :
