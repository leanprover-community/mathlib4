...
theorem polynomialFunctions.eq_adjoin_X (s : Set R) :
  ...
  refine le_antisymm ?_
    (Algebra.adjoin_le fun _ h => ⟨X, trivial, (mem_singletonmm⟩)
  rintro - ⟨p, -, rfl⟩
  ...
  · rw [pow_succ, ← mul_assoc, map_mul]
    exact mul_mem hn (Algebra.subset_adjoin <| mem_singleton_self _)
...
theorem polynomialFunctions.le_equalizer {A : Type*} [Semiring A] [Algebra R A] (s : Set R)
  ...
  exact φ.adjoin_le_equalizer ψ fun x hx => (mem_singletonymm ▸ h
...
theorem polynomialFunctions.starClosure_le_equalizer {A : Type*} [StarRing R] [ContinuousStar R]
  ...
  exact StarAlgHom.adjoin_le_equalizer φ ψ fun x hx => (mem_singletonymm ▸ h
