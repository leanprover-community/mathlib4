...
theorem hasProdFilter_apply {γ : Type*} [CommMonoid β] [ContinuousMul β]
    {f : γ → C(α, β)} {g : C(α, β)} {L : Filter (Finset γ)} (hf : HasProdFilter L f g) (x : α) :
    HasProdFilter L (fun i : γ => f i x) (g x) := by
  let ev : C(α, β) →* β := (Pi.evalMonoidHom _ x).comp coeFnMonoidHom
  exact hf.map ev (continuous_eval_const x)

@[to_additive]
theorem multipliableFilter_apply [CommMonoid β] [ContinuousMul β] {γ : Type*} {f : γ → C(α, β)}
    {L : Filter (Finset γ)} (hf : MultipliableFilter L f) (x : α) :
    MultipliableFilter L fun i : γ => f i x :=
  (hasProdFilter_apply hf.hasProdFilter x).multipliableFilter

@[to_additive]
theorem tprodFilter_apply [T2Space β] [CommMonoid β] [ContinuousMul β] {γ : Type*} {f : γ → C(α, β)}
    {L : Filter (Finset γ)} [L.NeBot] (hf : MultipliableFilter L f) (x : α) :
    ∏'[L] i : γ, f i x = (∏'[L] i : γ, f i) x :=
  (hasProdFilter_apply hf.hasProdFilter x).tprodFilter_eq
