...
section MvPolynomialFunctions

open StarSubalgebra MvPolynomial
open scoped MvPolynomial

variable {σ : Type*}

/-- Multivariate polynomial functions are dense in `C(s, ℝ)` when `s` is compact. -/
theorem mvPolynomialFunctions.topologicalClosure (s : Set (σ → ℝ)) [CompactSpace s] :
    (mvPolynomialFunctions s).topologicalClosure = ⊤ :=
  ContinuousMap.subalgebra_topologicalClosure_eq_top_of_separatesPoints _
    (mvPolynomialFunctions_separatesPoints s)

/-- The star subalgebra generated by multivariate polynomial functions is dense in `C(s, 𝕜)` when
  `s` is compact and `𝕜` is either `ℝ` or `ℂ`. -/
...
/-- Every real-valued continuous function on `s` is a uniform limit of multivariate polynomials,
  when `s` is compact. -/
theorem continuousMap_mem_mvPolynomialFunctions_closure (s : Set (σ → ℝ)) [CompactSpace s]
    (f : C(s, ℝ)) :
    f ∈ (mvPolynomialFunctions s).topologicalClosure := by
  rw [mvPolynomialFunctions.topologicalClosure]
  simp

/-- Every real-valued continuous function on `s` is within any `ε > 0` of some multivariate
  polynomial, when `s` is compact. -/
theorem exists_mvPolynomial_near_continuousMap (s : Set (σ → ℝ)) [CompactSpace s]
    (f : C(s, ℝ)) (ε : ℝ) (pos : 0 < ε) :
...
/-- Every real-valued function `s → ℝ` which is continuous can be approximated up to any `ε > 0`
by some multivariate polynomial, when `s` is compact. -/
theorem exists_mvPolynomial_near_of_continuous (s : Set (σ → ℝ)) [CompactSpace s]
    (f : s → ℝ) (c : Continuous f) (ε : ℝ) (pos : 0 < ε) :
    ∃ p : MvPolynomial σ ℝ, ∀ x : s, |eval x p - f x| < ε := by
...
/-- An induction principle for `C(s, 𝕜)`. -/
@[elab_as_elim]
theorem ContinuousMap.induction_on' {𝕜 : Type*} [RCLike 𝕜] {s : Set (σ → 𝕜)}
    {p : C(s, 𝕜) → Prop} (const : ∀ r, p (.const s r)) (eval : ∀ i, p (.restrict s <| .eval i))
    (star_eval : ∀ i, p (star (.restrict s <| .eval i)))
    (add : ∀ f g, p f → p g → p (f + g)) (mul : ∀ f g, p f → p g → p (f * g))
    (closure : (∀ f ∈ (mvPolynomialFunctions s).starClosure, p f) → ∀ f, p f) (f : C(s, 𝕜)) :
    p f := by
  refine closure (fun f hf => ?_) f
  rw [mvPolynomialFunctions.starClosure_eq_adjoin_X] at hf
  induction hf using Algebra.adjoin_induction with
  | mem f hf =>
    simp only [toContinuousMapOnAlgHom_apply, Set.instInvolutiveStar, Set.mem_union,
      Set.mem_setOf_eq, Set.mem_star] at hf
    rw [← exists_or] at hf
    obtain ⟨i, hi | hi⟩ := hf
    · simp only [← hi, toContinuousMapOn_X_eq_restrict_eval, eval]
    · rw [eq_star_iff_eq_star] at hi
      simp only [hi, toContinuousMapOn_X_eq_restrict_eval, star_eval]
  | algebraMap r => exact const r
  | add _ _ _ _ hf hg => exact add _ _ hf hg
  | mul _ _ _ _ hf hg => exact mul _ _ hf hg
...
open Topology in
@[elab_as_elim]
theorem ContinuousMap.induction_on_of_compact' {𝕜 : Type*} [RCLike 𝕜] {s : Set (σ → 𝕜)}
    [CompactSpace s] {p : C(s, 𝕜) → Prop} (const : ∀ r, p (.const s r))
    (eval : ∀ i, p (.restrict s <| .eval i)) (star_eval : ∀ i, p (star (.restrict s <| .eval i)))
    (add : ∀ f g, p f → p g → p (f + g)) (mul : ∀ f g, p f → p g → p (f * g))
    (frequently : ∀ f, (∃ᶠ g in 𝓝 f, p g) → p f) (f : C(s, 𝕜)) :
    p f := by
  refine f.induction_on' const eval star_eval add mul fun h f ↦ frequently f ?_
  have := mvPolynomialFunctions.starClosure_topologicalClosure s ▸ mem_top (x := f)
  rw [← SetLike.mem_coe, topologicalClosure_coe, mem_closure_iff_frequently] at this
  exact this.mp <| .of_forall h

/-- Continuous algebra homomorphisms from `C(s, ℝ)` into an `ℝ`-algebra `A` which agree
at all `X i : MvPolynomial σ 𝕜` (interpreted as continuous maps) are, in fact, equal. -/
@[ext (iff := false)]
theorem ContinuousMap.algHom_ext_map_X' {A : Type*} [Ring A]
    [Algebra ℝ A] [TopologicalSpace A] [T2Space A] {s : Set (σ → ℝ)} [CompactSpace s]
    {φ ψ : C(s, ℝ) →ₐ[ℝ] A} (hφ : Continuous φ) (hψ : Continuous ψ)
    (h : ∀ i, φ (toContinuousMapOnAlgHom s (X i)) = ψ (toContinuousMapOnAlgHom s (X i))) :
    φ = ψ := by
  suffices (⊤ : Subalgebra ℝ C(s, ℝ)) ≤ AlgHom.equalizer φ ψ from
    AlgHom.ext fun x => this (by trivial)
  rw [← mvPolynomialFunctions.topologicalClosure s]
  exact Subalgebra.topologicalClosure_minimal (mvPolynomialFunctions s)
    (mvPolynomialFunctions.le_equalizer s φ ψ h) (isClosed_eq hφ hψ)

/-- Continuous star algebra homomorphisms from `C(s, 𝕜)` into a star `𝕜`-algebra `A` which agree
at all `X i : MvPolynomial σ 𝕜` (interpreted as continuous maps) are, in fact, equal. -/
@[ext (iff := false)]
theorem ContinuousMap.starAlgHom_ext_map_X' {𝕜 A : Type*} [RCLike 𝕜] [Ring A] [StarRing A]
    [Algebra 𝕜 A] [TopologicalSpace A] [T2Space A] {s : Set (σ → 𝕜)} [CompactSpace s]
    {φ ψ : C(s, 𝕜) →⋆ₐ[𝕜] A} (hφ : Continuous φ) (hψ : Continuous ψ)
    (h : ∀ i, φ (toContinuousMapOnAlgHom s (X i)) = ψ (toContinuousMapOnAlgHom s (X i))) :
    φ = ψ := by
  suffices (⊤ : StarSubalgebra 𝕜 C(s, 𝕜)) ≤ StarAlgHom.equalizer φ ψ from
    StarAlgHom.ext fun x => this mem_top
  rw [← mvPolynomialFunctions.starClosure_topologicalClosure s]
  exact StarSubalgebra.topologicalClosure_minimal
    (mvPolynomialFunctions.starClosure_le_equalizer s φ ψ h) (isClosed_eq hφ hψ)

end MvPolynomialFunctions
...
lemma ContinuousMapZero.nonUnitalStarAlgHom_apply_mul_eq_zero {𝕜 A : Type*}
    [RCLike 𝕜] [NonUnitalRing A] [StarRing A] [TopologicalSpace A] [IsTopologicalSemiring A]
