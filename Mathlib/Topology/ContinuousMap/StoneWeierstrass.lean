...
theorem sublattice_closure_eq_top (L : Set C(X, ℝ)) (nA : L.Nonempty)
  ...
  have lt_h : ∀ x z, f z - ε < (h x : X → ℝ) z := by
    ...
    exact ⟨y, ym, zm⟩
  have h_eq : ∀ x, (h x : X → ℝ) x = f x := by intro x; simp +zetaDelta [w₁]
  ...
  fconstructor
  · dsimp
    simp +zetaDelta only [Finset.inf'_lt_iff, ContinuousMap.inf'_apply]
    exact Set.exists_set_mem_of_union_eq_top _ _ xs_w z
  · dsimp
    simp +zetaDelta only [Finset.lt_inf'_iff, ContinuousMap.inf'_apply]
    rintro x -
    apply lt_h
