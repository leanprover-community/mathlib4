...
def evalCLM (𝕜 : Type*) {R : Type*} [NormedField 𝕜] [NormedCommRing R] [NormedSpace 𝕜 R] (x : X) :
    C(X, R)₀ →L[𝕜] R :=
  (ContinuousMap.evalCLM 𝕜 x).comp (toContinuousMapCLM 𝕜 : C(X, R)₀ →L[𝕜] C(X, R))

@[simp]
lemma evalCLM_apply {𝕜 : Type*} {R : Type*} [NormedField 𝕜] [NormedCommRing R] [NormedSpace 𝕜 R]
