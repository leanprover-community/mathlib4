...
theorem supportDiscreteWithin_iff_locallyFiniteWithin [T1Space X] [Zero Y] {f : X → Y}
  ...
  have : f.support = (U \ {x | f x = (0 : X → Y) x}) := by
    ext x
    simp only [mem_support, ne_eq, Pi.zero_apply, Set.mem_sdiff, mem_ofPred, iff_and_self]
    exact (h ·)
...
theorem eq_zero_codiscreteWithin [Zero Y] [T1Space X] (D : locallyFinsuppWithin U Y) :
  ...
  have : D.support = (U \ {x | D x = (0 : X → Y) x}) := by
    ext x
    simp only [mem_support, ne_eq, Pi.zero_apply, Set.mem_sdiff, Set.mem_ofPred, iff_and_self]
...
theorem discreteSupport [Zero Y] [T1Space X] (D : locallyFinsuppWithin U Y) :
  ...
  have : D.support = {x | D x = 0}ᶜ ∩ U := by
    ...
    · intro hx
      rw [mem_inter_iff, mem_compl_iff, mem_ofPred] at hx
      tauto
  rw [this]
...
protected def addSubmonoid [AddMonoid Y] : AddSubmonoid (X → Y) where
  ...
  zero_mem' := by
    simp only [support_subset_iff, ne_eq, mem_ofPred, Pi.zero_apply, not_true_eq_false,
  ...
  add_mem' {f g} hf hg := by
    ...
    · intro z hz
      ...
      intro a ha
      simp_all only [support_subset_iff, ne_eq, mem_ofPred,
