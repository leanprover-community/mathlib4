...
@[continuity]
lemma continuous_matrix [TopologicalSpace α] {f : α → Matrix m n R}
...
@[continuity]
lemma Continuous.matrix_map [TopologicalSpace S] {A : X → Matrix m n S} {f : S → R}
...
lemma Continuous.matrix_conjTranspose [Star R] [ContinuousStar R] {A : X → Matrix m n R}
...
@[continuity]
lemma Continuous.matrix_col {A : X → n → R} (hA : Continuous A) : Continuous fun x => col (A x) :=
  continuous_matrix fun i _ => (continuous_apply i).comp hA
#align continuous.matrix_col Continuous.matrix_col

@[continuity]
lemma Continuous.matrix_row {A : X → n → R} (hA : Continuous A) : Continuous fun x => row (A x) :=
  continuous_matrix fun _ _ => (continuous_apply _).comp hA
#align continuous.matrix_row Continuous.matrix_row

@[continuity]
lemma Continuous.matrix_diagonal [Zero R] [DecidableEq n] {A : X → n → R} (hA : Continuous A) :
...
@[continuity]
lemma Continuous.matrix_dotProduct [Fintype n] [Mul R] [AddCommMonoid R] [ContinuousAdd R]
...
@[continuity]
lemma Continuous.matrix_mul [Fintype n] [Mul R] [AddCommMonoid R] [ContinuousAdd R]
...
@[continuity]
lemma Continuous.matrix_vecMulVec [Mul R] [ContinuousMul R] {A : X → m → R} {B : X → n → R}
...
@[continuity]
lemma Continuous.matrix_mulVec [NonUnitalNonAssocSemiring R] [ContinuousAdd R] [ContinuousMul R]
...
@[continuity]
lemma Continuous.matrix_vecMul [NonUnitalNonAssocSemiring R] [ContinuousAdd R] [ContinuousMul R]
...
@[continuity]
lemma Continuous.matrix_submatrix {A : X → Matrix l n R} (hA : Continuous A) (e₁ : m → l)
...
@[continuity]
lemma Continuous.matrix_reindex {A : X → Matrix l n R} (hA : Continuous A) (e₁ : l ≃ m)
...
@[continuity]
lemma Continuous.matrix_trace [Fintype n] [AddCommMonoid R] [ContinuousAdd R]
...
@[continuity]
lemma Continuous.matrix_det [Fintype n] [DecidableEq n] [CommRing R] [TopologicalRing R]
...
@[continuity]
lemma Continuous.matrix_updateColumn [DecidableEq n] (i : n) {A : X → Matrix m n R}
...
@[continuity]
lemma Continuous.matrix_updateRow [DecidableEq m] (i : m) {A : X → Matrix m n R} {B : X → n → R}
...
@[continuity]
lemma Continuous.matrix_cramer [Fintype n] [DecidableEq n] [CommRing R] [TopologicalRing R]
...
@[continuity]
lemma Continuous.matrix_adjugate [Fintype n] [DecidableEq n] [CommRing R] [TopologicalRing R]
...
lemma continuousAt_matrix_inv [Fintype n] [DecidableEq n] [CommRing R] [TopologicalRing R]
...
@[continuity]
lemma Continuous.matrix_fromBlocks {A : X → Matrix n l R} {B : X → Matrix n m R}
...
@[continuity]
lemma Continuous.matrix_blockDiagonal [Zero R] [DecidableEq p] {A : X → p → Matrix m n R}
...
@[continuity]
lemma Continuous.matrix_blockDiag {A : X → Matrix (m × p) (n × p) R} (hA : Continuous A) :
...
@[continuity]
lemma Continuous.matrix_blockDiagonal' [Zero R] [DecidableEq l]
...
@[continuity]
lemma Continuous.matrix_blockDiag' {A : X → Matrix (Σi, m' i) (Σi, n' i) R} (hA : Continuous A) :
    Continuous fun x => blockDiag' (A x) :=
  continuous_pi fun _i => continuous_matrix fun _j _k => hA.matrix_elem _ _
#align continuous.matrix_block_diag' Continuous.matrix_blockDiag'
...
lemma HasSum.matrix_transpose {f : X → Matrix m n R} {a : Matrix m n R} (hf : HasSum f a) :
...
@[simp]
lemma summable_matrix_transpose {f : X → Matrix m n R} :
...
lemma Matrix.transpose_tsum [T2Space R] {f : X → Matrix m n R} : (∑' x, f x)ᵀ = ∑' x, (f x)ᵀ := by
...
lemma HasSum.matrix_conjTranspose [StarAddMonoid R] [ContinuousStar R] {f : X → Matrix m n R}
...
lemma Summable.matrix_conjTranspose [StarAddMonoid R] [ContinuousStar R] {f : X → Matrix m n R}
...
@[simp]
lemma summable_matrix_conjTranspose [StarAddMonoid R] [ContinuousStar R] {f : X → Matrix m n R} :
...
lemma Matrix.conjTranspose_tsum [StarAddMonoid R] [ContinuousStar R] [T2Space R]
...
lemma HasSum.matrix_diagonal [DecidableEq n] {f : X → n → R} {a : n → R} (hf : HasSum f a) :
...
lemma Summable.matrix_diagonal [DecidableEq n] {f : X → n → R} (hf : Summable f) :
    Summable fun x => diagonal (f x) :=
  hf.hasSum.matrix_diagonal.summable
#align summable.matrix_diagonal Summable.matrix_diagonal

@[simp]
lemma summable_matrix_diagonal [DecidableEq n] {f : X → n → R} :
...
lemma Matrix.diagonal_tsum [DecidableEq n] [T2Space R] {f : X → n → R} :
...
lemma HasSum.matrix_diag {f : X → Matrix n n R} {a : Matrix n n R} (hf : HasSum f a) :
...
lemma HasSum.matrix_blockDiagonal [DecidableEq p] {f : X → p → Matrix m n R}
...
lemma Summable.matrix_blockDiagonal [DecidableEq p] {f : X → p → Matrix m n R} (hf : Summable f) :
...
lemma summable_matrix_blockDiagonal [DecidableEq p] {f : X → p → Matrix m n R} :
...
lemma Matrix.blockDiagonal_tsum [DecidableEq p] [T2Space R] {f : X → p → Matrix m n R} :
...
lemma HasSum.matrix_blockDiag {f : X → Matrix (m × p) (n × p) R} {a : Matrix (m × p) (n × p) R}
...
lemma Summable.matrix_blockDiag {f : X → Matrix (m × p) (n × p) R} (hf : Summable f) :
...
lemma HasSum.matrix_blockDiagonal' [DecidableEq l] {f : X → ∀ i, Matrix (m' i) (n' i) R}
    {a : ∀ i, Matrix (m' i) (n' i) R} (hf : HasSum f a) :
    HasSum (fun x => blockDiagonal' (f x)) (blockDiagonal' a) :=
  hf.map (blockDiagonal'AddMonoidHom m' n' R) continuous_id.matrix_blockDiagonal'
#align has_sum.matrix_block_diagonal' HasSum.matrix_blockDiagonal'

lemma Summable.matrix_blockDiagonal' [DecidableEq l] {f : X → ∀ i, Matrix (m' i) (n' i) R}
    (hf : Summable f) : Summable fun x => blockDiagonal' (f x) :=
  hf.hasSum.matrix_blockDiagonal'.summable
#align summable.matrix_block_diagonal' Summable.matrix_blockDiagonal'

lemma summable_matrix_blockDiagonal' [DecidableEq l] {f : X → ∀ i, Matrix (m' i) (n' i) R} :
    (Summable fun x => blockDiagonal' (f x)) ↔ Summable f :=
...
lemma Matrix.blockDiagonal'_tsum [DecidableEq l] [T2Space R]
...
lemma HasSum.matrix_blockDiag' {f : X → Matrix (Σi, m' i) (Σi, n' i) R}
...
lemma Summable.matrix_blockDiag' {f : X → Matrix (Σi, m' i) (Σi, n' i) R} (hf : Summable f) :
    Summable fun x => blockDiag' (f x) :=
  hf.hasSum.matrix_blockDiag'.summable
#align summable.matrix_block_diag' Summable.matrix_blockDiag'
