...
@[continuity]
theorem Continuous.matrix_col {ι : Type*} [Unique ι] {A : X → n → R} (hA : Continuous A) :
    Continuous fun x => col (ι := ι) (A x) :=
  continuous_matrix fun i _ => (continuous_apply i).comp hA
#align continuous.matrix_col Continuous.matrix_col

@[continuity]
theorem Continuous.matrix_row {ι : Type*} [Unique ι] {A : X → n → R} (hA : Continuous A) :
    Continuous fun x => row (ι := ι) (A x) :=
