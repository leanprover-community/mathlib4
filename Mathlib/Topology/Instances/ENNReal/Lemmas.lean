...
theorem continuousOn_sub :
  ...
  rw [ContinuousOn]
  rintro ⟨x, y⟩ hp
  simp only [Ne, Set.mem_ofPred, Prod.mk_inj] at hp
...
theorem continuous_sub_left {a : ℝ≥0∞} (a_ne_top : a ≠ ∞) : Continuous (a - ·) := by
  ...
  simp only [a_ne_top, Ne, mem_ofPred, Prod.mk_inj, false_and, not_false_iff]
...
theorem continuous_sub_right (a : ℝ≥0∞) : Continuous fun x : ℝ≥0∞ => x - a := by
  ...
  · rw [show (fun x => x - a) = (fun p : ℝ≥0∞ × ℝ≥0∞ => p.fst - p.snd) ∘ fun x => ⟨x, a⟩ by rfl]
    ...
    intro x
    simp only [a_infty, Ne, mem_ofPred, Prod.mk_inj, and_false, not_false_iff]
...
lemma Dense.lipschitzWith_extend {α β : Type*}
  ...
  have : Dense {p : α × α | edist (hs.extend f p.1) (hs.extend f p.2) ≤ K * edist p.1 p.2} := by
    ...
    simp only [Set.mem_ofPred, Ax, Ay]
    exact hf ⟨x, hx⟩ ⟨y, hy⟩
  simpa only [Dense, IsClosed.closure_eq, Set.mem_ofPred, Prod.forall] using! this
