...
lemma Nat.uniformEmbedding_coe_rat : UniformEmbedding ((↑) : ℕ → ℚ) :=
...
lemma Nat.closedEmbedding_coe_rat : ClosedEmbedding ((↑) : ℕ → ℚ) :=
...
lemma Int.uniformEmbedding_coe_rat : UniformEmbedding ((↑) : ℤ → ℚ) :=
...
lemma Int.closedEmbedding_coe_rat : ClosedEmbedding ((↑) : ℤ → ℚ) :=
...
lemma uniformContinuous_add : UniformContinuous fun p : ℚ × ℚ => p.1 + p.2 :=
...
lemma uniformContinuous_neg : UniformContinuous (@Neg.neg ℚ _) :=
...
lemma uniformContinuous_abs : UniformContinuous (abs : ℚ → ℚ) :=
  Metric.uniformContinuous_iff.2 fun ε ε0 =>
    ⟨ε, ε0, fun h =>
