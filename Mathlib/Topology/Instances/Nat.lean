...
noncomputable instance (priority := 10000) : Dist ℕ :=
  ⟨fun x y => dist (x : ℝ) y⟩
...
instance (priority := 10000) : MetricSpace ℕ := Nat.uniformEmbedding_coe_real.comapMetricSpace _
...
instance (priority := 10000) : ProperSpace ℕ :=
  ⟨fun x r => by
    rw [closedBall_eq_Icc]
    exact (Set.finite_Icc _ _).isCompact⟩

instance (priority := 10000) : NoncompactSpace ℕ :=
