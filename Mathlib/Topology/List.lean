...
@[simp]
lemma nhds_nil : 𝓝 ([] : List α) = pure [] := by
...
lemma Filter.Tendsto.cons {α : Type*} {f : α → β} {g : α → List β} {a : Filter α} {b : β}
...
lemma tendsto_cons_iff {β : Type*} {f : List α → β} {b : Filter β} {a : α} {l : List α} :
...
lemma continuous_cons : Continuous fun x : α × List α => (x.1::x.2 : List α) :=
...
lemma tendsto_nhds {β : Type*} {f : List α → β} {r : List α → Filter β}
...
lemma continuousAt_length : ∀ l : List α, ContinuousAt List.length l := by
...
lemma tendsto_insertNth {β} {n : ℕ} {a : α} {l : List α} {f : β → α} {g : β → List α}
...
lemma continuous_insertNth {n : ℕ} : Continuous fun p : α × List α => insertNth n p.1 p.2 :=
...
lemma continuous_removeNth {n : ℕ} : Continuous fun l : List α => removeNth l n :=
  continuous_iff_continuousAt.mpr fun _a => tendsto_removeNth
#align list.continuous_remove_nth List.continuous_removeNth

@[to_additive]
lemma tendsto_prod [Monoid α] [ContinuousMul α] {l : List α} :
...
@[to_additive]
lemma continuous_prod [Monoid α] [ContinuousMul α] : Continuous (prod : List α → α) :=
...
lemma tendsto_insertNth {n : ℕ} {i : Fin (n + 1)} {a : α} :
    ∀ {l : Vector α n},
...
lemma continuous_removeNth {n : ℕ} {i : Fin (n + 1)} :
