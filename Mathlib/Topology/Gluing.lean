...
theorem rel_equiv : Equivalence D.Rel :=
  ⟨fun x => Or.inl (refl x), by
    rintro a b (⟨⟨⟩⟩ | ⟨x, e₁, e₂⟩)
    -- `erw` works around the `ConcreteCategory`/`HasForget` mismatch:
...
theorem ι_eq_iff_rel (i j : D.J) (x : D.U i) (y : D.U j) :
  ...
  · delta GlueData.ι
    ...
    erw [TopCat.comp_app, sigmaIsoSigma_inv_apply, ← ConcreteCategory.comp_apply,
      ← ConcreteCategory.comp_apply, colimit.ι_desc_assoc, ← ConcreteCategory.comp_apply,
      ← ConcreteCategory.comp_apply, colimit.ι_desc_assoc]
...
theorem image_inter (i j : D.J) :
  ...
  · rintro ⟨⟨x₁, eq₁⟩, ⟨x₂, eq₂⟩⟩
    ...
    · exact ⟨inv (D.f i i) x₁, by
        ...
        -- See https://github.com/leanprover-community/mathlib4/issues/5026
        -- `erw` works around the `ConcreteCategory`/`HasForget` mismatch:
        erw [TopCat.comp_app, CategoryTheory.IsIso.inv_hom_id_apply (D.f i i), eq₁]⟩
    · subst eq₁
      exact ⟨y, by simp [e₁]⟩
  · rintro ⟨x, hx⟩
