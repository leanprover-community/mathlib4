...
lemma inducing_induced (f : X → Y) : @Inducing X Y (TopologicalSpace.induced f ‹_›) _ f :=
  @Inducing.mk _ _ (TopologicalSpace.induced f ‹_›) _ _ rfl

lemma inducing_id : Inducing (@id X) :=
  ⟨induced_id.symm⟩
#align inducing_id inducing_id

protected lemma Inducing.comp (hg : Inducing g) (hf : Inducing f) :
    Inducing (g ∘ f) :=
  ⟨by rw [hf.induced, hg.induced, induced_compose]⟩
#align inducing.comp Inducing.comp

lemma Inducing.of_comp_iff (hg : Inducing g) :
...
lemma continuousAt_iff' (hf : Inducing f) {x : X} (h : range f ∈ 𝓝 (f x)) :
    ContinuousAt (g ∘ f) x ↔ ContinuousAt g (f x) := by
  simp_rw [ContinuousAt, Filter.Tendsto, ← hf.map_nhds_of_mem _ h, Filter.map_map, comp]
#align inducing.continuous_at_iff' Inducing.continuousAt_iff'

protected lemma continuous (hf : Inducing f) : Continuous f :=
  hf.continuous_iff.mp continuous_id
#align inducing.continuous Inducing.continuous

lemma closure_eq_preimage_closure_image (hf : Inducing f) (s : Set X) :
...
lemma isClosed_iff (hf : Inducing f) {s : Set X} :
...
lemma of_subsingleton [Subsingleton X] (f : X → Y) : Inducing f :=
...
lemma Function.Injective.embedding_induced [t : TopologicalSpace Y] (hf : Injective f) :
...
lemma Embedding.mk' (f : X → Y) (inj : Injective f) (induced : ∀ x, comap f (𝓝 (f x)) = 𝓝 x) :
    Embedding f :=
  ⟨inducing_iff_nhds.2 fun x => (induced x).symm, inj⟩
#align embedding.mk' Embedding.mk'

lemma embedding_id : Embedding (@id X) :=
  ⟨inducing_id, fun _ _ h => h⟩
#align embedding_id embedding_id

protected lemma Embedding.comp (hg : Embedding g) (hf : Embedding f) :
    Embedding (g ∘ f) :=
  { hg.toInducing.comp hf.toInducing with inj := fun _ _ h => hf.inj <| hg.inj h }
#align embedding.comp Embedding.comp

lemma Embedding.of_comp_iff (hg : Embedding g) : Embedding (g ∘ f) ↔ Embedding f := by
...
protected lemma Function.LeftInverse.embedding {f : X → Y} {g : Y → X} (h : LeftInverse f g)
...
lemma Embedding.map_nhds_eq (hf : Embedding f) (x : X) :
    (𝓝 x).map f = 𝓝[range f] f x :=
  hf.1.map_nhds_eq x
#align embedding.map_nhds_eq Embedding.map_nhds_eq

lemma Embedding.map_nhds_of_mem (hf : Embedding f) (x : X) (h : range f ∈ 𝓝 (f x)) :
    (𝓝 x).map f = 𝓝 (f x) :=
  hf.1.map_nhds_of_mem x h
#align embedding.map_nhds_of_mem Embedding.map_nhds_of_mem

lemma Embedding.tendsto_nhds_iff {f : ι → Y} {l : Filter ι} {y : Y}
...
lemma Embedding.continuous_iff (hg : Embedding g) :
    Continuous f ↔ Continuous (g ∘ f) :=
  Inducing.continuous_iff hg.1
#align embedding.continuous_iff Embedding.continuous_iff

lemma Embedding.continuous (hf : Embedding f) : Continuous f :=
  Inducing.continuous hf.1
#align embedding.continuous Embedding.continuous

lemma Embedding.closure_eq_preimage_closure_image (hf : Embedding f) (s : Set X) :
...
lemma Embedding.of_subsingleton [Subsingleton X] (f : X → Y) : Embedding f :=
...
lemma quotientMap_iff : QuotientMap f ↔ Surjective f ∧ ∀ s : Set Y, IsOpen s ↔ IsOpen (f ⁻¹' s) :=
  and_congr Iff.rfl TopologicalSpace.ext_iff
#align quotient_map_iff quotientMap_iff

lemma quotientMap_iff_closed :
...
protected lemma id : QuotientMap (@id X) :=
  ⟨fun x => ⟨x, rfl⟩, coinduced_id.symm⟩
#align quotient_map.id QuotientMap.id

protected lemma comp (hg : QuotientMap g) (hf : QuotientMap f) : QuotientMap (g ∘ f) :=
  ⟨hg.left.comp hf.left, by rw [hg.right, hf.right, coinduced_compose]⟩
#align quotient_map.comp QuotientMap.comp

protected lemma of_quotientMap_compose (hf : Continuous f) (hg : Continuous g)
...
lemma of_inverse {g : Y → X} (hf : Continuous f) (hg : Continuous g) (h : LeftInverse g f) :
    QuotientMap g :=
...
protected lemma continuous_iff (hf : QuotientMap f) : Continuous g ↔ Continuous (g ∘ f) := by
...
protected lemma continuous (hf : QuotientMap f) : Continuous f :=
  hf.continuous_iff.mp continuous_id
#align quotient_map.continuous QuotientMap.continuous

protected lemma surjective (hf : QuotientMap f) : Surjective f :=
  hf.1
#align quotient_map.surjective QuotientMap.surjective

protected lemma isOpen_preimage (hf : QuotientMap f) {s : Set Y} : IsOpen (f ⁻¹' s) ↔ IsOpen s :=
  ((quotientMap_iff.1 hf).2 s).symm
#align quotient_map.is_open_preimage QuotientMap.isOpen_preimage

protected lemma isClosed_preimage (hf : QuotientMap f) {s : Set Y} :
...
protected lemma id : IsOpenMap (@id X) := fun s hs => by rwa [image_id]
#align is_open_map.id IsOpenMap.id

protected lemma comp (hg : IsOpenMap g) (hf : IsOpenMap f) :
    IsOpenMap (g ∘ f) := fun s hs => by rw [image_comp]; exact hg _ (hf _ hs)
#align is_open_map.comp IsOpenMap.comp

lemma isOpen_range (hf : IsOpenMap f) : IsOpen (range f) := by
  rw [← image_univ]
  exact hf _ isOpen_univ
#align is_open_map.is_open_range IsOpenMap.isOpen_range

lemma image_mem_nhds (hf : IsOpenMap f) {x : X} {s : Set X} (hx : s ∈ 𝓝 x) : f '' s ∈ 𝓝 (f x) :=
...
lemma range_mem_nhds (hf : IsOpenMap f) (x : X) : range f ∈ 𝓝 (f x) :=
  hf.isOpen_range.mem_nhds <| mem_range_self _
#align is_open_map.range_mem_nhds IsOpenMap.range_mem_nhds

lemma mapsTo_interior (hf : IsOpenMap f) {s : Set X} {t : Set Y} (h : MapsTo f s t) :
...
lemma image_interior_subset (hf : IsOpenMap f) (s : Set X) :
...
lemma nhds_le (hf : IsOpenMap f) (x : X) : 𝓝 (f x) ≤ (𝓝 x).map f :=
  le_map fun _ => hf.image_mem_nhds
#align is_open_map.nhds_le IsOpenMap.nhds_le

lemma of_nhds_le (hf : ∀ x, 𝓝 (f x) ≤ map f (𝓝 x)) : IsOpenMap f := fun _s hs =>
...
lemma of_inverse {f' : Y → X} (h : Continuous f') (l_inv : LeftInverse f f')
...
lemma interior_preimage_subset_preimage_interior (hf : IsOpenMap f) {s : Set Y} :
...
lemma preimage_interior_eq_interior_preimage (hf₁ : IsOpenMap f) (hf₂ : Continuous f)
...
lemma preimage_closure_subset_closure_preimage (hf : IsOpenMap f) {s : Set Y} :
...
lemma preimage_closure_eq_closure_preimage (hf : IsOpenMap f) (hfc : Continuous f) (s : Set Y) :
...
lemma preimage_frontier_subset_frontier_preimage (hf : IsOpenMap f) {s : Set Y} :
...
lemma preimage_frontier_eq_frontier_preimage (hf : IsOpenMap f) (hfc : Continuous f) (s : Set Y) :
    f ⁻¹' frontier s = frontier (f ⁻¹' s) := by
...
lemma of_isEmpty [h : IsEmpty X] (f : X → Y) : IsOpenMap f := of_nhds_le h.elim

end IsOpenMap

lemma isOpenMap_iff_nhds_le : IsOpenMap f ↔ ∀ x : X, 𝓝 (f x) ≤ (𝓝 x).map f :=
  ⟨fun hf => hf.nhds_le, IsOpenMap.of_nhds_le⟩
#align is_open_map_iff_nhds_le isOpenMap_iff_nhds_le

lemma isOpenMap_iff_interior : IsOpenMap f ↔ ∀ s, f '' interior s ⊆ interior (f '' s) :=
...
protected lemma id : IsClosedMap (@id X) := fun s hs => by rwa [image_id]
#align is_closed_map.id IsClosedMap.id

protected lemma comp (hg : IsClosedMap g) (hf : IsClosedMap f) : IsClosedMap (g ∘ f) := by
  intro s hs
  rw [image_comp]
  exact hg _ (hf _ hs)
#align is_closed_map.comp IsClosedMap.comp

lemma closure_image_subset (hf : IsClosedMap f) (s : Set X) :
...
lemma of_nonempty (h : ∀ s, IsClosed s → s.Nonempty → IsClosed (f '' s)) :
    IsClosedMap f := by
...
lemma isClosed_range (hf : IsClosedMap f) : IsClosed (range f) :=
...
lemma to_quotientMap (hcl : IsClosedMap f) (hcont : Continuous f)
...
lemma Inducing.isClosedMap (hf : Inducing f) (h : IsClosed (range f)) : IsClosedMap f := by
  intro s hs
...
lemma isClosedMap_iff_closure_image :
...
lemma IsClosedMap.closure_image_eq_of_continuous
...
lemma IsClosedMap.lift'_closure_map_eq
...
lemma IsClosedMap.mapClusterPt_iff_lift'_closure
...
lemma OpenEmbedding.isOpenMap (hf : OpenEmbedding f) : IsOpenMap f :=
  hf.toEmbedding.toInducing.isOpenMap hf.isOpen_range
#align open_embedding.is_open_map OpenEmbedding.isOpenMap

lemma OpenEmbedding.map_nhds_eq (hf : OpenEmbedding f) (x : X) :
    map f (𝓝 x) = 𝓝 (f x) :=
...
lemma OpenEmbedding.open_iff_image_open (hf : OpenEmbedding f) {s : Set X} :
    IsOpen s ↔ IsOpen (f '' s) :=
...
lemma OpenEmbedding.tendsto_nhds_iff {f : ι → Y} {l : Filter ι} {y : Y} (hg : OpenEmbedding g) :
...
lemma openEmbedding_of_embedding_open (h₁ : Embedding f) (h₂ : IsOpenMap f) :
    OpenEmbedding f :=
  ⟨h₁, h₂.isOpen_range⟩
...
lemma openEmbedding_iff_embedding_open :
...
lemma openEmbedding_of_continuous_injective_open
...
lemma openEmbedding_iff_continuous_injective_open :
...
lemma isOpenMap_iff (hg : OpenEmbedding g) :
...
lemma of_comp_iff (f : X → Y) (hg : OpenEmbedding g) :
...
lemma tendsto_nhds_iff {g : ι → X} {l : Filter ι} {x : X} (hf : ClosedEmbedding f) :
...
lemma closed_iff_preimage_closed (hf : ClosedEmbedding f) {s : Set Y}
...
lemma _root_.closedEmbedding_of_embedding_closed (h₁ : Embedding f) (h₂ : IsClosedMap f) :
    ClosedEmbedding f :=
  ⟨h₁, image_univ (f := f) ▸ h₂ univ isClosed_univ⟩
#align closed_embedding_of_embedding_closed closedEmbedding_of_embedding_closed

lemma _root_.closedEmbedding_of_continuous_injective_closed (h₁ : Continuous f) (h₂ : Injective f)
...
lemma _root_.closedEmbedding_id : ClosedEmbedding (@id X) :=
  ⟨embedding_id, IsClosedMap.id.isClosed_range⟩
#align closed_embedding_id closedEmbedding_id

lemma comp (hg : ClosedEmbedding g) (hf : ClosedEmbedding f) :
    ClosedEmbedding (g ∘ f) :=
...
lemma of_comp_iff (hg : ClosedEmbedding g) :
...
lemma closure_image_eq (hf : ClosedEmbedding f) (s : Set X) :
    closure (f '' s) = f '' closure s :=
