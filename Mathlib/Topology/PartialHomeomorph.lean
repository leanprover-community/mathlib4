...
@[simp, mfld_simps]
lemma mk_coe (e : PartialEquiv X Y) (a b c d) : (PartialHomeomorph.mk e a b c d : X → Y) = e :=
  rfl
#align local_homeomorph.mk_coe PartialHomeomorph.mk_coe

@[simp, mfld_simps]
lemma mk_coe_symm (e : PartialEquiv X Y) (a b c d) :
...
@[simp, mfld_simps]
lemma map_target {x : Y} (h : x ∈ e.target) : e.symm x ∈ e.source :=
  e.map_target' h
#align local_homeomorph.map_target PartialHomeomorph.map_target

@[simp, mfld_simps]
lemma left_inv {x : X} (h : x ∈ e.source) : e.symm (e x) = x :=
  e.left_inv' h
#align local_homeomorph.left_inv PartialHomeomorph.left_inv

@[simp, mfld_simps]
lemma right_inv {x : Y} (h : x ∈ e.target) : e (e.symm x) = x :=
  e.right_inv' h
...
lemma eq_of_partialEquiv_eq {e e' : PartialHomeomorph X Y}
...
lemma eventually_right_inverse' {x} (hx : x ∈ e.source) :
    ∀ᶠ y in 𝓝 (e x), e (e.symm y) = y :=
  e.eventually_right_inverse (e.map_source hx)
#align local_homeomorph.eventually_right_inverse' PartialHomeomorph.eventually_right_inverse'

lemma eventually_ne_nhdsWithin {x} (hx : x ∈ e.source) :
    ∀ᶠ x' in 𝓝[≠] x, e x' ≠ e x :=
...
lemma nhdsWithin_source_inter {x} (hx : x ∈ e.source) (s : Set X) : 𝓝[e.source ∩ s] x = 𝓝[s] x :=
...
lemma nhdsWithin_target_inter {x} (hx : x ∈ e.target) (s : Set Y) : 𝓝[e.target ∩ s] x = 𝓝[s] x :=
...
protected lemma ext_iff {e e' : PartialHomeomorph X Y} :
...
lemma image_mem_nhds {x} (hx : x ∈ e.source) {s : Set X} (hs : s ∈ 𝓝 x) : e '' s ∈ 𝓝 (e x) :=
  e.map_nhds_eq hx ▸ Filter.image_mem_map hs
#align local_homeomorph.image_mem_nhds PartialHomeomorph.image_mem_nhds

lemma map_nhdsWithin_eq {x} (hx : x ∈ e.source) (s : Set X) :
...
lemma map_nhdsWithin_preimage_eq {x} (hx : x ∈ e.source) (s : Set Y) :
...
lemma eventually_nhds {x : X} (p : Y → Prop) (hx : x ∈ e.source) :
...
lemma eventually_nhds' {x : X} (p : X → Prop) (hx : x ∈ e.source) :
...
lemma eventually_nhdsWithin {x : X} (p : Y → Prop) {s : Set X}
...
lemma eventually_nhdsWithin' {x : X} (p : X → Prop) {s : Set X}
...
lemma isOpen_inter_preimage {s : Set Y} (hs : IsOpen s) : IsOpen (e.source ∩ e ⁻¹' s) :=
...
lemma isOpen_inter_preimage_symm {s : Set X} (hs : IsOpen s) : IsOpen (e.target ∩ e.symm ⁻¹' s) :=
...
lemma isOpen_image_iff_of_subset_source {s : Set X} (hs : s ⊆ e.source) :
...
lemma toPartialEquiv (h : e.IsImage s t) : e.toPartialEquiv.IsImage s t :=
  h
...
lemma leftInvOn_piecewise {e' : PartialHomeomorph X Y} [∀ i, Decidable (i ∈ s)]
...
lemma inter_eq_of_inter_eq_of_eqOn {e' : PartialHomeomorph X Y} (h : e.IsImage s t)
...
lemma symm_eqOn_of_inter_eq_of_eqOn {e' : PartialHomeomorph X Y} (h : e.IsImage s t)
...
lemma isImage_source_target_of_disjoint (e' : PartialHomeomorph X Y)
...
@[simp, mfld_simps]
lemma restrOpen_toPartialEquiv (s : Set X) (hs : IsOpen s) :
...
lemma restrOpen_source (s : Set X) (hs : IsOpen s) : (e.restrOpen s hs).source = e.source ∩ s :=
  rfl
...
@[simp, mfld_simps]
lemma restr_toPartialEquiv (s : Set X) :
...
lemma restr_source' (s : Set X) (hs : IsOpen s) : (e.restr s).source = e.source ∩ s := by
  rw [e.restr_source, hs.interior_eq]
#align local_homeomorph.restr_source' PartialHomeomorph.restr_source'

lemma restr_toPartialEquiv' (s : Set X) (hs : IsOpen s) :
    (e.restr s).toPartialEquiv = e.toPartialEquiv.restr s := by
  rw [e.restr_toPartialEquiv, hs.interior_eq]
#align local_homeomorph.restr_to_local_equiv' PartialHomeomorph.restr_toPartialEquiv'

lemma restr_eq_of_source_subset {e : PartialHomeomorph X Y} {s : Set X} (h : e.source ⊆ s) :
    e.restr s = e :=
...
@[simp, mfld_simps]
lemma restr_univ {e : PartialHomeomorph X Y} : e.restr univ = e :=
  restr_eq_of_source_subset (subset_univ _)
#align local_homeomorph.restr_univ PartialHomeomorph.restr_univ

lemma restr_source_inter (s : Set X) : e.restr (e.source ∩ s) = e.restr s := by
...
@[simp, mfld_simps]
lemma refl_partialEquiv : (PartialHomeomorph.refl X).toPartialEquiv = PartialEquiv.refl X :=
  rfl
#align local_homeomorph.refl_local_equiv PartialHomeomorph.refl_partialEquiv

@[simp, mfld_simps]
lemma refl_symm : (PartialHomeomorph.refl X).symm = PartialHomeomorph.refl X :=
  rfl
...
@[simp, mfld_simps]
lemma ofSet_toPartialEquiv : (ofSet s hs).toPartialEquiv = PartialEquiv.ofSet s :=
  rfl
...
@[simp, mfld_simps]
lemma ofSet_univ_eq_refl : ofSet univ isOpen_univ = PartialHomeomorph.refl X := by ext <;> simp
...
lemma trans_of_set' {s : Set Y} (hs : IsOpen s) :
...
lemma ofSet_trans {s : Set X} (hs : IsOpen s) : (ofSet s hs).trans e = e.restr s :=
...
lemma ofSet_trans' {s : Set X} (hs : IsOpen s) :
    (ofSet s hs).trans e = e.restr (e.source ∩ s) := by
  rw [ofSet_trans, restr_source_inter]
#align local_homeomorph.of_set_trans' PartialHomeomorph.ofSet_trans'

@[simp, mfld_simps]
lemma ofSet_trans_ofSet {s : Set X} (hs : IsOpen s) {s' : Set X} (hs' : IsOpen s') :
...
lemma restr_trans (s : Set X) : (e.restr s).trans e' = (e.trans e').restr s :=
...
lemma eqOnSource_iff (e e' : PartialHomeomorph X Y) :
...
lemma symm_trans_self : e.symm.trans e ≈ PartialHomeomorph.ofSet e.target e.open_target :=
  e.symm.self_trans_symm
#align local_homeomorph.symm_trans_self PartialHomeomorph.symm_trans_self

lemma eq_of_eqOnSource_univ {e e' : PartialHomeomorph X Y} (h : e ≈ e') (s : e.source = univ)
    (t : e.target = univ) : e = e' :=
...
@[simp, mfld_simps]
lemma prod_symm (eX : PartialHomeomorph X X') (eY : PartialHomeomorph Y Y') :
...
@[simp, mfld_simps]
lemma prod_trans (e : PartialHomeomorph X Y) (f : PartialHomeomorph Y Z)
...
lemma prod_eq_prod_of_nonempty {eX eX' : PartialHomeomorph X X'} {eY eY' : PartialHomeomorph Y Y'}
...
@[simp]
lemma symm_piecewise (e e' : PartialHomeomorph X Y) {s : Set X} {t : Set Y}
...
lemma secondCountableTopology_source [SecondCountableTopology Y] :
...
lemma openEmbedding_restrict : OpenEmbedding (e.source.restrict e) := by
...
@[simp, mfld_simps]
lemma refl_toPartialHomeomorph :
...
@[simp, mfld_simps]
lemma symm_toPartialHomeomorph : e.symm.toPartialHomeomorph = e.toPartialHomeomorph.symm :=
  rfl
#align homeomorph.symm_to_local_homeomorph Homeomorph.symm_toPartialHomeomorph

@[simp, mfld_simps]
lemma trans_toPartialHomeomorph :
...
lemma transPartialHomeomorph_eq_trans (e : X ≃ₜ Y) (f' : PartialHomeomorph Y Z) :
...
@[simp, mfld_simps]
lemma transPartialHomeomorph_trans (e : X ≃ₜ Y) (f : PartialHomeomorph Y Z)
...
@[simp, mfld_simps]
lemma trans_transPartialHomeomorph (e : X ≃ₜ Y) (e' : Y ≃ₜ Z) (f'' : PartialHomeomorph Z Z') :
...
@[simp, mfld_simps]
lemma partialHomeomorphSubtypeCoe_coe : (s.partialHomeomorphSubtypeCoe hs : s → X) = (↑) :=
  rfl
#align topological_space.opens.local_homeomorph_subtype_coe_coe TopologicalSpace.Opens.partialHomeomorphSubtypeCoe_coe

@[simp, mfld_simps]
lemma partialHomeomorphSubtypeCoe_source : (s.partialHomeomorphSubtypeCoe hs).source = Set.univ :=
  rfl
...
@[simp, mfld_simps]
lemma partialHomeomorphSubtypeCoe_target : (s.partialHomeomorphSubtypeCoe hs).target = s := by
...
lemma transHomeomorph_eq_trans (e : PartialHomeomorph X Y) (f' : Y ≃ₜ Z) :
...
@[simp, mfld_simps]
lemma transHomeomorph_transHomeomorph (e : PartialHomeomorph X Y) (f' : Y ≃ₜ Z) (f'' : Z ≃ₜ Z') :
...
@[simp, mfld_simps]
lemma trans_transHomeomorph (e : PartialHomeomorph X Y) (e' : PartialHomeomorph Y Z)
    (f'' : Z ≃ₜ Z') :
...
lemma subtypeRestr_def : e.subtypeRestr hs = (s.partialHomeomorphSubtypeCoe hs).trans e :=
  rfl
...
lemma subtypeRestr_symm_eqOn {U : Opens X} (hU : Nonempty U) :
...
lemma subtypeRestr_symm_eqOn_of_le {U V : Opens X} (hU : Nonempty U) (hV : Nonempty V)
