...
protected def Filter.Germ.Continuous {x : X} (f : Germ (𝓝 x) Y) : Prop :=
  RestrictGermPredicate (fun _ f ↦ f.Tendsto (𝓝 f.value)) {x} x f

lemma Continuous.germ {f : X → Y} (hf : Continuous f) (x : X) : (f : Germ (𝓝 x) Y).Continuous :=
  forall_restrictGermPredicate_of_forall (fun _ ↦ hf.continuousAt) x
...
lemma Filter.Germ.continuous_iff_forall {x : X} {f : Germ (𝓝 x) Y} :
  ...
  simp [Germ.Continuous, restrictGermPredicate_iff_forall, ContinuousAt]
...
lemma Filter.Germ.continuous_iff_exists {x : X} {f : Germ (𝓝 x) Y} :
    f.Continuous ↔ ∃ f' : X → Y, f = f' ∧ ∀ᶠ x' in 𝓝 x, ContinuousAt f' x' := by
  simp [Germ.Continuous, restrictGermPredicate_iff_exists, ContinuousAt]
...
lemma Filter.Germ.continuous_coe_iff {x : X} {f : X → Y} :
  ...
  simp [Germ.Continuous, ContinuousAt]
...
lemma Filter.Germ.Continuous.tendsto {x : X} {f : Germ (𝓝 x) Y} (hf : f.Continuous) :
    f.Tendsto (𝓝 f.value) := by
  revert hf
...
def ContinuousGerm.comp {x : X} {y : Y} (f : ContinuousGerm y Z) (g : ContinuousGerm x Y)
  ...
  toGerm := f.toGerm.compTendsto' g.toGerm (h ▸ g.continuous.tendsto)
