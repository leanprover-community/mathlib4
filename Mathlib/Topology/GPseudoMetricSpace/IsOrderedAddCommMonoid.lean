...
## Main Definitions:
- `IsOrderedAddCommMonoid α`: a mixin which says that there exists a `PartialOrder α` and a
`AddCommMonoid α` such that ordering is maintained under adding constants.
- `IsOrderedCancelAddCommMonoid α`: a mixin which says that ordering of `α` is maintained under both
adding and cancelling the adding of constants.
