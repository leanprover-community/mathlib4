...
@[to_additive]
lemma mulTSupport_eq_empty_iff {f : X → α} : mulTSupport f = ∅ ↔ f = 1 := by
...
@[to_additive]
lemma image_eq_one_of_nmem_mulTSupport {f : X → α} {x : X} (hx : x ∉ mulTSupport f) : f x = 1 :=
...
lemma tsupport_mul_subset_left {α : Type*} [MulZeroClass α] {f g : X → α} :
...
lemma tsupport_mul_subset_right {α : Type*} [MulZeroClass α] {f g : X → α} :
...
lemma tsupport_smul_subset_left {M α} [TopologicalSpace X] [Zero M] [Zero α] [SMulWithZero M α]
    (f : X → M) (g : X → α) : (tsupport fun x => f x • g x) ⊆ tsupport f :=
  closure_mono <| support_smul_subset_left f g
#align tsupport_smul_subset_left tsupport_smul_subset_left

lemma tsupport_smul_subset_right {M α} [TopologicalSpace X] [Zero α] [SMulZeroClass M α]
    (f : X → M) (g : X → α) : (tsupport fun x => f x • g x) ⊆ tsupport g :=
  closure_mono <| support_smul_subset_right f g

@[to_additive]
lemma mulTSupport_mul [TopologicalSpace X] [Monoid α] {f g : X → α} :
...
@[to_additive]
lemma not_mem_mulTSupport_iff_eventuallyEq : x ∉ mulTSupport f ↔ f =ᶠ[𝓝 x] 1 := by
...
@[to_additive]
lemma continuous_of_mulTSupport [TopologicalSpace β] {f : α → β}
...
@[to_additive]
lemma hasCompactMulSupport_def : HasCompactMulSupport f ↔ IsCompact (closure (mulSupport f)) := by
  rfl
#align has_compact_mul_support_def hasCompactMulSupport_def
#align has_compact_support_def hasCompactSupport_def

@[to_additive]
lemma exists_compact_iff_hasCompactMulSupport [R1Space α] :
...
@[to_additive]
lemma intro [R1Space α] {K : Set α} (hK : IsCompact K)
...
@[to_additive]
lemma intro' {K : Set α} (hK : IsCompact K) (h'K : IsClosed K)
...
@[to_additive]
lemma of_mulSupport_subset_isCompact [R1Space α] {K : Set α}
    (hK : IsCompact K) (h : mulSupport f ⊆ K) : HasCompactMulSupport f :=
  hK.closure_of_subset h

@[to_additive]
lemma isCompact (hf : HasCompactMulSupport f) : IsCompact (mulTSupport f) :=
  hf
...
@[to_additive]
lemma _root_.hasCompactMulSupport_iff_eventuallyEq :
...
@[to_additive]
lemma _root_.isCompact_range_of_mulSupport_subset_isCompact [TopologicalSpace β]
...
@[to_additive]
lemma isCompact_range [TopologicalSpace β] (h : HasCompactMulSupport f)
...
@[to_additive]
lemma mono' {f' : α → γ} (hf : HasCompactMulSupport f)
...
@[to_additive]
lemma comp_left (hf : HasCompactMulSupport f) (hg : g 1 = 1) :
...
@[to_additive]
lemma _root_.hasCompactMulSupport_comp_left (hg : ∀ {x}, g x = 1 ↔ x = 1) :
...
@[to_additive]
lemma comp_closedEmbedding (hf : HasCompactMulSupport f) {g : α' → α}
...
@[to_additive]
lemma comp₂_left (hf : HasCompactMulSupport f)
...
@[to_additive]
lemma mulTSupport_extend_one_subset :
...
@[to_additive]
lemma extend_one : HasCompactMulSupport (g.extend f 1) :=
...
@[to_additive]
lemma mulTSupport_extend_one (inj : g.Injective) :
    mulTSupport (g.extend f 1) = g '' mulTSupport f :=
...
@[to_additive]
lemma continuous_extend_one [TopologicalSpace β] {U : Set α'} (hU : IsOpen U) {f : U → β}
...
@[to_additive]
lemma locallyFinite_mulSupport_iff [CommMonoid M] {f : ι → X → M} :
...
lemma LocallyFinite.smul_left [Zero R] [Zero M] [SMulWithZero R M]
...
lemma LocallyFinite.smul_right [Zero M] [SMulZeroClass R M]
