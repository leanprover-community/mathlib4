...
variable (cov : IsCoveringMap (proj P))

/-- If the projection from the étalé space is a covering map, it induces a monodromy functor. -/
@[simps] noncomputable def monodromy : FundamentalGroupoid B ⥤ Type _ where
  obj b := F b.as
  map f := Equiv.preimageSigmaFst ∘ cov.monodromyFunctor.map f ∘ Equiv.preimageSigmaFst.symm

theorem monodromy_eq_of_section {x y : B} (γ : Path x y)
    {U : Opens B} {s : Π b : U, F b} (hs : Sheafify P s) (hγU : ∀ t, γ t ∈ U) :
    (monodromy cov).map ⟦γ⟧ (s ⟨x, γ.source ▸ hγU 0⟩) = s ⟨y, γ.target ▸ hγU 1⟩ := by
  let γU : Path (⟨x, γ.source ▸ hγU 0⟩ : U) (⟨y, γ.target ▸ hγU 1⟩ : U) :=
    { toFun t := ⟨γ t, hγU t⟩, source' := Subtype.ext γ.source, target' := Subtype.ext γ.target }
  simp_rw [monodromy_map, Function.comp_apply]
  exact congr_arg _ (cov.monodromy_mapFn ⟦γU.map (continuous_sheafify_section hs)⟧)
