...
theorem lowerSemicontinuousOn_iff_isClosed_epigraph {f : α → γ} {s : Set α} (hs : IsClosed s) :
  ...
  simp_rw [lowerSemicontinuousOn_iff, lowerSemicontinuousWithinAt_iff, eventually_nhdsWithin_iff,
    ← isOpen_compl_iff, compl_ofPred, isOpen_iff_eventually, mem_ofPred, not_and, not_le]
  constructor
  · intro hf ⟨x, y⟩ h
    by_cases hx : x ∈ s
