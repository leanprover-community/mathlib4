...
theorem Filter.HasBasis.cauchy_iff {ι} {p : ι → Prop} {s : ι → Rel α α} (h : (𝓤 α).HasBasis p s)
    {f : Filter α} :
...
theorem le_nhds_of_cauchy_adhp_aux {f : Filter α} {x : α}
  ...
  exact fun z hz => hU (Rel.prodMk_mem_comp hxy (ht <| mk_mem_prod hy hz)) rfl
...
theorem CauchySeq.mem_entourage {β : Type*} [SemilatticeSup β] {u : β → α} (h : CauchySeq u)
    {V : Rel α α} (hV : V ∈ 𝓤 α) : ∃ k₀, ∀ i j, k₀ ≤ i → k₀ ≤ j → (u i, u j) ∈ V := by
  haveI := h.nonempty
...
theorem CauchySeq.subseq_subseq_mem {V : ℕ → Rel α α} (hV : ∀ n, V n ∈ 𝓤 α) {u : ℕ → α}
...
theorem CauchySeq.eventually_eventually [Preorder β] {u : β → α} (hu : CauchySeq u)
    {V : Rel α α} (hV : V ∈ 𝓤 α) : ∀ᶠ k in atTop, ∀ᶠ l in atTop, (u k, u l) ∈ V :=
...
theorem CauchySeq.subseq_mem {V : ℕ → Rel α α} (hV : ∀ n, V n ∈ 𝓤 α) {u : ℕ → α}
...
theorem Filter.Tendsto.subseq_mem_entourage {V : ℕ → Rel α α} (hV : ∀ n, V n ∈ 𝓤 α) {u : ℕ → α}
...
theorem Filter.HasBasis.cauchySeq_iff {γ} [Nonempty β] [SemilatticeSup β] {u : β → α} {p : γ → Prop}
    {s : γ → Rel α α} (h : (𝓤 α).HasBasis p s) :
...
theorem Filter.HasBasis.cauchySeq_iff' {γ} [Nonempty β] [SemilatticeSup β] {u : β → α}
    {p : γ → Prop} {s : γ → Rel α α} (H : (𝓤 α).HasBasis p s) :
...
theorem cauchySeq_of_controlled [SemilatticeSup β] [Nonempty β] (U : β → Rel α α)
...
theorem isComplete_iUnion_separated {ι : Sort*} {s : ι → Set α} (hs : ∀ i, IsComplete (s i))
    {U : Rel α α} (hU : U ∈ 𝓤 α) (hd : ∀ (i j : ι), ∀ x ∈ s i, ∀ y ∈ s j, (x, y) ∈ U → i = j) :
    IsComplete (⋃ i, s i) := by
  set S := ⋃ i, s i
  intro l hl hls
...
theorem eq_pure_of_cauchy {f : Filter α} (hf : Cauchy f) : ∃ x : α, f = pure x := by
  ...
  obtain ⟨x, rfl, _, _, _⟩ := Rel.exists_eq_singleton_of_prod_subset_id
...
theorem TotallyBounded.exists_subset {s : Set α} (hs : TotallyBounded s) {U : Rel α α}
...
theorem Filter.HasBasis.totallyBounded_iff {ι} {p : ι → Prop} {U : ι → Rel α α}
...
theorem totallyBounded_of_forall_isSymm {s : Set α}
    (h : ∀ V ∈ 𝓤 α, Rel.IsSymm V → ∃ t : Set α, Set.Finite t ∧ s ⊆ ⋃ y ∈ t, ball y V) :
    TotallyBounded s :=
...
def interUnionBalls (xs : ℕ → α) (u : ℕ → ℕ) (V : ℕ → Rel α α) : Set α :=
  ⋂ n, ⋃ m ≤ u n, UniformSpace.ball (xs m) (Prod.swap ⁻¹' V n)

lemma totallyBounded_interUnionBalls {p : ℕ → Prop} {U : ℕ → Rel α α}
...
theorem isCompact_closure_interUnionBalls {p : ℕ → Prop} {U : ℕ → Rel α α}
...
variable {f : Filter α} (hf : Cauchy f) {U : ℕ → Rel α α} (U_mem : ∀ n, U n ∈ 𝓤 α)
...
theorem complete_of_convergent_controlled_sequences (U : ℕ → Rel α α) (U_mem : ∀ n, U n ∈ 𝓤 α)
...
theorem secondCountable_of_separable [SeparableSpace α] : SecondCountableTopology α := by
  ...
  obtain
    ⟨t : ℕ → Rel α α, hto : ∀ i : ℕ, t i ∈ (𝓤 α).sets ∧ IsOpen (t i) ∧ (t i).IsSymm,
