...
instance (f : CauchyFilter α) : NeBot f.1 := fast_instance% f.2.1
...
instance : UniformSpace (CauchyFilter α) := fast_instance%
  UniformSpace.ofCore
...
instance : CompleteSpace (CauchyFilter α) := fast_instance%
...
instance [Inhabited α] : Inhabited (CauchyFilter α) := fast_instance%
  ⟨pureCauchy default⟩

instance [h : Nonempty α] : Nonempty (CauchyFilter α) := fast_instance%
...
instance inhabited [Inhabited α] : Inhabited (Completion α) := fast_instance%
  inferInstanceAs <| Inhabited (Quotient _)

instance uniformSpace : UniformSpace (Completion α) := fast_instance%
  SeparationQuotient.instUniformSpace

instance completeSpace : CompleteSpace (Completion α) := fast_instance%
  SeparationQuotient.instCompleteSpace

instance t0Space : T0Space (Completion α) := fast_instance% SeparationQuotient.instT0Space
...
instance : Coe α (Completion α) := fast_instance%
  ⟨coe' α⟩
...
instance AbstractCompletion.inhabited : Inhabited (AbstractCompletion α) := fast_instance%
  ⟨cPkg⟩
...
instance separableSpace_completion [SeparableSpace α] : SeparableSpace (Completion α) := fast_instance%
