...
theorem lebesgue_number_lemma {ι : Sort*} {U : ι → Set α} (hK : IsCompact K)
    ...
    ∃ V ∈ 𝓤 α, ∀ x ∈ K, ∃ i, SetRel.ball V x ⊆ U i := by
  have : ∀ x ∈ K, ∃ i, ∃ V ∈ 𝓤 α, (V ○ V).ball x ⊆ U i := fun x hx ↦ by
  ...
  rcases hK.elim_nhds_subcover' (fun x hx ↦ SetRel.ball (W x hx) x)
    (fun x hx ↦ SetRel.ball_mem_nhds _ (hW x hx)) with ⟨t, ht⟩
  refine ⟨⋂ x ∈ t, W x x.2, (biInter_finset_mem _).2 fun x _ ↦ hW x x.2, fun x hx ↦ ?_⟩
  rcases mem_iUnion₂.1 (ht hx) with ⟨y, hyt, hxy⟩
  exact ⟨ind y y.2, fun z hz ↦ hWU _ _ ⟨x, mem_iInter₂.1 hz _ hyt, hxy⟩⟩

theorem lebesgue_number_lemma_nhds' {U : (x : α) → x ∈ K → Set α} (hK : IsCompact K)
    (hU : ∀ x hx, U x hx ∈ 𝓝 x) : ∃ V ∈ 𝓤 α, ∀ x ∈ K, ∃ y : K, SetRel.ball V x ⊆ U y y.2 := by
...
theorem lebesgue_number_lemma_nhds {U : α → Set α} (hK : IsCompact K) (hU : ∀ x ∈ K, U x ∈ 𝓝 x) :
    ∃ V ∈ 𝓤 α, ∀ x ∈ K, ∃ y, SetRel.ball V x ⊆ U y := by
...
theorem lebesgue_number_lemma_nhdsWithin' {U : (x : α) → x ∈ K → Set α} (hK : IsCompact K)
    (hU : ∀ x hx, U x hx ∈ 𝓝[K] x) : ∃ V ∈ 𝓤 α, ∀ x ∈ K, ∃ y : K, SetRel.ball V x ∩ K ⊆ U y y.2 :=
...
theorem lebesgue_number_lemma_nhdsWithin {U : α → Set α} (hK : IsCompact K)
    (hU : ∀ x ∈ K, U x ∈ 𝓝[K] x) : ∃ V ∈ 𝓤 α, ∀ x ∈ K, ∃ y, SetRel.ball V x ∩ K ⊆ U y :=
  (lebesgue_number_lemma_nhds hK (fun x hx => Filter.mem_inf_principal'.1 (hU x hx))).imp
    fun _ ⟨V_uni, hV⟩ => ⟨V_uni, fun x hx => (hV x hx).imp fun _ hy => (inter_subset _ _ _).2 hy⟩
...
protected theorem Filter.HasBasis.lebesgue_number_lemma {ι' ι : Sort*} {p : ι' → Prop}
    {V : ι' → SetRel α α} {U : ι → Set α} (hbasis : (𝓤 α).HasBasis p V) (hK : IsCompact K)
    (hopen : ∀ j, IsOpen (U j)) (hcover : K ⊆ ⋃ j, U j) :
    ∃ i, p i ∧ ∀ x ∈ K, ∃ j, (V i).ball x ⊆ U j := by
  refine (hbasis.exists_iff ?_).1 (lebesgue_number_lemma hK hopen hcover)
  exact fun s t hst ht x hx ↦ (ht x hx).imp fun i hi ↦ Subset.trans (by gcongr) hi

protected theorem Filter.HasBasis.lebesgue_number_lemma_nhds' {ι' : Sort*} {p : ι' → Prop}
    {V : ι' → SetRel α α} {U : (x : α) → x ∈ K → Set α} (hbasis : (𝓤 α).HasBasis p V)
    (hK : IsCompact K) (hU : ∀ x hx, U x hx ∈ 𝓝 x) :
    ∃ i, p i ∧ ∀ x ∈ K, ∃ y : K, (V i).ball x ⊆ U y y.2 := by
  refine (hbasis.exists_iff ?_).1 (lebesgue_number_lemma_nhds' hK hU)
  exact fun s t hst ht x hx ↦ (ht x hx).imp fun y hy ↦ Subset.trans (by gcongr) hy

protected theorem Filter.HasBasis.lebesgue_number_lemma_nhds {ι' : Sort*} {p : ι' → Prop}
    {V : ι' → SetRel α α} {U : α → Set α} (hbasis : (𝓤 α).HasBasis p V) (hK : IsCompact K)
    (hU : ∀ x ∈ K, U x ∈ 𝓝 x) : ∃ i, p i ∧ ∀ x ∈ K, ∃ y, (V i).ball x ⊆ U y := by
  refine (hbasis.exists_iff ?_).1 (lebesgue_number_lemma_nhds hK hU)
  exact fun s t hst ht x hx ↦ (ht x hx).imp fun y hy ↦ Subset.trans (by gcongr) hy
...
protected theorem Filter.HasBasis.lebesgue_number_lemma_nhdsWithin' {ι' : Sort*} {p : ι' → Prop}
    ...
    ∃ i, p i ∧ ∀ x ∈ K, ∃ y : K, (V i).ball x ∩ K ⊆ U y y.2 := by
  refine (hbasis.exists_iff ?_).1 (lebesgue_number_lemma_nhdsWithin' hK hU)
  exact fun s t hst ht x hx ↦ (ht x hx).imp fun y hy ↦ Subset.trans (by gcongr) hy

protected theorem Filter.HasBasis.lebesgue_number_lemma_nhdsWithin {ι' : Sort*} {p : ι' → Prop}
    {V : ι' → SetRel α α} {U : α → Set α} (hbasis : (𝓤 α).HasBasis p V) (hK : IsCompact K)
    (hU : ∀ x ∈ K, U x ∈ 𝓝[K] x) : ∃ i, p i ∧ ∀ x ∈ K, ∃ y, (V i).ball x ∩ K ⊆ U y := by
  refine (hbasis.exists_iff ?_).1 (lebesgue_number_lemma_nhdsWithin hK hU)
  exact fun s t hst ht x hx ↦ (ht x hx).imp fun y hy ↦ Subset.trans (by gcongr) hy
...
theorem lebesgue_number_lemma_sUnion {S : Set (Set α)}
    ...
    ∃ V ∈ 𝓤 α, ∀ x ∈ K, ∃ s ∈ S, SetRel.ball V x ⊆ s := by
...
then `{⋃ x ∈ K, (V i).ball x | p i}` is a basis of `𝓝ˢ K`.

Here "`{s i | p i}` is a basis of a filter `l`" means `Filter.HasBasis l p s`. -/
theorem IsCompact.nhdsSet_basis_uniformity {p : ι → Prop} {V : ι → SetRel α α}
    (hbasis : (𝓤 α).HasBasis p V) (hK : IsCompact K) :
    (𝓝ˢ K).HasBasis p fun i => ⋃ x ∈ K, (V i).ball x where
  mem_iff' U := by
    constructor
    · intro H
      have HKU : K ⊆ ⋃ _ : Unit, interior U := by
        simpa only [iUnion_const, subset_interior_iff_mem_nhdsSet] using H
      obtain ⟨i, hpi, hi⟩ : ∃ i, p i ∧ ⋃ x ∈ K, (V i).ball x ⊆ interior U := by
    ...
    · rintro ⟨i, hpi, hi⟩
      ...
      exact SetRel.ball_mem_nhds _ <| hbasis.mem_of_mem hpi
...
theorem Disjoint.exists_uniform_thickening {A B : Set α} (hA : IsCompact A) (hB : IsClosed B)
    (h : Disjoint A B) :
    ∃ V ∈ 𝓤 α, Disjoint (⋃ x ∈ A, SetRel.ball V x) (⋃ x ∈ B, SetRel.ball V x) := by
  ...
  rw [SetRel.mem_ball] at hxa hxb
  exact hUAB (mem_iUnion₂_of_mem ha <| hVU <| SetRel.prodMk_mem_comp (SetRel.symm V hxb) hxa) hb

theorem Disjoint.exists_uniform_thickening_of_basis {p : ι → Prop} {s : ι → SetRel α α}
    (hU : (𝓤 α).HasBasis p s) {A B : Set α} (hA : IsCompact A) (hB : IsClosed B)
    (h : Disjoint A B) : ∃ i, p i ∧ Disjoint (⋃ x ∈ A, (s i).ball x) (⋃ x ∈ B, (s i).ball x) := by
  rcases h.exists_uniform_thickening hA hB with ⟨V, hV, hVAB⟩
  rcases hU.mem_iff.1 hV with ⟨i, hi, hiV⟩
  exact ⟨i, hi, hVAB.mono (by gcongr) (by gcongr)⟩
...
theorem lebesgue_number_of_compact_open {K U : Set α} (hK : IsCompact K)
    (hU : IsOpen U) (hKU : K ⊆ U) : ∃ V ∈ 𝓤 α, IsOpen V ∧ ∀ x ∈ K, SetRel.ball V x ⊆ U :=
  let ⟨V, ⟨hV, hVo⟩, hVU⟩ :=
...
theorem nhdsSet_diagonal_eq_uniformity [CompactSpace α] : 𝓝ˢ (diagonal α) = 𝓤 α := by
  ...
  exact mem_of_superset hU fun ⟨x, y⟩ hxy => mem_iUnion₂.2
    ⟨(y, y), rfl, hxy, refl_mem_uniformity hU⟩
