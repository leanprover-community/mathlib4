...
lemma IsTransitiveRel.mem_filter_prod_comm {s : SetRel X X} {f g h : Filter X} [g.NeBot] [s.IsTrans]
    (hfg : s ∈ f ×ˢ g) (hgh : s ∈ g ×ˢ h) :
    s ∈ f ×ˢ h := by
  ...
  refine ⟨_, ht, _, hw, prod_subset_trans htu hvw <| g.nonempty_of_mem ?_⟩
  simp [hu, hv]
