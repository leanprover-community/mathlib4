...
instance : CompleteLattice (UniformSpace α) where
  ...
  isLUB_sSup_of_exists_isLUB _ _ := isGLB_upperBounds.mp UniformSpace.isGLB_sInf
  isGLB_sInf_of_exists_isGLB _ _ := UniformSpace.isGLB_sInf
  exists_isLUB _ := ⟨_, isGLB_upperBounds.mp UniformSpace.isGLB_sInf⟩
  exists_isGLB _ := ⟨_, UniformSpace.isGLB_sInf⟩
