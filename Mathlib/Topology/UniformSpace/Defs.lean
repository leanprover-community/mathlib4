...
/-- Relation `fun f g ↦ Tendsto (Function.prod f g) l (𝓤 α)` is transitive. -/
theorem Filter.Tendsto.uniformity_trans {l : Filter β} {f₁ f₂ f₃ : β → α}
    (h₁₂ : Tendsto (Function.prod f₁ f₂) l (𝓤 α))
    (h₂₃ : Tendsto (Function.prod f₂ f₃) l (𝓤 α)) : Tendsto (Function.prod f₁ f₃) l (𝓤 α) := by
...
/-- Relation `fun f g ↦ Tendsto (Function.prod f g) l (𝓤 α)` is symmetric. -/
theorem Filter.Tendsto.uniformity_symm {l : Filter β} {f : β → α × α} (h : Tendsto f l (𝓤 α)) :
    Tendsto (fun x => ((f x).2, (f x).1)) l (𝓤 α) :=
  tendsto_swap_uniformity.comp h

/-- Relation `fun f g ↦ Tendsto (Function.prod f g) l (𝓤 α)` is reflexive. -/
theorem tendsto_diag_uniformity (f : β → α) (l : Filter β) :
    Tendsto (Function.prod f f) l (𝓤 α) := fun _s hs =>
