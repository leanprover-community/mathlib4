...
This file defines a notion of separation of a set relative to an entourage.

For an entourage `U`, a `U`-separated set `s` is a set such that every pair of elements of `s` is
`U`-far.
