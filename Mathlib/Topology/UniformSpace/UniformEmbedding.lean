...
theorem comap_uniformity_of_spaced_out {α} {f : α → β} {s : Set (β × β)} (hs : s ∈ 𝓤 β)
    (hf : Pairwise fun x y => (f x, f y) ∉ s) : comap (Prod.map f f) (𝓤 β) = 𝓟 Rel.id := by
...
theorem closure_image_mem_nhds_of_isUniformInducing {s : Set (α × α)} {e : α → β} (b : β)
  ...
  obtain ⟨U, ⟨hU, hUo, hsymm⟩, hs⟩ :
    ∃ U, (U ∈ 𝓤 β ∧ IsOpen U ∧ Rel.IsSymm U) ∧ Prod.map e e ⁻¹' U ⊆ s := by
    ...
    have : Cauchy g :=
      ⟨‹NeBot g›, fun _ hs =>
        ...
        (g ×ˢ g).sets_of_superset hg fun ⟨_, _⟩ ⟨⟨c₁, c₁t, hc₁⟩, ⟨c₂, c₂t, hc₂⟩⟩ =>
          ...
          comp_s₁ <| Rel.prodMk_mem_comp hc₁ <| comp_s₂ <| Rel.prodMk_mem_comp (prod_t this) hc₂⟩
