...
@[continuity, fun_prop]
theorem Continuous.eval_const (hf : Continuous f) (x : α) : Continuous (f · x) :=
...
theorem Filter.Tendsto.eval_const {ι : Type*} {l : Filter ι} {f : ι → F} {g : F}
    (hf : Tendsto f l (𝓝 g)) (a : α) : Tendsto (f · a) l (𝓝 (g a)) :=
  ((continuous_id.eval_const a).tendsto _).comp hf

theorem Filter.Tendsto.coeFun {ι : Type*} {l : Filter ι} {f : ι → F} {g : F}
    (hf : Tendsto f l (𝓝 g)) : Tendsto (fun i ↦ ⇑(f i)) l (𝓝 ⇑g) :=
  (continuous_id.coeFun.tendsto _).comp hf

nonrec theorem ContinuousAt.eval_const (hf : ContinuousAt f z) (x : α) : ContinuousAt (f · x) z :=
  hf.eval_const x

nonrec theorem ContinuousAt.coeFun (hf : ContinuousAt f z) : ContinuousAt (fun z ↦ ⇑(f z)) z :=
  hf.coeFun

nonrec theorem ContinuousWithinAt.eval_const (hf : ContinuousWithinAt f s z) (x : α) :
...
theorem ContinuousOn.eval_const (hf : ContinuousOn f s) (x : α) : ContinuousOn (f · x) s :=
  fun z hz ↦ (hf z hz).eval_const x

theorem ContinuousOn.coeFun (hf : ContinuousOn f s) (x : α) : ContinuousOn (f · x) s :=
  fun z hz ↦ (hf z hz).eval_const x
