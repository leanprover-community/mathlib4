...
instance AlexandrovDiscrete.locPathConnectedSpace [AlexandrovDiscrete X] :
  ...
  apply LocPathConnectedSpace.of_bases nhds_basis_exterior_singleton
  simp only [forall_const, IsPathConnected, mem_exterior_singleton]
  intro x
  exists x, specializes_rfl
  intro y hy
  symm
  apply hy.joinedIn <;> rewrite [mem_exterior_singleton] <;> [assumption; rfl]
