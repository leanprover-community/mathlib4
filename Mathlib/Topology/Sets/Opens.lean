...
@[simp] lemma mk_empty : (⟨∅, isOpen_empty⟩ : Opens α) = ⊥ := rfl
...
@[simp] lemma mk_univ : (⟨univ, isOpen_univ⟩ : Opens α) = ⊤ := rfl
...
lemma openEmbedding' (U : Opens α) : OpenEmbedding (Subtype.val : U → α) :=
