...
instance (priority := 10000) : FunLike (α →ᵇ β) α β where
  coe f := f.toFun
...
instance (priority := 10000) : BoundedContinuousMapClass (α →ᵇ β) α β where
  map_continuous f := f.continuous_toFun
  map_bounded f := f.map_bounded'

instance (priority := 10000) [FunLike F α β] [BoundedContinuousMapClass F α β] : CoeTC F (α →ᵇ β) :=
  ⟨fun f =>
    { toFun := f
...
instance (priority := 10000) : Dist (α →ᵇ β) :=
...
instance (priority := 10000) : PseudoMetricSpace (α →ᵇ β) where
...
instance (priority := 10000) {α β} [TopologicalSpace α] [MetricSpace β] : MetricSpace (α →ᵇ β) where
...
instance (priority := 10000) [Inhabited β] : Inhabited (α →ᵇ β) :=
  ⟨const α default⟩
...
instance (priority := 10000) [CompleteSpace β] : CompleteSpace (α →ᵇ β) :=
...
@[to_additive]
instance (priority := 10000) : One (α →ᵇ β) :=
  ⟨const α 1⟩
...
instance (priority := 10000) : Add (α →ᵇ β) where
  add f g :=
...
instance (priority := 10000) hasNatScalar : SMul ℕ (α →ᵇ β) where
  smul n f :=
...
instance (priority := 10000) addMonoid : AddMonoid (α →ᵇ β) :=
  DFunLike.coe_injective.addMonoid _ coe_zero coe_add fun _ _ => coe_nsmul _ _

instance (priority := 10000) : LipschitzAdd (α →ᵇ β) where
  lipschitz_add :=
    ⟨LipschitzAdd.C β, by
...
@[to_additive]
instance (priority := 10000) : AddCommMonoid (α →ᵇ β) :=
...
instance (priority := 10000) : Norm (α →ᵇ β) :=
  ⟨fun u => dist u 0⟩
...
instance (priority := 10000) hasIntScalar : SMul ℤ (α →ᵇ β) where
  smul n f :=
    { toContinuousMap := n • f.toContinuousMap
...
instance (priority := 10000) : AddCommGroup (α →ᵇ β) :=
  DFunLike.coe_injective.addCommGroup _ coe_zero coe_add coe_neg coe_sub (fun _ _ => coe_nsmul _ _)
    fun _ _ => coe_zsmul _ _

instance (priority := 10000) seminormedAddCommGroup : SeminormedAddCommGroup (α →ᵇ β) where
  dist_eq f g := by simp only [norm_eq, dist_eq, dist_eq_norm, sub_apply]

instance (priority := 10000) normedAddCommGroup {α β} [TopologicalSpace α] [NormedAddCommGroup β] :
...
instance (priority := 10000) : SMul 𝕜 (α →ᵇ β) where
  smul c f :=
...
instance (priority := 10000) [SMul 𝕜ᵐᵒᵖ β] [IsCentralScalar 𝕜 β] : IsCentralScalar 𝕜 (α →ᵇ β) where
  op_smul_eq_smul _ _ := ext fun _ => op_smul_eq_smul _ _

instance (priority := 10000) : BoundedSMul 𝕜 (α →ᵇ β) where
...
instance (priority := 10000) : MulAction 𝕜 (α →ᵇ β) :=
...
instance (priority := 10000) : DistribMulAction 𝕜 (α →ᵇ β) :=
...
instance (priority := 10000) module : Module 𝕜 (α →ᵇ β) :=
...
instance (priority := 10000) normedSpace [NormedField 𝕜] [NormedSpace 𝕜 β] : NormedSpace 𝕜 (α →ᵇ β) :=
  ⟨fun c f => by
...
instance (priority := 10000) : Mul (α →ᵇ R) where
  mul f g :=
...
instance (priority := 10000) : NonUnitalRing (α →ᵇ R) :=
...
instance (priority := 10000) nonUnitalSeminormedRing : NonUnitalSeminormedRing (α →ᵇ R) :=
  { BoundedContinuousFunction.seminormedAddCommGroup with
    norm_mul := fun _ _ =>
...
instance (priority := 10000) nonUnitalNormedRing [NonUnitalNormedRing R] : NonUnitalNormedRing (α →ᵇ R) :=
...
instance (priority := 10000) hasNatPow : Pow (α →ᵇ R) ℕ where
  pow f n :=
...
instance (priority := 10000) : NatCast (α →ᵇ R) :=
...
instance (priority := 10000) : IntCast (α →ᵇ R) :=
  ⟨fun n => BoundedContinuousFunction.const _ n⟩
...
instance (priority := 10000) ring : Ring (α →ᵇ R) :=
...
instance (priority := 10000) : SeminormedRing (α →ᵇ R) :=
...
instance (priority := 10000) [NormedRing R] : NormedRing (α →ᵇ R) :=
  { show Ring (α →ᵇ R) from inferInstance,  -- porting note: this was not present in the original
...
instance (priority := 10000) commRing [SeminormedCommRing R] : CommRing (α →ᵇ R) :=
  { BoundedContinuousFunction.ring with
    mul_comm := fun _ _ => ext fun _ => mul_comm _ _ }

instance (priority := 10000) [SeminormedCommRing R] : SeminormedCommRing (α →ᵇ R) :=
...
instance (priority := 10000) [NormedCommRing R] : NormedCommRing (α →ᵇ R) :=
...
instance (priority := 10000) algebra : Algebra 𝕜 (α →ᵇ γ) :=
...
instance (priority := 10000) : NormedAlgebra 𝕜 (α →ᵇ γ) :=
...
instance (priority := 10000) hasSMul' : SMul (α →ᵇ 𝕜) (α →ᵇ β) where
  smul f g :=
...
instance (priority := 10000) module' : Module (α →ᵇ 𝕜) (α →ᵇ β) :=
...
instance (priority := 10000) : BoundedSMul (α →ᵇ 𝕜) (α →ᵇ β) :=
...
instance (priority := 10000) starAddMonoid : StarAddMonoid (α →ᵇ β) where
...
instance (priority := 10000) `Pi.instStarForAll`. Upon inspecting the goal, one sees `⊢ ↑(star f) = star ↑f`. -/
@[simp]
...
instance (priority := 10000) : NormedStarGroup (α →ᵇ β) where
  norm_star f := by simp only [norm_eq, star_apply, norm_star]

instance (priority := 10000) : StarModule 𝕜 (α →ᵇ β) where
...
instance (priority := 10000) partialOrder : PartialOrder (α →ᵇ β) :=
...
instance (priority := 10000) semilatticeInf : SemilatticeInf (α →ᵇ β) :=
...
instance (priority := 10000) semilatticeSup : SemilatticeSup (α →ᵇ β) :=
  { BoundedContinuousFunction.partialOrder with
    sup := fun f g =>
      { toFun := fun t => f t ⊔ g t
...
instance (priority := 10000) lattice : Lattice (α →ᵇ β) :=
...
instance (priority := 10000) : NormedLatticeAddCommGroup (α →ᵇ β) :=
