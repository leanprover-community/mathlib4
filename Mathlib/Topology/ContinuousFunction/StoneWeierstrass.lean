...
variable {𝕜 : Type*} {X : Type*} [SMul ℝ 𝕜] [RCLike 𝕜] [TopologicalSpace X]
...
theorem polynomialFunctions.starClosure_topologicalClosure {𝕜 : Type*} [SMul ℝ 𝕜] [RCLike 𝕜] (s : Set 𝕜)
...
theorem ContinuousMap.algHom_ext_map_X {A : Type*} [Ring A]
    [SMul ℝ A] [Algebra ℝ A] [TopologicalSpace A] [T2Space A] {s : Set ℝ} [CompactSpace s]
...
at `X : 𝕜[X]` (interpreted as a continuous map) are, in fact, equal. -/
@[ext]
theorem ContinuousMap.starAlgHom_ext_map_X {𝕜 A : Type*} [SMul ℝ 𝕜] [RCLike 𝕜] [Ring A] [StarRing A]
    [SMul 𝕜 A] [Algebra 𝕜 A] [TopologicalSpace A] [T2Space A] {s : Set 𝕜} [CompactSpace s]
