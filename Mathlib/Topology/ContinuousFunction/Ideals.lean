...
When `R = 𝕜` with `ROrCLike 𝕜` and `X` is compact Hausdorff, then this Galois connection can be
...
compact Hausdorff and `ROrCLike 𝕜`, more can be obtained. In particular, in that context this map is
...
* `WeakDual.CharacterSpace.homeoEval`: this is `WeakDual.CharacterSpace.continuousMapEval`
  upgraded to a homeomorphism when `X` is compact Hausdorff and `ROrCLike 𝕜`.

## Main statements

* `ContinuousMap.idealOfSet_ofIdeal_eq_closure`: when `X` is compact Hausdorff and
  `ROrCLike 𝕜`, `idealOfSet 𝕜 (setOfIdeal I) = I.closure` for any ideal `I : Ideal C(X, 𝕜)`.
* `ContinuousMap.setOfIdeal_ofSet_eq_interior`: when `X` is compact Hausdorff and `ROrCLike 𝕜`,
  `setOfIdeal (idealOfSet 𝕜 s) = interior s` for any `s : Set X`.
* `ContinuousMap.ideal_isMaximal_iff`: when `X` is compact Hausdorff and `ROrCLike 𝕜`, a closed
...
variable {X 𝕜 : Type*} [ROrCLike 𝕜] [TopologicalSpace X]
...
theorem idealOfSet_ofIdeal_eq_closure (I : Ideal C(X, 𝕜)) :
  ...
  have : ∃ g' : C(X, ℝ≥0), (algebraMapCLM ℝ≥0 𝕜 : C(ℝ≥0, 𝕜)).comp g' ∈ I ∧ ∀ x ∈ t, 0 < g' x := by
    ...
    · intro x hx
      ...
      simp only [normSq_eq_def', ROrCLike.conj_mul, ofReal_pow]
      rfl
...
theorem setOfIdeal_ofSet_eq_interior (s : Set X) : setOfIdeal (idealOfSet 𝕜 s) = interior s := by
  ...
  exact
    ⟨⟨fun x => g x, continuous_ofReal.comp (map_continuous g)⟩, by
      ...
      simpa only [coe_mk, hgx (Set.mem_singleton x), Pi.one_apply, ROrCLike.ofReal_one] using
        one_ne_zero⟩
...
variable [CompactSpace X] [T2Space X] [ROrCLike 𝕜]
...
theorem continuousMapEval_bijective : Bijective (continuousMapEval X 𝕜) := by
  ...
  · contrapose! hxy
    rcases exists_continuous_zero_one_of_isClosed (isClosed_singleton : _root_.IsClosed {x})
      ...
      ⟨f, fx, fy, -⟩
    rw [DFunLike.ne_iff]
    use (⟨fun (x : ℝ) => (x : 𝕜), ROrCLike.continuous_ofReal⟩ : C(ℝ, 𝕜)).comp f
    simpa only [continuousMapEval_apply_apply, ContinuousMap.comp_apply, coe_mk, Ne.def,
      ROrCLike.ofReal_inj] using
