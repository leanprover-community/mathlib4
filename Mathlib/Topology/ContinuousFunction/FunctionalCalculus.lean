...
class FunctionalCalculus (H : Type _) {F A : Type _} [FunLike H F (fun _ ↦ A)]
    (f : F) (a : A) where
  toHom : H
  map_point' : toHom f = a
...
class FunctionalCalculusComp (H₁ H₂ H₃ : Type _) {F₁ F₂ F₃ A : Type}
    ...
    [FunctionalCalculus H₂ f₂ (fc H₁ f₁ a f)]
    [FunctionalCalculus H₃ f₃ b] where
  fc_comp' : ∀ g : F₂, fc H₂ f₂ (fc H₁ f₁ a f) g = fc H₃ f₃ b (cmp g)

-- applied to `f₂`, this means
-- `fc H₁ f₁ a = fc H₂ f₂ (fc H₁ f₁ a f) f₂ = fc H₃ f₃ b (cmp f₂)`
...
lemma fc_polynomial_def' {R A : Type _} [CommSemiring R] [Semiring A] [Algebra R A] {a : A}
    (p : R[X]) : fc (R[X] →ₐ[R] A) (aeval (R := R) p X) (fc (R[X] →ₐ[R] A) X a p) = aeval a :=
  rfl

noncomputable instance {R A : Type _} [CommSemiring R] [Semiring A] [Algebra R A] {a : A}
    {p : R[X]} :
