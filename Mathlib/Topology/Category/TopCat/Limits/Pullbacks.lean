...
def pullbackCone (f : X ⟶ Z) (g : Y ⟶ Z) : PullbackCone f g :=
  PullbackCone.mk (pullbackFst f g) (pullbackSnd f g)
    (by
      ...
      -- `exact h` before https://github.com/leanprover/lean4/issues/2644
...
theorem range_pullback_to_prod {X Y Z : TopCat} (f : X ⟶ Z) (g : Y ⟶ Z) :
  ...
  constructor
  · rintro ⟨y, rfl⟩
    change (_ ≫ _ ≫ f) _ = (_ ≫ _ ≫ g) _ -- new `change` afterhttps://github.com/leanprover-community/mathlib4/issues/13170
    simp [pullback.condition]
  · rintro (h : f (_, _).1 = g (_, _).2)
    use (pullbackIsoProdSubtype f g).inv ⟨⟨_, _⟩, h⟩
    change (forget TopCat).map _ _ = _ -- new `change` afterhttps://github.com/leanprover-community/mathlib4/issues/13170
    apply Concrete.limit_ext
    rintro ⟨⟨⟩⟩ <;>
    erw [← comp_apply, ← comp_apply, limit.lift_π] <;> -- now `erw` afterhttps://github.com/leanprover-community/mathlib4/issues/13170
    -- This used to be `simp` before https://github.com/leanprover/lean4/issues/2644
    aesop_cat
...
theorem range_pullback_map {W X Y Z S T : TopCat} (f₁ : W ⟶ S) (f₂ : X ⟶ S) (g₁ : Y ⟶ T)
  ...
  apply Concrete.limit_ext
  rintro (_ | _ | _) <;>
  erw [← comp_apply, ← comp_apply] -- now `erw` afterhttps://github.com/leanprover-community/mathlib4/issues/13170
  ...
  · simp only [cospan_left, limit.lift_π, PullbackCone.mk_pt, PullbackCone.mk_π_app,
    ...
    erw [hx₁] -- now `erw` afterhttps://github.com/leanprover-community/mathlib4/issues/13170
...
theorem pullback_snd_image_fst_preimage (f : X ⟶ Z) (g : Y ⟶ Z) (U : Set X) :
  ...
  · rintro ⟨y, hy, eq⟩
  -- next 5 lines were
  -- `exact ⟨(TopCat.pullbackIsoProdSubtype f g).inv ⟨⟨_, _⟩, eq⟩, by simpa, by simp⟩` beforehttps://github.com/leanprover-community/mathlib4/issues/13170
