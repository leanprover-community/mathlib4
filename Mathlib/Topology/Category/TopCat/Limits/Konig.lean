...
/-- An arrow between two objects of a finite set `G` of objects of `J`. -/
abbrev FiniteDiagramArrow {J : Type u} [SmallCategory J] (G : Finset J) :=
  Σ' (X Y : J) (_ : X ∈ G) (_ : Y ∈ G), X ⟶ Y

/-- A finite set of objects of `J` together with a finite set of arrows between them. -/
abbrev FiniteDiagram (J : Type u) [SmallCategory J] :=
