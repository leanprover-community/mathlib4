...
theorem isInducing_prod_map {W X Y Z : TopCat.{u}} {f : W ⟶ X} {g : Y ⟶ Z} (hf : IsInducing f)
...
theorem isEmbedding_prod_map {W X Y Z : TopCat.{u}} {f : W ⟶ X} {g : Y ⟶ Z} (hf : IsEmbedding f)
  ...
  ⟨isInducing_prod_map hf.isInducing hg.isInducing, by
  ...
  classical
    ...
    · rintro ⟨h⟩
      ...
      refine ⟨(homeoOfIso <| h.coconePointUniqueUpToIso
        (binaryCofanIsColimit X Y)).symm.isOpenEmbedding.comp IsOpenEmbedding.inl,
          (homeoOfIso <| h.coconePointUniqueUpToIso
            (binaryCofanIsColimit X Y)).symm.isOpenEmbedding.comp IsOpenEmbedding.inr, ?_⟩
      ...
      · intro T f g
        ...
        · revert h x
          ...
          · rw [continuousOn_iff_continuous_restrict]
            convert_to Continuous (f ∘ (Homeomorph.ofEmbedding _ h₁.isEmbedding).symm)
            · ext ⟨x, hx⟩
              exact dif_pos hx
        ...
        · revert h x
          ...
          · rw [continuousOn_iff_continuous_restrict]
            ...
            convert_to Continuous
                (g ∘ (Homeomorph.ofEmbedding _ h₂.isEmbedding).symm ∘ Subtype.map _ this)
            · ext ⟨x, hx⟩
              exact dif_neg hx
            apply Continuous.comp
