...
def Set.proj : Set ((i : {i : ι // J i}) → X i) := precomp (Subtype.val (p := J)) '' C
...
def P (o : Ordinal) : Prop :=
  ∀ {I : Type u} [Inhabited I] [LinearOrder I] [IsWellOrder I (·<·)],
    o ≤ Ordinal.type (·<· : I → I → Prop) →
    (∀ (C : Set (I → Bool)), IsClosed C → contained C o →
    LinearIndependent ℤ (GoodProducts.eval C))

theorem GoodProducts.P0 : P 0 := @fun I _ _ _ _ C _ hsC ↦ by
...
theorem GoodProducts.Plimit (o : Ordinal) (ho : Ordinal.IsLimit o) :
    (∀ (o' : Ordinal), o' < o → P o') → P o := by
  intro h I _ _ _ hho C hC hsC
