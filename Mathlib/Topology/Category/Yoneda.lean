...
-- variable [∀ (X : C), TopologicalSpace ((forget _).obj X)]

-- abbrev F (X : C) := (forget C).obj X

-- abbrev F' {X Y : C} (f : X ⟶ Y) := (forget C).map f

-- variable [∀ {X Y : C}, ContinuousMapClass (F X ⟶ F Y) (F X) (F Y)]
...
--   obj X := C(Y, F X)
--   map f g := ContinuousMap.comp (F' f) g
--   map_id := by
--     intro X
--     ext f y
--     simp only [F', CategoryTheory.Functor.map_id, comp_apply, ContinuousMap.coe_coe, types_id_apply]
--     sorry
--   map_comp := sorry
...
-- lemma piComparison_fac {α : Type} (X : α → C) [HasCoproduct X] :
--     piComparison (coyoneda.{w, w'} Y) (fun x ↦ op (X x)) =
--     (coyoneda Y).map ((opCoproductIsoProduct X).inv ≫ (TopCat.sigmaIsoSigma X).inv.op) ≫
--     (equivEquivIso (sigmaEquiv Y (fun x ↦ (X x)))).inv ≫ (Types.productIso _).inv := by
--   rw [← Category.assoc, Iso.eq_comp_inv]
--   ext
--   simp only [coyoneda', unop_op, piComparison, types_comp_apply,
--     Types.productIso_hom_comp_eval_apply, Types.pi_lift_π_apply, comp_apply, TopCat.coe_of,
