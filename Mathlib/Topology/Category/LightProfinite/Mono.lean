...
variable {X : Profinite} {Y : LightDiagram} (f : X ⟶ Y.toProfinite)

noncomputable def lightDiagramOfHom :
  ...
  map := fun h ⟨x, hx⟩ ↦ ⟨Y.diagram.map h x, (by
    obtain ⟨y, hx⟩ := hx
    refine ⟨y, by simpa [← hx] using NatTrans.naturality_apply Y.cone.π h _⟩)⟩
  map_id _ := by -- `aesop` can handle this but is a bit slow
    simp only [Functor.comp_obj, id_eq, eq_mp_eq_cast, cast_eq, eq_mpr_eq_cast,
      CategoryTheory.Functor.map_id, FintypeCat.id_apply]
    rfl
  map_comp _ _ := by -- `aesop` can handle this but is a bit slow
    simp only [Functor.comp_obj, id_eq, eq_mp_eq_cast, cast_eq, eq_mpr_eq_cast, Functor.map_comp,
...
def lightProfiniteConeOfHom_π_app (n : ℕᵒᵖ) :
    X ⟶ (lightDiagramOfHom f ⋙ FintypeCat.toProfinite).obj n where
...
/-- The cone on `lightDiagramOfHom` -/
def lightProfiniteConeOfHom :
    Cone (lightDiagramOfHom f ⋙ FintypeCat.toProfinite) where
  pt := X
  π := {
...
theorem ext {Y : LightDiagram} {a b : Y.cone.pt}
...
instance [Mono f] : IsIso ((Profinite.limitConeIsLimit ((lightDiagramOfHom f) ⋙
  ...
  · have hf : Function.Injective f := by rwa [← Profinite.mono_iff_injective]
    ...
    apply LightDiagram.ext
    intro n
    apply_fun fun f : (Profinite.limitCone ((lightDiagramOfHom f) ⋙
...
def lightProfiniteIsLimitOfMono [Mono f] : IsLimit (lightProfiniteConeOfHom f) :=
  Limits.IsLimit.ofIsoLimit (Profinite.limitConeIsLimit _)
    (Limits.Cones.ext (asIso ((Profinite.limitConeIsLimit ((lightDiagramOfHom f) ⋙
...
-/
noncomputable def lightProfiniteOfMono [Mono f] : LightDiagram :=
  ⟨lightDiagramOfHom f, lightProfiniteConeOfHom f, lightProfiniteIsLimitOfMono f⟩
