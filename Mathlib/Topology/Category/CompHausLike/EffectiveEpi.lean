...
def effectiveEpiStruct {B X : CompHausLike P} (π : X ⟶ B) (hπ : Function.Surjective π) :
  ...
  desc e h :=
    { hom := (QuotientMap.of_surjective_continuous hπ π.hom.continuous).lift e.hom fun a b hab ↦
      DFunLike.congr_fun (h ⟨fun _ ↦ a, continuous_const⟩ ⟨fun _ ↦ b, continuous_const⟩
        (by ext; exact hab)) a }
  fac e h := InducedCategory.hom_ext
      ((QuotientMap.of_surjective_continuous hπ π.hom.continuous).lift_comp e.hom
        fun a b hab ↦ DFunLike.congr_fun (h ⟨fun _ ↦ a, continuous_const⟩
          ⟨fun _ ↦ b, continuous_const⟩ (by ext; exact hab)) a)
  uniq e h g hm := by
    suffices g.hom = (QuotientMap.of_surjective_continuous hπ π.hom.continuous).liftEquiv
      ⟨e.hom, fun a b hab ↦ DFunLike.congr_fun
        (h ⟨fun _ ↦ a, continuous_const⟩ ⟨fun _ ↦ b, continuous_const⟩ (by ext; exact hab)) a⟩
      from InducedCategory.hom_ext (by assumption)
    rw [← Equiv.symm_apply_eq (QuotientMap.of_surjective_continuous hπ π.hom.continuous).liftEquiv]
    ext
