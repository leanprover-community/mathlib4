...
lemma surjective_of_effectiveEpi {X Y : CompHausLike P} (f : X ⟶ Y) [EffectiveEpi f]
    (h : P <| TopCat.of <| Set.range f) :
    Function.Surjective f := by
  have : CompactSpace (Set.range f) := sorry
  let f' : X ⟶ CompHausLike.of _ (Set.range f) h :=
    ⟨fun x ↦ ⟨f x, Set.mem_range_self x⟩, Continuous.subtype_mk f.continuous _⟩
  let d := EffectiveEpi.desc f f' (fun _ _ h ↦
    (ContinuousMap.ext fun x ↦ Subtype.ext (DFunLike.congr_fun h x)))
  sorry

open List in
theorem effectiveEpi_tfae
    {B X : CompHausLike P} (π : X ⟶ B) :
    TFAE
    [ EffectiveEpi π
    , Epi π
