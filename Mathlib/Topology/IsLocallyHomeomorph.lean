...
definition of `IsLocallyHomeomorphOn f s`, since it only requires `e : PartialHomeomorph X Y` to
...
definition of `IsLocallyHomeomorph f`, since it only requires `e : PartialHomeomorph X Y` to
agree with `f` on its source `e.source`, as opposed to on the whole space `X`. -/
