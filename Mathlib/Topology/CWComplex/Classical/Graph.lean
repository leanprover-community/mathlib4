...
lemma OneSkeletonGraph.exists_isLoopAt_iff (e : cell C 1) :
    (∃ x : cell C 0, (OneSkeletonGraph C).IsLoopAt e x) ↔
      ∃ x : cell C 0, cellFrontier 1 e = closedCell 0 x := by
  refine exists_congr fun x ↦ ?_
  change cellFrontier 1 e = closedCell 0 x ∪ closedCell 0 x ↔ _
  rw [union_self]

lemma OneSkeletonGraph.exists_isLoopAt_iff_subsingleton (e : cell C 1) :
    (∃ x : cell C 0, (OneSkeletonGraph C).IsLoopAt e x) ↔ (cellFrontier 1 e).Subsingleton := by
  rw [OneSkeletonGraph.exists_isLoopAt_iff]
...
lemma OneSkeletonGraph.not_exists_isLoopAt_iff_nontrivial (e : cell C 1) :
...
@[simp]
lemma OneSkeletonGraph.adj_iff (x y : cell C 0) : (OneSkeletonGraph C).Adj x y ↔
