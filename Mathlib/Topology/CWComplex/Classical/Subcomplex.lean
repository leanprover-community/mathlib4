...
lemma RelCWComplex.Subcomplex.finiteDimensional_iInf_of_exists_finiteDimensional
  ...
  eventually_isEmpty_cell := by
    ...
    simp_all only [cell_def, isEmpty_coe_sort, Filter.eventually_atTop, iInf_I]
    obtain ⟨n, hn⟩ := h
    use n
    intro m hm
    rw [← subset_empty_iff]
  ...
  iInf_iSup_eq {ι} f E := by
    ...
    · by_cases h' : ∀ a, Nonempty (f a)
      · have h'' := Classical.nonempty_pi.mpr h'
        simp [eq_iff, coe_iInf_eq_of_nonempty, coe_iSup_eq_of_nonempty,  ← iInf_eq_iInter,
          ← iSup_eq_iUnion, iInf_iSup_eq, -SetLike.coe_set_eq]
      · push Not at h'
