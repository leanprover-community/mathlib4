...
lemma close_enough {x l y : ℝ} (hlu : l < x) (hy' : 2 * |x - y| ≤ |x - l|)
: l < y := by
...
lemma geometry_neg
  ...
  : v.1 1 < 0 := by
  have Q := @geometry_pos
    ⟨-y.1,by simp; intro hc;rw [hc] at hy;simp_all⟩
    ⟨-v.1,by simp; exact v.2⟩ (by simp; tauto)  (by simp;tauto)
...
lemma same_set₀ : fun y : {v : Fin 2 → ℝ // v ≠ 0} ↦ y.1 1 < 2 * y.1 1
                = fun y                            ↦ 0     <     y.1 1 := by
...
lemma same_set₂ : fun y : {v : Fin 2 → ℝ // v ≠ 0} ↦ y.1 1 < (y.1 1) / 2
                = fun y                            ↦ 0     >     y.1 1 := by
                  ext y; constructor; repeat (intro;linarith)
...
theorem dist_cone_pos_pos {a : Fin 2 → ℝ} (h₀ : a 1 = 0) (h₁ : a 0 > 0)
  {s : ℝ} (hs : s ≥ 0)
  : (a 0 / (s+1) > 0) ∧ ∀ y, dist y a ≤ (a 0 / (s+1)) → y 1 > 0 → s ≤ y 0 / y 1
    := by
      constructor
...
theorem dist_cone_pos_neg {a : Fin 2 → ℝ} (h₀ : a 1 = 0) (h₁ : a 0 > 0)
  {s : ℝ} (hs : s ≥ 0)
  ...
  · intro y hy hy'
    have : dist ![y 0, -y 1] a = dist y a := by
      ...
      | inr h =>
        ...
        rw [this];simp;rw [h₀];simp;
    have Q := Q₀.2 (![y 0,- y 1]) (by rw [this]; exact hy) (by simp;tauto)
    simp at Q;linarith

theorem dist_cone_neg_pos {a : Fin 2 → ℝ} (h₀ : a 1 = 0) (h₁ : a 0 <  0)
  {s : ℝ} (hs : s ≥ 0)
  : (- a 0 / (s+1) > 0) ∧ ∀ y, dist y a ≤ (- a 0 / (s+1)) → y 1 > 0 → y 0 / y 1 ≤  - s
    := by
  have Q := @dist_cone_pos_neg ![-a 0,a 1] h₀ (by simp;exact h₁) s hs
  rw [h₀] at Q
  have hn : ![-a 0, 0] = -a := by
    ext z;simp;
    ...
    | inr h =>
      have : z = 1 := by apply Fin.eq_of_val_eq; simp_all
      rw [this];simp; exact h₀
  constructor
  · exact Q.1
  · intro y hay hy
    have Q₂ := Q.2
    have W := @Q₂ (-y) (by simp; rw [hn, dist_neg_neg]; exact hay) (by simp;tauto)
    simp at W;exact W

theorem dist_cone_neg_neg {a : Fin 2 → ℝ} (h₀ : a 1 = 0) (h₁ : a 0 <  0)
  {s : ℝ} (hs : s ≥ 0)
  : (- a 0 / (s+1) > 0) ∧ ∀ y, dist y a ≤ (- a 0 / (s+1)) → y 1 < 0 → y 0 / y 1 ≥ s
    := by
  have Q := @dist_cone_pos_pos ![-a 0,a 1] (by simp;tauto) (by simp;tauto) s hs
...
theorem dist_cone_pos {a : Fin 2 → ℝ} (h₀ : a 1 = 0) (h₁ : a 0 > 0)
  {s : ℝ} (hs : s ≥ 0)
  : ∃ δ > 0, ∀ y, dist y a ≤ δ →
      (y 1 < 0 → y 0 / y 1 ≤  -s)
      ∧ (y 1 > 0 → y 0 / y 1 ≥  s) := by
...
theorem dist_cone_neg {a : Fin 2 → ℝ} (h₀ : a 1 = 0) (h₁ : a 0 < 0)
  {s : ℝ} (hs : s ≥ 0)
  : ∃ δ > 0, ∀ y, dist y a ≤ δ →
      (y 1 > 0 → y 0 / y 1 ≤  -s)
      ∧ (y 1 < 0 → y 0 / y 1 ≥  s) := by
