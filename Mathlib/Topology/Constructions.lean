...
lemma surjective_projRestrict :
  ...
  fun ⟨_, ⟨y, ⟨hy₁, hy₂⟩⟩⟩ ↦ ⟨⟨y, hy₁⟩, Subtype.ext hy₂⟩

lemma proj_eq_self {x : (i : ι) → π i} (h : ∀ i, x i ≠ default → J i) : Proj π J x = x := by
  ext i
...
@[simp]
lemma proj_comp_of_subset (h : ∀ i, J i → K i) : Proj π J ∘ Proj π K = Proj π J := by
  ext x i
  dsimp [Proj]
  split_ifs with h₁ h₂
  · rfl
  · simp only [h i h₁, not_true] at h₂
  · rfl

lemma proj_eq_of_subset (h : ∀ i, J i → K i) : (C.proj K).proj J = C.proj J := by
  ext x
  refine ⟨fun ⟨y, ⟨⟨z, hz⟩, hy⟩⟩  ↦ ?_, fun ⟨y, hy⟩ ↦ ?_⟩
  · rw [← hy, ← hz.2, (by rfl : Proj π J (Proj π K z) = (Proj π J ∘ Proj π K) z),
