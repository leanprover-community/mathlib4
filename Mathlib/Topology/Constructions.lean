...
instance Prod.hasIndiscreteTopology [TopologicalSpace X] [TopologicalSpace Y]
    [h : HasIndiscreteTopology X] [hY : HasIndiscreteTopology Y] : HasIndiscreteTopology (X × Y) :=
  ⟨inf_eq_top_iff.2 <| by simp [h.eq_top, hY.eq_top]⟩

instance Pi.hasIndiscreteTopology {ι : Type*} {Y : ι → Type v} [∀ i, TopologicalSpace (Y i)]
    [h : ∀ i, HasIndiscreteTopology (Y i)] : HasIndiscreteTopology ((i : ι) → Y i) :=
