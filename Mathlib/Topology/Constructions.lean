...
variable {ι : Type*} {X : ι → Type*} {κ : Type*} [TopologicalSpace X]
  [T : ∀ i, TopologicalSpace (X i)] {f : X → ∀ i : ι, X i}
...
@[continuity, fun_prop]
theorem continuous_apply (i : ι) : Continuous fun p : ∀ i, X i => p i :=
...
theorem continuousAt_apply (i : ι) (x : ∀ i, X i) : ContinuousAt (fun p : ∀ i, X i => p i) x :=
  (continuous_apply i).continuousAt

theorem Filter.Tendsto.apply_nhds {l : Filter Y} {f : Y → ∀ i, X i} {x : ∀ i, X i}
...
protected theorem Continuous.piMap {Y : ι → Type*} [∀ i, TopologicalSpace (Y i)]
    {f : ∀ i, X i → Y i} (hf : ∀ i, Continuous (f i)) : Continuous (Pi.map f) :=
  continuous_pi fun i ↦ (hf i).comp (continuous_apply i)

theorem nhds_pi {a : ∀ i, X i} : 𝓝 a = pi fun i => 𝓝 (a i) := by
  simp only [nhds_iInf, nhds_induced, Filter.pi]

protected theorem IsOpenMap.piMap {Y : ι → Type*} [∀ i, TopologicalSpace (Y i)] {f : ∀ i, X i → Y i}
...
protected theorem IsOpenQuotientMap.piMap {Y : ι → Type*} [∀ i, TopologicalSpace (Y i)]
    {f : ∀ i, X i → Y i} (hf : ∀ i, IsOpenQuotientMap (f i)) : IsOpenQuotientMap (Pi.map f) :=
  ⟨.piMap fun i ↦ (hf i).1, .piMap fun i ↦ (hf i).2, .piMap (fun i ↦ (hf i).3) <|
    .of_forall fun i ↦ (hf i).1⟩

theorem tendsto_pi_nhds {f : Y → ∀ i, X i} {g : ∀ i, X i} {u : Filter Y} :
    Tendsto f u (𝓝 g) ↔ ∀ x, Tendsto (fun i => f i x) u (𝓝 (g x)) := by
  rw [nhds_pi, Filter.tendsto_pi]

theorem continuousAt_pi {f : X → ∀ i, X i} {x : X} :
    ContinuousAt f x ↔ ∀ i, ContinuousAt (fun y => f y i) x :=
  tendsto_pi_nhds

@[fun_prop]
theorem continuousAt_pi' {f : X → ∀ i, X i} {x : X} (hf : ∀ i, ContinuousAt (fun y => f y i) x) :
    ContinuousAt f x :=
  continuousAt_pi.2 hf

@[fun_prop]
protected theorem ContinuousAt.piMap {Y : ι → Type*} [∀ i, TopologicalSpace (Y i)]
    {f : ∀ i, X i → Y i} {x : ∀ i, X i} (hf : ∀ i, ContinuousAt (f i) (x i)) :
...
theorem Pi.continuous_precomp' {ι' : Type*} (φ : ι' → ι) :
    Continuous (fun (f : (∀ i, X i)) (j : ι') ↦ f (φ j)) :=
...
theorem Pi.continuous_postcomp' {X : ι → Type*} [∀ i, TopologicalSpace (X i)]
    {g : ∀ i, X i → X i} (hg : ∀ i, Continuous (g i)) :
    Continuous (fun (f : (∀ i, X i)) (i : ι) ↦ g i (f i)) :=
...
lemma Pi.induced_precomp' {ι' : Type*} (φ : ι' → ι) :
    induced (fun (f : (∀ i, X i)) (j : ι') ↦ f (φ j)) Pi.topologicalSpace =
...
lemma Pi.continuous_restrict (S : Set ι) :
    Continuous (S.restrict : (∀ i : ι, X i) → (∀ i : S, X i)) :=
...
lemma Pi.induced_restrict_sUnion (𝔖 : Set (Set ι)) :
    induced (⋃₀ 𝔖).restrict (Pi.topologicalSpace (Y := fun i : (⋃₀ 𝔖) ↦ X i)) =
    ⨅ S ∈ 𝔖, induced S.restrict Pi.topologicalSpace := by
  simp_rw [Pi.induced_restrict, iInf_sUnion]

theorem Filter.Tendsto.update [DecidableEq ι] {l : Filter Y} {f : Y → ∀ i, X i} {x : ∀ i, X i}
    (hf : Tendsto f l (𝓝 x)) (i : ι) {g : Y → X i} {xi : X i} (hg : Tendsto g l (𝓝 xi)) :
...
theorem ContinuousAt.update [DecidableEq ι] {x : X} (hf : ContinuousAt f x) (i : ι) {g : X → X i}
    (hg : ContinuousAt g x) : ContinuousAt (fun a => update (f a) i (g a)) x :=
  hf.tendsto.update i hg

theorem Continuous.update [DecidableEq ι] (hf : Continuous f) (i : ι) {g : X → X i}
...
theorem continuous_update [DecidableEq ι] (i : ι) :
    Continuous fun f : (∀ j, X j) × X i => update f.1 i f.2 :=
...
theorem continuous_mulSingle [∀ i, One (X i)] [DecidableEq ι] (i : ι) :
    Continuous fun x => (Pi.mulSingle i x : ∀ i, X i) :=
...
theorem Filter.Tendsto.finCons
    {f : Y → X 0} {g : Y → ∀ j : Fin n, X j.succ} {l : Filter Y} {x : X 0} {y : ∀ j, X (Fin.succ j)}
...
theorem ContinuousAt.finCons {f : X → X 0} {g : X → ∀ j : Fin n, X (Fin.succ j)} {x : X}
    (hf : ContinuousAt f x) (hg : ContinuousAt g x) :
    ContinuousAt (fun a => Fin.cons (f a) (g a)) x :=
  hf.tendsto.finCons hg

theorem Continuous.finCons {f : X → X 0} {g : X → ∀ j : Fin n, X (Fin.succ j)}
...
theorem Filter.Tendsto.finSnoc
    {f : Y → ∀ j : Fin n, X j.castSucc} {g : Y → X (Fin.last _)}
    {l : Filter Y} {x : ∀ j, X (Fin.castSucc j)} {y : X (Fin.last _)}
    (hf : Tendsto f l (𝓝 x)) (hg : Tendsto g l (𝓝 y)) :
...
theorem ContinuousAt.finSnoc {f : X → ∀ j : Fin n, X j.castSucc} {g : X → X (Fin.last _)} {x : X}
    (hf : ContinuousAt f x) (hg : ContinuousAt g x) :
    ContinuousAt (fun a => Fin.snoc (f a) (g a)) x :=
  hf.tendsto.finSnoc hg

theorem Continuous.finSnoc {f : X → ∀ j : Fin n, X j.castSucc} {g : X → X (Fin.last _)}
...
theorem Filter.Tendsto.finInsertNth
    (i : Fin (n + 1)) {f : Y → X i} {g : Y → ∀ j : Fin n, X (i.succAbove j)} {l : Filter Y}
    {x : X i} {y : ∀ j, X (i.succAbove j)} (hf : Tendsto f l (𝓝 x)) (hg : Tendsto g l (𝓝 y)) :
...
theorem ContinuousAt.finInsertNth
    (i : Fin (n + 1)) {f : X → X i} {g : X → ∀ j : Fin n, X (i.succAbove j)} {x : X}
    (hf : ContinuousAt f x) (hg : ContinuousAt g x) :
...
theorem Continuous.finInsertNth
    (i : Fin (n + 1)) {f : X → X i} {g : X → ∀ j : Fin n, X (i.succAbove j)}
...
theorem isOpen_set_pi {i : Set ι} {s : ∀ a, Set (X a)} (hi : i.Finite)
    (hs : ∀ a ∈ i, IsOpen (s a)) : IsOpen (pi i s) := by
  rw [pi_def]; exact hi.isOpen_biInter fun a ha => (hs _ ha).preimage (continuous_apply _)

theorem isOpen_pi_iff {s : Set (∀ a, X a)} :
    IsOpen s ↔
      ∀ f, f ∈ s → ∃ (I : Finset ι) (u : ∀ a, Set (X a)),
...
theorem isOpen_pi_iff' [Finite ι] {s : Set (∀ a, X a)} :
    IsOpen s ↔
      ∀ f, f ∈ s → ∃ u : ∀ a, Set (X a), (∀ a, IsOpen (u a) ∧ f a ∈ u a) ∧ univ.pi u ⊆ s := by
...
theorem isClosed_set_pi {i : Set ι} {s : ∀ a, Set (X a)} (hs : ∀ a ∈ i, IsClosed (s a)) :
    IsClosed (pi i s) := by
  rw [pi_def]; exact isClosed_biInter fun a ha => (hs _ ha).preimage (continuous_apply _)

theorem mem_nhds_of_pi_mem_nhds {I : Set ι} {s : ∀ i, Set (X i)} (a : ∀ i, X i) (hs : I.pi s ∈ 𝓝 a)
    {i : ι} (hi : i ∈ I) : s i ∈ 𝓝 (a i) := by
  rw [nhds_pi] at hs; exact mem_of_pi_mem_pi hs hi

theorem set_pi_mem_nhds {i : Set ι} {s : ∀ a, Set (X a)} {x : ∀ a, X a} (hi : i.Finite)
    (hs : ∀ a ∈ i, s a ∈ 𝓝 (x a)) : pi i s ∈ 𝓝 x := by
  rw [pi_def, biInter_mem hi]
  exact fun a ha => (continuous_apply a).continuousAt (hs a ha)

theorem set_pi_mem_nhds_iff {I : Set ι} (hI : I.Finite) {s : ∀ i, Set (X i)} (a : ∀ i, X i) :
    I.pi s ∈ 𝓝 a ↔ ∀ i : ι, i ∈ I → s i ∈ 𝓝 (a i) := by
  rw [nhds_pi, pi_mem_pi_iff hI]

theorem interior_pi_set {I : Set ι} (hI : I.Finite) {s : ∀ i, Set (X i)} :
    interior (pi I s) = I.pi fun i => interior (s i) := by
  ext a
  simp only [Set.mem_pi, mem_interior_iff_mem_nhds, set_pi_mem_nhds_iff hI]

theorem exists_finset_piecewise_mem_of_mem_nhds [DecidableEq ι] {s : Set (∀ a, X a)} {x : ∀ a, X a}
    (hs : s ∈ 𝓝 x) (y : ∀ a, X a) : ∃ I : Finset ι, I.piecewise x y ∈ s := by
  simp only [nhds_pi, Filter.mem_pi'] at hs
  rcases hs with ⟨I, t, htx, hts⟩
  refine ⟨I, hts fun i hi => ?_⟩
  simpa [Finset.mem_coe.1 hi] using mem_of_mem_nhds (htx i)

theorem pi_generateFrom_eq {X : ι → Type*} {g : ∀ a, Set (Set (X a))} :
    (@Pi.topologicalSpace ι X fun a => generateFrom (g a)) =
      generateFrom
        { t | ∃ (s : ∀ a, Set (X a)) (i : Finset ι), (∀ a ∈ i, s a ∈ g a) ∧ t = pi (↑i) s } := by
...
theorem pi_eq_generateFrom :
    Pi.topologicalSpace =
      generateFrom
        { g | ∃ (s : ∀ a, Set (X a)) (i : Finset ι), (∀ a ∈ i, IsOpen (s a)) ∧ g = pi (↑i) s } :=
  calc Pi.topologicalSpace
  _ = @Pi.topologicalSpace ι X fun _ => generateFrom { s | IsOpen s } := by
...
theorem induced_to_pi {X : Type*} (f : X → ∀ i, X i) :
...
/-- Suppose `X i` is a family of topological spaces indexed by `i : ι`, and `X` is a type
endowed with a family of maps `f i : X → X i` for every `i : ι`, hence inducing a
map `g : X → Π i, X i`. This lemma shows that infimum of the topologies on `X` induced by
the `f i` as `i : ι` varies is simply the topology on `X` induced by `g : X → Π i, X i`
where `Π i, X i` is endowed with the usual product topology. -/
theorem inducing_iInf_to_pi {X : Type*} (f : ∀ i, X → X i) :
    @IsInducing X (∀ i, X i) (⨅ i, induced (f i) inferInstance) _ fun x i => f i x :=
  letI := ⨅ i, induced (f i) inferInstance; ⟨(induced_to_pi _).symm⟩

variable [Finite ι] [∀ i, DiscreteTopology (X i)]

/-- A finite product of discrete spaces is discrete. -/
instance Pi.discreteTopology : DiscreteTopology (∀ i, X i) :=
