...
-- @[mk_iff Relation.ReflTransGen.cases_tail_iff] -- **TODO** port of this attribute is in progress
inductive ReflTransGen (r : α → α → Prop) (a : α) : α → Prop
  | refl : ReflTransGen r a a
  | tail {b c} : ReflTransGen r a b → r b c → ReflTransGen r a c

-- **TODO** this should be auto-generated by the
-- `@[mk_iff Relation.ReflTransGen.cases_tail_iff]` attribute on `ReflTransGen`
lemma ReflTransGen.cases_tail_iff (r : α → α → Prop) (a c : α) :
    ReflTransGen r a c ↔ c = a ∨ ∃ (b : α), ReflTransGen r a b ∧ r b c := by
  refine ⟨?_, ?_⟩
  { rintro (h | h)
    { left; rfl }
    { right; exact ⟨_, h, by assumption⟩ } }
  { rintro (rfl | ⟨b, hab, hbc⟩)
    { exact ReflTransGen.refl }
    { exact ReflTransGen.tail hab hbc } }

attribute [refl] ReflTransGen.refl

/-- `ReflGen r`: reflexive closure of `r` -/
-- @[mk_iff] -- **TODO** this doesn't work yet
inductive ReflGen (r : α → α → Prop) (a : α) : α → Prop
  | refl : ReflGen r a a
  | single {b} : r a b → ReflGen r a b

-- **TODO** this should be auto-generated by
-- @[mk_iff] on `ReflGen`
theorem refl_gen_iff (r : α → α → Prop) (a b : α) : ReflGen r a b ↔ b = a ∨ r a b := by
  refine ⟨?_, ?_⟩
  { rintro (h | h)
    { left; rfl }
    { right; exact h } }
...
-- @[mk_iff] -- **TODO** port of this attribute is in progress
inductive TransGen (r : α → α → Prop) (a : α) : α → Prop
  | single {b} : r a b → TransGen r a b
  | tail {b c} : TransGen r a b → r b c → TransGen r a c

-- **TODO** this should be auto-generated by the
-- `@[mk_iff]` attribute on `TransGen`
lemma trans_gen_iff (r : α → α → Prop) (a c : α) :
    TransGen r a c ↔ r a c ∨ ∃ (b : α), TransGen r a b ∧ r b c := by
  refine ⟨?_, ?_⟩
  { rintro (h | h)
    { left; exact h }
    { right; exact ⟨_, h, by assumption⟩ } }
...
theorem transitive_trans_gen : Transitive (TransGen r) := fun _ _ _ => TransGen.trans
...
theorem reflexive_refl_trans_gen : Reflexive (ReflTransGen r) := fun _ => refl

theorem transitive_refl_trans_gen : Transitive (ReflTransGen r) := fun _ _ _ => trans
...
-- remark: in mathlib3 the proof was shorter because lean 3 could unify { } with ⦃ ⦄ so you
-- didn't need the `fun`
  ⟨reflexive_join hr, fun hab => symmetric_join hab, fun hab hbc => transitive_join ht h hab hbc⟩

theorem equivalence_join_refl_trans_gen
    (h : ∀ a b c, r a b → r a c → ∃ d, ReflGen r b d ∧ ReflTransGen r c d) :
    Equivalence (Join (ReflTransGen r)) :=
  equivalence_join reflexive_refl_trans_gen transitive_refl_trans_gen fun _ _ _ => church_rosser h
...
theorem refl_trans_gen_of_equivalence {r' : α → α → Prop} (hr : Equivalence r) :
  ...
  refl_trans_gen_of_transitive_reflexive hr.1 (fun _ _ _ => hr.trans)
