...
protected lemma Function.isEmpty [IsEmpty β] (f : α → β) : IsEmpty α :=
  ⟨fun x ↦ IsEmpty.false (f x)⟩
#align function.is_empty Function.isEmpty

lemma Function.Surjective.isEmpty [IsEmpty α] {f : α → β} (hf : f.Surjective) : IsEmpty β :=
...
lemma isEmpty_iff : IsEmpty α ↔ α → False :=
...
protected lemma prop_iff {p : Prop} : IsEmpty p ↔ ¬p :=
  isEmpty_iff
