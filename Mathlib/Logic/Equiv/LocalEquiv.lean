...
/-- Combine a family of `LocalEquiv`'s with disjoint sources and disjoint targets -/
@[simps (config := { isSimp := false })]
noncomputable def disjoint_iUnion {I : Type _} [Nonempty I] (h : I → LocalEquiv α β)
    (disj₁ : Pairwise (Disjoint on fun i : I => (h i).source))
    (disj₂ : Pairwise (Disjoint on fun i : I => (h i).target)) : LocalEquiv α β
    where
...
theorem disjoint_iUnion_apply' {I : Type _} [Nonempty I] (h : I → LocalEquiv α β) (i : I)
