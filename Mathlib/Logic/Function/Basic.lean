...
theorem eq_mp_bijective {α β : Sort*} (h : α = β) : Function.Bijective (Eq.mp h) := by
...
theorem eq_mpr_bijective {α β : Sort*} (h : α = β) : Function.Bijective (Eq.mpr h) := by
  cases h
  refine ⟨fun _ _ ↦ id, fun x ↦ ⟨x, rfl⟩⟩
#align eq_mpr_bijective eq_mpr_bijective

theorem cast_bijective {α β : Sort*} (h : α = β) : Function.Bijective (cast h) := by
  cases h
  refine ⟨fun _ _ ↦ id, fun x ↦ ⟨x, rfl⟩⟩
