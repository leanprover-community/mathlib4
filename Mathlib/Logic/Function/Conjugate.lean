...
protected lemma comp_eq (h : Semiconj f ga gb) : f ∘ ga = gb ∘ f :=
  funext h
...
lemma inverses_right (h : Semiconj f ga gb) (ha : RightInverse ga' ga) (hb : LeftInverse gb' gb) :
    Semiconj f ga' gb' :=
...
definitionally equal but have different dot-notation lemmas. -/
lemma semiconj (h : Commute f g) : Semiconj f g g := h

@[refl]
lemma refl (f : α → α) : Commute f f :=
  fun _ ↦ Eq.refl _
#align function.commute.refl Function.Commute.refl

@[symm]
lemma symm (h : Commute f g) : Commute g f :=
  fun x ↦ (h x).symm
