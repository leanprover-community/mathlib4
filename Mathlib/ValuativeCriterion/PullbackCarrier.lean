...
-- lemma SpecOfPoint_SpecTensorTo (T : Triplet f g) (p : Spec T.tensor) :
--     SpecOfPoint (T.SpecTensorTo.val.base p)
--       = (Spec.map (Triplet.tensorCongr (T.ofPoint_SpectensorTo p)).hom).val.base p := by
--   rw [SpecOfPoint]
