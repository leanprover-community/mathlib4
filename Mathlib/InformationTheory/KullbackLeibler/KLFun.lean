...
lemma convexOn_klFun : ConvexOn ℝ (Ici 0) klFun :=
...
lemma klFun_nonneg {x : ℝ} (hx : 0 ≤ x) : 0 ≤ klFun x := by
  rcases hx.eq_or_lt with rfl | hx
  · simp
  refine ConvexOn.nonneg_of_todo (f := klFun) ?_ ?_ ?_ hx
  · exact convexOn_klFun.subset (Ioi_subset_Ici le_rfl) (convex_Ioi _)
  · simp
  · simp

lemma klFun_eq_zero_iff {x : ℝ} (hx : 0 ≤ x) : klFun x = 0 ↔ x = 1 := by
  refine ⟨fun h ↦ ?_, fun h ↦ by simp [h]⟩
  refine strictConvexOn_klFun.eq_of_isMinOn
    (isMinOn_iff.mpr fun y hy ↦ h ▸ klFun_nonneg hy) (isMinOn_iff.mpr fun y hy ↦ ?_)
    hx (zero_le_one' ℝ)
  rw [klFun, log_one, mul_zero, zero_add, sub_self]
  exact klFun_nonneg hy
