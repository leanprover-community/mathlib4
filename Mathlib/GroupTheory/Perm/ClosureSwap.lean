...
@[simp] lemma fixedBy_one : fixedBy α (1 : G) = univ := by
  ext
  simp

@[simp] lemma fixedBy_inv (g : G) : fixedBy α g⁻¹ = fixedBy α g := by
  ext
  simp_rw [mem_fixedBy, inv_smul_eq_iff, eq_comm]

lemma fixedBy_inter_subset_fixedBy_mul (g h : G) :
    fixedBy α g ∩ fixedBy α h ⊆ fixedBy α (g * h) := by
  intro x ⟨hx1, hx2⟩
  rw [mem_fixedBy, mul_smul, hx2, hx1]
...
theorem finite_compl_fixedBy_closure_iff {S : Set G} :
  ...
  ⟨fun h g hg ↦ h g (subset_closure hg), fun h g hg ↦ by
    ...
    simp_rw [← compl_inter, compl_subset_compl, fixedBy_inter_subset_fixedBy_mul]⟩
...
theorem mem_closure_isSwap {S : Set (Perm α)} (hS : ∀ f ∈ S, f.IsSwap) {f : Perm α} :
  ...
  suffices : (fixedBy α f)ᶜ ⊆ supp → f ∈ closure S
