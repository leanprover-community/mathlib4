...
theorem Equiv.Perm.IsSwap.finite_compl_fixedBy {σ : Perm α} (h : σ.IsSwap) :
  ...
  obtain ⟨x, y, _, rfl⟩ := h.exists_distinct_eq_swap
...
theorem swap_mem_closure_isSwap {S : Set (Perm α)} (hS : ∀ f ∈ S, f.IsSwap) {x y : α} :
  ...
  · obtain ⟨σ, hσ, a, ha, hσa⟩ := this
    obtain ⟨z, w, hzw, rfl⟩ := (hS σ hσ).exists_distinct_eq_swap
  ...
  · obtain ⟨x, y, -, rfl⟩ := (hS f hf).exists_distinct_eq_swap ; rwa [swap_inv]
...
theorem mem_closure_isSwap' {f : Perm α} :
  ...
  · exact subset_closure ⟨x, f x, h, Perm.swap_isSwapOn⟩
