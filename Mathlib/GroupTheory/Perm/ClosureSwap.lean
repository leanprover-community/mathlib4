...
theorem SubmonoidClass.swap_mem_trans {a b c : α} {C} [SetLike C (Perm α)]
  ...
  · exact Equiv.swap_self a ▸ one_mem M
  rw [Equiv.swap_comm, ← Equiv.swap_mul_swap_mul_swap hab' hac]
...
theorem swap_mem_closure_isSwap {S : Set (Perm α)} (hS : ∀ f ∈ S, f.IsSwap) {x y : α} :
  ...
  · obtain ⟨x, y, -, rfl⟩ := hS f hf; rwa [Equiv.swap_inv]
  · exact orbit_eq_iff.mpr hf ▸ ⟨⟨Equiv.swap z y, hz⟩, Equiv.swap_apply_right z y⟩
