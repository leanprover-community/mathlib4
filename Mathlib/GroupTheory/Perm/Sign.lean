...
theorem aux1 {G : Type*} [Group G] [MulAction G α] {S : Set G} (hS : ∀ g ∈ S, g⁻¹ ∈ S)
    {T : Set α} {σ : G} (hσ : σ ∈ Subgroup.closure S) {a : α} (ha : a ∈ T) (h : σ • a ∉ T) :
    ∃ σ ∈ S, ∃ a ∈ T, σ • a ∉ T := by
  have key0 : ¬ Subgroup.closure S ≤ MulAction.stabilizer G T :=
    fun h' ↦ h (h' hσ ▸ Set.smul_mem_smul_set ha)
  rw [Subgroup.closure_le, Set.not_subset] at key0
  obtain ⟨τ, hτ, hτ'⟩ := key0
  rw [SetLike.mem_coe, ← Subgroup.inv_mem_iff, MulAction.mem_stabilizer_iff] at hτ'
  rw [Set.ext_iff, not_forall] at hτ'
  obtain ⟨b, hτ'⟩ := hτ'
  rw [Set.mem_inv_smul_set_iff, not_iff] at hτ'
  by_cases hb : b ∈ T
  · exact ⟨τ, hτ, b, hb, hτ'.mpr hb⟩
  · exact ⟨τ⁻¹, hS τ hτ, τ • b, not_imp_comm.mp hτ'.mp hb, (inv_smul_smul τ b).symm ▸ hb⟩

theorem aux2
    {H : Subgroup (Perm α)} {T : Set α} (hT : T.Pairwise (fun x y ↦ swap x y ∈ H))
    {a b : α} (ha : a ∈ T) (hb : b ∉ T) (hab : swap a b ∈ H) :
    ({b} ∪ T).Pairwise (fun x y ↦ swap x y ∈ H) := by
...
theorem closure_of_isSwap_of_isPretransitive [Finite α] (S : Set (Perm α)) (hS : ∀ σ ∈ S, σ.IsSwap)
  ...
  have key0 : ¬ Subgroup.closure S ≤ MulAction.stabilizer (Perm α) T
  · -- we can find `a ∈ T` and `b ∉ T`
    obtain ⟨a, ha⟩ := hT'
  ...
  exact ⟨b, hb, aux2 ih ha hb (Subgroup.subset_closure hab)⟩

/-- A transitive permutation group generated by transpositions must be the whole symmetric group -/
theorem surjective_of_isSwap_of_isPretransitive [Group β] [MulAction β α] [Finite α] (S : Set β)
    (hS1 : ∀ σ ∈ S, Perm.IsSwap (MulAction.toPermHom β α σ)) (hS2 : Subgroup.closure S = ⊤)
    [h : MulAction.IsPretransitive β α] : Function.Surjective (MulAction.toPermHom β α) := by
  rw [← MonoidHom.range_top_iff_surjective]
  have := MulAction.IsPretransitive.of_compHom (α := α) (MulAction.toPermHom β α).rangeRestrict
