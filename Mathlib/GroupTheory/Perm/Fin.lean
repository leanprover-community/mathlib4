...
@[simp]
lemma Equiv.Perm.decomposeFin_symm_of_refl {n : ℕ} (p : Fin (n + 1)) :
...
@[simp]
lemma Equiv.Perm.decomposeFin_symm_of_one {n : ℕ} (p : Fin (n + 1)) :
...
@[simp, nolint simpNF]
lemma Equiv.Perm.decomposeFin_symm_apply_zero {n : ℕ} (p : Fin (n + 1)) (e : Perm (Fin n)) :
...
@[simp]
lemma Equiv.Perm.decomposeFin_symm_apply_succ {n : ℕ} (e : Perm (Fin n)) (p : Fin (n + 1))
...
-- See https://github.com/leanprover-community/mathlib4/issues/12232
@[simp, nolint simpNF]
lemma Equiv.Perm.decomposeFin_symm_apply_one {n : ℕ} (e : Perm (Fin (n + 1))) (p : Fin (n + 2)) :
...
@[simp]
lemma Equiv.Perm.decomposeFin.symm_sign {n : ℕ} (p : Fin (n + 1)) (e : Perm (Fin n)) :
...
lemma finRotate_succ_eq_decomposeFin {n : ℕ} :
...
@[simp]
lemma support_finRotate {n : ℕ} : support (finRotate (n + 2)) = Finset.univ := by
  ext
  simp
#align support_fin_rotate support_finRotate

lemma support_finRotate_of_le {n : ℕ} (h : 2 ≤ n) : support (finRotate n) = Finset.univ := by
  obtain ⟨m, rfl⟩ := exists_add_of_le h
  rw [add_comm, support_finRotate]
#align support_fin_rotate_of_le support_finRotate_of_le

lemma isCycle_finRotate {n : ℕ} : IsCycle (finRotate (n + 2)) := by
...
lemma isCycle_finRotate_of_le {n : ℕ} (h : 2 ≤ n) : IsCycle (finRotate n) := by
  obtain ⟨m, rfl⟩ := exists_add_of_le h
  rw [add_comm]
...
@[simp]
lemma cycleType_finRotate {n : ℕ} : cycleType (finRotate (n + 2)) = {n + 2} := by
...
lemma cycleType_finRotate_of_le {n : ℕ} (h : 2 ≤ n) : cycleType (finRotate n) = {n} := by
  obtain ⟨m, rfl⟩ := exists_add_of_le h
...
lemma cycleRange_of_gt {n : ℕ} {i j : Fin n.succ} (h : i < j) : cycleRange i j = j := by
...
lemma cycleRange_of_le {n : ℕ} {i j : Fin n.succ} (h : j ≤ i) :
...
lemma coe_cycleRange_of_le {n : ℕ} {i j : Fin n.succ} (h : j ≤ i) :
...
@[simp]
lemma cycleRange_zero' {n : ℕ} (h : 0 < n) : cycleRange ⟨0, h⟩ = 1 := by
  cases' n with n
  · cases h
...
@[simp]
lemma sign_cycleRange {n : ℕ} (i : Fin n) : Perm.sign (cycleRange i) = (-1) ^ (i : ℕ) := by
  simp [cycleRange]
#align fin.sign_cycle_range Fin.sign_cycleRange

@[simp]
lemma succAbove_cycleRange {n : ℕ} (i j : Fin n) :
...
@[simp]
lemma cycleRange_succAbove {n : ℕ} (i : Fin (n + 1)) (j : Fin n) :
...
@[simp]
lemma cycleType_cycleRange {n : ℕ} {i : Fin (n + 1)} (h0 : i ≠ 0) :
...
lemma isThreeCycle_cycleRange_two {n : ℕ} : IsThreeCycle (cycleRange 2 : Perm (Fin (n + 3))) := by
