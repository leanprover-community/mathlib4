...
theorem conj_support_eq (k : ConjAct (Perm α)) (g : Perm α) (a : α) :
  ...
  simp only [mem_support, ConjAct.smul_def, not_iff_not, coe_mul, Function.comp_apply,
...
/- NB. The converse of the next theorem is false. Commuting with every cycle of `g`
  means that we belong to the kernel of the action of `Perm α` on `g.cycleFactorsFinset` -/
...
#align commute_of_mem_cycles_factors_commute
...
theorem mem_cycleFactorsFinset_support {g c : Perm α}
  ...
  apply mem_support_of_commute
  exact (self_mem_cycle_factors_commute hc).symm
...
theorem isInvariant_of_support_le {c : Perm α} {s : Finset α}
  ...
  · simp only [hcs hx', true_iff_iff]
    exact hcs (apply_mem_support.mpr hx')
...
def subtypePermOfSupport (c : Perm α) : Equiv.Perm c.support :=
...
lemma ofSubtype_eq_iff {g c : Perm α} {s : Finset α}
    (hg : ∀ x, x ∈ s ↔  g x ∈ s) :
...
theorem zpow_eq_ofSubtype_subtypePerm_iff
  ...
  · intro h; ext ⟨x, hx⟩; let h' := Perm.congr_fun h x
    simp only [h', subtypePerm_apply_zpow_of_mem, Subtype.coe_mk,
      subtypePerm_apply]
  ...
  · intro h; ext x
    ...
    · rw [ofSubtype_apply_of_not_mem]
      ...
      intro hx'
      apply hx
      apply hc; apply support_zpow_le; exact hx'
      exact hx
...
theorem IsCycle.commute_iff {g c : Perm α} (hc : c.IsCycle) :
  ...
  · rw [iff_true_left _]
    · rw [ofSubtype_apply_of_not_mem, ← not_mem_support]
      · intro ha'; apply ha
        apply support_zpow_le
        exact ha'
      exact ha
    exact fun b => False.elim (ha b)

theorem IsCycle.forall_commute_iff (g z : Perm α) :
...
theorem subtypePerm_on_cycleFactorsFinset {g c : Perm α}
    (hc : c ∈ g.cycleFactorsFinset) :
    g.subtypePerm (mem_cycleFactorsFinset_support hc) =
...
theorem commute_of_mem_cycleFactorsFinset_iff {g k c : Perm α}
    (hc : c ∈ g.cycleFactorsFinset) :
    Commute k c ↔
  ...
  intro n
  unfold subtypePermOfSupport
  rw [subtypePerm_on_cycleFactorsFinset hc]
  rfl
...
theorem zpow_mod_card_support_cycleOf_self_apply [Fintype α]
    (f : Perm α) (n : ℤ) (x : α) :
    (f ^ (n % ((cycleOf f x).support.card) : ℤ) : Perm α) x =
      (f ^ n) x := by
  by_cases hx : f x = x
...
theorem cycle_zpow_mem_support_iff {g : Perm α}
  ...
  have : (g ^ m) x = x ↔ g ^ m = 1 := by
    constructor
    · intro hgm
      simp [IsCycle.pow_eq_one_iff hg]
      use x
