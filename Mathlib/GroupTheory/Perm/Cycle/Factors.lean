...
lemma pairwise'_commute_of_mem_zpowers :
    Pairwise' fun (i j : f.cycleFactorsFinset) ↦
      ∀ (x y : Perm α), x ∈ Subgroup.zpowers ↑i → y ∈ Subgroup.zpowers ↑j → Commute x y :=
  f.pairwise'_disjoint_of_mem_zpowers.mono
...
lemma disjoint_ofSubtype_noncommPiCoprod (u : Perm (Function.fixedPoints f))
    ...
    Disjoint (ofSubtype u) ((Subgroup.noncommPiCoprod f.pairwise'_commute_of_mem_zpowers) v) := by
  apply Finset.noncommProd_induction
  · intro a _ b _ h
    apply (pairwise'_apply f.pairwise'_commute_of_mem_zpowers) h <;> simp
  · intro x y
...
lemma commute_ofSubtype_noncommPiCoprod (u : Perm (Function.fixedPoints f))
    (v : (c : { x // x ∈ f.cycleFactorsFinset }) → (Subgroup.zpowers (c : Perm α))) :
    Commute (ofSubtype u) ((Subgroup.noncommPiCoprod f.pairwise'_commute_of_mem_zpowers) v) :=
