...
instance sumCongrHom.decidableMemRange {α β : Type*} [DecidableEq α] [DecidableEq β] [Fintype α]
    [Fintype β] : DecidablePred (· ∈ range (sumCongrHom α β)) := fun _ => inferInstance
#align equiv.perm.sum_congr_hom.decidable_mem_range Equiv.Perm.sumCongrHom.decidableMemRange

@[simp]
theorem sumCongrHom.card_range {α β : Type*} [Fintype <| range (sumCongrHom α β)]
    [Fintype (Perm α × Perm β)] :
    Fintype.card (range (sumCongrHom α β)) = Fintype.card (Perm α × Perm β) :=
...
instance sigmaCongrRightHom.decidableMemRange {α : Type*} {β : α → Type*} [DecidableEq α]
    ...
    DecidablePred (· ∈ range (sigmaCongrRightHom β)) := fun _ => inferInstance
...
theorem sigmaCongrRightHom.card_range {α : Type*} {β : α → Type*}
    [Fintype <| range (sigmaCongrRightHom β)] [Fintype (∀ a, Perm (β a))] :
    Fintype.card (range (sigmaCongrRightHom β)) = Fintype.card (∀ a, Perm (β a)) :=
...
instance subtypeCongrHom.decidableMemRange {α : Type*} (p : α → Prop) [DecidablePred p]
    ...
    DecidablePred (· ∈ range (subtypeCongrHom p)) := fun _ => inferInstance
...
theorem subtypeCongrHom.card_range {α : Type*} (p : α → Prop) [DecidablePred p]
    [Fintype <| range (subtypeCongrHom p)] [Fintype (Perm { a // p a } × Perm { a // ¬p a })] :
    Fintype.card (range (subtypeCongrHom p)) =
...
noncomputable def subgroupOfMulAction (G H : Type*) [Group G] [MulAction G H] [FaithfulSMul G H] :
    G ≃* range (MulAction.toPermHom G H) :=
