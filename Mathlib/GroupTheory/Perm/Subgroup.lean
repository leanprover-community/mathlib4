...
@[simp]
lemma sumCongrHom.card_range {α β : Type*} [Fintype (sumCongrHom α β).range]
...
@[simp]
lemma sigmaCongrRightHom.card_range {α : Type*} {β : α → Type*}
...
@[simp]
lemma subtypeCongrHom.card_range {α : Type*} (p : α → Prop) [DecidablePred p]
