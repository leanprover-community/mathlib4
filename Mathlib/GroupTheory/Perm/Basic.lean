...
@[simp]
lemma swap_smul_self_smul [MulAction (Perm α) β] (i j : α) (x : β) :
    swap i j • swap i j • x = x := by simp [smul_smul]

lemma swap_smul_involutive [MulAction (Perm α) β] (i j : α) :
