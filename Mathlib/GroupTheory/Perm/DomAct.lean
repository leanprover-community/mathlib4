...
* `DomAct.mem_stabilizer_iff` proves that the stabilizer of `f : α → ι`
  in `(Equiv.Perm α)ᵈᵃ` is the set of `g : (Equiv.Perm α)ᵈᵃ` such that `f ∘ (mk.symm g) = f`.

  The natural equivalence from `stabilizer (Perm α)ᵈᵃ f` to `{ g : Perm α // p ∘ g = f }`
  can be obtained as `subtypeEquiv mk.symm (fun _ => mem_stabilizer_iff)`

* `DomAct.stabilizerMulEquiv` is the `MulEquiv` from
  the MulOpposite of this stabilizer to the product,
  for `i : ι`, of `Equiv.Perm {a // f a = i}`.

* Under `Fintype α` and `Fintype ι`, `DomAct.stabilizer_card p` computes
  the cardinality of the type of permutations preserving `p` :
  `Fintype.card {g : Perm α // f ∘ g = f} = ∏ i, (Fintype.card {a // f a = i})!`.

* Without `Fintype ι`, `DomAct.stabilizer_card' p` gives an equivalent
...
lemma mem_stabilizer_iff {g : (Perm α)ᵈᵃ} :
    g ∈ stabilizer (Perm α)ᵈᵃ f ↔ f ∘ (mk.symm g :) = f := by
...
variable (f) in
/-- The `MulEquiv` from the `MulOpposite` of `MulAction.stabilizer (Perm α)ᵈᵃ f`
  to the product of the `Equiv.Perm {a // f a = i}` -/
def stabilizerMulEquiv : (stabilizer (Perm α)ᵈᵃ f)ᵐᵒᵖ ≃* (∀ i, Perm {a // f a = i}) where
...
lemma stabilizerMulEquiv_apply (g : (stabilizer (Perm α)ᵈᵃ f)ᵐᵒᵖ) {a : α} {i : ι} (h : f a = i) :
...
theorem stabilizer_card [DecidableEq α] [DecidableEq ι] [Fintype ι] :
  ...
  rw [← Nat.card_eq_fintype_card,
    ...
    Nat.card_congr (DomAct.stabilizerMulEquiv f).toEquiv, Nat.card_pi]
