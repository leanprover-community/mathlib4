...
lemma fixingSubmonoid_fixedPoints_gc :
...
lemma fixingSubmonoid_union {s t : Set α} :
...
lemma fixingSubmonoid_iUnion {ι : Sort*} {s : ι → Set α} :
...
lemma fixedPoints_submonoid_sup {P Q : Submonoid M} :
...
lemma fixedPoints_submonoid_iSup {ι : Sort*} {P : ι → Submonoid M} :
...
lemma fixingSubgroup_fixedPoints_gc :
...
lemma fixingSubgroup_union {s t : Set α} :
...
lemma fixingSubgroup_iUnion {ι : Sort*} {s : ι → Set α} :
...
lemma fixedPoints_subgroup_sup {P Q : Subgroup M} :
    fixedPoints (↥(P ⊔ Q)) α = fixedPoints P α ∩ fixedPoints Q α :=
...
lemma fixedPoints_subgroup_iSup {ι : Sort*} {P : ι → Subgroup M} :
    fixedPoints (↥(iSup P)) α = ⋂ i, fixedPoints (P i) α :=
