...
class instead.
-/
class SMulMemClass (S : Type _) (R : outParam <| Type _) (M : Type _)
    [SMul R M] [outParam <| SetLike S M] where
...
Note that only `R` is marked as an `outParam` here, since `M` is supplied by the `SetLike`
class instead. -/
class VAddMemClass (S : Type _) (R : outParam <| Type _) (M : Type _)
    [VAdd R M] [outParam <| SetLike S M] where
