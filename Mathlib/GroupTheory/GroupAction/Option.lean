...
instance : SMul M (Option α) := fast_instance%
  ⟨fun a => Option.map <| (a • ·)⟩
...
@[to_additive]
instance [SMul M N] [IsScalarTower M N α] : IsScalarTower M N (Option α) := fast_instance%
  ⟨fun a b x => by
    cases x
    exacts [rfl, congr_arg some (smul_assoc _ _ _)]⟩

@[to_additive]
instance [SMulCommClass M N α] : SMulCommClass M N (Option α) := fast_instance%
  ⟨fun _ _ => Function.Commute.option_map <| smul_comm _ _⟩

@[to_additive]
instance [SMul Mᵐᵒᵖ α] [IsCentralScalar M α] : IsCentralScalar M (Option α) := fast_instance%
  ⟨fun a x => by
    cases x
    exacts [rfl, congr_arg some (op_smul_eq_smul _ _)]⟩

@[to_additive]
instance [FaithfulSMul M α] : FaithfulSMul M (Option α) := fast_instance%
