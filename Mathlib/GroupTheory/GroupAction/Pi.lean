...
@[to_additive]
instance (priority := 10000) smul' {g : I → Type*} [∀ i, SMul (f i) (g i)] : SMul (∀ i, f i) (∀ i : I, g i) :=
...
instance (priority := 10000) isScalarTower {α β : Type*} [SMul α β] [∀ i, SMul β <| f i]
...
@[to_additive Pi.vaddAssocClass']
instance (priority := 10000) isScalarTower' {g : I → Type*} {α : Type*} [∀ i, SMul α <| f i]
...
@[to_additive Pi.vaddAssocClass'']
instance (priority := 10000) isScalarTower'' {g : I → Type*} {h : I → Type*} [∀ i, SMul (f i) (g i)]
...
@[to_additive]
instance (priority := 10000) smulCommClass {α β : Type*} [∀ i, SMul α <| f i] [∀ i, SMul β <| f i]
...
@[to_additive]
instance (priority := 10000) smulCommClass' {g : I → Type*} {α : Type*} [∀ i, SMul α <| g i]
...
@[to_additive]
instance (priority := 10000) smulCommClass'' {g : I → Type*} {h : I → Type*} [∀ i, SMul (g i) (h i)]
...
@[to_additive]
instance (priority := 10000) isCentralScalar {α : Type*} [∀ i, SMul α <| f i] [∀ i, SMul αᵐᵒᵖ <| f i]
...
@[to_additive]
instance (priority := 10000) faithfulSMul {α : Type*} [Nonempty I] [∀ i, SMul α <| f i] [∀ i, Nonempty (f i)]
...
@[to_additive]
instance (priority := 10000) mulAction (α) {m : Monoid α} [∀ i, MulAction α <| f i] :
...
@[to_additive]
instance (priority := 10000) mulAction' {g : I → Type*} {m : ∀ i, Monoid (f i)} [∀ i, MulAction (f i) (g i)] :
...
instance (priority := 10000) smulZeroClass (α) {n : ∀ i, Zero <| f i} [∀ i, SMulZeroClass α <| f i] :
...
instance (priority := 10000) smulZeroClass' {g : I → Type*} {n : ∀ i, Zero <| g i} [∀ i, SMulZeroClass (f i) (g i)] :
  @SMulZeroClass (∀ i, f i) (∀ i : I, g i) (@Pi.instZero I g n) where
  smul_zero := by intros; ext x; exact smul_zero _
#align pi.smul_zero_class' Pi.smulZeroClass'

instance (priority := 10000) distribSMul (α) {n : ∀ i, AddZeroClass <| f i} [∀ i, DistribSMul α <| f i] :
...
instance (priority := 10000) distribSMul' {g : I → Type*} {n : ∀ i, AddZeroClass <| g i}
...
instance (priority := 10000) distribMulAction (α) {m : Monoid α} {n : ∀ i, AddMonoid <| f i}
...
instance (priority := 10000) distribMulAction' {g : I → Type*} {m : ∀ i, Monoid (f i)} {n : ∀ i, AddMonoid <| g i}
...
instance (priority := 10000) mulDistribMulAction (α) {m : Monoid α} {n : ∀ i, Monoid <| f i}
...
instance (priority := 10000) mulDistribMulAction' {g : I → Type*} {m : ∀ i, Monoid (f i)} {n : ∀ i, Monoid <| g i}
...
instance (priority := 10000) hasSMul {ι R M : Type*} [SMul R M] : SMul R (ι → M) :=
  Pi.instSMul
...
instance (priority := 10000) smulCommClass {ι α β M : Type*} [SMul α M] [SMul β M] [SMulCommClass α β M] :
