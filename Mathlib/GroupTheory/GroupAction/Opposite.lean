...
@[to_additive]
instance (priority := 10000) mulAction (R : Type*) [Monoid R] [MulAction R α] : MulAction R αᵐᵒᵖ :=
  { one_smul := fun x => unop_injective <| one_smul R (unop x)
    mul_smul := fun r₁ r₂ x => unop_injective <| mul_smul r₁ r₂ (unop x) }

instance (priority := 10000) distribMulAction (R : Type*) [Monoid R] [AddMonoid α] [DistribMulAction R α] :
...
instance (priority := 10000) mulDistribMulAction (R : Type*) [Monoid R] [Monoid α] [MulDistribMulAction R α] :
...
@[to_additive]
instance (priority := 10000) isScalarTower {M N} [SMul M N] [SMul M α] [SMul N α] [IsScalarTower M N α] :
    IsScalarTower M N αᵐᵒᵖ :=
  ⟨fun _ _ _ => unop_injective <| smul_assoc _ _ _⟩

@[to_additive]
instance (priority := 10000) smulCommClass {M N} [SMul M α] [SMul N α] [SMulCommClass M N α] : SMulCommClass M N αᵐᵒᵖ :=
  ⟨fun _ _ _ => unop_injective <| smul_comm _ _ _⟩

@[to_additive]
instance (priority := 10000) isCentralScalar (R : Type*) [SMul R α] [SMul Rᵐᵒᵖ α] [IsCentralScalar R α] :
...
instance (priority := 10000) Mul.toHasOppositeSMul [Mul α] : SMul αᵐᵒᵖ α :=
  ⟨fun c x => x * c.unop⟩
...
instance (priority := 10000) MulAction.OppositeRegular.isPretransitive {G : Type*} [Group G] :
...
@[to_additive]
instance (priority := 10000) Semigroup.opposite_smulCommClass [Semigroup α] :
...
@[to_additive]
instance (priority := 10000) Semigroup.opposite_smulCommClass' [Semigroup α] : SMulCommClass α αᵐᵒᵖ α :=
  SMulCommClass.symm _ _ _
#align semigroup.opposite_smul_comm_class' Semigroup.opposite_smulCommClass'
#align add_semigroup.opposite_vadd_comm_class' AddSemigroup.opposite_vaddCommClass'

@[to_additive]
instance (priority := 10000) CommSemigroup.isCentralScalar [CommSemigroup α] : IsCentralScalar α α :=
...
instance (priority := 10000) Monoid.toOppositeMulAction [Monoid α] :
...
@[to_additive]
instance (priority := 10000) IsScalarTower.opposite_mid {M N} [Mul N] [SMul M N] [SMulCommClass M N N] :
...
@[to_additive]
instance (priority := 10000) SMulCommClass.opposite_mid {M N} [Mul N] [SMul M N] [IsScalarTower M N N] :
...
instance (priority := 10000) LeftCancelMonoid.toFaithfulSMul_opposite [LeftCancelMonoid α] :
    FaithfulSMul αᵐᵒᵖ α :=
...
instance (priority := 10000) CancelMonoidWithZero.toFaithfulSMul_opposite [CancelMonoidWithZero α]
