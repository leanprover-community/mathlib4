...
- `DomAct M` (notation: `Mᵈᵃ`): type synonym for `Mᵐᵒᵖ`; if `M` acts on `α`,
  then `Mᵈᵃ` acts on `α → β` for any type `β`;

We also define actions of `Mᵈᵃ` on:
...
To overcome this difficulty, we introduce a type synonym `DomAct M := Mᵐᵒᵖ` (notation:
`Mᵈᵃ`). This new type carries the same algebraic structures as `Mᵐᵒᵖ` but acts on `α → β` by this
new action. So, e.g., `Mᵈᵃ` acts on `(M → M) → M` by `DomAct.mk c • F f = F (fun a ↦ c • f a)`
while `(Mᵈᵃ)ᵈᵃ` (which is isomorphic to `M`) acts on `(M → M) → M` by
`DomAct.mk (DomAct.mk c) • F f = F (fun a ↦ f (c • a))`.

### Action on bundled homomorphisms

If the action of `M` on `A` preserves some structure, then `Mᵈᵃ` acts on bundled homomorphisms from
...
The left action can be written in terms of `DomAct` as `(DomAct.mk c)⁻¹ • f`. As for higher
level dynamics objects (orbits, invariant functions etc), they coincide for the left and for the
right action, so lemmas can be formulated in terms of `DomAct`.
...
/-- If `M` acts multiplicatively/additively on `α`, then `DomAct M` acts multiplicatively/additively
on `α → β` as well as some bundled maps from `α`. This is a type synonym for `PreOpposite M`,
...
/-- Equivalence between `M` and `Mᵈᵃ`. -/
def mk : M ≃ Mᵈᵃ := MulOpposite.opEquiv
...
@[to_additive] instance [Mul Mᵐᵒᵖ] [IsRightCancelMul Mᵐᵒᵖ] : IsRightCancelMul Mᵈᵃ := ‹_›
@[to_additive] instance [Mul Mᵐᵒᵖ] [IsCancelMul Mᵐᵒᵖ] : IsCancelMul Mᵈᵃ := ‹_›

@[to_additive (attr := simp)]
lemma mk_one [One M] : mk (1 : M) = 1 := rfl

@[to_additive (attr := simp)]
lemma symm_mk_one [One M] : mk.symm (1 : Mᵈᵃ) = 1 := rfl

@[to_additive (attr := simp)]
lemma mk_mul [Mul M] (a b : M) : mk (a * b) = mk b * mk a := rfl

@[to_additive (attr := simp)]
lemma symm_mk_mul [Mul M] (a b : Mᵈᵃ) : mk.symm (a * b) = mk.symm b * mk.symm a := rfl

@[to_additive (attr := simp)]
lemma mk_inv [Inv M] (a : M) : mk (a⁻¹) = (mk a)⁻¹ := rfl

@[to_additive (attr := simp)]
lemma symm_mk_inv [Inv M] (a : Mᵈᵃ) : mk.symm (a⁻¹) = (mk.symm a)⁻¹ := rfl

@[to_additive (attr := simp)]
lemma mk_pow [Monoid M] (a : M) (n : ℕ) : mk (a ^ n) = mk a ^ n := rfl

@[to_additive (attr := simp)]
lemma symm_mk_pow [Monoid M] (a : Mᵈᵃ) (n : ℕ) : mk.symm (a ^ n) = mk.symm a ^ n := rfl

@[to_additive (attr := simp)]
lemma mk_zpow [DivInvMonoid M] (a : M) (n : ℤ) : mk (a ^ n) = mk a ^ n := rfl

@[to_additive (attr := simp)]
lemma symm_mk_zpow [DivInvMonoid M] (a : Mᵈᵃ) (n : ℤ) : mk.symm (a ^ n) = mk.symm a ^ n := rfl

variable {β α N : Type*}

@[to_additive]
instance [SMul M α] : SMul Mᵈᵃ (α → β) where
  smul c f a := f (mk.symm c • a)

@[to_additive]
theorem smul_apply [SMul M α] (c : Mᵈᵃ) (f : α → β) (a : α) : (c • f) a = f (mk.symm c • a) := rfl

@[to_additive]
instance [SMul M α] [SMul N β] : SMulCommClass Mᵈᵃ N (α → β) where
  smul_comm _ _ _ := rfl

@[to_additive]
instance [SMul M α] [SMul N β] : SMulCommClass N Mᵈᵃ (α → β) where
  smul_comm _ _ _ := rfl

@[to_additive]
instance [SMul M α] [SMul N α] [SMulCommClass M N α] : SMulCommClass Mᵈᵃ Nᵈᵃ (α → β) where
  smul_comm _ _ f := funext fun _ ↦ congr_arg f (smul_comm _ _ _).symm

@[to_additive]
instance [SMul M α] [FaithfulSMul M α] [Nontrivial β] : FaithfulSMul Mᵈᵃ (α → β) where
...
instance [SMul M α] [Zero β] : SMulZeroClass Mᵈᵃ (α → β) where
  smul_zero _ := rfl

instance {A : Type*} [SMul M α] [AddZeroClass A] : DistribSMul Mᵈᵃ (α → A) where
  smul_add _ _ _ := rfl

@[to_additive]
instance [Monoid M] [MulAction M α] : MulAction Mᵈᵃ (α → β) where
  one_smul f := funext fun _ ↦ congr_arg f (one_smul _ _)
  mul_smul _ _ f := funext fun _ ↦ congr_arg f (mul_smul _ _ _)

instance {A : Type*} [Monoid M] [MulAction M α] [AddMonoid A] : DistribMulAction Mᵈᵃ (α → A) where
  smul_zero _ := rfl
  smul_add _ _ _ := rfl

instance {A : Type*} [Monoid M] [MulAction M α] [Monoid A] : MulDistribMulAction Mᵈᵃ (α → A) where
  smul_mul _ _ _ := rfl
  smul_one _ := rfl
...
instance [DistribSMul M' A] [SMulCommClass M M' A] : SMulCommClass Mᵈᵃ M'ᵈᵃ (A →+ B) :=
  DFunLike.coe_injective.smulCommClass (fun _ _ ↦ rfl) (fun _ _ ↦ rfl)

instance [DistribSMul M' B] : SMulCommClass Mᵈᵃ M' (A →+ B) :=
  DFunLike.coe_injective.smulCommClass (fun _ _ ↦ rfl) (fun _ _ ↦ rfl)

theorem smul_addMonoidHom_apply (c : Mᵈᵃ) (f : A →+ B) (a : A) : (c • f) a = f (mk.symm c • a) :=
  rfl

@[simp]
theorem mk_smul_addMonoidHom_apply (c : M) (f : A →+ B) (a : A) : (mk c • f) a = f (c • a) := rfl

theorem coe_smul_addMonoidHom (c : Mᵈᵃ) (f : A →+ B) : ⇑(c • f) = c • ⇑f :=
  rfl
...
instance [Monoid M] [AddMonoid A] [DistribMulAction M A] [AddZeroClass B] :
    MulAction Mᵈᵃ (A →+ B) := DFunLike.coe_injective.mulAction (⇑) fun _ _ ↦ rfl

instance [Monoid M] [AddMonoid A] [DistribMulAction M A] [AddCommMonoid B] :
    DistribMulAction Mᵈᵃ (A →+ B) :=
