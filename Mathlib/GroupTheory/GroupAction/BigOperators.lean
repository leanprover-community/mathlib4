...
lemma List.smul_sum {r : α} {l : List β} : r • l.sum = (l.map (r • ·)).sum :=
...
lemma List.smul_prod {r : α} {l : List β} : r • l.prod = (l.map (r • ·)).prod :=
...
lemma Multiset.smul_sum {r : α} {s : Multiset β} : r • s.sum = (s.map (r • ·)).sum :=
...
lemma Multiset.smul_prod {r : α} {s : Multiset β} : r • s.prod = (s.map (r • ·)).prod :=
