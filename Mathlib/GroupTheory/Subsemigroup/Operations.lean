...
instance toSemigroup {M : Type*} [Semigroup M] {A : Type*} [SetLike A M] [MulMemClass A M]
    (S : A) : Semigroup S :=
  { Subtype.coe_injective.semigroup Subtype.val fun _ _ => by rfl with
    toMul := MulMemClass.mul _ }
...
instance toCommSemigroup {M} [CommSemigroup M] {A : Type*} [SetLike A M] [MulMemClass A M]
    (S : A) : CommSemigroup S :=
  { Subtype.coe_injective.commSemigroup Subtype.val fun _ _ => by rfl with
    toSemigroup := toSemigroup _ }
