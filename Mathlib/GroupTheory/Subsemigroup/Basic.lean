...
@[to_additive]
instance (priority := 10000) : SetLike (Subsemigroup M) M :=
  ⟨Subsemigroup.carrier, fun p q h => by cases p; cases q; congr⟩

@[to_additive]
instance (priority := 10000) : MulMemClass (Subsemigroup M) M where mul_mem := fun {_ _ _} => Subsemigroup.mul_mem' _
...
instance (priority := 10000) : Top (Subsemigroup M) :=
  ⟨{  carrier := Set.univ
...
instance (priority := 10000) : Bot (Subsemigroup M) :=
  ⟨{  carrier := ∅
      mul_mem' := False.elim }⟩

@[to_additive]
instance (priority := 10000) : Inhabited (Subsemigroup M) :=
  ⟨⊥⟩
...
instance (priority := 10000) : Inf (Subsemigroup M) :=
  ⟨fun S₁ S₂ =>
    { carrier := S₁ ∩ S₂
...
@[to_additive]
instance (priority := 10000) : InfSet (Subsemigroup M) :=
  ⟨fun s =>
    { carrier := ⋂ t ∈ s, ↑t
      mul_mem' := fun hx hy =>
...
instance (priority := 10000) : CompleteLattice (Subsemigroup M) :=
...
@[to_additive]
instance (priority := 10000) [hn : Nonempty M] : Nontrivial (Subsemigroup M) :=
  ⟨⟨⊥, ⊤, fun h => by
      obtain ⟨x⟩ := id hn
