...
lemma ext {S T : Subsemigroup M} (h : ∀ x, x ∈ S ↔ x ∈ T) : S = T :=
  SetLike.ext h
...
protected lemma mul_mem {x y : M} : x ∈ S → y ∈ S → x * y ∈ S :=
...
lemma subset_closure : s ⊆ closure s := fun _ hx => mem_closure.2 fun _ hS => hS hx
...
lemma closure_induction {p : M → Prop} {x} (h : x ∈ closure s) (Hs : ∀ x ∈ s, p x)
...
@[to_additive (attr := elab_as_elim) "If `s` is a dense set in an additive monoid `M`,
  ...
  `p (x + y)`."]
lemma dense_induction {p : M → Prop} (x : M) {s : Set M} (hs : closure s = ⊤) (Hs : ∀ x ∈ s, p x)
...
lemma closure_eq : closure (S : Set M) = S :=
...
lemma eqOn_closure {f g : M →ₙ* N} {s : Set M} (h : Set.EqOn f g s) :
