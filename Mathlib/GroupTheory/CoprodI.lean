...
@[simp]
lemma lift_of {N} [Monoid N] (fi : ∀ i, M i →* N) {i} (m : M i) : lift fi (of m) = fi i m := by
...
-/
lemma lift_injective_of_ping_pong : Function.Injective (lift f) := by
  classical
...
-/
lemma _root_.FreeGroup.injective_lift_of_ping_pong : Function.Injective (FreeGroup.lift a) := by
