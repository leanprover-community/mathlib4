...
def focalSubgroup : Subgroup G :=
  closure { g | ∃ x ∈ H, ∃ u : G, u * x * u⁻¹ ∈ H ∧ g = ⁅x, u⁆ }
...
lemma focalSubgroup_le_h : focalSubgroup H ≤ H := by
  ...
  rintro _ ⟨x, hxP, w, huxuP, rfl⟩
  have : w * x⁻¹ * w⁻¹ ∈ H := by
    convert H.inv_mem huxuP using 1; group
  have : x * (w * x⁻¹ * w⁻¹) ∈ H := H.mul_mem hxP this
...
lemma focalSubgroupOf_mk'_conj_eq {h : G} (hh : h ∈ H) (g : G)
  ...
  have hmem : (⟨h, hh⟩ : H)⁻¹ * ⟨g⁻¹ * h * g, hconj⟩ ∈ focalSubgroupOf H := by
    ...
    exact ⟨h⁻¹, H.inv_mem hh, g⁻¹,
      by convert H.inv_mem hconj using 1; group, by group⟩
...
lemma commutator_le_focalSubgroupOf : commutator H ≤ focalSubgroupOf H := by
  have h_comm_in_focal : ∀ (a b : H),
    ...
    subset_closure ⟨a, a.2, b,
      H.1.mul_mem (H.mul_mem b.2 a.2) (H.inv_mem b.2), by simp⟩
