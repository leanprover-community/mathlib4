...
-/
@[to_additive /--The additive version of the focal subgroup,
  generated by elements of the form `-x + (u + x - u)`. -/]
...
-/
@[to_additive /--The additive version of the focal subgroup considered as a subgroup of `H`. -/]
def focalSubgroupOf : Subgroup H :=
  (focalSubgroup H).subgroupOf H

end Subgroup

lemma focal_gen_eq_commutator (x u : G) :
    x⁻¹ * (u * x * u⁻¹) = ⁅x⁻¹, u⁆ := by
  group
...
lemma focalSubgroup_le_commutator : focalSubgroup H ≤ commutator G := by
  ...
  rw [hg_eq, focal_gen_eq_commutator x u, commutator_eq_closure]
...
lemma focalSubgroupOf_mk'_conj_eq {h : G} (hh : h ∈ H) (g : G)
  ...
  have hmem : (⟨h, hh⟩ : H)⁻¹ * ⟨g⁻¹ * h * g, hconj⟩ ∈ focalSubgroupOf H := by
    change _ ∈ (focalSubgroup H).subgroupOf H
    rw [mem_subgroupOf]
    apply subset_closure
    exact ⟨h, hh, g⁻¹, by simpa using hconj, by simp⟩
...
theorem transfer_restricted_to_p_eq_pow [Finite G] (x : H) :
  ...
  rw [MonoidHom.transfer_eq_prod_quotient_orbitRel_zpowers_quot φ x,
    Finset.prod_congr rfl (fun q _ => focalSubgroupOf_mk'_conj_eq H (H.pow_mem x.2 _) q.out.out _),
    focalSubgroup_prod_pow_eq_pow_sum H x, sum_minimalPeriod_eq_index H x]
