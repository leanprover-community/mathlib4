...
@[simp]
theorem liftOn₂_mk {α : Type*} (m m' : M) (s s' : nonZeroDivisors ℕ)
    (f : M → nonZeroDivisors ℕ → M → nonZeroDivisors ℕ → α)
    (h : ∀ (m n m' n' : M) (s t s' t' : nonZeroDivisors ℕ),
...
theorem nnqsmul_mk (a : ℚ≥0) (m : M) (s : nonZeroDivisors ℕ) :
    a • mk m s =
    mk (a.num • m) (⟨a.den, mem_nonZeroDivisors_iff_ne_zero.mpr (a.den_ne_zero)⟩ * s) := by
...
theorem mk_eq_mk_iff_smul_eq_smul {m m' : M} {s s' : nonZeroDivisors ℕ} :
...
theorem mk_left_injective (s : nonZeroDivisors ℕ) :
    Function.Injective (fun (m : M) ↦ mk m s) := by
  intro m n h
  simp_rw [mk_eq_mk_iff_smul_eq_smul, Submonoid.smul_def] at h
...
theorem mk_left_surjective (s : nonZeroDivisors ℕ) :
    Function.Surjective (fun (m : M) ↦ mk m s) := by
  intro m
  induction m using ind with | mk m t
  use DivisibleBy.div (s • m) t.val
  rw [mk_eq_mk]
  use 1
  rw [Submonoid.smul_def t, DivisibleBy.div_cancel _ (by simp)]
...
def coeAddEquiv : M ≃+ DivisibleHull M where
  ...
  right_inv m := by
    ...
    simp [mk_eq_mk_iff_smul_eq_smul, Submonoid.smul_def, DivisibleBy.div_cancel]
...
instance : SMul ℚ (DivisibleHull M) where
  smul a x := (SignType.sign a : ℤ) • (show ℚ≥0 from ⟨|a|, by simp⟩) • x

theorem qsmul_def (a : ℚ) (x : DivisibleHull M) :
    a • x = (SignType.sign a : ℤ) • (show ℚ≥0 from ⟨|a|, by simp⟩) • x :=
  rfl
