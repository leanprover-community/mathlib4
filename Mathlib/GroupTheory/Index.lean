...
@[to_additive]
lemma index_comap_of_surjective {G' : Type*} [Group G'] {f : G' →* G}
...
@[to_additive]
lemma index_comap {G' : Type*} [Group G'] (f : G' →* G) :
...
@[to_additive]
lemma relindex_comap {G' : Type*} [Group G'] (f : G' →* G) (K : Subgroup G') :
...
lemma relindex_mul_index (h : H ≤ K) : H.relindex K * K.index = H.index :=
...
@[to_additive]
lemma index_dvd_of_le (h : H ≤ K) : K.index ∣ H.index :=
...
@[to_additive]
lemma relindex_dvd_index_of_le (h : H ≤ K) : H.relindex K ∣ H.index :=
...
@[to_additive]
lemma relindex_subgroupOf (hKL : K ≤ L) :
...
lemma relindex_mul_relindex (hHK : H ≤ K) (hKL : K ≤ L) :
...
@[to_additive]
lemma inf_relindex_right : (H ⊓ K).relindex K = H.relindex K := by
...
@[to_additive]
lemma inf_relindex_left : (H ⊓ K).relindex H = K.relindex H := by
...
lemma relindex_inf_mul_relindex : H.relindex (K ⊓ L) * K.relindex L = (H ⊓ K).relindex L := by
...
@[to_additive (attr := simp)]
lemma relindex_sup_left [K.Normal] : K.relindex (K ⊔ H) = K.relindex H := by
...
@[to_additive]
lemma relindex_dvd_index_of_normal [H.Normal] : H.relindex K ∣ H.index :=
...
@[to_additive]
lemma relindex_dvd_of_le_left (hHK : H ≤ K) : K.relindex L ∣ H.relindex L :=
...
lemma index_eq_two_iff : H.index = 2 ↔ ∃ a, ∀ b, Xor' (b * a ∈ H) (b ∈ H) := by
...
@[to_additive]
lemma mul_mem_iff_of_index_two (h : H.index = 2) {a b : G} : a * b ∈ H ↔ (a ∈ H ↔ b ∈ H) := by
...
@[to_additive]
lemma mul_self_mem_of_index_two (h : H.index = 2) (a : G) : a * a ∈ H := by
...
@[to_additive (attr := simp)]
lemma index_top : (⊤ : Subgroup G).index = 1 :=
...
@[to_additive (attr := simp)]
lemma index_bot : (⊥ : Subgroup G).index = Nat.card G :=
...
@[to_additive]
lemma index_bot_eq_card [Fintype G] : (⊥ : Subgroup G).index = Fintype.card G :=
...
@[to_additive (attr := simp)]
lemma relindex_top_left : (⊤ : Subgroup G).relindex H = 1 :=
  index_top
...
@[to_additive (attr := simp)]
lemma relindex_top_right : H.relindex ⊤ = H.index := by
...
@[to_additive (attr := simp)]
lemma relindex_bot_left : (⊥ : Subgroup G).relindex H = Nat.card H := by
...
@[to_additive]
lemma relindex_bot_left_eq_card [Fintype H] : (⊥ : Subgroup G).relindex H = Fintype.card H :=
...
@[to_additive (attr := simp)]
lemma relindex_bot_right : H.relindex ⊥ = 1 := by rw [relindex, subgroupOf_bot_eq_top, index_top]
...
@[to_additive (attr := simp)]
lemma relindex_self : H.relindex H = 1 := by rw [relindex, subgroupOf_self, index_top]
#align subgroup.relindex_self Subgroup.relindex_self
#align add_subgroup.relindex_self AddSubgroup.relindex_self

@[to_additive]
lemma index_ker {H} [Group H] (f : G →* H) : f.ker.index = Nat.card (Set.range f) := by
...
@[to_additive]
lemma relindex_ker {H} [Group H] (f : G →* H) (K : Subgroup G) :
...
lemma card_mul_index : Nat.card H * H.index = Nat.card G := by
...
@[to_additive]
lemma nat_card_dvd_of_injective {G H : Type*} [Group G] [Group H] (f : G →* H)
...
@[to_additive]
lemma nat_card_dvd_of_le (hHK : H ≤ K) : Nat.card H ∣ Nat.card K :=
...
@[to_additive]
lemma nat_card_dvd_of_surjective {G H : Type*} [Group G] [Group H] (f : G →* H)
...
@[to_additive]
lemma card_dvd_of_surjective {G H : Type*} [Group G] [Group H] [Fintype G] [Fintype H]
...
@[to_additive]
lemma index_map {G' : Type*} [Group G'] (f : G →* G') :
...
@[to_additive]
lemma index_map_dvd {G' : Type*} [Group G'] {f : G →* G'} (hf : Function.Surjective f) :
...
@[to_additive]
lemma dvd_index_map {G' : Type*} [Group G'] {f : G →* G'} (hf : f.ker ≤ H) :
...
@[to_additive]
lemma index_map_eq {G' : Type*} [Group G'] {f : G →* G'} (hf1 : Function.Surjective f)
...
@[to_additive]
lemma index_eq_card [Fintype (G ⧸ H)] : H.index = Fintype.card (G ⧸ H) :=
...
@[to_additive]
lemma index_dvd_card [Fintype G] : H.index ∣ Fintype.card G := by
...
@[to_additive]
lemma index_eq_zero_of_relindex_eq_zero (h : H.relindex K = 0) : H.index = 0 :=
...
@[to_additive]
lemma index_inf_ne_zero (hH : H.index ≠ 0) (hK : K.index ≠ 0) : (H ⊓ K).index ≠ 0 := by
...
@[to_additive]
lemma relindex_inf_le : (H ⊓ K).relindex L ≤ H.relindex L * K.relindex L := by
...
@[to_additive]
lemma index_inf_le : (H ⊓ K).index ≤ H.index * K.index := by
...
@[to_additive]
lemma relindex_iInf_ne_zero {ι : Type*} [_hι : Finite ι] {f : ι → Subgroup G}
...
@[to_additive]
lemma relindex_iInf_le {ι : Type*} [Fintype ι] (f : ι → Subgroup G) :
...
@[to_additive]
lemma index_iInf_ne_zero {ι : Type*} [Finite ι] {f : ι → Subgroup G}
...
@[to_additive]
lemma index_iInf_le {ι : Type*} [Fintype ι] (f : ι → Subgroup G) :
...
lemma index_eq_one : H.index = 1 ↔ H = ⊤ :=
  ⟨fun h =>
...
lemma relindex_eq_one : H.relindex K = 1 ↔ K ≤ H :=
...
lemma card_eq_one : Nat.card H = 1 ↔ H = ⊥ :=
...
@[to_additive]
lemma index_ne_zero_of_finite [hH : Finite (G ⧸ H)] : H.index ≠ 0 := by
...
@[to_additive]
lemma finiteIndex_of_finite_quotient [Finite (G ⧸ H)] : FiniteIndex H :=
...
@[to_additive]
lemma finiteIndex_of_le [FiniteIndex H] (h : H ≤ K) : FiniteIndex K :=
