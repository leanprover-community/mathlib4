...
@[to_additive]
instance (priority := 10000) finite_quotient_of_finiteIndex [FiniteIndex H] : Finite (G ⧸ H) :=
...
@[to_additive]
instance (priority := 10000) : FiniteIndex (⊤ : Subgroup G) :=
  ⟨ne_of_eq_of_ne index_top one_ne_zero⟩

@[to_additive]
instance (priority := 10000) [FiniteIndex H] [FiniteIndex K] : FiniteIndex (H ⊓ K) :=
...
@[to_additive]
instance (priority := 10000) finiteIndex_ker {G' : Type*} [Group G'] (f : G →* G') [Finite f.range] :
    f.ker.FiniteIndex :=
...
instance (priority := 10000) finiteIndex_normalCore [H.FiniteIndex] : H.normalCore.FiniteIndex := by
...
instance (priority := 10000) finiteIndex_center [Finite (commutatorSet G)] [Group.FG G] : FiniteIndex (center G) := by
