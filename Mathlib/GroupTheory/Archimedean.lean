...
The main result is `AddSubgroup.zmultiples_eq_of_isLeast_mem_and_pos`: A subgroup of a decidable
archimedean abelian group is cyclic, if its set of positive elements has a minimal element.
...
generated by `a`. -/]
theorem Subgroup.zpowers_eq_of_isLeast_mem_and_one_lt {H : Subgroup G} {a : G}
    (ha : IsLeast { g : G | g ∈ H ∧ 1 < g } a) : zpowers a = H := by
  obtain ⟨⟨a_in, a_pos⟩, a_min⟩ := ha
  refine H.zpowers_le.mpr a_in |>.antisymm fun g g_in ↦ ?_
  ...
  have h_zero : g / (a ^ k) = 1 := by
    ...
    contradiction
  simp [div_eq_one.mp h_zero]

@[to_additive (attr := deprecated (since := "2026-08-30")) AddSubgroup.cyclic_of_min]
alias Subgroup.cyclic_of_min := Subgroup.zpowers_eq_of_isLeast_mem_and_one_lt
