...
/-!
# Subgroups of archimedean groups

This file proves a few facts about subgroups of ordered groups
which satisfy the `Archimedean` property, that is:
...
theorem eq_zpowers_of_disjoint_Ioo {H : Subgroup G} {a : G} (h₀ : 1 < a)
    (hd : Disjoint (H : Set G) (Ioo 1 a)) : ∃ b, H = zpowers b := by
  rcases eq_or_ne H ⊥ with rfl | hbot
  · exact ⟨1, zpowers_one_eq_bot.symm⟩
  · exact (exists_isLeast_one_lt hbot h₀ hd).imp fun _ ↦ eq_zpowers_of_isLeast_one_lt
