...
@[to_additive "Given a subgroup `H` of a decidable linearly ordered
archimedean abelian group `G`, if there exists a minimal element `a` of `H ∩ G_{>0}` then `H` is
generated by `a`. "]
theorem eq_zpowers_of_isLeast {H : Subgroup G} {a : G}
    (ha : IsLeast { g : G | g ∈ H ∧ 1 < g } a) : H = zpowers a := by
  obtain ⟨⟨a_in, a_pos⟩, a_min⟩ := ha
  refine le_antisymm (fun g g_in ↦ ?_) (zpowers_le.mpr a_in)
...
@[to_additive (attr := deprecated (since := "2024-11-19"))]
alias cyclic_of_min := eq_zpowers_of_isLeast
...
@[to_additive  "If an additive subgroup of a linear ordered
additive commutative group is disjoint with the interval `Set.Ioo 0 a` for some positive `a`, then
this is a cyclic subgroup."]
theorem eq_zpowers_of_isolated_one {H : Subgroup G} {a : G} (h₀ : 1 < a)
    (hd : Disjoint (H : Set G) (Ioo 1 a)) : ∃ b, H = zpowers b := by
  rcases eq_or_ne H ⊥ with rfl | hbot
  · exact ⟨1, zpowers_one_eq_bot.symm⟩
  · exact (exists_isLeast_one_lt hbot h₀ hd).imp fun _ => eq_zpowers_of_isLeast

end Subgroup

/-- Every subgroup of `ℤ` is cyclic. -/
theorem Int.addSubgroup_eq_zmultiples (H : AddSubgroup ℤ) : ∃ a, H = .zmultiples a :=
  have : Ioo (0 : ℤ) 1 = ∅ := eq_empty_of_forall_not_mem fun _ hm =>
    hm.1.not_le (lt_add_one_iff.1 hm.2)
  AddSubgroup.eq_zmultiples_of_isolated_zero one_pos <| by simp [this]
