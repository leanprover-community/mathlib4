...
theorem Fintype.finiteIndex_of_iUnion_leftCoset_eq_univ_aux
    ...
    (H : ι → { x // x ∈ s }) (hH : Function.Surjective H)
    (g : ι → G) (hG : ⋃ k, g k • (H k : Set G) = Set.univ) :
  ...
  | succ r ih =>
    ...
    · rw [Set.iUnion₂_congr fun i hi => by rw [(Finset.mem_filter.mp hi).2]] at h
    ...
    rw [Set.eq_univ_iff_forall, not_forall] at h
    obtain ⟨x, hx⟩ := h
    ...
    have : x • (C : Set G) ∩ ⋃ k ∈ Finset.univ.filter (H · = C), g k • (H k : Set G) = ∅ := by
      ...
      suffices ∀ i ∈ x • (C : Set G), ∀ (x : ι), H x = C → i ∉ g x • (H x : Set G) by
        simpa using this
      replace hx : ∀ i, H i = C → x ∉ g i • (H i : Set G) := by simpa using hx
      intro y hy₁ i hi hy₂
      apply hx i hi
    ...
    let f : κ → G
    | Sum.inl ⟨k, i⟩ => g k * x⁻¹ * g i
    | Sum.inr i => g i
    let K : κ → { x // x ∈ s.erase ↑C }
    | Sum.inl ⟨_, i⟩ => ⟨H i, hκ₁ i⟩
    | Sum.inr i => ⟨H i, hκ₁ i⟩
...
theorem Fintype.finiteIndex_of_iUnion_leftCoset_eq_univ (G ι : Type*) [Group G] [Fintype ι]
  ...
  let H' (i : ι) : { x // x ∈ s } := ⟨Set.rangeFactorization H i, by
