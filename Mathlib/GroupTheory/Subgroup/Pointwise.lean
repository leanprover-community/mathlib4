...
theorem closure_induction_left {p : (x : G) → x ∈ closure s → Prop} (H1 : p 1 (one_mem _))
...
theorem closure_induction_right {p : (x : G) → x ∈ closure s → Prop} (H1 : p 1 (one_mem _))
  ...
  | mul_right x hx y hy ih =>
    cases hy with
    | inl hy => exact mul_right _ hx _ hy ih
    | inr hy => simpa only [inv_inv] using mul_right_inv _ hx _ hy ih
