...
def zpowers (g : G) : Subgroup G :=
  Subgroup.copy (MonoidHom.range (zpowersHom G g)) (Set.range (g ^ · : ℤ → G)) rfl
...
theorem range_zpowersHom (g : G) : MonoidHom.range (zpowersHom G g) = zpowers g :=
  rfl
...
def zmultiples (a : A) : AddSubgroup A :=
  AddSubgroup.copy (AddMonoidHom.range (zmultiplesHom A a)) (Set.range ((· • a) : ℤ → A)) rfl
#align add_subgroup.zmultiples AddSubgroup.zmultiples

@[simp]
theorem range_zmultiplesHom (a : A) : AddMonoidHom.range (zmultiplesHom A a) = zmultiples a :=
  rfl
...
@[simp] lemma Int.range_castAddHom {A : Type*} [AddGroupWithOne A] :
    AddMonoidHom.range (Int.castAddHom A) = AddSubgroup.zmultiples 1 := by
  ext a
...
theorem MonoidHom.map_zpowers (f : G →* N) (x : G) :
  ...
  rw [Subgroup.zpowers_eq_closure, Subgroup.zpowers_eq_closure, map_closure f, Set.image_singleton]
