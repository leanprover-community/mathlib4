...
@[to_additive (attr := aesop safe apply (rule_sets [SetLike])) "An additive subgroup is closed under subtraction."]
...
@[to_additive (attr := simp, aesop safe 20 apply (rule_sets [SetLike])) "The `AddSubgroup` generated by a set includes the set."]
