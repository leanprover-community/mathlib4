...
theorem subset_closure : k ⊆ closure k := fun x hx => mem_closure.2 fun _ hK => hK hx
...
@[to_additive (attr := simp) "An additive subgroup `K` includes `closure k` if and only if it includes `k`"]
