...
noncomputable def equivPair (i) : NormalWord d ≃ Pair d i :=
  ...
  haveI leftInv : Function.LeftInverse (rcons i) toFun :=
    fun w => ext_smul i <| by
      simp +zetaDelta only [rcons, Word.equivPair_symm,
  ...
  le_antisymm
    (by
      ...
      have hw : Reduced φ w := by
        simp +zetaDelta only [not_exists, ne_eq, Reduced, List.find?, List.mem_cons,
      ...
      have := hw.eq_empty_of_mem_range hφ (by
        simp +zetaDelta only [Word.prod, List.map_cons, List.prod_cons, List.prod_nil,
