...
alias Subgroup.mem_closure_singleton_iff_existsUnique_zpow :=
  Subgroup.mem_zpowers_iff_existsUnique_zpow
...
@[to_additive (attr := deprecated (since := "2026-08-30")) AddSubgroup.cyclic_of_isolated_zero]
alias Subgroup.cyclic_of_isolated_one := Subgroup.isCyclic_of_disjoint_Ioo_one
...
is isomorphic (and order-isomorphic) to the closure of an element in the other group. -/]
noncomputable def LinearOrderedCommGroup.zpowersOrderMonoidIso {G G' : Type*}
  ...
  if hx : x = 1 then by
    ...
    · intros
      simp
    · intro ⟨a, ha⟩ ⟨b, hb⟩
      rw [hx, zpowers_one_eq_bot, mem_bot] at ha hb
      simp [ha, hb]
  else by
    ...
    have hxc : zpowers x = zpowers x' := by rcases max_cases x x⁻¹ <;> simp_all
    have hyc : zpowers y = zpowers y' := by rcases max_cases y y⁻¹ <;> simp_all
    have Hx : ∀ (z : zpowers x), z ∈ zpowers ⟨x', by simp [hxc]⟩ := by
      simp_rw [Subtype.forall, hxc, mem_zpowers_iff, Subtype.ext_iff, SubgroupClass.coe_zpow,
        imp_self, implies_true]
    have Hy : ∀ (z : zpowers y), z ∈ zpowers ⟨y', by simp [hyc]⟩ := by
      simp_rw [Subtype.forall, hyc, mem_zpowers_iff, Subtype.ext_iff, SubgroupClass.coe_zpow,
        imp_self, implies_true]
...
@[to_additive (attr := deprecated (since := "2026-08-30"))]
alias LinearOrderedCommGroup.closure_equiv_closure := LinearOrderedCommGroup.zpowersOrderMonoidIso
...
lemma Subgroup.isLeast_of_closure_iff_eq_mabs {a b : G} :
  ...
  constructor <;> intro h
  · have := Subgroup.zpowers_eq_of_isLeast_mem_and_one_lt h
    have ha : a ∈ zpowers b := by simp [this]
    obtain ⟨n, rfl⟩ := ha
    obtain ⟨m, hm⟩ := h.left.left
    beta_reduce at hm
    have key : m * n = 1 := by
      rw [← zpow_right_inj h.left.right, zpow_mul', hm, zpow_one]
    rw [Int.mul_eq_one_iff_eq_one_or_neg_one] at key
    rw [eq_comm]
    rcases key with ⟨rfl, rfl⟩ | ⟨rfl, rfl⟩ <;>
    simp [h.left.right.le, h.left.right, mabs]
...
noncomputable def LinearOrderedAddCommGroup.int_orderAddMonoidIso_of_isLeast_pos {G : Type*}
  ...
  replace this := AddSubgroup.zmultiples_eq_of_isLeast_mem_and_pos this
  ...
  let e' : (⊤ : AddSubgroup G) ≃+o AddSubgroup.zmultiples x :=
    ⟨AddEquiv.subsemigroupCongr (by simp [this]),
      (AddEquiv.strictMono_subsemigroupCongr _).le_iff_le⟩
  let g : (⊤ : AddSubgroup ℤ) ≃+o ℤ :=
    ⟨AddSubsemigroup.topEquiv, (AddSubsemigroup.strictMono_topEquiv).le_iff_le⟩
  let g' : AddSubgroup.zmultiples (1 : ℤ) ≃+o (⊤ : AddSubgroup ℤ) :=
    ⟨(.subsemigroupCongr (by simp)), (AddEquiv.strictMono_subsemigroupCongr _).le_iff_le⟩
  let f := zmultiplesOrderAddMonoidIso x (1 : ℤ) (by simp [h.left.ne'])
