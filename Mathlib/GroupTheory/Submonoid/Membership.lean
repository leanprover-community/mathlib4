...
instance (priority := 100) instGroupOfPowers {x : M} {n : ℕ} (hpos : 0 < n) (hx : x ^ n = 1) :
    Group (powers x) where
  inv := fun ⟨x, hx⟩ => ⟨x ^ (n - 1), by
    rw [Submonoid.mem_powers_iff] at hx ⊢
    obtain ⟨k, hk⟩ := hx
    use k * (n - 1)
    rw [← hk, pow_mul]⟩
  mul_left_inv := fun ⟨y, hy⟩ => by
    simp only [ge_iff_le, Submonoid.mk_mul_mk]
    congr
    rw [Submonoid.mem_powers_iff] at hy
    obtain ⟨k, hk⟩ := hy
    rw [← pow_succ', Nat.sub_add_cancel hpos, ← hk, ← pow_mul, mul_comm, pow_mul, hx, one_pow]
