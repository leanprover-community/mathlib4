...
abbrev groupOfPowers {x : M} {n : ℕ} (hpos : 0 < n) (hx : x ^ n = 1) : Group (powers x) where
  ...
  zpow_neg' m x := Subtype.ext <| by
    ...
    rw [mul_comm, pow_mul, ← pow_eq_pow_mod _ hx, mul_comm k, mul_assoc, pow_mul _ ((m + 1) % n),
      ← pow_eq_pow_mod _ hx, pow_mul, pow_mul]
  zpow_succ' m x := Subtype.ext <| by
    obtain ⟨_, k, rfl⟩ := x
    simp [← pow_mul, Int.natMod]; norm_cast
...
attribute [to_additive addGroupOfMultiples "The additive submonoid generated by an element is
an additive group if that element has finite order."] Submonoid.groupOfPowers
