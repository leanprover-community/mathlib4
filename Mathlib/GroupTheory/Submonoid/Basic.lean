...
instance : Top (Submonoid M) := fast_instance%
  ⟨{  carrier := Set.univ
...
instance : Bot (Submonoid M) := fast_instance%
  ⟨{  carrier := {1}
...
@[to_additive]
instance : Inhabited (Submonoid M) := fast_instance%
  ⟨⊥⟩
...
instance : Inf (Submonoid M) := fast_instance%
  ⟨fun S₁ S₂ =>
    { carrier := S₁ ∩ S₂
...
@[to_additive]
instance : InfSet (Submonoid M) := fast_instance%
  ⟨fun s =>
    { carrier := ⋂ t ∈ s, ↑t
...
instance : CompleteLattice (Submonoid M) := fast_instance%
...
@[to_additive]
instance [Subsingleton M] : Unique (Submonoid M) := fast_instance%
  ⟨⟨⊥⟩, fun a => @Subsingleton.elim _ (subsingleton_iff.mpr ‹_›) a _⟩

@[to_additive]
instance [Nontrivial M] : Nontrivial (Submonoid M) := fast_instance%
