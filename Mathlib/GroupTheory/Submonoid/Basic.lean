...
@[to_additive]
instance (priority := 10000) : SetLike (Submonoid M) M where
  coe s := s.carrier
  coe_injective' p q h := by cases p; cases q; congr; exact SetLike.coe_injective' h

@[to_additive]
instance (priority := 10000) : SubmonoidClass (Submonoid M) M where
...
instance (priority := 10000) : Top (Submonoid M) :=
  ⟨{  carrier := Set.univ
...
instance (priority := 10000) : Bot (Submonoid M) :=
  ⟨{  carrier := {1}
...
@[to_additive]
instance (priority := 10000) : Inhabited (Submonoid M) :=
  ⟨⊥⟩
...
instance (priority := 10000) : Inf (Submonoid M) :=
  ⟨fun S₁ S₂ =>
    { carrier := S₁ ∩ S₂
...
@[to_additive]
instance (priority := 10000) : InfSet (Submonoid M) :=
  ⟨fun s =>
    { carrier := ⋂ t ∈ s, ↑t
...
instance (priority := 10000) : CompleteLattice (Submonoid M) :=
...
@[to_additive]
instance (priority := 10000) [Subsingleton M] : Unique (Submonoid M) :=
  ⟨⟨⊥⟩, fun a => @Subsingleton.elim _ (subsingleton_iff.mpr ‹_›) a _⟩

@[to_additive]
instance (priority := 10000) [Nontrivial M] : Nontrivial (Submonoid M) :=
