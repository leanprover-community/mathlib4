...
@[to_additive (attr := ext) "Two `add_submonoid`s are equal if they have the same elements."]
...
@[to_additive (attr := simp) "The `add_submonoid` generated by a set includes the set."]
