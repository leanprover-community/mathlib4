...
structure _root_.Pushout.NormalWord (d : Data φ) extends CoprodI.Word G where
  left : H
...
noncomputable instance mulAction [DecidableEq ι] [∀ i, DecidableEq (G i)] :
    MulAction (Pushout φ) (NormalWord d) :=
  MulAction.ofEndHom <|
    lift
...
def prod (w : NormalWord d) : Pushout φ :=
...
@[simp]
theorem prod_smul (g : Pushout φ) (w : NormalWord d) :
...
noncomputable def equiv : Pushout φ ≃ NormalWord d :=
  { toFun := fun g => g • .empty
    invFun := fun w => w.prod
    left_inv := fun g => by
...
theorem prod_injective : Function.Injective (prod : NormalWord d → Pushout φ) := by
  letI := Classical.decEq ι
  letI := fun i => Classical.decEq (G i)
  classical exact equiv.symm.injective

instance : FaithfulSMul (Pushout φ) (NormalWord d) :=
...
theorem of_injective (hφ : ∀ i, Function.Injective (φ i)) (i : ι) :
  ...
  refine Function.Injective.of_comp
    (f := ((. • .) : Pushout φ → NormalWord d → NormalWord d)) ?_
  intros _ _ h
