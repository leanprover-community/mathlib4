...
instance : Subgroup.Normal (commutator G) := fast_instance% Subgroup.commutator_normal ⊤ ⊤
...
instance commutator_characteristic : (commutator G).Characteristic := fast_instance%
  Subgroup.commutator_characteristic ⊤ ⊤
#align commutator_characteristic commutator_characteristic

instance [Finite (commutatorSet G)] : Group.FG (commutator G) := fast_instance% by
...
instance commGroup : CommGroup (Abelianization G) := fast_instance%
...
instance : Inhabited (Abelianization G) := fast_instance%
  ⟨1⟩

instance [Unique G] : Unique (Abelianization G) := fast_instance% Quotient.instUniqueQuotient _

instance [Fintype G] [DecidablePred (· ∈ commutator G)] : Fintype (Abelianization G) := fast_instance%
  QuotientGroup.fintype (commutator G)

instance [Finite G] : Finite (Abelianization G) := fast_instance%
  Quotient.finite _
...
instance [Finite (commutatorSet G)] : Finite (commutatorRepresentatives G) := fast_instance%
