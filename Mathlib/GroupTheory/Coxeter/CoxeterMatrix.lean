...
*Coxeter matrix* if its diagonal entries are equal to 1 and its off-diagonal entries are not equal
to 1. In this file, we define the matrices `Aₙ` (`n : ℕ`), `Bₙ` (`n : ℕ`), `Dₙ` (`n : ℕ`),
`I₂ₘ` (`m : ℕ`), `E₆`, `E₇`, `E₈`, `F₄`, `G₂`, `H₃`, and `H₄`. We prove that they are Coxeter
matrices.

Up to reindexing, these are exactly the Coxeter matrices whose corresponding Coxeter group is finite
and irreducible. However, we do not prove that in this file.
...
theorem isCoxeter_Aₙ : (Aₙ n).IsCoxeter where
  symmetric := by
...
theorem isCoxeter_Bₙ : (Bₙ n).IsCoxeter where
...
theorem isCoxeter_Dₙ : (Dₙ n).IsCoxeter where
  symmetric := by simp [Matrix.IsSymm]; aesop
...
theorem isCoxeter_I₂ₘ (m : ℕ) : (I₂ₘ m).IsCoxeter where
  symmetric := by simp [Matrix.IsSymm]; aesop
...
theorem isCoxeter_E₆ : E₆.IsCoxeter where
...
theorem isCoxeter_F₄ : F₄.IsCoxeter where
  symmetric := by simp [Matrix.IsSymm]; decide
  diagonal := by decide
  off_diagonal := by decide
...
theorem isCoxeter_G₂ : G₂.IsCoxeter where
  symmetric := by simp [Matrix.IsSymm]; decide
  diagonal := by decide
  off_diagonal := by decide
...
theorem isCoxeter_H₃ : H₃.IsCoxeter where
  symmetric := by simp [Matrix.IsSymm]; decide
  diagonal := by decide
  off_diagonal := by decide
