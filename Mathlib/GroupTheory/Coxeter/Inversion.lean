...
theorem getD_rightInvSeq_mul_self (ω : List B) (j : ℕ) :
  ...
  · rw [getElem?_eq_getElem hj]
    simp [← mul_assoc]
  · rw [getElem?_eq_none_iff.mpr (by grind)]
    simp
...
theorem IsReduced.nodup_rightInvSeq {ω : List B} (rω : cs.IsReduced ω) : List.Nodup (ris ω) := by
  ...
  rw [getElem?_eq_getElem (by simp; grind), getElem?_eq_getElem (by simp; grind)] at dup
  ...
  have h₃ : t' = (ris ω).getD j' 1                    := by
    rw [h₂, cs.getD_rightInvSeq, cs.getD_rightInvSeq,
      (Nat.sub_add_cancel (by grind) : j' - 1 + 1 = j'), eraseIdx_eq_take_drop_succ,
      ...
      ← add_assoc, Nat.sub_add_cancel (by grind), mul_left_inj, mul_right_inj]
    congr 2
...
lemma getElem_succ_leftInvSeq_alternatingWord
    ...
    MulAut.conj (s i) ((lis (alternatingWord j i (2 * p)))[k]'(by simp; grind)) := by
  rw [cs.getElem_leftInvSeq (alternatingWord i j (2 * p)) (k + 1) (by simp [h]),
    cs.getElem_leftInvSeq (alternatingWord j i (2 * p)) k (by simp; grind)]
...
theorem getElem_leftInvSeq_alternatingWord
    (i j : B) (p k : ℕ) (h : k < 2 * p) :
    (lis (alternatingWord i j (2 * p)))[k]'(by simp; grind) =
  ...
  | succ k hk =>
    simp only [getElem_succ_leftInvSeq_alternatingWord cs i j p k h, hk _ _ (by grind),
