...
theorem not_isReduced_alternatingWord (i i' : B) {m : ℕ} (hM : M i i' ≠ 0) (hm : m > M i i') :
  ...
  | refl => -- Base case; m = M i i' + 1
    ...
    have : M i i' * 2 - (M i i' + 1) = M i i' - 1 := by grind
    rw [this]
    calc
...
theorem isLeftDescent_iff {w : W} {i : B} :
  ...
  constructor
  · intro _
    exact (cs.length_simple_mul w i).resolve_left (by grind)
  · grind
...
theorem not_isLeftDescent_iff {w : W} {i : B} :
  ...
  constructor
  · intro _
    exact (cs.length_simple_mul w i).resolve_right (by grind)
  · grind
...
theorem isRightDescent_iff {w : W} {i : B} :
  ...
  constructor
  · intro _
    exact (cs.length_mul_simple w i).resolve_left (by grind)
  · grind
...
theorem not_isRightDescent_iff {w : W} {i : B} :
  ...
  constructor
  · intro _
    exact (cs.length_mul_simple w i).resolve_right (by grind)
  · grind
