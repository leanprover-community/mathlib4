...
lemma getElem_alternatingWord_swapIndices (i j : B) (p k : ℕ) (h : k + 1 < p) :
  ...
  rw [getElem_alternatingWord i j p (k+1) (by grind), getElem_alternatingWord j i p k (by grind)]
...
lemma listTake_alternatingWord (i j : B) (p k : ℕ) (h : k < 2 * p) :
  ...
  induction k with
    ...
    | succ k h' =>
      have hk : k < 2 * p := by grind
      apply h' at hk
      by_cases h_even : Even k
      · simp only [h_even, ↓reduceIte] at hk
        simp only [Nat.not_even_iff_odd.mpr (Even.add_one h_even), ↓reduceIte]
        rw [← List.take_concat_get (by simp; grind), alternatingWord_succ, ← hk]
        apply congr_arg
        rw [getElem_alternatingWord i j (2*p) k (by grind)]
...
lemma listTake_succ_alternatingWord (i j : B) (p : ℕ) (k : ℕ) (h : k + 1 < 2 * p) :
  ...
  rw [listTake_alternatingWord j i p k (by grind), listTake_alternatingWord i j p (k+1) h]
  by_cases h_even : Even k
