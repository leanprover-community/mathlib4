...
* `CoxeterMatrix.typeAToPermHom`: The surjective homomorphism from the type A Coxeter group
  to the symmetric group.

## Main results

* `swapFun_isLiftable`: Adjacent transpositions satisfy the Coxeter relations.
* `typeAToPermHom_surjective`: The homomorphism is surjective.
...
theorem swapFun_isLiftable (n : ℕ) :
    (CoxeterMatrix.A n).IsLiftable (swapFun n) := by
  intro i j
  by_cases hij : i = j
  ...
  · -- Adjacent case: M i j = 3, need (swap i * swap j)^3 = 1
    have hM : (CoxeterMatrix.A n).M i j = 3 := A_adjacent _ i j hadj
    rw [hM]
    apply mul_pow_three_eq_one_of_braid <;> grind [pow_two]
  · -- Far case: M i j = 2, need (swap i * swap j)^2 = 1
    have hM : (CoxeterMatrix.A n).M i j = 2 := A_not_adjacent _ i j hij
        (by grind) (by grind)
    rw [hM]
    have hcomm : Commute (swapFun n i) (swapFun n j) :=
      swap_mul_swap_comm_of_disjoint
        (by simp only [ne_eq, Fin.ext_iff, Fin.val_castSucc]; omega)
        (by simp only [ne_eq, Fin.ext_iff, Fin.val_castSucc, Fin.val_succ]; omega)
        (by simp only [ne_eq, Fin.ext_iff, Fin.val_castSucc, Fin.val_succ]; omega)
        (by simp only [ne_eq, Fin.ext_iff, Fin.val_succ]; omega)
    exact hcomm.mul_pow_eq_one (by grind [pow_two]) (by grind [pow_two])
...
def typeAToPermHom (n : ℕ) : (CoxeterMatrix.A n).Group →* Perm (Fin (n + 1)) :=
  (CoxeterMatrix.A n).toCoxeterSystem.lift ⟨swapFun n, swapFun_isLiftable n⟩

@[simp]
theorem typeAToPermHom_simple (n : ℕ) (i : Fin n) :
    typeAToPermHom n ((CoxeterMatrix.A n).simple i) = swapFun n i :=
  CoxeterSystem.lift_apply_simple _ _ i

/-- The homomorphism from the type A Coxeter group to the symmetric group is surjective. -/
theorem typeAToPermHom_surjective (n : ℕ) : Function.Surjective (typeAToPermHom n) := by
  intro τ
  ...
  | mem y hy =>
    obtain ⟨i, rfl⟩ := hy
    use (CoxeterMatrix.A n).simple i
    simp [swapFun]
  | one =>
    use 1
    simp
