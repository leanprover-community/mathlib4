...
theorem isNilpotent_of_product_of_sylow_group
    (e : (∀ p : (Fintype.card G).primeDivisors, ∀ P : Sylow p G, (↑P : Subgroup G)) ≃* G) :
    IsNilpotent G := by
  classical
    let ps := (Fintype.card G).primeDivisors
    have : ∀ (p : ps) (P : Sylow p G), IsNilpotent (↑P : Subgroup G) := by
      intro p P
      haveI : Fact (Nat.Prime ↑p) := Fact.mk (Nat.prime_of_mem_primeDivisors (Finset.coe_mem p))
