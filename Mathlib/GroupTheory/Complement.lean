...
@[to_additive]
lemma isComplement_univ_singleton {g : G} : IsComplement (univ : Set G) {g} :=
...
@[to_additive]
lemma isComplement_singleton_univ {g : G} : IsComplement ({g} : Set G) univ :=
...
@[to_additive]
lemma isComplement_singleton_left {g : G} : IsComplement {g} S ↔ S = univ := by
  refine'
...
@[to_additive]
lemma isComplement_singleton_right {g : G} : IsComplement S {g} ↔ S = univ := by
  refine'
...
@[to_additive]
lemma isComplement_univ_left : IsComplement univ S ↔ ∃ g : G, S = {g} := by
  refine'
...
@[to_additive]
lemma isComplement_univ_right : IsComplement S univ ↔ ∃ g : G, S = {g} := by
  refine'
    ⟨fun h => Set.exists_eq_singleton_iff_nonempty_subsingleton.mpr ⟨_, fun a ha b hb => _⟩, _⟩
  · obtain ⟨a, _⟩ := h.2 1
    exact ⟨a.1.1, a.1.2⟩
