...
theorem Group.nat_card_center_add_sum_card_noncenter_eq_card [Finite G] :
  ...
  rintro ⟨g⟩ hg
  simp (config := { instances := true }) only [noncenter, Set.not_subsingleton_iff,
    Set.toFinset_setOf, Finset.mem_univ, true_and, forall_true_left, Finset.mem_sdiff,
    Finset.mem_filter, Set.not_nontrivial_iff] at hg
