...
TODO: write the rest of this docstrnig copying (and/or referenceing) `NoncommPiCoprod`
-/
...
theorem noncommCoprod_unique (f : M × N →* P) :
    (f.comp (inl M N)).noncommCoprod (f.comp (inr M N)) (fun _ _ => (inl_inr_commute _ _).map f)
      = f :=
  ext fun x => by simp [coprod_apply, inl_apply, inr_apply, ← map_mul]

@[to_additive (attr := simp)]
theorem noncommCoprod_inl_inr {M N : Type*} [Monoid M] [Monoid N]:
    (inl M N).noncommCoprod (inr M N) (inl_inr_commute (M := M) (N := N)) = id (M × N) :=
