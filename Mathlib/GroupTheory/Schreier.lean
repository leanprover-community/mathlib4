...
lemma closure_mul_image_eq (hR : R ∈ rightTransversals (H : Set G)) (hR1 : (1 : G) ∈ R)
...
/-- **Schreier's Lemma**: If `R : Set G` is a `rightTransversal` of `H : Subgroup G`
  with `1 ∈ R`, and if `G` is generated by `S : Set G`, then `H` is generated by the `Set`
  `(R * S).image (fun g ↦ g * (toFun hR g)⁻¹)`. -/
lemma closure_mul_image_eq_top (hR : R ∈ rightTransversals (H : Set G)) (hR1 : (1 : G) ∈ R)
...
/-- **Schreier's Lemma**: If `R : Finset G` is a `rightTransversal` of `H : Subgroup G`
  ...
  `(R * S).image (fun g ↦ g * (toFun hR g)⁻¹)`. -/
lemma closure_mul_image_eq_top' [DecidableEq G] {R S : Finset G}
...
lemma card_commutator_dvd_index_center_pow [Finite (commutatorSet G)] :
...
/-- A theorem of Schur: The size of the commutator subgroup is bounded in terms of the number of
  commutators. -/
lemma card_commutator_le_of_finite_commutatorSet [Finite (commutatorSet G)] :
