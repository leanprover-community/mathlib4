...
- `closure_mul_image_eq` : **Schreier's Lemma**: If `R : Set G` and `H : Subgroup G` are complement
  with `1 ∈ R`, and if `G` is generated by `S : Set G`,
...
/-- **Schreier's Lemma**: If `R : Set G` and `H : Subgroup G` are complement with `1 ∈ R`, and if
...
/-- **Schreier's Lemma**: If `R : Finset G` and `H : Subgroup G` are complement with `1 ∈ R`, and
