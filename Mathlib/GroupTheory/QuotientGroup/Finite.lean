...
@[to_additive (attr := implicit_reducible) /-- If `ker(G →+ H)` and `H` are finite, then `G` is finite. -/]
...
@[to_additive (attr := implicit_reducible) /-- If `F` and `coker(F →+ G)` are finite, then `G` is finite. -/]
