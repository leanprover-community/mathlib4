...
noncomputable def quotientInfEquivProdNormalizerQuotient (H N : Subgroup G)
    (hLE : H ≤ N.normalizer)
    (hNinf : (N.subgroupOf H).Normal := Subgroup.inf_subgroupOf_normal_of_le_normalizer hLE)
    (hNsup : (N.subgroupOf (H ⊔ N)).Normal := Subgroup.subgroupOf_sup_normal_of_le_normalizer hLE) :
    _ ⧸ N.subgroupOf H ≃* _ ⧸ N.subgroupOf (H ⊔ N) :=
  let
