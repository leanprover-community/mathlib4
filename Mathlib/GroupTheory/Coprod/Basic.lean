...
theorem mk_of_inv_mul : ∀ x : G ⊕ H, mk (of (x.map Inv.inv Inv.inv)) * mk (of x) = 1
  | Sum.inl _ => map_mul_eq_one inl (inv_mul_self _)
  | Sum.inr _ => map_mul_eq_one inr (inv_mul_self _)

@[to_additive]
theorem con_inv_mul_self (x : FreeMonoid (G ⊕ H)) :
...
instance : Inv (G ∗ H) where
  inv := Quotient.map' (fun w => ofList (w.toList.map (Sum.map Inv.inv Inv.inv)).reverse) fun _ _ ↦
    (coprodCon G H).map_of_mul_left_rel_one _ con_inv_mul_self
...
instance : Group (G ∗ H) where
  inv_mul_self := mk_surjective.forall.2 fun x => mk_eq_mk.2 (con_inv_mul_self x)
