import Mathlib.GroupTheory.FreeProd.Basic

open Function List

/-- An alternative implementation of `FreeSum` that uses fully cancelled words as the data type. -/
structure FreeSum.Word (M N : Type _) [AddMonoid M] [AddMonoid N] where
  toList : List (M ⊕ N)
...
@[simps symm_apply] def to_word : M ⋆ N ≃* M ⋆ʷ N :=
{ to_fun := clift
...
@[simp] lemma of_word_inl (x : M) : to_word.symm (word.inl x : M ⋆ʷ N) = inl x := of_word_of _
@[simp] lemma of_word_inr (x : N) : to_word.symm (word.inr x : M ⋆ʷ N) = inr x := of_word_of _

namespace word

def lift (f : M →* P) (g : N →* P) : M ⋆ʷ N →* P :=
(lift f g).comp (to_word : M ⋆ N ≃* M ⋆ʷ N).symm.to_monoid_hom
