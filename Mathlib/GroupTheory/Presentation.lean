...
structure Group.Presentation (G : Type*) [Group G] (ι : Type*)
    extends Group.Generators G ι where
...
def ofFreeGroupHom (φ : FreeGroup ι →* G) (hφ : Function.Surjective φ) (rel : Set (FreeGroup ι))
  ...
  rel := rel
  lift_ker_eq_normalClosure := by rw [Generators.ofFreeGroupHom_lift]; exact hrel
...
@[simp]
lemma ofFreeGroupHom_rel (φ : FreeGroup ι →* G) (hφ : Function.Surjective φ)
    (rel : Set (FreeGroup ι)) (hrel : φ.ker = Subgroup.normalClosure rel) :
    (Group.Presentation.ofFreeGroupHom φ hφ rel hrel).rel = rel := rfl
...
theorem lift_rel_eq_one {r : FreeGroup ι} (hr : r ∈ P.rel) : P.lift r = 1 := by
  rw [← MonoidHom.mem_ker, Generators.lift, lift_ker_eq_normalClosure]
