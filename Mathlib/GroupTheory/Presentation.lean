...
* `Group.Presentation.isFinitelyPresented` and
  `Group.isFinitelyPresented_iff_exists_finite_presentation`: a finite presentation witnesses
...
* Finiteness is expressed by instance arguments rather than bundled fields: a generating family is
  finite when `[Finite α]`, a presentation when `[Finite α] [Finite P.rel]`.
* `Group.Presentation.symmRel` symmetrises the relators, and both signs are genuinely needed:
  the conjugates of `rel` alone generate only a submonoid of `FreeGroup α`, whereas the kernel
  `Subgroup.normalClosure P.rel` that they must exhaust is a subgroup.
...
lemma lift_eq_one_of_mem_symmRel {x : FreeGroup α} (hx : x ∈ P.symmRel) : P.lift x = 1 := by
  rcases hx with h | h
  · exact P.lift_eq_one_of_mem_rel h
  · simpa using P.lift_eq_one_of_mem_rel (Set.mem_inv.mp h)
...
theorem isFinitelyPresented [Finite α] (P : Group.Presentation G α) [Finite P.rel] :
...
theorem Group.isFinitelyPresented_iff_exists_finite_presentation :
    Group.IsFinitelyPresented G ↔
      ∃ (α : Type) (_ : Finite α) (P : Group.Presentation G α), P.rel.Finite := by
  refine ⟨fun h ↦ ?_, fun ⟨_, _, P, hP⟩ ↦ have := hP.to_subtype; P.isFinitelyPresented⟩
  obtain ⟨n, φ, hφ, s, hs, hsφ⟩ := h.out
  obtain ⟨v, rfl⟩ := FreeGroup.lift.surjective φ
  exact ⟨Fin n, inferInstance,
    { toGenerators := Group.Generators.ofLiftSurjective v hφ
      rel := s
      ker_eq_normalClosure := hsφ.symm }, hs⟩
