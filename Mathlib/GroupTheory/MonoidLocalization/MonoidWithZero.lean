...
theorem map_eq_zero_iff (f : LocalizationMap S N) {m : M} : f.toMap m = 0 ↔ ∃ s : S, s * m = 0 := by
  simp_rw [← f.map_zero, eq_iff_exists, mul_zero]

theorem nonZeroDivisors_le_comap (f : LocalizationMap S N) :
    nonZeroDivisors M ≤ (nonZeroDivisors N).comap f.toMap := by
...
theorem map_nonZeroDivisors_le (f : LocalizationMap S N) :
    (nonZeroDivisors M).map f.toMap ≤ nonZeroDivisors N :=
