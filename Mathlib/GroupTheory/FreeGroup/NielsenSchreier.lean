...
theorem loopOfHom_eq_id {a b : Generators G} (e) (H : T a b (Sum.inl e) ∨ T b a (Sum.inr e)) :
...
lemma endIsFree : IsFreeGroup (End (root' T)) :=
  IsFreeGroup.ofUniqueLift
    ((wideSubquiverEquivSetTotal <| wideSubquiverSymmetrify (Generators G) T)ᶜ : Set _)
    (fun e => loopOfHom T (of e.val.hom))
    (by
      intro X _ f
      let f' : Labelling (Generators G) X := fun a b e =>
        if h : T a b (Sum.inl e) ∨ T b a (Sum.inr e) then 1 else f ⟨⟨a, b, e⟩, h⟩
