...
@[to_additive "The additive submonoid generated by an element is an additive group
if that element has finite order."]
noncomputable abbrev IsOfFinOrder.groupPowers (hx : IsOfFinOrder x) : Group (powers x) where
  inv g := g ^ (((isOfFinOrder_iff_pow_eq_one x).1 hx).choose - 1)
  mul_left_inv g := Subtype.ext <| by
    obtain ⟨_, k, rfl⟩ := g
  ...
  zpow z g := g ^ z.natMod ((isOfFinOrder_iff_pow_eq_one x).1 hx).choose
  zpow_zero' z := by simp only [Int.natMod, Int.zero_emod, Int.toNat_zero, _root_.pow_zero]
  zpow_neg' m g := Subtype.ext <| by
    obtain ⟨_, k, rfl⟩ := g
    obtain ⟨hpos, hx⟩ := ((isOfFinOrder_iff_pow_eq_one x).1 hx).choose_spec
  ...
  zpow_succ' m g := Subtype.ext <| by
    obtain ⟨_, k, rfl⟩ := g
    obtain ⟨_, hx⟩ := ((isOfFinOrder_iff_pow_eq_one x).1 hx).choose_spec
    ...
    rw [← pow_mul _ m, mul_comm, pow_mul, ← _root_.pow_succ, ← pow_mul, mul_comm, pow_mul]
