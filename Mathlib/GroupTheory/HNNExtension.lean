...
instance (priority := 10000) : Group (HNNExtension G A B φ) := by
...
instance (priority := 10000) TransversalPair.nonempty : Nonempty (TransversalPair G A B) := by
...
instance (priority := 10000) : Inhabited (NormalWord d) := ⟨empty⟩

instance (priority := 10000) : MulAction G (NormalWord d) :=
...
instance (priority := 10000) : FaithfulSMul G (NormalWord d) := ⟨by simp [group_smul_def]⟩
...
noncomputable instance (priority := 10000) : MulAction (HNNExtension G A B φ) (NormalWord d) :=
...
instance (priority := 10000) : FaithfulSMul (HNNExtension G A B φ) (NormalWord d) :=
