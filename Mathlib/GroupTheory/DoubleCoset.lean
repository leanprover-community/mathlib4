...
theorem union_quotToDoset (H K : Subgroup G) : ⋃ q, quotToDoset H K q = Set.univ := by
  ...
  simp only [h5, Subgroup.coe_mk, ← mul_assoc, one_mul, inv_mul_self, mul_inv_cancel_right]
...
theorem doset_union_leftCoset (H K : Subgroup G) (a : G) :
  ...
  · rintro ⟨y, h_h⟩
    ...
    simp only [← mul_assoc, one_mul, mul_inv_self, mul_inv_cancel_right, InvMemClass.coe_inv]
  · rintro ⟨x, hx, y, hy, hxy⟩
    refine ⟨⟨x, hx⟩, ?_⟩
    simp only [hxy, ← mul_assoc, hy, one_mul, inv_mul_self, Subgroup.coe_mk, inv_mul_cancel_right]
