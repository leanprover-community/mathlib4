...
theorem AddSubgroup.zmultiples_le_zmultiples_iff [AddGroup G] (g : G) (i j : ℤ) :
  ...
  rw [← zmultiplesHom_apply, ← AddMonoidHom.map_zmultiples, ← zmultiplesHom_apply,
    ...
    Int.zmultiples_sup, Int.zmultiples_sup,
...
theorem Subgroup.zpowers_zpow_sup [Group G] (g : G) (i j : ℤ) :
  ...
  · rw [zpowers_le]
    exact mem_zpowers_iff.2 ⟨i / (i.gcd j : ℤ),
      by rw [← zpow_mul, Int.mul_ediv_cancel' (Int.gcd_dvd_left ..)]⟩
  · rw [zpowers_le]
    exact mem_zpowers_iff.2 ⟨j / (i.gcd j : ℤ),
  ...
  · rw [zpowers_le]
    ...
    rw [← h]
    exact mul_mem_sup (mem_zpowers_iff.2 ⟨_, rfl⟩) (mem_zpowers_iff.2 ⟨_, rfl⟩)
...
theorem IsCyclic.subgroup_eq_subgroup_iff [Group G] [Finite G] [IsCyclic G]
  ...
  rw [le_antisymm_iff, IsCyclic.subgroup_le_subgroup_iff, IsCyclic.subgroup_le_subgroup_iff,
    dvd_dvd_iff_associated, associated_iff_eq]
