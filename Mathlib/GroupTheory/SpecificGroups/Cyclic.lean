...
/-- The isomorphism between `(Multiplicative G) / ⟨.ofAdd g⟩` and `Multiplicative (G / ⟨g⟩)`. -/
def multiplicativeQuotientZpowersZmultiplesMulEquiv [AddCommGroup G] (g : G) :
    Multiplicative G ⧸ zpowers (Multiplicative.ofAdd g) ≃* Multiplicative (G ⧸ zmultiples g) :=
  MulEquiv.refl _

lemma multiplicativeQuotientZpowersZmultiplesMulEquiv_mk_eq [AddCommGroup G] (g : G)
    (h : Multiplicative G) :
    multiplicativeQuotientZpowersZmultiplesMulEquiv g (QuotientGroup.mk h) =
    .ofAdd (QuotientAddGroup.mk h) := rfl

lemma multiplicativeQuotientZpowersZmultiplesMulEquiv_symm_mk_eq [AddCommGroup G] (g : G)
    (h : G) :
    (multiplicativeQuotientZpowersZmultiplesMulEquiv g).symm (QuotientAddGroup.mk h) =
    QuotientGroup.mk (.ofAdd h) := rfl

/-- The kernel of `zpowersHom G g` is equal to the subgroup generated by `orderOf g`. -/
theorem zpowersHom_ker_eq [Group G] (g : G) :
    (zpowersHom G g).ker = zpowers (Multiplicative.ofAdd ↑(orderOf g)) := by
  ext n
...
theorem zmultiplesHom_ker_eq [AddGroup G] (g : G) :
  ...
  ext n
  rw [AddMonoidHom.mem_ker, mem_zmultiples_iff, zmultiplesHom_apply,
...
noncomputable def addEquivOfPrimeCardEq {p : ℕ} [Fintype G] [Fintype H] [AddGroup G] [AddGroup H]
    [Fact p.Prime] (hG : Fintype.card G = p) (hH : Fintype.card H = p) : G ≃+ H := by
  apply addEquivofAddCyclicCardEq (isAddCyclic_of_prime_card hG) (isAddCyclic_of_prime_card hH)
  rw [← Nat.card_eq_fintype_card] at hG hH
  exact hG.trans hH.symm
