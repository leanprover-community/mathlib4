...
/-- In an additive group generated by `g`, the index of `zmultiples (i • g)` is
`gcd i (addOrderOf g)`. -/
theorem AddSubgroup.index_zmultiples_zsmul [AddGroup G] {g : G} (hg : zmultiples g = ⊤) (i : ℤ) :
    (zmultiples (i • g)).index = i.gcd (addOrderOf g) := by
  have hrange : (zmultiplesHom G g).range = ⊤ := by
    rw [← AddSubgroup.map_top, ← Int.zmultiples_one, AddMonoidHom.map_zmultiples]
    simpa using hg
  rw [← zmultiplesHom_apply, ← AddMonoidHom.map_zmultiples, AddSubgroup.index_map,
    zmultiplesHom_ker_eq, Int.zmultiples_sup, Int.index_zmultiples, hrange,
    AddSubgroup.index_top, mul_one, Int.natAbs_natCast]

@[to_additive existing]
theorem Subgroup.index_zpowers_zpow [Group G] {g : G} (hg : zpowers g = ⊤) (i : ℤ) :
    (zpowers (g ^ i)).index = i.gcd (orderOf g) := by
  refine AddSubgroup.index_zmultiples_zsmul (G := Additive G) ?_ i
  exact (AddSubgroup.eq_top_iff' _).mpr <| (Subgroup.eq_top_iff' ..).mp hg
