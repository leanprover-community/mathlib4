...
-- In this file `W` denotes a group written multiplicatively, with identity element `1`,
-- and `S` denotes a set of generators of `W` such that `S = S⁻¹` (i.e. the elements of `S`
-- are reflections/involutions) and `1 ∈ S`.  Every element of `W` is the product of a finite
-- sequence of elements of `S`.

-- `(W, S)` is said to be a Coxeter system if it satisfies the following condition:

--  * For `sᵢ, sⱼ` in `S`, let `mᵢⱼ` be the order of `sᵢsⱼ` and let


A Coxeter system is a pair `(W, S)` where `W` is a group generated by a set of
reflections (involutions):

`S = {s₁,s₂,...,sₙ}`,

subject to relations determined by a Coxeter matrix `M = (mᵢⱼ)`.

The Coxeter matrix is a symmetic matrix with entries `mᵢⱼ` representing the order of
the product `sᵢsⱼ` for `i ≠ j` and `mᵢᵢ = 1`.
...
where `1` is the identity element of `W`.
