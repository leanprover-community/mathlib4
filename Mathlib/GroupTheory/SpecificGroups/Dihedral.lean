...
/-- If $n$ is odd and $n \ne 1$, then the Dihedral group of order $2n$ has trivial center. -/
...
/-- If $n$ is even and $n \ne 2$, then the Dihedral group of order $2n$ has center equal to
the subgroup generated by $r^{n/2}$. -/
theorem center_of_even_ne_two {n : ℕ} (heven : Even n) (h4 : 4 ≤ n) :
    Subgroup.center (DihedralGroup n) = Subgroup.closure {r (n.div 2)} := by
  haveI : NeZero n := ⟨by omega⟩
  obtain ⟨m, hm⟩ := heven
  have hn_eq : n = 2 * m := by omega
  have hndiv2 : n.div 2 = m := by rw [hn_eq]; exact Nat.mul_div_cancel_left m (by norm_num)
  have hm_lt : m < n := by omega
  have hm2 : (m : ZMod n) * 2 = 0 := by
    have : (m : ZMod n) * 2 = (2 * m : ℕ) := by push_cast; ring
    rw [this, ← hn_eq, ZMod.natCast_self]
  ext x
  simp only [Subgroup.mem_center_iff, Subgroup.mem_closure_singleton]
  constructor
  · intro hx
    rcases x with i | i
    · -- r i in center ⟹ i = -i ⟹ 2i = 0 ⟹ i ∈ {0, m}
      have heq : i = -i := sr.inj (by simpa using hx (sr 0))
      have h2i : i + i = 0 := by
        have : i + i = i + (-i) := congrArg (i + ·) heq
        simp [this]
      have hval : (i.val + i.val) % n = 0 := by simpa [ZMod.val_add] using congr_arg ZMod.val h2i
      have hi_dvd : n ∣ i.val + i.val := Nat.dvd_of_mod_eq_zero hval
      rcases hi_dvd with ⟨k, hk⟩
      have hk_le : k ≤ 1 := by
        by_contra hk_not
        have hk_two : 2 ≤ k := by omega
        have hle : 2 * n ≤ n * k := by
          simpa [mul_comm] using Nat.mul_le_mul_left n hk_two
        rw [← hk] at hle
        have hi_lt : i.val < n := i.val_lt
        omega
      interval_cases k
      · use 0; simp only [zpow_zero]
        have hi0 : i.val = 0 := by omega
        have : i = 0 := ZMod.val_injective n (hi0.trans ZMod.val_zero.symm)
        simp [this]
      · use 1; simp only [zpow_one, r.injEq, hndiv2]
        have him : i.val = m := by omega
        have hm_val : (m : ZMod n).val = m := ZMod.val_cast_of_lt hm_lt
        exact (ZMod.val_injective n (him.trans hm_val.symm)).symm
    · -- sr i in center ⟹ 2 = 0 in ZMod n, contradiction
      have heq : i - 1 = i + 1 := sr.inj (by simpa using hx (r 1))
      ...
      have h2val : (2 : ZMod n).val = 2 := ZMod.val_natCast_of_lt (by omega : 2 < n)
      omega
  · -- r(m)^k commutes with all elements
    intro ⟨k, hk⟩
    rw [← hk, r_zpow, hndiv2]
    intro g
    rcases g with j | j
    ...
    · simp only [sr_mul_r, r_mul_sr, sr.injEq]
      have h2mk : (m : ZMod n) * 2 * ↑k = 0 := by simp [hm2]
      calc j + (m : ZMod n) * ↑k = j - (m : ZMod n) * ↑k + (m * 2) * ↑k := by ring
        _ = j - (m : ZMod n) * ↑k + 0 := by rw [h2mk]
        _ = j - (m : ZMod n) * ↑k := by ring
