...
lemma discr_zeta_eq_discr_zeta_sub_one (hζ : IsPrimitiveRoot ζ n) :
...
lemma discr_prime_pow_ne_two [IsCyclotomicExtension {p ^ (k + 1)} K L] [hp : Fact (p : ℕ).Prime]
...
lemma discr_prime_pow [hcycl : IsCyclotomicExtension {p ^ k} K L] [hp : Fact (p : ℕ).Prime]
...
lemma discr_prime_pow_eq_unit_mul_pow [IsCyclotomicExtension {p ^ k} K L]
...
lemma discr_odd_prime [IsCyclotomicExtension {p} K L] [hp : Fact (p : ℕ).Prime]
