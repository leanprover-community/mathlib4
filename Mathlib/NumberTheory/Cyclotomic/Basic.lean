...
theorem iff_union_singleton_one :
  ...
  · rw [eq_self_sdiff_zero S, eq_self_sdiff_zero (S ∪ {1}), union_sdiff_distrib,
    ...
    · refine (iff_adjoin_eq_top _ A _).2 ⟨fun s hs _ ↦ ⟨1, by simp [mem_singleton.1 hs]⟩, ?_⟩
...
theorem _root_.IsPrimitiveRoot.adjoin_isCyclotomicExtension {ζ : B} {n : ℕ} [NeZero n]
  ...
  { exists_isPrimitiveRoot := fun hi hi' => by
      rw [Set.mem_singleton] at hi
      refine ⟨⟨ζ, subset_adjoin <| Set.mem_singleton_self ζ⟩, ?_⟩
    ...
    adjoin_roots := fun ⟨x, hx⟩ => by
      ...
      · rw [Set.mem_singleton] at hb
        ...
        simp only [mem_singleton, exists_eq_left, mem_ofPred_eq, hb]
...
theorem isSplittingField_X_pow_sub_one : IsSplittingField K L (X ^ n - 1) :=
  { splits' := splits_X_pow_sub_one K L (mem_singleton_self n)
    adjoin_rootSet' := by
      ...
      simp only [mem_singleton, ne_eq, exists_eq_left, NeZero.ne, not_false_eq_true, true_and,
        mem_ofPred_eq]
...
instance isCyclotomicExtension [IsDomain A] [IsFractionRing A K] [NeZero ((n : ℕ) : A)] :
  ...
  adjoin_roots x := by
    ...
    · refine subset_adjoin ?_
      simp only [mem_singleton, exists_eq_left, mem_ofPred_eq]
...
theorem isCyclotomicExtension_singleton_iff_eq_adjoin (C : Subalgebra A B) {ζ : B}
  ...
  · simp only [Set.mem_singleton, exists_eq_left]
  ...
  · simpa only [Set.mem_singleton, ne_eq, forall_eq, NeZero.ne n, not_false_eq_true,
