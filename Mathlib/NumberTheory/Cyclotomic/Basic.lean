...
theorem algEquiv_eq_of_apply_eq [IsCyclotomicExtension S K L] {f g : L ≃ₐ[K] L}
    (H : ∀ n ∈ S, n ≠ 0 → ∃ r : L, IsPrimitiveRoot r n ∧ f r = g r) : f = g := by
  ext x
  have hx := ‹IsCyclotomicExtension S K L›.adjoin_roots x
...
theorem isMulCommutative [IsCyclotomicExtension S K L] : IsMulCommutative (L ≃ₐ[K] L) := by
  refine ⟨⟨fun f g ↦ algEquiv_eq_of_apply_eq S K L fun n hn h1 ↦ ?_⟩⟩
  obtain ⟨r, hr⟩ := ‹IsCyclotomicExtension S K L›.exists_isPrimitiveRoot hn h1
  use r, hr
