...
def approxOrderOf (A : Type*) [Group A] [SeminormedGroup A] (n : ℕ) (δ : ℝ) : Set A :=
  thickening δ {y | orderOf y = n}

@[to_additive mem_approx_add_orderOf_iff]
theorem mem_approxOrderOf_iff {A : Type*} [Group A] [SeminormedGroup A] {n : ℕ} {δ : ℝ} {a : A} :
...
def wellApproximable (A : Type*) [Group A] [SeminormedGroup A] (δ : ℕ → ℝ) : Set A :=
  blimsup (fun n => approxOrderOf A n (δ n)) atTop fun n => 0 < n

@[to_additive mem_add_wellApproximable_iff]
theorem mem_wellApproximable_iff {A : Type*} [Group A] [SeminormedGroup A] {δ : ℕ → ℝ} {a : A} :
...
variable {A : Type*} [CommGroup A] [SeminormedCommGroup A] {a : A} {m n : ℕ} (δ : ℝ)
...
lemma _root_.NormedAddCommGroup.exists_norm_nsmul_le {A : Type*} [AddCommGroup A]
