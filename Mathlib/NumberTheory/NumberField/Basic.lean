...
instance (priority := 10000) [NumberField L] [Algebra K L] : FiniteDimensional K L :=
...
instance (priority := 10000) inst_ringOfIntegersAlgebra [Algebra K L] : Algebra (𝓞 K) (𝓞 L) :=
  RingHom.toAlgebra
...
instance (priority := 10000) [NumberField K] : IsFractionRing (𝓞 K) K :=
  integralClosure.isFractionRing_of_finite_extension ℚ _

instance (priority := 10000) : IsIntegralClosure (𝓞 K) ℤ K :=
  integralClosure.isIntegralClosure _ _

instance (priority := 10000) [NumberField K] : IsIntegrallyClosed (𝓞 K) :=
...
instance (priority := 10000) : CharZero (𝓞 K) :=
  CharZero.of_module _ K

instance (priority := 10000) : IsNoetherian ℤ (𝓞 K) :=
...
instance (priority := 10000) : IsDedekindDomain (𝓞 K) :=
  IsIntegralClosure.isDedekindDomain ℤ ℚ K _

instance (priority := 10000) : Free ℤ (𝓞 K) :=
  IsIntegralClosure.module_free ℤ ℚ K (𝓞 K)

instance (priority := 10000) : IsLocalization (Algebra.algebraMapSubmonoid (𝓞 K) ℤ⁰) K :=
...
instance (priority := 10000) numberField : NumberField ℚ where
...
is a number field. -/
instance (priority := 10000) {f : Polynomial ℚ} [hf : Fact (Irreducible f)] : NumberField (AdjoinRoot f) where
