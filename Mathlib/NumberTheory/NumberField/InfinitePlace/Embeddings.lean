...
`φ` to `K` is `ψ`. -/
class LiesOver (φ : L →+* ℂ) (ψ : K →+* ℂ) : Prop where
...
protected abbrev Extension := { φ : L →+* ℂ // LiesOver φ ψ }
...
def mixedEmbeddingsOver : Set (L →+* ℂ) := { φ | LiesOver φ ψ ∧ IsMixed K φ }
/-- The set of all complex embeddings of `L` that lie over `ψ` and are unmixed. -/
def unmixedEmbeddingsOver : Set (L →+* ℂ) := { φ | LiesOver φ ψ ∧ IsUnmixed K φ }
...
theorem union_unmixedEmbeddingsOver_mixedEmbeddingsOver :
    (unmixedEmbeddingsOver L ψ) ∪ (mixedEmbeddingsOver L ψ) = { φ | LiesOver φ ψ } := by
