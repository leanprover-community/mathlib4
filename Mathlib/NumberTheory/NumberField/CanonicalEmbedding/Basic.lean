...
theorem negAt_apply_isReal_and_mem (x : mixedSpace K) {w : {w // IsReal w}} (hw : w ∈ s) :
  ...
  simp_rw [negAt, ContinuousLinearEquiv.prodCongr_apply, piCongrRight_apply, if_pos hw,
    ContinuousLinearEquiv.neg_apply]

@[simp]
theorem negAt_apply_isReal_and_notMem (x : mixedSpace K) {w : {w // IsReal w}} (hw : w ∉ s) :
    (negAt s x).1 w = x.1 w := by
  simp_rw [negAt, ContinuousLinearEquiv.prodCongr_apply, piCongrRight_apply, if_neg hw,
...
theorem negAt_symm :
  ...
  ext x w
  · by_cases hw : w ∈ s
    · simp_rw [negAt_apply_isReal_and_mem _ hw, negAt, ContinuousLinearEquiv.prodCongr_symm,
        ContinuousLinearEquiv.prodCongr_apply, piCongrRight_symm_apply, if_pos hw, symm_neg,
        neg_apply]
    · simp_rw [negAt_apply_isReal_and_notMem _ hw, negAt, ContinuousLinearEquiv.prodCongr_symm,
        ContinuousLinearEquiv.prodCongr_apply, piCongrRight_symm_apply, if_neg hw, refl_symm,
        refl_apply]
  · rfl
