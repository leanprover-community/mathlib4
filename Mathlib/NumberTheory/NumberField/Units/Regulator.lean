...
theorem regulator_ne_zero : regulator K ≠ 0 := by
  classical
  exact ZLattice.covolume_ne_zero (unitLattice K) volume

theorem regulator_pos : 0 < regulator K := by
  classical
  exact ZLattice.covolume_pos (unitLattice K) volume
