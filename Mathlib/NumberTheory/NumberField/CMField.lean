...
/--
The index of the image of `unitsMulComplexConjInv` in the torsion-subgroup divides `2`.
-/
theorem index_unitsMulComplexConjInv_range_dvd :
    (unitsMulComplexConjInv F : (𝓞 K)ˣ →* torsion K).range.index ∣ 2 := by
...
/--
The index of the subgroup of `(𝓞 K)ˣ` generated by the real units and the roots of unity. This
index is equal to `1` or `2`, see `IsCMField.indexRealUnits_eq_one_or_two` and
`IsCMField.indexRealUnits_eq_two_iff`.
-/
noncomputable abbrev indexRealUnits : ℕ := (realUnits K ⊔ Units.torsion K).index
...
-/
theorem indexRealUnits_eq_one_or_two :
    indexRealUnits K = 1 ∨ indexRealUnits K = 2 :=
  CMExtension.indexRealUnits_eq_one_or_two (maximalRealSubfield K) K
