...
-- This cannot be an `abbrev` otherwise Lean unfolds it the wrong way; we want this to be
-- unfolded using `coe_unitsComplexConj` below.
noncomputable def unitsComplexConj : (𝓞 K)ˣ ≃* (𝓞 K)ˣ :=
  Units.mapEquiv <| RingOfIntegers.mapRingEquiv (complexConj K)

@[simp]
theorem coe_unitsComplexConj (x : (𝓞 K)ˣ) :
    (unitsComplexConj K x : 𝓞 K) = ringOfIntegersComplexConj K (x : 𝓞 K) := rfl
...
theorem unitsComplexConj_eq_self_iff (u : (𝓞 K)ˣ) :
  ...
  simp_rw [Units.ext_iff, RingOfIntegers.ext_iff, coe_unitsComplexConj,
    coe_ringOfIntegersComplexConj, mem_realUnits_iff, Units.complexConj_eq_self_iff,
    RingOfIntegers.ext_iff, IsScalarTower.algebraMap_apply (𝓞 K⁺) (𝓞 K) K]
...
noncomputable def unitsMulComplexConjInv : (𝓞 K)ˣ →* torsion K where
  toFun := fun u ↦ ⟨u * (unitsComplexConj K u)⁻¹, (mem_torsion K).mpr fun _ ↦ by
    simp only [← map_inv, coe_mul, coe_unitsComplexConj, coe_ringOfIntegersComplexConj,
      map_units_inv, map_inv₀, map_mul, infinitePlace_complexConj, ne_eq, map_eq_zero,
      FaithfulSMul.algebraMap_eq_zero_iff, Units.ne_zero, not_false_eq_true, mul_inv_cancel₀]⟩
  map_one' := by simp
  map_mul' := by
    intro _ _
