...
- `IsCyclotomicExtension.Rat.galEquivZMod_stabilizer`: assume that the prime `p` does not divide
  ...
  stabilizer of `P` in `Gal(ℚ(ζₙ)/ℚ)`, aka the decomposition group `P`, is the subgroup
  generated by the class of `p` in `(ℤ/nℤ)ˣ`.
...
theorem galEquivZMod_stabilizer :
  ...
  · rintro _ ⟨σ, hσ, rfl⟩
    exact galEquivZMod_mem_zpowers_of_mem_stabilizer n K p P hn hσ
