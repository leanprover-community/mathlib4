...
open Filter Ideal InfinitePlace mixedEmbedding euclidean fundamentalCone Submodule Topology Units

private theorem ideal.tendsto_norm_le_and_mk_eq_div_atop_aux₁ {C : ClassGroup (𝓞 K)}
...
open Classical in
private def ideal.tendsto_norm_le_and_mk_eq_div_atop_aux₂ (J : (Ideal (𝓞 K))⁰) (s : ℝ) :
...
theorem ideal.tendsto_norm_le_and_mk_eq_div_atop (C : ClassGroup (𝓞 K)) :
    Tendsto (fun s : ℝ ↦
  ...
  · simp_rw [ideal.tendsto_norm_le_and_mk_eq_div_atop_aux₁ K hJ, id_eq,
      Nat.card_congr (ideal.tendsto_norm_le_and_mk_eq_div_atop_aux₂ K J _),
...
theorem ideal.tendsto_norm_le_div_atop₀ :
  ...
  convert Filter.Tendsto.congr' ?_
    (tendsto_finset_sum Finset.univ (fun C _  ↦ ideal.tendsto_norm_le_and_mk_eq_div_atop K C))
