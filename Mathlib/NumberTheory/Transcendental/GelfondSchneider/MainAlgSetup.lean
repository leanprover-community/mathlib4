...
lemma c₁_pow_sub_one_mul_c₁_pow_mul_c₁_pow_eq :
  ...
  · simp_rw [← pow_add]
    ...
    have hbl : b q t * h7.l q u ≤ h7.m * q := by
      ...
      have hu := (finProdFinEquiv.symm u).1.isLt
      simpa [mul_comm] using Nat.mul_le_mul ht hu
    rw [Nat.sub_add_cancel (by grind), Nat.sub_add_cancel hal, Nat.sub_add_cancel hbl]
  ...
  · apply mul_le_mul ?_ ?_ (by dsimp [house];positivity) (by dsimp [house];positivity)
    · apply mul_le_mul ?_ ?_ (by dsimp [house];positivity) (by dsimp [house];positivity)
      ...
      · apply Bound.pow_le_pow_right_of_le_one_or_one_le
        ...
        · exact @Setup.al_le_mq h7 q hq0 h2mq u t
    · apply Bound.pow_le_pow_right_of_le_one_or_one_le
        (Or.inl ⟨one_le_house_of_isIntegral h7.isIntegral_c₁γ h7.c₁γ_ne_zero, ?_⟩)
      · exact @Setup.bl_le_mq h7 q hq0 h2mq u t
  · apply mul_le_mul ?_ ?_ (by dsimp [house];positivity) (by dsimp [house];positivity)
    · apply mul_le_mul ?_ ?_ (by dsimp [house];positivity) (by dsimp [house];positivity)
