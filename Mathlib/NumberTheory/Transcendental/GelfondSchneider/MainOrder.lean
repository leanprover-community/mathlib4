...
lemma vecMul_V_eq_zero (hR : h7.R q hq0 h2mq = 0) :
    (fun t ↦ h7.σ ((h7.η q hq0 h2mq) t)) ᵥ* (h7.V q) = 0 := by
  ext k
  ...
  have hk' : ∑ t, h7.σ ((h7.η q hq0 h2mq) t) * (h7.ρ q t) ^ (k : ℕ) = 0 := by
    simpa [Complex.exp_zero, mul_one] using hk
  simp [Matrix.vecMul, dotProduct, V, Matrix.vandermonde_apply]
  exact hk'

lemma ηvec_eq_zero (hVecMulEq0 : (fun t ↦ h7.σ ((h7.η q hq0 h2mq) t)) ᵥ* (h7.V q) = 0) :
