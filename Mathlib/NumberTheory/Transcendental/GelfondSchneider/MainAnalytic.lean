    ...
    · rw [mul_comm, ← zsmul_eq_mul]
      ...
      · apply IsIntegral.mul
        · simp only [nsmul_eq_mul, zsmul_eq_mul, Int.cast_pow]
          ...
          · apply IsIntegral.mul <| isIntegral_intCast (B := h7.K) h7.c₁
            · apply isIntegral_natCast (B := h7.K)
      ...
      · have : h7.c₁ ^ (h7.m * q - ((b q x) * (h7.l₀' q hq0 h2mq + 1))) *
        ...
        apply IsIntegral.mul
        · apply IsIntegral.pow
          · apply isIntegral_intCast (B := h7.K) h7.c₁
        · rw [← mul_pow]
          apply IsIntegral.pow
        ...
        · simp only [nsmul_eq_mul, zsmul_eq_mul, Int.cast_pow]
          ...
          · apply IsIntegral.add
            · apply IsIntegral.mul <| isIntegral_intCast (B := h7.K) h7.c₁
              · apply isIntegral_natCast (B := h7.K)
            ·rw [mul_comm, mul_assoc]
             apply IsIntegral.mul <| isIntegral_natCast (B := h7.K) _
             rw [mul_comm, ← zsmul_eq_mul]
