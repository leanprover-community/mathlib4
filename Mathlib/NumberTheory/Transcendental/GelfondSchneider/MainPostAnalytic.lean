...
lemma exists_nonzero_iteratedFDeriv : deriv^[h7.r q hq0 h2mq]
 (h7.R q hq0 h2mq) (h7.l₀' q hq0 h2mq + 1) ≠ 0 := by
  ...
  simpa [iteratedDeriv_eq_iterate] using (analyticOrderAt_eq_nat_iff_iteratedDeriv_eq_zero
    (f := h7.R q hq0 h2mq) (z₀ := (↑↑(h7.l₀' q hq0 h2mq) + 1 : ℂ)) hA1).mp Hrprop |>.2
...
lemma order_geq_n_foo (l' : Fin (h7.m)) :
  ...
  intros H
  have H' : ∀ i < h7.n q, iteratedDeriv i (h7.R q hq0 h2mq) (l' + 1) = 0 := by
    intro i hi
    simpa [iteratedDeriv_eq_iterate] using H i hi
  exact (natCast_le_analyticOrderAt_iff_iteratedDeriv_eq_zero (f := h7.R q hq0 h2mq)
    (z₀ := l' + 1) (n := h7.n q) (by fun_prop)).mpr H'
...
lemma eq5 : h7.c₅ ^ (-(h7.r q hq0 h2mq) : ℝ) < norm (Algebra.norm ℚ (rho h7 q hq0 h2mq)) := by
  ...
  have h1 : 1 ≤ ‖(h7.cρ q hq0 h2mq) ^ Module.finrank ℚ h7.K‖ *
    ...
    have H := Algebra.norm_algebraMap (R := ℚ) (S := h7.K) (h7.cρ q hq0 h2mq : ℚ)
    ...
    · rw [mul_add]
      ...
      * (2 * h7.m * ↑q) = h7.h * h7.r q hq0 h2mq + h7.h * 2 * h7.m * ↑q := by
      ...
      have : ((h7.h : ℤ) * h7.r q hq0 h2mq + ↑(h7.h) * 2 * ↑(h7.m) * ↑q)  =
         ((h7.h : ℤ) * (↑(h7.r q hq0 h2mq) + 2 * ↑(h7.m) * ↑q)) := by ring
      rw [this]
      dsimp [c₅]
      norm_cast
      nth_rw 2 [pow_mul]
  ...
  (h7.c₁) ^ (b q t * (↑(h7.l₀' q hq0 h2mq) + 1)) := by
    ...
    · simp only [smul_eq_mul]
      ...
      + (a q t * (↑(h7.l₀' q hq0 h2mq) + 1))) = (h7.m * q) := by
        ...
        rw [mul_comm h7.m]
        apply mul_le_mul (((finProdFinEquiv.symm.toFun t).1).isLt) ?_ zero_le zero_le
    ...
    · simp only [smul_eq_mul]
      ...
      have : (h7.m * q - (b q t * (↑(h7.l₀' q hq0 h2mq) + 1))
        ...
        refine add_tsub_cancel_of_le ?_
        rw [mul_comm h7.m]
        apply mul_le_mul (((finProdFinEquiv.symm.toFun t).2).isLt) ?_ zero_le zero_le
        · exact (h7.l₀' q hq0 h2mq).isLt
      rw [this]
