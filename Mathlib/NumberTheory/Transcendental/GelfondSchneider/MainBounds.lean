...
omit [DecidableEq (K →+* ℂ)] in
lemma c₁₁_nonneg : 0 ≤ (c₁₁ K) := le_trans zero_le_one (one_le_c₁₁)
...
lemma one_le_c₁₀ : 1 ≤ (c₁₀ α β α' β' γ') := by
  ...
  have h3 : 1 ≤ (2 : ℝ) * (m K) * (c₄ α' β' γ') * (c₉ α β K) := by
    simpa [mul_assoc] using one_le_mul_of_one_le_of_one_le h2 (one_le_c₉ α β)
  have h4 : 1 ≤ (c₉ α β K) ^ (2 * (m K) : ℝ) := by
    exact Real.one_le_rpow ((one_le_c₉ α β)) (by positivity)
...
lemma abs_R : (q * q) * (((c₄ α' β' γ') ^ ((r α β σ α' β' γ' hirr htriv habc) q hq0 h2mq : ℝ) * ((r
    ...
    · apply mul_le_mul (eq6b.extracted_1_1 α β σ α' β' γ' hirr htriv habc q hq0 h2mq) le_rfl ?_
      ...
      (apply mul_nonneg (mul_nonneg (Real.rpow_nonneg (le_trans zero_le_one
          (one_le_c₄ α' β' γ')) _)
        (by positivity)) (Real.rpow_nonneg (c₉_nonneg α β) _))
    · unfold c₁₀
      nth_rw 2 [Real.mul_rpow _
          (by apply Real.rpow_nonneg (c₉_nonneg α β) ((2 * ↑(m K) : ℝ)))]
      · nth_rw 2 [Real.mul_rpow _ (by grind [c₉_nonneg α β])]
        · nth_rw 2 [Real.mul_rpow (by positivity)
          ...
          · simp only [← mul_assoc, mul_assoc ((2*(m K) : ℝ) ^ ((r α β σ α' β' γ' hirr htriv habc) q
            ...
            · apply mul_le_mul (by simp) ?_
              ...
              · rw [Real.rpow_add
                  (by grind [c₉_pos α β]), mul_comm, mul_assoc]
                · apply mul_le_mul
                    (by rfl) ?_ ?_ (Real.rpow_nonneg (c₉_nonneg α β) _)
                  · apply mul_le_mul ?_ ?_ (by positivity)
                      (by apply Real.rpow_nonneg (Real.rpow_nonneg (c₉_nonneg α β) _) _)
                    · rw [← Real.rpow_mul (c₉_nonneg α β)]
                      · apply Real.rpow_le_rpow_of_exponent_le (one_le_c₉ α β)
                  ...
                  · apply mul_nonneg (Real.rpow_nonneg (c₉_nonneg α β)
...
lemma norm_z_minus_km_lower_bound_on_sphere (km : Fin ((m K))) :
  ...
  calc _ = ((m K) * (1 + ((r α β σ α' β' γ' hirr htriv habc) q hq0 h2mq : ℝ) / (q : ℝ)) - (m K) :
      ℝ) := by ring
  ...
  calc
    ...
    _ ≤ (((m K) * ((r α β σ α' β' γ' hirr htriv habc) q hq0 h2mq : ℝ) / (q : ℝ))⁻¹) ^ (((r α β σ α'
        ...
        C ^ Finset.card (Finset.range (m K) \ {↑((l₀' α β σ α' β' γ' hirr htriv habc) q hq0
          ...
          apply mul_le_mul (by simp only [Real.rpow_natCast, le_refl]) ?_ (by positivity)
              (by positivity)
          apply prod_bound
          · intro x hx
            positivity
    ...
    _ ≤ ((c₁₁ K)) ^ ((r α β σ α' β' γ' hirr htriv habc) q hq0 h2mq : ℝ) *
        (q / ((r α β σ α' β' γ' hirr htriv habc) q hq0 h2mq)) ^ ((m K) * (r α β σ α' β' γ' hirr
          ...
          have : #(Finset.range (m K) \ {↑((l₀' α β σ α' β' γ' hirr htriv habc) q hq0 h2mq)}) = ((m
              K) - 1) := by grind
          rw [this]
          unfold C
          rw [← pow_mul]
          ...
          refine (div_le_div_iff_of_pos_left (by simp only [Nat.cast_pos]; exact hq0)
            ...
            (by simp only [Nat.cast_pos]
                exact r_qt_0 α β σ α' β' γ' hirr htriv habc q hq0 h2mq)).mpr ?_
          norm_cast
...
lemma one_le_c₁₂ : 1 ≤ (c₁₂ α β α' β' γ') := by
  ...
  apply one_le_mul_of_one_le_of_one_le ?_ ((one_le_c₁₀ α β α' β' γ'))
    ...
    · rw [mul_assoc, mul_assoc]
      · apply mul_le_mul (le_trans ((abs_Rb α β σ α' β' γ' hirr htriv habc) q hq0 h2mq hz) (abs_R α
        ...
        · simp only [one_div, norm_inv, norm_pow, norm_prod, Complex.norm_div]
          ...
          exact this
        · apply mul_nonneg (Real.rpow_nonneg (c₁₀_nonneg α β α' β' γ') _)
            (by positivity)
  · simp only [← mul_assoc]
    rw [mul_comm]
    unfold c₁₂
...
theorem ρᵣ_eq_log_zpow_mul_S : ρᵣ α β σ α' β' γ' hirr htriv habc q hq0 h2mq =
  ...
  have hr : (r α β σ α' β' γ' hirr htriv habc) q hq0 h2mq ≤ (r α β σ α' β' γ' hirr htriv habc) q
      hq0 h2mq := by rfl
  have :
...
lemma eq7 (_l' : Fin ((m K))) :
  ...
  (hcauchy α β σ α' β' γ' hirr htriv
      habc) q hq0 h2mq ▸ ρᵣ_eq_log_zpow_mul_S α β σ α' β' γ' hirr htriv habc q hq0 h2mq
...
lemma hf : ∀ z ∈ Metric.sphere 0 ((m K) * (1 + ↑((r α β σ α' β' γ' hirr htriv habc) q hq0 h2mq : ℝ)
  ...
  apply mul_le_mul ?_ (S_norm_bound α β σ α' β' γ' hirr htriv habc q hq0 h2mq hz) (by positivity)
      (by positivity)
  · apply inv_anti₀ ?_ ((norm_sub_l0_lower_bound_on_sphere α β σ α' β' γ' hirr htriv habc) q hq0
    ...
    · refine Real.sqrt_ne_zero'.mp ?_
      · refine (Real.sqrt_ne_zero (by positivity)).mpr ?_
        ...
        · simp only [ne_eq, mul_eq_zero, Nat.cast_eq_zero, not_or]
          refine ⟨by simp [m], by simp_rw [(r_ne_zero α β σ α' β' γ' hirr htriv habc)]; simp only
              [not_false_eq_true]⟩
  ...
  · simp only [zpow_neg, zpow_natCast, mul_assoc]
    ...
    · simp only [mul_assoc]
      ...
      · apply mul_le_mul
        · nth_rw 1 [← Real.rpow_one (x:= (m K))]
          ...
          · unfold m; simp only [Nat.cast_add, Nat.cast_mul, Nat.cast_ofNat]
            ...
            trans
            · apply one_lt_two
            · simp only [lt_add_iff_pos_left, Nat.ofNat_pos, mul_pos_iff_of_pos_left,
                Nat.cast_pos]
        ...
        · simp only [← mul_assoc]
          ...
          · simp only [mul_assoc]
            ...
            apply mul_le_mul ?_ ?_
                (by positivity) (Real.rpow_nonneg (c₁₂_nonneg α β α' β' γ') _)
            · simp only [Real.rpow_natCast, le_refl]
            · ring_nf
              rw [mul_rotate]
              simp only [mul_assoc]
          ...
          · apply mul_nonneg
            · apply mul_nonneg
              · positivity
              · apply Real.rpow_nonneg (c₁₂_nonneg α β α' β' γ')
            · positivity
          · positivity
        · apply mul_nonneg
          · positivity
      ...
      · apply mul_nonneg (by positivity)
        · apply mul_nonneg (by positivity)
            (mul_nonneg (Real.rpow_nonneg (c₁₂_nonneg α β α' β' γ') _)
                (by positivity))
      · apply Real.rpow_nonneg
        rw [add_comm]
        trans
...
lemma r_rpow_lt_c₁₅_rpow :
  ...
  have h68 : N ≤ (c₁₄ α β α' β' γ') ^ r * B := by
    dsimp [N, r, B]
    simpa using (norm_algebraNorm_rho_le α β σ α' β' γ' hirr htriv habc q hq0 h2mq)
  have htmp :
  ...
  have hApos : 0 < (c₁₄ α β α' β' γ') ^ r := by
    exact Real.rpow_pos_of_pos (lt_of_lt_of_le zero_lt_one (one_le_c₁₄ α β σ α' β' γ' hirr htriv
        habc)) _
  have hmulrpow : ((c₁₅ α β α' β' γ')) ^ r = ((c₁₄ α β α' β' γ') ^ r) * ((c₅ α' β' γ') ^ r) := by
    unfold c₁₅
    rw [Real.mul_rpow (le_trans zero_le_one (one_le_c₁₄ α β σ α' β' γ' hirr htriv habc)) (c₅_pos
        α' β' γ').le]
  calc
    ((GelfondSchneider.r α β σ α' β' γ' hirr htriv habc) q hq0 h2mq : ℝ) ^ ((((GelfondSchneider.r α
        ...
        = B⁻¹ := by
            ...
            rw [show ((((GelfondSchneider.r α β σ α' β' γ' hirr htriv habc) q hq0 h2mq : ℝ) - 3 * (h
                K)) / 2) =
                - (-((GelfondSchneider.r α β σ α' β' γ' hirr htriv habc) q hq0 h2mq : ℝ) / 2 + 3 *
                    (h K) / 2) by ring]
    ...
    _ = ((c₁₅ α β α' β' γ')) ^ ((GelfondSchneider.r α β σ α' β' γ' hirr htriv habc) q hq0 h2mq : ℝ)
        := by rfl
