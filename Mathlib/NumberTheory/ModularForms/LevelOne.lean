...
lemma one_mem_strictPeriods_SL2Z : (1 : ℝ) ∈ Γ(1).strictPeriods := by
  rw [Gamma_one_top, ← MonoidHom.range_eq_map, Subgroup.strictPeriods_SL2Z]
  ...
  · simp only [Metric.mem_closedBall, dist_zero_right]
    ...
    · obtain ⟨ξ, hξ, hξ₂⟩ := exists_one_half_le_im_and_norm_le hk f
      ...
      exact ⟨_, norm_qParam_le_of_one_half_le_im hξ,
        by simpa only [← eq_cuspFunction f _ one_mem_strictPeriods_SL2Z one_ne_zero,
...
private theorem levelOne_nonpos_wt_const (hk : k ≤ 0) (f : F) :
  ...
  simpa only [← eq_cuspFunction f _ one_mem_strictPeriods_SL2Z one_ne_zero, Nat.cast_one,
    Function.const_apply] using cuspFunction_eqOn_const_of_nonpos_wt hk f hQ
