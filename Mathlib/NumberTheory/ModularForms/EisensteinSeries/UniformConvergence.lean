...
lemma norm_def (x : Fin 2 → ℤ) : ‖x‖ = max (x 0).natAbs (x 1).natAbs := by
...
lemma div_max_sq_ge_one (x : Fin 2 → ℤ) (hx : x ≠ 0) :
  ...
  · have : x 0 ≠ 0 := by
      rwa [← norm_ne_zero_iff, norm_def, H0, Nat.cast_ne_zero, Int.natAbs_ne_zero] at hx
    simp only [norm_def, H0, Int.cast_natAbs, Int.cast_abs, div_pow, _root_.sq_abs, ne_eq,
      OfNat.ofNat_ne_zero, not_false_eq_true, pow_eq_zero_iff, Int.cast_eq_zero, this, div_self,
      le_refl]
  · have : x 1 ≠ 0 := by
      rwa [← norm_ne_zero_iff, norm_def, H1, Nat.cast_ne_zero, Int.natAbs_ne_zero] at hx
    simp only [norm_def, H1, Int.cast_natAbs, Int.cast_abs, div_pow, _root_.sq_abs, ne_eq,
      OfNat.ofNat_ne_zero, not_false_eq_true, pow_eq_zero_iff, Int.cast_eq_zero, this, div_self,
      le_refl]

lemma r_mul_max_le {x : Fin 2 → ℤ} (hx : x ≠ 0) : r z * ‖x‖ ≤ Complex.abs (x 0 * z + x 1) := by
  let n := ‖x‖
  have hn0 : n ≠ 0 := by rwa [norm_ne_zero_iff]
  have h11 : x 0 * (z : ℂ) + x 1 = (x 0 / n * z + x 1 / n) * n := by
    rw [div_mul_eq_mul_div, ← add_div, div_mul_cancel₀ _ (mod_cast hn0)]
  rw [norm_def, h11, map_mul, Complex.abs_ofReal, abs_norm, norm_def]
  gcongr
  · rcases div_max_sq_ge_one x hx with H1 | H2
    · simpa only [norm_def, ofReal_div, ofReal_intCast, n] using auxbound1 z (x 1 / n) H1
    · simpa only [norm_def, ofReal_div, ofReal_intCast, n] using auxbound2 z (x 0 / n) H2
...
lemma summable_one_div_norm_rpow {k : ℝ} (hk : 2 < k) :
  ...
  · simp only [finTwoArrowEquiv_symm_apply, Function.comp_def]
    ...
    suffices Summable fun n : ℕ ↦ ∑' (_ : box (α := ℤ × ℤ) n), (n : ℝ) ^ (-k) by
      ...
      simp only [← Int.mem_box.mp p.2, Nat.cast_max, norm_def, Matrix.cons_val_zero,
