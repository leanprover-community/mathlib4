...
lemma csqrt_eq {z : ℂ} (hz : z ≠ 0) : csqrt z = cexp ((log z) * (1 / 2)) := by
  unfold csqrt
  rw [cpow_def]
  simp [one_div, hz]
...
lemma csqrt_pow_24_eq (z : ℂ) (hz : z ≠ 0) : (csqrt z) ^ 24 = z ^ 12 := by
  rw [csqrt_eq (by simp [hz]), ← exp_nat_mul]
  ring_nf
...
lemma logDeriv_eta_comp_eq_logDeriv_csqrt_eta (z : ℍ) :
  ...
  rw [logDeriv_eta_comp_div_eq z, show ((csqrt) * η) = (fun x ↦ (csqrt) x * η x) by rfl,
      ...
      (csqrt_differentiableAt (mem_slitPlane z))
  ...
  rw [csqrt_deriv (mem_slitPlane z), div_eq_mul_inv, logDeriv_eta_eq_E2 z,
...
lemma eta_comp_eqOn_const_mul_csqrt_eta :
  ...
  · intro x hx
    exact (((csqrt_differentiableAt (mem_slitPlane ⟨x, hx⟩))).mul
