...
in this file, whereas the other, `Real.exists_genContFract_conv_eq_rat`, uses
...
see `Real.genContFract_conv_eq_convergent`.
...
theorem genContFract_conv_eq_convergent (ξ : ℝ) (n : ℕ) :
  ...
  · rw [convs_succ, ih (fract ξ)⁻¹, convergent_succ, one_div]
    norm_cast
#align real.continued_fraction_convergent_eq_convergent Real.genContFract_conv_eq_convergent
...
theorem exists_genContFract_conv_eq_rat {q : ℚ}
    (h : |ξ - q| < 1 / (2 * (q.den : ℝ) ^ 2)) : ∃ n, (GenContFract.of ξ).convs n = q := by
  obtain ⟨n, hn⟩ := exists_rat_eq_convergent h
  exact ⟨n, hn.symm ▸ genContFract_conv_eq_convergent ξ n⟩
#align real.exists_continued_fraction_convergent_eq_rat Real.exists_genContFract_conv_eq_rat
