...
lemma fermatNumber_ne_one (n : ℕ) : fermatNumber n ≠ 1 := by have := three_le_fermatNumber n; grind
...
theorem prod_fermatNumber (n : ℕ) : ∏ k ∈ range n, fermatNumber k = fermatNumber n - 2 := by
  ...
  rw [prod_range_succ, hn, fermatNumber, fermatNumber, mul_comm,
    (show 2 ^ 2 ^ n + 1 - 2 = 2 ^ 2 ^ n - 1 by grind), ← sq_sub_sq]
  ring_nf
  grind
...
theorem coprime_fermatNumber_fermatNumber {m n : ℕ} (hmn : m ≠ n) :
  ...
  wlog hmn' : m < n
  · simpa only [coprime_comm] using this hmn.symm (by grind)
