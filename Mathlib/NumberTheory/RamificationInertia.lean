...
theorem FinrankQuotientMap.span_eq_top [IsDomain R] [IsDomain S] [Algebra K L] [IsNoetherian R S]
  ...
  let A'' : Matrix (Fin n) (Fin n) R := A' -- Porting note: Do without this
  let A : Matrix (Fin n) (Fin n) R := A'' - 1
  let B := A.adjugate
  have A_smul : ∀ i, (∑ j, A i j • a j) = 0 := by
    intros
    simp only [Matrix.sub_apply, ne_eq, Matrix.one_apply, sub_smul, Finset.sum_sub_distrib, hA',
      Finset.sum_univ_ite, sub_self]
...
set_option synthInstance.checkSynthOrder false -- Porting note: TODO
...
-- Porting note: Can we do without this?
  let Q : ℕ → Prop :=
