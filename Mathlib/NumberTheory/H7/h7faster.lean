...
def c₇ : ℝ := ((|↑h7.c₁| * |↑h7.c₁| * (|↑h7.c₁| * (house h7.α' * (|↑h7.c₁| * house h7.γ')))) ^ h7.m)
...
lemma eq6a : house (rho h7 q hq0 h2mq) ≤
  ...
  · simp only [ sum_const, card_univ, Fintype.card_fin]
    simp only [nsmul_eq_mul]
    simp only [Nat.cast_mul, Real.rpow_natCast, Int.cast_abs,
     Nat.abs_cast, Int.cast_natCast,
      zpow_natCast, mul_eq_mul_left_iff, mul_eq_zero, Nat.cast_eq_zero, or_self]
    left
    simp only [mul_assoc]
...
lemma bound_n_leq_r : (h7.c₄ ^ (h7.n q : ℝ) * ((h7.n q : ℝ) ^ (((h7.n q : ℝ)+ 1)/2)) ≤
  ((h7.c₄ ^ (h7.r q hq0 h2mq : ℝ)) *
  ((h7.r q hq0 h2mq : ℝ)^((1/2)* ((h7.r q hq0 h2mq : ℝ) + 1))))) := sorry

lemma q_le_2sqrtmr : q^2 = 2*h7.m*h7.r q hq0 h2mq := by sorry
  --refine Eq.symm (Nat.mul_div_cancel' h2mq)

lemma sqt_etc : Real.sqrt (2*h7.m*(h7.r q hq0 h2mq)) =
  Real.sqrt (2*h7.m) * (h7.r q hq0 h2mq)^1/2:=sorry

def c₈ : ℝ := 2 * h7.m * h7.c₄ * h7.c₆ * Real.sqrt (2*h7.m) * h7.c₇^(2*h7.m)

lemma eq6b : (q*q) * ((((h7.c₄ ^ (h7.n q : ℝ) *
  ((h7.n q : ℝ) ^ (((h7.n q : ℝ)+ 1)/2)))) *
  (h7.c₆* q) ^(h7.r q hq0 h2mq) * (h7.c₇)^q)) ≤
  h7.c₈^(h7.r q hq0 h2mq) * (h7.r q hq0 h2mq)^((h7.r q hq0 h2mq) + 3/2) := by {

    calc
         _ ≤ ((2*h7.m)^h7.r q hq0 h2mq * h7.r q hq0 h2mq) *
             ((( ((h7.c₄ ^ (h7.r q hq0 h2mq : ℝ)) *
             ((h7.r q hq0 h2mq : ℝ)^((1/2)* ((h7.r q hq0 h2mq : ℝ) + 1))))) *
             (((h7.c₆* Real.sqrt (2*h7.m) * (h7.r q hq0 h2mq)^1/2) ^(h7.r q hq0 h2mq)) *
             (h7.c₇)^2*h7.m*h7.r q hq0 h2mq))) := ?_

         _ ≤ h7.c₈^(h7.r q hq0 h2mq) *
           (h7.r q hq0 h2mq)^((h7.r q hq0 h2mq) + 3/2) := ?_
...
lemma holS :
  ...
  ·
   ...
   -- obtain ⟨l', hl'⟩ := H
    ...
    ·
      intros z hz
      have: ↑↑(h7.l₀' q hq0 h2mq) + 1 ∈ S.U h7 := by {sorry
      }
...
def sys_coeff_foo_S : ρᵣ h7 q hq0 h2mq =
  ...
  simp only [sub_self, tsub_self, pow_zero, Nat.factorial_zero,
    Nat.cast_one, div_one, zero_mul,
    add_zero, one_mul, exists_and_right] at this
  obtain ⟨R2,hR⟩ := this
  obtain ⟨R2,hR2⟩ := R2
  rw [hR]
  unfold R₁
...
lemma q_frac : ((↑q + ↑(h7.r q hq0 h2mq)) / ↑q : ℝ ) = (1 + ↑(h7.r q hq0 h2mq) / (q : ℝ)) := by
  rw [add_div]
...
lemma abs_R : norm ((h7.R q hq0 h2mq) z) ≤ (c₁₀)^ (h7.r q hq0 h2mq) *
 (h7.r q hq0 h2mq)^(1/2*((h7.r q hq0 h2mq)+3)) := by
  ...
  calc _ ≤ ∑ t, (‖(canonicalEmbedding h7.K) ((algebraMap (𝓞 h7.K) h7.K)
       ...
       (h7.r q hq0 h2mq) ^ (((h7.r q hq0 h2mq : ℝ ) + 1) / 2)) * (h7.c₉) ^ (h7.r q hq0 h2mq + q : ℝ)) := ?_
  ...
  · simp only [sum_const, card_univ,
    ...
    · apply mul_le_mul
      · apply mul_le_mul
        ...
        · apply Real.rpow_nonneg
          exact zero_leq_c₄ h7
      · simp only [norm_mul, Real.norm_eq_abs]--nth_rw 2 [Real.exp_mul]
        rw [Real.rpow_def_of_pos (x:= h7.c₉)]
        · simp only [Nat.abs_cast, Real.exp_le_exp]
          calc _ =  ↑q * (|1 + ‖h7.β‖| * |Real.log ‖h7.α‖| * ↑h7.m) * |1 + ↑(h7.r q hq0 h2mq) / ↑q| := ?_
               _ ≤ (2 * ↑h7.m * (h7.n q)) * (|1 + ‖h7.β‖| * |Real.log ‖h7.α‖| * ↑h7.m) * |1 + ↑(h7.r q hq0 h2mq) / ↑q| := ?_

               _ = 2 * ↑h7.m * |1 + ‖h7.β‖| * |Real.log ‖h7.α‖| * ↑h7.m * (h7.n q) * |1 + ↑(h7.r q hq0 h2mq) / ↑q| := ?_
               _ ≤ (2 * ↑h7.m * |1 + ‖h7.β‖| * |Real.log ‖h7.α‖| * ↑h7.m) * (h7.r q hq0 h2mq * |1 + ↑(h7.r q hq0 h2mq) / ↑q|) := ?_
               _ ≤ (2 * ↑h7.m * |1 + ‖h7.β‖| * |Real.log ‖h7.α‖| * ↑h7.m) * (↑(h7.r q hq0 h2mq) + ↑q) := ?_
          ...
          · sorry
          · unfold c₉
            simp only [Real.log_exp, le_refl]
        · unfold c₉
          apply Real.exp_pos
      · positivity
      · sorry
...
lemma norm_sub_l0_lower_bound_on_sphere :
  ...
  calc _ = (h7.m * (1 + (h7.r q hq0 h2mq : ℝ) / (q : ℝ)) - h7.m : ℝ) := by ring
       _ ≤ ‖z‖ - ‖(h7.l₀' q hq0 h2mq : ℂ)‖ := by
         ...
         have : h7.m * (1 + (h7.r q hq0 h2mq : ℝ) / (q : ℝ)) - (h7.l₀' q hq0 h2mq : ℝ) =
           h7.m * (1 + (h7.r q hq0 h2mq : ℝ) / (q : ℝ)) + (- (h7.l₀' q hq0 h2mq : ℝ)) := rfl
         rw [this]
         rw [add_assoc]
         ...
         exact hlm
       _ ≤ ‖z - (h7.l₀' q hq0 h2mq : ℂ)‖ := norm_sub_norm_le z (h7.l₀' q hq0 h2mq)

include h2mq hz in
lemma norm_sub_l0_lower_bound_on_sphere_inv :
    ‖z - (h7.l₀' q hq0 h2mq : ℂ)‖ ≤ 1/(h7.m * (h7.r q hq0 h2mq : ℝ) / (q : ℝ)) := by
...
lemma abs_denom : norm (((z - (h7.l₀' q hq0 h2mq : ℂ)) ^ (-(h7.r q hq0 h2mq : ℤ))) *
  ...
  calc
    ...
    _ ≤ (h7.m * (h7.r q hq0 h2mq : ℝ) / (q : ℝ)) ^ (-(h7.r q hq0 h2mq : ℤ)) *
        (∏ km ∈ Finset.range (h7.m) \ { (h7.l₀' q hq0 h2mq : ℕ) },
          norm (( (h7.m*q) / (h7.m * h7.r q hq0 h2mq) : ℝ) ^ (h7.r q hq0 h2mq))) := ?_
  ...
  · apply mul_le_mul
    ...
    · simp only [zpow_neg, zpow_natCast, inv_nonneg]
      ...
      · refine div_nonneg ?_ ?_
        ...
        · simp only [Nat.cast_nonneg]
  · apply mul_le_mul
    · simp only [zpow_neg, zpow_natCast, le_refl]
    · apply Finset.prod_le_prod
      ...
      · intros x hx
        ...
        · rw [← norm_div]
          apply norm_nonneg
        · exact rneq0 h7 q hq0 h2mq
        · --rw [← norm_div];rw [← norm_div]
          have : ‖↑↑((h7.l₀' q hq0 h2mq : ℕ) : ℂ) - ((x:ℕ) : ℂ )‖ ≤ norm (h7.m : ℂ) := by {
            simp only [mem_sdiff, Finset.mem_range, Finset.mem_singleton] at hx
            have : (h7.l₀' q hq0 h2mq) - x ≤ h7.m := by {
              simp only [tsub_le_iff_right]
              refine Nat.le_add_right_of_le ?_
              exact Fin.is_le'
            }
            have : ‖↑↑((h7.l₀' q hq0 h2mq : ℕ) : ℂ) - ((x:ℕ) : ℂ )‖
              = (h7.l₀' q hq0 h2mq : ℂ) - (x :ℂ) := by {
              sorry
          ...
          have := h7.norm_sub_l0_lower_bound_on_sphere_inv q hq0 h2mq hz
          simp only [one_div, inv_div] at this
          --simp only [norm_mul, Real.norm_natCast, ge_iff_le]
          refine div_le_div₀ ?_ ?_ ?_ ?_
          · positivity
          · calc _ ≤ norm (((h7.l₀' q hq0 h2mq : ℕ) : ℂ)) + norm ((x : ℕ): ℂ) := ?_
                 _ ≤ ‖(↑h7.m * ↑q : ℂ)‖ := ?_
            · apply norm_sub_le
            · simp only [norm_natCast, Complex.norm_mul]
              sorry
              -- have etc (a b c d : ℝ) (ha0 : 0 < a )(hb0 : 0 < b )
              --    (ha : a ≤ c) (hb : b ≤ d) : (a + b) ≤ a * b := by {
              --     refine (mul_inv_le_iff₀ hb0).mp ?_
              --     rw [add_mul]
              --     sorry
              --    }
            · simp only [norm_natCast, norm_mul, Real.norm_natCast, le_refl]
          ·
             have := h7.r_qt_0 q hq0 h2mq
             simp only [norm_mul, Real.norm_natCast, gt_iff_lt]
             refine Left.mul_pos ?_ ?_
             · simp only [Nat.cast_pos]; exact h7.hm
             · simp only [Nat.cast_pos]; exact h7.r_qt_0 q hq0 h2mq
...
lemma eq7 (l' : Fin (h7.m)) :
  ...
  · sorry--apply h7.sys_coeff_foo_S q hq0 h2mq
...
(h7.r q hq0 h2mq) ^ ((h7.r q hq0 h2mq) * (3 - h7.m : ℝ) / 2 + 3 / 2) := by
  let Cnum : ℝ := sorry
...
lemma use6and8 :
  ...
  calc _ ≤  ‖ρᵣ h7 q hq0 h2mq‖ * (house (rho h7 q hq0 h2mq)) ^ (h7.h - 1 : ℝ) := ?_

       _ ≤ (h7.c₈ ^ h7.r q hq0 h2mq * ↑(h7.r q hq0 h2mq :ℝ) ^
          (h7.r q hq0 h2mq + 3 / 2))^(h7.h-1 : ℝ) *
          ((c₁₃ ^ h7.r q hq0 h2mq * ↑(h7.r q hq0 h2mq : ℝ) ^
           (h7.r q hq0 h2mq * (3 - h7.m : ℝ) / 2 + 3 / 2 : ℝ))) := ?_

       _ ≤ ((h7.c₁₄)^(h7.r q hq0 h2mq : ℝ)) * (↑(h7.r q hq0 h2mq: ℝ))^(
         (↑h7.h - 1 : ℝ ) * ((h7.r q hq0 h2mq) + 3/2) +
         (h7.r q hq0 h2mq * (3 - h7.m : ℝ) / 2 + 3 / 2)) := ?_
