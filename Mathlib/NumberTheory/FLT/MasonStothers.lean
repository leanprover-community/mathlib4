...
theorem divRadical_dvd_wronskian_left (a b : k[X]) : (divRadical a) ∣ wronskian a b :=
  by
  rw [wronskian]
  apply dvd_sub
  · apply dvd_mul_of_dvd_left
...
theorem divRadical_dvd_wronskian_right (a b : k[X]) : (divRadical b) ∣ wronskian a b :=
  by
...
protected theorem IsCoprime.divRadical {a b : k[X]} (h : IsCoprime a b) :
    IsCoprime (divRadical a) (divRadical b) :=
  by
...
private theorem abc_subcall {a b c w : k[X]} {hw : w ≠ 0} (wab : w = wronskian a b) (ha : a ≠ 0)
    ...
    (abc_dr_dvd_w : divRadical (a * b * c) ∣ w) : c.natDegree + 1 ≤ (radical (a * b * c)).natDegree :=
  by
