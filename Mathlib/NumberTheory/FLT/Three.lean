...
private lemma lambda_pow_dvd_a_add_b : λ ^ (3 * S.multiplicity - 2) ∣ S.a + S.b := by
  ...
  rw [show 3 * multiplicity S = 3 * multiplicity S - 2 + 1 + 1 by grind, pow_succ, pow_succ,
...
private lemma lambda_not_dvd_x : ¬ λ ∣ S.x := fun h ↦ by
  ...
  have := S.two_le_multiplicity
  rw [show 3 * multiplicity S - 2 + 1 + 1 = 3 * multiplicity S by grind, mul_pow, ← pow_mul,
...
private lemma x_mul_y_mul_z_eq_u_mul_w_cube : S.x * S.y * S.z = S.u * S.w ^ 3 := by
  suffices hh : λ ^ (3 * S.multiplicity - 2) * S.x * λ * S.y * λ * S.z =
    ...
    rw [mul_comm _ (λ ^ (3 * multiplicity S)), ← pow_succ, ← pow_succ,
      show 3 * multiplicity S - 2 + 1 + 1 = 3 * multiplicity S by grind, mul_assoc, mul_assoc,
      mul_assoc] at hh
...
private lemma formula2 :
  ...
  rw [← formula1 S]
  congrm ?_ + ?_ + ?_
  · have : (S.multiplicity-1)*3+1 = 3*S.multiplicity-2 := by have := S.two_le_multiplicity; grind
...
private lemma lambda_sq_div_u₅_mul : λ ^ 2 ∣ S.u₅ * (λ ^ (S.multiplicity - 1) * S.X) ^ 3 := by
  ...
  have : 3*(S.multiplicity-1) = 2+(3*S.multiplicity-5) := by have := S.two_le_multiplicity; grind
...
lemma Solution'_descent_multiplicity_lt :
  ...
  exact Nat.pred_lt <| by have := S.two_le_multiplicity; grind
