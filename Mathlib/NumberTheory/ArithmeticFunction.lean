...
@[simp]
lemma pdiv_zeta [DivisionSemiring R] (f : ArithmeticFunction R) :
    pdiv f zeta = f := by
  ext n
...
lemma map_prod {ι : Type*} [CommMonoidWithZero R] (g : ι → ℕ) {f : Nat.ArithmeticFunction R}
...
lemma nat_cast {f : ArithmeticFunction ℕ} [Semiring R] (h : f.IsMultiplicative) :
...
lemma int_cast {f : ArithmeticFunction ℤ} [Ring R] (h : f.IsMultiplicative) :
    IsMultiplicative (f : ArithmeticFunction R) :=
                                 -- porting note: was `by simp [cop, h]`
  ⟨by simp [h], fun {m n} cop => by simp [h.2 cop]⟩
...
lemma multiplicative_factorization [CommMonoidWithZero R] (f : ArithmeticFunction R)
...
lemma iff_ne_zero [MonoidWithZero R] {f : ArithmeticFunction R} :
    IsMultiplicative f ↔
...
lemma eq_iff_eq_on_prime_powers [CommMonoidWithZero R] (f : ArithmeticFunction R)
...
@[simp]
lemma cardFactors_one : Ω 1 = 0 := by simp [cardFactors]
...
lemma sum_eq_iff_sum_smul_moebius_eq [AddCommGroup R] {f g : ℕ → R} :
...
lemma sum_eq_iff_sum_mul_moebius_eq [Ring R] {f g : ℕ → R} :
    (∀ n > 0, ∑ i in n.divisors, f i = g n) ↔
...
lemma prod_eq_iff_prod_pow_moebius_eq [CommGroup R] {f g : ℕ → R} :
    (∀ n > 0, ∏ i in n.divisors, f i = g n) ↔
...
lemma prod_eq_iff_prod_pow_moebius_eq_of_nonzero [CommGroupWithZero R] {f g : ℕ → R}
...
set. -/
lemma sum_eq_iff_sum_mul_moebius_eq_on [Ring R] {f g : ℕ → R}
    (s : Set ℕ) (hs : ∀ m n, m ∣ n → n ∈ s → m ∈ s) :
    (∀ n > 0, n ∈ s → (∑ i in n.divisors, f i) = g n) ↔
      ∀ n > 0, n ∈ s →
...
well-behaved set. -/
lemma prod_eq_iff_prod_pow_moebius_eq_on [CommGroup R] {f g : ℕ → R}
    (s : Set ℕ) (hs : ∀ m n, m ∣ n → n ∈ s → m ∈ s) :
    (∀ n > 0, n ∈ s → (∏ i in n.divisors, f i) = g n) ↔
...
a well-behaved set. -/
lemma prod_eq_iff_prod_pow_moebius_eq_on_of_nonzero [CommGroupWithZero R]
