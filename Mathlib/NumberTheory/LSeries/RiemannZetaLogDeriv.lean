...
theorem logDeriv_riemannZeta_one_sub {s : ℂ} (hs : ∀ n : ℤ, s ≠ n) (hz : riemannZeta s ≠ 0) :
  ...
  have : cos (π * s / 2) ≠ 0 := by
    ...
    exact fun ⟨k, _⟩ ↦ hs (2 * k + 1) (mod_cast by grind)
  ...
  have : (fun z ↦ riemannZeta (1 - z)) =ᶠ[𝓝 s]
  ...
  rw [(by rfl : (fun _ ↦ _) = riemannZeta ∘ (1 - ·)), logDeriv_comp, logDeriv_fun_mul,
    logDeriv_fun_mul, logDeriv_fun_mul, logDeriv_fun_mul, ← digamma_def] at this
