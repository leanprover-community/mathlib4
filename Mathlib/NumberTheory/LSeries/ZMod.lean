...
lemma differentiableAt_LFunction (Φ : ZMod N → ℂ) (s : ℂ) (hs : s ≠ 1 ∨ ∑ j, Φ j = 0) :
  ...
  apply (differentiable_const_cpow_neg ..).mul
...
lemma differentiable_completedLFunction₀ (Φ : ZMod N → ℂ) :
  ...
  refine .add ?_ ?_ <;>
  refine (differentiable_const_cpow_neg _).mul (.sum fun i _ ↦ .const_mul ?_ _)
...
lemma differentiableAt_completedLFunction (Φ : ZMod N → ℂ) (s : ℂ) (hs₀ : s ≠ 0 ∨ Φ 0 = 0)
  ...
  · -- term with `1 / s`
    refine (differentiable_const_cpow_neg _ s).mul (hs₀.elim ?_ ?_)
