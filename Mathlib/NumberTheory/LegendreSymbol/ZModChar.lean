...
theorem χ₄_int_eq_if_mod_four (n : ℤ) :
  ...
  have help : ∀ m : ℤ, 0 ≤ m → m < 4 → χ₄ m = if m % 2 = 0 then 0 else if m = 1 then 1 else -1 := by
    decide
  rw [← Int.emod_emod_of_dvd n (by grind : (2 : ℤ) ∣ 4), ← ZMod.intCast_mod n 4]
  exact help (n % 4) (Int.emod_nonneg n (by grind)) (Int.emod_lt_abs n (by grind))
...
theorem χ₄_eq_neg_one_pow {n : ℕ} (hn : n % 2 = 1) : χ₄ n = (-1) ^ (n / 2) := by
  ...
  nth_rewrite 3 [show 4 = 2 * 2 by grind]
  ...
  exact help _ (Nat.mod_lt n (by grind)) <| (Nat.mod_mod_of_dvd n (by grind : 2 ∣ 4)).trans hn
...
theorem χ₈_int_eq_if_mod_eight (n : ℤ) :
  ...
  have help :
    ...
    decide
  rw [← Int.emod_emod_of_dvd n (by grind : (2 : ℤ) ∣ 8), ← ZMod.intCast_mod n 8]
  exact help (n % 8) (Int.emod_nonneg n (by grind)) (Int.emod_lt_abs n (by grind))
...
theorem χ₈'_int_eq_χ₄_mul_χ₈ (a : ℤ) : χ₈' a = χ₄ a * χ₈ a := by
  rw [← @cast_intCast 8 (ZMod 4) _ 4 _ (by grind) a]
