...
def fastJacobiSymAux (a b : ℕ) (flip : Bool)
  ...
  if ha4 : a % 4 = 0 then
    fastJacobiSymAux (a / 4) b flip
      ((a.div_le_self 4).trans_lt hab) hb2
      (a.div_pos (Nat.le_of_dvd ha0 (Nat.dvd_of_mod_eq_zero ha4)) (by decide)) hb1
  else if ha2 : a % 2 = 0 then
    fastJacobiSymAux (a / 2) b (xor (b % 8 = 3 ∨ b % 8 = 5) flip)
      ((a.div_le_self 2).trans_lt hab) hb2
    ...
    fastJacobiSymAux (b % a) a (xor (a % 4 = 3 ∧ b % 4 = 3) flip)
      (b.mod_lt ha0) (Nat.mod_two_ne_zero.mp ha2)
      (Nat.pos_of_ne_zero hba) (lt_of_le_of_ne ha0 (Ne.symm ha1))
decreasing_by
...
theorem fastJacobiSymAux.eq_jacobiSym (a b : ℕ) (flip : Bool)
    (hab : a < b) (hb2 : b % 2 = 1) (ha0 : a > 0) (hb1 : b > 1) :
    fastJacobiSymAux a b flip hab hb2 ha0 hb1 = if flip then -J(a | b) else J(a | b) := by
  unfold fastJacobiSymAux
  split_ifs with h₁ h₂ h₃ h₄ h₅ h₆ h₇ h₈ h₉
  any_goals apply (fastJacobiSymAux.eq_jacobiSym _ _ _ _ _ _ _).trans
...
def fastJacobiSym (a : ℤ) (b : ℕ) : ℤ :=
  if hb0 : b = 0 then
    ...
    fastJacobiSymAux (a % b).natAbs b false ((Int.natAbs_lt_natAbs_of_nonneg_of_lt
      (a.emod_nonneg hb0'.ne') (a.emod_lt_of_pos hb0')).trans_eq (Int.natAbs_ofNat b))
      (Nat.mod_two_ne_zero.mp hb2) (Int.natAbs_pos.mpr hab)
      (lt_of_le_of_ne (Nat.one_le_iff_ne_zero.mpr hb0) (Ne.symm hb1))
decreasing_by exact b.div_lt_self (Nat.pos_of_ne_zero hb0) one_lt_two

@[csimp]
theorem fastJacobiSym.eq : jacobiSym = fastJacobiSym :=
  funext₂ h
where
  h (a : ℤ) (b : ℕ) : jacobiSym a b = fastJacobiSym a b := by
    ...
    · exact jacobiSym.eq_zero_iff.mpr ⟨hb0, ne_of_gt <|
        ...
        Int.ofNat_dvd_left.mp (Int.dvd_of_emod_eq_zero (mod_cast hb2))⟩
    · rw [← h a (b / 2)]
    ...
    · rw [fastJacobiSymAux.eq_jacobiSym, if_neg Bool.false_ne_true, jacobiSym.mod_left a b,
