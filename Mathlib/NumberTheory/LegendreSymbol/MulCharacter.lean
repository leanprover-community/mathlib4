...
instance (priority := 10000) MulChar.instFunLike : FunLike (MulChar R R') R R' :=
  ⟨fun χ => χ.toFun,
...
instance (priority := 10000) : MulCharClass (MulChar R R') R R' where
  map_mul χ := χ.map_mul'
  map_one χ := χ.map_one'
...
noncomputable instance (priority := 10000) hasOne : One (MulChar R R') :=
  ⟨trivial R R'⟩
#align mul_char.has_one MulChar.hasOne

noncomputable instance (priority := 10000) inhabited : Inhabited (MulChar R R') :=
  ⟨1⟩
...
instance (priority := 10000) hasMul : Mul (MulChar R R') :=
  ⟨mul⟩
...
noncomputable instance (priority := 10000) hasInv : Inv (MulChar R R') :=
  ⟨inv⟩
...
noncomputable instance (priority := 10000) commGroup : CommGroup (MulChar R R') :=
  { one := 1
    mul := (· * ·)
    inv := Inv.inv
