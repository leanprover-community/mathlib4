...
lemma is_prime_of_minimal_nat_zero_lt_and_lt_one : p.Prime := by
  ...
  · rintro a b rfl
    ...
    have hap : a < a * b := lt_mul_of_one_lt_right (by grind) (by grind)
    have hbp : b < a * b := lt_mul_of_one_lt_left (by grind) (by grind)
    have ha :=
...
-- For 0 < logb n₀ n below we also need to exclude n = 1.
  ...
  nth_rw 2 [← mul_one 1]
  have : 0 < logb n₀ n := logb_pos (mod_cast hn₀) (by norm_cast; grind)
