  ...
  if hfg : f + g ≈ 0 then by
    ...
    if hf : f ≈ 0 then by
      have hfg' : f + g ≈ g := by
        ...
        show LimZero (f + g - g); · simpa only [sub_zero, add_sub_eq_left] using hf
      ...
      if hg : g ≈ 0 then by
        have hfg' : f + g ≈ f := by
          ...
          show LimZero (f + g - f); · simpa only [add_sub_eq_right, sub_zero] using hg
...
theorem add_eq_max_of_ne {f g : PadicSeq p} (hfgne : f.norm ≠ g.norm) :
  ...
  if hf : f ≈ 0 then by
    have : LimZero (f - 0) := hf
    have : f + g ≈ g := show LimZero (f + g - g) by simpa only [sub_zero, add_sub_eq_left]
    ...
    if hg : g ≈ 0 then by
      have : LimZero (g - 0) := hg
      have : f + g ≈ f := show LimZero (f + g - f) by rw [add_sub_eq_right]; simpa only [sub_zero]
