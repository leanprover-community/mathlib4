...
* `padicValNat_factorial`: Legendre's Theorem. The `p`-adic valuation of `n!` is the sum of the
  ...
  greater than `log p n`. See `Nat.Prime.padicValNat_factorial` for the same result but stated in
  the language of prime padicValNat.
...
* `padicValNat_choose`: Kummer's Theorem. The `p`-adic valuation of `n.choose k` is the number
  ...
  `Ico 1 b` where `b` is any bound greater than `log p n`. See `Nat.Prime.padicValNat_choose` for
  the same result but stated in the language of prime padicValNat.
...
/-- The `p`-adic value of an integer `z ≠ 0` is the padicValNat of `p` in `z`. -/
theorem of_int_padicValNat {z : ℤ} :
...
/-- The padicValNat of `p : ℕ` in `a : ℤ` is finite exactly when `a ≠ 0`. -/
...
theorem isSquare_iff_even_factorization {a : ℚ} :
  ...
  · intro ⟨hR, hf⟩
    ...
    · refine Int.isSquare_iff_nonneg_even_factorization.mpr ⟨num_nonneg.mpr hR, fun p hp ↦ ?_⟩
      ...
      simpa [Nat.factorization_def _ hp, ← padicValNat_def, padicValInt] using num_even
    · refine Nat.isSquare_iff_even_factorization.mpr (fun p hp ↦ ?_)
      ...
      rwa [Nat.factorization_def _ hp, ← padicValNat_def]
