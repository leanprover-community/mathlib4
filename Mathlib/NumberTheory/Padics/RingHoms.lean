...
@[simp]
lemma cast_toZModPow (m n : ℕ) (h : m ≤ n) (x : ℤ_[p]) :
...
lemma denseRange_natCast : DenseRange (Nat.cast : ℕ → ℤ_[p]) := by
  intro x
...
lemma denseRange_intCast : DenseRange (Int.cast : ℤ → ℤ_[p]) := by
  intro x
...
@[simp]
lemma nthHom_zero : nthHom f 0 = 0 := by
