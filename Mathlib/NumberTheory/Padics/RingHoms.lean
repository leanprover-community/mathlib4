...
and the integers modulo powers of `p`, `ℤ/p^nℤ`

## Main declarations

We show that `ℤ_[p]` has a ring hom to `ℤ/p^nℤ` for each `n`.
...
* `PadicInt.toZMod`: ring homomorphism to `ZMod p`
* `PadicInt.toZModPow`: ring homomorphism to `ZMod (p^n)`
* `PadicInt.ker_toZMod` / `PadicInt.ker_toZModPow`: the kernels of these maps are the ideals
  generated by `p^n`
* `PadicInt.residueField` shows that the residue field of `ℤ_[p]` is isomorhic to `ZMod p`.

We also establish the universal property of `ℤ_[p]` as a projective limit.
Given a family of compatible ring homs `f_k : R → ℤ/p^nℤ`,
