...
We show that if `G` is a topological monoid, then `D(G, R)` inherits a convolution product from `G`,
and that if `G` is locally compact, this makes `D(G, R)` into a ring (and `C(G, R)` into a module
over it). Moreover, if `G` is abelian and compact, then `D(G, R)` is a commutative ring.
-/
...
lemma convolveFunRight_apply (μ : D(G, R)) (f : C(G, R)) (x : G) :
    μ.convolveFunRight f x = μ ⟨fun y ↦ f (x * y), by fun_prop⟩ := by
...
lemma one_def : (1 : D(G, R)) = dirac R 1 := rfl
...
def diracHom : G →* D(G, R) where
  toFun := dirac R
  map_one' := by ext; simp
  map_mul' _ _ := by ext; simp [dirac_mul_dirac]

-- this should be autogenerated by `@[simps]` but it does not work on non-exposed defs at present
@[simp] lemma diracHom_apply (x : G) : diracHom x = dirac R x := (rfl)
...
The compactness and separation assumptions arise since the product of two measures is
defined by an iterated integral over `G × G`; a Fubini-type theorem is needed to swap the
order of the two integration variables. The Fubini theorem we use
