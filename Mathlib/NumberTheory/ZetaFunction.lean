...
theorem summable_exp_neg_pi_mul_nat_sq {t : ℝ} (ht : 0 < t) :
    Summable fun n : ℕ => rexp (-π * t * (n + 1) ^ 2) := by
...
theorem zetaKernel₂_one_div {t : ℝ} (ht : 0 < t) : zetaKernel₂ (1 / t) = sqrt t * zetaKernel₂ t :=
  by
  have aux : ∀ {u : ℝ} (hu : 1 < u), zetaKernel₂ (1 / u) = sqrt u * zetaKernel₂ u := by
    intro u hu
...
theorem isBigO_atTop_zetaKernel₁ : IsBigO atTop zetaKernel₁ fun t => exp (-π * t) := by
  ...
  have h' : tendsto (fun t : ℝ => ↑t * I) at_top (comap im at_top) := by
...
theorem isBigO_zero_zetaKernel₂_rpow (a : ℝ) : IsBigO (𝓝[>] 0) zetaKernel₂ fun t => t ^ a := by
  ...
  have aux2 : is_O at_top (fun t => exp (-π * t) * sqrt t) fun t => t ^ (-a) := by
...
theorem isBigO_zero_zetaKernel₁ : IsBigO (𝓝[>] 0) zetaKernel₁ fun t => t ^ (-(1 / 2) : ℝ) := by
  have : zetaKernel₁ =ᶠ[𝓝[>] 0] zetaKernel₂ + fun t => (1 / sqrt t - 1) / 2 := by
    refine'
...
theorem hasMellin_one_div_sqrt_Ioc {s : ℂ} (hs : 1 / 2 < re s) :
  ...
  have h1 : eq_on (fun t => 1 / ↑(sqrt t) : ℝ → ℂ) (fun t => ↑t ^ (-1 / 2 : ℂ)) (Ioc 0 1) := by
    intro t ht
...
theorem mellin_zetaKernel₁_eq_tsum {s : ℂ} (hs : 1 / 2 < s.re) :
    mellin zetaKernel₁ s = π ^ (-s) * Gamma s * ∑' n : ℕ, 1 / (n + 1) ^ (2 * s) := by
  ...
  have h_norm : ∀ (n : ℕ) {t : ℝ} (ht : 0 < t), ‖f n t‖ = bd n t := by
    intro n t ht
  ...
  have hf_meas : ∀ n : ℕ, ae_strongly_measurable (f n) (volume.restrict <| Ioi 0) := by
    intro n
  ...
  have h_sum0 : ∀ {t : ℝ} (ht : 0 < t), HasSum (fun n => f n t) (t ^ (s - 1) * zetaKernel₁ t) := by
    intro t ht
    have :=
...
theorem completed_zeta_eq_tsum_of_one_lt_re {s : ℂ} (hs : 1 < re s) :
    riemannCompletedZeta s = π ^ (-s / 2) * Gamma (s / 2) * ∑' n : ℕ, 1 / (n + 1) ^ s := by
...
theorem riemannZeta_two : riemannZeta 2 = π ^ 2 / 6 := by
...
theorem riemannZeta_one_sub {s : ℂ} (hs : ∀ n : ℕ, s ≠ -n) (hs' : s ≠ 1) :
    riemannZeta (1 - s) =
      2 ^ (1 - s) * π ^ (-s) * Gamma s * sin (π * (1 - s) / 2) * riemannZeta s := by
...
theorem riemannZeta_neg_nat_eq_bernoulli (k : ℕ) :
    riemannZeta (-k) = (-1) ^ k * bernoulli (k + 1) / (k + 1) := by
  ...
  · -- k = 2 * m + 1 : the interesting case
    ...
    rw [show Complex.sin (↑π * (1 - (2 * ↑m + 2)) / 2) = -(-1) ^ m by
