...
variable (M C : Type*) [CommRing C] [IsDomain C] [IsIntegrallyClosed C] [Field M] [Algebra C M]
  [IsFractionRing C M] [Algebra K M] [FiniteDimensional K M] [Algebra A C] [Algebra B C]
  [Module.Finite A C] [Algebra A M] [IsScalarTower A C M] [IsScalarTower A K M] [Module.Finite B C]
...
variable {B} (P : Ideal B) [P.IsMaximal] (M C : Type*) [CommRing C] [IsDomain C]
  [IsIntegrallyClosed C] [Field M] [Algebra C M] [IsFractionRing C M] [Algebra K M]
  [FiniteDimensional K M] [Algebra A M] [Algebra A C] [Algebra B C] [Module.Finite A C]
  [IsScalarTower A C M] [IsScalarTower A K M] [Module.Finite B C] [NoZeroSMulDivisors B C]
  [Algebra L M] [Algebra B M] [IsScalarTower B C M] [IsScalarTower B L M] [FiniteDimensional L M]
  [IsScalarTower A B C] [IsDedekindDomain C] [NoZeroSMulDivisors A B] [NoZeroSMulDivisors A C]
  [IsScalarTower K L M]

theorem ncard_primesOver_mul_ncard_primesOver [IsGalois K L] [IsGalois K M] [P.LiesOver p]
    (hp : p ≠ ⊥) :
