...
theorem bernoulli'_odd_eq_zero {n : ℕ} (h_odd : Odd n) (hlt : 1 < n) : bernoulli' n = 0 := by
  let B := PowerSeries.mk fun n => bernoulli' n / (n ! : ℚ)
...
theorem sum_range_pow (n p : ℕ) :
  ...
  have hps :
    ...
    suffices
      (PowerSeries.mk fun p => ∑ k ∈ range n, (k : ℚ) ^ p * algebraMap ℚ ℚ p !⁻¹) =
    ...
    have h_r : exp ℚ ^ n - 1 = X * PowerSeries.mk fun p => coeff ℚ (p + 1) (exp ℚ ^ n) := by
