...
noncomputable
def Ideal.ResidueField.lift {R S : Type*} [CommRing R] [CommRing S] (I : Ideal R) [I.IsPrime]
...
lemma Ideal.ResidueField.lift_algebraMap
    {R S : Type*} [CommRing R] [CommRing S] (I : Ideal R) [I.IsPrime]
    (f : R →+* S) (hf₁ : I ≤ RingHom.ker f)
...
noncomputable
def Ideal.ResidueField.liftₐ {R S T : Type*} [CommRing R] [CommRing S] (I : Ideal S) [I.IsPrime]
    [CommRing T] [Algebra R S] [Algebra R T]
    (f : S →ₐ[R] T) (hf₁ : I ≤ RingHom.ker f)
    (hf₂ : I.primeCompl ≤ (IsUnit.submonoid T).comap f) : I.ResidueField →ₐ[R] T where
  __ := Ideal.ResidueField.lift I f.toRingHom hf₁ hf₂
  commutes' r := by simp [IsScalarTower.algebraMap_apply R S I.ResidueField]

@[simp]
lemma Ideal.ResidueField.liftₐ_algebraMap
    {R S T : Type*} [CommRing R] [CommRing S] (I : Ideal S) [I.IsPrime]
    [CommRing T] [Algebra R S] [Algebra R T]
    (f : S →ₐ[R] T) (hf₁ : I ≤ RingHom.ker f)
    (hf₂ : I.primeCompl ≤ (IsUnit.submonoid T).comap f) (r : S) :
    liftₐ I f hf₁ hf₂ (algebraMap _ _ r) = f r :=
  lift_algebraMap _ _ _ hf₂ _

@[simp]
lemma Ideal.ResidueField.liftₐ_comp_toScalarTower
    {R S T : Type*} [CommRing R] [CommRing S] (I : Ideal S) [I.IsPrime]
    [CommRing T] [Algebra R S] [Algebra R T]
    (f : S →ₐ[R] T) (hf₁ : I ≤ RingHom.ker f)
    (hf₂ : I.primeCompl ≤ (IsUnit.submonoid T).comap f) :
    (liftₐ I f hf₁ hf₂).comp (IsScalarTower.toAlgHom _ S _) = f :=
...
lemma Ideal.ResidueField.algHom_ext
    {R S T : Type*} [CommRing R] [CommRing S] (I : Ideal S) [I.IsPrime]
    [CommRing T] [Algebra R S] [Algebra R T]
    (f g : I.ResidueField →ₐ[R] T)
    (H : f.comp (IsScalarTower.toAlgHom R S _) = g.comp (IsScalarTower.toAlgHom R S _)) :
    f = g :=
  AlgHom.coe_ringHom_injective (ringHom_ext _ _ _ congr($H))

@[simp]
lemma Ideal.ResidueField.map_algebraMap {R A : Type*} [CommRing R] [CommRing A] (I : Ideal R)
    [I.IsPrime] (J : Ideal A) [J.IsPrime]
    (f : R →+* A) (hf : I = J.comap f) (r) :
    Ideal.ResidueField.map I J f hf (algebraMap R _ r) = algebraMap _ _ (f r) :=
  (IsLocalRing.ResidueField.map_residue (Localization.localRingHom I J f hf) _).trans
    (by simp [Localization.localRingHom_to_map]; rfl)
