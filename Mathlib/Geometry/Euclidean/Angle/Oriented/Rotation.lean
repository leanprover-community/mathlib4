...
def rotationAux (θ : Real.Angle) : V →ₗᵢ[ℝ] V :=
  LinearMap.isometryOfInner
    ...
    (by
      intro x y
      simp only [ROrCLike.conj_to_real, id.def, LinearMap.smul_apply, LinearMap.add_apply,
...
theorem kahler_rotation_left (x y : V) (θ : Real.Angle) :
  ...
  -- Porting note: this needed the `Complex.conj_ofReal` instead of `ROrCLike.conj_ofReal`;
