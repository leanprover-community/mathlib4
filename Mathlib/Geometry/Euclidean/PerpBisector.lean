...
`c -ᵥ midpoint ℝ p₁ p₂`. -/
lemma mem_perpBisector_iff_inner_eq_zero' :
...
lemma mem_perpBisector_iff_inner_pointReflection_vsub_eq_zero :
...
lemma mem_perpBisector_pointReflection_iff_inner_eq_zero :
...
lemma perpBisector_nonempty : (perpBisector p₁ p₂ : Set P).Nonempty :=
...
lemma mem_perpBisector_iff_inner_eq_inner :
...
lemma mem_perpBisector_iff_inner_eq :
...
lemma mem_perpBisector_iff_dist_eq : c ∈ perpBisector p₁ p₂ ↔ dist c p₁ = dist c p₂ := by
...
@[simp] lemma perpBisector_ne_bot : perpBisector p₁ p₂ ≠ ⊥ := by
...
lemma Isometry.preimage_perpBisector {f : P → P'} (h : Isometry f) (p₁ p₂ : P) :
    f ⁻¹' (perpBisector (f p₁) (f p₂)) = perpBisector p₁ p₂ := by
  ext x; simp [mem_perpBisector_iff_dist_eq, h.dist_eq]

lemma Isometry.mapsTo_perpBisector {f : P → P'} (h : Isometry f) (p₁ p₂ : P) :
