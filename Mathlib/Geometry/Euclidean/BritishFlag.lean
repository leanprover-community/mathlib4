...
theorem norm_sq_add_norm_sub_add_sq_eq_norm_sub_sq_add_norm_sub_sq_add_two_inner (x u v : V) :
    ‖x‖ * ‖x‖ + ‖x - (u + v)‖ * ‖x - (u + v)‖ =
      ‖x - u‖ * ‖x - u‖ + ‖x - v‖ * ‖x - v‖ + 2 * ⟪u, v⟫ := by
  simp only [← real_inner_self_eq_norm_mul_norm, inner_sub_left, inner_sub_right, inner_add_left,
...
theorem norm_sq_add_norm_sub_add_sq_eq_norm_sub_sq_add_norm_sub_sq_iff_inner_eq_zero (x u v : V) :
    ‖x‖ * ‖x‖ + ‖x - (u + v)‖ * ‖x - (u + v)‖ = ‖x - u‖ * ‖x - u‖ + ‖x - v‖ * ‖x - v‖ ↔
      ⟪u, v⟫ = 0 := by
...
private theorem dist_sq_add_dist_sq_eq_dist_sq_add_dist_sq_add_two_inner {a b c d : P}
    ...
    dist p a * dist p a + dist p c * dist p c =
      dist p b * dist p b + dist p d * dist p d + 2 * ⟪b -ᵥ a, d -ᵥ a⟫ := by
...
theorem dist_sq_add_dist_sq_eq_dist_sq_add_dist_sq_add_two_mul_dist_mul_dist_mul_cos_angle
    ...
    dist p a * dist p a + dist p c * dist p c =
      dist p b * dist p b + dist p d * dist p d +
        2 * dist b a * dist d a * Real.cos (∠ b a d) := by
...
theorem dist_sq_add_dist_sq_eq_dist_sq_add_dist_sq_iff_angle_eq_pi_div_two {a b c d : P}
    (h : midpoint ℝ a c = midpoint ℝ b d) (p : P) :
    dist p a * dist p a + dist p c * dist p c = dist p b * dist p b + dist p d * dist p d ↔
      ∠ b a d = π / 2 := by
