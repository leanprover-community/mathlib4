...
variable {s} in
lemma eq_touchpoint_of_isTangentAt_insphere {i : Fin (n + 1)} {p : P}
    (ht : s.insphere.IsTangentAt p (affineSpan ℝ (Set.range (s.faceOpposite i).points))) :
    p = s.touchpoint ∅ i :=
  eq_touchpoint_of_isTangentAt_exsphere ht
