...
lemma inter_orthRadius_eq_singleton_of_dist_eq_radius {s : Sphere P} {p : P}
  ...
  ext p'
  simp only [Set.mem_inter_iff, Metric.mem_sphere, mem_coe', SetLike.mem_coe, mem_singleton
  constructor
  · rintro ⟨hp's, hp'i⟩
...
lemma inter_orthRadius_eq_singleton_iff {s : Sphere P} {p q : P} :
  ...
  constructor
  · intro h
    have hq : q ∈ (s ∩ s.orthRadius p : Set P) := h ▸ mem_singleton_self _
...
lemma inter_orthRadius_eq_of_dist_le_radius_of_norm_eq_one [hf2 : Fact (Module.finrank ℝ V = 2)]
  ...
  ext p'
  refine ⟨fun h ↦ ?_, fun h ↦ ?_⟩
  · rw [← vsub_vadd p' p, Set.mem_insert_iff, mem_singletonight_cancel_iff,
