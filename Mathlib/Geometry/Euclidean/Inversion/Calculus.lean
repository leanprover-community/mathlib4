...
theorem hasFDerivAt_inversion (hx : x ≠ c) :
  ...
  -- the line `simp (config := { unfoldPartialApp := true }) only [inversion]` is failing
  -- I think something has gone wrong with `unfoldPartialApp`.
  -- A minimization would be very helpful!
  sorry
