...
instance AlgebraicGeometry.SheafedSpace.section_over_bot_unique {X : SheafedSpace CommRingCat} :
  ...
  uniq := fun a ↦ by
    ...
    have h := TopCat.Sheaf.eq_of_locally_eq X.sheaf U
    simp only [IsEmpty.forall_iff, true_implies] at h
    rw [show iSup U = ⊥ by simp only [iSup_eq_bot, implies_true]] at h
    exact h a 0
...
pushforward of `(Scheme.specObj (X.stalk x)).presheaf` along the constant map `fun _ ↦ x`.
-/
noncomputable def ObjToPushforwardObjHomOfMem {X : LocallyRingedSpace} (x : X)
    (O : (TopologicalSpace.Opens ↑↑X.toPresheafedSpace)ᵒᵖ) (hxO : x ∈ O.unop) :
    X.presheaf.obj O ⟶ (⟨fun _ ↦ x, continuous_const⟩ _*
    (Scheme.specObj (X.stalk x)).presheaf).obj O :=
  let x' : O.unop := ⟨x, hxO⟩
  let hom1 := @TopCat.Presheaf.germ CommRingCat _ _ X.toTopCat X.presheaf O.unop x'
  let hom2 := StructureSheaf.toOpen (X.stalk x) ⊤
  CategoryTheory.CategoryStruct.comp hom1 (CategoryTheory.CategoryStruct.comp hom2
    ((Scheme.specObj (X.stalk x)).presheaf.map (TopologicalSpace.Opens.leTop _).op))
...
of the pushforward of `(Scheme.specObj (X.stalk x)).presheaf` along the constant map `fun _ ↦ x`.
-/
noncomputable def ObjToPushforwardObjHomOfNotMem {X : LocallyRingedSpace} (x : X)
    (O : (TopologicalSpace.Opens ↑↑X.toPresheafedSpace)ᵒᵖ) (hxO : x ∉ O.unop) :
    X.presheaf.obj O ⟶ (⟨fun _ ↦ x, continuous_const⟩ _*
    (Scheme.specObj (X.stalk x)).presheaf).obj O where
  toFun := fun _ ↦ 0
  map_one' := by
    rw [TopCat.Presheaf.pushforwardObj_obj, CategoryTheory.Functor.op_obj,
      (le_bot_iff.mp fun _ ↦ hxO : (@TopologicalSpace.Opens.map (Scheme.specObj (X.stalk x))
...
`(Scheme.specObj (X.stalk x)).presheaf` along the constant map `fun _ ↦ x`.
-/
noncomputable def ObjToPushforwardObjHom {X : LocallyRingedSpace} (x : X)
...
This theorem is a basis for constructing the canonical morphism from the spectrum of `X.stalk x` to
`X`.
-/
theorem ObjToPushforwardObjHom_naturality {X : LocallyRingedSpace} (x : X) :
    ...
    (Scheme.specObj (X.stalk x)).presheaf).map f) := by
  intro O1 O2 f
  ext s
  ...
  · have hxO1 : x ∈ O1.unop := CategoryTheory.le_of_op_hom f hxO2
    ...
    rfl
  · simp only [hxO2]
    have : Unique (((Scheme.specObj (X.stalk x)).presheaf.obj
        { unop := (TopologicalSpace.Opens.map ⟨fun _ ↦ x, continuous_const⟩).obj O2.unop })) := by
      erw [(le_bot_iff.mp fun _ ↦ hxO2 : (@TopologicalSpace.Opens.map (Scheme.specObj (X.stalk x))
        X.toTopCat ⟨fun _ ↦ x, continuous_const⟩).obj O2.unop = ⊥)]
      exact @Spec.structureSheaf.section_over_bot_unique (X.stalk x) _
