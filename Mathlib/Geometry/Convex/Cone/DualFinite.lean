...
be written as the intersection of finitely many halfspaces. This is the counterpart to FG
(finitely generated) which states that the cone is the conic hull of a finite set.
...
/-- A DualFG cone is DualFG w.r.t. the identity pairing. -/
...
lemma DualFG.dual_of_finite (s : Finset M) : (dual p s).DualFG p := by use s
