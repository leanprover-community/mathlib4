...
theorem MDifferentiableWithinAt.smul {f : M → 𝕜} {g : M → V}
  ...
  ((smooth_smul.of_le le_top).mdifferentiable le_rfl _).comp_mdifferentiableWithinAt x
    (hf.prod_mk hg)
...
theorem MDifferentiableAt.smul {f : M → 𝕜} {g : M → V} (hf : MDifferentiableAt I 𝓘(𝕜) f x)
  ...
  ((smooth_smul.of_le le_top).mdifferentiable le_rfl _).comp x (hf.prod_mk hg)
