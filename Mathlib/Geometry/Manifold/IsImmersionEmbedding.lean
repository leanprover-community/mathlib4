...
lemma mk_of_charts (equiv : (E × F) ≃L[𝕜] E'') (domChart : OpenPartialHomeomorph M H)
    ...
    (hwrittenInExtend : EqOn ((codChart.extend J) ∘ f ∘ (domChart.extend I).symm) (equiv ∘ (·, 0))
      (domChart.extend I).target) : IsImmersionAt% F n f x := by
...
noncomputable def domChart (h : IsImmersionAt% F n f x) : OpenPartialHomeomorph M H := by
...
noncomputable def codChart (h : IsImmersionAt% F n f x) : OpenPartialHomeomorph N G := by
  rw [IsImmersionAt_def] at h
  exact LiftSourceTargetPropertyAt.codChart h

lemma mem_domChart_source (h : IsImmersionAt% F n f x) : x ∈ h.domChart.source := by
  rw [IsImmersionAt_def] at h
  exact LiftSourceTargetPropertyAt.mem_domChart_source h

lemma mem_codChart_source (h : IsImmersionAt% F n f x) : f x ∈ h.codChart.source := by
  rw [IsImmersionAt_def] at h
  exact LiftSourceTargetPropertyAt.mem_codChart_source h

lemma domChart_mem_maximalAtlas (h : IsImmersionAt% F n f x) :
    h.domChart ∈ IsManifold.maximalAtlas I n M := by
  rw [IsImmersionAt_def] at h
  exact LiftSourceTargetPropertyAt.domChart_mem_maximalAtlas h

lemma codChart_mem_maximalAtlas (h : IsImmersionAt% F n f x) :
    h.codChart ∈ IsManifold.maximalAtlas J n N := by
  rw [IsImmersionAt_def] at h
  exact LiftSourceTargetPropertyAt.codChart_mem_maximalAtlas h

lemma source_subset_preimage_source (h : IsImmersionAt% F n f x) :
...
noncomputable def equiv (h : IsImmersionAt% F n f x) : (E × F) ≃L[𝕜] E'' := by
  rw [IsImmersionAt_def] at h
  exact Classical.choose <| LiftSourceTargetPropertyAt.property h

lemma writtenInCharts (h : IsImmersionAt% F n f x) :
...
lemma property (h : IsImmersionAt% F n f x) :
...
-/
lemma map_target_subset_target (h : IsImmersionAt% F n f x) :
...
lemma congr_of_eventuallyEq {x : M} (hf : IsImmersionAt% F n f x) (hfg : f =ᶠ[𝓝 x] g) :
    IsImmersionAt% F n g x := by
  rw [IsImmersionAt_def]
...
lemma congr_iff {x : M} (hfg : f =ᶠ[𝓝 x] g) :
    IsImmersionAt% F n f x ↔ IsImmersionAt% F n g x :=
  ⟨fun h ↦ h.congr_of_eventuallyEq hfg, fun h ↦ h.congr_of_eventuallyEq hfg.symm⟩

lemma trans_F (h : IsImmersionAt% F n f x) (e : F ≃L[𝕜] F') : IsImmersionAt% F' n f x := by
...
lemma congr_F (e : F ≃L[𝕜] F') : IsImmersionAt% F n f x ↔ IsImmersionAt% F' n f x :=
  ⟨fun h ↦ trans_F (e := e) h, fun h ↦ trans_F (e := e.symm) h⟩

/- The set of points where `IsImmersionAt` holds is open. -/
lemma isOpen_isImmersionAt : IsOpen {x : M | IsImmersionAt% F n f x} := by
