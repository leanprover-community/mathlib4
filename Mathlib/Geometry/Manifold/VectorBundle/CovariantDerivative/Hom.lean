...
private theorem homBundleAux_tensorial
  ...
  smul hf hv := by
    simp only [homBundleAux, Pi.smul_apply', map_smul, ← Pi.smul_def']
    rw [hcov₁.leibniz hv hf]
    rw [hcov₂.leibniz
      (MDifferentiableAt.clm_bundle_apply hϕ hv) hf]
    simp [smul_sub]
  add hv hv' := by
    rename_i ϕ hϕ v v'
    simp only [homBundleAux, Pi.add_apply, map_add, ← Pi.add_def]
    rw [hcov₂.add
...
def homBundleAt
  ...
  cont := by
    ...
    · let e₁ := VectorBundle.continuousLinearEquivAt 𝕜 F₁ V₁ x
      let e₂ := VectorBundle.continuousLinearEquivAt 𝕜 F₂ V₂ x
      rw [← (e₁.arrowCongrSL e₂).toHomeomorph.comp_continuous_iff]
