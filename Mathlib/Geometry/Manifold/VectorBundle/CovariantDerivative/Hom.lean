...
## Main definitions
* `IsCovariantDerivativeOn.homBundleAt`: the induced covariant derivative on the Hom-bundle
  evaluated at a single point `x`.
* `IsCovariantDerivativeOn.homBundle`:  proof that `homBundleAt` satisfies the properties
  on a set `s`.
...
open Classical in
/--
Given `cov₁` and `cov₂`, which are covariant derivatives on fiber bundles `V₁` and `V₂`
respectively, `homBundleAt` is the induced covariant derivative on the Hom-bundle `Hom(V₁, V₂)`
evaluated at a single point `x`. It is defined through the Leibniz rule:

`(∇_X ϕ) v = ∇_X (ϕ v) - ϕ ∇_X v`,

where `ϕ : V₁ x →L[𝕜] V₂ x` is a section of the Hom-bundle, and `v` is a section of `V₁`
...
/-- Proof that `homBundleAt` locally satisfies the properties of a covariant derivative. -/
theorem homBundle
    (hcov₁ : IsCovariantDerivativeOn F₁ cov₁ s) (hcov₂ : IsCovariantDerivativeOn F₂ cov₂ s) :
