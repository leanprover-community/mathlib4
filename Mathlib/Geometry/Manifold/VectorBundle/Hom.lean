...
lemma Bundle.Trivialization.mdifferentiableWithinAt_apply [ContMDiffVectorBundle 1 F₁ E₁ IB]
    (e : Trivialization F₁ (TotalSpace.proj : TotalSpace F₁ E₁ → B)) [MemTrivializationAtlas e]
    {x : B} (hx : x ∈ e.baseSet) {s : Set B} {σ : Π b, E₁ b} (hσ : MDiffAt[s] (T% σ) x) :
    MDiffAt[s] (fun b ↦ e.continuousLinearMapAt 𝕜 b (σ b)) x := by
  -- TODO: goal states here and above look different, investigate!
  -- apply (mdifferentiableWithinAt_section IB σ).mp hσ |>.congr_of_eventuallyEq
  -- · apply mem_nhdsWithin_of_mem_nhds
  --   filter_upwards [e.open_baseSet.mem_nhds hx] with x' hx'
  --   simp [hx']
  -- · simp [hx]
...
-- Note: In the next lemma, the assumption `∀ᶠ b in 𝓝 x, MDiffAt[s] n (T% σ) b` is almost
-- equivalent to `MDiffAt[s] n (T% σ) x` but not quite: it is stronger if `k = ∞`.
lemma MDifferentiableWithinAt.clm_bundle_of_apply
    [∀ x, IsTopologicalAddGroup (E₁ x)] [∀ x, ContinuousSMul 𝕜 (E₁ x)]
    [ContMDiffVectorBundle 1 F₁ E₁ IB] [ContMDiffVectorBundle 1 F₂ E₂ IB]
    (h : ∀ (σ : Π x : B, E₁ x),
      (∀ᶠ b in 𝓝 x, MDiffAt[s] (T% σ) b) → MDiffAt[s] (T% (fun x ↦ φ x (σ x))) x) :
    MDifferentiableWithinAt IB (IB.prod 𝓘(𝕜, F₁ →L[𝕜] F₂))
      (fun x ↦ TotalSpace.mk' (F₁ →L[𝕜] F₂) x (φ x)) s x := by
  refine (mdifferentiableWithinAt_hom_bundle fun x ↦ ⟨x, φ x⟩).mpr ⟨mdifferentiableWithinAt_id, ?_⟩
  -- TODO: missing API lemma mdifferentiableWithinAt_iff_source!
  rw [mdifferentiableWithinAt_iff_source, mdifferentiableWithinAt_iff_differentiableWithinAt]
  set t₁ := trivializationAt F₁ E₁ x
  set t₂ := trivializationAt F₂ E₂ x
  apply mdifferentiableWithinAt_clm_apply.mpr
  set ψ := extChartAt IB x
  intro u
  have C₀ : MDiffAt[s] (fun b ↦ t₂.continuousLinearMapAt 𝕜 b (φ b (t₁.symmL 𝕜 b u)))
      (ψ.symm (ψ x)) := by
    rw [extChartAt_to_inv x]
    apply t₂.mdifferentiableWithinAt_apply (FiberBundle.mem_baseSet_trivializationAt' x)
    apply h
    filter_upwards [t₁.open_baseSet.mem_nhds (FiberBundle.mem_baseSet_trivializationAt' x)] with
      x' hx'
    exact (t₁.mdifferentiableAt_symm_const hx' _).mdifferentiableWithinAt
  -- TODO missing API lemma mdifferentiableWithinAt_extChartAt_symm_range_self
  have := C₀.comp' (ψ x) (mdifferentiableWithinAt_extChartAt_symm_range_self x)
  simpa [inter_comm, t₁, t₂, mdifferentiableWithinAt_iff_differentiableWithinAt, inCoordinates]

-- Note: In the next lemma, the assumption `∀ᶠ b in 𝓝 x, CMDiffAt k (T% σ) b` is almost equivalent
-- to `CMDiffAt k (T% σ) x` but not quite: it is stronger if `k = ∞`.
lemma MDifferentiableAt.clm_bundle_of_apply
    [∀ x, IsTopologicalAddGroup (E₁ x)] [∀ x, ContinuousSMul 𝕜 (E₁ x)]
    [ContMDiffVectorBundle 1 F₁ E₁ IB] [ContMDiffVectorBundle 1 F₂ E₂ IB]
    (h : ∀ (σ : Π x : B, E₁ x),
      (∀ᶠ b in 𝓝 x, MDiffAt (T% σ) b) → MDiffAt (T% (fun x ↦ φ x (σ x))) x) :
    MDifferentiableAt IB (IB.prod 𝓘(𝕜, F₁ →L[𝕜] F₂)) (fun x ↦ TotalSpace.mk' (F₁ →L[𝕜] F₂) x (φ x))
    x := by
  simp_rw [← mdifferentiableWithinAt_univ] at h ⊢
  exact MDifferentiableWithinAt.clm_bundle_of_apply (fun σ hσ ↦ h σ hσ)

lemma MDifferentiableOn.clm_bundle_of_apply
    [∀ x, IsTopologicalAddGroup (E₁ x)] [∀ x, ContinuousSMul 𝕜 (E₁ x)]
    [ContMDiffVectorBundle 1 F₁ E₁ IB] [ContMDiffVectorBundle 1 F₂ E₂ IB]
    (h : ∀ (σ : Π x : B, E₁ x),
      (∀ x ∈ s, (∀ᶠ b in 𝓝 x, MDiffAt[s] (T% σ) b) → MDiffAt[s] (T% (fun x ↦ φ x (σ x))) x)) :
    MDifferentiableOn IB (IB.prod 𝓘(𝕜, F₁ →L[𝕜] F₂)) (fun x ↦ TotalSpace.mk' (F₁ →L[𝕜] F₂) x (φ x))
    s :=
  fun x hx ↦ MDifferentiableAt.clm_bundle_of_apply (fun σ hσ ↦ h σ x hx hσ)

lemma MDifferentiable.clm_bundle_of_apply
    [∀ x, IsTopologicalAddGroup (E₁ x)] [∀ x, ContinuousSMul 𝕜 (E₁ x)]
    [ContMDiffVectorBundle 1 F₁ E₁ IB] [ContMDiffVectorBundle 1 F₂ E₂ IB]
    (h : ∀ (σ : Π x : B, E₁ x),
      (∀ x, (∀ᶠ b in 𝓝 x, MDiffAt (T% σ) b) → MDiffAt (T% (fun x ↦ φ x (σ x))) x)) :
    MDifferentiable IB (IB.prod 𝓘(𝕜, F₁ →L[𝕜] F₂)) (fun x ↦ TotalSpace.mk' (F₁ →L[𝕜] F₂) x (φ x)) :=
  fun x ↦ MDifferentiableAt.clm_bundle_of_apply fun σ ↦ h σ x
