...
where
  ...
  localFrame_coeff_fin  {e : Trivialization F (TotalSpace.proj (F := F) (E := V))}
      ...
      (he : x ∈ e.baseSet)
      {n : ℕ} (b : Basis (Fin n) 𝕜 F) {σ : Π x, V x} (hσ : P' σ) (i : Fin n) :
      P ((LinearMap.piApply (e.localFrame_coeff I b i)) σ)
...
lemma localFrame [FiniteDimensional 𝕜 F] {ι : Type*} {e : Trivialization F TotalSpace.proj} [MemTrivializationAtlas e]
  (b : Basis ι 𝕜 F) (he : x ∈ e.baseSet) {σ : (x : M) → V x} (hσ : R.P' σ) (i : ι) :
      R.P' (e.localFrame b i) := by
  sorry

lemma localFrame_coeff [FiniteDimensional 𝕜 F] {ι : Type*} {e : Trivialization F TotalSpace.proj} [MemTrivializationAtlas e]
  (b : Basis ι 𝕜 F) (he : x ∈ e.baseSet) {σ : (x : M) → V x} (hσ : R.P' σ) (i : ι) :
      R.P ((LinearMap.piApply (e.localFrame_coeff I b i)) σ) := by
  sorry
