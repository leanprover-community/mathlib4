...
lemma StructureGroupoid.restr_mem_of_eqOn' {G : StructureGroupoid H} [ClosedUnderRestriction G]
    {e e' : OpenPartialHomeomorph H H} (he' : e' ∈ G) {s : Set H}
    (heq : (e.source ∩ interior s).EqOn e e') (hsub : e.source ∩ interior s ⊆ e'.source) :
    e.restr s ∈ G :=
  G.mem_of_eqOnSource (closedUnderRestriction' he' (e.open_source.inter isOpen_interior))
    (e.restr_eqOnSource_of_eqOn heq hsub)
