...
lemma isMIntegralCurveOn_comp_add {dt : ℝ} :
  ...
  have h2 := hγ.comp_add (-dt)
  rw [show {t : ℝ | t + -dt ∈ {t | t + dt ∈ s}} = s by ext t; simp,
    show (γ ∘ (· + dt)) ∘ (· + -dt) = γ by ext t; simp] at h2
  exact h2
...
lemma IsMIntegralCurveAt.comp_add (hγ : IsMIntegralCurveAt γ v t₀) (dt : ℝ) :
  ...
  obtain ⟨ε, hε, h⟩ := hγ
  refine ⟨ε, hε, ?_⟩
  have hs : {t | t + dt ∈ Metric.ball t₀ ε} = Metric.ball (t₀ - dt) ε := by
    ext t
    simp_rw [mem_ofPred_eq, Metric.mem_ball, Real.dist_eq, ← sub_add, add_sub_right_comm]
  exact hs ▸ h.comp_add dt
