...
variable
  ...
  {HB : Type uH} [TopologicalSpace HB] {IB : ModelWithCorners ℝ EB HB} {n : WithTop ℕ∞}
  {B : Type uB} [TopologicalSpace B] [ChartedSpace HB B]
...
#check TangentBundle IB B
#check TangentBundle IB
#check (TangentSpace IB : B → Type uE)

-- The tangent bundle as a fiber family: B → Type*
#check (TangentSpace IB : B → Type uE)

example : TopologicalSpace EB := by infer_instance

#check @instTopologicalSpaceTangentBundle ℝ _ _ _ _ _ _ IB B _ _ _
#check @TangentSpace.fiberBundle ℝ _ _ _ _ _ _ IB B _ _ _

#synth TopologicalSpace (TangentBundle IB B)

example : TotalSpace EB (@TangentSpace ℝ _ _ _ _ _ _ IB B _ _) = TangentBundle IB B := rfl

noncomputable
example : FiberBundle EB (@TangentSpace ℝ _ _ _ _ _ _ IB B _ _) :=
  @TangentSpace.fiberBundle ℝ _ _ _ _ _ _ IB B _ _ _
...
noncomputable
example : TopologicalSpace (TangentBundle IB B) := by infer_instance

example : VectorBundle ℝ EB (@TangentSpace ℝ _ _ _ _ _ _ IB B _ _) := by infer_instance

noncomputable
example : TopologicalSpace (TangentBundle IB B) := instTopologicalSpaceTangentBundle
...
noncomputable instance : (x : B) → TopologicalSpace (V (TangentSpace IB) x) := by
  unfold V
  infer_instance
...
noncomputable instance : (x : B) → AddCommGroup (V (TangentSpace IB) x) := by
  unfold V
  infer_instance
...
noncomputable instance (x : B) : Module ℝ (V (TangentSpace IB) x) := by
  unfold V
  infer_instance
...
TopologicalSpace (TotalSpace (EB →L[ℝ] ℝ) (V (@TangentSpace ℝ _ _ _ _ _ _ IB B _ _))) := by
  unfold V
  infer_instance
...
noncomputable instance : FiberBundle (EB →L[ℝ] ℝ) (V (@TangentSpace ℝ _ _ _ _ _ _ IB B _ _)) := by
  unfold V
  infer_instance
...
noncomputable instance :
ContMDiffVectorBundle n (EB →L[ℝ] ℝ) (V (@TangentSpace ℝ _ _ _ _ _ _ IB B _ _)) IB := by
  unfold V
  infer_instance
...
instance (x : B) : ContinuousAdd (V (@TangentSpace ℝ _ _ _ _ _ _ IB B _ _) x) := by
  unfold V
  infer_instance
...
instance (x : B) : ContinuousSMul ℝ (V (@TangentSpace ℝ _ _ _ _ _ _ IB B _ _) x) := by
  unfold V
  infer_instance
...
instance (x : B) : IsTopologicalAddGroup (V (@TangentSpace ℝ _ _ _ _ _ _ IB B _ _) x) := by
  unfold V
  infer_instance
...
lemma convex_condition2 (x : B) : Convex ℝ (condition2 E x) := by
  ...
  simpa using lt_of_le_of_lt' aux this
...
#check ((SmoothPartitionOfUnity.exists_isSubordinate_chartAt_source IB B) :
  ∃ (f : SmoothPartitionOfUnity B IB B), f.IsSubordinate fun x ↦ (chartAt HB x).source)

#check @SmoothPartitionOfUnity.IsSubordinate B
...
variable [FiniteDimensional ℝ EB] [IsManifold IB ∞ B] [T2Space B] [SigmaCompactSpace B]

variable [FiniteDimensional ℝ E] [IsManifold I ∞ M] [T2Space M] [SigmaCompactSpace M]
variable [NormedAddCommGroup (TangentBundle I M)] [NormedSpace ℝ (TangentBundle I M)]

noncomputable
def g (p : M) (v w : TangentSpace I p) : ℝ :=
  let ψ := extChartAt I p
  let dψ := mfderiv I (modelWithCornersSelf ℝ E) ψ p
  @inner ℝ E _ (dψ v) (dψ w)

example : true := by
  obtain ⟨f, hf⟩ := SmoothPartitionOfUnity.exists_isSubordinate_chartAt_source I M
  let g_global : ∀ (p : M), TangentSpace I p → TangentSpace I p → ℝ :=
    fun p v w ↦ ∑ᶠ i : M, (f i p) * g p v w
  exact sorry
