...
@[simps]
noncomputable def toPartitionOfUnity : PartitionOfUnity ι M s :=
...
noncomputable def finsupport : Finset ι := ρ.toPartitionOfUnity.finsupport x₀
...
noncomputable def fintsupport (x : M) : Finset ι :=
...
noncomputable def toSmoothPartitionOfUnity (f : BumpCovering ι M s) (hf : ∀ i, CMDiff ∞ (f i)) :
...
noncomputable def ind (x : M) (hx : x ∈ s) : ι :=
...
@[implicit_reducible]
protected noncomputable def fintype [CompactSpace M] : Fintype ι :=
...
noncomputable def toBumpCovering : BumpCovering ι M s where
...
noncomputable def toSmoothPartitionOfUnity : SmoothPartitionOfUnity ι I M s :=
...
noncomputable def single (i : ι) (s : Set M) : SmoothPartitionOfUnity ι I M s :=
...
noncomputable instance [Inhabited ι] (s : Set M) : Inhabited (SmoothPartitionOfUnity ι I M s) :=
  ⟨single I default s⟩
