...
lemma StructureGroupoid.trans (G : StructureGroupoid H) {e e' : LocalHomeomorph H H} (he : e ∈ G)
    (he' : e' ∈ G) : e ≫ₕ e' ∈ G :=
  G.trans' e e' he he'
#align structure_groupoid.trans StructureGroupoid.trans

lemma StructureGroupoid.symm (G : StructureGroupoid H) {e : LocalHomeomorph H H} (he : e ∈ G) :
    e.symm ∈ G :=
  G.symm' e he
#align structure_groupoid.symm StructureGroupoid.symm

lemma StructureGroupoid.id_mem (G : StructureGroupoid H) : LocalHomeomorph.refl H ∈ G :=
  G.id_mem'
#align structure_groupoid.id_mem StructureGroupoid.id_mem

lemma StructureGroupoid.locality (G : StructureGroupoid H) {e : LocalHomeomorph H H}
    (h : ∀ x ∈ e.source, ∃ s, IsOpen s ∧ x ∈ s ∧ e.restr s ∈ G) : e ∈ G :=
  G.locality' e h
#align structure_groupoid.locality StructureGroupoid.locality

lemma StructureGroupoid.eq_on_source (G : StructureGroupoid H) {e e' : LocalHomeomorph H H}
    (he : e ∈ G) (h : e' ≈ e) : e' ∈ G :=
...
lemma StructureGroupoid.le_iff {G₁ G₂ : StructureGroupoid H} : G₁ ≤ G₂ ↔ ∀ e, e ∈ G₁ → e ∈ G₂ :=
  Iff.rfl
...
lemma mem_groupoid_of_pregroupoid {PG : Pregroupoid H} {e : LocalHomeomorph H H} :
...
lemma mem_pregroupoid_of_eq_on_source (PG : Pregroupoid H) {e e' : LocalHomeomorph H H}
...
lemma closedUnderRestriction' {G : StructureGroupoid H} [ClosedUnderRestriction G]
...
lemma idRestrGroupoid_mem {s : Set H} (hs : IsOpen s) : ofSet s hs ∈ @idRestrGroupoid H _ :=
  ⟨s, hs, refl _⟩
...
groupoid. -/
lemma closedUnderRestriction_iff_id_le (G : StructureGroupoid H) :
...
@[simp, mfld_simps]
lemma chartedSpaceSelf_atlas {H : Type*} [TopologicalSpace H] {e : LocalHomeomorph H H} :
...
lemma chartAt_self_eq {H : Type*} [TopologicalSpace H] {x : H} :
...
lemma mem_chart_target (x : M) : chartAt H x x ∈ (chartAt H x).target :=
  (chartAt H x).map_source (mem_chart_source _ _)
#align mem_chart_target mem_chart_target

lemma chart_source_mem_nhds (x : M) : (chartAt H x).source ∈ 𝓝 x :=
  (chartAt H x).open_source.mem_nhds <| mem_chart_source H x
#align chart_source_mem_nhds chart_source_mem_nhds

lemma chart_target_mem_nhds (x : M) : (chartAt H x).target ∈ 𝓝 (chartAt H x x) :=
...
lemma achart_def (x : M) : achart H x = ⟨chartAt H x, chart_mem_atlas H x⟩ :=
  rfl
#align achart_def achart_def

@[simp, mfld_simps]
lemma coe_achart (x : M) : (achart H x : LocalHomeomorph M H) = chartAt H x :=
  rfl
#align coe_achart coe_achart

@[simp, mfld_simps]
lemma achart_val (x : M) : (achart H x).1 = chartAt H x :=
  rfl
...
lemma ChartedSpace.secondCountable_of_countable_cover [SecondCountableTopology H] {s : Set M}
...
lemma ChartedSpace.secondCountable_of_sigma_compact [SecondCountableTopology H]
...
is locally compact. -/
lemma ChartedSpace.locallyCompactSpace [LocallyCompactSpace H] : LocallyCompactSpace M := by
...
locally connected. -/
lemma ChartedSpace.locallyConnectedSpace [LocallyConnectedSpace H] : LocallyConnectedSpace M := by
...
lemma chartAt_comp (H : Type*) [TopologicalSpace H] (H' : Type*) [TopologicalSpace H']
...
lemma modelProd_range_prod_id {H : Type*} {H' : Type*} {α : Type*} (f : H → α) :
...
@[ext]
lemma ModelProd.ext {x y : ModelProd α β} (h₁ : x.1 = y.1) (h₂ : x.2 = y.2) : x = y :=
  Prod.ext h₁ h₂
...
@[simp, mfld_simps]
lemma piChartedSpace_chartAt {ι : Type*} [Fintype ι] (H : ι → Type*)
...
lemma open_source' (he : e ∈ c.atlas) : IsOpen[c.toTopologicalSpace] e.source := by
...
lemma open_target (he : e ∈ c.atlas) : IsOpen e.target := by
...
notation. -/
lemma StructureGroupoid.compatible {H : Type*} [TopologicalSpace H] (G : StructureGroupoid H)
...
lemma hasGroupoid_of_le {G₁ G₂ : StructureGroupoid H} (h : HasGroupoid M G₁) (hle : G₁ ≤ G₂) :
    HasGroupoid M G₂ :=
  ⟨fun he he' ↦ hle (h.compatible he he')⟩
#align has_groupoid_of_le hasGroupoid_of_le

lemma hasGroupoid_of_pregroupoid (PG : Pregroupoid H) (h : ∀ {e e' : LocalHomeomorph M H},
...
lemma StructureGroupoid.subset_maximalAtlas [HasGroupoid M G] : atlas H M ⊆ G.maximalAtlas M :=
...
lemma StructureGroupoid.chart_mem_maximalAtlas [HasGroupoid M G] (x : M) :
...
lemma mem_maximalAtlas_iff {e : LocalHomeomorph M H} :
...
lemma StructureGroupoid.compatible_of_mem_maximalAtlas {e e' : LocalHomeomorph M H}
...
lemma StructureGroupoid.id_mem_maximalAtlas : LocalHomeomorph.refl H ∈ G.maximalAtlas H :=
...
lemma StructureGroupoid.mem_maximalAtlas_of_mem_groupoid {f : LocalHomeomorph H H} (hf : f ∈ G) :
...
@[simp, mfld_simps]
lemma singletonChartedSpace_chartAt_eq (h : e.source = Set.univ) {x : α} :
...
lemma singletonChartedSpace_chartAt_source (h : e.source = Set.univ) {x : α} :
...
lemma singletonChartedSpace_mem_atlas_eq (h : e.source = Set.univ) (e' : LocalHomeomorph α H)
...
lemma singleton_hasGroupoid (h : e.source = Set.univ) (G : StructureGroupoid H)
...
lemma singletonChartedSpace_chartAt_eq {f : α → H} (h : OpenEmbedding f) {x : α} :
...
lemma singleton_hasGroupoid {f : α → H} (h : OpenEmbedding f) (G : StructureGroupoid H)
...
lemma chartAt_inclusion_symm_eventuallyEq {U V : Opens M} (hUV : U ≤ V) {x : U} :
