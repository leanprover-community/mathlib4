...
@[to_additive]
lemma smooth_inv : Smooth I I fun x : G => x⁻¹ :=
  LieGroup.smooth_inv
...
lemma topologicalGroup_of_lieGroup : TopologicalGroup G :=
...
@[to_additive]
lemma ContMDiffWithinAt.inv {f : M → G} {s : Set M} {x₀ : M}
...
@[to_additive]
lemma ContMDiffAt.inv {f : M → G} {x₀ : M} (hf : ContMDiffAt I' I n f x₀) :
...
@[to_additive]
lemma ContMDiffOn.inv {f : M → G} {s : Set M} (hf : ContMDiffOn I' I n f s) :
...
@[to_additive]
lemma ContMDiff.inv {f : M → G} (hf : ContMDiff I' I n f) : ContMDiff I' I n fun x => (f x)⁻¹ :=
  fun x => (hf x).inv
#align cont_mdiff.inv ContMDiff.inv
#align cont_mdiff.neg ContMDiff.neg

@[to_additive]
nonrec lemma SmoothWithinAt.inv {f : M → G} {s : Set M} {x₀ : M}
...
@[to_additive]
nonrec lemma SmoothAt.inv {f : M → G} {x₀ : M} (hf : SmoothAt I' I f x₀) :
...
@[to_additive]
nonrec lemma SmoothOn.inv {f : M → G} {s : Set M} (hf : SmoothOn I' I f s) :
...
@[to_additive]
nonrec lemma Smooth.inv {f : M → G} (hf : Smooth I' I f) : Smooth I' I fun x => (f x)⁻¹ :=
  hf.inv
#align smooth.inv Smooth.inv
#align smooth.neg Smooth.neg

@[to_additive]
lemma ContMDiffWithinAt.div {f g : M → G} {s : Set M} {x₀ : M}
...
@[to_additive]
lemma ContMDiffAt.div {f g : M → G} {x₀ : M} (hf : ContMDiffAt I' I n f x₀)
...
@[to_additive]
lemma ContMDiffOn.div {f g : M → G} {s : Set M} (hf : ContMDiffOn I' I n f s)
...
@[to_additive]
lemma ContMDiff.div {f g : M → G} (hf : ContMDiff I' I n f) (hg : ContMDiff I' I n g) :
...
@[to_additive]
nonrec lemma SmoothWithinAt.div {f g : M → G} {s : Set M} {x₀ : M}
...
@[to_additive]
nonrec lemma SmoothAt.div {f g : M → G} {x₀ : M} (hf : SmoothAt I' I f x₀)
...
@[to_additive]
nonrec lemma SmoothOn.div {f g : M → G} {s : Set M} (hf : SmoothOn I' I f s)
...
@[to_additive]
nonrec lemma Smooth.div {f g : M → G} (hf : Smooth I' I f) (hg : Smooth I' I g) :
    Smooth I' I (f / g) :=
  hf.div hg
...
lemma smoothAt_inv₀ {x : G} (hx : x ≠ 0) : SmoothAt I I (fun y ↦ y⁻¹) x :=
...
lemma hasContinuousInv₀_of_hasSmoothInv₀ : HasContinuousInv₀ G :=
  { continuousAt_inv₀ := fun _ hx ↦ (smoothAt_inv₀ I hx).continuousAt }

lemma SmoothOn_inv₀ : SmoothOn I I (Inv.inv : G → G) {0}ᶜ := fun _x hx =>
