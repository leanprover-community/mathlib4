...
2. Because of `ModelProd` we cannot assume, e.g., that a `LieGroup` is modelled on `𝓘(𝕜, E)`. So,
...
`continuousMul_of_contMDiffMul` cannot be instances because otherwise Lean would have to search for
