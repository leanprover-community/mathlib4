...
theorem ContMDiffWithinAt.comp {t : Set M'} {g : M' → M''} (x : M)
  ...
  have A : ∀ᶠ y in 𝓝[e.symm ⁻¹' s ∩ range I] e x, f (e.symm y) ∈ t ∧ f (e.symm y) ∈ e'.source := by
    ...
    simp +zetaDelta only [e.map_source hx', true_and, e.left_inv hx', st hx's, *]
  ...
  · filter_upwards [A]
    rintro x' ⟨ht, hfx'⟩
    simp +zetaDelta only [*, mem_preimage, writtenInExtChartAt, (· ∘ ·), mem_inter_iff, e'.left_inv,
      true_and]
    exact mem_range_self _
  · filter_upwards [A]
    rintro x' ⟨-, hfx'⟩
    simp +zetaDelta only [*, (· ∘ ·), writtenInExtChartAt, e'.left_inv]
  · simp +zetaDelta only [e, e', writtenInExtChartAt, (· ∘ ·), mem_extChartAt_source,
