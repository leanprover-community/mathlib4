...
line_unique_of_pts : ∀ {a b L M}, a ≠ b → OnLine a L → OnLine b L → OnLine a M → OnLine b M → L = M
...
B_of_three_online_ne : ∀ {a b c L}, a ≠ b → a ≠ c → b ≠ c → OnLine a L → OnLine b L → OnLine c L →
...
one of the points-/
sameside_or_of_diffside : ∀ {a b c L}, ¬OnLine a L → ¬OnLine b L → ¬OnLine c L → ¬SameSide a b L →
...
not_sameside_of_circle_inter : ∀ {a b c d L α β},  c ≠ d → α ≠ β →  OnLine a L → OnLine b L →
  OnCircle c α → OnCircle c β → OnCircle d α → OnCircle d β → CenterCircle a α → CenterCircle b β →
  CirclesInter α β → ¬SameSide c d L
...
area_zero_iff_online : ∀ {a b c L}, a ≠ b → OnLine a L → OnLine b L → (area a b c = 0 ↔ OnLine c L)
...
def out_circle a α := ¬OnCircle a α ∧ ¬InCircle a α
...
def eq_tri a b c := triangle a b c ∧ length a b = length a c ∧ length b a = length b c
  ∧ length c a = length c b
/--Definition of an isosoles triangle-/
def iso_tri a b c := triangle a b c ∧ length a b = length a c
