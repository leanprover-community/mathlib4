...
step : ∀ (i : Set.Iio length),
  r (toFun ⟨i, show i ≤ length from le_of_lt i.2⟩)
    (toFun ⟨i.1 + 1, show i.1 + 1 ≤ length from  (Order.succ_le_iff_of_not_isMax $ not_isMax i.1).mpr i.2⟩)
...
step : ∀ (i : Set.Iio length),
  r (toFun ⟨i, show i ≤ length from le_of_lt i.2⟩)
    (toFun ⟨Order.succ i.1, show Order.succ i.1 ≤ length from (Order.succ_le_iff_of_not_isMax $ not_isMax i.1).mpr i.2⟩)
