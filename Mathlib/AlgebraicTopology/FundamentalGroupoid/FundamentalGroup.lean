...
instance (X : Type u) [TopologicalSpace X] (x : X) : Group (FundamentalGroup X x) := fast_instance% by
  dsimp only [FundamentalGroup]
  infer_instance

instance (X : Type u) [TopologicalSpace X] (x : X) : Inhabited (FundamentalGroup X x) := fast_instance% by
  dsimp only [FundamentalGroup]
  infer_instance
