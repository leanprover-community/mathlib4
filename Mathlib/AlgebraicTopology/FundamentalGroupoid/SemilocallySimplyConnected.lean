...
theorem Path.subpathOn_trans {X : Type*} [TopologicalSpace X] {x y : X} (γ : Path x y)
  ...
  We construct a homotopy H(s,t) that continuously interpolates between:
    - H(0,t): the concatenated path (γ.subpathOn a b) · (γ.subpathOn b c)
    - H(1,t): the direct path γ.subpathOn a c

  The key insight: both paths trace γ from a to c, but with different "speeds".

  ## Case 1: Trivial case (a = c)
  If a = c, then by a ≤ b ≤ c, we have a = b = c.
  All three subpaths are constant paths at γ(a), so they're trivially homotopic.

  ## Case 2: Main case (a < c)

  Define H(s,t) = γ(φ(s,t)) where φ: I × I → [a,c] is a reparametrization function.

  ### The Moving Breakpoint p(s):
  At s=0 (concatenated path), we switch from first to second segment at t=1/2.
  At s=1 (direct path), we "pass through b" at time t_b = (b-a)/(c-a).

  Define: p(s) = (1-s)/2 + s·(b-a)/(c-a)

  This linearly interpolates the breakpoint from 1/2 to (b-a)/(c-a).

  ### The Piecewise Reparametrization φ(s,t):

  φ(s,t) = { a + (t/p(s))·(b-a)              if t ≤ p(s)  [Region 1: a→b]
           { b + ((t-p(s))/(1-p(s)))·(c-b)   if t ≥ p(s)  [Region 2: b→c]

  ### Verification Requirements:
  1. Continuity: The two branches agree at t = p(s):
     - Region 1 at t=p(s): a + (p(s)/p(s))·(b-a) = a + (b-a) = b ✓
     - Region 2 at t=p(s): b + ((p(s)-p(s))/(1-p(s)))·(c-b) = b ✓

  2. H(0,t) = concatenated path:
     - p(0) = 1/2
     - t ≤ 1/2: φ(0,t) = a + 2t·(b-a) = subpathOn a b reparametrized to [0,1/2]
     - t ≥ 1/2: φ(0,t) = b + (2t-1)·(c-b) = subpathOn b c reparametrized to [1/2,1]

  3. H(1,t) = direct path:
     - p(1) = (b-a)/(c-a)
     - For all t: φ(1,t) simplifies to a + t·(c-a) = direct subpathOn a c

  4. Fixed endpoints:
     - H(s,0) = γ(φ(s,0)) = γ(a) for all s
     - H(s,1) = γ(φ(s,1)) = γ(c) for all s
  -/

  -- Case split: trivial case vs main case
  by_cases h_eq : a.val = c.val
  case pos =>
    -- Trivial case: a = c, so a = b = c
    -- From a ≤ b ≤ c and a.val = c.val, derive a = b = c
    have ha_eq_c : a = c := Subtype.ext h_eq
    have ha_eq_b : a = b := by
      have hb_le_a : b ≤ a := by
        rw [← ha_eq_c] at hbc
        exact hbc
      exact le_antisymm hab hb_le_a
    -- Now a = b = c, so all subpaths are constant paths
    subst ha_eq_b ha_eq_c
    -- Both sides are now subpathOn a a, which are constant paths
    convert Path.Homotopic.refl _
    ext t
    simp [Path.trans, Path.subpathOn, Path.extend]
    split_ifs <;> simp [Set.IccExtend]
  case neg =>
    -- Main case: a.val ≠ c.val, which with a ≤ c gives a < c
    have ha_lt_c : a.val < c.val := by
      have hac : a ≤ c := hab.trans hbc
      -- a ≤ c means a.val ≤ c.val for unitInterval
      have hac_val : a.val ≤ c.val := hac
      exact lt_of_le_of_ne hac_val h_eq

    -- Define the moving breakpoint: p(s) = (1-s)/2 + s*(b-a)/(c-a)
    -- At s=0: p=1/2 (breakpoint for concatenated path)
    -- At s=1: p=(b-a)/(c-a) (where direct path passes through b)
    let p : ℝ → ℝ := fun s => (1 - s) / 2 + s * ((b.val - a.val) / (c.val - a.val))

    -- Define the piecewise reparametrization φ(s,t)
    let φ : ℝ → ℝ → ℝ := fun s t =>
      if t ≤ p s then
        -- Region 1: t ≤ p(s), maps [0, p(s)] to [a, b]
        a.val + (t / p s) * (b.val - a.val)
      else
        -- Region 2: t > p(s), maps [p(s), 1] to [b, c]
        b.val + ((t - p s) / (1 - p s)) * (c.val - b.val)

    -- Prove that φ maps [0,1]² into [a,c] ⊆ [0,1]
    have hφ_mem : ∀ s t : ℝ, s ∈ unitInterval → t ∈ unitInterval → φ s t ∈ unitInterval := by
      intro s t hs ht
      simp only [φ]
      split_ifs with h
      case pos =>
        -- t ≤ p(s), so φ(s,t) = a + (t/p(s))*(b-a) ∈ [a, b]
        -- Need: 0 ≤ a + (t/p(s))*(b-a) ≤ 1
        constructor
        · -- Lower bound: 0 ≤ a + (t/p(s))*(b-a)
          have ha_nonneg : 0 ≤ a.val := a.property.1
          ...
          · apply mul_nonneg
            ...
            · -- 0 ≤ b - a
              linarith [hab_val]
        · -- Upper bound: a + (t/p(s))*(b-a) ≤ 1
          have hb_le_one : b.val ≤ 1 := b.property.2
          have hab_val : a.val ≤ b.val := hab
          by_cases hp_zero : p s = 0
          · -- p(s) = 0, then b = a (from p(s) ≥ 0 structure)
            -- p(s) = 0 means (1-s)/2 + s*(b-a)/(c-a) = 0
            -- Since both terms ≥ 0, both must be 0
            have hs_eq_one : s = 1 := by
              by_contra h_ne
              have hs_lt_one : s < 1 := lt_of_le_of_ne hs.2 h_ne
              simp only [p] at hp_zero
              have h1_pos : 0 < (1 - s) / 2 := by
                apply div_pos
                · linarith
                · norm_num
              have h2_nonneg : 0 ≤ s * ((b.val - a.val) / (c.val - a.val)) := by
                apply mul_nonneg
                · exact hs.1
                · apply div_nonneg
                  · linarith [hab_val]
                  · linarith [ha_lt_c]
              have : 0 < (1 - s) / 2 + s * ((b.val - a.val) / (c.val - a.val)) := by
                linarith
              rw [hp_zero] at this
              linarith
            have hab_eq : a = b := by
              ...
              have hab_ne : a.val ≠ b.val := by
                intro heq
                exact h_ne (Subtype.ext heq)
              have hab_lt : a.val < b.val := lt_of_le_of_ne hab_val hab_ne
              simp only [p] at hp_zero
              rw [hs_eq_one] at hp_zero
              simp at hp_zero
              rcases hp_zero with hba_zero | hca_zero
              · -- b - a = 0, so b = a
                have : b.val = a.val := by linarith
                exact hab_ne this.symm
              · -- c - a = 0, so c = a, contradicts ha_lt_c
                have : c.val = a.val := by linarith
                linarith [ha_lt_c]
            -- Now a = b, so φ(s,t) = a + (t/0) * 0
            subst hab_eq
            simp
            exact a.property.2
          · -- p(s) > 0
            have hp_nonneg : 0 ≤ p s := by
              simp only [p]
              apply add_nonneg
              · apply div_nonneg
                · linarith [hs.2]
              ...
              · apply mul_nonneg
                ...
                · apply div_nonneg
                  · linarith [hab_val]
                  · linarith [ha_lt_c]
            have hp_pos : 0 < p s := lt_of_le_of_ne hp_nonneg (Ne.symm hp_zero)
            have ht_div_le : t / p s ≤ 1 := by
              rw [div_le_one hp_pos]
              exact h
            calc a.val + t / p s * (b.val - a.val)
                ≤ a.val + 1 * (b.val - a.val) := by
                  apply add_le_add_left
                  apply mul_le_mul_of_nonneg_right
                  · exact ht_div_le
                  · linarith [hab_val]
              _ = a.val + (b.val - a.val) := by ring
              _ = b.val := by ring
              _ ≤ 1 := hb_le_one
      case neg =>
        -- t > p(s), so φ(s,t) = b + ((t-p(s))/(1-p(s)))*(c-b) ∈ [b, c]
        constructor
        · -- Lower bound: 0 ≤ b + ((t-p(s))/(1-p(s)))*(c-b)
          have hb_nonneg : 0 ≤ b.val := b.property.1
          have hbc_val : b.val ≤ c.val := hbc
          apply add_nonneg
          · exact hb_nonneg
          · apply mul_nonneg
            · apply div_nonneg
              · -- 0 ≤ t - p(s)
                have h_gt : p s < t := not_le.mp h
                linarith
              · -- 0 ≤ 1 - p(s), i.e., p(s) ≤ 1
                have hp_le_one : p s ≤ 1 := by
                  simp only [p]
                  calc (1 - s) / 2 + s * ((b.val - a.val) / (c.val - a.val))
                      ≤ (1 - s) / 2 + s * 1 := by
                        apply add_le_add_left
                        apply mul_le_mul_of_nonneg_left
                        · have hba_div_le : (b.val - a.val) / (c.val - a.val) ≤ 1 := by
                            rw [div_le_one]
                            · linarith [hbc_val]
                            · linarith [ha_lt_c]
                          exact hba_div_le
                        · exact hs.1
                    _ = (1 - s) / 2 + s := by ring
                    _ ≤ 1 := by linarith [hs.1, hs.2]
                linarith [hp_le_one]
            · linarith [hbc_val]
        · -- Upper bound: b + ((t-p(s))/(1-p(s)))*(c-b) ≤ 1
          have hc_le_one : c.val ≤ 1 := c.property.2
          have hbc_val : b.val ≤ c.val := hbc
          have hp_le_one : p s ≤ 1 := by
            simp only [p]
            calc (1 - s) / 2 + s * ((b.val - a.val) / (c.val - a.val))
          ...
          have hp_nonneg : 0 ≤ p s := by
            ...
            · apply mul_nonneg
              ...
              · have hab_val : a.val ≤ b.val := hab
                ...
                · linarith [hab_val]
                · linarith [ha_lt_c]
          have h1_minus_p_pos : 0 < 1 - p s := by
            have : p s < 1 := by
              by_contra h_ge
              push_neg at h_ge
              have hp_eq_one : p s = 1 := le_antisymm hp_le_one h_ge
              have h_gt : p s < t := not_le.mp h
              linarith [ht.2, h_gt, hp_eq_one]
            linarith [this]
          have ht_div_le : (t - p s) / (1 - p s) ≤ 1 := by
            rw [div_le_one h1_minus_p_pos]
            linarith [ht.2]
          calc b.val + (t - p s) / (1 - p s) * (c.val - b.val)
              ≤ b.val + 1 * (c.val - b.val) := by
                apply add_le_add_left
                apply mul_le_mul_of_nonneg_right
                · exact ht_div_le
                · linarith [hbc_val]
            _ = b.val + (c.val - b.val) := by ring
            _ = c.val := by ring
            _ ≤ 1 := hc_le_one

    -- Construct the homotopy H(s,t) = γ(φ(s,t))
    refine ⟨{
      toFun := fun st : unitInterval × unitInterval =>
        γ ⟨φ st.1.val st.2.val, hφ_mem st.1.val st.2.val st.1.property st.2.property⟩,
    ...
    case hzero =>
      -- Prove H(0,t) = concatenated path
      -- At s=0: p(0) = 1/2, so φ(0,t) gives the piecewise parametrization
      intro t
      simp only [ContinuousMap.coe_mk, Path.trans, Path.subpathOn]
      -- At s=0: p(0) = 1/2
      have hp_half : p 0 = 1 / 2 := by simp [p]
      simp [hp_half, φ]
      split_ifs with h1
      · -- t ≤ 1/2: φ(0,t) = a + 2t(b-a), trans gives same via extend
        simp [Path.extend, Set.IccExtend]
        congr 1
        apply Subtype.ext
        simp [Set.projIcc]
        left
        have h_lb : 0 ≤ 2 * t.val := by linarith [t.property.1]
        have h_ub : 2 * t.val ≤ 1 := by linarith [h1]
        calc t.val * 2
            = 2 * t.val := by ring
          _ = min 1 (2 * t.val) := by rw [min_eq_right h_ub]
          _ = max 0 (min 1 (2 * t.val)) := by rw [max_eq_right (le_min (by norm_num) h_lb)]
      · -- t > 1/2: φ(0,t) = b + 2(t-1/2)(c-b), trans gives same via extend
        simp [Path.extend, Set.IccExtend]
        congr 1
        apply Subtype.ext
        simp [Set.projIcc]
        left
        have h_lb : 0 ≤ 2 * t.val - 1 := by linarith [h1]
        have h_ub : 2 * t.val - 1 ≤ 1 := by linarith [t.property.2]
        calc (t.val - 2⁻¹) / (1 - 2⁻¹)
            = (t.val - 1/2) / (1/2) := by norm_num
          _ = 2 * (t.val - 1/2) := by field_simp
          _ = 2 * t.val - 1 := by ring
          _ = min 1 (2 * t.val - 1) := by rw [min_eq_right h_ub]
          _ = max 0 (min 1 (2 * t.val - 1)) := by rw [max_eq_right (le_min (by norm_num) h_lb)]

    case hone =>
      -- Prove H(1,t) = direct path
      -- At s=1: p(1) = (1-1)/2 + 1*(b-a)/(c-a) = (b-a)/(c-a)
      intro t
      simp only [ContinuousMap.coe_mk, Path.subpathOn, Path.extend, Set.IccExtend]
      have hp_val : p 1 = (b.val - a.val) / (c.val - a.val) := by simp [p]
      congr 1
      apply Subtype.ext
      simp [Set.projIcc, φ, hp_val]
      split_ifs with h1
      · -- t ≤ (b-a)/(c-a): φ(1,t) = a + t/p(1)*(b-a) = a + t*(c-a)
        sorry
      · -- t > (b-a)/(c-a): φ(1,t) = b + (t-p(1))/(1-p(1))*(c-b) = a + t*(c-a)
        sorry

    case hprop =>
      -- Prove endpoints are fixed: H(s,0) = γ(a) and H(s,1) = γ(c) for all s
      intro s x hx
      simp only [Set.mem_insert_iff, Set.mem_singleton_iff] at hx
      rcases hx with rfl | rfl
      · -- x = 0: prove γ(φ(s,0)) = γ(a)
        simp [ContinuousMap.coe_mk]
        -- φ(s,0) = a since t=0 gives a + 0*(b-a) = a
        congr 1
        simp [φ]
        -- The condition 0 ≤ p(s) is always true
        split_ifs with hp_nonneg
        · simp
        · -- Contradicts 0 ≤ p(s), which must hold
          exfalso
          apply hp_nonneg
          -- p(s) = (1-s)/2 + s*(b-a)/(c-a) ≥ 0 since all terms are ≥ 0
          simp only [p]
          apply add_nonneg
          · -- (1-s)/2 ≥ 0
            apply div_nonneg
          ...
          · -- s*(b-a)/(c-a) ≥ 0
            ...
            · apply div_nonneg
              · have : a.val ≤ b.val := hab
                linarith
              · linarith [ha_lt_c]
      · -- x = 1: prove γ(φ(s,1)) = γ(c)
        simp [ContinuousMap.coe_mk]
        -- φ(s,1) = c regardless of which branch
        congr 1
        simp [φ]
        split_ifs with h
        · -- 1 ≤ p(s) case
          -- p(s) = (1-s)/2 + s*(b-a)/(c-a) ≤ max(1/2, (b-a)/(c-a)) ≤ 1 typically
          -- For p(s) ≥ 1, we need (b-a)/(c-a) ≥ 1, so b ≥ c, but b ≤ c, so b = c
          have hb_eq_c : b = c := by
            by_contra hne
            have hb_lt_c : b.val < c.val := lt_of_le_of_ne hbc (fun eq => hne (Subtype.ext eq))
            -- If b < c, then (b-a)/(c-a) < 1, and p(s) is a convex combination
            -- of 1/2 and (b-a)/(c-a), so p(s) < 1
            simp only [p] at h
            have ratio_lt_one : (b.val - a.val) / (c.val - a.val) < 1 := by
              rw [div_lt_one]
              · linarith [hb_lt_c]
              · linarith [ha_lt_c]
            -- p(s) is a convex combination: p(s) = (1-s)*1/2 + s*(b-a)/(c-a)
            -- Both 1/2 and (b-a)/(c-a) are < 1, so p(s) < 1
            have hp_lt_one : (1 - s.val) / 2 + s.val * ((b.val - a.val) / (c.val - a.val)) < 1 := by
              -- Case split on whether s = 1
              by_cases hs : s.val = 1
              · -- If s = 1, then p(1) = (b-a)/(c-a) < 1
                simp [hs]
                exact ratio_lt_one
              · -- If s < 1, then we can show p(s) < 1 more carefully
                have hs_lt : s.val < 1 := lt_of_le_of_ne s.property.2 hs
                calc (1 - s.val) / 2 + s.val * ((b.val - a.val) / (c.val - a.val))
                    < (1 - s.val) / 2 + s.val * 1 := by
                      apply add_lt_add_left
                      by_cases hs_pos : s.val = 0
                      · -- If s = 0, then h says 1 ≤ 1/2, contradiction
                        exfalso
                        simp [hs_pos] at h
                        linarith [h]
                      · have : 0 < s.val := lt_of_le_of_ne s.property.1 (Ne.symm hs_pos)
                        exact mul_lt_mul_of_pos_left ratio_lt_one this
                    _ = (1 - s.val) / 2 + s.val := by ring
                    _ < 1 := by linarith [hs_lt]
            linarith [hp_lt_one, h]
          -- If b = c, then φ(s,1) = a + (1/p(s))*(b-a)
          -- We want this to equal c = b
          -- So we need (1/p(s))*(b-a) = b-a, i.e., 1/p(s) = 1, i.e., p(s) = 1
          subst hb_eq_c
          ext
          simp
          -- Goal: a - a/p(s) + b/p(s) = b
          -- Equivalently: a + (b-a)/p(s) = b
          -- Need: (b-a)/p(s) = b-a, so p(s) = 1
          have hp_eq_one : p s.val = 1 := by
            apply le_antisymm
            -- p(s) ≤ 1 follows from the formula
            · simp only [p]
              -- p(s) = (1-s)/2 + s*(b-a)/(c-a) = (1-s)/2 + s when c=b
              -- Then p(s) ≤ (1-s)/2 + s = (1+s)/2 ≤ 1
              have hba_ne : b.val - a.val ≠ 0 := by linarith [ha_lt_c]
              rw [div_self hba_ne]
              calc (1 - s.val) / 2 + s.val * 1
                  = (1 - s.val) / 2 + s.val := by ring
                  _ = (1 - s.val + 2 * s.val) / 2 := by ring
                  _ = (1 + s.val) / 2 := by ring
                  _ ≤ (1 + 1) / 2 := by
          ...
          rw [hp_eq_one]
          ring
        · -- 1 > p(s), the typical case: b + (1-p(s))/(1-p(s))*(c-b) = c
          -- This simplifies to b + (c-b) = c
          have hp_ne_one : 1 - p s.val ≠ 0 := by
            have hp_lt_one : p s.val < 1 := not_le.mp h
            linarith
          field_simp [hp_ne_one]
          ext
          simp
