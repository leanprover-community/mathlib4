...
theorem Path.subpathOn_trans {X : Type*} [TopologicalSpace X] {x y : X} (γ : Path x y)
  ...
  -- The difficulty: the concatenated path is piecewise-defined (split at t=1/2)
  -- while the direct path is uniform. Constructing a continuous interpolation
  -- between these requires careful handling of the breakpoint.
  sorry
