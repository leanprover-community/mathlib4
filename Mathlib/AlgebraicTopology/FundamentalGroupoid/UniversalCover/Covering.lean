...
projection `UniversalCover.proj` is a covering map when `X` is locally path-connected and
semilocally simply connected, and that the universal cover is simply connected. Its projection
has image the path component of `x₀`; fibers outside this component are empty.
...
/-- The projection `UniversalCover x₀ → X` is a covering map when `X` is locally path-connected
and semilocally simply connected. -/
public theorem isCoveringMap [LocallyPathConnectedSpace X]
  ...
  | inl h =>
    ...
    simpa using preimage_subset_iUnion_sheet (x₀ := x₀) hU_pc hxU
  | inr h =>
    have : Nonempty (X → UniversalCover x₀) :=
      ⟨fun _ ↦ ofBasedPath x₀ (BasedPath.refl x₀)⟩
    have h_open_iff : ∀ q : Path.Homotopic.Quotient x₀ x, ∀ {W : Set X}, W ⊆ U →
      ...
      rwa [Set.image_preimage_inter,
        Set.inter_eq_left.mpr (hWU.trans (surjOn_proj_sheet hU_pc hxU q))] at h
    refine ((IsEvenlyCovered.of_trivialization (t :=
      IsOpen.trivializationDiscrete (f := proj (x₀ := x₀)) (sheet U hxU) U hU_open h_open_iff
        (injOn_proj_sheet hU_triv hxU) (surjOn_proj_sheet hU_pc hxU)
        (pairwise_disjoint_sheet hU_triv hxU)
        (preimage_subset_iUnion_sheet hU_pc hxU)) ?_).to_isEvenlyCovered_preimage)
    rw [IsOpen.trivializationDiscrete_baseSet]
    exact hxU

public instance (priority := 100) discreteTopology_fiber [LocallyPathConnectedSpace X]
...
theorem liftPath_apply_one_eq_ofBasedPath_append [LocallyPathConnectedSpace X]
  ...
  have hΓ_zero : Γ 0 = ofBasedPath x₀ α := by
    have h0 : ((α.toPath.trans (γ.initialSegmentFamily 0)).cast rfl (by simp)).Homotopic
        α.toPath := by
      simpa only [Path.initialSegmentFamily_zero, Path.cast_trans _ _ rfl rfl,
        Path.cast_cast, Path.cast_rfl_rfl] using Path.Homotopic.trans_refl α.toPath
...
public instance instSimplyConnectedSpace [LocallyPathConnectedSpace X]
    [SemilocallySimplyConnectedSpace X] (x₀ : X) :
  ...
  let γ : Path (BasedPath.endpoint α) (BasedPath.endpoint α) :=
    p.map (continuous_proj x₀)
  have hγ0 : γ 0 = proj (ofBasedPath x₀ α) := γ.source
  have hp_eq_lift :
  ...
  have hγ_null :
    ...
    have h := congrArg ((Path.Homotopic.Quotient.mk α.toPath).symm.trans ·) h_append_eq
    simpa only [Path.Homotopic.Quotient.mk_trans, ← Path.Homotopic.Quotient.trans_assoc,
  ...
  change Path.Homotopic.Quotient.mk (p.map (continuous_proj x₀)) =
    Path.Homotopic.Quotient.mk ((Path.refl _).map (continuous_proj x₀))
  rw [Path.map_refl, Path.Homotopic.Quotient.mk_refl]
  exact hγ_null
