...
variable {J : Type*} [Category J] [HasColimitsOfShape J TypeCat.{u}]
...
instance {ι : Type v} [Finite ι] (X : ι → SSet.{u}) [HasCoproduct X]
    [∀ j, (X j).Finite] :
    (∐ X).Finite := by
  have : HasColimitsOfShape (Discrete ι) TypeCat.{u} := by
...
lemma range_eq_iSup_sigma_ι
    {ι : Type v} [HasColimitsOfShape (Discrete ι) TypeCat.{u}]
