...
def primitiveEdge {n : ℕ} {i : Fin (n + 1)}
  ...
  obtain rfl | hn : n = 2 ∨ 2 < n := by
    rw [eq_comm, or_comm, ← le_iff_lt_or_eq]; grind
  · revert i j; decide
...
def primitiveTriangle {n : ℕ} (i : Fin (n + 4))
  ...
  refine ⟨stdSimplex.triangle
    (n := n+3) ⟨k, by grind⟩ ⟨k+1, by grind⟩ ⟨k+2, by grind⟩ ?_ ?_, ?_⟩
  ...
  have hS : ¬ ({i, (⟨k, by grind⟩ : Fin (n + 4)), (⟨k + 1, by grind⟩ : Fin (n + 4)),
      (⟨k + 2, by grind⟩ : Fin (n + 4))} = Finset.univ) := fun hS ↦ by
    obtain ⟨i, hi⟩ := i
    by_cases hk : k = 0
    · subst hk
