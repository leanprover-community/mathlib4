...
@[reducible, inline]
def toSSet (A : X.Subcomplex) : SSet.{u} := A.toFunctor
...
@[reducible, inline]
def ι (A : Subcomplex X) : Quiver.Hom (V := SSet) A X := Subfunctor.ι A
...
@[reducible, inline]
def homOfLE : (S₁ : SSet.{u}) ⟶ (S₂ : SSet.{u}) := Subfunctor.homOfLe h
...
@[reducible, inline]
def ofSimplex {n : ℕ} (x : X _⦋n⦌) : X.Subcomplex := Subfunctor.ofSection x
...
@[reducible, inline]
def range : Y.Subcomplex := Subfunctor.range f

/-- The morphism `X ⟶ Subcomplex.range f` induced by `f : X ⟶ Y`. -/
@[reducible, inline]
def toRange : X ⟶ Subcomplex.range f := Subfunctor.toRange f
