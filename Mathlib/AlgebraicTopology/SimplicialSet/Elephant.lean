...
/-- A path in any `n + 1`-truncated simplicial set `X` is defined by further 1-truncating `X`, then taking the 1-truncated path. -/
...
class StrictSegal where
  ...
  spine_spineToSimplex {m : ℕ} {hmn : m ≤ n + 1} : X.spine ∘ spineToSimplex = id
  spineToSimplex_spine {m : ℕ} {hmn : m ≤ n + 1} : spineToSimplex ∘ X.spine = id
...
def test (n : ℕ) : n ≤ n := by exact Nat.le_refl n
...
instance strictSegal_of_strictSegal
  ...
  spine_spineToSimplex {n} :=
    Truncated.StrictSegal.spine_spineToSimplex (m := n + 1) (hmn := Nat.le_refl _)
  spineToSimplex_spine {n} :=
    Truncated.StrictSegal.spineToSimplex_spine (m := n + 1) (hmn := Nat.le_refl _)
