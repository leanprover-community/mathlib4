...
@[no_expose]
noncomputable def homInvId (e : Edge x y) : CompStruct e e.inv (id x) :=
  Nonempty.some (by simp [CompStruct.nonempty_iff])

/-- `Edge.inv` is a left inverse. -/
@[no_expose]
noncomputable def invHomId (e : Edge x y) : CompStruct e.inv e (id y) :=
  Nonempty.some (by simp [CompStruct.nonempty_iff])

/-- In a Kan complex, this is a chosen `InvStruct` structure of any edge. -/
noncomputable abbrev invStruct (e : Edge x y) : e.InvStruct where
  inv := e.inv
  homInvId := e.homInvId
  invHomId := e.invHomId
