...
theorem comp_Hσ_eq {Y : C} {n a q : ℕ} {φ : Y ⟶ X _⦋n + 1⦌} (v : HigherFacesVanish q φ)
    (hnaq : n = a + q) :
    φ ≫ (Hσ q).f (n + 1) = -φ ≫ X.δ ⟨a + 1, by grind⟩ ≫ X.σ ⟨a, by grind⟩ := by
  have hnaq_shift (d : ℕ) : n + d = a + d + q := by grind
  ...
  swap
  · rintro ⟨k, hk⟩
    suffices φ ≫ X.δ (⟨a + 2 + k, by grind⟩ : Fin (n + 2)) = 0 by
      simp only [this, Fin.natAdd_mk, Fin.cast_mk, zero_comp, smul_zero]
    convert v ⟨a + k + 1, by grind⟩ (by rw [Fin.val_mk]; grind)
    dsimp
    grind
  ...
  · -- c + a = 0
    ...
    rintro ⟨i, hi⟩ _
    simp only
    have hia : (⟨i, by grind⟩ : Fin (n + 2)) ≤
        Fin.castSucc (⟨a, by grind⟩ : Fin (n + 1)) := by
...
theorem comp_Hσ_eq_zero {Y : C} {n q : ℕ} {φ : Y ⟶ X _⦋n + 1⦌} (v : HigherFacesVanish q φ)
  ...
  · simp only [hσ'_eq (show n + 1 = 0 + q by grind) (c_mk (n + 2) (n + 1) rfl), pow_zero,
...
theorem induction {Y : C} {n q : ℕ} {φ : Y ⟶ X _⦋n + 1⦌} (v : HigherFacesVanish q φ) :
  ...
  by_cases hqn : n < q
  · rw [v.comp_Hσ_eq_zero hqn, zero_comp, add_zero, v j (by grind)]
  -- we now assume that n≥q, and write n=a+q
  obtain ⟨a, ha⟩ := Nat.le.dest (not_lt.mp hqn)
  rw [v.comp_Hσ_eq (show n = a + q by grind), neg_comp, add_neg_eq_zero, assoc, assoc]
  rcases n with - | m
  ...
  have haj : a < j := (Ne.le_iff_lt hj₂).mp (by grind)
  ...
  · -- case where `a<m`
    ...
    · rw [Fin.le_iff_val_le_val]
      dsimp
      grind
    simp only [← assoc, v j (by grind), zero_comp]
