...
instance (priority := 10000) : Epi A.e :=
  A.2.2
...
instance (priority := 10000) : Fintype (IndexSet Δ) :=
  Fintype.ofInjective
    (fun A =>
...
instance (priority := 10000) : Inhabited (IndexSet Δ) :=
  ⟨id Δ⟩
...
instance (priority := 10000) : Category (Split C) where
  Hom := Split.Hom
  id S :=
    { F := 𝟙 _
      f := fun n => 𝟙 _ }
