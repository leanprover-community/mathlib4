...
theorem d_squared (n : ℕ) : objD X (n + 1) ≫ objD X n = 0 := by
  ...
  · -- φ(S) is contained in Sᶜ
    intro ij hij
    simp +zetaDelta only [S, Finset.mem_univ, Finset.compl_filter, Finset.mem_filter, true_and,
      Fin.val_succ, Fin.coe_castLT] at hij ⊢
    linarith
  · -- φ : S → Sᶜ is injective
    rintro ⟨i, j⟩ hij ⟨i', j'⟩ hij' h
    rw [Prod.mk.inj_iff]
    exact ⟨by simpa +zetaDelta using congr_arg Prod.snd h,
      by simpa +zetaDelta [Fin.castSucc_castLT] using congr_arg Fin.castSucc (congr_arg Prod.fst h)⟩
  ...
  · -- identification of corresponding terms in both sums
    ...
    congr 1
    · simp +zetaDelta only [Fin.val_succ, pow_add, pow_one, mul_neg, neg_neg, mul_one]
      apply mul_comm
