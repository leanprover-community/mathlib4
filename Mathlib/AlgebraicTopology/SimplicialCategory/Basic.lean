...
/-structure SSet.Path (X : SSet.{v}) (x y : X _[0]) where
  p : X _[1]
  p₀ : X.δ 1 p = x
  p₁ : X.δ 1 p = x-/
...
@[simp]
lemma sHomWhiskerRight_id (K L : C) :
    sHomWhiskerRight (𝟙 K) L = 𝟙 _ := by
...
@[simp]
lemma sHomWhiskerLeft_id (K L : C) :
    sHomWhiskerLeft K (𝟙 L) = 𝟙 _ := by
...
lemma sHom_whisker_exchange {K K' L L' : C} (f : K ⟶ K') (g : L ⟶ L') :
    sHomWhiskerLeft K' g ≫ sHomWhiskerRight f L' = sHomWhiskerRight f L ≫ sHomWhiskerLeft K g :=
