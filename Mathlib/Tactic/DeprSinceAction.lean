...
syntax "warning:" build ":" num ":" num ": After here, please add (since := " str ") or whatever date is appropriate `⟨"num "," num "⟩`" : build
...
def getFromLinter (fl : FromLinter) : TSyntax `build → FromLinter
  ...
  | `(build| warning: $fil:build: $_ : $_ : After here, please add (since := $_) or whatever date is appropriate `⟨$line, $col⟩`) =>
...
--run_cmd
--  let stx ← `(build| warning: ././././Mathlib/NumberTheory/SumFourSquares.lean:98:13: After here, please add (since := "2024-06-13") or whatever date is appropriate `⟨98, 44⟩`)
--  let s := getFromLinter {} ⟨stx⟩
--  let xx := s.toList[0]!.2[0]!
--  logInfo s!"{(xx.line, xx.col)}"
...
def updateDeprecationsCLI (args : Parsed) : IO UInt32 := do
  ...
  let env : Environment ← importModules (leakEnv := true) #[{module := `Mathlib.Tactic.DeprSinceAction}] {}
