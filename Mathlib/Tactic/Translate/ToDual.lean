...
/-- `to_dual_for src := e` tells `to_dual` to translate `src` to `e`.

TODO: this currently doesn't accept the `(relevant_arg := ...)`/`(dont_translate := ...)` syntax.
...
elab tk:"to_dual_for" src:ident " := " tgt:term : command => Command.liftTermElabM do
  ...
  addTranslationFor data tk src tgt [] none
