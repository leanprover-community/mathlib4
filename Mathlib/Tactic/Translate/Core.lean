...
register_option linter.translate.warnInvalid : Bool := {
  defValue := true
...
where
  ...
  insertTranslationAux (src : Name) (t : TranslateData) (info : TranslationInfo) : CoreM Unit := do
    ...
    /- We use `asyncMode := .local` because theorems are translated on an asynchronous environment
    branch, where the default `.mainOnly` mode panics. On the main branch, `.local` behaves
    identically to `.mainOnly`, so entries added there are persisted as usual. Entries added on an
    asynchronous branch only concern auxiliary declarations of the theorem being translated on that
    branch, so it is fine that they are discarded at the end of the branch. -/
    modifyEnv (t.translations.addEntry · (src, info) (asyncMode := .local))
...
/-- Run `addDecl` on the translated declaration `decl` with name `name` and value `value`.
If the kernel throws an error, try to emit a better error message. -/
def addTranslatedDecl (t : TranslateData) (decl : Declaration) (name : Name) (value : Expr) :
    MetaM Unit := do
  ...
  try
    ...
    try
      withoutExporting <| check value
    catch ex =>
      throwError "@[{t.attrName}] failed to add declaration `{name}`.\n  \
...
def updateAndAddDecl (t : TranslateData) (tgt : Name) (srcDecl : ConstantInfo)
  ...
  addTranslatedDecl t decl.1.toDeclaration! decl.1.name (decl.1.value! (allowOpaque := true))
  return decl

/-- Unfold `simp`, `gcongr` and `hcongr`/`congr_simp` auxlemmas in the given expression.
...
def findTargetName (env : Environment) (t : TranslateData) (src rootSrc rootTgt : Name) :
  ...
  /- Other auxiliary declarations, such as `_proof_i` declarations shared with previous
  declarations, or macro-scoped names like `wrapped✝` generated by `irreducible_def`, get a fresh
  auxiliary name in the namespace of `rootTgt`: the name generator prefix is set to `rootTgt` in
  `addTranslationAttr`. The name must live in the namespace of `rootTgt` because theorems are
  translated on an asynchronous environment branch, which only accepts declarations whose name has
  prefix `rootTgt`. -/
  let name ← mkAuxDeclName (Name.mkSimple s!"_{t.attrName.toString}")
  /- We advance the name generator to reserve the generated name: the name is only added to the
  environment after translating the auxiliary declarations of `src`, which get their names from
  the same generator, and `mkAuxDeclName` avoids reusing an index only based on the constants
  that are already present in the environment. -/
  setDeclNGen (← getDeclNGen).next
  return name

/-- Returns an array of auxiliary constants in `e` that might have been generated
...
-/
def findAuxDeclsIn (e : Expr) (pre : Name) : CoreM (Array Name) := do
  let env ← withoutExporting getEnv
  return e.foldConsts #[] fun n l ↦
...
@[inherit_doc findAuxDeclsIn]
def findAuxDecls (decl : ConstantInfo) (pre : Name) : CoreM (Array Name) :=
  findAuxDeclsIn (Expr.app decl.type (decl.value! (allowOpaque := true))) pre
...
/-- Register the namespaces of the prefixes of the declaration name `name`, mirroring the private
`registerNamePrefixes` in `Lean.AddDecl`.

`addDecl` registers these namespaces itself, but for asynchronously translated theorems it only
runs on the asynchronous environment branch, where namespace registrations are discarded. So we
need to register the namespaces on the main branch ourselves. (Declarations written by the user
are not affected by this: `theorem Foo.bar ...` is expanded into `namespace Foo theorem bar ...`
on the command level, and the `namespace` command registers the namespace.) -/
...
/-- Translate the theorem `src` to `tgt`: the type is translated on the current thread and `tgt`
is added to the environment via `Environment.addConstAsync`, while the proof is translated in an
asynchronous task. This way, retrieving the (possibly still in-progress) asynchronously
elaborated proof of `src` does not block the elaboration thread.

`transformAuxDecl` is used to recursively translate the auxiliary declarations occurring in the
type (on the current thread) and in the proof (in the asynchronous task).

Note: unlike in the synchronous code path of `transformDeclRec`, the `relevant_arg` option is
inferred from just the type of `src`, without taking the proof into account. For theorems, the
type determines how the constant is used in later declarations, so this makes no practical
difference. -/
def transformThmDeclAsync (t : TranslateData) (cfg : Config) (src tgt : Name)
    (srcInfo : AsyncConstantInfo) (reorder : ArgReorder) (rename : NameMap Name)
    (transformAuxDecl : Name → CoreM Unit) : CoreM Unit := do
  let sig := srcInfo.sig.get
  -- we first unfold all auxlemmas, since they are not always able to be translated on their own
  let srcType ← withoutExporting do MetaM.run' do unfoldSimpAuxLemmas sig.type
  -- we then transform all auxiliary declarations occurring in the type
  for n in ← findAuxDeclsIn srcType src do
    transformAuxDecl n
  -- now translate the type
  let (type, relevantArg?) ← withoutExporting do MetaM.run' do
    let renamed := renameBinderNames (t.guessNameExt.getState (← getEnv)) rename srcType
    let (type, relevantArg?) ← applyReplacementForall t cfg.dontTranslate renamed
    return (← reorderForall reorder type, relevantArg?)
  let relevantArg ← getRelevantArg t cfg relevantArg? src
  insertTranslation t src tgt { reorder } relevantArg cfg.ref
  if type == srcType then
  ...
  -- compute the exported constant kind of `tgt`, mirroring the logic in `Lean.addDecl`
  let exportedKind? ←
    if isPrivateName tgt then
      pure <| if ← ResolveName.backward.privateInPublic.getM then some .thm else none
    else if (← getEnv).header.isModule then
      pure (some .axiom)
    else
      pure (some .thm)
  -- add `tgt` to the environment; the proof will be committed by the asynchronous task
  let async ← (← getEnv).addConstAsync tgt .thm (exportedKind? := exportedKind?)
  async.commitSignature { name := tgt, levelParams := sig.levelParams, type }
  setEnv async.mainEnv
  -- now add the metadata that does not depend on the proof.
  -- `src` is a theorem, so we don't need to consider `noncomputable`, `compileDecl`,
  -- reducibility hints or matcher info.
  modifyEnv (registerNamePrefixes · tgt)
  addDeclarationRangesFromSyntax tgt (← getRef) cfg.ref
  if isProtected (← getEnv) src then
    modifyEnv (addProtected · tgt)
  -- necessary so that e.g. equations can be generated for `tgt`
  enableRealizationsForConst tgt
  -- finally, translate the proof in an asynchronous task
  let cancelTk ← IO.CancelToken.new
  let act ← withRef cfg.ref <| Core.wrapAsyncAsSnapshot (cancelTk? := cancelTk)
      (desc := s!"@[{t.attrName}] translating {src} to {tgt}") fun srcDecl => do
    setEnv async.asyncEnv
    try
      unless srcDecl.all == [src] do
        throwError "`{t.attrName}` does not support mutually recursive declarations."
      -- we first unfold all auxlemmas, since they are not always able to be translated on their own
      let value ← withoutExporting do MetaM.run' do
        unfoldSimpAuxLemmas <| srcDecl.value! (allowOpaque := true)
      -- we then transform all auxiliary declarations occurring in the proof
      for n in ← findAuxDeclsIn value src do
        transformAuxDecl n
      trace[translate] "Value before translation:{indentExpr value}"
      -- now translate the proof
      let value ← withoutExporting do MetaM.run' do
        let (value, _) ← applyReplacementLambda t cfg.dontTranslate value
        reorderLambda reorder value
      trace[translate] "generating\n{tgt} : {type} :={indentExpr value}"
      MetaM.run' <| addTranslatedDecl t
        (.thmDecl { name := tgt, levelParams := sig.levelParams, type, value }) tgt value
    catch ex =>
      unless ex.isInterrupt do
        logError ex.toMessageData
        /- Try to still add `tgt`, with a `sorry` proof, so that the kernel environment contains
        `tgt` and subsequent declarations that use `tgt` don't produce confusing knock-on kernel
        errors. Note that when `addDecl` throws a kernel error, it has already added a fallback
        constant itself. -/
        unless (← withoutExporting getEnv).containsOnBranch tgt do
          try
            withOptions (fun o => Elab.async.set (warn.sorry.set o false) false) do
              let value ← MetaM.run' do mkSorry type (synthetic := true)
              addDecl <| .thmDecl { name := tgt, levelParams := sig.levelParams, type, value }
          catch _ => pure ()
    -- commit the results to the main environment branch
    if (← withoutExporting getEnv).containsOnBranch tgt then
      async.commitCheckEnv (← getEnv)
  -- The task is chained on the constant info of `src`,
  -- so it only starts running once the proof of `src` has been elaborated.
  Core.logSnapshotTask {
    stx? := some cfg.ref
    task := ← BaseIO.mapTask act srcInfo.constInfo
    cancelTk? := cancelTk }
...
Theorems are translated asynchronously: the type is translated on the current thread, but the
proof is translated in a background task, so that we don't block the elaboration thread on the
(possibly still in-progress) asynchronous elaboration of the proof of `src`.
See `transformThmDeclAsync`.
-/
partial def transformDeclRec (t : TranslateData) (cfg : Config) (rootSrc rootTgt src : Name)
  ...
  if env.setExporting false |>.contains tgt then
    ...
    return
  /- For theorems, we translate the type on the current thread, and translate the proof in an
  asynchronous task, so that retrieving the (possibly still in-progress) proof of `src` does not
  block the elaboration thread.

  We exclude translations with unfold boundaries (i.e. `to_dual`): there, the translated type
  cannot be computed without also translating the value, because unfold boundaries are only
  inserted if the translation fails without them. We also exclude `@[defeq]` theorems, because
  `inferDefEqAttr` needs the translated proof and must tag `tgt` on the main environment branch;
  proofs of `@[defeq]` theorems are `rfl`-shaped and hence cheap to wait for. -/
  let srcInfo ← withoutExporting do getAsyncConstInfo src
  if src == rootSrc && srcInfo.kind == .thm && t.unfoldBoundaries?.isNone &&
      Elab.async.get (← getOptions) && !defeqAttr.hasTag env src && !isMarkedMeta env src then
    transformThmDeclAsync t cfg src tgt srcInfo reorder rename
      (transformDeclRec t cfg rootSrc rootTgt ·)
    return
...
partial def checkExistingType (t : TranslateData) (src tgt : Name) (cfg : Config) (lint := true) :
  ...
  let srcDecl ← getConstVal src
  let tgtDecl ← getConstVal tgt
  ...
  let levels ← mkFreshLevelMVars srcDecl.levelParams.length
...
def elabTranslationAttr (declName : Name) (stx : Syntax) : CoreM Config := do
  ...
  | `(attrArgs| $hint $[$opts:bracketedOption]* $[$tgt]? $[$doc]?) =>
    MetaM.run' <| forallTelescope (← getConstVal declName).type fun xs _ => do
...
partial def addTranslationAttr (t : TranslateData) (src : Name) (cfg : Config)
  ...
  if alreadyExists then
    ...
    unless (← withoutExporting do getAsyncConstInfo src).kind matches .defn | .thm | .opaque do
      throwError "`{t.attrName}` cannot translate `{.ofConstName src}` because it has no value."
    let reorder := cfg.reorder?.getD {}
    /- We add the docstring before creating `tgt`: theorems are created on an asynchronous
    environment branch, and the docstring of such a declaration is looked up in the environment
    branch of the declaration, so it needs to be present before the branch is created. -/
    if let some doc := cfg.doc then
      addDocStringCore tgt doc
    /- Base the names of generated auxiliary declarations on `tgt`. This name generator state is
    also inherited (through a fork) by the asynchronous proof translation task for theorems, whose
    environment branch only accepts declarations whose name has prefix `tgt`. -/
    withDeclNameForAuxNaming tgt do
