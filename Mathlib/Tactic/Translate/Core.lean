...
def targetName (t : TranslateData) (cfg : Config) (src : Name) : CoreM Name := do
  ...
  -- pre-pend the first components from `src` to create a name of the same depth.
  ...
  let resultingName := if cfg.target == .anonymous then
    translatedNamespace.str translatedName
  else
    pre1 ++ cfg.target
  ...
  if cfg.target == pre2.str translatedName && !cfg.allowAutoName then
  ...
  if cfg.target != .anonymous then
    trace[translate_detail] "The automatically generated name would be {translatedNamespace.str translatedName}"
