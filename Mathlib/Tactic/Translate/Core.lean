...
def targetName (t : TranslateData) (cfg : Config) (src : Name) : CoreM Name := do
  ...
  let autoGeneratedName := GuessName.guessName (t.guessNameExt.getState (← getEnv)) s
  let translatedNamespace := translateNamespace (← getEnv) pre
  let depth := cfg.target.getNumParts
  let (pre1, pre2) := translatedNamespace.splitAt (depth - 1)
  let resultingName := if cfg.target == .anonymous then
    translatedNamespace.str autoGeneratedName
  else
    pre1 ++ cfg.target
  ...
  if cfg.target == pre2.str autoGeneratedName && !cfg.allowAutoName then
  ...
  if cfg.target != .anonymous then
    trace[translate_detail] "The automatically generated name would be {translatedNamespace.str autoGeneratedName}"
