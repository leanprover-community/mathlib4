...
/-- Returns a `NameSet` of auxiliary constants in `e` that might have been generated
when adding `pre` to the environment, and which hence might need to be translated.
Examples include `pre.match_5`, `pre._proof_2` and `someOtherDeclaration._proof_2`.
...
def findAuxDecls (e : Expr) (pre : Name) (env : Environment) : NameSet :=
  e.foldConsts ∅ fun n l ↦
    if (env.find? n).any (·.hasValue) &&
      ...
      (privateToUserName n).getPrefix == privateToUserName pre) then
      l.insert n
    else
      l
...
partial def transformDeclRec (t : TranslateData) (ref : Syntax) (pre tgt_pre src : Name)
  ...
  for n in findAuxDecls srcDecl.type pre (env.setExporting false) do
    -- We pass the same `dontTranslate` argument to the auxiliary declarations as a best guess,
    -- since we still lack a heuristic that automatically infers it.
    transformDeclRec t ref pre tgt_pre n (dontTranslate := dontTranslate)
  for n in findAuxDecls (srcDecl.value! (allowOpaque := true)) pre (env.setExporting false) do
    transformDeclRec t ref pre tgt_pre n (dontTranslate := dontTranslate)
