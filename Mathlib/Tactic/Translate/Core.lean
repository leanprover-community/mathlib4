...
structure Config : Type where
  ...
  trace : Bool := false
  /-- The given name of the target: `none` means no explicit name was provided,
...
def targetName (t : TranslateData) (cfg : Config) (src : Name) : CoreM Name := do
  if cfg.self then
    if let some target := cfg.target then
      logWarning m!"`{t.attrName} self` ignores the provided name {target}"
    return src
  if cfg.none then
    if let some target := cfg.target then
      logWarning m!"`{t.attrName} private` ignores the provided name {target}"
  ...
  let (resultingName, matchesAutogenerated) :=
    autoGeneratedName.modifySuffix cfg.target
  ...
  if !cfg.allowAutoName && matchesAutogenerated then
    Linter.logLintIf linter.translateGenerateName cfg.ref m!"\
      `{t.attrName}` correctly autogenerated target name for {src}.\n\
      You may remove the explicit argument {cfg.target.get!}."
  if cfg.target.isSome then
