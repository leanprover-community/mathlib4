...
def targetName (t : TranslateData) (cfg : Config) (src : Name) : CoreM Name := do
  ...
  -- Auto-generated name of the result declaration, without prior namespace components.
  ...
  let resultingName := if cfg.tgt == .anonymous then pre.str autoGeneratedName else pre1 ++ cfg.tgt
  ...
  if cfg.tgt == pre2.str autoGeneratedName && !cfg.allowAutoName then
  ...
  if cfg.tgt != .anonymous then
    trace[translate_detail] "The automatically generated name would be {pre.str autoGeneratedName}"
