...
/-- Syntax for specifying a single config option -/
syntax configOption := ident " := " term
/-- Syntax for `initialize_simps_projections`. -/
syntax simpsProj := ppSpace ident (" (" simpsRule,+ ")")? (" (" configOption,+ ")")?
...
/-- Execute the projection as specified by configs -/
def applyConfigOptions (projs : Array ParsedProjectionData) (configs : Array (Name × Syntax)) :
    CoreM (Array ParsedProjectionData) := do
  let projs : Array ParsedProjectionData := configs.foldl (init := projs) fun projs rule ↦
    match rule with
    | ⟨nm, stx⟩ =>
      if (projs.map (·.newName)).contains nm then
        projs.map fun proj ↦ if proj.newName = nm then
          ...
          proj else
        projs.push {strName := nm, newName := nm, strStx := stx, newStx := stx}
  trace[simps.debug] "Projection info after applying the config options: {projs}."
  unless (projs.map (·.newName)).toList.Nodup do throwError "\
    Config options modified projection rules cannot be set as config options\n\
    And the same Config option cannot be set twice"
  pure projs
...
def getRawProjections (stx : Syntax) (str : Name) (traceIfExists : Bool := false)
  (rules : Array ProjectionRule := #[]) (trc := false) (configs : Array (Name × Syntax) := #[]) :
  ...
  let projs ← applyConfigOptions projs configs
...
/-- parse a config option -/
def elabConfigOption : Syntax → CommandElabM (Name × Syntax)
  | `(configOption| $id1 := $t)  => return ⟨id1.getId, t⟩
  | _                            => Elab.throwUnsupportedSyntax
