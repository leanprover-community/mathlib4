...
norm_num_simproc [simp, seval] evalNatFib (Nat.fib _) where
  eval {_ _} e := ...
```
This declares `evalNatFib : NormNumExt`, tags it `@[norm_num Nat.fib _]`, and additionally
declares `evalNatFib.simproc` and registers it under `[simp, seval]`. Registering in `seval`
is what makes the procedure available to `grind`. Both declarations are generated as
`public meta`, which `@[norm_num]` requires, so the command does not need to be used inside a
`public meta section`.
...
syntax (docComment)? "norm_num_simproc " "[" Lean.Parser.Term.attrInstance,* "] " ident
...
elab_rules : command
  | `($[$doc?:docComment]?norm_num_simproc [$attrs,*] $name:ident ($pat:term) $val:declVal) => do
    ...
    elabCommand (← `($[$doc?:docComment]?@[norm_num $pat] public meta def $name : NormNumExt
      $val:declVal))
    elabCommand (← `(
      /-- The simproc companion of a `norm_num` extension, generated by `norm_num_simproc`. -/
      public meta def $procName : Simp.Simproc := NormNumExt.toSimproc $name))
