...
lemma IsInt.raw_refl (n : ℤ) : IsInt n n := ⟨rfl⟩
...
lemma isNat_natAbs_pos : {n : ℤ} → {a : ℕ} → IsNat n a → IsNat n.natAbs a
  | _, _, ⟨rfl⟩ => ⟨rfl⟩

lemma isNat_natAbs_neg : {n : ℤ} → {a : ℕ} → IsInt n (.negOfNat a) → IsNat n.natAbs a
  | _, _, ⟨rfl⟩ => ⟨by simp⟩
...
lemma isNat_natCast {R} [AddMonoidWithOne R] (n m : ℕ) :
...
lemma isNat_intCast {R} [Ring R] (n : ℤ) (m : ℕ) :
    IsNat n m → IsNat (n : R) m := by rintro ⟨⟨⟩⟩; exact ⟨by simp⟩

lemma isintCast {R} [Ring R] (n m : ℤ) :
