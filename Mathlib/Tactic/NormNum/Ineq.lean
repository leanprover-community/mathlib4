...
theorem isNat_le_true [Semiring α] [OrderedSemiring α] : {a b : α} → {a' b' : ℕ} →
    IsNat a a' → IsNat b b' → Nat.ble a' b' = true → a ≤ b
  | _, _, _, _, ⟨rfl⟩, ⟨rfl⟩, h => Nat.mono_cast (Nat.le_of_ble_eq_true h)

theorem isNat_lt_false [Semiring α] [OrderedSemiring α] {a b : α} {a' b' : ℕ}
    (ha : IsNat a a') (hb : IsNat b b') (h : Nat.ble b' a' = true) : ¬a < b :=
  not_lt_of_le (isNat_le_true hb ha h)

theorem isRat_le_true [Ring α] [LinearOrderedRing α] : {a b : α} → {na nb : ℤ} → {da db : ℕ} →
...
theorem isRat_lt_true [Ring α] [LinearOrderedRing α] [Nontrivial α] :
...
theorem isRat_le_false [Ring α] [LinearOrderedRing α] [Nontrivial α]
    {a b : α} {na nb : ℤ} {da db : ℕ}
    (ha : IsRat a na da) (hb : IsRat b nb db) (h : decide (nb * da < na * db)) : ¬a ≤ b :=
  not_le_of_lt (isRat_lt_true hb ha h)

theorem isRat_lt_false [Ring α] [LinearOrderedRing α] {a b : α} {na nb : ℤ} {da db : ℕ}
    (ha : IsRat a na da) (hb : IsRat b nb db) (h : decide (nb * da ≤ na * db)) : ¬a < b :=
  not_lt_of_le (isRat_le_true hb ha h)

/-! # (In)equalities -/

theorem isNat_lt_true [Semiring α] [OrderedSemiring α] [CharZero α] : {a b : α} → {a' b' : ℕ} →
    IsNat a a' → IsNat b b' → Nat.ble b' a' = false → a < b
  | _, _, _, _, ⟨rfl⟩, ⟨rfl⟩, h =>
    Nat.cast_lt.2 <| ble_eq_false.1 h

theorem isNat_le_false [Semiring α] [OrderedSemiring α] [CharZero α] {a b : α} {a' b' : ℕ}
    (ha : IsNat a a') (hb : IsNat b b') (h : Nat.ble a' b' = false) : ¬a ≤ b :=
  not_le_of_lt (isNat_lt_true hb ha h)

theorem isInt_le_true [Ring α] [OrderedRing α] : {a b : α} → {a' b' : ℤ} →
    IsInt a a' → IsInt b b' → decide (a' ≤ b') → a ≤ b
  | _, _, _, _, ⟨rfl⟩, ⟨rfl⟩, h => Int.cast_mono <| of_decide_eq_true h

theorem isInt_lt_true [Ring α] [OrderedRing α] [Nontrivial α] : {a b : α} → {a' b' : ℤ} →
    IsInt a a' → IsInt b b' → decide (a' < b') → a < b
  | _, _, _, _, ⟨rfl⟩, ⟨rfl⟩, h => Int.cast_lt.2 <| of_decide_eq_true h

theorem isInt_le_false [Ring α] [OrderedRing α] [Nontrivial α] {a b : α} {a' b' : ℤ}
    (ha : IsInt a a') (hb : IsInt b b') (h : decide (b' < a')) : ¬a ≤ b :=
  not_le_of_lt (isInt_lt_true hb ha h)

theorem isInt_lt_false [Ring α] [OrderedRing α] {a b : α} {a' b' : ℤ}
    (ha : IsInt a a') (hb : IsInt b b') (h : decide (b' ≤ a')) : ¬a < b :=
...
@[norm_num _ ≤ _] def evalLE : NormNumExt where eval {v β} e := do
  ...
  let rec intArm : MetaM (Result e) := do
    let rα ← synthInstanceQ (q(Ring $α) : Q(Type u)) <|> throwError "not a ring"
    let _i ← synthInstanceQ (q(OrderedRing $α) : Q(Type u)) <|> throwError "not an ordered ring"
    guard <|← withNewMCtxDepth <| isDefEq f q(LE.le (α := $α))
    haveI' : $e =Q ($a ≤ $b) := ⟨⟩
    let ⟨za, na, pa⟩ ← ra.toInt rα
    let ⟨zb, nb, pb⟩ ← rb.toInt rα
  ...
  let rec ratArm : MetaM (Result e) := do
    ...
    let _r ← synthInstanceQ (q(Field $α))
    let _i ← synthInstanceQ (q(LinearOrderedField $α) : Q(Type u)) <|>
      throwError "not a linear ordered field"
    guard <|← withNewMCtxDepth <| isDefEq f q(LE.le (α := $α))
    haveI' : $e =Q ($a ≤ $b) := ⟨⟩
  ...
  | .isNat ra na pa, .isNat rb nb pb =>
    let _r ← synthInstanceQ (q(Semiring $α) : Q(Type u)) <|> throwError "not a semiring"
    let _i ← synthInstanceQ (q(OrderedSemiring $α) : Q(Type u)) <|> throwError "not an ordered semiring"
