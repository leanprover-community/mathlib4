...
def Later.crushListReduceOptionM [Monad m] [MonadBacktrack σ m]
...
where
  ...
  -- We extend `exceeds` to `Option γ` in the arguments that may be `best?`.
  -- This tells us what to do when there isn't a `best` yet; in particular, nothing is fatally
  -- bigger than an absent `best`.
  ...
  -- This function takes the best out of all of the possibilities given in `alts`.
  -- If nothing better than the best was found, we return `pure none`. If every alt failed, the
