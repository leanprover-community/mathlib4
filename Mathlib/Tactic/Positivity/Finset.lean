...
def evalFinsetSum : PositivityExt where eval {u α} zα pα e := do
  ...
  | ~q(@Finset.sum $ι _ $instα $s $f) =>
    ...
    let p_pos : Option Q(0 < $e) := ← (do
      ...
      let .some im ← trySynthInstanceQ q(AddCommMonoid $α) | pure none
      let .some io ← trySynthInstanceQ q(PartialOrder $α) | pure none
      let .some i ← trySynthInstanceQ q(IsOrderedCancelAddMonoid $α) | pure none
      assertInstancesCommute
      let pr : Q(∀ i, 0 < $f i) ← mkLambdaFVars #[i] pbody
      return some q(@sum_pos $ι $α $im $io $i $f $s (fun i _ ↦ $pr i) $ps))
    ...
    -- Fall back to showing that the sum is nonnegative
      ...
      let pr : Q(∀ i, 0 ≤ $f i) ← mkLambdaFVars #[i] pbody
      let im ← synthInstanceQ q(AddCommMonoid $α)
      let io ← synthInstanceQ q(PartialOrder $α)
      let i ← synthInstanceQ q(IsOrderedAddMonoid $α)
      assertInstancesCommute
      return .nonnegative q(@sum_nonneg $ι $α $im $io $i $f $s fun i _ ↦ $pr i)
