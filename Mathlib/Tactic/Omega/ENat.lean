...
2. Simplify arithmetic expressions involving infinities, making (in)equalities either trivial
   or free of infinities. This step uses the `enat_to_nat_top_simps` simp set.
3. Translate the remaining goals from `ENat` to `Nat` using the `enat_to_nat_coe_simps` simp set.
...
attribute [enat_to_nat_top_simps] top_add ENat.sub_top ENat.top_sub_coe ENat.mul_top ENat.top_mul

@[enat_to_nat_top_simps] lemma not_lt_top (x : ENat) :
    ¬(⊤ < x) := by cases x <;> simp

@[enat_to_nat_coe_simps] lemma coe_add (m n : ℕ) :
    (m : ENat) + (n : ENat) = ((m + n : ℕ) : ENat) := rfl

@[enat_to_nat_coe_simps] lemma coe_sub (m n : ℕ) :
    (m : ENat) - (n : ENat) = ((m - n : ℕ) : ENat) := rfl

@[enat_to_nat_coe_simps] lemma coe_mul (m n : ℕ) :
    (m : ENat) * (n : ENat) = ((m * n : ℕ) : ENat) := rfl

@[enat_to_nat_coe_simps] lemma coe_ofNat (n : ℕ) [n.AtLeastTwo] :
    (OfNat.ofNat n : ENat) = (OfNat.ofNat n : ℕ) := rfl

@[enat_to_nat_coe_simps] lemma coe_zero : (0 : ENat) = ((0 : ℕ) : ENat) := rfl

@[enat_to_nat_coe_simps] lemma coe_one : (1 : ENat) = ((1 : ℕ) : ENat) := rfl

attribute [enat_to_nat_coe_simps] ENat.coe_inj ENat.coe_le_coe ENat.coe_lt_coe

open Qq Lean Elab Tactic Term Meta in
/-- Finds the first `ENat` in the context and applies the `cases` tactic to it.
Then simplifies expressions involving `⊤` using the `enat_to_nat_top_simps` simp set. -/
elab "cases_first_ENat" : tactic => focus do
  ...
  g.withContext do
    ...
    evalTactic (← `(tactic| all_goals try simp only [enat_to_nat_top_simps] at *))
...
macro "enat_to_nat" : tactic => `(tactic| focus (
    ...
    (try simp only [enat_to_nat_top_simps, enat_to_nat_coe_simps] at *)
  )
)
