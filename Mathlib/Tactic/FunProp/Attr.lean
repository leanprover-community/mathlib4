...
/-- The attribute `fun_prop` is used to mark function property definitions like `Continuous`
...
Option `always_try_transition`: some definitions can have `@[fun_prop always_try_transition]`
  ...
  (i.e. written at `f ∘ g` for nontrivial `f` and `g`). If `always_try_transition` is enabled,
  then `fun_prop` can apply transition theorems at any stage of the proof search.
...
syntax (name := funPropAttr) "fun_prop" (&"always_try_transition")? : attr


@[inherit_doc funPropAttr]
initialize
