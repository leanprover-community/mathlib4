...
Writing
```lean
#deprecate_to new_name new_name₂ ... new_nameₙ
...
`#deprecate_to` makes an effort to match `old_name`, the "visible" name, with
...
-/
elab tk:"#deprecate_to" id:ident* dat:(ppSpace str ppSpace)? ppLine cmd:command : command => do
  let oldEnv ← getEnv
  try
    elabCommand cmd
  finally
