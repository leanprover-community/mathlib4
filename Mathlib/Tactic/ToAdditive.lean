...
def Lean.Name.replace (n : Name) (s t : String) : Name :=
  match n with
  | .str p s' => if s' = s then .str p t else .str (p.replace s t) s'
  | .num p n => .num (p.replace s t) n
  | .anonymous => n
...
def findTargetName (env : Environment) (src pre tgt_pre : Name) : CoreM Name :=
  ...
  else if src.components.contains `_auxLemma then
    return src.replace "_auxLemma" "_auxAddLemma"
...
def findAuxDecls (e : Expr) (pre mainModule : Name) : NameSet :=
  ...
  e.foldConsts ∅ fun n l ↦
    if n.getPrefix == pre || n.components.contains `_auxLemma || isPrivateName n || n.hasMacroScopes then
      l.insert n
    else
      l
