...
def findTargetName (env : Environment) (src pre tgt_pre : Name) : CoreM Name :=
  let privatePrefix := privateHeader ++ env.mainModule |>.num 0
  ...
  /- This covers equation lemmas (for other declarations). -/
  else if let some post := privatePrefix.isPrefixOf? src then
    ...
    -- Note to future self: if this errors we could do this instead by calling `getEqnsFor?`
    | some addName => return src.updatePrefix <| privatePrefix ++ addName
  -- Note: this additivizes lemmas generated by `simp`.
  -- We do not currently check whether such lemmas actually should be additivized.
...
attribute. We will only translate it if the naming rules of `to_additive`
-/
def findAuxDecls (e : Expr) (pre mainModule : Name) : NameSet :=
let auxLemma := mainModule ++ `_auxLemma
let privatePrefix := privateHeader ++ mainModule |>.num 0
e.foldConsts ∅ fun n l ↦
  if n.getPrefix == pre || n.getPrefix == auxLemma || privatePrefix.isPrefixOf n then
    l.insert n
  else
    l
  ...
  try
    ...
    | Exception.error _ msg =>
      if pre.isPrefixOf src then
        ...
        trace[to_additive] "There was an error translating {src}."
        return
    | _ => panic! "unreachable"
...
* Option 2: It didn't additivize a declaration that should be additivized.
  ...
  If this was an auxiliary declaration, then it is possible that there was an error additivizing
  the auxiliary declaration. Use `@[to_additive?]` to see if this is the case.

  Otherwise, this happened because the heuristic applied, and the first argument contains a fixed
  type, like `ℕ` or `ℝ`. Solutions:
