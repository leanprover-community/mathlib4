...
def computeTendsto (f : Q(ℝ → ℝ)) (source : Q(Filter ℝ)) :
  ...
  | ~q(atBot) =>
    ...
    return ⟨q($limit), q(tendsto_comp_neg_atTop_iff.mp $h_tendsto)⟩
  | ~q(𝓝[<] $c) =>
    let ⟨limit, h_tendsto⟩ ← computeTendstoAtTop q(fun x ↦ $f ($c - x⁻¹))
    return ⟨q($limit), q(tendsto_nhdsLT_of_tendsto_atTop $f $c $h_tendsto)⟩
  | ~q(𝓝[>] $c) =>
    let ⟨limit, h_tendsto⟩ ← computeTendstoAtTop q(fun x ↦ $f ($c + x⁻¹))
    return ⟨q($limit), q(tendsto_nhdsGT_of_tendsto_atTop $f $c $h_tendsto)⟩
  | ~q(𝓝[≠] $c) =>
    ...
    if !(← isDefEq limit_left limit_right) then
      ...
      | ~q(𝓝 $a), ~q(𝓝 $b) =>
        ...
        return ⟨q($limit_left), q(tendsto_nhdsNE_of_tendsto_atTop_nhds_of_eq
      ...
      return ⟨q($limit_left),
        q(tendsto_nhdsNE_of_tendsto_atTop $f $c $h_tendsto_left $h_tendsto_right)⟩
...
def proveIsLittleOReal (source : Q(Filter ℝ)) (f g : Q(ℝ → ℝ)) : TacticM Q($f =o[$source] $g) := do
  ...
  | .top h_tendsto => return q(IsLittleO.of_tendsto_div_atTop $h_tendsto)
  | .bot h_tendsto => return q(IsLittleO.of_tendsto_div_atBot $h_tendsto)
...
def proveIsBigOWithReal (C : Q(ℝ)) (source : Q(Filter ℝ)) (f g : Q(ℝ → ℝ)) :
  ...
  | ~q(𝓝 $a) =>
    ...
    appendGoals extraGoals
    return q(isBigOWith_of_div_tendsto_nhds $h_tendsto $hC_pos $ha)
...
def proveIsBigOReal (source : Q(Filter ℝ)) (f g : Q(ℝ → ℝ)) : TacticM Q($f =O[$source] $g) := do
  let ⟨limit, h_tendsto⟩ ← computeTendsto q(fun x ↦ $g x / $f x) source
  match limit with
  | ~q(atTop) => return q(isBigO_of_div_tendsto_atTop $h_tendsto)
  | ~q(atBot) => return q(isBigO_of_div_tendsto_atBot $h_tendsto)
  | ~q(𝓝 $c) =>
    let h_ne : Q($c ≠ 0) ← mkFreshExprMVarQ q($c ≠ 0)
    let extraGoals ← evalTacticAt
      (← `(tactic| norm_num)) h_ne.mvarId!
    appendGoals extraGoals
    return q(isBigO_of_div_tendsto_nhds_of_ne_zero $h_tendsto $h_ne)
...
def proveIsThetaReal (source : Q(Filter ℝ)) (f g : Q(ℝ → ℝ)) :
  ...
  | ~q(𝓝 $c) =>
    ...
    appendGoals extraGoals
    return q(isTheta_of_div_tendsto_nhds_ne_zero $h_tendsto $hc)
