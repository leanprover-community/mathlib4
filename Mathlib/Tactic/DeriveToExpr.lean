...
def mkToExprHeader (argNames : Array Name) (nestedOcc : NestedOccurence): TermElabM Header := do
  -- The auxiliary functions we produce are `indtype -> Expr`.
  let header ← mkHeader ``ToExpr 1 argNames nestedOcc
  return header
...
def mkLocalInstanceLetDecls (ctx : Deriving.Context) (argNames : Array Name) :
  ...
  for i in [:ctx.typeInfos.size] do
    let indVal       := ctx.typeInfos[i]!.getIndVal
...
def mkAuxFunction (ctx : Deriving.Context) (i : Nat) : TermElabM Command := do
  ...
  let header     ← mkToExprHeader argNames nestedOcc
  ...
  let levels := indVal.levelParams.toArray.map mkIdent
  Term.withLevelNames indVal.levelParams do Term.elabBinders binders fun _ => do
...
def mkInstanceCmds (ctx : Deriving.Context) (typeNames : Array Name) :
  ...
  for i in [:ctx.typeInfos.size] do
    ...
    if typeNames.contains indVal.name then
      ...
      let binders      := binders ++ (← mkInstImplicitBinders ``ToExpr nestedOcc argNames)
...
def mkToExprInstanceCmds (declNames : Array Name) : TermElabM (Array Syntax) := do
  let ctx ← mkContext "toExpr" declNames[0]! false
