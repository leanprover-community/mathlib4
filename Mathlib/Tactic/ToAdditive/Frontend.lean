...
/-- Unfold `proof_nnn` aux lemmas using `unfoldAuxLemmas` in the type and value of `decl`.
The `simp` tactic generates such lemmas that are impossible to translate by themselves,
using the current algorithm.
TODO: consider trying to additivize them anyway, and on failure resort to unfolding them.
TODO: this definition currently also unfolds auxiliary declarations generated by
`declAbstractNestedProofs`, which is not great for performance (cf. #23559).
Unlike auxiliary declarations generated by `simp`, these can be translated by themselves
(this likely requires a core change if we don't want to do this with backtracking as in the first
TODO). -/
...
which is sort of an inverse to `declUnfoldAuxLemmas`. -/
...
partial def transformDeclAux
  ...
  -- we first unfold all `proof_nnn` aux lemmas, since they are not always able
  -- to be additivized on their own. See `declUnfoldAuxLemmas` docstring for a TODO
  let srcDecl ← MetaM.run' do declUnfoldAuxLemmas srcDecl
  -- we then transform all remaining auxiliary declarations generated when elaborating `pre`
