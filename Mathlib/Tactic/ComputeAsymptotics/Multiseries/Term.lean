...
In this module we show how to find the limit of the monomial
of the form `coef * b1(x)^d1 * b2(x)^d2 * ...`
...
`basis : List (ℝ → ℝ)` is given, one can interpret `⟨coef, exps⟩ : Term` as function
