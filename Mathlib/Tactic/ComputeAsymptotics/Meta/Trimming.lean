...
partial def trimWithoutOracle {basis : Q(Basis)} (ms : Q(MultiseriesExpansion $basis))
    (h_sorted : Q(MultiseriesExpansion.Sorted $ms))
  ...
  | ~q(List.cons $basis_hd $basis_tl) =>
    ...
    let h_extracted_sorted : Q(MultiseriesExpansion.Sorted $ms_extracted) :=
      q($h_eq_extracted ▸ $h_sorted)
    match ms_extracted with
    | ~q(MultiseriesExpansion.mk .nil $f) =>
      return some {
        val := q($ms_extracted)
        h_sorted := q($h_extracted_sorted)
    ...
    | ~q(MultiseriesExpansion.mk (.cons $exp $coef $tl) $f) =>
      ...
      | .neq h_coef_ne_zero =>
        return some {
          ...
          h_sorted := q(MultiseriesExpansion.Sorted.cons $coef_trimmed.h_sorted
            $h_comp $h_tl_sorted)
          h_approx :=
      ...
      | .eq h_coef_eq_zero =>
        ...
        return some {
          ...
          h_sorted := q($tl_trimmed.h_sorted)
...
partial def trim {basis : Q(Basis)} (ms : Q(MultiseriesExpansion $basis))
  ...
  match ← trimWithoutOracle q($ms) q($h_sorted) q($h_approx) q($h_basis) allZero with
  ...
  | none =>
    ...
    return {
      ...
      h_sorted := q(MultiseriesExpansion.Sorted.nil $f)
...
def trimPartialMS (ms : MS) :
  ...
  let res ← trim ms.val ms.h_sorted ms.h_approx ms.h_basis true
  let newMs : MS := {
    basis := q($ms.basis)
    logBasis := q($ms.logBasis)
    val := q($res.val)
