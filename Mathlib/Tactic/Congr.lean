...
-- syntax (name := congr) "congr" (ppSpace (colGt num))? : tactic
...
-/
@[tactic congr] def congrTacticImpl : Tactic := fun stx =>
match stx with
| `(tactic|congr $(x?)?) =>
  withMainContext do
...
-- TODO: use `congr' with p (: n)?` to call `ext p (: n)?` to all subgoals generated by `congr'`.
-- this may depend on `ext` implementing this case, or on correctly relating to `ext`.
