...
def importInfo (importStx : Syntax) : Option (Syntax × Bool × Bool × Bool) := do
  ...
  -- Check for modifiers by looking for atoms with specific values, recursively searching in optional nodes
  let rec hasKeyword (s : Syntax) (kw : String) : Bool :=
    s.getArgs.any fun arg =>
      (arg.isAtom && arg.getAtomVal == kw) || hasKeyword arg kw
  let isPublic := hasKeyword importStx "public"
  let isMeta := hasKeyword importStx "meta"
  let isAll := hasKeyword importStx "all"
