...
def Lean.Environment.localDefinitionDependencies (env : Environment) (stx id : Syntax) :
  ...
  let defs := constInfos.filter
    fun constInfo => !(getOriginalConstKind? env constInfo.name == some .thm || constInfo.isCtor)
...
def upstreamableDeclLinter : Linter where run := withSetOptionIn fun stx ↦ do
    ...
    if id != .missing then
      ...
      if (skipDef && if let some constInfo := env.find? name
         then !(getOriginalConstKind? env constInfo.name == some .thm || constInfo.isCtor)
         else true) ||
