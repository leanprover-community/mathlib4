...
abbrev Correspondence := Std.HashMap String.Pos String.Pos
...
def atomOrIdentEndPos {m} [Monad m] [MonadLog m] [AddMessageContext m] [MonadOptions m]
    ...
    m String.Pos := do
  let afterOrig := readWhile orig pp
  if verbose? then
    if afterOrig == pp && (!afterOrig.isEmpty) then
      logWarning m!"No change at{indentD m!"'{afterOrig}'"}\n{k.map MessageData.ofConstName}\n\n\
      Maybe because the `SyntaxNodeKind`s contain:\n\
      hygieneInfo: {k.contains `hygieneInfo}\n\
      choice: {k.contains `choice}"
  pure afterOrig.startPos
...
elab "#show_corr " cmd:command : command => do
  ...
  if let some pretty := ← Mathlib.Linter.pretty stxNoSpaces then
    let (_, corr) ← generateCorrespondence true Std.HashMap.emptyWithCapacity #[cmd.raw.getKind] cmd pretty.toSubstring
    let fm ← getFileMap
    let sorted := corr.toArray.qsort (·.1 < ·.1 )
    logInfo m!"{sorted.map fun (a, b) => (fm.toPosition a, b)}\n{pretty}"
