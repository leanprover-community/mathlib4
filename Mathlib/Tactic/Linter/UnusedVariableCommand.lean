...
def getDeclBinders {m} [Monad m] [MonadQuotation m] [MonadRef m] (stx : Syntax) :
  ...
  let (id, hyps, typ) := ← match stx with
    ...
    --| `($_:declModifiers instance  $_:optNamedPrio $did:(declId)?        $[$as]* : $t $_:declVal) => return (default, default, (← `($(mkIdent `toFalse) $t)))
    | `($_:declModifiers instance $(_optNamedPrio)? $(did)? $as* : $t $_:declVal) => return (did.getD default, as, (← `($(mkIdent `toFalse) $t)))
