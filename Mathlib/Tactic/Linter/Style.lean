...
private def isBadNameWithUnderscoreAux (moduleAutoSuffix : String) (badIfEligible : Bool) : Name → Bool
  | .num .. => false -- internal
  | .str pre s =>
    ...
    let badIfEligible := badIfEligible || s.contains '_'
...
* names that contain a component equal to `formatter` or `parenthesizer` (these are usually
  autogenerated from parser names)
