...
private def isBadNameWithUnderscoreAux (moduleAutoSuffix : String) (badIfEligible : Bool) :
    Name → Bool
  | .num .. => false -- internal
  | .str pre s =>
    let stillEligible := !(s == "Simps" || s == "formatter" || s == "parenthesizer" ||
      s.endsWith moduleAutoSuffix ||
      (s.dropSuffix Char.isDigit |>.endsWith '_')) -- ends with "_<number>" or an underscore
    let badIfEligible := badIfEligible ||
      /- exempt individual components that contain "«" or seem to be autogenerated instance names,
      but don't exempt the whole name for this reason. -/
      (s.contains '_' && !s.contains '«' && !s.startsWith "inst")
    stillEligible && isBadNameWithUnderscoreAux moduleAutoSuffix badIfEligible pre
...
* names containing guillemets `«»` (these tend to be `term<something>` declarations,
  i.e. internal names for notation, not user-facing commands, or else have been named deliberately),
