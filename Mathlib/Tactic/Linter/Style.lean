...
/--
Check whether a definition name contains an underscore that does not fit the naming convention.

Namely, we do **not** lint against
* names containing a `Simps` component (these are probably custom simps projections, i.e. affect
  how `simps` names its auto-generated lemmas: we usually prefer a generated name `coe_support`
  over `coeSupport`, which requires a projection named `coe_support`)
* `Simproc`s, `LibraryNote`s
* names that end in `_<number>` or in `_<project>` (these tend to be autogenerated instance names)
* (`Trailing`)`ParserDescr`s, created by `syntax`; formatters and parenthesizers (human-named or
  otherwise)
* deprecated names
* autogenerated names (according to `Batteries.Tactic.Lint.isAutoDecl`, after `privateToUserName`)
* non-defs or defs with `Prop` types
* components of names of the form `<keyword>_` where `<keyword>` is a parsing keyword, such as
  `section_`
* if a def is namespaced under a theorem, we do not lint the part of the name coming from the
  theorem

Does not assume the declaration is in the environment yet.

We do not assume private usernames have already been turned into their corresponding user-facing
names.
-/
public def isBadDefNameWithUnderscore (declName : Name) : MetaM Bool := do
  let env ← getEnv
  -- Only lint declarations that are either definitions or not in the environment.
  -- (Mirrors `wasOriginallyDefn`, except for the default value of `false` for nonexistent decls.)
  if (getOriginalConstKind? env declName |>.map (· matches .defn)) matches some false then
    return false
  -- Only lint definitions which are not autogenerated.
  -- TODO: remove if `isAutoDecl` handles private names correctly.
  if ← isAutoDecl <| privateToUserName declName then
    return false
  -- Exempt deprecated names
  if isDeprecated env declName ||
    -- Exempt formatters/parenthesizers
    (PrettyPrinter.formatterAttribute.ext.getState env).declNames.contains declName ||
    (PrettyPrinter.parenthesizerAttribute.ext.getState env).declNames.contains declName
  then return false
  -- We use `findConstVal?` instead of `getConstVal` to enable testing names not in the environment.
  if let some { type .. } := env.findConstVal? declName then
    -- We also exempt simprocs: these should be named like normal lemmas.
    -- check if their type is `Lean.Meta.Simp.Simproc`.
    if type.isConstOf `Lean.Meta.Simp.Simproc ||
      -- Exempt parser names, including human-written ones
      type.isConstOf ``ParserDescr || type.isConstOf ``TrailingParserDescr ||
      -- Exempt library notes
      type.isConstOf `Batteries.Util.LibraryNote
    then return false
    -- `Prop`-typed `defs` should be caught by a different linter anyway;
