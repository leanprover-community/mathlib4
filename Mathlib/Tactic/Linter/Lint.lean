...
The `dupNamespace` linter produces a warning when a declaration name's component is repeated
...
/--
The `dupNamespace` linter produces a warning when a declaration name's component is repeated
several times. This includes consecutive repetitions (such as, `Nat.Nat.foo` and `One.two.two`),
but is not limited to them: for example, `Nat.One.Nat`, `Nat.Prime.Nat.Prime.foo` and
`Nat.Prime.Nat.bar` would all trigger a warning.
