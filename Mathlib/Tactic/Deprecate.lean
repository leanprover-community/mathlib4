...
/-!
# `#deprecate` -- a deprecation tool

Writing
```lean
#deprecate new_name new_name₂ ... new_nameₙ
...
namespace Mathlib.Tactic.Deprecate
...
`#deprecate` makes an effort to match `old_name`, the "visible" name, with
