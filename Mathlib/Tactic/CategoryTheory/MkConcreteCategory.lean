...
one-field structure `Hom` which wraps a term of `FC`, with field `Hom.hom'` and internal constructor
`Hom._mkInternal`, and uses it as the type of morphisms in the category. It then creates:
...
* `Hom.Simps.hom`, so `simps` uses the public projection `hom` instead of `hom'`;
...
private meta def categoryNamespaceIdent (cat : TSyntax `term) (message : String) :
  ...
  if let some n := headTermIdent? cat.raw then
    let name ← liftCoreM <| realizeGlobalConstNoOverloadWithInfo (mkIdentFrom cat n)
    pure <| mkIdent (privateToUserName name)
  else
    throwErrorAt cat message

/-- Elaborate commands in an absolute namespace, restoring the caller's scope even on failure. -/
private meta def elabInNamespace (ns : Ident) (body : CommandElabM Unit) : CommandElabM Unit :=
  withScope (fun scope => { scope with currNamespace := .anonymous }) do
...
private meta def registerToAdditiveExisting (src tgt : Name) : CommandElabM Unit := do
  let srcIdent := mkIdent (rootNamespace ++ src)
  let tgtIdent := mkIdent (rootNamespace ++ tgt)
...
private meta def elabMkConcreteCategoryCore (mods : Syntax) (cat FC idTerm compTerm : TSyntax `term)
  ...
  elabCommand <| ← (set_option hygiene false in `(command|
    instance instCategory : CategoryTheory.Category $cat where
      Hom X Y := Hom (X := X) (Y := Y)
      id X := Hom._mkInternal (X := X) (Y := X) (($idTerm) X)
      comp {X Y Z} f g := Hom._mkInternal (X := X) (Y := Z) (($compTerm) g.hom' f.hom')))
  ...
  elabCommand <| ← (set_option hygiene false in `(command|
    instance instConcreteCategory :
      ...
      hom := fun f => Hom.hom' f
      ofHom := fun {X Y} f => Hom._mkInternal (X := X) (Y := Y) f
