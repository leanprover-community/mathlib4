...
def Kind.Context : Kind → Type
  | Kind.monoidal => Monoidal.Context
  | Kind.bicategory => Bicategory.Context
...
def pureCoherenceMain (mvarId : MVarId) : MetaM (List MVarId) := do
  ...
  match k with
  | .monoidal => withTraceNode k.name (fun _ => return m!"{checkEmoji} {k.name}") do
    Monoidal.pureCoherence mvarId
  | .bicategory =>  withTraceNode k.name (fun _ => return m!"{checkEmoji} {k.name}") do
    Bicategory.pureCoherence mvarId
...
def coherence (mvarId : MVarId) : MetaM (List MVarId) := do
  ...
  match k with
  | .monoidal => withTraceNode k.name (fun _ => return m!"{checkEmoji} {k.name}") do
    Monoidal.monoidal mvarId
  | .bicategory => withTraceNode k.name (fun _ => return m!"{checkEmoji} {k.name}") do
    Bicategory.bicategory mvarId
