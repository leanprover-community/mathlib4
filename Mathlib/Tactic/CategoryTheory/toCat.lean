...
theorem eq_app' {C D : Cat.{v, u}} {f g : C ⟶ D} {η θ : f ⟶ g} (w : η = θ) (X : C) :
    η.app X = θ.app X :=
...
initialize registerBuiltinAttribute {
  ...
  | `(attr| to_app $[(attr := $stx?,*)]?) => MetaM.run' do
    ...
    addRelatedDecl src "_app" ref stx? fun type value levels => do
      pure (← toAppExpr (← mkExpectedTypeHint value type), levels)
...
example {a b c d : B} (F : OplaxFunctor B Cat) (f : a ⟶ b) (g : b ⟶ c) (h : c ⟶ d) : True := by
  ...
  have := to_app_of% @OplaxFunctor.mapComp_assoc_right B _ Cat _ F
