...
macro_rules
  | `(tactic| cfc_tac) => `(tactic|
         try (first |
              assumption |
              infer_instance |
              aesop (rule_sets := [$(Lean.mkIdent `CFC):ident])))
  ...
  | `(tactic| cfc_cont_tac) =>
    `(tactic| try (first
      | fun_prop (disch := aesop (config := {warnOnNonterminal := false})
          (rule_sets := [$(Lean.mkIdent `CFC):ident]))
      | assumption))
...
macro_rules
  | `(tactic| cfc_zero_tac) =>
      `(tactic| try (first | aesop (rule_sets := [$(Lean.mkIdent `CFC):ident]) | assumption))
