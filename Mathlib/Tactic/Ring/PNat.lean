...
instance {n} : CSLiftVal (no_index (OfNat.ofNat (n+1)) : ℕ+) (n + 1) := fast_instance% ⟨rfl⟩

instance {n h} : CSLiftVal (Nat.toPNat n h) n := fast_instance% ⟨rfl⟩

instance {n} : CSLiftVal (Nat.succPNat n) (n + 1) := fast_instance% ⟨rfl⟩

instance {n} : CSLiftVal (Nat.toPNat' n) (n.pred + 1) := fast_instance% ⟨rfl⟩

instance {n k} : CSLiftVal (PNat.divExact n k) (n.div k + 1) := fast_instance% ⟨rfl⟩
