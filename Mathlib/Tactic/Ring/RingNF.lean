...
structure Config extends RingConfig, AtomM.Recurse.Config where
...
-/
def evalExpr (cfg : RingConfig) (e : Expr) : AtomM Simp.Result := do
...
def ringNFTarget (s : IO.Ref AtomM.State) (cfg : Config) : TacticM Unit := withMainContext do
  ...
  let r ← AtomM.recurse s cfg.toConfig (evalExpr cfg.toRingConfig) (cleanup cfg) tgt
-- >>>>>>> master
...
def ringNFLocalDecl (s : IO.Ref AtomM.State) (cfg : Config) (fvarId : FVarId) :
  ...
  let goal ← getMainGoal
  let myres ← AtomM.recurse s cfg.toConfig (evalExpr cfg.toRingConfig) (cleanup cfg) tgt
...
elab (name := ring1NF) "ring1_nf" tk:"!"? cfg:optConfig : tactic => do
  ...
  if tk.isSome then cfg := { cfg with red := .default, zetaDelta := true }
  let s ← IO.mkRef {}
  liftMetaMAtMain fun g ↦ AtomM.RecurseM.run s cfg.toConfig (evalExpr cfg.toRingConfig)
    (cleanup cfg) <| proveEq g (cfg.toRingConfig)
...
@[tactic ringNFConv] def elabRingNFConv : Tactic := fun stx ↦ match stx with
  | `(conv| ring_nf $[!%$tk]? $cfg:optConfig) => withMainContext do
    ...
    Conv.applySimpResult
      (← AtomM.recurse s cfg.toConfig (evalExpr cfg.toRingConfig) (cleanup cfg)
