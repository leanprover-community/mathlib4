...
abbrev M := ReaderT Context CanonAtomM
...
-/
partial def M.run
    {α : Type} (s : IO.Ref CanonAtomM.State) (cfg : RingNF.Config)
    (red : TransparencyMode := cfg.red) (s' : IO.Ref Canonicalizer.State) (x : M α) : MetaM α := do
...
initialize ringCleanupRef.set fun e => do
  M.run (← IO.mkRef {}) { recursive := false } .reducible (← IO.mkRef {}) fun nctx _ _=>
    return (← nctx.simp { expr := e } ({} : Lean.Meta.Simp.Methods).toMethodsRef nctx.ctx
      |>.run {}).1.expr

open Elab.Tactic Parser.Tactic
/-- Use `ring_nf` to rewrite the main goal. -/
def ringNFTarget (s : IO.Ref CanonAtomM.State) (cfg : Config)
    (red : TransparencyMode := cfg.red) (s' : IO.Ref Canonicalizer.State): TacticM Unit :=
  withMainContext do
...
def ringNFLocalDecl (s : IO.Ref CanonAtomM.State) (cfg : Config)
    (red : TransparencyMode := cfg.red) (s' : IO.Ref Canonicalizer.State) (fvarId : FVarId) :
...
elab (name := ringNF) "ring_nf" tk:"!"? cfg:optConfig loc:(location)? : tactic => do
  ...
  let s ← IO.mkRef {}
  let s' ← IO.mkRef {}
  withLocation loc (ringNFLocalDecl s cfg cfg.red s') (ringNFTarget s cfg cfg.red s')
...
@[tactic ringNFConv] def elabRingNFConv : Tactic := fun stx ↦ match stx with
  | `(conv| ring_nf $[!%$tk]? $cfg:optConfig) => withMainContext do
    ...
    let s ← IO.mkRef {}
    let s' ← IO.mkRef {}
    Conv.applySimpResult (← M.run s cfg cfg.red s' <| rewrite (← instantiateMVars (← Conv.getLhs)))
