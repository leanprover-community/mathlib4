...
-/
def evalAddOverlap (char : ℕ) (rα : Option (Q(Ring $α)))
    (cpα : Option (Q(CharP $α $char))) {a b : Q($α)} (va : ExProd sα a) (vb : ExProd sα b) :
...
-/
partial def evalAdd (char : ℕ) (rα : Option (Q(Ring $α)))
    (cpα : Option (Q(CharP $α $char))) {a b : Q($α)} (va : ExSum sα a) (vb : ExSum sα b) :
...
-/
partial def ExBase.evalNatCast (char : ℕ) (rα : Option (Q(Ring $α)))
    (cpα : Option (Q(CharP $α $char))) {a : Q(ℕ)} (va : ExBase sℕ a) :
...
* `a - b = a + -b`
-/
def evalSub {α : Q(Type u)} (char : ℕ) (sα : Q(CommSemiring $α))
    (cpα : Option (Q(CharP $α $char))) {a b : Q($α)}
...
partial def eval {u : Lean.Level} {α : Q(Type u)} (sα : Q(CommSemiring $α))
  ...
  | ``HSub.hSub, some rα, _ | ``Sub.sub, some rα, _ => match e with
    | ~q($a - $b) => do
      let ⟨_, va, pa⟩ ← eval sα c a
      let ⟨_, vb, pb⟩ ← eval sα c b
      let ⟨c, vc, p⟩ ← evalSub c.char sα c.cpα rα va vb
