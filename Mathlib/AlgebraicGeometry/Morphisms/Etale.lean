...
instance : MorphismProperty.HasOfPostcompProperty
  ...
  intro X Y f ⟨hft, hfu⟩
  exact inferInstaceAs% (Etale (pullback.diagonal f))
...
instance : Category X.Etale :=
  inferInstaceAs% (Category (MorphismProperty.Over @Etale ⊤ X))
...
instance : (Etale.forget X).Full :=
  inferInstaceAs% ((MorphismProperty.Comma.forget _ _ _ _ _).Full)
instance : (Etale.forget X).Faithful :=
  inferInstaceAs% ((MorphismProperty.Comma.forget _ _ _ _ _).Faithful)
