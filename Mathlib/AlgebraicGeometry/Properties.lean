...
instance : T0Space X.carrier := fast_instance% by
...
instance : QuasiSober X.carrier := fast_instance% by
...
instance {R : CommRingCat} [H : _root_.IsReduced R] : IsReduced (Scheme.Spec.obj <| op R) := fast_instance% by
...
instance [h : IsIntegral X] : IsDomain (X.presheaf.obj (op ⊤)) := fast_instance%
...
instance is_irreducible_of_isIntegral [IsIntegral X] : IrreducibleSpace X.carrier := fast_instance% by
  by_contra H
...
instance {R : CommRingCat} [IsDomain R] : IsIntegral (Scheme.Spec.obj <| op R) := fast_instance%
