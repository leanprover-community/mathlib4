...
open CategoryTheory Functor CategoryTheory.Limits AlgebraicGeometry
...
instance GrpObjAsOverPullback [GrpObj (asOver M S)] : GrpObj (asOver (pullback (M ↘ S) f) T) :=
...
lemma μ_pullback_left_fst_fst (X Y : Over S) :
    (LaxMonoidal.μ (Over.pullback f) X Y).left ≫
      pullback.fst _ _ ≫ pullback.fst _ _ = pullback.fst _ _ ≫ pullback.fst _ _ := by
...
lemma prodComparisonIso_pullback_Spec_inv_left_fst_fst' {R S : CommRingCat.{u}} (f : R ⟶ S)
    ...
    (prodComparisonIso (Over.pullback (Spec.map f)) (.mk fA) (.mk fB)).inv.left ≫
...
lemma prodComparisonIso_pullback_Spec_inv_left_snd' {R S : CommRingCat.{u}} (f : R ⟶ S)
    {A B : Scheme.{u}} (fA : A ⟶ Spec R) (fB : B ⟶ Spec R) :
    (by exact (prodComparisonIso (Over.pullback (Spec.map f)) (.mk fA) (.mk fB)).inv.left) ≫
