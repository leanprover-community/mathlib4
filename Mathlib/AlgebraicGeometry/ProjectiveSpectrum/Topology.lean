...
@[simp]
lemma mem_zeroLocus (x : ProjectiveSpectrum 𝒜) (s : Set A) :
    x ∈ zeroLocus 𝒜 s ↔ s ⊆ x.asHomogeneousIdeal :=
  Iff.rfl
#align projective_spectrum.mem_zero_locus ProjectiveSpectrum.mem_zeroLocus

@[simp]
lemma zeroLocus_span (s : Set A) : zeroLocus 𝒜 (Ideal.span s) = zeroLocus 𝒜 s := by
  ext x
...
lemma coe_vanishingIdeal (t : Set (ProjectiveSpectrum 𝒜)) :
...
lemma mem_vanishingIdeal (t : Set (ProjectiveSpectrum 𝒜)) (f : A) :
...
@[simp]
lemma vanishingIdeal_singleton (x : ProjectiveSpectrum 𝒜) :
...
lemma subset_zeroLocus_iff_le_vanishingIdeal (t : Set (ProjectiveSpectrum 𝒜)) (I : Ideal A) :
...
lemma subset_zeroLocus_iff_subset_vanishingIdeal (t : Set (ProjectiveSpectrum 𝒜)) (s : Set A) :
...
lemma subset_vanishingIdeal_zeroLocus (s : Set A) : s ⊆ vanishingIdeal (zeroLocus 𝒜 s) :=
  (gc_set _).le_u_l s
#align projective_spectrum.subset_vanishing_ideal_zero_locus ProjectiveSpectrum.subset_vanishingIdeal_zeroLocus

lemma ideal_le_vanishingIdeal_zeroLocus (I : Ideal A) :
...
lemma homogeneousIdeal_le_vanishingIdeal_zeroLocus (I : HomogeneousIdeal 𝒜) :
...
lemma subset_zeroLocus_vanishingIdeal (t : Set (ProjectiveSpectrum 𝒜)) :
...
lemma zeroLocus_anti_mono {s t : Set A} (h : s ⊆ t) : zeroLocus 𝒜 t ⊆ zeroLocus 𝒜 s :=
  (gc_set _).monotone_l h
#align projective_spectrum.zero_locus_anti_mono ProjectiveSpectrum.zeroLocus_anti_mono

lemma zeroLocus_anti_mono_ideal {s t : Ideal A} (h : s ≤ t) :
...
lemma zeroLocus_anti_mono_homogeneousIdeal {s t : HomogeneousIdeal 𝒜} (h : s ≤ t) :
    zeroLocus 𝒜 (t : Set A) ⊆ zeroLocus 𝒜 (s : Set A) :=
...
lemma vanishingIdeal_anti_mono {s t : Set (ProjectiveSpectrum 𝒜)} (h : s ⊆ t) :
...
@[simp]
lemma zeroLocus_singleton_one : zeroLocus 𝒜 ({1} : Set A) = ∅ :=
...
lemma zeroLocus_sup_homogeneousIdeal (I J : HomogeneousIdeal 𝒜) :
...
lemma zeroLocus_union (s s' : Set A) : zeroLocus 𝒜 (s ∪ s') = zeroLocus _ s ∩ zeroLocus _ s' :=
  (gc_set 𝒜).l_sup
#align projective_spectrum.zero_locus_union ProjectiveSpectrum.zeroLocus_union

lemma vanishingIdeal_union (t t' : Set (ProjectiveSpectrum 𝒜)) :
...
lemma zeroLocus_iSup_ideal {γ : Sort*} (I : γ → Ideal A) :
...
lemma zeroLocus_iSup_homogeneousIdeal {γ : Sort*} (I : γ → HomogeneousIdeal 𝒜) :
...
lemma vanishingIdeal_iUnion {γ : Sort*} (t : γ → Set (ProjectiveSpectrum 𝒜)) :
...
lemma zeroLocus_inf (I J : Ideal A) :
...
lemma union_zeroLocus (s s' : Set A) :
...
lemma zeroLocus_singleton_mul (f g : A) :
...
@[simp]
lemma zeroLocus_singleton_pow (f : A) (n : ℕ) (hn : 0 < n) :
...
lemma sup_vanishingIdeal_le (t t' : Set (ProjectiveSpectrum 𝒜)) :
...
lemma mem_compl_zeroLocus_iff_not_mem {f : A} {I : ProjectiveSpectrum 𝒜} :
...
lemma isOpen_iff (U : Set (ProjectiveSpectrum 𝒜)) : IsOpen U ↔ ∃ s, Uᶜ = zeroLocus 𝒜 s := by
  simp only [@eq_comm _ Uᶜ]; rfl
#align projective_spectrum.is_open_iff ProjectiveSpectrum.isOpen_iff

lemma isClosed_iff_zeroLocus (Z : Set (ProjectiveSpectrum 𝒜)) :
...
lemma isClosed_zeroLocus (s : Set A) : IsClosed (zeroLocus 𝒜 s) := by
  rw [isClosed_iff_zeroLocus]
  exact ⟨s, rfl⟩
#align projective_spectrum.is_closed_zero_locus ProjectiveSpectrum.isClosed_zeroLocus

lemma zeroLocus_vanishingIdeal_eq_closure (t : Set (ProjectiveSpectrum 𝒜)) :
...
lemma vanishingIdeal_closure (t : Set (ProjectiveSpectrum 𝒜)) :
...
lemma isOpen_basicOpen {a : A} : IsOpen (basicOpen 𝒜 a : Set (ProjectiveSpectrum 𝒜)) :=
  (basicOpen 𝒜 a).isOpen
#align projective_spectrum.is_open_basic_open ProjectiveSpectrum.isOpen_basicOpen

@[simp]
lemma basicOpen_eq_zeroLocus_compl (r : A) :
...
lemma basicOpen_eq_union_of_projection (f : A) :
...
@[simp]
lemma as_ideal_le_as_ideal (x y : ProjectiveSpectrum 𝒜) :
...
@[simp]
lemma as_ideal_lt_as_ideal (x y : ProjectiveSpectrum 𝒜) :
    x.asHomogeneousIdeal < y.asHomogeneousIdeal ↔ x < y :=
  Iff.rfl
#align projective_spectrum.as_ideal_lt_as_ideal ProjectiveSpectrum.as_ideal_lt_as_ideal

lemma le_iff_mem_closure (x y : ProjectiveSpectrum 𝒜) :
