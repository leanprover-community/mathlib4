...
def fromOfGlobalSections : X ⟶ Proj 𝒜 := by
  ...
  rintro x y
  let e : pullback ((openCoverOfMapIrreleventEqTop 𝒜 f hf).f x)
      ((openCoverOfMapIrreleventEqTop 𝒜 f hf).f y) ≅ (X.basicOpen (f (x.snd.fst * y.snd.fst))) :=
...
lemma fromOfGlobalSections_preimage_basicOpen {r : A} {n : ℕ} (hn : 0 < n) (hr : r ∈ 𝒜 n) :
  ...
  · intro x hx
    ...
    simp only [openCoverOfMapIrreleventEqTop, Scheme.openCoverOfISupEqTop_X,
  ...
  · intro x hx
    let I : (openCoverOfMapIrreleventEqTop 𝒜 f hf).I₀ := ⟨n, r, hn, hr⟩
    obtain ⟨x, rfl⟩ : x ∈ ((openCoverOfMapIrreleventEqTop 𝒜 f hf).f I).opensRange := by
