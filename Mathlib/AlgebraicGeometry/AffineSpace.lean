...
open Opposite in
@[simps]
def CommRingCat.coyoneda : Type vᵒᵖ ⥤ CommRingCat.{u} ⥤ CommRingCat.{max u v} where
  obj n :=
...
def CommRingCat.coyonedaAdjunction (R : CommRingCat.{u}) :
...
open Opposite in
@[simps!]
def CommRingCat.coyonedaUnique [Unique n] : coyoneda.obj (op n) ≅ 𝟭 _ :=
  NatIso.ofComponents (fun X ↦ (RingEquiv.piUnique _).toCommRingCatIso) (fun f ↦ by ext; simp)

open Opposite in
@[simps]
def CommRingCat.units : CommRingCat.{u} ⥤ CommGrp.{u} where
  obj R := CommGrp.of Rˣ
  map f := CommGrp.ofHom (Units.map f.hom)

open Opposite in
@[simps]
def CommRingCat.monoidAlgebra (R : CommRingCat.{max u v}) : CommGrp.{v} ⥤ Under R where
...
def CommRingCat.forgetUnitsAdjoint (R : CommRingCat.{u}) :
...
def Under.equivalenceOfIsInitial {C : Type*} [Category C] {X : C} (hX : IsInitial X) :
    Under X ≌ C where
...
def CommRingCat.unitsAdjoint {R : CommRingCat.{u}} (hR : IsInitial R) :
    monoidAlgebra R ⋙ Under.forget R ⊣ units :=
  (forgetUnitsAdjoint R).comp (Under.equivalenceOfIsInitial hR).toAdjunction

instance (R : CommRingCat) : (CommRingCat.monoidAlgebra.{u, u} R).IsLeftAdjoint :=
  ⟨_, ⟨CommRingCat.forgetUnitsAdjoint R⟩⟩

instance : CommRingCat.units.IsRightAdjoint :=
  ⟨_, ⟨CommRingCat.unitsAdjoint initialIsInitial⟩⟩

@[simps! apply_left symm_apply]
def Scheme.isOverEquiv {X Y S : Scheme} [X.Over S] [Y.Over S] :
    { f : X ⟶ Y // f.IsOver S } ≃ (Over.mk (X ↘ S) ⟶ Over.mk (Y ↘ S)) where
  toFun f := Over.homMk f.1
  invFun f := ⟨f.left, ⟨f.w⟩⟩
  left_inv _ := by ext1; rfl
  right_inv _ := by ext1; rfl

open Opposite in
def yonedaEquiv :
    (Over.forget S).op ⋙ Scheme.Γ ⋙ CommRingCat.coyoneda.obj (op n) ⋙ forget _ ≅
      yoneda.obj (Over.mk (𝔸(n; S) ↘ S)) :=
  NatIso.ofComponents
    (fun X ↦ ((homOverEquiv (X := (unop X).left) S).symm.trans Scheme.isOverEquiv).toIso) (by
    intros X Y f
    ext g
    apply Over.OverMorphism.ext
    show homOfVector ((unop Y).left ↘ S) _ = f.unop.left ≫ homOfVector ((unop X).left ↘ S) g
    apply hom_ext <;> simp)
