...
instance (priority := 100) isIso_of_isEmpty {X Y : Scheme} (f : X ⟶ Y) [IsEmpty Y] :
    IsIso f := by
  haveI : IsEmpty X := f.val.base.1.isEmpty
  have : Epi f.val.base := by
...
instance initial_isEmpty : IsEmpty (⊥_ Scheme) :=
  ⟨fun x => ((initial.to Scheme.empty : _).val.base x).elim⟩
...
lemma sigmaι_eq_iff (i j : ι) (x y) :
    (Sigma.ι f i).val.base x = (Sigma.ι f j).val.base y ↔
...
lemma exists_sigmaι_eq (x : (∐ f : _)) : ∃ i y, (Sigma.ι f i).val.base y = x := by
  obtain ⟨i, y, e⟩ := (disjointGlueData f).ι_jointly_surjective ((sigmaIsoGlued f).hom.val.base x)
...
lemma sigmaMk_mk (i) (x : f i) :
    sigmaMk f (.mk i x) = (Sigma.ι f i).val.base x := by
