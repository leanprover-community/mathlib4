...
instance (priority := 10000) : HasTerminal Scheme :=
  hasTerminal_of_hasTerminal_of_preservesLimit Scheme.Spec

instance (priority := 10000) : IsAffine (⊤_ Scheme.{u}) :=
  isAffineOfIso (PreservesTerminal.iso Scheme.Spec).inv

instance (priority := 10000) : HasFiniteLimits Scheme :=
...
instance (priority := 10000) Scheme.hom_unique_of_empty_source (X : Scheme.{u}) : Unique (∅ ⟶ X) :=
...
instance (priority := 10000) : IsEmpty Scheme.empty.carrier :=
  show IsEmpty PEmpty by infer_instance

instance (priority := 10000) spec_punit_isEmpty : IsEmpty (Scheme.Spec.obj (op <| CommRingCat.of PUnit)).carrier :=
...
instance (priority := 10000) : HasInitial Scheme :=
...
instance (priority := 10000) initial_isEmpty : IsEmpty (⊥_ Scheme).carrier :=
...
instance (priority := 10000) : HasStrictInitialObjects Scheme :=
