...
lemma equiv_of_X_eq_of_Y_eq {P Q : F × F × F} (hPz : P z ≠ 0) (hQz : Q z ≠ 0)
  ...
  simp_rw [Units.smul_def, Units.val_div_eq_div_val, Units.val_mk0, smul_eq, div_pow, mul_comm,
    mul_div, ← hx, ← hy, mul_div_cancel_right₀ _ <| pow_ne_zero _ hQz, mul_div_cancel_left₀ _ hQz]

lemma equiv_some_of_Z_ne_zero {P : F × F × F} (hPz : P z ≠ 0) :
    P ≈ (P x / P z ^ 2, P y / P z ^ 3, 1) :=
  equiv_of_X_eq_of_Y_eq hPz one_ne_zero
    (by linear_combination (norm := ring1) -P x * div_self (pow_ne_zero 2 hPz))
    (by linear_combination (norm := ring1) -P y * div_self (pow_ne_zero 3 hPz))
...
lemma eval_polynomial_of_Z_ne_zero {P : F × F × F} (hPz : P z ≠ 0) :
    eval ![P x, P y, P z] W.polynomial / P z ^ 6 =
...
lemma eval_polynomialX_of_Z_ne_zero {P : F × F × F} (hPz : P z ≠ 0) :
    eval ![P x, P y, P z] W.polynomialX / P z ^ 4 =
...
lemma eval_polynomialY_of_Z_ne_zero {P : F × F × F} (hPz : P z ≠ 0) :
    eval ![P x, P y, P z] W.polynomialY / P z ^ 3 =
...
lemma nonsingular_zero [Nontrivial R] : W'.Nonsingular (1, 1, 0) := by
  ...
  exact fun h => one_ne_zero <| by linear_combination (norm := ring1) h.left - h.right.left
...
lemma X_ne_zero_of_Z_eq_zero [NoZeroDivisors R] {P : R × R × R} (hP : W'.Nonsingular P)
    (hPz : P z = 0) : P x ≠ 0 := by
  intro hPx
  rwa [nonsingular_of_Z_eq_zero hPz, equation_of_Z_eq_zero hPz, hPx, zero_pow three_ne_zero,
    pow_eq_zero_iff two_ne_zero, hP.left, zero_pow two_ne_zero, mul_zero, mul_zero, mul_zero,
    ne_self_iff_false, or_false, or_false, and_false] at hP

lemma isUnit_X_of_Z_eq_zero {P : F × F × F} (hP : W.Nonsingular P) (hPz : P z = 0) : IsUnit (P x) :=
...
lemma equiv_of_Z_eq_zero {P Q : F × F × F} (hP : W.Nonsingular P) (hQ : W.Nonsingular Q)
  ...
  simp_rw [Units.smul_def, smul_eq, Units.val_mul, Units.val_div_eq_div_val, IsUnit.unit_spec,
