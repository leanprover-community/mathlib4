...
lemma map_eq {S : Type s} (f : R → S) (P : R × R × R) : f ∘ P = (f (P x), f (P y), f (P z)) := by
  rfl
...
lemma map_equiv_map {f : R →+* S} (hf : Function.Bijective f) (P Q : R × R × R) :
  ...
  simp_rw [map_eq, Units.smul_def, smul_eq, Units.coe_map, RingHom.toMonoidHom_eq_coe,
    MonoidHom.coe_ofClass, Prod.mk.injEq, ← map_pow, ← map_mul, hf.injective.eq_iff] at hu
...
lemma Y_sub_Y_of_isUnit_Z {P Q : R × R × R} (hPz : IsUnit <| P z) (hQz : IsUnit <| Q z) :
    P y * Q z ^ 3 - Q y * P z ^ 3 =
      P z ^ 3 * Q z ^ 3 * (P y * hPz.unit⁻¹ ^ 3 - Q y * hQz.unit⁻¹ ^ 3) := by
  nth_rw 1 [mul_comm <| _ * _, sub_mul, mul_comm <| _ ^ 3]
  simp_rw [mul_mul_mul_comm, ← mul_pow, IsUnit.val_inv_mul, one_pow, mul_one]

lemma Y_sub_Y_of_Z_ne_zero {P Q : F × F × F} (hPz : P z ≠ 0) (hQz : Q z ≠ 0) :
    P y * Q z ^ 3 - Q y * P z ^ 3 = P z ^ 3 * Q z ^ 3 * (P y / P z ^ 3 - Q y / Q z ^ 3) := by
...
lemma Y_eq_iff_of_isUnit_Z {P Q : R × R × R} (hPz : IsUnit <| P z) (hQz : IsUnit <| Q z) :
  ...
  rw [← sub_eq_zero, Y_sub_Y_of_isUnit_Z hPz hQz, ((hPz.pow 3).mul (hQz.pow 3)).mul_right_eq_zero,
    sub_eq_zero]

@[deprecated (since := "2025-05-26")] alias Y_eq_iff := Y_eq_iff_of_isUnit_Z

lemma Y_eq_iff_of_Z_ne_zero {P Q : F × F × F} (hPz : P z ≠ 0) (hQz : Q z ≠ 0) :
    P y * Q z ^ 3 = Q y * P z ^ 3 ↔ P y / P z ^ 3 = Q y / Q z ^ 3 := by
...
lemma Y_ne_iff_of_isUnit_Z {P Q : R × R × R} (hPz : IsUnit <| P z) (hQz : IsUnit <| Q z) :
    IsUnit (P y * Q z ^ 3 - Q y * P z ^ 3) ↔
      IsUnit (P y * hPz.unit⁻¹ ^ 3 - Q y * hQz.unit⁻¹ ^ 3) := by
  simp_rw [Y_sub_Y_of_isUnit_Z hPz hQz, IsUnit.mul_iff, isUnit_pow_iff three_ne_zero, hPz, hQz,
    true_and]
...
lemma eval_polynomial_smul (P : R × R × R) (u : R) :
    eval (![(u • P) x, (u • P) y, (u • P) z]) W'.polynomial =
      u ^ 6 * eval ![P x, P y, P z] W'.polynomial := by
...
lemma equation_smul (P : R × R × R) {u : R} (hu : IsUnit u) :
  ...
  simp_rw [Equation, eval_polynomial_smul, (hu.pow 6).mul_right_eq_zero]
...
lemma equation_of_Z_ne_zero {P : F × F × F} (hPz : P z ≠ 0) :
  ...
  simp_rw [equation_of_isUnit_Z hPz.isUnit, Units.val_inv_eq_inv_val, IsUnit.unit_spec, inv_pow,
    div_eq_mul_inv]
...
lemma eval_polynomialX_smul (P : R × R × R) (u : R) :
    eval (![(u • P) x, (u • P) y, (u • P) z]) W'.polynomialX =
      u ^ 4 * eval ![P x, P y, P z] W'.polynomialX := by
...
lemma eval_polynomialY_smul (P : R × R × R) (u : R) :
    eval (![(u • P) x, (u • P) y, (u • P) z]) W'.polynomialY =
      u ^ 3 * eval ![P x, P y, P z] W'.polynomialY := by
...
lemma eval_polynomialZ_smul (P : R × R × R) (u : R) :
    eval (![(u • P) x, (u • P) y, (u • P) z]) W'.polynomialZ =
      u ^ 5 * eval ![P x, P y, P z] W'.polynomialZ := by
...
lemma nonsingular_smul (P : R × R × R) {u : R} (hu : IsUnit u) :
  ...
  simp_rw [Nonsingular, equation_smul P hu, eval_polynomialX_smul, eval_polynomialY_smul,
    eval_polynomialZ_smul, Ideal.span_insert, Ideal.span_singleton_mul_left_unit <| hu.pow _]
...
lemma nonsingular_some (a b : R) : W'.Nonsingular (a, b, 1) ↔ W'.toAffine.Nonsingular a b := by
  ...
  exact ⟨-2 * a, -3 * b, by linear_combination (norm := ring1) -6 * h⟩
...
lemma nonsingular_of_Z_ne_zero {P : F × F × F} (hPz : P z ≠ 0) :
  ...
  simp_rw [nonsingular_of_isUnit_Z hPz.isUnit, Units.val_inv_eq_inv_val, IsUnit.unit_spec, inv_pow,
    div_eq_mul_inv]
...
lemma map_equiv_map_of_field (f : F →+* K) {P Q : F × F × F} (hP : W.Nonsingular P)
  ...
  refine ⟨fun h => ?_, map_equiv_map_of_equiv f⟩
  by_cases hz : IsUnit <| f <| P z
  · refine equiv_of_X_eq_of_Y_eq (hz.of_map f) (((isUnit_Z_of_equiv h).mp hz).of_map f) ?_ ?_ <;>
      apply f.injective <;> map_simp
    exacts [X_eq_of_equiv h, Y_eq_of_equiv h]
  · exact equiv_of_Z_eq_zero hP hQ (not_ne_iff.mp <| hz ∘ IsUnit.map f ∘ Ne.isUnit)
      (not_ne_iff.mp <| hz ∘ (isUnit_Z_of_equiv h).mpr ∘ IsUnit.map f ∘ Ne.isUnit)
...
lemma map_equation (hf : Function.Injective f) (P : R × R × R) :
  ...
  simp only [Equation, map_polynomial, eval_map, map_eq, ← map_fin3, ← eval₂_comp,
