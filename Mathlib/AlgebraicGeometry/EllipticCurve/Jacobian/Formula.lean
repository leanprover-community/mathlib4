...
@[deprecated "Unfold `addXYZ` instead" (since := "2025-05-04")]
lemma addXYZ_Z (P Q : R × R × R) : (W'.addXYZ P Q) z = addZ P Q :=
  rfl
...
@[simp]
lemma map_dblXYZ : (W'.map f).dblXYZ (f ∘ P) = f ∘ W'.dblXYZ P := by
...
@[simp]
lemma map_addXYZ : (W'.map f).addXYZ (f ∘ P) (f ∘ Q) = f ∘ W'.addXYZ P Q := by
