...
`![P 0, P 1, P 2]`, so the auxiliary lemma `fin3_def` can be used to convert between the two forms.
...
However, `u • Q` is again not definitionally equal to `![u * Q 0, u * Q 1, u * Q 2]`, so the
...
lemma fin3_def {R : Type u} (P : Fin 3 → R) : P = ![P 0, P 1, P 2] := by
  ext x; fin_cases x <;> rfl

lemma smul_fin3 {R : Type u} [CommRing R] (P : Fin 3 → R) (u : Rˣ) :
    u • P = ![u * P 0, u * P 1, u * P 2] := by
  rw [fin3_def P]
  matrix_simp
  rfl
...
lemma eval_polynomial (P : Fin 3 → R) : eval P W.polynomial =
    P 1 ^ 2 * P 2 + W.a₁ * P 0 * P 1 * P 2 + W.a₃ * P 1 * P 2 ^ 2
      - (P 0 ^ 3 + W.a₂ * P 0 ^ 2 * P 2 + W.a₄ * P 0 * P 2 ^ 2 + W.a₆ * P 2 ^ 3) := by
  rw [polynomial]
  eval_simp
...
lemma eval_polynomialY (P : Fin 3 → R) :
    eval P W.polynomialY = 2 * P 1 * P 2 + W.a₁ * P 0 * P 2 + W.a₃ * P 2 ^ 2 := by
  rw [polynomialY]
  eval_simp
...
lemma eval_polynomialZ (P : Fin 3 → R) : eval P W.polynomialZ =
    P 1 ^ 2 + W.a₁ * P 0 * P 1 + 2 * W.a₃ * P 1 * P 2
      - (W.a₂ * P 0 ^ 2 + 2 * W.a₄ * P 0 * P 2 + 3 * W.a₆ * P 2 ^ 2) := by
  rw [polynomialZ]
  eval_simp

/-- Euler's homogeneous function theorem. -/
theorem polynomial_relation (P : Fin 3 → R) : 3 * eval P W.polynomial =
    P 0 * eval P W.polynomialX + P 1 * eval P W.polynomialY + P 2 * eval P W.polynomialZ := by
...
lemma nonsingular_smul_iff (P : Fin 3 → R) (u : Rˣ) : W.nonsingular (u • P) ↔ W.nonsingular P :=
  have (u : Rˣ) {P : Fin 3 → R} (h : W.nonsingular <| u • P) :
      W.nonsingular P := by
...
lemma equiv_of_Zeq0 {P Q : Fin 3 → F} (hP : W.nonsingular P) (hQ : W.nonsingular Q) (hPz : P 2 = 0)
    (hQz : Q 2 = 0) : P ≈ Q := by
  rw [fin3_def P, hPz] at hP ⊢
  rw [fin3_def Q, hQz] at hQ ⊢
  simp [nonsingular_iff, equation_iff] at hP hQ
  simp [pow_eq_zero hP.left.symm, pow_eq_zero hQ.left.symm] at *
  exact ⟨Units.mk0 (P 1 / Q 1) <| div_ne_zero hP hQ, by simp [div_mul_cancel _ hQ]⟩

lemma equiv_zero_of_Zeq0 {P : Fin 3 → F} (h : W.nonsingular P) (hPz : P 2 = 0) : P ≈ ![0, 1, 0] :=
  equiv_of_Zeq0 h W.nonsingular_zero hPz rfl

lemma equiv_some_of_Zne0 {P : Fin 3 → F} (hPz : P 2 ≠ 0) : P ≈ ![P 0 / P 2, P 1 / P 2, 1] :=
  ⟨Units.mk0 _ hPz, by simp [← fin3_def P, mul_div_cancel' _ hPz]⟩

lemma nonsingular_iff_affine_of_Zne0 {P : Fin 3 → F} (hPz : P 2 ≠ 0) :
...
lemma nonsingular_of_affine_of_Zne0 {P : Fin 3 → F}
    (h : W.toAffine.nonsingular (P 0 / P 2) (P 1 / P 2)) (hPz : P 2 ≠ 0) : W.nonsingular P :=
  (nonsingular_iff_affine_of_Zne0 hPz).mpr h

lemma nonsingular_affine_of_Zne0 {P : Fin 3 → F} (h : W.nonsingular P) (hPz : P 2 ≠ 0) :
...
def addX'_of_Xne (P Q : Fin 3 → R) : R :=
  P 2 * Q 2 * (P 1 * Q 2 - P 2 * Q 1) ^ 2
    + W.a₁ * P 2 * Q 2 * (P 1 * Q 2 - P 2 * Q 1) * (P 0 * Q 2 - P 2 * Q 0)
    - (W.a₂ * P 2 * Q 2 + P 0 * Q 2 + P 2 * Q 0) * (P 0 * Q 2 - P 2 * Q 0) ^ 2
...
def addX'_of_Yne (P : Fin 3 → R) : R :=
  (3 * P 0 ^ 2 + 2 * W.a₂ * P 0 * P 2 + W.a₄ * P 2 ^ 2 - W.a₁ * P 1 * P 2) ^ 2
    + W.a₁ * P 2 * (3 * P 0 ^ 2 + 2 * W.a₂ * P 0 * P 2 + W.a₄ * P 2 ^ 2 - W.a₁ * P 1 * P 2)
      * (P 1 - W.negY P)
    - (W.a₂ * P 2 ^ 2 + 2 * P 0 * P 2) * (P 1 - W.negY P) ^ 2
...
def addY'_of_Xne (P Q : Fin 3 → R) : R :=
  (P 1 * Q 2 - P 2 * Q 1) * (W.addX'_of_Xne P Q - P 0 * Q 2 * (P 0 * Q 2 - P 2 * Q 0) ^ 2)
    + P 1 * Q 2 * (P 0 * Q 2 - P 2 * Q 0) ^ 3
...
def addY'_of_Yne (P : Fin 3 → R) : R :=
  (3 * P 0 ^ 2 + 2 * W.a₂ * P 0 * P 2 + W.a₄ * P 2 ^ 2 - W.a₁ * P 1 * P 2)
      * (W.addX'_of_Yne P - P 0 * P 2 * (P 1 - W.negY P) ^ 2)
...
def addX_of_Xne (P Q : Fin 3 → R) : R :=
  W.addX'_of_Xne P Q * (P 0 * Q 2 - P 2 * Q 0)
...
def addX_of_Yne (P : Fin 3 → R) : R :=
  W.addX'_of_Yne P * P 2 * (P 1 - W.negY P)
...
lemma negY_divZ {P : Fin 3 → F} (hPz : P 2 ≠ 0) :
    W.negY P / P 2 = W.toAffine.negY (P 0 / P 2) (P 1 / P 2) := by
  rw [negY, Affine.negY]
  linear_combination (norm := ring1) -W.a₃ * mul_inv_cancel hPz

lemma Yne_of_Yne {P Q : Fin 3 → F} (hP : W.nonsingular P) (hQ : W.nonsingular Q) (hPz : P 2 ≠ 0)
    (hQz : Q 2 ≠ 0) (hx : P 0 * Q 2 = P 2 * Q 0) (hy : P 1 * Q 2 ≠ P 2 * W.negY Q) :
    P 1 ≠ W.negY P := by
  simp only [mul_comm <| P 2, ne_eq, ← div_eq_div_iff hPz hQz] at hx hy
  have hy' : P 1 / P 2 = Q 1 / Q 2 := Affine.Yeq_of_Yne (nonsingular_affine_of_Zne0 hP hPz).left
...
lemma addX_div_addZ_of_Yne {P Q : Fin 3 → F} (hP : W.nonsingular P) (hQ : W.nonsingular Q)
    (hPz : P 2 ≠ 0) (hQz : Q 2 ≠ 0) (hx : P 0 * Q 2 = P 2 * Q 0) (hy : P 1 * Q 2 ≠ P 2 * W.negY Q) :
    W.addX_of_Yne P / W.addZ_of_Yne P = W.toAffine.addX (P 0 / P 2) (Q 0 / Q 2)
      (W.toAffine.slope (P 0 / P 2) (Q 0 / Q 2) (P 1 / P 2) (Q 1 / Q 2)) := by
...
lemma addX_div_addZ_of_Xne {P Q : Fin 3 → F} (hPz : P 2 ≠ 0) (hQz : Q 2 ≠ 0)
    (hx : P 0 * Q 2 ≠ P 2 * Q 0) : W.addX_of_Xne P Q / addZ_of_Xne P Q =
      W.toAffine.addX (P 0 / P 2) (Q 0 / Q 2)
        (W.toAffine.slope (P 0 / P 2) (Q 0 / Q 2) (P 1 / P 2) (Q 1 / Q 2)) := by
  rw [Affine.slope_of_Xne <| by rwa [ne_eq, div_eq_div_iff hPz hQz, mul_comm <| Q 0]]
...
lemma addY'_div_addZ_of_Yne {P Q : Fin 3 → F} (hP : W.nonsingular P) (hQ : W.nonsingular Q)
    (hPz : P 2 ≠ 0) (hQz : Q 2 ≠ 0) (hx : P 0 * Q 2 = P 2 * Q 0) (hy : P 1 * Q 2 ≠ P 2 * W.negY Q) :
    W.addY'_of_Yne P / W.addZ_of_Yne P = W.toAffine.addY' (P 0 / P 2) (Q 0 / Q 2) (P 1 / P 2)
      (W.toAffine.slope (P 0 / P 2) (Q 0 / Q 2) (P 1 / P 2) (Q 1 / Q 2)) := by
  have := sub_ne_zero_of_ne <| Yne_of_Yne hP hQ hPz hQz hx hy
...
lemma addY'_div_addZ_of_Xne {P Q : Fin 3 → F} (hPz : P 2 ≠ 0) (hQz : Q 2 ≠ 0)
    (hx : P 0 * Q 2 ≠ P 2 * Q 0) : W.addY'_of_Xne P Q / addZ_of_Xne P Q =
      W.toAffine.addY' (P 0 / P 2) (Q 0 / Q 2) (P 1 / P 2)
        (W.toAffine.slope (P 0 / P 2) (Q 0 / Q 2) (P 1 / P 2) (Q 1 / Q 2)) := by
  rw [Affine.addY', ← addX_div_addZ_of_Xne hPz hQz hx,
    Affine.slope_of_Xne <| by rwa [ne_eq, div_eq_div_iff hPz hQz, mul_comm <| Q 0]]
...
lemma addZ_ne_zero_of_Xne {P Q : Fin 3 → F} (hPz : P 2 ≠ 0) (hQz : Q 2 ≠ 0)
    (hx : P 0 * Q 2 ≠ P 2 * Q 0) : addZ_of_Xne P Q ≠ 0 :=
...
lemma addY_div_addZ_of_Yne {P Q : Fin 3 → F} (hP : W.nonsingular P) (hQ : W.nonsingular Q)
    (hPz : P 2 ≠ 0) (hQz : Q 2 ≠ 0) (hx : P 0 * Q 2 = P 2 * Q 0) (hy : P 1 * Q 2 ≠ P 2 * W.negY Q) :
    W.addY_of_Yne P / W.addZ_of_Yne P = W.toAffine.addY (P 0 / P 2) (Q 0 / Q 2) (P 1 / P 2)
      (W.toAffine.slope (P 0 / P 2) (Q 0 / Q 2) (P 1 / P 2) (Q 1 / Q 2)) := by
  rw [Affine.addY, ← addX_div_addZ_of_Yne hP hQ hPz hQz hx hy,
    ← addY'_div_addZ_of_Yne hP hQ hPz hQz hx hy]
  exact negY_divZ <| addZ_ne_zero_of_Yne hP hQ hPz hQz hx hy

lemma addY_div_addZ_of_Xne {P Q : Fin 3 → F} (hPz : P 2 ≠ 0) (hQz : Q 2 ≠ 0)
    (hx : P 0 * Q 2 ≠ P 2 * Q 0) : W.addY_of_Xne P Q / addZ_of_Xne P Q =
      W.toAffine.addY (P 0 / P 2) (Q 0 / Q 2) (P 1 / P 2)
        (W.toAffine.slope (P 0 / P 2) (Q 0 / Q 2) (P 1 / P 2) (Q 1 / Q 2)) := by
...
noncomputable def add (P Q : Fin 3 → R) : Fin 3 → R :=
  if P 2 = 0 then Q else if Q 2 = 0 then P else if P 0 * Q 2 = P 2 * Q 0 then
    if P 1 * Q 2 = P 2 * W.negY Q then ![0, 1, 0] else
      ![W.addX_of_Yne P, W.addY_of_Yne P, W.addZ_of_Yne P]
  else ![W.addX_of_Xne P Q, W.addY_of_Xne P Q, addZ_of_Xne P Q]

@[simp]
lemma add_of_Zeq0_left {P Q : Fin 3 → R} (hPz : P 2 = 0) : W.add P Q = Q :=
  if_pos hPz
...
@[simp]
lemma add_of_Zeq0_right {P Q : Fin 3 → R} (hPz : P 2 ≠ 0) (hQz : Q 2 = 0) : W.add P Q = P := by
  rw [add, if_neg hPz, if_pos hQz]

lemma add_zero_right {P : Fin 3 → R} (hPz : P 2 ≠ 0) : W.add P ![0, 1, 0] = P :=
  W.add_of_Zeq0_right hPz rfl

@[simp]
lemma add_of_Yeq {P Q : Fin 3 → R} (hPz : P 2 ≠ 0) (hQz : Q 2 ≠ 0) (hx : P 0 * Q 2 = P 2 * Q 0)
    (hy : P 1 * Q 2 = P 2 * W.negY Q) : W.add P Q = ![0, 1, 0] := by
  rw [add, if_neg hPz, if_neg hQz, if_pos hx, if_pos hy]

@[simp]
lemma add_of_Yne {P Q : Fin 3 → R} (hPz : P 2 ≠ 0) (hQz : Q 2 ≠ 0) (hx : P 0 * Q 2 = P 2 * Q 0)
    (hy : P 1 * Q 2 ≠ P 2 * W.negY Q) :
    W.add P Q = ![W.addX_of_Yne P, W.addY_of_Yne P, W.addZ_of_Yne P] := by
  rw [add, if_neg hPz, if_neg hQz, if_pos hx, if_neg hy]

@[simp]
lemma add_of_Xne {P Q : Fin 3 → R} (hPz : P 2 ≠ 0) (hQz : Q 2 ≠ 0) (hx : P 0 * Q 2 ≠ P 2 * Q 0) :
...
lemma add_smul_equiv (P Q : Fin 3 → R) (u v : Rˣ) : W.add (u • P) (v • Q) ≈ W.add P Q := by
  ...
  · have huz : u * P 2 ≠ 0 := mul_ne_zero u.ne_zero hPz
    by_cases hQz : Q 2 = 0
    · rw [W.add_of_Zeq0_right hPz hQz, W.add_of_Zeq0_right huz <| by simp only [smul_fin3_ext, hQz, mul_zero]]
      exact ⟨u, rfl⟩
    · have hvz : v * Q 2 ≠ 0 := mul_ne_zero v.ne_zero hQz
      by_cases hx : P 0 * Q 2 = P 2 * Q 0
      · by_cases hy : P 1 * Q 2 = P 2 * W.negY Q
        · rw [W.add_of_Yeq huz hvz (by erw [mul_mul_mul_comm, hx, mul_mul_mul_comm]; rfl) <|
            by simp only [smul_fin3_ext, negY_smul, mul_mul_mul_comm, hy], W.add_of_Yeq hPz hQz hx hy]
        · rw [W.add_of_Yne huz hvz (by erw [mul_mul_mul_comm, hx, mul_mul_mul_comm]; rfl) <|
            by simpa only [smul_fin3_ext, negY_smul, mul_mul_mul_comm]
              using hy ∘ mul_left_cancel₀ huv, addX_of_Yne_smul, addY_of_Yne_smul, addZ_of_Yne_smul,
            W.add_of_Yne hPz hQz hx hy]
          exact ⟨u ^ 6, by simp only [smul_fin3, ← Units.val_pow_eq_pow_val]; rfl⟩
      · rw [W.add_of_Xne huz hvz <|
          by simpa only [smul_fin3_ext, mul_mul_mul_comm] using hx ∘ mul_left_cancel₀ huv,
...
@[simp]
lemma add_map_of_Zeq0_left {P : Fin 3 → R} {Q : PointClass R} (hPz : P 2 = 0) :
    W.add_map ⟦P⟧ Q = Q := by
  rcases Q with ⟨Q⟩
...
@[simp]
lemma add_map_of_Zeq0_right {P Q : Fin 3 → R} (hPz : P 2 ≠ 0) (hQz : Q 2 = 0) :
    W.add_map ⟦P⟧ ⟦Q⟧ = ⟦P⟧ := by
  rw [add_map_eq, W.add_of_Zeq0_right hPz hQz]

lemma add_map_zero_right {P : Fin 3 → R} (hPz : P 2 ≠ 0) : W.add_map ⟦P⟧ ⟦![0, 1, 0]⟧ = ⟦P⟧ := by
  rw [add_map_eq, W.add_zero_right hPz]

@[simp]
lemma add_map_of_Yeq {P Q : Fin 3 → R} (hPz : P 2 ≠ 0) (hQz : Q 2 ≠ 0) (hx : P 0 * Q 2 = P 2 * Q 0)
    (hy : P 1 * Q 2 = P 2 * W.negY Q) : W.add_map ⟦P⟧ ⟦Q⟧ = ⟦![0, 1, 0]⟧ := by
  rw [add_map_eq, W.add_of_Yeq hPz hQz hx hy]

@[simp]
lemma add_map_of_Yne {P Q : Fin 3 → R} (hPz : P 2 ≠ 0) (hQz : Q 2 ≠ 0) (hx : P 0 * Q 2 = P 2 * Q 0)
    (hy : P 1 * Q 2 ≠ P 2 * W.negY Q) :
    W.add_map ⟦P⟧ ⟦Q⟧ = ⟦![W.addX_of_Yne P, W.addY_of_Yne P, W.addZ_of_Yne P]⟧ := by
  rw [add_map_eq, W.add_of_Yne hPz hQz hx hy]

@[simp]
lemma add_map_of_Xne {P Q : Fin 3 → R} (hPz : P 2 ≠ 0) (hQz : Q 2 ≠ 0)
    (hx : P 0 * Q 2 ≠ P 2 * Q 0) :
...
lemma add_map_of_Zeq0_right' {P : PointClass F} {Q : Fin 3 → F} (hP : W.nonsingular_lift P)
    (hQ : W.nonsingular Q) (hQz : Q 2 = 0) : W.add_map P ⟦Q⟧ = P := by
  rcases P with ⟨P⟩
  by_cases hPz : P 2 = 0
  ...
  · by_cases hQz : Q 2 = 0
    ...
    · by_cases hx : P 0 * Q 2 = P 2 * Q 0
      · by_cases hy : P 1 * Q 2 = P 2 * W.negY Q
        ...
        · erw [W.add_of_Yne hPz hQz hx hy,
          ...
          exact W.toAffine.nonsingular_add (nonsingular_affine_of_Zne0 hP hPz)
            ...
            fun _ => (negY_divZ hQz).symm ▸ (mul_comm (P 2) _ ▸ hy) ∘ (div_eq_div_iff hPz hQz).mp
      · erw [W.add_of_Xne hPz hQz hx,
        ...
        exact W.toAffine.nonsingular_add (nonsingular_affine_of_Zne0 hP hPz)
          (nonsingular_affine_of_Zne0 hQ hQz)
          fun h => (hx <| mul_comm (Q 0) _ ▸ (div_eq_div_iff hPz hQz).mp h).elim
...
noncomputable def toAffine {P : Fin 3 → F} (h : W.nonsingular P) : W.toAffine.Point :=
  if hPz : P 2 = 0 then 0 else Affine.Point.some <| nonsingular_affine_of_Zne0 h hPz

lemma toAffine_of_Zeq0 {P : Fin 3 → F} {h : W.nonsingular P} (hPz : P 2 = 0) : toAffine h = 0 :=
  dif_pos hPz

lemma toAffine_zero : toAffine W.nonsingular_zero = 0 :=
  toAffine_of_Zeq0 rfl

lemma toAffine_of_Zne0 {P : Fin 3 → F} {h : W.nonsingular P} (hPz : P 2 ≠ 0) :
...
lemma toAffine_add {P Q : Fin 3 → F} (hP : W.nonsingular P) (hQ : W.nonsingular Q) :
  ...
  · by_cases hQz : Q 2 = 0
    ...
    · by_cases hx : P 0 * Q 2 = P 2 * Q 0
      · have hx' : P 0 / P 2 = Q 0 / Q 2 := (div_eq_div_iff hPz hQz).mpr <| mul_comm (P 2) _ ▸ hx
        rw [toAffine_of_Zne0 hPz, toAffine_of_Zne0 hQz]
        by_cases hy : P 1 * Q 2 = P 2 * W.negY Q
        · have hy' : P 1 / P 2 = W.negY Q / Q 2 :=
            (div_eq_div_iff hPz hQz).mpr <| mul_comm (P 2) _ ▸ hy
          simp only [W.add_of_Yeq hPz hQz hx hy]
          rw [toAffine_zero, Affine.Point.some_add_some_of_Yeq hx' <| by rwa [← negY_divZ hQz]]
        · have hy' : P 1 / P 2 ≠ W.negY Q / Q 2 :=
            (mul_comm (P 2) _ ▸ hy) ∘ (div_eq_div_iff hPz hQz).mp
      ...
      · have hx' : P 0 / P 2 ≠ Q 0 / Q 2 := (mul_comm (P 2) _ ▸ hx) ∘ (div_eq_div_iff hPz hQz).mp
...
lemma toAffine_lift_of_Zeq0 {P : Fin 3 → F} {h : W.nonsingular_lift ⟦P⟧} (hPz : P 2 = 0) :
    toAffine_lift ⟨h⟩ = 0 :=
...
lemma toAffine_lift_of_Zne0 {P : Fin 3 → F} {h : W.nonsingular_lift ⟦P⟧} (hPz : P 2 ≠ 0) :
