...
@[simp]
protected lemma map_neg : (W'.map f).toProjective.neg (f ∘ P) = f ∘ W'.neg P := by
...
lemma baseChange_add [Algebra R F] [Algebra S F] [IsScalarTower R S F] [Algebra R K] [Algebra S K]
    [IsScalarTower R S K] (f : F →ₐ[S] K) {P Q : Fin 3 → F}
