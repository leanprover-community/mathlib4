...
such that `W.add P P = u • ![0, 1, 0]` where `P = W.neg P`. -/
...
specifically, the unit `u` such that `W.add P Q = u • ![0, 1, 0]` where `P x / P z = Q x / Q z` but
`P ≠ W.neg P`. -/
...
lemma baseChange_dblU [Algebra R F] [Algebra S F] [IsScalarTower R S F] {K : Type v} [Field K]
    [Algebra R K] [Algebra S K] [IsScalarTower R S K] (f : F →ₐ[S] K) (P : Fin 3 → F) :
