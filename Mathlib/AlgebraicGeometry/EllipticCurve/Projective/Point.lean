...
lemma neg_of_Z_eq_zero [NoZeroDivisors R] {P : R × R × R} (hP : W'.Equation P) (hPz : P z = 0) :
    W'.neg P = -P y • (0, 1, 0) := by
  rw [neg, X_eq_zero_of_Z_eq_zero hP hPz, negY_of_Z_eq_zero hP hPz, hPz, smul_eq, mul_zero, mul_one]
...
lemma nonsingular_neg {P : F × F × F} (hP : W.Nonsingular P) : W.Nonsingular <| W.neg P := by
  by_cases hPz : P z = 0
  · rw [neg_of_Z_eq_zero hP.left hPz, nonsingular_smul _ (isUnit_Y_of_Z_eq_zero hP hPz).neg]
    exact nonsingular_zero
  · rw [neg_of_Z_ne_zero hPz, nonsingular_smul _ <| Ne.isUnit hPz]
    exact nonsingular_neg_of_Z_ne_zero hP hPz

lemma addZ_neg (P : R × R × R) : W'.addZ P (W'.neg P) = 0 := by
  rw [addZ, neg, negY]
  ring1

lemma addX_neg (P : R × R × R) : W'.addX P (W'.neg P) = 0 := by
  rw [addX, neg, negY]
  ring1

lemma negAddY_neg {P : R × R × R} (hP : W'.Equation P) : W'.negAddY P (W'.neg P) = W'.dblZ P := by
  linear_combination (norm := (rw [negAddY, neg, dblZ, negY]; ring1))
    -3 * (P y - W'.negY P) * (equation_iff _).mp hP

lemma addY_neg {P : R × R × R} (hP : W'.Equation P) : W'.addY P (W'.neg P) = -W'.dblZ P := by
  rw [addY, addX_neg, negAddY_neg hP, addZ_neg, negY, mul_zero, sub_zero, mul_zero, sub_zero]

lemma addXYZ_neg {P : R × R × R} (hP : W'.Equation P) :
    W'.addXYZ P (W'.neg P) = -W'.dblZ P • (0, 1, 0) := by
  rw [addXYZ, addX_neg, addY_neg hP, addZ_neg, smul_eq, mul_zero, mul_one]
...
lemma negMap_of_Z_eq_zero {P : F × F × F} (hP : W.Nonsingular P) (hPz : P z = 0) :
    W.negMap ⟦P⟧ = ⟦(0, 1, 0)⟧ := by
  rw [negMap_eq, neg_of_Z_eq_zero hP.left hPz, mk_smul _ (isUnit_Y_of_Z_eq_zero hP hPz).neg]
...
lemma add_of_Z_eq_zero_left [NoZeroDivisors R] {P Q : R × R × R} (hP : W'.Equation P)
...
lemma add_of_Z_eq_zero_right [NoZeroDivisors R] {P Q : R × R × R} (hQ : W'.Equation Q)
...
lemma nonsingular_add {P Q : F × F × F} (hP : W.Nonsingular P) (hQ : W.Nonsingular Q) :
  ...
  · by_cases hQz : Q z = 0
    · rw [add_of_Z_eq_zero hP hQ hPz hQz,
        nonsingular_smul _ <| (isUnit_Y_of_Z_eq_zero hP hPz).pow 4]
      exact nonsingular_zero
...
lemma addMap_of_Z_eq_zero_left {P : F × F × F} {Q : PointClass F} (hP : W.Nonsingular P)
  ...
  · rw [addMap_eq, add_of_Z_eq_zero hP hQ hPz hQz,
      mk_smul _ <| (isUnit_Y_of_Z_eq_zero hP hPz).pow 4, Quotient.eq]
    exact Setoid.symm <| equiv_zero_of_Z_eq_zero hQ hQz
  · rw [addMap_eq, add_of_Z_eq_zero_left hP.left hPz hQz,
      mk_smul _ <| ((isUnit_Y_of_Z_eq_zero hP hPz).pow 2).mul <| Ne.isUnit hQz]
...
lemma addMap_of_Z_eq_zero_right {P : PointClass F} {Q : F × F × F} (hP : W.NonsingularLift P)
  ...
  · rw [addMap_eq, add_of_Z_eq_zero_right hQ.left hPz hQz,
      mk_smul _ (((isUnit_Y_of_Z_eq_zero hQ hQz).pow 2).mul <| Ne.isUnit hPz).neg]
...
lemma zero_point : (0 : W'.Point).point = ⟦(0, 1, 0)⟧ :=
  rfl
...
lemma toAffine_some {X Y : F} (h : W.Nonsingular (X, Y, 1)) :
  ...
  simp_rw [toAffine_of_Z_ne_zero h one_ne_zero, div_one]
...
lemma toAffine_smul (P : F × F × F) {u : F} (hu : IsUnit u) :
  ...
  · by_cases hPz : P z = 0
    ...
    · rw [toAffine_of_Z_ne_zero ((nonsingular_smul P hu).mpr hP) <| mul_ne_zero hu.ne_zero hPz,
      ...
      simp_rw [mul_div_mul_left _ _ hu.ne_zero, and_self]
...
abbrev Affine.Point.toProjective {W : Affine R} (P : W.Point) :
