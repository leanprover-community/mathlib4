...
def Cover.toPresieveOver {X : Over S} (𝒰 : Cover.{u} (precoverage P) X.left)
    {g : ∀ j, 𝒰.X j ⟶ S} [𝒰.Over S g] : Presieve X :=
  Presieve.ofArrows (fun i ↦ (𝒰.X i).asOver S) (fun i ↦ (𝒰.f i).asOver S)

/-- The presieve defined by a `P`-cover of `S`-schemes with `Q`. -/
def Cover.toPresieveOverProp {X : Q.Over ⊤ S} (𝒰 : Cover.{u} (precoverage P) X.left)
    {g : ∀ j, 𝒰.X j ⟶ S} [𝒰.Over S g] (h : ∀ j, Q (𝒰.X j ↘ S)) : Presieve X :=
...
lemma Cover.overEquiv_generate_toPresieveOver_eq_ofArrows {X : Over S}
    (𝒰 : Cover.{u} (precoverage P) X.left) {g : ∀ j, 𝒰.X j ⟶ S}
    [𝒰.Over S g] : Sieve.overEquiv X (Sieve.generate 𝒰.toPresieveOver) =
  ...
  · rintro ⟨U, h, g, ⟨k⟩, hcomp⟩
    refine ⟨(𝒰.X k).asOver S, Over.homMk h (by simp [← hcomp, Cover.Over.comp_over]),
      (𝒰.f k).asOver S, ⟨k⟩, ?_⟩
    ext : 1
    simpa

lemma Cover.toPresieveOver_le_arrows_iff {X : Over S} (R : Sieve X)
    (𝒰 : Cover.{u} (precoverage P) X.left) {g : ∀ j, 𝒰.X j ⟶ S} [𝒰.Over S g] :
...
def overPretopology : Pretopology (Over S) where
  coverings Y := {R | ∃ (𝒰 : Cover.{u} (precoverage P) Y.left) (g : ∀ j, 𝒰.X j ⟶ S)
    (_ : 𝒰.Over S g), R = 𝒰.toPresieveOver}
  has_isos {X Y} f _ := ⟨coverOfIsIso f.left, _, inferInstance, (Presieve.ofArrows_pUnit f).symm⟩
  pullbacks := by
    rintro Y X f _ ⟨𝒰, g, h, rfl⟩
    refine ⟨𝒰.pullbackCoverOver' S f.left, _, inferInstance, ?_⟩
    simpa [Cover.toPresieveOver] using!
      (Presieve.ofArrows_pullback f (fun i ↦ (𝒰.X i).asOver S) (fun i ↦ (𝒰.f i).asOver S)).symm
  transitive := by
    rintro X _ T ⟨𝒰, g, h, rfl⟩ H
    choose V gV hOver hV using H
    refine ⟨𝒰.bind (fun j => V ((𝒰.f j).asOver S) ⟨j⟩), _, inferInstance, ?_⟩
    convert!
...
lemma overGrothendieckTopology_eq_toGrothendieck_overPretopology :
  ...
  · intro hR
    ...
    letI : 𝒰.Over S (fun i ↦ 𝒰.f i ≫ X.hom) := { comp_over := fun i ↦ rfl }
    use 𝒰.toPresieveOver, ⟨𝒰, fun i ↦ 𝒰.f i ≫ X.hom, inferInstance, rfl⟩
    rwa [Cover.toPresieveOver_le_arrows_iff]
  · rintro ⟨T, ⟨𝒰, g, h, rfl⟩, hT⟩
...
lemma mem_overGrothendieckTopology (X : Over S) (R : Sieve X) :
    R ∈ S.overGrothendieckTopology P X ↔
      ∃ (𝒰 : Cover.{u} (precoverage P) X.left) (g : ∀ j, 𝒰.X j ⟶ S) (_ : 𝒰.Over S g),
        𝒰.toPresieveOver ≤ R.arrows := by
  rw [overGrothendieckTopology_eq_toGrothendieck_overPretopology]
  constructor
  · rintro ⟨T, ⟨𝒰, g, h, rfl⟩, hle⟩
    use 𝒰, g, h
  · rintro ⟨𝒰, g, h𝒰, hle⟩
    exact ⟨𝒰.toPresieveOver, ⟨𝒰, g, h𝒰, rfl⟩, hle⟩
...
def smallPretopology : Pretopology (Q.Over ⊤ S) where
  coverings Y :=
    {R | ∃ (𝒰 : Cover.{u} (precoverage P) Y.left) (g : ∀ j, 𝒰.X j ⟶ S) (_ : 𝒰.Over S g)
      (h : ∀ j : 𝒰.I₀, Q (𝒰.X j ↘ S)), R = 𝒰.toPresieveOverProp h}
  has_isos {X Y} f := ⟨coverOfIsIso f.left, _, inferInstance, fun _ ↦ Y.prop,
    (Presieve.ofArrows_pUnit _).symm⟩
  pullbacks := by
    rintro Y X f _ ⟨𝒰, g, h, p, rfl⟩
    refine ⟨𝒰.pullbackCoverOverProp' S f.left (Q := Q) Y.prop X.prop p, _, inferInstance, ?_, ?_⟩
    · intro j
  ...
  transitive := by
    rintro X _ T ⟨𝒰, g, h, p, rfl⟩ H
    choose V gV hOver pV hV using H
    let 𝒱j (j : 𝒰.I₀) : (Cover (precoverage P) ((𝒰.X j).asOverProp S (p j)).left) :=
      V ((𝒰.f j).asOverProp S) ⟨j⟩
    refine ⟨𝒰.bind (fun j ↦ 𝒱j j), _, inferInstance, fun j ↦ pV _ _ _, ?_⟩
    convert!
...
lemma mem_toGrothendieck_smallPretopology (X : Q.Over ⊤ S) (R : Sieve X) :
  ...
  · choose Y f y hf hP hy using h
    ...
    letI : 𝒰.Over S (fun i ↦ (Y i).hom) :=
      { comp_over := fun i ↦ by exact comp_over (f i).left S }
    refine ⟨𝒰.toPresieveOverProp fun i ↦ MorphismProperty.Comma.prop _, ?_, ?_⟩
    · use 𝒰, fun i ↦ (Y i).hom, inferInstance, fun i ↦ MorphismProperty.Comma.prop _
    · rintro - - ⟨i⟩
      exact hf i
...
lemma mem_smallGrothendieckTopology [P.HasOfPostcompProperty P] (X : P.Over ⊤ S) (R : Sieve X) :
    R ∈ S.smallGrothendieckTopology P X ↔
      ∃ (𝒰 : Cover.{u} (precoverage P) X.left) (g : ∀ j, 𝒰.X j ⟶ S) (_ : 𝒰.Over S g)
        (h : ∀ j, P (𝒰.X j ↘ S)), 𝒰.toPresieveOverProp h ≤ R.arrows := by
  rw [smallGrothendieckTopology_eq_toGrothendieck_smallPretopology _ le_rfl]
  constructor
  · rintro ⟨T, ⟨𝒰, g, h, p, rfl⟩, hle⟩
    use 𝒰, g, h, p
  · rintro ⟨𝒰, g, h𝒰, p, hle⟩
    exact ⟨𝒰.toPresieveOverProp p, ⟨𝒰, g, h𝒰, p, rfl⟩, hle⟩
