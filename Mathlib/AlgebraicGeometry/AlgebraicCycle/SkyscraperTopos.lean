...
variable {C : Type*} [Category* C]
  {J : GrothendieckTopology C} (Φ : J.Point) {A : Type*} [Category* A] [HasProducts A]

#check (Φ.skyscraperSheafAdjunction (A := RingCat)).unit.app




/-instance (R : Cᵒᵖ ⥤ RingCat)
    (M : ModuleCat ((Point.presheafFiber Φ).obj R).carrier) (U : Cᵒᵖ) :
    Module (R.obj U)
    ((Φ.skyscraperPresheaf ((forget₂ (ModuleCat ↑(Φ.presheafFiber.obj R)) Ab).obj M)).obj U) where
      smul r f := by

        sorry--fun x ↦ sorry --Φ.toPresheafFiber U
        --sorry
      mul_smul := sorry
      one_smul := sorry
      smul_zero := sorry
...
--variable (M : ModuleCat ((Point.sheafFiber Φ).obj R).carrier)
#check Φ.skyscraperSheaf
...
instance (R : Cᵒᵖ ⥤ RingCat)
    (M : ModuleCat ((Point.presheafFiber Φ).obj R).carrier) (U : Cᵒᵖ) :
    Module (R.obj U)
    ((Φ.skyscraperPresheaf ((forget₂ (ModuleCat ↑(Φ.presheafFiber.obj R)) Ab).obj M)).obj U) := by
  let f := (((Φ.skyscraperPresheafAdjunction (A := RingCat)).unit.app R).app U).hom
  --simp at f
  simp [skyscraperPresheaf]-/
  sorry
  --#check Module.compHom --M.1 (((Φ.skyscraperPresheafAdjunction (A := RingCat)).unit.app R).app U).hom
  --simp [skyscraperPresheaf, skyscraperPresheafFunctor]

  --sorry


def fds (R : Cᵒᵖ ⥤ RingCat) (M : Type*) [AddCommGroup M]
    [Module ((Point.presheafFiber Φ).obj R).carrier M] (U : Cᵒᵖ) : Module (R.obj U) M := by
  let m : Module (↑(((Φ.presheafFiber ⋙ Φ.skyscraperPresheafFunctor).obj R).obj U)) M := by

    sorry
  exact Module.compHom M (((Φ.skyscraperPresheafAdjunction (A := RingCat)).unit.app R).app U).hom

/-
instance (R : Cᵒᵖ ⥤ RingCat) (M : Type*) [AddCommGroup M]
    [Module ((Point.presheafFiber Φ).obj R).carrier M]
    (U : Cᵒᵖ) : Module ↑(R.obj U) ((Φ.skyscraperPresheaf (AddCommGrpCat.of M)).obj U) := by
  --apply (config := {allowSynthFailures := true})
  let m : Module (↑(((Φ.presheafFiber ⋙ Φ.skyscraperPresheafFunctor).obj R).obj U)) M := by sorry
  let k := Module.compHom M (((Φ.skyscraperPresheafAdjunction (A := RingCat)).unit.app R).app U).hom
  simp at k
  /-simp [skyscraperPresheaf, skyscraperPresheafFunctor, Functor.flip]
  erw [Functor.comp_obj]
  rw [piFunctor_obj_obj]-/
  /-
  Right, I guess it really comes back to whether we have a map from R.obj U to Φ.presheafFiber.obj R
  This should really only exist if we know that Φ(U) is inhabited, so this all seems a bit useless
  -/
  --sorry-/

noncomputable
def skyscraperPresheafOfModules  (R : Cᵒᵖ ⥤ RingCat) (M : Type*) [AddCommGroup M]
    [Module ((Point.presheafFiber Φ).obj R).carrier M] : PresheafOfModules R := by
    --apply PresheafOfModules.ofPresheaf (Φ.skyscraperPresheaf (.of M : Ab))

    sorry
    --(M : ModuleCat ((Point.presheafFiber Φ).obj R).carrier) : PresheafOfModules R := by
  --#check Φ.skyscraperPresheaf ((forget₂ (ModuleCat _) Ab).obj M)
  --apply PresheafOfModules.ofPresheaf (Φ.skyscraperPresheaf ((forget₂ (ModuleCat _) Ab).obj M))

  --apply (config := {allowSynthFailures := true}) PresheafOfModules.ofPresheaf <| Φ.skyscraperPresheaf ((forget₂ (ModuleCat _) Ab).obj M)


  --sorry

variable (R : Sheaf J RingCat)
/-
What is the argument that the skyscraper sheaf has a module structure over
-/
def skyscraperSheafOfModulesFunctor :
    ModuleCat ((Point.sheafFiber Φ).obj R).carrier ⥤ SheafOfModules R := by sorry

/--
The underlying sheaf of Abelian groups of the skyscraper sheaf as a sheaf of modules is the
skyscraper sheaf
-/
...
#check Point.sheafFiber
#check Φ.toPresheafFiber
/-
We wish to define the skyscraper sheaf at a "point" in a topos,
-/



--#check GrothendieckTopology.Point.skyscraperSheaf Φ M
