...
`finite_of_finite_type_of_isJacobsonRing`) makes it a finite extension.
...
* `riemann_roch_of_structureSheaf_of_locallyOfFiniteType`: the Riemann–Roch theorem
  `riemann_roch_of_structureSheaf` with the residue-field hypothesis discharged: for a curve
  locally of finite type over `k`, only the finiteness and eventual vanishing of `Hⁿ(X, 𝒪ₓ)`
...
/--
**Riemann–Roch for a curve locally of finite type over `k`.** The residue-field hypothesis of
`riemann_roch_of_structureSheaf` is discharged by Zariski's lemma, so only the cohomological
finiteness and vanishing of the single sheaf `𝒪ₓ` remain: for `X` proper over `k` these are
the (not yet formalized) finiteness theorem for coherent cohomology and Grothendieck vanishing.
-/
theorem riemann_roch_of_structureSheaf_of_locallyOfFiniteType {N : ℕ}
  ...
  riemann_roch_of_structureSheaf k
