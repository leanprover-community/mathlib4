...
@[simp]
lemma zeroLocus_pow (I : Ideal R) {n : ℕ} (hn : n ≠ 0) :
...
lemma localization_away_comap_range (S : Type v) [CommSemiring S] [Algebra R S] (r : R)
...
lemma localization_away_openEmbedding (S : Type v) [CommSemiring S] [Algebra R S] (r : R)
...
lemma isCompact_basicOpen (f : R) : IsCompact (basicOpen f : Set (PrimeSpectrum R)) := by
...
lemma isLocalRingHom_iff_comap_closedPoint {S : Type v} [CommSemiring S] [LocalRing S]
...
@[simp]
lemma comap_closedPoint {S : Type v} [CommSemiring S] [LocalRing S] (f : R →+* S)
...
@[simp]
lemma PrimeSpectrum.comap_residue (T : Type u) [CommRing T] [LocalRing T]
