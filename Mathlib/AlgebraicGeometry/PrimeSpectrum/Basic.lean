...
lemma punit (x : PrimeSpectrum PUnit) : False :=
...
@[simp]
lemma primeSpectrumProd_symm_inl_asIdeal (x : PrimeSpectrum R) :
...
@[simp]
lemma primeSpectrumProd_symm_inr_asIdeal (x : PrimeSpectrum S) :
...
@[simp]
lemma mem_zeroLocus (x : PrimeSpectrum R) (s : Set R) : x ∈ zeroLocus s ↔ s ⊆ x.asIdeal :=
  Iff.rfl
#align prime_spectrum.mem_zero_locus PrimeSpectrum.mem_zeroLocus

@[simp]
lemma zeroLocus_span (s : Set R) : zeroLocus (Ideal.span s : Set R) = zeroLocus s := by
  ext x
...
lemma coe_vanishingIdeal (t : Set (PrimeSpectrum R)) :
...
lemma mem_vanishingIdeal (t : Set (PrimeSpectrum R)) (f : R) :
...
@[simp]
lemma vanishingIdeal_singleton (x : PrimeSpectrum R) :
...
lemma subset_zeroLocus_iff_le_vanishingIdeal (t : Set (PrimeSpectrum R)) (I : Ideal R) :
...
lemma subset_zeroLocus_iff_subset_vanishingIdeal (t : Set (PrimeSpectrum R)) (s : Set R) :
...
lemma subset_vanishingIdeal_zeroLocus (s : Set R) : s ⊆ vanishingIdeal (zeroLocus s) :=
  (gc_set R).le_u_l s
#align prime_spectrum.subset_vanishing_ideal_zero_locus PrimeSpectrum.subset_vanishingIdeal_zeroLocus

lemma le_vanishingIdeal_zeroLocus (I : Ideal R) : I ≤ vanishingIdeal (zeroLocus I) :=
  (gc R).le_u_l I
#align prime_spectrum.le_vanishing_ideal_zero_locus PrimeSpectrum.le_vanishingIdeal_zeroLocus

@[simp]
lemma vanishingIdeal_zeroLocus_eq_radical (I : Ideal R) :
...
@[simp]
lemma zeroLocus_radical (I : Ideal R) : zeroLocus (I.radical : Set R) = zeroLocus I :=
...
lemma subset_zeroLocus_vanishingIdeal (t : Set (PrimeSpectrum R)) :
...
lemma zeroLocus_anti_mono {s t : Set R} (h : s ⊆ t) : zeroLocus t ⊆ zeroLocus s :=
  (gc_set R).monotone_l h
#align prime_spectrum.zero_locus_anti_mono PrimeSpectrum.zeroLocus_anti_mono

lemma zeroLocus_anti_mono_ideal {s t : Ideal R} (h : s ≤ t) :
...
lemma zeroLocus_subset_zeroLocus_singleton_iff (f g : R) :
...
@[simp]
lemma zeroLocus_singleton_one : zeroLocus ({1} : Set R) = ∅ :=
...
lemma zeroLocus_empty_iff_eq_top {I : Ideal R} : zeroLocus (I : Set R) = ∅ ↔ I = ⊤ := by
  constructor
  · contrapose!
    intro h
...
@[simp]
lemma zeroLocus_univ : zeroLocus (Set.univ : Set R) = ∅ :=
  zeroLocus_empty_of_one_mem (Set.mem_univ 1)
#align prime_spectrum.zero_locus_univ PrimeSpectrum.zeroLocus_univ

lemma vanishingIdeal_eq_top_iff {s : Set (PrimeSpectrum R)} : vanishingIdeal s = ⊤ ↔ s = ∅ := by
...
lemma zeroLocus_union (s s' : Set R) : zeroLocus (s ∪ s') = zeroLocus s ∩ zeroLocus s' :=
  (gc_set R).l_sup
#align prime_spectrum.zero_locus_union PrimeSpectrum.zeroLocus_union

lemma vanishingIdeal_union (t t' : Set (PrimeSpectrum R)) :
...
lemma vanishingIdeal_iUnion {ι : Sort*} (t : ι → Set (PrimeSpectrum R)) :
...
lemma union_zeroLocus (s s' : Set R) :
...
@[simp]
lemma zeroLocus_pow (I : Ideal R) {n : ℕ} (hn : 0 < n) :
...
@[simp]
lemma zeroLocus_singleton_pow (f : R) (n : ℕ) (hn : 0 < n) :
...
lemma sup_vanishingIdeal_le (t t' : Set (PrimeSpectrum R)) :
...
lemma mem_compl_zeroLocus_iff_not_mem {f : R} {I : PrimeSpectrum R} :
...
lemma isOpen_iff (U : Set (PrimeSpectrum R)) : IsOpen U ↔ ∃ s, Uᶜ = zeroLocus s := by
  simp only [@eq_comm _ Uᶜ]; rfl
#align prime_spectrum.is_open_iff PrimeSpectrum.isOpen_iff

lemma isClosed_iff_zeroLocus (Z : Set (PrimeSpectrum R)) : IsClosed Z ↔ ∃ s, Z = zeroLocus s := by
...
lemma isClosed_iff_zeroLocus_ideal (Z : Set (PrimeSpectrum R)) :
...
lemma isClosed_iff_zeroLocus_radical_ideal (Z : Set (PrimeSpectrum R)) :
...
lemma isClosed_zeroLocus (s : Set R) : IsClosed (zeroLocus s) := by
  rw [isClosed_iff_zeroLocus]
  exact ⟨s, rfl⟩
#align prime_spectrum.is_closed_zero_locus PrimeSpectrum.isClosed_zeroLocus

lemma isClosed_singleton_iff_isMaximal (x : PrimeSpectrum R) :
...
lemma zeroLocus_vanishingIdeal_eq_closure (t : Set (PrimeSpectrum R)) :
...
lemma vanishingIdeal_closure (t : Set (PrimeSpectrum R)) :
...
lemma closure_singleton (x) : closure ({x} : Set (PrimeSpectrum R)) = zeroLocus x.asIdeal := by
...
lemma isRadical_vanishingIdeal (s : Set (PrimeSpectrum R)) : (vanishingIdeal s).IsRadical := by
...
lemma vanishingIdeal_anti_mono_iff {s t : Set (PrimeSpectrum R)} (ht : IsClosed t) :
...
lemma vanishingIdeal_strict_anti_mono_iff {s t : Set (PrimeSpectrum R)} (hs : IsClosed s)
...
lemma t1Space_iff_isField [IsDomain R] : T1Space (PrimeSpectrum R) ↔ IsField R := by
  refine' ⟨_, fun h => _⟩
  · intro h
...
lemma isIrreducible_zeroLocus_iff_of_radical (I : Ideal R) (hI : I.IsRadical) :
...
lemma isIrreducible_zeroLocus_iff (I : Ideal R) :
...
lemma isIrreducible_iff_vanishingIdeal_isPrime {s : Set (PrimeSpectrum R)} :
...
lemma preimage_comap_zeroLocus_aux (f : R →+* S) (s : Set R) :
...
@[simp]
lemma comap_asIdeal (y : PrimeSpectrum S) : (comap f y).asIdeal = Ideal.comap f y.asIdeal :=
  rfl
#align prime_spectrum.comap_as_ideal PrimeSpectrum.comap_asIdeal

@[simp]
lemma comap_id : comap (RingHom.id R) = ContinuousMap.id _ := by
  ext
  rfl
#align prime_spectrum.comap_id PrimeSpectrum.comap_id

@[simp]
lemma comap_comp (f : R →+* S) (g : S →+* S') : comap (g.comp f) = (comap f).comp (comap g) :=
  rfl
#align prime_spectrum.comap_comp PrimeSpectrum.comap_comp

lemma comap_comp_apply (f : R →+* S) (g : S →+* S') (x : PrimeSpectrum S') :
...
@[simp]
lemma preimage_comap_zeroLocus (s : Set R) : comap f ⁻¹' zeroLocus s = zeroLocus (f '' s) :=
...
lemma comap_singleton_isClosed_of_surjective (f : R →+* S) (hf : Function.Surjective f)
...
lemma comap_singleton_isClosed_of_isIntegral (f : R →+* S) (hf : f.IsIntegral)
    (x : PrimeSpectrum S) (hx : IsClosed ({x} : Set (PrimeSpectrum S))) :
    IsClosed ({comap f x} : Set (PrimeSpectrum R)) :=
...
lemma localization_comap_inducing [Algebra R S] (M : Submonoid R) [IsLocalization M S] :
...
lemma localization_comap_injective [Algebra R S] (M : Submonoid R) [IsLocalization M S] :
...
lemma localization_comap_embedding [Algebra R S] (M : Submonoid R) [IsLocalization M S] :
...
lemma localization_comap_range [Algebra R S] (M : Submonoid R) [IsLocalization M S] :
...
lemma comap_inducing_of_surjective (hf : Surjective f) : Inducing (comap f) where
  induced := by
...
lemma image_comap_zeroLocus_eq_zeroLocus_comap (hf : Surjective f) (I : Ideal S) :
...
lemma isClosed_range_comap_of_surjective (hf : Surjective f) :
...
lemma closedEmbedding_comap_of_surjective (hf : Surjective f) : ClosedEmbedding (comap f) :=
...
@[simp]
lemma mem_basicOpen (f : R) (x : PrimeSpectrum R) : x ∈ basicOpen f ↔ f ∉ x.asIdeal :=
  Iff.rfl
#align prime_spectrum.mem_basic_open PrimeSpectrum.mem_basicOpen

lemma isOpen_basicOpen {a : R} : IsOpen (basicOpen a : Set (PrimeSpectrum R)) :=
  (basicOpen a).isOpen
#align prime_spectrum.is_open_basic_open PrimeSpectrum.isOpen_basicOpen

@[simp]
lemma basicOpen_eq_zeroLocus_compl (r : R) :
...
lemma basicOpen_mul (f g : R) : basicOpen (f * g) = basicOpen f ⊓ basicOpen g :=
...
lemma basicOpen_mul_le_left (f g : R) : basicOpen (f * g) ≤ basicOpen f := by
  rw [basicOpen_mul f g]
  exact inf_le_left
#align prime_spectrum.basic_open_mul_le_left PrimeSpectrum.basicOpen_mul_le_left

lemma basicOpen_mul_le_right (f g : R) : basicOpen (f * g) ≤ basicOpen g := by
  rw [basicOpen_mul f g]
  exact inf_le_right
#align prime_spectrum.basic_open_mul_le_right PrimeSpectrum.basicOpen_mul_le_right

@[simp]
lemma basicOpen_pow (f : R) (n : ℕ) (hn : 0 < n) : basicOpen (f ^ n) = basicOpen f :=
...
lemma isTopologicalBasis_basic_opens :
...
lemma isBasis_basic_opens : TopologicalSpace.Opens.IsBasis (Set.range (@basicOpen R _)) := by
...
lemma isCompact_basicOpen (f : R) : IsCompact (basicOpen f : Set (PrimeSpectrum R)) :=
...
@[simp]
lemma basicOpen_eq_bot_iff (f : R) : basicOpen f = ⊥ ↔ IsNilpotent f := by
...
lemma localization_away_comap_range (S : Type v) [CommRing S] [Algebra R S] (r : R)
...
lemma localization_away_openEmbedding (S : Type v) [CommRing S] [Algebra R S] (r : R)
...
@[simp]
lemma asIdeal_le_asIdeal (x y : PrimeSpectrum R) : x.asIdeal ≤ y.asIdeal ↔ x ≤ y :=
  Iff.rfl
#align prime_spectrum.as_ideal_le_as_ideal PrimeSpectrum.asIdeal_le_asIdeal

@[simp]
lemma asIdeal_lt_asIdeal (x y : PrimeSpectrum R) : x.asIdeal < y.asIdeal ↔ x < y :=
  Iff.rfl
#align prime_spectrum.as_ideal_lt_as_ideal PrimeSpectrum.asIdeal_lt_asIdeal

lemma le_iff_mem_closure (x y : PrimeSpectrum R) :
...
lemma le_iff_specializes (x y : PrimeSpectrum R) : x ≤ y ↔ x ⤳ y :=
...
lemma isLocalRingHom_iff_comap_closedPoint {S : Type v} [CommRing S] [LocalRing S] (f : R →+* S) :
...
@[simp]
lemma comap_closedPoint {S : Type v} [CommRing S] [LocalRing S] (f : R →+* S) [IsLocalRingHom f] :
...
lemma specializes_closedPoint (x : PrimeSpectrum R) : x ⤳ closedPoint R :=
...
lemma closedPoint_mem_iff (U : TopologicalSpace.Opens <| PrimeSpectrum R) :
...
@[simp]
lemma PrimeSpectrum.comap_residue (x : PrimeSpectrum (ResidueField R)) :
