...
theorem row_eq_prod {μ : YoungDiagram} {i : ℕ} :
    μ.row i = ({i} : Finset ℕ) ×ˢ Finset.range (μ.rowLen i) := by
  ext ⟨a, b⟩
...
theorem col_eq_prod {μ : YoungDiagram} {j : ℕ} :
    μ.col j = Finset.range (μ.colLen j) ×ˢ ({j} : Finset ℕ) := by
  ext ⟨a, b⟩
