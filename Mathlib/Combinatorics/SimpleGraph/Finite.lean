...
omit [Fintype <| G.neighborSet v] in
variable {G v} in
theorem degree_pos_iff_nonempty [Finite <| G.neighborSet v] :
    0 < G.degree v ↔ (G.neighborSet v).Nonempty :=
  G.degree_pos_iff_exists_adj v

omit [Fintype <| G.neighborSet v] in
variable {G v} in
theorem Adj.degree_pos_left [Finite <| G.neighborSet v] {w : V} (h : G.Adj v w) : 0 < G.degree v :=
  G.degree_pos_iff_nonempty.mpr ⟨_, h⟩

omit [Fintype <| G.neighborSet v] in
variable {G v} in
theorem Adj.degree_pos_right [Finite <| G.neighborSet v] {w : V} (h : G.Adj w v) : 0 < G.degree v :=
...
theorem IsRegularOfDegree.bot : (⊥ : SimpleGraph V).IsRegularOfDegree 0 := by
  intro v
  have : degree ⊥ v = edegree ⊥ v := ENat.coe_toNat <| Set.encard_ne_top_iff.mpr Set.finite_empty
  simp [← this]
