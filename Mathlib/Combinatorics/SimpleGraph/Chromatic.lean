...
lemma not_col_lt_chrom (hr : r < G.chromaticNumber)  : ¬ G.Colorable r :=
by
  contrapose hr
  rw [not_not] at hr
  rw [not_lt]
...
lemma not_col_chrom_succ (hcrom: G.chromaticNumber = r + 1)  : ¬ G.Colorable r :=
by
...
/-- If C is an (r + 1)-coloring of G that never uses color i then we can define an r-coloring of G -/
def Coloring.not_surj_toColoring {i : Fin (r + 1)} (C : G.Coloring (Fin (r + 1)))
(hc: ∀ a, C a ≠ i) : G.Coloring (Fin r) :=
⟨λ v ↦ if hcv : C v = Fin.last r then ⟨i, Fin.val_lt_last (hcv ▸ (hc v)).symm⟩
       else ⟨C v, Fin.val_lt_last hcv⟩,
  by
    intro _ b had; dsimp; split_ifs with h1 h2 h3
...
lemma Coloring.colorClasses_adj  {i j : Fin r}  (C : G.Coloring (Fin r)) (hcrom: G.chromaticNumber = r)
(hij: i ≠ j) :  ∃ v w, C v = i ∧ C w = j ∧ G.Adj v w  := by
  cases r with
  | zero => exact i.elim0
  | succ r =>
