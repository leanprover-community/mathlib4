...
lemma not_col_lt_chromaticNumber (hr : r < G.chromaticNumber) : ¬ G.Colorable r := by
  contrapose! hr
  exact Colorable.chromaticNumber_le hr


/-- If χ(G) = r + 1 then G is not r-colorable -/
lemma not_col_chromaticNumber_eq_succ (hcrom: G.chromaticNumber = r + 1) : ¬ G.Colorable r := by
  apply not_col_lt_chromaticNumber
  rw [hcrom]
...
lemma Coloring.colorClasses_adj  {i j : Fin r} (C : G.Coloring (Fin r))
  ...
  | succ r =>
    by_contra! hc
    exact not_col_chromaticNumber_eq_succ hcrom ⟨_,(C.not_colorClasses_adj_toColoring hc hij).valid⟩
