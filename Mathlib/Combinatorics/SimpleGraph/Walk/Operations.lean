...
protected def copy {u v u' v'} (p : Walk G u v) (hu : u = u') (hv : v = v') : Walk G u' v' :=
  hu ▸ hv ▸ p

@[simp]
theorem copy_rfl_rfl {u v} (p : Walk G u v) : p.copy rfl rfl = p := rfl

@[simp]
theorem copy_copy {u v u' v' u'' v''} (p : Walk G u v)
...
@[simp]
theorem copy_nil {u u'} (hu : u = u') : (Walk.nil : Walk G u u).copy hu hu = nil := by
  subst_vars
  rfl
...
protected def reverseAux {u v w : V} : Walk G u v → Walk G u w → Walk G v w
  | nil, q => q
  | cons h p, q => Walk.reverseAux p (cons h.inv q)
...
@[simp]
theorem nil_append {u v : V} (p : Walk G u v) : nil.append p = p :=
  rfl

@[simp]
theorem append_nil {u v : V} (p : Walk G u v) : p.append nil = p := by
  induction p <;> simp [*]

theorem append_assoc {u v w x : V} (p : Walk G u v) (q : Walk G v w) (r : Walk G w x) :
    p.append (q.append r) = (p.append q).append r := by
  induction p <;> simp [*]

@[simp]
theorem append_copy_copy {u v w u' v' w'} (p : Walk G u v) (q : Walk G v w)
...
protected theorem append_reverseAux {u v w x : V}
    (p : Walk G u v) (q : Walk G v w) (r : Walk G u x) :
    (p.append q).reverseAux r = q.reverseAux (p.reverseAux r) := by
  induction p with
  | nil => rfl
  | cons h _ ih => exact ih q (cons h.inv r)

@[simp]
protected theorem reverseAux_append {u v w x : V}
    (p : Walk G u v) (q : Walk G u w) (r : Walk G w x) :
    (p.reverseAux q).append r = p.reverseAux (q.append r) := by
  induction p with
  | nil => rfl
  | cons h _ ih => simp [ih (cons h.inv q)]

protected theorem reverseAux_eq_reverse_append {u v w : V} (p : Walk G u v) (q : Walk G u w) :
...
@[simp]
theorem reverse_copy {u v u' v'} (p : Walk G u v) (hu : u = u') (hv : v = v') :
    (p.copy hu hv).reverse = p.reverse.copy hv hu := by
  subst_vars
  rfl

@[simp]
theorem reverse_append {u v w : V} (p : Walk G u v) (q : Walk G v w) :
...
@[simp]
theorem reverse_reverse {u v : V} (p : Walk G u v) : p.reverse.reverse = p := by
  induction p with
  | nil => rfl
  | cons _ _ ih => simp [ih]

theorem reverse_surjective {u v : V} : Function.Surjective (reverse : Walk G u v → _) :=
  RightInverse.surjective reverse_reverse

theorem reverse_injective {u v : V} : Function.Injective (reverse : Walk G u v → _) :=
  RightInverse.injective reverse_reverse

theorem reverse_bijective {u v : V} : Function.Bijective (reverse : Walk G u v → _) :=
  ⟨reverse_injective, reverse_surjective⟩

@[simp]
theorem length_copy {u v u' v'} (p : Walk G u v) (hu : u = u') (hv : v = v') :
    (p.copy hu hv).length = p.length := by
  subst_vars
  rfl

@[simp]
theorem length_append {u v w : V} (p : Walk G u v) (q : Walk G v w) :
...
@[simp]
protected theorem length_reverseAux {u v w : V} (p : Walk G u v) (q : Walk G u w) :
...
@[simp]
theorem length_reverse {u v : V} (p : Walk G u v) : p.reverse.length = p.length := by simp [reverse]

theorem getVert_append {u v w : V} (p : Walk G u v) (q : Walk G v w) (i : ℕ) :
    (p.append q).getVert i = if i < p.length then p.getVert i else q.getVert (i - p.length) := by
  induction p generalizing i <;> cases i <;> simp [*]

theorem getVert_reverse {u v : V} (p : Walk G u v) (i : ℕ) :
...
def concatRecAux {u v : V} : (p : Walk G u v) → motive v u p.reverse
  | nil => Hnil
  | cons h p => reverse_cons h p ▸ Hconcat p.reverse h.inv (concatRecAux p)
...
@[elab_as_elim]
def concatRec {u v : V} (p : Walk G u v) : motive u v p :=
...
theorem concatRec_concat {u v w : V} (p : Walk G u v) (h : step G v w) :
  ...
  trans concatRecAux @Hnil @Hconcat (cons h.inv p.reverse)
  · congr
    simp
...
@[simp]
theorem support_copy {u v u' v'} (p : Walk G u v) (hu : u = u') (hv : v = v') :
    (p.copy hu hv).support = p.support := by
  subst_vars
  rfl

theorem support_append {u v w : V} (p : Walk G u v) (p' : Walk G v w) :
    (p.append p').support = p.support ++ p'.support.tail := by
  induction p <;> cases p' <;> simp [*]

@[simp]
theorem support_reverse {u v : V} (p : Walk G u v) : p.reverse.support = p.support.reverse := by
  induction p <;> simp [support_append, *]

theorem support_append_eq_support_dropLast_append {u v w : V} (p : Walk G u v) (p' : Walk G v w) :
    (p.append p').support = p.support.dropLast ++ p'.support := by
  induction p <;> simp_all [List.dropLast_cons_of_ne_nil]

theorem tail_support_append {u v w : V} (p : Walk G u v) (p' : Walk G v w) :
...
@[simp]
theorem mem_tail_support_append_iff {t u v w : V} (p : Walk G u v) (p' : Walk G v w) :
    t ∈ (p.append p').support.tail ↔ t ∈ p.support.tail ∨ t ∈ p'.support.tail := by
  rw [tail_support_append, List.mem_append]

@[simp, nolint unusedHavesSuffices]
theorem mem_support_append_iff {t u v w : V} (p : Walk G u v) (p' : Walk G v w) :
...
theorem subset_support_append_left {V : Type u} {G : SimpleGraph V} {u v w : V}
    (p : Walk G u v) (q : Walk G v w) : p.support ⊆ (p.append q).support := by
  simp [support_append]

@[simp]
theorem subset_support_append_right {V : Type u} {G : SimpleGraph V} {u v w : V}
    (p : Walk G u v) (q : Walk G v w) : q.support ⊆ (p.append q).support := by
  intro
  simp +contextual [mem_support_append_iff]

theorem coe_support_append {u v w : V} (p : Walk G u v) (p' : Walk G v w) :
    ((p.append p').support : Multiset V) = {u} + p.support.tail + p'.support.tail := by
  rw [support_append, ← Multiset.coe_add, coe_support]

theorem coe_support_append' [DecidableEq V] {u v w : V} (p : Walk G u v) (p' : Walk G v w) :
...
@[simp]
theorem ofSupport_support {u v : V} (p : Walk G u v) :
  ...
  | cons (v := u') h₁ (.cons (v := v') h₂ p) =>
    ...
    simp_all only [support_cons, List.head_cons, List.getLast_cons_cons, ofSupport_cons_cons,
      cons_copy]
    congr 2
...
@[simp]
theorem darts_copy {u v u' v'} (p : Walk G u v) (hu : u = u') (hv : v = v') :
    (p.copy hu hv).darts = p.darts := by
  subst_vars
  rfl

@[simp]
theorem darts_append {u v w : V} (p : Walk G u v) (p' : Walk G v w) :
    (p.append p').darts = p.darts ++ p'.darts := by
  induction p <;> simp [*]

@[simp]
theorem darts_reverse {u v : V} (p : Walk G u v) :
    p.reverse.darts = (p.darts.map dartSymm).reverse := by
  induction p <;> simp [*]

theorem mem_darts_reverse {u v : V} {d : GraphLike.darts G} {p : Walk G u v} :
    d ∈ p.reverse.darts ↔ dartSymm d ∈ p.darts := by simp

@[simp]
theorem ofDarts_darts {u v : V} {p : Walk G u v} (hp : ¬p.Nil) :
...
@[simp]
theorem edges_copy {u v u' v'} (p : Walk G u v) (hu : u = u') (hv : v = v') :
    (p.copy hu hv).edges = p.edges := by
  subst_vars
  rfl

@[simp]
theorem edges_append {u v w : V} (p : Walk G u v) (p' : Walk G v w) :
...
theorem edges_reverse {u v : V} (p : Walk G u v) : p.reverse.edges = p.edges.reverse := by
  simp [edges, - List.map_subtype]

/-- This is dual statement for `dart_src_mem_support_of_mem_darts`. -/
theorem dart_snd_mem_support_of_mem_darts {u v : V} (p : Walk G u v) {d : GraphLike.darts G}
    (h : d ∈ p.darts) : d.val.snd ∈ p.support := by
  simpa using p.reverse.dart_src_mem_support_of_mem_darts
    (by simp [h] : dartSymm d ∈ p.reverse.darts)
...
theorem snd_mem_support_of_mem_edges {t u v w : V} (p : Walk G v w) (he : s(t, u) ∈ p.edges) :
    u ∈ p.support :=
  p.fst_mem_support_of_mem_edges (Sym2.eq_swap ▸ he)

theorem mem_support_of_mem_edges {u v w : V} {e : Sym2 V} {p : Walk G u v} (he : e ∈ p.edges)
    (hv : w ∈ e) : w ∈ p.support :=
  hv.elim fun _ heq ↦ p.fst_mem_support_of_mem_edges <| heq ▸ he

theorem edges_nodup_of_support_nodup {u v : V} {p : Walk G u v} (h : p.support.Nodup) :
    p.edges.Nodup := by
  induction p with
  | nil => simp
  | cons _ p' ih =>
    simp only [support_cons, List.nodup_cons, edges_cons] at h ⊢
    exact ⟨fun h' ↦ by simpa [h.1] using fst_mem_support_of_mem_edges p' h', ih h.2⟩

theorem nodup_tail_support_reverse {u : V} {p : Walk G u u} :
...
@[simp]
theorem edgeSet_copy {u v u' v'} (p : Walk G u v) (hu : u = u') (hv : v = v') :
    (p.copy hu hv).edgeSet = p.edgeSet := by ext; simp

@[simp]
lemma nil_append_iff {p : Walk G u v} {q : Walk G v w} : (p.append q).Nil ↔ p.Nil ∧ q.Nil := by
  cases p <;> cases q <;> simp

lemma Nil.append {p : Walk G u v} {q : Walk G v w} (hp : p.Nil) (hq : q.Nil) :
    (p.append q).Nil := by
  simp [hp, hq]

@[simp]
lemma nil_reverse {p : Walk G v w} : p.reverse.Nil ↔ p.Nil := by
  cases p <;> simp

/-- The walk obtained by removing the first `n` darts of a walk. -/
def drop {u v : V} (p : Walk G u v) (n : ℕ) : Walk G (p.getVert n) v :=
  match p, n with
  | .nil, _ => .nil
...
@[simp]
lemma drop_length (p : Walk G u v) (n : ℕ) : (p.drop n).length = p.length - n := by
  induction p generalizing n <;> cases n <;> simp [*, drop]

set_option backward.isDefEq.respectTransparency false in
@[simp]
lemma drop_getVert (p : Walk G u v) (n m : ℕ) : (p.drop n).getVert m = p.getVert (n + m) := by
  induction p generalizing n <;> cases n <;> simp [*, drop, add_right_comm]

set_option backward.isDefEq.respectTransparency false in
lemma drop_add_heq (p : Walk G u v) (n m : ℕ) : p.drop (n + m) ≍ (p.drop n).drop m := by
  rw [add_comm]
  induction p generalizing n <;> cases n <;> simp [*, drop]

lemma drop_add_eq (p : Walk G u v) (n m : ℕ) :
...
lemma darts_drop (p : Walk G u v) (n : ℕ) : (p.drop n).darts = p.darts.drop n := by
  induction p generalizing n <;> cases n <;> simp [*, drop]

lemma edges_drop (p : Walk G u v) (n : ℕ) : (p.drop n).edges = p.edges.drop n := by
  induction p generalizing n <;> cases n <;> simp [*, drop]

/-- The walk obtained by taking the first `n` darts of a walk. -/
def take {u v : V} (p : Walk G u v) (n : ℕ) : Walk G u (p.getVert n) :=
  match p, n with
  | .nil, _ => .nil
...
@[simp]
lemma take_zero (p : Walk G u v) : p.take 0 = nil.copy rfl p.getVert_zero.symm := by
  cases p <;> simp [take]

@[simp]
lemma take_length (p : Walk G u v) (n : ℕ) : (p.take n).length = n ⊓ p.length := by
  induction p generalizing n <;> cases n <;> simp [*, take]

set_option backward.isDefEq.respectTransparency false in
@[simp]
lemma take_getVert (p : Walk G u v) (n m : ℕ) : (p.take n).getVert m = p.getVert (n ⊓ m) := by
  induction p generalizing n m <;> cases n <;> cases m <;> simp [*, take]

set_option backward.isDefEq.respectTransparency false in
lemma take_add_heq (p : Walk G u v) (n m : ℕ) :
    p.take (n + m) ≍ (p.take n).append ((p.drop n).take m) := by
  rw [add_comm]
  induction p generalizing n <;> cases n <;> simp [take, drop]
  grind [drop_getVert]

lemma take_add_eq (p : Walk G u v) (n m : ℕ) :
    p.take (n + m) = ((p.take n).append ((p.drop n).take m)).copy rfl (drop_getVert ..) :=
  eq_of_heq <| take_add_heq .. |>.trans <| by simp [Walk.copy]

set_option backward.isDefEq.respectTransparency false in
lemma nil_take_iff (p : Walk G u v) (n : ℕ) : (p.take n).Nil ↔ p.Nil ∨ n = 0 := by
  cases p <;> cases n <;> simp [take]

lemma take_support_eq_support_take_succ {u v} (p : Walk G u v) (n : ℕ) :
    (p.take n).support = p.support.take (n + 1) := by
  induction p generalizing n <;> cases n <;> simp [*, take]

lemma take_take (p : Walk G u v) (n m : ℕ) :
...
lemma take_of_length_le {u v n} {p : Walk G u v} (h : p.length ≤ n) :
...
lemma darts_take (p : Walk G u v) (n : ℕ) : (p.take n).darts = p.darts.take n := by
  induction p generalizing n <;> cases n <;> simp [*, take]

lemma edges_take (p : Walk G u v) (n : ℕ) : (p.take n).edges = p.edges.take n := by
  induction p generalizing n <;> cases n <;> simp [*, take]
...
@[simp]
lemma snd_reverse (p : Walk G u v) : p.reverse.snd = p.penultimate := by
  simpa using getVert_reverse p 1

@[simp]
lemma penultimate_reverse (p : Walk G u v) : p.reverse.penultimate = p.snd := by
  cases p <;> simp [snd, getVert_append]

/-- The walk obtained by removing the first dart of a walk. A nil walk stays nil. -/
def tail (p : Walk G u v) : Walk G (p.snd) v := p.drop 1

@[simp]
lemma drop_zero {u v} (p : Walk G u v) :
    p.drop 0 = p.copy (getVert_zero p).symm rfl := by
  cases p <;> simp [Walk.drop]

lemma nil_drop_of_length_le {u v n} {p : Walk G u v} (h : p.length ≤ n) :
    (p.drop n).Nil := by
  rw [nil_iff_length_eq, drop_length, Nat.sub_eq_zero_of_le h]

lemma drop_support_eq_support_drop_min {u v} (p : Walk G u v) (n : ℕ) :
    (p.drop n).support = p.support.drop (n ⊓ p.length) := by
  induction p generalizing n <;> cases n <;> simp [*, drop]

@[simp]
theorem append_take_drop_eq (p : Walk G u v) (n : ℕ) : (p.take n).append (p.drop n) = p := by
  apply ext_support
...
lemma cons_tail_eq (p : Walk G u v) (hp : ¬ p.Nil) :
    cons (p.step_snd hp) p.tail = p := by
  cases p <;> simp at hp ⊢
...
lemma concat_dropLast {p : Walk G u v} (hp : step G p.penultimate v) :
  ...
  induction p with
  | nil =>
    simp only [getVert_nil] at hp
    exact (hp.ne rfl).elim
  | cons hadj p hind =>
    cases p with
    | nil =>
      obtain rfl := Subsingleton.elim hadj hp
      rfl
    | _ => simp [hind]

@[simp] lemma cons_support_tail {p : Walk G u v} (hp : ¬p.Nil) :
    u :: p.tail.support = p.support := by
  change (cons (p.step_snd hp) p.tail).support = _
  rw [cons_tail_eq _ hp]

theorem support_dropLast_concat {p : Walk G u v} (hp : ¬p.Nil) :
    p.dropLast.support ++ [v] = p.support := by
  rw [← support_concat _ <| step_penultimate hp, concat_dropLast]

@[simp]
theorem support_dropLast {p : Walk G u v} (hp : ¬p.Nil) :
    p.dropLast.support = p.support.dropLast := by
  simp [← support_dropLast_concat hp]

@[simp] lemma length_tail_add_one {p : Walk G u v} (hp : ¬ p.Nil) :
    p.tail.length + 1 = p.length := by
  rw [← length_cons (p.step_snd hp), cons_tail_eq _ hp]

lemma length_dropLast_add_one {p : Walk G u v} (hp : ¬p.Nil) :
    p.dropLast.length + 1 = p.length := by
  rw [← length_concat _ <| p.step_penultimate hp, concat_dropLast]

@[simp]
lemma length_dropLast (p : Walk G u v) : p.dropLast.length = p.length - 1 := by
  cases p <;> simp [← length_dropLast_add_one not_nil_cons]

protected lemma Nil.tail {p : Walk G v w} (hp : p.Nil) : p.tail.Nil := by
  cases p <;> simp at hp ⊢

lemma not_nil_of_tail_not_nil {p : Walk G v w} (hp : ¬ p.tail.Nil) : ¬ p.Nil := mt Nil.tail hp

protected lemma Nil.dropLast {p : Walk G v w} (hp : p.Nil) : p.dropLast.Nil := by
  cases p <;> simp at hp ⊢

@[simp] lemma nil_copy {u' v' : V} {p : Walk G u v} (hu : u = u') (hv : v = v') :
    (p.copy hu hv).Nil = p.Nil := by
  subst_vars
  rfl

lemma Nil.eq_copy_nil {p : Walk G u v} (h : p.Nil) :
    p = Walk.nil.copy rfl h.eq := by
  have := h.eq
  subst this
  simp [nil_iff_eq_nil.mp h]

lemma drop_of_length_le {u v n} {p : Walk G u v} (h : p.length ≤ n) :
    p.drop n = nil.copy rfl (p.getVert_of_length_le h) :=
  (nil_drop_of_length_le h).eq_copy_nil

lemma support_tail_of_not_nil (p : Walk G u v) (hp : ¬ p.Nil) :
    p.tail.support = p.support.tail := by
  rw [← cons_support_tail hp, List.tail_cons]

@[simp] lemma getVert_copy {u v w x : V} (p : Walk G u v) (i : ℕ) (h : u = w) (h' : v = x) :
    (p.copy h h').getVert i = p.getVert i := by
  subst_vars
  rfl

@[simp] lemma getVert_tail {u v n} (p : Walk G u v) :
    p.tail.getVert n = p.getVert (n + 1) := by
  cases p <;> simp

lemma getVert_mem_tail_support {u v : V} {p : Walk G u v} (hp : ¬p.Nil) :
...
lemma ext_getVert_le_length {u v} {p q : Walk G u v} (hl : p.length = q.length)
...
lemma ext_getVert {u v} {p q : Walk G u v} (h : ∀ k, p.getVert k = q.getVert k) :
    p = q := by
