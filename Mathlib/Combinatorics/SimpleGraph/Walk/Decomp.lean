...
theorem support_dropUntil_subset_support {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
...
theorem darts_takeUntil_subset_darts {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
...
theorem darts_dropUntil_subset_darts {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
...
theorem edges_takeUntil_subset_edges {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
    (p.takeUntil u h).edges ⊆ p.edges :=
  List.map_subset _ (p.darts_takeUntil_subset_darts h)

@[deprecated (since := "2026-05-25")] alias edges_takeUntil_subset := edges_takeUntil_subset_edges

theorem edges_dropUntil_subset_edges {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
    (p.dropUntil u h).edges ⊆ p.edges :=
  List.map_subset _ (p.darts_dropUntil_subset_darts h)
...
lemma notMem_support_takeUntil_support_takeUntil_subset_support {p : G.Walk u v} {w x : V}
    (h : x ≠ w) (hw : w ∈ p.support) (hx : x ∈ (p.takeUntil w hw).support) :
