...
the containment relation `G ⊑ H`. Types are guest-first (`Copy G H`, `UnlabeledCopy G H`,
`IsContained G H`, …) and operations are host-first via dot notation (`H.copyCount G`,
`H.unlabeledCopyCount G`, …).
...
* `SimpleGraph.UnlabeledCopy G H` is the type of unlabeled copies of `G` in `H`, implemented as
  the subtype of `SimpleGraph.Subgraph`s of `H` isomorphic to `G`.
...
* `SimpleGraph.unlabeledCopyCount H G` is the number of unlabeled copies of `G` in `H`, i.e. the
  number of `SimpleGraph.Subgraph`s of `H` isomorphic to `G`.
...
* Make `copyCount`/`unlabeledCopyCount` computable (not necessarily efficiently).
...
lemma unlabeledCopyCount_le_copyCount [Finite V] [Finite W] :
  ...
  classical
  letI : Fintype {f : G →g H // Injective f} := Fintype.ofFinite _
  rw [unlabeledCopyCount_eq_card_image_copyToSubgraph, copyCount, Nat.card_eq_fintype_card]
  exact card_image_le
