...
lemma isClique_iff_induce_eq : G.IsClique s ↔ G.induce s = ⊤ := by
  rw [isClique_iff]
  constructor
  · intro h
    ext ⟨v, hv⟩ ⟨w, hw⟩
...
lemma IsClique.mono (h : G ≤ H) : G.IsClique s → H.IsClique s := by
  simp_rw [isClique_iff]
  exact Set.Pairwise.mono' h
#align simple_graph.is_clique.mono SimpleGraph.IsClique.mono

lemma IsClique.subset (h : t ⊆ s) : G.IsClique s → G.IsClique t := by
  simp_rw [isClique_iff]
  exact Set.Pairwise.mono h
...
lemma cliqueFree_of_card_lt [Fintype α] (hc : card α < n) : G.CliqueFree n := by
  by_contra h
...
lemma cliqueFree_completeMultipartiteGraph {ι : Type*} [Fintype ι] (V : ι → Type*)
...
lemma mem_cliqueFinset_iff : s ∈ G.cliqueFinset n ↔ G.IsNClique n s :=
