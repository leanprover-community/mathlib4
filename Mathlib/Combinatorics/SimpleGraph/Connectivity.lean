...
@[simp]
lemma copy_rfl_rfl {u v} (p : G.Walk u v) : p.copy rfl rfl = p := rfl
#align simple_graph.walk.copy_rfl_rfl SimpleGraph.Walk.copy_rfl_rfl

@[simp]
lemma copy_copy {u v u' v' u'' v''} (p : G.Walk u v)
...
@[simp]
lemma copy_nil {u u'} (hu : u = u') : (Walk.nil : G.Walk u u).copy hu hu = Walk.nil := by
  subst_vars
  rfl
...
lemma exists_eq_cons_of_ne {u v : V} (hne : u ≠ v) :
...
@[simp]
lemma getVert_zero {u v} (w : G.Walk u v) : w.getVert 0 = u := by cases w <;> rfl
#align simple_graph.walk.get_vert_zero SimpleGraph.Walk.getVert_zero

lemma getVert_of_length_le {u v} (w : G.Walk u v) {i : ℕ} (hi : w.length ≤ i) :
    w.getVert i = v := by
...
@[simp]
lemma getVert_length {u v} (w : G.Walk u v) : w.getVert w.length = v :=
  w.getVert_of_length_le rfl.le
#align simple_graph.walk.get_vert_length SimpleGraph.Walk.getVert_length

lemma adj_getVert_succ {u v} (w : G.Walk u v) {i : ℕ} (hi : i < w.length) :
...
@[simp]
lemma cons_append {u v w x : V} (h : G.Adj u v) (p : G.Walk v w) (q : G.Walk w x) :
    (cons h p).append q = cons h (p.append q) := rfl
#align simple_graph.walk.cons_append SimpleGraph.Walk.cons_append

@[simp]
lemma cons_nil_append {u v w : V} (h : G.Adj u v) (p : G.Walk v w) :
    (cons h nil).append p = cons h p := rfl
#align simple_graph.walk.cons_nil_append SimpleGraph.Walk.cons_nil_append

@[simp]
lemma append_nil {u v : V} (p : G.Walk u v) : p.append nil = p := by
  induction p with
  | nil => rfl
...
@[simp]
lemma nil_append {u v : V} (p : G.Walk u v) : nil.append p = p :=
  rfl
#align simple_graph.walk.nil_append SimpleGraph.Walk.nil_append

lemma append_assoc {u v w x : V} (p : G.Walk u v) (q : G.Walk v w) (r : G.Walk w x) :
...
@[simp]
lemma append_copy_copy {u v w u' v' w'} (p : G.Walk u v) (q : G.Walk v w)
...
@[simp]
lemma concat_cons {u v w x : V} (h : G.Adj u v) (p : G.Walk v w) (h' : G.Adj w x) :
...
@[simp]
lemma reverse_nil {u : V} : (nil : G.Walk u u).reverse = nil := rfl
#align simple_graph.walk.reverse_nil SimpleGraph.Walk.reverse_nil

lemma reverse_singleton {u v : V} (h : G.Adj u v) : (cons h nil).reverse = cons (G.symm h) nil :=
  rfl
#align simple_graph.walk.reverse_singleton SimpleGraph.Walk.reverse_singleton

@[simp]
lemma cons_reverseAux {u v w x : V} (p : G.Walk u v) (q : G.Walk w x) (h : G.Adj w u) :
...
@[simp]
protected lemma append_reverseAux {u v w x : V}
...
@[simp]
protected lemma reverseAux_append {u v w x : V}
...
protected lemma reverseAux_eq_reverse_append {u v w : V} (p : G.Walk u v) (q : G.Walk u w) :
...
@[simp]
lemma reverse_cons {u v w : V} (h : G.Adj u v) (p : G.Walk v w) :
...
@[simp]
lemma reverse_copy {u v u' v'} (p : G.Walk u v) (hu : u = u') (hv : v = v') :
...
@[simp]
lemma reverse_append {u v w : V} (p : G.Walk u v) (q : G.Walk v w) :
...
@[simp]
lemma reverse_concat {u v w : V} (p : G.Walk u v) (h : G.Adj v w) :
...
@[simp]
lemma reverse_reverse {u v : V} (p : G.Walk u v) : p.reverse.reverse = p := by
  induction p with
  | nil => rfl
  | cons _ _ ih => simp [ih]
...
@[simp]
lemma length_nil {u : V} : (nil : G.Walk u u).length = 0 := rfl
#align simple_graph.walk.length_nil SimpleGraph.Walk.length_nil

@[simp]
lemma length_cons {u v w : V} (h : G.Adj u v) (p : G.Walk v w) :
    (cons h p).length = p.length + 1 := rfl
#align simple_graph.walk.length_cons SimpleGraph.Walk.length_cons

@[simp]
lemma length_copy {u v u' v'} (p : G.Walk u v) (hu : u = u') (hv : v = v') :
...
@[simp]
lemma length_concat {u v w : V} (p : G.Walk u v) (h : G.Adj v w) :
    (p.concat h).length = p.length + 1 := length_append _ _
#align simple_graph.walk.length_concat SimpleGraph.Walk.length_concat

@[simp]
protected lemma length_reverseAux {u v w : V} (p : G.Walk u v) (q : G.Walk u w) :
...
@[simp]
lemma length_reverse {u v : V} (p : G.Walk u v) : p.reverse.length = p.length := by simp [reverse]
#align simple_graph.walk.length_reverse SimpleGraph.Walk.length_reverse

lemma eq_of_length_eq_zero {u v : V} : ∀ {p : G.Walk u v}, p.length = 0 → u = v
  | nil, _ => rfl
#align simple_graph.walk.eq_of_length_eq_zero SimpleGraph.Walk.eq_of_length_eq_zero

@[simp]
lemma exists_length_eq_zero_iff {u v : V} : (∃ p : G.Walk u v, p.length = 0) ↔ u = v := by
  constructor
  · rintro ⟨p, hp⟩
...
@[simp]
lemma length_eq_zero_iff {u : V} {p : G.Walk u u} : p.length = 0 ↔ p = nil := by cases p <;> simp
#align simple_graph.walk.length_eq_zero_iff SimpleGraph.Walk.length_eq_zero_iff

lemma getVert_append {u v w : V} (p : G.Walk u v) (q : G.Walk v w) (i : ℕ) :
...
lemma getVert_reverse {u v : V} (p : G.Walk u v) (i : ℕ) :
...
@[simp]
lemma concatRec_concat {u v w : V} (p : G.Walk u v) (h : G.Adj v w) :
...
lemma concat_ne_nil {u v : V} (p : G.Walk u v) (h : G.Adj v u) : p.concat h ≠ nil := by
  cases p <;> simp [concat]
#align simple_graph.walk.concat_ne_nil SimpleGraph.Walk.concat_ne_nil

lemma concat_inj {u v v' w : V} {p : G.Walk u v} {h : G.Adj v w} {p' : G.Walk u v'}
...
@[simp]
lemma support_nil {u : V} : (nil : G.Walk u u).support = [u] := rfl
#align simple_graph.walk.support_nil SimpleGraph.Walk.support_nil

@[simp]
lemma support_cons {u v w : V} (h : G.Adj u v) (p : G.Walk v w) :
    (cons h p).support = u :: p.support := rfl
#align simple_graph.walk.support_cons SimpleGraph.Walk.support_cons

@[simp]
lemma support_concat {u v w : V} (p : G.Walk u v) (h : G.Adj v w) :
...
@[simp]
lemma support_copy {u v u' v'} (p : G.Walk u v) (hu : u = u') (hv : v = v') :
...
@[simp]
lemma support_reverse {u v : V} (p : G.Walk u v) : p.reverse.support = p.support.reverse := by
  induction p <;> simp [support_append, *]
#align simple_graph.walk.support_reverse SimpleGraph.Walk.support_reverse

@[simp]
lemma support_ne_nil {u v : V} (p : G.Walk u v) : p.support ≠ [] := by cases p <;> simp
...
lemma support_eq_cons {u v : V} (p : G.Walk u v) : p.support = u :: p.support.tail := by
  cases p <;> simp
#align simple_graph.walk.support_eq_cons SimpleGraph.Walk.support_eq_cons

@[simp]
lemma start_mem_support {u v : V} (p : G.Walk u v) : u ∈ p.support := by cases p <;> simp
#align simple_graph.walk.start_mem_support SimpleGraph.Walk.start_mem_support

@[simp]
lemma end_mem_support {u v : V} (p : G.Walk u v) : v ∈ p.support := by induction p <;> simp [*]
#align simple_graph.walk.end_mem_support SimpleGraph.Walk.end_mem_support

@[simp]
lemma support_nonempty {u v : V} (p : G.Walk u v) : { w | w ∈ p.support }.Nonempty :=
  ⟨u, by simp⟩
#align simple_graph.walk.support_nonempty SimpleGraph.Walk.support_nonempty

lemma mem_support_iff {u v w : V} (p : G.Walk u v) : w ∈ p.support ↔ w = u ∨ w ∈ p.support.tail :=
  by cases p <;> simp
...
@[simp]
lemma end_mem_tail_support_of_ne {u v : V} (h : u ≠ v) (p : G.Walk u v) : v ∈ p.support.tail := by
...
@[simp]
lemma subset_support_append_left {V : Type u} {G : SimpleGraph V} {u v w : V}
...
@[simp]
lemma subset_support_append_right {V : Type u} {G : SimpleGraph V} {u v w : V}
...
lemma coe_support {u v : V} (p : G.Walk u v) : (p.support : Multiset V) = {u} + p.support.tail :=
  by cases p <;> rfl
...
lemma coe_support_append' [DecidableEq V] {u v w : V} (p : G.Walk u v) (p' : G.Walk v w) :
...
lemma chain_adj_support {u v w : V} (h : G.Adj u v) :
...
lemma chain'_adj_support {u v : V} : ∀ (p : G.Walk u v), List.Chain' G.Adj p.support
  | nil => List.Chain.nil
  | cons h p => chain_adj_support h p
#align simple_graph.walk.chain'_adj_support SimpleGraph.Walk.chain'_adj_support

lemma chain_dartAdj_darts {d : G.Dart} {v w : V} (h : d.snd = v) (p : G.Walk v w) :
...
lemma chain'_dartAdj_darts {u v : V} : ∀ (p : G.Walk u v), List.Chain' G.DartAdj p.darts
  | nil => trivial
...
lemma cons_map_snd_darts {u v : V} (p : G.Walk u v) : (u :: p.darts.map (·.snd)) = p.support := by
  induction p <;> simp! [*]
#align simple_graph.walk.cons_map_snd_darts SimpleGraph.Walk.cons_map_snd_darts

lemma map_snd_darts {u v : V} (p : G.Walk u v) : p.darts.map (·.snd) = p.support.tail := by
...
lemma dart_snd_mem_support_of_mem_darts {u v : V} (p : G.Walk u v) {d : G.Dart}
...
lemma fst_mem_support_of_mem_edges {t u v w : V} (p : G.Walk v w) (he : s(t, u) ∈ p.edges) :
    t ∈ p.support := by
...
lemma snd_mem_support_of_mem_edges {t u v w : V} (p : G.Walk v w) (he : s(t, u) ∈ p.edges) :
    u ∈ p.support := by
...
@[simp]
lemma isCycle_copy {u u'} (p : G.Walk u u) (hu : u = u') :
...
lemma IsTrail.reverse {u v : V} (p : G.Walk u v) (h : p.IsTrail) : p.reverse.IsTrail := by
  simpa [isTrail_def] using h
#align simple_graph.walk.is_trail.reverse SimpleGraph.Walk.IsTrail.reverse

@[simp]
lemma reverse_isTrail_iff {u v : V} (p : G.Walk u v) : p.reverse.IsTrail ↔ p.IsTrail := by
  constructor <;>
    · intro h
      convert h.reverse _
...
lemma IsTrail.of_append_left {u v w : V} {p : G.Walk u v} {q : G.Walk v w}
...
lemma IsTrail.of_append_right {u v w : V} {p : G.Walk u v} {q : G.Walk v w}
...
lemma IsTrail.count_edges_le_one [DecidableEq V] {u v : V} {p : G.Walk u v} (h : p.IsTrail)
...
lemma IsTrail.count_edges_eq_one [DecidableEq V] {u v : V} {p : G.Walk u v} (h : p.IsTrail)
...
@[simp]
lemma isPath_iff_eq_nil {u : V} (p : G.Walk u u) : p.IsPath ↔ p = nil := by
  cases p <;> simp [IsPath.nil]
#align simple_graph.walk.is_path_iff_eq_nil SimpleGraph.Walk.isPath_iff_eq_nil

lemma IsPath.reverse {u v : V} {p : G.Walk u v} (h : p.IsPath) : p.reverse.IsPath := by
  simpa [isPath_def] using h
#align simple_graph.walk.is_path.reverse SimpleGraph.Walk.IsPath.reverse

@[simp]
lemma isPath_reverse_iff {u v : V} (p : G.Walk u v) : p.reverse.IsPath ↔ p.IsPath := by
...
lemma IsPath.of_append_left {u v w : V} {p : G.Walk u v} {q : G.Walk v w} :
...
lemma IsPath.of_append_right {u v w : V} {p : G.Walk u v} {q : G.Walk v w}
...
@[simp]
lemma IsCycle.not_of_nil {u : V} : ¬(nil : G.Walk u u).IsCycle := fun h => h.ne_nil rfl
...
lemma cons_isCycle_iff {u v : V} (p : G.Walk v u) (h : G.Adj u v) :
...
lemma IsPath.length_lt [Fintype V] {u v : V} {p : G.Walk u v} (hp : p.IsPath) :
...
lemma mem_support_iff_exists_append {V : Type u} {G : SimpleGraph V} {u v w : V}
...
@[simp]
lemma count_support_takeUntil_eq_one {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
...
lemma count_edges_takeUntil_le_one {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) (x : V) :
...
@[simp]
lemma takeUntil_copy {u v w v' w'} (p : G.Walk v w) (hv : v = v') (hw : w = w')
...
@[simp]
lemma dropUntil_copy {u v w v' w'} (p : G.Walk v w) (hv : v = v') (hw : w = w')
    (h : u ∈ (p.copy hv hw).support) :
...
lemma support_takeUntil_subset {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
...
lemma support_dropUntil_subset {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
...
lemma darts_takeUntil_subset {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
...
lemma darts_dropUntil_subset {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
...
lemma edges_takeUntil_subset {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
...
lemma edges_dropUntil_subset {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
...
lemma length_takeUntil_le {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
...
lemma length_dropUntil_le {u v w : V} (p : G.Walk v w) (h : u ∈ p.support) :
...
protected lemma IsTrail.takeUntil {u v w : V} {p : G.Walk v w} (hc : p.IsTrail)
...
protected lemma IsTrail.dropUntil {u v w : V} {p : G.Walk v w} (hc : p.IsTrail)
...
protected lemma IsPath.takeUntil {u v w : V} {p : G.Walk v w} (hc : p.IsPath)
...
protected lemma IsPath.dropUntil {u v w : V} {p : G.Walk v w} (hc : p.IsPath)
...
protected lemma IsTrail.rotate {u v : V} {c : G.Walk v v} (hc : c.IsTrail) (h : u ∈ c.support) :
...
protected lemma IsCircuit.rotate {u v : V} {c : G.Walk v v} (hc : c.IsCircuit)
...
protected lemma IsCycle.rotate {u v : V} {c : G.Walk v v} (hc : c.IsCycle) (h : u ∈ c.support) :
...
@[simp]
protected lemma isPath {u v : V} (p : G.Path u v) : (p : G.Walk u v).IsPath := p.property
#align simple_graph.path.is_path SimpleGraph.Path.isPath

@[simp]
protected lemma isTrail {u v : V} (p : G.Path u v) : (p : G.Walk u v).IsTrail :=
  p.property.isTrail
...
lemma mk'_mem_edges_singleton {u v : V} (h : G.Adj u v) :
...
lemma count_support_eq_one [DecidableEq V] {u v w : V} {p : G.Path u v}
...
lemma count_edges_eq_one [DecidableEq V] {u v : V} {p : G.Path u v} (e : Sym2 V)
...
@[simp]
lemma bypass_copy {u v u' v'} (p : G.Walk u v) (hu : u = u') (hv : v = v') :
    (p.copy hu hv).bypass = p.bypass.copy hu hv := by
  subst_vars
  rfl
#align simple_graph.walk.bypass_copy SimpleGraph.Walk.bypass_copy

lemma bypass_isPath {u v : V} (p : G.Walk u v) : p.bypass.IsPath := by
  induction p with
  | nil => simp!
  | cons _ p' ih =>
    simp only [bypass]
...
lemma length_bypass_le {u v : V} (p : G.Walk u v) : p.bypass.length ≤ p.length := by
  induction p with
  | nil => rfl
  | cons _ _ ih =>
    simp only [bypass]
...
lemma support_bypass_subset {u v : V} (p : G.Walk u v) : p.bypass.support ⊆ p.support := by
  induction p with
  | nil => simp!
  | cons _ _ ih =>
    simp! only
...
lemma support_toPath_subset {u v : V} (p : G.Walk u v) :
...
lemma darts_bypass_subset {u v : V} (p : G.Walk u v) : p.bypass.darts ⊆ p.darts := by
  induction p with
  | nil => simp!
  | cons _ _ ih =>
    simp! only
...
lemma darts_toPath_subset {u v : V} (p : G.Walk u v) : (p.toPath : G.Walk u v).darts ⊆ p.darts :=
  darts_bypass_subset _
#align simple_graph.walk.darts_to_path_subset SimpleGraph.Walk.darts_toPath_subset

lemma edges_toPath_subset {u v : V} (p : G.Walk u v) : (p.toPath : G.Walk u v).edges ⊆ p.edges :=
...
@[simp]
lemma map_nil : (nil : G.Walk u u).map f = nil := rfl
#align simple_graph.walk.map_nil SimpleGraph.Walk.map_nil

@[simp]
lemma map_cons {w : V} (h : G.Adj w u) : (cons h p).map f = cons (f.map_adj h) (p.map f) := rfl
#align simple_graph.walk.map_cons SimpleGraph.Walk.map_cons

@[simp]
lemma map_copy (hu : u = u') (hv : v = v') :
...
@[simp]
lemma map_id (p : G.Walk u v) : p.map Hom.id = p := by
  induction p with
  | nil => rfl
...
@[simp]
lemma map_map : (p.map f).map f' = p.map (f'.comp f) := by
  induction p with
  | nil => rfl
  | cons _ _ ih => simp [ih]
...
@[simp]
lemma map_eq_nil_iff {p : G.Walk u u} : p.map f = nil ↔ p = nil := by cases p <;> simp
#align simple_graph.walk.map_eq_nil_iff SimpleGraph.Walk.map_eq_nil_iff

@[simp]
lemma length_map : (p.map f).length = p.length := by induction p <;> simp [*]
...
@[simp]
lemma reverse_map : (p.map f).reverse = p.reverse.map f := by induction p <;> simp [map_append, *]
#align simple_graph.walk.reverse_map SimpleGraph.Walk.reverse_map

@[simp]
lemma support_map : (p.map f).support = p.support.map f := by induction p <;> simp [*]
#align simple_graph.walk.support_map SimpleGraph.Walk.support_map

@[simp]
lemma darts_map : (p.map f).darts = p.darts.map f.mapDart := by induction p <;> simp [*]
#align simple_graph.walk.darts_map SimpleGraph.Walk.darts_map

@[simp]
lemma edges_map : (p.map f).edges = p.edges.map (Sym2.map f) := by
  induction p with
  | nil => rfl
  | cons _ _ ih =>
...
lemma map_isPath_of_injective (hinj : Function.Injective f) (hp : p.IsPath) :
    (p.map f).IsPath := by
  induction p with
  | nil => simp
  | cons _ _ ih =>
...
protected lemma IsPath.of_map {f : G →g G'} (hp : (p.map f).IsPath) : p.IsPath := by
  induction p with
  | nil => simp
  | cons _ _ ih =>
...
lemma map_isPath_iff_of_injective (hinj : Function.Injective f) : (p.map f).IsPath ↔ p.IsPath :=
...
lemma map_isTrail_iff_of_injective (hinj : Function.Injective f) :
...
lemma map_isCycle_iff_of_injective {p : G.Walk u u} (hinj : Function.Injective f) :
...
@[simp]
lemma mapLe_isTrail {G G' : SimpleGraph V} (h : G ≤ G') {u v : V} {p : G.Walk u v} :
...
@[simp]
lemma mapLe_isPath {G G' : SimpleGraph V} (h : G ≤ G') {u v : V} {p : G.Walk u v} :
...
@[simp]
lemma mapLe_isCycle {G G' : SimpleGraph V} (h : G ≤ G') {u : V} {p : G.Walk u u} :
...
lemma mapEmbedding_injective (f : G ↪g G') (u v : V) :
...
lemma transfer_self : p.transfer G p.edges_subset_edgeSet = p := by
  induction p <;> simp [*]
#align simple_graph.walk.transfer_self SimpleGraph.Walk.transfer_self

variable {H : SimpleGraph V}

lemma transfer_eq_map_of_le (hp) (GH : G ≤ H) :
...
@[simp]
lemma edges_transfer (hp) : (p.transfer H hp).edges = p.edges := by
  induction p <;> simp [*]
#align simple_graph.walk.edges_transfer SimpleGraph.Walk.edges_transfer

@[simp]
lemma support_transfer (hp) : (p.transfer H hp).support = p.support := by
  induction p <;> simp [*]
#align simple_graph.walk.support_transfer SimpleGraph.Walk.support_transfer

@[simp]
lemma length_transfer (hp) : (p.transfer H hp).length = p.length := by
  induction p <;> simp [*]
#align simple_graph.walk.length_transfer SimpleGraph.Walk.length_transfer

variable {p}

protected lemma IsPath.transfer (hp) (pp : p.IsPath) :
...
protected lemma IsCycle.transfer {q : G.Walk u u} (qc : q.IsCycle) (hq) :
...
@[simp]
lemma transfer_transfer (hp) {K : SimpleGraph V} (hp') :
...
@[simp]
lemma transfer_append {w : V} (q : G.Walk v w) (hpq) :
...
@[simp]
lemma toDeleteEdges_nil (s : Set (Sym2 V)) {v : V} (hp) :
...
@[simp]
lemma toDeleteEdges_cons (s : Set (Sym2 V)) {u v w : V} (h : G.Adj u v) (p : G.Walk v w) (hp) :
...
@[simp]
lemma map_toDeleteEdges_eq (s : Set (Sym2 V)) {p : G.Walk v w} (hp) :
...
protected lemma IsPath.toDeleteEdges (s : Set (Sym2 V))
...
protected lemma IsCycle.toDeleteEdges (s : Set (Sym2 V))
...
@[simp]
lemma toDeleteEdges_copy {v u u' v' : V} (s : Set (Sym2 V))
...
protected lemma Reachable.elim {p : Prop} {u v : V} (h : G.Reachable u v)
    (hp : G.Walk u v → p) : p :=
  Nonempty.elim h hp
#align simple_graph.reachable.elim SimpleGraph.Reachable.elim

protected lemma Reachable.elim_path {p : Prop} {u v : V} (h : G.Reachable u v)
...
protected lemma Walk.reachable {G : SimpleGraph V} {u v : V} (p : G.Walk u v) : G.Reachable u v :=
  ⟨p⟩
#align simple_graph.walk.reachable SimpleGraph.Walk.reachable

protected lemma Adj.reachable {u v : V} (h : G.Adj u v) : G.Reachable u v :=
  h.toWalk.reachable
#align simple_graph.adj.reachable SimpleGraph.Adj.reachable

@[refl]
protected lemma Reachable.refl (u : V) : G.Reachable u u := ⟨Walk.nil⟩
...
@[symm]
protected lemma Reachable.symm {u v : V} (huv : G.Reachable u v) : G.Reachable v u :=
...
@[trans]
protected lemma Reachable.trans {u v w : V} (huv : G.Reachable u v) (hvw : G.Reachable v w) :
    G.Reachable u w :=
  huv.elim fun puv => hvw.elim fun pvw => ⟨puv.append pvw⟩
#align simple_graph.reachable.trans SimpleGraph.Reachable.trans

lemma reachable_iff_reflTransGen (u v : V) :
...
protected lemma Reachable.map {u v : V} {G : SimpleGraph V} {G' : SimpleGraph V'} (f : G →g G')
...
lemma Iso.reachable_iff {G : SimpleGraph V} {G' : SimpleGraph V'} {φ : G ≃g G'} {u v : V} :
...
lemma Iso.symm_apply_reachable {G : SimpleGraph V} {G' : SimpleGraph V'} {φ : G ≃g G'} {u : V}
...
lemma Preconnected.map {G : SimpleGraph V} {H : SimpleGraph V'} (f : G →g H) (hf : Surjective f)
...
lemma Iso.preconnected_iff {G : SimpleGraph V} {H : SimpleGraph V'} (e : G ≃g H) :
...
lemma Connected.map {G : SimpleGraph V} {H : SimpleGraph V'} (f : G →g H) (hf : Surjective f)
...
lemma Iso.connected_iff {G : SimpleGraph V} {H : SimpleGraph V'} (e : G ≃g H) :
...
@[elab_as_elim]
protected lemma ind {β : G.ConnectedComponent → Prop}
...
@[elab_as_elim]
protected lemma ind₂ {β : G.ConnectedComponent → G.ConnectedComponent → Prop}
...
@[simp]
protected lemma lift_mk {β : Sort*} {f : V → β}
...
protected lemma «exists» {p : G.ConnectedComponent → Prop} :
...
protected lemma «forall» {p : G.ConnectedComponent → Prop} :
    (∀ c : G.ConnectedComponent, p c) ↔ ∀ v, p (G.connectedComponentMk v) :=
  (surjective_quot_mk G.Reachable).forall
#align simple_graph.connected_component.forall SimpleGraph.ConnectedComponent.forall

lemma _root_.SimpleGraph.Preconnected.subsingleton_connectedComponent (h : G.Preconnected) :
...
@[simp]
lemma map_id (C : ConnectedComponent G) : C.map Hom.id = C := by
  refine' C.ind _
  exact fun _ => rfl
#align simple_graph.connected_component.map_id SimpleGraph.ConnectedComponent.map_id

@[simp]
lemma map_comp (C : G.ConnectedComponent) (φ : G →g G') (ψ : G' →g G'') :
...
@[simp]
lemma iso_image_comp_eq_map_iff_eq_comp {C : G.ConnectedComponent} :
...
@[simp]
lemma iso_inv_image_comp_eq_iff_eq_map {C : G.ConnectedComponent} :
...
@[simp]
lemma connectedComponentEquiv_refl :
...
@[simp]
lemma connectedComponentEquiv_symm (φ : G ≃g G') :
...
@[simp]
lemma connectedComponentEquiv_trans (φ : G ≃g G') (φ' : G' ≃g G'') :
...
@[simp]
lemma supp_inj {C D : G.ConnectedComponent} : C.supp = D.supp ↔ C = D :=
...
@[simp]
lemma mem_supp_iff (C : G.ConnectedComponent) (v : V) :
...
lemma connectedComponentMk_mem {v : V} : v ∈ G.connectedComponentMk v :=
  rfl
...
lemma toSubgraph_cons_nil_eq_subgraphOfAdj (h : G.Adj u v) :
...
lemma mem_verts_toSubgraph (p : G.Walk u v) : w ∈ p.toSubgraph.verts ↔ w ∈ p.support := by
...
@[simp]
lemma verts_toSubgraph (p : G.Walk u v) : p.toSubgraph.verts = { w | w ∈ p.support } :=
  Set.ext fun _ => p.mem_verts_toSubgraph
#align simple_graph.walk.verts_to_subgraph SimpleGraph.Walk.verts_toSubgraph

lemma mem_edges_toSubgraph (p : G.Walk u v) {e : Sym2 V} :
...
@[simp]
lemma edgeSet_toSubgraph (p : G.Walk u v) : p.toSubgraph.edgeSet = { e | e ∈ p.edges } :=
  Set.ext fun _ => p.mem_edges_toSubgraph
#align simple_graph.walk.edge_set_to_subgraph SimpleGraph.Walk.edgeSet_toSubgraph

@[simp]
lemma toSubgraph_append (p : G.Walk u v) (q : G.Walk v w) :
...
@[simp]
lemma toSubgraph_reverse (p : G.Walk u v) : p.reverse.toSubgraph = p.toSubgraph := by
  induction p with
  | nil => simp
  | cons _ _ _ =>
...
@[simp]
lemma toSubgraph_rotate [DecidableEq V] (c : G.Walk v v) (h : u ∈ c.support) :
...
@[simp]
lemma toSubgraph_map (f : G →g G') (p : G.Walk u v) : (p.map f).toSubgraph = p.toSubgraph.map f :=
  by induction p <;> simp [*, Subgraph.map_sup]
#align simple_graph.walk.to_subgraph_map SimpleGraph.Walk.toSubgraph_map

@[simp]
lemma finite_neighborSet_toSubgraph (p : G.Walk u v) : (p.toSubgraph.neighborSet w).Finite := by
  induction p with
  | nil =>
...
lemma set_walk_self_length_zero_eq (u : V) : {p : G.Walk u u | p.length = 0} = {Walk.nil} := by
  ext p
  simp
...
lemma coe_finsetWalkLength_eq (n : ℕ) (u v : V) :
...
lemma Walk.mem_finsetWalkLength_iff_length_eq {n : ℕ} {u v : V} (p : G.Walk u v) :
...
lemma set_walk_length_toFinset_eq (n : ℕ) (u v : V) :
...
lemma reachable_iff_exists_finsetWalkLength_nonempty (u v : V) :
...
lemma isBridge_iff_adj_and_forall_walk_mem_edges {v w : V} :
...
lemma reachable_deleteEdges_iff_exists_cycle.aux [DecidableEq V] {u v w : V}
...
lemma isBridge_iff_adj_and_forall_cycle_not_mem {v w : V} : G.IsBridge s(v, w) ↔
...
lemma isBridge_iff_mem_and_forall_cycle_not_mem {e : Sym2 V} :
