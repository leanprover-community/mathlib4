...
lemma exists_cons_eq_concat {u v w : V} (h : G.Adj u v) (p : G.Walk v w) :
...
lemma exists_concat_eq_cons {u v w : V} :
...
lemma edges_subset_edgeSet {u v : V} :
...
lemma isTrail_def {u v : V} (p : G.Walk u v) : p.IsTrail ↔ p.edges.Nodup :=
...
lemma isCircuit_def {u : V} (p : G.Walk u u) : p.IsCircuit ↔ p.IsTrail ∧ p ≠ nil :=
...
lemma exists_boundary_dart {u v : V} (p : G.Walk u v) (S : Set V) (uS : u ∈ S) (vS : v ∉ S) :
...
@[simp]
lemma transfer_append (q : G.Walk v w) (hpq) :
...
@[simp]
lemma toDeleteEdges_copy (s : Set (Sym2 V))
...
protected lemma Reachable.map {G : SimpleGraph V} {G' : SimpleGraph V'} (f : G →g G')
...
lemma isBridge_iff_adj_and_forall_cycle_not_mem {v w : V} : G.IsBridge ⟦(v, w)⟧ ↔
