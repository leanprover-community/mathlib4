...
private theorem card_nonuniformWitness_sdiff_biUnion_star (hV : V ∈ P.parts) (hUV : U ≠ V)
  ...
  -- ⊢ ∑ a in filter (fun B => B ⊆ SimpleGraph.nonuniformWitness G ε U V ∧ Finset.N …
  ...
  · suffices ∀ B ∈ (atomise U <| P.nonuniformWitnesses G ε U).parts,
    ...
    intro B hB
    -- ⊢ Finset.card (B \ Finset.biUnion (filter (fun x => x ⊆ B) (chunk hP G ε hU).p …
    unfold chunk
    -- ⊢ Finset.card (B \ Finset.biUnion (filter (fun x => x ⊆ B) (if hUcard : Finset …
    split_ifs with h₁
    -- ⊢ Finset.card (B \ Finset.biUnion (filter (fun x => x ⊆ B) (equitabilise (_ :  …
    · convert card_parts_equitabilise_subset_le _ (card_aux₁ h₁) hB
      -- 🎉 no goals
    · convert card_parts_equitabilise_subset_le _ (card_aux₂ hP hU h₁) hB
      -- 🎉 no goals
  rw [sum_const]
  -- ⊢ Finset.card (filter (fun B => B ⊆ SimpleGraph.nonuniformWitness G ε U V ∧ Fi …
...
private theorem one_sub_eps_mul_card_nonuniformWitness_le_card_star (hV : V ∈ P.parts)
  ...
  -- ⊢ (1 - ε / 10) * ↑(Finset.card (SimpleGraph.nonuniformWitness G ε U V)) ≤ ↑(Fi …
...
theorem card_chunk (hm : m ≠ 0) : (chunk hP G ε hU).parts.card = 4 ^ P.parts.card := by
  unfold chunk
  -- ⊢ Finset.card (if hUcard : Finset.card U = Fintype.card α / stepBound (Finset. …
  split_ifs
  -- ⊢ Finset.card (equitabilise (_ : (4 ^ Finset.card P.parts - (Fintype.card α /  …
...
theorem card_eq_of_mem_parts_chunk (hs : s ∈ (chunk hP G ε hU).parts) :
  ...
  unfold chunk at hs
  -- ⊢ Finset.card s = Fintype.card α / stepBound (Finset.card P.parts) ∨ Finset.ca …
...
private theorem le_sum_card_subset_chunk_parts (h𝒜 : 𝒜 ⊆ (chunk hP G ε hU).parts) (hs : s ∈ 𝒜) :
  ...
  -- ⊢ ↑(Finset.card 𝒜) * ↑(Fintype.card α / stepBound (Finset.card P.parts)) * ↑(F …
  refine' mul_le_mul _ _ (cast_nonneg _) (cast_nonneg _)
  -- ⊢ ↑(Finset.card 𝒜) * ↑(Fintype.card α / stepBound (Finset.card P.parts)) ≤ ↑(F …
  · rw [← (ofSubset _ h𝒜 rfl).sum_card_parts, ofSubset_parts, ← cast_mul, cast_le]
    -- ⊢ Finset.card 𝒜 * (Fintype.card α / stepBound (Finset.card P.parts)) ≤ ∑ i in  …
...
private theorem sum_card_subset_chunk_parts_le (m_pos : (0 : ℝ) < m)
  ...
  -- ⊢ ↑(Finset.card (Finset.biUnion 𝒜 id)) * ↑(Fintype.card α / stepBound (Finset. …
  refine' mul_le_mul _ _ (cast_nonneg _) (by positivity)
  -- ⊢ ↑(Finset.card (Finset.biUnion 𝒜 id)) ≤ ↑(Finset.card 𝒜) * (↑(Fintype.card α  …
  · norm_cast
    -- ⊢ Finset.card (Finset.biUnion 𝒜 id) ≤ Finset.card 𝒜 * (Fintype.card α / stepBo …
...
private theorem one_sub_le_m_div_m_add_one_sq [Nonempty α]
  ...
  -- ⊢ 1 - ε ^ 5 / 50 ≤ 1 - 2 * (1 / (↑(Fintype.card α / stepBound (Finset.card P.p …
...
private theorem m_add_one_div_m_le_one_add [Nonempty α]
  ...
  rw [same_add_div]
  -- ⊢ (1 + 1 / ↑(Fintype.card α / stepBound (Finset.card P.parts))) ^ 2 ≤ 1 + ε ^  …
  swap; · sz_positivity
...
private theorem density_sub_eps_le_sum_density_div_card [Nonempty α]
  ...
  refine' this.trans _
  -- ⊢ (1 - ε ^ 5 / 50) * ↑(SimpleGraph.edgeDensity G (Finset.biUnion A id) (Finset …
  ...
  push_cast
  -- ⊢ ∑ x in A ×ˢ B, ↑(Finset.card (interedges G.Adj x.fst x.snd)) * ((1 - ε ^ 5 / …
  apply sum_le_sum
  -- ⊢ ∀ (i : Finset α × Finset α), i ∈ A ×ˢ B → ↑(Finset.card (interedges G.Adj i. …
  simp only [and_imp, Prod.forall, mem_product]
  -- ⊢ ∀ (a b : Finset α), a ∈ A → b ∈ B → ↑(Finset.card (interedges G.Adj a b)) *  …
  rintro x y hx hy
  -- ⊢ ↑(Finset.card (interedges G.Adj x y)) * ((1 - ε ^ 5 / 50) / (↑(Finset.card ( …
...
private theorem sum_density_div_card_le_density_add_eps [Nonempty α]
  ...
  conv_rhs =>
    ...
    simp only [cast_sum, mul_sum, sum_div, Rat.cast_sum, Rat.cast_div,
      mul_div_left_comm ((1 : ℝ) + _)]
  push_cast
  -- ⊢ ∑ x in Finset.product A B, ↑(Finset.card (interedges G.Adj x.fst x.snd)) / ( …
  apply sum_le_sum
  -- ⊢ ∀ (i : Finset α × Finset α), i ∈ Finset.product A B → ↑(Finset.card (intered …
  simp only [and_imp, Prod.forall, mem_product, show A.product B = A ×ˢ B by rfl]
  -- ⊢ ∀ (a b : Finset α), a ∈ A → b ∈ B → ↑(Finset.card (interedges G.Adj a b)) /  …
  intro x y hx hy
  -- ⊢ ↑(Finset.card (interedges G.Adj x y)) / (↑(Finset.card x) * ↑(Finset.card y) …
  rw [mul_mul_mul_comm, mul_comm (x.card : ℝ), mul_comm (y.card : ℝ), div_le_iff, mul_assoc]
  -- ⊢ ↑(Finset.card (interedges G.Adj x y)) ≤ ↑(Finset.card (interedges G.Adj x y) …
  · refine' le_mul_of_one_le_right (cast_nonneg _) _
    -- ⊢ 1 ≤ (1 + ε ^ 5 / 49) / (↑(Finset.card (sup A id)) * ↑(Finset.card (sup B id) …
    rw [div_mul_eq_mul_div, one_le_div]
    -- ⊢ ↑(Finset.card (sup A id)) * ↑(Finset.card (sup B id)) ≤ (1 + ε ^ 5 / 49) * ( …
    ...
    · rw [sq, mul_mul_mul_comm, mul_comm (_ / (m : ℝ)), mul_comm (_ / (m : ℝ))]
      -- ⊢ ↑(Finset.card (sup A id)) * ↑(Finset.card (sup B id)) ≤ ↑(Finset.card A) * ↑ …
...
private theorem average_density_near_total_density [Nonempty α]
  ...
  -- ⊢ ↑(SimpleGraph.edgeDensity G (Finset.biUnion A id) (Finset.biUnion B id)) - ε …
...
private theorem edgeDensity_chunk_aux [Nonempty α]
  ...
  -- ⊢ ↑(SimpleGraph.edgeDensity G U V) ^ 2 - ε ^ 5 / 25 ≤ ((∑ ab in Finset.product …
  ...
  -- ⊢ ↑(SimpleGraph.edgeDensity G U V) ^ 2 - ε ^ 5 / 25 ≤ (↑(SimpleGraph.edgeDensi …
  rw [sub_sq, sub_add, sub_le_sub_iff_left]
  -- ⊢ 2 * ↑(SimpleGraph.edgeDensity G U V) * (ε ^ 5 / 50) - (ε ^ 5 / 50) ^ 2 ≤ ε ^ …
...
private theorem edgeDensity_star_not_uniform [Nonempty α]
  ...
  -- ⊢ 3 / 4 * ε ≤ |(∑ ab in Finset.product (star hP G ε hU V) (star hP G ε hV U),  …
  set p : ℝ :=
    (∑ ab in (star hP G ε hU V).product (star hP G ε hV U), (G.edgeDensity ab.1 ab.2 : ℝ)) /
      ((star hP G ε hU V).card * (star hP G ε hV U).card)
  set q : ℝ :=
...
theorem edgeDensity_chunk_not_uniform [Nonempty α] (hPα : P.parts.card * 16 ^ P.parts.card ≤ card α)
  ...
  calc
    ↑(G.edgeDensity U V) ^ 2 - ε ^ 5 / 25 + ε ^ 4 / ↑3 ≤ ↑(G.edgeDensity U V) ^ 2 - ε ^ 5 / ↑25 +
      ...
      -- ⊢ ε ^ 4 / 3 ≤ ↑(Finset.card (star hP G ε hU V)) * ↑(Finset.card (star hP G ε h …
      ...
      -- ⊢ ε ^ 4 / 3 ≤ ↑(Finset.card (star hP G ε hU V)) / 4 ^ Finset.card P.parts * (↑ …
    ...
    _ ≤ (∑ ab in (chunk hP G ε hU).parts.product (chunk hP G ε hV).parts,
      ...
      -- ⊢ ↑(SimpleGraph.edgeDensity G U V) ^ 2 - ε ^ 5 / 25 + ↑(Finset.card (star hP G …
      ...
      · simp_rw [sp, card_product, card_chunk (m_pos hPα).ne', ← mul_pow]
        -- ⊢ 3 / 4 * ε ≤ |(∑ x in star hP G ε hU V ×ˢ star hP G ε hV U, ↑(SimpleGraph.edg …
        norm_num
        -- ⊢ 3 / 4 * ε ≤ |(∑ x in star hP G ε hU V ×ˢ star hP G ε hV U, ↑(SimpleGraph.edg …
        exact edgeDensity_star_not_uniform hPα hPε hε₁ hUVne hUV
        -- 🎉 no goals
      · rw [sp, card_product]
        -- ⊢ ↑(SimpleGraph.edgeDensity G U V) ^ 2 - ε ^ 5 / 25 ≤ ((∑ i in (chunk hP G ε h …
        apply (edgeDensity_chunk_aux hPα hPε hU hV).trans
        -- ⊢ ((∑ ab in Finset.product (chunk ?m.2764446 G ε hU).parts (chunk ?m.2764446 G …
