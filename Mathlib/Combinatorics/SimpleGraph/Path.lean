...
protected lemma IsPath.concat {p : G.Walk u v} (hp : p.IsPath) (hu : w ∉ p.support)
    {h : G.Adj v w} : (p.concat h).IsPath := (concat_isPath_iff _ _).2 ⟨hp, hu⟩
...
lemma IsPath.not_start_mem_tail_support_of_not_nil {p : G.Walk u v} (hp : p.IsPath) (hn : ¬ p.Nil) :
    u ∉ p.tail.support :=
  fun h ↦ ((cons_isPath_iff ..).1 ((cons_tail_eq _ hn) ▸ hp)).2 h

lemma IsPath.not_end_mem_dropLast_support_of_not_nil {p : G.Walk u v} (hp : p.IsPath)
    (hn : ¬ p.Nil) : v ∉ p.dropLast.support :=
...
lemma IsPath.eq_snd_of_start_mem_edge  {u v x : V} {p : G.Walk u v} (hp : p.IsPath)
...
lemma IsPath.eq_penultimate_of_end_mem_edge {u v x : V} {p : G.Walk u v} (hp : p.IsPath)
    (hs : s(x, v) ∈ p.edges) : x = p.penultimate := by
  have h' : s(x, v) ∈ p.reverse.edges := by simp [hs]
  convert hp.reverse.eq_snd_of_start_mem_edge h' using 1
  simp

lemma IsPath.cons_drop_isCycle {p : G.Walk u v} (hp : p.IsPath) {m : ℕ} (ha : G.Adj v (p.getVert m))
...
lemma Connected.connected_delete_edge_of_not_isBridge (hG : G.Connected) {x y : V}
  ...
  have hxP₁ := Walk.end_not_mem_support_takeUntil hP hyP hxy.ne
