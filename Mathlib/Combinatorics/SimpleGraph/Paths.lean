...
theorem IsTrail.of_cons {u v w : V} {h : G.Adj u v} {p : Walk G v w} :
    (cons h p).IsTrail → p.IsTrail := by simp [isTrail_def]

@[simp]
theorem isTrail_cons {u v w : V} (h : G.Adj u v) (p : Walk G v w) :
...
protected lemma IsTrail.cons {w : Walk G u' v} (hw : w.IsTrail) (hu : G.Adj u u')
...
theorem IsPath.of_cons {u v w : V} {h : G.Adj u v} {p : Walk G v w} :
    (cons h p).IsPath → p.IsPath := by simp [isPath_def]

@[simp]
theorem cons_isPath_iff {u v w : V} (h : G.Adj u v) (p : Walk G v w) :
...
lemma not_nil_of_isCycle_cons {p : Walk G u v} {h : G.Adj v u} (hc : (cons h p).IsCycle) :
    ¬ p.Nil := by
  have := length_cons h p ▸ IsCycle.three_le_length hc
  rw [Walk.not_nil_iff_lt_length]
  lia

theorem cons_isCycle_iff {u v : V} (p : Walk G v u) (h : G.Adj u v) :
...
lemma IsCycle.getVert_injOn {p : Walk G u u} (hpc : p.IsCycle) :
  ...
  intro n hn m hm hnm
  rw [← length_tail_add_one (p.not_nil_of_tail_not_nil (not_nil_of_isCycle_cons hpc)),
    Set.mem_setOf] at hn hm
  have := ((cons_isCycle_iff _ _).mp hpc).1.getVert_injOn
    (by lia : n - 1 ≤ p.tail.length) (by lia : m - 1 ≤ p.tail.length)
    (by simp_all [getVert_tail, Nat.sub_add_cancel hn.1, Nat.sub_add_cancel hm.1])
  lia
...
theorem IsCycle.count_support_of_mem {c : Walk G v v} (hc : c.IsCycle) (hu : u ∈ c.support)
  ...
  have : List.count u c.support.tail = 1 :=
    List.count_eq_one_of_mem hc.support_nodup <| this.resolve_left <| by rw [head_support]; exact hv
  have := c.head_support ▸ List.head?_eq_some_head c.support_ne_nil
  grind
...
def toPath (p : Walk G u v) : G.Path u v :=
