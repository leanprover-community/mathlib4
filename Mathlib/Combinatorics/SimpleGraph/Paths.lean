...
lemma IsCycle.three_le_length {v : V} {p : G.Walk v v} (hp : p.IsCycle) : 3 ≤ p.length := by
  ...
  | .cons _ (.cons _ (.cons _ _)) => simp_rw [SimpleGraph.Walk.length_cons]; grind
...
lemma IsPath.getVert_injOn {p : G.Walk u v} (hp : p.IsPath) :
  ...
  | @cons v w u h p ihp =>
    ...
    · simp only [hn0, getVert_zero, Walk.getVert_cons p h hm0] at hnm
      ...
      exact (hvp (Walk.mem_support_iff_exists_getVert.mpr ⟨(m - 1), ⟨hnm.symm, by grind⟩⟩)).elim
    · simp only [hm0, Walk.getVert_cons p h hn0] at hnm
      have hvp : v ∉ p.support := by simp_all
      exact (hvp (Walk.mem_support_iff_exists_getVert.mpr ⟨(n - 1), ⟨hnm, by grind⟩⟩)).elim
    · simp only [Walk.getVert_cons _ _ hn0, Walk.getVert_cons _ _ hm0] at hnm
      have := ihp hp.of_cons (by grind : (n - 1) ≤ p.length)
        (by grind : (m - 1) ≤ p.length) hnm
      grind
...
lemma IsPath.getVert_eq_start_iff {i : ℕ} {p : G.Walk u w} (hp : p.IsPath) (hi : i ≤ p.length) :
  ...
  by_cases hi : i = 0
  · exact hi
  · apply hp.getVert_injOn (by rw [Set.mem_setOf]; grind) (by rw [Set.mem_setOf]; grind)
    simp [h]

lemma IsPath.getVert_eq_end_iff {i : ℕ} {p : G.Walk u w} (hp : p.IsPath) (hi : i ≤ p.length) :
    p.getVert i = w ↔ i = p.length := by
  have := hp.reverse.getVert_eq_start_iff (by grind : p.reverse.length - i ≤ p.reverse.length)
  simp only [length_reverse, getVert_reverse, show p.length - (p.length - i) = i by grind] at this
  rw [this]
  grind
...
lemma IsPath.getVert_injOn_iff (p : G.Walk u v) : Set.InjOn p.getVert {i | i ≤ p.length} ↔
  ...
  | cons h q ih =>
    ...
    refine ⟨ih (by
      ...
      have := hinj (by rw [length_cons]; grind : n + 1 ≤ (q.cons h).length)
          (by rw [length_cons]; grind : m + 1 ≤ (q.cons h).length)
          (by simpa [getVert_cons] using hnm)
      grind), fun h' => ?_⟩
    obtain ⟨n, ⟨hn, hnl⟩⟩ := mem_support_iff_exists_getVert.mp h'
    have := hinj (by rw [length_cons]; grind : (n + 1) ≤ (q.cons h).length)
      (by grind : 0 ≤ (q.cons h).length) (show (q.cons h).getVert (n + 1) = (q.cons h).getVert 0
        from by rwa [getVert_cons _ _ (by grind : n + 1 ≠ 0), getVert_zero])
    grind
...
lemma IsCycle.getVert_injOn {p : G.Walk u u} (hpc : p.IsCycle) :
  ...
  have := ((Walk.cons_isCycle_iff _ _).mp hpc).1.getVert_injOn
      (by grind : n - 1 ≤ p.tail.length) (by grind : m - 1 ≤ p.tail.length)
      (by simp_all [SimpleGraph.Walk.getVert_tail, show n - 1 + 1 = n by grind,
          show m - 1 + 1 = m by grind])
  grind
...
lemma IsCycle.getVert_injOn' {p : G.Walk u u} (hpc : p.IsCycle) :
  ...
  have : p.length - n = p.length - m := Walk.length_reverse _ ▸ hpc.reverse.getVert_injOn
    (by simp only [Walk.length_reverse, Set.mem_setOf_eq]; grind)
    (by simp only [Walk.length_reverse, Set.mem_setOf_eq]; grind)
    (by simp [Walk.getVert_reverse, show p.length - (p.length - n) = n by grind, hnm,
      show p.length - (p.length - m) = m by grind])
  grind

lemma IsCycle.snd_ne_penultimate {p : G.Walk u u} (hp : p.IsCycle) : p.snd ≠ p.penultimate := by
  intro h
  have := hp.three_le_length
  apply hp.getVert_injOn (by simp; grind) (by simp; grind) at h
  grind
...
lemma IsCycle.getVert_endpoint_iff {i : ℕ} {p : G.Walk u u} (hpc : p.IsCycle) (hl : i ≤ p.length) :
  ...
  intro h hi
  exact hpc.getVert_injOn (by simp only [Set.mem_setOf_eq]; grind)
    (by simp only [Set.mem_setOf_eq]; grind) (h.symm ▸ (Walk.getVert_length p).symm)
...
lemma IsCycle.getVert_sub_one_ne_getVert_add_one {i : ℕ} {p : G.Walk u u} (hpc : p.IsCycle)
  ...
  · intro h'
    rw [p.getVert_of_length_le (by grind : p.length ≤ i + 1),
      hpc.getVert_endpoint_iff (by grind)] at h'
    grind
  intro h'
  have := hpc.getVert_injOn' (by simp only [Set.mem_setOf_eq, Nat.sub_le_iff_le_add]; grind)
    (by simp only [Set.mem_setOf_eq]; grind) h'
  grind
...
lemma endpoint_notMem_support_takeUntil {p : G.Walk u v} (hp : p.IsPath) (hw : w ∈ p.support)
  ...
  have : n = p.length := hp.getVert_injOn (by rw [Set.mem_setOf]; grind) (by simp)
