...
instance : HyperGraphLike V (V × V) (Sym2 V) (SimpleGraph V) where
  ...
  adj_def G u v := ⟨fun huv ↦ ⟨s(u, v), (u, v), (v, u), by simp [huv.ne], by simp [huv],
    by simp [huv.symm], huv, huv.symm⟩, by grind⟩
...
instance : GraphLike V (V × V) (Sym2 V) (SimpleGraph V) where
  order_eq_two G e := Sym2.inductionOn e fun u v he => by
    ...
    have h : edgeIncidents G (s(u, v)) = {(u, v), (v, u)} := by
      ext i
      simp only [mem_edgeIncidents_iff, isIncident_def, Set.mem_insert_iff,
        Set.mem_singleton_iff]
      refine ⟨by grind, ?_⟩
      rintro (rfl | rfl)
      · exact ⟨u, he, rfl, rfl⟩
      · exact ⟨v, he.symm, Sym2.eq_swap, rfl⟩
    rw [h]
    exact Set.encard_pair hne
  exists_isSource_of_mem_edgeSet G e he := by
    induction e using Sym2.inductionOn with
    | hf x y =>
      simp only [mem_edgeIncidents_iff, isIncident_def, exists_and_left, ↓existsAndEq, and_true,
        isSource_def, Prod.exists]
      use x, y, ⟨he, rfl⟩, he
  exists_isTarget_of_mem_edgeSet G e he := by
    induction e using Sym2.inductionOn with
    | hf x y =>
      simp only [mem_edgeIncidents_iff, isIncident_def, exists_and_left, ↓existsAndEq, and_true,
        isTarget_def, Prod.exists]
      use x, y, ⟨he, rfl⟩, he
...
lemma edgeFun_eq {i : V × V} (hi : G.Adj i.1 i.2) : edgeFun G i = Part.some (s(i.1, i.2)) := by
  ext e
  rw [mem_edgeFun_mem_edgeIncidents_comm]
  simp [hi, eq_comm]

lemma endPoint_eq {i : V × V} (hi : G.Adj i.1 i.2) : endPoint G i = Part.some i.1 := by
  ext v
  rw [mem_endPoint_mem_vertexIncidents_comm]
  simp [hi, eq_comm]

instance : HyperGraphLike.loopless V (V × V) (Sym2 V) (SimpleGraph V) where
