...
This files gives simple-graph-specific properties of walks.
-/
...
@[deprecated (since := "2026-02-26")] alias _root_.SimpleGraph.Walk := Walk
...
theorem edges_subset_edgeSet {u v : V} :
    ∀ (p : Walk G u v) ⦃e : Sym2 V⦄, e ∈ p.edges → e ∈ G.edgeSet
  | cons h' p', e, h => by
    cases h
    · exact h'
...
@[simp]
theorem edges_cons {u v w : V} (h : G.Adj u v) (p : Walk G v w) :
...
@[simp]
theorem edgeSet_cons {u v w : V} (h : G.Adj u v) (p : Walk G v w) :
