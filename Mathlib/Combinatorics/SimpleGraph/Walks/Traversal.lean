...
def getVert {u v : V} : Walk G u v → ℕ → V
  | nil, _ => u
  | cons _ _, 0 => u
...
@[simp]
theorem getVert_length {u v} (w : Walk G u v) : w.getVert w.length = v :=
  w.getVert_of_length_le rfl.le

theorem adj_getVert_succ {u v} (w : Walk G u v) {i : ℕ} (hi : i < w.length) :
    G.Adj (w.getVert i) (w.getVert (i + 1)) := by
  induction w generalizing i with
  | nil => cases hi
  | cons hxy _ ih =>
    cases i
    · simpa [getVert] using hxy.adj
...
@[simp]
theorem getVert_mem_support {u v : V} (p : Walk G u v) (i : ℕ) : p.getVert i ∈ p.support := by
  induction p generalizing i <;> cases i <;> simp [*]

/-- Use `support_getElem_eq_getVert` to rewrite in the reverse direction. -/
lemma getVert_eq_support_getElem {u v : V} {n : ℕ} (p : Walk G u v) (h : n ≤ p.length) :
...
lemma support_getElem_eq_getVert {u v : V} {n : ℕ} (p : Walk G u v) (h) :
...
lemma getVert_eq_getD_support {u v : V} (p : Walk G u v) (n : ℕ) :
...
@[simp]
lemma getVert_support_idxOf [DecidableEq V] (p : Walk G u v) (h : w ∈ p.support) :
    p.getVert (p.support.idxOf w) = w := by
  grind [getVert_eq_support_getElem]

theorem getVert_comp_val_eq_get_support {u v : V} (p : Walk G u v) :
    p.getVert ∘ Fin.val = p.support.get := by
  grind [getVert_eq_support_getElem, length_support]

theorem range_getVert_eq_range_support_getElem {u v : V} (p : Walk G u v) :
...
theorem darts_getElem_eq_getVert {p : Walk G u v} (n : ℕ) (h : n < p.darts.length) :
  ...
  ext <;> simp [p.getVert_eq_support_getElem (le_of_lt h), p.getVert_eq_support_getElem h,
    darts_getElem_eq_fst_snd]

theorem adj_of_infix_support {u v u' v'} {p : Walk G u v} (h : [u', v'] <:+: p.support) :
    G.Adj u' v' := by
...
abbrev snd (p : Walk G u v) : V := p.getVert 1

@[simp] lemma adj_snd {p : Walk G v w} (hp : ¬ p.Nil) :
    G.Adj v p.snd := by
...
@[simp]
lemma support_getElem_one {p : Walk G u v} (hp) : p.support[1]'hp = p.snd := by
  grind [getVert_eq_support_getElem]

/-- Use `support_getElem_one` to rewrite in the reverse direction. -/
lemma snd_eq_support_getElem_one {p : Walk G u v} (hnil : ¬p.Nil) :
...
@[simp]
lemma adj_penultimate {p : Walk G v w} (hp : ¬ p.Nil) : G.Adj p.penultimate w := by
...
@[simp]
lemma support_getElem_length_sub_one_eq_penultimate {p : Walk G u v} :
    p.support[p.length - 1] = p.penultimate := by
  grind [getVert_eq_support_getElem]

/-- The first dart of a walk. -/
@[simps]
abbrev firstDart (p : Walk G v w) (hp : ¬ p.Nil) : GraphLike.darts G := ⟨(v, p.snd), p.adj_snd hp⟩

/-- The last dart of a walk. -/
@[simps]
abbrev lastDart (p : Walk G v w) (hp : ¬ p.Nil) : GraphLike.darts G :=
  ⟨(p.penultimate, w), p.adj_penultimate hp⟩

lemma edge_firstDart (p : Walk G v w) (hp : ¬ p.Nil) :
    dartSym2 (p.firstDart hp) = s(v, p.snd) := rfl

lemma edge_lastDart (p : Walk G v w) (hp : ¬ p.Nil) :
    dartSym2 (p.lastDart hp) = s(p.penultimate, w) := rfl

theorem firstDart_eq {p : Walk G v w} (h₁ : ¬ p.Nil) (h₂ : 0 < p.darts.length) :
    p.firstDart h₁ = p.darts[0] := by
  simp [darts_getElem_eq_getVert]

theorem lastDart_eq {p : Walk G v w} (h₁ : ¬ p.Nil) (h₂ : 0 < p.darts.length) :
    p.lastDart h₁ = p.darts[p.darts.length - 1] := by
  simp (disch := grind) [darts_getElem_eq_getVert, p.getVert_of_length_le]

/-- Use `firstDart_eq_head_darts` to rewrite in the reverse direction. -/
@[simp]
theorem head_darts_eq_firstDart {p : Walk G v w} (hnil : p.darts ≠ []) :
    p.darts.head hnil = p.firstDart (darts_eq_nil.not.mp hnil) := by
  grind [firstDart_eq]

/-- Use `head_darts_eq_firstDart` to rewrite in the reverse direction. -/
theorem firstDart_eq_head_darts {p : Walk G v w} (hnil : ¬p.Nil) :
...
theorem head_darts_fst {G : SimpleGraph V} {a b : V} (p : Walk G a b) (hp : p.darts ≠ []) :
    (p.darts.head hp).val.fst = a := by
  simp

@[simp]
theorem firstDart_mem_darts {p : Walk G v w} (hnil : ¬p.Nil) : p.firstDart hnil ∈ p.darts :=
  p.firstDart_eq_head_darts _ ▸ List.head_mem _

@[simp]
theorem getLast_darts_eq_lastDart {p : Walk G v w} (hnil : p.darts ≠ []) :
    p.darts.getLast hnil = p.lastDart (darts_eq_nil.not.mp hnil) := by
  grind [lastDart_eq, not_nil_iff_lt_length]

theorem lastDart_eq_getLast_darts {p : Walk G v w} (hnil : ¬p.Nil) :
    p.lastDart hnil = p.darts.getLast (darts_eq_nil.not.mpr hnil) := by
  grind [lastDart_eq, not_nil_iff_lt_length]

@[deprecated "Use `getLast_darts_eq_lastDart`" (since := "2025-12-10")]
theorem getLast_darts_snd {G : SimpleGraph V} {a b : V} (p : Walk G a b) (hp : p.darts ≠ []) :
    (p.darts.getLast hp).val.snd = b := by
  simp

@[simp]
theorem lastDart_mem_darts {p : Walk G v w} (hnil : ¬p.Nil) : p.lastDart hnil ∈ p.darts :=
  p.lastDart_eq_getLast_darts _ ▸ List.getLast_mem _

/-- Use `mk_start_snd_eq_head_edges` to rewrite in the reverse direction. -/
@[simp]
theorem head_edges_eq_mk_start_snd {p : Walk G v w} (hp) : p.edges.head hp = s(v, p.snd) := by
  simp [p.edge_firstDart, Walk.edges, -List.map_subtype]

/-- Use `head_edges_eq_mk_start_snd` to rewrite in the reverse direction. -/
theorem mk_start_snd_eq_head_edges {p : Walk G v w} (hnil : ¬p.Nil) :
    s(v, p.snd) = p.edges.head (edges_eq_nil.not.mpr hnil) :=
  head_edges_eq_mk_start_snd _ |>.symm

theorem mk_start_snd_mem_edges {p : Walk G v w} (hnil : ¬p.Nil) : s(v, p.snd) ∈ p.edges :=
...
@[simp]
theorem getLast_edges_eq_mk_penultimate_end {p : Walk G v w} (hp) :
    p.edges.getLast hp = s(p.penultimate, w) := by
  simp [p.edge_lastDart, Walk.edges, -List.map_subtype]

/-- Use `getLast_edges_eq_mk_penultimate_end` to rewrite in the reverse direction. -/
theorem mk_penultimate_end_eq_getLast_edges {p : Walk G v w} (hnil : ¬p.Nil) :
    s(p.penultimate, w) = p.edges.getLast (edges_eq_nil.not.mpr hnil) :=
  getLast_edges_eq_mk_penultimate_end _ |>.symm

theorem mk_penultimate_end_mem_edges {p : Walk G v w} (hnil : ¬p.Nil) :
