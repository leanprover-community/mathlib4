...
theorem adj_getVert_succ {u v} (w : Walk G u v) {i : ℕ} (hi : i < w.length) :
  ...
  | cons hxy _ ih =>
    cases i
    · simp only [getVert, getVert_zero]
      exact hxy
    · exact ih (Nat.succ_lt_succ_iff.1 hi)

@[simp]
lemma getVert_cons_succ {u v w n} (p : Walk G v w) (h : G.Adj u v) :
    (p.cons h).getVert (n + 1) = p.getVert n := rfl

lemma getVert_cons {u v w n} (p : Walk G v w) (h : G.Adj u v) (hn : n ≠ 0) :
...
lemma snd_cons {u v w} (q : Walk G v w) (hadj : G.Adj u v) :
...
@[simp]
lemma penultimate_nil : (@nil _ _ _ G v).penultimate = v := rfl

@[simp]
lemma penultimate_cons_nil (h : G.Adj u v) : (cons h nil).penultimate = u := rfl

@[simp]
lemma penultimate_cons_cons {w'} (h : G.Adj u v) (h₂ : G.Adj v w) (p : Walk G w w') :
    (cons h (cons h₂ p)).penultimate = (cons h₂ p).penultimate := rfl

lemma penultimate_cons_of_not_nil (h : G.Adj u v) (p : Walk G v w) (hp : ¬ p.Nil) :
    (cons h p).penultimate = p.penultimate :=
  p.notNilRec (by simp) hp h

@[simp]
lemma adj_penultimate {p : Walk G v w} (hp : ¬ p.Nil) :
