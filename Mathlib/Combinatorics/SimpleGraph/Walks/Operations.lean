...
open Function DartLike SimpleGraph
...
lemma ext_support {u v} {p q : Walk G u v} (h : p.support = q.support) : p = q := by
  ...
  have : Prod.fst ∘ Subtype.val = fun d : GraphLike.darts G ↦ fst d.val := rfl
  have : Prod.snd ∘ Subtype.val = fun d : GraphLike.darts G ↦ DartLike.snd d.val := rfl
...
theorem support_dropLast_concat {p : Walk G u v} (hp : ¬p.Nil) :
  ...
  rw [← List.concat_eq_append, ← support_concat _ <| step_penultimate hp, concat_dropLast]
