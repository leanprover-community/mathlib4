...
theorem copy_cons {u v w u' w'} (h : G.Adj u v) (p : Walk G v w) (hu : u = u') (hw : w = w') :
    (Walk.cons h p).copy hu hw = Walk.cons (hu ▸ h) (p.copy rfl hw) := by
  subst_vars
  rfl

@[simp]
theorem cons_copy {u v w v' w'} (h : G.Adj u v) (p : Walk G v' w') (hv : v' = v) (hw : w' = w) :
...
the end of a walk. -/
def concat {u v w : V} (p : Walk G u v) (h : G.Adj v w) : Walk G u w := p.append (cons h nil)

theorem concat_eq_append {u v w : V} (p : Walk G u v) (h : G.Adj v w) :
...
@[simp]
theorem cons_append {u v w x : V} (h : G.Adj u v) (p : Walk G v w) (q : Walk G w x) :
    (cons h p).append q = cons h (p.append q) := rfl

@[simp]
theorem cons_nil_append {u v w : V} (h : G.Adj u v) (p : Walk G v w) :
...
@[simp]
theorem concat_cons {u v w x : V} (h : G.Adj u v) (p : Walk G v w) (h' : G.Adj w x) :
    (cons h p).concat h' = cons h (p.concat h') := rfl

theorem append_concat {u v w x : V} (p : Walk G u v) (q : Walk G v w) (h : G.Adj w x) :
    p.append (q.concat h) = (p.append q).concat h := append_assoc _ _ _

theorem concat_append {u v w x : V} (p : Walk G u v) (h : G.Adj v w) (q : Walk G w x) :
    (p.concat h).append q = p.append (cons h q) := by
  rw [concat_eq_append, ← append_assoc, cons_nil_append]

/-- A non-trivial `cons` walk is representable as a `concat` walk. -/
theorem exists_cons_eq_concat {u v w : V} (h : G.Adj u v) (p : Walk G v w) :
    ∃ (x : V) (q : Walk G u x) (h' : G.Adj x w), cons h p = q.concat h' := by
...
theorem exists_concat_eq_cons {u v w : V} :
    ∀ (p : Walk G u v) (h : G.Adj v w),
      ∃ (x : V) (h' : G.Adj u x) (q : Walk G x w), p.concat h = cons h' q
  | nil, h => ⟨_, h, nil, rfl⟩
...
@[simp]
theorem reverse_cons {u v w : V} (h : G.Adj u v) (p : Walk G v w) :
...
@[simp]
theorem reverse_concat {u v w : V} (p : Walk G u v) (h : G.Adj v w) :
...
@[simp]
theorem length_concat {u v w : V} (p : Walk G u v) (h : G.Adj v w) :
...
variable {motive : ∀ u v : V, Walk G u v → Sort*} (Hnil : ∀ {u : V}, motive u u nil)
  (Hconcat : ∀ {u v w : V} (p : Walk G u v) (h : G.Adj v w), motive u v p → motive u w (p.concat h))
...
@[simp]
theorem concatRec_concat {u v w : V} (p : Walk G u v) (h : G.Adj v w) :
...
theorem concat_ne_nil {u v : V} (p : Walk G u v) (h : G.Adj v u) : p.concat h ≠ nil := by
  cases p <;> simp [concat]

theorem concat_inj {u v v' w : V} {p : Walk G u v} {h : G.Adj v w} {p' : Walk G u v'}
...
@[simp]
theorem support_concat {u v w : V} (p : Walk G u v) (h : G.Adj v w) :
...
theorem support_eq_concat {u v : V} (p : Walk G u v) : p.support = p.support.dropLast.concat v := by
  cases p with
  | nil => rfl
  | cons h p =>
...
theorem support_subset_support_concat {u v w : V} (p : Walk G u v) (hadj : G.Adj v w) :
...
@[simp]
theorem darts_concat {u v w : V} (p : Walk G u v) (h : G.Adj v w) :
...
@[simp]
theorem edges_concat {u v w : V} (p : Walk G u v) (h : G.Adj v w) :
...
@[simp]
theorem edgeSet_concat {u v w : V} (p : Walk G u v) (h : G.Adj v w) :
...
lemma drop_cons_eq (h : G.Adj u v) (p : Walk G v w) (n : ℕ) (hn : n ≠ 0) :
...
lemma take_cons_eq (h : G.Adj u v) (p : Walk G v w) (n : ℕ) (hn : n ≠ 0) :
...
@[simp]
lemma penultimate_concat {t u v} (p : Walk G u v) (h : G.Adj v t) :
...
@[simp]
lemma tail_nil : (@nil _ _ _ G v).tail = .nil := rfl

@[simp]
lemma tail_cons_nil (h : G.Adj u v) : (Walk.cons h .nil).tail = .nil := rfl

@[simp]
lemma tail_cons (h : G.Adj u v) (p : Walk G v w) :
...
@[simp]
lemma dropLast_nil : (@nil _ _ _ G v).dropLast = nil := rfl

@[simp]
lemma dropLast_cons_nil (h : G.Adj u v) : (cons h nil).dropLast = nil := rfl

@[simp]
lemma dropLast_cons_cons {w'} (h : G.Adj u v) (h₂ : G.Adj v w) (p : Walk G w w') :
    (cons h (cons h₂ p)).dropLast = cons h (cons h₂ p).dropLast := rfl

lemma dropLast_cons_of_not_nil (h : G.Adj u v) (p : Walk G v w) (hp : ¬ p.Nil) :
    (cons h p).dropLast = cons h (p.dropLast.copy rfl (penultimate_cons_of_not_nil _ _ hp).symm) :=
  p.notNilRec (by simp) hp h

@[simp]
lemma dropLast_concat {t u v} (p : Walk G u v) (h : G.Adj v t) :
...
@[simp]
lemma concat_dropLast (p : Walk G u v) (hp : G.Adj p.penultimate v) :
