...
private lemma delete_incidence_set_is_bipartite_with_of_is_bipartite_with {s t : Finset V} {a : V}
    ...
    (G.deleteIncidenceSet a).IsBipartiteWith ↑s ↑t :=
  ⟨by grind [hG.disjoint], fun _ _ h ↦ by grind [G.deleteIncidenceSet_adj.mp h, hG.mem_of_adj]⟩

private lemma edgeSet_decompose (a : V) :
    G.edgeSet = (G.deleteIncidenceSet a).edgeSet ∪ G.incidenceSet a := by
  rw [edgeSet_deleteIncidenceSet, Set.diff_union_self]
  exact (Set.union_eq_self_of_subset_right (incidenceSet_subset G a)).symm

private lemma disjoint_edgeSet_decompose (a : V) :
    Disjoint (G.deleteIncidenceSet a).edgeSet (G.incidenceSet a) := by
  rw [Set.disjoint_iff_inter_eq_empty]
  ext e
  cases e with | h x y =>
  simp only [deleteIncidenceSet, edgeSet_deleteEdges, Set.diff_inter_self, Set.mem_empty_iff_false]

private lemma incidenceSet_ncard_le {s t : Finset V} {a : V}
    (hG : G.IsBipartiteWith (insert a ↑s) ↑t) :
    (G.incidenceSet a).Finite ∧ (G.incidenceSet a).ncard ≤ t.card := by
  have h₁ : G.incidenceSet a ⊆ (fun u ↦ s(u, a)) '' { x | x ∈ t ∧ G.Adj a x} := by
    intros e he
    cases e with | h x y =>
    simp only [incidenceSet, Set.mem_setOf_eq, mem_edgeSet, Sym2.mem_iff, Set.mem_image, Sym2.eq,
      Sym2.rel_iff', Prod.mk.injEq, Prod.swap_prod_mk] at *
    obtain ⟨hxy, hx | hy⟩ := he
    · use y
      simp only [hx, hxy, and_true, and_self, or_true]
      have h₀ := hG.disjoint (x := {x})
      have h₁ := hG.mem_of_adj hxy
      simp only [Set.le_eq_subset, Set.singleton_subset_iff, Set.mem_insert_iff, SetLike.mem_coe,
        Set.bot_eq_empty, Set.subset_empty_iff, Set.singleton_ne_empty, imp_false] at h₀ h₁
      grind
    · use x
      simp only [hy, hxy.symm, and_self, true_or, and_true]
      have h₀ := hG.disjoint (x := {y})
      have h₁ := hG.mem_of_adj hxy
      simp only [Set.le_eq_subset, Set.singleton_subset_iff, Set.mem_insert_iff, SetLike.mem_coe,
        Set.bot_eq_empty, Set.subset_empty_iff, Set.singleton_ne_empty, imp_false] at h₀ h₁
      grind
  have h₂ : {x | x ∈ t ∧ G.Adj a x}.Finite := Set.Finite.inter_of_left (finite_toSet t) (G.Adj a)
  have h₃ : (G.incidenceSet a).Finite := Set.Finite.of_surjOn (fun u ↦ s(u, a)) h₁ h₂
  have h₄ : (G.incidenceSet a).ncard ≤ ((fun u ↦ s(u, a)) '' {x | x ∈ t ∧ G.Adj a x}).ncard :=
    Set.ncard_le_ncard h₁ (Set.Finite.image (fun u ↦ s(u, a)) h₂)
  have h₅ : ((fun u ↦ s(u, a)) '' {x | x ∈ t ∧ G.Adj a x}).ncard =
            {x | x ∈ t ∧ G.Adj a x}.ncard := by
    simp [Set.ncard_image_iff h₂]
  have h₆ : ({x | x ∈ t ∧ G.Adj a x}).ncard ≤ (t : Set V).ncard :=
    Set.ncard_inter_le_ncard_left _ _ (Set.finite_mem_finset t)
...
theorem IsBipartiteWith.encard_edgeSet_le {s t : Set V} (h : G.IsBipartiteWith s t) :
  ...
  · obtain ⟨hs, ht⟩ := hst
    ...
    rw [h₂, h₃, h₄]
    norm_cast
  · obtain hs | ht : s.Infinite ∨ t.Infinite := by tauto
    · simp only [hs, Set.Infinite.encard_eq]
      by_cases ht₀ : t.encard = 0
      · simp only [ht₀, mul_zero, nonpos_iff_eq_zero, Set.encard_eq_zero, edgeSet_eq_empty]
        ext x y
        simp only [bot_adj, iff_false]
        intro hxy
        apply h.mem_of_adj at hxy
        simp only [Set.encard_eq_zero] at ht₀
        simp [ht₀] at hxy
      · simp [ht₀]
    · simp only [ht, Set.Infinite.encard_eq]
      ...
      · simp only [hs₀, zero_mul, nonpos_iff_eq_zero, Set.encard_eq_zero, edgeSet_eq_empty]
        ...
        intro hxy
        apply h.mem_of_adj at hxy
        simp only [Set.encard_eq_zero] at hs₀
        simp [hs₀] at hxy
      · simp [hs₀]
