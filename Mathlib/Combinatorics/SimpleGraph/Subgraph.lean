...
def ofFunction {u : Set V} (f : V → V) (h : ∀ v ∈ u, G.Adj v (f v)) : Subgraph G :=
  G.toSubgraph (SimpleGraph.ofFunction f (fun v hv ↦ (h v hv).ne.symm ) ⊓ G) inf_le_right

@[simp]
def ofFunction_verts {u : Set V} (f : V → V) (h : ∀ v ∈ u, G.Adj v (f v)) :
    (Subgraph.ofFunction f h).verts = Set.univ := by
  unfold ofFunction
  rw [toSubgraph_verts]
