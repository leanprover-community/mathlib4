...
theorem getVert_adj_exists_of_toSubgraph_adj {u v u' v'} (w : G.Walk u v)
    (hadj : (w.toSubgraph).Adj u' v') : ∃ i, s(w.getVert i, w.getVert (i + 1)) =
    s(u', v') ∧ i < w.length := by
  ...
  | .cons h p =>
    ...
    | inr hr =>
      obtain ⟨i, hi⟩ := getVert_adj_exists_of_toSubgraph_adj _ hr
      use (i + 1)
