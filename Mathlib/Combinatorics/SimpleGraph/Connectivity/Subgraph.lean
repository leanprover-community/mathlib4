...
theorem toSubgraph_adj_penultimate {u v} (w : G.Walk u v) (h : ¬ w.Nil) :
  ...
  simpa [show w.length - 1 + 1 = w.length from by grind]
    using w.toSubgraph_adj_getVert (by grind : w.length - 1 < w.length)
...
lemma neighborSet_toSubgraph_internal {u} {i : ℕ} {p : G.Walk u v} (hp : p.IsPath)
  ...
  have hadj1 := ((show i - 1 + 1 = i from by grind) ▸
    p.toSubgraph_adj_getVert (by grind : (i - 1) < p.length)).symm
  ext v
  ...
  refine ⟨?_, by aesop⟩
  rintro ⟨i', ⟨hl, _⟩ | ⟨_, hl⟩⟩ <;>
    apply hp.getVert_injOn (by rw [Set.mem_setOf_eq]; grind)
      (by rw [Set.mem_setOf_eq]; grind) at hl <;> aesop
...
lemma ncard_neighborSet_toSubgraph_internal_eq_two {u} {i : ℕ} {p : G.Walk u v} (hp : p.IsPath)
  ...
  have : p.getVert (i - 1) ≠ p.getVert (i + 1) := by
    intro h
    have := hp.getVert_injOn (by rw [Set.mem_setOf_eq]; grind) (by rw [Set.mem_setOf_eq]; grind) h
    grind
  simp_all
...
lemma neighborSet_toSubgraph_endpoint {u} {p : G.Walk u u} (hpc : p.IsCycle) :
  ...
  refine ⟨?_, by aesop⟩
  rintro ⟨i, hl | hr⟩
  · rw [hpc.getVert_endpoint_iff (by grind)] at hl
    cases hl.1 <;> aesop
  · rcases (hpc.getVert_endpoint_iff (by grind)).mp hr.2 with h1 | h2
    · contradiction
...
lemma neighborSet_toSubgraph_internal {u} {i : ℕ} {p : G.Walk u u} (hpc : p.IsCycle)
  ...
  · apply hpc.getVert_injOn' (by rw [Set.mem_setOf_eq]; grind)
      (by rw [Set.mem_setOf_eq]; grind) at hl1
    simp_all
  · apply hpc.getVert_injOn (by rw [Set.mem_setOf_eq]; grind)
      (by rw [Set.mem_setOf_eq]; grind) at hr2
    aesop
...
lemma ncard_neighborSet_toSubgraph_eq_two {u v} {p : G.Walk u u} (hpc : p.IsCycle)
  ...
  push_neg at he
  rw [← hi.1, hpc.neighborSet_toSubgraph_internal he.1 (by grind)]
  exact Set.ncard_pair (hpc.getVert_sub_one_ne_getVert_add_one (by grind))
