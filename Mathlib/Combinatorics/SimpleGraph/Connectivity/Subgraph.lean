...
lemma IsCycle.neighborSet_toSubgraph_endpoint {u : V} {p : G.Walk u u} (hpc : p.IsCycle) :
...
lemma IsCycle.neighborSet_toSubgraph_internal {u : V} {i : ℕ} {p : G.Walk u u} (hpc : p.IsCycle)
    (h : i ≠ 0) (h' : i < p.length) :
