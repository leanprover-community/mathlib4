...
theorem biSup_fromEdgeSet_singleton : G = ⨆ e ∈ G.edgeSet, fromEdgeSet {e} := by
  ...
  exact Set.disjoint_left.mpr G.edgeSet_subset_compl_diagSet |>.sdiff_eq_left.symm

theorem sSup_edge : G = sSup { edge u v | (u : V) (v : V) (_ : G.Adj u v) } := by
  apply G.biSup_fromEdgeSet_singleton.trans
