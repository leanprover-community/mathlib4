...
theorem chromaticNumber_le_two_iff_bipartite {V : Type*} {G : SimpleGraph V} :
...
theorem chromaticNumber_eq_two_iff {V : Type*} {G : SimpleGraph V} :
  ...
  ⟨fun h ↦ ⟨chromaticNumber_le_two_iff_bipartite.mp (by simp [h]),
            two_ge_chromaticNumber_iff_ne_bot.mp (by simp [h])⟩,
   fun ⟨h₁, h₂⟩ ↦ ENat.eq_of_forall_natCast_le_iff fun _ ↦
      ⟨fun h ↦ Std.le_trans h (chromaticNumber_le_two_iff_bipartite.mpr h₁),
       fun h ↦ Std.le_trans h (two_ge_chromaticNumber_iff_ne_bot.mpr h₂)⟩⟩
