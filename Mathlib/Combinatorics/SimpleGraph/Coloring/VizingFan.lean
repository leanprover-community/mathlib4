...
lemma missingColors_ncard_ge
  ...
  -- |incidentColors| + |missingColors| = card α, and |incidentColors| ≤ deg v.
...
/-- `w` extends a fan with last vertex `vk` if `c(u, w) ∈ missingColors c vk`. -/
...
/-- Induction on fan length for `rotate_termA`. -/
private lemma rotate_termA_aux
  ...
  induction n with
  | zero =>
    -- Base case: l.length = 1, so l = [v_only].
    ...
    subst v_only
    apply extend_coloring_with_missing_both e_uv he_uv c γ h_γ_u
    simpa using h_γ_vk
  | succ k ih =>
    intro c l h_ne h_len h_fan γ h_γ_u h_γ_vk
    -- l has length k + 2 ≥ 2.
    ...
    -- SUB-CLAIM 1: extract the last fan edge `e_k` (between u and v_k)
    -- with `c(e_k) ∈ missingColors c v_{k-1}`. Proof: decompose
    -- l = l.dropLast ++ [v_k]; apply `chain'_append` to get the
    -- boundary relation between v_{k-1} (the last of dropLast) and v_k.
    have h_last_edge : ∃ e_k : (G.deleteEdges {e_uv.val}).edgeSet,
      ...
      -- h_rel : ∀ x ∈ l.dropLast.getLast?, ∀ y ∈ [v_k].head?, ...
    ...
    obtain ⟨c'', h_c''_e_k, h_c''_other⟩ :=
      recolor_one_edge_with_missing c e_k he_k_val γ h_γ_u h_γ_vk
...
lemma IsFan.rotate_termA
  ...
  have h_len : l.length = (l.length - 1) + 1 := by
    ...
    omega
  exact rotate_termA_aux u v e_uv he_uv (l.length - 1) c l h_ne h_len h_fan γ
    h_γ_u h_γ_vk

/-! ### Maximal fans -/

set_option linter.unusedDecidableInType false in
/-- Every fan extends to a maximal fan (induction on `Fintype.card V - l.length`). -/
private lemma exists_max_fan_of_fan
    [Fintype V] [DecidableEq V] [DecidableRel G.Adj]
    ...
    · obtain ⟨w, hw_notin, hw_ext⟩ := h_ext
      ...
      have h_l'_fan : IsFan c u v (l ++ [w]) := by
        ...
        · -- Nodup
          ...
          subst hx_w
          exact hw_notin hx_l
        · -- Chain' via List.IsChain.append (constructor form)
...
· -- Case A: a common missing color exists, extend directly.
  ...
  exact extend_coloring_with_missing_both e_uv he_uv c γ hγ_u hγ_v
· -- Case B: no common missing color. The fan + Kempe argument.
  push Not at h_common
  -- Apply the dichotomy on the (already-maximal) fan `l`.
  rcases IsFan.dichotomy h_fan h_ne h_max with h_term_a | h_term_b
  · -- Term-A: a color γ is missing at both u and v_k.
    obtain ⟨γ, hγ_u, hγ_vk⟩ := h_term_a
    exact IsFan.rotate_termA e_uv he_uv h_fan h_ne γ hγ_u hγ_vk
  · -- Term-B: every colour missing at v_k is realised by an edge from
    set v_k := l.getLast h_ne with hv_k_def
    ...
    · -- Sub-case B2: `u` and `v_k` in the same `ab`-component.
      ...
      have hb_missing_vjm1 : b ∈ missingColors c v_jm1 := by
        ...
        exact he_col
      -- Three-endpoint lemma: u, v_k, v_jm1 all have degree ≤ 1 in the
      ...
      have h_not_reach_jm1 :
        ...
        have h_vk_ne_vjm1 : v_k ≠ v_jm1 := by
          ...
          -- `v_k = l.getLast = (v_j :: l_post).getLast`, hence in
          -- `v_j :: l_post`.
        ...
        -- Lower bound: 2 * (card - 1) ≤ sum deg, via tree bound and
    ...
    · -- Sub-case B1: `u` and `v_k` in different `ab`-components.
      have h_eb_inSZ : inSwapZone c a b u e_b := by
        refine ⟨Or.inr he_b_col, u, ?_, SimpleGraph.Reachable.refl u⟩
        change u ∈ e_b.val
        rw [he_b_val]; exact Sym2.mem_mk_left u v_j
      ...
      · -- Sub-case B1(b): `v_jm1` reachable from `u`. Use the full fan.
        ...
        exact IsFan.rotate_termA e_uv he_uv h_full_fan_c' h_ne b
          h_b_missing_u h_b_missing_vk
      · -- Sub-case B1(a): `v_jm1` unreachable. Use the shorter fan.
        have h_l_pre_fan_c' : IsFan (swapKempe c a b u) u v l_pre := by
          obtain ⟨h_head_l, h_nodup_l, h_chain_l⟩ := h_fan
          refine ⟨?_, ?_, ?_⟩
          · rw [hl_split] at h_head_l
...
lemma vizingAdjacencyLemma
  ...
  obtain ⟨l, h_ne, h_fan, h_max⟩ := exists_max_fan_of_fan (Fintype.card V)
