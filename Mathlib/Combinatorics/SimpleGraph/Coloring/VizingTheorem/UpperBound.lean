...
/-!
# Upper Bound in Vizing's Theorem

This file proves the upper bound on the chromatic index by induction on the
number of edges.
-/
    ...
    · -- At least one edge: remove it and apply the induction hypothesis.
      ...
      have h_c'_nonempty :
        ...
        have h_chi_ne_top :
          ...
          exact ENat.natCast_ne_top _ h_chi_le_card
        have h_chi_le :
            (H.deleteEdges {e.val}).lineGraph.chromaticNumber ≤
              ((H.maxDegree + 1 : ℕ) : ℕ∞) := by
          rw [← ENat.natCast_toNat h_chi_ne_top]; exact_mod_cast h_toNat_le
      ...
      | inl h_color_free =>
        ...
        exact ENat.toNat_le_of_le_natCast h_col.chromaticNumber_le
        ...
        have h_missing_u_card :
          ...
          have h_erase :
            ...
            simp only [SimpleGraph.mem_incidenceFinset, SimpleGraph.incidenceSet,
                       SimpleGraph.edgeSet_deleteEdges, Set.mem_ofPred_eq,
