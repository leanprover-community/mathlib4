...
/--
A `ColoringOn β s` of `G : SimpleGraph α` is a `β`-coloring of all vertices of `G` that is
a valid coloring on the set `s` -/
abbrev ColoringOn (β : Type*) (s : Set α) := ((⊤ : Subgraph G).induce s).spanningCoe.Coloring β

/-- `G.ColorableOn n s` iff the subgraph of `G` induced by `s` is `n` - colorable. -/
abbrev ColorableOn (n : ℕ) (s : Set α) := Nonempty (G.ColoringOn (Fin n) s)

variable {s t u : Set α} {β : Type*} {G}
/--
`C₂ : G.ColoringOn β t` extends `C₁ : G.ColoringOn β s` if `s ⊆ t` and `C₂` agrees with `C₁`
on `s`
-/
protected def ColoringOn.Extends (C₂ : G.ColoringOn β t) (C₁ : G.ColoringOn β s) : Prop :=
  s ⊆ t ∧ ∀ ⦃v⦄, v ∈ s → C₂ v = C₁ v

namespace ColoringOn

@[refl, simp]
lemma extends_refl {C₁ : G.ColoringOn β s} : C₁.Extends C₁ := ⟨subset_refl _, fun _ _ ↦ rfl⟩

@[trans, simp]
lemma extends_trans {C₃ : G.ColoringOn β u} {C₂ : G.ColoringOn β t} {C₁ : G.ColoringOn β s}
    (h1 : C₂.Extends C₁) (h2: C₃.Extends C₂) : C₃.Extends C₁ := by
  refine ⟨subset_trans h1.1 h2.1,?_⟩
  intro v hv
  rw [← h1.2 hv, h2.2 (h1.1 hv)]

/-- Construct a `G.ColoringOn β t` from `C : G.ColoringOn β s` and proof that `s = t` -/
@[simp]
def copy (C : G.ColoringOn β s) (h : s = t) : G.ColoringOn β t where
  toFun := C.toFun
  map_rel' := by
    subst h
    exact C.map_rel'

theorem copy_rfl  (C : G.ColoringOn β s)  : C.copy rfl = C := rfl

theorem copy_copy {s t u} (C : G.ColoringOn β s) (hs : s = t) (ht : t = u) :
    (C.copy hs).copy ht = C.copy (hs.trans ht) := by
  subst_vars
  rfl

lemma copy_def (C: G.ColoringOn β s) (h : s = t) {v : α} :
  (C.copy h) v  = C v := rfl

lemma copy_extends {C₂ : G.ColoringOn β t} {C₁ : G.ColoringOn β s} (hc : C₂.Extends C₁)
  {h : t = u} : (C₂.copy h).Extends C₁ :=
    ⟨fun _ hx ↦ h ▸ hc.1 hx, fun _ hv ↦ by rw [copy_def]; exact hc.2 hv⟩

/-- A `G.ColoringOn β Set.univ` is a `G.Coloring (Fin n)` -/
def toColoring (C : G.ColoringOn β Set.univ) : G.Coloring β :=
...
def partColoringOfSingleton {β : Type*} (a : α) (c : β) : G.ColoringOn β ({a} : Set α) where
  toFun := fun _ ↦ c
  map_rel':= by simp
...
def partColoringOfNotAdj {β : Type*} {a b : α} (h : ¬ G.Adj a b) (c : β) :
    G.ColoringOn β ({a, b} : Set α) where
  toFun := fun _ ↦ c
  map_rel':= by
    intro x y hadj he
...
-/
def ColoringOn.union (C₁ : G.ColoringOn β s) (C₂ : G.ColoringOn β t)
    (h : ∀ ⦃v w⦄, v ∈ s → w ∈ t \ s → G.Adj v w → C₁ v ≠ C₂ w) : G.ColoringOn β (s ∪ t) where
...
@[simp]
lemma ColoringOn.union_def {v : α} (C₁ : G.ColoringOn β s) (C₂ : G.ColoringOn β t)
    (h : ∀ ⦃v w⦄, v ∈ s → w ∈ t \ s → G.Adj v w → C₁ v ≠ C₂ w) :
  (C₁.union C₂ h) v = ite (v ∈ s) (C₁ v) (C₂ v) := rfl

@[simp]
lemma ColoringOn.union_extends (C₁ : G.ColoringOn β s) (C₂ : G.ColoringOn β t)
    (h : ∀ ⦃v w⦄, v ∈ s → w ∈ t \ s → G.Adj v w → C₁ v ≠ C₂ w) : (C₁.union C₂ h).Extends C₁ :=
  ⟨Set.subset_union_left, fun _ hv ↦ by rw [union_def, if_pos hv]⟩

@[simp]
lemma ColoringOn.union_extends_disjoint (hd : Disjoint s t) (C₁ : G.ColoringOn β s)
    (C₂ : G.ColoringOn β t) (h : ∀ ⦃v w⦄, v ∈ s → w ∈ t \ s → G.Adj v w → C₁ v ≠ C₂ w) :
...
protected def ColoringOn.insert (a : α) {c : β} (C : G.ColoringOn β s)
    (h : ∀ ⦃v⦄, v ∈ s → G.Adj a v → C v ≠ c) : G.ColoringOn β (insert a s) :=
...
@[simp]
lemma ColoringOn.insert_def (C : G.ColoringOn β s) (h : ∀ ⦃x⦄, x ∈ s → G.Adj a x → C x ≠ c) :
    (C.insert a h) v = ite (v = a) c (C v) := by
  rw [ColoringOn.insert, copy_def, union_def]
  simp

@[simp]
lemma ColoringOn.insert_extends {c : β} (C : G.ColoringOn β s)
    (h : ∀ ⦃x⦄, x ∈ s → G.Adj a x → C x ≠ c) :
    (C.insert a h).Extends (G.partColoringOfSingleton a c) := copy_extends (union_extends ..)

@[simp]
lemma ColoringOn.insert_extends_not_mem (C : G.ColoringOn β s)
