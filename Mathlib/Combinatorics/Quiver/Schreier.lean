...
The symmetrification adds a reverse edge for each forward edge, allowing us to represent
paths that use inverses of generators. Reachability in this symmetrified quiver characterises
the orbit of the subgroup generated by `ι`.
-/
...
def pathFromWord (x : SchreierGraph V ι) : ∀ w : List (S × Bool),
  ...
  | [] => by
      ...
      exact Path.nil
  | (s, true) :: w => by
      have rest := pathFromWord x w
      have edge : (evalWord ι w • x) ⟶ (ι s • (evalWord ι w • x)) := ⟨s, rfl⟩
      have endpoint_eq :
          ι s • (evalWord ι w • x) = evalWord ι ((s, true) :: w) • x := by
        rw [evalWord_cons_true, mul_smul]
      exact (rest.cons (Sum.inl edge)).cast rfl endpoint_eq
  | (s, false) :: w => by
      have rest := pathFromWord x w
      have edge : ((ι s)⁻¹ • (evalWord ι w • x)) ⟶ (evalWord ι w • x) :=
        ⟨s, by simp [← mul_smul]⟩
      have endpoint_eq :
          (ι s)⁻¹ • (evalWord ι w • x) = evalWord ι ((s, false) :: w) • x := by
        rw [evalWord_cons_false, mul_smul]
      exact (rest.cons (Sum.inr edge)).cast rfl endpoint_eq
