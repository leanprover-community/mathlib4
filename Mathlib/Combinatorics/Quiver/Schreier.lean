...
variable (V : Type*) {M : Type*} [Monoid M] [MonoidAction M V] {S : Type*} (ι : S → M)

/-- The monoid acts on the vertices of the Schreier graph. -/
instance : MonoidAction M (SchreierGraph V ι) where
  one_smul x := by
    ext
...
variable {V : Type*} {M : Type*} [Group M] [MonoidAction M V] {S : Type*} (ι : S → M)
...
lemma map_smul_of_comp_labelling_eq {W : Type*} [MonoidAction M W]
