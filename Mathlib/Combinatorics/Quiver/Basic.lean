...
attribute [order_dual self (reorder := 3 4)] Quiver.Hom
...
@[simps, order_dual self (reorder := 3 4)]
...
@[simp, order_dual self (reorder := 2 3)]
theorem empty_arrow {V : Type u} (a b : Empty V) : (a ⟶ b) = PEmpty := rfl

/-- A quiver is thin if it has no parallel arrows. -/
@[order_dual IsThin' "`isThin'` is equivalent to `IsThin`.
It is used by the `order_dual` attribute"]
...
objects. -/
@[order_dual self (reorder := 3 4, 6 7, 8 9)]
def homOfEq {X Y : V} (f : X ⟶ Y) {X' Y' : V} (hX : X = X') (hY : Y = Y') : X' ⟶ Y' := by
  subst hX hY
  exact f

@[simp, order_dual self (reorder := 3 4, 6 7, 8 9, 10 11, 12 13)]
...
@[order_dual self (reorder := 3 4, 5 6, 7 8)]
lemma homOfEq_injective {X Y X' Y' : V} (hX : X = X') (hY : Y = Y')
