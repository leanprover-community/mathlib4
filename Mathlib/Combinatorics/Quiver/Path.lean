...
lemma length_ne_zero_iff_eq_comp (p : Path a b) :
  ...
  refine ⟨fun h ↦ ?_, ?_⟩
  · have h_len : p.length = (p.length - 1) + 1 := by grind
  ...
  fun ⟨p, hp⟩ ⟨q, hq⟩ =>
    ...
    | _, _, .cons (b := v') p' α, .cons (b := v'') q' β =>
      ...
      | _, _, isTrue (Eq.refl _) =>
        if h : α = β then
          have hp' : p'.length ≤ n := by simp [Quiver.Path.length] at hp; grind
          have hq' : q'.length ≤ n := by simp [Quiver.Path.length] at hq; grind
