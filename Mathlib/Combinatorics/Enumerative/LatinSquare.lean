...
lemma sum_card_eq_sum_card_fiber_biUnion
    ...
    ∑ x ∈ (s.biUnion B), Finset.card {j | j ∈ s ∧ x ∈ B j} := by
      ...
      haveI : DecidableRel r := fun j x => inferInstanceAs (Decidable (x ∈ B j))
      have h := Finset.sum_card_bipartiteAbove_eq_sum_card_bipartiteBelow (α := ι) (β := α)
      have g := h r (s := s) (t := s.biUnion B)
