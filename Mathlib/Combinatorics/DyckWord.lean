...
def DyckPath := List Step deriving Append
...
lemma length_even : ∃ k, p.length = 2 * k := by
  use p.count U
  rw [← count_U_add_count_D_eq_length, h.1, two_mul]
...
lemma count_U_eq_count_D_of_firstReturn :
  ...
  have := findIdx_get (w := (length_range p.length).symm ▸ p.firstReturn_lt_length h hl)
...
lemma get_firstReturn_eq_D : p.get ⟨p.firstReturn, p.firstReturn_lt_length h hl⟩ = D := by
  have eq := p.count_U_eq_count_D_of_firstReturn h hl
  have le := h.2 p.firstReturn
  have frl := p.firstReturn_lt_length h hl
...
lemma firstReturn_pos : 0 < p.firstReturn := by
  ...
  have eqD := p.get_firstReturn_eq_D h hl
  have eqU := p.head_U h hl
...
theorem leftPart_isBalanced : p.leftPart.IsBalanced := by
  ...
  rw [take_succ, get?_eq_get frl, Option.to_list_some, count_append, count_append,
    p.get_firstReturn_eq_D h hl, p.take_eq_U_cons_tail h hl (p.firstReturn_pos h hl)] at eq
  simp only [count_cons, count_nil, ite_true, ite_false, zero_add, add_zero, add_left_inj] at eq
  refine' ⟨eq, fun i ↦ _⟩
  have lt := p.count_D_lt_count_U_of_lt_firstReturn h
  unfold leftPart
  cases' lt_or_ge i p.firstReturn with e e
  · replace lt := lt i e
    rw [p.take_eq_U_cons_tail h hl (by omega)] at lt
    simp only [count_cons, ite_true, ite_false, add_zero, Nat.lt_add_one_iff,
      p.take_tail_comm h hl] at lt
    rw [p.take_tail_take_of_lt h hl e]
    exact lt
  · rw [take_all_of_le, eq]
    rw [length_tail, length_take, min_eq_left (p.firstReturn_lt_length h hl).le]
    omega

theorem eq_U_leftPart_D_rightPart : p = ↑[U] ++ p.leftPart ++ ↑[D] ++ p.rightPart := by
  nth_rw 1 [← p.take_append_drop (p.firstReturn + 1)]; congr
  rw [take_succ, get?_eq_get (p.firstReturn_lt_length h hl), Option.to_list_some,
    p.get_firstReturn_eq_D h hl]; congr
  rw [p.take_eq_U_cons_tail h hl (p.firstReturn_pos h hl), singleton_append]; rfl

theorem leftPart_length_lt : p.leftPart.length < p.length := by
  conv_rhs => rw [p.eq_U_leftPart_D_rightPart h hl]
...
theorem rightPart_isBalanced : p.rightPart.IsBalanced := by
  ...
  have ceq : (↑[U] ++ p.leftPart ++ ↑[D]).count U = (↑[U] ++ p.leftPart ++ ↑[D]).count D := by
    ...
    simp only [(p.leftPart_isBalanced h hl).1, count_singleton', ite_true, ite_false]
    omega
  rw [p.eq_U_leftPart_D_rightPart h hl, count_append _ _ p.rightPart,
    count_append _ _ p.rightPart, ceq, add_right_inj] at h1
  refine' ⟨h1, fun i ↦ _⟩
  rw [p.eq_U_leftPart_D_rightPart h hl] at h2
...
variable {q r : DyckPath} (hq : IsBalanced q)

theorem compose_firstReturn : (↑[U] ++ q ++ ↑[D] ++ r).firstReturn = q.length + 1 := by
  rw [firstReturn]
  ...
  · intro j hji
    ...
    have l1 : j + 1 - (↑[U] ++ q).length = 0 := by rw [length_append, length_singleton]; omega
  ...
  · simp only [get_range, decide_eq_true_eq]
    rw [take_append_eq_append_take, length_append, length_append, length_singleton,
      length_singleton, show q.length + 1 + 1 - (1 + q.length + 1) = 0 by omega, take_zero,
      append_nil, take_all_of_le]
    iterate 4 rw [count_append]
    simp_rw [count_singleton', ite_true, ite_false, hq.1]; omega
    rw [length_append, length_append, length_singleton, length_singleton]; omega

theorem compose_leftPart_eq_leftPart : (↑[U] ++ q ++ ↑[D] ++ r).leftPart = q := by
  rw [leftPart, compose_firstReturn hq, append_assoc, take_append_eq_append_take,
    length_append, length_singleton, show q.length + 1 - (1 + q.length) = 0 by omega, take_zero,
...
theorem compose_rightPart_eq_rightPart : (↑[U] ++ q ++ ↑[D] ++ r).rightPart = r := by
  rw [rightPart, compose_firstReturn hq, drop_append_eq_append_drop, length_append, length_append,
    length_singleton, length_singleton, show q.length + 1 + 1 - (1 + q.length + 1) = 0 by omega,
...
theorem length_eq_two_mul_iff_numNodes_eq {n : ℕ} (st : { p : DyckPath // p.IsBalanced }) :
  ...
  constructor <;> intro h
  · have ll := p.leftPart_length_lt bal j
    have lr := p.rightPart_length_lt bal j
    rw [le, h, mul_lt_mul_left zero_lt_two] at ll
    rw [re, h, mul_lt_mul_left zero_lt_two] at lr
    have tl := (ih sl ll ⟨_, bl⟩).mp le
    have tr := (ih sr lr ⟨_, br⟩).mp re
    ...
    conv at h =>
      rw [p.eq_U_leftPart_D_rightPart bal j, length_append, length_append, length_append,
        ...
        mul_eq_mul_left_iff]
      simp only [show ¬2 = 0 by omega, or_false]
    exact h
...
-- #eval treeEquivToFun ⟨[U, U, D, D, U, D, U, U, U, D, U, D, D, D], by decide⟩
