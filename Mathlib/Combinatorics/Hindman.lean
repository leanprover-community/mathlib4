...
theorem FP.mul {M} [Semigroup M] {a : Stream' M} {m : M} (hm : m ∈ Set.ofPred (FP a)) :
    ∃ n, ∀ m' ∈ Set.ofPred (FP (a.drop n)), m * m' ∈ Set.ofPred (FP a) := by
...
theorem exists_idempotent_ultrafilter_le_FP {M} [Semigroup M] (a : Stream' M) :
    ∃ U : Ultrafilter M, U * U = U ∧ ∀ᶠ m in U, m ∈ Set.ofPred (FP a) := by
  let S : Set (Ultrafilter M) := ⋂ n, { U | ∀ᶠ m in U, m ∈ Set.ofPred (FP (a.drop n)) }
...
theorem exists_FP_of_large {M} [Semigroup M] (U : Ultrafilter M) (U_idem : U * U = U) (s₀ : Set M)
  ...
  suffices ∀ (a : Stream' M), ∀ m ∈ Set.ofPred (FP a), ∀ p, a = Stream'.corec elem succ p → m ∈ p.val by
    intro m hm
...
theorem FP_partition_regular {M} [Semigroup M] (a : Stream' M) (s : Set (Set M)) (sfin : s.Finite)
    (scov : Set.ofPred (FP a) ⊆ ⋃₀ s) : ∃ c ∈ s, ∃ b : Stream' M, Set.ofPred (FP b) ⊆ c :=
...
theorem exists_FP_of_finite_cover {M} [Semigroup M] [Nonempty M] (s : Set (Set M)) (sfin : s.Finite)
    (scov : ⊤ ⊆ ⋃₀ s) : ∃ c ∈ s, ∃ a : Stream' M, Set.ofPred (FP a) ⊆ c :=
  let ⟨U, hU⟩ :=
...
theorem FP_drop_subset_FP {M} [Semigroup M] (a : Stream' M) (n : ℕ) : Set.ofPred (FP (a.drop n)) ⊆ Set.ofPred (FP a) := by
...
@[to_additive]
theorem FP.singleton {M} [Semigroup M] (a : Stream' M) (i : ℕ) : a.get i ∈ Set.ofPred (FP a) := by
...
theorem FP.mul_two {M} [Semigroup M] (a : Stream' M) (i j : ℕ) (ij : i < j) :
    a.get i * a.get j ∈ Set.ofPred (FP a) := by
...
theorem FP.finset_prod {M} [CommMonoid M] (a : Stream' M) (s : Finset ℕ) (hs : s.Nonempty) :
    (s.prod fun i => a.get i) ∈ Set.ofPred (FP a) := by
