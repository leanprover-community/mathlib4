...
lemma adj_tgt_src (hd : d ∈ darts G) : Adj G (target G d) (source G d) :=
  symm_of (Adj G) (adj_source_target hd)

/-- The two vertices of the dart as an unordered pair. -/
@[expose] def dartSym2 (d : darts G) : Sym2 V := s(source G d.val, target G d.val)

@[simp]
theorem dartSym2_mk (h : d ∈ darts G) : dartSym2 (⟨d, h⟩ : darts G) = s(source G d, target G d) :=
  rfl
