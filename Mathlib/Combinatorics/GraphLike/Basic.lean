...
lemma adj_target_source (hd : d ∈ D(G)) : Adj G (target G d) (source G d) :=
...
@[simp]
theorem dartSym2_mk (h : d ∈ D(G)) : dartSym2 (⟨d, h⟩ : D(G)) = s(source G d, target G d) :=
  rfl
...
@[simp]
theorem dartSymm_mk (h : d ∈ D(G)) : dartSymm (⟨d, h⟩) = ⟨inv Gr d, inv_mem_darts_iff.mpr h⟩ :=
  rfl
...
theorem dartSym2_eq_iff [noMultiEdgeSymmGraphLike V D E Gr] (d₁ d₂ : D(G)) :
  ...
  rintro (⟨h1, h2⟩ | ⟨h1, h2⟩)
  · exact Or.inl <| dart_eq_of_source_target_eq h1 h2
  exact Or.inr <| dart_eq_of_source_target_eq (inv_source G q ▸ h1) (inv_target G q ▸ h2)
