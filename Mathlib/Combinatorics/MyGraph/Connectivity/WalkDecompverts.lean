...
import Mathlib.Combinatorics.MyGraph.Walkverts
...
namespace MyGraph.Walk

universe u

variable {V : Type u} {G : MyGraph V} {v w u : V}
...
The lemma `MyGraph.Walk.count_support_takeUntil_eq_one` specifies where this split occurs. -/
@[simp]
...
theorem mem_support_iff_exists_append {V : Type u} {G : MyGraph V} {u v w : V}
...
theorem mem_support_iff_exists_getVert {u v w : V} {p : G.Walk v w} :
  ...
  constructor
  · intro h
    obtain ⟨q, r, hqr⟩ := MyGraph.Walk.mem_support_iff_exists_append.mp h
    use q.length
