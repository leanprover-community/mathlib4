...
See `MyGraph.coe_finsetWalkLength_eq` for the relationship between this `Finset` and
...
context. In particular, we use this definition for `MyGraph.Path.instFintype`. -/
...
/- See `MyGraph.adjMatrix_pow_apply_eq_card_walk` for the cardinality in terms of the `n`th
...
variable {G} in
lemma disjiUnion_supp_toFinset_eq_supp_toFinset {G' : MyGraph V} (h : G ≤ G')
...
lemma ncard_oddComponents_mono [Finite V] {G' : MyGraph V} (h : G ≤ G') :
