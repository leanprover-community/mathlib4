...
import Mathlib.Combinatorics.MyGraph.Basic
...
* `MyGraph.interedges`: Finset of edges of a graph.
* `MyGraph.edgeDensity`: Edge density of a graph.
-/
...
variable (G : MyGraph α) [DecidableRel G.Adj] {s s₁ s₂ t t₁ t₂ : Finset α} {a b : α}
...
/-- Extension for the `positivity` tactic: `Rel.edgeDensity` and `MyGraph.edgeDensity` are
always nonnegative. -/
@[positivity]
unsafe def positivity_edge_density : expr → tactic strictness
  ...
  | q(MyGraph.edgeDensity $(G) $(s) $(t)) =>
    nonnegative <$> mk_mapp `` MyGraph.edgeDensity_nonneg [none, G, none, s, t]
  | e =>
    pp e >>=
      fail ∘
        format.bracket "The expression `"
          "` isn't of the form `Rel.edgeDensity r s t` nor `MyGraph.edgeDensity G s t`"
