...
* `MyGraph.Walk` (with accompanying pattern definitions
  `MyGraph.Walk.nil'` and `MyGraph.Walk.cons'`)

* `MyGraph.Walk.map` for the induced map on walks,
...
walk visits is `MyGraph.Walk.support`.

See `MyGraph.Walk.nil'` and `MyGraph.Walk.cons'` for patterns that
...
def Adj.toWalk {G : MyGraph V} {u v : V} (h : G.Adj u v) : G.Walk u v :=
...
/-- The reversed version of `MyGraph.Walk.cons`, concatenating an edge to
the end of a walk. -/
...
/-- Auxiliary definition for `MyGraph.Walk.concatRec` -/
...
/-- Recursor on walks by inducting on `MyGraph.Walk.concat`.

This is inducting from the opposite end of the walk compared
to `MyGraph.Walk.rec`, which inducts on `MyGraph.Walk.cons`. -/
@[elab_as_elim]
...
This is defined to be the list of edges underlying `MyGraph.Walk.darts`. -/
...
@[simp]
theorem head_support {G : MyGraph V} {a b : V} (p : G.Walk a b) :
    p.support.head (by simp) = a := by cases p <;> simp

@[simp]
theorem getLast_support {G : MyGraph V} {a b : V} (p : G.Walk a b) :
...
@[simp]
theorem subset_support_append_left {V : Type u} {G : MyGraph V} {u v w : V}
    (p : G.Walk u v) (q : G.Walk v w) : p.support ⊆ (p.append q).support := by
  simp only [Walk.support_append, List.subset_append_left]

@[simp]
theorem subset_support_append_right {V : Type u} {G : MyGraph V} {u v w : V}
...
@[simp]
theorem head_darts_fst {G : MyGraph V} {a b : V} (p : G.Walk a b) (hp : p.darts ≠ []) :
...
@[simp]
theorem getLast_darts_snd {G : MyGraph V} {a b : V} (p : G.Walk a b) (hp : p.darts ≠ []) :
...
protected def transfer {u v : V} (p : G.Walk u v)
    (H : MyGraph V) (h : ∀ e, e ∈ p.edges → e ∈ H.edgeSet) : H.Walk u v :=
  match p with
  | nil => nil
  | cons' u v w _ p =>
...
@[simp]
theorem transfer_transfer (hp) {K : MyGraph V} (hp') :
...
This is an abbreviation for `MyGraph.Walk.toDeleteEdges`. -/
