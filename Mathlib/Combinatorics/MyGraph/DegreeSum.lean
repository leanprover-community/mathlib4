...
- `MyGraph.sum_degrees_eq_twice_card_edges` is the degree-sum formula.
- `MyGraph.even_card_odd_degree_vertices` is the handshaking lemma.
- `MyGraph.odd_card_odd_degree_vertices_ne` is that the number of odd-degree
  vertices different from a given odd-degree vertex is odd.
- `MyGraph.exists_ne_odd_degree_of_exists_odd_degree` is that the existence of an
...
more specifically refer to `MyGraph.even_card_odd_degree_vertices`. -/
...
See `MyGraph.sum_degrees_eq_twice_card_edges` for the general version. -/
...
/-- The handshaking lemma.  See also `MyGraph.sum_degrees_eq_twice_card_edges`. -/
