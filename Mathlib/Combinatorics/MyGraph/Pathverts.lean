...
* `MyGraph.Walk.IsTrail`, `MyGraph.Walk.IsPath`, and `MyGraph.Walk.IsCycle`.

* `MyGraph.Path`

* `MyGraph.Path.map` for the induced map on paths,
  given an (injective) graph homomorphism.

* `MyGraph.Reachable` for the relation of whether there exists
  a walk between a given pair of vertices

* `MyGraph.Preconnected` and `MyGraph.Connected` are predicates
...
* `MyGraph.ConnectedComponent` is the type of connected components of
  a given graph.

* `MyGraph.IsBridge` for whether an edge is a bridge edge

## Main statements

* `MyGraph.isBridge_iff_mem_and_forall_cycle_not_mem` characterizes bridge edges in terms of
...
Use `MyGraph.Walk.IsPath.mk'` for a simpler constructor. -/
...
lemma IsPath.of_adj {G : MyGraph V} {u v : V} (h : G.Adj u v) : h.toWalk.IsPath := by
  aesop
...
lemma IsCycle.getVert_injOn {p : G.Walk u u} (hpc : p.IsCycle) :
  ...
  intro n hn m hm hnm
  rw [← MyGraph.Walk.length_tail_add_one
  ...
  have := ((Walk.cons_isCycle_iff _ _).mp hpc).1.getVert_injOn
      ...
      (by simp_all [MyGraph.Walk.getVert_tail, show n - 1 + 1 = n by omega,
...
/-- The reverse of a path is another path.  See also `MyGraph.Walk.reverse`. -/
@[symm, simps]
...
/-- Given a walk, produces a path with the same endpoints using `MyGraph.Walk.bypass`. -/
...
@[simp]
theorem mapLe_isTrail {G G' : MyGraph V} (h : G ≤ G') {u v : V} {p : G.Walk u v} :
...
@[simp]
theorem mapLe_isPath {G G' : MyGraph V} (h : G ≤ G') {u v : V} {p : G.Walk u v} :
...
@[simp]
theorem mapLe_isCycle {G G' : MyGraph V} (h : G ≤ G') {u : V} {p : G.Walk u u} :
...
protected theorem Walk.reachable {G : MyGraph V} {u v : V} (p : G.Walk u v) : G.Reachable u v :=
  ⟨p⟩
...
@[mono]
protected lemma Reachable.mono {u v : V} {G G' : MyGraph V}
...
theorem Iso.reachable_iff {G : MyGraph V} {G' : MyGraph V'} {φ : G ≃g G'} {u v : V} :
...
-- lemma mem_coe_supp_of_adj {v w : V} {H : MyGraph V} {c : ConnectedComponent H.coe}
