...
-/
import Mathlib.Combinatorics.MyGraph.Regularity.Increment
...
`Combinatorics/MyGraph/Regularity` folder:
* `Combinatorics/MyGraph/Regularity/Bound`: Definition of the bound on the number of parts.
  Numerical inequalities involving the lemma constants.
* `Combinatorics/MyGraph/Regularity/Energy`: Definition of the energy of a My graph along a
  partition.
* `Combinatorics/MyGraph/Regularity/Uniform`: Definition of uniformity of a My graph along
  a pair of parts and along a partition.
* `Combinatorics/MyGraph/Regularity/Equitabilise`: Construction of an equipartition with
  a prescribed number of parts of each size and almost refining a given partition.
* `Combinatorics/MyGraph/Regularity/Chunk`: Break up one part of the current equipartition.
  Check that density between non-uniform parts increases, and that density between uniform parts
  doesn't decrease too much.
* `Combinatorics/MyGraph/Regularity/Increment`: Gather all those broken up parts into the new
  equipartition (aka *increment partition*). Check that energy increases by at least a fixed amount.
* `Combinatorics/MyGraph/Regularity/Lemma`: Wrap everything up into an induction on the energy.
...
variable {α : Type*} [DecidableEq α] [Fintype α] (G : MyGraph α) [DecidableRel G.Adj] {ε : ℝ}
  {l : ℕ}
