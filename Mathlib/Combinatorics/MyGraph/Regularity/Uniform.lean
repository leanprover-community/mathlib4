...
* `MyGraph.IsUniform`: Graph uniformity of a pair of finsets of vertices.
* `MyGraph.nonuniformWitness`: `G.nonuniformWitness ε s t` and `G.nonuniformWitness ε t s`
...
variable (G : MyGraph α) [DecidableRel G.Adj] (ε : 𝕜) {s t : Finset α} {a b : α}
...
variable [DecidableEq α] {A : Finset α} (P : Finpartition A) (G : MyGraph α)
...
theorem nonUniforms_mono {ε ε' : 𝕜} (h : ε ≤ ε') : P.nonUniforms G ε' ⊆ P.nonUniforms G ε :=
  monotone_filter_right _ fun _ => mt <| MyGraph.IsUniform.mono h
...
theorem nonUniforms_bot (hε : 0 < ε) : (⊥ : Finpartition A).nonUniforms G ε = ∅ := by
  ...
  rwa [MyGraph.isUniform_singleton]
...
lemma isUniform_one : P.IsUniform G (1 : 𝕜) := by
  ...
  refine (card_filter_le _
    (fun uv => ¬MyGraph.IsUniform G 1 (Prod.fst uv) (Prod.snd uv))).trans ?_
