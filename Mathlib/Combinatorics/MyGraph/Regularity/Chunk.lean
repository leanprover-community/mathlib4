...
-/
import Mathlib.Combinatorics.MyGraph.Regularity.Bound
import Mathlib.Combinatorics.MyGraph.Regularity.Equitabilise
import Mathlib.Combinatorics.MyGraph.Regularity.Uniform
...
variable {α : Type*} [Fintype α] [DecidableEq α] {P : Finpartition (univ : Finset α)}
  (hP : P.IsEquipartition) (G : MyGraph α) [DecidableRel G.Adj] (ε : ℝ) {U : Finset α}
...
private theorem density_sub_eps_le_sum_density_div_card [Nonempty α]
  ...
  conv_rhs => -- Porting note: LHS and RHS need separate treatment to get the desired form
    simp only [MyGraph.edgeDensity_def, sum_div, Rat.cast_div, div_div]
  conv_lhs =>
    rw [MyGraph.edgeDensity_def, MyGraph.interedges, ← sup_eq_biUnion, ← sup_eq_biUnion,
...
private theorem sum_density_div_card_le_density_add_eps [Nonempty α]
  ...
  conv_lhs => -- Porting note: LHS and RHS need separate treatment to get the desired form
    simp only [MyGraph.edgeDensity, edgeDensity, sum_div, Rat.cast_div, div_div]
  conv_rhs =>
    rw [MyGraph.edgeDensity, edgeDensity, ← sup_eq_biUnion, ← sup_eq_biUnion,
