...
* `MyGraph.IsNClique`: Predicate for a set of vertices to be an `n`-clique.
* `MyGraph.cliqueFinset`: Finset of `n`-cliques of a graph.
* `MyGraph.CliqueFree`: Predicate for a graph to have no `n`-cliques.
-/

open Finset Fintype Function MyGraph.Walk

namespace MyGraph

variable {α β : Type*} (G H : MyGraph α)
...
theorem not_cliqueFree_of_top_embedding {n : ℕ} (f : (⊤ : MyGraph (Fin n)) ↪g G) :
    ¬G.CliqueFree n := by
...
noncomputable def topEmbeddingOfNotCliqueFree {n : ℕ} (h : ¬G.CliqueFree n) :
  ...
  let f : (⊤ : MyGraph (Fin #h.choose)) ↪g G := by
    ...
    · apply ha (by simp) (by simp)
      apply (Subtype.val_injective.comp (g.toEmbedding.inj')).ne_iff.2 h'⟩
  convert f
  exact hb.symm

theorem not_cliqueFree_iff (n : ℕ) : ¬G.CliqueFree n ↔ Nonempty ((⊤ : MyGraph (Fin n)) ↪g G) :=
  ⟨fun h ↦ ⟨topEmbeddingOfNotCliqueFree h⟩, fun ⟨f⟩ ↦ not_cliqueFree_of_top_embedding f⟩

theorem cliqueFree_iff {n : ℕ} : G.CliqueFree n ↔ IsEmpty ((⊤ : MyGraph (Fin n)) ↪g G) := by
  rw [← not_iff_not, not_cliqueFree_iff, not_isEmpty_iff]

theorem not_cliqueFree_card_of_top_embedding [Fintype α] (f : (⊤ : MyGraph α) ↪g G) :
    ¬G.CliqueFree (card α) := by
  rw [not_cliqueFree_iff]
  exact ⟨(Iso.completeGraph (Fintype.equivFin α)).symm.toEmbedding.trans f⟩

@[simp]
theorem cliqueFree_bot (h : 2 ≤ n) : (⊥ : MyGraph α).CliqueFree n := by
  intro t ht
...
theorem CliqueFree.comap {H : MyGraph β} (f : H ↪g G) : G.CliqueFree n → H.CliqueFree n := by
...
/-- See `MyGraph.cliqueFree_of_chromaticNumber_lt` for a tighter bound. -/
...
protected theorem CliqueFree.replaceVertex [DecidableEq α] (h : G.CliqueFree n) (s t : α)
  ...
  · obtain ⟨x, hx⟩ := mt
    ...
    · unfold replaceVertex at hφ
      ...
      convert @hφ a b <;> simp only [← φ.apply_eq_iff_eq, MyGraph.irrefl, hx]
  · use φ
...
def CliqueFreeOn (G : MyGraph α) (s : Set α) (n : ℕ) : Prop :=
...
-- TODO: Restate using `MyGraph.IndepSet` once we have it
@[simp]
...
@[simp]
theorem cliqueSet_zero (G : MyGraph α) : G.cliqueSet 0 = {∅} := Set.ext fun s => by simp

@[simp]
theorem cliqueSet_one (G : MyGraph α) : G.cliqueSet 1 = Set.range singleton :=
  Set.ext fun s => by simp [eq_comm]

@[simp]
theorem cliqueSet_bot (hn : 1 < n) : (⊥ : MyGraph α).cliqueSet n = ∅ :=
  (cliqueFree_bot hn).cliqueSet

@[simp]
theorem cliqueSet_map (hn : n ≠ 1) (G : MyGraph α) (f : α ↪ β) :
...
variable {α : Type*} {G : MyGraph α}

/-- The maximum number of vertices in a clique of a graph `G`. -/
noncomputable def cliqueNum (G : MyGraph α) : ℕ := sSup {n | ∃ s, G.IsNClique n s}
...
structure IsMaximumClique [Fintype α] (G : MyGraph α) (s : Finset α) : Prop where
  (isClique : G.IsClique s)
...
-- def IndepSetFreeOn (G : MyGraph α) (s : Set α) (n : ℕ) : Prop :=
...
-- noncomputable def indepNum (G : MyGraph α) : ℕ := sSup {n | ∃ s, G.IsNIndepSet n s}
...
-- -/
-- @[mk_iff]
-- structure IsMaximumIndepSet [Fintype α] (G : MyGraph α) (s : Finset α) : Prop where
--   isIndepSet : G.IsIndepSet s
