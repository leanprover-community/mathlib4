...
variable {V : Type u} (G : MyGraph V) {n : ℕ}
...
-/
abbrev Coloring (α : Type v) := G →g (⊤ : MyGraph α)
...
/-- Construct a term of `MyGraph.Coloring` using a function that
assigns vertices to colors and a proof that it is as proper coloring.

(Note: this is a definitionally the constructor for `MyGraph.Hom`,
...
noncomputable instance [Fintype V] [Fintype α] : Fintype (Coloring G α) := by
  classical
  change Fintype (RelHom G.Adj (⊤ : MyGraph α).Adj)
...
lemma chromaticNumber_eq_biInf {G : MyGraph V} :
    G.chromaticNumber = ⨅ n ∈ setOf G.Colorable, (n : ℕ∞) := rfl

lemma chromaticNumber_eq_iInf {G : MyGraph V} :
    G.chromaticNumber = ⨅ n : {m | G.Colorable m}, (n : ℕ∞) := by
  rw [chromaticNumber, iInf_subtype]

lemma Colorable.chromaticNumber_eq_sInf {G : MyGraph V} {n} (h : G.Colorable n) :
...
theorem colorable_of_fintype (G : MyGraph V) [Fintype V] : G.Colorable (Fintype.card V) :=
...
theorem Colorable.of_embedding {V' : Type*} {G' : MyGraph V'} (f : G ↪g G') {n : ℕ}
...
theorem colorable_chromaticNumber_of_fintype (G : MyGraph V) [Finite V] :
    G.Colorable (ENat.toNat G.chromaticNumber) := by
  cases nonempty_fintype V
  exact colorable_chromaticNumber G.colorable_of_fintype

theorem chromaticNumber_le_one_of_subsingleton (G : MyGraph V) [Subsingleton V] :
...
theorem chromaticNumber_eq_zero_of_isempty (G : MyGraph V) [IsEmpty V] :
    G.chromaticNumber = 0 := by
  rw [← nonpos_iff_eq_zero, ← Nat.cast_zero, chromaticNumber_le_iff_colorable]
  apply colorable_of_isEmpty

theorem isEmpty_of_chromaticNumber_eq_zero (G : MyGraph V) [Finite V]
...
theorem Colorable.mono_left {G' : MyGraph V} (h : G ≤ G') {n : ℕ} (hc : G'.Colorable n) :
    G.Colorable n :=
  ⟨hc.some.comp (.ofLE h)⟩

theorem chromaticNumber_le_of_forall_imp {V' : Type*} {G' : MyGraph V'}
...
theorem chromaticNumber_bot [Nonempty V] : (⊥ : MyGraph V).chromaticNumber = 1 := by
  have : (⊥ : MyGraph V).Colorable 1 := ⟨.mk 0 <| by simp⟩
  exact this.chromaticNumber_le.antisymm <| Order.one_le_iff_pos.2 <| chromaticNumber_pos this

@[simp]
theorem chromaticNumber_top [Fintype V] : (⊤ : MyGraph V).chromaticNumber = Fintype.card V := by
