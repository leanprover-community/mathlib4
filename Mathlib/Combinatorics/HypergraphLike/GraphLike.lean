...
public import Mathlib.Combinatorics.HypergraphLike.Degree
...
@[instance_reducible]
noncomputable def GraphLike.ofEOrderEqTwo (eorder_eq_two : ∀ ⦃e : E⦄, e ∈ E(G) → eorder G e = 2)
    (exists_isSource : ∀ ⦃e : E⦄, e ∈ E(G) → ∃ i : I(G), edgeMap' G i = e ∧ IsSource G (i : I))
    (exists_isTarget : ∀ ⦃e : E⦄, e ∈ E(G) → ∃ i : I(G), edgeMap' G i = e ∧ IsTarget G (i : I)) :
    GraphLike G := by
  have exists_other (i : I(G)) : ∃ j : I(G), edgeMap' G j = edgeMap' G i ∧ j ≠ i := by
    obtain ⟨j, k, hj, hk, hjk⟩ := one_lt_encard_iff.mp
      (show 1 < eorder G (edgeMap' G i : E) by simp [eorder_eq_two (edgeMap' G i).property])
    obtain rfl | hji := eq_or_ne j i
    · exact ⟨k, Subtype.ext hk, fun hki ↦ hjk hki.symm⟩
    exact ⟨j, Subtype.ext hj, hji⟩
  refine {
    other' i := (exists_other i).choose
    edgeMap'_other' i := (exists_other i).choose_spec.1
    other'_ne i := (exists_other i).choose_spec.2
    edgeMap'_eq i j hij := ?_
    exists_isSource_of_mem_edgeSet := exists_isSource
    exists_isTarget_of_mem_edgeSet := exists_isTarget }
  obtain ⟨a, b, hab, hf⟩ := encard_eq_two.mp (eorder_eq_two (edgeMap' G i).property)
  have hm (k : I(G)) (hk : edgeMap' G k = edgeMap' G i) : k = a ∨ k = b := by
...
@[instance_reducible]
noncomputable def GraphLike.ofEOrderEqTwoOfEdgeMap [Nonempty E]
    (eorder_eq_two : ∀ ⦃e : E⦄, e ∈ E(G) → eorder G e = 2)
    (exists_isSource : ∀ ⦃e : E⦄, e ∈ E(G) → ∃ i, edgeMap G i = e ∧ IsSource G i)
    (exists_isTarget : ∀ ⦃e : E⦄, e ∈ E(G) → ∃ i, edgeMap G i = e ∧ IsTarget G i) :
    GraphLike G := by
  refine ofEOrderEqTwo eorder_eq_two (fun e he ↦ ?_) (fun e he ↦ ?_)
  · obtain ⟨i, hie, hs⟩ := exists_isSource he
    exact ⟨⟨i, hs.mem_incs⟩, (val_edgeMap'_eq_edgeMap ⟨i, hs.mem_incs⟩).trans hie, hs⟩
  obtain ⟨i, hie, ht⟩ := exists_isTarget he
  exact ⟨⟨i, ht.mem_incs⟩, (val_edgeMap'_eq_edgeMap ⟨i, ht.mem_incs⟩).trans hie, ht⟩

/-- Construct a `GraphLike` instance from natural-number order two, choosing the other incidence
classically. -/
@[instance_reducible]
noncomputable def GraphLike.ofOrderEqTwo (order_eq_two : ∀ ⦃e : E⦄, e ∈ E(G) → order G e = 2)
    ...
    (exists_isTarget : ∀ ⦃e : E⦄, e ∈ E(G) → ∃ i : I(G), edgeMap' G i = e ∧ IsTarget G (i : I)) :
    GraphLike G :=
  ofEOrderEqTwo (fun _ he ↦ (ENat.toNat_eq_iff (by decide)).mp (order_eq_two he))
    exists_isSource exists_isTarget

/-- Construct a `GraphLike` instance from natural-number order two and source and target incidences
specified using `edgeMap`, choosing the other incidence classically. -/
@[instance_reducible]
noncomputable def GraphLike.ofOrderEqTwoOfEdgeMap [Nonempty E]
  ...
  ofEOrderEqTwoOfEdgeMap (fun _ he ↦ (ENat.toNat_eq_iff (by decide)).mp (order_eq_two he))
    exists_isSource exists_isTarget
...
lemma eorder_eq_two (he : e ∈ E(G)) : eorder G e = 2 := by
  obtain ⟨i, rfl, _⟩ := exists_isSource_of_mem_edgeSet he
  refine encard_eq_two.mpr ⟨i, other' i, (other'_ne i).symm, ?_⟩
  ext j
  refine ⟨fun hj ↦ edgeMap'_eq i j (Subtype.ext hj.symm), ?_⟩
  rintro (rfl | rfl)
  · rfl
  exact congrArg Subtype.val (edgeMap'_other' i)

lemma order_eq_two (he : e ∈ E(G)) : order G e = 2 := by
  simp [order, eorder_eq_two he]

lemma edgeMap_surjOn [Nonempty E] (G : Gr) [GraphLike G] : SurjOn (edgeMap G) I(G) E(G) :=
  edgeMap_surjOn_iff.mpr fun e he ↦ by simp [eorder_eq_two he]

lemma edgeMap_range [Nonempty E] (G : Gr) [GraphLike G] :
    range (fun i : I(G) ↦ edgeMap G (i : I)) = E(G) :=
  edgeMap_range_of_eorder_pos fun e he ↦ by simp [eorder_eq_two he]

lemma edgeMap_image_incs [Nonempty E] (G : Gr) [GraphLike G] : edgeMap G '' I(G) = E(G) :=
  edgeMap_image_incs_of_eorder_pos fun e he ↦ by simp [eorder_eq_two he]
...
lemma edgeMap_other [Nonempty I] [Nonempty E] (hi : i ∈ I(G)) :
  ...
  simpa only [val_edgeMap'_eq_edgeMap, other, dite_eq_left hi] using
    congrArg Subtype.val (edgeMap'_other' ⟨i, hi⟩)
...
lemma edgeFiber_edgeMap [Nonempty I] [Nonempty E] (hi : i ∈ I(G)) :
  ...
  simp only [← eorder_eq_encard_edgeFiber, eorder_eq_two (edgeMap_mem_edges hi),
    encard_pair (other_ne hi).symm, le_refl]

lemma eorder_le_two : eorder G e ≤ 2 := by
  by_cases he : e ∈ E(G)
  · exact (eorder_eq_two he).le
  simp [eorder_of_notMem_edges he]

lemma order_le_two : order G e ≤ 2 := ENat.toNat_le_of_le_natCast eorder_le_two
...
lemma exists_isSource_of_mem_edgeSet [Nonempty E] (he : e ∈ E(G)) :
  ...
  exact ⟨i, (val_edgeMap'_eq_edgeMap i).symm.trans hi, hs⟩
...
lemma exists_isTarget_of_mem_edgeSet [Nonempty E] (he : e ∈ E(G)) :
  ...
  exact ⟨i, (val_edgeMap'_eq_edgeMap i).symm.trans hi, ht⟩

lemma exists_pair_edgeMap_iff [Nonempty E] (he : e ∈ E(G)) :
    ∃ i j, i ≠ j ∧ ∀ x, x ∈ I(G) ∧ edgeMap G x = e ↔ x = i ∨ x = j := by
  simpa [eorder_eq_encard_edgeFiber, encard_eq_two, Set.ext_iff] using eorder_eq_two he

@[simp]
lemma other_other [Nonempty I] (hi : i ∈ I(G)) : other G (other G i) = i := by
  let j : I(G) := ⟨i, hi⟩
  have hj : other' (other' j) = j :=
    edgeMap'_eq j (other' (other' j)) ((edgeMap'_other' (other' j)).trans (edgeMap'_other' j)).symm
...
@[simp]
lemma other_inj [Nonempty I] (hi : i ∈ I(G)) (hj : j ∈ I(G)) : other G i = other G j ↔ i = j :=
  other_injOn.eq_iff hi hj
...
lemma exists_isLink_of_mem_edgeSet (he : e ∈ E(G)) : ∃ u v, IsLink G e u v := by
  ...
  · let b := other' i
    have hb : (edgeMap' G b : E) = e := (congrArg Subtype.val (edgeMap'_other' i)).trans hei
...
lemma mem_edges_iff_exists_isLink : e ∈ E(G) ↔ ∃ u v, IsLink G e u v :=
  ⟨exists_isLink_of_mem_edgeSet, fun ⟨_, _, h⟩ ↦ h.edge_mem_edgeSet⟩
...
lemma one_lt_eorder_iff : 1 < eorder G e ↔ ∃ u v, IsLink G e u v :=
  ⟨fun h ↦ exists_isLink_of_mem_edgeSet (mem_edges_of_eorder_pos (lt_trans zero_lt_one h)),
    fun ⟨_, _, h⟩ ↦ h.one_lt_eorder⟩
...
lemma IsLink.right_unique (h : IsLink G e u v) (h' : IsLink G e u w) : v = w :=
  h.right_unique_of_eorder_eq_two h' (eorder_eq_two h.edge_mem_edgeSet)

lemma IsLink.left_unique (h : IsLink G e u w) (h' : IsLink G e v w) : u = v :=
  h.left_unique_of_eorder_eq_two h' (eorder_eq_two h.edge_mem_edgeSet)
...
lemma IsLink.incVerts_eq (h : IsLink G e u v) : incVerts G e = {u, v} :=
  h.incVerts_eq_of_eorder_eq_two (eorder_eq_two h.edge_mem_edgeSet)
...
lemma IsLink.eq_of_mem_incVerts_of_ne (h : IsLink G e u v) (hw : w ∈ incVerts G e) (hne : w ≠ u) :
    w = v := (h.mem_incVerts_iff.mp hw).resolve_left hne
...
lemma isLink_iff_of_verts_eq_singleton (hV : V(G) = {v}) :
  ...
  refine ⟨fun h ↦ ⟨h.edge_mem_edgeSet, ?_, ?_⟩, ?_⟩
  · simpa [hV] using h.left_mem_vertexSet
  · simpa [hV] using h.right_mem_vertexSet
  rintro ⟨he, rfl, rfl⟩
  obtain ⟨x, y, h⟩ := exists_isLink_of_mem_edgeSet he
  obtain rfl : x = w := by simpa [hV] using h.left_mem_vertexSet
  obtain rfl : y = x := by simpa [hV] using h.right_mem_vertexSet
  exact h
...
lemma incVerts_encard_le_two : (incVerts G e).encard ≤ 2 :=
  encard_incVerts_le_eorder.trans eorder_le_two

lemma incVerts_ncard_le_two : (incVerts G e).ncard ≤ 2 :=
  (encard_le_coe_iff_finite_ncard_le.mp incVerts_encard_le_two).2

lemma edgeMap_preimage_singleton_injOn_of_GraphLike [Nonempty E] :
    InjOn (fun e ↦ edgeMap G ⁻¹' {e}) E(G) :=
  edgeMap_preimage_singleton_injOn fun e he ↦ by simp [eorder_eq_two he]
