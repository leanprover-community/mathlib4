...
infinity. -/
theorem corners_theorem_nat (hε : 0 < ε) :
...
theorem roth_3ap_theorem (ε : ℝ) (hε : 0 < ε) :
...
theorem roth_3ap_theorem_nat (ε : ℝ) (hε : 0 < ε) :
    ∃ n₀ : ℕ, ∀ n, n₀ ≤ n → ∀ A ⊆ range n, ε * n ≤ A.card → ¬ ThreeAPFree (A : Set ℕ) := by
  obtain ⟨n₀, hn₀⟩ := roth_theorem (ε / 3) (by positivity)
  refine ⟨n₀ + 1, fun n hn A hAn hAε hA ↦ ?_⟩
...
theorem rothNumberNat_isLittleO_id :
  ...
  refine fun ε hε ↦ (roth_theorem_nat ε hε).imp fun n₀ ↦ forall₂_imp fun n hn h ↦ ?__
