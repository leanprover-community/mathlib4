...
/-- A nonempty set with no doubling is the left translate of its stabilizer. -/
@[to_additive /-- A nonempty set with no doubling is the left-translate of its stabilizer. -/]
lemma smul_stabilizer_of_no_doubling (hA : #(A * A) ≤ #A) (ha : a ∈ A) :
    a •> (stabilizer G A : Set G) = A := (smul_stabilizer_of_no_doubling_aux hA ha).1

/-- A nonempty set with no doubling is the right translate of its stabilizer. -/
@[to_additive /-- A nonempty set with no doubling is the right translate of its stabilizer. -/]
