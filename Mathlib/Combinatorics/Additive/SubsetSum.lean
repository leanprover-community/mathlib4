...
lemma self_subset_subsetSum : A ⊆ A.subsetSum :=
...
lemma card_subsetSum_insert_max (A_lt_a : ∀ x ∈ A, x < a)
  ...
  have insert_subset : insert 0 A ⊆ (insert a A).subsetSum :=
    insert_subset subsetSum_zero_mem
      (self_subset_subsetSum.trans (subsetSum_mono (subset_insert a A)))
...
theorem zero_notMem_subsetSum_card_lower_bound (hA : 0 ∉ A) :
...
theorem subsetSum_card_lower_bound : A.card.choose 2 + 1 ≤ A.subsetSum.card := by
  ...
  calc A.card.choose 2 + 1
    ...
    _ ≤ (A.erase 0).subsetSum.card := zero_notMem_subsetSum_card_lower_bound (by simp)
