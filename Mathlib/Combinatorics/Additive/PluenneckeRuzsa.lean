...
private theorem mul_aux (hA : A.Nonempty)
  ...
  · simp
  refine mod_cast (div_le_div_iff₀ ?_ ?_).1 (h A' (by simp [hA'.ne_empty, hAA', *])) <;>
  exact cast_pos.2 (card_pos.2 (by assumption))
...
theorem pluennecke_ruzsa_inequality_pow_div_pow_mul (hA : A.Nonempty) (B : Finset G) (m n : ℕ) :
  ...
  calc
    ...
    _ ≤ ((#(C * B) / #C) ^ m * #C) * ((#(C * B) / #C : ℚ≥0) ^ n * #C) := by
      gcongr <;> exact card_mul_pow_le (fun A' hA' ↦ mul_aux hC hCmin _ (hA'.trans hCA)) _
