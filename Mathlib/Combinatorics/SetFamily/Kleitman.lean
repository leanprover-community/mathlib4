...
theorem Finset.card_bunionᵢ_le_of_intersecting (s : Finset ι) (f : ι → Finset (Finset α))
  ...
  classical
    ...
    refine' le_of_mul_le_mul_left _ (pow_pos zero_lt_two <| Fintype.card α + 1)
    ...
    refine' (add_le_add_left
      (ih ((card_le_of_subset <| subset_cons _).trans hs) _
      fun i hi => (hf₁ _ <| subset_cons _ hi).2.2) _).trans _
