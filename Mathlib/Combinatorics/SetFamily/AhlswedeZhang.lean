...
def truncatedSup (s : Finset α) (a : α) : α :=
  if h : a ∈ lowerClosure (s : Set α) then {b ∈ s | a ≤ b}.sup' (private_decl% (sup_aux h)) id
  else ⊤

lemma truncatedSup_of_mem (h : a ∈ lowerClosure (s : Set α)) :
    truncatedSup s a = {b ∈ s | a ≤ b}.sup' (private_decl% (sup_aux h)) id := dite_eq_left h
...
def truncatedInf (s : Finset α) (a : α) : α :=
  if h : a ∈ upperClosure (s : Set α) then {b ∈ s | b ≤ a}.inf' (private_decl% (inf_aux h)) id
  else ⊥

lemma truncatedInf_of_mem (h : a ∈ upperClosure (s : Set α)) :
    truncatedInf s a = {b ∈ s | b ≤ a}.inf' (private_decl% (inf_aux h)) id := dite_eq_left h
