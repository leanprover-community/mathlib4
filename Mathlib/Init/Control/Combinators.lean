...
-/
prelude
import Mathlib.Init.Control.Monad
import Mathlib.Init.Control.Alternative
...
lean 3 declaration is
  forall {m : Type.{u} -> Type.{v}} [_inst_1 : Monad.{u v} m] [_inst_2 : Alternative.{u v} m] {α : Type.{w}} {β : Type.{u}}, (α -> (m β)) -> (List.{w} α) -> (m β)
but is expected to have type
  forall {m : Type.{u} -> Type.{v}} [_inst_2 : Alternative.{u v} m] {α : Type.{w}} {β : Type.{u}}, (α -> (m β)) -> (List.{w} α) -> (m β)
