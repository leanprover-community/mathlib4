...
-- porting note: new
instance (priority := 10000) : @Trans α α α LE.le LE.le LE.le := ⟨le_trans⟩
instance (priority := 10000) : @Trans α α α LT.lt LT.lt LT.lt := ⟨lt_trans⟩
instance (priority := 10000) : @Trans α α α LT.lt LE.le LT.lt := ⟨lt_of_lt_of_le⟩
instance (priority := 10000) : @Trans α α α LE.le LT.lt LT.lt := ⟨lt_of_le_of_lt⟩
instance (priority := 10000) : @Trans α α α GE.ge GE.ge GE.ge := ⟨ge_trans⟩
instance (priority := 10000) : @Trans α α α GT.gt GT.gt GT.gt := ⟨gt_trans⟩
instance (priority := 10000) : @Trans α α α GT.gt GE.ge GT.gt := ⟨gt_of_gt_of_ge⟩
instance (priority := 10000) : @Trans α α α GE.ge GT.gt GT.gt := ⟨gt_of_ge_of_gt⟩
...
instance (priority := 10000) (a b : α) : Decidable (a < b) :=
...
instance (priority := 10000) : IsTotalPreorder α (· ≤ ·)
    where
  trans := @le_trans _ _
  total := le_total

-- TODO(Leo): decide whether we should keep this instance or not
instance (priority := 10000) isStrictWeakOrder_of_linearOrder : IsStrictWeakOrder α (· < ·) :=
...
-- TODO(Leo): decide whether we should keep this instance or not
instance (priority := 10000) isStrictTotalOrder_of_linearOrder : IsStrictTotalOrder α (· < ·) where
...
instance (priority := 10000) : Std.TransCmp (compare (α := α)) where
  symm a b := by
    cases h : compare a b <;>
