...
instance (priority := 900) : IsTotalPreorder α (· ≤ ·)
    where
  trans := @le_trans _ _
  total := le_total

-- TODO(Leo): decide whether we should keep this instance or not
instance (priority := 900) isStrictWeakOrder_of_linearOrder : IsStrictWeakOrder α (· < ·) :=
...
-- TODO(Leo): decide whether we should keep this instance or not
instance (priority := 900) isStrictTotalOrder_of_linearOrder : IsStrictTotalOrder α (· < ·) where
...
instance (priority := 900) : Std.TransCmp (compare (α := α)) where
  symm a b := by
    cases h : compare a b <;>
