...
instance decidableExists : ∀ (l : List α), Decidable (∃ x ∈ l, p x)
  | []    => isFalse (by simp)
  | x::xs =>
    match ‹DecidablePred p› x with
    | isTrue h₁ => isTrue ⟨x, mem_cons_self _ _, h₁⟩
    | isFalse h₁ => match decidableExists xs with
      | isTrue h₂  => isTrue <| by
...
#align list.decidable_bex List.decidableExists

instance decidableForall (l : List α) : Decidable (∀ x ∈ l, p x) :=
...
#align list.decidable_ball List.decidableForall

@[deprecated] alias List.decidableBex := List.decidableExists -- 2024-03-23
@[deprecated] alias List.decidableBall := List.decidableForall -- 2024-03-23
