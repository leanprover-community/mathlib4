...
lemma bool_iff_false {b : Bool} : ¬b ↔ b = false := by cases b <;> decide
...
lemma decide_false_iff (p : Prop) {_ : Decidable p} : decide p = false ↔ ¬p :=
...
lemma coe_or_iff (a b : Bool) : a || b ↔ a ∨ b := by simp
#align bor_coe_iff Bool.coe_or_iff

lemma coe_and_iff (a b : Bool) : a && b ↔ a ∧ b := by simp
#align band_coe_iff Bool.coe_and_iff

lemma coe_xor_iff (a b : Bool) : xor a b ↔ Xor' (a = true) (b = true) := by
