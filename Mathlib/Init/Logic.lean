...
theorem left_comm : Std.Commutative f → Std.Associative f → LeftCommutative f :=
  fun hcomm hassoc a b c ↦
    calc  a*(b*c)
      _ = (a*b)*c := Eq.symm (hassoc.assoc a b c)
      _ = (b*a)*c := hcomm.comm a b ▸ rfl
      _ = b*(a*c) := hassoc.assoc b a c

theorem right_comm : Std.Commutative f → Std.Associative f → RightCommutative f :=
  fun hcomm hassoc a b c ↦
    calc  (a*b)*c
      _ = a*(b*c) := hassoc.assoc a b c
      _ = a*(c*b) := hcomm.comm b c ▸ rfl
      _ = (a*c)*b := Eq.symm (hassoc.assoc a c b)
