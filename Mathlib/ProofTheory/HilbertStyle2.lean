...
class Deduction {F : Type u} {Γ : Set F} [LogicalConnective F] (Bew : Set F → F → Type*) where
  axm : ∀ {f}, f ∈ Γ → Bew Γ f
...
abbrev modus_ponens {Γ₁ Γ₂ p q} (d₁ : Bew Γ₁ (p ⭢ q)) (d₂ : Bew Γ₂ p) : Bew (Γ₁ ∪ Γ₂) q :=
  HasModusPonens.modus_ponens d₁ d₂
...
def disj₃' {Γ p q r} (d₁ : Bew Γ (p ⭢ r)) (d₂ : Bew Γ (q ⭢ r)) (d₃ : Bew Γ (p ⋎ q)) : Bew Γ r :=
  (((disj₃ _ _ _ _) ⨀ d₁) ⨀ d₂) ⨀ d₃
...
def dtl {Γ p q} (d : Bew Γ (p ⭢ q)) : Bew (insert p Γ) q := modus_ponens' (weakening' (by simp) d)
  (axm (by simp))
...
def assoc (Γ p q r) : Bew Γ ((p ⋏ q) ⋏ r ⭤ p ⋏ (q ⋏ r)) := iff_intro (by apply assoc_left)
  (by apply assoc_right)
...
def conj_symm_iff (Γ p q) : Bew Γ ((p ⋏ q) ⭤ (q ⋏ p)) := iff_intro (by apply conj_symm)
  (by apply conj_symm)
...
variable (Bew : Set F → F → Type u) [hd : Deduction Bew] [HasModusPonens Bew] [HasDT Bew]
  [Minimal Bew] [Classical Bew]
...
lemma conj₃'! {Γ : Set F} {p q : F} (d₁ : Γ ⊢! p) (d₂: Γ ⊢! q) : Γ ⊢! (p ⋏ q) :=
  ⟨conj₃' d₁.some d₂.some⟩
...
lemma iff_intro! {Γ : Set F} {p q : F} (dpq : Γ ⊢! (p ⭢ q)) (dqp : Γ ⊢! (q ⭢ p)) : Γ ⊢! (p ⭤ q) :=
  ⟨iff_intro dpq.some dqp.some⟩
...
lemma iff_left'! {Γ : Set F} {p q : F} (dpq : Γ ⊢! (p ⭤ q)) (dq : Γ ⊢! q) : Γ ⊢! p :=
  ⟨iff_left' dpq.some dq.some⟩

lemma iff_right'! {Γ : Set F} {p q : F} (dpq : Γ ⊢! (p ⭤ q)) (dp : Γ ⊢! p) : Γ ⊢! q :=
  ⟨iff_right' dpq.some dp.some⟩
...
lemma consistent_of_subset {Γ Δ : Set F} (h : Γ ⊆ Δ) (hConsis : Consistent Bew Δ) :
    Consistent Bew Γ := by
  intro hD;
  exact hConsis ⟨hd.weakening' h hD.some⟩;

lemma consistent_of_insert {p} (hConsis : Consistent Bew (insert p Γ)) : Consistent Bew Γ :=
  consistent_of_subset Bew (by simp) hConsis

lemma consistent_no_falsum_subset {Γ Δ} (hConsis : Consistent Bew Γ) (hΔ : Δ ⊆ Γ) : ⊥ ∉ Δ :=
  consistent_no_falsum Bew $ consistent_of_subset Bew hΔ hConsis

lemma consistent_subset_undeducible_falsum {Γ Δ} (hConsis : Consistent Bew Γ) (hΔ : Δ ⊆ Γ) :
    (Δ ⊬! ⊥) := by
  by_contra hC;
  simp_all [Consistent, Undeducible, Deducible];
  exact hConsis.false $ hd.weakening' hΔ hC.some;

lemma consistent_neither_undeducible {Γ : Set F} (hConsis : Consistent Bew Γ) (p) :
    (Γ ⊬! p) ∨ (Γ ⊬! ~p) := by
...
lemma inconsistent_of_deducible {Γ : Set F} (d : Γ ⊢! ⊥) : Inconsistent Bew Γ := by
  simpa [Inconsistent];

lemma inconsistent_insert_falsum : Inconsistent Bew (insert ⊥ Γ) := ⟨hd.axm (by simp)⟩

lemma inconsistent_insert {Γ p} (h : Inconsistent Bew (insert p Γ)) :
    (∃ Δ, (Δ ⊆ Γ) ∧ ((insert p Δ) ⊢! ⊥)) := by
  existsi Γ;
  constructor;
  · rfl;
...
lemma consistent_iff_insert_neg {Γ p} : Consistent Bew (insert (~p) Γ) ↔ (Γ ⊬! p) :=
  (inconsistent_iff_insert_neg Bew).not

lemma consistent_either {Γ : Set F} (hConsis : Consistent Bew Γ) (p) :
    (Consistent Bew (insert p Γ)) ∨ (Consistent Bew (insert (~p) Γ)) := by
