...
Currently, this postpones until the expected type is available (`<= ty`). We could also try to
postpone within `by`.
...
| `(private%$tk $t) => withRef t do
  ...
  if !(← getEnv).isExporting then
    ...
    -- Note that `by` disables implicit lambda anyway, so the `implicitLambda := false` is
    -- unnecessary, but records the intention.
    -- However, we need to re-assert no implicit lambda inside the by block as well.
    Term.elabTerm (← `(term| by as_aux_lemma => exact @$t)) ty
