    ...
    pure { rv with
      ...
      value := ← forallTelescope rv.type fun xs body => do
        ...
        (← whnfD <| ← inferType major).withApp fun head args => do
          ...
          if !iv.isRec && rv'.numMotives == 1 && iv.numCtors == 1 && iv.numIndices == 0 then
            let rule := rv.rules[0]!
            let val := .beta (replaceConst repl rule.rhs) xs[*...rv.getFirstIndexIdx].copy
            ...
            let val := mkAppN val args[*...rv'.numParams].copy
            let val := .app val <| ← mkLambdaFVars xs[rv.getFirstIndexIdx...*].copy body
            let val := mkAppN val xs[rv.getFirstIndexIdx...*].copy
            let val := mkAppN val <| rv.rules.toArray.map fun rule =>
              .beta (replaceConst repl rule.rhs) xs[*...rv.getFirstIndexIdx].copy
            mkLambdaFVars xs val
      hints := .opaque
      safety := .partial
    }
