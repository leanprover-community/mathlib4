...
If there was a provided name that is identical to the adjusted name, we optionally warn about this:
`warnIfIdentical` controls if we emit this warning, and `warnPrefix` is the attribute name used
as prefix for the generated warning message. `pos` is the syntax location where to emit the warning.
-/
def Lean.Name.modifySuffix (name : Name) (suffix : Option Name)
    (warnIfIdentical : Bool := true) (warnPrefix : String := "") (pos : Syntax) : AttrM Name := do
  match suffix with
  | none => return name
  | some suffix =>
    let instead :=
    ...
    if warnIfIdentical && name == instead then
      logWarningAt pos m!"`{warnPrefix}` correctly autogenerated the provided name \
      `{.ofConstName name}`.\nYou may remove it."
    return instead
