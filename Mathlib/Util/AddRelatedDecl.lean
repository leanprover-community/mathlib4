...
`pos` is the syntax location where to emit the warning (usually the syntax of the specified name).

If `suffix` is `some`, then `pos` should also be `some`.
-/
def Lean.Name.modifySuffix (name : Name) (suffix : Option Name)
    (allowIdentical := false) (warnPrefix : String)
    (pos : Option Syntax) : AttrM Name := do
  match suffix with
  | none => return name
  | some suffix =>
    let instead :=
    ...
    if !allowIdentical && name == instead then
      let some pos := pos
      | throwError "error: calling `modifySuffix` with a provided suffix requires specifying \
          the syntax position of the suffix"
      Linter.logLintIf linter.generateName pos m!"`{warnPrefix}` \
        correctly autogenerated the provided target name `{.ofConstName name}`.\nYou may remove it."
