...
partial def parallelMapM [MonadTask m] [MonadLiftT BaseIO m]
    (L : MLList m α) (f : α → m β) (numThreads : Nat := 32) (chunkSize : Nat := 1)
    (outOfOrder : Bool := false) : MLList m (Except IO.Error β) :=
  if chunkSize ≤ 1 then
    core L f
  else
    ...
    | .ok l => MLList.ofArray (l.map .ok)).join
where
  core {α' β'} (L' : MLList m α') (f' : α' → m β') : MLList m (Except IO.Error β') :=
    squash do
      return run f' (← unconsAsTask L') []
  unconsAsTask {α'} (L' : MLList m α') : m (Task (Option (α' × MLList m α'))) := do
  ...
  run {α' β'} (f' : α' → m β') (uncons? : Option (Task (Option (α' × MLList m α'))))
      (pool : List (Task (Except _ β'))) : MLList m (Except _ β') :=
        ...
        if ← IO.hasFinished uncons then
          ...
          | hd::tl => do
              if outOfOrder then
               ...
               let w : List (Task (Option (α' × MLList m α') ⊕ (Nat × Except _ β'))) :=
                ...
                let w : List (Task (Option (α' × MLList m α') ⊕ (Except _ β'))) :=
...
reducing parallelism.
-/
def parallelMap [Monad m] [MonadTask m] [MonadLiftT BaseIO m] (L : MLList m α)
