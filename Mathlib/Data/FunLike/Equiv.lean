...
instance (priority := 10000) : EquivLike (MyIso A B) A (λ _, B) :=
...
instance (priority := 10000) : CoeFun (MyIso A B) := DFunLike.instCoeFunForAll
...
instance (priority := 10000) : MyIsoClass (MyIso A B) A B :=
  { coe := MyIso.toFun,
    inv := MyIso.invFun,
...
instance (priority := 10000) : CoolerIsoClass (CoolerIso A B) A B :=
  { coe := CoolerIso.toFun,
    coe_injective' := λ f g h, by cases f; cases g; congr',
