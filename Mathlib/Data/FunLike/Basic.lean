...
instance (priority := 10000) : FunLike (MyHom A B) A B :=
  { coe := MyHom.toFun, coe_injective' := fun f g h => by cases f; cases g; congr' }

/-- Helper instance for when there's too many metavariables to apply
`DFunLike.coe` directly. -/
instance (priority := 10000) : CoeFun (MyHom A B) (fun _ => A → B) := ⟨MyHom.toFun⟩
...
instance (priority := 10000) : MyHomClass (MyHom A B) A B :=
  { coe := MyHom.toFun,
...
instance (priority := 10000) : CoolerHomClass (CoolHom A B) A B :=
  { coe := CoolHom.toFun,
    coe_injective' := λ f g h, by cases f; cases g; congr',
