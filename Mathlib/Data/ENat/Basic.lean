...
variable {M : Type → Type} [Applicative M] [LawfulApplicative M]

example {α : Type} (h : ¬ Function.Injective (pure : α → M α)) {β : Type} (x y : β) :
    (pure x : M β) = pure y := by
  rw [Function.Injective] at h
  push_neg at h
  rcases h with ⟨a, b, h⟩
  classical
  let f : α → β := fun c => if a = c then x else y
  have hfa : f a = x := if_pos rfl
  have hfb : f b = y := if_neg h.2
  rw [← hfa, ← hfb, ← map_pure, ← map_pure, h.1]
