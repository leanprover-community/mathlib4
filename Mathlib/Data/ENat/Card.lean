...
@[simp]
protected theorem eNatCast_card (α : Type*) [Finite α] : (Nat.card α : ℕ∞) = ENat.card α := by
...
theorem card_of_subsingleton (a : α) [Subsingleton α] : Nat.card α = 1 := by
  rw [← Nat.cast_inj (R := ℕ∞), Nat.eNatCast_card]
...
/-
* Lemmas about sets to deprecate or move to `Set.Card`
-/
...
/-
* Lemmas about cardinals to move to a cardinal file somewhere.
-/
