...
Provides a recursor for FinEnum types like the one for Fintypes, but capable of producing
non-truncated data.
-/
...
def recEmptyOption {P : (α : Type u) → [FinEnum α] → Sort v}
  ...
  case succ n ih =>
    have ⟨fe, fecardeq⟩ :
      (e : FinEnum <| ULift.{u} <| Fin n) ×' (e.card = n) := ⟨FinEnum.mk n Equiv.ulift.{u}, rfl⟩
