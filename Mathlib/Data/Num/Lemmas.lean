...
@[norm_cast]
lemma to_nat_inj {m n : Num} : (m : ℕ) = n ↔ m = n := toNat_injective.eq_iff
...
@[simp, norm_cast]
lemma of_natCast {α} [AddMonoidWithOne α] (n : ℕ) : ((n : Num) : α) = n := by
...
lemma castNum_eq_bitwise {f : Num → Num → Num} {g : Bool → Bool → Bool}
    (p : PosNum → PosNum → Num)
...
@[simp, norm_cast]
lemma castNum_or : ∀ m n : Num, ↑(m ||| n) = (↑m ||| ↑n : ℕ) := by
...
lemma castNum_shiftRight (m : Num) (n : Nat) : ↑(m >>> n) = (m : ℕ) >>> (n : ℕ)  := by
...
@[simp]
lemma castNum_testBit (m n) : testBit m n = Nat.testBit m n := by
...
@[simp, norm_cast]
lemma of_intCast [AddGroupWithOne α] (n : ℤ) : ((n : ZNum) : α) = n := by
  rw [← cast_to_int, to_of_int]
#align znum.of_int_cast ZNum.of_intCast

@[simp, norm_cast]
lemma of_natCast [AddGroupWithOne α] (n : ℕ) : ((n : ZNum) : α) = n := by
