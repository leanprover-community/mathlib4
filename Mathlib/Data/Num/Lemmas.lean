...
theorem castNum_testBit (m n) : testBit m n = Nat.testBit m n := by
  ...
  | pos m =>
    ...
    · rfl
    · rw [PosNum.cast_bit1, ← two_mul, ← Nat.bit_true, Nat.testBit_bit_zero]
    · rw [PosNum.cast_bit0, ← two_mul, ← Nat.bit_false, Nat.testBit_bit_zero]
    · simp
    · rw [PosNum.cast_bit1, ← two_mul, ← Nat.bit_true, Nat.testBit_bit_succ, IH]
    · rw [PosNum.cast_bit0, ← two_mul, ← Nat.bit_false, Nat.testBit_bit_succ, IH]
