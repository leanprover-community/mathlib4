...
lemma recF_eq {α : TypeVec n} {β : Type _} (g : F (α.append1 β) → β) (a : q.P.A)
...
lemma recF_eq' {α : TypeVec n} {β : Type _} (g : F (α.append1 β) → β) (x : q.P.W α) :
...
lemma Fix.ind_rec {β : Type _} (g₁ g₂ : Fix F α → β)
    (h :
...
lemma Fix.rec_unique {β : Type _} (g : F (append1 α β) → β) (h : Fix F α → β)
