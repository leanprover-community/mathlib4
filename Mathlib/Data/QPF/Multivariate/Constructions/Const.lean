...
instance Const.inhabited {A α} [Inhabited A] : Inhabited (Const n A α) := fast_instance% ⟨(default : A)⟩
...
instance MvFunctor : MvFunctor (Const n A) where map _f := fast_instance% Const.map
