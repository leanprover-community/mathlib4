...
theorem X_pow_dvd_iff {f : R[X]} {n : ℕ} : X ^ n ∣ f ↔ ∀ d < n, f.coeff d = 0 :=
  ⟨fun ⟨g, hgf⟩ d hd => by
    ...
    · simp only [Nat.zero_eq, pow_zero, isUnit_one, IsUnit.dvd]
  ...
  degree_sub_lt
    (by
      rw [hq.degree_mul, degree_C_mul_X_pow _ hp, degree_eq_natDegree h.2,
        degree_eq_natDegree hq0, ← WithBot.coe_add, tsub_add_cancel_of_le hlt])
...
/-- See `divByMonic`. -/
noncomputable def divModByMonicAux : ∀ (p : R[X]) {q : R[X]},
    Monic q → R[X] × R[X] | p => fun q hq =>
...
theorem degree_modByMonic_lt [Nontrivial R] :
    ∀ (p : R[X]) {q : R[X]} (hq : Monic q), degree (p %ₘ q) < degree q | p => fun q hq =>
...
theorem modByMonic_eq_self_iff [Nontrivial R] (hq : Monic q) : p %ₘ q = p ↔ degree p < degree q :=
  ⟨fun h => h ▸ degree_modByMonic_lt _ hq, fun h => by
    ...
    unfold modByMonic divModByMonicAux <;> rw [dif_pos hq, if_neg (mt And.left this)]⟩
...
theorem modByMonic_eq_sub_mul_div :
    ∀ (p : R[X]) {q : R[X]} (hq : Monic q), p %ₘ q = p - q * (p /ₘ q) | p => fun q hq =>
    if (h : degree q ≤ degree p ∧ p ≠ 0) then by
      have wf := div_wf_lemma h hq
      have ih :=
...
theorem divByMonic_eq_zero_iff [Nontrivial R] (hq : Monic q) : p /ₘ q = 0 ↔ degree p < degree q :=
  ⟨fun h => by
    ...
    fun h => by
    have : ¬degree q ≤ degree p := not_le_of_gt h
    unfold divByMonic divModByMonicAux <;> rw [dif_pos hq, if_neg (mt And.left this)]⟩
...
theorem ker_evalRingHom (x : R) : (evalRingHom x).ker = Ideal.span {X - C x} := by
  funext y
  simpa only [Ideal.mem_span_singleton, dvd_iff_isRoot]
...
theorem rootMultiplicity_eq_multiplicity (p : R[X]) (a : R) : rootMultiplicity a p =
    if h0 : p = 0 then 0 else
    (multiplicity (X - C a) p).get (multiplicity_X_sub_C_finite a h0) := by
...
theorem rootMultiplicity_pos' {p : R[X]} {x : R} :
  ...
  rw [pos_iff_ne_zero, Ne.def, rootMultiplicity_eq_zero_iff, not_imp, and_comm]
