...
theorem splits_prod_iff {ι : Type u} {s : ι → K[X]} {t : Finset ι} :
  ...
  -- ⊢ Splits i (∏ x in insert a t, s x) ↔ Splits i (s a) ∧ ∀ (x : ι), x ∈ t → Spli …
...
theorem adjoin_rootSet_eq_range [Algebra F K] [Algebra F L] {p : F[X]}
  ...
  -- ⊢ Subalgebra.map f (Algebra.adjoin F (rootSet p K)) = Subalgebra.map f ⊤ ↔ Alg …
...
theorem eq_prod_roots_of_splits {p : K[X]} {i : K →+* L} (hsplit : Splits i p) :
  ...
  -- ⊢ map i p = ↑C (leadingCoeff (map i p)) * Multiset.prod (Multiset.map (fun a = …
                           -- ⊢ ↑C (leadingCoeff (map i p)) * Multiset.prod (Multiset.map (fun a => X - ↑C a …
...
theorem eq_X_sub_C_of_splits_of_single_root {x : K} {h : K[X]} (h_splits : Splits i h)
  ...
  -- ⊢ ↑C (↑i (leadingCoeff h)) * Multiset.prod (Multiset.map (fun a => X - ↑C a) { …
  simp
  -- 🎉 no goals
set_option linter.uppercaseLean3 false in
...
theorem mem_lift_of_splits_of_roots_mem_range (R : Type*) [CommRing R] [Algebra R K] {f : K[X]}
  ...
  -- ⊢ Multiset.prod (Multiset.map (fun a => X - ↑C a) (roots f)) ∈ liftsRing (alge …
...
theorem splits_iff_card_roots {p : K[X]} :
  ...
  · intro hroots
    ...
    -- ⊢ ∃ s, map (RingHom.id K) p = ↑C (↑(RingHom.id K) (leadingCoeff p)) * Multiset …
    use p.roots
    -- ⊢ map (RingHom.id K) p = ↑C (↑(RingHom.id K) (leadingCoeff p)) * Multiset.prod …
    simp only [RingHom.id_apply, map_id]
    -- ⊢ p = ↑C (leadingCoeff p) * Multiset.prod (Multiset.map (fun x => X - ↑C x) (r …
...
theorem aeval_root_derivative_of_splits [Algebra K L] {P : K[X]} (hmo : P.Monic)
  ...
  -- ⊢ ↑(aeval r) (↑derivative P) = Multiset.prod (Multiset.map (fun a => r - a) (M …
  replace hP := (splits_id_iff_splits (algebraMap K L)).2 hP
  -- ⊢ ↑(aeval r) (↑derivative P) = Multiset.prod (Multiset.map (fun a => r - a) (M …
  rw [aeval_def, ← eval_map, ← derivative_map]
  -- ⊢ eval r (↑derivative (map (algebraMap K L) P)) = Multiset.prod (Multiset.map  …
  nth_rw 1 [eq_prod_roots_of_monic_of_splits_id hmo hP]
  -- ⊢ eval r (↑derivative (Multiset.prod (Multiset.map (fun a => X - ↑C a) (roots  …
...
theorem prod_roots_eq_coeff_zero_of_monic_of_split {P : K[X]} (hmo : P.Monic)
  ...
  nth_rw 1 [eq_prod_roots_of_monic_of_splits_id hmo hP]
  -- ⊢ coeff (Multiset.prod (Multiset.map (fun a => X - ↑C a) (roots P))) 0 = (-1)  …
  rw [coeff_zero_eq_eval_zero, eval_multiset_prod, Multiset.map_map]
  -- ⊢ Multiset.prod (Multiset.map (eval 0 ∘ fun a => X - ↑C a) (roots P)) = (-1) ^ …
  simp_rw [Function.comp_apply, eval_sub, eval_X, zero_sub, eval_C]
  -- ⊢ Multiset.prod (Multiset.map (fun x => -x) (roots P)) = (-1) ^ natDegree P *  …
  conv_lhs =>
    congr
    congr
    ext
...
theorem sum_roots_eq_nextCoeff_of_monic_of_split {P : K[X]} (hmo : P.Monic)
  ...
  nth_rw 1 [eq_prod_roots_of_monic_of_splits_id hmo hP]
  -- ⊢ nextCoeff (Multiset.prod (Multiset.map (fun a => X - ↑C a) (roots P))) = -Mu …
  rw [Monic.nextCoeff_multiset_prod _ _ fun a ha => _]
  -- ⊢ Multiset.sum (Multiset.map (fun i => nextCoeff (X - ↑C i)) (roots P)) = -Mul …
