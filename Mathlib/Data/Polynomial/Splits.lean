...
@[simp]
lemma splits_zero : Splits i (0 : K[X]) :=
...
lemma splits_of_natDegree_le_one {f : K[X]} (hf : natDegree f ≤ 1) : Splits i f :=
...
lemma splits_of_natDegree_eq_one {f : K[X]} (hf : natDegree f = 1) : Splits i f :=
...
lemma splits_pow {f : K[X]} (hf : f.Splits i) (n : ℕ) : (f ^ n).Splits i := by
...
lemma splits_id_iff_splits {f : K[X]} : (f.map i).Splits (RingHom.id L) ↔ f.Splits i := by
...
lemma roots_ne_zero_of_splits' {f : K[X]} (hs : Splits i f) (hf0 : natDegree (f.map i) ≠ 0) :
    (f.map i).roots ≠ 0 :=
...
lemma map_rootOfSplits' {f : K[X]} (hf : f.Splits i) (hfd) :
...
lemma natDegree_eq_card_roots' {p : K[X]} {i : K →+* L} (hsplit : Splits i p) :
...
lemma splits_mul_iff {f g : K[X]} (hf : f ≠ 0) (hg : g ≠ 0) :
...
lemma splits_prod_iff {ι : Type u} {s : ι → K[X]} {t : Finset ι} :
...
lemma degree_eq_one_of_irreducible_of_splits {p : K[X]} (hp : Irreducible p)
...
lemma roots_ne_zero_of_splits {f : K[X]} (hs : Splits i f) (hf0 : natDegree f ≠ 0) :
    (f.map i).roots ≠ 0 :=
...
lemma rootOfSplits'_eq_rootOfSplits {f : K[X]} (hf : f.Splits i) (hfd) :
...
lemma degree_eq_card_roots {p : K[X]} {i : K →+* L} (p_ne_zero : p ≠ 0) (hsplit : Splits i p) :
    p.degree = Multiset.card (p.map i).roots := by
  rw [degree_eq_natDegree p_ne_zero, natDegree_eq_card_roots hsplit]
#align polynomial.degree_eq_card_roots Polynomial.degree_eq_card_roots

lemma roots_map {f : K[X]} (hf : f.Splits <| RingHom.id K) : (f.map i).roots = f.roots.map i :=
...
lemma image_rootSet [Algebra F K] [Algebra F L] {p : F[X]} (h : p.Splits (algebraMap F K))
...
lemma adjoin_rootSet_eq_range [Algebra F K] [Algebra F L] {p : F[X]}
...
lemma eq_prod_roots_of_splits_id {p : K[X]} (hsplit : Splits (RingHom.id K) p) :
...
lemma eq_prod_roots_of_monic_of_splits_id {p : K[X]} (m : Monic p)
...
lemma splits_of_splits_id {f : K[X]} : Splits (RingHom.id K) f → Splits i f :=
...
lemma splits_iff_exists_multiset {f : K[X]} :
    Splits i f ↔
...
lemma sum_roots_eq_nextCoeff_of_monic_of_split {P : K[X]} (hmo : P.Monic)
