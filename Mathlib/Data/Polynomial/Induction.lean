...
@[elab_as_elim]
protected lemma induction_on {M : R[X] → Prop} (p : R[X]) (h_C : ∀ a, M (C a))
...
-/
@[elab_as_elim]
protected lemma induction_on' {M : R[X] → Prop} (p : R[X]) (h_add : ∀ p q, M p → M q → M (p + q))
...
lemma mem_span_C_coeff : f ∈ Ideal.span { g : R[X] | ∃ i : ℕ, g = C (coeff f i) } := by
