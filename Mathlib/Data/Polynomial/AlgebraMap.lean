...
-/
@[ext 1100]
lemma algHom_ext' [Algebra R A'] [Algebra R B'] {f g : A'[X] →ₐ[R] B'}
...
@[simp]
lemma algHom_eval₂_algebraMap {R A B : Type*} [CommSemiring R] [Semiring A] [Semiring B]
...
@[simp]
lemma eval₂_algebraMap_X {R A : Type*} [CommSemiring R] [Semiring A] [Algebra R A] (p : R[X])
...
@[simp]
lemma ringHom_eval₂_cast_int_ringHom {R S : Type*} [Ring R] [Ring S] (p : ℤ[X]) (f : R →+* S)
...
@[simp]
lemma eval₂_int_castRingHom_X {R : Type*} [Ring R] (p : ℤ[X]) (f : ℤ[X] →+* R) :
...
@[simp]
lemma adjoin_X : Algebra.adjoin R ({X} : Set R[X]) = ⊤ := by
...
@[ext 1200]
lemma algHom_ext {f g : R[X] →ₐ[R] A} (h : f X = g X) : f = g :=
...
@[simp]
lemma aeval_monomial {n : ℕ} {r : R} : aeval x (monomial n r) = algebraMap _ _ r * x ^ n :=
  eval₂_monomial _ _
#align polynomial.aeval_monomial Polynomial.aeval_monomial

-- porting note: removed `@[simp]` because `simp` can prove this
lemma aeval_X_pow {n : ℕ} : aeval x ((X : R[X]) ^ n) = x ^ n :=
  eval₂_X_pow _ _
set_option linter.uppercaseLean3 false in
#align polynomial.aeval_X_pow Polynomial.aeval_X_pow

-- porting note: removed `@[simp]` because `simp` can prove this
lemma aeval_add : aeval x (p + q) = aeval x p + aeval x q :=
  AlgHom.map_add _ _ _
#align polynomial.aeval_add Polynomial.aeval_add

-- porting note: removed `@[simp]` because `simp` can prove this
lemma aeval_one : aeval x (1 : R[X]) = 1 :=
  AlgHom.map_one _
...
-- porting note: removed `@[simp]` because `simp` can prove this
@[deprecated]
lemma aeval_bit0 : aeval x (bit0 p) = bit0 (aeval x p) :=
  AlgHom.map_bit0 _ _
#align polynomial.aeval_bit0 Polynomial.aeval_bit0

-- porting note: removed `@[simp]` because `simp` can prove this
@[deprecated]
lemma aeval_bit1 : aeval x (bit1 p) = bit1 (aeval x p) :=
  AlgHom.map_bit1 _ _
...
lemma aeval_comp {A : Type*} [CommSemiring A] [Algebra R A] (x : A) :
...
@[simp]
lemma aeval_X_left : aeval (X : R[X]) = AlgHom.id R R[X] :=
...
lemma aeval_algHom_apply {F : Type*} [AlgHomClass F R A B] (f : F) (x : A) (p : R[X]) :
...
@[simp]
lemma aeval_fn_apply {X : Type*} (g : R[X]) (f : X → R) (x : X) :
...
lemma map_aeval_eq_aeval_map {S T U : Type*} [CommSemiring S] [CommSemiring T] [Semiring U]
...
lemma aeval_eq_zero_of_dvd_aeval_eq_zero [CommSemiring S] [CommSemiring T] [Algebra S T]
...
lemma aeval_eq_sum_range' [Algebra R S] {p : R[X]} {n : ℕ} (hn : p.natDegree < n) (x : S) :
...
lemma isRoot_of_aeval_algebraMap_eq_zero [Algebra R S] {p : R[X]}
...
@[simp]
lemma aevalTower_X : aevalTower g y X = y :=
  eval₂_X _ _
set_option linter.uppercaseLean3 false in
...
@[simp]
lemma aevalTower_comp_C : (aevalTower g y : R[X] →+* A').comp C = g :=
...
@[simp]
lemma aevalTower_comp_algebraMap : (aevalTower g y : R[X] →+* A').comp (algebraMap R R[X]) = g :=
...
@[simp]
lemma aevalTower_comp_toAlgHom : (aevalTower g y).comp (IsScalarTower.toAlgHom S R R[X]) = g :=
...
@[simp]
lemma aevalTower_id : aevalTower (AlgHom.id S S) = aeval := by
  ext s
  simp only [eval_X, aevalTower_X, coe_aeval_eq_eval]
#align polynomial.aeval_tower_id Polynomial.aevalTower_id

@[simp]
lemma aevalTower_ofId : aevalTower (Algebra.ofId S A') = aeval := by
  ext
...
lemma dvd_term_of_dvd_eval_of_dvd_terms {z p : S} {f : S[X]} (i : ℕ) (dvd_eval : p ∣ f.eval z)
...
lemma dvd_term_of_isRoot_of_dvd_terms {r p : S} {f : S[X]} (i : ℕ) (hr : f.IsRoot r)
...
-/
lemma eval_mul_X_sub_C {p : R[X]} (r : R) : (p * (X - C r)).eval r = 0 := by
...
lemma not_isUnit_X_sub_C [Nontrivial R] (r : R) : ¬IsUnit (X - C r) :=
...
lemma aeval_endomorphism {M : Type*} [CommRing R] [AddCommGroup M] [Module R M] (f : M →ₗ[R] M)
