...
theorem reverse_natDegree_le (f : R[X]) : f.reverse.natDegree ≤ f.natDegree := by
  ...
  intro n hn
  -- Porting note: Needed to add the below line for different coercion behaviour
  have : ∀ n, Nat.cast n = WithBot.some n := fun _ ↦ rfl
  rw [this (natDegree f), this n, WithBot.coe_lt_coe] at hn
