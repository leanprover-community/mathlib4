...
def parallel.aux1 :
    List (Computation α) × WSequence (Computation α) →
      α ⊕ (List (Computation α) × WSequence (Computation α))
  | (l, S) =>
    rmap
      (fun l' =>
...
def parallel (S : WSequence (Computation α)) : Computation α :=
...
theorem terminates_parallel.aux :
  ...
  · intro s IH l S m
    ...
    · have H2 : corec parallel.aux1 (l, S) = think _ := destruct_eq_think (by
      ...
      have := H1 _ h
      rcases Sequence.destruct S with (_ | ⟨_ | c, S'⟩) <;> simp [parallel.aux1] <;> apply IH <;>
        simp [this]

theorem terminates_parallel {S : WSequence (Computation α)} {c} (h : c ∈ S) [T : Terminates c] :
    Terminates (parallel S) := by
  suffices
    ∀ (n) (l : List (Computation α)) (S c), c ∈ l ∨ some (some c) = Sequence.get? S n →
  ...
  · cases' o with a a
    ...
    have H : Sequence.destruct S = some (some c, Sequence.tail S) := by
      simp [Sequence.destruct, (· <$> ·), ← a]
    induction' h : parallel.aux2 l with a l'
  ...
  · cases' o with a a
    ...
    · have C : corec parallel.aux1 (l, S) = _ := destruct_eq_think (by
      ...
      have TT : ∀ l', Terminates (corec parallel.aux1 (l', S.tail)) := by
        ...
        rw [a]
        rfl
      induction' e : Sequence.get? S 0 with o
      · have D : Sequence.destruct S = none := by
          dsimp [Sequence.destruct]
          rw [e]
          rfl
        rw [D]
        simp only
        have TT := TT l'
        rwa [Sequence.destruct_eq_nil D, Sequence.tail_nil] at TT
      · have D : Sequence.destruct S = some (o, S.tail) := by
          dsimp [Sequence.destruct]
          rw [e]
          rfl
        rw [D]
...
theorem exists_of_mem_parallel {S : WSequence (Computation α)} {a} (h : a ∈ parallel S) :
    ∃ c ∈ S, a ∈ c := by
  suffices
  ...
  · induction' e : Sequence.destruct S with a <;> rw [e] at h'
    · exact
        let ⟨d, o, ad⟩ := IH _ _ h'
        let ⟨c, cl, ac⟩ := this a ⟨d, o.resolve_right (WSequence.not_mem_nil _), ad⟩
        ⟨c, Or.inl cl, ac⟩
    · cases' a with o S'
      ...
      · refine ⟨d, Or.inr ?_, ad⟩
        rw [Sequence.destruct_eq_cons e]
        exact Sequence.mem_cons_of_mem _ dS'
      · simp at dl
        cases' dl with dc dl
        · rw [dc] at ad
          refine ⟨c, Or.inr ?_, ad⟩
          rw [Sequence.destruct_eq_cons e]
          apply Sequence.mem_cons
        · exact
            let ⟨c, cl, ac⟩ := this a ⟨d, dl, ad⟩
            ⟨c, Or.inl cl, ac⟩
      · refine ⟨d, Or.inr ?_, ad⟩
...
theorem map_parallel (f : α → β) (S) : map f (parallel S) = parallel (S.map (map f)) := by
  ...
  exact
    ...
    | _, _, ⟨l, S, rfl, rfl⟩ => by
      ...
      induction' S using WSequence.recOn with c S S <;> simp <;>
        exact ⟨_, _, rfl, rfl⟩

theorem parallel_empty (S : WSequence (Computation α)) (h : S.head ~> none) :
    parallel S = empty _ :=
  eq_empty_of_not_terminates fun ⟨⟨a, m⟩⟩ => by
    let ⟨c, cs, _⟩ := exists_of_mem_parallel m
    let ⟨n, nm⟩ := WSequence.exists_get?_of_mem cs
    let ⟨c', h'⟩ := WSequence.head_some_of_get?_some nm
    injection h h'

-- The reason this isn't trivial from exists_of_mem_parallel is because it eliminates to Sort
def parallelRec {S : WSequence (Computation α)} (C : α → Sort v) (H : ∀ s ∈ S, ∀ a ∈ s, C a) {a}
    (h : a ∈ parallel S) : C a := by
  let T : WSequence (Computation (α × Computation α)) := S.map fun c => c.map fun a => (a, c)
  have : S = T.map (map fun c => c.1) := by
    rw [← WSequence.map_comp]
    refine (WSequence.map_id _).symm.trans (congr_arg (fun f => WSequence.map f S) ?_)
    funext c
  ...
  have : a ∈ c ∧ c ∈ S := by
    ...
    rcases WSequence.exists_of_mem_map dT' with ⟨c', cs', e'⟩
    rw [← e'] at ad'
...
theorem parallel_promises {S : WSequence (Computation α)} {a} (H : ∀ s ∈ S, s ~> a) :
    parallel S ~> a :=
  fun _ ma' =>
  let ⟨_, cs, ac⟩ := exists_of_mem_parallel ma'
  H _ cs ac

theorem mem_parallel {S : WSequence (Computation α)} {a} (H : ∀ s ∈ S, s ~> a) {c} (cs : c ∈ S)
    (ac : a ∈ c) : a ∈ parallel S := by
  haveI := terminates_of_mem ac
  haveI := terminates_parallel cs
  exact mem_of_promises _ (parallel_promises H)

theorem parallel_congr_lem {S T : WSequence (Computation α)} {a} (H : S.LiftRel Equiv T) :
    (∀ s ∈ S, s ~> a) ↔ ∀ t ∈ T, t ~> a :=
  ⟨fun h1 _ tT =>
    let ⟨_, sS, se⟩ := WSequence.exists_of_liftRel_right H tT
    (promises_congr se _).1 (h1 _ sS),
    fun h2 _ sS =>
    let ⟨_, tT, se⟩ := WSequence.exists_of_liftRel_left H sS
...
theorem parallel_congr_left {S T : WSequence (Computation α)} {a} (h1 : ∀ s ∈ S, s ~> a)
  ...
  ⟨fun h => by
    ...
    exact
      ...
      let ⟨t, tT, st⟩ := WSequence.exists_of_liftRel_left H sS
      let aT := (st _).1 as
    ...
    exact
      let ⟨s, sS, as⟩ := exists_of_mem_parallel h
      let ⟨t, tT, st⟩ := WSequence.exists_of_liftRel_right H sS
      let aT := (st _).2 as
      mem_parallel h1 tT aT⟩

theorem parallel_congr_right {S T : WSequence (Computation α)} {a} (h2 : ∀ t ∈ T, t ~> a)
    (H : S.LiftRel Equiv T) : parallel S ~ parallel T :=
