...
/--
The definition of `QuotType`. It is needed to avoid name clashes
between `QuotType.rec` `QuotType.recOn` and the autogenerated recursors.
-/
class QuotTypeStruct (Q : Sort*) (α : outParam Sort*) (r : outParam (α → α → Prop)) where
...
-/
abbrev QuotType := QuotTypeStruct

export QuotTypeStruct (mkQ)

namespace QuotType

export QuotTypeStruct (mkQ toQuot)

alias toQuot_mkQ := QuotTypeStruct.toQuot_mkQ
alias sound := QuotTypeStruct.sound
...
meta def mkQ_Impl : TermElab := fun stx typ? => do
  ...
  if let .some _ ← trySynthInstanceQ q(@Hint $H $h $Q $α $r $inst) then
    return q(@QuotTypeStruct.mkQ $Q $α $r $inst)
  ...
  pure q(@QuotTypeStruct.mkQ $Q $α $r $inst)
...
theorem lift_mkQ {β : Sort*} (f : α → β) (h : ∀ a₁ a₂, r a₁ a₂ → f a₁ = f a₂) (a : α) :
    QuotType.lift f h (⟦a⟧ : Q) = f a := by
  rw [QuotType.lift, toQuot_mkQ]

theorem liftOn_mkQ {β : Sort*} (a : α) (f : α → β) (h : ∀ a₁ a₂, r a₁ a₂ → f a₁ = f a₂) :
    QuotType.liftOn (⟦a⟧ : Q) f h = f a :=
  lift_mkQ f h a

theorem exists_rep (q : Q) : ∃ a, ⟦a⟧ = q :=
  QuotTypeStruct.mkQ_surjective q
...
protected theorem liftIndepPr1
    ...
    (QuotType.lift (QuotType.indep f) (QuotType.indepCoherent f h) q).1 = q := by
...
protected def rec {motive : Q → Sort*}
    ...
    (q : Q) :
    motive q :=
  Eq.ndrecOn (QuotType.liftIndepPr1 f h q)
    ((QuotType.lift (QuotType.indep f) (QuotType.indepCoherent f h) q).2)
...
theorem rec_mkQ {motive : Q → Sort*}
    ...
    (h : (a b : α) → (p : r a b) → Eq.ndrec (f a) (sound p) = f b)
    (a : α) :
    _root_.QuotType.rec (motive := motive) f h ⟦a⟧ = f a := by
  rw [_root_.QuotType.rec, ← heq_iff_eq, eqRec_heq_iff_heq, lift_mkQ]
...
protected abbrev recOnSubsingleton {motive : Q → Sort*}
    ...
    (f : (a : α) → motive ⟦a⟧) :
    motive q :=
  _root_.QuotType.recOn q f (fun _ _ _ ↦ Subsingleton.elim _ _)
...
protected abbrev hrecOn {motive : Q → Sort*}
    ...
    motive q :=
  _root_.QuotType.recOn q f fun a b p ↦ eq_of_heq <| (eqRec_heq_self _ _).trans (h a b p)

theorem hrecOn_mkQ {motive : Q → Sort*}
    (a : α)
    (f : (a : α) → motive ⟦a⟧)
    (h : (a b : α) → r a b → f a ≍ f b) :
    QuotType.hrecOn (motive := motive) ⟦a⟧ f h = f a := by
  simp
...
protected def map (f : α → β) (h : (ra ⇒ rb) f f) : Qa → Qb :=
  (QuotType.lift fun x ↦ ⟦f x⟧) fun _ _ ↦ (QuotType.sound <| h ·)

open Relator in
@[simp]
theorem map_mkQ (f : α → β) (h : (ra ⇒ rb) f f) (a : α) :
    QuotType.map f h (⟦a⟧ : Qa) = (⟦f a⟧ : Qb) :=
  lift_mkQ _ _ _
...
protected def lift₂'
    ...
    Qa → Qb → φ :=
  QuotType.lift (fun a ↦ QuotType.lift (f a) (hb a))
    (fun a₁ a₂ h ↦ funext (QuotType.ind (fun b ↦ by simpa [lift_mkQ] using ha a₁ a₂ b h)))
...
lemma lift₂'_mkQ
    ...
    (hb : ∀ a b₁ b₂, rb b₁ b₂ → f a b₁ = f a b₂)
    (a : α) (b : β) :
    QuotType.lift₂' f ha hb (⟦a⟧ : Qa) (⟦b⟧ : Qb) = f a b := by
  simp [QuotType.lift₂']
...
lemma liftOn₂'_mkQ
    ...
    (ha : ∀ a₁ a₂ b, ra a₁ a₂ → f a₁ b = f a₂ b)
    (hb : ∀ a b₁ b₂, rb b₁ b₂ → f a b₁ = f a b₂) :
    QuotType.liftOn₂' (⟦a⟧ : Qa) (⟦b⟧ : Qb) f ha hb = f a b := by
  simp
...
protected abbrev lift₂ [IsRefl α ra] [IsRefl β rb]
    ...
    Qa → Qb → φ :=
  QuotType.lift₂' f (h · _ · · · (refl _)) (h · · _ · (refl _) ·)

lemma lift₂_mkQ [IsRefl α ra] [IsRefl β rb]
    (f : α → β → φ)
    (h : ∀ a₁ b₁ a₂ b₂, ra a₁ a₂ → rb b₁ b₂ → f a₁ b₁ = f a₂ b₂)
    (a : α) (b : β) :
    QuotType.lift₂ f h (⟦a⟧ : Qa) (⟦b⟧ : Qb) = f a b := by
  simp
...
protected abbrev liftOn₂ [IsRefl α ra] [IsRefl β rb]
  ...
  QuotType.lift₂ f h qa qb

lemma liftOn₂_mkQ [IsRefl α ra] [IsRefl β rb]
    (a : α) (b : β)
    (f : α → β → φ)
    (h : ∀ a₁ b₁ a₂ b₂, ra a₁ a₂ → rb b₁ b₂ → f a₁ b₁ = f a₂ b₂) :
    QuotType.liftOn₂ (⟦a⟧ : Qa) (⟦b⟧ : Qb) f h = f a b := by
  simp
...
protected def map₂ [IsRefl α ra] [IsRefl β rb] (f : α → β → γ)
  ...
  (QuotType.lift₂ fun x y ↦ ⟦f x y⟧) fun _ _ _ _ ↦ (QuotType.sound <| h · ·)

open Relator in
@[simp]
theorem map₂_mkQ [IsRefl α ra] [IsRefl β rb] (f : α → β → γ) (h : (ra ⇒ rb ⇒ rc) f f)
    (a : α) (b : β) :
    QuotType.map₂ f h (⟦a⟧ : Qa) (⟦b⟧ : Qb) = (⟦f a b⟧ : Qc) := by
  simp [QuotType.map₂]
...
protected def recOnSubsingleton₂ {motive : Qa → Qb → Sort*}
    ...
    motive qa qb :=
  QuotType.recOnSubsingleton (_h := fun _ ↦ QuotType.ind inferInstance qb) qa
    fun a ↦ QuotType.recOnSubsingleton qb fun b ↦ f a b
...
protected def hrecOn₂ [IsRefl α ra] [IsRefl β rb] {motive : Qa → Qb → Sort*}
    ...
    motive qa qb :=
  QuotType.hrecOn qa
    (fun a ↦ QuotType.hrecOn qb (f a) (fun b₁ b₂ pb ↦ h _ _ _ _ (refl _) pb))
...
theorem hrecOn₂_mkQ [IsRefl α ra] [IsRefl β rb] {motive : Qa → Qb → Sort*}
    ...
    (h : ∀ a₁ b₁ a₂ b₂, ra a₁ a₂ → rb b₁ b₂ → f a₁ b₁ ≍ f a₂ b₂) :
    QuotType.hrecOn₂ (motive := motive) ⟦a⟧ ⟦b⟧ f h = f a b := by
  simp [QuotType.hrecOn₂]
...
instance (priority := 800) [IsEquiv α r] [dec : DecidableRel r] : DecidableEq Q :=
  fun q₁ q₂ ↦ QuotType.recOnSubsingleton₂ q₁ q₂ fun a₁ a₂ ↦
    match dec a₁ a₂ with
...
theorem mkQ_surjective :
  ...
  QuotTypeStruct.mkQ_surjective

@[simp]
theorem lift_surjective {β : Sort*} {f : α → β} {h : ∀ a b, r a b → f a = f b} :
    Function.Surjective (QuotType.lift f h : Q → β) ↔ Function.Surjective f :=
  ⟨fun hf => by simpa only [lift_mkQ, Function.comp_def] using hf.comp QuotType.exists_rep,
    fun hf y => let ⟨x, hx⟩ := hf y; ⟨mkQ x, by simpa only [lift_mkQ] using hx⟩⟩

@[simp]
lemma liftOn_surjective {β : Sort*} {f : α → β} {h : ∀ a b, r a b → f a = f b} :
    Function.Surjective (fun x : Q ↦ QuotType.liftOn x f h) ↔ Function.Surjective f :=
  lift_surjective

@[simp]
theorem lift_comp_mkQ {β : Sort*} {f : α → β} (h : ∀ a b, r a b → f a = f b) :
    (QuotType.lift f h : Q → β) ∘ mkQ = f :=
  funext <| QuotType.lift_mkQ f h

instance (priority := 800) (f : α → Prop) [hf : DecidablePred f] (h : ∀ a b, r a b → f a = f b) :
    DecidablePred (QuotType.lift f h : Q → Prop) :=
  fun q ↦ QuotType.recOnSubsingleton q fun _ ↦ by simpa using hf _
...
instance (priority := 800)
    ...
    (h : ∀ a₁ b₁ a₂ b₂, ra a₁ a₂ → rb b₁ b₂ → f a₁ b₁ = f a₂ b₂)
    (qa : Qa) (qb : Qb) :
    Decidable (QuotType.lift₂ f h qa qb) :=
  QuotType.recOnSubsingleton₂ qa qb fun _ _ ↦ by simpa using hf _ _
