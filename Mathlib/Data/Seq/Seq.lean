...
theorem val_cons (s : Seq' α) (x : α) : (cons x s).val = some x::s.val :=
  rfl
#align stream.seq.val_cons Seq'.val_cons
...
theorem get?_mk (f hf) : @get? α ⟨f, hf⟩ = f :=
  rfl
#align stream.seq.nth_mk Seq'.get?_mk
...
def omap (f : β → γ) : Option (α × β) → Option (α × γ)
  | none => none
  | some (a, b) => some (a, f b)
#align stream.seq.omap Seq'.omap
...
#align stream.seq.destruct Seq'.destruct
...
theorem destruct_eq_nil {s : Seq' α} : destruct s = none → s = nil := by
  ...
  · apply Subtype.eq
    ...
    exacts [f0, s.2 IH]
  · contradiction
#align stream.seq.destruct_eq_nil Seq'.destruct_eq_nil
...
#align stream.seq.destruct_eq_cons Seq'.destruct_eq_cons

@[simp]
theorem destruct_nil : destruct (nil : Seq' α) = none :=
  rfl
#align stream.seq.destruct_nil Seq'.destruct_nil
...
#align stream.seq.destruct_cons Seq'.destruct_cons
...
#align stream.seq.head_eq_destruct Seq'.head_eq_destruct
...
theorem get?_tail (s : Seq' α) (n) : get? (tail s) n = get? s (n + 1) :=
  rfl
#align stream.seq.nth_tail Seq'.get?_tail
...
theorem ofList_get (l : List α) (n : ℕ) : (ofList l).get? n = l.get? n :=
  rfl
#align stream.seq.of_list_nth Seq'.ofList_get
...
def drop (s : Seq' α) : ℕ → Seq' α
  | 0 => s
  | n + 1 => tail (drop s n)
#align stream.seq.drop Seq'.drop
...
#align stream.seq.head_dropn Seq'.head_dropn
...
def ret (a : α) : Seq1 α :=
  (a, nil)
#align stream.seq1.ret Seq1.ret
...
#align stream.seq1.join_map_ret Seq1.join_map_ret
...
theorem bind_ret (f : α → β) : ∀ s, bind s (ret ∘ f) = map f s
  | ⟨a, s⟩ => by
    ...
    rw [map_comp, ret]
    simp
#align stream.seq1.bind_ret Seq1.bind_ret
