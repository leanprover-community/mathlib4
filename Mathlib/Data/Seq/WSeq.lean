...
#align stream.wseq.destruct WSeq.destruct
...
#align stream.wseq.lift_rel_destruct WSeq.liftRel_destruct
...
theorem liftRel_destruct_iff {R : α → β → Prop} {s : WSeq α} {t : WSeq β} :
  ...
  ⟨liftRel_destruct, fun h =>
    ⟨fun s t =>
      ...
      intro s t
      apply Or.inl⟩⟩
#align stream.wseq.lift_rel_destruct_iff WSeq.liftRel_destruct_iff
...
theorem destruct_congr {s t : WSeq α} :
  ...
  liftRel_destruct
#align stream.wseq.destruct_congr WSeq.destruct_congr

theorem destruct_congr_iff {s t : WSeq α} :
    s ~ʷ t ↔ Computation.LiftRel (BisimO (· ~ʷ ·)) (destruct s) (destruct t) :=
  liftRel_destruct_iff
#align stream.wseq.destruct_congr_iff WSeq.destruct_congr_iff
...
#align stream.wseq.destruct_nil WSeq.destruct_nil
...
#align stream.wseq.destruct_cons WSeq.destruct_cons
...
#align stream.wseq.destruct_think WSeq.destruct_think

@[simp]
theorem seq_destruct_nil : Seq'.destruct (nil : WSeq α) = none :=
  Seq'.destruct_nil
#align stream.wseq.seq_destruct_nil WSeq.seq_destruct_nil

@[simp]
theorem seq_destruct_cons (a : α) (s) : Seq'.destruct (cons a s) = some (some a, s) :=
  Seq'.destruct_cons _ _
#align stream.wseq.seq_destruct_cons WSeq.seq_destruct_cons

@[simp]
theorem seq_destruct_think (s : WSeq α) : Seq'.destruct (think s) = some (none, s) :=
  Seq'.destruct_cons _ _
#align stream.wseq.seq_destruct_think WSeq.seq_destruct_think
...
#align stream.wseq.destruct_flatten WSeq.destruct_flatten
...
#align stream.wseq.dropn_nil WSeq.dropn_nil
...
#align stream.wseq.destruct_tail WSeq.destruct_tail
...
theorem destruct_dropn : ∀ (s : WSeq α) (n), destruct (drop s n) = destruct s >>= drop.aux n
  ...
  | s, n + 1 => by
    ...
    rfl
#align stream.wseq.destruct_dropn WSeq.destruct_dropn
...
theorem destruct_some_of_destruct_tail_some {s : WSeq α} {a} (h : some a ∈ destruct (tail s)) :
  ...
  · have := mem_unique td (ret_mem _)
    contradiction
  · exact ⟨_, ht'⟩
#align stream.wseq.destruct_some_of_destruct_tail_some WSeq.destruct_some_of_destruct_tail_some
...
#align stream.wseq.productive_dropn WSeq.productive_dropn
...
#align stream.wseq.mem_of_mem_dropn WSeq.mem_of_mem_dropn
...
theorem exists_dropn_of_mem {s : WSeq α} {a} (h : a ∈ s) :
  ...
  ⟨n, by
    ...
    rw [i] at om
    exact ⟨_, om⟩⟩
#align stream.wseq.exists_dropn_of_mem WSeq.exists_dropn_of_mem
...
#align stream.wseq.lift_rel_dropn_destruct WSeq.liftRel_dropn_destruct
...
#align stream.wseq.dropn_congr WSeq.dropn_congr
...
theorem destruct_ofSeq (s : Seq' α) :
  ...
  destruct_eq_pure <| by
    ...
    dsimp only [(· <$> ·)]
    simp [destruct]
#align stream.wseq.destruct_of_seq WSeq.destruct_ofSeq
...
#align stream.wseq.dropn_of_seq WSeq.dropn_ofSeq
...
def ret (a : α) : WSeq α :=
  ofList [a]
#align stream.wseq.ret WSeq.ret
...
#align stream.wseq.map_ret WSeq.map_ret
...
theorem destruct_map (f : α → β) (s : WSeq α) :
  ...
  · intro c1 c2 h
    ...
    exact ⟨s, rfl, rfl⟩
  · exact ⟨s, rfl, rfl⟩
#align stream.wseq.destruct_map WSeq.destruct_map
...
#align stream.wseq.destruct_append.aux WSeq.destruct_append.aux
...
theorem destruct_append (s t : WSeq α) :
  ...
  · exact ⟨s, t, rfl, rfl⟩
#align stream.wseq.destruct_append WSeq.destruct_append
...
#align stream.wseq.destruct_join.aux WSeq.destruct_join.aux
...
#align stream.wseq.destruct_join WSeq.destruct_join
...
#align stream.wseq.join_ret WSeq.join_ret
...
theorem join_map_ret (s : WSeq α) : join (map ret s) ~ʷ s := by
  ...
  · exact ⟨s, rfl, rfl⟩
#align stream.wseq.join_map_ret WSeq.join_map_ret
...
theorem bind_ret (f : α → β) (s) : bind s (ret ∘ f) ~ʷ map f s := by
  ...
  rw [map_comp]
  apply join_map_ret
#align stream.wseq.bind_ret WSeq.bind_ret

@[simp]
theorem ret_bind (a : α) (f : α → WSeq β) : bind (ret a) f ~ʷ f a := by simp [bind]
#align stream.wseq.ret_bind WSeq.ret_bind
