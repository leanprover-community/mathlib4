...
theorem Pairwise.coind_trans {R : α → α → Prop} [IsTrans α R] {s : Seq α}
  ...
  | succ k ih =>
    obtain ⟨z, hz⟩ := ge_stable (m := i + k + 1) _ (by cutsat) hy
