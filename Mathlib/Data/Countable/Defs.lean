...
instance (priority := 10000) : Countable ℕ :=
  ⟨⟨id, injective_id⟩⟩
...
instance (priority := 10000) {β : Type v} [Countable β] : Countable (ULift.{u} β) :=
...
instance (priority := 10000) [Countable α] : Countable (PLift α) :=
...
instance (priority := 10000) {n : ℕ} : Countable (Fin n) :=
...
instance (priority := 10000) : Countable PUnit.{u} :=
  Subsingleton.to_countable

instance (priority := 100) Prop.countable (p : Prop) : Countable p :=
  Subsingleton.to_countable

instance (priority := 10000) Bool.countable : Countable Bool :=
  ⟨⟨fun b => cond b 0 1, Bool.injective_iff.2 Nat.one_ne_zero⟩⟩

instance (priority := 10000) Prop.countable' : Countable Prop :=
...
instance (priority := 10000) {β : Type v} [Uncountable β] : Uncountable (ULift.{u} β) :=
  .of_equiv _ Equiv.ulift.symm

instance (priority := 10000) [Uncountable α] : Uncountable (PLift α) :=
