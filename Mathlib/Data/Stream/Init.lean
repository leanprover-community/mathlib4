...
@[ext]
protected lemma ext {s₁ s₂ : Stream' α} : (∀ n, get s₁ n = get s₂ n) → s₁ = s₂ :=
  fun h => funext h
#align stream.ext Stream'.ext

@[simp]
lemma get_zero_cons (a : α) (s : Stream' α) : get (a::s) 0 = a :=
  rfl
...
@[simp]
lemma get_drop (n m : Nat) (s : Stream' α) : get (drop m s) n = get s (n + m) :=
  rfl
...
@[simp] lemma get_tail {s : Stream' α} : s.tail.get n = s.get (n + 1) := rfl

@[simp] lemma tail_drop' {s : Stream' α} : tail (drop i s) = s.drop (i+1) := by
...
lemma get_succ (n : Nat) (s : Stream' α) : get s (succ n) = get (tail s) n :=
  rfl
#align stream.nth_succ Stream'.get_succ

@[simp]
lemma get_succ_cons (n : Nat) (s : Stream' α) (x : α) : get (x::s) n.succ = get s n :=
  rfl
...
lemma head_drop (a : Stream' α) (n : ℕ) : (a.drop n).head = a.get n := by simp
...
lemma all_def (p : α → Prop) (s : Stream' α) : All p s = ∀ n, p (get s n) :=
  rfl
#align stream.all_def Stream'.all_def

lemma any_def (p : α → Prop) (s : Stream' α) : Any p s = ∃ n, p (get s n) :=
  rfl
...
lemma mem_cons_of_mem {a : α} {s : Stream' α} (b : α) : a ∈ s → a ∈ b::s := fun ⟨n, h⟩ =>
...
lemma mem_of_get_eq {n : Nat} {s : Stream' α} {a : α} : a = get s n → a ∈ s := fun h =>
  Exists.intro n h
...
@[simp]
lemma get_map (n : Nat) (s : Stream' α) : get (map f s) n = f (get s n) :=
  rfl
...
lemma mem_map {a : α} {s : Stream' α} : a ∈ s → f a ∈ map f s := fun ⟨n, h⟩ =>
...
@[simp]
lemma get_zip (n : Nat) (s₁ : Stream' α) (s₂ : Stream' β) :
...
@[simp]
lemma get_enum (s : Stream' α) (n : ℕ) : get (enum s) n = (n, s.get n) :=
  rfl
...
@[simp]
lemma get_const (n : Nat) (a : α) : get (const a) n = a :=
  rfl
...
lemma get_succ_iterate' (n : Nat) (f : α → α) (a : α) :
...
@[simp]
lemma get_zero_iterate (f : α → α) (a : α) : get (iterate f a) 0 = a :=
  rfl
...
lemma get_of_bisim (bisim : IsBisimulation R) :
...
lemma get_unfolds_head_tail : ∀ (n : Nat) (s : Stream' α),
...
lemma unfolds_head_eq : ∀ s : Stream' α, unfolds head tail s = s := fun s =>
...
lemma get_interleave_left : ∀ (n : Nat) (s₁ s₂ : Stream' α),
    get (s₁ ⋈ s₂) (2 * n) = get s₁ n
  | 0, s₁, s₂ => rfl
  | n + 1, s₁, s₂ => by
...
lemma get_interleave_right : ∀ (n : Nat) (s₁ s₂ : Stream' α),
...
lemma get_even : ∀ (n : Nat) (s : Stream' α), get (even s) n = get s (2 * n)
  | 0, s => rfl
  | succ n, s => by
...
lemma get_odd : ∀ (n : Nat) (s : Stream' α), get (odd s) n = get s (2 * n + 1) := fun n s => by
...
lemma append_append_stream : ∀ (l₁ l₂ : List α) (s : Stream' α),
    l₁ ++ l₂ ++ₛ s = l₁ ++ₛ (l₂ ++ₛ s)
  | [], l₂, s => rfl
...
lemma take_succ' {s : Stream' α} : ∀ n, s.take (n+1) = s.take n ++ [s.get n]
  | 0 => rfl
...
@[simp]
lemma take_take {s : Stream' α} : ∀ {m n}, (s.take n).take m = s.take (min n m)
...
@[simp] lemma concat_take_get {s : Stream' α} : s.take n ++ [s.get n] = s.take (n+1) :=
  (take_succ' n).symm

lemma get?_take {s : Stream' α} : ∀ {k n}, k < n → (s.take n).get? k = s.get k
  | 0, n+1, _ => rfl
...
@[simp] lemma dropLast_take {xs : Stream' α} :
...
@[simp]
lemma append_take_drop : ∀ (n : Nat) (s : Stream' α),
...
lemma mem_cycle {a : α} {l : List α} : ∀ h : l ≠ [], a ∈ l → a ∈ cycle l h := fun h ainl => by
...
@[simp]
lemma get_tails : ∀ (n : Nat) (s : Stream' α), get (tails s) n = drop n (tail s) := by
  intro n; induction' n with n' ih
  · intros
    rfl
  · intro s
...
@[simp]
lemma get_inits : ∀ (n : Nat) (s : Stream' α), get (inits s) n = take (succ n) s := by
  intro n; induction' n with n' ih
  · intros
    rfl
  · intros
...
lemma get_nats (n : Nat) : get nats n = n :=
  rfl
#align stream.nth_nats Stream'.get_nats

lemma nats_eq : nats = cons 0 (map succ nats) := by
  apply Stream'.ext; intro n
  cases n
  · rfl
  rw [get_succ]; rfl
