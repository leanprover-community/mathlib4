...
lemma col_injective : Function.Injective (col : (m → α) → _) :=
  fun _x _y h => funext fun i => congr_fun₂ h i ()

@[simp] lemma col_inj {v w : m → α} : col v = col w ↔ v = w := col_injective.eq_iff

@[simp] lemma col_zero [Zero α] : col (0 : m → α) = 0 := rfl

@[simp] lemma col_eq_zero [Zero α] (v : m → α) : col v = 0 ↔ v = 0 := col_inj

@[simp]
lemma col_add [Add α] (v w : m → α) : col (v + w) = col v + col w := by
  ext
  rfl
#align matrix.col_add Matrix.col_add

@[simp]
lemma col_smul [SMul R α] (x : R) (v : m → α) : col (x • v) = x • col v := by
  ext
  rfl
#align matrix.col_smul Matrix.col_smul

lemma row_injective : Function.Injective (row : (n → α) → _) :=
  fun _x _y h => funext fun j => congr_fun₂ h () j

@[simp] lemma row_inj {v w : n → α} : row v = row w ↔ v = w := row_injective.eq_iff

@[simp] lemma row_zero [Zero α] : row (0 : n → α) = 0 := rfl

@[simp] lemma row_eq_zero [Zero α] (v : n → α) : row v = 0 ↔ v = 0 := row_inj

@[simp]
lemma row_add [Add α] (v w : m → α) : row (v + w) = row v + row w := by
  ext
  rfl
#align matrix.row_add Matrix.row_add

@[simp]
lemma row_smul [SMul R α] (x : R) (v : m → α) : row (x • v) = x • row v := by
  ext
  rfl
...
@[simp]
lemma conjTranspose_col [Star α] (v : m → α) : (col v)ᴴ = row (star v) := by
  ext
  rfl
#align matrix.conj_transpose_col Matrix.conjTranspose_col

@[simp]
lemma conjTranspose_row [Star α] (v : m → α) : (row v)ᴴ = col (star v) := by
  ext
  rfl
#align matrix.conj_transpose_row Matrix.conjTranspose_row

lemma row_vecMul [Fintype m] [NonUnitalNonAssocSemiring α] (M : Matrix m n α) (v : m → α) :
    Matrix.row (v ᵥ* M) = Matrix.row v * M := by
  ext
  rfl
#align matrix.row_vec_mul Matrix.row_vecMul

lemma col_vecMul [Fintype m] [NonUnitalNonAssocSemiring α] (M : Matrix m n α) (v : m → α) :
    Matrix.col (v ᵥ* M) = (Matrix.row v * M)ᵀ := by
  ext
  rfl
#align matrix.col_vec_mul Matrix.col_vecMul

lemma col_mulVec [Fintype n] [NonUnitalNonAssocSemiring α] (M : Matrix m n α) (v : n → α) :
    Matrix.col (M *ᵥ v) = M * Matrix.col v := by
  ext
  rfl
#align matrix.col_mul_vec Matrix.col_mulVec

lemma row_mulVec [Fintype n] [NonUnitalNonAssocSemiring α] (M : Matrix m n α) (v : n → α) :
...
@[simp]
lemma updateRow_self [DecidableEq m] : updateRow M i b i = b :=
...
lemma updateRow_apply [DecidableEq m] {i' : m} :
...
lemma updateColumn_apply [DecidableEq n] {j' : n} :
...
@[simp]
lemma updateColumn_subsingleton [Subsingleton n] (A : Matrix m n R) (i : n) (b : m → R) :
...
@[simp]
lemma updateRow_subsingleton [Subsingleton m] (A : Matrix m n R) (i : m) (b : n → R) :
...
lemma map_updateRow [DecidableEq m] (f : α → β) :
...
lemma map_updateColumn [DecidableEq n] (f : α → β) :
...
lemma updateRow_transpose [DecidableEq n] : updateRow Mᵀ j c = (updateColumn M j c)ᵀ := by
  ext
  rw [transpose_apply, updateRow_apply, updateColumn_apply]
  rfl
#align matrix.update_row_transpose Matrix.updateRow_transpose

lemma updateColumn_transpose [DecidableEq m] : updateColumn Mᵀ i b = (updateRow M i b)ᵀ := by
  ext
  rw [transpose_apply, updateRow_apply, updateColumn_apply]
  rfl
#align matrix.update_column_transpose Matrix.updateColumn_transpose

lemma updateRow_conjTranspose [DecidableEq n] [Star α] :
...
lemma updateColumn_conjTranspose [DecidableEq m] [Star α] :
...
@[simp]
lemma updateRow_eq_self [DecidableEq m] (A : Matrix m n α) (i : m) : A.updateRow i (A i) = A :=
  Function.update_eq_self i A
#align matrix.update_row_eq_self Matrix.updateRow_eq_self

@[simp]
lemma updateColumn_eq_self [DecidableEq n] (A : Matrix m n α) (i : n) :
...
lemma diagonal_updateColumn_single [DecidableEq n] [Zero α] (v : n → α) (i : n) (x : α) :
...
lemma diagonal_updateRow_single [DecidableEq n] [Zero α] (v : n → α) (i : n) (x : α) :
...
lemma updateRow_submatrix_equiv [DecidableEq l] [DecidableEq m] (A : Matrix m n α) (i : l)
    (r : o → α) (e : l ≃ m) (f : o ≃ n) :
...
lemma submatrix_updateRow_equiv [DecidableEq l] [DecidableEq m] (A : Matrix m n α) (i : m)
    (r : n → α) (e : l ≃ m) (f : o ≃ n) :
...
lemma updateColumn_submatrix_equiv [DecidableEq o] [DecidableEq n] (A : Matrix m n α) (j : o)
...
lemma submatrix_updateColumn_equiv [DecidableEq o] [DecidableEq n] (A : Matrix m n α) (j : n)
...
lemma updateRow_reindex [DecidableEq l] [DecidableEq m] (A : Matrix m n α) (i : l) (r : o → α)
    (e : m ≃ l) (f : n ≃ o) :
...
lemma reindex_updateRow [DecidableEq l] [DecidableEq m] (A : Matrix m n α) (i : m) (r : n → α)
    (e : m ≃ l) (f : n ≃ o) :
...
lemma updateColumn_reindex [DecidableEq o] [DecidableEq n] (A : Matrix m n α) (j : o) (c : l → α)
    (e : m ≃ l) (f : n ≃ o) :
...
lemma reindex_updateColumn [DecidableEq o] [DecidableEq n] (A : Matrix m n α) (j : n) (c : m → α)
    (e : m ≃ l) (f : n ≃ o) :
