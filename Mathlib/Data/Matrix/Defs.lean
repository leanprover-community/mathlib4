...
/-- Given maps `(f : l → m)` and `(g : o → n)` reindexing the rows and columns of
a matrix `M : Matrix m n α`, the matrix `M.submatrix f g : Matrix l o α` is defined
by `(M.submatrix f g) i j = M (f i) (g j)` for `(i,j) : l × o`.
Note that the total number of row and columns does not have to be preserved. -/
def submatrix (A : Matrix m n α) (f : l → m) (g : o → n) : Matrix l o α :=
  of fun i j => A (f i) (g j)

@[simp]
theorem submatrix_apply (A : Matrix m n α) (f : l → m) (g : o → n) (i j) :
...
@[simp]
theorem transpose_submatrix (A : Matrix m n α) (f : l → m) (g : o → n) :
    (A.submatrix f g)ᵀ = Aᵀ.submatrix g f :=
  ext fun _ _ => rfl
