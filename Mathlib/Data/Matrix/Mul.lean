...
Put another way, `M *ᵥ v` is the vector whose entries are those of `M * replicateCol ι v`
(see `replicateCol_mulVec`).
...
Put another way, `v ᵥ* M` is the vector whose entries are those of `replicateRow ι v * M`
(see `replicateRow_vecMul`).
