...
@[simp]
lemma smul_stdBasisMatrix [SMulZeroClass R α] (r : R) (i : m) (j : n) (a : α) :
...
@[simp]
lemma stdBasisMatrix_zero (i : m) (j : n) : stdBasisMatrix i j (0 : α) = 0 := by
  unfold stdBasisMatrix
  ext
  simp
#align matrix.std_basis_matrix_zero Matrix.stdBasisMatrix_zero

lemma stdBasisMatrix_add (i : m) (j : n) (a b : α) :
...
lemma mulVec_stdBasisMatrix [Fintype m] (i : n) (j : m) (c : α) (x : m → α) :
...
@[elab_as_elim]
protected lemma induction_on' [Finite m] [Finite n] {P : Matrix m n α → Prop} (M : Matrix m n α)
...
@[elab_as_elim]
protected lemma induction_on [Finite m] [Finite n] [Nonempty m] [Nonempty n]
...
@[simp]
lemma apply_of_ne (h : ¬(i = i' ∧ j = j')) : stdBasisMatrix i j c i' j' = 0 := by
...
@[simp]
lemma apply_of_col_ne (i i' : m) {j j' : n} (hj : j ≠ j') (a : α) :
...
@[simp]
lemma diag_zero (h : j ≠ i) : diag (stdBasisMatrix i j c) = 0 :=
...
@[simp]
lemma trace_zero (h : j ≠ i) : trace (stdBasisMatrix i j c) = 0 := by
...
@[simp]
lemma mul_left_apply_same (b : n) (M : Matrix n n α) :
...
@[simp]
lemma mul_right_apply_same (a : n) (M : Matrix n n α) :
...
@[simp]
lemma mul_left_apply_of_ne (a b : n) (h : a ≠ i) (M : Matrix n n α) :
...
@[simp]
lemma mul_right_apply_of_ne (a b : n) (hbj : b ≠ j) (M : Matrix n n α) :
...
lemma row_eq_zero_of_commute_stdBasisMatrix {i j k : n} {M : Matrix n n α}
    (hM : Commute (stdBasisMatrix i j 1) M) (hkj : k ≠ j) : M j k = 0 := by
  have := ext_iff.mpr hM i k
  aesop

lemma col_eq_zero_of_commute_stdBasisMatrix {i j k : n} {M : Matrix n n α}
    (hM : Commute (stdBasisMatrix i j 1) M) (hki : k ≠ i) : M k i = 0 := by
  have := ext_iff.mpr hM k j
  aesop

lemma diag_eq_of_commute_stdBasisMatrix {i j : n} {M : Matrix n n α}
...
lemma mem_range_scalar_iff_commute_stdBasisMatrix {M : Matrix n n α} :
