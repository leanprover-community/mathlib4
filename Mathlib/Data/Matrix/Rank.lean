...
lemma rank_mul_le_right [StrongRankCondition R] (A : Matrix m n R) (B : Matrix n o R) :
    (A * B).rank ≤ B.rank := by
  rw [rank, rank, mulVecLin_mul]
...
lemma rank_eq_finrank_range_toLin [Finite m] [DecidableEq n] {M₁ M₂ : Type*} [AddCommGroup M₁]
...
lemma rank_le_card_height [Fintype m] [StrongRankCondition R] (A : Matrix m n R) :
