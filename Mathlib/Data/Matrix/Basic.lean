...
lemma map_smul' [Mul α] [Mul β] (f : α → β) (r : α) (A : Matrix n n α)
...
lemma map_op_smul' [Mul α] [Mul β] (f : α → β) (r : α) (A : Matrix n n α)
...
@[simp]
lemma diagonal_smul [Monoid R] [AddMonoid α] [DistribMulAction R α] (r : R) (d : n → α) :
...
@[simp]
lemma diagonal_neg [DecidableEq n] [AddGroup α] (d : n → α) :
...
@[simp]
lemma coe_scalar : (scalar n : α → Matrix n n α) = fun a => a • (1 : Matrix n n α) :=
  rfl
...
lemma scalar_inj [Nonempty n] {r s : α} : scalar n r = scalar n s ↔ r = s := by
  constructor
  · intro h
    inhabit n
...
lemma smul_eq_mul_diagonal [DecidableEq n] (M : Matrix m n α) (a : α) :
...
lemma scalar.commute [DecidableEq n] (r : α) (M : Matrix n n α) : Commute (scalar n r) M := by
...
lemma dotProduct_mulVec [Fintype n] [Fintype m] [NonUnitalSemiring R] (v : m → R)
...
@[simp]
lemma mulVec_zero [Fintype n] (A : Matrix m n α) : mulVec A 0 = 0 := by
  ext
  simp [mulVec]
#align matrix.mul_vec_zero Matrix.mulVec_zero

@[simp]
lemma zero_vecMul [Fintype m] (A : Matrix m n α) : vecMul 0 A = 0 := by
  ext
  simp [vecMul]
#align matrix.zero_vec_mul Matrix.zero_vecMul

@[simp]
lemma zero_mulVec [Fintype n] (v : n → α) : mulVec (0 : Matrix m n α) v = 0 := by
  ext
  simp [mulVec]
#align matrix.zero_mul_vec Matrix.zero_mulVec

@[simp]
lemma vecMul_zero [Fintype m] (v : m → α) : vecMul v (0 : Matrix m n α) = 0 := by
  ext
  simp [vecMul]
...
lemma mulVec_one [Fintype n] (A : Matrix m n α) : mulVec A 1 = fun i => ∑ j, A i j := by
  ext; simp [mulVec, dotProduct]
#align matrix.mul_vec_one Matrix.mulVec_one

lemma vec_one_mul [Fintype m] (A : Matrix m n α) : vecMul 1 A = fun j => ∑ i, A i j := by
...
lemma neg_vecMul [Fintype m] (v : m → α) (A : Matrix m n α) : vecMul (-v) A = -vecMul v A := by
  ext
  apply neg_dotProduct
#align matrix.neg_vec_mul Matrix.neg_vecMul

lemma vecMul_neg [Fintype m] (v : m → α) (A : Matrix m n α) : vecMul v (-A) = -vecMul v A := by
  ext
  apply dotProduct_neg
#align matrix.vec_mul_neg Matrix.vecMul_neg

lemma neg_mulVec [Fintype n] (v : n → α) (A : Matrix m n α) : mulVec (-A) v = -mulVec A v := by
  ext
  apply neg_dotProduct
#align matrix.neg_mul_vec Matrix.neg_mulVec

lemma mulVec_neg [Fintype n] (v : n → α) (A : Matrix m n α) : mulVec A (-v) = -mulVec A v := by
  ext
  apply dotProduct_neg
...
lemma mulVec_transpose [Fintype m] (A : Matrix m n α) (x : m → α) : mulVec Aᵀ x = vecMul x A := by
  ext
  apply dotProduct_comm
#align matrix.mul_vec_transpose Matrix.mulVec_transpose

lemma vecMul_transpose [Fintype n] (A : Matrix m n α) (x : n → α) : vecMul x Aᵀ = mulVec A x := by
  ext
  apply dotProduct_comm
...
-/
@[simp]
lemma conjTranspose_apply [Star α] (M : Matrix m n α) (i j) :
...
-/
@[simp]
lemma conjTranspose_smul [Star R] [Star α] [SMul R α] [StarModule R α] (c : R)
...
@[simp]
lemma conjTranspose_eq_transpose_of_trivial [Star α] [TrivialStar α] (A : Matrix m n α) :
...
lemma star_mul [Fintype n] [NonUnitalSemiring α] [StarRing α] (M N : Matrix n n α) :
...
lemma submatrix_diagonal [Zero α] [DecidableEq m] [DecidableEq l] (d : m → α) (e : l → m)
