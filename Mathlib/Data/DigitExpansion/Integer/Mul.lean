...
lemma iterate_shift_neg (x : realHensel Z b) (k : ℕ) : shift^[k] (-x) = - shift^[k] x := by
  ...
  · simp
  · simp only [Function.iterate_succ_apply', IH, Subtype.ext_iff, val_shift]
    simp only [NegMemClass.coe_neg, val_iterate_shift, val_shift, shift_neg]

lemma shift_add_hom_comm {F : Type*}
    [FunLike F (realHensel Z b) (realHensel Z b)]
    [AddMonoidHomClass F (realHensel Z b) (realHensel Z b)]
    (T : F) (x : realHensel Z b) : shift (T x) = T (shift x) := by
  rw [← base_nsmul_shift_eq x, map_nsmul, shift_nsmul_comm, base_nsmul_shift_eq,
      base_nsmul_shift_eq]

lemma iterate_shift_add_hom_comm {F : Type*}
    [FunLike F (realHensel Z b) (realHensel Z b)]
    [AddMonoidHomClass F (realHensel Z b) (realHensel Z b)]
...
lemma unique_add_hom (h : realHensel Z b) :
  ...
  · rw [← neg_zero, le_neg] at fneg
    ...
    · simp only [smul_neg, ← hk, hr]
      ...
      · rw [← neg_neg x, map_neg]
        obtain ⟨l, s, hx⟩ := exists_shift_nsmul_one_of_nonpos xneg
        rw [← hx, neg_nsmul, iterate_shift_neg, iterate_shift_nsmul_comm _ k,
            ← iterate_shift_add_hom_comm, map_nsmul, hU, ← iterate_shift_nsmul_comm,
            iterate_shift_neg, ← Function.iterate_add_apply, ← Function.iterate_add_apply,
            neg_nsmul]
