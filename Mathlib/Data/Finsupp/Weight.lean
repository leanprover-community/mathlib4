...
theorem degree_mapDomain_eq {τ : Type*} (f : σ → τ) [AddCommMonoid M] [Subsingleton (AddUnits M)]
    (x : σ →₀ M) : degree (x.mapDomain f) = degree x := by
  classical
...
theorem degree_comapDomain_le {τ : Type*} {f : σ → τ} [AddCommMonoid M] [PartialOrder M]
