...
instance (priority := 10000) [Encodable α] [Encodable β] [Zero β] [∀ x : β, Decidable (x ≠ 0)] : Encodable (α →₀ β) :=
  letI : DecidableEq α := Encodable.decidableEqOfEncodable _
  .ofEquiv _ finsuppEquivDFinsupp

instance (priority := 10000) [Countable α] [Countable β] [Zero β] : Countable (α →₀ β) := by
