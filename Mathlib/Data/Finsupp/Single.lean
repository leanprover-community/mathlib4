...
theorem support_single_ne_bot (i : α) (h : b ≠ 0) : (single i b).support ≠ ⊥ := by
  simpa only [support_single_of_ne_zero _ h] using singleton_ne_empty _

theorem support_single_disjoint {b' : M} (hb : b ≠ 0) (hb' : b' ≠ 0) {i j : α} :
    Disjoint (single i b).support (single j b').support ↔ i ≠ j := by
  rw [support_single_of_ne_zero _ hb, support_single_of_ne_zero _ hb', disjoint_singleton]
...
theorem support_eq_singleton {f : α →₀ M} {a : α} :
  ...
  ⟨fun h =>
    ...
    fun h => h.2.symm ▸ support_single_of_ne_zero _ h.1⟩
...
theorem support_eq_singleton' {f : α →₀ M} {a : α} :
  ...
  ⟨fun h =>
    ...
    ⟨_, h.1, h.2⟩,
    fun ⟨_b, hb, hf⟩ => hf.symm ▸ support_single_of_ne_zero _ hb⟩
...
theorem single_of_embDomain_single (l : α →₀ M) (f : α ↪ β) (a : β) (b : M) (hb : b ≠ 0)
  ...
  classical
    have h_map_support : Finset.map f l.support = {a} := by
      rw [← support_embDomain, h, support_single_of_ne_zero _ hb]
