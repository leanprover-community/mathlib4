...
This file provides results about the `FinsuppWith.support` of sums of collections of `Finsupp`,
...
theorem List.support_sum_subset [AddMonoid M] (l : List (ι →₀ M)) :
    l.sum.support ⊆ l.foldr (FinsuppWith.support · ⊔ ·) ∅ := by
...
theorem Multiset.support_sum_subset [AddCommMonoid M] (s : Multiset (ι →₀ M)) :
    s.sum.support ⊆ (s.map FinsuppWith.support).sup := by
...
theorem Multiset.mem_sup_map_support_iff [Zero M] {s : Multiset (ι →₀ M)} {x : ι} :
    x ∈ (s.map FinsuppWith.support).sup ↔ ∃ f ∈ s, x ∈ f.support :=
...
theorem Finset.mem_sup_support_iff [Zero M] {s : Finset (ι →₀ M)} {x : ι} :
    x ∈ s.sup FinsuppWith.support ↔ ∃ f ∈ s, x ∈ f.support :=
  Multiset.mem_sup_map_support_iff

theorem List.support_sum_eq [AddMonoid M] (l : List (ι →₀ M))
    (hl : l.Pairwise (_root_.Disjoint on FinsuppWith.support)) :
    l.sum.support = l.foldr (FinsuppWith.support · ⊔ ·) ∅ := by
  induction' l with hd tl IH
  · simp
  · simp only [List.pairwise_cons] at hl
    ...
    suffices _root_.Disjoint hd.support (tl.foldr (fun x y ↦ (FinsuppWith.support x ⊔ y)) ∅) by
...
theorem Multiset.support_sum_eq [AddCommMonoid M] (s : Multiset (ι →₀ M))
    (hs : s.Pairwise (_root_.Disjoint on FinsuppWith.support)) :
    s.sum.support = (s.map FinsuppWith.support).sup := by
  induction' s using Quot.inductionOn with a
  obtain ⟨l, hl, hd⟩ := hs
  suffices a.Pairwise (_root_.Disjoint on FinsuppWith.support) by
...
theorem Finset.support_sum_eq [AddCommMonoid M] (s : Finset (ι →₀ M))
    (hs : (s : Set (ι →₀ M)).PairwiseDisjoint FinsuppWith.support) :
    (s.sum id).support = Finset.sup s FinsuppWith.support := by
  classical
  suffices s.1.Pairwise (_root_.Disjoint on FinsuppWith.support) by
