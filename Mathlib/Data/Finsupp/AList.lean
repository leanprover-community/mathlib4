...
@[simp]
lemma toAList_keys_toFinset [DecidableEq α] (f : α →₀ M) :
...
@[simp]
lemma mem_toAlist {f : α →₀ M} {x : α} : x ∈ f.toAList ↔ f x ≠ 0 := by
...
@[simp]
lemma lookupFinsupp_apply [DecidableEq α] (l : AList fun _x : α => M) (a : α) :
    l.lookupFinsupp a = (l.lookup a).getD 0 := by
    convert rfl; congr
#align alist.lookup_finsupp_apply AList.lookupFinsupp_apply

@[simp]
lemma lookupFinsupp_support [DecidableEq α] [DecidableEq M] (l : AList fun _x : α => M) :
...
lemma lookupFinsupp_eq_iff_of_ne_zero [DecidableEq α] {l : AList fun _x : α => M} {a : α} {x : M}
...
lemma lookupFinsupp_eq_zero_iff [DecidableEq α] {l : AList fun _x : α => M} {a : α} :
...
@[simp]
lemma empty_lookupFinsupp : lookupFinsupp (∅ : AList fun _x : α => M) = 0 := by
  classical
    ext
    simp
...
@[simp]
lemma insert_lookupFinsupp [DecidableEq α] (l : AList fun _x : α => M) (a : α) (m : M) :
...
@[simp]
lemma singleton_lookupFinsupp (a : α) (m : M) :
...
@[simp]
lemma _root_.Finsupp.toAList_lookupFinsupp (f : α →₀ M) : f.toAList.lookupFinsupp = f := by
  ext a
  classical
    by_cases h : f a = 0
...
lemma lookupFinsupp_surjective : Function.Surjective (@lookupFinsupp α M _) := fun f =>
