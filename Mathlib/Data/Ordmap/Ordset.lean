...
instance (priority := 10000) BalancedSz.dec : DecidableRel BalancedSz := fun _ _ => Or.decidable
...
instance (priority := 10000) Balanced.dec : DecidablePred (@Balanced α)
  | nil => by
    unfold Balanced
    infer_instance
  | node _ l _ r => by
...
instance (priority := 10000) instEmptyCollection : EmptyCollection (Ordset α) :=
  ⟨nil⟩
#align ordset.has_emptyc Ordset.instEmptyCollection

instance (priority := 10000) instInhabited : Inhabited (Ordset α) :=
  ⟨nil⟩
#align ordset.inhabited Ordset.instInhabited

instance (priority := 10000) instSingleton : Singleton α (Ordset α) :=
  ⟨Ordset.singleton⟩
...
instance (priority := 10000) Empty.instDecidablePred : DecidablePred (@Empty α _) :=
...
instance (priority := 10000) instInsert [IsTotal α (· ≤ ·)] [@DecidableRel α (· ≤ ·)] : Insert α (Ordset α) :=
  ⟨Ordset.insert⟩
...
instance (priority := 10000) instMembership : Membership α (Ordset α) :=
  ⟨fun x s => mem x s⟩
#align ordset.has_mem Ordset.instMembership

instance (priority := 10000) mem.decidable (x : α) (s : Ordset α) : Decidable (x ∈ s) :=
