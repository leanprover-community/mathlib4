...
theorem insertWith.valid_aux [IsTotalLE α] [DecidableLE α] (f : α → α) (x : α)
...
theorem insertWith.valid [IsTotalLE α] [DecidableLE α] (f : α → α) (x : α)
...
theorem insert.valid [IsTotalLE α] [DecidableLE α] (x : α) {t} (h : Valid t) :
...
theorem insert'.valid [IsTotalLE α] [DecidableLE α]
...
protected def insert [IsTotalLE α] [DecidableLE α] (x : α) (s : Ordset α) :
    Ordset α :=
  ⟨Ordnode.insert x s.1, insert.valid _ s.2⟩

instance instInsert [IsTotalLE α] [DecidableLE α] : Insert α (Ordset α) :=
  ⟨Ordset.insert⟩

/-- O(log n). Insert an element into the set, preserving balance and the BST property.
  If an equivalent element is already in the set, the set is returned as is. -/
nonrec def insert' [IsTotalLE α] [DecidableLE α] (x : α) (s : Ordset α) :
    Ordset α :=
