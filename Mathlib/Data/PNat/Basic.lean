...
instance (priority := 10000) : WellFoundedLT ℕ+ := WellFoundedRelation.isWellFounded
...
instance (priority := 10000) covariantClass_add_le : CovariantClass ℕ+ ℕ+ (· + ·) (· ≤ ·) :=
  Positive.covariantClass_add_le

instance (priority := 10000) covariantClass_add_lt : CovariantClass ℕ+ ℕ+ (· + ·) (· < ·) :=
  Positive.covariantClass_add_lt

instance (priority := 10000) contravariantClass_add_le : ContravariantClass ℕ+ ℕ+ (· + ·) (· ≤ ·) :=
  Positive.contravariantClass_add_le

instance (priority := 10000) contravariantClass_add_lt : ContravariantClass ℕ+ ℕ+ (· + ·) (· < ·) :=
...
instance (priority := 10000) : OrderBot ℕ+ where
  bot := 1
  bot_le a := a.property
