...
instance : SMul ℕ (BitVec w) := fast_instance% ⟨fun x y => ofFin <| x • y.toFin⟩
instance : SMul ℤ (BitVec w) := fast_instance% ⟨fun x y => ofFin <| x • y.toFin⟩
instance : Pow (BitVec w) ℕ  := fast_instance% ⟨fun x n => ofFin <| x.toFin ^ n⟩
