...
@[simp, norm_cast]
protected lemma coe_natCast (n : ℕ) : (↑(↑n : ℝ≥0) : ℝ) = n :=
...
@[simp, norm_cast]
protected lemma coe_ofScientific (m : ℕ) (s : Bool) (e : ℕ) :
...
@[simp]
lemma mk_natCast (n : ℕ) : @Eq ℝ≥0 (⟨(n : ℝ), n.cast_nonneg⟩ : ℝ≥0) n :=
...
lemma lt_of_toNNReal_lt {r p : ℝ} (h : r.toNNReal < p.toNNReal) : r < p :=
