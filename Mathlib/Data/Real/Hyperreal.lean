...
theorem isSt_sSup {x : ℝ*} (hni : ¬Infinite x) : IsSt x (sSup { y : ℝ | (y : ℝ*) < x }) :=
  ...
  ⟨gt_of_not_le fun c =>
      ...
      not_gt_of_le (csSup_le HR₁ hc) <| sub_lt_self R hδ,
    gt_of_not_le fun c =>
      have hc : ↑(R + δ / 2) < x :=
        lt_of_lt_of_le (add_lt_add_left (coe_lt_coe.2 (half_lt_self hδ)) R) c
      not_gt_of_le (le_csSup HR₂ hc) <| (lt_add_iff_pos_right _).mpr <| half_pos hδ⟩
...
theorem st_eq_sSup {x : ℝ*} : st x = sSup { y : ℝ | (y : ℝ*) < x } := by
  ...
  · rw [hx.st_eq]
    ...
    | inr hx =>
      ...
      exact Set.eq_empty_of_forall_not_mem fun y hy ↦ hy.out.not_gt (hx _)
