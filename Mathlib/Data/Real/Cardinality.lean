...
lemma increasing_cantorFunction (h1 : 0 < c) (h2 : c < 1 / 2) {n : ℕ} {f g : ℕ → Bool}
...
lemma cantorFunction_injective (h1 : 0 < c) (h2 : c < 1 / 2) :
...
lemma mk_Ioi_real (a : ℝ) : #(Ioi a) = 𝔠 := by
...
/-- The cardinality of the interval (-∞, a). -/
lemma mk_Iio_real (a : ℝ) : #(Iio a) = 𝔠 := by
  refine' le_antisymm (mk_real ▸ mk_set_le _) _
