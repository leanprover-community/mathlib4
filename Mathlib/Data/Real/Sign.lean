...
lemma sign_apply_eq (r : ℝ) : sign r = -1 ∨ sign r = 0 ∨ sign r = 1 := by
...
lemma sign_int_cast (z : ℤ) : sign (z : ℝ) = ↑(Int.sign z) := by
...
lemma sign_mul_nonneg (r : ℝ) : 0 ≤ sign r * r := by
  obtain hn | rfl | hp := lt_trichotomy r (0 : ℝ)
  · rw [sign_of_neg hn]
...
lemma sign_mul_pos_of_ne_zero (r : ℝ) (hr : r ≠ 0) : 0 < sign r * r := by
...
@[simp]
lemma inv_sign (r : ℝ) : (sign r)⁻¹ = sign r := by
...
@[simp]
lemma sign_inv (r : ℝ) : sign r⁻¹ = sign r := by
  obtain hn | rfl | hp := lt_trichotomy r (0 : ℝ)
