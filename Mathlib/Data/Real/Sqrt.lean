...
/-- Although the instance `ROrCLike.toStarOrderedRing` exists, it is locked behind the
...
hierarchy; otherwise in order to access it we would need to import `Data.ROrCLike.Basic` -/
