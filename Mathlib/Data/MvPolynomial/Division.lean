...
@[simp]
lemma coeff_divMonomial (s : σ →₀ ℕ) (x : MvPolynomial σ R) (s' : σ →₀ ℕ) :
    coeff s' (x /ᵐᵒⁿᵒᵐⁱᵃˡ s) = coeff (s + s') x :=
  rfl
#align mv_polynomial.coeff_div_monomial MvPolynomial.coeff_divMonomial

@[simp]
lemma support_divMonomial (s : σ →₀ ℕ) (x : MvPolynomial σ R) :
...
lemma modMonomial_add_divMonomial (x : MvPolynomial σ R) (s : σ →₀ ℕ) :
...
@[simp]
lemma X_mul_divMonomial (i : σ) (x : MvPolynomial σ R) :
...
@[simp]
lemma X_divMonomial (i : σ) : (X i : MvPolynomial σ R) /ᵐᵒⁿᵒᵐⁱᵃˡ Finsupp.single i 1 = 1 :=
  divMonomial_monomial (Finsupp.single i 1)
set_option linter.uppercaseLean3 false in
#align mv_polynomial.X_div_monomial MvPolynomial.X_divMonomial

@[simp]
lemma mul_X_divMonomial (x : MvPolynomial σ R) (i : σ) :
...
@[simp]
lemma X_mul_modMonomial (i : σ) (x : MvPolynomial σ R) :
...
@[simp]
lemma mul_X_modMonomial (x : MvPolynomial σ R) (i : σ) :
...
@[simp]
lemma modMonomial_X (i : σ) : (X i : MvPolynomial σ R) %ᵐᵒⁿᵒᵐⁱᵃˡ Finsupp.single i 1 = 0 :=
  monomial_modMonomial _
set_option linter.uppercaseLean3 false in
#align mv_polynomial.mod_monomial_X MvPolynomial.modMonomial_X

lemma divMonomial_add_modMonomial_single (x : MvPolynomial σ R) (i : σ) :
...
lemma modMonomial_add_divMonomial_single (x : MvPolynomial σ R) (i : σ) :
