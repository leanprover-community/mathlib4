...
lemma expand_C (p : ℕ) (r : R) : expand p (C r : MvPolynomial σ R) = C r :=
  eval₂Hom_C _ _ _
set_option linter.uppercaseLean3 false in
#align mv_polynomial.expand_C MvPolynomial.expand_C

@[simp]
lemma expand_X (p : ℕ) (i : σ) : expand p (X i : MvPolynomial σ R) = X i ^ p :=
  eval₂Hom_X' _ _ _
set_option linter.uppercaseLean3 false in
#align mv_polynomial.expand_X MvPolynomial.expand_X

@[simp]
lemma expand_monomial (p : ℕ) (d : σ →₀ ℕ) (r : R) :
...
lemma expand_one_apply (f : MvPolynomial σ R) : expand 1 f = f := by
...
lemma expand_comp_bind₁ (p : ℕ) (f : σ → MvPolynomial τ R) :
...
lemma expand_bind₁ (p : ℕ) (f : σ → MvPolynomial τ R) (φ : MvPolynomial σ R) :
...
@[simp]
lemma rename_comp_expand (f : σ → τ) (p : ℕ) :
