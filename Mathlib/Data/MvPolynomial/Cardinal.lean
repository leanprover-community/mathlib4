...
@[simp]
lemma cardinal_mk_eq_max_lift [Nonempty σ] [Nontrivial R] :
...
@[simp]
lemma cardinal_mk_eq_lift [IsEmpty σ] : #(MvPolynomial σ R) = Cardinal.lift.{u} #R :=
...
lemma cardinal_lift_mk_le_max {σ : Type u} {R : Type v} [CommSemiring R] : #(MvPolynomial σ R) ≤
...
of `#R`, `#σ` and `ℵ₀` -/
lemma cardinal_mk_le_max : #(MvPolynomial σ R) ≤ max (max #R #σ) ℵ₀ :=
