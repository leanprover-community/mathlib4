...
lemma powersetAux_eq_map_coe {l : List α} : powersetAux l = (sublists l).map (↑) :=
  rfl
#align multiset.powerset_aux_eq_map_coe Multiset.powersetAux_eq_map_coe

@[simp]
lemma mem_powersetAux {l : List α} {s} : s ∈ powersetAux l ↔ s ≤ ↑l :=
...
lemma powersetAux_perm_powersetAux' {l : List α} : powersetAux l ~ powersetAux' l := by
...
@[simp]
lemma powersetAux'_nil : powersetAux' (@nil α) = [0] :=
  rfl
#align multiset.powerset_aux'_nil Multiset.powersetAux'_nil

@[simp]
lemma powersetAux'_cons (a : α) (l : List α) :
...
lemma powerset_aux'_perm {l₁ l₂ : List α} (p : l₁ ~ l₂) : powersetAux' l₁ ~ powersetAux' l₂ := by
...
lemma powersetAux_perm {l₁ l₂ : List α} (p : l₁ ~ l₂) : powersetAux l₁ ~ powersetAux l₂ :=
...
lemma powerset_coe (l : List α) : @powerset α l = ((sublists l).map (↑) : List (Multiset α)) :=
...
@[simp]
lemma powerset_zero : @powerset α 0 = {0} :=
  rfl
#align multiset.powerset_zero Multiset.powerset_zero

@[simp]
lemma powerset_cons (a : α) (s) : powerset (a ::ₘ s) = powerset s + map (cons a) (powerset s) :=
...
@[simp]
lemma mem_powerset {s t : Multiset α} : s ∈ powerset t ↔ s ≤ t :=
  Quotient.inductionOn₂ s t <| by simp [Subperm, and_comm]
#align multiset.mem_powerset Multiset.mem_powerset

lemma map_single_le_powerset (s : Multiset α) : s.map singleton ≤ powerset s :=
  Quotient.inductionOn s fun l => by
...
@[simp]
lemma card_powerset (s : Multiset α) : card (powerset s) = 2 ^ card s :=
  Quotient.inductionOn s <| by simp
#align multiset.card_powerset Multiset.card_powerset

lemma revzip_powersetAux {l : List α} ⦃x⦄ (h : x ∈ revzip (powersetAux l)) : x.1 + x.2 = ↑l := by
...
lemma revzip_powersetAux' {l : List α} ⦃x⦄ (h : x ∈ revzip (powersetAux' l)) :
    x.1 + x.2 = ↑l := by
...
lemma revzip_powersetAux_lemma {α : Type*} [DecidableEq α] (l : List α) {l' : List (Multiset α)}
...
lemma revzip_powersetAux_perm_aux' {l : List α} :
...
lemma revzip_powersetAux_perm {l₁ l₂ : List α} (p : l₁ ~ l₂) :
    revzip (powersetAux l₁) ~ revzip (powersetAux l₂) := by
  haveI := Classical.decEq α
...
lemma powersetCardAux_eq_map_coe {n} {l : List α} :
...
@[simp]
lemma mem_powersetCardAux {n} {l : List α} {s} : s ∈ powersetCardAux n l ↔ s ≤ ↑l ∧ card s = n :=
...
lemma powersetCardAux_perm {n} {l₁ l₂ : List α} (p : l₁ ~ l₂) :
...
lemma powersetCard_coe' (n) (l : List α) : @powersetCard α n l = powersetCardAux n l :=
  rfl
#align multiset.powerset_len_coe' Multiset.powersetCard_coe'

lemma powersetCard_coe (n) (l : List α) :
...
@[simp]
lemma powersetCard_zero_left (s : Multiset α) : powersetCard 0 s = {0} :=
...
lemma powersetCard_zero_right (n : ℕ) : @powersetCard α (n + 1) 0 = 0 :=
  rfl
#align multiset.powerset_len_zero_right Multiset.powersetCard_zero_right

@[simp]
lemma powersetCard_cons (n : ℕ) (a : α) (s) :
...
lemma powersetCard_one (s : Multiset α) : powersetCard 1 s = s.map singleton :=
  Quotient.inductionOn s fun l ↦ by
    simp [powersetCard_coe, sublistsLen_one, map_reverse, Function.comp]

@[simp]
lemma mem_powersetCard {n : ℕ} {s t : Multiset α} : s ∈ powersetCard n t ↔ s ≤ t ∧ card s = n :=
  Quotient.inductionOn t fun l => by simp [powersetCard_coe']
#align multiset.mem_powerset_len Multiset.mem_powersetCard

@[simp]
lemma card_powersetCard (n : ℕ) (s : Multiset α) :
...
lemma powersetCard_le_powerset (n : ℕ) (s : Multiset α) : powersetCard n s ≤ powerset s :=
  Quotient.inductionOn s fun l => by
...
lemma powersetCard_mono (n : ℕ) {s t : Multiset α} (h : s ≤ t) :
...
@[simp]
lemma powersetCard_eq_empty {α : Type*} (n : ℕ) {s : Multiset α} (h : card s < n) :
    powersetCard n s = 0 :=
...
@[simp]
lemma powersetCard_card_add (s : Multiset α) {i : ℕ} (hi : 0 < i) :
...
lemma powersetCard_map {β : Type*} (f : α → β) (n : ℕ) (s : Multiset α) :
...
lemma pairwise_disjoint_powersetCard (s : Multiset α) :
...
@[simp]
lemma nodup_powerset {s : Multiset α} : Nodup (powerset s) ↔ Nodup s :=
...
protected lemma Nodup.powersetCard {n : ℕ} {s : Multiset α} (h : Nodup s) :
