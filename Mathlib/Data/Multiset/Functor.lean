...
def traverse : Multiset α' → F (Multiset β') := by
  refine QuotLike.lift (Functor.map ofList ∘ Traversable.traverse f) ?_
...
theorem comp_traverse {G H : Type _ → Type _} [Applicative G] [Applicative H] [CommApplicative G]
  ...
  refine QuotLike.inductionOn x ?_
  intro
  simp only [traverse, mkQ_to_coe, lift_coe, Function.comp_apply, Functor.map_map, functor_norm]
