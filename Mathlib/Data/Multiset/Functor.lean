...
def traverse : Multiset α' → F (Multiset β') := by
  refine QuotLike.lift (Functor.map Coe.coe ∘ Traversable.traverse f) ?_
...
theorem id_traverse {α : Type*} (x : Multiset α) : traverse (pure : α → Id α) x = x := by
  refine QuotLike.inductionOn x ?_
  intro
...
theorem comp_traverse {G H : Type _ → Type _} [Applicative G] [Applicative H] [CommApplicative G]
  ...
  refine QuotLike.inductionOn x ?_
  intro
  simp only [traverse, Coe.coe, traverse_comp, mkQ_to_coe, lift_coe, Function.comp_apply,
    Comp.map_mk, Functor.map_map]
...
theorem map_traverse {G : Type* → Type _} [Applicative G] [CommApplicative G] {α β γ : Type _}
  ...
  refine QuotLike.inductionOn x ?_
  intro
  simp only [traverse, mkQ_to_coe, lift_coe, Function.comp_apply, Functor.map_map, map_comp_coe]
...
theorem traverse_map {G : Type* → Type _} [Applicative G] [CommApplicative G] {α β γ : Type _}
  ...
  refine QuotLike.inductionOn x ?_
  intro
  simp only [traverse, mkQ_to_coe, map_coe, lift_coe, Function.comp_apply]
...
theorem naturality {G H : Type _ → Type _} [Applicative G] [Applicative H] [CommApplicative G]
  ...
  refine QuotLike.inductionOn x ?_
  intro
  simp only [traverse, mkQ_to_coe, lift_coe, Function.comp_apply,
