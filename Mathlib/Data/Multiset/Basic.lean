...
lemma card_le_card {s t : Multiset α} (h : s ≤ t) : card s ≤ card t :=
  leInductionOn h Sublist.length_le
#align multiset.card_le_of_le Multiset.card_le_card

@[mono]
lemma card_mono : Monotone (@card α) := fun _a _b => card_le_card
...
lemma card_lt_card {s t : Multiset α} (h : s < t) : card s < card t :=
...
lemma erase_cons_tail_of_mem (h : a ∈ s) :
...
lemma map_hcongr {β' : Type v} {m : Multiset α} {f : α → β} {f' : α → β'} (h : β = β')
...
lemma map_erase_of_mem [DecidableEq α] [DecidableEq β] (f : α → β)
