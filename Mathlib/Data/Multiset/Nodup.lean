...
protected lemma Nodup.product {t : Multiset β} : Nodup s → Nodup t → Nodup (s ×ˢ t) :=
  Quotient.inductionOn₂ s t fun l₁ l₂ d₁ d₂ => by simp [List.Nodup.product d₁ d₂]
#align multiset.nodup.product Multiset.Nodup.product

protected lemma Nodup.sigma {σ : α → Type*} {t : ∀ a, Multiset (σ a)} :
...
@[simp]
lemma nodup_bind {s : Multiset α} {t : α → Multiset β} :
