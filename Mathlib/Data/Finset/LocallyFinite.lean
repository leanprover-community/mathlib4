...
lemma _root_.BddBelow.finite_of_bddAbove {s : Set α} (h₀ : BddBelow s) (h₁ : BddAbove s) :
    s.Finite := by
  let ⟨a, ha⟩ := h₀
  let ⟨b, hb⟩ := h₁
...
lemma Icc_eq_cons_Ico (h : a ≤ b) : Icc a b = (Ico a b).cons b right_not_mem_Ico := by
...
lemma Icc_eq_cons_Ioc (h : a ≤ b) : Icc a b = (Ioc a b).cons a left_not_mem_Ioc := by
...
lemma Ioc_eq_cons_Ioo (h : a < b) : Ioc a b = (Ioo a b).cons b right_not_mem_Ioo := by
...
lemma Ico_eq_cons_Ioo (h : a < b) : Ico a b = (Ioo a b).cons a left_not_mem_Ioo := by
...
lemma Ici_eq_cons_Ioi (a : α) : Ici a = (Ioi a).cons a not_mem_Ioi_self := by
...
lemma Iic_eq_cons_Iio (b : α) : Iic b = (Iio b).cons b not_mem_Iio_self := by
...
@[simp]
lemma map_add_left_Icc (a b c : α) :
...
@[simp]
lemma map_add_right_Icc (a b c : α) :
...
@[simp]
lemma map_add_left_Ico (a b c : α) :
...
@[simp]
lemma map_add_right_Ico (a b c : α) :
...
@[simp]
lemma map_add_left_Ioc (a b c : α) :
...
@[simp]
lemma map_add_right_Ioc (a b c : α) :
...
@[simp]
lemma map_add_left_Ioo (a b c : α) :
...
@[simp]
lemma map_add_right_Ioo (a b c : α) :
...
@[simp]
lemma image_add_left_Icc (a b c : α) : (Icc a b).image ((· + ·) c) = Icc (c + a) (c + b) := by
...
@[simp]
lemma image_add_left_Ico (a b c : α) : (Ico a b).image ((· + ·) c) = Ico (c + a) (c + b) := by
...
@[simp]
lemma image_add_left_Ioc (a b c : α) : (Ioc a b).image ((· + ·) c) = Ioc (c + a) (c + b) := by
...
@[simp]
lemma image_add_left_Ioo (a b c : α) : (Ioo a b).image ((· + ·) c) = Ioo (c + a) (c + b) := by
...
@[simp]
lemma image_add_right_Icc (a b c : α) : (Icc a b).image (· + c) = Icc (a + c) (b + c) := by
...
lemma image_add_right_Ico (a b c : α) : (Ico a b).image (· + c) = Ico (a + c) (b + c) := by
...
lemma image_add_right_Ioc (a b c : α) : (Ioc a b).image (· + c) = Ioc (a + c) (b + c) := by
...
lemma image_add_right_Ioo (a b c : α) : (Ioo a b).image (· + c) = Ioo (a + c) (b + c) := by
...
@[to_additive]
lemma prod_prod_Ioi_mul_eq_prod_prod_off_diag [Fintype ι] [LinearOrder ι]
