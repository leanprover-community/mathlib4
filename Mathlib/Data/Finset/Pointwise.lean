...
@[to_additive (attr := simp, aesop safe apply (rule_sets := [finsetNonempty]))]
lemma inv_nonempty_iff : s⁻¹.Nonempty ↔ s.Nonempty := image_nonempty
...
@[to_additive (attr := simp)]
lemma inv_eq_empty : s⁻¹ = ∅ ↔ s = ∅ := image_eq_empty
...
@[to_additive (attr := simp)]
lemma card_inv (s : Finset α) : s⁻¹.card = s.card := card_image_of_injective _ inv_injective
#align finset.card_inv Finset.card_inv
#align finset.card_neg Finset.card_neg

@[to_additive (attr := simp)]
lemma preimage_inv (s : Finset α) : s.preimage (·⁻¹) (inv_injective.injOn _) = s⁻¹ :=
...
@[to_additive]
lemma mem_mul {x : α} : x ∈ s * t ↔ ∃ y ∈ s, ∃ z ∈ t, y * z = x := mem_image₂
...
@[to_additive]
lemma mem_smul {x : β} : x ∈ s • t ↔ ∃ y ∈ s, ∃ z ∈ t, y • z = x :=
  mem_image₂
...
lemma mem_vsub : a ∈ s -ᵥ t ↔ ∃ b ∈ s, ∃ c ∈ t, b -ᵥ c = a :=
  mem_image₂
...
lemma singleton_vsub (a : β) : ({a} : Finset β) -ᵥ t = t.image (a -ᵥ ·) :=
  image₂_singleton_left
...
@[to_additive]
lemma image_smul_distrib [DecidableEq α] [DecidableEq β] [Monoid α] [Monoid β] [FunLike F α β]
