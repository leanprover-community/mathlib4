...
lemma Nonempty.exists_mem {s : Finset α} (h : s.Nonempty) : ∃ x : α, x ∈ s :=
  h
...
lemma mem_cons_of_mem {a b : α} {s : Finset α} {hb : b ∉ s} (ha : a ∈ s) : a ∈ cons b s hb :=
...
@[simp, norm_cast]
lemma coe_disjUnion {s t : Finset α} (h : Disjoint s t) :
...
@[simp] lemma subset_insert (a : α) (s : Finset α) : s ⊆ insert a s := fun _b => mem_insert_of_mem
...
lemma ssubset_iff : s ⊂ t ↔ ∃ a ∉ s, insert a s ⊆ t := mod_cast @Set.ssubset_iff_insert α s t
...
lemma union_comm (s₁ s₂ : Finset α) : s₁ ∪ s₂ = s₂ ∪ s₁ := sup_comm _ _
...
@[simp]
lemma union_assoc (s₁ s₂ s₃ : Finset α) : s₁ ∪ s₂ ∪ s₃ = s₁ ∪ (s₂ ∪ s₃) := sup_assoc _ _ _
...
@[simp]
lemma union_idempotent (s : Finset α) : s ∪ s = s := sup_idem _
...
@[simp]
lemma union_left_idem (s t : Finset α) : s ∪ (s ∪ t) = s ∪ t := sup_left_idem _ _
#align finset.union_left_idem Finset.union_left_idem

-- Porting note (#10618): @[simp] can prove this
lemma union_right_idem (s t : Finset α) : s ∪ t ∪ t = s ∪ t := sup_right_idem _ _
#align finset.union_right_idem Finset.union_right_idem

@[simp]
lemma inter_left_idem (s t : Finset α) : s ∩ (s ∩ t) = s ∩ t := inf_left_idem _ _
#align finset.inter_left_idem Finset.inter_left_idem

-- Porting note (#10618): @[simp] can prove this
lemma inter_right_idem (s t : Finset α) : s ∩ t ∩ t = s ∩ t := inf_right_idem _ _
#align finset.inter_right_idem Finset.inter_right_idem

lemma inter_union_distrib_left (s t u : Finset α) : s ∩ (t ∪ u) = s ∩ t ∪ s ∩ u :=
  inf_sup_left _ _ _
#align finset.inter_distrib_left Finset.inter_union_distrib_left

lemma union_inter_distrib_right (s t u : Finset α) : (s ∪ t) ∩ u = s ∩ u ∪ t ∩ u :=
  inf_sup_right _ _ _
#align finset.inter_distrib_right Finset.union_inter_distrib_right

lemma union_inter_distrib_left (s t u : Finset α) : s ∪ t ∩ u = (s ∪ t) ∩ (s ∪ u) :=
  sup_inf_left _ _ _
#align finset.union_distrib_left Finset.union_inter_distrib_left

lemma inter_union_distrib_right (s t u : Finset α) : s ∩ t ∪ u = (s ∪ u) ∩ (t ∪ u) :=
  sup_inf_right _ _ _
...
@[simp] lemma insert_erase (h : a ∈ s) : insert a (erase s a) = s :=
  ext fun x => by
...
lemma inter_sdiff (s t u : Finset α) : s ∩ (t \ u) = (s ∩ t) \ u := (inter_sdiff_assoc _ _ _).symm
...
lemma Nontrivial.sdiff_singleton_nonempty {c : α} {s : Finset α} (hS : s.Nontrivial) :
    (s \ {c}).Nonempty := by
...
lemma filter_singleton (a : α) : filter p {a} = if p a then {a} else ∅ := by
  classical
    ext x
...
@[simp]
lemma toFinset_nsmul (s : Multiset α) : ∀ n ≠ 0, (n • s).toFinset = s.toFinset
...
@[simp]
lemma toFinset_filter (s : Multiset α) (p : α → Prop) [DecidablePred p] :
...
@[simp]
lemma toFinset_filter (s : List α) (p : α → Bool) :
...
@[simp]
lemma Finset.union_symm_inl (h : Disjoint s t) (x : s) :
    Equiv.Finset.union s t h (Sum.inl x) = ⟨x, Finset.mem_union.mpr <| Or.inl x.2⟩ :=
  rfl

@[simp]
lemma Finset.union_symm_inr (h : Disjoint s t) (y : t) :
