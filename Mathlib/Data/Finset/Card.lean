...
@[gcongr]
lemma card_le_card : s ⊆ t → s.card ≤ t.card :=
  Multiset.card_le_card ∘ val_le_iff.mpr
#align finset.card_le_of_subset Finset.card_le_card

@[mono]
lemma card_mono : Monotone (@card α) := by apply card_le_card
...
lemma card_le_two : card {a, b} ≤ 2 := card_insert_le _ _

lemma card_le_three : card {a, b, c} ≤ 3 :=
  (card_insert_le _ _).trans (Nat.succ_le_succ card_le_two)

lemma card_le_four : card {a, b, c, d} ≤ 4 :=
  (card_insert_le _ _).trans (Nat.succ_le_succ card_le_three)

lemma card_le_five : card {a, b, c, d, e} ≤ 5 :=
  (card_insert_le _ _).trans (Nat.succ_le_succ card_le_four)

lemma card_le_six : card {a, b, c, d, e, f} ≤ 6 :=
...
@[simp]
lemma card_pair (h : a ≠ b) : ({a, b} : Finset α).card = 2 := by
...
lemma Multiset.dedup_card_eq_card_iff_nodup {m : Multiset α} :
    card m.dedup = card m ↔ m.Nodup :=
  .trans ⟨fun h ↦ eq_of_le_of_card_le (dedup_le m) h.ge, congr_arg _⟩ dedup_eq_self

lemma Multiset.toFinset_card_eq_card_iff_nodup {m : Multiset α} :
...
lemma le_card_iff_exists_subset_card : n ≤ s.card ↔ ∃ t ⊆ s, t.card = n := by
...
lemma _root_.Multiset.toFinset_card_eq_one_iff [DecidableEq α] (s : Multiset α) :
...
lemma one_lt_card_iff_nontrivial : 1 < s.card ↔ s.Nontrivial := by
...
lemma card_eq_succ : s.card = n + 1 ↔ ∃ a t, a ∉ t ∧ insert a t = s ∧ t.card = n :=
  ⟨fun h =>
...
lemma card_eq_two : s.card = 2 ↔ ∃ x y, x ≠ y ∧ s = {x, y} := by
  constructor
  · rw [card_eq_succ]
    simp_rw [card_eq_one]
...
lemma card_eq_three : s.card = 3 ↔ ∃ x y z, x ≠ y ∧ x ≠ z ∧ y ≠ z ∧ s = {x, y, z} := by
  constructor
  · rw [card_eq_succ]
    simp_rw [card_eq_two]
...
lemma strongInduction_eq {p : Finset α → Sort*} (H : ∀ s, (∀ t ⊂ s, p t) → p s)
