...
theorem noncommFold_eq_fold (s : Multiset α) [Commutative op] (a : α) :
    noncommFold op s (fun x _ y _ _ => Commutative.comm x y) a = fold op a s := by
  induction s using Quotient.inductionOn
  simp
