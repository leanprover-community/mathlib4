...
theorem fold_ite' {g : α → β} (hb : op b b = b) (p : α → Prop) [DecidablePred p] :
  ...
  classical
    induction s using Finset.induction_on with | empty => simp [hb] | @insert x s hx IH =>
...
theorem fold_op_rel_iff_and {r : β → β → Prop} (hr : ∀ {x y z}, r x (op y z) ↔ r x y ∧ r x z)
  ...
  classical
    induction s using Finset.induction_on with | empty => simp | @insert a s ha IH =>
...
theorem fold_union_empty_singleton [DecidableEq α] (s : Finset α) :
  ...
  | insert has ih => rw [fold_insert has, ih, insert_eq]
