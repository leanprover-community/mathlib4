...
theorem fold_hom {op' : γ → γ → γ} [Commutative op'] [Associative op'] {m : β → γ}
...
@[simp]
theorem fold_insert_idem [DecidableEq α] [hi : Idempotent op] :
    (insert a s).fold op b f = f a * s.fold op b f := by
  by_cases h : a ∈ s
  · rw [← insert_erase h]
...
theorem fold_image_idem [DecidableEq α] {g : γ → α} {s : Finset γ} [hi : Idempotent op] :
...
theorem fold_ite [Idempotent op] {g : α → β} (p : α → Prop) [DecidablePred p] :
    Finset.fold op b (fun i => ite (p i) (f i) (g i)) s =
      op (Finset.fold op b f (s.filter p)) (Finset.fold op b g (s.filter fun i => ¬p i)) :=
  fold_ite' (Idempotent.idempotent _) _
#align finset.fold_ite Finset.fold_ite
