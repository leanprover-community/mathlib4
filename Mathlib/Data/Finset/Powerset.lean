...
@[simp]
lemma powersetCard_mono {n} {s t : Finset α} (h : s ⊆ t) : powersetCard n s ⊆ powersetCard n t :=
...
@[simp]
lemma powersetCard_zero (s : Finset α) : s.powersetCard 0 = {∅} := by
...
@[simp]
lemma map_val_val_powersetCard (s : Finset α) (i : ℕ) :
...
lemma powersetCard_one (s : Finset α) :
...
lemma powersetCard_eq_filter {n} {s : Finset α} :
...
lemma powersetCard_succ_insert [DecidableEq α] {x : α} {s : Finset α} (h : x ∉ s) (n : ℕ) :
...
@[simp]
lemma powersetCard_self (s : Finset α) : powersetCard s.card s = {s} := by
  ext
...
lemma pairwise_disjoint_powersetCard (s : Finset α) :
...
lemma powersetCard_sup [DecidableEq α] (u : Finset α) (n : ℕ) (hn : n < u.card) :
...
lemma powersetCard_map {β : Type*} (f : α ↪ β) (n : ℕ) (s : Finset α) :
