...
theorem range_orderEmbOfFin (s : Finset α) {k : ℕ} (h : s.card = k) :
  ...
  simp only [orderEmbOfFin, RelEmbedding.coe_trans, OrderEmbedding.subtype_apply,
    OrderIso.coe_toOrderEmbedding, EquivLike.range_comp, Subtype.range_coe_subtype, setOf_mem]
