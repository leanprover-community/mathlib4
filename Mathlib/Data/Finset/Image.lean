...
@[simp]
theorem attach_image_val [DecidableEq α] {s : Finset α} : s.attach.image Subtype.val = s := by
  apply eq_of_veq
  conv_rhs => rw [← dedup_eq_self]
  simp_rw [← Multiset.attach_map_val s.val, image_val, attach_val, mem_val]
