...
@[simp]
lemma map_val (f : α ↪ β) (s : Finset α) : (map f s).1 = s.1.map f :=
  rfl
#align finset.map_val Finset.map_val

@[simp]
lemma map_empty (f : α ↪ β) : (∅ : Finset α).map f = ∅ :=
  rfl
...
@[simp]
lemma mem_map {b : β} : b ∈ s.map f ↔ ∃ a ∈ s, f a = b :=
...
@[simp 1100]
lemma mem_map_equiv {f : α ≃ β} {b : β} : b ∈ s.map f.toEmbedding ↔ f.symm b ∈ s := by
  rw [mem_map]
  exact
    ⟨by
      rintro ⟨a, H, rfl⟩
...
lemma mem_map' (f : α ↪ β) {a} {s : Finset α} : f a ∈ s.map f ↔ a ∈ s :=
  mem_map_of_injective f.2
#align finset.mem_map' Finset.mem_map'

lemma mem_map_of_mem (f : α ↪ β) {a} {s : Finset α} : a ∈ s → f a ∈ s.map f :=
  (mem_map' _).2
#align finset.mem_map_of_mem Finset.mem_map_of_mem

lemma forall_mem_map {f : α ↪ β} {s : Finset α} {p : ∀ a, a ∈ s.map f → Prop} :
...
@[simp, norm_cast]
lemma coe_map (f : α ↪ β) (s : Finset α) : (s.map f : Set β) = f '' s :=
  Set.ext (by simp only [mem_coe, mem_map, Set.mem_image, implies_true])
#align finset.coe_map Finset.coe_map

lemma coe_map_subset_range (f : α ↪ β) (s : Finset α) : (s.map f : Set β) ⊆ Set.range f :=
  calc
...
lemma map_toFinset [DecidableEq α] [DecidableEq β] {s : Multiset α} :
...
@[simp]
lemma map_subset_map {s₁ s₂ : Finset α} : s₁.map f ⊆ s₂.map f ↔ s₁ ⊆ s₂ :=
...
@[simp]
lemma disjoint_map {s t : Finset α} (f : α ↪ β) :
...
lemma map_disjUnion {f : α ↪ β} (s₁ s₂ : Finset α) (h) (h' := (disjoint_map _).mpr h) :
...
lemma map_disjUnion' {f : α ↪ β} (s₁ s₂ : Finset α) (h') (h := (disjoint_map _).mp h') :
    (s₁.disjUnion s₂ h).map f = (s₁.map f).disjUnion (s₂.map f) h' :=
  map_disjUnion _ _ _
#align finset.map_disj_union' Finset.map_disjUnion'

lemma map_union [DecidableEq α] [DecidableEq β] {f : α ↪ β} (s₁ s₂ : Finset α) :
...
lemma map_inter [DecidableEq α] [DecidableEq β] {f : α ↪ β} (s₁ s₂ : Finset α) :
    (s₁ ∩ s₂).map f = s₁.map f ∩ s₂.map f :=
...
@[simp]
lemma map_insert [DecidableEq α] [DecidableEq β] (f : α ↪ β) (a : α) (s : Finset α) :
...
@[simp]
lemma map_cons (f : α ↪ β) (a : α) (s : Finset α) (ha : a ∉ s) :
...
@[simp]
lemma map_nonempty : (s.map f).Nonempty ↔ s.Nonempty := by
...
lemma map_disjiUnion {f : α ↪ β} {s : Finset α} {t : β → Finset γ} {h} :
...
lemma disjiUnion_map {s : Finset α} {t : α → Finset β} {f : β ↪ γ} {h} :
...
@[simp]
lemma image_val (f : α → β) (s : Finset α) : (image f s).1 = (s.1.map f).dedup :=
  rfl
#align finset.image_val Finset.image_val

@[simp]
lemma image_empty (f : α → β) : (∅ : Finset α).image f = ∅ :=
  rfl
...
lemma image_congr (h : (s : Set α).EqOn f g) : Finset.image f s = Finset.image g s := by
  ext
  simp_rw [mem_image, ← bex_def]
  exact bex_congr fun x hx => by rw [h hx]
#align finset.image_congr Finset.image_congr

lemma _root_.Function.Injective.mem_finset_image (hf : Injective f) :
...
@[simp, norm_cast]
lemma coe_image {f : α → β} : ↑(s.image f) = f '' ↑s :=
...
lemma image_toFinset [DecidableEq α] {s : Multiset α} :
...
lemma image_val_of_injOn (H : Set.InjOn f s) : (image f s).1 = s.1.map f :=
  (s.2.map_on H).dedup
#align finset.image_val_of_inj_on Finset.image_val_of_injOn

@[simp]
lemma image_id [DecidableEq α] : s.image id = s :=
  ext fun _ => by simp only [mem_image, exists_prop, id, exists_eq_right]
#align finset.image_id Finset.image_id

@[simp]
lemma image_id' [DecidableEq α] : (s.image fun x => x) = s :=
  image_id
#align finset.image_id' Finset.image_id'

lemma image_image [DecidableEq γ] {g : β → γ} : (s.image f).image g = s.image (g ∘ f) :=
  eq_of_veq <| by simp only [image_val, dedup_map_dedup_eq, Multiset.map_map]
#align finset.image_image Finset.image_image

lemma image_comm {β'} [DecidableEq β'] [DecidableEq γ] {f : β → γ} {g : α → β} {f' : α → β'}
...
lemma _root_.Function.Semiconj.finset_image [DecidableEq α] {f : α → β} {ga : α → α} {gb : β → β}
    (h : Function.Semiconj f ga gb) : Function.Semiconj (image f) (image ga) (image gb) := fun _ =>
  image_comm h
#align function.semiconj.finset_image Function.Semiconj.finset_image

lemma _root_.Function.Commute.finset_image [DecidableEq α] {f g : α → α}
...
lemma image_subset_image {s₁ s₂ : Finset α} (h : s₁ ⊆ s₂) : s₁.image f ⊆ s₂.image f := by
...
lemma image_mono (f : α → β) : Monotone (Finset.image f) := fun _ _ => image_subset_image
#align finset.image_mono Finset.image_mono

lemma image_subset_image_iff {t : Finset α} (hf : Injective f) :
...
lemma coe_image_subset_range : ↑(s.image f) ⊆ Set.range f :=
  calc
    ↑(s.image f) = f '' ↑s := coe_image
    _ ⊆ Set.range f := Set.image_subset_range f ↑s
#align finset.coe_image_subset_range Finset.coe_image_subset_range

lemma image_filter {p : β → Prop} [DecidablePred p] :
...
lemma image_union [DecidableEq α] {f : α → β} (s₁ s₂ : Finset α) :
...
lemma image_inter_subset [DecidableEq α] (f : α → β) (s t : Finset α) :
...
lemma image_inter_of_injOn [DecidableEq α] {f : α → β} (s t : Finset α)
...
lemma image_inter [DecidableEq α] (s₁ s₂ : Finset α) (hf : Injective f) :
    (s₁ ∩ s₂).image f = s₁.image f ∩ s₂.image f :=
  image_inter_of_injOn _ _ <| hf.injOn _
#align finset.image_inter Finset.image_inter

@[simp]
lemma image_singleton (f : α → β) (a : α) : image f {a} = {f a} :=
...
@[simp]
lemma image_insert [DecidableEq α] (f : α → β) (a : α) (s : Finset α) :
...
lemma erase_image_subset_image_erase [DecidableEq α] (f : α → β) (s : Finset α) (a : α) :
...
@[simp]
lemma image_erase [DecidableEq α] {f : α → β} (hf : Injective f) (s : Finset α) (a : α) :
...
@[simp]
lemma image_eq_empty : s.image f = ∅ ↔ s = ∅ :=
...
lemma image_sdiff [DecidableEq α] {f : α → β} (s t : Finset α) (hf : Injective f) :
    (s \ t).image f = s.image f \ t.image f :=
  coe_injective <| by
    push_cast
...
lemma image_symmDiff [DecidableEq α] {f : α → β} (s t : Finset α) (hf : Injective f) :
    (s ∆ t).image f = s.image f ∆ t.image f :=
  coe_injective <| by
    push_cast
...
@[simp]
lemma _root_.Disjoint.of_image_finset {s t : Finset α} {f : α → β}
...
@[simp]
lemma attach_image_val [DecidableEq α] {s : Finset α} : s.attach.image Subtype.val = s :=
...
@[simp]
lemma attach_insert [DecidableEq α] {a : α} {s : Finset α} :
    attach (insert a s) =
...
lemma map_eq_image (f : α ↪ β) (s : Finset α) : s.map f = s.image f :=
  eq_of_veq (s.map f).2.dedup.symm
#align finset.map_eq_image Finset.map_eq_image

@[simp]
lemma disjoint_image {s t : Finset α} {f : α → β} (hf : Injective f) :
...
lemma image_const {s : Finset α} (h : s.Nonempty) (b : β) : (s.image fun _ => b) = singleton b :=
  ext fun b' => by
...
@[simp]
lemma map_erase [DecidableEq α] (f : α ↪ β) (s : Finset α) (a : α) :
    (s.erase a).map f = (s.map f).erase (f a) := by
  simp_rw [map_eq_image]
  exact s.image_erase f.2 a
#align finset.map_erase Finset.map_erase

lemma image_biUnion [DecidableEq γ] {f : α → β} {s : Finset α} {t : β → Finset γ} :
...
lemma biUnion_image [DecidableEq γ] {s : Finset α} {t : α → Finset β} {f : β → γ} :
...
lemma image_biUnion_filter_eq [DecidableEq α] (s : Finset β) (g : β → α) :
...
lemma biUnion_singleton {f : α → β} : (s.biUnion fun a => {f a}) = s.image f :=
...
`Embedding.subtype`. -/
@[simp]
lemma subtype_map (p : α → Prop) [DecidablePred p] {s : Finset α} :
...
lemma subtype_map_of_mem {p : α → Prop} [DecidablePred p] {s : Finset α} (h : ∀ x ∈ s, p x) :
...
lemma not_mem_map_subtype_of_not_property {p : α → Prop} (s : Finset { x // p x }) {a : α}
...
subtype. -/
lemma map_subtype_subset {t : Set α} (s : Finset t) : ↑(s.map (Embedding.subtype _)) ⊆ t := by
  intro a ha
  rw [mem_coe] at ha
...
@[simp]
lemma mem_fin {n} {s : Finset ℕ} : ∀ a : Fin n, a ∈ s.fin n ↔ (a : ℕ) ∈ s
  | ⟨a, ha⟩ => by simp [Finset.fin, ha, and_comm]
#align finset.mem_fin Finset.mem_fin

@[mono]
lemma fin_mono {n} : Monotone (Finset.fin n) := fun s t h x => by simpa using @h x
#align finset.fin_mono Finset.fin_mono

@[simp]
lemma fin_map {n} {s : Finset ℕ} : (s.fin n).map Fin.valEmbedding = s.filter (· < n) := by
  simp [Finset.fin, Finset.map_map]
#align finset.fin_map Finset.fin_map

lemma subset_image_iff [DecidableEq β] {s : Set α} {t : Finset β} {f : α → β} :
...
lemma range_sdiff_zero {n : ℕ} : range (n + 1) \ {0} = (range n).image Nat.succ := by
...
lemma Multiset.toFinset_map [DecidableEq α] [DecidableEq β] (f : α → β) (m : Multiset α) :
...
@[simp]
lemma finsetCongr_apply (e : α ≃ β) (s : Finset α) : e.finsetCongr s = s.map e.toEmbedding :=
  rfl
#align equiv.finset_congr_apply Equiv.finsetCongr_apply

@[simp]
lemma finsetCongr_refl : (Equiv.refl α).finsetCongr = Equiv.refl _ := by
  ext
  simp
#align equiv.finset_congr_refl Equiv.finsetCongr_refl

@[simp]
lemma finsetCongr_symm (e : α ≃ β) : e.finsetCongr.symm = e.symm.finsetCongr :=
  rfl
...
lemma finsetCongr_toEmbedding (e : α ≃ β) :
