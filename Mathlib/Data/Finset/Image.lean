...
lemma filter_image {p : β → Prop} [DecidablePred p] :
...
@[simp]
lemma filterMap_val : (filterMap f s' f_inj).1 = s'.1.filterMap f := rfl

@[simp]
lemma filterMap_empty : (∅ : Finset α).filterMap f f_inj = ∅ := rfl

@[simp]
lemma mem_filterMap {b : β} : b ∈ s.filterMap f f_inj ↔ ∃ a ∈ s, f a = some b :=
  s.val.mem_filterMap f

@[simp, norm_cast]
lemma coe_filterMap : (s.filterMap f f_inj : Set β) = {b | ∃ a ∈ s, f a = some b} :=
  Set.ext (by simp only [mem_coe, mem_filterMap, Option.mem_def, Set.mem_setOf_eq, implies_true])

@[simp]
lemma filterMap_some : s.filterMap some (by simp) = s :=
  ext fun _ => by simp only [mem_filterMap, Option.some.injEq, exists_eq_right]

lemma filterMap_mono (h : s ⊆ t) :
