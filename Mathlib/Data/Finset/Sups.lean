...
@[simp, norm_cast]
lemma coe_sups : (↑(s ⊻ t) : Set α) = ↑s ⊻ ↑t :=
  coe_image₂ _ _ _
#align finset.coe_sups Finset.coe_sups

lemma card_sups_le : (s ⊻ t).card ≤ s.card * t.card :=
...
lemma image_subset_sups_left : b ∈ t → (s.image fun a => a ⊔ b) ⊆ s ⊻ t :=
  image_subset_image₂_left
#align finset.image_subset_sups_left Finset.image_subset_sups_left

lemma image_subset_sups_right : a ∈ s → t.image (a ⊔ ·) ⊆ s ⊻ t :=
...
lemma singleton_sups_singleton : ({a} ⊻ {b} : Finset α) = {a ⊔ b} :=
  image₂_singleton
...
lemma biUnion_image_sup_left : (s.biUnion fun a => t.image <| (· ⊔ ·) a) = s ⊻ t :=
  biUnion_image_left
#align finset.bUnion_image_sup_left Finset.biUnion_image_sup_left

lemma biUnion_image_sup_right : (t.biUnion fun b => s.image fun a => a ⊔ b) = s ⊻ t :=
  biUnion_image_right
...
@[simp, norm_cast]
lemma coe_infs : (↑(s ⊼ t) : Set α) = ↑s ⊼ ↑t :=
  coe_image₂ _ _ _
#align finset.coe_infs Finset.coe_infs

lemma card_infs_le : (s ⊼ t).card ≤ s.card * t.card :=
  card_image₂_le _ _ _
...
lemma image_subset_infs_left : b ∈ t → (s.image fun a => a ⊓ b) ⊆ s ⊼ t :=
  image_subset_image₂_left
#align finset.image_subset_infs_left Finset.image_subset_infs_left

lemma image_subset_infs_right : a ∈ s → t.image (a ⊓ ·) ⊆ s ⊼ t :=
  image_subset_image₂_right (f := (· ⊓ ·))
...
lemma singleton_infs_singleton : ({a} ⊼ {b} : Finset α) = {a ⊓ b} :=
  image₂_singleton
...
lemma biUnion_image_inf_left : (s.biUnion fun a => t.image <| (· ⊓ ·) a) = s ⊼ t :=
  biUnion_image_left
#align finset.bUnion_image_inf_left Finset.biUnion_image_inf_left

lemma biUnion_image_inf_right : (t.biUnion fun b => s.image fun a => a ⊓ b) = s ⊼ t :=
  biUnion_image_right
...
@[simp]
lemma mem_disjSups : c ∈ s ○ t ↔ ∃ a ∈ s, ∃ b ∈ t, Disjoint a b ∧ a ⊔ b = c := by
  simp [disjSups, and_assoc]
#align finset.mem_disj_sups Finset.mem_disjSups

lemma disjSups_subset_sups : s ○ t ⊆ s ⊻ t := by
...
lemma card_disjSups_le : (s ○ t).card ≤ s.card * t.card :=
...
lemma forall_disjSups_iff {p : α → Prop} :
...
@[simp]
lemma disjSups_subset_iff : s ○ t ⊆ u ↔ ∀ a ∈ s, ∀ b ∈ t, Disjoint a b → a ⊔ b ∈ u :=
  forall_disjSups_iff
#align finset.disj_sups_subset_iff Finset.disjSups_subset_iff

lemma Nonempty.of_disjSups_left : (s ○ t).Nonempty → s.Nonempty := by
...
lemma disjSups_inter_subset_right : s ○ (t₁ ∩ t₂) ⊆ s ○ t₁ ∩ s ○ t₂ := by
...
lemma disjSups_comm : s ○ t = t ○ s := by
  ext
...
lemma disjSups_assoc : ∀ s t u : Finset α, s ○ t ○ u = s ○ (t ○ u) := by
...
lemma disjSups_left_comm : s ○ (t ○ u) = t ○ (s ○ u) := by
  simp_rw [← disjSups_assoc, disjSups_comm s]
#align finset.disj_sups_left_comm Finset.disjSups_left_comm

lemma disjSups_right_comm : s ○ t ○ u = s ○ u ○ t := by simp_rw [disjSups_assoc, disjSups_comm]
#align finset.disj_sups_right_comm Finset.disjSups_right_comm

lemma disjSups_disjSups_disjSups_comm : s ○ t ○ (u ○ v) = s ○ u ○ (t ○ v) := by
