...
@[simp]
lemma _root_.map_finset_sup [SemilatticeSup β] [OrderBot β]
...
lemma sup_mem (s : Set α) (w₁ : ⊥ ∈ s) (w₂ : ∀ᵉ (x ∈ s) (y ∈ s), x ⊔ y ∈ s)
...
@[simp]
lemma _root_.map_finset_inf [SemilatticeInf β] [OrderTop β]
...
lemma inf_mem (s : Set α) (w₁ : ⊤ ∈ s) (w₂ : ∀ᵉ (x ∈ s) (y ∈ s), x ⊓ y ∈ s)
...
@[simp]
lemma sup'_insert [DecidableEq β] {b : β} :
...
@[simp]
lemma sup'_singleton {b : β} : ({b} : Finset β).sup' (singleton_nonempty _) f = f b :=
  rfl
#align finset.sup'_singleton Finset.sup'_singleton

@[simp]
lemma sup'_le_iff {a : α} : s.sup' H f ≤ a ↔ ∀ b ∈ s, f b ≤ a := by
...
lemma sup'_product_left {t : Finset γ} (h : (s ×ˢ t).Nonempty) (f : β × γ → α) :
    (s ×ˢ t).sup' h f = s.sup' h.fst fun i => t.sup' h.snd fun i' => f ⟨i, i'⟩ :=
  eq_of_forall_ge_iff fun a => by simp [@forall_swap _ γ]
#align finset.sup'_product_left Finset.sup'_product_left

lemma sup'_product_right {t : Finset γ} (h : (s ×ˢ t).Nonempty) (f : β × γ → α) :
...
lemma sup'_mem (s : Set α) (w : ∀ᵉ (x ∈ s) (y ∈ s), x ⊔ y ∈ s) {ι : Type*}
...
@[simp]
lemma _root_.map_finset_sup' [SemilatticeSup β] [FunLike F α β] [SupHomClass F α β]
...
@[simp]
lemma inf'_insert [DecidableEq β] {b : β} :
...
@[simp]
lemma inf'_singleton {b : β} : ({b} : Finset β).inf' (singleton_nonempty _) f = f b :=
  rfl
...
lemma inf'_le_of_le {a : α} {b : β} (hb : b ∈ s) (h : f b ≤ a) :
...
lemma inf'_product_left {t : Finset γ} (h : (s ×ˢ t).Nonempty) (f : β × γ → α) :
...
lemma inf'_product_right {t : Finset γ} (h : (s ×ˢ t).Nonempty) (f : β × γ → α) :
...
lemma inf'_mem (s : Set α) (w : ∀ᵉ (x ∈ s) (y ∈ s), x ⊓ y ∈ s) {ι : Type*}
...
@[simp]
lemma _root_.map_finset_inf' [SemilatticeInf β] [FunLike F α β] [InfHomClass F α β]
    (f : F) {s : Finset ι} (hs) (g : ι → α) :
...
lemma sup'_inf_distrib_left (f : ι → α) (a : α) :
