...
lemma iUnion_sigma {γ : α → Type*} (s : Sigma γ → Set β) : ⋃ ia, s ia = ⋃ i, ⋃ a, s ⟨i, a⟩ :=
  iSup_sigma

lemma iUnion_sigma' {γ : α → Type*} (s : ∀ i, γ i → Set β) :
    ⋃ i, ⋃ a, s i a = ⋃ ia : Sigma γ, s ia.1 ia.2 :=
  iSup_sigma' _

lemma iInter_sigma {γ : α → Type*} (s : Sigma γ → Set β) : ⋂ ia, s ia = ⋂ i, ⋂ a, s ⟨i, a⟩ :=
  iInf_sigma

lemma iInter_sigma' {γ : α → Type*} (s : ∀ i, γ i → Set β) :
    ⋂ i, ⋂ a, s i a = ⋂ ia : Sigma γ, s ia.1 ia.2 :=
  iInf_sigma' _
...
@[simp]
lemma mapsTo_sUnion {S : Set (Set α)} {t : Set β} {f : α → β} :
    MapsTo f (⋃₀ S) t ↔ ∀ s ∈ S, MapsTo f s t :=
  sUnion_subset_iff
#align set.maps_to_sUnion Set.mapsTo_sUnion

@[simp]
lemma mapsTo_iUnion {s : ι → Set α} {t : Set β} {f : α → β} :
...
@[simp]
lemma mapsTo_sInter {s : Set α} {T : Set (Set β)} {f : α → β} :
    MapsTo f s (⋂₀ T) ↔ ∀ t ∈ T, MapsTo f s t :=
  forall₂_swap
#align set.maps_to_sInter Set.mapsTo_sInter

@[simp]
lemma mapsTo_iInter {s : Set α} {t : ι → Set β} {f : α → β} :
...
/- ./././Mathport/Syntax/Translate/Expr.lean:107:6: warning: expanding binder group (i hi) -/
/- ./././Mathport/Syntax/Translate/Expr.lean:107:6: warning: expanding binder group (i hi) -/
lemma InjOn.image_biInter_eq {p : ι → Prop} {s : ∀ i, p i → Set α} (hp : ∃ i, p i)
...
lemma image_sUnion {f : α → β} {s : Set (Set α)} : (f '' ⋃₀ s) = ⋃₀ (image f '' s) := by
  ext b
...
lemma iUnion_image_right : ⋃ b ∈ t, (f · b) '' s = image2 f s t := by
...
lemma seq_def {s : Set (α → β)} {t : Set α} : seq s t = ⋃ f ∈ s, f '' t := by
...
lemma iUnion_univ_pi {ι : α → Type*} (t : (a : α) → ι a → Set (π a)) :
...
lemma directedOn_iUnion {r} {f : ι → Set α} (hd : Directed (· ⊆ ·) f)
...
lemma directedOn_sUnion {r} {S : Set (Set α)} (hd : DirectedOn (· ⊆ ·) S)
...
lemma sigmaToiUnion_injective (h : Pairwise fun i j => Disjoint (t i) (t j)) :
...
lemma sigmaToiUnion_bijective (h : Pairwise fun i j => Disjoint (t i) (t j)) :
...
lemma sSup_iUnion (t : ι → Set β) : sSup (⋃ i, t i) = ⨆ i, sSup (t i) := by
