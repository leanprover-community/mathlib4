...
lemma finite_coe_iff {s : Set α} : Finite s ↔ s.Finite := .rfl
...
protected lemma toFinset_empty (h : (∅ : Set α).Finite) : h.toFinset = ∅ := by
  simp
...
lemma finite_univ_iff : (@univ α).Finite ↔ Finite α := (Equiv.Set.univ α).finite_iff
...
lemma Finite.sep {s : Set α} (hs : s.Finite) (p : α → Prop) : { a ∈ s | p a }.Finite :=
  hs.subset <| sep_subset _ _
#align set.finite.sep Set.Finite.sep

lemma Finite.inter_of_left {s : Set α} (hs : s.Finite) (t : Set α) : (s ∩ t).Finite :=
  hs.subset <| inter_subset_left _ _
#align set.finite.inter_of_left Set.Finite.inter_of_left

lemma Finite.inter_of_right {s : Set α} (hs : s.Finite) (t : Set α) : (t ∩ s).Finite :=
...
lemma finite_iUnion [Finite ι] {f : ι → Set α} (H : ∀ i, (f i).Finite) : (⋃ i, f i).Finite :=
...
@[simp]
protected lemma Finite.insert (a : α) {s : Set α} (hs : s.Finite) : (insert a s).Finite :=
...
lemma Finite.surjOn_iff_bijOn_of_mapsTo : SurjOn f s s ↔ BijOn f s s := by
...
lemma Finite.injOn_iff_bijOn_of_mapsTo : InjOn f s ↔ BijOn f s s := by
...
protected lemma Finite.offDiag {s : Set α} (hs : s.Finite) : s.offDiag.Finite :=
...
lemma empty_card' {h : Fintype.{u} (∅ : Set α)} : @Fintype.card (∅ : Set α) h = 0 := by
  simp
#align set.empty_card' Set.empty_card'
...
lemma infinite_of_not_bddBelow : ¬BddBelow s → s.Infinite := mt Finite.bddBelow
...
lemma DirectedOn.exists_mem_subset_of_finset_subset_biUnion {α ι : Type*} {f : ι → Set α}
