...
@[simp]
lemma mem_sigma_iff : x ∈ s.Sigma t ↔ x.1 ∈ s ∧ x.2 ∈ t x.1 :=
  Iff.rfl
#align set.mem_sigma_iff Set.mem_sigma_iff

lemma mk_sigma_iff : (⟨i, a⟩ : Σ i, α i) ∈ s.Sigma t ↔ i ∈ s ∧ a ∈ t i :=
  Iff.rfl
#align set.mk_sigma_iff Set.mk_sigma_iff

lemma mk_mem_sigma (hi : i ∈ s) (ha : a ∈ t i) : (⟨i, a⟩ : Σi, α i) ∈ s.Sigma t :=
  ⟨hi, ha⟩
...
@[simp]
lemma sigma_empty : (s.Sigma fun i ↦ (∅ : Set (α i))) = ∅ :=
  ext fun _ ↦ and_false_iff _
#align set.sigma_empty Set.sigma_empty

@[simp]
lemma empty_sigma : (∅ : Set ι).Sigma t = ∅ :=
  ext fun _ ↦ false_and_iff _
#align set.empty_sigma Set.empty_sigma

lemma univ_sigma_univ : ((@univ ι).Sigma fun _ ↦ @univ (α i)) = univ :=
...
@[simp]
lemma union_sigma : (s₁ ∪ s₂).Sigma t = s₁.Sigma t ∪ s₂.Sigma t :=
...
lemma Nonempty.sigma_fst : (s.Sigma t : Set _).Nonempty → s.Nonempty := fun ⟨x, hx⟩ ↦ ⟨x.1, hx.1⟩
