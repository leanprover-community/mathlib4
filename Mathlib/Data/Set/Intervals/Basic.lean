...
theorem Ioo_inter_Iio (a b c : α) : Ioo a b ∩ Iio c = Ioo a (min b c) := by
  ext; simp_rw [mem_inter_iff, mem_Ioo, mem_Iio, lt_min_iff, and_assoc]

theorem Iio_inter_Ioo (a b c : α) : Iio a ∩ Ioo b c = Ioo b (min a c) := by
  rw [Set.inter_comm, Set.Ioo_inter_Iio, min_comm]

theorem Ioo_inter_Ioi (a b c : α) : Ioo a b ∩ Ioi c = Ioo (max a c) b := by
  ext; simp_rw [mem_inter_iff, mem_Ioo, mem_Ioi, max_lt_iff, and_assoc, and_comm]

theorem Ioi_inter_Ioo (a b c : α) : Set.Ioi a ∩ Set.Ioo b c = Set.Ioo (max a b) c := by
