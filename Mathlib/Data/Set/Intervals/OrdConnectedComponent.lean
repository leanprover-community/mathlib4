...
lemma mem_ordConnectedComponent : y ∈ ordConnectedComponent s x ↔ [[x, y]] ⊆ s :=
  Iff.rfl
#align set.mem_ord_connected_component Set.mem_ordConnectedComponent

lemma dual_ordConnectedComponent :
...
lemma ordConnectedComponent_subset : ordConnectedComponent s x ⊆ s := fun _ hy =>
  hy right_mem_uIcc
#align set.ord_connected_component_subset Set.ordConnectedComponent_subset

lemma subset_ordConnectedComponent {t} [h : OrdConnected s] (hs : x ∈ s) (ht : s ⊆ t) :
...
@[simp]
lemma self_mem_ordConnectedComponent : x ∈ ordConnectedComponent s x ↔ x ∈ s := by
...
@[simp]
lemma nonempty_ordConnectedComponent : (ordConnectedComponent s x).Nonempty ↔ x ∈ s :=
...
@[simp]
lemma ordConnectedComponent_eq_empty : ordConnectedComponent s x = ∅ ↔ x ∉ s := by
...
@[simp]
lemma ordConnectedComponent_empty : ordConnectedComponent ∅ x = ∅ :=
  ordConnectedComponent_eq_empty.2 (not_mem_empty x)
#align set.ord_connected_component_empty Set.ordConnectedComponent_empty

@[simp]
lemma ordConnectedComponent_univ : ordConnectedComponent univ x = univ := by
...
lemma mem_ordConnectedComponent_comm :
...
@[simp]
lemma ordConnectedProj_eq {x y : s} :
...
lemma ordConnectedSection_subset : ordConnectedSection s ⊆ s :=
...
lemma disjoint_left_ordSeparatingSet : Disjoint s (ordSeparatingSet s t) :=
...
lemma disjoint_right_ordSeparatingSet : Disjoint t (ordSeparatingSet s t) :=
...
lemma disjoint_ordT5Nhd : Disjoint (ordT5Nhd s t) (ordT5Nhd t s) := by
