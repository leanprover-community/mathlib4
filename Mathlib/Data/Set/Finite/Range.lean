...
/-- If two injective functions from a finite domain have different ranges, then the range of the
first contains an element not in the range of the second. -/
lemma _root_.Function.Injective.exists_not_mem_range_of_range_ne {α γ : Type*} [Finite α]
    {f g : α → γ} (hf : Function.Injective f) (hg : Function.Injective g)
    (h : range f ≠ range g) : ∃ i, f i ∉ range g := by
  classical
  have := Fintype.ofFinite α
  by_contra! h_sub
  have : range f ⊆ range g := range_subset_iff.mpr h_sub
  grind [eq_of_subset_of_card_le, card_range_of_injective]
