...
@[simp] lemma union_univ (s : Set α) : s ∪ univ = univ := sup_top_eq _
#align set.union_univ Set.union_univ

@[simp] lemma univ_union (s : Set α) : univ ∪ s = univ := top_sup_eq _
...
@[simp, mfld_simps] lemma inter_univ (a : Set α) : a ∩ univ = a := inf_top_eq _
#align set.inter_univ Set.inter_univ

@[simp, mfld_simps] lemma univ_inter (a : Set α) : univ ∩ a = a := top_inf_eq _
...
lemma inter_distrib_left (s t u : Set α) : s ∩ (t ∪ u) = s ∩ t ∪ s ∩ u := inf_sup_left _ _ _
#align set.inter_distrib_left Set.inter_distrib_left

lemma inter_union_distrib_left {s t u : Set α} : s ∩ (t ∪ u) = s ∩ t ∪ s ∩ u := inf_sup_left _ _ _
#align set.inter_union_distrib_left Set.inter_union_distrib_left

lemma inter_distrib_right (s t u : Set α) : (s ∪ t) ∩ u = s ∩ u ∪ t ∩ u := inf_sup_right _ _ _
#align set.inter_distrib_right Set.inter_distrib_right

lemma union_inter_distrib_right {s t u : Set α} : (s ∪ t) ∩ u = s ∩ u ∪ t ∩ u := inf_sup_right _ _ _
#align set.union_inter_distrib_right Set.union_inter_distrib_right

lemma union_distrib_left (s t u : Set α) : s ∪ t ∩ u = (s ∪ t) ∩ (s ∪ u) := sup_inf_left _ _ _
#align set.union_distrib_left Set.union_distrib_left

lemma union_inter_distrib_left {s t u : Set α} : s ∪ t ∩ u = (s ∪ t) ∩ (s ∪ u) := sup_inf_left _ _ _
#align set.union_inter_distrib_left Set.union_inter_distrib_left

lemma union_distrib_right (s t u : Set α) : s ∩ t ∪ u = (s ∪ u) ∩ (t ∪ u) := sup_inf_right _ _ _
