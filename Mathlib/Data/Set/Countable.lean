...
on `s`. -/
lemma countable_iff_exists_injOn {s : Set α} : s.Countable ↔ ∃ f : α → ℕ, InjOn f s :=
...
lemma countable_setOf_finite_subset {s : Set α} (hs : s.Countable) :
...
them are nonempty. -/
lemma countable_setOf_nonempty_of_disjoint {f : β → Set α}
