...
theorem inj_on_iUnion_of_predirected {s : ι → Set α} (hs : Predirected (· ⊆ ·) s) {f : α → β}
...
theorem bijOn_iUnion_of_predirected {s : ι → Set α} (hs : Predirected (· ⊆ ·) s) {t : ι → Set β}
    {f : α → β} (H : ∀ i, BijOn f (s i) (t i)) : BijOn f (⋃ i, s i) (⋃ i, t i) :=
  bijOn_iUnion H <| inj_on_iUnion_of_predirected hs fun i => (H i).injOn

theorem bijOn_iInter_of_predirected [Nonempty ι] {s : ι → Set α} (hs : Predirected (· ⊆ ·) s)
    {t : ι → Set β} {f : α → β} (H : ∀ i, BijOn f (s i) (t i)) : BijOn f (⋂ i, s i) (⋂ i, t i) :=
  bijOn_iInter H <| inj_on_iUnion_of_predirected hs fun i => (H i).injOn
