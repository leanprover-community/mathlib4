...
lemma forall_bool' {p : Bool → Prop} (b : Bool) : (∀ x, p x) ↔ p b ∧ p !b :=
...
lemma exists_bool' {p : Bool → Prop} (b : Bool) : (∃ x, p x) ↔ p b ∨ p !b :=
...
lemma not_ne_id : not ≠ id := fun h ↦ false_ne_true <| congrFun h true
...
lemma bne_eq_xor : bne = xor := by funext a b; revert a b; decide
