...
lemma any_iff_exists {p : α → Bool} : any l p ↔ ∃ a ∈ l, p a := by
  induction' l with a l ih
...
lemma any_iff_exists_prop : (any l fun a => p a) ↔ ∃ a ∈ l, p a := by simp [any_iff_exists]
#align list.any_iff_exists_prop List.any_iff_exists_prop

lemma any_of_mem {p : α → Bool} (h₁ : a ∈ l) (h₂ : p a) : any l p :=
