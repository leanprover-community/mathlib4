...
instance (priority := 10000) [Inhabited α] [SemilatticeInf α] : Inhabited (CFilter α α) :=
  ⟨{  f := id
      pt := default
      inf := (· ⊓ ·)
...
instance (priority := 10000) : CoeFun (CFilter α σ) fun _ ↦ σ → α :=
  ⟨CFilter.f⟩
...
instance (priority := 10000) (s : Set α) : Inhabited (principal s).Realizer :=
