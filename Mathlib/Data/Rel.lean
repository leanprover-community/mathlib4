...
instance : CompleteLattice (Rel α β) := fast_instance% show CompleteLattice (α → β → Prop) from inferInstance
instance : Inhabited (Rel α β) := fast_instance% show Inhabited (α → β → Prop) from inferInstance
