...
theorem extract_succ_start {l: Array α} (h: i < l.size):
  ...
  · intro _
    ...
    rw [extract_overflow_eq_extract_to_stop h2, extract_overflow_eq_extract_to_stop h2]
    refine extract_succ_start_aux h h ?_
    exact anyM.proof_1 l

theorem extract_succ_stop_aux {α: Type u_1}
    {j i: Nat} {l: Array α} {h1: j < l.size} (h2: i ≤ j) :
    extract.loop l (Nat.succ j - i) i #[] = extract.loop l (j - i) i #[] ++ #[l[j]'h1] := by
  by_cases h3: i = j
  · rw [h3]
  ...
  · unfold extract.loop
    ...
    rw [extract_succ_stop_aux, Array.append_assoc]
    exact h6
termination_by j - i

theorem extract_succ_stop {l: Array α} (h1: j < l.size) (h2: i ≤ j):
  ...
  exact extract_succ_stop_aux h2
...
theorem extract_append_left {a b: Array α} {i j : Nat} (h: j ≤ a.size):
  ...
  then by
    ...
    have hiab : i < (a++b).size := by
      rw [size_append]
      exact Nat.lt_add_right (size b) hia
...
theorem extract_extract_aux {a : Array α } : (s1 + s2 ≤ a.size) →
  ...
  · have hle1: s1 + s2 ≤ s1 + e2 := by
    ...
    have happend1 : a.extract s1 e1 = a.extract s1 (s1 + s2) ++ a.extract (s1 + s2) e1 := by
      refine extract_mid_split ?h1 ?h2 h3
      exact Nat.le_add_right s1 s2
      apply Nat.le_trans hle1 h2
    have hsize1: s2 ≥ size (extract a s1 (s1 + s2)) := by
      rw [size_extract_to_stop h1, Nat.add_comm, Nat.add_sub_cancel]
      exact Nat.le_refl s2
    rw [happend1, extract_append_right hsize1]
    simp only [size_extract, h1, Nat.min_eq_left]
    have hs: (s1 + s2 - s1) = s2 := by
      rw [Nat.add_comm, Nat.add_sub_cancel]
    rw [hs, Nat.sub_self]
    have happend2: a.extract (s1 + s2) e1 =
...
theorem extract_extract {a : Array α } (h1 : s1 + s2 ≤ a.size)
  ...
  · rw [extract_overflow_eq_extract_to_stop (Nat.le_of_not_ge h : e1 ≥ a.size),
  ...
  · have h4: s1 + (extract a s1 a.size).size = a.size := by
    ...
    rw [extract_overflow_eq_extract_to_stop (Nat.le_of_not_ge h : e1 ≥ a.size),
    extract_overflow_eq_extract_to_stop (Nat.le_of_not_ge h3 : s1 + e2 ≥ a.size),
    extract_overflow_eq_extract_to_stop h5,
