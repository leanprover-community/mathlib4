...
instance inhabited [∀ i, Inhabited (α i)] : Inhabited (DArray n α) :=
  ⟨⟨default⟩⟩
...
theorem revList_reverse_aux :
    ∀ (i) (h : i ≤ n) (t : List α),
      (a.iterateAux (fun _ ↦ (· :: ·)) i h []).reverseAux t =
        a.revIterateAux (fun _ ↦ (· :: ·)) i h t
  | 0, _, _ => rfl
  | i + 1, _, _ => revList_reverse_aux i _ _
#align array.rev_list_reverse_aux Array'.revList_reverse_aux

@[simp]
theorem revList_reverse : a.revList.reverse = a.toList :=
  revList_reverse_aux _ _ _
  ...
  | i + 1, h =>
    ...
    ⟨fun ⟨j, ji1, e⟩ ↦
      ...
      fun m ↦ by
        ...
        · let ⟨j, ji, e⟩ := IH.2 m'
          exact ⟨j, Nat.le_succ_of_le ji, e⟩⟩
#align array.mem_rev_list_aux Array'.mem_revList_aux
...
theorem revList_foldr_aux :
    ∀ {i} (h : i ≤ n),
      (DArray.iterateAux a (fun _ ↦ (· :: ·)) i h []).foldr f b =
        DArray.iterateAux a (fun _ ↦ f) i h b
  | 0, _ => rfl
  | j + 1, h => congr_arg (f (read a ⟨j, h⟩)) (revList_foldr_aux _)
#align array.rev_list_foldr_aux Array'.revList_foldr_aux
...
theorem revList_length_aux (a : Array' n α) (i h) :
    (a.iterateAux (fun _ ↦ (· :: ·)) i h []).length = i := by
  induction i <;> simp [*, DArray.iterateAux]
#align array.rev_list_length_aux Array'.revList_length_aux

@[simp]
theorem revList_length (a : Array' n α) : a.revList.length = n :=
  revList_length_aux a _ _
