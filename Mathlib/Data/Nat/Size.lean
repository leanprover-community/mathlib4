...
@[simp]
lemma size_zero : size 0 = 0 := by simp [size]
#align nat.size_zero Nat.size_zero

@[simp]
lemma size_bit {b n} (h : bit b n ≠ 0) : size (bit b n) = succ (size n) := by
  rw [size]
  conv =>
    lhs
    rw [binaryRec]
...
@[simp]
lemma size_bit0 {n} (h : n ≠ 0) : size (bit0 n) = succ (size n) :=
...
@[simp]
lemma size_shiftLeft' {b m n} (h : shiftLeft' b m n ≠ 0) :
...
@[simp, nolint simpNF]
lemma size_shiftLeft {m} (h : m ≠ 0) (n) : size (m <<< n) = size m + n :=
...
lemma size_le {m n : ℕ} : size m ≤ n ↔ m < 2 ^ n :=
