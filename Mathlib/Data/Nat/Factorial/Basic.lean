...
theorem pow_succ_le_ascFactorial (n : ℕ) : ∀ k : ℕ, (n + 1) ^ k ≤ n.ascFactorial k
  ...
  | k + 1 => by
    rw [pow_succ]
    exact Nat.mul_le_mul (Nat.add_le_add_right le_self_add _) (pow_succ_le_ascFactorial k)
  ...
  | k + 1 => by
    ...
    exact
      Nat.mul_le_mul_of_nonneg_left
        ((ascFactorial_le_pow_add k).trans (Nat.pow_le_pow_of_le_left (le_succ _) _))
...
theorem succ_descFactorial_succ (n : ℕ) :
    ∀ k : ℕ, (n + 1).descFactorial (k + 1) = (n + 1) * n.descFactorial k := by
  intro n
  induction n with
  | zero =>  rw [descFactorial_zero, descFactorial_one, mul_one]
  | succ k ih =>
    rw [descFactorial_succ, ih, descFactorial_succ, succ_sub_succ,
      mul_left_comm]
