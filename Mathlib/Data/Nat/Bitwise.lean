...
theorem exists_most_significant_bit {n : ℕ} (h : n ≠ 0) :
  ...
  | f b n hn =>
    by_cases h' : n = 0
    · subst h'
      rw [show b = true by revert h; cases b <;> simp]
...
theorem bitwise_swap {f : Bool → Bool → Bool} :
  ...
  induction m using Nat.strongRecOn generalizing n with | ind m ih =>
  rcases m with m | m <;> rcases n with n | n <;> try rw [bitwise_zero_left, bitwise_zero_right]
...
macro "bitwise_assoc_tac" : tactic => set_option hygiene false in `(tactic| (
  induction n using Nat.binaryRec generalizing m k with
  | z => simp
  | f b n hn =>
    induction m using Nat.binaryRec with
    | z => simp
    | f b' m hm =>
