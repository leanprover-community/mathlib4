...
lemma coprime_fintype_prod_left_iff [Fintype ι] {s : ι → ℕ} {x : ℕ} :
    Coprime (∏ i, s i) x ↔ ∀ i, Coprime (s i) x := by
  simp [coprime_prod_left_iff]

lemma coprime_fintype_prod_right_iff [Fintype ι] {x : ℕ} {s : ι → ℕ} :
