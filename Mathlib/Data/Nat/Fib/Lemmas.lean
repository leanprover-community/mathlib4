...
`fib (x + a) ^ 2 - fib (x + 2 * a) * fib x = fib a ^ 2 * (-1) ^ x`.
-/
...
lemma fib_matrix_eq : ∀ {n : ℕ}, n ≠ 0 →
  ...
  | 1, _ => by simp
  | n + 2, _ => by
    rw [pow_succ, ← fib_matrix_eq (by simp)]
    simp
    grind [fib_add_two]
...
lemma fib_succ_mul_fib_pred_sub_fib_sq {n : ℕ} (hn : n ≠ 0) :
  ...
  calc fib (n + 1) * fib (n - 1) - fib n ^ 2
      = ((!![fib (n + 1), fib n; fib n, fib (n - 1)] : Matrix _ _ ℤ)).det := by simp [pow_two]
    _ = ((!![fib (n + 1), fib n; fib n, fib (n - 1)].map (castRingHom ℤ))).det := rfl
    _ = (!![1, 1; 1, 0] ^ n).det := by
      rw [fib_matrix_eq hn, Matrix.map_pow]
      congr
      simp [← Matrix.ext_iff]
    _ = (-1) ^ n := by simp

/-- Cassini's identity for nonzero even `n`: `fib (n + 1) * fib (n - 1) = 1 + fib n ^ 2.` -/
lemma fib_succ_mul_fib_pred_of_ne_zero_even {n : ℕ} (hn : n ≠ 0) (hn2 : Even n) :
    fib (n + 1) * fib (n - 1) = 1 + fib n ^ 2 := by
  have := fib_succ_mul_fib_pred_sub_fib_sq hn
  obtain ⟨a, rfl⟩ := hn2
  grind [pow_mul]
...
lemma fib_succ_mul_fib_pred_of_odd {n : ℕ} (hn : Odd n) :
  ...
  have := fib_succ_mul_fib_pred_sub_fib_sq (n := n) (by grind)
  obtain ⟨a, rfl⟩ := hn
  grind [pow_mul]

/-- **Catalan's identity**: for nonzero `x` and `a`,
`fib (x + a) ^ 2 - fib (x + 2 * a) * fib x = fib a ^ 2 * (-1) ^ x`. -/
lemma fib_add_sub_fib_add_two_mul_mul_fib {x a : ℕ} (hx : x ≠ 0) (ha : a ≠ 0) :
    (fib (x + a) ^ 2 - fib (x + 2 * a) * fib x : ℤ) = fib a ^ 2 * (-1) ^ x :=
  calc ((fib (x + a)) ^ 2 - (fib (x + 2 * a)) * (fib x) : ℤ)
...
theorem fib_sq_sub_fib_sub_mul_fub_add {n r : ℕ} (hr : r ≠ 0) (hrn : r < n) :
  ...
  have := fib_add_sub_fib_add_two_mul_mul_fib (x := n - r) (a := r) (by grind) hr
  grind
