...
@[simp, norm_cast]
lemma natCast_inj {x y : ℕ} : (x : PartENat) = y ↔ x = y :=
  Part.some_inj
...
@[simp]
lemma coe_coeHom : natCast_AddMonoidHom = some :=
  rfl
...
@[simp]
lemma withTopEquiv_top : withTopEquiv ⊤ = ⊤ :=
  toWithTop_top'
...
@[simp]
lemma withTopEquiv_le {x y : PartENat} : withTopEquiv x ≤ withTopEquiv y ↔ x ≤ y :=
  toWithTop_le
#align part_enat.with_top_equiv_le PartENat.withTopEquiv_le

@[simp]
lemma withTopEquiv_lt {x y : PartENat} : withTopEquiv x < withTopEquiv y ↔ x < y :=
  toWithTop_lt
...
@[simp]
lemma withTopEquiv_symm_top : withTopEquiv.symm ⊤ = ⊤ :=
  rfl
...
@[simp]
lemma withTopEquiv_symm_zero : withTopEquiv.symm 0 = 0 :=
  rfl
