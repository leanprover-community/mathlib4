...
lemma zero_union_range_succ : {0} ∪ range succ = univ := by
  ext n
  cases n <;> simp
#align nat.zero_union_range_succ Nat.zero_union_range_succ

@[simp]
protected lemma range_succ : range succ = { i | 0 < i } := by
...
lemma range_rec {α : Type*} (x : α) (f : ℕ → α → α) :
...
lemma range_casesOn {α : Type*} (x : α) (f : ℕ → α) :
