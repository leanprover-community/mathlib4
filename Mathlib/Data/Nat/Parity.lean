...
instance (priority := 10000) : DecidablePred (Even : ℕ → Prop) := fun _ => decidable_of_iff _ even_iff.symm
...
instance (priority := 10000) : DecidablePred (Odd : ℕ → Prop) := fun _ => decidable_of_iff _ odd_iff.symm
