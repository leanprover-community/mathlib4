...
lemma factorization_choose_le_one (p_large : n < p ^ 2) : (choose n k).factorization p ≤ 1 := by
...
`centralBinom n`. -/
lemma factorization_centralBinom_of_two_mul_self_lt_three_mul (n_big : 2 < n) (p_le_n : p ≤ n)
...
lemma factorization_centralBinom_eq_zero_of_two_mul_lt (h : 2 * n < p) :
...
at most `2n`. -/
lemma prod_pow_factorization_centralBinom (n : ℕ) :
