...
-/
lemma choose_le_centralBinom (r n : ℕ) : choose (2 * n) r ≤ centralBinom n :=
  calc
...
lemma two_le_centralBinom (n : ℕ) (n_pos : 0 < n) : 2 ≤ centralBinom n :=
  calc
...
-/
lemma succ_mul_centralBinom_succ (n : ℕ) :
...
-/
lemma four_pow_lt_mul_centralBinom (n : ℕ) (n_big : 4 ≤ n) : 4 ^ n < n * centralBinom n := by
...
lemma two_dvd_centralBinom_succ (n : ℕ) : 2 ∣ centralBinom (n + 1) := by
  use (n + 1 + n).choose n
...
lemma succ_dvd_centralBinom (n : ℕ) : n + 1 ∣ n.centralBinom := by
