...
theorem sum_pow_of_commute (x : α → R) (s : Finset α)
  ...
  have : ∀ i ∈ s, Commute (x a) (x i) :=
    fun _ hb => hc (mem_insert_self a s) (mem_insert_of_mem hb) (ne_of_mem_of_not_mem hb ha).symm
  rw [sum_insert ha, (Commute.sum_right s _ _ this).add_pow, sum_range]
