...
@[simp]
lemma choose_zero_right (n : ℕ) : choose n 0 = 1 := by cases n <;> rfl
#align nat.choose_zero_right Nat.choose_zero_right

@[simp]
lemma choose_zero_succ (k : ℕ) : choose 0 (succ k) = 0 :=
  rfl
...
lemma choose_succ_succ' (n k : ℕ) : choose (n + 1) (k + 1) = choose n k + choose n (k + 1) :=
  rfl
...
lemma choose_two_right (n : ℕ) : choose n 2 = n * (n - 1) / 2 := by
...
lemma choose_eq_factorial_div_factorial {n k : ℕ} (hk : k ≤ n) :
...
lemma add_choose (i j : ℕ) : (i + j).choose j = (i + j)! / (i ! * j !) := by
...
lemma add_choose_mul_factorial_mul_factorial (i j : ℕ) :
...
lemma factorial_mul_factorial_dvd_factorial {n k : ℕ} (hk : k ≤ n) : k ! * (n - k)! ∣ n ! := by
...
lemma factorial_mul_factorial_dvd_factorial_add (i j : ℕ) : i ! * j ! ∣ (i + j)! := by
...
@[simp]
lemma choose_symm {n k : ℕ} (hk : k ≤ n) : choose n (n - k) = choose n k := by
...
lemma choose_symm_of_eq_add {n a b : ℕ} (h : n = a + b) : Nat.choose n a = Nat.choose n b := by
...
lemma choose_symm_half (m : ℕ) : choose (2 * m + 1) (m + 1) = choose (2 * m + 1) m := by
...
lemma choose_succ_right_eq (n k : ℕ) : choose n (k + 1) * (k + 1) = choose n k * (n - k) := by
...
@[simp]
lemma choose_succ_self_right : ∀ n : ℕ, (n + 1).choose n = n + 1
  | 0 => rfl
...
lemma choose_mul_succ_eq (n k : ℕ) : n.choose k * (n + 1) = (n + 1).choose k * (n + 1 - k) := by
  cases k with
  | zero => simp
  | succ k =>
...
lemma ascFactorial_eq_factorial_mul_choose (n k : ℕ) :
...
lemma factorial_dvd_ascFactorial (n k : ℕ) : k ! ∣ n.ascFactorial k :=
...
lemma descFactorial_eq_factorial_mul_choose (n k : ℕ) : n.descFactorial k = k ! * n.choose k := by
...
lemma factorial_dvd_descFactorial (n k : ℕ) : k ! ∣ n.descFactorial k :=
...
lemma choose_eq_descFactorial_div_factorial (n k : ℕ) : n.choose k = n.descFactorial k / k ! := by
  apply mul_left_cancel₀ (factorial_ne_zero k)
...
lemma choose_le_middle (r n : ℕ) : choose n r ≤ choose n (n / 2) := by
...
lemma choose_le_succ (a c : ℕ) : choose a c ≤ choose a.succ c := by
  cases c <;> simp [Nat.choose_succ_succ]
#align nat.choose_le_succ Nat.choose_le_succ

lemma choose_le_add (a b c : ℕ) : choose a c ≤ choose (a + b) c := by
...
lemma choose_mono (b : ℕ) : Monotone fun a => choose a b := fun _ _ => choose_le_choose b
...
@[simp]
lemma multichoose_zero_right (n : ℕ) : multichoose n 0 = 1 := by cases n <;> simp [multichoose]
#align nat.multichoose_zero_right Nat.multichoose_zero_right

@[simp]
lemma multichoose_zero_succ (k : ℕ) : multichoose 0 (k + 1) = 0 := by simp [multichoose]
...
@[simp]
lemma multichoose_one (k : ℕ) : multichoose 1 k = 1 := by
  induction' k with k IH; · simp
  simp [multichoose_succ_succ 0 k, IH]
#align nat.multichoose_one Nat.multichoose_one

@[simp]
lemma multichoose_two (k : ℕ) : multichoose 2 k = k + 1 := by
  induction' k with k IH; · simp
  rw [multichoose, IH]
...
@[simp]
lemma multichoose_one_right (n : ℕ) : multichoose n 1 = n := by
