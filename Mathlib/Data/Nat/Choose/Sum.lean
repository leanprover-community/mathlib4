...
theorem sub_pow [CommRing R] (x y : R) (n : ℕ) :
  ...
  have : (-1 : R) ^ (n - m) = (-1) ^ (n + m) := by
    rw [mem_range] at hm
    simp [show n + m = n - m + 2 * m by grind, pow_add]
  rw [neg_pow, this]
  ring
  ...
  mul_right_injective₀ two_ne_zero <|
    calc
      ...
      _ = (∑ i ∈ range (m + 1), (2 * m + 1).choose i) +
        ...
        rw [range_eq_Ico, sum_Ico_reflect _ _ (by grind)]
        congr
        grind
      _ = ∑ i ∈ range (2 * m + 2), (2 * m + 1).choose i := sum_range_add_sum_Ico _ (by grind)
...
theorem choose_middle_le_pow (n : ℕ) : (2 * n + 1).choose n ≤ 4 ^ n := by
  have t : (2 * n + 1).choose n ≤ ∑ i ∈ range (n + 1), (2 * n + 1).choose i :=
    single_le_sum (fun x _ ↦ by grind) (self_mem_range_succ n)
...
theorem sum_Icc_choose (n k : ℕ) : ∑ m ∈ Icc k n, m.choose k = (n + 1).choose (k + 1) := by
  ...
  · rw [choose_eq_zero_of_lt (by grind), Icc_eq_empty_of_lt h, sum_empty]
  · induction n, h using le_induction with
    | base => simp
    | succ n _ ih =>
      rw [← Ico_insert_right (by grind), sum_insert (by simp), Ico_add_one_right_eq_Icc, ih,
