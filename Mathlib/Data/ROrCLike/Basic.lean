...
/-!
# `ROrCLike`: a typeclass for ℝ or ℂ

This file defines the typeclass `ROrCLike` intended to have only two instances:
...
The coercion from reals into an `ROrCLike` field is done by registering `ROrCLike.ofReal` as
...
A few lemmas requiring heavier imports are in `Mathlib/Data/ROrCLike/Lemmas.lean`.
-/
...
-/
class ROrCLike (K : semiOutParam (Type*)) extends DenselyNormedField K, StarRing K,
...
#align is_R_or_C ROrCLike

scoped[ComplexOrder] attribute [instance 100] ROrCLike.toPartialOrder
attribute [instance 100] ROrCLike.toDecidableEq
...
/-- Coercion from `ℝ` to an `ROrCLike` field. -/
...
noncomputable instance (priority := 900) algebraMapCoe : CoeTC ℝ K :=
  ⟨ofReal⟩
#align is_R_or_C.algebra_map_coe ROrCLike.algebraMapCoe

theorem ofReal_alg (x : ℝ) : (x : K) = x • (1 : K) :=
  Algebra.algebraMap_eq_smul_one x
#align is_R_or_C.of_real_alg ROrCLike.ofReal_alg

theorem real_smul_eq_coe_mul (r : ℝ) (z : K) : r • z = (r : K) * z :=
  Algebra.smul_def r z
#align is_R_or_C.real_smul_eq_coe_mul ROrCLike.real_smul_eq_coe_mul

theorem real_smul_eq_coe_smul [AddCommGroup E] [Module K E] [Module ℝ E] [IsScalarTower ℝ K E]
    (r : ℝ) (x : E) : r • x = (r : K) • x := by rw [ROrCLike.ofReal_alg, smul_one_smul]
#align is_R_or_C.real_smul_eq_coe_smul ROrCLike.real_smul_eq_coe_smul

theorem algebraMap_eq_ofReal : ⇑(algebraMap ℝ K) = ofReal :=
  rfl
#align is_R_or_C.algebra_map_eq_of_real ROrCLike.algebraMap_eq_ofReal

@[simp, rorCLike_simps]
theorem re_add_im (z : K) : (re z : K) + im z * I = z :=
  ROrCLike.re_add_im_ax z
#align is_R_or_C.re_add_im ROrCLike.re_add_im

@[simp, norm_cast, rorCLike_simps]
theorem ofReal_re : ∀ r : ℝ, re (r : K) = r :=
  ROrCLike.ofReal_re_ax
#align is_R_or_C.of_real_re ROrCLike.ofReal_re

@[simp, norm_cast, rorCLike_simps]
theorem ofReal_im : ∀ r : ℝ, im (r : K) = 0 :=
  ROrCLike.ofReal_im_ax
#align is_R_or_C.of_real_im ROrCLike.ofReal_im

@[simp, rorCLike_simps]
theorem mul_re : ∀ z w : K, re (z * w) = re z * re w - im z * im w :=
  ROrCLike.mul_re_ax
#align is_R_or_C.mul_re ROrCLike.mul_re

@[simp, rorCLike_simps]
theorem mul_im : ∀ z w : K, im (z * w) = re z * im w + im z * re w :=
  ROrCLike.mul_im_ax
#align is_R_or_C.mul_im ROrCLike.mul_im

theorem ext_iff {z w : K} : z = w ↔ re z = re w ∧ im z = im w :=
  ⟨fun h => h ▸ ⟨rfl, rfl⟩, fun ⟨h₁, h₂⟩ => re_add_im z ▸ re_add_im w ▸ h₁ ▸ h₂ ▸ rfl⟩
#align is_R_or_C.ext_iff ROrCLike.ext_iff

theorem ext {z w : K} (hre : re z = re w) (him : im z = im w) : z = w :=
  ext_iff.2 ⟨hre, him⟩
#align is_R_or_C.ext ROrCLike.ext

@[norm_cast]
theorem ofReal_zero : ((0 : ℝ) : K) = 0 :=
  algebraMap.coe_zero
#align is_R_or_C.of_real_zero ROrCLike.ofReal_zero

@[rorCLike_simps]
theorem zero_re' : re (0 : K) = (0 : ℝ) :=
  map_zero re
#align is_R_or_C.zero_re' ROrCLike.zero_re'

@[norm_cast]
theorem ofReal_one : ((1 : ℝ) : K) = 1 :=
  map_one (algebraMap ℝ K)
#align is_R_or_C.of_real_one ROrCLike.ofReal_one

@[simp, rorCLike_simps]
theorem one_re : re (1 : K) = 1 := by rw [← ofReal_one, ofReal_re]
#align is_R_or_C.one_re ROrCLike.one_re

@[simp, rorCLike_simps]
theorem one_im : im (1 : K) = 0 := by rw [← ofReal_one, ofReal_im]
#align is_R_or_C.one_im ROrCLike.one_im

theorem ofReal_injective : Function.Injective ((↑) : ℝ → K) :=
  (algebraMap ℝ K).injective
#align is_R_or_C.of_real_injective ROrCLike.ofReal_injective

@[norm_cast]
theorem ofReal_inj {z w : ℝ} : (z : K) = (w : K) ↔ z = w :=
  algebraMap.coe_inj
#align is_R_or_C.of_real_inj ROrCLike.ofReal_inj

set_option linter.deprecated false in
@[deprecated, rorCLike_simps] -- Porting note (#10618): was `simp` but `simp` can prove it
theorem bit0_re (z : K) : re (bit0 z) = bit0 (re z) :=
  map_bit0 _ _
#align is_R_or_C.bit0_re ROrCLike.bit0_re

set_option linter.deprecated false in
@[deprecated, simp, rorCLike_simps]
theorem bit1_re (z : K) : re (bit1 z) = bit1 (re z) := by simp only [bit1, map_add, bit0_re, one_re]
#align is_R_or_C.bit1_re ROrCLike.bit1_re

set_option linter.deprecated false in
@[deprecated, rorCLike_simps] -- Porting note (#10618): was `simp` but `simp` can prove it
theorem bit0_im (z : K) : im (bit0 z) = bit0 (im z) :=
  map_bit0 _ _
#align is_R_or_C.bit0_im ROrCLike.bit0_im

set_option linter.deprecated false in
@[deprecated, simp, rorCLike_simps]
theorem bit1_im (z : K) : im (bit1 z) = bit0 (im z) := by
  simp only [bit1, map_add, bit0_im, one_im, add_zero]
#align is_R_or_C.bit1_im ROrCLike.bit1_im

theorem ofReal_eq_zero {x : ℝ} : (x : K) = 0 ↔ x = 0 :=
  algebraMap.lift_map_eq_zero_iff x
#align is_R_or_C.of_real_eq_zero ROrCLike.ofReal_eq_zero

theorem ofReal_ne_zero {x : ℝ} : (x : K) ≠ 0 ↔ x ≠ 0 :=
  ofReal_eq_zero.not
#align is_R_or_C.of_real_ne_zero ROrCLike.ofReal_ne_zero

@[simp, rorCLike_simps, norm_cast]
theorem ofReal_add (r s : ℝ) : ((r + s : ℝ) : K) = r + s :=
  algebraMap.coe_add _ _
#align is_R_or_C.of_real_add ROrCLike.ofReal_add

set_option linter.deprecated false in
@[deprecated, simp, rorCLike_simps, norm_cast]
theorem ofReal_bit0 (r : ℝ) : ((bit0 r : ℝ) : K) = bit0 (r : K) :=
  ofReal_add _ _
#align is_R_or_C.of_real_bit0 ROrCLike.ofReal_bit0

set_option linter.deprecated false in
@[deprecated, simp, rorCLike_simps, norm_cast]
theorem ofReal_bit1 (r : ℝ) : ((bit1 r : ℝ) : K) = bit1 (r : K) :=
  map_bit1 (algebraMap ℝ K) r
#align is_R_or_C.of_real_bit1 ROrCLike.ofReal_bit1

@[simp, norm_cast, rorCLike_simps]
theorem ofReal_neg (r : ℝ) : ((-r : ℝ) : K) = -r :=
  algebraMap.coe_neg r
#align is_R_or_C.of_real_neg ROrCLike.ofReal_neg

@[simp, norm_cast, rorCLike_simps]
theorem ofReal_sub (r s : ℝ) : ((r - s : ℝ) : K) = r - s :=
  map_sub (algebraMap ℝ K) r s
#align is_R_or_C.of_real_sub ROrCLike.ofReal_sub

@[simp, rorCLike_simps, norm_cast]
theorem ofReal_sum {α : Type*} (s : Finset α) (f : α → ℝ) :
    ((∑ i in s, f i : ℝ) : K) = ∑ i in s, (f i : K) :=
  map_sum (algebraMap ℝ K) _ _
#align is_R_or_C.of_real_sum ROrCLike.ofReal_sum
...
theorem I_re : re (I : K) = 0 :=
  I_re_ax
set_option linter.uppercaseLean3 false in
#align is_R_or_C.I_re ROrCLike.I_re

@[simp, rorCLike_simps]
theorem I_im (z : K) : im z * im (I : K) = im z :=
  mul_im_I_ax z
set_option linter.uppercaseLean3 false in
#align is_R_or_C.I_im ROrCLike.I_im
...
@[rorCLike_simps] -- porting note (#10618): was `simp`
theorem I_mul_re (z : K) : re (I * z) = -im z := by
  simp only [I_re, zero_sub, I_im', zero_mul, mul_re]
set_option linter.uppercaseLean3 false in
#align is_R_or_C.I_mul_re ROrCLike.I_mul_re

theorem I_mul_I : (I : K) = 0 ∨ (I : K) * I = -1 :=
  I_mul_I_ax
set_option linter.uppercaseLean3 false in
#align is_R_or_C.I_mul_I ROrCLike.I_mul_I
...
theorem conj_re (z : K) : re (conj z) = re z :=
  ROrCLike.conj_re_ax z
#align is_R_or_C.conj_re ROrCLike.conj_re

@[simp, rorCLike_simps]
theorem conj_im (z : K) : im (conj z) = -im z :=
  ROrCLike.conj_im_ax z
#align is_R_or_C.conj_im ROrCLike.conj_im

@[simp, rorCLike_simps]
theorem conj_I : conj (I : K) = -I :=
  ROrCLike.conj_I_ax
set_option linter.uppercaseLean3 false in
#align is_R_or_C.conj_I ROrCLike.conj_I

@[simp, rorCLike_simps]
theorem conj_ofReal (r : ℝ) : conj (r : K) = (r : K) := by
  rw [ext_iff]
  simp only [ofReal_im, conj_im, eq_self_iff_true, conj_re, and_self_iff, neg_zero]
#align is_R_or_C.conj_of_real ROrCLike.conj_ofReal

set_option linter.deprecated false in
@[deprecated, rorCLike_simps] -- Porting note (#10618): was `simp` but `simp` can prove it
theorem conj_bit0 (z : K) : conj (bit0 z) = bit0 (conj z) :=
  map_bit0 _ _
#align is_R_or_C.conj_bit0 ROrCLike.conj_bit0

set_option linter.deprecated false in
@[deprecated, rorCLike_simps] -- Porting note (#10618): was `simp` but `simp` can prove it
theorem conj_bit1 (z : K) : conj (bit1 z) = bit1 (conj z) :=
  map_bit1 _ _
#align is_R_or_C.conj_bit1 ROrCLike.conj_bit1

@[rorCLike_simps] -- Porting note (#10618): was a `simp` but `simp` can prove it
theorem conj_neg_I : conj (-I) = (I : K) := by rw [map_neg, conj_I, neg_neg]
set_option linter.uppercaseLean3 false in
#align is_R_or_C.conj_neg_I ROrCLike.conj_neg_I
...
#align is_R_or_C.conj_eq_re_sub_im ROrCLike.conj_eq_re_sub_im
...
#align is_R_or_C.sub_conj ROrCLike.sub_conj
...
#align is_R_or_C.conj_smul ROrCLike.conj_smul

theorem add_conj (z : K) : z + conj z = 2 * re z :=
  calc
    z + conj z = re z + im z * I + (re z - im z * I) := by rw [re_add_im, conj_eq_re_sub_im]
    _ = 2 * re z := by rw [add_add_sub_cancel, two_mul]
#align is_R_or_C.add_conj ROrCLike.add_conj

theorem re_eq_add_conj (z : K) : ↑(re z) = (z + conj z) / 2 := by
  rw [add_conj, mul_div_cancel_left (re z : K) two_ne_zero]
#align is_R_or_C.re_eq_add_conj ROrCLike.re_eq_add_conj
...
#align is_R_or_C.im_eq_conj_sub ROrCLike.im_eq_conj_sub
...
abbrev conjToRingEquiv : K ≃+* Kᵐᵒᵖ :=
  starRingEquiv
#align is_R_or_C.conj_to_ring_equiv ROrCLike.conjToRingEquiv
...
def normSq : K →*₀ ℝ where
  ...
  map_mul' z w := by
    ...
    ring
#align is_R_or_C.norm_sq ROrCLike.normSq

theorem normSq_apply (z : K) : normSq z = re z * re z + im z * im z :=
  rfl
#align is_R_or_C.norm_sq_apply ROrCLike.normSq_apply

theorem norm_sq_eq_def {z : K} : ‖z‖ ^ 2 = re z * re z + im z * im z :=
  norm_sq_eq_def_ax z
#align is_R_or_C.norm_sq_eq_def ROrCLike.norm_sq_eq_def

theorem normSq_eq_def' (z : K) : normSq z = ‖z‖ ^ 2 :=
  norm_sq_eq_def.symm
#align is_R_or_C.norm_sq_eq_def' ROrCLike.normSq_eq_def'

@[rorCLike_simps]
theorem normSq_zero : normSq (0 : K) = 0 :=
  normSq.map_zero
#align is_R_or_C.norm_sq_zero ROrCLike.normSq_zero

@[rorCLike_simps]
theorem normSq_one : normSq (1 : K) = 1 :=
  normSq.map_one
#align is_R_or_C.norm_sq_one ROrCLike.normSq_one

theorem normSq_nonneg (z : K) : 0 ≤ normSq z :=
  add_nonneg (mul_self_nonneg _) (mul_self_nonneg _)
#align is_R_or_C.norm_sq_nonneg ROrCLike.normSq_nonneg

@[rorCLike_simps] -- porting note (#10618): was `simp`
theorem normSq_eq_zero {z : K} : normSq z = 0 ↔ z = 0 :=
  map_eq_zero _
#align is_R_or_C.norm_sq_eq_zero ROrCLike.normSq_eq_zero

@[simp, rorCLike_simps]
theorem normSq_pos {z : K} : 0 < normSq z ↔ z ≠ 0 := by
  rw [lt_iff_le_and_ne, Ne, eq_comm]; simp [normSq_nonneg]
#align is_R_or_C.norm_sq_pos ROrCLike.normSq_pos

@[simp, rorCLike_simps]
theorem normSq_neg (z : K) : normSq (-z) = normSq z := by simp only [normSq_eq_def', norm_neg]
#align is_R_or_C.norm_sq_neg ROrCLike.normSq_neg

@[simp, rorCLike_simps]
theorem normSq_conj (z : K) : normSq (conj z) = normSq z := by
  simp only [normSq_apply, neg_mul, mul_neg, neg_neg, rorCLike_simps]
#align is_R_or_C.norm_sq_conj ROrCLike.normSq_conj

@[rorCLike_simps] -- porting note (#10618): was `simp`
theorem normSq_mul (z w : K) : normSq (z * w) = normSq z * normSq w :=
  map_mul _ z w
#align is_R_or_C.norm_sq_mul ROrCLike.normSq_mul

theorem normSq_add (z w : K) : normSq (z + w) = normSq z + normSq w + 2 * re (z * conj w) := by
  simp only [normSq_apply, map_add, rorCLike_simps]
  ring
#align is_R_or_C.norm_sq_add ROrCLike.normSq_add

theorem re_sq_le_normSq (z : K) : re z * re z ≤ normSq z :=
  le_add_of_nonneg_right (mul_self_nonneg _)
#align is_R_or_C.re_sq_le_norm_sq ROrCLike.re_sq_le_normSq

theorem im_sq_le_normSq (z : K) : im z * im z ≤ normSq z :=
  le_add_of_nonneg_left (mul_self_nonneg _)
#align is_R_or_C.im_sq_le_norm_sq ROrCLike.im_sq_le_normSq

theorem mul_conj (z : K) : z * conj z = ‖z‖ ^ 2 := by
  apply ext <;> simp [← ofReal_pow, norm_sq_eq_def, mul_comm]

#align is_R_or_C.mul_conj ROrCLike.mul_conj

theorem conj_mul (z : K) : conj z * z = ‖z‖ ^ 2 := by rw [mul_comm, mul_conj]
#align is_R_or_C.conj_mul ROrCLike.conj_mul
...
#align is_R_or_C.norm_sq_sub ROrCLike.normSq_sub

theorem sqrt_normSq_eq_norm {z : K} : Real.sqrt (normSq z) = ‖z‖ := by
  rw [normSq_eq_def', Real.sqrt_sq (norm_nonneg _)]
#align is_R_or_C.sqrt_norm_sq_eq_norm ROrCLike.sqrt_normSq_eq_norm
...
theorem ofReal_inv (r : ℝ) : ((r⁻¹ : ℝ) : K) = (r : K)⁻¹ :=
  map_inv₀ _ r
#align is_R_or_C.of_real_inv ROrCLike.ofReal_inv
...
theorem div_re (z w : K) : re (z / w) = re z * re w / normSq w + im z * im w / normSq w := by
  simp only [div_eq_mul_inv, mul_assoc, sub_eq_add_neg, neg_mul, mul_neg, neg_neg, map_neg,
    rorCLike_simps]
#align is_R_or_C.div_re ROrCLike.div_re
...
theorem div_im (z w : K) : im (z / w) = im z * re w / normSq w - re z * im w / normSq w := by
  simp only [div_eq_mul_inv, mul_assoc, sub_eq_add_neg, add_comm, neg_mul, mul_neg, map_neg,
    rorCLike_simps]
#align is_R_or_C.div_im ROrCLike.div_im

@[rorCLike_simps] -- porting note (#10618): was `simp`
theorem conj_inv (x : K) : conj x⁻¹ = (conj x)⁻¹ :=
  star_inv' _
#align is_R_or_C.conj_inv ROrCLike.conj_inv
...
#align is_R_or_C.of_real_nat_cast ROrCLike.ofReal_natCast

@[simp, rorCLike_simps] -- Porting note: removed `norm_cast`
theorem natCast_re (n : ℕ) : re (n : K) = n := by rw [← ofReal_natCast, ofReal_re]
#align is_R_or_C.nat_cast_re ROrCLike.natCast_re

@[simp, rorCLike_simps, norm_cast]
theorem natCast_im (n : ℕ) : im (n : K) = 0 := by rw [← ofReal_natCast, ofReal_im]
#align is_R_or_C.nat_cast_im ROrCLike.natCast_im
...
lemma norm_nsmul [NormedAddCommGroup E] [NormedSpace K E] (n : ℕ) (x : E) : ‖n • x‖ = n • ‖x‖ := by
  rw [nsmul_eq_smul_cast K, norm_smul, ROrCLike.norm_natCast, nsmul_eq_mul]

theorem mul_self_norm (z : K) : ‖z‖ * ‖z‖ = normSq z := by rw [normSq_eq_def', sq]
#align is_R_or_C.mul_self_norm ROrCLike.mul_self_norm

attribute [rorCLike_simps] norm_zero norm_one norm_eq_zero abs_norm norm_inv norm_div

-- Porting note: removed @[simp, rorCLike_simps], b/c generalized to `norm_ofNat`
theorem norm_two : ‖(2 : K)‖ = 2 := norm_ofNat 2
#align is_R_or_C.norm_two ROrCLike.norm_two
...
#align is_R_or_C.abs_re_le_norm ROrCLike.abs_re_le_norm

theorem abs_im_le_norm (z : K) : |im z| ≤ ‖z‖ := by
  rw [mul_self_le_mul_self_iff (abs_nonneg _) (norm_nonneg _), abs_mul_abs_self, mul_self_norm]
  apply im_sq_le_normSq
#align is_R_or_C.abs_im_le_norm ROrCLike.abs_im_le_norm

theorem norm_re_le_norm (z : K) : ‖re z‖ ≤ ‖z‖ :=
  abs_re_le_norm z
#align is_R_or_C.norm_re_le_norm ROrCLike.norm_re_le_norm

theorem norm_im_le_norm (z : K) : ‖im z‖ ≤ ‖z‖ :=
  abs_im_le_norm z
#align is_R_or_C.norm_im_le_norm ROrCLike.norm_im_le_norm

theorem re_le_norm (z : K) : re z ≤ ‖z‖ :=
  (abs_le.1 (abs_re_le_norm z)).2
#align is_R_or_C.re_le_norm ROrCLike.re_le_norm

theorem im_le_norm (z : K) : im z ≤ ‖z‖ :=
  (abs_le.1 (abs_im_le_norm _)).2
#align is_R_or_C.im_le_norm ROrCLike.im_le_norm
...
#align is_R_or_C.im_eq_zero_of_le ROrCLike.im_eq_zero_of_le

theorem re_eq_self_of_le {a : K} (h : ‖a‖ ≤ re a) : (re a : K) = a := by
  rw [← conj_eq_iff_re, conj_eq_iff_im, im_eq_zero_of_le h]
#align is_R_or_C.re_eq_self_of_le ROrCLike.re_eq_self_of_le
...
#align is_R_or_C.abs_re_div_norm_le_one ROrCLike.abs_re_div_norm_le_one

theorem abs_im_div_norm_le_one (z : K) : |im z / ‖z‖| ≤ 1 := by
  rw [abs_div, abs_norm]
  exact div_le_one_of_le (abs_im_le_norm _) (norm_nonneg _)
#align is_R_or_C.abs_im_div_norm_le_one ROrCLike.abs_im_div_norm_le_one

theorem norm_I_of_ne_zero (hI : (I : K) ≠ 0) : ‖(I : K)‖ = 1 := by
  rw [← mul_self_inj_of_nonneg (norm_nonneg I) zero_le_one, one_mul, ← norm_mul,
    I_mul_I_of_nonzero hI, norm_neg, norm_one]
set_option linter.uppercaseLean3 false in
#align is_R_or_C.norm_I_of_ne_zero ROrCLike.norm_I_of_ne_zero

theorem re_eq_norm_of_mul_conj (x : K) : re (x * conj x) = ‖x * conj x‖ := by
  rw [mul_conj, ← ofReal_pow]; simp [-ofReal_pow]
#align is_R_or_C.re_eq_norm_of_mul_conj ROrCLike.re_eq_norm_of_mul_conj
...
#align is_R_or_C.norm_sq_re_add_conj ROrCLike.norm_sq_re_add_conj

theorem norm_sq_re_conj_add (x : K) : ‖conj x + x‖ ^ 2 = re (conj x + x) ^ 2 := by
  rw [add_comm, norm_sq_re_add_conj]
#align is_R_or_C.norm_sq_re_conj_add ROrCLike.norm_sq_re_conj_add
...
#align is_R_or_C.is_cau_seq_re ROrCLike.isCauSeq_re
...
#align is_R_or_C.is_cau_seq_im ROrCLike.isCauSeq_im

/-- The real part of a K Cauchy sequence, as a real Cauchy sequence. -/
noncomputable def cauSeqRe (f : CauSeq K norm) : CauSeq ℝ abs :=
  ⟨_, isCauSeq_re f⟩
#align is_R_or_C.cau_seq_re ROrCLike.cauSeqRe

/-- The imaginary part of a K Cauchy sequence, as a real Cauchy sequence. -/
noncomputable def cauSeqIm (f : CauSeq K norm) : CauSeq ℝ abs :=
  ⟨_, isCauSeq_im f⟩
#align is_R_or_C.cau_seq_im ROrCLike.cauSeqIm
...
#align is_R_or_C.is_cau_seq_norm ROrCLike.isCauSeq_norm

end ROrCLike

section Instances

noncomputable instance Real.ROrCLike : ROrCLike ℝ where
...
#align real.is_R_or_C Real.ROrCLike
...
theorem lt_iff_re_im : z < w ↔ re z < re w ∧ im z = im w := by
  simp_rw [lt_iff_le_and_ne, @ROrCLike.le_iff_re_im K]
  constructor
  · rintro ⟨⟨hr, hi⟩, heq⟩
...
scoped[ComplexOrder] attribute [instance] ROrCLike.toStarOrderedRing
...
def toStrictOrderedCommRing : StrictOrderedCommRing K where
  zero_le_one := by simp [@ROrCLike.le_iff_re_im K]
...
scoped[ComplexOrder] attribute [instance] ROrCLike.toStrictOrderedCommRing

theorem toOrderedSMul : OrderedSMul ℝ K :=
  OrderedSMul.mk' fun a b r hab hr => by
    replace hab := hab.le
    rw [ROrCLike.le_iff_re_im] at hab
    rw [ROrCLike.le_iff_re_im, smul_re, smul_re, smul_im, smul_im]
    exact hab.imp (fun h => mul_le_mul_of_nonneg_left h hr.le) (congr_arg _)

scoped[ComplexOrder] attribute [instance] ROrCLike.toOrderedSMul
...
local notation "reR" => @ROrCLike.re ℝ _
local notation "imR" => @ROrCLike.im ℝ _
local notation "IR" => @ROrCLike.I ℝ _
local notation "normSqR" => @ROrCLike.normSq ℝ _

@[simp, rorCLike_simps]
theorem re_to_real {x : ℝ} : reR x = x :=
  rfl
#align is_R_or_C.re_to_real ROrCLike.re_to_real

@[simp, rorCLike_simps]
theorem im_to_real {x : ℝ} : imR x = 0 :=
  rfl
#align is_R_or_C.im_to_real ROrCLike.im_to_real

@[rorCLike_simps]
theorem conj_to_real {x : ℝ} : conj x = x :=
  rfl
#align is_R_or_C.conj_to_real ROrCLike.conj_to_real

@[simp, rorCLike_simps]
theorem I_to_real : IR = 0 :=
  rfl
set_option linter.uppercaseLean3 false in
#align is_R_or_C.I_to_real ROrCLike.I_to_real

@[simp, rorCLike_simps]
theorem normSq_to_real {x : ℝ} : normSq x = x * x := by simp [ROrCLike.normSq]
#align is_R_or_C.norm_sq_to_real ROrCLike.normSq_to_real

@[simp]
theorem ofReal_real_eq_id : @ofReal ℝ _ = id :=
  rfl
#align is_R_or_C.coe_real_eq_id ROrCLike.ofReal_real_eq_id

end CleanupLemmas

section LinearMaps

/-- The real part in an `ROrCLike` field, as a linear map. -/
def reLm : K →ₗ[ℝ] ℝ :=
  { re with map_smul' := smul_re }
#align is_R_or_C.re_lm ROrCLike.reLm

@[simp, rorCLike_simps]
theorem reLm_coe : (reLm : K → ℝ) = re :=
  rfl
#align is_R_or_C.re_lm_coe ROrCLike.reLm_coe

/-- The real part in an `ROrCLike` field, as a continuous linear map. -/
noncomputable def reCLM : K →L[ℝ] ℝ :=
  reLm.mkContinuous 1 fun x => by
    rw [one_mul]
    exact abs_re_le_norm x
#align is_R_or_C.re_clm ROrCLike.reCLM

@[simp, rorCLike_simps, norm_cast]
theorem reCLM_coe : ((reCLM : K →L[ℝ] ℝ) : K →ₗ[ℝ] ℝ) = reLm :=
  rfl
#align is_R_or_C.re_clm_coe ROrCLike.reCLM_coe

@[simp, rorCLike_simps]
theorem reCLM_apply : ((reCLM : K →L[ℝ] ℝ) : K → ℝ) = re :=
  rfl
#align is_R_or_C.re_clm_apply ROrCLike.reCLM_apply

@[continuity]
theorem continuous_re : Continuous (re : K → ℝ) :=
  reCLM.continuous
#align is_R_or_C.continuous_re ROrCLike.continuous_re

/-- The imaginary part in an `ROrCLike` field, as a linear map. -/
def imLm : K →ₗ[ℝ] ℝ :=
  { im with map_smul' := smul_im }
#align is_R_or_C.im_lm ROrCLike.imLm

@[simp, rorCLike_simps]
theorem imLm_coe : (imLm : K → ℝ) = im :=
  rfl
#align is_R_or_C.im_lm_coe ROrCLike.imLm_coe

/-- The imaginary part in an `ROrCLike` field, as a continuous linear map. -/
noncomputable def imCLM : K →L[ℝ] ℝ :=
  imLm.mkContinuous 1 fun x => by
    rw [one_mul]
    exact abs_im_le_norm x
#align is_R_or_C.im_clm ROrCLike.imCLM

@[simp, rorCLike_simps, norm_cast]
theorem imCLM_coe : ((imCLM : K →L[ℝ] ℝ) : K →ₗ[ℝ] ℝ) = imLm :=
  rfl
#align is_R_or_C.im_clm_coe ROrCLike.imCLM_coe

@[simp, rorCLike_simps]
theorem imCLM_apply : ((imCLM : K →L[ℝ] ℝ) : K → ℝ) = im :=
  rfl
#align is_R_or_C.im_clm_apply ROrCLike.imCLM_apply

@[continuity]
theorem continuous_im : Continuous (im : K → ℝ) :=
  imCLM.continuous
#align is_R_or_C.continuous_im ROrCLike.continuous_im
...
#align is_R_or_C.conj_ae ROrCLike.conjAe

@[simp, rorCLike_simps]
theorem conjAe_coe : (conjAe : K → K) = conj :=
  rfl
#align is_R_or_C.conj_ae_coe ROrCLike.conjAe_coe

/-- Conjugate as a linear isometry -/
noncomputable def conjLIE : K ≃ₗᵢ[ℝ] K :=
  ⟨conjAe.toLinearEquiv, fun _ => norm_conj⟩
#align is_R_or_C.conj_lie ROrCLike.conjLIE

@[simp, rorCLike_simps]
theorem conjLIE_apply : (conjLIE : K → K) = conj :=
  rfl
#align is_R_or_C.conj_lie_apply ROrCLike.conjLIE_apply

/-- Conjugate as a continuous linear equivalence -/
noncomputable def conjCLE : K ≃L[ℝ] K :=
  @conjLIE K _
#align is_R_or_C.conj_cle ROrCLike.conjCLE

@[simp, rorCLike_simps]
theorem conjCLE_coe : (@conjCLE K _).toLinearEquiv = conjAe.toLinearEquiv :=
  rfl
#align is_R_or_C.conj_cle_coe ROrCLike.conjCLE_coe

@[simp, rorCLike_simps]
theorem conjCLE_apply : (conjCLE : K → K) = conj :=
  rfl
#align is_R_or_C.conj_cle_apply ROrCLike.conjCLE_apply
...
/-- The natural isomorphism between `𝕜` satisfying `ROrCLike 𝕜` and `ℝ` when `ROrCLike.I = 0`. -/
@[simps]
...
/-- The natural `ℝ`-linear isometry equivalence between `𝕜` satisfying `ROrCLike 𝕜` and `ℝ` when
`ROrCLike.I = 0`. -/
@[simps]
