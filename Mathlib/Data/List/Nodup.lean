...
@[simp]
lemma forall_mem_ne {a : α} {l : List α} : (∀ a' : α, a' ∈ l → ¬a = a') ↔ a ∉ l :=
  ⟨fun h m => h _ m rfl, fun h _ m e => h (e.symm ▸ m)⟩
#align list.forall_mem_ne List.forall_mem_ne

@[simp]
lemma nodup_nil : @Nodup α [] :=
  Pairwise.nil
#align list.nodup_nil List.nodup_nil

@[simp]
lemma nodup_cons {a : α} {l : List α} : Nodup (a :: l) ↔ a ∉ l ∧ Nodup l := by
  simp only [Nodup, pairwise_cons, forall_mem_ne]
#align list.nodup_cons List.nodup_cons

protected lemma Pairwise.nodup {l : List α} {r : α → α → Prop} [IsIrrefl α r] (h : Pairwise r l) :
    Nodup l :=
  h.imp ne_of_irrefl
#align list.pairwise.nodup List.Pairwise.nodup

lemma rel_nodup {r : α → β → Prop} (hr : Relator.BiUnique r) : (Forall₂ r ⇒ (· ↔ ·)) Nodup Nodup
...
lemma not_nodup_cons_of_mem : a ∈ l → ¬Nodup (a :: l) :=
  imp_not_comm.1 Nodup.not_mem
#align list.not_nodup_cons_of_mem List.not_nodup_cons_of_mem

protected lemma Nodup.sublist : l₁ <+ l₂ → Nodup l₂ → Nodup l₁ :=
  Pairwise.sublist
...
lemma nodup_iff_sublist {l : List α} : Nodup l ↔ ∀ a, ¬[a, a] <+ l :=
...
lemma nodup_iff_injective_get {l : List α} :
...
lemma nodup_iff_nthLe_inj {l : List α} :
...
lemma Nodup.get_inj_iff {l : List α} (h : Nodup l) {i j : Fin l.length} :
    l.get i = l.get j ↔ i = j :=
  (nodup_iff_injective_get.1 h).eq_iff

set_option linter.deprecated false in
@[deprecated Nodup.get_inj_iff] -- 2023-01-10
lemma Nodup.nthLe_inj_iff {l : List α} (h : Nodup l) {i j : ℕ} (hi : i < l.length)
...
lemma nodup_iff_get?_ne_get? {l : List α} :
...
-- Porting note (#10756): new theorem
lemma get_indexOf [DecidableEq α] {l : List α} (H : Nodup l) (i : Fin l.length) :
    indexOf (get l i) l = i :=
...
lemma nodup_iff_count_le_one [DecidableEq α] {l : List α} : Nodup l ↔ ∀ a, count a l ≤ 1 :=
...
lemma nodup_iff_count_eq_one [DecidableEq α] : Nodup l ↔ ∀ a ∈ l, count a l = 1 :=
...
@[simp]
lemma count_eq_one_of_mem [DecidableEq α] {a : α} {l : List α} (d : Nodup l) (h : a ∈ l) :
    count a l = 1 :=
...
lemma count_eq_of_nodup [DecidableEq α] {a : α} {l : List α} (d : Nodup l) :
...
lemma nodup_append {l₁ l₂ : List α} : Nodup (l₁ ++ l₂) ↔ Nodup l₁ ∧ Nodup l₂ ∧ Disjoint l₁ l₂ :=
  by simp only [Nodup, pairwise_append, disjoint_iff_ne]
#align list.nodup_append List.nodup_append

lemma disjoint_of_nodup_append {l₁ l₂ : List α} (d : Nodup (l₁ ++ l₂)) : Disjoint l₁ l₂ :=
...
lemma inj_on_of_nodup_map {f : α → β} {l : List α} (d : Nodup (map f l)) :
...
lemma nodup_map_iff_inj_on {f : α → β} {l : List α} (d : Nodup l) :
...
lemma nodup_map_iff {f : α → β} {l : List α} (hf : Injective f) : Nodup (map f l) ↔ Nodup l :=
  ⟨Nodup.of_map _, Nodup.map hf⟩
#align list.nodup_map_iff List.nodup_map_iff

@[simp]
lemma nodup_attach {l : List α} : Nodup (attach l) ↔ Nodup l :=
...
lemma Nodup.pmap {p : α → Prop} {f : ∀ a, p a → β} {l : List α} {H}
...
@[simp]
lemma nodup_reverse {l : List α} : Nodup (reverse l) ↔ Nodup l :=
  pairwise_reverse.trans <| by simp only [Nodup, Ne, eq_comm]
#align list.nodup_reverse List.nodup_reverse

lemma Nodup.erase_eq_filter [DecidableEq α] {l} (d : Nodup l) (a : α) :
...
lemma Nodup.erase [DecidableEq α] (a : α) : Nodup l → Nodup (l.erase a) :=
  Nodup.sublist <| erase_sublist _ _
#align list.nodup.erase List.Nodup.erase

lemma Nodup.erase_get [DecidableEq α] {l : List α} (hl : l.Nodup) :
...
lemma nodup_bind {l₁ : List α} {f : α → List β} :
    Nodup (l₁.bind f) ↔
...
lemma Nodup.sigma {σ : α → Type*} {l₂ : ∀ a , List (σ a)} (d₁ : Nodup l₁)
...
protected lemma Nodup.filterMap {f : α → Option β} (h : ∀ a a' b, b ∈ f a → b ∈ f a' → a = a') :
...
protected lemma Nodup.insert [DecidableEq α] (h : l.Nodup) : (l.insert a).Nodup :=
...
lemma Nodup.union [DecidableEq α] (l₁ : List α) (h : Nodup l₂) : (l₁ ∪ l₂).Nodup := by
  induction' l₁ with a l₁ ih generalizing l₂
  · exact h
  · exact (ih h).insert
#align list.nodup.union List.Nodup.union

lemma Nodup.inter [DecidableEq α] (l₂ : List α) : Nodup l₁ → Nodup (l₁ ∩ l₂) :=
  Nodup.filter _
#align list.nodup.inter List.Nodup.inter

lemma Nodup.diff_eq_filter [DecidableEq α] :
...
lemma Nodup.map_update [DecidableEq α] {l : List α} (hl : l.Nodup) (f : α → β) (x : α) (y : β) :
...
lemma Nodup.pairwise_of_forall_ne {l : List α} {r : α → α → Prop} (hl : l.Nodup)
...
lemma Nodup.pairwise_of_set_pairwise {l : List α} {r : α → α → Prop} (hl : l.Nodup)
...
@[simp]
lemma Nodup.pairwise_coe [IsSymm α r] (hl : l.Nodup) :
...
-- Porting note (#10756): new theorem
lemma Nodup.take_eq_filter_mem [DecidableEq α] :
...
lemma Option.toList_nodup {α} : ∀ o : Option α, o.toList.Nodup
  | none => List.nodup_nil
