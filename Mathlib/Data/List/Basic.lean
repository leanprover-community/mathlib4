...
theorem mem_getLast?_append_of_mem_getLast?_right {l₁ l₂ : List α} {x : α} (h : x ∈ l₂.getLast?) :
    x ∈ (l₁ ++ l₂).getLast? := by
  cases l₂
  · contradiction
  · rw [List.getLast?_append_cons]
    exact h
#align list.last'_append List.mem_getLast?_append_of_mem_getLast?_right
...
theorem mem_head?_append_of_mem_head?_left {s t : List α} {x : α} (h : x ∈ s.head?) :
    x ∈ (s ++ t).head? := by
  cases s
  · contradiction
  · exact h
#align list.head'_append List.mem_head?_append_of_mem_head?_left
