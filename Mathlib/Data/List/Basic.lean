...
theorem sizeOf_lt_sizeOf_of_mem [SizeOf α] {x : α} {l : List α} (hx : x ∈ l) :
  ...
  induction l with
  | nil => exact sizeOf_lt_of_mem hx
  | cons h t ih =>
    cases hx <;> rw [cons.sizeOf_spec]
    · omega
    · specialize ih ‹_›; omega
...
theorem filterMap_eq_flatMap_toList (f : α → Option β) (l : List α) :
  ...
  induction l with
  | nil => simp [filterMap_cons]
  | cons a l ih => simp [filterMap_cons]; rcases f a <;> simp [ih]
