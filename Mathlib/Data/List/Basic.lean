...
@[simp] lemma append_left_eq_self {x y : List α} : x ++ y = y ↔ x = [] := by
  rw [← append_left_inj (s₁ := x), nil_append]

@[simp] lemma self_eq_append_left {x y : List α} : y = x ++ y ↔ x = [] := by
  rw [eq_comm, append_left_eq_self]

@[simp] lemma append_right_eq_self {x y : List α} : x ++ y = x ↔ y = [] := by
  rw [← append_right_inj (t₁ := y), append_nil]

@[simp] lemma self_eq_append_right {x y : List α} : x = x ++ y ↔ y = [] := by
...
@[simp] lemma head_replicate (n : ℕ) (a : α) (h) : head (replicate n a) h = a := by
  cases n <;> simp at h ⊢

/-! ### pure -/

lemma mem_pure {α} (x y : α) : x ∈ (pure y : List α) ↔ x = y := by simp
...
lemma reverse_concat' (l : List α) (a : α) : (l ++ [a]).reverse = a :: l.reverse := by
...
@[simp]
lemma head!_nil [Inhabited α] : ([] : List α).head! = default := rfl

@[simp] lemma head_cons_tail (x : List α) (h : x ≠ []) : x.head h :: x.tail = x := by
  cases x <;> simp at h ⊢
...
lemma get_eq_get? (l : List α) (i : Fin l.length) :
...
@[simp]
lemma reverseRecOn_nil {motive : List α → Sort*} (nil : motive [])
...
lemma reverseRecOn_concat {motive : List α → Sort*} (x : α) (xs : List α) (nil : motive [])
    (append_singleton : ∀ (l : List α) (a : α), motive l → motive (l ++ [a])) :
...
@[simp]
lemma bidirectionalRec_nil {motive : List α → Sort*}
...
@[simp]
lemma bidirectionalRec_singleton {motive : List α → Sort*}
    (nil : motive []) (singleton : ∀ a : α, motive [a])
...
@[simp]
lemma bidirectionalRec_cons_append {motive : List α → Sort*}
    (nil : motive []) (singleton : ∀ a : α, motive [a])
    (cons_append : ∀ (a : α) (l : List α) (b : α), motive l → motive (a :: (l ++ [b])))
    (a : α) (l : List α) (b : α) :
...
lemma sublist_cons_of_sublist (a : α) (h : l₁ <+ l₂) : l₁ <+ a :: l₂ := h.cons _
...
lemma tail_sublist : ∀ l : List α, tail l <+ l
  | [] => .slnil
  | a::l => sublist_cons a l
#align list.tail_sublist List.tail_sublist

@[gcongr] protected lemma Sublist.tail : ∀ {l₁ l₂ : List α}, l₁ <+ l₂ → tail l₁ <+ tail l₂
  | _, _, slnil => .slnil
  | _, _, Sublist.cons _ h => (tail_sublist _).trans h
  | _, _, Sublist.cons₂ _ h => h

lemma Sublist.of_cons_cons {l₁ l₂ : List α} {a b : α} (h : a :: l₁ <+ b :: l₂) : l₁ <+ l₂ :=
  h.tail
#align list.sublist_of_cons_sublist_cons List.Sublist.of_cons_cons

@[deprecated] -- 2024-04-07
lemma sublist_of_cons_sublist_cons {a} (h : a :: l₁ <+ a :: l₂) : l₁ <+ l₂ := h.of_cons_cons
...
-- fun e => if_pos e
lemma indexOf_cons_eq {a b : α} (l : List α) : b = a → indexOf a (b :: l) = 0
  | e => by rw [← e]; exact indexOf_cons_self b l
#align list.index_of_cons_eq List.indexOf_cons_eq

-- fun n => if_neg n
@[simp]
lemma indexOf_cons_ne {a b : α} (l : List α) : b ≠ a → indexOf a (b :: l) = succ (indexOf a l)
...
lemma ext_get?' {l₁ l₂ : List α} (h' : ∀ n < max l₁.length l₂.length, l₁.get? n = l₂.get? n) :
    l₁ = l₂ := by
  apply ext
  intro n
...
lemma ext_get?_iff {l₁ l₂ : List α} : l₁ = l₂ ↔ ∀ n, l₁.get? n = l₂.get? n :=
  ⟨by rintro rfl _; rfl, ext_get?⟩

lemma ext_get_iff {l₁ l₂ : List α} :
...
lemma ext_get?_iff' {l₁ l₂ : List α} : l₁ = l₂ ↔
...
lemma bind_pure_eq_map (f : α → β) (l : List α) : l.bind (pure ∘ f) = map f l :=
  .symm <| map_eq_bind ..
#align list.bind_ret_eq_map List.bind_pure_eq_map

set_option linter.deprecated false in
@[deprecated bind_pure_eq_map] -- 2024-03-24
lemma bind_ret_eq_map (f : α → β) (l : List α) : l.bind (List.ret ∘ f) = map f l :=
...
lemma _root_.Function.LeftInverse.list_map {f : α → β} {g : β → α} (h : LeftInverse f g) :
    LeftInverse (map f) (map g)
  | [] => by simp_rw [map_nil]
  | x :: xs => by simp_rw [map_cons, h x, h.list_map xs]

nonrec lemma _root_.Function.RightInverse.list_map {f : α → β} {g : β → α}
    (h : RightInverse f g) : RightInverse (map f) (map g) :=
  h.list_map

nonrec lemma _root_.Function.Involutive.list_map {f : α → α}
    (h : Involutive f) : Involutive (map f) :=
  Function.LeftInverse.list_map h

@[simp]
lemma map_leftInverse_iff {f : α → β} {g : β → α} :
    LeftInverse (map f) (map g) ↔ LeftInverse f g :=
  ⟨fun h x => by injection h [x], (·.list_map)⟩

@[simp]
lemma map_rightInverse_iff {f : α → β} {g : β → α} :
    RightInverse (map f) (map g) ↔ RightInverse f g := map_leftInverse_iff

@[simp]
lemma map_involutive_iff {f : α → α} :
    Involutive (map f) ↔ Involutive f := map_leftInverse_iff

lemma _root_.Function.Injective.list_map {f : α → β} (h : Injective f) :
    Injective (map f)
  | [], [], _ => rfl
  | x :: xs, y :: ys, hxy => by
...
lemma _root_.Function.Surjective.list_map {f : α → β} (h : Surjective f) :
    Surjective (map f) :=
  let ⟨_, h⟩ := h.hasRightInverse; h.list_map.surjective

@[simp]
lemma map_surjective_iff {f : α → β} : Surjective (map f) ↔ Surjective f := by
  refine ⟨fun h x => ?_, (·.list_map)⟩
  let ⟨[y], hxy⟩ := h [x]
  exact ⟨_, List.singleton_injective hxy⟩

lemma _root_.Function.Bijective.list_map {f : α → β} (h : Bijective f) : Bijective (map f) :=
  ⟨h.1.list_map, h.2.list_map⟩

@[simp]
lemma map_bijective_iff {f : α → β} : Bijective (map f) ↔ Bijective f := by
...
@[simp]
lemma drop_tail (l : List α) (n : ℕ) : l.tail.drop n = l.drop (n + 1) := by
...
@[simp]
lemma modifyNth_nil (f : α → α) (n : ℕ) :
    modifyNth f n [] = [] := by cases n <;> rfl

@[simp]
lemma modifyNth_zero_cons (f : α → α) (a : α) (l : List α) :
    modifyNth f 0 (a :: l) = f a :: l := rfl

@[simp]
lemma modifyNth_succ_cons (f : α → α) (n : ℕ) (a : α) (l : List α) :
...
lemma of_mem_filter {a : α} {l} (h : a ∈ filter p l) : p a := (mem_filter.1 h).2
...
lemma mem_filter_of_mem {a : α} {l} (h₁ : a ∈ l) (h₂ : p a) : a ∈ filter p l :=
  mem_filter.2 ⟨h₁, h₂⟩
...
@[simp] lemma takeWhile_nil : List.takeWhile p [] = [] := rfl

lemma takeWhile_cons {x : α} :
    List.takeWhile p (x :: l) = (match p x with
      | true  => x :: takeWhile p l
      | false => []) :=
  rfl

lemma takeWhile_cons_of_pos {x : α} (h : p x) :
    List.takeWhile p (x :: l) = x :: takeWhile p l := by
  simp [takeWhile_cons, h]

lemma takeWhile_cons_of_neg {x : α} (h : ¬ p x) :
...
lemma erase_get [DecidableEq ι] {l : List ι} (i : Fin l.length) :
...
lemma eraseIdx_eq_take_drop_succ {l : List ι} {i : ℕ} :
...
@[simp]
lemma forall_cons (p : α → Prop) (x : α) : ∀ l : List α, Forall p (x :: l) ↔ p x ∧ Forall p l
  | [] => (and_true_iff _).symm
  | _ :: _ => Iff.rfl
#align list.all₂_cons List.forall_cons

lemma forall_iff_forall_mem : ∀ {l : List α}, Forall p l ↔ ∀ x ∈ l, p x
  | [] => (iff_true_intro <| forall_mem_nil _).symm
  | x :: l => by rw [forall_mem_cons, forall_cons, forall_iff_forall_mem]
#align list.all₂_iff_forall List.forall_iff_forall_mem

lemma Forall.imp (h : ∀ x, p x → q x) : ∀ {l : List α}, Forall p l → Forall q l
  | [] => id
  | x :: l => by
...
@[simp]
lemma forall_map_iff {p : β → Prop} (f : α → β) : Forall p (l.map f) ↔ Forall (p ∘ f) l := by
  induction l <;> simp [*]
