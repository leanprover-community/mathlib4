...
lemma prod_rotate_eq_one_of_prod_eq_one [Group α] :
...
from right to left. -/
lemma get_eq_get_rotate (l : List α) (n : ℕ) (k : Fin l.length) :
...
lemma nthLe_rotate' (l : List α) (n k : ℕ) (hk : k < l.length) :
...
set_option linter.deprecated false in
@[simp]
lemma nthLe_cyclicPermutations (l : List α) (n : ℕ) (hn : n < length (cyclicPermutations l)) :
...
set_option linter.deprecated false in
@[simp]
lemma mem_cyclicPermutations_iff {l l' : List α} : l ∈ cyclicPermutations l' ↔ l ~r l' := by
  constructor
  · intro h
    obtain ⟨k, hk, rfl⟩ := nthLe_of_mem h
    simp
...
@[simp]
lemma cyclicPermutations_eq_nil_iff {l : List α} : cyclicPermutations l = [[]] ↔ l = [] := by
...
lemma Nodup.cyclicPermutations {l : List α} (hn : Nodup l) : Nodup (cyclicPermutations l) := by
  cases' l with x l
  · simp
...
lemma IsRotated.cyclicPermutations {l l' : List α} (h : l ~r l') :
