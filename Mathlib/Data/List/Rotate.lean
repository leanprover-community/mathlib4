...
lemma rotate_mod (l : List α) (n : ℕ) : l.rotate (n % l.length) = l.rotate n := by simp [rotate]
#align list.rotate_mod List.rotate_mod

@[simp]
lemma rotate_nil (n : ℕ) : ([] : List α).rotate n = [] := by simp [rotate]
#align list.rotate_nil List.rotate_nil

@[simp]
lemma rotate_zero (l : List α) : l.rotate 0 = l := by simp [rotate]
#align list.rotate_zero List.rotate_zero

-- Porting note: removing simp, simp can prove it
lemma rotate'_nil (n : ℕ) : ([] : List α).rotate' n = [] := by cases n <;> rfl
#align list.rotate'_nil List.rotate'_nil

@[simp]
lemma rotate'_zero (l : List α) : l.rotate' 0 = l := by cases l <;> rfl
...
@[simp]
lemma length_rotate' : ∀ (l : List α) (n : ℕ), (l.rotate' n).length = l.length
  | [], _ => by simp
  | a :: l, 0 => rfl
...
@[simp]
lemma rotate'_length (l : List α) : rotate' l l.length = l := by
  rw [rotate'_eq_drop_append_take le_rfl]; simp
#align list.rotate'_length List.rotate'_length

@[simp]
lemma rotate'_length_mul (l : List α) : ∀ n : ℕ, l.rotate' (l.length * n) = l
  | 0 => by simp
  | n + 1 =>
    calc
...
@[simp]
lemma mem_rotate : ∀ {l : List α} {a : α} {n : ℕ}, a ∈ l.rotate n ↔ a ∈ l
  | [], _, n => by simp
  | a :: l, _, 0 => by simp
...
@[simp]
lemma length_rotate (l : List α) (n : ℕ) : (l.rotate n).length = l.length := by
...
lemma rotate_eq_drop_append_take {l : List α} {n : ℕ} :
...
@[simp]
lemma rotate_append_length_eq (l l' : List α) : (l ++ l').rotate l.length = l' ++ l := by
  rw [rotate_eq_rotate']
...
@[simp]
lemma nodup_rotate {l : List α} {n : ℕ} : Nodup (l.rotate n) ↔ Nodup l :=
  (rotate_perm l n).nodup_iff
#align list.nodup_rotate List.nodup_rotate

@[simp]
lemma rotate_eq_nil_iff {l : List α} {n : ℕ} : l.rotate n = [] ↔ l = [] := by
  induction' n with n hn generalizing l
  · simp
  · cases' l with hd tl
    · simp
...
@[simp]
lemma nil_eq_rotate_iff {l : List α} {n : ℕ} : [] = l.rotate n ↔ [] = l := by
  rw [eq_comm, rotate_eq_nil_iff, eq_comm]
#align list.nil_eq_rotate_iff List.nil_eq_rotate_iff

@[simp]
lemma rotate_singleton (x : α) (n : ℕ) : [x].rotate n = [x] :=
  rotate_replicate x 1 n
#align list.rotate_singleton List.rotate_singleton

lemma zipWith_rotate_distrib {α β γ : Type*} (f : α → β → γ) (l : List α) (l' : List β) (n : ℕ)
    (h : l.length = l'.length) :
...
lemma get_rotate (l : List α) (n : ℕ) (k : Fin (l.rotate n).length) :
    (l.rotate n).get k =
...
lemma head?_rotate {l : List α} {n : ℕ} (h : n < l.length) : head? (l.rotate n) = l.get? n := by
...
lemma nthLe_rotate (l : List α) (n k : ℕ) (hk : k < (l.rotate n).length) :
...
set_option linter.deprecated false in
lemma nthLe_rotate_one (l : List α) (k : ℕ) (hk : k < (l.rotate 1).length) :
...
lemma rotate_injective (n : ℕ) : Function.Injective fun l : List α => l.rotate n := by
...
@[simp]
lemma rotate_eq_rotate {l l' : List α} {n : ℕ} : l.rotate n = l'.rotate n ↔ l = l' :=
...
@[simp]
lemma rotate_eq_singleton_iff {l : List α} {n : ℕ} {x : α} : l.rotate n = [x] ↔ l = [x] := by
  rw [rotate_eq_iff, rotate_singleton]
#align list.rotate_eq_singleton_iff List.rotate_eq_singleton_iff

@[simp]
lemma singleton_eq_rotate_iff {l : List α} {n : ℕ} {x : α} : [x] = l.rotate n ↔ [x] = l := by
...
lemma Nodup.rotate_congr_iff {l : List α} (hl : l.Nodup) {i j : ℕ} :
...
lemma Nodup.rotate_eq_self_iff {l : List α} (hl : l.Nodup) {n : ℕ} :
...
@[refl]
lemma IsRotated.refl (l : List α) : l ~r l :=
  ⟨0, by simp⟩
#align list.is_rotated.refl List.IsRotated.refl

@[symm]
lemma IsRotated.symm (h : l ~r l') : l' ~r l := by
  obtain ⟨n, rfl⟩ := h
  cases' l with hd tl
  · exists 0
...
@[simp]
lemma isRotated_reverse_iff : l.reverse ~r l'.reverse ↔ l ~r l' := by
  simp [isRotated_reverse_comm_iff]
#align list.is_rotated_reverse_iff List.isRotated_reverse_iff

lemma isRotated_iff_mod : l ~r l' ↔ ∃ n ≤ l.length, l.rotate n = l' := by
...
lemma isRotated_iff_mem_map_range : l ~r l' ↔ l' ∈ (List.range (l.length + 1)).map l.rotate := by
...
-- @[congr]
lemma IsRotated.map {β : Type*} {l₁ l₂ : List α} (h : l₁ ~r l₂) (f : α → β) :
...
@[simp]
lemma cyclicPermutations_nil : cyclicPermutations ([] : List α) = [[]] :=
  rfl
#align list.cyclic_permutations_nil List.cyclicPermutations_nil

lemma cyclicPermutations_cons (x : α) (l : List α) :
...
lemma cyclicPermutations_of_ne_nil (l : List α) (h : l ≠ []) :
...
lemma length_cyclicPermutations_cons (x : α) (l : List α) :
...
@[simp]
lemma length_cyclicPermutations_of_ne_nil (l : List α) (h : l ≠ []) :
...
@[simp]
lemma cyclicPermutations_ne_nil : ∀ l : List α, cyclicPermutations l ≠ []
  | a::l, h => by simpa using congr_arg length h

@[simp]
lemma get_cyclicPermutations (l : List α) (n : Fin (length (cyclicPermutations l))) :
...
@[simp]
lemma head_cyclicPermutations (l : List α) :
...
@[simp]
lemma head?_cyclicPermutations (l : List α) : (cyclicPermutations l).head? = l := by
  rw [head?_eq_head, head_cyclicPermutations]

lemma cyclicPermutations_injective : Function.Injective (@cyclicPermutations α) := fun l l' h ↦ by
  simpa using congr_arg head? h

@[simp]
lemma cyclicPermutations_inj {l l' : List α} :
    cyclicPermutations l = cyclicPermutations l' ↔ l = l' :=
  cyclicPermutations_injective.eq_iff

lemma length_mem_cyclicPermutations (l : List α) (h : l' ∈ cyclicPermutations l) :
...
@[simp]
lemma mem_cyclicPermutations_iff : l ∈ cyclicPermutations l' ↔ l ~r l' := by
  constructor
...
@[simp]
lemma cyclicPermutations_eq_nil_iff {l : List α} : cyclicPermutations l = [[]] ↔ l = [] :=
  cyclicPermutations_injective.eq_iff' rfl
#align list.cyclic_permutations_eq_nil_iff List.cyclicPermutations_eq_nil_iff

@[simp]
lemma cyclicPermutations_eq_singleton_iff {l : List α} {x : α} :
...
protected lemma IsRotated.cyclicPermutations {l l' : List α} (h : l ~r l') :
...
@[simp]
lemma isRotated_cyclicPermutations_iff {l l' : List α} :
