...
@[norm_cast]
lemma coe_toFinsupp : (l.toFinsupp : ℕ → M) = (l.getD · 0) :=
  rfl
...
@[simp]
lemma toFinsupp_nil [DecidablePred fun i => getD ([] : List M) i 0 ≠ 0] :
...
lemma toFinsupp_append {R : Type*} [AddZeroClass R] (l₁ l₂ : List R)
...
lemma toFinsupp_cons_eq_single_add_embDomain {R : Type*} [AddZeroClass R] (x : R) (xs : List R)
    [DecidablePred (getD (x::xs) · 0 ≠ 0)] [DecidablePred (getD xs · 0 ≠ 0)] :
    toFinsupp (x::xs) =
...
lemma toFinsupp_concat_eq_toFinsupp_add_single {R : Type*} [AddZeroClass R] (x : R) (xs : List R)
...
lemma toFinsupp_eq_sum_map_enum_single {R : Type*} [AddMonoid R] (l : List R)
    [DecidablePred (getD l · 0 ≠ 0)] :
