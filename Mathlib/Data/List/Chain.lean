...
theorem chain'_iff_get {R} : ∀ {l : List α}, Chain' R l ↔
  ...
  | a :: b :: t => by
    ...
    simp only [length_cons, get_cons_succ, Fin.zero_eta, get_cons_zero, zero_add, Fin.mk_one,
      get_cons_cons_one, ge_iff_le, succ_sub_succ_eq_sub, nonpos_iff_eq_zero, add_eq_zero_iff,
      and_false, tsub_zero, add_pos_iff, or_true, forall_true_left, and_congr_right_iff]
    dsimp [succ_sub_one]
