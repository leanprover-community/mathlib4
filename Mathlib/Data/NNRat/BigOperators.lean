...
@[norm_cast]
lemma coe_list_sum (l : List ℚ≥0) : (l.sum : ℚ) = (l.map (↑)).sum :=
  map_list_sum coeHom _
#align nnrat.coe_list_sum NNRat.coe_list_sum

@[norm_cast]
lemma coe_list_prod (l : List ℚ≥0) : (l.prod : ℚ) = (l.map (↑)).prod :=
  map_list_prod coeHom _
#align nnrat.coe_list_prod NNRat.coe_list_prod

@[norm_cast]
lemma coe_multiset_sum (s : Multiset ℚ≥0) : (s.sum : ℚ) = (s.map (↑)).sum :=
  map_multiset_sum coeHom _
#align nnrat.coe_multiset_sum NNRat.coe_multiset_sum

@[norm_cast]
lemma coe_multiset_prod (s : Multiset ℚ≥0) : (s.prod : ℚ) = (s.map (↑)).prod :=
  map_multiset_prod coeHom _
#align nnrat.coe_multiset_prod NNRat.coe_multiset_prod

@[norm_cast]
lemma coe_sum {s : Finset α} {f : α → ℚ≥0} : ↑(∑ a in s, f a) = ∑ a in s, (f a : ℚ) :=
  map_sum coeHom _ _
#align nnrat.coe_sum NNRat.coe_sum

lemma toNNRat_sum_of_nonneg {s : Finset α} {f : α → ℚ} (hf : ∀ a, a ∈ s → 0 ≤ f a) :
...
@[norm_cast]
lemma coe_prod {s : Finset α} {f : α → ℚ≥0} : ↑(∏ a in s, f a) = ∏ a in s, (f a : ℚ) :=
  map_prod coeHom _ _
#align nnrat.coe_prod NNRat.coe_prod

lemma toNNRat_prod_of_nonneg {s : Finset α} {f : α → ℚ} (hf : ∀ a ∈ s, 0 ≤ f a) :
    (∏ a in s, f a).toNNRat = ∏ a in s, (f a).toNNRat := by
