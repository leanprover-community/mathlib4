...
Ported by Deniz Aydin from the lean3 core's data/int/basic.lean.

It mainly proves some simp lemmas and the ring properties but there's also some stuff about
nat_abs and definitions for sign, gcd, and various divs and mods
...
-- I made these two notations because otherwise I can't pattern match with them in the theorem statement
notation "of_nat" => ofNat
notation "neg_succ" => negSucc
...
protected lemma coe_nat_eq (n : ℕ) : ↑n = of_nat n := rfl
...
lemma of_nat_zero : of_nat (0 : ℕ) = (0 : ℤ) := rfl

lemma of_nat_one  : of_nat (1 : ℕ) = (1 : ℤ) := rfl

/- ## Definitions of basic functions -/

lemma sub_nat_nat_of_sub_eq_zero {m n : ℕ} (h : n - m = 0) :
  sub_nat_nat m n = of_nat (m - n) := by
    show (match (n - m) with
    | 0 => ofNat (m-n)
    | succ k => -[1+ k]
    ) = of_nat (m - n)
...
lemma neg_succ_of_nat_inj_iff {m n : ℕ} : neg_succ m = neg_succ n ↔ m = n :=
...
by
  have H : ∀k , n - m = k → P m n (@Nat.casesOn (λ _ => ℤ) k (of_nat (m-n)) neg_succ) := by
    intros k h
    cases k with
    | zero =>
        simp
...
lemma sub_nat_nat_add_right {m n : ℕ} : sub_nat_nat m (m + n + 1) = neg_succ n := by

  change (match m + n + 1 - m with
  | 0 => of_nat (m - (m + n + 1))
  | succ k => -[1+ k]) = neg_succ n
  simp
...
lemma eq_zero_of_nat_abs_eq_zero {a : ℤ} (H : nat_abs a = 0) : a = 0 := match a with
| (ofNat _)   => congr_arg of_nat H
| (negSucc _) => absurd H (succ_ne_zero _)
...
lemma nat_abs_mul_self {a : ℤ} : ↑(nat_abs a * nat_abs a) = a * a := match a with
| (ofNat _)   => rfl
| (negSucc _) => rfl
...
lemma nat_abs_eq (a : ℤ) : a = nat_abs a ∨ a = -↑(nat_abs a) := match a with
| ofNat _    => Or.inl rfl
| -[1+ _]   => Or.inr rfl
...
lemma of_nat_mul_neg_of_nat (m : ℕ) : ∀ n, of_nat m * neg_of_nat n = neg_of_nat (m * n)
| 0        => rfl
| (succ n) => by simp[neg_of_nat, negOfNat]

lemma neg_of_nat_mul_of_nat (m n : ℕ) : neg_of_nat m * of_nat n = neg_of_nat (m * n) := by
    rw [Int.mul_comm]
...
lemma neg_of_nat_mul_neg_succ_of_nat (m n : ℕ) :
  neg_of_nat n * -[1+ m] = of_nat (n * succ m) := by
...
| (of_nat m), -[1+ n],    (of_nat k) => by simp [neg_of_nat_eq_sub_nat_nat_zero] rw [Int.add_comm, ← sub_nat_nat_add] rfl
| (of_nat m), -[1+ n],   -[1+ k]     => by simp rw [← Nat.left_distrib, succ_add] rfl
| -[1+ m],    (of_nat n), (of_nat k) => by simp [Nat.mul_comm] rw [← Nat.right_distrib, Nat.mul_comm]

| -[1+ m],    (of_nat n), -[1+ k]    => by simp[neg_of_nat_eq_sub_nat_nat_zero] rw [Int.add_comm, ← sub_nat_nat_add] rfl
...
lemma of_nat_sub {n m : ℕ} (h : m ≤ n) : of_nat (n - m) = of_nat n - of_nat m := by
  show of_nat (n - m) = of_nat n + neg_of_nat m
  match m, h with
  | 0, h      => rfl
  | succ m, h =>
                show of_nat (n - succ m) = sub_nat_nat n (succ m)
                simp[sub_nat_nat, subNatNat]
...
| (of_nat 0)     => rfl
| (of_nat (n+1)) => show _ = -[1+ (1*n)+0] by rw [Nat.one_mul] rfl
| -[1+ n]        => show _ = of_nat _ by rw [Nat.one_mul] rfl
