...
theorem nonneg_or_nonpos_of_mul_nonneg {a b : ℤ} : 0 ≤ a * b → 0 ≤ a ∧ 0 ≤ b ∨ a ≤ 0 ∧ b ≤ 0 := by
  ...
  · exact .inl ⟨ha, hb⟩
  · refine .inr ⟨?_, ge_of_not_le hb⟩
    obtain _ | _ := Int.mul_eq_zero.mp <|
      Int.le_antisymm (Int.mul_nonpos_of_nonneg_of_nonpos ha <| ge_of_not_le hb) h
    all_goals omega
  · refine .inr ⟨ge_of_not_le ha, ?_⟩
    obtain _ | _ := Int.mul_eq_zero.mp <|
      Int.le_antisymm (Int.mul_nonpos_of_nonpos_of_nonneg (ge_of_not_le ha) hb) h
    all_goals omega
  · exact .inr ⟨ge_of_not_le ha, ge_of_not_le hb⟩
