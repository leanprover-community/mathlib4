...
lemma isUnit_eq_or_eq_neg {a b : ℤ} (ha : IsUnit a) (hb : IsUnit b) : a = b ∨ a = -b :=
...
lemma eq_of_mul_eq_one {z w : ℤ} (h : z * w = 1) : z = w :=
...
lemma isUnit_iff_natAbs_eq {n : ℤ} : IsUnit n ↔ n.natAbs = 1 := by
...
@[norm_cast]
lemma ofNat_isUnit {n : ℕ} : IsUnit (n : ℤ) ↔ IsUnit n := by
  simp [isUnit_iff_natAbs_eq]
#align int.of_nat_is_unit Int.ofNat_isUnit

lemma isUnit_mul_self {a : ℤ} (ha : IsUnit a) : a * a = 1 :=
