...
lemma lt_succ_self (a : ℤ) : a < succ a := by unfold succ; grind

lemma pred_self_lt (a : ℤ) : pred a < a := by unfold pred; grind
...
lemma inductionOn'_sub_one (hz : z ≤ b) :
  ...
  obtain ⟨n, hn⟩ := Int.eq_negSucc_of_lt_zero (show z - 1 - b < 0 by grind)
  rw [hn]
  obtain _ | n := n
  · change _ = -1 at hn
    have : z = b := by grind
    subst this; rw [inductionOn'_self]; exact heq_of_eq rfl
  · have : z = b + -[n+1] := by rw [Int.negSucc_eq] at hn ⊢; grind
    subst this
    refine (cast_heq _ _).trans ?_
    congr
    symm
    rw [Int.inductionOn', cast_eq_iff_heq, show b + -[n+1] - b = -[n+1] by grind]
...
protected lemma le_induction_down {m : ℤ} {motive : ∀ n, n ≤ m → Prop} (base : motive m m.le_refl)
    (pred : ∀ n hmn, motive n hmn → motive (n - 1) (by grind)) : ∀ n hmn, motive n hmn := fun n ↦
  Int.inductionOn' n m (fun _ ↦ base) (fun k hle _ hle' ↦ by grind)
...
@[elab_as_elim] protected def strongRec (n : ℤ) : motive n := by
  refine if hnm : n < m then lt n hnm else ge n (by grind) (n.inductionOn' m lt ?_ ?_)
  · intro _n _ ih l _
...
lemma sign_add_eq_of_sign_eq : ∀ {m n : ℤ}, m.sign = n.sign → (m + n).sign = n.sign := by
  ...
  rintro ((_ | m) | m) ((_ | n) | n) <;> simp [this, this.symm] <;> grind
...
lemma toNat_lt_of_ne_zero {n : ℕ} (hn : n ≠ 0) : m.toNat < n ↔ m < n := by grind
...
lemma natMod_lt {n : ℕ} (hn : n ≠ 0) : m.natMod n < n :=
  (toNat_lt_of_ne_zero hn).2 <| emod_lt_of_pos _ <| by grind
