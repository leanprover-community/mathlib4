...
@[simp] lemma here_append {q} : here ++ q = q := by
  dsimp only [(. ++ .), Append.append, Path.append, here]
  have := Nat.zero_add q.length
  congr; refine heq_of_eq_of_heq (nil_append q.bitvec) ?_
  congr; exact proof_irrel_heq _ _

@[simp] lemma append_here {q} : q ++ here = q := by
  dsimp only [(. ++ .), Append.append, Path.append, here]
  congr
  exact append_nil q.bitvec
