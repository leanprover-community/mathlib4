...
lemma bitsToNat_toList {n : ℕ} (x : Bitvec n) : Bitvec.toNat x = bitsToNat (Vector.toList x) :=
  rfl
...
-- mul_left_comm
lemma toNat_append {m : ℕ} (xs : Bitvec m) (b : Bool) :
...
lemma ofFin_val {n : ℕ} (i : Fin <| 2 ^ n) : (ofFin i).toNat = i.val := by
  rw [ofFin, toNat_ofNat, Nat.mod_eq_of_lt]
  apply i.is_lt
#align bitvec.of_fin_val Bitvec.ofFin_val

lemma addLsb_eq_twice_add_one {x b} : addLsb x b = 2 * x + cond b 1 0 := by
  simp [addLsb, two_mul]
#align bitvec.add_lsb_eq_twice_add_one Bitvec.addLsb_eq_twice_add_one

lemma toNat_eq_foldr_reverse {n : ℕ} (v : Bitvec n) :
...
lemma toNat_lt {n : ℕ} (v : Bitvec n) : v.toNat < 2 ^ n := by
...
lemma addLsb_div_two {x b} : addLsb x b / 2 = x := by
  cases b <;>
...
lemma decide_addLsb_mod_two {x b} : decide (addLsb x b % 2 = 1) = b := by
  cases b <;>
...
lemma ofNat_toNat {n : ℕ} (v : Bitvec n) : Bitvec.ofNat n v.toNat = v := by
  cases' v with xs h
