...
instance (priority := 10000) Sym.hasCoe (α : Type*) (n : ℕ) : CoeOut (Sym α n) (Multiset α) :=
  ⟨Sym.toMultiset⟩
#align sym.has_coe Sym.hasCoe

-- Porting note: instance needed for Data.Finset.Sym
instance (priority := 10000) [DecidableEq α] : DecidableEq (Sym α n) := Subtype.instDecidableEqSubtype
...
symmetric power.
-/
instance (priority := 10000) : Coe (Vector α n) (Sym α n) where coe x := ofVector x
...
-/
instance (priority := 10000) : Membership α (Sym α n) :=
  ⟨fun a s => a ∈ s.1⟩

instance (priority := 10000) decidableMem [DecidableEq α] (a : α) (s : Sym α n) : Decidable (a ∈ s) :=
  s.1.decidableMem _
...
instance (priority := 10000) instZeroSym : Zero (Sym α 0) :=
  ⟨⟨0, rfl⟩⟩

instance (priority := 10000) : EmptyCollection (Sym α 0) :=
  ⟨0⟩
...
instance (priority := 10000) uniqueZero : Unique (Sym α 0) :=
...
instance (priority := 10000) [Subsingleton α] (n : ℕ) : Subsingleton (Sym α n) :=
  ⟨by
    cases n
...
instance (priority := 10000) inhabitedSym [Inhabited α] (n : ℕ) : Inhabited (Sym α n) :=
  ⟨replicate n default⟩
...
instance (priority := 10000) (n : ℕ) [IsEmpty α] : IsEmpty (Sym α n.succ) :=
  ⟨fun s => by
    obtain ⟨a, -⟩ := exists_mem s
    exact isEmptyElim a⟩

instance (priority := 10000) (n : ℕ) [Unique α] : Unique (Sym α n) :=
  Unique.mk' _
...
instance (priority := 10000) (n : ℕ) [Nontrivial α] : Nontrivial (Sym α (n + 1)) :=
