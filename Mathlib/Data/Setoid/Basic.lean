...
instance (priority := 10000) Setoid.decidableRel (r : Setoid α) [h : DecidableRel r.r] : DecidableRel r.Rel :=
  h
...
instance (priority := 10000) : LE (Setoid α) :=
...
instance (priority := 10000) : Inf (Setoid α) :=
  ⟨fun r s =>
...
instance (priority := 10000) : InfSet (Setoid α) :=
  ⟨fun S =>
...
instance (priority := 10000) : PartialOrder (Setoid α) where
  le := (· ≤ ·)
  lt r s := r ≤ s ∧ ¬s ≤ r
  le_refl _ _ _ := id
...
instance (priority := 10000) completeLattice : CompleteLattice (Setoid α) :=
