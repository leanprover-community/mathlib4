...
@[deprecated (since := "2024-08-09")] alias ext' := ext
...
@[simp]
theorem ker_mk_eq (r : Setoid α) : ker (Quotient.mk r) = r :=
  ext fun _ _ => Quotient.eq

theorem ker_apply_mk_out {f : α → β} (a : α) : f (Quotient.mk (.ker f) a).out = f a :=
  @Quotient.mk_out _ (Setoid.ker f) a

@[deprecated (since := "2024-08-09")] alias ker_apply_mk_out' := ker_apply_mk_out
...
def liftEquiv (r : Setoid α) : { f : α → β // r ≤ ker f } ≃ (Quotient r → β) where
  ...
  invFun f := ⟨f ∘ Quotient.mk _, fun x y h => by simp [ker_def, Quotient.sound' h]⟩
...
theorem lift_unique {r : Setoid α} {f : α → β} (H : r ≤ ker f) (g : Quotient r → β)
    (Hg : f = g ∘ Quotient.mk _) : Quotient.lift f H = g := by
  ext ⟨x⟩
...
def map (r : Setoid α) (f : α → β) : Setoid β :=
  EqvGen.Setoid fun x y => ∃ a b, f a = x ∧ f b = y ∧ r a b
...
theorem comap_eq {f : α → β} {r : Setoid β} : comap f r = ker (Quotient.mk r ∘ f) :=
...
noncomputable def comapQuotientEquiv (f : α → β) (r : Setoid β) :
    Quotient (comap f r) ≃ Set.range (Quotient.mk r ∘ f) :=
...
def correspondence (r : Setoid α) : { s // r ≤ s } ≃o Setoid (Quotient r) where
  ...
  invFun s := ⟨comap (Quotient.mk _) s, fun x y h => by rw [comap_rel, Quotient.eq.2 h]⟩
  left_inv s := rfl
