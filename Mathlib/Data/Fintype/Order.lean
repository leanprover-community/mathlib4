...
theorem Finite.exists_le [IsDirectedLE α] (f : β → α) : ∃ M, ∀ i, f i ≤ M :=
  directed_id.finite_le _

theorem Finite.exists_ge [IsDirectedGE α] (f : β → α) : ∃ M, ∀ i, M ≤ f i :=
  directed_id.finite_le (r := (· ≥ ·)) _

theorem Set.Finite.exists_le [IsDirectedLE α] {s : Set α} (hs : s.Finite) :
    ∃ M, ∀ i ∈ s, i ≤ M :=
  directed_id.finite_set_le hs

theorem Set.Finite.exists_ge [IsDirectedGE α] {s : Set α} (hs : s.Finite) :
    ∃ M, ∀ i ∈ s, M ≤ i :=
  directed_id.finite_set_le (r := (· ≥ ·)) hs

@[simp]
theorem Finite.bddAbove_range [IsDirectedLE α] (f : β → α) : BddAbove (Set.range f) := by
...
@[simp]
theorem Finite.bddBelow_range [IsDirectedGE α] (f : β → α) : BddBelow (Set.range f) := by
