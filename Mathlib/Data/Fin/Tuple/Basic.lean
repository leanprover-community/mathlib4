...
lemma tuple0_le {α : Fin 0 → Type*} [∀ i, Preorder (α i)] (f g : ∀ i, α i) : f ≤ g :=
  finZeroElim
...
@[simp]
lemma tail_cons : tail (cons x p) = p := by
...
@[simp]
lemma elim0_append {α : Type*} (v : Fin n → α) :
...
@[simp]
lemma repeat_one {α : Type*} (a : Fin n → α) : Fin.repeat 1 a = a ∘ cast (one_mul _) := by
  generalize_proofs h
  apply funext
...
lemma repeat_rev {α : Type*} (a : Fin n → α) (k : Fin (m * n)) :
    Fin.repeat m a k.rev = Fin.repeat m (a ∘ Fin.rev) k :=
  congr_arg a k.modNat_rev

lemma repeat_comp_rev {α} (a : Fin n → α) :
...
lemma append_left_snoc {n m} {α : Type*} (xs : Fin n → α) (x : α) (ys : Fin m → α) :
...
lemma append_snoc {α} (as : Fin n → α) (bs : Fin m → α) (b : α) :
...
@[simp]
lemma extractNth_insertNth {i : Fin (n + 1)} (x : α i) (p : ∀ j : Fin n, α (i.succAbove j)) :
    i.extractNth (i.insertNth x p) = (x, p) := by
  simp [extractNth]

@[simp]
lemma insertNth_extractNth {i : Fin (n + 1)} (f : ∀ j, α j) :
