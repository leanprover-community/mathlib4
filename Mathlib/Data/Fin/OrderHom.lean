...
@[simp]
lemma succAbove_pred_self (p : Fin (n + 1)) (h : p ≠ 0) :
...
lemma succAbove_lt_succAbove_iff {i j : Fin n} (p : Fin (n + 1)) :
    succAbove p i < succAbove p j ↔ i < j := (strictMono_succAbove p).lt_iff_lt
lemma succAbove_le_succAbove_iff {i j : Fin n} (p : Fin (n + 1)) :
...
@[simp]
lemma succAbove_ne_last_last {a : Fin (n + 2)} (h : a ≠ last (n + 1)) :
...
lemma succAbove_ne_last {a : Fin (n + 2)} {b : Fin (n + 1)}
...
lemma succAbove_last_apply (i : Fin n) : succAbove (last n) i = castSucc i := by
  rw [succAbove_last]
...
@[simp]
lemma exists_succAbove_eq_iff {x y : Fin (n + 1)} : (∃ z, x.succAbove z = y) ↔ y ≠ x := by
...
@[simp]
lemma range_succ (n : ℕ) : Set.range (Fin.succ : Fin n → Fin (n + 1)) = {0}ᶜ := by
  rw [← succAbove_zero]
...
@[simp]
lemma zero_succAbove {n : ℕ} (i : Fin n) : (0 : Fin (n + 1)).succAbove i = i.succ := by
  rfl
#align fin.zero_succ_above Fin.zero_succAbove

@[simp]
lemma succ_succAbove_zero {n : ℕ} [NeZero n] (i : Fin n) : succAbove i.succ 0 = 0 :=
...
@[simp]
lemma one_succAbove_succ {n : ℕ} (j : Fin n) :
...
@[simp]
lemma one_succAbove_one {n : ℕ} : (1 : Fin (n + 3)).succAbove 1 = 2 := by
...
lemma predAbove_succ_of_le (p i : Fin n) (h : p ≤ i) :
    p.predAbove (succ i) = i := by
  rw [predAbove_of_succ_le _ _ (succ_le_succ_iff.mpr h), pred_succ]
@[simp]
lemma predAbove_succ_self (p : Fin n) : p.predAbove (succ p) = p :=
...
lemma predAbove_castSucc_of_le (p i : Fin n) (h : i ≤ p) :
    p.predAbove (castSucc i) = i := by
  rw [predAbove_of_le_castSucc _ _ (castSucc_le_castSucc_iff.mpr h), castPred_castSucc]
@[simp]
lemma predAbove_castSucc_self (p : Fin n) : p.predAbove (castSucc p) = p :=
  predAbove_castSucc_of_le _ _ le_rfl

lemma predAbove_pred_of_lt (p i : Fin (n + 1)) (h : i < p) (hp := ((zero_le i).trans_lt h).ne')
...
lemma predAbove_pred_self (p : Fin (n + 1)) (hp : p ≠ 0) :
...
@[simp]
lemma predAbove_right_zero [NeZero n] {i : Fin n} : predAbove (i : Fin n) 0 = 0 := by
  cases n
  · exact i.elim0
...
@[simp]
lemma succ_predAbove_zero [NeZero n] {j : Fin (n + 1)} (h : j ≠ 0) :
    succ (predAbove 0 j) = j := by
  rcases exists_succ_eq_of_ne_zero h with ⟨k, rfl⟩
  rw [predAbove_zero_succ]
@[simp]
lemma predAbove_zero_of_ne_zero [NeZero n] {i : Fin (n + 1)} (hi : i ≠ 0) :
...
lemma predAbove_left_monotone (i : Fin (n + 1)) :
