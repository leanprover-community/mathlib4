...
theorem coe_int_add_eq_ite {n : Nat} (u v : Fin n) :
  ...
  rw [Fin.add_def]
  split
  · rw [natCast_emod, Int.emod_eq_of_lt] <;> grind
  · rw [natCast_emod, Int.emod_eq_sub_self_emod, Int.emod_eq_of_lt] <;> grind
...
theorem nontrivial_iff_two_le : Nontrivial (Fin n) ↔ 2 ≤ n := by
  simp [← not_subsingleton_iff_nontrivial, subsingleton_iff_le_one]; grind
...
lemma castLT_sub_nezero {n : ℕ} {i j : Fin n} (hij : i < j) :
    letI : NeZero (n - i.1) := neZero_iff.mpr (by grind)
...
theorem val_add_one_of_lt' {n : ℕ} [NeZero n] {i : Fin n} (h : i + 1 < n) :
    (i + 1).val = i.val + 1 := by
  simpa [add_def] using Nat.mod_eq_of_lt (by grind)
