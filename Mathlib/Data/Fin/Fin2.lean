...
instance (priority := 10000) IsLT.zero (n) : IsLT 0 (succ n) :=
  ⟨succ_pos _⟩

instance (priority := 10000) IsLT.succ (m n) [l : IsLT m n] : IsLT (succ m) (succ n) :=
  ⟨succ_lt_succ l.h⟩
...
instance (priority := 10000) : Inhabited (Fin2 1) :=
  ⟨fz⟩
