...
* `mkQ'` `⟦a⟧'`         : the quotient map inferred from the input type
                            via typeclass `QuotLike.HasQuot`
...
Using `QuotLike` API for type synonyms of `Quot` and `Quotient` will avoid defeq abuse of the types
...
/-- The quotient map. Inferred from the input type via typeclass `QuotLike.HasQuot`. -/
...
/-- The quotient map. Inferred from the hint via typeclass `QuotLike.Hint` or
