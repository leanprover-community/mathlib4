...
lemma dual_indep_iff_exists' : (M✶.Indep I) ↔ I ⊆ M.E ∧ (∃ B, M.Base B ∧ Disjoint I B) := Iff.rfl

@[simp] lemma dual_ground : M✶.E = M.E := rfl

@[simp] lemma dual_indep_iff_exists (hI : I ⊆ M.E := by aesop_mat) :
    M✶.Indep I ↔ (∃ B, M.Base B ∧ Disjoint I B) := by
  rw [dual_indep_iff_exists', and_iff_right hI]

lemma dual_dep_iff_forall : (M✶.Dep I) ↔ (∀ B, M.Base B → (I ∩ B).Nonempty) ∧ I ⊆ M.E := by
...
@[simp] lemma dual_base_iff (hB : B ⊆ M.E := by aesop_mat) : M✶.Base B ↔ M.Base (M.E \ B) := by
...
lemma setOf_dual_base_eq : {B | M✶.Base B} = (fun X ↦ M.E \ X) '' {B | M.Base B} := by
  ext B
...
lemma dual_involutive : Function.Involutive (dual : Matroid α → Matroid α) := dual_dual

lemma dual_injective : Function.Injective (dual : Matroid α → Matroid α) :=
  dual_involutive.injective

@[simp] lemma dual_inj {M₁ M₂ : Matroid α} : M₁✶ = M₂✶ ↔ M₁ = M₂ :=
  dual_injective.eq_iff

lemma eq_dual_comm {M₁ M₂ : Matroid α} : M₁ = M₂✶ ↔ M₂ = M₁✶ := by
  rw [← dual_inj, dual_dual, eq_comm]

lemma eq_dual_iff_dual_eq {M₁ M₂ : Matroid α} : M₁ = M₂✶ ↔ M₁✶ = M₂ :=
  dual_involutive.eq_iff.symm

lemma Base.compl_base_of_dual (h : M✶.Base B) : M.Base (M.E \ B) :=
  (dual_base_iff'.1 h).1

lemma Base.compl_base_dual (h : M.Base B) : M✶.Base (M.E \ B) := by
...
lemma Base.inter_basis_iff_compl_inter_basis_dual (hB : M.Base B) (hX : X ⊆ M.E := by aesop_mat):
...
lemma base_iff_dual_base_compl (hB : B ⊆ M.E := by aesop_mat) :
    M.Base B ↔ M✶.Base (M.E \ B) := by
  rw [dual_base_iff, diff_diff_cancel_left hB]

lemma ground_not_base (M : Matroid α) [h : RkPos M✶] : ¬M.Base M.E := by
  rwa [rkPos_iff_empty_not_base, dual_base_iff, diff_empty] at h

lemma Base.ssubset_ground [h : RkPos M✶] (hB : M.Base B) : B ⊂ M.E :=
  hB.subset_ground.ssubset_of_ne (by rintro rfl; exact M.ground_not_base hB)

lemma Indep.ssubset_ground [h : RkPos M✶] (hI : M.Indep I) : I ⊂ M.E := by
...
lemma coindep_def : M.Coindep X ↔ M✶.Indep X := Iff.rfl

lemma Coindep.indep (hX : M.Coindep X) : M✶.Indep X :=
  hX

@[simp] lemma dual_coindep_iff : M✶.Coindep X ↔ M.Indep X := by
  rw [Coindep, dual_dual]

lemma Indep.coindep (hI : M.Indep I) : M✶.Coindep I :=
  dual_coindep_iff.2 hI

lemma coindep_iff_exists' : M.Coindep X ↔ (∃ B, M.Base B ∧ B ⊆ M.E \ X) ∧ X ⊆ M.E := by
...
lemma coindep_iff_exists (hX : X ⊆ M.E := by aesop_mat) :
    M.Coindep X ↔ ∃ B, M.Base B ∧ B ⊆ M.E \ X := by
  rw [coindep_iff_exists', and_iff_left hX]

lemma coindep_iff_subset_compl_base : M.Coindep X ↔ ∃ B, M.Base B ∧ X ⊆ M.E \ B := by
...
lemma Coindep.subset_ground (hX : M.Coindep X) : X ⊆ M.E :=
  hX.indep.subset_ground

lemma Coindep.exists_base_subset_compl (h : M.Coindep X) : ∃ B, M.Base B ∧ B ⊆ M.E \ X :=
  (coindep_iff_exists h.subset_ground).1 h

lemma Coindep.exists_subset_compl_base (h : M.Coindep X) : ∃ B, M.Base B ∧ X ⊆ M.E \ B :=
