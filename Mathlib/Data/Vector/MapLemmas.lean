...
lemma mapAccumr_bisim {f₁ : α → σ₁ → σ₁ × β} {f₂ : α → σ₂ → σ₂ × β} {s₁ : σ₁} {s₂ : σ₂}
      (R : σ₁ → σ₂ → Prop) (h₀ : R s₁ s₂)
...
lemma mapAccumr_bisim_tail {f₁ : α → σ₁ → σ₁ × β} {f₂ : α → σ₂ → σ₂ × β} {s₁ : σ₁} {s₂ : σ₂}
    (h : ∃ R : σ₁ → σ₂ → Prop, R s₁ s₂ ∧
...
lemma mapAccumr₂_bisim {ys : Vector β n} {f₁ : α → β → σ₁ → σ₁ × γ}
    {f₂ : α → β → σ₂ → σ₂ × γ} {s₁ : σ₁} {s₂ : σ₂}
    (R : σ₁ → σ₂ → Prop) (h₀ : R s₁ s₂)
...
lemma mapAccumr₂_bisim_tail {ys : Vector β n} {f₁ : α → β → σ₁ → σ₁ × γ}
    {f₂ : α → β → σ₂ → σ₂ × γ} {s₁ : σ₁} {s₂ : σ₂}
    (h : ∃ R : σ₁ → σ₂ → Prop, R s₁ s₂ ∧
...
protected lemma map_eq_mapAccumr :
...
-/
lemma mapAccumr_eq_map {f : α → σ → σ × β} {s₀ : σ} (S : Set σ) (h₀ : s₀ ∈ S)
...
protected lemma map₂_eq_mapAccumr₂ :
...
/--
  ...
  Hence, then we can rewrite `mapAccumr₂` into just `map₂`
-/
lemma mapAccumr₂_eq_map₂ {f : α → β → σ → σ × γ} {s₀ : σ} (S : Set σ) (h₀ : s₀ ∈ S)
...
-/
@[simp]
lemma mapAccumr₂_unused_input_left [Inhabited α] (f : α → β → σ → σ × γ)
...
-/
@[simp]
lemma mapAccumr₂_unused_input_right [Inhabited β] (f : α → β → σ → σ × γ)
