...
@[simp]
lemma zipWith_single_single (f : ∀ i, β₁ i → β₂ i → β i) (hf : ∀ i, f i 0 0 = 0)
    {i} (b₁ : β₁ i) (b₂ : β₂ i) :
...
lemma eq_mk_support (f : Π₀ i, β i) : f = mk f.support fun i => f i := by aesop
...
lemma support_subset_iff {s : Set ι} {f : Π₀ i, β i} : ↑f.support ⊆ s ↔ ∀ i ∉ s, f i = 0 := by
...
lemma _root_.map_dfinsupp_prod
