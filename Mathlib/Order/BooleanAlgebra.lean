...
theorem sdiff_lt_sdiff_right (h : x < y) (hz : z ≤ x) : x \ z < y \ z :=
  (sdiff_le_sdiff_right h.le).gt_of_not_le
...
theorem sup_lt_of_lt_sdiff_left (h : y < z \ x) (hxz : x ≤ z) : x ⊔ y < z := by
  ...
  refine (sup_le_sup_left h.le _).gt_of_not_le fun h' => h.not_le ?_
  rw [← sdiff_idem]
...
theorem sup_lt_of_lt_sdiff_right (h : x < z \ y) (hyz : y ≤ z) : x ⊔ y < z := by
  ...
  refine (sup_le_sup_right h.le _).gt_of_not_le fun h' => h.not_le ?_
  rw [← sdiff_idem]
