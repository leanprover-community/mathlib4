...
def OrderIso.dualAntisymmetrization :
  ...
  toFun a := (Quotient.map' toDual) (fun _ _ => And.symm) (ofDual a)
  invFun a := toDual ((Quotient.map' ofDual) (fun _ _ => And.symm) a)
  left_inv a := by
    obtain ⟨a⟩ := a
    exact Quotient.inductionOn' a fun a => by simp_rw [Quotient.map'_mk'']
  right_inv a := Quotient.inductionOn' a fun a => by
    simp_rw [Quotient.map'_mk'']
  map_rel_iff' := @fun a b => by
    obtain ⟨a⟩ := a; obtain ⟨b⟩ := b
    exact Quotient.inductionOn₂' a b fun _ _ => Iff.rfl

@[simp]
theorem OrderIso.dualAntisymmetrization_apply (a : α) :
    OrderIso.dualAntisymmetrization _ (toDual <| toAntisymmetrization _ a) =
      toAntisymmetrization _ (toDual a) := by
  unfold OrderIso.dualAntisymmetrization
  simp [Quotient.map'_mk'', toAntisymmetrization]
...
theorem OrderIso.dualAntisymmetrization_symm_apply (a : α) :
    (OrderIso.dualAntisymmetrization _).symm (toAntisymmetrization _ <| toDual a) =
      toDual (toAntisymmetrization _ a) := by
  unfold OrderIso.dualAntisymmetrization
  simp [Quotient.map'_mk'', toAntisymmetrization, OrderIso.symm]
