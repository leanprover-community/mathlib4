...
instance [@DecidableRel α (· ≤ ·)] [@DecidableRel α (· < ·)] [IsTotal α (· ≤ ·)] :
  ...
  { instPartialOrderAntisymmetrization with
    le_total := Quotient.ind₂ <| total_of (· ≤ ·),
    decidableLE := fun _ _ => show Decidable (Quotient.lift₂ _ _ _ _) from inferInstance,
    decidableLT := fun _ _ => show Decidable (Quotient.lift₂ _ _ _ _) from inferInstance }
...
theorem OrderHom.antisymmetrization_apply_mk (f : α →o β) (a : α) :
  ...
  Quotient.map_mk f (liftFun_antisymmRel f) _
...
def OrderIso.dualAntisymmetrization :
  ...
  left_inv a := Quotient.inductionOn a fun a => by simp_rw [Quotient.map_mk, id]
  right_inv a := Quotient.inductionOn a fun a => by simp_rw [Quotient.map_mk, id]
