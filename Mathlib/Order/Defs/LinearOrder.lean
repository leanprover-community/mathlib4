...
/-- Auxiliary class to share implementations between linear orders.

This avoids choosing between calling the lattice operations `inf`/`sup` or `min`/`max`,
and so unlike `LinearOrder` can be extended by derived classes using the former convention without
creating duplicate fields. -/
class LinearOrderAux (α : Type*) extends PartialOrder α, Ord α where
...
class LinearOrder (α : Type*) extends LinearOrderAux α, Min α, Max α where
...
attribute [instance 900] LinearOrderAux.decidableLE
attribute [instance 900] LinearOrderAux.decidableEq
attribute [instance 900] LinearOrderAux.decidableLT

lemma le_total : ∀ a b : α, a ≤ b ∨ b ≤ a := LinearOrderAux.le_total
...
lemma compare_lt_iff_lt : compare a b = .lt ↔ a < b := by
  rw [LinearOrderAux.compare_eq_compareOfLessAndEq, compareOfLessAndEq]
...
theorem cmp_eq_compareOfLessAndEq (a b : α) : cmp a b = compareOfLessAndEq a b :=
  (cmp_eq_compare ..).trans (LinearOrderAux.compare_eq_compareOfLessAndEq ..)
