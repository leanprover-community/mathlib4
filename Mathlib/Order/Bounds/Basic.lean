...
def SemilatticeSup.ofIsLUB [PartialOrder α]
    (sup : α → α → α)
    (isLUB_pair : ∀ a b : α, IsLUB {a, b} (sup a b)) :
...
def Lattice.ofIsLUBofIsGLB [PartialOrder α]
    (sup inf : α → α → α)
    (isLUB_pair : ∀ a b : α, IsLUB {a, b} (sup a b))
    (isGLB_pair : ∀ a b : α, IsGLB {a, b} (inf a b)) :
    Lattice α where
