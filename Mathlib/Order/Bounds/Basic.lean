...
protected lemma BddAbove.insert [IsDirected α (· ≤ ·)] {s : Set α} (a : α) :
...
protected lemma BddBelow.insert [IsDirected α (· ≥ ·)] {s : Set α} (a : α) :
...
protected lemma IsGreatest.image2 (ha : IsGreatest s a) (hb : IsGreatest t b) :
...
protected lemma IsLeast.image2 (ha : IsLeast s a) (hb : IsLeast t b) :
...
lemma isLUB_prod {s : Set (α × β)} (p : α × β) :
...
lemma isGLB_prod {s : Set (α × β)} (p : α × β) :
...
protected lemma ScottContinuous.monotone (h : ScottContinuous f) : Monotone f := by
  refine' fun a b hab =>
