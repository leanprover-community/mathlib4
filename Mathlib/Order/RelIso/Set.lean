...
instance (r : α → α → Prop) [IsTrans α r] (p : Set α) : IsTrans p (Subrel r p) := fast_instance%
  ⟨fun x y z => @IsTrans.trans α r _ x y z⟩

instance (r : α → α → Prop) [IsIrrefl α r] (p : Set α) : IsIrrefl p (Subrel r p) := fast_instance%
