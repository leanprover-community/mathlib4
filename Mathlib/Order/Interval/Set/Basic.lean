...
@[simp]
theorem Iio_top [PartialOrder α] [OrderTop α] : Iio (⊤ : α) = {⊤}ᶜ := by
  grind [lt_top_iff_ne_top]
...
@[simp]
theorem Ioi_bot [PartialOrder α] [OrderBot α] : Ioi (⊥ : α) = {⊥}ᶜ := by
  grind [bot_lt_iff_ne_bot]
