...
@[simp, grind =, push] theorem mem_Iio : x ∈ Iio b ↔ x < b := Iff.rfl
...
@[simp, grind =, push] theorem mem_Iic : x ∈ Iic b ↔ x ≤ b := Iff.rfl
...
@[simp, grind =, push] theorem mem_Ici : x ∈ Ici a ↔ a ≤ x := Iff.rfl
theorem Ici_def (a : α) : { x | a ≤ x } = Ici a := rfl

/-- `Ioi a` is the left-open right-infinite interval $(a, ∞)$. -/
def Ioi (a : α) := { x | a < x }

@[simp, grind =, push] theorem mem_Ioi : x ∈ Ioi a ↔ a < x := Iff.rfl
