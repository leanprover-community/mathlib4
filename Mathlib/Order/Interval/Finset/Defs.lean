...
lemma Icc_toDual : Icc (toDual a) (toDual b) = (Icc b a).map toDual.toEmbedding := map_refl.symm
#align Icc_to_dual Icc_toDual

lemma Ico_toDual : Ico (toDual a) (toDual b) = (Ioc b a).map toDual.toEmbedding := map_refl.symm
#align Ico_to_dual Ico_toDual

lemma Ioc_toDual : Ioc (toDual a) (toDual b) = (Ico b a).map toDual.toEmbedding := map_refl.symm
#align Ioc_to_dual Ioc_toDual

lemma Ioo_toDual : Ioo (toDual a) (toDual b) = (Ioo b a).map toDual.toEmbedding := map_refl.symm
#align Ioo_to_dual Ioo_toDual

lemma Icc_ofDual (a b : αᵒᵈ) : Icc (ofDual a) (ofDual b) = (Icc b a).map ofDual.toEmbedding :=
  map_refl.symm
#align Icc_of_dual Icc_ofDual

lemma Ico_ofDual (a b : αᵒᵈ) : Ico (ofDual a) (ofDual b) = (Ioc b a).map ofDual.toEmbedding :=
  map_refl.symm
#align Ico_of_dual Ico_ofDual

lemma Ioc_ofDual (a b : αᵒᵈ) : Ioc (ofDual a) (ofDual b) = (Ico b a).map ofDual.toEmbedding :=
  map_refl.symm
#align Ioc_of_dual Ioc_ofDual

lemma Ioo_ofDual (a b : αᵒᵈ) : Ioo (ofDual a) (ofDual b) = (Ioo b a).map ofDual.toEmbedding :=
  map_refl.symm
...
lemma BddBelow.finite_of_bddAbove [Preorder α] [LocallyFiniteOrder α]
