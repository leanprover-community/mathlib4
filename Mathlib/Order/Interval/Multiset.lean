...
lemma Ico_filter_le_of_le_left [DecidablePred (c ≤ ·)] (hca : c ≤ a) :
...
lemma Ico_filter_le_of_right_le [DecidablePred (b ≤ ·)] :
...
lemma Ico_filter_le_of_left_le [DecidablePred (c ≤ ·)] (hac : a ≤ c) :
...
@[simp]
lemma Ico_inter_Ico_of_le [DecidableEq α] {a b c d : α} (h : b ≤ c) : Ico a b ∩ Ico c d = 0 :=
...
lemma Ico_inter_Ico : Ico a b ∩ Ico c d = Ico (max a c) (min b d) := by
