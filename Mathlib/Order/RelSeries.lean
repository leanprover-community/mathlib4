...
def append (p q : RelSeries r) (connect : p.last ~[r] q.head) : RelSeries r where
  ...
  toFun := Fin.append p q ∘ Fin.cast (by cutsat)
  step i := by
    obtain hi | rfl | hi :=
      lt_trichotomy i (Fin.castLE (by cutsat) (Fin.last _ : Fin (p.length + 1)))
    ...
    · set x := _; set y := _
      ...
      convert q.step ⟨i - (p.length + 1), Nat.sub_lt_left_of_lt_add hi <| by cutsat⟩
  ...
  · intro i h1 h2
    ...
    · simp only [append, Function.comp_apply, Fin.cast_mk, List.getElem_append]
      ...
      · simp_all only [length_toList]
        have : Fin.mk i h3' = Fin.natAdd _ ⟨i - p.length - 1, by cutsat⟩ := by simp_all; cutsat
...
def insertNth (p : RelSeries r) (i : Fin p.length) (a : α)
  ...
  step m := by
    ...
    · rw [Nat.lt_iff_add_one_le, le_iff_lt_or_eq] at hm
      obtain hm | hm := hm
      · convert p.step ⟨m.1 - 1, Nat.sub_lt_right_of_lt_add (by cutsat) m.2⟩
        · change Fin.insertNth _ _ _ _ = _
...
def reverse (p : RelSeries r) : RelSeries r.inv where
  ...
  step i := by
    ...
    convert p.step ⟨p.length - (i.1 + 1), Nat.sub_lt_self (by cutsat) hi⟩
    · ext; simp
    · ext
...
def take {r : SetRel α α} (p : RelSeries r) (i : Fin (p.length + 1)) : RelSeries r where
  length := i
  toFun := fun ⟨j, h⟩ => p.toFun ⟨j, by cutsat⟩
  step := fun ⟨j, h⟩ => p.step ⟨j, by cutsat⟩
...
def drop (p : RelSeries r) (i : Fin (p.length + 1)) : RelSeries r where
  length := p.length - i
  toFun := fun ⟨j, h⟩ => p.toFun ⟨j+i, by cutsat⟩
  step := fun ⟨j, h⟩ => by
    convert p.step ⟨j+i.1, by cutsat⟩
    simp only [Fin.succ_mk]; cutsat
...
instance FiniteDimensionalOrder.ofUnique (γ : Type*) [Preorder γ] [Unique γ] :
  ...
  exists_longest_relSeries := ⟨.singleton _ default, fun x ↦ by
    by_contra! r
    exact (x.step ⟨0, by cutsat⟩).ne <| Subsingleton.elim _ _⟩
