...
@[to_dual IsDirectedGE /-- A class for an `IsDirected` relation `≥`. -/]
abbrev IsDirectedLE (α : Type*) [LE α] : Prop := IsDirected α (· ≤ ·)
...
theorem exists_ge_ge [LE α] [IsDirectedLE α] (a b : α) : ∃ c, a ≤ c ∧ b ≤ c :=
  directed_of (· ≤ ·) a b

theorem exists_le_le [LE α] [IsDirectedGE α] (a b : α) : ∃ c, c ≤ a ∧ c ≤ b :=
  directed_of (· ≥ ·) a b

instance OrderDual.isDirected_ge [LE α] [IsDirectedLE α] : IsDirectedGE αᵒᵈ := by
  assumption

instance OrderDual.isDirected_le [LE α] [IsDirectedGE α] : IsDirectedLE αᵒᵈ := by
  assumption

/-- A monotone function on an upwards-directed type is directed. -/
theorem directed_of_isDirected_le [LE α] [IsDirectedLE α] {f : α → β} {r : β → β → Prop}
    (H : ∀ ⦃i j⦄, i ≤ j → r (f i) (f j)) : Directed r f :=
  directed_id.mono_comp _ H

theorem Monotone.directed_le [Preorder α] [IsDirectedLE α] [Preorder β] {f : α → β} :
    Monotone f → Directed (· ≤ ·) f :=
  directed_of_isDirected_le

theorem Antitone.directed_ge [Preorder α] [IsDirectedLE α] [Preorder β] {f : α → β}
    (hf : Antitone f) : Directed (· ≥ ·) f :=
  directed_of_isDirected_le hf

/-- An antitone function on a downwards-directed type is directed. -/
theorem directed_of_isDirected_ge [LE α] [IsDirectedGE α] {r : β → β → Prop} {f : α → β}
    (hf : ∀ a₁ a₂, a₁ ≤ a₂ → r (f a₂) (f a₁)) : Directed r f :=
  directed_of_isDirected_le (α := αᵒᵈ) fun _ _ ↦ hf _ _

theorem Monotone.directed_ge [Preorder α] [IsDirectedGE α] [Preorder β] {f : α → β}
    (hf : Monotone f) : Directed (· ≥ ·) f :=
  directed_of_isDirected_ge hf

theorem Antitone.directed_le [Preorder α] [IsDirectedGE α] [Preorder β] {f : α → β}
    (hf : Antitone f) : Directed (· ≤ ·) f :=
  directed_of_isDirected_ge hf
...
protected theorem IsMin.isBot [IsDirectedGE α] (h : IsMin a) : IsBot a := fun b =>
  let ⟨_, hca, hcb⟩ := exists_le_le a b
  (h hca).trans hcb

protected theorem IsMax.isTop [IsDirectedLE α] (h : IsMax a) : IsTop a :=
  h.toDual.isBot
...
theorem isTop_or_exists_gt [IsDirectedLE α] (a : α) : IsTop a ∨ ∃ b, a < b :=
  (em (IsMax a)).imp IsMax.isTop not_isMax_iff.mp

theorem isBot_or_exists_lt [IsDirectedGE α] (a : α) : IsBot a ∨ ∃ b, b < a :=
  @isTop_or_exists_gt αᵒᵈ _ _ a

theorem isBot_iff_isMin [IsDirectedGE α] : IsBot a ↔ IsMin a :=
  ⟨IsBot.isMin, IsMin.isBot⟩

theorem isTop_iff_isMax [IsDirectedLE α] : IsTop a ↔ IsMax a :=
...
variable (β) in
theorem exists_lt_of_directed_ge [IsDirectedGE β] :
    ∃ a b : β, a < b := by
...
variable (β) in
theorem exists_lt_of_directed_le [IsDirectedLE β] :
    ∃ a b : β, a < b :=
  let ⟨a, b, h⟩ := exists_lt_of_directed_ge βᵒᵈ
  ⟨b, a, h⟩

protected theorem IsMin.not_isMax [IsDirectedGE β] {b : β} (hb : IsMin b) : ¬ IsMax b := by
  intro hb'
...
protected theorem IsMin.not_isMax' [IsDirectedLE β] {b : β} (hb : IsMin b) : ¬ IsMax b :=
  fun hb' ↦ hb'.toDual.not_isMax hb.toDual

protected theorem IsMax.not_isMin [IsDirectedLE β] {b : β} (hb : IsMax b) : ¬ IsMin b :=
  fun hb' ↦ hb.toDual.not_isMax hb'.toDual

protected theorem IsMax.not_isMin' [IsDirectedGE β] {b : β} (hb : IsMax b) : ¬ IsMin b :=
...
lemma constant_of_monotone_antitone [IsDirectedLE α] (hf : Monotone f) (hf' : Antitone f)
    (a b : α) : f a = f b := by
...
-- see Note [lower instance priority]
instance (priority := 100) OrderTop.to_isDirected_le [LE α] [OrderTop α] : IsDirectedLE α :=
  ⟨fun _ _ => ⟨⊤, le_top _, le_top _⟩⟩

-- see Note [lower instance priority]
instance (priority := 100) OrderBot.to_isDirected_ge [LE α] [OrderBot α] : IsDirectedGE α :=
