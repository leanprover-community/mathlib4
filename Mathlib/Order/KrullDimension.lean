...
/-!
# Krull dimensions of binary relations & preordered sets

If `α` is a set with a binary relation `r`, then `krullDimOfRel r` is the supremum of the rightmost
index of all relation series of `r`. In case that there is no relation series `a₀, a₁, ..., aₙ` in
`α`, then its Krull dimension is defined to be negative infinity; if the length of all relation
series `a₀, a₁, ..., aₙ` is unbounded, then its Krull dimension is defined to be positive infinity.
...
lemma eq_of_relIso (f : r ≃r s) : krullDimOfRel r = krullDimOfRel s :=
  le_antisymm (le_of_map r f fun _ _ h => f.2.mpr h) $ le_of_map _ f.symm fun _ _ h => f.2.mp $ by
...
lemma le_of_strictMono (f : α → β) (hf : StrictMono f) : krullDim α ≤ krullDim β := by
  refine krullDimOfRel.le_of_map (· < ·) f hf
...
lemma eq_of_orderIso (f : α ≃o β) : krullDim α = krullDim β :=
  krullDimOfRel.eq_of_relIso (· < ·) ⟨f, fun {_ _} => f.lt_iff_lt⟩
