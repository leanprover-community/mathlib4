...
lemma height_le {a : α} {n : ℕ∞} (h : ∀ (p : LTSeries α), p.last = a → p.length ≤ n) :
  ...
  rw [show p.length = p'.length by simp [p']; grind]
  apply h
  simp [p']
...
lemma length_le_height {p : LTSeries α} {x : α} (hlast : p.last ≤ x) :
  ...
  · let p' := p.eraseLast.snoc x (by
      ...
      · apply p.step ⟨p.length - 1, by grind⟩
      · convert hlast
...
lemma exists_series_of_le_height (a : α) {n : ℕ} (h : n ≤ height a) :
  ...
  | top =>
    ...
    simp only at hp
    by_contra! hnm
    apply ha (p.drop ⟨m-n, by grind⟩) (by simp) (by simp; grind)
  | coe m =>
...
lemma height_eq_iSup_lt_height (x : α) : height x = ⨆ y < x, height y + 1 := by
  ...
  · apply height_le
    ...
    | succ n =>
      ...
      apply le_iSup_of_le (by rw [← hp]; exact p.eraseLast_last_rel_last (by grind))
      rw [height_add_const]
...
lemma coe_lt_height_iff {x : α} {n : ℕ} (hfin : height x < ⊤) :
  ...
  mp h := by
    ...
    · rw [← hp]
      ...
      simp [Fin.last]; grind
    · exact height_eq_index_of_length_eq_height_last (by simp [hlen, hp, hx]) ⟨n, by grind⟩
  mpr := fun ⟨y, hyx, hy⟩ =>
...
lemma height_eq_coe_iff {x : α} {n : ℕ} :
  ...
  case succ n =>
    ...
    congr! 3
    rename_i y _
    cases height y <;> simp; norm_cast; grind
...
lemma height_eq_coe_iff_minimal_le_height {a : α} {n : ℕ} :
  ...
  · suffices ∃ x < a, ↑n ≤ height x by
    ...
    use p.eraseLast.last, p.eraseLast_last_rel_last (by grind)
...
@[simp] lemma height_coe_withBot (x : α) : height (x : WithBot α) = height x + 1 := by
  ...
  · apply height_le
    ...
    let p' : LTSeries α := {
      length := p.length - 1
      toFun := fun ⟨i, hi⟩ => (p ⟨i+1, by grind⟩).unbot (by
        apply ne_bot_of_gt (a := p.head)
        apply p.strictMono
        exact compare_gt_iff_gt.mp rfl)
      step := fun i => by simpa [WithBot.unbot_lt_iff] using p.step ⟨i + 1, by grind⟩ }
