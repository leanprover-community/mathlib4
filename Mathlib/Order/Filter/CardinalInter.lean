...
def fin_from_regular (c : Cardinal) {hreg : Cardinal.IsRegular c} (n : ℕ) : n < c:= by
...
alias ⟨_, Cardinal.biUnion⟩ := Cardinal.biUnion_iff

/-- A set is in the `cardinalInterFilter` generated by `g` if and only if
it contains an intersection of `c` elements of `g`. -/
theorem mem_cardinaleGenerate_iff {s : Set α} {hreg : Cardinal.IsRegular c} (hc : 2 < c) :
