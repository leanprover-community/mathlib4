...
/--
theorem cardinal_iInter_mem [Countable ι] {s : ι → Set α} : (⋂ i, s i) ∈ l ↔ ∀ i, s i ∈ l :=
  sInter_range s ▸ (cardinal_sInter_mem (countable_range _)).trans forall_range_iff

  --

theorem cardinal_bInter_mem {ι : Type*} {S : Set ι} (hS : Cardinal.mk S < c) {s : ∀ i ∈ S, Set α} :
