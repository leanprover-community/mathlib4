...
def Filter.ofCardinalUnion (p : Set α → Prop) (hc : 2 < c)
...
instance Filter.cardinalInter_ofCardinalUnion (p : Set α → Prop) (hc : 2 < c) (h₁ h₂) :
    CardinalInterFilter (Filter.ofCardinalUnion p hc h₁ h₂) c :=
  cardinalInter_ofCardinalInter ..

@[simp]
theorem Filter.mem_ofCardinalUnion {p : Set α → Prop} (hc : 2 < c) {hunion hmono s} :
...
theorem mem_cardinaleGenerate_iff {s : Set α} (hc : 2 < c) :
    s ∈ cardinalGenerate g hc ↔ ∃ S : Set (Set α), S ⊆ g ∧ (#S < c) ∧ ⋂₀ S ⊆ s := by
  constructor <;> intro h
  · induction' h with s hs s t _ st ih S Sct _ ih
    · refine ⟨{s}, singleton_subset_iff.mpr hs, ?_⟩
      norm_num; exact ⟨ lt_trans Nat.one_lt_ofNat hc, subset_rfl⟩
    · exact ⟨∅, by
        refine ⟨empty_subset g, ?_ ⟩
        constructor
        · have : 0 < c := lt_trans zero_lt_two hc
          rwa [mk_eq_zero]
        · simp
        ⟩
