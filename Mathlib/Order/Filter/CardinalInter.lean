...
/-- `Filter.CardinalGenerateSets c g` is the collection of sets that, assuming `2 < c`, form the
...
/-- If `c` is regular then a set is in the `CardinalInterFilter` generated by `g` if and only if it
contains an intersection of fewer than `c` elements of `g`. -/
