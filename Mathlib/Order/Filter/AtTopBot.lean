...
theorem Tendsto.eventually_ne_at_top' [Preorder β] [NoMaxOrder β] {f : α → β} {l : Filter α}
    (hf : Tendsto f l atTop) (c : α) : ∀ᶠ x in l, x ≠ c :=
  (hf.eventually_ne_at_top (f c)).mono fun x => ne_of_apply_ne f
#align filter.tendsto.eventually_ne_at_top' Filter.Tendsto.eventually_ne_at_top'
...
#align filter.frequently_at_top' Filter.frequently_at_top'

theorem frequently_at_bot' [SemilatticeInf α] [Nonempty α] [NoMinOrder α] {p : α → Prop} :
    (∃ᶠ x in at_bot, p x) ↔ ∀ a, ∃ b < a, p b :=
  @frequently_at_top' αᵒᵈ _ _ _ _
#align filter.frequently_at_bot' Filter.frequently_at_bot'
...
@[simp]
theorem comap_abs_atTop : comap (abs : α → α) atTop = at_bot ⊔ at_top := by
  refine'
...
#align filter.tendsto_at_top' Filter.tendsto_at_top'
...
#align filter.tendsto_at_bot' Filter.tendsto_at_bot'
...
theorem Tendsto.subseq_mem {F : Filter α} {V : ℕ → Set α} (h : ∀ n, V n ∈ F) {u : ℕ → α}
  ...
  extraction_forall_of_eventually'
    (fun n => tendsto_at_top'.mp hu _ (h n) : ∀ n, ∃ N, ∀ k ≥ N, u k ∈ V n)
...
theorem map_coe_atTop_of_ici_subset [SemilatticeSup α] {a : α} {s : Set α} (h : Ici a ⊆ s) :
...
#align filter.map_coe_at_top_of_Ici_subset Filter.map_coe_atTop_of_ici_subset

/-- The image of the filter `at_top` on `Ici a` under the coercion equals `at_top`. -/
@[simp]
theorem map_coe_ici_atTop [SemilatticeSup α] (a : α) : map (coe : Ici a → α) atTop = at_top :=
  map_coe_atTop_of_ici_subset (Subset.refl _)
#align filter.map_coe_Ici_at_top Filter.map_coe_ici_atTop

/-- The image of the filter `at_top` on `Ioi a` under the coercion equals `at_top`. -/
@[simp]
theorem map_coe_ioi_atTop [SemilatticeSup α] [NoMaxOrder α] (a : α) :
    map (coe : Ioi a → α) atTop = at_top :=
  let ⟨b, hb⟩ := exists_gt a
  map_coe_at_top_of_Ici_subset <| Ici_subset_Ioi.2 hb
#align filter.map_coe_Ioi_at_top Filter.map_coe_ioi_atTop

/-- The `at_top` filter for an open interval `Ioi a` comes from the `at_top` filter in the ambient
order. -/
theorem atTop_ioi_eq [SemilatticeSup α] (a : α) : at_top = comap (coe : Ioi a → α) atTop := by
  nontriviality
...
order. -/
@[simp]
theorem map_coe_iio_atBot [SemilatticeInf α] [NoMinOrder α] (a : α) :
    map (coe : Iio a → α) atBot = at_bot :=
  @map_coe_ioi_atTop αᵒᵈ _ _ _
#align filter.map_coe_Iio_at_bot Filter.map_coe_iio_atBot
...
order. -/
@[simp]
theorem map_coe_iic_atBot [SemilatticeInf α] (a : α) : map (coe : Iic a → α) atBot = at_bot :=
  @map_coe_ici_atTop αᵒᵈ _ _
#align filter.map_coe_Iic_at_bot Filter.map_coe_iic_atBot
...
@[simp]
theorem tendsto_comp_coe_ioi_atTop [SemilatticeSup α] [NoMaxOrder α] {a : α} {f : α → β}
    {l : Filter β} : Tendsto (fun x : Ioi a => f x) atTop l ↔ Tendsto f atTop l := by
  rw [← map_coe_Ioi_at_top a, tendsto_map'_iff]
#align filter.tendsto_comp_coe_Ioi_at_top Filter.tendsto_comp_coe_ioi_atTop

@[simp]
theorem tendsto_comp_coe_ici_atTop [SemilatticeSup α] {a : α} {f : α → β} {l : Filter β} :
    Tendsto (fun x : Ici a => f x) atTop l ↔ Tendsto f atTop l := by
  rw [← map_coe_Ici_at_top a, tendsto_map'_iff]
#align filter.tendsto_comp_coe_Ici_at_top Filter.tendsto_comp_coe_ici_atTop

@[simp]
theorem tendsto_comp_coe_iio_atBot [SemilatticeInf α] [NoMinOrder α] {a : α} {f : α → β}
    {l : Filter β} : Tendsto (fun x : Iio a => f x) atBot l ↔ Tendsto f atBot l := by
  rw [← map_coe_Iio_at_bot a, tendsto_map'_iff]
#align filter.tendsto_comp_coe_Iio_at_bot Filter.tendsto_comp_coe_iio_atBot

@[simp]
theorem tendsto_comp_coe_iic_atBot [SemilatticeInf α] {a : α} {f : α → β} {l : Filter β} :
    Tendsto (fun x : Iic a => f x) atBot l ↔ Tendsto f atBot l := by
  rw [← map_coe_Iic_at_bot a, tendsto_map'_iff]
#align filter.tendsto_comp_coe_Iic_at_bot Filter.tendsto_comp_coe_iic_atBot
...
theorem unbounded_of_tendsto_at_top' [Nonempty α] [SemilatticeInf α] [Preorder β] [NoMaxOrder β]
...
theorem unbounded_of_tendsto_at_bot' [Nonempty α] [SemilatticeInf α] [Preorder β] [NoMinOrder β]
...
theorem not_tendsto_iff_exists_frequently_nmem {α ι : Type _} {x : ι → α} {f : Filter α}
    {l : Filter ι} : ¬Tendsto x l f ↔ ∃ s ∈ f, ∃ᶠ n in l, x n ∉ s := by
