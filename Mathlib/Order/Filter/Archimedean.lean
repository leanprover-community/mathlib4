...
@[simp]
lemma Nat.comap_cast_atTop [StrictOrderedSemiring R] [Archimedean R] :
...
lemma tendsto_nat_cast_atTop_iff [StrictOrderedSemiring R] [Archimedean R] {f : α → ℕ}
...
lemma tendsto_nat_cast_atTop_atTop [StrictOrderedSemiring R] [Archimedean R] :
...
lemma Filter.Eventually.nat_cast_atTop [StrictOrderedSemiring R] [Archimedean R] {p : R → Prop}
    (h : ∀ᶠ (x:R) in atTop, p x) : ∀ᶠ (n:ℕ) in atTop, p n := by
  rw [←Nat.comap_cast_atTop (R := R)]; exact h.comap _

@[simp] lemma Int.comap_cast_atTop [StrictOrderedRing R] [Archimedean R] :
...
@[simp]
lemma Int.comap_cast_atBot [StrictOrderedRing R] [Archimedean R] :
...
lemma tendsto_int_cast_atTop_iff [StrictOrderedRing R] [Archimedean R] {f : α → ℤ}
...
lemma tendsto_int_cast_atBot_iff [StrictOrderedRing R] [Archimedean R] {f : α → ℤ}
...
lemma tendsto_int_cast_atTop_atTop [StrictOrderedRing R] [Archimedean R] :
...
lemma Filter.Eventually.int_cast_atTop [StrictOrderedRing R] [Archimedean R] {p : R → Prop}
    (h : ∀ᶠ (x:R) in atTop, p x) : ∀ᶠ (n:ℤ) in atTop, p n := by
  rw [←Int.comap_cast_atTop (R := R)]; exact h.comap _

lemma Filter.Eventually.int_cast_atBot [StrictOrderedRing R] [Archimedean R] {p : R → Prop}
    (h : ∀ᶠ (x:R) in atBot, p x) : ∀ᶠ (n:ℤ) in atBot, p n := by
  rw [←Int.comap_cast_atBot (R := R)]; exact h.comap _

@[simp]
lemma Rat.comap_cast_atTop [LinearOrderedField R] [Archimedean R] :
...
@[simp] lemma Rat.comap_cast_atBot [LinearOrderedField R] [Archimedean R] :
...
lemma tendsto_rat_cast_atTop_iff [LinearOrderedField R] [Archimedean R] {f : α → ℚ}
    {l : Filter α} : Tendsto (fun n => (f n : R)) l atTop ↔ Tendsto f l atTop := by
  rw [← @Rat.comap_cast_atTop R, tendsto_comap_iff]; rfl
#align tendsto_coe_rat_at_top_iff tendsto_rat_cast_atTop_iff

lemma tendsto_rat_cast_atBot_iff [LinearOrderedField R] [Archimedean R] {f : α → ℚ}
    {l : Filter α} : Tendsto (fun n => (f n : R)) l atBot ↔ Tendsto f l atBot := by
  rw [← @Rat.comap_cast_atBot R, tendsto_comap_iff]; rfl
#align tendsto_coe_rat_at_bot_iff tendsto_rat_cast_atBot_iff

lemma Filter.Eventually.rat_cast_atTop [LinearOrderedField R] [Archimedean R] {p : R → Prop}
    (h : ∀ᶠ (x:R) in atTop, p x) : ∀ᶠ (n:ℚ) in atTop, p n := by
  rw [←Rat.comap_cast_atTop (R := R)]; exact h.comap _

lemma Filter.Eventually.rat_cast_atBot [LinearOrderedField R] [Archimedean R] {p : R → Prop}
    (h : ∀ᶠ (x:R) in atBot, p x) : ∀ᶠ (n:ℚ) in atBot, p n := by
  rw [←Rat.comap_cast_atBot (R := R)]; exact h.comap _

-- porting note: new lemma
lemma atTop_hasAntitoneBasis_of_archimedean [StrictOrderedSemiring R] [Archimedean R] :
...
lemma atTop_hasCountableBasis_of_archimedean [StrictOrderedSemiring R] [Archimedean R] :
...
lemma atBot_hasCountableBasis_of_archimedean [LinearOrderedRing R] [Archimedean R] :
...
lemma Tendsto.atTop_nsmul_const {f : α → ℕ} (hr : 0 < r) (hf : Tendsto f l atTop) :
    Tendsto (fun x => f x • r) l atTop := by
...
lemma Tendsto.atTop_nsmul_neg_const {f : α → ℕ} (hr : r < 0) (hf : Tendsto f l atTop) :
...
lemma Tendsto.atTop_zsmul_const {f : α → ℤ} (hr : 0 < r) (hf : Tendsto f l atTop) :
    Tendsto (fun x => f x • r) l atTop := by
  refine' tendsto_atTop.mpr fun s => _
  obtain ⟨n : ℕ, hn : s ≤ n • r⟩ := Archimedean.arch s hr
...
lemma Tendsto.atTop_zsmul_neg_const {f : α → ℤ} (hr : r < 0) (hf : Tendsto f l atTop) :
...
lemma Tendsto.atBot_zsmul_const {f : α → ℤ} (hr : 0 < r) (hf : Tendsto f l atBot) :
    Tendsto (fun x => f x • r) l atBot := by
...
lemma Tendsto.atBot_zsmul_neg_const {f : α → ℤ} (hr : r < 0) (hf : Tendsto f l atBot) :
