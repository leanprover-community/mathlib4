...
@[simp]
lemma mem_cofinite {s : Set α} : s ∈ @cofinite α ↔ sᶜ.Finite :=
  Iff.rfl
#align filter.mem_cofinite Filter.mem_cofinite

@[simp]
lemma eventually_cofinite {p : α → Prop} : (∀ᶠ x in cofinite, p x) ↔ { x | ¬p x }.Finite :=
  Iff.rfl
#align filter.eventually_cofinite Filter.eventually_cofinite

lemma hasBasis_cofinite : HasBasis cofinite (fun s : Set α => s.Finite) compl :=
  ⟨fun s =>
...
@[simp]
lemma cofinite_eq_bot_iff : @cofinite α = ⊥ ↔ Finite α := by
  simp [← empty_mem_iff_bot, finite_univ_iff]

@[simp]
lemma cofinite_eq_bot [Finite α] : @cofinite α = ⊥ := cofinite_eq_bot_iff.2 ‹_›
...
lemma le_cofinite_iff_compl_singleton_mem : l ≤ cofinite ↔ ∀ x, {x}ᶜ ∈ l := by
...
lemma atTop_le_cofinite [Preorder α] [NoMaxOrder α] : (atTop : Filter α) ≤ cofinite :=
...
lemma coprodᵢ_cofinite {α : ι → Type*} [Finite ι] :
...
@[simp]
lemma disjoint_cofinite_left : Disjoint cofinite l ↔ ∃ s ∈ l, Set.Finite s := by
...
@[simp]
lemma disjoint_cofinite_right : Disjoint l cofinite ↔ ∃ s ∈ l, Set.Finite s :=
...
lemma countable_compl_sInter_sets [l.IsCountablyGenerated] (h : cofinite ≤ l) :
...
lemma Tendsto.countable_compl_preimage_sInter_sets {f : α → β}
...
lemma Nat.cofinite_eq_atTop : @cofinite ℕ = atTop := by
...
lemma Nat.frequently_atTop_iff_infinite {p : ℕ → Prop} :
...
lemma Filter.Tendsto.exists_within_forall_le {α β : Type*} [LinearOrder β] {s : Set α}
...
lemma Filter.Tendsto.exists_forall_le [Nonempty α] [LinearOrder β] {f : α → β}
...
lemma Filter.Tendsto.exists_within_forall_ge [LinearOrder β] {s : Set α} (hs : s.Nonempty)
...
lemma Filter.Tendsto.exists_forall_ge [Nonempty α] [LinearOrder β] {f : α → β}
...
lemma Function.Surjective.le_map_cofinite {f : α → β} (hf : Surjective f) :
...
lemma Function.Injective.nat_tendsto_atTop {f : ℕ → ℕ} (hf : Injective f) :
