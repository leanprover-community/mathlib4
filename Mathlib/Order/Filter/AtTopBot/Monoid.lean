...
@[to_additive /-- In an ordered additive monoid, if `f` and `g` tend to `+∞`, then so does `f + g`. -/]
...
@[to_additive /-- In an ordered cancellative additive monoid, if `C + f x → +∞`, then `f x → +∞`. -/]
...
@[to_additive /-- In an ordered cancellative additive monoid, if `f x + C → +∞`, then `f x → +∞`. -/]
