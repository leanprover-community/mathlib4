...
theorem eventually_atBot_prod_self [Nonempty α] [Preorder α] [IsPrecodirectedOrder α]
    {p : α × α → Prop} : (∀ᶠ x in atBot, p x) ↔ ∃ a, ∀ k l, k ≤ a → l ≤ a → p (k, l) := by
  simp [← prod_atBot_atBot_eq, (@atBot_basis α _ _).prod_self.eventually_iff]

theorem eventually_atTop_prod_self [Nonempty α] [Preorder α] [IsPredirectedOrder α]
