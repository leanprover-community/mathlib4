...
section IsDirected
variable [Preorder α] [IsDirectedLE α] {p : α → Prop}
...
theorem atTop_neBot_iff {α : Type*} [Preorder α] :
    (atTop : Filter α).NeBot ↔ Nonempty α ∧ IsDirectedLE α := by
...
theorem atBot_neBot_iff {α : Type*} [Preorder α] :
    (atBot : Filter α).NeBot ↔ Nonempty α ∧ IsDirectedGE α :=
...
variable [Preorder α] [IsDirectedGE α] {p : α → Prop}
...
section IsDirected
variable [Preorder α] [IsDirectedLE α] {F : Filter β} {u : α → β}
...
section IsCodirected
variable [Nonempty α] [Preorder α] [IsDirectedGE α] {F : Filter β} {u : α → β}
...
section IsDirected
variable [Nonempty α] [Preorder α] [IsDirectedLE α] {f : α → β} {l : Filter β}
...
section IsCodirected
variable [Nonempty α] [Preorder α] [IsDirectedGE α] {f : α → β} {l : Filter β}
...
theorem map_atTop_eq_of_gc_preorder
    [Preorder α] [IsDirectedLE α] [Preorder β] [IsDirectedLE β] {f : α → β}
    (hf : Monotone f) (b : β)
...
theorem map_atTop_eq_of_gc
    [Preorder α] [IsDirectedLE α] [PartialOrder β] [IsDirectedLE β]
...
theorem map_atBot_eq_of_gc_preorder
    [Preorder α] [IsDirectedGE α] [Preorder β] [IsDirectedGE β] {f : α → β}
    (hf : Monotone f) (b : β)
    (hgi : ∀ c ≤ b, ∃ x, f x = c ∧ ∀ a, c ≤ f a ↔ x ≤ a) : map f atBot = atBot :=
  map_atTop_eq_of_gc_preorder (α := αᵒᵈ) (β := βᵒᵈ) hf.dual _ hgi

theorem map_atBot_eq_of_gc [Preorder α] [IsDirectedGE α]
    [PartialOrder β] [IsDirectedGE β] {f : α → β} (g : β → α) (b' : β)
    (hf : Monotone f) (gc : ∀ a, ∀ b ≤ b', b ≤ f a ↔ g b ≤ a) (hgi : ∀ b ≤ b', f (g b) ≤ b) :
    map f atBot = atBot :=
  map_atTop_eq_of_gc (α := αᵒᵈ) (β := βᵒᵈ) _ _ hf.dual gc hgi

theorem map_val_atTop_of_Ici_subset [Preorder α] [IsDirectedLE α] {a : α} {s : Set α}
...
@[simp]
theorem map_val_Ici_atTop [Preorder α] [IsDirectedLE α] (a : α) :
...
@[simp]
theorem map_val_Ioi_atTop [Preorder α] [IsDirectedLE α] [NoMaxOrder α] (a : α) :
...
order. -/
theorem atTop_Ioi_eq [Preorder α] [IsDirectedLE α] (a : α) :
...
order. -/
theorem atTop_Ici_eq [Preorder α] [IsDirectedLE α] (a : α) :
...
order. -/
@[simp]
theorem map_val_Iio_atBot [Preorder α] [IsDirectedGE α] [NoMinOrder α] (a : α) :
    map ((↑) : Iio a → α) atBot = atBot :=
  map_val_Ioi_atTop (OrderDual.toDual a)

/-- The `atBot` filter for an open interval `Iio a` comes from the `atBot` filter in the ambient
order. -/
theorem atBot_Iio_eq [Preorder α] [IsDirectedGE α] (a : α) :
...
order. -/
@[simp]
theorem map_val_Iic_atBot [Preorder α] [IsDirectedGE α] (a : α) :
    map ((↑) : Iic a → α) atBot = atBot :=
  map_val_Ici_atTop (OrderDual.toDual a)

/-- The `atBot` filter for an open interval `Iic a` comes from the `atBot` filter in the ambient
order. -/
theorem atBot_Iic_eq [Preorder α] [IsDirectedGE α] (a : α) :
    atBot = comap ((↑) : Iic a → α) atBot :=
  atTop_Ici_eq (OrderDual.toDual a)

theorem tendsto_Ioi_atTop [Preorder α] [IsDirectedLE α]
...
theorem tendsto_Iio_atBot [Preorder α] [IsDirectedGE α]
    {a : α} {f : β → Iio a} {l : Filter β} :
    Tendsto f l atBot ↔ Tendsto (fun x => (f x : α)) l atBot :=
  tendsto_Ioi_atTop (α := αᵒᵈ)

theorem tendsto_Ici_atTop [Preorder α] [IsDirectedLE α]
    {a : α} {f : β → Ici a} {l : Filter β} :
    Tendsto f l atTop ↔ Tendsto (fun x => (f x : α)) l atTop := by
  rw [atTop_Ici_eq, tendsto_comap_iff, Function.comp_def]

theorem tendsto_Iic_atBot [Preorder α] [IsDirectedGE α]
    {a : α} {f : β → Iic a} {l : Filter β} :
    Tendsto f l atBot ↔ Tendsto (fun x => (f x : α)) l atBot :=
  tendsto_Ici_atTop (α := αᵒᵈ)

@[simp]
theorem tendsto_comp_val_Ioi_atTop [Preorder α] [IsDirectedLE α] [NoMaxOrder α]
...
@[simp]
theorem tendsto_comp_val_Ici_atTop [Preorder α] [IsDirectedLE α]
    {a : α} {f : α → β} {l : Filter β} :
    Tendsto (fun x : Ici a => f x) atTop l ↔ Tendsto f atTop l := by
  rw [← map_val_Ici_atTop a, tendsto_map'_iff, Function.comp_def]

@[simp]
theorem tendsto_comp_val_Iio_atBot [Preorder α] [IsDirectedGE α] [NoMinOrder α]
    {a : α} {f : α → β} {l : Filter β} :
    Tendsto (fun x : Iio a => f x) atBot l ↔ Tendsto f atBot l :=
  tendsto_comp_val_Ioi_atTop (α := αᵒᵈ)

@[simp]
theorem tendsto_comp_val_Iic_atBot [Preorder α] [IsDirectedGE α]
    {a : α} {f : α → β} {l : Filter β} :
...
theorem HasAntitoneBasis.comp_mono [Nonempty ι] [Preorder ι] [IsDirectedLE ι] [Preorder ι']
    {l : Filter α}
