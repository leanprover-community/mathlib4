...
def rmap (r : Set.Rel α β) (l : Filter α) : Filter β where
  sets := { s | r.core s ∈ l }
  univ_sets := by simp
  sets_of_superset hs st := mem_of_superset hs (Set.Rel.core_mono st)
  inter_sets hs ht := by
    simp only [Set.mem_setOf_eq]
    convert inter_mem hs ht
    rw [← Set.Rel.core_inter]

theorem rmap_sets (r : Set.Rel α β) (l : Filter α) : (l.rmap r).sets = r.core ⁻¹' l.sets :=
  rfl

@[simp]
theorem mem_rmap (r : Set.Rel α β) (l : Filter α) (s : Set β) : s ∈ l.rmap r ↔ r.core s ∈ l :=
  Iff.rfl

@[simp]
theorem rmap_rmap (r : Set.Rel α β) (s : Set.Rel β γ) (l : Filter α) :
    rmap s (rmap r l) = rmap (r.comp s) l :=
  filter_eq <| by simp [rmap_sets, Set.preimage, Set.Rel.core_comp]

@[simp]
theorem rmap_compose (r : Set.Rel α β) (s : Set.Rel β γ) : rmap s ∘ rmap r = rmap (r.comp s) :=
...
def RTendsto (r : Set.Rel α β) (l₁ : Filter α) (l₂ : Filter β) :=
  l₁.rmap r ≤ l₂

theorem rtendsto_def (r : Set.Rel α β) (l₁ : Filter α) (l₂ : Filter β) :
...
def rcomap (r : Set.Rel α β) (f : Filter β) : Filter α where
  sets := Set.Rel.image {(s, t) : _ × _ | r.core s ⊆ t} f.sets
...
theorem rcomap_sets (r : Set.Rel α β) (f : Filter β) :
    (rcomap r f).sets = Set.Rel.image {(s, t) : _ × _ | r.core s ⊆ t} f.sets :=
  rfl

theorem rcomap_rcomap (r : Set.Rel α β) (s : Set.Rel β γ) (l : Filter γ) :
    rcomap r (rcomap s l) = rcomap (r.comp s) l :=
  filter_eq <| by
    ext t
    simp only [rcomap_sets, Set.Rel.image, Filter.mem_sets, Set.mem_setOf_eq,Set.Rel.core_comp]
    constructor
    · rintro ⟨u, ⟨v, vsets, hv⟩, h⟩
      exact ⟨v, vsets, Set.Subset.trans (Set.Rel.core_mono hv) h⟩
    rintro ⟨t, tsets, ht⟩
    exact ⟨Set.Rel.core s t, ⟨t, tsets, Set.Subset.rfl⟩, ht⟩

@[simp]
theorem rcomap_compose (r : Set.Rel α β) (s : Set.Rel β γ) :
    rcomap r ∘ rcomap s = rcomap (r.comp s) :=
  funext <| rcomap_rcomap _ _

theorem rtendsto_iff_le_rcomap (r : Set.Rel α β) (l₁ : Filter α) (l₂ : Filter β) :
    RTendsto r l₁ l₂ ↔ l₁ ≤ l₂.rcomap r := by
  rw [rtendsto_def]
  simp_rw [← l₂.mem_sets]
  constructor
  · simpa [Filter.le_def, rcomap, Set.Rel.mem_image]
...
to relations. -/
def rcomap' (r : Set.Rel α β) (f : Filter β) : Filter α where
  sets := Set.Rel.image {(s, t) : _ × _ | r.preimage s ⊆ t} f.sets
  univ_sets := ⟨Set.univ, univ_mem, Set.subset_univ _⟩
  sets_of_superset := fun ⟨a', ha', ma'a⟩ ab => ⟨a', ha', ma'a.trans ab⟩
  inter_sets := fun ⟨a', ha₁, ha₂⟩ ⟨b', hb₁, hb₂⟩ =>
...
@[simp]
theorem mem_rcomap' (r : Set.Rel α β) (l : Filter β) (s : Set α) :
    s ∈ l.rcomap' r ↔ ∃ t ∈ l, r.preimage t ⊆ s :=
  Iff.rfl

theorem rcomap'_sets (r : Set.Rel α β) (f : Filter β) :
    (rcomap' r f).sets = Set.Rel.image {(s, t) | r.preimage s ⊆ t} f.sets :=
  rfl

@[simp]
theorem rcomap'_rcomap' (r : Set.Rel α β) (s : Set.Rel β γ) (l : Filter γ) :
    rcomap' r (rcomap' s l) = rcomap' (r.comp s) l :=
  Filter.ext fun t => by
    simp only [mem_rcomap', Set.Rel.preimage_comp]
    constructor
    · rintro ⟨u, ⟨v, vsets, hv⟩, h⟩
      exact ⟨v, vsets, (Set.Rel.preimage_mono hv).trans h⟩
    rintro ⟨t, tsets, ht⟩
    exact ⟨s.preimage t, ⟨t, tsets, Set.Subset.rfl⟩, ht⟩

@[simp]
theorem rcomap'_compose (r : Set.Rel α β) (s : Set.Rel β γ) :
    rcomap' r ∘ rcomap' s = rcomap' (r.comp s) :=
...
theorem rtendsto'_def (r : Set.Rel α β) (l₁ : Filter α) (l₂ : Filter β) :
    RTendsto' r l₁ l₂ ↔ ∀ s ∈ l₂, r.preimage s ∈ l₁ := by
  unfold RTendsto' rcomap'; constructor
  · simpa [le_def, Set.Rel.mem_image] using fun h s hs => h _ _ hs Set.Subset.rfl
  · simpa [le_def, Set.Rel.mem_image] using fun h s t ht => mem_of_superset (h t ht)

theorem tendsto_iff_rtendsto (l₁ : Filter α) (l₂ : Filter β) (f : α → β) :
    Tendsto f l₁ l₂ ↔ RTendsto (Function.graph f) l₁ l₂ := by
  simp [tendsto_def, Function.graph, rtendsto_def, Set.Rel.core, Set.preimage]

theorem tendsto_iff_rtendsto' (l₁ : Filter α) (l₂ : Filter β) (f : α → β) :
    Tendsto f l₁ l₂ ↔ RTendsto' (Function.graph f) l₁ l₂ := by
  simp [tendsto_def, Function.graph, rtendsto'_def, Set.Rel.preimage, Set.preimage]
