...
lemma prod_mem_prod (hs : s ∈ f) (ht : t ∈ g) : s ×ˢ t ∈ f ×ˢ g :=
...
@[simp]
lemma prod_mem_prod_iff [f.NeBot] [g.NeBot] : s ×ˢ t ∈ f ×ˢ g ↔ s ∈ f ∧ t ∈ g :=
  ⟨fun h =>
...
lemma mem_prod_principal {s : Set (α × β)} :
...
lemma prod_top : f ×ˢ (⊤ : Filter β) = f.comap Prod.fst := by
  dsimp only [SProd.sprod]
  rw [Filter.prod, comap_top, inf_top_eq]
#align filter.prod_top Filter.prod_top

lemma top_prod : (⊤ : Filter α) ×ˢ g = g.comap Prod.snd := by
  dsimp only [SProd.sprod]
...
lemma eventually_prod_iff {p : α × β → Prop} :
    (∀ᶠ x in f ×ˢ g, p x) ↔
...
lemma tendsto_fst : Tendsto Prod.fst (f ×ˢ g) f :=
  tendsto_inf_left tendsto_comap
#align filter.tendsto_fst Filter.tendsto_fst

lemma tendsto_snd : Tendsto Prod.snd (f ×ˢ g) g :=
...
lemma Tendsto.prod_mk {h : Filter γ} {m₁ : α → β} {m₂ : α → γ}
...
lemma tendsto_prod_swap : Tendsto (Prod.swap : α × β → β × α) (f ×ˢ g) (g ×ˢ f) :=
...
lemma mem_prod_iff_left {s : Set (α × β)} :
...
lemma mem_prod_iff_right {s : Set (α × β)} :
...
lemma tendsto_swap4_prod {h : Filter γ} {k : Filter δ} :
...
lemma prod_eq : f ×ˢ g = (f.map Prod.mk).seq g := f.map_prod id g
...
lemma inf_prod {f₁ f₂ : Filter α} : (f₁ ⊓ f₂) ×ˢ g = (f₁ ×ˢ g) ⊓ (f₂ ×ˢ g) := by
  rw [prod_inf_prod, inf_idem]

lemma prod_inf {g₁ g₂ : Filter β} : f ×ˢ (g₁ ⊓ g₂) = (f ×ˢ g₁) ⊓ (f ×ˢ g₂) := by
  rw [prod_inf_prod, inf_idem]
...
@[simp]
lemma prod_pure {b : β} : f ×ˢ pure b = map (fun a => (a, b)) f := by
...
@[simp]
lemma prod_eq_bot : f ×ˢ g = ⊥ ↔ f = ⊥ ∨ g = ⊥ := by
...
@[simp] lemma prod_bot : f ×ˢ (⊥ : Filter β) = ⊥ := prod_eq_bot.2 <| Or.inr rfl
#align filter.prod_bot Filter.prod_bot

@[simp] lemma bot_prod : (⊥ : Filter α) ×ˢ g = ⊥ := prod_eq_bot.2 <| Or.inl rfl
#align filter.bot_prod Filter.bot_prod

lemma prod_neBot : NeBot (f ×ˢ g) ↔ NeBot f ∧ NeBot g := by
  simp only [neBot_iff, Ne, prod_eq_bot, not_or]
#align filter.prod_ne_bot Filter.prod_neBot

protected lemma NeBot.prod (hf : NeBot f) (hg : NeBot g) : NeBot (f ×ˢ g) := prod_neBot.2 ⟨hf, hg⟩
...
lemma tendsto_prod_iff' {g' : Filter γ} {s : α → β × γ} :
...
lemma coprod_eq_prod_top_sup_top_prod (f : Filter α) (g : Filter β) :
