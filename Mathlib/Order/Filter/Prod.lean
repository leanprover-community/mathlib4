...
theorem prod_assoc (f : Filter α) (g : Filter β) (h : Filter γ) :
  ...
  simp (config := { unfoldPartialApp := true }) only [(· ∘ ·), Equiv.prodAssoc_symm_apply]
...
theorem prod_assoc_symm (f : Filter α) (g : Filter β) (h : Filter γ) :
  ...
  simp (config := { unfoldPartialApp := true }) only [Function.comp, Equiv.prodAssoc_apply]
...
theorem tendsto_prod_iff' {g' : Filter γ} {s : α → β × γ} :
  ...
  dsimp only [SProd.sprod]
  unfold Filter.prod
  simp (config := { unfoldPartialApp := true }) only [tendsto_inf, tendsto_comap_iff, (· ∘ ·)]
#align filter.tendsto_prod_iff' Filter.tendsto_prod_iff'
