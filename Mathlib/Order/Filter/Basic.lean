...
instance {α : Type*} : Membership (Set α) (Filter α) := fast_instance%
  ⟨fun U F => U ∈ F.sets⟩
...
instance inhabitedMem : Inhabited { s : Set α // s ∈ f } := fast_instance%
  ⟨⟨univ, f.univ_sets⟩⟩
...
instance : Inf (Filter α) := fast_instance%
  ⟨fun f g : Filter α =>
...
instance : Top (Filter α) := fast_instance%
  ⟨{  sets := { s | ∀ x, x ∈ s }
...
instance instCompleteLatticeFilter : CompleteLattice (Filter α) := fast_instance%
...
instance : Inhabited (Filter α) := fast_instance% ⟨⊥⟩
...
instance instNontrivialFilter [Nonempty α] : Nontrivial (Filter α) := fast_instance%
...
instance : DistribLattice (Filter α) := fast_instance%
...
instance : Coframe (Filter α) := fast_instance%
  { Filter.instCompleteLatticeFilter with
...
instance : Pure Filter := fast_instance%
  ⟨fun x =>
    { sets := { s | x ∈ s }
...
instance map_neBot [hf : NeBot f] : NeBot (f.map m) := fast_instance%
  hf.map m
...
instance pure_neBot {α : Type u} {a : α} : NeBot (pure a) := fast_instance%
...
instance : CommApplicative (Filter : Type u → Type u) := fast_instance%
