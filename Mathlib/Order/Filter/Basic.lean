...
lemma NeBot.nonempty (f : Filter α) [hf : f.NeBot] : Nonempty α :=
...
lemma iInf_principal {ι : Sort w} [Finite ι] (f : ι → Set α) : ⨅ i, 𝓟 (f i) = 𝓟 (⋂ i, f i) := by
...
@[simp]
lemma eventually_all {ι : Sort*} [Finite ι] {l} {p : ι → α → Prop} :
...
lemma Eventually.forall_mem {α : Type*} {f : Filter α} {s : Set α} {P : α → Prop}
...
protected lemma EventuallyEq.rfl {l : Filter α} {f : α → β} : f =ᶠ[l] f :=
...
lemma EventuallyEq.pow_const {γ} [Pow β γ] {f g : α → β} {l : Filter α} (h : f =ᶠ[l] g) (c : γ):
...
lemma pure_le_principal (a : α) : pure a ≤ 𝓟 s ↔ a ∈ s := by
  simp
...
lemma _root_.Function.LeftInverse.filter_map {f : α → β} {g : β → α} (hfg : LeftInverse g f) :
    LeftInverse (map g) (map f) := fun F ↦ by
  rw [map_map, hfg.comp_eq_id, map_id]

lemma _root_.Function.LeftInverse.filter_comap {f : α → β} {g : β → α} (hfg : LeftInverse g f) :
    RightInverse (comap g) (comap f) := fun F ↦ by
  rw [comap_comap, hfg.comp_eq_id, comap_id]

nonrec lemma _root_.Function.RightInverse.filter_map {f : α → β} {g : β → α}
    (hfg : RightInverse g f) : RightInverse (map g) (map f) :=
  hfg.filter_map

nonrec lemma _root_.Function.RightInverse.filter_comap {f : α → β} {g : β → α}
    (hfg : RightInverse g f) : LeftInverse (comap g) (comap f) :=
  hfg.filter_comap

lemma _root_.Set.LeftInvOn.filter_map_Iic {f : α → β} {g : β → α} (hfg : LeftInvOn g f s) :
...
nonrec lemma _root_.Set.RightInvOn.filter_map_Iic {f : α → β} {g : β → α}
...
lemma comap_injective {f : α → β} (hf : Surjective f) : Injective (comap f) :=
...
lemma seq_eq_filter_seq {α β : Type u} (f : Filter (α → β)) (g : Filter α) :
    f <*> g = seq f g :=
  rfl
...
lemma tendsto_iff_forall_eventually_mem {f : α → β} {l₁ : Filter α} {l₂ : Filter β} :
...
lemma not_tendsto_iff_exists_frequently_nmem {f : α → β} {l₁ : Filter α} {l₂ : Filter β} :
...
protected lemma Tendsto.iterate {f : α → α} {l : Filter α} (h : Tendsto f l l) :
    ∀ n, Tendsto (f^[n]) l l
  | 0 => tendsto_id
...
lemma Tendsto.sup_sup {f : α → β} {x₁ x₂ : Filter α} {y₁ y₂ : Filter β}
...
lemma Filter.EventuallyEq.comp_tendsto {f' : α → β} (H : f =ᶠ[l] f') {g : γ → α} {lc : Filter γ}
    (hg : Tendsto g lc l) : f ∘ g =ᶠ[lc] f' ∘ g :=
  hg.eventually H
#align filter.eventually_eq.comp_tendsto Filter.EventuallyEq.comp_tendsto

lemma Filter.map_mapsTo_Iic_iff_tendsto {m : α → β} :
...
lemma Filter.map_mapsTo_Iic_iff_mapsTo {m : α → β} :
...
lemma Filter.map_surjOn_Iic_iff_le_map {m : α → β} :
...
lemma Filter.map_surjOn_Iic_iff_surjOn {m : α → β} :
...
lemma Filter.filter_injOn_Iic_iff_injOn {m : α → β} :
