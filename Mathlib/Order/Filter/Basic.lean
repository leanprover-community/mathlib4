...
instance (priority := 10000) {α : Type*} : Membership (Set α) (Filter α) :=
  ⟨fun U F => U ∈ F.sets⟩
...
instance (priority := 10000) inhabitedMem : Inhabited { s : Set α // s ∈ f } :=
  ⟨⟨univ, f.univ_sets⟩⟩
...
instance (priority := 10000) : PartialOrder (Filter α) where
...
instance (priority := 10000) : Inf (Filter α) :=
  ⟨fun f g : Filter α =>
...
instance (priority := 10000) : Top (Filter α) :=
  ⟨{  sets := { s | ∀ x, x ∈ s }
...
instance (priority := 10000) instCompleteLatticeFilter : CompleteLattice (Filter α) :=
...
instance (priority := 10000) : Inhabited (Filter α) := ⟨⊥⟩
...
instance (priority := 10000) unique [IsEmpty α] : Unique (Filter α) where
  default := ⊥
...
instance (priority := 10000) instNontrivialFilter [Nonempty α] : Nontrivial (Filter α) :=
...
instance (priority := 10000) : DistribLattice (Filter α) :=
...
instance (priority := 10000) : Coframe (Filter α) :=
  { Filter.instCompleteLatticeFilter with
...
instance (priority := 10000) : Trans ((· =ᶠ[l] ·) : (α → β) → (α → β) → Prop) (· =ᶠ[l] ·) (· =ᶠ[l] ·) where
...
instance (priority := 10000) : Pure Filter :=
  ⟨fun x =>
    { sets := { s | x ∈ s }
...
instance (priority := 10000) : Bind Filter :=
  ⟨@Filter.bind⟩

instance (priority := 10000) : Functor Filter where map := @Filter.map

instance (priority := 10000) : LawfulFunctor (Filter : Type u → Type u) where
  id_map _ := map_id
...
instance (priority := 10000) : Alternative Filter where
  seq := fun x y => x.seq (y ())
  failure := ⊥
  orElse x y := x ⊔ y ()
...
instance (priority := 10000) map_neBot [hf : NeBot f] : NeBot (f.map m) :=
  hf.map m
...
instance (priority := 10000) pure_neBot {α : Type u} {a : α} : NeBot (pure a) :=
...
instance (priority := 10000) : LawfulApplicative (Filter : Type u → Type u) where
  map_pure := map_pure
  seqLeft_eq _ _ := rfl
  seqRight_eq _ _ := rfl
  seq_pure := seq_pure
...
instance (priority := 10000) : CommApplicative (Filter : Type u → Type u) :=
