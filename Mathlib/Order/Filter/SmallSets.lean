...
lemma smallSets_eq_generate {f : Filter α} : f.smallSets = generate (powerset '' f.sets) := by
  simp_rw [generate_eq_biInf, smallSets, iInf_image]
  rfl
#align filter.small_sets_eq_generate Filter.smallSets_eq_generate

lemma HasBasis.smallSets {p : ι → Prop} {s : ι → Set α} (h : HasBasis l p s) :
...
/-- `g` converges to `f.smallSets` if for all `s ∈ f`, eventually we have `g x ⊆ s`. -/
lemma tendsto_smallSets_iff {f : α → Set β} :
...
lemma eventually_smallSets {p : Set α → Prop} :
...
lemma eventually_smallSets' {p : Set α → Prop} (hp : ∀ ⦃s t⦄, s ⊆ t → p t → p s) :
...
lemma frequently_smallSets {p : Set α → Prop} :
...
lemma HasAntitoneBasis.tendsto_smallSets {ι} [Preorder ι] {s : ι → Set α}
...
lemma smallSets_iInf {f : ι → Filter α} : (iInf f).smallSets = ⨅ i, (f i).smallSets :=
...
lemma Tendsto.smallSets_mono {s t : α → Set β} (ht : Tendsto t la lb.smallSets)
...
lemma Tendsto.of_smallSets {s : α → Set β} {f : α → β} (hs : Tendsto s la lb.smallSets)
...
@[simp]
lemma eventually_smallSets_eventually {p : α → Prop} :
...
@[simp]
lemma eventually_smallSets_forall {p : α → Prop} :
...
@[simp]
lemma eventually_smallSets_subset {s : Set α} : (∀ᶠ t in l.smallSets, t ⊆ s) ↔ s ∈ l :=
