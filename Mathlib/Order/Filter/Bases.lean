...
lemma HasBasis.to_image_id' (h : l.HasBasis p s) : l.HasBasis (fun t ↦ ∃ i, p i ∧ s i = t) id :=
  ⟨fun _ ↦ by simp [h.mem_iff]⟩

lemma HasBasis.to_image_id {ι : Type*} {p : ι → Prop} {s : ι → Set α} (h : l.HasBasis p s) :
...
-- Porting note: use `∃ i, p i ∧ _` instead of `∃ i (hi : p i), _`.
lemma HasBasis.le_iff (hl : l.HasBasis p s) : l ≤ l' ↔ ∀ t ∈ l', ∃ i, p i ∧ s i ⊆ t := by
...
protected lemma HasBasis.ker (h : HasBasis l p s) : l.ker = ⋂ (i) (_ : p i), s i :=
...
protected lemma HasAntitoneBasis.map {l : Filter α} {s : ι'' → Set α}
...
protected lemma HasAntitoneBasis.comap {l : Filter α} {s : ι'' → Set α}
...
lemma HasBasis.isCountablyGenerated [Countable ι] {f : Filter α} {p : ι → Prop} {s : ι → Set α}
...
lemma isCountablyGenerated_seq [Countable ι'] (x : ι' → Set α) :
