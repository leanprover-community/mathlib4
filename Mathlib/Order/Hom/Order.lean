...
instance (priority := 10000) [SemilatticeSup β] : Sup (α →o β) where
...
instance (priority := 10000) [SemilatticeSup β] : SemilatticeSup (α →o β) :=
...
instance (priority := 10000) [SemilatticeInf β] : Inf (α →o β) where
...
instance (priority := 10000) [SemilatticeInf β] : SemilatticeInf (α →o β) :=
  { (_ : PartialOrder (α →o β)), (dualIso α β).symm.toGaloisInsertion.liftSemilatticeInf with
    inf := (· ⊓ ·) }

instance (priority := 10000) lattice [Lattice β] : Lattice (α →o β) :=
  { (_ : SemilatticeSup (α →o β)), (_ : SemilatticeInf (α →o β)) with }

@[simps]
instance (priority := 10000) [Preorder β] [OrderBot β] : Bot (α →o β) where
  bot := const α ⊥

instance (priority := 10000) orderBot [Preorder β] [OrderBot β] : OrderBot (α →o β) where
  bot := ⊥
  bot_le _ _ := bot_le

@[simps]
instance (priority := 10000) instTopOrderHom [Preorder β] [OrderTop β] : Top (α →o β) where
  top := const α ⊤

instance (priority := 10000) orderTop [Preorder β] [OrderTop β] : OrderTop (α →o β) where
  top := ⊤
  le_top _ _ := le_top

instance (priority := 10000) [CompleteLattice β] : InfSet (α →o β) where
...
instance (priority := 10000) [CompleteLattice β] : SupSet (α →o β) where
...
instance (priority := 10000) [CompleteLattice β] : CompleteLattice (α →o β) :=
