...
structure CircularOrderHom (α β : Type _) [Btw α] [Btw β] where
  toFun : α → β
...
class CircularOrderHomClass (F : Type _) (α β : outParam (Type _)) [Btw α] [Btw β]
    extends FunLike F α fun _ ↦ β where
  btw_apply (f : F) {a b c : α} : btw a b c → btw (f a) (f b) (f c)

export CircularOrderHomClass (btw_apply)

instance {α β : Type _} [Btw α] [Btw β] : CircularOrderHomClass (α →co β) α β where
  coe f := f.toFun
...
variable [Btw α] [Btw β] [Btw γ] [Btw δ]

@[simps (config := .asFn)]
protected def id (α : Type _) [Btw α] : α →co α := ⟨id, id⟩
...
structure CircularOrderEmbedding (α β : Type _) [Btw α] [Btw β] extends α ↪ β where
...
class CircularOrderEmbeddingClass (F : Type _) (α β : outParam (Type _)) [Btw α] [Btw β]
    extends EmbeddingLike F α β where
...
structure CircularOrderIso (α β : Type _) [Btw α] [Btw β] extends α ≃ β where
  btw_apply_iff' {a b c : α} : btw (toEquiv a) (toEquiv b) (toEquiv c) ↔ btw a b c

class CircularOrderIsoClass (F : Type _) (α β : outParam (Type _)) [Btw α] [Btw β]
