...
instance (priority := 10000) max_idem : Std.IdempotentOp (α := α) max where
  idempotent := by simp
#align max_idem max_idem

-- short-circuit type class inference
/-- An instance asserting that `min a a = a` -/
instance (priority := 10000) min_idem : Std.IdempotentOp (α := α) min where
  idempotent := by simp
...
instance (priority := 10000) : Std.Commutative (α := α) max where
  comm := max_comm

instance (priority := 10000) : Std.Associative (α := α) max where
  assoc := max_assoc
...
instance (priority := 10000) : Std.Commutative (α := α) min where
  comm := min_comm

instance (priority := 10000) : Std.Associative (α := α) min where
  assoc := min_assoc
