...
instance [HasCompl α] : HasCompl (ULift.{v} α) where compl x := fast_instance% up <| x.downᶜ

@[simp] theorem up_compl [HasCompl α] (a : α) : up (aᶜ) = (up a)ᶜ := rfl
@[simp] theorem down_compl [HasCompl α] (a : ULift α) : down aᶜ = (down a)ᶜ := rfl

instance [Preorder α] : Preorder (ULift.{v} α) := fast_instance%
  Preorder.lift ULift.down

instance [PartialOrder α] : PartialOrder (ULift.{v} α) := fast_instance%
