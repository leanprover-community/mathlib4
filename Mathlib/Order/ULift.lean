...
instance (priority := 10000) [HasCompl α] : HasCompl (ULift.{v} α) where compl x := up <| x.downᶜ

@[simp] theorem up_compl [HasCompl α] (a : α) : up (aᶜ) = (up a)ᶜ := rfl
@[simp] theorem down_compl [HasCompl α] (a : ULift α) : down aᶜ = (down a)ᶜ := rfl

instance (priority := 10000) [Preorder α] : Preorder (ULift.{v} α) :=
  Preorder.lift ULift.down

instance (priority := 10000) [PartialOrder α] : PartialOrder (ULift.{v} α) :=
