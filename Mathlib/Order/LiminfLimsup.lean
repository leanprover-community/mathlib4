...
lemma bliminf_eq_liminf {f : Filter β} {u : β → α} {p : β → Prop} :
    bliminf u f p = liminf u (f ⊓ 𝓟 {x | p x}) := by
  simp only [bliminf_eq, liminf_eq, eventually_inf_principal, mem_setOf_eq]
...
theorem bliminf_eq_liminf_subtype {f : Filter β} {u : β → α} {p : β → Prop} :
    bliminf u f p = liminf (u ∘ ((↑) : { x | p x } → β)) (comap (↑) f) := by
  rw [bliminf_eq_liminf, liminf, liminf, ← map_map, map_comap_setCoe_val]
...
theorem liminf_le_liminf {α : Type*} [ConditionallyCompleteLattice β] {f : Filter α} {u v : α → β}
  ...
  limsInf_le_limsInf hu hv fun a ha =>
    show ∀ᶠ n in map v f, a ≤ n from
    eventually_map.mpr ((eventually_map.mp ha).and h |>.mono fun _ ⟨hau, huv⟩ => hau.trans huv)
...
lemma limsInf_principal_eq_csSup (h : BddBelow s) (hs : s.Nonempty) : limsInf (𝓟 s) = sInf s := by
  simp only [limsInf, eventually_principal]; exact csSup_lowerBounds_eq_csInf h hs
...
theorem bliminf_congr {f : Filter β} {u v : β → α} {p : β → Prop} (h : ∀ᶠ a in f, p a → u a = v a) :
    bliminf u f p = bliminf v f p := by
  simp only [bliminf_eq]
  congr with b
  exact eventually_congr (h.mono fun x hx => by
    constructor <;> intro ha hp <;> [rw [← hx hp]; rw [hx hp]] <;> exact ha hp)

theorem liminf_congr {α : Type*} [ConditionallyCompleteLattice β] {f : Filter α} {u v : α → β}
    (h : ∀ᶠ a in f, u a = v a) : liminf u f = liminf v f := by
  rw [liminf_eq]; congr with b
  exact eventually_congr (h.mono fun x hx => by simp [hx])
...
theorem liminf_const {α : Type*} [ConditionallyCompleteLattice β] {f : Filter α} [NeBot f]
    (b : β) : liminf (fun _ => b) f = b := by
  simpa only [liminf_eq, eventually_const] using csSup_Iic
...
theorem HasBasis.limsup_eq_sInf_iUnion_iInter {ι ι' : Type*} {f : ι → α} {v : Filter ι}
    {p : ι' → Prop} {s : ι' → Set ι} (hv : v.HasBasis p s) :
    limsup f v = sInf (⋃ (j : Subtype p), ⋂ (i : s j), Ici (f i)) := by
  simp_rw [limsup_eq, hv.eventually_iff]
  congr 1
  ext x
  simp only [mem_setOf_eq, iInter_coe_set, mem_iUnion, mem_iInter, mem_Ici, Subtype.exists,
    exists_prop]

theorem HasBasis.limsup_eq_sInf_univ_of_empty {f : ι → α} {v : Filter ι}
    {p : ι' → Prop} {s : ι' → Set ι} (hv : v.HasBasis p s) (i : ι') (hi : p i) (h'i : s i = ∅) :
    limsup f v = sInf univ := by
  simp [hv.eq_bot_iff.2 ⟨i, hi, h'i⟩, limsup_eq]
...
@[simp]
theorem limsup_nat_add (f : ℕ → α) (k : ℕ) :
    limsup (fun i => f (i + k)) atTop = limsup f atTop := by
  rw [← Function.comp_def, limsup, limsup, ← map_map, map_add_atTop_eq_nat]
...
/-- Same as limsup_const applied to `⊤` but without the `NeBot f` assumption -/
@[simp]
theorem liminf_const_top {f : Filter β} : liminf (fun _ : β => (⊤ : α)) f = (⊤ : α) := by
  rw [liminf_eq, eq_top_iff]
  exact le_sSup (Eventually.of_forall fun _ => le_rfl)
...
theorem HasBasis.limsInf_eq_iSup_sInf {p : ι → Prop} {s : ι → Set α} {f : Filter α}
  ...
  le_antisymm
    (sSup_le fun _ ha =>
      let ⟨_, hi, ha⟩ := h.eventually_iff.1 ha
      le_iSup₂_of_le _ hi <| le_sInf ha)
    (iSup₂_le fun i hi => le_sSup <| h.eventually_iff.2 ⟨i, hi, fun _ => sInf_le⟩)
...
theorem limsInf_eq_iSup_sInf {f : Filter α} : limsInf f = ⨆ s ∈ f, sInf s :=
  f.basis_sets.limsInf_eq_iSup_sInf
...
theorem bliminf_congr' {f : Filter β} {p q : β → Prop} {u : β → α}
    (h : ∀ᶠ x in f, u x ≠ ⊤ → (p x ↔ q x)) : bliminf u f p = bliminf u f q := by
  simp only [bliminf_eq]
  congr with a
  refine eventually_congr (h.mono fun b hb => ?_)
  rcases eq_or_ne (u b) ⊤ with hu | hu; · simp [hu]
...
theorem liminf_eq_iSup_iInf {f : Filter β} {u : β → α} : liminf u f = ⨆ s ∈ f, ⨅ a ∈ s, u a :=
  (f.basis_sets.map u).limsInf_eq_iSup_sInf.trans <| by simp only [sInf_image, id]

theorem liminf_eq_iSup_iInf_of_nat {u : ℕ → α} : liminf u atTop = ⨆ n : ℕ, ⨅ i ≥ n, u i :=
  (atTop_basis.map u).limsInf_eq_iSup_sInf.trans <| by simp only [sInf_image, iSup_const]; rfl

theorem liminf_eq_iSup_iInf_of_nat' {u : ℕ → α} : liminf u atTop = ⨆ n : ℕ, ⨅ i : ℕ, u (i + n) := by
  simp only [liminf_eq_iSup_iInf_of_nat, iInf_ge_eq_iInf_nat_add]

theorem HasBasis.liminf_eq_iSup_iInf {p : ι → Prop} {s : ι → Set β} {f : Filter β} {u : β → α}
    (h : f.HasBasis p s) : liminf u f = ⨆ (i) (_ : p i), ⨅ a ∈ s i, u a :=
  (h.map u).limsInf_eq_iSup_sInf.trans <| by simp only [sInf_image]

lemma HasBasis.bliminf_eq_iSup_iInf {p : ι → Prop} {s : ι → Set β} {f : Filter β} {u : β → α}
    (hf : f.HasBasis p s) {q : β → Prop} :
    bliminf u f q = ⨆ (i) (_ : p i), ⨅ a ∈ s i, ⨅ (_ : q a), u a := by
  simp only [bliminf_eq_liminf, (hf.inf_principal _).liminf_eq_iSup_iInf, mem_inter_iff, iInf_and,
    mem_setOf_eq]

theorem bliminf_eq_iSup_biInf {f : Filter β} {p : β → Prop} {u : β → α} :
    bliminf u f p = ⨆ s ∈ f, ⨅ (b) (_ : p b ∧ b ∈ s), u b := by
  simp only [f.basis_sets.bliminf_eq_iSup_iInf, iInf_and', id, and_comm]

theorem bliminf_eq_iSup_biInf_of_nat {p : ℕ → Prop} {u : ℕ → α} :
    bliminf u atTop p = ⨆ i, ⨅ (j) (_ : p j ∧ i ≤ j), u j := by
  simp only [atTop_basis.bliminf_eq_iSup_iInf, @and_comm (p _), iInf_and, mem_Ici, iSup_true]
...
theorem liminf_eq_sSup_sInf {ι R : Type*} (F : Filter ι) [CompleteLattice R] (a : ι → R) :
    liminf a F = sSup ((fun I => sInf (a '' I)) '' F.sets) := by
  apply le_antisymm
  · rw [liminf_eq]
    apply sSup_le fun b hb => ?_
    exact le_sSup_of_le (mem_image_of_mem _ hb)
      (le_sInf fun _ ⟨_, hn, he⟩ => he ▸ hn)
  · refine sSup_le fun b hb => ?_
    rcases (mem_image _ F.sets b).mp hb with ⟨I, ⟨I_mem_F, hI⟩⟩
    rw [liminf_eq]
    refine le_sSup ?_
    filter_upwards [I_mem_F] with i hi
    exact hI ▸ sInf_le (mem_image_of_mem _ hi)
...
theorem le_limsup_of_frequently_le' {α β} [CompleteLattice β] {f : Filter α} {u : α → β} {x : β}
    (h : ∃ᶠ a in f, x ≤ u a) : x ≤ limsup u f := by
  rw [limsup_eq]
  refine le_sInf fun b hb => ?_
  have hbx : ∃ᶠ _ in f, x ≤ b := by
    contrapose! h
    exact hb.mp (h.mono fun a hbx hba hax => hbx (hax.trans hba))
  exact hbx.exists.choose_spec
...
theorem _root_.CompleteLatticeHom.apply_liminf_iterate (f : CompleteLatticeHom α α) (a : α) :
    f (liminf (fun n => f^[n] a) atTop) = liminf (fun n => f^[n] a) atTop := by
  rw [liminf_eq_iSup_iInf_of_nat', _root_.map_iSup]
  simp_rw [_root_.map_iInf, ← Function.comp_apply (f := f), ← Function.iterate_succ' f,
    ← Nat.add_succ]
  conv_rhs => rw [iSup_split _ (0 < ·)]
  simp only [not_lt, Nat.le_zero, iSup_iSup_eq_left, add_zero, iSup_nat_gt_zero_eq,
    Nat.succ_eq_add_one]
  rw [eq_comm, sup_eq_left]
  refine le_trans ?_ (le_iSup (fun i => ⨅ j, f^[j + (i + 1)] a) 0)
  simp only [zero_add, le_iInf_iff]
  exact fun i => iInf_le (fun i => f^[i] a) (i + 1)
...
theorem bliminf_sup_le_and : bliminf u f p ⊔ bliminf u f q ≤ bliminf u f fun x => p x ∧ q x :=
  sup_le (bliminf_antitone <| by tauto) (bliminf_antitone <| by tauto)
...
theorem bliminf_or_le_inf : (bliminf u f fun x => p x ∨ q x) ≤ bliminf u f p ⊓ bliminf u f q :=
  le_inf (bliminf_antitone <| by tauto) (bliminf_antitone <| by tauto)
...
theorem _root_.OrderIso.apply_bliminf [CompleteLattice γ] (e : α ≃o γ) :
    e (bliminf u f p) = bliminf (e ∘ u) f p := by
  simp only [bliminf_eq, map_sSup, Function.comp_apply, e.image_eq_preimage_symm,
    Set.preimage_setOf_eq, e.symm_apply_le]
...
theorem _root_.sInfHom.le_apply_bliminf [CompleteLattice γ] (g : sInfHom α γ) :
    bliminf (g ∘ u) f p ≤ g (bliminf u f p) := by
  simp only [bliminf_eq_iSup_biInf, Function.comp]
  refine le_trans ?_ ((OrderHomClass.mono g).le_map_iSup₂ _)
  simp only [map_iInf, le_refl]
...
lemma liminf_sup_filter {g} : liminf u (f ⊔ g) = liminf u f ⊓ liminf u g := by
  refine le_antisymm
    (le_inf (liminf_le_liminf_of_le le_sup_left) (liminf_le_liminf_of_le le_sup_right)) ?_
  simp_rw [liminf_eq, sSup_inf_eq, inf_sSup_eq, mem_setOf_eq, iSup₂_le_iff]
  intro a ha b hb
  exact le_sSup ⟨ha.mono fun _ h ↦ inf_le_left.trans h, hb.mono fun _ h ↦ inf_le_right.trans h⟩
...
@[simp]
theorem bliminf_or_eq_inf : (bliminf u f fun x => p x ∨ q x) = bliminf u f p ⊓ bliminf u f q := by
  simp only [bliminf_eq_liminf, ← liminf_sup_filter, ← inf_sup_left, sup_principal, setOf_or]
...
@[simp]
lemma bliminf_inf_not : bliminf u f p ⊓ bliminf u f (¬p ·) = liminf u f := by
  simp_rw [← bliminf_or_eq_inf, or_not, bliminf_true]
...
@[simp]
lemma bliminf_not_inf : bliminf u f (¬p ·) ⊓ bliminf u f p = liminf u f := by
  simpa only [not_not] using bliminf_inf_not (p := (¬p ·))
...
lemma liminf_piecewise {s : Set β} [DecidablePred (· ∈ s)] {v} :
    liminf (s.piecewise u v) f = bliminf u f (· ∈ s) ⊓ bliminf v f (· ∉ s) := by
  rw [← bliminf_inf_not (p := (· ∈ s))]
  refine congr_arg₂ _ (bliminf_congr ?_) (bliminf_congr ?_) <;>
    filter_upwards with _ h using by simp [h]
...
theorem inf_liminf [NeBot f] (a : α) : a ⊓ liminf u f = liminf (fun x => a ⊓ u x) f := by
  simp only [liminf_eq_iSup_iInf, iInf_inf_eq, inf_iSup₂_eq]
  congr; ext s; congr; ext hs; congr
  exact (biInf_const (nonempty_of_mem hs)).symm
...
theorem inf_limsup (a : α) : a ⊓ limsup u f = limsup (fun x => a ⊓ u x) f := by
  simp only [limsup_eq_iInf_iSup]
  rw [inf_comm, biInf_inf (⟨univ, univ_mem⟩ : ∃ i : Set β, i ∈ f)]
  simp_rw [iSup₂_inf_eq, inf_comm (a := a)]
...
theorem frequently_lt_of_limsInf_lt {f : Filter α} [ConditionallyCompleteLinearOrder α] {a : α}
    (hf : f.IsCobounded (· ≥ ·) := by isBoundedDefault)
    (h : limsInf f < a) : ∃ᶠ n in f, n < a := by
  contrapose! h
  exact le_limsInf_of_le hf h
...
theorem eventually_lt_of_limsup_lt {f : Filter α} [ConditionallyCompleteLinearOrder β] {u : α → β}
    ...
    (hu : f.IsBoundedUnder (· ≤ ·) u := by isBoundedDefault) :
    ∀ᶠ a in f, u a < b := by
  obtain ⟨c, hc, hcb⟩ : ∃ (c : β) (_ : c ∈ { c : β | ∀ᶠ n : α in f, u n ≤ c }), c < b := by
    simp_rw [exists_prop]
    exact exists_lt_of_csInf_lt hu h
  exact hc.mono fun x hx => lt_of_le_of_lt hx hcb
...
theorem frequently_lt_of_liminf_lt {b : β}
    (hu : f.IsCoboundedUnder (· ≥ ·) u := by isBoundedDefault)
    (h : liminf u f < b) : ∃ᶠ x in f, u x < b := by
  contrapose! h
  apply le_limsInf_of_le hu
  simpa using h
...
theorem le_liminf_iff {x : β} (h₁ : f.IsCoboundedUnder (· ≥ ·) u := by isBoundedDefault)
    (h₂ : f.IsBoundedUnder (· ≥ ·) u := by isBoundedDefault) :
    x ≤ liminf u f ↔ ∀ y < x, ∀ᶠ a in f, y < u a := by
  refine ⟨fun h _ h' ↦ eventually_lt_of_lt_liminf (h'.trans_le h) h₂, fun h ↦ ?_⟩
  by_cases h' : ∀ y < x, ∃ z, y < z ∧ z < x
  · rw [← forall_lt_iff_le]
    intro y y_x
    rcases h' y y_x with ⟨z, y_z, z_x⟩
    exact y_z.trans_le (le_liminf_of_le h₁ ((h z z_x).mono (fun _ ↦ le_of_lt)))
  · apply le_liminf_of_le h₁
    push_neg +distrib at h'
    rcases h' with ⟨z, z_x, hz⟩
    exact (h z z_x).mono <| fun w hw ↦ (hz (u w)).resolve_left (not_le_of_gt hw)
...
theorem le_liminf_iff' [DenselyOrdered β] {x : β}
    ...
    (h₂ : f.IsBoundedUnder (· ≥ ·) u := by isBoundedDefault) :
    x ≤ liminf u f ↔ ∀ y < x, ∀ᶠ a in f, y ≤ u a := by
  refine ⟨fun h _ h' ↦ (eventually_lt_of_lt_liminf (h'.trans_le h) h₂).mono fun _ ↦ le_of_lt, ?_⟩
  rw [← forall_lt_iff_le]
  intro h y y_x
  obtain ⟨z, y_z, z_x⟩ := exists_between y_x
  exact y_z.trans_le (le_liminf_of_le h₁ (h z z_x))

theorem liminf_le_iff {x : β} (h₁ : f.IsCoboundedUnder (· ≥ ·) u := by isBoundedDefault)
    (h₂ : f.IsBoundedUnder (· ≥ ·) u := by isBoundedDefault) :
    liminf u f ≤ x ↔ ∀ y > x, ∃ᶠ a in f, u a < y := by
  refine ⟨fun h _ h' ↦ frequently_lt_of_liminf_lt h₁ (h.trans_lt h'), fun h ↦ ?_⟩
  by_cases h' : ∀ y > x, ∃ z, x < z ∧ z < y
  · rw [← forall_gt_iff_le]
    intro y x_y
    rcases h' y x_y with ⟨z, x_z, z_y⟩
    exact (liminf_le_of_frequently_le ((h z x_z).mono (fun _ ↦ le_of_lt)) h₂).trans_lt z_y
  · apply liminf_le_of_frequently_le _ h₂
    push_neg +distrib at h'
    rcases h' with ⟨z, x_z, hz⟩
    exact (h z x_z).mono <| fun w hw ↦ (hz (u w)).resolve_right (not_le_of_gt hw)
...
theorem liminf_le_iff' [DenselyOrdered β] {x : β}
    ...
    (h₂ : f.IsBoundedUnder (· ≥ ·) u := by isBoundedDefault) :
    liminf u f ≤ x ↔ ∀ y > x, ∃ᶠ a in f, u a ≤ y := by
  refine ⟨fun h _ h' ↦ (frequently_lt_of_liminf_lt h₁ (h.trans_lt h')).mono fun _ ↦ le_of_lt, ?_⟩
  rw [← forall_gt_iff_le]
  intro h y x_y
  obtain ⟨z, x_z, z_y⟩ := exists_between x_y
  exact (liminf_le_of_frequently_le (h z x_z) h₂).trans_lt z_y
...
-- The linter erroneously claims that I'm not referring to `c`
set_option linter.unusedVariables false in
theorem gt_mem_sets_of_limsInf_gt (h : f.IsBounded (· ≥ ·)) (l : b < f.limsInf) :
    ∀ᶠ a in f, b < a :=
  let ⟨c, (hc : ∀ᶠ a in f, c ≤ a), hcb⟩ := exists_lt_of_lt_csSup h l
  mem_of_superset hc fun _a => hcb.trans_le
...
noncomputable def limsup_reparam
    (f : ι → α) (s : ι' → Set ι) (p : ι' → Prop) [Countable (Subtype p)] [Nonempty (Subtype p)]
    (j : Subtype p) : Subtype p :=
  let m : Set (Subtype p) := {j | BddAbove (range fun (i : s j) ↦ f i)}
  let g : ℕ → Subtype p := (exists_surjective_nat _).choose
  have Z : ∃ n, g n ∈ m ∨ ∀ j, j ∉ m := by
    by_cases H : ∃ j, j ∈ m
    · rcases H with ⟨j, hj⟩
...
theorem HasBasis.limsup_eq_ciInf_ciSup {v : Filter ι}
    ...
    limsup f v = ⨅ (j : Subtype p), ⨆ (i : s (limsup_reparam f s p j)), f i := by
  classical
  rcases H with ⟨j0, hj0⟩
  let m : Set (Subtype p) := {j | BddAbove (range (fun (i : s j) ↦ f i))}
  have : ∀ (j : Subtype p), Nonempty (s j) := fun j ↦ Nonempty.coe_sort (hs j)
  have A : ⋃ (j : Subtype p), ⋂ (i : s j), Ici (f i) =
         ⋃ (j : Subtype p), ⋂ (i : s (limsup_reparam f s p j)), Ici (f i) := by
    apply Subset.antisymm
    · apply iUnion_subset (fun j ↦ ?_)
      by_cases hj : j ∈ m
      · have : j = limsup_reparam f s p j := by
          unfold limsup_reparam
          rw [if_pos hj]
        conv_lhs => rw [this]
        apply subset_iUnion _ j
      · simp only [m, mem_setOf_eq, ← nonempty_iInter_Ici_iff, not_nonempty_iff_eq_empty] at hj
        simp only [hj, empty_subset]
    · apply iUnion_subset (fun j ↦ ?_)
      exact subset_iUnion (fun (k : Subtype p) ↦ (⋂ (i : s k), Ici (f i)))
        (limsup_reparam f s p j)
  have B : ∀ (j : Subtype p), ⋂ (i : s (limsup_reparam f s p j)), Ici (f i) =
                                Ici (⨆ (i : s (limsup_reparam f s p j)), f i) := by
    intro j
    apply (Ici_ciSup _).symm
    change limsup_reparam f s p j ∈ m
    by_cases Hj : j ∈ m
    · have : limsup_reparam f s p j = j := by unfold limsup_reparam; rw [if_pos Hj]
      rw [this]; exact Hj
    · -- When j ∉ m, limsup_reparam returns g (Nat.find Z) which is in m (if m is nonempty)
      show limsup_reparam f s p j ∈ m
      unfold limsup_reparam
      rw [if_neg Hj]
      have Z : ∃ n, (exists_surjective_nat (Subtype p)).choose n ∈ m ∨ ∀ j, j ∉ m := by
        rcases (exists_surjective_nat (Subtype p)).choose_spec j0 with ⟨n, rfl⟩
      ...
      · exact hZ
      · exact (hZ j0 hj0).elim
  simp_rw [hv.limsup_eq_sInf_iUnion_iInter, A, B, sInf_iUnion_Ici]
...
theorem HasBasis.limsup_eq_ite {v : Filter ι} {p : ι' → Prop} {s : ι' → Set ι}
    ...
    limsup f v = if ∃ (j : Subtype p), s j = ∅ then sInf univ else
      ...
      else ⨅ (j : Subtype p), ⨆ (i : s (limsup_reparam f s p j)), f i := by
  by_cases H : ∃ (j : Subtype p), s j = ∅
  · rw [if_pos H]
    rcases H with ⟨j, hj⟩
    simp [hv.limsup_eq_sInf_univ_of_empty j j.2 hj]
  rw [if_neg H]
  by_cases H' : ∀ (j : Subtype p), ¬BddAbove (range (fun (i : s j) ↦ f i))
  · have A : ∀ (j : Subtype p), ⋂ (i : s j), Ici (f i) = ∅ := by
      simp_rw [← not_nonempty_iff_eq_empty, nonempty_iInter_Ici_iff]
      exact H'
    simp_rw [if_pos H', hv.limsup_eq_sInf_iUnion_iInter, A, iUnion_empty]
  rw [if_neg H']
  apply hv.limsup_eq_ciInf_ciSup
  · push_neg at H
    simpa only [nonempty_iff_ne_empty] using H
  · push_neg at H'
    exact H'
...
theorem OrderIso.liminf_apply {γ} [ConditionallyCompleteLattice β] [ConditionallyCompleteLattice γ]
  ...
  have hbdd : BddAbove {a | ∀ᶠ n in f, a ≤ u n} := hu_co
  rw [g.map_csSup' hne hbdd]
  congr 1
  ext b
  simp only [Set.mem_image, Set.mem_setOf_eq]
  constructor
  · rintro ⟨a, ha, rfl⟩
    exact ha.mono fun n hn => g.monotone hn
  · intro hb
    exact ⟨g.symm b, hb.mono fun n hn => by rwa [← g.le_iff_le, g.apply_symm_apply],
      g.apply_symm_apply b⟩
...
theorem liminf_min [ConditionallyCompleteLinearOrder β] {f : Filter α} {u v : α → β}
  ...
  have bddmin := IsBoundedUnder.inf h₃ h₄
  have cobddmin : f.IsCoboundedUnder (· ≥ ·) (fun a ↦ min (u a) (v a)) := by
    rcases h₁ with ⟨b, hb⟩
    exact ⟨b, fun c hc => hb c (eventually_map.mpr
      ((eventually_map.mp hc).mono fun _ h => le_trans h (min_le_left _ _)))⟩
  apply le_antisymm
  · exact le_min (c := liminf (fun a ↦ min (u a) (v a)) f)
      (liminf_le_liminf (Eventually.of_forall (fun a : α ↦ min_le_left (u a) (v a))) bddmin h₁)
      (liminf_le_liminf (Eventually.of_forall (fun a : α ↦ min_le_right (u a) (v a))) bddmin h₂)
  · refine (le_liminf_iff cobddmin bddmin).2 (fun b hb ↦ ?_)
    have hu := eventually_lt_of_lt_liminf (lt_of_lt_of_le hb (min_le_left _ _)) h₃
    have hv := eventually_lt_of_lt_liminf (lt_of_lt_of_le hb (min_le_right _ _)) h₄
    refine mem_of_superset (inter_mem hu hv) (fun _ ↦ by simp)
...
theorem liminf_finset_inf' [ConditionallyCompleteLinearOrder β] {f : Filter α}
    ...
    (h₁ : ∀ i ∈ s, f.IsCoboundedUnder (· ≥ ·) (F i) := by exact fun _ _ ↦ by isBoundedDefault)
    (h₂ : ∀ i ∈ s, f.IsBoundedUnder (· ≥ ·) (F i) := by exact fun _ _ ↦ by isBoundedDefault) :
    liminf (fun a ↦ inf' s hs (fun i ↦ F i a)) f = inf' s hs (fun i ↦ liminf (F i) f) := by
  have bddinf := isBoundedUnder_ge_finset_inf' hs h₂
  apply le_antisymm
  · refine Finset.le_inf' hs (fun i ↦ liminf (F i) f) fun i i_s ↦ ?_
    exact liminf_le_liminf (Eventually.of_forall (fun a ↦ Finset.inf'_le _ i_s)) bddinf (h₁ i i_s)
  · have h₃ : ∃ i ∈ s, f.IsCoboundedUnder (· ≥ ·) (F i) := by
      rcases hs with ⟨i, i_s⟩
      use i, i_s
      exact h₁ i i_s
    have cobddinf := isCoboundedUnder_ge_finset_inf' hs h₃
    refine (le_liminf_iff cobddinf bddinf).2 (fun b hb ↦ ?_)
    simp only [lt_inf'_iff, eventually_all_finset] at hb ⊢
    exact fun i i_s ↦ eventually_lt_of_lt_liminf (hb i i_s) (h₂ i i_s)

theorem liminf_finset_inf [ConditionallyCompleteLinearOrder β] [OrderTop β] {f : Filter α}
    {F : ι → α → β} {s : Finset ι}
    (h₁ : ∀ i ∈ s, f.IsCoboundedUnder (· ≥ ·) (F i) := by exact fun _ _ ↦ by isBoundedDefault)
    (h₂ : ∀ i ∈ s, f.IsBoundedUnder (· ≥ ·) (F i) := by exact fun _ _ ↦ by isBoundedDefault) :
    liminf (fun a ↦ inf s (fun i ↦ F i a)) f = inf s (fun i ↦ liminf (F i) f) := by
  rcases eq_or_neBot f with (rfl | _)
  · simp [liminf_eq, isGreatest_univ.csSup_eq]
  rcases Finset.eq_empty_or_nonempty s with (rfl | s_nemp)
  · simp only [inf_empty, liminf_const]
  rw [← Finset.inf'_eq_inf s_nemp fun i ↦ liminf (F i) f, ← liminf_finset_inf' s_nemp h₁ h₂]
  congr
  ext a
  exact Eq.symm (Finset.inf'_eq_inf s_nemp (fun i ↦ F i a))
