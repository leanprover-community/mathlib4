...
@[simp]
lemma parts_map {β : Type*} [Lattice β] [OrderBot β] {a : α} {e : α ≃o β} {P : Finpartition a} :
...
lemma parts_top_subset (a : α) [Decidable (a = ⊥)] : (⊤ : Finpartition a).parts ⊆ {a} := by
  intro b hb
...
@[simp]
lemma parts_inf (P Q : Finpartition a) :
...
lemma card_bind (Q : ∀ i ∈ P.parts, Finpartition i) :
...
lemma exists_mem (ha : a ∈ s) : ∃ t ∈ P.parts, a ∈ t := by
...
lemma existsUnique_mem (ha : a ∈ s) : ∃! t, t ∈ P.parts ∧ a ∈ t := by
...
lemma part_surjOn : Set.SurjOn P.part s P.parts := fun p hp ↦ by
...
lemma exists_subset_part_bijOn : ∃ r ⊆ s, Set.BijOn P.part r P.parts := by
...
lemma card_parts_le_card (P : Finpartition s) : P.parts.card ≤ s.card := by
  rw [← card_bot s]
...
lemma mem_part_ofSetoid_iff_rel {s : Setoid α} [DecidableRel s.r] {b : α} :
...
lemma atomise_empty (hs : s.Nonempty) : (atomise s ∅).parts = {s} := by
...
lemma biUnion_filter_atomise (ht : t ∈ F) (hts : t ⊆ s) :
