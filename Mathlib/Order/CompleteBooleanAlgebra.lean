...
theorem iSup_inf_of_monotone {ι : Type*} [Preorder ι] [IsDirectedLE ι] {f g : ι → α}
...
theorem iSup_inf_of_antitone {ι : Type*} [Preorder ι] [IsDirectedGE ι] {f g : ι → α}
...
theorem iInf_sup_of_monotone {ι : Type*} [Preorder ι] [IsDirectedGE ι] {f g : ι → α}
    (hf : Monotone f) (hg : Monotone g) : ⨅ i, f i ⊔ g i = (⨅ i, f i) ⊔ ⨅ i, g i :=
  @iSup_inf_of_antitone αᵒᵈ _ _ _ _ _ _ hf.dual_right hg.dual_right

theorem iInf_sup_of_antitone {ι : Type*} [Preorder ι] [IsDirectedLE ι] {f g : ι → α}
    (hf : Antitone f) (hg : Antitone g) : ⨅ i, f i ⊔ g i = (⨅ i, f i) ⊔ ⨅ i, g i :=
