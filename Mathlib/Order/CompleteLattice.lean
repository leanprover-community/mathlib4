...
@[simp]
theorem to_dual_Sup (s : Set α) : toDual (supₛ s) = infₛ (of_dual ⁻¹' s) :=
  rfl
#align to_dual_Sup to_dual_Sup

@[simp]
theorem to_dual_Inf (s : Set α) : toDual (infₛ s) = supₛ (of_dual ⁻¹' s) :=
  rfl
#align to_dual_Inf to_dual_Inf

@[simp]
theorem of_dual_Sup (s : Set αᵒᵈ) : ofDual (supₛ s) = infₛ (to_dual ⁻¹' s) :=
  rfl
#align of_dual_Sup of_dual_Sup

@[simp]
theorem of_dual_Inf (s : Set αᵒᵈ) : ofDual (infₛ s) = supₛ (to_dual ⁻¹' s) :=
  rfl
...
theorem Inf_le_Inf_of_subset_insert_top (h : s ⊆ insert ⊤ t) : infₛ t ≤ infₛ s :=
...
@[simp]
theorem Sup_diff_singleton_bot (s : Set α) : supₛ (s \ {⊥}) = supₛ s :=
...
theorem eq_singleton_top_of_Inf_eq_top_of_nonempty : infₛ s = ⊤ → s.Nonempty → s = {⊤} :=
...
theorem Inf_eq_bot : infₛ s = ⊥ ↔ ∀ b > ⊥, ∃ a ∈ s, a < b :=
  @Sup_eq_top αᵒᵈ _ _
#align Inf_eq_bot Inf_eq_bot

theorem lt_supr_iff {f : ι → α} : a < supᵢ f ↔ ∃ i, a < f i :=
...
theorem Sup_range : supₛ (range f) = supᵢ f :=
  rfl
#align Sup_range Sup_range

theorem Sup_eq_supr' (s : Set α) : supₛ s = ⨆ a : s, a := by rw [supᵢ, Subtype.range_coe]
...
theorem Sup_image' {s : Set β} {f : β → α} : supₛ (f '' s) = ⨆ a : s, f a := by
...
theorem Inf_range : infₛ (range f) = infi f :=
  rfl
#align Inf_range Inf_range

theorem Inf_eq_infi' (s : Set α) : infₛ s = ⨅ a : s, a :=
  @Sup_eq_supr' αᵒᵈ _ _
...
theorem Inf_image' {s : Set β} {f : β → α} : infₛ (f '' s) = ⨅ a : s, f a :=
...
--@[ematch]
theorem le_supr (f : ι → α) (i : ι) : f i ≤ supᵢ f :=
  le_Sup ⟨i, rfl⟩
...
theorem le_supr_of_le (i : ι) (h : a ≤ f i) : a ≤ supᵢ f :=
  h.trans <| le_supr _ i
...
theorem supr_le (h : ∀ i, f i ≤ a) : supᵢ f ≤ a :=
...
theorem supr_mono (h : ∀ i, f i ≤ g i) : supᵢ f ≤ supᵢ g :=
...
theorem supr_mono' {g : ι' → α} (h : ∀ i, ∃ i', f i ≤ g i') : supᵢ f ≤ supᵢ g :=
...
@[simp]
theorem supr_le_iff : supᵢ f ≤ a ↔ ∀ i, f i ≤ a :=
...
theorem supr_lt_iff : supᵢ f < a ↔ ∃ b, b < a ∧ ∀ i, f i ≤ b :=
  ⟨fun h => ⟨supᵢ f, h, le_supr f⟩, fun ⟨b, h, hb⟩ => (supr_le hb).trans_lt h⟩
...
theorem Sup_eq_supr {s : Set α} : supₛ s = ⨆ a ∈ s, a :=
...
theorem Monotone.le_map_Sup [CompleteLattice β] {s : Set α} {f : α → β} (hf : Monotone f) :
    (⨆ a ∈ s, f a) ≤ f (supₛ s) := by rw [Sup_eq_supr] <;> exact hf.le_map_supr₂ _
...
theorem OrderIso.map_Sup [CompleteLattice β] (f : α ≃o β) (s : Set α) : f (supₛ s) = ⨆ a ∈ s, f a :=
  by simp only [Sup_eq_supr, OrderIso.map_supr]
#align order_iso.map_Sup OrderIso.map_Sup

theorem OrderIso.map_Inf [CompleteLattice β] (f : α ≃o β) (s : Set α) : f (infₛ s) = ⨅ a ∈ s, f a :=
...
theorem supr_const [Nonempty ι] : (⨆ b : ι, a) = a := by rw [supᵢ, range_const, Sup_singleton]
...
@[simp]
theorem supr_eq_bot : supᵢ s = ⊥ ↔ ∀ i, s i = ⊥ :=
...
theorem supr_subtype {p : ι → Prop} {f : Subtype p → α} : supᵢ f = ⨆ (i) (h : p i), f ⟨i, h⟩ :=
...
@[simp]
theorem supr_false {s : False → α} : supᵢ s = ⊥ :=
...
theorem supr_true {s : True → α} : supᵢ s = s trivial :=
  supr_pos trivial
...
theorem supr_and {p q : Prop} {s : p ∧ q → α} : supᵢ s = ⨆ (h₁) (h₂), s ⟨h₁, h₂⟩ :=
  le_antisymm (supr_le fun ⟨i, h⟩ => le_supr₂ i h) (supr₂_le fun i h => le_supr _ _)
...
theorem supr_of_empty' {α ι} [SupSet α] [IsEmpty ι] (f : ι → α) : supᵢ f = supₛ (∅ : Set α) :=
  congr_arg supₛ (range_eq_empty f)
#align supr_of_empty' supr_of_empty'

theorem infi_of_empty' {α ι} [InfSet α] [IsEmpty ι] (f : ι → α) : infi f = infₛ (∅ : Set α) :=
  congr_arg infₛ (range_eq_empty f)
#align infi_of_empty' infi_of_empty'

theorem supr_of_empty [IsEmpty ι] (f : ι → α) : supᵢ f = ⊥ :=
...
theorem Sup_image2 {f : β → γ → α} {s : Set β} {t : Set γ} :
    supₛ (image2 f s t) = ⨆ (a ∈ s) (b ∈ t), f a b := by rw [← image_prod, Sup_image, bsupr_prod]
#align Sup_image2 Sup_image2

theorem Inf_image2 {f : β → γ → α} {s : Set β} {t : Set γ} :
    infₛ (image2 f s t) = ⨅ (a ∈ s) (b ∈ t), f a b := by rw [← image_prod, Inf_image, binfi_prod]
...
theorem supr_eq_top (f : ι → α) : supᵢ f = ⊤ ↔ ∀ b < ⊤, ∃ i, b < f i := by
...
@[simp]
theorem Sup_Prop_eq {s : Set Prop} : supₛ s = ∃ p ∈ s, p :=
  rfl
#align Sup_Prop_eq Sup_Prop_eq

@[simp]
theorem Inf_Prop_eq {s : Set Prop} : infₛ s = ∀ p ∈ s, p :=
  rfl
...
@[simp]
theorem supr_apply {α : Type _} {β : α → Type _} {ι : Sort _} [∀ i, SupSet (β i)] {f : ι → ∀ a, β a}
...
@[simp]
theorem infi_apply {α : Type _} {β : α → Type _} {ι : Sort _} [∀ i, InfSet (β i)] {f : ι → ∀ a, β a}
    {a : α} : (⨅ i, f i) a = ⨅ i, f i a :=
...
theorem monotone_Sup_of_monotone {s : Set (α → β)} (m_s : ∀ f ∈ s, Monotone f) :
    Monotone (supₛ s) := fun x y h => supr_mono fun f => m_s f f.2 h
#align monotone_Sup_of_monotone monotone_Sup_of_monotone

theorem monotone_Inf_of_monotone {s : Set (α → β)} (m_s : ∀ f ∈ s, Monotone f) :
    Monotone (infₛ s) := fun x y h => infi_mono fun f => m_s f f.2 h
...
theorem sup_Inf_le_infi_sup : a ⊔ infₛ s ≤ ⨅ b ∈ s, a ⊔ b :=
...
theorem disjoint_Sup_left {a : Set α} {b : α} (d : Disjoint (supₛ a) b) {i} (hi : i ∈ a) :
    Disjoint i b :=
...
theorem disjoint_Sup_right {a : Set α} {b : α} (d : Disjoint b (supₛ a)) {i} (hi : i ∈ a) :
    Disjoint b i :=
