...
lemma ScottContinuousOn.prodMk {g : α → γ} (hD : ∀ a b : α, a ≤ b → {a, b} ∈ D)
  ...
  constructor
  · simp only [mem_image, forall_exists_index, and_imp, forall_apply_eq_imp_iff₂, mem_ofPred,
      Prod.mk_le_mk]
    intro b hb
...
lemma ScottContinuous.sup₂ :
  ...
  simp only [IsLUB, IsLeast, upperBounds, Prod.forall, mem_ofPred, Prod.mk_le_mk] at hdp
  ...
  simp only [lowerBounds, mem_ofPred, Prod.forall, Prod.mk_le_mk] at e1
