...
instance Colex.isStrictOrder [LinearOrder ι] [h : ∀ a, PartialOrder (β a)] :
    IsStrictOrder (Colex (∀ i, β i)) (· < ·) :=
  letI : ∀ (a : ιᵒᵈ), PartialOrder (β a) := h
...
noncomputable instance Colex.linearOrder [LinearOrder ι] [WellFoundedGT ι]
    [h : ∀ a, LinearOrder (β a)] : LinearOrder (Colex (∀ i, β i)) :=
  letI : ∀ (a : ιᵒᵈ), LinearOrder (β a) := h
...
theorem toColex_monotone : Monotone (@toColex (∀ i, β i)) :=
  letI : ∀ (i : ιᵒᵈ), PartialOrder (β i) := ‹∀ (i : ι), PartialOrder (β i)›
...
theorem Colex.noMaxOrder' [Preorder ι] [h : ∀ i, LT (β i)] (i : ι) [NoMaxOrder (β i)] :
...
smaller than the original function. -/
theorem colex_asc {α} [Preorder ι] [h : DecidableEq ι] [LT α] {f : ι → α} {i j : ι} (h₁ : i ≤ j)
    (h₂ : f i < f j) : toColex (f ∘ Equiv.swap i j) < toColex f := by
  rw [Equiv.swap_comm]
  letI : DecidableEq (ιᵒᵈ) := h
