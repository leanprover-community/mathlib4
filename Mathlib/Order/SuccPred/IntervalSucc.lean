...
* `Monotone.pairwise'_disjoint_on_Ioc_succ`: if `α` is a linear succ order, `β` is a preorder, and
...
theorem pairwise'_disjoint_on_Ioo_succ [SuccOrder α] [Preorder β] {f : α → β} (hf : Monotone f) :
    Pairwise' (Disjoint on fun n => Ioo (f n) (f (succ n))) :=
  hf.pairwise'_disjoint_on_Ico_succ.mono fun _ _ h => h.mono Ioo_subset_Ico_self Ioo_subset_Ico_self
...
theorem pairwise'_disjoint_on_Ioc_pred [PredOrder α] [Preorder β] {f : α → β} (hf : Monotone f) :
    Pairwise' (Disjoint on fun n => Ioc (f (pred n)) (f n)) := by
  simpa using! hf.dual.pairwise'_disjoint_on_Ico_succ

/-- If `α` is a linear pred order, `β` is a preorder, and `f : α → β` is a monotone function, then
the intervals `Set.Ico (f Order.pred n) (f n)` are pairwise disjoint. -/
theorem pairwise'_disjoint_on_Ico_pred [PredOrder α] [Preorder β] {f : α → β} (hf : Monotone f) :
    Pairwise' (Disjoint on fun n => Ico (f (pred n)) (f n)) := by
  simpa using! hf.dual.pairwise'_disjoint_on_Ioc_succ

/-- If `α` is a linear pred order, `β` is a preorder, and `f : α → β` is a monotone function, then
the intervals `Set.Ioo (f Order.pred n) (f n)` are pairwise disjoint. -/
theorem pairwise'_disjoint_on_Ioo_pred [PredOrder α] [Preorder β] {f : α → β} (hf : Monotone f) :
    Pairwise' (Disjoint on fun n => Ioo (f (pred n)) (f n)) := by
  simpa using! hf.dual.pairwise'_disjoint_on_Ioo_succ
...
theorem pairwise'_disjoint_on_Ioc_succ [SuccOrder α] [Preorder β] {f : α → β} (hf : Antitone f) :
    Pairwise' (Disjoint on fun n => Ioc (f (succ n)) (f n)) :=
  hf.dual_left.pairwise'_disjoint_on_Ioc_pred

/-- If `α` is a linear succ order, `β` is a preorder, and `f : α → β` is an antitone function, then
the intervals `Set.Ico (f (Order.succ n)) (f n)` are pairwise disjoint. -/
theorem pairwise'_disjoint_on_Ico_succ [SuccOrder α] [Preorder β] {f : α → β} (hf : Antitone f) :
    Pairwise' (Disjoint on fun n => Ico (f (succ n)) (f n)) :=
  hf.dual_left.pairwise'_disjoint_on_Ico_pred

/-- If `α` is a linear succ order, `β` is a preorder, and `f : α → β` is an antitone function, then
the intervals `Set.Ioo (f (Order.succ n)) (f n)` are pairwise disjoint. -/
theorem pairwise'_disjoint_on_Ioo_succ [SuccOrder α] [Preorder β] {f : α → β} (hf : Antitone f) :
    Pairwise' (Disjoint on fun n => Ioo (f (succ n)) (f n)) :=
  hf.dual_left.pairwise'_disjoint_on_Ioo_pred
...
theorem pairwise_disjoint_on_Ioc_pred [PredOrder α] [Preorder β] {f : α → β} (hf : Antitone f) :
  ...
  hf.dual_left.pairwise'_disjoint_on_Ioc_succ

/-- If `α` is a linear pred order, `β` is a preorder, and `f : α → β` is an antitone function, then
the intervals `Set.Ico (f n) (f (Order.pred n))` are pairwise disjoint. -/
theorem pairwise'_disjoint_on_Ico_pred [PredOrder α] [Preorder β] {f : α → β} (hf : Antitone f) :
    Pairwise' (Disjoint on fun n => Ico (f n) (f (pred n))) :=
  hf.dual_left.pairwise'_disjoint_on_Ico_succ

/-- If `α` is a linear pred order, `β` is a preorder, and `f : α → β` is an antitone function, then
the intervals `Set.Ioo (f n) (f (Order.pred n))` are pairwise disjoint. -/
theorem pairwise'_disjoint_on_Ioo_pred [PredOrder α] [Preorder β] {f : α → β} (hf : Antitone f) :
    Pairwise' (Disjoint on fun n => Ioo (f n) (f (pred n))) :=
  hf.dual_left.pairwise'_disjoint_on_Ioo_succ
