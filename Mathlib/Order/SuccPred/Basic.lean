...
theorem lt_succ_iff_not_isMax : a < succ a ↔ ¬IsMax a :=
  ⟨not_isMax_of_lt, fun ha => (le_succ a).gt_of_not_le fun h => ha <| max_of_succ_le h⟩
...
theorem succ_le_succ (h : a ≤ b) : succ a ≤ succ b := by
  ...
  · by_cases hba : b ≤ a
    ...
    · exact succ_le_of_lt ((h.gt_of_not_le hba).trans_le <| le_succ b)
...
lemma le_succ_of_wcovBy (h : a ⩿ b) : b ≤ succ a := by
  ...
  · by_contra hba
    exact h.2 (lt_succ_of_not_isMax hab.lt.not_isMax) <| hab.lt.succ_le.gt_of_not_le hba
...
theorem pred_lt_iff_not_isMin : pred a < a ↔ ¬IsMin a :=
  ⟨not_isMin_of_lt, fun ha => (pred_le a).gt_of_not_le fun h => ha <| min_of_le_pred h⟩
...
lemma pred_le_of_wcovBy (h : a ⩿ b) : pred b ≤ a := by
  obtain hab | ⟨-, hba⟩ := h.covBy_or_le_and_le
  · by_contra hba
    exact h.2 (hab.lt.le_pred.gt_of_not_le hba) (pred_lt_of_not_isMin hab.lt.not_isMin)
