...
theorem pred_le_iff_eq_or_le : pred a ≤ b ↔ b = pred a ∨ a ≤ b := by
  by_cases ha : IsMin a
  · rw [ha.pred_eq, or_iff_right_of_imp le_of_eq']
...
instance [hα : Nonempty α] : IsEmpty (PredOrder (WithTop α)) :=
  ⟨by
    ...
    | top => exact hα.elim fun a => (min_of_le_pred h.ge).not_gt <| coe_lt_top a
    | coe a =>
...
instance [hα : Nonempty α] : IsEmpty (SuccOrder (WithBot α)) :=
  ⟨by
    ...
    | bot => exact hα.elim fun a => (max_of_succ_le h.le).not_gt <| bot_lt_coe a
    | coe a =>
