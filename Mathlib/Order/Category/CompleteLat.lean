...
instance : CoeSort CompleteLat (Type*) := fast_instance%
  Bundled.coeSort

instance (X : CompleteLat) : CompleteLattice X := fast_instance%
  X.str
...
instance : Inhabited CompleteLat := fast_instance%
  ⟨of PUnit⟩
...
instance : ConcreteCategory CompleteLat := fast_instance% by
