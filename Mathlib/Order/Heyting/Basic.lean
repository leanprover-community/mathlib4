...
instance instBiheytingAlgebra : BiheytingAlgebra PUnit.{u + 1} :=
  { PUnit.instLinearOrder.{u} with
    ...
    hnot := fun _ => unit,
    himp := fun _ _ => unit,
    le_top := fun _ => by trivial,
    le_sup_left := fun _ _ => by trivial,
    le_sup_right := fun _ _ => by trivial,
    sup_le := fun _ _ _ _ _ => by trivial,
    inf_le_left := fun _ _ => by trivial,
    inf_le_right := fun _ _ => by trivial,
    le_inf := fun _ _ _ _ _ => by trivial,
    bot_le := fun _ => by trivial,
