...
@[simp] theorem extent_ofObjects : (ofObjects r s).extent = lowerPolar r (upperPolar r s) := rfl
...
theorem le_ofObjects_of_subset 

#exit
/-- The concept generated from the lower polar of a set, i.e. the smallest concept containing the
set of objects `s`. -/
-- We don't use `simps!`, as the autogenerated lemma `ofObjects_extent` gives us a name clash.
@[simps!]
