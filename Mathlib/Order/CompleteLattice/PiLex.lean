...
noncomputable instance completeLattice : CompleteLattice (Πₗ i, α i) where
  isLUB_sSup_of_exists_isLUB _ _ := by exact isLUB_sSup
  isGLB_sInf_of_exists_isGLB _ _ := by exact isGLB_sInf
  exists_isLUB s := ⟨sSup s, by exact isLUB_sSup⟩
  exists_isGLB s := ⟨sInf s, by exact isGLB_sInf⟩
...
noncomputable instance completeLattice : CompleteLattice (Colex ((i : ι) → α i)) where
  isLUB_sSup_of_exists_isLUB _ _ := by exact Lex.isLUB_sSup (ι := ιᵒᵈ)
  isGLB_sInf_of_exists_isGLB _ _ := by exact Lex.isGLB_sInf (ι := ιᵒᵈ)
  exists_isLUB s := ⟨sSup s, by exact Lex.isLUB_sSup (ι := ιᵒᵈ)⟩
  exists_isGLB s := ⟨sInf s, by exact Lex.isGLB_sInf (ι := ιᵒᵈ)⟩
