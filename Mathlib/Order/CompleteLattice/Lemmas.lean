...
theorem le_iInf_sup (f : ι → α) (a : α) :
    (⨅ i, f i) ⊔ a ≤  ⨅ i, (f i ⊔ a) :=
  le_iInf fun i ↦ sup_le_sup_right (iInf_le f i) a

theorem iSup_inf_le (f : ι → α) (a : α) :
    ⨆ i, (f i ⊓ a) ≤ (⨆ i, f i) ⊓ a :=
  @le_iInf_sup αᵒᵈ ι _ f a

lemma le_biInf_sup (f : β → α) (s : Set β) (a : α) :
    (⨅ i ∈ s, f i) ⊔ a ≤ ⨅ i ∈ s, f i ⊔ a :=
  le_iInf₂ fun _ hi ↦ sup_le_sup_right (biInf_le f hi) a

lemma biSup_inf_le (f : β → α) (s : Set β) (a : α) :
    ⨆ i ∈ s, (f i ⊓ a) ≤ (⨆ i ∈ s, f i) ⊓ a :=
  @le_biInf_sup αᵒᵈ β _ f s a
