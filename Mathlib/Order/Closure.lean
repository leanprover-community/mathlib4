...
lemma mem_closed_iff (x : α) : x ∈ c.closed ↔ c x = x :=
  Iff.rfl
...
@[simp]
lemma closure_is_closed (x : α) : c x ∈ c.closed :=
  c.idempotent x
...
@[simp]
lemma closure_le_closed_iff_le (x : α) {y : α} (hy : c.closed y) : c x ≤ y ↔ x ≤ y := by
...
`mk₃` constructor. -/
lemma eq_mk₃_closed (c : ClosureOperator α) :
    c =
