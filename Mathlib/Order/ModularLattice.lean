...
instance (priority := 10000) : IsWeakLowerModularLattice (OrderDual α) :=
...
instance (priority := 10000) : IsWeakUpperModularLattice (OrderDual α) :=
...
instance (priority := 10000) : IsLowerModularLattice (OrderDual α) :=
...
instance (priority := 10000) : IsUpperModularLattice (OrderDual α) :=
...
instance (priority := 10000) : IsModularLattice αᵒᵈ :=
  ⟨fun y z xz =>
    le_of_eq
      (by
...
instance (priority := 10000) isModularLattice_Iic : IsModularLattice (Set.Iic a) :=
...
instance (priority := 10000) isModularLattice_Ici : IsModularLattice (Set.Ici a) :=
  ⟨@fun x y z xz => (sup_inf_le_assoc_of_le (y : α) xz : (↑x ⊔ ↑y) ⊓ ↑z ≤ ↑x ⊔ ↑y ⊓ ↑z)⟩
...
instance (priority := 10000) complementedLattice_Iic : ComplementedLattice (Set.Iic a) :=
  ⟨fun ⟨x, hx⟩ =>
...
instance (priority := 10000) complementedLattice_Ici : ComplementedLattice (Set.Ici a) :=
  ⟨fun ⟨x, hx⟩ =>
    let ⟨y, hy⟩ := exists_isCompl x
