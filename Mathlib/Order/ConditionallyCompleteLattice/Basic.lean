...
theorem WithTop.coe_infi [Nonempty ι] [InfSet α] (f : ι → α) :
  ...
  rw [infᵢ, infᵢ, WithTop.coe_Inf' (range_nonempty f), ← range_comp]; rfl
...
theorem WithTop.coe_supr [Preorder α] [SupSet α] (f : ι → α) (h : BddAbove (Set.range f)) :
    ↑(⨆ i, f i) = (⨆ i, f i : WithTop α) := by
    rw [supᵢ, supᵢ, WithTop.coe_Sup' h, ← range_comp]; rfl
...
class ConditionallyCompleteLattice (α : Type _) extends Lattice α, SupSet α, InfSet α where
  le_cSup : ∀ s a, BddAbove s → a ∈ s → a ≤ supₛ s
  cSup_le : ∀ s a, Set.Nonempty s → a ∈ upperBounds s → supₛ s ≤ a
  cInf_le : ∀ s a, BddBelow s → a ∈ s → infₛ s ≤ a
  le_cInf : ∀ s a, Set.Nonempty s → a ∈ lowerBounds s → a ≤ infₛ s
...
instance (priority := 100) CompleteLinearOrder.toConditionallyCompleteLinearOrderBot {α : Type _}
  ...
  { CompleteLattice.toConditionallyCompleteLattice, h with cSup_empty := supₛ_empty }
#align
...
theorem is_glb_cInf (ne : s.Nonempty) (H : BddBelow s) : IsGLB s (infₛ s) :=
  ⟨fun _ => cInf_le H, fun _ => le_cInf ne⟩
...
theorem subset_Icc_cInf_cSup (hb : BddBelow s) (ha : BddAbove s) : s ⊆ Icc (infₛ s) (supₛ s) :=
  fun _ hx => ⟨cInf_le hb hx, le_cSup ha hx⟩
...
theorem cSup_lower_bounds_eq_cInf {s : Set α} (h : BddBelow s) (hs : s.Nonempty) :
  ...
  (is_lub_cSup h <| hs.mono fun _ hx _ hy => hy hx).unique (is_glb_cInf hs h).isLUB
...
theorem cInf_upper_bounds_eq_cSup {s : Set α} (h : BddAbove s) (hs : s.Nonempty) :
  ...
  (is_glb_cInf h <| hs.mono fun _ hx _ hy => hy hx).unique (is_lub_cSup hs h).isGLB
...
theorem exists_between_of_forall_le (sne : s.Nonempty) (tne : t.Nonempty)
  ...
  ⟨infₛ t, fun x hx => le_cInf tne <| hst x hx, fun _ hy => cInf_le (sne.mono hst) hy⟩
...
theorem cSup_inter_le (hs : BddAbove s) (ht : BddAbove t) (hst : (s ∩ t).Nonempty) :
    supₛ (s ∩ t) ≤ supₛ s ⊓ supₛ t :=
  (cSup_le hst) fun _ hx => le_inf (le_cSup hs hx.1) (le_cSup ht hx.2)
...
@[simp]
theorem csupr_const [hι : Nonempty ι] {a : α} : (⨆ _b : ι, a) = a := by
  rw [supᵢ, range_const, cSup_singleton]
#align csupr_const csupr_const

@[simp]
theorem cinfi_const [Nonempty ι] {a : α} : (⨅ _b : ι, a) = a :=
...
theorem cinfi_eq_of_forall_ge_of_forall_gt_exists_lt [Nonempty ι] {f : ι → α} (h₁ : ∀ i, b ≤ f i)
  ...
  exact @csupr_eq_of_forall_le_of_forall_lt_exists_gt αᵒᵈ _ _ _ _ ‹_› ‹_› ‹_›
...
theorem cSup_eq_of_is_forall_le_of_forall_le_imp_ge (hs : s.Nonempty) (h_is_ub : ∀ a ∈ s, a ≤ b)
  ...
  (cSup_le hs h_is_ub).antisymm ((h_b_le_ub _) fun _ => le_cSup ⟨b, h_is_ub⟩)
...
theorem Inf_eq_argmin_on (hs : s.Nonempty) :
  ...
  IsLeast.cInf_eq ⟨argminOn_mem _ _ _ _, fun _ ha => argminOn_le id _ _ ha⟩
...
theorem MonotoneOn.map_Inf {β : Type _} [ConditionallyCompleteLattice β] {f : α → β}
  ...
  (hf.map_isLeast (is_least_Inf hs)).cInf_eq.symm
...
theorem le_cSup_iff' {s : Set α} {a : α} (h : BddAbove s) :
    a ≤ supₛ s ↔ ∀ b, b ∈ upperBounds s → a ≤ b :=
  ⟨fun h _ hb => le_trans h (cSup_le' hb), fun hb => hb _ fun _ => le_cSup h⟩
#align le_cSup_iff' le_cSup_iff'
...
noncomputable instance : CompleteLinearOrder (WithTop α) :=
  { WithTop.linearOrder, WithTop.lattice, WithTop.orderTop, WithTop.orderBot with
    sup := HasSup.sup
    le_supₛ := fun s => (is_lub_Sup s).1
    supₛ_le := fun s => (is_lub_Sup s).2
    inf := HasInf.inf
    le_infₛ := fun s => (is_glb_Inf s).2
    infₛ_le := fun s => (is_glb_Inf s).1 }
...
theorem cSup_image_le {s : Set α} (hs : s.Nonempty) {B : α} (hB : B ∈ upperBounds s) :
    supₛ (f '' s) ≤ f B :=
  cSup_le (Nonempty.image f hs) (h_mono.mem_upperBounds_image hB)
...
theorem cInf_image_le {s : Set α} {c : α} (hcs : c ∈ s) (h_bdd : BddBelow s) :
    infₛ (f '' s) ≤ f c := by
  let f' : αᵒᵈ → βᵒᵈ := f
  exact @le_cSup_image αᵒᵈ βᵒᵈ _ _ _
...
theorem le_cInf_image {s : Set α} (hs : s.Nonempty) {B : α} (hB : B ∈ lowerBounds s) :
    f B ≤ infₛ (f '' s) := by
  let f' : αᵒᵈ → βᵒᵈ := f
  exact @cSup_image_le αᵒᵈ βᵒᵈ _ _ _
...
theorem l_cSup (gc : GaloisConnection l u) {s : Set α} (hne : s.Nonempty) (hbdd : BddAbove s) :
    l (supₛ s) = ⨆ x : s, l x :=
  Eq.symm <| IsLUB.csupr_set_eq (gc.isLUB_l_image <| is_lub_cSup hne hbdd) hne
...
theorem map_cSup (e : α ≃o β) {s : Set α} (hne : s.Nonempty) (hbdd : BddAbove s) :
    e (supₛ s) = ⨆ x : s, e x :=
  e.to_galoisConnection.l_cSup hne hbdd
...
theorem map_csupr (e : α ≃o β) {f : ι → α} (hf : BddAbove (range f)) :
    e (⨆ i, f i) = ⨆ i, e (f i) :=
  e.to_galoisConnection.l_csupr hf
...
theorem map_csupr_set (e : α ≃o β) {s : Set γ} {f : γ → α} (hf : BddAbove (f '' s))
  ...
  e.to_galoisConnection.l_csupr_set hf hne
...
theorem cSup_image2_eq_cInf_cSup (h₁ : ∀ b, GaloisConnection (swap l b ∘ ofDual) (toDual ∘ u₁ b))
...
theorem cSup_image2_eq_cInf_cInf (h₁ : ∀ b, GaloisConnection (swap l b ∘ ofDual) (toDual ∘ u₁ b))
    (h₂ : ∀ a, GaloisConnection (l a ∘ ofDual) (toDual ∘ u₂ a)) :
...
theorem cInf_image2_eq_cSup_cInf (h₁ : ∀ b, GaloisConnection (toDual ∘ l₁ b) (swap u b ∘ ofDual))
    (h₂ : ∀ a, GaloisConnection (l₂ a) (u a)) :
...
theorem cInf_image2_eq_cSup_cSup (h₁ : ∀ b, GaloisConnection (toDual ∘ l₁ b) (swap u b ∘ ofDual))
    (h₂ : ∀ a, GaloisConnection (toDual ∘ l₂ a) (u a ∘ ofDual)) :
...
noncomputable instance WithTop.conditionallyCompleteLattice {α : Type _}
  ...
  { WithTop.lattice, instSupSetWithTop, instInfSetWithTop with
    le_cSup := fun _ a _ haS => (WithTop.is_lub_Sup' ⟨a, haS⟩).1 haS
    cSup_le := fun _ _ hS haS => (WithTop.is_lub_Sup' hS).2 haS
    cInf_le := fun _ _ hS haS => (WithTop.is_glb_Inf' hS).1 haS
    le_cInf := fun _ a _ haS => (WithTop.is_glb_Inf' ⟨a, haS⟩).2 haS }
...
noncomputable instance WithTop.WithBot.completeLattice {α : Type _}
  ...
  { instInfSetWithTop, instSupSetWithTop, WithTop.boundedOrder, WithTop.lattice with
    le_supₛ := fun S a haS => (WithTop.is_lub_Sup' ⟨a, haS⟩).1 haS
    supₛ_le := fun S a ha => by
      ...
      · show ite _ _ _ ≤ a
        ...
        · convert @bot_le (WithTop (WithBot α)) _ _ a
          convert @WithBot.cSup_empty α _
          rw [h]
          rfl
        · exfalso
          apply h₂
      ...
      show ite _ _ _ ≤ a by
        ...
        · cases a
          ...
          · apply WithTop.some_le_some.2
            ...
            intro b _
            exact bot_le
    le_infₛ := fun S a haS => (WithTop.is_glb_Inf' ⟨a, haS⟩).2 haS }
