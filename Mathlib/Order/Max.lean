...
theorem IsMin.not_gt (h : IsMin a) : ¬b < a := fun hb => hb.not_ge <| h hb.le

theorem IsMax.not_gt (h : IsMax a) : ¬a < b := fun hb => hb.not_ge <| h hb.le

@[simp]
theorem not_isMin_of_lt (h : b < a) : ¬IsMin a := fun ha => ha.not_gt h

@[simp]
theorem not_isMax_of_lt (h : a < b) : ¬IsMax a := fun ha => ha.not_gt h
...
theorem isMin_iff_forall_not_lt : IsMin a ↔ ∀ b, ¬b < a :=
  ⟨fun h _ => h.not_gt, fun h _ hba => of_not_not fun hab => h _ <| hba.lt_of_not_ge hab⟩
