...
- `Order.PFilter P`: The type of nonempty, downward directed, upward closed subsets of `P`.
               This is dual to `Mathlib/Order/Ideal.lean`, so it simply wraps `Order.Ideal Pᵒᵈ`.
- `Order.IsPFilter P`: a predicate for when a `Set P` is a filter.

Note the relation between `Mathlib/Topology/Order/Filter.lean` and `Order/PFilter`: for any type `α`,
