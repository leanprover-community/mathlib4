...
theorem LE.le.not_incompRel [LE α] (h : a ≤ b) : ¬ a ‖ b := fun h' ↦ h'.not_le h
...
theorem IncompRel.not_lt (h : a ‖ b) : ¬ a < b := mt le_of_lt h.not_le
theorem IncompRel.not_gt (h : a ‖ b) : ¬ b < a := mt le_of_lt h.not_ge
theorem LT.lt.not_incompRel (h : a < b) : ¬ a ‖ b := fun h' ↦ h'.not_lt h

theorem not_le_iff_lt_or_incompRel : ¬ b ≤ a ↔ a < b ∨ a ‖ b := by
  rw [lt_iff_le_not_le, incompRel_iff]
  tauto

/-- Exactly one of the following is true. -/
theorem lt_or_antisymmRel_or_gt_or_incompRel (a b : α) : a < b ∨ a ⋚ b ∨ b < a ∨ a ‖ b := by
  simp_rw [lt_iff_le_not_le]
  tauto

@[trans]
theorem incompRel_of_incompRel_of_antisymmRel (h₁ : a ‖ b) (h₂ : b ⋚ c) : a ‖ c :=
...
@[trans]
theorem incompRel_of_antisymmRel_of_incompRel (h₁ : a ⋚ b) (h₂ : b ‖ c) : a ‖ c :=
...
theorem AntisymmRel.incompRel_congr (h₁ : a ⋚ b) (h₂ : c ⋚ d) : a ‖ c ↔ b ‖ d where
  mp h := (h₁.symm.trans_incompRel h).trans_antisymmRel h₂
  mpr h := (h₁.trans_incompRel h).trans_antisymmRel h₂.symm

theorem AntisymmRel.incompRel_congr_left (h : a ⋚ b) : a ‖ c ↔ b ‖ c :=
  h.incompRel_congr (antisymmRel_refl _ c)

theorem AntisymmRel.incompRel_congr_right (h : b ⋚ c) : a ‖ b ↔ a ‖ c :=
  (antisymmRel_refl _ a).incompRel_congr h

end Preorder

/-- Exactly one of the following is true. -/
theorem lt_or_eq_or_gt_or_incompRel [PartialOrder α] (a b : α) : a < b ∨ a = b ∨ b < a ∨ a ‖ b := by
