...
`Iso (x, x ⊔ y) (x ⊓ y, y)` for `IsMaximal x (x ⊔ y)`. This is the strongest equivalence relation
for which we can prove the Jordan Hölder theorem. For any specific `JordanHolderLattice`, one must
verify that `Iso` implies the desired weaker notion (e.g., isomorphism of quotients).
...
`IsMaximal x (x ⊔ y)`. This will imply isomorphism of quotients for subgroups and submodules. -/
...
theorem Iso.rel
  ...
  have : IsEquiv (X × X) e := { refl := h_refl, symm := h_symm, trans := h_trans }
