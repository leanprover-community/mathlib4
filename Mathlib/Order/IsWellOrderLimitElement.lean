...
This file is deprecated. You should use `Order.IsSuccLimit` on a `SuccOrder` instead.
-/
...
lemma Nat.not_isWellOrderLimitElement (a : ℕ) [IsWellOrderLimitElement a] : False := by
  ...
  · simpa [Nat.wellOrderSucc_eq] using IsWellOrderLimitElement.wellOrderSucc_lt (Nat.lt_succ_self a)
