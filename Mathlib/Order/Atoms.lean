...
variable (α) in
/-- A simple partial ordered `BoundedOrder` with decidable equality has decidable `LE`. -/
@[implicit_reducible]
def IsSimpleOrder.decidableLEOfDecidableEq [DecidableEq α] : DecidableLE α := fun a b ↦
