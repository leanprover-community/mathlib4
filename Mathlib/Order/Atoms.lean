...
lemma IsAtom.of_isAtom_coe_Iic {a : Set.Iic x} (ha : IsAtom a) : IsAtom (a : α) :=
...
lemma isAtom_iff_le_of_ge : IsAtom a ↔ a ≠ ⊥ ∧ ∀ b ≠ ⊥, b ≤ a → a ≤ b :=
  and_congr Iff.rfl <|
...
@[simp]
lemma bot_covBy_iff : ⊥ ⋖ a ↔ IsAtom a := by
...
lemma atom_le_iSup [Order.Frame α] (ha : IsAtom a) {f : ι → α} :
    a ≤ iSup f ↔ ∃ i, a ≤ f i := by
...
lemma IsCoatom.of_isCoatom_coe_Ici {a : Set.Ici x} (ha : IsCoatom a) : IsCoatom (a : α) :=
  @IsAtom.of_isAtom_coe_Iic αᵒᵈ _ _ x a ha
#align is_coatom.of_is_coatom_coe_Ici IsCoatom.of_isCoatom_coe_Ici

lemma isCoatom_iff_ge_of_le : IsCoatom a ↔ a ≠ ⊤ ∧ ∀ b ≠ ⊤, a ≤ b → b ≤ a :=
...
@[simp]
lemma covBy_top_iff : a ⋖ ⊤ ↔ IsCoatom a :=
...
lemma iInf_le_coatom [Order.Coframe α] (ha : IsCoatom a) {f : ι → α} :
    iInf f ≤ a ↔ ∃ i, f i ≤ a :=
...
@[simp]
lemma Set.Ici.isAtom_iff {b : Set.Ici a} : IsAtom b ↔ a ⋖ b := by
  rw [← bot_covBy_iff]
...
@[simp]
lemma Set.Iic.isCoatom_iff {a : Set.Iic b} : IsCoatom a ↔ ↑a ⋖ b := by
  rw [← covBy_top_iff]
...
lemma IsAtom.inf_eq_bot_of_ne [SemilatticeInf α] [OrderBot α] {a b : α} (ha : IsAtom a)
...
lemma IsAtom.disjoint_of_ne [SemilatticeInf α] [OrderBot α] {a b : α} (ha : IsAtom a)
...
lemma IsCoatom.sup_eq_top_of_ne [SemilatticeSup α] [OrderTop α] {a b : α} (ha : IsCoatom a)
...
@[simp]
lemma isCoatomic_dual_iff_isAtomic [OrderBot α] : IsCoatomic αᵒᵈ ↔ IsAtomic α :=
...
@[simp]
lemma isAtomic_dual_iff_isCoatomic [OrderTop α] : IsAtomic αᵒᵈ ↔ IsCoatomic α :=
...
lemma isAtomic_iff_forall_isAtomic_Iic [OrderBot α] :
...
lemma isCoatomic_iff_forall_isCoatomic_Ici [OrderTop α] :
...
lemma isAtomic_of_orderBot_wellFounded_lt [OrderBot α]
...
lemma isCoatomic_of_orderTop_gt_wellFounded [OrderTop α]
...
lemma le_iff_atom_le_imp {α} [BooleanAlgebra α] [IsAtomic α] {x y : α} :
...
lemma eq_iff_atom_le_iff {α} [BooleanAlgebra α] [IsAtomic α] {x y : α} :
    x = y ↔ ∀ a, IsAtom a → (a ≤ x ↔ a ≤ y) := by
...
@[simp]
lemma isCoatomistic_dual_iff_isAtomistic : IsCoatomistic αᵒᵈ ↔ IsAtomistic α :=
...
@[simp]
lemma isAtomistic_dual_iff_isCoatomistic : IsAtomistic αᵒᵈ ↔ IsCoatomistic α :=
...
lemma eq_iff_atom_le_iff {a b : α} : a = b ↔ ∀ c, IsAtom c → (c ≤ a ↔ c ≤ b) := by
  refine ⟨fun h => h ▸ by simp, fun h => ?_⟩
  exact le_antisymm (le_iff_atom_le_imp.2 fun a ha hx => (h a ha).1 hx)
    (le_iff_atom_le_imp.2 fun a ha hy => (h a ha).2 hy)
...
lemma isSimpleOrder_iff_isSimpleOrder_orderDual [LE α] [BoundedOrder α] :
...
lemma IsSimpleOrder.bot_ne_top [LE α] [BoundedOrder α] [IsSimpleOrder α] : (⊥ : α) ≠ (⊤ : α) := by
...
lemma isSimpleOrder_iff_isAtom_top [PartialOrder α] [BoundedOrder α] :
...
lemma isSimpleOrder_iff_isCoatom_bot [PartialOrder α] [BoundedOrder α] :
...
lemma isSimpleOrder_Iic_iff_isAtom [PartialOrder α] [OrderBot α] {a : α} :
...
lemma isSimpleOrder_Ici_iff_isCoatom [PartialOrder α] [OrderTop α] {a : α} :
...
lemma isAtom_of_map_bot_of_image [OrderBot α] [OrderBot β] (f : β ↪o α) (hbot : f ⊥ = ⊥) {b : β}
...
lemma isCoatom_of_map_top_of_image [OrderTop α] [OrderTop β] (f : β ↪o α) (htop : f ⊤ = ⊤)
...
lemma isAtom_of_u_bot [OrderBot α] [OrderBot β] {l : α → β} {u : β → α}
...
lemma isAtom_iff [OrderBot α] [IsAtomic α] [OrderBot β] {l : α → β} {u : β → α}
...
lemma isCoatom_of_image [OrderTop α] [OrderTop β] {l : α → β} {u : β → α}
...
lemma isCoatom_iff [OrderTop α] [IsCoatomic α] [OrderTop β] {l : α → β} {u : β → α}
...
lemma isCoatom_of_l_top [OrderTop α] [OrderTop β] {l : α → β} {u : β → α}
...
lemma isCoatom_iff [OrderTop α] [OrderTop β] [IsCoatomic β] {l : α → β} {u : β → α}
...
lemma isAtom_of_image [OrderBot α] [OrderBot β] {l : α → β} {u : β → α}
...
lemma isAtom_iff [OrderBot α] [OrderBot β] [IsAtomic β] {l : α → β} {u : β → α}
...
@[simp]
lemma isAtom_iff [OrderBot α] [OrderBot β] (f : α ≃o β) (a : α) : IsAtom (f a) ↔ IsAtom a :=
...
@[simp]
lemma isCoatom_iff [OrderTop α] [OrderTop β] (f : α ≃o β) (a : α) :
    IsCoatom (f a) ↔ IsCoatom a :=
  f.dual.isAtom_iff a
#align order_iso.is_coatom_iff OrderIso.isCoatom_iff

lemma isSimpleOrder_iff [BoundedOrder α] [BoundedOrder β] (f : α ≃o β) :
...
lemma isSimpleOrder [BoundedOrder α] [BoundedOrder β] [h : IsSimpleOrder β] (f : α ≃o β) :
    IsSimpleOrder α :=
  f.isSimpleOrder_iff.mpr h
#align order_iso.is_simple_order OrderIso.isSimpleOrder

protected lemma isAtomic_iff [OrderBot α] [OrderBot β] (f : α ≃o β) :
...
protected lemma isCoatomic_iff [OrderTop α] [OrderTop β] (f : α ≃o β) :
...
lemma isAtom_iff_isCoatom : IsAtom a ↔ IsCoatom b :=
...
lemma isCoatom_iff_isAtom : IsCoatom a ↔ IsAtom b :=
...
lemma isCoatomic_of_isAtomic_of_complementedLattice_of_isModular [IsAtomic α] :
    IsCoatomic α :=
  ⟨fun x => by
...
lemma isAtomic_of_isCoatomic_of_complementedLattice_of_isModular [IsCoatomic α] :
    IsAtomic α :=
...
lemma isAtomic_iff_isCoatomic : IsAtomic α ↔ IsCoatomic α :=
...
@[simp] lemma isAtom_iff {p : Prop} : IsAtom p ↔ p := by
  simp [IsAtom, show ⊥ = False from rfl, fun q r : Prop => show q < r ↔ _ ∧ _ from .rfl]

@[simp] lemma isCoatom_iff {p : Prop} : IsCoatom p ↔ ¬ p := by
...
protected lemma eq_bot_iff [∀ i, Bot (π i)] {f : ∀ i, π i} : f = ⊥ ↔ ∀ i, f i = ⊥ :=
  ⟨(· ▸ by simp), fun h => funext fun i => by simp [h]⟩

lemma isAtom_iff {f : ∀ i, π i} [∀ i, PartialOrder (π i)] [∀ i, OrderBot (π i)] :
...
lemma isAtom_single [DecidableEq ι] [∀ i, PartialOrder (π i)] [∀ i, OrderBot (π i)] {a : π i}
    (h : IsAtom a) : IsAtom (Function.update (⊥ : ∀ i, π i) i a) :=
  isAtom_iff.2 ⟨i, by simpa, fun j hji => Function.update_noteq hji _ _⟩

lemma isAtom_iff_eq_single [DecidableEq ι] [∀ i, PartialOrder (π i)]
