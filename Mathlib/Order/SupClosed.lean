...
def SemilatticeSup.toCompleteSemilatticeSup [SemilatticeSup α] (sSup : Set α → α)
  ...
  isLUB_sSup_of_exists_isLUB _ _ := isLUB_supClosure.mp <| h _ supClosed_supClosure
  exists_isLUB _ := ⟨_ , isLUB_supClosure.mp <| h _ supClosed_supClosure⟩
...
def SemilatticeInf.toCompleteSemilatticeInf [SemilatticeInf α] (sInf : Set α → α)
  ...
  isGLB_sInf_of_exists_isGLB _ _ := isGLB_infClosure.mp <| h _ infClosed_infClosure
  exists_isGLB _ := ⟨_, isGLB_infClosure.mp <| h _ infClosed_infClosure⟩
