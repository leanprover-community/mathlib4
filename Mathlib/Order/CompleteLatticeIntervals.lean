...
noncomputable abbrev subsetConditionallyCompleteLinearOrder [Inhabited s]
  ...
  { subsetSupSet s, subsetInfSet s with
...
noncomputable instance Set.Icc.completeLattice [Lattice α] [ConditionallyCompleteLattice α]
...
noncomputable instance [LinearOrder α] [ConditionallyCompleteLinearOrder α]
    {a b : α} [Fact (a ≤ b)] :
    CompleteLinearOrder (Set.Icc a b) :=
  { Set.Icc.completeLattice, Subtype.instLinearOrder _, LinearOrder.toBiheytingAlgebra _ with }

lemma Set.Icc.coe_sSup [Lattice α] [ConditionallyCompleteLattice α] {a b : α} (h : a ≤ b)
    {S : Set (Set.Icc a b)} (hS : S.Nonempty) : have : Fact (a ≤ b) := ⟨h⟩
    ↑(sSup S) = sSup ((↑) '' S : Set α) :=
  congrArg Subtype.val (dif_neg hS.ne_empty)

lemma Set.Icc.coe_sInf [Lattice α] [ConditionallyCompleteLattice α] {a b : α} (h : a ≤ b)
    {S : Set (Set.Icc a b)} (hS : S.Nonempty) : have : Fact (a ≤ b) := ⟨h⟩
    ↑(sInf S) = sInf ((↑) '' S : Set α) :=
  congrArg Subtype.val (dif_neg hS.ne_empty)

lemma Set.Icc.coe_iSup [Lattice α] [ConditionallyCompleteLattice α] {a b : α} (h : a ≤ b)
...
lemma Set.Icc.coe_iInf [Lattice α] [ConditionallyCompleteLattice α] {a b : α} (h : a ≤ b)
    [Nonempty ι] {S : ι → Set.Icc a b} : have : Fact (a ≤ b) := ⟨h⟩
    ↑(iInf S) = (⨅ i, S i : α) :=
