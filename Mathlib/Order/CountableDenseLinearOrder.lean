...
instance : Inhabited (PartialIso α β) := fast_instance% ⟨⟨∅, fun _p h _q ↦ (Finset.not_mem_empty _ h).elim⟩⟩

instance : Preorder (PartialIso α β) := fast_instance% Subtype.preorder _
