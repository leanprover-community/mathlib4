...
instance {m n : Type u → Type*} {g : Type} [MonadLift m n] :
    MonadLiftT (RandGT g m) (RandGT g n) where
...
def next {m : Type → Type*} {g : Type} [RandomGen g] [Monad m] : RandGT g m Nat := do
  let rng := (← get).down
...
def split {m : Type → Type*} {g : Type} [RandomGen g] [Monad m] : RandGT g m g := do
  let rng := (← get).down
...
def range {m : Type → Type*} {g : Type} [RandomGen g] [Monad m] : RandGT g m (Nat × Nat) := do
  let rng := (← get).down
...
variable {g : Type} {m : Type u → Type*} [Monad m]
...
instance {m : Type u → Type*} {m' : Type (max u v) → Type*} {α : Type u}
    [ULiftable m m'] [Random m α] : Random m' (ULift α) where
...
instance {m : Type u → Type*} {m' : Type (max u v) → Type*} {α : Type u} [Preorder α]
    [ULiftable m m'] [BoundedRandom m α] [Monad m'] : BoundedRandom m' (ULift α) where
  randomR lo hi h := do
    let ⟨x⟩ ← ULiftable.up (BoundedRandom.randomR lo.down hi.down h)
...
variable {m : Type u → Type*} {m₀ : Type → Type}
