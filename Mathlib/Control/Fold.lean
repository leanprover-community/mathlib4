...
def Foldl.ofFreeMonoid (f : β → α → β) : FreeMonoid α →* Monoid.Foldl β
  ...
  map_mul' := by
    intros
    simp (config := { unfoldPartialApp := true }) only [FreeMonoid.toList_mul, flip, unop_op,
...
theorem foldlm.ofFreeMonoid_comp_of {m} [Monad m] [LawfulMonad m] (f : α → β → m α) :
  ...
  ext1 x
  simp (config := { unfoldPartialApp := true }) [(· ∘ ·), foldlM.ofFreeMonoid, foldlM.mk, flip]
  rfl
...
theorem foldrm.ofFreeMonoid_comp_of {m} [Monad m] [LawfulMonad m] (f : β → α → m α) :
  ...
  ext
  simp (config := { unfoldPartialApp := true }) [(· ∘ ·), foldrM.ofFreeMonoid, foldrM.mk, flip]
  ...
  Eq.symm <|
    calc
      ...
      _ = (unop (Foldl.ofFreeMonoid (flip cons) (foldMap FreeMonoid.of xs)) []).reverse :=
          by simp (config := { unfoldPartialApp := true }) [flip, List.foldr_reverse,
            Foldl.ofFreeMonoid, unop_op]
      _ = toList xs :=
...
theorem foldMap_map [Monoid γ] (f : α → β) (g : β → γ) (xs : t α) :
  ...
  simp (config := { unfoldPartialApp := true }) only [foldMap, traverse_map, Function.comp]
...
theorem toList_map (f : α → β) (xs : t α) : toList (f <$> xs) = f <$> toList xs := by
  simp (config := { unfoldPartialApp := true }) only [toList_spec, Free.map_eq_map, foldMap_hom,
    foldMap_map, FreeMonoid.ofList_toList, FreeMonoid.map_of, (· ∘ ·)]
...
theorem foldl_map (g : β → γ) (f : α → γ → α) (a : α) (l : t β) :
  ...
  simp (config := { unfoldPartialApp := true }) only [foldl, foldMap_map, (· ∘ ·), flip]
#align traversable.foldl_map Traversable.foldl_map

@[simp]
theorem foldr_map (g : β → γ) (f : γ → α → α) (a : α) (l : t β) :
    foldr f a (g <$> l) = foldr (f ∘ g) a l := by
  simp (config := { unfoldPartialApp := true }) only [foldr, foldMap_map, (· ∘ ·), flip]
...
theorem foldlm_map (g : β → γ) (f : α → γ → m α) (a : α) (l : t β) :
  ...
  simp (config := { unfoldPartialApp := true }) only [foldlm, foldMap_map, (· ∘ ·), flip]
...
theorem foldrm_map (g : β → γ) (f : γ → α → m α) (a : α) (l : t β) :
  ...
  simp (config := { unfoldPartialApp := true }) only [foldrm, foldMap_map, (· ∘ ·), flip]
