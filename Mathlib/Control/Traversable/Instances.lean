...
lemma Option.id_traverse {α} (x : Option α) : Option.traverse (pure : α → Id α) x = x := by
  cases x <;> rfl
...
protected lemma id_traverse {α} (xs : List α) : List.traverse (pure : α → Id α) xs = xs := by
...
@[simp]
lemma traverse_nil : traverse f ([] : List α') = (pure [] : F (List β')) :=
  rfl
...
protected lemma id_traverse {σ α} (x : σ ⊕ α) :
    Sum.traverse (pure : α → Id α) x = x := by cases x <;> rfl
#align sum.id_traverse Sum.id_traverse

protected lemma comp_traverse {α β γ : Type u} (f : β → F γ) (g : α → G β) (x : σ ⊕ α) :
