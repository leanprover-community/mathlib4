...
instance : CommApplicative Id := by constructor; intros; rfl
...
instance {f : Type u → Type w} {g : Type v → Type u} [Applicative f] [Applicative g]
  ...
  refine { @instLawfulApplicativeComp f g _ _ _ _ with commutative_prod := ?_ }
  intros
...
instance {α} [Monoid α] : LawfulApplicative (Const α) := by
  constructor <;> intros <;> simp [mul_assoc, (· <$> ·), Seq.seq, pure] <;> rfl
...
instance {α} [AddMonoid α] : LawfulApplicative (AddConst α) := by
  constructor <;> intros <;> simp [add_assoc, (· <$> ·), Seq.seq, pure] <;> rfl
