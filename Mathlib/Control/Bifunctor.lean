...
! This file was ported from Lean 3 source control.bifunctor
...
class Bifunctor (F : Type u₀ → Type u₁ → Type u₂) where
  /-- `map` equivalent for a `Bifunctor`-/
...
class LawfulBifunctor (F : Type u₀ → Type u₁ → Type u₂) [Bifunctor F] where
  /-- `bimap` respects the identity function -/
...
instance IsLawfulBifunctor.const : LawfulBifunctor Const := by refine' { .. } <;> intros <;> rfl
...
instance IsLawfulBifunctor.flip [LawfulBifunctor F] : LawfulBifunctor (flip F) := by
