  ...
  { SymAlg.addCommMonoid with
    ...
    mul_zero := fun _ => by
      rw [mul_def, unsym_zero, zero_mul, mul_zero, add_zero,
        mul_zero, sym_zero]
    mul_one := fun _ => by
      rw [mul_def, unsym_one, mul_one, one_mul, add_self, invOf_mul_cancel_left, sym_unsym]
    one_mul := fun _ => by
      rw [mul_def, unsym_one, mul_one, one_mul, add_self, invOf_mul_cancel_left, sym_unsym]
...
theorem unsym_mul_self [Semiring α] [Invertible (2 : α)] (a : αˢʸᵐ) :
  ...
  rw [mul_def, unsym_sym, add_self, invOf_mul_cancel_left]

theorem sym_mul_self [Semiring α] [Invertible (2 : α)] (a : α) : sym (a * a) = sym a * sym a := by
  rw [sym_mul_sym, add_self, invOf_mul_cancel_left]
