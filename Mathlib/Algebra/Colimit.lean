...
/-!
# Colimit of modules, abelian groups, rings, and fields.

See Atiyah-Macdonald PP.32-33, Matsumura PP.269-270

It is constructed as a quotient of the free module (for the module case) or quotient of
the free commutative ring (for the ring case).

## Main definitions

* `Module.Colimit G f`
* `AddCommGroup.Colimit G f`
* `Ring.Colimit G f`
...
/-- The colimit of a diagram is the modules glued together along the maps. -/
def Colimit [DecidableEq ι] : Type max v w :=
  DirectSum ι G ⧸
    (span R <|
      { a |
        ∃ (i j : _) (H : i ≤ j) (x : _),
...
instance addCommGroup : AddCommGroup (Colimit G f) :=
  Quotient.addCommGroup _

instance module : Module R (Colimit G f) :=
  Quotient.module _

instance inhabited : Inhabited (Colimit G f) :=
  ⟨0⟩

instance unique [IsEmpty ι] : Unique (Colimit G f) :=
  inferInstanceAs <| Unique (Quotient _)

variable (R ι)

/-- The canonical map from a component to the colimit. -/
...
theorem exists_of [Nonempty ι] [IsDirected ι (· ≤ ·)] (z : Colimit G f) :
...
@[elab_as_elim]
protected theorem induction_on [Nonempty ι] [IsDirected ι (· ≤ ·)] {C : Colimit G f → Prop}
    (z : Colimit G f) (ih : ∀ i x, C (of R ι G f i x)) : C z :=
...
variable (R ι G f) in
/-- The universal property of the colimit: maps from the components to another module
that respect the directed system structure (i.e. make some diagram commute) give rise
to a unique map out of the colimit. -/
...
theorem lift_unique [IsDirected ι (· ≤ ·)] (F : Colimit G f →ₗ[R] P) (x) :
    F x =
  ...
  · exact Colimit.induction_on x fun i x => by rw [lift_of]; rfl
...
lemma lift_injective [IsDirected ι (· ≤ ·)]
  ...
  intros z hz
  induction z using Colimit.induction_on with
  | ih _ g =>
    rw [lift_of] at hz
...
/--
Consider colimits `lim G` and `lim G'` with direct system `f` and `f'` respectively, any
...
theorem of.zero_exact_aux [∀ i (k : G i), Decidable (k ≠ 0)] [Nonempty ι] [IsDirected ι (· ≤ ·)]
    {x : DirectSum ι G} (H : (Submodule.Quotient.mk x : Colimit G f) = (0 : Colimit G f)) :
    ∃ j,
      (∀ k ∈ x.support, k ≤ j) ∧
...
/-- The colimit of a directed system is the abelian groups glued together along the maps. -/
def Colimit [DecidableEq ι] (f : ∀ i j, i ≤ j → G i →+ G j) : Type _ :=
  @Module.Colimit ℤ _ ι _ G _ _ (fun i j hij => (f i j hij).toIntLinearMap) _
...
attribute [local instance] Colimit.directedSystem

variable [DecidableEq ι]

instance : AddCommGroup (Colimit G f) :=
  Module.Colimit.addCommGroup G fun i j hij => (f i j hij).toIntLinearMap

instance : Inhabited (Colimit G f) :=
  ⟨0⟩

instance [IsEmpty ι] : Unique (Colimit G f) := Module.Colimit.unique _ _

/-- The canonical map from a component to the colimit. -/
def of (i) : G i →+ Colimit G f :=
  (Module.Colimit.of ℤ ι G (fun i j hij => (f i j hij).toIntLinearMap) i).toAddMonoidHom
...
/-- The universal property of the colimit: maps from the components to another abelian group
that respect the directed system structure (i.e. make some diagram commute) give rise
to a unique map out of the colimit. -/
def lift : Colimit G f →+ P :=
  (Module.Colimit.lift ℤ ι G (fun i j hij => (f i j hij).toIntLinearMap)
...
theorem lift_unique [IsDirected ι (· ≤ ·)] (F : Colimit G f →+ P) (x) :
    F x = lift G f P (fun i => F.comp (of G f i)) (fun i j hij x => by simp) x := by
  cases isEmpty_or_nonempty ι
  · simp_rw [Subsingleton.elim x 0, _root_.map_zero]
  · exact Colimit.induction_on x fun i x => by simp
...
@[simp] lemma map_id [IsDirected ι (· ≤ ·)] :
    map (fun _ ↦ AddMonoidHom.id _) (fun _ _ _ ↦ rfl) = AddMonoidHom.id (Colimit G f) :=
  DFunLike.ext _ _ fun x ↦ (isEmpty_or_nonempty ι).elim (by subsingleton) fun _ ↦
    x.induction_on fun i g ↦ by simp
...
/-- The colimit of a diagram is the rings glued together along the maps. -/
def Colimit : Type max v w :=
  FreeCommRing (Σi, G i) ⧸
    Ideal.span
      { a |
...
instance zero : Zero (Colimit G f) := by
  unfold Colimit
  exact ⟨0⟩
...
-- failed to synthesize CommMonoidWithZero (Ring.Colimit G f)
theorem of_f {i j} (hij) (x) : of G f j (f i j hij x) = of G f i x :=
...
nonrec theorem Polynomial.exists_of [Nonempty ι] [IsDirected ι (· ≤ ·)]
    (q : Polynomial (Colimit G fun i j h => f' i j h)) :
...
@[elab_as_elim]
theorem induction_on [Nonempty ι] [IsDirected ι (· ≤ ·)] {C : Colimit G f → Prop}
    (z : Colimit G f) (ih : ∀ i x, C (of G f i x)) : C z :=
...
theorem of.zero_exact_aux [Nonempty ι] [IsDirected ι (· ≤ ·)] {x : FreeCommRing (Σi, G i)}
    (H : (Ideal.Quotient.mk _ x : Colimit G fun i j h => f' i j h)
        = (0 : Colimit G fun i j h => f' i j h)) :
...
variable (G f) in
/-- The universal property of the colimit: maps from the components to another ring
that respect the directed system structure (i.e. make some diagram commute) give rise
to a unique map out of the colimit.
-/
...
instance nontrivial [DirectedSystem G fun i j h => f' i j h] :
    Nontrivial (Ring.Colimit G fun i j h => f' i j h) :=
  ⟨⟨0, 1,
      Nonempty.elim (by infer_instance) fun i : ι => by
        change (0 : Ring.Colimit G fun i j h => f' i j h) ≠ 1
        rw [← (Ring.Colimit.of _ _ _).map_one]
        · intro H; rcases Ring.Colimit.of.zero_exact H.symm with ⟨j, hij, hf⟩
          rw [(f' i j hij).map_one] at hf
          exact one_ne_zero hf⟩⟩

theorem exists_inv {p : Ring.Colimit G f} : p ≠ 0 → ∃ y, p * y = 1 :=
  Ring.Colimit.induction_on p fun i x H =>
    ⟨Ring.Colimit.of G f i x⁻¹, by
      rw [← (Ring.Colimit.of _ _ _).map_mul,
        mul_inv_cancel₀ fun h : x = 0 => H <| by rw [h, (Ring.Colimit.of _ _ _).map_zero],
        (Ring.Colimit.of _ _ _).map_one]⟩
...
noncomputable def inv (p : Ring.Colimit G f) : Ring.Colimit G f :=
  if H : p = 0 then 0 else Classical.choose (Colimit.exists_inv G f H)

protected theorem mul_inv_cancel {p : Ring.Colimit G f} (hp : p ≠ 0) : p * inv G f p = 1 := by
  rw [inv, dif_neg hp, Classical.choose_spec (Colimit.exists_inv G f hp)]

protected theorem inv_mul_cancel {p : Ring.Colimit G f} (hp : p ≠ 0) : inv G f p * p = 1 := by
  rw [_root_.mul_comm, Colimit.mul_inv_cancel G f hp]
...
protected noncomputable abbrev field [DirectedSystem G fun i j h => f' i j h] :
    Field (Ring.Colimit G fun i j h => f' i j h) where
  ...
  mul_inv_cancel := fun _ => Colimit.mul_inv_cancel G fun i j h => f' i j h
