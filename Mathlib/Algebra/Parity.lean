...
@[to_additive]
instance (priority := 10000) [DecidablePred (IsSquare : α → Prop)] : DecidablePred (IsSquare : αᵐᵒᵖ → Prop) :=
...
instance (priority := 10000) [DecidablePred (IsSquare : α → Prop)] : DecidablePred (Even : Additive α → Prop) :=
...
instance (priority := 10000) [DecidablePred (Even : α → Prop)] : DecidablePred (IsSquare : Multiplicative α → Prop) :=
