...
@[simp]
lemma isSquare_ofAdd_iff {a : α} : IsSquare (Multiplicative.ofAdd a) ↔ Even a := Iff.rfl

@[simp]
lemma even_toAdd_iff {a : Multiplicative α} :
...
lemma even_abs [AddGroup α] [LinearOrder α] {a : α} : Even |a| ↔ Even a := by
  cases abs_choice a
...
lemma even_iff_exists_two_mul (a : α) : Even a ↔ ∃ b, a = 2 * b := by
...
lemma Dvd.dvd.even (hab : a ∣ b) (ha : Even a) : Even b :=
  ha.trans_dvd hab
...
set_option linter.deprecated false in
lemma odd_iff_exists_bit1 : Odd a ↔ ∃ b, a = bit1 b :=
...
lemma Even.odd_add : Even a → Odd b → Odd (b + a) :=
  fun he ho ↦ by simp only [he.add_odd ho, add_comm b a]

lemma Odd.add_even (ha : Odd a) (hb : Even b) : Odd (a + b) := by
  rw [add_comm]
  exact hb.add_odd ha
...
lemma odd_two_mul_add_one (a : α) : Odd (2 * a + 1) :=
  ⟨a, rfl⟩
...
@[simp]
lemma Odd.mul : Odd a → Odd b → Odd (a * b) := by
  rintro ⟨a, rfl⟩ ⟨b, rfl⟩
  refine' ⟨2 * a * b + b + a, _⟩
...
lemma Odd.pos [Nontrivial α] {a : α} (hn : Odd a) : 0 < a := by
  obtain ⟨k, rfl⟩ := hn
...
lemma Odd.neg (ha : Odd a) : Odd (-a) := by
  obtain ⟨k, hk⟩ := ha
  use -(k + 1)
