...
lemma div_eq_of_eq_mul (hb : b ≠ 0) : a = c * b → a / b = c :=
  IsUnit.div_eq_of_eq_mul hb.isUnit
#align div_eq_of_eq_mul div_eq_of_eq_mul

lemma eq_div_of_mul_eq (hc : c ≠ 0) : a * c = b → a = b / c :=
...
lemma mul_mul_div (a : G₀) (hb : b ≠ 0) : a = a * b * (1 / b) :=
...
@[simp]
lemma divp_mk0 (a : G₀) {b : G₀} (hb : b ≠ 0) : a /ₚ Units.mk0 b hb = a / b :=
  divp_eq_div _ _
...
lemma div_mul_right (b : G₀) (ha : a ≠ 0) : a / (a * b) = 1 / b :=
...
lemma mul_div_cancel_left (b : G₀) (ha : a ≠ 0) : a * b / a = b :=
...
lemma div_div_cancel_left' (ha : a ≠ 0) : a / b / a = b⁻¹ :=
