...
@[congr]
lemma Invertible.congr [Monoid α] (a b : α) [Invertible a] [Invertible b] (h : a = b) :
...
lemma mul_right_inj_of_invertible [Monoid α] (c : α) [Invertible c] :
    a * c = b * c ↔ a = b :=
  ⟨fun h => by simpa using congr_arg (· * ⅟c) h, congr_arg (· * _)⟩

lemma mul_left_inj_of_invertible [Monoid α] (c : α) [Invertible c] :
    c * a = c * b ↔ a = b :=
  ⟨fun h => by simpa using congr_arg (⅟c * ·) h, congr_arg (_ * ·)⟩

lemma invOf_mul_eq_iff_eq_mul_left [Monoid α] [Invertible (c : α)] :
    ⅟c * a = b ↔ a = c * b := by
  rw [← mul_left_inj_of_invertible (c := c), mul_invOf_self_assoc]

lemma mul_left_eq_iff_eq_invOf_mul [Monoid α] [Invertible (c : α)] :
    c * a = b ↔ a = ⅟c * b := by
  rw [← mul_left_inj_of_invertible (c := ⅟c), invOf_mul_self_assoc]

lemma mul_invOf_eq_iff_eq_mul_right [Monoid α] [Invertible (c : α)] :
    a * ⅟c = b ↔ a = b * c := by
  rw [← mul_right_inj_of_invertible (c := c), mul_invOf_mul_self_cancel]

lemma mul_right_eq_iff_eq_mul_invOf [Monoid α] [Invertible (c : α)] :
    a * c = b ↔ a = b * ⅟c := by
