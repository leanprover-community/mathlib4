...
@[to_additive]
lemma mulSupport_eq_preimage (f : α → M) : mulSupport f = f ⁻¹' {1}ᶜ :=
  rfl
...
lemma mem_mulSupport {f : α → M} {x : α} : x ∈ mulSupport f ↔ f x ≠ 1 :=
  Iff.rfl
#align function.mem_mul_support Function.mem_mulSupport
#align function.mem_support Function.mem_support

@[to_additive (attr := simp)]
lemma mulSupport_subset_iff {f : α → M} {s : Set α} : mulSupport f ⊆ s ↔ ∀ x, f x ≠ 1 → x ∈ s :=
  Iff.rfl
...
@[to_additive]
lemma mulSupport_subset_iff' {f : α → M} {s : Set α} :
...
@[to_additive]
lemma mulSupport_eq_iff {f : α → M} {s : Set α} :
...
@[to_additive]
lemma mulSupport_disjoint_iff {f : α → M} {s : Set α} :
...
@[to_additive]
lemma disjoint_mulSupport_iff {f : α → M} {s : Set α} : Disjoint s (mulSupport f) ↔ EqOn f 1 s :=
...
@[to_additive (attr := simp)]
lemma mulSupport_along_fiber_finite_of_finite (f : α × β → M) (a : α)
...
@[to_additive]
lemma mulSupport_mul [MulOneClass M] (f g : α → M) :
    (mulSupport fun x => f x * g x) ⊆ mulSupport f ∪ mulSupport g :=
...
@[to_additive]
lemma mulSupport_pow [Monoid M] (f : α → M) (n : ℕ) :
...
@[to_additive (attr := simp)]
lemma mulSupport_inv : (mulSupport fun x => (f x)⁻¹) = mulSupport f :=
...
@[to_additive (attr := simp)]
lemma mulSupport_inv' : mulSupport f⁻¹ = mulSupport f :=
  mulSupport_inv f
#align function.mul_support_inv' Function.mulSupport_inv'
#align function.support_neg' Function.support_neg'

@[to_additive]
lemma mulSupport_mul_inv : (mulSupport fun x => f x * (g x)⁻¹) ⊆ mulSupport f ∪ mulSupport g :=
...
@[to_additive]
lemma mulSupport_div : (mulSupport fun x => f x / g x) ⊆ mulSupport f ∪ mulSupport g :=
...
@[simp]
lemma support_one : support (1 : α → R) = univ :=
  support_const one_ne_zero
#align function.support_one Function.support_one

@[simp]
lemma mulSupport_zero : mulSupport (0 : α → R) = univ :=
...
lemma mulSupport_nat_cast (hn : n ≠ 1) : mulSupport (n : α → R) = univ :=
...
lemma mulSupport_int_cast (hn : n ≠ 1) : mulSupport (n : α → R) = univ :=
...
lemma support_mul_subset_left [MulZeroClass R] (f g : α → R) :
...
--@[simp] Porting note: removing simp, bad lemma LHS not in normal form
lemma support_mul_subset_right [MulZeroClass R] (f g : α → R) :
...
lemma support_smul_subset_right [AddMonoid A] [Monoid B] [DistribMulAction B A] (b : B)
...
lemma support_smul_subset_left [Zero M] [Zero β] [SMulWithZero M β] (f : α → M) (g : α → β) :
...
lemma support_prod_subset [CommMonoidWithZero A] (s : Finset α) (f : α → β → A) :
...
lemma support_prod [CommMonoidWithZero A] [NoZeroDivisors A] [Nontrivial A] (s : Finset α)
...
lemma mulSupport_one_add [One R] [AddLeftCancelMonoid R] (f : α → R) :
...
lemma mulSupport_add_one [One R] [AddRightCancelMonoid R] (f : α → R) :
...
lemma mulSupport_one_sub' [One R] [AddGroup R] (f : α → R) : mulSupport (1 - f) = support f := by
  rw [sub_eq_add_neg, mulSupport_one_add', support_neg']
#align function.mul_support_one_sub' Function.mulSupport_one_sub'

lemma mulSupport_one_sub [One R] [AddGroup R] (f : α → R) :
    (mulSupport fun x => 1 - f x) = support f :=
...
@[to_additive]
lemma image_inter_mulSupport_eq {s : Set β} {g : β → α} :
