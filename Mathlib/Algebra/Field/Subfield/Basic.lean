...
theorem rangeRestrictField_eq_ofInjective (f : K →+* L) :
   (RingHom.rangeRestrictField f).toFun = (Equiv.ofInjective f (RingHom.injective f)).toFun :=
  rfl

/-- Ring isomorphism from a division ring to its range -/
@[simps]
noncomputable def rangeRestrictFieldEquiv (f : K →+* L) :
    K ≃+* f.fieldRange where
  toFun := RingHom.rangeRestrictField f
  invFun := (Equiv.ofInjective f (RingHom.injective f)).invFun
  left_inv := by
    rw [Function.leftInverse_iff_comp]
    ext x
    simp only [Function.comp_apply, id_eq]
    rw [show (f.rangeRestrictField x) = (RingHom.rangeRestrictField f).toFun x by rfl,
      rangeRestrictField_eq_ofInjective]
    simp
  right_inv := by
    rw [Function.rightInverse_iff_comp]
    ext x
    simp only [Function.comp_apply, id_eq]
    have (y : K) : (f.rangeRestrictField y) = (RingHom.rangeRestrictField f).toFun y := rfl
    rw [this, rangeRestrictField_eq_ofInjective]
    simp
  map_mul' := by simp
  map_add' := by simp
