...
theorem rangeRestrictField_bijective (f : K →+* L) :
    Function.Bijective (rangeRestrictField f) := by
  rw [show DFunLike.coe (rangeRestrictField f) = (Equiv.ofInjective f (RingHom.injective f)).toFun
    by rfl]
