...
structure IsField (R : Type*) [Semiring R] : Prop where
...
theorem Semifield.toIsField (R : Type*) [Semifield R] : IsField R where
  __ := ‹Semifield R›
  mul_inv_cancel {a} ha := ⟨a⁻¹, mul_inv_cancel₀ ha⟩

/-- Transferring from `Field` to `IsField`. -/
theorem Field.toIsField (R : Type*) [Field R] : IsField R :=
  Semifield.toIsField _

@[simp]
theorem IsField.nontrivial {R : Type*} [Semiring R] (h : IsField R) : Nontrivial R :=
  ⟨h.exists_pair_ne⟩

@[simp]
theorem not_isField_of_subsingleton (R : Type*) [Semiring R] [Subsingleton R] : ¬IsField R :=
  fun h =>
...
noncomputable def IsField.toSemifield {R : Type*} [Semiring R] (h : IsField R) : Semifield R where
  __ := ‹Semiring R›
  __ := h
...
noncomputable def IsField.toField {R : Type*} [Ring R] (h : IsField R) : Field R where
...
-/
theorem uniq_inv_of_isField (R : Type*) [Ring R] (hf : IsField R) :
