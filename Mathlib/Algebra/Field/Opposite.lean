...
@[to_additive]
instance (priority := 10000) ratCast [RatCast α] : RatCast αᵐᵒᵖ :=
  ⟨fun n => op n⟩
...
instance (priority := 10000) divisionSemiring [DivisionSemiring α] : DivisionSemiring αᵐᵒᵖ :=
  { MulOpposite.groupWithZero α, MulOpposite.semiring α with }

instance (priority := 10000) divisionRing [DivisionRing α] : DivisionRing αᵐᵒᵖ :=
...
instance (priority := 10000) semifield [Semifield α] : Semifield αᵐᵒᵖ :=
  { MulOpposite.divisionSemiring α, MulOpposite.commSemiring α with }

instance (priority := 10000) field [Field α] : Field αᵐᵒᵖ :=
...
instance (priority := 10000) divisionSemiring [DivisionSemiring α] : DivisionSemiring αᵃᵒᵖ :=
  { AddOpposite.groupWithZero α, AddOpposite.semiring α with }

instance (priority := 10000) divisionRing [DivisionRing α] : DivisionRing αᵃᵒᵖ :=
  { AddOpposite.ring α, AddOpposite.groupWithZero α, AddOpposite.ratCast α with
    ratCast_mk := fun a b hb h => unop_injective <| by
      rw [unop_ratCast, Rat.cast_def, unop_mul, unop_inv, unop_natCast, unop_intCast,
        div_eq_mul_inv] }

instance (priority := 10000) semifield [Semifield α] : Semifield αᵃᵒᵖ :=
  { AddOpposite.divisionSemiring, AddOpposite.commSemiring α with }

instance (priority := 10000) field [Field α] : Field αᵃᵒᵖ :=
