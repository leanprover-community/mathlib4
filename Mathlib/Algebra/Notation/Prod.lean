...
@[to_additive existing (attr := simp) (reorder := 5 6) smul_fst]
lemma pow_fst (p : α × β) (c : E) : (p ^ c).fst = p.fst ^ c := rfl

@[to_additive existing (attr := simp) (reorder := 5 6) smul_snd]
lemma pow_snd (p : α × β) (c : E) : (p ^ c).snd = p.snd ^ c := rfl

@[to_additive existing (attr := simp) (reorder := 5 6 7) smul_mk]
...
@[to_additive existing (attr := simp) (reorder := 5 6) smul_swap]
