...
theorem normSq_eq_zero : normSq a = 0 ↔ a = 0 := by
  ...
  rw [normSq_def', add_eq_zero_iff_of_nonneg_left, add_eq_zero_iff_of_nonneg_left,
    add_eq_zero_iff_of_nonneg_left] at h
...
@[norm_cast] lemma normSq_ratCast (q : ℚ) : normSq (q : ℍ[R]) = (q : ℍ[R]) ^ 2 := by
