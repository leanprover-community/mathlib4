...
theorem succ_dvd_or_succ_dvd_of_succ_sum_dvd_mul {p : M} (hp : Prime p) {a b : M} {k l : ℕ} :
...
theorem not_prime_pow {a : M} {n : ℕ} (hn : n ≠ 1) : ¬Prime (a ^ n) := fun hp =>
...
variable [CancelCommMonoidWithZero M] [MonoidNPow M]
