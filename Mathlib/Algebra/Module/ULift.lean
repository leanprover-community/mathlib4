...
@[to_additive]
instance (priority := 10000) smulLeft [SMul R M] : SMul (ULift R) M :=
  ⟨fun s x => s.down • x⟩
...
instance (priority := 10000) isScalarTower [SMul R M] [SMul M N] [SMul R N] [IsScalarTower R M N] :
...
instance (priority := 10000) [SMul R M] [SMul Rᵐᵒᵖ M] [IsCentralScalar R M] : IsCentralScalar R (ULift M) :=
  ⟨fun r m => congr_arg up <| op_smul_eq_smul r m.down⟩

-- Porting note: TODO this takes way longer to elaborate than it should
@[to_additive]
instance (priority := 10000) mulAction [Monoid R] [MulAction R M] : MulAction (ULift R) M where
  smul := (· • ·)
...
@[to_additive]
instance (priority := 10000) mulAction' [Monoid R] [MulAction R M] : MulAction R (ULift M) where
  smul := (· • ·)
...
instance (priority := 10000) smulZeroClass [Zero M] [SMulZeroClass R M] : SMulZeroClass (ULift R) M :=
  { ULift.smulLeft with smul_zero := fun _ => smul_zero _ }
#align ulift.smul_zero_class ULift.smulZeroClass

instance (priority := 10000) smulZeroClass' [Zero M] [SMulZeroClass R M] : SMulZeroClass R (ULift M) where
  smul_zero c := by { ext; simp [smul_zero] }
#align ulift.smul_zero_class' ULift.smulZeroClass'

instance (priority := 10000) distribSMul [AddZeroClass M] [DistribSMul R M] : DistribSMul (ULift R) M where
  smul_add _ := smul_add _
#align ulift.distrib_smul ULift.distribSMul

instance (priority := 10000) distribSMul' [AddZeroClass M] [DistribSMul R M] : DistribSMul R (ULift M) where
  smul_add c f g := by
    ext
    simp [smul_add]
#align ulift.distrib_smul' ULift.distribSMul'
...
instance (priority := 10000) mulDistribMulAction [Monoid R] [Monoid M] [MulDistribMulAction R M] :
...
instance (priority := 10000) smulWithZero [Zero R] [Zero M] [SMulWithZero R M] : SMulWithZero (ULift R) M :=
  { ULift.smulLeft with
    smul_zero := fun _ => smul_zero _
    zero_smul := zero_smul _ }
#align ulift.smul_with_zero ULift.smulWithZero

instance (priority := 10000) smulWithZero' [Zero R] [Zero M] [SMulWithZero R M] : SMulWithZero R (ULift M)
    where
  smul_zero _ := ULift.ext _ _ <| smul_zero _
  zero_smul _ := ULift.ext _ _ <| zero_smul _ _
#align ulift.smul_with_zero' ULift.smulWithZero'

instance (priority := 10000) mulActionWithZero [MonoidWithZero R] [Zero M] [MulActionWithZero R M] :
...
instance (priority := 10000) module [Semiring R] [AddCommMonoid M] [Module R M] : Module (ULift R) M :=
  { ULift.smulWithZero with
...
instance (priority := 10000) module' [Semiring R] [AddCommMonoid M] [Module R M] : Module R (ULift M) :=
  { ULift.smulWithZero' with
