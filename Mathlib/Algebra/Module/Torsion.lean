...
lemma CompleteLattice.Independent.linear_independent' {ι R M : Type*} {v : ι → M} [Ring R]
...
lemma torsionBySet_eq_torsionBySet_span :
...
lemma isTorsionBy_iff_torsionBy_eq_top : IsTorsionBy R M a ↔ torsionBy R M a = ⊤ := by
...
lemma torsionBy_isTorsionBy : IsTorsionBy R (torsionBy R M a) a := fun _ => smul_torsionBy _ _
...
lemma iSup_torsionBySet_ideal_eq_torsionBySet_iInf [DecidableEq ι] :
...
lemma supIndep_torsionBySet_ideal [DecidableEq ι] : S.SupIndep fun i => torsionBySet R M <| p i :=
...
lemma iSup_torsionBy_eq_torsionBy_prod [DecidableEq ι] :
...
lemma supIndep_torsionBy [DecidableEq ι] : S.SupIndep fun i => torsionBy R M <| q i := by
...
lemma torsionBySet_isInternal {p : ι → Ideal R}
...
lemma torsionBy_isInternal {q : ι → R} (hq : (S : Set ι).Pairwise <| (IsCoprime on q))
...
lemma isTorsion'_iff_torsion'_eq_top : IsTorsion' M S ↔ torsion' R M S = ⊤ :=
...
lemma torsion'_isTorsion' : IsTorsion' (torsion' R M S) S := fun ⟨_, ⟨a, h⟩⟩ => ⟨a, Subtype.ext h⟩
...
torsion module. -/
-- @[simp] Porting note: simp can prove this
lemma torsion_torsion_eq_top : torsion R (torsion R M) = ⊤ :=
...
lemma torsion_isTorsion : Module.IsTorsion R (torsion R M) :=
...
lemma noZeroSMulDivisors_iff_torsion_eq_bot : NoZeroSMulDivisors R M ↔ torsion R M = ⊥ := by
  constructor <;> intro h
