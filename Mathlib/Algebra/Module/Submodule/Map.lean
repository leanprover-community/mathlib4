...
noncomputable def equivMapOfInjective (f : F) (i : Injective f) (p : Submodule R M) :
  ...
  { Equiv.Set.image f p i with
    ...
    map_smul' := by
      intros
      -- Note:https://github.com/leanprover-community/mathlib4/issues/8386 changed `map_smulₛₗ` into `map_smulₛₗ _`
...
def comap [SemilinearMapClass F σ₁₂ M M₂] (f : F) (p : Submodule R₂ M₂) : Submodule R M :=
  { p.toAddSubmonoid.comap f with
    carrier := f ⁻¹' p
    -- Note:https://github.com/leanprover-community/mathlib4/issues/8386 added `map_smulₛₗ _`
