...
variable {R R₁ R₂ R₃ S : Type*}
variable {M M₁ M₂ M₃ N N₁ N₂ : Type*}

namespace Submodule

section AddCommMonoid

variable [Semiring R] [Semiring R₁] [Semiring R₂] [Semiring R₃] [Semiring S]
variable [AddCommMonoid M] [AddCommMonoid M₁] [AddCommMonoid M₂] [AddCommMonoid M₃]
variable [AddCommMonoid N]
variable [Module R M] [Module R₁ M₁] [Module R₂ M₂] [Module R₃ M₃] [Module S N]
variable {σ : R →+* S} {σ₁₂ : R₁ →+* R₂} {σ₂₃ : R₂ →+* R₃} {σ₁₃ : R₁ →+* R₃}
variable [RingHomCompTriple σ₁₂ σ₂₃ σ₁₃]
variable (p p' : Submodule R M) (q q' : Submodule S N)
variable {x : M}
...
/-- The pushforward of a submodule `p ⊆ M` by `f : M → N` -/
def map (f : M →ₛₗ[σ] N) (p : Submodule R M) : Submodule S N :=
...
@[simp]
theorem map_coe (f : M →ₛₗ[σ] N) (p : Submodule R M) : (map f p : Set N) = f '' p :=
  rfl

theorem map_toAddSubmonoid (f : M →ₛₗ[σ] N) (p : Submodule R M) :
    (p.map f).toAddSubmonoid = p.toAddSubmonoid.map (f : M →+ N) :=
...
@[simp]
theorem mem_map {f : M →ₛₗ[σ] N} {p : Submodule R M} {x : N} :
    x ∈ map f p ↔ ∃ y, y ∈ p ∧ f y = x :=
  Iff.rfl

theorem mem_map_of_mem {f : M →ₛₗ[σ] N} {p : Submodule R M} {r} (h : r ∈ p) : f r ∈ map f p :=
  Set.mem_image_of_mem _ h

theorem apply_coe_mem_map (f : M →ₛₗ[σ] N) {p : Submodule R M} (r : p) : f r ∈ map f p :=
...
theorem map_comp [RingHomSurjective σ₁₂] [RingHomSurjective σ₂₃] [RingHomSurjective σ₁₃]
    (f : M₁ →ₛₗ[σ₁₂] M₂) (g : M₂ →ₛₗ[σ₂₃] M₃) (p : Submodule R₁ M₁) :
    map (g.comp f : M₁ →ₛₗ[σ₁₃] M₃) p = map g (map f p) :=
  SetLike.coe_injective <| by simp only [← image_comp, map_coe, LinearMap.coe_comp, comp_apply]

@[gcongr]
theorem map_mono {f : M →ₛₗ[σ] N} {p p' : Submodule R M} : p ≤ p' → map f p ≤ map f p' :=
  image_mono

@[simp]
protected theorem map_zero : map (0 : M →ₛₗ[σ] N) p = ⊥ :=
  have : ∃ x : M, x ∈ p := ⟨0, p.zero_mem⟩
  ext <| by simp [this, eq_comm]

theorem map_add_le (f g : M →ₛₗ[σ] N) : map (f + g) p ≤ map f p ⊔ map g p := by
  rintro x ⟨m, hm, rfl⟩
  exact add_mem_sup (mem_map_of_mem hm) (mem_map_of_mem hm)

theorem map_inf_le (f : M →ₛₗ[σ] N) {p q : Submodule R M} :
    (p ⊓ q).map f ≤ p.map f ⊓ q.map f :=
  image_inter_subset f p q

theorem map_inf (f : M →ₛₗ[σ] N) {p q : Submodule R M} (hf : Injective f) :
    (p ⊓ q).map f = p.map f ⊓ q.map f :=
  SetLike.coe_injective <| Set.image_inter hf

lemma map_iInf {ι : Sort*} [Nonempty ι] {p : ι → Submodule R M} (f : M →ₛₗ[σ] N)
...
theorem range_map_nonempty (p : Submodule R M) :
    (Set.range (fun ϕ => Submodule.map ϕ p : (M →ₛₗ[σ] N) → Submodule S N)).Nonempty :=
...
variable {τ : S →+* R} [RingHomInvPair σ τ] [RingHomInvPair τ σ]
...
noncomputable def equivMapOfInjective (f : M →ₛₗ[σ] N) (i : Injective f) (p : Submodule R M) :
    p ≃ₛₗ[σ] p.map f :=
...
@[simp]
theorem coe_equivMapOfInjective_apply (f : M →ₛₗ[σ] N) (i : Injective f) (p : Submodule R M)
    (x : p) : (equivMapOfInjective f i p x : N) = f x :=
  rfl

@[simp]
theorem map_equivMapOfInjective_symm_apply (f : M →ₛₗ[σ] N) (i : Injective f) (p : Submodule R M)
...
/-- The pullback of a submodule `p ⊆ N` along `f : M → N` -/
def comap (f : M →ₛₗ[σ] N) (p : Submodule S N) : Submodule R M :=
...
@[simp]
theorem comap_coe (f : M →ₛₗ[σ] N) (p : Submodule S N) : (comap f p : Set M) = f ⁻¹' p :=
  rfl
...
@[simp]
theorem mem_comap {f : M →ₛₗ[σ] N} {p : Submodule S N} : x ∈ comap f p ↔ f x ∈ p :=
  Iff.rfl
...
@[gcongr]
theorem comap_mono {f : M →ₛₗ[σ] N} {q q' : Submodule S N} : q ≤ q' → comap f q ≤ comap f q' :=
  preimage_mono
...
theorem map_le_iff_le_comap {f : M →ₛₗ[σ] N} {p : Submodule R M} {q : Submodule S N} :
    map f p ≤ q ↔ p ≤ comap f q :=
  image_subset_iff

theorem gc_map_comap (f : M →ₛₗ[σ] N) : GaloisConnection (map f) (comap f)
  | _, _ => map_le_iff_le_comap

@[simp]
theorem map_bot (f : M →ₛₗ[σ] N) : map f ⊥ = ⊥ :=
  (gc_map_comap f).l_bot

@[simp]
theorem map_sup (f : M →ₛₗ[σ] N) : map f (p ⊔ p') = map f p ⊔ map f p' :=
  (gc_map_comap f : GaloisConnection (map f) (comap f)).l_sup

@[simp]
theorem map_iSup {ι : Sort*} (f : M →ₛₗ[σ] N) (p : ι → Submodule R M) :
    map f (⨆ i, p i) = ⨆ i, map f (p i) :=
  (gc_map_comap f : GaloisConnection (map f) (comap f)).l_iSup

lemma disjoint_map {f : M →ₛₗ[σ] N} (hf : Function.Injective f) {p q : Submodule R M}
...
@[simp]
theorem comap_top (f : M →ₛₗ[σ] N) : comap f ⊤ = ⊤ :=
  rfl

@[simp]
theorem comap_inf (f : M →ₛₗ[σ] N) : comap f (q ⊓ q') = comap f q ⊓ comap f q' :=
  rfl

@[simp]
theorem comap_iInf [RingHomSurjective σ] {ι : Sort*} (f : M →ₛₗ[σ] N)
    (p : ι → Submodule S N) : comap f (⨅ i, p i) = ⨅ i, comap f (p i) :=
  (gc_map_comap f).u_iInf

@[simp]
theorem comap_finsetInf [RingHomSurjective σ] {ι : Type*} (f : M →ₛₗ[σ] N)
    (s : Finset ι) (p : ι → Submodule S N) : comap f (s.inf p) = s.inf fun i ↦ comap f (p i) := by
  simp [Finset.inf_eq_iInf]

@[simp]
theorem comap_zero : comap (0 : M →ₛₗ[σ] N) q = ⊤ :=
  ext <| by simp

theorem map_comap_le [RingHomSurjective σ] (f : M →ₛₗ[σ] N) (q : Submodule S N) :
    map f (comap f q) ≤ q :=
  (gc_map_comap f).l_u_le _

theorem le_comap_map [RingHomSurjective σ] (f : M →ₛₗ[σ] N) (p : Submodule R M) :
    p ≤ comap f (map f p) :=
...
variable [RingHomSurjective σ] {f : M →ₛₗ[σ] N}
...
theorem map_comap_eq_of_surjective (p : Submodule S N) : (p.comap f).map f = p :=
...
theorem map_sup_comap_of_surjective (p q : Submodule S N) :
    (p.comap f ⊔ q.comap f).map f = p ⊔ q :=
  (giMapComap hf).l_sup_u _ _

theorem map_iSup_comap_of_surjective {ι : Sort*} (S : ι → Submodule S N) :
    (⨆ i, (S i).comap f).map f = iSup S :=
  (giMapComap hf).l_iSup_u _

theorem map_inf_comap_of_surjective (p q : Submodule S N) :
    (p.comap f ⊓ q.comap f).map f = p ⊓ q :=
  (giMapComap hf).l_inf_u _ _

theorem map_iInf_comap_of_surjective {ι : Sort*} (S : ι → Submodule S N) :
    (⨅ i, (S i).comap f).map f = iInf S :=
  (giMapComap hf).l_iInf_u _

theorem comap_le_comap_iff_of_surjective {p q : Submodule S N} : p.comap f ≤ q.comap f ↔ p ≤ q :=
  (giMapComap hf).u_le_u_iff

lemma comap_lt_comap_iff_of_surjective {p q : Submodule S N} : p.comap f < q.comap f ↔ p < q := by
...
lemma comap_lt_of_lt_map_of_injective {p : Submodule R M} {q : Submodule S N}
...
def orderIsoMapComapOfBijective (f : M →ₛₗ[σ] N) (hf : Bijective f) :
    Submodule R M ≃o Submodule S N where
  toFun := map f
  invFun := comap f
...
/-- A linear isomorphism induces an order isomorphism of submodules. -/
@[simps! apply]
def orderIsoMapComap (f : M ≃ₛₗ[σ] N) :
    Submodule R M ≃o Submodule S N := orderIsoMapComapOfBijective (f : M →ₛₗ[σ] N) f.bijective

@[simp]
lemma orderIsoMapComap_symm_apply (f : M ≃ₛₗ[σ] N) (p : Submodule S N) :
    (orderIsoMapComap f).symm p = comap (f : M →ₛₗ[σ] N) p :=
  rfl

variable {e : M ≃ₛₗ[σ] N}
variable {p}

@[simp] protected lemma map_eq_bot_iff : p.map (e : M →ₛₗ[σ] N) = ⊥ ↔ p = ⊥ :=
  map_eq_bot_iff (orderIsoMapComap e)

@[simp] protected lemma map_eq_top_iff : p.map (e : M →ₛₗ[σ] N) = ⊤ ↔ p = ⊤ :=
  map_eq_top_iff (orderIsoMapComap e)

protected lemma map_ne_bot_iff : p.map (e : M →ₛₗ[σ] N) ≠ ⊥ ↔ p ≠ ⊥ := by simp

protected lemma map_ne_top_iff : p.map (e : M →ₛₗ[σ] N) ≠ ⊤ ↔ p ≠ ⊤ := by simp
...
-- TODO: this section can be semilinearized for free

variable [Ring R] [AddCommGroup M] [Module R M] (p : Submodule R M)
variable [AddCommGroup N] [Module R N]

@[simp]
protected theorem map_neg (f : M →ₗ[R] N) : map (-f) p = map f p :=
  ext fun _ =>
...
@[simp]
lemma comap_neg {f : M →ₗ[R] N} {p : Submodule R N} :
    p.comap (-f) = p.comap f := by
  ext; simp

lemma map_toAddSubgroup (f : M →ₗ[R] N) (p : Submodule R M) :
    (p.map f).toAddSubgroup = p.toAddSubgroup.map (f : M →+ N) :=
  rfl
...
theorem comap_smul (f : V →ₗ[K] W) (p : Submodule K W) (a : K) (h : a ≠ 0) :
    p.comap (a • f) = p.comap f := by
  ext b; simp only [Submodule.mem_comap, p.smul_mem_iff h, LinearMap.smul_apply]

protected theorem map_smul (f : V →ₗ[K] W) (p : Submodule K V) (a : K) (h : a ≠ 0) :
    p.map (a • f) = p.map f :=
...
theorem comap_smul' (f : V →ₗ[K] W) (p : Submodule K W) (a : K) :
    p.comap (a • f) = ⨅ _ : a ≠ 0, p.comap f := by
  classical by_cases h : a = 0 <;> simp [h, comap_smul]

theorem map_smul' (f : V →ₗ[K] W) (p : Submodule K V) (a : K) :
...
variable [AddCommMonoid M] [AddCommMonoid N] [Module R M] [Module S N]
variable {σ : R →+* S} {τ : S →+* R}
variable [RingHomInvPair σ τ] [RingHomInvPair τ σ]
variable (p : Submodule R M) (q : Submodule S N)

@[simp high]
theorem mem_map_equiv {e : M ≃ₛₗ[σ] N} {x : N} :
    x ∈ p.map (e : M →ₛₗ[σ] N) ↔ e.symm x ∈ p := by
...
theorem map_equiv_eq_comap_symm (e : M ≃ₛₗ[σ] N) (K : Submodule R M) :
    K.map (e : M →ₛₗ[σ] N) = K.comap (e.symm : N →ₛₗ[τ] M) :=
  Submodule.ext fun _ => by rw [mem_map_equiv, mem_comap, LinearEquiv.coe_coe]

theorem comap_equiv_eq_map_symm (e : M ≃ₛₗ[σ] N) (K : Submodule S N) :
    K.comap (e : M →ₛₗ[σ] N) = K.map (e.symm : N →ₛₗ[τ] M) :=
  (map_equiv_eq_comap_symm e.symm K).symm

variable {p}

theorem map_symm_eq_iff (e : M ≃ₛₗ[σ] N) {K : Submodule S N} :
    K.map (e.symm : N →ₛₗ[τ] M) = p ↔ p.map (e : M →ₛₗ[σ] N) = K := by
  rw [map_equiv_eq_comap_symm]
  exact (orderIsoMapComap e).symm_apply_eq.trans eq_comm

theorem orderIsoMapComap_apply' (e : M ≃ₛₗ[σ] N) (p : Submodule R M) :
    orderIsoMapComap e p = comap (e.symm : N →ₛₗ[τ] M) p :=
  p.map_equiv_eq_comap_symm _

theorem orderIsoMapComap_symm_apply' (e : M ≃ₛₗ[σ] N) (p : Submodule S N) :
    (orderIsoMapComap e).symm p = map (e.symm : N →ₛₗ[τ] M) p :=
  p.comap_equiv_eq_map_symm _

theorem inf_comap_le_comap_add (f₁ f₂ : M →ₛₗ[σ] N) :
...
lemma surjOn_iff_le_map [RingHomSurjective σ] {f : M →ₛₗ[σ] N} {p : Submodule R M}
    {q : Submodule S N} : Set.SurjOn f p q ↔ q ≤ p.map f :=
  Iff.rfl
...
variable [Semiring R] [CommSemiring S] [CommSemiring R₂]
variable [AddCommMonoid M] [AddCommMonoid M₂] [Module R M] [Module R₂ M₂]
variable [AddCommMonoid N] [AddCommMonoid N₁] [AddCommMonoid N₂]
variable [Module S N] [Module S N₁] [Module S N₂]
variable {σ : R →+* S}
variable (p : Submodule R M) (q : Submodule S N)
variable (pₗ : Submodule S N₁) (qₗ : Submodule S N₂)

theorem comap_le_comap_smul (f : M →ₛₗ[σ] N) (c : S) : comap f q ≤ comap (c • f) q := by
  simp only [SetLike.le_def, mem_comap, LinearMap.smul_apply]
  exact fun _ h ↦ smul_mem _ _ h

theorem map_smul_le_map [RingHomSurjective σ] (f : M →ₛₗ[σ] N) (c : S) :
...
variable {module_M : Module R M} {module_N : Module S N}
variable {σ : R →+* S} {τ : S →+* R}
variable {reστ : RingHomInvPair σ τ} {reτσ : RingHomInvPair τ σ}
variable (e : M ≃ₛₗ[σ] N)
...
def submoduleMap (p : Submodule R M) : p ≃ₛₗ[σ] ↥(p.map (e : M →ₛₗ[σ] N) : Submodule S N) :=
  { ((e : M →ₛₗ[σ] N).domRestrict p).codRestrict (p.map (e : M →ₛₗ[σ] N)) fun x =>
      ⟨x, by
        simp only [LinearMap.domRestrict_apply, and_true, SetLike.coe_mem,
          SetLike.mem_coe]⟩ with
    invFun := fun y =>
      ⟨(e.symm : N →ₛₗ[τ] M) y, by
        rcases y with ⟨y', hy⟩
...
theorem submoduleMap_symm_apply (p : Submodule R M)
    (x : (p.map (e : M →ₛₗ[σ] N) : Submodule S N)) : ↑((e.submoduleMap p).symm x) = e.symm x :=
  rfl
