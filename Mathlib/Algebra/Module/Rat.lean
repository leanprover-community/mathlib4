...
instance SMulCommClass.nnrat [Monoid α] [AddCommMonoid M] [DistribSMul α M] [Module ℚ≥0 M] :
    SMulCommClass ℚ≥0 α M where
  smul_comm r x y := (map_nnrat_smul (DistribSMul.toAddMonoidHom M x) r y).symm

instance SMulCommClass.rat [Monoid α] [AddCommGroup M] [DistribSMul α M] [Module ℚ M] :
