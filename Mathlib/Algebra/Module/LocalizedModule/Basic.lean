...
theorem instIsTorsionFree (S : Submonoid R) [Module.IsTorsionFree R M] [IsLocalization S A]
    [IsLocalizedModule S f] : Module.IsTorsionFree A N where
  isSMulRegular c hc x y hxy := by
    ...
    simp only [Function.uncurry_apply_pair, mk'_smul_mk', mk'_eq_mk'_iff] at hxy ⊢
    obtain ⟨u, hl⟩ := hxy
