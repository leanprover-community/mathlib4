...
- `Module.IsFinitelyPresented`: A module is finitely presented if it is generated by some
  finite set `s` and the kernel of the presentation `Rˢ → M` is also finitely generated.

## Main results

- `Module.isFinitelyPresented_iff_finite`: If `R` is Noetherian, then f.p. iff f.g. on `R`-modules.

Suppose `0 → K → M → N → 0` is an exact sequence of `R`-modules.

- `Module.isFinitelyPresented_of_surjective`: If `M` is f.p., `K` is f.g., then `N` is f.p.

- `Module.IsFinitelyPresented.fg_ker`: If `M` is f.g., `N` is f.p., then `K` is f.g.

- `Module.isFinitelyPresented_of_ker`: If `N` and `K` is f.p., then `M` is also f.p.

- `Module.IsFinitelyPresented.isLocalizedModule_map`: If `M` and `N` are `R`-modules and `M` is
  f.p., and `S` is a submonoid of `R`, then `Hom(Mₛ, Nₛ)` is the localization of `Hom(M, N)`.
...
-/
class Module.IsFinitelyPresented : Prop where
  out : ∃ (s : Finset M), Submodule.span R (s : Set M) = ⊤ ∧
    (LinearMap.ker (Finsupp.linearCombination R ((↑) : s → M))).FG

@[deprecated Module.IsFinitelyPresented (since := "2026-07-16")]
abbrev Module.FinitePresentation (R M : Type*) [Semiring R] [AddCommMonoid M] [Module R M] : Prop :=
  Module.IsFinitelyPresented R M

instance (priority := 100) [h : Module.IsFinitelyPresented R M] : Module.Finite R M := by
  obtain ⟨s, hs₁, _⟩ := h
  exact ⟨s, hs₁⟩
...
theorem Module.IsFinitelyPresented.exists_fin [fp : Module.IsFinitelyPresented R M] :
...
alias Module.FinitePresentation.exists_fin := Module.IsFinitelyPresented.exists_fin

/-- A finitely presented module is isomorphic to the quotient of a finite free module by a finitely
generated submodule. -/
theorem Module.IsFinitelyPresented.equiv_quotient [Module.IsFinitelyPresented R M] [Small.{v} R] :
    ∃ (L : Type v) (_ : AddCommGroup L) (_ : Module R L) (K : Submodule R L)
      (_ : M ≃ₗ[R] L ⧸ K), Module.Free R L ∧ Module.Finite R L ∧ K.FG :=
  have ⟨_n, _K, e, fg⟩ := Module.IsFinitelyPresented.exists_fin R M
...
@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.equiv_quotient := Module.IsFinitelyPresented.equiv_quotient
...
lemma Module.isFinitelyPresented_of_finite [IsNoetherianRing R] [h : Module.Finite R M] :
    Module.IsFinitelyPresented R M := by
  obtain ⟨s, hs⟩ := h
  exact ⟨s, hs, IsNoetherian.noetherian _⟩

@[deprecated (since := "2026-07-16")]
alias Module.finitePresentation_of_finite := Module.isFinitelyPresented_of_finite

lemma Module.isFinitelyPresented_iff_finite [IsNoetherianRing R] :
    Module.IsFinitelyPresented R M ↔ Module.Finite R M :=
  ⟨fun _ ↦ inferInstance, fun _ ↦ isFinitelyPresented_of_finite R M⟩

@[deprecated (since := "2026-07-16")]
alias Module.finitePresentation_iff_finite := Module.isFinitelyPresented_iff_finite

variable {R M N}

lemma Module.isFinitelyPresented_of_free_of_surjective [Module.Free R M] [Module.Finite R M]
    (l : M →ₗ[R] N)
    (hl : Function.Surjective l) (hl' : (LinearMap.ker l).FG) :
    Module.IsFinitelyPresented R N := by
...
@[deprecated (since := "2026-07-16")]
alias Module.finitePresentation_of_free_of_surjective :=
  Module.isFinitelyPresented_of_free_of_surjective

-- Ideally this should be an instance but it makes mathlib much slower.
variable (R M) in
lemma Module.isFinitelyPresented_of_projective [Projective R M] [Module.Finite R M] :
    IsFinitelyPresented R M :=
  have ⟨_n, _f, _g, surj, _, hfg⟩ := Finite.exists_comp_eq_id_of_projective R M
  Module.isFinitelyPresented_of_free_of_surjective _ surj
    (LinearMap.ker_eq_range_of_comp_eq_id hfg ▸ .of_finite)

@[deprecated (since := "2026-07-16")]
alias Module.finitePresentation_of_projective := Module.isFinitelyPresented_of_projective

variable {ι} [Finite ι]

instance : Module.IsFinitelyPresented R R := Module.isFinitelyPresented_of_projective _ _
instance : Module.IsFinitelyPresented R (ι →₀ R) := Module.isFinitelyPresented_of_projective _ _
instance : Module.IsFinitelyPresented R (ι → R) := Module.isFinitelyPresented_of_projective _ _

lemma Module.isFinitelyPresented_of_surjective [h : Module.IsFinitelyPresented R M] (l : M →ₗ[R] N)
    (hl : Function.Surjective l) (hl' : (LinearMap.ker l).FG) :
    Module.IsFinitelyPresented R N := by
  classical
  obtain ⟨s, hs, hs'⟩ := h
  ...
  apply Module.isFinitelyPresented_of_free_of_surjective (l ∘ₗ linearCombination R Subtype.val)
    (hl.comp H)
...
@[deprecated (since := "2026-07-16")]
alias Module.finitePresentation_of_surjective := Module.isFinitelyPresented_of_surjective

lemma Module.IsFinitelyPresented.fg_ker [Module.Finite R M]
    [h : Module.IsFinitelyPresented R N] (l : M →ₗ[R] N) (hl : Function.Surjective l) :
...
@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.fg_ker := Module.IsFinitelyPresented.fg_ker

lemma Module.IsFinitelyPresented.fg_ker_iff [Module.IsFinitelyPresented R M]
    (l : M →ₗ[R] N) (hl : Function.Surjective l) :
    Submodule.FG (LinearMap.ker l) ↔ Module.IsFinitelyPresented R N :=
  ⟨isFinitelyPresented_of_surjective l hl, fun _ ↦ fg_ker l hl⟩

@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.fg_ker_iff := Module.IsFinitelyPresented.fg_ker_iff

lemma Module.isFinitelyPresented_of_ker [Module.IsFinitelyPresented R N]
    (l : M →ₗ[R] N) (hl : Function.Surjective l) [Module.IsFinitelyPresented R (LinearMap.ker l)] :
    Module.IsFinitelyPresented R M := by
  ...
  have inst : Module.Finite R (LinearMap.ker (l ∘ₗ π)) :=
    .of_fg <| Module.IsFinitelyPresented.fg_ker _ (hl.comp H)
  ...
  exact (Module.IsFinitelyPresented.fg_ker f hf).map (Submodule.subtype _)

@[deprecated (since := "2026-07-16")]
alias Module.finitePresentation_of_ker := Module.isFinitelyPresented_of_ker

/-- Given a split exact sequence `0 → M → N → P → 0` with `N` finitely presented,
then `M` is also finitely presented. -/
lemma Module.isFinitelyPresented_of_split_exact
    {P : Type*} [AddCommGroup P] [Module R P]
    [Module.IsFinitelyPresented R N]
  ...
  obtain ⟨e, rfl, rfl⟩ := ((Function.Exact.split_tfae' H).out 0 2 rfl rfl).mp
    ⟨hf, l, hl⟩
  refine Module.isFinitelyPresented_of_surjective (LinearMap.fst _ _ _ ∘ₗ e.toLinearMap)
...
@[deprecated (since := "2026-07-16")]
alias Module.finitePresentation_of_split_exact := Module.isFinitelyPresented_of_split_exact

/-- Given an exact sequence `0 → M → N → P → 0`
with `N` finitely presented and `P` projective, then `M` is also finitely presented. -/
lemma Module.isFinitelyPresented_of_projective_of_exact
    {P : Type*} [AddCommGroup P] [Module R P]
    [Module.IsFinitelyPresented R N] [Module.Projective R P]
    (f : M →ₗ[R] N) (g : N →ₗ[R] P)
  ...
  Module.isFinitelyPresented_of_split_exact f g l hl hf H

@[deprecated (since := "2026-07-16")]
alias Module.finitePresentation_of_projective_of_exact :=
  Module.isFinitelyPresented_of_projective_of_exact

lemma Module.IsFinitelyPresented.of_equiv (e : M ≃ₗ[R] N) [Module.IsFinitelyPresented R M] :
    Module.IsFinitelyPresented R N := by
  simp [← Module.IsFinitelyPresented.fg_ker_iff e.toLinearMap e.surjective, Submodule.fg_bot]

@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.of_equiv := Module.IsFinitelyPresented.of_equiv

lemma LinearEquiv.isFinitelyPresented_iff (e : M ≃ₗ[R] N) :
    Module.IsFinitelyPresented R M ↔ Module.IsFinitelyPresented R N :=
  ⟨fun _ ↦ .of_equiv e, fun _ ↦ .of_equiv e.symm⟩

@[deprecated (since := "2026-07-16")]
alias LinearEquiv.finitePresentation_iff := LinearEquiv.isFinitelyPresented_iff

namespace Module.IsFinitelyPresented
...
variable (M N) in
instance prod [Module.IsFinitelyPresented R M] [Module.IsFinitelyPresented R N] :
    Module.IsFinitelyPresented R (M × N) := by
  have hf : Function.Surjective (LinearMap.fst R M N) := LinearMap.fst_surjective
  have : IsFinitelyPresented R ↥(LinearMap.ker (LinearMap.fst R M N)) := by
    rw [LinearMap.ker_fst]
    exact .of_equiv (LinearEquiv.ofInjective (LinearMap.inr R M N) LinearMap.inr_injective)
  apply Module.isFinitelyPresented_of_ker (.fst R M N) hf

instance pi {ι : Type*} (M : ι → Type*)
    [∀ i, AddCommGroup (M i)] [∀ i, Module R (M i)] [∀ i, Module.IsFinitelyPresented R (M i)]
    [Finite ι] : Module.IsFinitelyPresented R (∀ i, M i) := by
  refine Module.pi_induction' (motive := fun N _ _ ↦ Module.IsFinitelyPresented R N)
      (motive' := fun N _ _ ↦ Module.IsFinitelyPresented R N) R ?_ ?_ ?_ ?_ M inferInstance
  · exact fun e (hN : Module.IsFinitelyPresented _ _) ↦ .of_equiv e
  · exact fun e (hN : Module.IsFinitelyPresented _ _) ↦ .of_equiv e
  · infer_instance
  · introv hN hN'
    infer_instance

end Module.IsFinitelyPresented

@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.of_subsingleton := Module.IsFinitelyPresented.of_subsingleton
@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.prod := Module.IsFinitelyPresented.prod
@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.pi := Module.IsFinitelyPresented.pi
...
variable (R M) in
lemma Module.IsFinitelyPresented.trans (S : Type*) [CommRing S] [Algebra R S]
    [Module S M] [IsScalarTower R S M] [Module.IsFinitelyPresented R S]
    [Module.IsFinitelyPresented S M] : Module.IsFinitelyPresented R M := by
  obtain ⟨n, K, e, hK⟩ := Module.IsFinitelyPresented.exists_fin S M
  let f : (Fin n → S) →ₗ[R] M := (e.symm ∘ₗ K.mkQ).restrictScalars R
  refine Module.isFinitelyPresented_of_surjective f (fun m ↦ ?_) ?_
...
@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.trans := Module.IsFinitelyPresented.trans

open TensorProduct in
instance {A} [CommRing A] [Algebra R A] [Module.IsFinitelyPresented R M] :
    Module.IsFinitelyPresented A (A ⊗[R] M) := by
  obtain ⟨n, f, hf⟩ := Module.Finite.exists_fin' R M
  have inst := Module.isFinitelyPresented_of_projective A (A ⊗[R] (Fin n → R))
  apply Module.isFinitelyPresented_of_surjective (f.baseChange A)
  ...
  have : Module.Finite R (LinearMap.ker f) :=
    .of_fg (Module.IsFinitelyPresented.fg_ker f hf)
  exact Submodule.fg_range _

open TensorProduct in
lemma Module.IsFinitelyPresented.of_isBaseChange
    {A} [CommRing A] [Algebra R A] [Module A N] [IsScalarTower R A N]
    (f : M →ₗ[R] N) (h : IsBaseChange A f) [Module.IsFinitelyPresented R M] :
    Module.IsFinitelyPresented A N :=
  Module.isFinitelyPresented_of_surjective
    h.equiv.toLinearMap h.equiv.surjective (by simpa using Submodule.fg_bot)

@[deprecated (since := "2026-07-16")]
alias FinitePresentation.of_isBaseChange := Module.IsFinitelyPresented.of_isBaseChange

open TensorProduct in
instance (S : Submonoid R) [Module.IsFinitelyPresented R M] :
    Module.IsFinitelyPresented (Localization S) (LocalizedModule S M) :=
  Module.IsFinitelyPresented.of_isBaseChange (LocalizedModule.mkLinearMap S M)
    ((isLocalizedModule_iff_isBaseChange S _ _).mp inferInstance)

lemma Module.IsFinitelyPresented.exists_lift_of_isLocalizedModule
    [h : Module.IsFinitelyPresented R M] (g : M →ₗ[R] N') :
...
@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.exists_lift_of_isLocalizedModule :=
  Module.IsFinitelyPresented.exists_lift_of_isLocalizedModule
...
localization at `S`. -/
lemma Module.exists_localizedMap_surjective_of_surjective [Module.IsFinitelyPresented R M]
  ...
  obtain ⟨φ, s, hφ⟩ := IsFinitelyPresented.exists_lift_of_isLocalizedModule S g (ϕ ∘ₗ f)
...
-/
lemma exists_bijective_map_powers [Module.Finite R M] [Module.IsFinitelyPresented R N]
  ...
  obtain ⟨l', s₀, H⟩ := Module.IsFinitelyPresented.exists_lift_of_isLocalizedModule S f
...
lemma Module.IsFinitelyPresented.exists_notMem_bijective [Module.Finite R M]
    [Module.IsFinitelyPresented R N] (f : M →ₗ[R] N) (p : Ideal R) [p.IsPrime] {Mₚ Nₚ : Type*}
...
@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.exists_notMem_bijective :=
  Module.IsFinitelyPresented.exists_notMem_bijective
...
-/
lemma Module.IsFinitelyPresented.exists_lift_equiv_of_isLocalizedModule
    [Module.IsFinitelyPresented R M] [Module.IsFinitelyPresented R N]
    (l : M' ≃ₗ[R] N') :
    ∃ (r : R) (hr : r ∈ S)
  ...
  obtain ⟨l', s, H⟩ := Module.IsFinitelyPresented.exists_lift_of_isLocalizedModule S g (l ∘ₗ f)
...
@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.exists_lift_equiv_of_isLocalizedModule :=
  Module.IsFinitelyPresented.exists_lift_equiv_of_isLocalizedModule

instance Module.IsFinitelyPresented.isLocalizedModule_map [Module.IsFinitelyPresented R M] :
  ...
  · intro h
    obtain ⟨h', s, e⟩ := Module.IsFinitelyPresented.exists_lift_of_isLocalizedModule S g (h ∘ₗ f)
    refine ⟨⟨h', s⟩, ?_⟩
    apply IsLocalizedModule.ext S f (IsLocalizedModule.map_units g)
...
@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.isLocalizedModule_map :=
  Module.IsFinitelyPresented.isLocalizedModule_map

instance Module.IsFinitelyPresented.isLocalizedModule_mapExtendScalars
...
@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.isLocalizedModule_mapExtendScalars :=
  Module.IsFinitelyPresented.isLocalizedModule_mapExtendScalars

instance [Module.IsFinitelyPresented R M] :
    IsLocalizedModule S (LocalizedModule.map S (M := M) (N := N)) :=
  Module.IsFinitelyPresented.isLocalizedModule_mapExtendScalars _ _ _ _
...
lemma IsLocalizedModule.exists_isLocalizedModule_powers_of_finitePresentation
    [Module.Finite R M] [Module.IsFinitelyPresented R M'] :
...
-/
noncomputable def Module.IsFinitelyPresented.linearEquivMap [Module.IsFinitelyPresented R M] :=
...
@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.linearEquivMap := Module.IsFinitelyPresented.linearEquivMap

lemma Module.IsFinitelyPresented.linearEquivMap_apply [Module.IsFinitelyPresented R M]
    (f : M →ₗ[R] N) : Module.IsFinitelyPresented.linearEquivMap S
...
@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.linearEquivMap_apply :=
  Module.IsFinitelyPresented.linearEquivMap_apply

@[simp]
lemma Module.IsFinitelyPresented.linearEquivMap_symm_apply [Module.IsFinitelyPresented R M]
    (f : M →ₗ[R] N) : (Module.IsFinitelyPresented.linearEquivMap S).symm ((IsLocalizedModule.map S
    (LocalizedModule.mkLinearMap S M) (LocalizedModule.mkLinearMap S N)) f) =
    (LocalizedModule.mkLinearMap S (M →ₗ[R] N)) f :=
  IsLocalizedModule.linearEquiv_symm_apply S _ _ f

@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.linearEquivMap_symm_apply :=
  Module.IsFinitelyPresented.linearEquivMap_symm_apply
...
-/
noncomputable def Module.IsFinitelyPresented.linearEquivMapExtendScalars
    [Module.IsFinitelyPresented R M] :=
  IsLocalizedModule.linearEquiv S (LocalizedModule.mkLinearMap S (M →ₗ[R] N))
...
@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.linearEquivMapExtendScalars :=
  Module.IsFinitelyPresented.linearEquivMapExtendScalars

lemma Module.IsFinitelyPresented.linearEquivMapExtendScalars_apply [Module.IsFinitelyPresented R M]
    (f : M →ₗ[R] N) : Module.IsFinitelyPresented.linearEquivMapExtendScalars S
...
@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.linearEquivMapExtendScalars_apply :=
  Module.IsFinitelyPresented.linearEquivMapExtendScalars_apply

@[simp]
lemma Module.IsFinitelyPresented.linearEquivMapExtendScalars_symm_apply
    [Module.IsFinitelyPresented R M] (f : M →ₗ[R] N) :
    (Module.IsFinitelyPresented.linearEquivMapExtendScalars S).symm
    ((IsLocalizedModule.mapExtendScalars S (LocalizedModule.mkLinearMap S M)
...
@[deprecated (since := "2026-07-16")]
alias Module.FinitePresentation.linearEquivMapExtendScalars_symm_apply :=
  Module.IsFinitelyPresented.linearEquivMapExtendScalars_symm_apply
