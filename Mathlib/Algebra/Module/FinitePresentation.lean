...
- `Module.FinitePresentation`: A module is finitely presented if it is finitely generated by some
  set `s` and the kernel of the presentation `Rˢ → M` is also finitely generated.
...
Also the instances finite + free => f.p. => finite is also provided
...
For finitely presented algeras, see `RingTheory/FinitePresentation`.
-/
...
instance (priority := 100) [IsNoetherianRing R] [h : Module.Finite R M] :
...
lemma Module.FinitePresentation.fg_ker [Module.Finite R M]
    ...
    Submodule.FG (LinearMap.ker l) := by
  classical
  obtain ⟨s, hs, hs'⟩ := h
...
open BigOperators
lemma Module.FinitePresentation.exists_lift_of_isLocalizedModule (S : Submonoid R)
...
open BigOperators
lemma Module.Finite.exists_smul_of_comp_eq_of_isLocalizedModule (S : Submonoid R)
    {N' : Type*} [AddCommGroup N'] [Module R N'] (f : N →ₗ[R] N') [IsLocalizedModule S f]
...
open BigOperators
lemma Module.FinitePresentation.isLocalizedModule_map (S : Submonoid R)
    ...
    [Module.FinitePresentation R M] : IsLocalizedModule S (IsLocalizedModule.map S f g) := by
  constructor
  · intro s
