...
def basis_nest_map : WithTop ι → Submodule R M
  | none => ⊤
  | some i => sInf {p | bm '' Set.Iio i ⊆ ↑p}
...
-/
def Basis.toNest : CompleteSublattice (Submodule R M) where
  ...
  sSupClosed' := by
    ...
    simp at hs
    simp
    rw [Set.subset_insert_iff] at hs
    cases' hs with h h'
    · apply Or.inr
      simp at h
      rw [sSup, CompleteLattice.toSupSet]
      sorry
    · apply Or.inl
      exact sup_eq_top_of_top_mem h'.1
  sInfClosed' := sorry
