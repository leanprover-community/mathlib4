...
-/
def LinearIndependent.NestMap : LatticeHom (WithBot (WithTop ι)) (Submodule R M) :=
...
-/
def LinearIndependent.toNest : Nest (Submodule R M) where
  carrier := Set.range (LinearIndependent.NestMap v)
