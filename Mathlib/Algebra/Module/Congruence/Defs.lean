...
section moduleConGen

variable {M} [Semiring S] [AddZeroClass M] [SMul S M]
...
inductive ModuleConGen.Rel (r : M → M → Prop) : M → M → Prop
  ...
  | smul (s : S) {x y : M} : Rel r x y → Rel r (s • x) (s • y)

/-- The `ModuleCon` generated by a module relation. -/
def moduleConGen (r : M → M → Prop) : ModuleCon S M where
  r := ModuleConGen.Rel r
  iseqv := { refl := fun _ => .refl, symm := .symm, trans := .trans }
  add' := .add
  smul := .smul
...
variable {M N} [Semiring S] [AddCommMonoid M] [Module S M] [AddCommMonoid N] [Module S N]
  (c : ModuleCon S M)

/-- The homomorphism on the quotient of an `S`-module by a congruence relation `c` induced by a
homomorphism constant on `c`'s equivalence classes. -/
def lift (f : M →ₗ[S] N) (H : ∀ a b, c.r a b → f a = f b) :
    c.Quotient →ₗ[S] N where
  toFun x := Quotient.liftOn x f H
  map_add' x y := Quotient.inductionOn₂ x y fun m n => map_add f m n
  map_smul' s x := Quotient.inductionOn x fun m => map_smul f s m

/-- The canonical linear map from `M` to its quotient `c.Quotient` by a `ModuleCon` `c`. -/
def mk' : M →ₗ[S] c.Quotient where
...
the quotient by `c`. -/
theorem eq {x y : M} : c.mk' x = c.mk' y ↔ c.r x y :=
  AddCon.eq c.toAddCon

/-- The diagram describing the universal property for quotients of modules commutes. -/
theorem lift_mk' (f : M →ₗ[S] N) (H : ∀ a b, c.r a b → f a = f b) (x : M) :
    lift c f H (c.mk' x) = f x :=
  rfl
