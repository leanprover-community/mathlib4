...
theorem intCast_mem_center [NonAssocRing M] (n : ℤ) : (n : M) ∈ Set.center M where
  ...
  left_assoc _ _ := match n with
    | (n : ℕ) => by norm_cast; rw [(natCast_mem_center _ n).left_assoc _ _]
    | Int.negSucc n => by
  ...
  mid_assoc _ _ := match n with
    | (n : ℕ) => by norm_cast; rw [(natCast_mem_center _ n).mid_assoc _ _]
    | Int.negSucc n => by
  ...
  right_assoc _ _ := match n with
    | (n : ℕ) => by norm_cast; rw [(natCast_mem_center _ n).right_assoc _ _]
    | Int.negSucc n => by
        simp only [Int.cast_negSucc, Nat.cast_add, Nat.cast_one, neg_add_rev]
