...
def powersHom [Monoid M] : M ≃ (Multiplicative ℕ →* M) :=
  Additive.ofMulEquiv.trans <| (multiplesHom _).trans <| AddMonoidHom.toMultiplicative''
#align powers_hom powersHom

/-- Monoid homomorphisms from `Multiplicative ℤ` are defined by the image
of `Multiplicative.ofAdd 1`. -/
def zpowersHom [Group G] : G ≃ (Multiplicative ℤ →* G) :=
  Additive.ofMulEquiv.trans <| (zmultiplesHom _).trans <| AddMonoidHom.toMultiplicative''
