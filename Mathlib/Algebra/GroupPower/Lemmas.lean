...
/-- Note that `AddCommMonoid.nat_smul_comm_class` requires stronger assumptions on `R`. -/
...
/-- Note that `AddCommMonoid.nat_is_scalar_tower` requires stronger assumptions on `R`. -/
