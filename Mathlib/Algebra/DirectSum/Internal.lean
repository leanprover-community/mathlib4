...
theorem coe_mul_apply_eq_sum_antidiagonal [AddMonoid ι] [HasAntidiagonal ι]
  ...
  classical
  simp_rw [coe_mul_apply_eq_dfinsuppSum, DFinsupp.sum, ← sum_product']
  refine sum_congr_of_eq_on_inter (by aesop) ?_ ?_
...
variable {A : ℕ → σ} [SetLike.GradedMonoid A]
...
theorem mul_apply_eq_zero {r r' : ⨁ i, A i} {m n : ℕ}
  ...
  obtain (hx | hx) : x.1 < m ∨ x.2 < n := by simp at hx; omega
