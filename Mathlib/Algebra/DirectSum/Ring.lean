...
theorem ringHom_ext' ⦃F G : (⨁ i, A i) →+* R⦄
  ...
  RingHom.ofClass_addMonoidHom_injective <| DirectSum.addHom_ext' h
...
@[simp]
theorem toSemiring_ofClass_addMonoidHom (f : ∀ i, A i →+ R) (hone hmul) :
...
def liftRingHom :
  ...
  right_inv F := by
    apply RingHom.ofClass_addMonoidHom_injective
    refine DirectSum.addHom_ext' (fun xi ↦ AddMonoidHom.ext (fun xv ↦ ?_))
    simp only [DirectSum.toAddMonoid_of, AddMonoidHom.comp_apply, toSemiring_ofClass_addMonoidHom]
