...
`FreeNonUnitalNonAssocAlgebra.of : α → FreeNonUnitalNonAssocAlgebra α` which has
the following universal property: if `A` is any `R`-algebra, and `f : α → A` is any function,
then this function is the composite of `FreeNonUnitalNonAssocAlgebra.of`
and a unique non-unital `R`-algebra homomorphism
`FreeNonUnitalNonAssocAlgebra.lift f : FreeNonUnitalNonAssocAlgebra α →ₙₐ[R] A`.

A typical element of `FreeNonUnitalNonAssocAlgebra α` is an `R`-linear combination of
...
"typical" element of `FreeAlgebra R α`.
-/
