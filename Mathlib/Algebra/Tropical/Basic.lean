...
instance (priority := 10000) [Inhabited R] : Inhabited (Tropical R) :=
  ⟨trop default⟩
...
instance (priority := 10000) [DecidableEq R] : DecidableEq (Tropical R) := fun _ _ =>
  decidable_of_iff _ injective_untrop.eq_iff

section Order

instance (priority := 10000) instLETropical [LE R] : LE (Tropical R) where le x y := untrop x ≤ untrop y
...
instance (priority := 10000) decidableLE [LE R] [DecidableRel ((· ≤ ·) : R → R → Prop)] :
...
instance (priority := 10000) instLTTropical [LT R] : LT (Tropical R) where lt x y := untrop x < untrop y
...
instance (priority := 10000) decidableLT [LT R] [DecidableRel ((· < ·) : R → R → Prop)] :
    DecidableRel ((· < ·) : Tropical R → Tropical R → Prop) := fun x y =>
  ‹DecidableRel (· < ·)› (untrop x) (untrop y)
#align tropical.decidable_lt Tropical.decidableLT

instance (priority := 10000) instPreorderTropical [Preorder R] : Preorder (Tropical R) :=
...
instance (priority := 10000) instPartialOrderTropical [PartialOrder R] : PartialOrder (Tropical R) :=
  { instPreorderTropical with le_antisymm := fun _ _ h h' => untrop_injective (le_antisymm h h') }

instance (priority := 10000) instZeroTropical [Top R] : Zero (Tropical R) :=
  ⟨trop ⊤⟩

instance (priority := 10000) instTopTropical [Top R] : Top (Tropical R) :=
  ⟨0⟩
...
instance (priority := 10000) [LE R] [OrderTop R] : OrderTop (Tropical R) :=
...
instance (priority := 10000) : Add (Tropical R) :=
  ⟨fun x y => trop (min (untrop x) (untrop y))⟩

instance (priority := 10000) instAddCommSemigroupTropical : AddCommSemigroup (Tropical R) where
  add := (· + ·)
...
instance (priority := 10000) instLinearOrderTropical : LinearOrder (Tropical R) :=
...
instance (priority := 10000) instAddCommMonoidTropical [OrderTop R] : AddCommMonoid (Tropical R) :=
...
instance (priority := 10000) [Add R] : Mul (Tropical R) :=
...
instance (priority := 10000) instOneTropical [Zero R] : One (Tropical R) :=
  ⟨trop 0⟩
...
instance (priority := 10000) instAddMonoidWithOneTropical [LinearOrder R] [OrderTop R] [Zero R] :
...
instance (priority := 10000) [Zero R] : Nontrivial (Tropical (WithTop R)) :=
  ⟨⟨0, 1, trop_injective.ne WithTop.top_ne_coe⟩⟩

instance (priority := 10000) [Neg R] : Inv (Tropical R) :=
  ⟨fun x => trop (-untrop x)⟩
...
instance (priority := 10000) [Sub R] : Div (Tropical R) :=
  ⟨fun x y => trop (untrop x - untrop y)⟩
...
instance (priority := 10000) instSemigroupTropical [AddSemigroup R] : Semigroup (Tropical R) where
  mul := (· * ·)
  mul_assoc _ _ _ := untrop_injective (add_assoc _ _ _)

instance (priority := 10000) instCommSemigroupTropical [AddCommSemigroup R] : CommSemigroup (Tropical R) :=
  { instSemigroupTropical with mul_comm := fun _ _ => untrop_injective (add_comm _ _) }

instance (priority := 10000) {α : Type*} [SMul α R] : Pow (Tropical R) α where pow x n := trop <| n • untrop x
...
instance (priority := 10000) instMulOneClassTropical [AddZeroClass R] : MulOneClass (Tropical R) where
  one := 1
  mul := (· * ·)
  one_mul _ := untrop_injective <| zero_add _
  mul_one _ := untrop_injective <| add_zero _

instance (priority := 10000) instMonoidTropical [AddMonoid R] : Monoid (Tropical R) :=
...
instance (priority := 10000) instCommMonoidTropical [AddCommMonoid R] : CommMonoid (Tropical R) :=
  { instMonoidTropical, instCommSemigroupTropical with }

instance (priority := 10000) instGroupTropical [AddGroup R] : Group (Tropical R) :=
...
instance (priority := 10000) [AddCommGroup R] : CommGroup (Tropical R) :=
...
instance (priority := 10000) covariant_mul [LE R] [Add R] [CovariantClass R R (· + ·) (· ≤ ·)] :
...
instance (priority := 10000) covariant_swap_mul [LE R] [Add R] [CovariantClass R R (Function.swap (· + ·)) (· ≤ ·)] :
...
instance (priority := 10000) covariant_add [LinearOrder R] : CovariantClass (Tropical R) (Tropical R) (· + ·) (· ≤ ·) :=
  ⟨fun x y z h => by
...
instance (priority := 10000) covariant_mul_lt [LT R] [Add R] [CovariantClass R R (· + ·) (· < ·)] :
    CovariantClass (Tropical R) (Tropical R) (· * ·) (· < ·) :=
  ⟨fun _ _ _ h => add_lt_add_left (untrop_lt_iff.2 h) _⟩
#align tropical.covariant_mul_lt Tropical.covariant_mul_lt

instance (priority := 10000) covariant_swap_mul_lt [Preorder R] [Add R]
...
instance (priority := 10000) instDistribTropical [LinearOrder R] [Add R] [CovariantClass R R (· + ·) (· ≤ ·)]
    [CovariantClass R R (Function.swap (· + ·)) (· ≤ ·)] :
...
instance (priority := 10000) : CommSemiring (Tropical R) :=
...
instance (priority := 10000) {R : Type*} [LinearOrderedAddCommMonoid R] : NoZeroDivisors (Tropical (WithTop R)) :=
  ⟨mul_eq_zero_iff.mp⟩
