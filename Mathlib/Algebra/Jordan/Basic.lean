...
instance (priority := 100) IsCommJordan.toIsJordan [CommMul A] [IsCommJordan A] : IsJordan A where
  ...
  lmul_lmul_comm_rmul a b := by
    rw [CommMul.mul_comm (a * a) (b * a), CommMul.mul_comm b a,
      IsCommJordan.lmul_comm_rmul_rmul, CommMul.mul_comm, CommMul.mul_comm b (a * a)]
  rmul_comm_rmul_rmul a b := by
    rw [CommMul.mul_comm b a, IsCommJordan.lmul_comm_rmul_rmul, CommMul.mul_comm]
...
theorem two_nsmul_lie_lmul_lmul_add_eq_lie_lmul_lmul_add (a b : A) :
  ...
  simp only [add_mul, mul_add, map_add, lie_add, add_lie, CommMul.mul_comm b a,
...
private theorem aux0 {a b c : A} : ⁅L (a + b + c), L ((a + b + c) * (a + b + c))⁆ =
  ...
  rw [CommMul.mul_comm b a, CommMul.mul_comm c a, CommMul.mul_comm c b]
...
private theorem aux3 {a b c : A} :
  ...
  conv_lhs => enter [1, 1, 2, 2, 2]; rw [CommMul.mul_comm a b]
  conv_lhs => enter [1, 2, 2, 2, 1]; rw [CommMul.mul_comm c a]
  conv_lhs => enter [   2, 2, 2, 2]; rw [CommMul.mul_comm b c]
