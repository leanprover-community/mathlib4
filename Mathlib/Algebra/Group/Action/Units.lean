...
/-- Note this instance has different defeqs than `Units.mulAction'`, but doesn't create a diamond
...
This instance however does create a (propeq) diamond for `MulDistribMulAction (ConjAct Mˣ) Mˣ` with
...
instance instMulDistribMulActionRight : MulDistribMulAction M Nˣ where
