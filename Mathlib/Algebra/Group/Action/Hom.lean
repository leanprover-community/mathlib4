...
@[to_additive]
abbrev compHom [Monoid N] (g : N →* M) : MonoidAction N α where
...
lemma compHom_smul_def
    {E F G : Type*} [Monoid E] [Monoid F] [MonoidAction F G] (f : E →* F) (a : E) (x : G) :
    letI : MonoidAction E G := MonoidAction.compHom _ f
    a • x = (f a) • x := rfl

/-- If an action is transitive, then composing this action with a surjective homomorphism gives
again a transitive action. -/
@[to_additive]
lemma isPretransitive_compHom {E F G : Type*} [Monoid E] [Monoid F] [MonoidAction F G]
    [IsPretransitive F G] {f : E →* F} (hf : Surjective f) :
    letI : MonoidAction E G := MonoidAction.compHom _ f
    IsPretransitive E G := by
  let _ : MonoidAction E G := MonoidAction.compHom _ f
  refine ⟨fun x y ↦ ?_⟩
...
@[to_additive]
lemma IsPretransitive.of_compHom {M N α : Type*} [Monoid M] [Monoid N] [MonoidAction N α]
...
def MonoidHom.smulOneHom {M N} [Monoid M] [MulOneClass N] [MonoidAction M N] [IsScalarTower M N N] :
    M →* N where
  toFun x := x • (1 : N)
...
def monoidHomEquivMonoidActionIsScalarTower (M N) [Monoid M] [Monoid N] :
    (M →* N) ≃ {_inst : MonoidAction M N // IsScalarTower M N N} where
  toFun f := ⟨MonoidAction.compHom N f, SMul.comp.isScalarTower _⟩
