...
This is similar to `IsScalarTower` but that the action of `G` on `R` and `S`
are all `MulDistribMulActions`.
...
instance : SMulDistribClass M N N where
  smul_distrib_smul := smul_mul'
