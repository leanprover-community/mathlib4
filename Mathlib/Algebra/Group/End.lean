...
theorem subtypePerm_inv (f : Perm α) (hf) :
    f⁻¹.subtypePerm hf = (f.subtypePerm (private_decl% (inv_aux.2 hf)) : Perm { x // p x })⁻¹ :=
  rfl

/-- See `Equiv.Perm.subtypePerm_inv`. -/
@[simp]
theorem inv_subtypePerm (f : Perm α) (hf) :
    (f.subtypePerm hf : Perm { x // p x })⁻¹ = f⁻¹.subtypePerm (private_decl% (inv_aux.1 hf)) :=
  rfl
...
theorem subtypePerm_pow (f : Perm α) (n : ℕ) (hf) :
    (f.subtypePerm hf : Perm { x // p x }) ^ n =
      (f ^ n).subtypePerm (private_decl% (pow_aux hf)) := by
  induction n with
  | zero => simp
...
theorem subtypePerm_zpow (f : Perm α) (n : ℤ) (hf) :
    (f.subtypePerm hf ^ n : Perm { x // p x }) =
      (f ^ n).subtypePerm (private_decl% (zpow_aux hf)) := by
  cases n with
