...
@[to_additive (attr := reducible) "A type endowed with `+` is an additive commutative semigroup,if it admits
...
@[to_additive (attr := reducible) "A type endowed with `0` and `+` is an `AddZeroClass`, if it admits an
...
@[to_additive (attr := reducible) "A type endowed with `0` and `+` is an additive monoid, if it admits an
...
@[to_additive (attr := reducible) "A type endowed with `0` and `+` is an additive left cancel monoid, if it
...
@[to_additive (attr := reducible) "A type endowed with `0` and `+` is an additive commutative monoid, if it
...
@[to_additive (attr := reducible) "A type endowed with `0` and `+` is an additive cancel commutative monoid,
...
@[to_additive (attr := reducible) "A type has an involutive negation if it admits a surjective map that
...
@[to_additive (attr := reducible) subNegMonoid "A type endowed with `0`, `+`, unary `-`, and binary `-` is a
...
@[to_additive (attr := reducible) subtractionMonoid "A type endowed with `0`, `+`, unary `-`, and binary `-`
...
See note [reducible non-instances]. -/
@[to_additive (attr := reducible) subtractionCommMonoid "A type endowed with `0`, `+`, unary `-`, and binary
...
@[to_additive (attr := reducible) "A type endowed with `0` and `+` is an additive group, if it admits an
...
@[to_additive (attr := reducible) "A type endowed with `0` and `+` is an additive commutative group, if it
...
@[to_additive (attr := reducible) "A type endowed with `+` is an additive semigroup, if it admits a
...
@[to_additive (attr := reducible) "A type endowed with `0` and `+` is an `AddZeroClass`, if it admits a
...
and `*` to a monoid. See note [reducible non-instances]. -/
@[to_additive (attr := reducible) "A type endowed with `0` and `+` is an additive monoid, if it admits a
...
`1`, `*` and `⁻¹` to a group. See note [reducible non-instances]. -/
@[to_additive (attr := reducible) "A type endowed with `0` and `+` is an additive group, if it admits a
