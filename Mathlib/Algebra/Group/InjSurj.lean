...
/-- A type endowed with `1` and `*` is a mul_one_class, if it admits an injective map that
preserves `1` and `*` to a mul_one_class.  See note [reducible non-instances]. -/
...
/-- A type endowed with `1`, `*`, `⁻¹`, and `/` is a `div_inv_monoid` if it admits an injective map
that preserves `1`, `*`, `⁻¹`, and `/` to a `div_inv_monoid`. See note [reducible non-instances]. -/
@[reducible, to_additive subNegMonoid "A type endowed with `0`, `+`, unary `-`, and binary `-` is a
`sub_neg_monoid` if it admits an injective map that preserves `0`, `+`, unary `-`, and binary `-` to
...
/-- A type endowed with `1`, `*`, `⁻¹`, and `/` is a `division_monoid` if it admits an injective map
that preserves `1`, `*`, `⁻¹`, and `/` to a `division_monoid`. See note [reducible non-instances]-/

@[reducible, to_additive "A type endowed with `0`, `+`, unary `-`, and binary `-` is a
`subtraction_monoid` if it admits an injective map that preserves `0`, `+`, unary `-`, and binary
`-` to a `subtraction_monoid`. This version takes custom `nsmul` and `zsmul` as `[has_smul ℕ M₁]`
and `[has_smul ℤ M₁]` arguments."]
...
/-- A type endowed with `1`, `*`, `⁻¹`, and `/` is a `division_comm_monoid` if it admits an
injective map that preserves `1`, `*`, `⁻¹`, and `/` to a `division_comm_monoid`.
See note [reducible non-instances]. -/
@[reducible, to_additive subtractionCommMonoid "A type endowed with `0`, `+`, unary `-`, and binary
`-` is a `subtraction_comm_monoid` if it admits an injective map that preserves `0`, `+`, unary `-`,
and binary `-` to a `subtraction_comm_monoid`. This version takes custom `nsmul` and `zsmul` as
...
/-- A type endowed with `1` and `*` is a mul_one_class, if it admits a surjective map that preserves
`1` and `*` from a mul_one_class. See note [reducible non-instances]. -/
@[reducible, to_additive "A type endowed with `0` and `+` is an add_zero_class, if it admits a
surjective map that preserves `0` and `+` to an add_zero_class."]
...
/-- A type endowed with `1`, `*`, `⁻¹`, and `/` is a `div_inv_monoid` if it admits a surjective map
that preserves `1`, `*`, `⁻¹`, and `/` to a `div_inv_monoid`. See note [reducible non-instances]. -/
@[reducible, to_additive subNegMonoid "A type endowed with `0`, `+`, unary `-`, and binary `-` is a
`sub_neg_monoid` if it admits a surjective map that preserves `0`, `+`, unary `-`, and binary `-` to
a `sub_neg_monoid`."]
