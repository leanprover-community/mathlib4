...
abbrev Group.ofLeftAxioms {G : Type u} [Mul G] [Inv G] [One G]
    ...
    (one_mul : ∀ a : G, 1 * a = a)
    (inv_mul_self : ∀ a : G, a⁻¹ * a = 1) : Group G :=
  { mul_assoc := assoc,
    one_mul := one_mul,
    inv_mul_self := inv_mul_self,
    mul_one := fun a => by
      have mul_inv_self : ∀ a : G, a * a⁻¹ = 1 := fun a =>
        calc a * a⁻¹ = 1 * (a * a⁻¹) := (one_mul _).symm
          ...
          _ = (a * a⁻¹)⁻¹ * (a * ((a⁻¹ * a) * a⁻¹)) := by
            simp only [assoc]
          _ = 1 := by
            rw [inv_mul_self, one_mul, inv_mul_self]
      rw [← inv_mul_self a, ← assoc, mul_inv_self a, one_mul] }
...
abbrev Group.ofRightAxioms {G : Type u} [Mul G] [Inv G] [One G]
    (assoc : ∀ a b c : G, (a * b) * c = a * (b * c))
    (mul_one : ∀ a : G, a * 1 = a)
    (mul_inv_self : ∀ a : G, a * a⁻¹ = 1) : Group G :=
  have inv_mul_self : ∀ a : G, a⁻¹ * a = 1 := fun a =>
    calc a⁻¹ * a = (a⁻¹ * a) * 1 := (mul_one _).symm
      ...
      _ = ((a⁻¹ * (a * a⁻¹)) * a) * (a⁻¹ * a)⁻¹ := by
        simp only [assoc]
      _ = 1 := by
        rw [mul_inv_self, mul_one, mul_inv_self]
  { mul_assoc := assoc,
    mul_one := mul_one,
    inv_mul_self := inv_mul_self,
    one_mul := fun a => by
      rw [← mul_inv_self a, assoc, inv_mul_self, mul_one] }
