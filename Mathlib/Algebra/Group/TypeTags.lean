...
instance (priority := 10000) [Subsingleton α] : Subsingleton (Additive α) := toMul.injective.subsingleton
instance (priority := 10000) [Subsingleton α] : Subsingleton (Multiplicative α) := toAdd.injective.subsingleton

instance (priority := 10000) [Inhabited α] : Inhabited (Additive α) :=
  ⟨ofMul default⟩

instance (priority := 10000) [Inhabited α] : Inhabited (Multiplicative α) :=
  ⟨ofAdd default⟩

instance (priority := 10000) [Unique α] : Unique (Additive α) := toMul.unique
instance (priority := 10000) [Unique α] : Unique (Multiplicative α) := toAdd.unique

instance (priority := 10000) [Finite α] : Finite (Additive α) :=
  Finite.of_equiv α (by rfl)

instance (priority := 10000) [Finite α] : Finite (Multiplicative α) :=
  Finite.of_equiv α (by rfl)

instance (priority := 10000) [h : Infinite α] : Infinite (Additive α) := h

instance (priority := 10000) [h : Infinite α] : Infinite (Multiplicative α) := h

instance (priority := 10000) [h : DecidableEq α] : DecidableEq (Multiplicative α) := h

instance (priority := 10000) [h : DecidableEq α] : DecidableEq (Additive α) := h

instance (priority := 10000) Additive.instNontrivial [Nontrivial α] : Nontrivial (Additive α) :=
  ofMul.injective.nontrivial
#align additive.nontrivial Additive.instNontrivial

instance (priority := 10000) Multiplicative.instNontrivial [Nontrivial α] : Nontrivial (Multiplicative α) :=
  ofAdd.injective.nontrivial
#align multiplicative.nontrivial Multiplicative.instNontrivial

instance (priority := 10000) Additive.add [Mul α] : Add (Additive α) where
  add x y := ofMul (toMul x * toMul y)

instance (priority := 10000) Multiplicative.mul [Add α] : Mul (Multiplicative α) where
...
instance (priority := 10000) Additive.addSemigroup [Semigroup α] : AddSemigroup (Additive α) :=
  { Additive.add with add_assoc := @mul_assoc α _ }

instance (priority := 10000) Multiplicative.semigroup [AddSemigroup α] : Semigroup (Multiplicative α) :=
  { Multiplicative.mul with mul_assoc := @add_assoc α _ }

instance (priority := 10000) Additive.addCommSemigroup [CommSemigroup α] : AddCommSemigroup (Additive α) :=
  { Additive.addSemigroup with add_comm := @mul_comm α _ }

instance (priority := 10000) Multiplicative.commSemigroup [AddCommSemigroup α] : CommSemigroup (Multiplicative α) :=
  { Multiplicative.semigroup with mul_comm := @add_comm α _ }

instance (priority := 10000) Additive.isLeftCancelAdd [Mul α] [IsLeftCancelMul α] : IsLeftCancelAdd (Additive α) :=
  ⟨@mul_left_cancel α _ _⟩

instance (priority := 10000) Multiplicative.isLeftCancelMul [Add α] [IsLeftCancelAdd α] :
    IsLeftCancelMul (Multiplicative α) :=
  ⟨@add_left_cancel α _ _⟩

instance (priority := 10000) Additive.isRightCancelAdd [Mul α] [IsRightCancelMul α] : IsRightCancelAdd (Additive α) :=
  ⟨@mul_right_cancel α _ _⟩

instance (priority := 10000) Multiplicative.isRightCancelMul [Add α] [IsRightCancelAdd α] :
    IsRightCancelMul (Multiplicative α) :=
  ⟨@add_right_cancel α _ _⟩

instance (priority := 10000) Additive.isCancelAdd [Mul α] [IsCancelMul α] : IsCancelAdd (Additive α) :=
  ⟨⟩

instance (priority := 10000) Multiplicative.isCancelMul [Add α] [IsCancelAdd α] : IsCancelMul (Multiplicative α) :=
  ⟨⟩

instance (priority := 10000) Additive.addLeftCancelSemigroup [LeftCancelSemigroup α] :
    AddLeftCancelSemigroup (Additive α) :=
  { Additive.addSemigroup, Additive.isLeftCancelAdd with }

instance (priority := 10000) Multiplicative.leftCancelSemigroup [AddLeftCancelSemigroup α] :
    LeftCancelSemigroup (Multiplicative α) :=
  { Multiplicative.semigroup, Multiplicative.isLeftCancelMul with }

instance (priority := 10000) Additive.addRightCancelSemigroup [RightCancelSemigroup α] :
    AddRightCancelSemigroup (Additive α) :=
  { Additive.addSemigroup, Additive.isRightCancelAdd with }

instance (priority := 10000) Multiplicative.rightCancelSemigroup [AddRightCancelSemigroup α] :
    RightCancelSemigroup (Multiplicative α) :=
  { Multiplicative.semigroup, Multiplicative.isRightCancelMul with }

instance (priority := 10000) [One α] : Zero (Additive α) :=
  ⟨Additive.ofMul 1⟩
...
instance (priority := 10000) [Zero α] : One (Multiplicative α) :=
...
instance (priority := 10000) Additive.addZeroClass [MulOneClass α] : AddZeroClass (Additive α) where
  zero := 0
  add := (· + ·)
  zero_add := @one_mul α _
  add_zero := @mul_one α _

instance (priority := 10000) Multiplicative.mulOneClass [AddZeroClass α] : MulOneClass (Multiplicative α) where
  one := 1
  mul := (· * ·)
  one_mul := @zero_add α _
  mul_one := @add_zero α _

instance (priority := 10000) Additive.addMonoid [h : Monoid α] : AddMonoid (Additive α) :=
...
instance (priority := 10000) Multiplicative.monoid [h : AddMonoid α] : Monoid (Multiplicative α) :=
...
instance (priority := 10000) Additive.addLeftCancelMonoid [LeftCancelMonoid α] : AddLeftCancelMonoid (Additive α) :=
  { Additive.addMonoid, Additive.addLeftCancelSemigroup with }

instance (priority := 10000) Multiplicative.leftCancelMonoid [AddLeftCancelMonoid α] :
    LeftCancelMonoid (Multiplicative α) :=
  { Multiplicative.monoid, Multiplicative.leftCancelSemigroup with }

instance (priority := 10000) Additive.addRightCancelMonoid [RightCancelMonoid α] : AddRightCancelMonoid (Additive α) :=
  { Additive.addMonoid, Additive.addRightCancelSemigroup with }

instance (priority := 10000) Multiplicative.rightCancelMonoid [AddRightCancelMonoid α] :
    RightCancelMonoid (Multiplicative α) :=
  { Multiplicative.monoid, Multiplicative.rightCancelSemigroup with }

instance (priority := 10000) Additive.addCommMonoid [CommMonoid α] : AddCommMonoid (Additive α) :=
  { Additive.addMonoid, Additive.addCommSemigroup with }

instance (priority := 10000) Multiplicative.commMonoid [AddCommMonoid α] : CommMonoid (Multiplicative α) :=
  { Multiplicative.monoid, Multiplicative.commSemigroup with }

instance (priority := 10000) Additive.neg [Inv α] : Neg (Additive α) :=
  ⟨fun x => ofAdd (toMul x)⁻¹⟩
...
instance (priority := 10000) Multiplicative.inv [Neg α] : Inv (Multiplicative α) :=
  ⟨fun x => ofMul (-toAdd x)⟩
...
instance (priority := 10000) Additive.sub [Div α] : Sub (Additive α) where
  sub x y := ofMul (toMul x / toMul y)
#align additive.has_sub Additive.sub

instance (priority := 10000) Multiplicative.div [Sub α] : Div (Multiplicative α) where
...
instance (priority := 10000) Additive.involutiveNeg [InvolutiveInv α] : InvolutiveNeg (Additive α) :=
  { Additive.neg with neg_neg := @inv_inv α _ }

instance (priority := 10000) Multiplicative.involutiveInv [InvolutiveNeg α] : InvolutiveInv (Multiplicative α) :=
  { Multiplicative.inv with inv_inv := @neg_neg α _ }

instance (priority := 10000) Additive.subNegMonoid [DivInvMonoid α] : SubNegMonoid (Additive α) :=
...
instance (priority := 10000) Multiplicative.divInvMonoid [SubNegMonoid α] : DivInvMonoid (Multiplicative α) :=
...
instance (priority := 10000) Additive.subtractionMonoid [DivisionMonoid α] : SubtractionMonoid (Additive α) :=
  { Additive.subNegMonoid, Additive.involutiveNeg with
    neg_add_rev := @mul_inv_rev α _
    neg_eq_of_add := @inv_eq_of_mul_eq_one_right α _ }

instance (priority := 10000) Multiplicative.divisionMonoid [SubtractionMonoid α] : DivisionMonoid (Multiplicative α) :=
...
instance (priority := 10000) Additive.subtractionCommMonoid [DivisionCommMonoid α] :
    SubtractionCommMonoid (Additive α) :=
  { Additive.subtractionMonoid, Additive.addCommSemigroup with }

instance (priority := 10000) Multiplicative.divisionCommMonoid [SubtractionCommMonoid α] :
    DivisionCommMonoid (Multiplicative α) :=
  { Multiplicative.divisionMonoid, Multiplicative.commSemigroup with }

instance (priority := 10000) Additive.addGroup [Group α] : AddGroup (Additive α) :=
  { Additive.subNegMonoid with add_left_neg := @mul_left_inv α _ }

instance (priority := 10000) Multiplicative.group [AddGroup α] : Group (Multiplicative α) :=
  { Multiplicative.divInvMonoid with mul_left_inv := @add_left_neg α _ }

instance (priority := 10000) Additive.addCommGroup [CommGroup α] : AddCommGroup (Additive α) :=
  { Additive.addGroup, Additive.addCommMonoid with }

instance (priority := 10000) Multiplicative.commGroup [AddCommGroup α] : CommGroup (Multiplicative α) :=
...
-/
instance (priority := 10000) Additive.coeToFun {α : Type*} {β : α → Sort*} [CoeFun α β] :
...
is often used for composition, without affecting the behavior of the function itself.
-/
instance (priority := 10000) Multiplicative.coeToFun {α : Type*} {β : α → Sort*} [CoeFun α β] :
