...
/-!
# Subgroups and `Finsupp`

* `Subgroup.mem_closure_range_iff` and friends express membership in a subgroup generated by a
  family of elements through `Finsupp.prod`.
* `Finsupp.mem_closure_setOf_single_iff` describes the additive subgroup of `α →₀ M` generated by
  the `Finsupp.single a y` with `a ∈ s` and `y` in a generating set of `M`: it consists exactly of
  the finitely supported functions supported in `s`. This is the `AddSubgroup` analogue of
  `Finsupp.supported_eq_span_single`, and refines `Finsupp.add_closure_setOf_eq_single`.
-/
...
For `s = Set.univ` and `G = Set.univ` this is `Finsupp.add_closure_setOf_eq_single`; the
`Submodule` analogue is `Finsupp.supported_eq_span_single`. -/
theorem mem_closure_setOf_single_iff (hG : AddSubgroup.closure G = ⊤) {f : α →₀ N} :
