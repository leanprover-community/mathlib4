...
/-- If the subgroup generated by a set `S` is equal to a normal subgroup `N`, then `N` is
the normal closure of `S`. -/
@[to_additive /-- If the additive subgroup generated by a set `S` is equal to a normal additive
subgroup `N`, then `N` is the additive normal closure of `S`. -/]
theorem normalClosure_eq_of_closure_eq {S : Set G} {N : Subgroup G} [N.Normal]
    (hS : closure S = N) : normalClosure S = N := by
  rw [← normalClosure_closure_eq_normalClosure, hS, normalClosure_eq_self]
