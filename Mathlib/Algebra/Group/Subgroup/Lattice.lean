...
theorem mem_closure_singleton [GroupZPow G] {x y : G} :
    y ∈ closure ({x} : Set G) ↔ ∃ n : ℤ, x ^ n = y := by
  refine
...
theorem closure_singleton_one : closure ({1} : Set G) = ⊥ := by
  let _ := DivInvMonoid.groupZPow G
...
@[to_additive]
theorem mem_closure_pair [GroupZPow C] {x y z : C} :
