...
@[to_additive]
lemma map_eq_one_iff [OneHomClass F M N] (f : F) (hf : Function.Injective f)
    {x : M} :
...
@[to_additive]
lemma map_ne_one_iff {R S F : Type*} [One R] [One S] [FunLike F R S] [OneHomClass F R S] (f : F)
...
@[to_additive]
lemma ne_one_of_map {R S F : Type*} [One R] [One S] [FunLike F R S] [OneHomClass F R S]
...
@[to_additive (attr := simp)]
lemma OneHom.coe_coe [OneHomClass F M N] (f : F) :
