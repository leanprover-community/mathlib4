...
@[simp]
theorem invOf_mul_cancel_left' (a b : α) {_ : Invertible a} : ⅟a * (a * b) = b := by
  rw [← mul_assoc, invOf_mul_self, one_mul]
example {G} [Group G] (a b : G) : a⁻¹ * (a * b) = b := inv_mul_cancel_left a b

theorem invOf_mul_cancel_left (a b : α) [Invertible a] : ⅟a * (a * b) = b :=
  invOf_mul_cancel_left' _ _

@[simp]
theorem mul_invOf_cancel_left' (a b : α) {_ : Invertible a} : a * (⅟a * b) = b := by
  rw [← mul_assoc, mul_invOf_self, one_mul]
example {G} [Group G] (a b : G) : a * (a⁻¹ * b) = b := mul_inv_cancel_left a b

theorem mul_invOf_cancel_left (a b : α) [Invertible a] : a * (⅟a * b) = b :=
  mul_invOf_cancel_left' a b

@[simp]
theorem invOf_mul_cancel_right' (a b : α) {_ : Invertible b} : a * ⅟b * b = a := by
  simp [mul_assoc]
example {G} [Group G] (a b : G) : a * b⁻¹ * b = a := inv_mul_cancel_right a b

theorem invOf_mul_cancel_right (a b : α) [Invertible b] : a * ⅟b * b = a :=
  invOf_mul_cancel_right' _ _

@[simp]
theorem mul_invOf_cancel_right' (a b : α) {_ : Invertible b} : a * b * ⅟b = a := by
  simp [mul_assoc]
example {G} [Group G] (a b : G) : a * b * b⁻¹ = a := mul_inv_cancel_right a b

theorem mul_invOf_cancel_right (a b : α) [Invertible b] : a * b * ⅟b = a :=
  mul_invOf_cancel_right' _ _

theorem invOf_eq_right_inv {a b : α} [Invertible a] (hac : a * b = 1) : ⅟a = b :=
  left_inv_eq_right_inv (invOf_mul_self _) hac

theorem invOf_eq_left_inv {a b : α} [Invertible a] (hac : b * a = 1) : ⅟a = b :=
  (left_inv_eq_right_inv hac (mul_invOf_self _)).symm

theorem invOf_eq_iff_right {a b : α} [Invertible a] : ⅟a = b ↔ a * b = 1 :=
  ⟨fun h ↦ by rw [← h, mul_invOf_self], invOf_eq_right_inv⟩

theorem invOf_eq_iff_left {a b : α} [Invertible a] : ⅟a = b ↔ b * a = 1 :=
  ⟨fun h ↦ by rw [← h, invOf_mul_self], invOf_eq_left_inv⟩

theorem invertible_unique (a b : α) [Invertible a] [Invertible b]
    (h : a = b) : ⅟a = ⅟b := by
  apply invOf_eq_right_inv
  rw [h, mul_invOf_self]

instance Invertible.subsingleton (a : α) : Subsingleton (Invertible a) :=
...
@[congr]
theorem Invertible.congr (a b : α) [Invertible a] [Invertible b] (h : a = b) :
    ⅟a = ⅟b :=
