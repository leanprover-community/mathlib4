...
instance : Group (MulAut M) where
  ...
  one_mul _ := rfl
  mul_one _ := rfl
  inv_mul_self := MulEquiv.self_trans_symm
...
def conj [Group G] : G →* MulAut G where
  toFun g :=
    { toFun := fun h => g * h * g⁻¹
      invFun := fun h => g⁻¹ * h * g
      left_inv := fun _ => by simp only [mul_assoc, inv_mul_cancel_left, inv_mul_self, mul_one]
      right_inv := fun _ => by simp only [mul_assoc, mul_inv_cancel_left, mul_inv_self, mul_one]
...
instance group : Group (AddAut A) where
  ...
  one_mul _ := rfl
  mul_one _ := rfl
  inv_mul_self := AddEquiv.self_trans_symm
  ...
  toFun g :=
    @Additive.ofMul (AddAut G)
      { toFun := fun h => g + h + -g
        ...
        invFun := fun h => -g + h + g
        left_inv := fun _ => by simp only [add_assoc, neg_add_cancel_left, neg_add_self, add_zero]
        right_inv := fun _ => by simp only [add_assoc, add_neg_cancel_left, add_neg_self, add_zero]
