...
additive monoids. -/]
theorem pi_le_iff [DecidableEq η] [Finite η] {H : ∀ i, Submonoid (f i)} {J : Submonoid (∀ i, f i)} :
...
theorem closure_pi [Finite η] {s : Π i, Set (f i)} (hs : ∀ i, 1 ∈ s i) :
    closure (Set.univ.pi fun i => s i) = pi Set.univ fun i => closure (s i) :=
  le_antisymm
    (closure_le.2 <| Set.pi_subset_pi_iff.2 <| .inl fun _ _ => subset_closure)
    (by
      classical
      apply pi_le_iff.mpr fun i => map_le_of_le_comap _ <| closure_le.2 fun _x hx =>
      subset_closure <| Set.mem_univ_pi.mpr fun j => by
        by_cases H : j = i
        · subst H
          simpa
        · simpa [H] using hs _)
