...
theorem closure_singleton_eq [MonoidNPow M] (x : M) :
    closure ({x} : Set M) = mrange (powersHom M x) :=
...
the element. -/
theorem mem_closure_singleton [MonoidNPow M] {x y : M} :
    y ∈ closure ({x} : Set M) ↔ ∃ n : ℕ, x ^ n = y := by
...
abbrev groupZPowPowers {x : M} {n : ℕ} (hpos : 0 < n) (hx : x ^ n = 1) :
    letI := groupPowers hpos hx
    GroupZPow (powers x) :=
  letI := groupPowers hpos hx
...
@[simp]
theorem map_powers {N : Type*} {F : Type*} [Monoid N] [MonoidNPow N] [FunLike F M N]
    [MonoidHomClass F M N] (f : F) (m : M) :
...
theorem closure_singleton_eq [AddMonoidNSMul A] (x : A) :
...
@[to_additive
      ...
      those two elements. -/]
theorem mem_closure_pair {A : Type*} [CommMonoid A] [MonoidNPow A] (a b c : A) :
...
theorem ofMul_image_powers_eq_multiples_ofMul [Monoid M] [MonoidNPow M] {x : M} :
...
theorem ofAdd_image_multiples_eq_powers_ofAdd [AddMonoid A] [AddMonoidNSMul A] {x : A} :
