...
theorem exists_finsetProd_of_mem_closure {M : Type*} [CommMonoid M] {s : Set M} {x : M}
  ...
  case mul_left y _ _ _ h_prod =>
    rcases h_prod with ⟨_, I, f, _, rfl⟩;
    exact ⟨Option I, Finset.univ, (Option.elim · y (f ·)),
          ⟨by intro i; induction i <;> simp [*],
          by simp [Finset.prod_attach]⟩⟩
