...
lemma mem_centralizer_iff [Mul M] {c : M} : c ∈ centralizer S ↔ ∀ m ∈ S, m * c = c * m :=
  Iff.rfl
...
lemma one_mem_centralizer [MulOneClass M] : (1 : M) ∈ centralizer S := by
...
@[simp]
lemma zero_mem_centralizer [MulZeroClass M] : (0 : M) ∈ centralizer S := by
  simp [mem_centralizer_iff]
...
@[simp]
lemma inv_mem_centralizer₀ [GroupWithZero M] (ha : a ∈ centralizer S) : a⁻¹ ∈ centralizer S :=
  (eq_or_ne a 0).elim
    (fun h => by
      rw [h, inv_zero]
...
@[simp]
lemma div_mem_centralizer₀ [GroupWithZero M] (ha : a ∈ centralizer S) (hb : b ∈ centralizer S) :
    a / b ∈ centralizer S := by
  rw [div_eq_mul_inv]
  exact mul_mem_centralizer ha (inv_mem_centralizer₀ hb)
#align set.div_mem_centralizer₀ Set.div_mem_centralizer₀
...
lemma centralizer_subset [Mul M] (h : S ⊆ T) : centralizer T ⊆ centralizer S := fun _ ht s hs =>
  ht s (h hs)
...
lemma center_subset_centralizer [Mul M] (S : Set M) : Set.center M ⊆ S.centralizer :=
...
lemma centralizer_eq_top_iff_subset {s : Set M} [Semigroup M] :
...
lemma centralizer_univ [Semigroup M] : centralizer univ = center M :=
