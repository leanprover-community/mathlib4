...
@[to_additive]
lemma SMul.smul_eq_hSMul {α β} [SMul α β] : (SMul.smul : α → β → β) = HSMul.hSMul := rfl
...
@[to_additive]
lemma mul_comm : ∀ a b : G, a * b = b * a := CommMagma.mul_comm
...
lemma zpow_natCast (a : G) : ∀ n : ℕ, a ^ (n : ℤ) = a ^ n
