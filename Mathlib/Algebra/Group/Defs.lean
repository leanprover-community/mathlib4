...
@[to_additive]
theorem npowRec'_two_mul {M : Type*} [Semigroup M] [One M]
    (mul_one : ∀ m : M, m * 1 = m) (one_mul : ∀ m : M, 1 * m = m) (k : ℕ) (m : M) :
...
@[nolint unusedArguments, nolint unusedHavesSuffices]
...
-- The unusedHavesSuffices linter is a false positive
@[nolint unusedArguments, nolint unusedHavesSuffices, to_additive existing]
