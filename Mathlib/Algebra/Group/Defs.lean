...
class AddMonoid (M : Type u) extends Zero M, Add M, AddSemigroup M, AddZeroClass M, NSMul M where
...
@[to_additive]
class Monoid (M : Type u) extends One M, Mul M, Semigroup M, MulOneClass M, NPow M where
  npow := npowRecAuto
