...
@[to_additive]
lemma mul_right_injective (a : G) : Injective (a * ·) := fun _ _ ↦ mul_left_cancel
...
@[to_additive]
lemma eq_one_iff_eq_one_of_mul_eq_one {a b : M} (h : a * b = 1) : a = 1 ↔ b = 1 := by
...
@[to_additive]
lemma mul_mul_mul_comm (a b c d : G) : a * b * (c * d) = a * c * (b * d) :=
...
lemma bit1_add [One M] (a b : M) : bit1 (a + b) = bit0 a + bit1 b :=
  (congr_arg (· + (1 : M)) <| bit0_add a b : _).trans (add_assoc _ _ _)
#align bit1_add bit1_add

lemma bit1_add' [One M] (a b : M) : bit1 (a + b) = bit1 a + bit0 b := by
...
@[simp]
lemma bit0_zero : bit0 (0 : M) = 0 :=
  add_zero _
#align bit0_zero bit0_zero

@[simp]
lemma bit1_zero [One M] : bit1 (0 : M) = 1 := by rw [bit1, bit0_zero, zero_add]
...
@[to_additive]
lemma inv_unique (hy : x * y = 1) (hz : x * z = 1) : y = z :=
...
@[to_additive (attr := simp)]
lemma mul_right_eq_self : a * b = a ↔ b = 1 := calc
...
@[to_additive (attr := simp)]
lemma mul_left_eq_self : a * b = b ↔ a = 1 := calc
...
@[to_additive (attr := simp)]
lemma inv_involutive : Function.Involutive (Inv.inv : G → G) :=
  inv_inv
#align inv_involutive inv_involutive
#align neg_involutive neg_involutive

@[to_additive (attr := simp)]
lemma inv_surjective : Function.Surjective (Inv.inv : G → G) :=
  inv_involutive.surjective
#align inv_surjective inv_surjective
#align neg_surjective neg_surjective

@[to_additive]
lemma inv_injective : Function.Injective (Inv.inv : G → G) :=
...
@[to_additive]
lemma inv_eq_iff_eq_inv : a⁻¹ = b ↔ a = b⁻¹ :=
...
@[to_additive]
lemma inv_comp_inv : Inv.inv ∘ Inv.inv = @id G :=
  inv_involutive.comp_self
#align inv_comp_inv inv_comp_inv
#align neg_comp_neg neg_comp_neg

@[to_additive]
lemma leftInverse_inv : LeftInverse (fun a : G ↦ a⁻¹) fun a ↦ a⁻¹ :=
  inv_inv
#align left_inverse_inv leftInverse_inv
#align left_inverse_neg leftInverse_neg

@[to_additive]
lemma rightInverse_inv : RightInverse (fun a : G ↦ a⁻¹) fun a ↦ a⁻¹ :=
  inv_inv
...
lemma inv_eq_one_div (x : G) : x⁻¹ = 1 / x := by rw [div_eq_mul_inv, one_mul]
#align inv_eq_one_div inv_eq_one_div
#align neg_eq_zero_sub neg_eq_zero_sub

@[to_additive]
lemma mul_one_div (x y : G) : x * (1 / y) = x / y :=
...
@[to_additive (attr := simp)]
lemma one_div (a : G) : 1 / a = a⁻¹ :=
  (inv_eq_one_div a).symm
#align one_div one_div
#align zero_sub zero_sub

@[to_additive]
lemma mul_div (a b c : G) : a * (b / c) = a * b / c := by simp only [mul_assoc, div_eq_mul_inv]
#align mul_div mul_div
#align add_sub add_sub

@[to_additive]
lemma div_eq_mul_one_div (a b : G) : a / b = a * (1 / b) := by rw [div_eq_mul_inv, one_div]
...
@[to_additive (attr := simp)]
lemma div_one (a : G) : a / 1 = a := by simp [div_eq_mul_inv]
#align div_one div_one
#align sub_zero sub_zero

@[to_additive]
lemma one_div_one : (1 : G) / 1 = 1 :=
  div_one _
...
@[to_additive]
lemma eq_inv_of_mul_eq_one_right (h : a * b = 1) : b = a⁻¹ :=
...
@[to_additive]
lemma eq_one_div_of_mul_eq_one_right (h : a * b = 1) : b = 1 / a :=
...
@[to_additive]
lemma div_ne_one_of_ne : a ≠ b → a / b ≠ 1 :=
  mt eq_of_div_eq_one
...
@[to_additive (attr := simp)]
lemma div_eq_inv_self : a / b = b⁻¹ ↔ a = 1 := by rw [div_eq_mul_inv, mul_left_eq_self]
#align div_eq_inv_self div_eq_inv_self
#align sub_eq_neg_self sub_eq_neg_self

@[to_additive]
lemma mul_left_surjective (a : G) : Surjective (a * ·) :=
...
@[to_additive]
lemma mul_right_surjective (a : G) : Function.Surjective fun x ↦ x * a := fun x ↦
...
@[to_additive]
lemma mul_eq_one_iff_inv_eq : a * b = 1 ↔ a⁻¹ = b :=
...
@[to_additive]
lemma div_right_injective : Function.Injective fun a ↦ b / a := by
  -- FIXME see above
  simp only [div_eq_mul_inv]
...
lemma div_self' (a : G) : a / a = 1 := by rw [div_eq_mul_inv, mul_right_inv a]
#align div_self' div_self'
#align sub_self sub_self

@[to_additive (attr := simp)]
lemma mul_div_cancel_right (a b : G) : a * b / b = a :=
...
@[to_additive (attr := simp)]
lemma mul_div_mul_right_eq_div (a b c : G) : a * c / (b * c) = a / b := by
...
@[to_additive (attr := simp)]
lemma div_left_inj : b / a = c / a ↔ b = c := by
...
lemma div_div_div_cancel_right' (a b c : G) : a / c / (b / c) = a / b := by
...
@[to_additive]
lemma leftInverse_div_mul_left (c : G) : Function.LeftInverse (fun x ↦ x / c) fun x ↦ x * c :=
...
@[to_additive]
lemma leftInverse_mul_left_div (c : G) : Function.LeftInverse (fun x ↦ x * c) fun x ↦ x / c :=
...
@[to_additive]
lemma leftInverse_mul_right_inv_mul (c : G) :
...
@[to_additive]
lemma leftInverse_inv_mul_mul_right (c : G) :
    Function.LeftInverse (fun x ↦ c⁻¹ * x) fun x ↦ c * x :=
...
@[to_additive]
lemma div_eq_of_eq_mul' {a b c : G} (h : a = b * c) : a / b = c := by
  rw [h, div_eq_mul_inv, mul_comm, inv_mul_cancel_left]
#align div_eq_of_eq_mul' div_eq_of_eq_mul'
#align sub_eq_of_eq_add' sub_eq_of_eq_add'

@[to_additive (attr := simp)]
lemma mul_div_mul_left_eq_div (a b c : G) : c * a / (c * b) = a / b := by
...
@[to_additive eq_sub_of_add_eq']
lemma eq_div_of_mul_eq'' (h : c * a = b) : a = b / c := by simp [h.symm]
#align eq_div_of_mul_eq'' eq_div_of_mul_eq''
#align eq_sub_of_add_eq' eq_sub_of_add_eq'

@[to_additive]
lemma eq_mul_of_div_eq' (h : a / b = c) : a = b * c := by simp [h.symm]
#align eq_mul_of_div_eq' eq_mul_of_div_eq'
#align eq_add_of_sub_eq' eq_add_of_sub_eq'

@[to_additive]
lemma mul_eq_of_eq_div' (h : b = c / a) : a * b = c := by
  rw [h, div_eq_mul_inv, mul_comm c, mul_inv_cancel_left]
#align mul_eq_of_eq_div' mul_eq_of_eq_div'
#align add_eq_of_eq_sub' add_eq_of_eq_sub'

@[to_additive sub_sub_self]
lemma div_div_self' (a b : G) : a / (a / b) = b := by simpa using mul_inv_cancel_left a b
...
@[to_additive (attr := simp)]
lemma div_div_cancel (a b : G) : a / (a / b) = b :=
  div_div_self' a b
...
@[to_additive eq_sub_iff_add_eq']
lemma eq_div_iff_mul_eq'' : a = b / c ↔ c * a = b := by rw [eq_div_iff_mul_eq', mul_comm]
#align eq_div_iff_mul_eq'' eq_div_iff_mul_eq''
#align eq_sub_iff_add_eq' eq_sub_iff_add_eq'

@[to_additive]
lemma div_eq_iff_eq_mul' : a / b = c ↔ a = b * c := by rw [div_eq_iff_eq_mul, mul_comm]
#align div_eq_iff_eq_mul' div_eq_iff_eq_mul'
#align sub_eq_iff_eq_add' sub_eq_iff_eq_add'

@[to_additive (attr := simp)]
lemma mul_div_cancel_left (a b : G) : a * b / a = b := by rw [div_eq_inv_mul, inv_mul_cancel_left]
#align mul_div_cancel''' mul_div_cancel_left
#align add_sub_cancel' add_sub_cancel_left

@[to_additive (attr := simp)]
lemma mul_div_cancel (a b : G) : a * (b / a) = b := by
  rw [← mul_div_assoc, mul_div_cancel_left]
#align mul_div_cancel'_right mul_div_cancel
#align add_sub_cancel'_right add_sub_cancel

@[to_additive (attr := simp)]
lemma div_mul_cancel_left (a b : G) : a / (a * b) = b⁻¹ := by rw [← inv_div, mul_div_cancel_left]
...
@[to_additive]
lemma mul_mul_inv_cancel'_right (a b : G) : a * (b * a⁻¹) = b := by
...
@[to_additive]
lemma div_eq_div_iff_mul_eq_mul : a / b = c / d ↔ a * d = c * b := by
...
@[to_additive]
lemma div_eq_div_iff_div_eq_div : a / b = c / d ↔ a / c = b / d := by
