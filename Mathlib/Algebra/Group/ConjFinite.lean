...
instance [Fintype α] [DecidableRel (IsConj : α → α → Prop)] : Fintype (ConjClasses α) := fast_instance%
  Quotient.fintype (IsConj.setoid α)

instance [Finite α] : Finite (ConjClasses α) := fast_instance%
  Quotient.finite _

instance [DecidableEq α] [Fintype α] : DecidableRel (IsConj : α → α → Prop) := fast_instance% fun a b =>
...
instance {x : ConjClasses α} : Fintype (carrier x) := fast_instance%
