...
@[to_additive]
instance instMul [Mul M] [Mul N] : Mul (M × N) := fast_instance%
...
@[to_additive]
instance instOne [One M] [One N] : One (M × N) := fast_instance%
  ⟨(1, 1)⟩
...
@[to_additive]
instance instInv [Inv M] [Inv N] : Inv (M × N) := fast_instance%
  ⟨fun p => (p.1⁻¹, p.2⁻¹)⟩
...
@[to_additive]
instance [InvolutiveInv M] [InvolutiveInv N] : InvolutiveInv (M × N) := fast_instance%
  { inv_inv := fun _ => ext (inv_inv _) (inv_inv _) }

@[to_additive]
instance instDiv [Div M] [Div N] : Div (M × N) := fast_instance%
  ⟨fun p q => ⟨p.1 / q.1, p.2 / q.2⟩⟩
...
instance [MulZeroClass M] [MulZeroClass N] : MulZeroClass (M × N) := fast_instance%
...
@[to_additive]
instance instSemigroup [Semigroup M] [Semigroup N] : Semigroup (M × N) := fast_instance%
  { mul_assoc := fun _ _ _ => mk.inj_iff.mpr ⟨mul_assoc _ _ _, mul_assoc _ _ _⟩ }

@[to_additive]
instance instCommSemigroup [CommSemigroup G] [CommSemigroup H] : CommSemigroup (G × H) := fast_instance%
  { mul_comm := fun _ _ => mk.inj_iff.mpr ⟨mul_comm _ _, mul_comm _ _⟩ }

instance [SemigroupWithZero M] [SemigroupWithZero N] : SemigroupWithZero (M × N) := fast_instance%
  { zero_mul := by simp,
    mul_zero := by simp }

@[to_additive]
instance instMulOneClass [MulOneClass M] [MulOneClass N] : MulOneClass (M × N) := fast_instance%
...
@[to_additive]
instance instMonoid [Monoid M] [Monoid N] : Monoid (M × N) := fast_instance%
...
instance [DivInvMonoid G] [DivInvMonoid H] : DivInvMonoid (G × H) := fast_instance%
...
@[to_additive]
instance [DivisionMonoid G] [DivisionMonoid H] : DivisionMonoid (G × H) := fast_instance%
...
instance [DivisionCommMonoid G] [DivisionCommMonoid H] : DivisionCommMonoid (G × H) := fast_instance%
  { mul_comm := fun ⟨g₁ , h₁⟩ ⟨_, _⟩ => by rw [mk_mul_mk, mul_comm g₁, mul_comm h₁]; rfl }

@[to_additive]
instance instGroup [Group G] [Group H] : Group (G × H) := fast_instance%
...
@[to_additive]
instance [LeftCancelSemigroup G] [LeftCancelSemigroup H] : LeftCancelSemigroup (G × H) := fast_instance%
  { mul_left_cancel := fun _ _ _ => mul_left_cancel }

@[to_additive]
instance [RightCancelSemigroup G] [RightCancelSemigroup H] : RightCancelSemigroup (G × H) := fast_instance%
  { mul_right_cancel := fun _ _ _ => mul_right_cancel }

@[to_additive]
instance [LeftCancelMonoid M] [LeftCancelMonoid N] : LeftCancelMonoid (M × N) := fast_instance%
  { mul_one := by simp,
    one_mul := by simp }

@[to_additive]
instance [RightCancelMonoid M] [RightCancelMonoid N] : RightCancelMonoid (M × N) := fast_instance%
  { mul_one := by simp,
    one_mul := by simp }

@[to_additive]
instance [CancelMonoid M] [CancelMonoid N] : CancelMonoid (M × N) := fast_instance%
  { mul_right_cancel := by simp only [mul_left_inj, imp_self, forall_const] }

@[to_additive]
instance instCommMonoid [CommMonoid M] [CommMonoid N] : CommMonoid (M × N) := fast_instance%
  { mul_comm := fun ⟨m₁, n₁⟩ ⟨_, _⟩ => by rw [mk_mul_mk, mk_mul_mk, mul_comm m₁, mul_comm n₁] }

@[to_additive]
instance [CancelCommMonoid M] [CancelCommMonoid N] : CancelCommMonoid (M × N) := fast_instance%
  { mul_comm := fun ⟨m₁, n₁⟩ ⟨_, _⟩ => by rw [mk_mul_mk, mk_mul_mk, mul_comm m₁, mul_comm n₁] }

instance [MulZeroOneClass M] [MulZeroOneClass N] : MulZeroOneClass (M × N) := fast_instance%
  { zero_mul := by simp,
    mul_zero := by simp }

instance [MonoidWithZero M] [MonoidWithZero N] : MonoidWithZero (M × N) := fast_instance%
  { zero_mul := by simp,
    mul_zero := by simp }

instance [CommMonoidWithZero M] [CommMonoidWithZero N] : CommMonoidWithZero (M × N) := fast_instance%
  { zero_mul := by simp,
    mul_zero := by simp }

@[to_additive]
instance instCommGroup [CommGroup G] [CommGroup H] : CommGroup (G × H) := fast_instance%
