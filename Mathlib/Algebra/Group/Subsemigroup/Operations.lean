...
theorem Subsemigroup.toAddSubsemigroup_closure (S : Set M) :
  ...
  le_antisymm
    (Subsemigroup.toAddSubsemigroup.le_symm_apply.1 <|
      SetLike.closure_le.2 (SetLike.subset_closure (AddSubsemigroup (Additive M))))
    (SetLike.closure_le.2 (SetLike.subset_closure (Subsemigroup M)))
...
theorem AddSubsemigroup.toSubsemigroup_closure (S : Set A) :
  ...
  le_antisymm
    (AddSubsemigroup.toSubsemigroup.to_galoisConnection.l_le <|
      SetLike.closure_le.2 <| SetLike.subset_closure (Subsemigroup (Multiplicative A)))
    (SetLike.closure_le.2 <| SetLike.subset_closure (AddSubsemigroup A))
...
def topEquiv : (⊤ : Subsemigroup M) ≃* M where
  toFun x := x
  invFun x := ⟨x, SetLike.mem_top _ x⟩
  left_inv x := x.eta _
  map_mul' _ _ := rfl
...
theorem closure_closure_coe_preimage {s : Set M} :
  ...
  eq_top_iff.2 fun x _ ↦ Subtype.recOn x fun _ hx' ↦
    closure_induction (fun _ h ↦ SetLike.subset_closure _ h) (fun _ _ _ _ ↦ mul_mem) hx'
...
theorem srange_eq_top_iff_surjective {N} [Mul N] {f : M →ₙ* N} :
  ...
  SetLike.ext'_iff.trans <| Iff.trans (by rw [coe_srange, SetLike.coe_top]) Set.range_eq_univ
...
theorem mclosure_preimage_le (f : M →ₙ* N) (s : Set N) : closure (f ⁻¹' s) ≤ (closure s).comap f :=
  SetLike.closure_le.2 fun _ hx => SetLike.mem_coe.2 <| mem_comap.2 <| SetLike.subset_closure _ hx
...
theorem map_mclosure (f : M →ₙ* N) (s : Set M) : (closure s).map f = closure (f '' s) :=
  Set.image_preimage.l_comm_of_u_comm (gc_map_comap f)
    (SetLike.gi (Subsemigroup N)).gc (SetLike.gi (Subsemigroup M)).gc fun _ ↦ rfl
...
theorem eq_top_iff' : S = ⊤ ↔ ∀ x : M, x ∈ S :=
  eq_top_iff.trans ⟨fun h m => h <| SetLike.mem_top _ m, fun h m _ => h m⟩
