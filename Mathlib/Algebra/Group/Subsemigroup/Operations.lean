...
theorem Subsemigroup.toAddSubsemigroup_closure (S : Set M) :
  ...
  le_antisymm
    (Subsemigroup.toAddSubsemigroup.le_symm_apply.1 <|
      LatticeSetLike.closure_le.2
        (LatticeSetLike.subset_closure (α := Additive M) (L := AddSubsemigroup (Additive M))))
    (LatticeSetLike.closure_le.2 (LatticeSetLike.subset_closure (α := M) (L := Subsemigroup M)))
...
theorem AddSubsemigroup.toSubsemigroup_closure (S : Set A) :
  ...
  le_antisymm
    (AddSubsemigroup.toSubsemigroup.to_galoisConnection.l_le <|
      LatticeSetLike.closure_le.2 <| LatticeSetLike.subset_closure
        (α := Multiplicative A) (L := Subsemigroup (Multiplicative A)))
    (LatticeSetLike.closure_le.2 <| LatticeSetLike.subset_closure (α := A) (L := AddSubsemigroup A))
...
theorem Subsemigroup.toAddSubsemigroup'_closure (S : Set (Multiplicative A)) :
  ...
  le_antisymm
    (Subsemigroup.toAddSubsemigroup'.to_galoisConnection.l_le <|
      LatticeSetLike.closure_le.2 <|
        LatticeSetLike.subset_closure (α := A) (L := AddSubsemigroup A))
    (LatticeSetLike.closure_le.2 <|
      LatticeSetLike.subset_closure (α := Multiplicative A) (L := Subsemigroup (Multiplicative A)))
...
def topEquiv : (⊤ : Subsemigroup M) ≃* M where
  toFun x := x
  invFun x := ⟨x, LatticeSetLike.mem_top x⟩
  left_inv x := x.eta _
  right_inv _ := rfl
  map_mul' _ _ := rfl
...
theorem closure_closure_coe_preimage {s : Set M} :
  ...
  eq_top_iff.2 fun x _ ↦ Subtype.recOn x fun _ hx' ↦
    closure_induction (fun _ h ↦ LatticeSetLike.subset_closure h) (fun _ _ _ _ ↦ mul_mem) hx'
...
theorem srange_eq_top_iff_surjective {N} [Mul N] {f : M →ₙ* N} :
  ...
  SetLike.ext'_iff.trans <| Iff.trans (by rw [coe_srange, LatticeSetLike.coe_top]) Set.range_eq_univ
...
theorem mclosure_preimage_le (f : M →ₙ* N) (s : Set N) : closure (f ⁻¹' s) ≤ (closure s).comap f :=
  LatticeSetLike.closure_le.2 fun _ hx =>
    SetLike.mem_coe.2 <| mem_comap.2 <| LatticeSetLike.subset_closure hx
...
theorem map_mclosure (f : M →ₙ* N) (s : Set M) : (closure s).map f = closure (f '' s) :=
  Set.image_preimage.l_comm_of_u_comm (gc_map_comap f)
    (LatticeSetLike.gi_closure (Subsemigroup N)).gc (LatticeSetLike.gi_closure (Subsemigroup M)).gc
    fun _ ↦ rfl
...
theorem eq_top_iff' : S = ⊤ ↔ ∀ x : M, x ∈ S :=
  eq_top_iff.trans ⟨fun h m => h <| LatticeSetLike.mem_top m, fun h m _ => h m⟩
