...
theorem not_mem_map_rangeS {p : S[X]} : p ∉ (mapRingHom f).rangeS ↔ ∃ n, p.coeff n ∉ f.rangeS :=
...
theorem not_mem_map_range {R S : Type*} [Ring R] [Ring S] (f : R →+* S) {p : S[X]} :
    p ∉ (mapRingHom f).range ↔ ∃ n, p.coeff n ∉ f.range :=
  not_mem_map_rangeS f
