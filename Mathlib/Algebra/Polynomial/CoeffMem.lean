...
lemma coeff_divModByMonicAux_mem_span_pow_mul_span : ∀ (p q : S[X]) (hq : q.Monic) (i),
  ...
  | p, q, hq, i => by
    ...
    by_cases hr' : r = 0
    · rw [divModByMonicAux]
      simp only [mul_ite, mul_one, mul_zero, hr', degree_zero, le_bot_iff, degree_eq_bot, ne_eq,
        not_true_eq_false, and_false, ↓reduceDIte, coeff_zero, add_zero, Submodule.zero_mem,
        and_true]
      split_ifs
