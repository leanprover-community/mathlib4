...
lemma coeff_divModByMonicAux_mem_span_pow_mul_span : ∀ (p q : S[X]) (hq : q.Monic) (i),
  ...
  | p, q, hq, i => by
    ...
    have H₀ (i) : p.coeff i ∈ spanCoeffs(q) ^ deg(p) * spanCoeffs(p) := by
      refine IsConcreteLE.le_iff.mp ?_ <| subset_span <| mem_range_self i
      calc
        span R coeffs(p)
    ...
    have H : span R coeffs(r) ≤ span R coeffs(p) ⊔ span R coeffs(q) * span R coeffs(p) := by
      ...
      apply sub_mem
      · exact IsConcreteLE.le_iff.mp le_sup_left (subset_span (mem_range_self _))
      · split_ifs
        · refine IsConcreteLE.le_iff.mp le_sup_right (mul_mem_mul ?_ ?_) <;> exact subset_span ⟨_, rfl⟩
        · exact zero_mem _
...
lemma coeff_modByMonic_mem_pow_natDegree_mul (p q : S[X])
  ...
  delta modByMonic
  split_ifs with H
  · refine IsConcreteLE.le_iff.mp ?_ (coeff_divModByMonicAux_mem_span_pow_mul_span (R := R) p q H i).2
...
lemma coeff_divByMonic_mem_pow_natDegree_mul (p q : S[X])
  ...
  delta divByMonic
  split_ifs with H
  · refine IsConcreteLE.le_iff.mp ?_ (coeff_divModByMonicAux_mem_span_pow_mul_span (R := R) p q H i).1
    gcongr <;> exact sup_le (by simpa) (by simpa [Submodule.span_le, Set.range_subset_iff])
  · simp
