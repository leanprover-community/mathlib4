...
theorem natDegree_eq_zero_of_derivative_eq_zero [Module.IsTorsionFree ℕ R] {f : R[X]}
...
theorem eq_C_of_derivative_eq_zero [Module.IsTorsionFree ℕ R] {f : R[X]} (h : derivative f = 0) :
    f = C (f.coeff 0) :=
...
theorem mem_support_derivative [Module.IsTorsionFree ℕ R] (p : R[X]) (n : ℕ) :
...
@[simp]
theorem degree_derivative_eq [Module.IsTorsionFree ℕ R] (p : R[X]) (hp : 0 < natDegree p) :
