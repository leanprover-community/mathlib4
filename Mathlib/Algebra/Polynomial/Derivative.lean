...
lemma natDegree_iterate_derivative (p : R[X]) (k : ℕ) :
...
@[simp]
lemma derivative_natCast {n : ℕ} : derivative (n : R[X]) = 0 := by
  rw [← map_natCast C n]
  exact derivative_C
...
@[simp]
lemma derivative_mul {f g : R[X]} : derivative (f * g) = derivative f * g + f * derivative g := by
...
lemma coeff_iterate_derivative {k} (p : R[X]) (m : ℕ) :
...
lemma pow_sub_one_dvd_derivative_of_pow_dvd {p q : R[X]} {n : ℕ}
...
lemma pow_sub_dvd_iterate_derivative_of_pow_dvd {p q : R[X]} {n : ℕ} (m : ℕ)
...
lemma pow_sub_dvd_iterate_derivative_pow (p : R[X]) (n m : ℕ) :
...
lemma iterate_derivative_X_pow_eq_natCast_mul (n k : ℕ) :
...
lemma derivative_prod [DecidableEq ι] {s : Multiset ι} {f : ι → R[X]} :
...
@[simp]
lemma derivative_intCast {n : ℤ} : derivative (n : R[X]) = 0 := by
  rw [← C_eq_intCast n]
  exact derivative_C
#align polynomial.derivative_int_cast Polynomial.derivative_intCast

lemma derivative_intCast_mul {n : ℤ} {f : R[X]} : derivative ((n : R[X]) * f) =
    n * derivative f := by
  simp
#align polynomial.derivative_int_cast_mul Polynomial.derivative_intCast_mul

@[simp]
lemma iterate_derivative_intCast_mul {n : ℤ} {k : ℕ} {f : R[X]} :
    derivative^[k] ((n : R[X]) * f) = n * derivative^[k] f := by
  induction' k with k ih generalizing f <;> simp [*]
...
lemma eval_multiset_prod_X_sub_C_derivative [DecidableEq R]
...
lemma iterate_derivative_X_sub_pow_self (n : ℕ) (c : R) :
