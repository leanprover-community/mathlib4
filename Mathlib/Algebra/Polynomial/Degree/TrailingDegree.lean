...
lemma trailingDegree_lt_wf : WellFounded fun p q : R[X] => trailingDegree p < trailingDegree q :=
...
lemma TrailingMonic.def : TrailingMonic p ↔ trailingCoeff p = 1 :=
  Iff.rfl
...
lemma trailingDegree_eq_natTrailingDegree (hp : p ≠ 0) :
...
lemma trailingDegree_eq_iff_natTrailingDegree_eq {p : R[X]} {n : ℕ} (hp : p ≠ 0) :
...
lemma trailingDegree_eq_iff_natTrailingDegree_eq_of_pos {p : R[X]} {n : ℕ} (hn : 0 < n) :
    p.trailingDegree = n ↔ p.natTrailingDegree = n := by
  constructor
  · intro H
...
lemma natTrailingDegree_eq_of_trailingDegree_eq_some {p : R[X]} {n : ℕ}
...
@[simp]
lemma natTrailingDegree_le_trailingDegree : ↑(natTrailingDegree p) ≤ trailingDegree p := by
  by_cases hp : p = 0;
...
lemma natTrailingDegree_eq_of_trailingDegree_eq [Semiring S] {q : S[X]}
...
lemma trailingDegree_le_of_ne_zero (h : coeff p n ≠ 0) : trailingDegree p ≤ n :=
...
lemma natTrailingDegree_le_of_ne_zero (h : coeff p n ≠ 0) : natTrailingDegree p ≤ n := by
...
@[simp] lemma trailingDegree_le_trailingDegree (h : coeff q (natTrailingDegree p) ≠ 0) :
...
lemma trailingDegree_ne_of_natTrailingDegree_ne {n : ℕ} :
...
lemma natTrailingDegree_le_of_trailingDegree_le {n : ℕ} {hp : p ≠ 0}
...
lemma natTrailingDegree_le_natTrailingDegree {hq : q ≠ 0}
...
@[simp]
lemma trailingDegree_monomial (ha : a ≠ 0) : trailingDegree (monomial n a) = n := by
...
lemma natTrailingDegree_monomial (ha : a ≠ 0) : natTrailingDegree (monomial n a) = n := by
...
lemma natTrailingDegree_monomial_le : natTrailingDegree (monomial n a) ≤ n :=
...
lemma le_trailingDegree_monomial : ↑n ≤ trailingDegree (monomial n a) :=
  letI := Classical.decEq R
...
@[simp]
lemma trailingDegree_C_mul_X_pow (n : ℕ) (ha : a ≠ 0) : trailingDegree (C a * X ^ n) = n := by
...
lemma le_trailingDegree_C_mul_X_pow (n : ℕ) (a : R) :
...
lemma coeff_eq_zero_of_lt_trailingDegree (h : (n : ℕ∞) < trailingDegree p) : coeff p n = 0 :=
...
lemma coeff_eq_zero_of_lt_natTrailingDegree {p : R[X]} {n : ℕ} (h : n < p.natTrailingDegree) :
    p.coeff n = 0 := by
...
@[simp]
lemma coeff_natTrailingDegree_pred_eq_zero {p : R[X]} {hp : (0 : ℕ∞) < natTrailingDegree p} :
...
lemma le_trailingDegree_X_pow (n : ℕ) : (n : ℕ∞) ≤ trailingDegree (X ^ n : R[X]) := by
...
lemma trailingCoeff_nonzero_iff_nonzero : trailingCoeff p ≠ 0 ↔ p ≠ 0 :=
...
lemma natTrailingDegree_mem_support_of_nonzero : p ≠ 0 → natTrailingDegree p ∈ p.support :=
...
lemma natTrailingDegree_le_of_mem_supp (a : ℕ) : a ∈ p.support → natTrailingDegree p ≤ a :=
...
lemma natTrailingDegree_eq_support_min' (h : p ≠ 0) :
...
lemma le_natTrailingDegree (hp : p ≠ 0) (hn : ∀ m < n, p.coeff m = 0) :
...
lemma natTrailingDegree_le_natDegree (p : R[X]) : p.natTrailingDegree ≤ p.natDegree := by
  by_cases hp : p = 0
...
lemma natTrailingDegree_mul_X_pow {p : R[X]} (hp : p ≠ 0) (n : ℕ) :
...
lemma le_trailingDegree_mul : p.trailingDegree + q.trailingDegree ≤ (p * q).trailingDegree := by
...
lemma le_natTrailingDegree_mul (h : p * q ≠ 0) :
...
lemma coeff_mul_natTrailingDegree_add_natTrailingDegree : (p * q).coeff
...
lemma trailingDegree_mul' (h : p.trailingCoeff * q.trailingCoeff ≠ 0) :
...
lemma natTrailingDegree_mul' (h : p.trailingCoeff * q.trailingCoeff ≠ 0) :
...
lemma natTrailingDegree_mul [NoZeroDivisors R] (hp : p ≠ 0) (hq : q ≠ 0) :
...
@[simp]
lemma trailingDegree_neg (p : R[X]) : trailingDegree (-p) = trailingDegree p := by
  unfold trailingDegree
  rw [support_neg]
#align polynomial.trailing_degree_neg Polynomial.trailingDegree_neg

@[simp]
lemma natTrailingDegree_neg (p : R[X]) : natTrailingDegree (-p) = natTrailingDegree p := by
  simp [natTrailingDegree]
#align polynomial.nat_trailing_degree_neg Polynomial.natTrailingDegree_neg

@[simp]
lemma natTrailingDegree_intCast (n : ℤ) : natTrailingDegree (n : R[X]) = 0 := by
...
@[simp]
lemma nextCoeffUp_C_eq_zero (c : R) : nextCoeffUp (C c) = 0 := by
  rw [nextCoeffUp]
  simp
set_option linter.uppercaseLean3 false in
#align polynomial.next_coeff_up_C_eq_zero Polynomial.nextCoeffUp_C_eq_zero

lemma nextCoeffUp_of_constantCoeff_eq_zero (p : R[X]) (hp : coeff p 0 = 0) :
...
lemma ne_zero_of_trailingDegree_lt {n : ℕ∞} (h : trailingDegree p < n) : p ≠ 0 := fun h₀ =>
  h.not_le (by simp [h₀])
