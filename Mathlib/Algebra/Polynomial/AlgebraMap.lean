...
@[simp]
lemma algebraMap_eq : algebraMap R R[X] = C :=
  rfl
...
@[simp]
lemma eval₂_intCastRingHom_X {R : Type*} [Ring R] (p : ℤ[X]) (f : ℤ[X] →+* R) :
...
@[ext 1200]
lemma algHom_ext {f g : R[X] →ₐ[R] B} (hX : f X = g X) :
    f = g :=
...
-- Porting note: removed `@[simp]` because `simp` can prove this
lemma aeval_natCast (n : ℕ) : aeval x (n : R[X]) = n :=
  map_natCast _ _
...
lemma comp_eq_aeval : p.comp q = aeval q p := rfl
...
lemma aeval_algHom_apply {F : Type*} [FunLike F A B] [AlgHomClass F R A B]
    (f : F) (x : A) (p : R[X]) :
...
@[simp]
lemma aeval_mem_adjoin_singleton :
