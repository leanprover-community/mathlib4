...
@[simp]
theorem splits_zero : Splits (Polynomial.map i (0 : K[X])) := by
  simp [Splits]

theorem splits_of_map_eq_C {f : K[X]} {a : L} (h : f.map i = C a) : Splits (Polynomial.map i f) := by
  simp [Splits, h]

@[simp]
theorem splits_C (a : K) : Splits (Polynomial.map i (C a)) := by
  simp [Splits]

theorem splits_of_map_degree_eq_one {f : K[X]} (hf : degree (f.map i) = 1) : Splits (Polynomial.map i f) :=
  Factors.of_degree_eq_one hf

theorem splits_of_degree_le_one {f : K[X]} (hf : degree f ≤ 1) : Splits (Polynomial.map i f) :=
  Factors.of_degree_le_one (degree_map_le.trans hf)

theorem splits_of_degree_eq_one {f : K[X]} (hf : degree f = 1) : Splits (Polynomial.map i f) :=
  splits_of_degree_le_one i hf.le

theorem splits_of_natDegree_le_one {f : K[X]} (hf : natDegree f ≤ 1) : Splits (Polynomial.map i f) :=
  splits_of_degree_le_one i (degree_le_of_natDegree_le hf)

theorem splits_of_natDegree_eq_one {f : K[X]} (hf : natDegree f = 1) : Splits (Polynomial.map i f) :=
  splits_of_natDegree_le_one i (le_of_eq hf)

theorem splits_mul {f g : K[X]} (hf : Splits (Polynomial.map i f)) (hg : Splits (Polynomial.map i g)) : Splits (Polynomial.map i (f * g)) := by
  simp [Splits, hf.mul hg]

theorem splits_of_splits_mul' {f g : K[X]} (hfg : (f * g).map i ≠ 0) (h : Splits (Polynomial.map i (f * g))) :
    Splits (Polynomial.map i f) ∧ Splits (Polynomial.map i g) := by
...
theorem splits_map_iff {L : Type*} [CommRing L] (i : K →+* L) (j : L →+* F) {f : K[X]} :
    Splits (Polynomial.map j (f.map i)) ↔ Splits (Polynomial.map (j.comp i) f) := by
  simp [Splits, Polynomial.map_map]

theorem splits_one : Splits (Polynomial.map i 1) :=
  splits_C i 1

theorem splits_of_isUnit [IsDomain K] {u : K[X]} (hu : IsUnit u) : (Polynomial.map i u).Splits :=
  (isUnit_iff.mp hu).choose_spec.2 ▸ splits_C _ _

theorem splits_X_sub_C {x : K} : (Polynomial.map i (X - C x)).Splits :=
  splits_of_degree_le_one _ <| degree_X_sub_C_le _

theorem splits_X : (Polynomial.map i X).Splits :=
  splits_of_degree_le_one _ degree_X_le

theorem splits_prod {ι : Type u} {s : ι → K[X]} {t : Finset ι} :
    (∀ j ∈ t, (Polynomial.map i (s j)).Splits) → (Polynomial.map i (∏ x ∈ t, s x)).Splits := by
  classical
...
theorem splits_pow {f : K[X]} (hf : (Polynomial.map i f).Splits) (n : ℕ) : (Polynomial.map i (f ^ n)).Splits := by
  rw [← Finset.card_range n, ← Finset.prod_const]
  exact splits_prod i fun j _ => hf

theorem splits_X_pow (n : ℕ) : (Polynomial.map i (X ^ n)).Splits :=
  splits_pow i (splits_X i) n

theorem splits_id_iff_splits {f : K[X]} : (Polynomial.map (RingHom.id L) (f.map i)).Splits ↔ (Polynomial.map i f).Splits := by
...
theorem Splits.comp_of_map_degree_le_one {f : K[X]} {p : K[X]} (hd : (p.map i).degree ≤ 1)
    (h : (Polynomial.map i f).Splits) : (Polynomial.map i (f.comp p)).Splits := by
...
theorem splits_iff_comp_splits_of_degree_eq_one {f : K[X]} {p : K[X]} (hd : (p.map i).degree = 1) :
    (Polynomial.map i f).Splits ↔ (Polynomial.map i (f.comp p)).Splits := by
...
theorem Splits.comp_of_degree_le_one {f : K[X]} {p : K[X]} (hd : p.degree ≤ 1)
    (h : (Polynomial.map i f).Splits) : (Polynomial.map i (f.comp p)).Splits :=
  Splits.comp_of_map_degree_le_one (degree_map_le.trans hd) h

theorem Splits.comp_X_sub_C (a : K) {f : K[X]}
    (h : (Polynomial.map i f).Splits) : (Polynomial.map i (f.comp (X - C a))).Splits :=
...
theorem Splits.comp_neg_X {f : K[X]} (h : (Polynomial.map i f).Splits) : (Polynomial.map i (f.comp (-X))).Splits :=
  Splits.comp_of_degree_le_one (by simpa using degree_X_sub_C_le (0 : K)) h

variable (i)

theorem exists_root_of_splits' {f : K[X]} (hs : Splits (Polynomial.map i f)) (hf0 : degree (f.map i) ≠ 0) :
    ∃ x, eval₂ i x f = 0 := by
  simpa only [eval_map] using hs.exists_eval_eq_zero hf0

theorem roots_ne_zero_of_splits' {f : K[X]} (hs : Splits (Polynomial.map i f)) (hf0 : natDegree (f.map i) ≠ 0) :
    (f.map i).roots ≠ 0 :=
...
def rootOfSplits' {f : K[X]} (hf : (Polynomial.map i f).Splits) (hfd : (f.map i).degree ≠ 0) : L :=
  Classical.choose <| exists_root_of_splits' i hf hfd

theorem map_rootOfSplits' {f : K[X]} (hf : (Polynomial.map i f).Splits) (hfd) :
    f.eval₂ i (rootOfSplits' i hf hfd) = 0 :=
  Classical.choose_spec <| exists_root_of_splits' i hf hfd

theorem natDegree_eq_card_roots' {p : K[X]} {i : K →+* L} (hsplit : Splits (Polynomial.map i p)) :
...
theorem degree_eq_card_roots' {p : K[X]} {i : K →+* L} (p_ne_zero : p.map i ≠ 0)
    (hsplit : Splits (Polynomial.map i p)) : (p.map i).degree = Multiset.card (p.map i).roots := by
...
theorem splits_iff (f : K[X]) :
    Splits (Polynomial.map i f) ↔ f = 0 ∨ ∀ {g : L[X]}, Irreducible g → g ∣ f.map i → degree g = 1 := by
  rw [Splits, factors_iff_splits, Polynomial.map_eq_zero]

/-- This lemma is for polynomials over a field. -/
theorem Splits.def {i : K →+* L} {f : K[X]} (h : Splits (Polynomial.map i f)) :
    f = 0 ∨ ∀ {g : L[X]}, Irreducible g → g ∣ f.map i → degree g = 1 :=
  (splits_iff i f).mp h

theorem splits_of_splits_mul {f g : K[X]} (hfg : f * g ≠ 0) (h : Splits (Polynomial.map i (f * g))) :
    Splits (Polynomial.map i f) ∧ Splits (Polynomial.map i g) :=
  splits_of_splits_mul' i (map_ne_zero hfg) h

theorem splits_of_splits_of_dvd {f g : K[X]} (hf0 : f ≠ 0) (hf : Splits (Polynomial.map i f)) (hgf : g ∣ f) :
    Splits (Polynomial.map i g) := by
  obtain ⟨f, rfl⟩ := hgf
  exact (splits_of_splits_mul i hf0 hf).1

theorem splits_of_splits_gcd_left [DecidableEq K] {f g : K[X]} (hf0 : f ≠ 0) (hf : Splits (Polynomial.map i f)) :
    Splits (Polynomial.map i (EuclideanDomain.gcd f g)) :=
  Polynomial.splits_of_splits_of_dvd i hf0 hf (EuclideanDomain.gcd_dvd_left f g)

theorem splits_of_splits_gcd_right [DecidableEq K] {f g : K[X]} (hg0 : g ≠ 0) (hg : Splits (Polynomial.map i g)) :
    Splits (Polynomial.map i (EuclideanDomain.gcd f g)) :=
...
theorem splits_mul_iff {f g : K[X]} (hf : f ≠ 0) (hg : g ≠ 0) :
    (Polynomial.map i (f * g)).Splits ↔ (Polynomial.map i f).Splits ∧ (Polynomial.map i g).Splits :=
...
theorem splits_prod_iff {ι : Type u} {s : ι → K[X]} {t : Finset ι} :
    (∀ j ∈ t, s j ≠ 0) → ((Polynomial.map i (∏ x ∈ t, s x)).Splits ↔ ∀ j ∈ t, (Polynomial.map i (s j)).Splits) := by
  classical
  refine
...
theorem degree_eq_one_of_irreducible_of_splits {p : K[X]} (hp : Irreducible p)
    (hp_splits : Splits (Polynomial.map (RingHom.id K) p)) : p.degree = 1 := by
...
theorem exists_root_of_splits {f : K[X]} (hs : Splits (Polynomial.map i f)) (hf0 : degree f ≠ 0) :
    ∃ x, eval₂ i x f = 0 :=
  exists_root_of_splits' i hs ((f.degree_map i).symm ▸ hf0)

theorem roots_ne_zero_of_splits {f : K[X]} (hs : Splits (Polynomial.map i f)) (hf0 : natDegree f ≠ 0) :
    (f.map i).roots ≠ 0 :=
...
simpler assumptions. -/
def rootOfSplits {f : K[X]} (hf : (Polynomial.map i f).Splits) (hfd : f.degree ≠ 0) : L :=
  rootOfSplits' i hf ((f.degree_map i).symm ▸ hfd)

/-- `rootOfSplits'` is definitionally equal to `rootOfSplits`. -/
theorem rootOfSplits'_eq_rootOfSplits {f : K[X]} (hf : (Polynomial.map i f).Splits) (hfd) :
...
theorem degree_eq_card_roots {p : K[X]} {i : K →+* L} (p_ne_zero : p ≠ 0) (hsplit : Splits (Polynomial.map i p)) :
    p.degree = Multiset.card (p.map i).roots := by
  rw [degree_eq_natDegree p_ne_zero, natDegree_eq_card_roots hsplit]

theorem roots_map {f : K[X]} (hf : (Polynomial.map (RingHom.id K) f).Splits) : (f.map i).roots = f.roots.map i :=
...
theorem Splits.mem_subfield_of_isRoot (F : Subfield K) {f : F[X]} (hnz : f ≠ 0)
    (hf : Splits (Polynomial.map (RingHom.id F) f)) {x : K} (hx : (f.map F.subtype).IsRoot x) :
    x ∈ F := by
  obtain ⟨x, _, rfl⟩ := Multiset.mem_map.mp
    (roots_map F.subtype hf ▸ mem_roots'.mpr ⟨Polynomial.map_ne_zero hnz, hx⟩)
  exact x.2

theorem image_rootSet [Algebra R K] [Algebra R L] {p : R[X]} (h : (Polynomial.map (algebraMap R K) p).Splits)
...
theorem adjoin_rootSet_eq_range [Algebra R K] [Algebra R L] {p : R[X]}
    (h : (Polynomial.map (algebraMap R K) p).Splits) (f : K →ₐ[R] L) :
...
theorem eq_prod_roots_of_splits {p : K[X]} {i : K →+* L} (hsplit : Splits (Polynomial.map i p)) :
...
theorem eq_prod_roots_of_splits_id {p : K[X]} (hsplit : Splits (Polynomial.map (RingHom.id K) p)) :
    p = C p.leadingCoeff * (p.roots.map fun a => X - C a).prod := by
  simpa using eq_prod_roots_of_splits hsplit

theorem Splits.dvd_of_roots_le_roots {p q : K[X]} (hp : (Polynomial.map (RingHom.id _) p).Splits) (hp0 : p ≠ 0)
...
theorem eval_eq_prod_roots_sub_of_splits_id {p : K[X]}
    (hsplit : Splits (Polynomial.map (RingHom.id K) p)) (v : K) :
...
theorem eq_prod_roots_of_monic_of_splits_id {p : K[X]} (m : Monic p)
    (hsplit : Splits (Polynomial.map (RingHom.id K) p)) : p = (p.roots.map fun a => X - C a).prod := by
...
theorem eq_X_sub_C_of_splits_of_single_root {x : K} {h : K[X]} (h_splits : Splits (Polynomial.map i h))
...
theorem mem_lift_of_splits_of_roots_mem_range [Algebra R K] {f : K[X]}
    (hs : (Polynomial.map (RingHom.id K) f).Splits) (hm : f.Monic) (hr : ∀ a ∈ f.roots, a ∈ (algebraMap R K).range) :
...
theorem splits_of_natDegree_eq_two {f : Polynomial K} {x : L} (h₁ : f.natDegree = 2)
    (h₂ : eval₂ i x f = 0) : Splits (Polynomial.map i f) := by
...
theorem splits_of_degree_eq_two {f : Polynomial K} {x : L} (h₁ : f.degree = 2)
    (h₂ : eval₂ i x f = 0) : Splits (Polynomial.map i f) :=
...
theorem splits_of_exists_multiset {f : K[X]} {s : Multiset L}
    (hs : f.map i = C (i f.leadingCoeff) * (s.map fun a : L => X - C a).prod) : Splits (Polynomial.map i f) :=
  factors_iff_exists_multiset.mpr ⟨s, leadingCoeff_map i ▸ hs⟩

theorem splits_of_splits_id {f : K[X]} (h : Splits (Polynomial.map (RingHom.id K) f)) : Splits (Polynomial.map i f) := by
  simpa using h.map i

end UFD

theorem splits_iff_exists_multiset {f : K[X]} :
    Splits (Polynomial.map i f) ↔
...
theorem splits_of_comp (j : L →+* F) {f : K[X]} (h : Splits (Polynomial.map (j.comp i) f))
    (roots_mem_range : ∀ a ∈ (f.map (j.comp i)).roots, a ∈ j.range) : Splits (Polynomial.map i f) := by
...
theorem splits_id_of_splits {f : K[X]} (h : Splits (Polynomial.map i f))
    (roots_mem_range : ∀ a ∈ (f.map i).roots, a ∈ i.range) : Splits (Polynomial.map (RingHom.id K) f) :=
  splits_of_comp (RingHom.id K) i h roots_mem_range

theorem splits_comp_of_splits (i : R →+* K) (j : K →+* L) {f : R[X]} (h : Splits (Polynomial.map i f)) :
    Splits (Polynomial.map (j.comp i) f) :=
  (splits_map_iff i j).mp (splits_of_splits_id _ <| (splits_map_iff i <| .id K).mpr h)

variable [Algebra R K] [Algebra R L]

theorem splits_of_algHom {f : R[X]} (h : Splits (Polynomial.map (algebraMap R K) f)) (e : K →ₐ[R] L) :
    Splits (Polynomial.map (algebraMap R L) f) := by
...
theorem splits_of_isScalarTower {f : R[X]} [Algebra K L] [IsScalarTower R K L]
    (h : Splits (Polynomial.map (algebraMap R K) f)) : Splits (Polynomial.map (algebraMap R L) f) :=
  splits_of_algHom h (IsScalarTower.toAlgHom R K L)

/-- A polynomial splits if and only if it has as many roots as its degree. -/
theorem splits_iff_card_roots {p : K[X]} :
    Splits (Polynomial.map (RingHom.id K) p) ↔ Multiset.card p.roots = p.natDegree := by
  constructor
  · intro H
...
theorem eval₂_derivative_of_splits [DecidableEq L] {P : K[X]} {f : K →+* L} (hP : (Polynomial.map f P).Splits)
    (x : L) :
...
theorem aeval_derivative_of_splits [Algebra K L] [DecidableEq L] {P : K[X]}
    (hP : (Polynomial.map (algebraMap K L) P).Splits) (r : L) :
...
theorem eval_derivative_of_splits [DecidableEq K] {P : K[X]} (hP : (Polynomial.map (.id K) P).Splits) (r : K) :
...
theorem aeval_root_derivative_of_splits [Algebra K L] [DecidableEq L] {P : K[X]} (hmo : P.Monic)
    (hP : (Polynomial.map (algebraMap K L) P).Splits) {r : L} (hr : r ∈ P.aroots L) :
...
theorem eval_derivative_eq_eval_mul_sum_of_splits {p : K[X]} {x : K}
    (h : (Polynomial.map (.id K) p).Splits) (hx : p.eval x ≠ 0) :
...
theorem coeff_zero_eq_leadingCoeff_mul_prod_roots_of_splits {P : K[X]} (hP : (Polynomial.map (.id K) P).Splits) :
...
theorem coeff_zero_eq_prod_roots_of_monic_of_splits {P : K[X]} (hmo : P.Monic)
    (hP : (Polynomial.map (RingHom.id K) P).Splits) : coeff P 0 = (-1) ^ P.natDegree * P.roots.prod := by
  simp [hmo, coeff_zero_eq_leadingCoeff_mul_prod_roots_of_splits hP]

theorem nextCoeff_eq_neg_sum_roots_mul_leadingCoeff_of_splits {P : K[X]} (hP : (Polynomial.map (.id K) P).Splits) :
...
theorem nextCoeff_eq_neg_sum_roots_of_monic_of_splits {P : K[X]} (hmo : P.Monic)
    (hP : (Polynomial.map (RingHom.id K) P).Splits) : P.nextCoeff = -P.roots.sum := by
