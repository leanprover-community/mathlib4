...
theorem factorial_smul_hasseDeriv : ⇑(k ! • @hasseDeriv R _ k) = (@derivative R _)^[k] := by
  ...
  simp only [_root_.smul_apply, coeff_smul, LinearMap.map_smul_of_tower, coeff_derivative,
...
theorem hasseDeriv_comp (k l : ℕ) :
  ...
  ext i : 2
  simp only [_root_.smul_apply, comp_apply, LinearMap.coe_comp, smul_monomial, hasseDeriv_apply,
