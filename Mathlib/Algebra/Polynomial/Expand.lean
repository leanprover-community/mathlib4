...
lemma expand_eq_comp_X_pow {f : R[X]} : expand R p f = f.comp (X ^ p) := rfl
...
lemma leadingCoeff_expand {p : ℕ} {f : R[X]} (hp : 0 < p) :
    (expand R p f).leadingCoeff = f.leadingCoeff := by
  simp_rw [leadingCoeff, natDegree_expand, coeff_expand_mul hp]

lemma monic_expand_iff {p : ℕ} {f : R[X]} (hp : 0 < p) : (expand R p f).Monic ↔ f.Monic := by
...
lemma map_contract {p : ℕ} (hp : p ≠ 0) {f : R →+* S} {q : R[X]} :
...
lemma contract_one {f : R[X]} : contract 1 f = f :=
  ext fun n ↦ by rw [coeff_contract one_ne_zero, mul_one]

section ExpChar

lemma expand_contract [CharP R p] [NoZeroDivisors R] {f : R[X]} (hf : Polynomial.derivative f = 0)
...
lemma rootMultiplicity_expand_pow :
