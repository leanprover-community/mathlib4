...
lemma eraseLead_mul_eq_mul_eraseLead_of_nextCoeff_zero {R : Type*} [Ring R] [NoZeroDivisors R]
  ...
  · --n < P.natDegree
    ...
    rw [← self_sub_monomial_natDegree_leadingCoeff, coeff_sub, coeff_monomial, if_neg (by cutsat)]
    rw [← self_sub_monomial_natDegree_leadingCoeff, mul_sub, coeff_sub,
      sub_zero, sub_zero, eq_sub_iff_add_eq, add_eq_left]
    rcases hn₂ : n
    · simpa [coeff_monomial, hp] using fun _ ↦ by cutsat
    · rw [coeff_X_sub_C_mul, coeff_monomial, coeff_monomial, if_neg (by cutsat),
        if_neg (by cutsat), mul_zero, sub_zero]
