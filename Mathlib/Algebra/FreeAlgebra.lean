...
This is an `R`-algebra equipped with a function `FreeAlgebra.ι : α → FreeAlgebra R α` which has
the following universal property: if `A` is any `R`-algebra, and `f : α → A` is any function,
then this function is the composite of `FreeAlgebra.ι` and a unique `R`-algebra homomorphism
`FreeAlgebra.lift f : FreeAlgebra R α →ₐ[R] A`.
