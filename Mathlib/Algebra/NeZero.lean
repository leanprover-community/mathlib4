...
lemma NeZero.ne (n : R) [h : NeZero n] : n ≠ 0 :=
  h.out
#align ne_zero.ne NeZero.ne

lemma NeZero.ne' (n : R) [h : NeZero n] : 0 ≠ n :=
  h.out.symm
#align ne_zero.ne' NeZero.ne'

lemma neZero_iff {n : R} : NeZero n ↔ n ≠ 0 :=
...
lemma not_neZero {n : R} : ¬NeZero n ↔ n = 0 := by simp [neZero_iff]
#align not_ne_zero not_neZero

lemma eq_zero_or_neZero (a : R) : a = 0 ∨ NeZero a :=
