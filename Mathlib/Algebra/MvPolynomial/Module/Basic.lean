...
/-- Construct an `MvPolynomial σ R`-linear map out of `AEval R M T` from a
`R`-linear map out of `M`. -/
def _root_.LinearMap.ofAEval {N} [AddCommMonoid N] [Module R N]
...
variable (Φ : σ → M →ₗ[R] M)

local instance [Fact (∀ i j, Φ i ∘ₗ Φ j = Φ j ∘ₗ Φ i)] :
    CommSemiring (Algebra.adjoin R (range Φ)) := by
  refine Algebra.adjoinCommSemiringOfComm R ?_
  simp only [mem_range, forall_exists_index, forall_apply_eq_imp_iff]
  exact Fact.out

variable (h : ∀ i j, Φ i ∘ₗ Φ j = Φ j ∘ₗ Φ i)
...
`Module.MvAEval'` is defined as a special case of `Module.AEval` in which the
`R`-algebra is `Algebra.adjoin R (range Φ)`. Because the commutative semiring
instance on the codomain isn't a global instance, it remains to be seen how
well this will work. -/
def MvAEval' :=
  letI := Fact.mk h
  MvAEval R M (A := Algebra.adjoin R (range Φ))
    (Subtype.coind Φ (Algebra.subset_adjoin <| Set.mem_range_self ·))

instance instAddCommGroup {R M} [CommSemiring R] [AddCommGroup M] [Module R M]
    {Φ : σ → M →ₗ[R] M} h : AddCommGroup <| MvAEval' Φ h :=
  inferInstanceAs (AddCommGroup M)

namespace MvAEval'

instance instAddCommMonoid : AddCommMonoid <| MvAEval' Φ h := inferInstanceAs (AddCommMonoid M)

instance instModuleOrig : Module R <| MvAEval' Φ h := inferInstanceAs (Module R M)

instance instFiniteOrig [Finite R M] : Finite R <| MvAEval' Φ h := inferInstanceAs (Finite R M)

instance instModulePolynomial : Module (MvPolynomial σ R) <| MvAEval' Φ h :=
  letI := Fact.mk h; Module.MvAEval.instModulePolynomial _

instance instIsScalarTowerOrigPolynomial :
    IsScalarTower R (MvPolynomial σ R) <| MvAEval' Φ h :=
  letI := Fact.mk h; MvAEval.instIsScalarTowerOrigPolynomial _

instance instFinitePolynomial [Finite R M] : Finite (MvPolynomial σ R) <| MvAEval' Φ h :=
  Finite.of_restrictScalars_finite R _ _

/--
A more explicit version of `MvAEval.of`.
-/
def of' : M ≃ₗ[R] MvAEval' Φ h := letI := Fact.mk h; MvAEval.of R M _
...
noncomputable instance polynomialModule :
  ...
  inferInstanceAs <| Module (MvPolynomial σ R) <|
    Module.MvAEval' (fun i => Finsupp.lmapDomain M R fun f =>
      ...
      · simp only [comp_apply, Finsupp.coe_update, update_apply, h, h.symm,
          ite_false, g.update_comm h]
...
instance isScalarTower' (M : Type u) [AddCommMonoid M] [Module R M] [Module S M]
    [IsScalarTower S R M] : IsScalarTower S (MvPolynomial σ R) (MvPolynomialModule σ R M) := by
  haveI : IsScalarTower R (MvPolynomial σ R) (MvPolynomialModule σ R M) := by
    unfold polynomialModule
    exact Module.MvAEval'.instIsScalarTowerOrigPolynomial _ _
  constructor
  intro x y z
