...
/-- Compatibility abbreviation for `uniqueAlgEquiv` specialized to `PUnit`.

Simp lemmas for the underlying isomorphism are generated by `@[simps]` on `uniqueAlgEquiv` and
are named `uniqueAlgEquiv_apply`, `uniqueAlgEquiv_symm_apply`, etc.; unfolding `pUnitAlgEquiv`
specializes them to `PUnit`. -/
abbrev pUnitAlgEquiv := uniqueAlgEquiv (R := R) PUnit

theorem uniqueAlgEquiv_monomial {σ : Type*} [Unique σ] {d : σ →₀ ℕ} {r : R} :
...
theorem pUnitAlgEquiv_monomial {d : PUnit →₀ ℕ} {r : R} :
    (MvPolynomial.uniqueAlgEquiv R PUnit) (MvPolynomial.monomial d r)
      = Polynomial.monomial (d default) r := by
  simpa using (uniqueAlgEquiv_monomial (R := R) (σ := PUnit) (d := d) (r := r))
...
theorem eval₂_uniqueAlgEquiv {σ : Type*} [Unique σ] {f : MvPolynomial σ R} {φ : R →+* S}
    {a : σ → S} :
...
theorem eval₂_uniqueAlgEquiv_symm {σ : Type*} [Unique σ] {f : Polynomial R} {φ : R →+* S}
    {a : σ → S} :
...
theorem eval₂_const_uniqueAlgEquiv_symm {σ : Type*} [Unique σ] {f : Polynomial R}
    {φ : R →+* S} {a : S} :
...
theorem eval₂_const_uniqueAlgEquiv {σ : Type*} [Unique σ] {f : MvPolynomial σ R}
    {φ : R →+* S} {a : S} :
...
attribute [deprecated "Use `uniqueAlgEquiv (R := R) PUnit`." (since := "2026-04-08")]
  pUnitAlgEquiv
attribute [deprecated uniqueAlgEquiv_monomial (since := "2026-04-08")] pUnitAlgEquiv_monomial
attribute [deprecated uniqueAlgEquiv_symm_monomial (since := "2026-04-08")]
  pUnitAlgEquiv_symm_monomial
attribute [deprecated eval₂_uniqueAlgEquiv_symm (since := "2026-04-08")]
  eval₂_pUnitAlgEquiv_symm
attribute [deprecated eval₂_const_uniqueAlgEquiv_symm (since := "2026-04-08")]
  eval₂_const_pUnitAlgEquiv_symm
attribute [deprecated eval₂_uniqueAlgEquiv (since := "2026-04-08")] eval₂_pUnitAlgEquiv
attribute [deprecated eval₂_const_uniqueAlgEquiv (since := "2026-04-08")]
