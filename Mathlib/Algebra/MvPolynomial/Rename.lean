...
lemma killCompl_C (r : R) : killCompl hf (C r) = C r := algHom_C _ _
...
lemma eval_rename (g : τ → R) (p : MvPolynomial σ R) : eval g (rename k p) = eval (g ∘ k) p :=
...
@[simp]
lemma coeff_rename_embDomain (f : σ ↪ τ) (φ : MvPolynomial σ R) (d : σ →₀ ℕ) :
