...
theorem rename_C (f : σ → τ) (r : R) : rename (R := R) f (C r) = C r :=
  eval₂_C _ _ _
set_option linter.uppercaseLean3 false in
#align mv_polynomial.rename_C MvPolynomial.rename_C

@[simp]
theorem rename_X (f : σ → τ) (i : σ) : rename (R := R) f (X i : MvPolynomial σ R) = X (f i) :=
  eval₂_X _ _ _
set_option linter.uppercaseLean3 false in
#align mv_polynomial.rename_X MvPolynomial.rename_X

theorem map_rename (f : R →+* S) (g : σ → τ) (p : MvPolynomial σ R) :
    map f (rename (R := R) g p) = rename (R := S) g (map f p) := by
...
theorem rename_rename (f : σ → τ) (g : τ → α) (p : MvPolynomial σ R) :
    rename (R := R) g (rename (R := R) f p) = rename (R := R) (g ∘ f) p :=
  show rename (R := R) g (eval₂ C (X ∘ f) p) = _ by
...
@[simp]
theorem rename_id (p : MvPolynomial σ R) : rename (R := R) id p = p :=
  eval₂_eta p
#align mv_polynomial.rename_id MvPolynomial.rename_id

theorem rename_monomial (f : σ → τ) (d : σ →₀ ℕ) (r : R) :
    rename (R := R) f (monomial d r) = monomial (d.mapDomain f) r := by
...
theorem rename_eq (f : σ → τ) (p : MvPolynomial σ R) :
    rename (R := R) f p = Finsupp.mapDomain (Finsupp.mapDomain f) p := by
...
theorem rename_injective (f : σ → τ) (hf : Function.Injective f) :
    Function.Injective (rename (R := R) f : MvPolynomial σ R → MvPolynomial τ R) := by
  have :
    (rename (R := R) f : MvPolynomial σ R → MvPolynomial τ R) = Finsupp.mapDomain (Finsupp.mapDomain f) :=
    funext (rename_eq (R := R) f)
  rw [this]
...
theorem killCompl_C (r : R) : killCompl (R := R) hf (C r) = C r := algHom_C _ _
...
@[simp]
theorem killCompl_rename_app (p : MvPolynomial σ R) : killCompl (R := R) hf (rename (R := R) f p) = p :=
...
theorem renameEquiv_refl : renameEquiv R (Equiv.refl σ) = AlgEquiv.refl :=
  AlgEquiv.ext (rename_id (σ := σ) (R := R))
...
theorem eval₂_rename : (rename (R := R) k p).eval₂ f g = p.eval₂ f (g ∘ k) := by
  apply MvPolynomial.induction_on p <;>
    · intros
      simp [*]
#align mv_polynomial.eval₂_rename MvPolynomial.eval₂_rename

theorem eval_rename (g : τ → R) (p : MvPolynomial σ R) : eval g (rename (R := R) k p) = eval (g ∘ k) p :=
  eval₂_rename _ _ _ _

theorem eval₂Hom_rename : eval₂Hom f g (rename (R := R) k p) = eval₂Hom f (g ∘ k) p :=
  eval₂_rename _ _ _ _
#align mv_polynomial.eval₂_hom_rename MvPolynomial.eval₂Hom_rename

theorem aeval_rename [SMul R S] [Algebra R S] : aeval (R := R) g (rename (R := R) k p) = aeval (R := R) (g ∘ k) p :=
...
theorem rename_eval₂ (g : τ → MvPolynomial σ R) :
    rename (R := R) k (p.eval₂ C (g ∘ k)) = (rename (R := R) k p).eval₂ C (rename (R := R) k ∘ g) := by
  apply MvPolynomial.induction_on p <;>
    · intros
      simp [*]
...
theorem rename_prod_mk_eval₂ (j : τ) (g : σ → MvPolynomial σ R) :
    rename (R := R) (Prod.mk j) (p.eval₂ C g) = p.eval₂ C fun x => rename (R := R) (Prod.mk j) (g x) := by
  apply MvPolynomial.induction_on p <;>
    · intros
      simp [*]
...
theorem eval₂_rename_prod_mk (g : σ × τ → S) (i : σ) (p : MvPolynomial τ R) :
    (rename (R := R) (Prod.mk i) p).eval₂ f g = eval₂ f (fun j => g (i, j)) p := by
  apply MvPolynomial.induction_on p <;>
    · intros
      simp [*]
...
theorem eval_rename_prod_mk (g : σ × τ → R) (i : σ) (p : MvPolynomial τ R) :
    eval g (rename (R := R) (Prod.mk i) p) = eval (fun j => g (i, j)) p :=
...
theorem exists_finset_rename (p : MvPolynomial σ R) :
    ∃ (s : Finset σ) (q : MvPolynomial { x // x ∈ s } R), p = rename (R := R) (↑) q := by
  classical
  ...
  · rintro p q ⟨s, p, rfl⟩ ⟨t, q, rfl⟩
    refine' ⟨s ∪ t, ⟨_, _⟩⟩
    · refine' rename (R := R) (Subtype.map id _) p + rename (R := R) (Subtype.map id _) q <;>
  ...
  · rintro p n ⟨s, p, rfl⟩
    ...
    · refine' rename (R := R) (Subtype.map id _) p * X ⟨n, s.mem_insert_self n⟩
...
theorem exists_finset_rename₂ (p₁ p₂ : MvPolynomial σ R) :
    ∃ (s : Finset σ) (q₁ q₂ : MvPolynomial s R), p₁ = rename (R := R) (↑) q₁ ∧ p₂ = rename (R := R) (↑) q₂ := by
  obtain ⟨s₁, q₁, rfl⟩ := exists_finset_rename p₁
  obtain ⟨s₂, q₂, rfl⟩ := exists_finset_rename p₂
  classical
    use s₁ ∪ s₂
    use rename (R := R) (Set.inclusion <| s₁.subset_union_left s₂) q₁
    use rename (R := R) (Set.inclusion <| s₁.subset_union_right s₂) q₂
...
theorem exists_fin_rename (p : MvPolynomial σ R) :
    ∃ (n : ℕ) (f : Fin n → σ) (_hf : Injective f) (q : MvPolynomial (Fin n) R), p = rename (R := R) f q := by
  obtain ⟨s, q, rfl⟩ := exists_finset_rename p
  let n := Fintype.card { x // x ∈ s }
  let e := Fintype.equivFin { x // x ∈ s }
  refine' ⟨n, (↑) ∘ e.symm, Subtype.val_injective.comp e.symm.injective, rename (R := R) e q, _⟩
...
theorem eval₂_cast_comp (f : σ → τ) (c : ℤ →+* R) (g : τ → R) (p : MvPolynomial σ ℤ) :
    eval₂ c (g ∘ f) p = eval₂ c g (rename (R := ℤ) f p) := by
...
theorem coeff_rename_mapDomain (f : σ → τ) (hf : Injective f) (φ : MvPolynomial σ R) (d : σ →₀ ℕ) :
    (rename (R := R) f φ).coeff (d.mapDomain f) = φ.coeff d := by
  classical
  apply φ.induction_on' (P := fun ψ => coeff (Finsupp.mapDomain f d) ((rename (R := R) f) ψ) = coeff d ψ)
...
theorem coeff_rename_embDomain (f : σ ↪ τ) (φ : MvPolynomial σ R) (d : σ →₀ ℕ) :
    (rename (R := R) f φ).coeff (d.embDomain f) = φ.coeff d := by
...
theorem coeff_rename_eq_zero (f : σ → τ) (φ : MvPolynomial σ R) (d : τ →₀ ℕ)
    (h : ∀ u : σ →₀ ℕ, u.mapDomain f = d → φ.coeff u = 0) : (rename (R := R) f φ).coeff d = 0 := by
  classical
...
theorem coeff_rename_ne_zero (f : σ → τ) (φ : MvPolynomial σ R) (d : τ →₀ ℕ)
    (h : (rename (R := R) f φ).coeff d ≠ 0) : ∃ u : σ →₀ ℕ, u.mapDomain f = d ∧ φ.coeff u ≠ 0 := by
  contrapose! h
...
theorem constantCoeff_rename {τ : Type*} (f : σ → τ) (φ : MvPolynomial σ R) :
    constantCoeff (rename (R := R) f φ) = constantCoeff φ := by
...
theorem support_rename_of_injective {p : MvPolynomial σ R} {f : σ → τ} [DecidableEq τ]
    ...
    (rename (R := R) f p).support = Finset.image (Finsupp.mapDomain f) p.support := by
  rw [rename_eq]
