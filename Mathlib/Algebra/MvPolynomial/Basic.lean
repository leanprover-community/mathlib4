...
@[simp]
lemma algHom_C {τ : Type*} (f : MvPolynomial σ R →ₐ[R] MvPolynomial τ R) (r : R) :
    f (C r) = C r :=
  f.commutes r
...
@[simp]
lemma X_ne_zero [Nontrivial R] (s : σ) :
    X (R := R) s ≠ 0 := by
  rw [ne_zero_iff]
...
@[simp]
lemma eval₂_id {g : σ → R} (p : MvPolynomial σ R) : eval₂ (RingHom.id _) g p = eval g p :=
  rfl
#align mv_polynomial.eval₂_id MvPolynomial.eval₂_id

lemma eval_eval₂ {S τ : Type*} {x : τ → S} [CommSemiring R] [CommSemiring S]
