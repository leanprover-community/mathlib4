...
@[simp] theorem mainVariable_zero : (0 : MvPolynomial σ R).vars.max = ⊥ := rfl

theorem ne_zero_of_mainVariable_ne_bot : p.vars.max ≠ ⊥ → p ≠ 0 :=
  mt fun h ↦ h ▸ rfl
...
@[simp] theorem mainVariable_C (r : R) : (C r : MvPolynomial σ R).vars.max = ⊥ := by
...
@[simp] theorem mainVariable_X [Nontrivial R] (i : σ) : (X i : MvPolynomial σ R).vars.max = i :=
  (pow_one (X i : MvPolynomial σ R)).symm ▸ mainVariable_X_pow i Nat.one_ne_zero

theorem mainVariable_eq_bot_iff_degrees_eq_zero : p.vars.max = ⊥ ↔ p.degrees = 0 := by
...
theorem mainVariable_eq_bot_iff_eq_C : p.vars.max = ⊥ ↔ ∃ r : R, p = C r := by
...
theorem degreeOf_of_mainVariable_eq_bot (i : σ) : p.vars.max = ⊥ → p.degreeOf i = 0 :=
