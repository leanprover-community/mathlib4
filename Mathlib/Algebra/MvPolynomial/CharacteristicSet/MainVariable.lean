...
* `MvPolynomial.max_vars p` : the largest variable index appearing in `p`.
...
@[simp] theorem max_vars_zero : (0 : MvPolynomial σ R).vars.max = ⊥ := rfl

theorem ne_zero_of_max_vars_ne_bot : p.vars.max ≠ ⊥ → p ≠ 0 :=
  mt fun h ↦ h ▸ rfl

@[simp] theorem max_vars_monomial {r : R} (s : σ →₀ ℕ) (h : r ≠ 0) :
    (monomial s r : MvPolynomial σ R).vars.max = s.support.max := by
  rw [vars_monomial h]

@[simp] theorem max_vars_C (r : R) : (C r : MvPolynomial σ R).vars.max = ⊥ := by
  rw [vars_C, Finset.max_empty]

@[simp] theorem max_vars_X_pow [Nontrivial R] (i : σ) {k : ℕ} (hk : k ≠ 0) :
    ((X i : MvPolynomial σ R) ^ k).vars.max = i := by
  rewrite [X_pow_eq_monomial, max_vars_monomial _ one_ne_zero, Finsupp.single]
  simp only [hk, reduceIte, Finset.max_singleton]

@[simp] theorem max_vars_X [Nontrivial R] (i : σ) : (X i : MvPolynomial σ R).vars.max = i :=
  (pow_one (X i : MvPolynomial σ R)).symm ▸ max_vars_X_pow i Nat.one_ne_zero

theorem max_vars_eq_bot_iff_degrees_eq_zero : p.vars.max = ⊥ ↔ p.degrees = 0 := by
  rw [Finset.max_eq_bot, vars_def, Multiset.toFinset_eq_empty]
...
theorem max_vars_eq_bot_iff_eq_C : p.vars.max = ⊥ ↔ ∃ r : R, p = C r := by
  refine ⟨fun h ↦ ?_, fun h ↦ h.choose_spec ▸ max_vars_C h.choose⟩
  rw [max_vars_eq_bot_iff_degrees_eq_zero] at h
...
theorem max_vars_add_le (p q : MvPolynomial σ R) :
    (p + q).vars.max ≤ p.vars.max ⊔ q.vars.max := by
  rewrite [← Finset.max_union]
  exact Finset.max_mono <| vars_add_subset p q

theorem max_vars_mul_le (p q : MvPolynomial σ R) :
    (p * q).vars.max ≤ p.vars.max ⊔ q.vars.max := by
  rewrite [← Finset.max_union]
  exact Finset.max_mono <| vars_mul p q

theorem max_vars_sum_le {α : Type*} (s : Finset α) (f : α → MvPolynomial σ R) :
    (∑ a ∈ s, f a).vars.max ≤ s.sup (fun a ↦ (f a).vars.max) := by
  classical
  refine Finset.induction_on s (by simp) ?_
  intro a s has ih
  rewrite [Finset.sum_insert has, Finset.sup_insert]
  exact (max_vars_add_le _ _).trans (sup_le_sup_left ih _)

theorem max_vars_prod_le {α : Type*} (s : Finset α) (f : α → MvPolynomial σ R) :
    (∏ a ∈ s, f a).vars.max ≤ s.sup (fun a ↦ (f a).vars.max) := by
  classical
  induction s using Finset.induction_on with
  | empty => simp only [Finset.prod_empty, Finset.sup_empty, le_bot_iff]; apply max_vars_C
  | insert a s has ih =>
    rewrite [Finset.prod_insert has, Finset.sup_insert]
    exact (max_vars_mul_le _ _).trans (sup_le_sup_left ih _)
...
theorem degreeOf_eq_zero_of_max_vars_lt {i : σ} :
    p.vars.max < i → p.degreeOf i = 0 := fun h ↦ by
...
theorem degreeOf_of_max_vars_eq_bot (i : σ) : p.vars.max = ⊥ → p.degreeOf i = 0 :=
  fun h ↦ degreeOf_eq_zero_of_max_vars_lt (h ▸ WithBot.bot_lt_coe i)
