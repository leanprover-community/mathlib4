...
theorem pointReflection_fixed_iff_of_injective_bit0 {x y : P} (h : Injective (fun g ↦ g + g : G → G)) :
    pointReflection x y = y ↔ y = x := by
  rw [pointReflection_apply, eq_comm, eq_vadd_iff_vsub_eq, ← neg_vsub_eq_vsub_rev,
    neg_eq_iff_add_eq_zero, ← add_zero 0, h.eq_iff, vsub_eq_zero_iff_eq, eq_comm]
...
theorem injective_pointReflection_left_of_injective_bit0 {G P : Type*} [AddCommGroup G]
    [AddTorsor G P] (h : Injective (fun g ↦ g + g : G → G)) (y : P) :
  ...
  rwa [pointReflection_apply, pointReflection_apply, vadd_eq_vadd_iff_sub_eq_vsub,
    vsub_sub_vsub_cancel_right, ← neg_vsub_eq_vsub_rev, neg_eq_iff_add_eq_zero, ← add_zero 0,
