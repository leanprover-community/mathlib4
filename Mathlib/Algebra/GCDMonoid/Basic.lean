...
-- Porting note: lower priority to avoid linter complaints about simp-normal form
@[simp 1100]
lemma normalize_gcd [NormalizedGCDMonoid α] :
...
@[simp]
lemma gcd_one_left' [GCDMonoid α] (a : α) : Associated (gcd 1 a) 1 :=
...
@[simp]
lemma gcd_one_right' [GCDMonoid α] (a : α) : Associated (gcd a 1) 1 :=
...
lemma exists_dvd_and_dvd_of_dvd_mul [GCDMonoid α] {m n k : α} (H : k ∣ m * n) :
...
lemma dvd_mul [GCDMonoid α] {k m n : α} : k ∣ m * n ↔ ∃ d₁ d₂, d₁ ∣ m ∧ d₂ ∣ n ∧ k = d₁ * d₂ := by
...
-- Porting note: lower priority to avoid linter complaints about simp-normal form
@[simp 1100]
lemma normalize_lcm [NormalizedGCDMonoid α] (a b : α) :
...
lemma prime_of_irreducible [GCDMonoid α] {x : α} (hi : Irreducible x) : Prime x :=
  ⟨hi.ne_zero,
    ⟨hi.1, fun a b h => by
...
lemma irreducible_iff_prime [GCDMonoid α] {p : α} : Irreducible p ↔ Prime p :=
