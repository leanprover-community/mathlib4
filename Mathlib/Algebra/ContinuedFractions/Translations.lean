...
lemma terminatedAt_iff_s_terminatedAt : g.TerminatedAt n ↔ g.s.TerminatedAt n := by rfl
...
lemma terminatedAt_iff_s_none : g.TerminatedAt n ↔ g.s.get? n = none := by rfl
#align generalized_continued_fraction.terminated_at_iff_s_none GeneralizedContinuedFraction.terminatedAt_iff_s_none

lemma part_num_none_iff_s_none : g.partialNumerators.get? n = none ↔ g.s.get? n = none := by
...
lemma terminatedAt_iff_part_num_none : g.TerminatedAt n ↔ g.partialNumerators.get? n = none := by
...
lemma part_denom_none_iff_s_none : g.partialDenominators.get? n = none ↔ g.s.get? n = none := by
...
lemma terminatedAt_iff_part_denom_none : g.TerminatedAt n ↔ g.partialDenominators.get? n = none :=
...
lemma part_num_eq_s_a {gp : Pair α} (s_nth_eq : g.s.get? n = some gp) :
...
lemma part_denom_eq_s_b {gp : Pair α} (s_nth_eq : g.s.get? n = some gp) :
...
lemma exists_s_a_of_part_num {a : α} (nth_part_num_eq : g.partialNumerators.get? n = some a) :
...
lemma exists_s_b_of_part_denom {b : α}
...
lemma nth_cont_eq_succ_nth_cont_aux : g.continuants n = g.continuantsAux (n + 1) :=
  rfl
...
@[simp]
lemma zeroth_continuant_aux_eq_one_zero : g.continuantsAux 0 = ⟨1, 0⟩ :=
  rfl
...
@[simp]
lemma first_continuant_aux_eq_h_one : g.continuantsAux 1 = ⟨g.h, 1⟩ :=
  rfl
...
lemma second_continuant_aux_eq {gp : Pair K} (zeroth_s_eq : g.s.get? 0 = some gp) :
...
lemma first_continuant_eq {gp : Pair K} (zeroth_s_eq : g.s.get? 0 = some gp) :
...
lemma first_numerator_eq {gp : Pair K} (zeroth_s_eq : g.s.get? 0 = some gp) :
...
lemma first_denominator_eq {gp : Pair K} (zeroth_s_eq : g.s.get? 0 = some gp) :
...
@[simp]
lemma zeroth_convergent'_aux_eq_zero {s : Stream'.Seq <| Pair K} :
...
lemma convergents'Aux_succ_none {s : Stream'.Seq (Pair K)} (h : s.head = none) (n : ℕ) :
...
lemma convergents'Aux_succ_some {s : Stream'.Seq (Pair K)} {p : Pair K} (h : s.head = some p)
