...
theorem partNumStream'_eq_s_a [Zero α] {gp : Pair α} (s_nth_eq : g.s.get? n = some gp) :
    g.partNumsStream' n = gp.a :=
  show (g.partNums.get? n).getD 0 = gp.a from partNum_eq_s_a s_nth_eq ▸ Option.getD_some

theorem partDenStream'_eq_s_b [One α] {gp : Pair α} (s_nth_eq : g.s.get? n = some gp) :
    g.partDensStream' n = gp.b :=
  show (g.partDens.get? n).getD 1 = gp.b from partDen_eq_s_b s_nth_eq ▸ Option.getD_some

theorem partNumStream'_of_terminatedAt [Zero α] (hg : g.TerminatedAt n) :
    g.partNumsStream' n = 0 :=
  show (g.partNums.get? n).getD 0 = 0 from terminatedAt_iff_partNum_none.mp hg ▸ Option.getD_none

theorem partDenStream'_of_terminatedAt [One α] (hg : g.TerminatedAt n) :
    g.partDensStream' n = 1 :=
