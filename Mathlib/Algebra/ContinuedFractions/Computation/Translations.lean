...
of three sections:
1. Recurrences and inversion lemmas for `CF.partQuots`: these lemmas give us inversion
...
3. Translation lemmas for the sequence: these lemmas show how the sequences of the involved
   structures (`CF.partQuots` and `CF.of`) are connected, i.e. how the values are moved along the
...
/-!
### Recurrences and Inversion Lemmas for `CF.partQuots`
...
theorem tail_toStream'_partQuots :
    Stream'.tail (toStream' (partQuots v)) =
      Stream'.map
        (fun o => Option.bind o (fun w => if fract w = 0 then none else some (fract w)⁻¹))
        (toStream' (partQuots v)) := by
  unfold partQuots
  refine Stream'.eq_of_bisim
    (fun s₁ s₂ => (∃ (v : K),
      s₁ = Stream'.tail (toStream' (corec partQuots.next? (fract v))) ∧
        s₂ = Stream'.map
          (fun o ↦ Option.bind o fun w ↦ if fract w = 0 then none else some (fract w)⁻¹)
          (toStream' (corec partQuots.next? (fract v)))) ∨ s₁ = s₂)
    ?_ (Or.inl ⟨v, rfl, rfl⟩)
  rintro s _ (⟨v, rfl, rfl⟩ | rfl)
  · by_cases hv₁ : fract v = 0
    · simp [hv₁]
    · by_cases hv₂ : fract (fract v)⁻¹ = 0
      · simp [hv₁, hv₂, Option.elim_some, Option.elim_none, - Option.elim]
        right
...
theorem partQuots_eq_none_of_fract_eq_zero {w : K}
    (partQuots_eq : get? (partQuots v) n = w) (fract_eq_zero : fract w = 0) :
    get? (partQuots v) (n + 1) = none := by
  rw [← get_toStream', Stream'.get_succ, tail_toStream'_partQuots]
  simp [partQuots_eq, fract_eq_zero]
#align generalized_continued_fraction.int_fract_pair.stream_eq_none_of_fr_eq_zero CF.partQuots_eq_none_of_fract_eq_zero
...
-/
theorem get?_partQuots_succ_eq_none_iff :
    get? (partQuots v) (n + 1) = none ↔
      get? (partQuots v) n = none ∨ ∃ w, get? (partQuots v) n = some w ∧ fract w = 0 := by
  rw [← get_toStream', Stream'.get_succ, tail_toStream'_partQuots, Stream'.get_map,
    get_toStream']
  cases get? (partQuots v) n <;> simp [imp_false]
#align generalized_continued_fraction.int_fract_pair.succ_nth_stream_eq_none_iff CF.get?_partQuots_succ_eq_none_iff

/-- Gives a recurrence to compute the `n + 1`th value of the sequence of integer and fractional
parts of a value in case of non-termination.
-/
theorem get?_partQuots_succ_eq_some_iff {w : K} :
    get? (partQuots v) (n + 1) = some w ↔
      ∃ w' : K, get? (partQuots v) n = some w' ∧ fract w' ≠ 0 ∧ (fract w')⁻¹ = w := by
  rw [← get_toStream', Stream'.get_succ, tail_toStream'_partQuots, Stream'.get_map,
    get_toStream']
  cases get? (partQuots v) n <;> simp [imp_false, ite_eq_iff]
#align generalized_continued_fraction.int_fract_pair.succ_nth_stream_eq_some_iff CF.get?_partQuots_succ_eq_some_iff

/-- An easier to use version of one direction of
`CF.get?_partQuots_succ_eq_some_iff`.
-/
theorem partQuots_succ_of_some {w : K} (h : get? (partQuots v) n = some w)
    (h' : fract w ≠ 0) : get? (partQuots v) (n + 1) = some (fract w)⁻¹ :=
  get?_partQuots_succ_eq_some_iff.mpr ⟨w, h, h', rfl⟩
#align generalized_continued_fraction.int_fract_pair.stream_succ_of_some CF.partQuots_succ_of_some
...
#align generalized_continued_fraction.int_fract_pair.stream_succ_of_int CF.IntFractPair.stream_succ_of_int
...
#align generalized_continued_fraction.int_fract_pair.exists_succ_nth_stream_of_fr_zero CF.IntFractPair.exists_succ_nth_stream_of_fr_zero
...
#align generalized_continued_fraction.int_fract_pair.stream_succ CF.IntFractPair.stream_succ
...
theorem IntFractPair.seq1_fst_eq_of : (IntFractPair.seq1 v).fst = IntFractPair.of v :=
  rfl
#align generalized_continued_fraction.int_fract_pair.seq1_fst_eq_of CF.IntFractPair.seq1_fst_eq_of

theorem of_h_eq_intFractPair_seq1_fst_b : (of v).h = (IntFractPair.seq1 v).fst.b := by
  cases aux_seq_eq : IntFractPair.seq1 v
  simp [of, aux_seq_eq]
#align generalized_continued_fraction.of_h_eq_int_fract_pair_seq1_fst_b CF.of_h_eq_intFractPair_seq1_fst_b
...
(`IntFractPair.stream`, `IntFractPair.seq1`, and `CF.of`) are
...
theorem IntFractPair.get?_seq1_eq_succ_get?_stream :
  ...
  rfl
#align generalized_continued_fraction.int_fract_pair.nth_seq1_eq_succ_nth_stream CF.IntFractPair.get?_seq1_eq_succ_get?_stream
...
theorem of_terminatedAt_iff_intFractPair_seq1_terminatedAt :
  ...
  Option.map_eq_none
#align generalized_continued_fraction.of_terminated_at_iff_int_fract_pair_seq1_terminated_at CF.of_terminatedAt_iff_intFractPair_seq1_terminatedAt
...
#align generalized_continued_fraction.of_terminated_at_n_iff_succ_nth_int_fract_pair_stream_eq_none CF.of_terminatedAt_n_iff_succ_nth_intFractPair_stream_eq_none
...
#align generalized_continued_fraction.int_fract_pair.exists_succ_nth_stream_of_gcf_of_nth_eq_some CF.IntFractPair.exists_succ_get?_stream_of_gcf_of_get?_eq_some
...
#align generalized_continued_fraction.nth_of_eq_some_of_succ_nth_int_fract_pair_stream CF.get?_of_eq_some_of_succ_get?_intFractPair_stream
...
#align generalized_continued_fraction.nth_of_eq_some_of_nth_int_fract_pair_stream_fr_ne_zero CF.get?_of_eq_some_of_get?_intFractPair_stream_fr_ne_zero
...
#align generalized_continued_fraction.of_s_head_aux CF.of_s_head_aux
...
#align generalized_continued_fraction.convergents'_of_int CF.convergents'_of_int
...
#align generalized_continued_fraction.convergents'_succ CF.convergents'_succ
