...
protected def compExactValue (pconts conts : Pair K) (fr : K) : K :=
  ...
  if fr = 0 then
    ...
    (nextContinuants 1 fr⁻¹ pconts conts).a / (nextContinuants 1 fr⁻¹ pconts conts).b
...
theorem compExactValue_correctness_of_stream_eq_some :
  ...
  · intro ifp_zero stream_zero_eq
    ...
    · suffices v = ⌊v⌋ by
        ...
        field_simp [compExactValue, nextContinuants, nextNumerator, nextDenominator, compExactValue]
    ...
    · field_simp [compExactValue, continuantsAux, nextContinuants, nextNumerator, nextDenominator,
        of_h_eq_floor, compExactValue, fract_ne_zero]
  ...
  · intro ifp_succ_n succ_nth_stream_eq
    ...
    · -- use the IH to show that the following equality suffices
      ...
      have : compExactValue ppconts pconts ifp_n.fr =
        ...
        unfold compExactValue
        dsimp only
        field_simp [ifp_n_fract_ne_zero, nextContinuants, nextNumerator,
          nextDenominator]
        ac_rfl
      rw [this]
