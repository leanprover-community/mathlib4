...
lemma mapRange_add (f : R →+ S) (x y : R[M]) :
    mapRange f (x + y) = mapRange f x + mapRange f y := by
  ext; simp [Finsupp.mapRange_add]
...
@[to_additive (attr := simp)]
lemma coeff_mapRangeRingHom (f : R →+* S) (x : R[M]) (m : M) :
    (mapRangeRingHom M f x).coeff m = f (x.coeff m) := by simp [mapRangeRingHom]
