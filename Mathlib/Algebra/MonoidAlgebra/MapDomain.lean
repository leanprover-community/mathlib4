...
@[to_additive]
noncomputable abbrev mapDomain (f : M → N) (v : MonoidAlgebra R M) : MonoidAlgebra R N :=
...
@[to_additive (attr := simp) mapDomain_one
/-- Like `Finsupp.mapDomain_zero`, but for the `1` we define in this file -/]
...
noncomputable def mapDomainRingHom (k : Type*) {H F : Type*} [Semiring k] [Monoid G] [Monoid H]
...
--       apply AddMonoidAlgebra.mapDomain_mul (α := G) (α₂ := G) (f := AddHom.id G)
