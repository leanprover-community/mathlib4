...
`DirectSum.decompose`. -/
noncomputable def decomposeAux : R[M] →ₐ[R] ⨁ i : ι, gradeBy R f i :=
  lift R _ M {
...
noncomputable instance gradeBy.gradedAlgebra : GradedAlgebra (gradeBy R f) :=
...
noncomputable instance grade.gradedAlgebra : GradedAlgebra (grade R : ι → Submodule _ _) :=
