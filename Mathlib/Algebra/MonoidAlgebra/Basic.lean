...
import Mathlib.Tactic.MinMaxTwo
...
def MonoidAlgebra : Type max u₁ u₂ :=
  G →₀ k
to_ama #align monoid_algebra MonoidAlgebra
...
to_ama #align monoid_algebra.inhabited MonoidAlgebra.inhabited

-- Porting note: The compiler couldn't derive this.
to_ama
instance MonoidAlgebra.addCommMonoid : AddCommMonoid (MonoidAlgebra k G) :=
  inferInstanceAs (AddCommMonoid (G →₀ k))
to_ama #align monoid_algebra.add_comm_monoid MonoidAlgebra.addCommMonoid
...
instance MonoidAlgebra.coeFun : CoeFun (MonoidAlgebra k G) fun _ => G → k :=
  Finsupp.instCoeFun
to_ama #align monoid_algebra.has_coe_to_fun MonoidAlgebra.coeFun
...
to_ama #align monoid_algebra.lift_nc MonoidAlgebra.liftNC
...
theorem _root_.AddMonoidAlgebra.liftNC_mul {g_hom : Type*}
    ...
    AddMonoidAlgebra.liftNC (f : k →+ R) g (a * b) =
      AddMonoidAlgebra.liftNC (f : k →+ R) g a * AddMonoidAlgebra.liftNC (f : k →+ R) g b :=
...
def _root_.AddMonoidAlgebra.liftNCRingHom (f : k →+* R) (g : Multiplicative G →* R)
    (h_comm : ∀ x y, Commute (f x) (g y)) :
    k[G] →+* R :=
  { AddMonoidAlgebra.liftNC (f : k →+ R) g with
    map_one' := AddMonoidAlgebra.liftNC_one _ _
    map_mul' := fun _a _b => AddMonoidAlgebra.liftNC_mul _ _ _ _ fun {_ _} _ => h_comm _ _ }
...
instance nonUnitalCommSemiring [CommSemiring k] [CommSemigroup G] :
  ...
  { MonoidAlgebra.nonUnitalSemiring with
    mul_comm := fun f g => by
      ...
      rw [Finset.sum_comm]
      -- another hack, similar to a previous one: consider automating it
      first | simp only [mul_comm] | simp only [add_comm] }
to_ama #align monoid_algebra.non_unital_comm_semiring MonoidAlgebra.nonUnitalCommSemiring

to_ama
instance nontrivial [Semiring k] [Nontrivial k] [Nonempty G] : Nontrivial (MonoidAlgebra k G) :=
  Finsupp.instNontrivial
to_ama #align monoid_algebra.nontrivial MonoidAlgebra.nontrivial
...
to_ama #align monoid_algebra.comm_semiring MonoidAlgebra.commSemiring

to_ama
instance unique [Semiring k] [Subsingleton k] : Unique (MonoidAlgebra k G) :=
  Finsupp.uniqueOfRight
to_ama #align monoid_algebra.unique MonoidAlgebra.unique

to_ama
instance addCommGroup [Ring k] : AddCommGroup (MonoidAlgebra k G) :=
  Finsupp.instAddCommGroup
to_ama #align monoid_algebra.add_comm_group MonoidAlgebra.addCommGroup
...
to_ama #align monoid_algebra.non_unital_non_assoc_ring MonoidAlgebra.nonUnitalNonAssocRing
...
to_ama #align monoid_algebra.non_unital_ring MonoidAlgebra.nonUnitalRing
...
to_ama #align monoid_algebra.non_assoc_ring MonoidAlgebra.nonAssocRing

to_ama
theorem intCast_def [Ring k] [MulOneClass G] (z : ℤ) :
    (z : MonoidAlgebra k G) = single (1 : G) (z : k) :=
  rfl
to_ama #align monoid_algebra.int_cast_def MonoidAlgebra.intCast_def

to_ama
instance ring [Ring k] [Monoid G] : Ring (MonoidAlgebra k G) :=
  { MonoidAlgebra.nonAssocRing, MonoidAlgebra.semiring with }
to_ama #align monoid_algebra.ring MonoidAlgebra.ring
...
to_ama #align monoid_algebra.non_unital_comm_ring MonoidAlgebra.nonUnitalCommRing
...
to_ama #align monoid_algebra.comm_ring MonoidAlgebra.commRing
...
to_ama #align monoid_algebra.smul_zero_class MonoidAlgebra.smulZeroClass
...
to_ama #align monoid_algebra.distrib_mul_action MonoidAlgebra.distribMulAction
...
to_ama G -- fails to place `toFun a := single (Multiplicative.toAdd a.2) a.1`
...
to_ama #align monoid_algebra.single_hom MonoidAlgebra.singleHom
to_ama #align monoid_algebra.single_hom_apply MonoidAlgebra.singleHom_apply
...
to_ama #align monoid_algebra.mul_single_apply_aux MonoidAlgebra.mul_single_apply_aux

to_ama
theorem mul_single_one_apply [MulOneClass G] (f : MonoidAlgebra k G) (r : k) (x : G) :
    (HMul.hMul (β := MonoidAlgebra k G) f (single 1 r)) x = f x * r :=
  -- another hack
  f.mul_single_apply_aux fun a => by first | rw [mul_one] | rw [add_zero]
to_ama #align monoid_algebra.mul_single_one_apply MonoidAlgebra.mul_single_one_apply
...
theorem mul_single_apply_of_not_exists_mul [Mul G] (r : k) {g g' : G} (x : MonoidAlgebra k G)
  ...
  classical
    ...
    · apply Finset.sum_eq_zero
      ...
      exfalso
      exact h ⟨_, rfl⟩
to_ama #align monoid_algebra.mul_single_apply_of_not_exists_mul MonoidAlgebra.mul_single_apply_of_not_exists_mul
...
to_ama #align monoid_algebra.single_mul_apply_aux MonoidAlgebra.single_mul_apply_aux

to_ama
theorem single_one_mul_apply [MulOneClass G] (f : MonoidAlgebra k G) (r : k) (x : G) :
    (single (1 : G) r * f) x = r * f x :=
  f.single_mul_apply_aux fun a => by first | rw [one_mul] | rw [zero_add]
to_ama #align monoid_algebra.single_one_mul_apply MonoidAlgebra.single_one_mul_apply
...
theorem single_mul_apply_of_not_exists_mul [Mul G] (r : k) {g g' : G} (x : MonoidAlgebra k G)
  ...
  classical
    ...
    · apply Finset.sum_eq_zero
      ...
      exfalso
      exact h ⟨_, rfl⟩
to_ama #align monoid_algebra.single_mul_apply_of_not_exists_mul MonoidAlgebra.single_mul_apply_of_not_exists_mul
...
to_ama #align monoid_algebra.lift_nc_smul MonoidAlgebra.liftNC_smul
...
variable (k) [Semiring k] [DistribSMul R k]
...
to_ama #align monoid_algebra.is_scalar_tower_self MonoidAlgebra.isScalarTower_self
...
to_ama #align monoid_algebra.smul_comm_class_self MonoidAlgebra.smulCommClass_self
...
instance smulCommClass_symm_self [SMulCommClass k R k] :
  ...
  ⟨fun t a b => by
    ...
    rw [← smul_comm]⟩
to_ama #align monoid_algebra.smul_comm_class_symm_self MonoidAlgebra.smulCommClass_symm_self
...
to_ama #align monoid_algebra.non_unital_alg_hom_ext MonoidAlgebra.nonUnitalAlgHom_ext
...
def liftMagma [Module k A] [IsScalarTower k A A] [SMulCommClass k A A] :
    ...
    { liftAddHom fun x => (smulAddHom k A).flip (f x) with
      ...
      map_mul' := fun a₁ a₂ => by
        ...
        simp_rw [Finsupp.mul_sum, Finsupp.sum_mul, smul_mul_smul, ← f.map_mul, mul_def,
          sum_comm a₂ a₁]
        rw [sum_sum_index (by exact h₁) (by exact h₂)]; congr; ext
        rw [sum_sum_index (by exact h₁) (by exact h₂)]; congr; ext
        rw [sum_single_index (by exact h₁ _)] <;> rfl /- `rfl` is only for `to_ama` -/ }
  ...
  right_inv F := by
    ...
    simp only [NonUnitalAlgHom.coe_mk, ofMagma_apply, NonUnitalAlgHom.toMulHom_eq_coe,
      sum_single_index, Function.comp_apply, one_smul, zero_smul, MulHom.coe_comp,
      NonUnitalAlgHom.coe_to_mulHom]
to_ama #align monoid_algebra.lift_magma MonoidAlgebra.liftMagma
to_ama #align monoid_algebra.lift_magma_apply_apply MonoidAlgebra.liftMagma_apply_apply
to_ama #align monoid_algebra.lift_magma_symm_apply MonoidAlgebra.liftMagma_symm_apply
  ...
  { Finsupp.singleAddHom 1 with
    ...
    map_mul' := fun x y => by
      ...
      simp only [ZeroHom.toFun_eq_coe, AddMonoidHom.toZeroHom_coe, singleAddHom_apply,
        single_mul_single, mul_one, zero_add] /- added `zero_add` for `to_ama` -/ }
to_ama #align monoid_algebra.single_one_ring_hom MonoidAlgebra.singleOneRingHom
to_ama #align monoid_algebra.single_one_ring_hom_apply MonoidAlgebra.singleOneRingHom_apply
...
to_ama #align monoid_algebra.map_domain_ring_hom MonoidAlgebra.mapDomainRingHom
to_ama #align monoid_algebra.map_domain_ring_hom_apply MonoidAlgebra.mapDomainRingHom_apply
...
theorem ringHom_ext {R} [Semiring k] [MulOneClass G] [Semiring R] {f g : MonoidAlgebra k G →+* R}
  ...
  RingHom.coe_addMonoidHom_injective <|
    addHom_ext fun a b => by
      first | rw [← single, ← one_mul a, ← mul_one b, ← single_mul_single]  -- another hack
            | rw [← single, ← zero_add a, ← mul_one b, ← single_mul_single]
      -- This used to be `rw`, but we need `erw` after leanprover/lean4#2644
      erw [AddMonoidHom.coe_coe f, AddMonoidHom.coe_coe g]; rw [f.map_mul, g.map_mul, h₁, h_of]
to_ama #align monoid_algebra.ring_hom_ext MonoidAlgebra.ringHom_ext
...
-- the `monoid_algebra.algebra` instance was unnamed in mathlib3
...
def singleOneAlgHom {A : Type*} [CommSemiring k] [Semiring A] [Algebra k A] [Monoid G] :
  ...
  { singleOneRingHom with
    commutes' := fun r => by
      ...
      simp
      rfl }
to_ama #align monoid_algebra.single_one_alg_hom MonoidAlgebra.singleOneAlgHom
to_ama #align monoid_algebra.single_one_alg_hom_apply MonoidAlgebra.singleOneAlgHom_apply
...
theorem coe_algebraMap {A : Type*} [CommSemiring k] [Semiring A] [Algebra k A] [Monoid G] :
  ...
  rfl
to_ama #align monoid_algebra.coe_algebra_map MonoidAlgebra.coe_algebraMap
...
to_ama #align monoid_algebra.single_eq_algebra_map_mul_of MonoidAlgebra.single_eq_algebraMap_mul_of
...
variable [CommSemiring k] [Monoid G] [Monoid H] [AddMonoid G] [AddMonoid H]
...
to_ama #align monoid_algebra.lift_nc_alg_hom MonoidAlgebra.liftNCAlgHom
...
to_ama #align monoid_algebra.alg_hom_ext MonoidAlgebra.algHom_ext
...
def lift : (G →* A) ≃ (MonoidAlgebra k G →ₐ[k] A) where
  ...
  left_inv f := by
    ext
    simp [liftNCAlgHom, liftNCRingHom] <;> rfl -- `<;> rfl` added for `to_ama`
  right_inv F := by
    ext
    simp [liftNCAlgHom, liftNCRingHom] <;> rfl -- `<;> rfl` added for `to_ama`
to_ama #align monoid_algebra.lift MonoidAlgebra.lift
...
theorem lift_apply' (F : G →* A) (f : MonoidAlgebra k G) :
  ...
  rfl
to_ama #align monoid_algebra.lift_apply' MonoidAlgebra.lift_apply'
...
theorem lift_def (F : G →* A) : ⇑(lift k G A F) = liftNC ((algebraMap k A : k →+* A) : k →+ A) F :=
  rfl
to_ama #align monoid_algebra.lift_def MonoidAlgebra.lift_def
...
theorem lift_symm_apply (F : MonoidAlgebra k G →ₐ[k] A) (x : G) :
  ...
  rfl
to_ama #align monoid_algebra.lift_symm_apply MonoidAlgebra.lift_symm_apply
...
to_ama #align monoid_algebra.lift_single MonoidAlgebra.lift_single

to_ama G
theorem lift_of (F : G →* A) (x) : lift k G A F (of k G x) = F x := by
  simp <;> rfl -- `<;> rfl` added for `to_ama`
to_ama #align monoid_algebra.lift_of MonoidAlgebra.lift_of

to_ama
theorem lift_unique' (F : MonoidAlgebra k G →ₐ[k] A) :
    F = lift k G A ((F : MonoidAlgebra k G →* A).comp (of k G)) :=
  ((lift k G A).apply_symm_apply F).symm
to_ama #align monoid_algebra.lift_unique' MonoidAlgebra.lift_unique'
...
theorem lift_unique (F : MonoidAlgebra k G →ₐ[k] A) (f : MonoidAlgebra k G) :
  ...
  conv_lhs =>
    rw [lift_unique' F]
    simp [lift_apply]
  all_goals rfl -- `all_goals rfl` added for `to_ama`
to_ama #align monoid_algebra.lift_unique MonoidAlgebra.lift_unique
...
to_ama #align monoid_algebra.map_domain_non_unital_alg_hom MonoidAlgebra.mapDomainNonUnitalAlgHom
to_ama #align monoid_algebra.map_domain_non_unital_alg_hom_apply MonoidAlgebra.mapDomainNonUnitalAlgHom_apply
...
to_ama #align monoid_algebra.map_domain_alg_hom MonoidAlgebra.mapDomainAlgHom
to_ama #align monoid_algebra.map_domain_alg_hom_apply MonoidAlgebra.mapDomainAlgHom_apply
...
theorem prod_single [CommSemiring k] [CommMonoid G] {s : Finset ι} {a : ι → G} {b : ι → k} :
  ...
  Finset.cons_induction_on s rfl fun a s has ih => by
    first | rw [prod_cons has, ih, single_mul_single, prod_cons has, prod_cons has] -- hack `to_ama`
          | rw [prod_cons has, ih, single_mul_single, sum_cons has, prod_cons has]
to_ama #align monoid_algebra.prod_single MonoidAlgebra.prod_single
