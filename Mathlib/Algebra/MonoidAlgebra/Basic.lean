...
noncomputable def mapRangeAlgHom (f : A →ₐ[R] B) : MonoidAlgebra A M →ₐ[R] MonoidAlgebra B M :=
  liftNCAlgHom (singleOneAlgHom.comp f) (of B M) fun x y ↦ by simp [commute_iff_eq]
...
lemma mapRangeAlgHom_single (f : A →ₐ[R] B) (m : M) (a : A) :
    mapRangeAlgHom f (single m a) = single m (f a) := by
...
noncomputable def mapRangeAlgEquiv (f : A ≃ₐ[R] B) :
  ...
  __ := mapRangeAlgHom f
  invFun := mapRangeAlgHom (f.symm : B →ₐ[R] A)
  left_inv _ := by aesop
...
section mapRange
variable [CommSemiring R] [CommSemiring S] [Semiring A] [Algebra R A] [Semiring B] [Algebra R B]
  [AddMonoid M] [AddMonoid N]

@[simp]
lemma mapDomainRingHom_comp_algebraMap (f : M →+ N) :
    (mapDomainRingHom A f).comp (algebraMap R A[M]) = algebraMap R A[N] := by
  ext; simp
...
/-- The algebra homomorphism of monoid algebras induced by a homomorphism of the base algebras. -/
noncomputable def mapRangeAlgHom (f : A →ₐ[R] B) : A[M] →ₐ[R] B[M] :=
  liftNCAlgHom (singleZeroAlgHom.comp f) (of B M) fun x y ↦ by simp [commute_iff_eq]
...
/-- The algebra isomorphism of monoid algebras induced by an isomorphism of the base algebras. -/
@[simps apply]
noncomputable def mapRangeAlgEquiv (f : A ≃ₐ[R] B) : A[M] ≃ₐ[R] B[M] where
  __ := mapRangeAlgHom f
  invFun := mapRangeAlgHom (f.symm : B →ₐ[R] A)
  left_inv _ := by aesop
  right_inv _ := by aesop
