...
-- adding
--variable [Add G] in
--/-- [recycled by `to_top`] The multiplication in a monoid algebra. We make it irreducible so that Lean doesn't unfold
--it trying to unify two things that are different. -/
--@[irreducible]
--def _root_.AddMonoidAlgebra.mul' (f g : AddMonoidAlgebra k (Multiplicative G)) : AddMonoidAlgebra k (Multiplicative G) :=
--  f.sum fun a₁ b₁ => g.sum fun a₂ b₂ => single (a₁ * a₂) (b₁ * b₂)
...
-- adding
variable [Add G] in
/-- [autogenerated by `to_top`] The product of `f g : AddMonoidAlgebra k G` is the finitely supported function
  whose value at `a` is the sum of `f x * g y` over all pairs `x, y`
  such that `x * y = a`. (Think of the group ring of a group.) -/
instance _root_.AddMonoidAlgebra.instMul : Mul (AddMonoidAlgebra k G) :=
...
-- adding
--variable [Add (Multiplicative G)] in
--theorem _root_.AddMonoidAlgebra.mul_def {f g : AddMonoidAlgebra k (Multiplicative G)} :
--    f * g = f.sum fun a₁ b₁ => g.sum fun a₂ b₂ => single (a₁ * a₂) (b₁ * b₂) := by with_unfolding_all rfl
...
variable [Monoid R] in -- this way, `to_add` does not convert `Monoid R` to `AddMonoid R`
to_ama
instance faithfulSMul [Semiring k] [DistribMulAction R k] [FaithfulSMul R k]
...
to_ama
instance smulCommClass [Semiring k] [DistribMulAction R k]
...
variable [Monoid R] in -- this way, `to_add` does not convert `Monoid R` to `AddMonoid R`
to_ama
instance isCentralScalar [Semiring k] [DistribMulAction R k] [DistribMulAction Rᵐᵒᵖ k]
...
def liftMagma [Module k A] [IsScalarTower k A A] [SMulCommClass k A A] :
  ...
  { (MonoidAlgebra.liftMagma k : (Multiplicative G →ₙ* A) ≃ (_ →ₙₐ[k] A)) with
    toFun := fun g =>
      { (MonoidAlgebra.liftMagma k g : _) with
        toFun := fun a => sum a fun m t => t • g (Multiplicative.ofAdd m) }
