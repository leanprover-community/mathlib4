...
instance inhabited : Inhabited (MonoidAlgebra R M) := coeffEquiv.inhabited
instance addCommMonoid : AddCommMonoid (MonoidAlgebra R M) := coeffEquiv.addCommMonoid
instance instIsCancelAdd [IsCancelAdd R] : IsCancelAdd (MonoidAlgebra R M) := coeffEquiv.isCancelAdd

instance coeFun : CoeFun (MonoidAlgebra k G) fun _ => G → k where
  coeFun f := f.coeff

/-- `MonoidAlgebra.single a r` for `a : M`, `r : R` is the element `ar : R[M]`. -/
def single (a : G) (b : k) : MonoidAlgebra k G := .ofCoeff <| .single a b
