...
-/
@[to_additive_dont_translate, to_additive /--
...
noncomputable instance MonoidAlgebra.addCommMonoid : AddCommMonoid (MonoidAlgebra k G) :=
...
/-- `MonoidAlgebra.single a r` for `a : M`, `r : R` is the element `ar : MonoidAlgebra R M`. -/
@[to_additive /-- `AddMonoidAlgebra.single a r` for `a : M`, `r : R` is the element `ar : R[M]`. -/]
noncomputable abbrev single (a : G) (b : k) : MonoidAlgebra k G := Finsupp.single a b
...
@[to_additive (attr := irreducible) mul']
noncomputable def mul' (f g : MonoidAlgebra k G) : MonoidAlgebra k G :=
...
@[to_additive instMul /-- The product of `f g : k[G]` is the finitely supported function
whose value at `a` is the sum of `f x * g y` over all pairs `x, y`
such that `x + y = a`. (Think of the product of multivariate
polynomials where `α` is the additive monoid of monomial exponents.) -/]
noncomputable instance instMul : Mul (MonoidAlgebra k G) := ⟨MonoidAlgebra.mul'⟩
...
@[to_additive]
noncomputable instance nonUnitalNonAssocSemiring : NonUnitalNonAssocSemiring (MonoidAlgebra k G) :=
...
@[to_additive]
noncomputable instance nonUnitalSemiring : NonUnitalSemiring (MonoidAlgebra k G) :=
...
@[to_additive /-- The unit of the multiplication is `single 0 1`, i.e. the function
that is `1` at `0` and zero elsewhere. -/]
noncomputable instance one : One (MonoidAlgebra k G) :=
  ⟨single 1 1⟩
...
@[to_additive]
noncomputable instance nonAssocSemiring : NonAssocSemiring (MonoidAlgebra k G) :=
  { MonoidAlgebra.nonUnitalNonAssocSemiring with
...
noncomputable instance smulZeroClass [Semiring k] [SMulZeroClass R k] :
    SMulZeroClass R (MonoidAlgebra k G) :=
  Finsupp.smulZeroClass

@[to_additive distribSMul]
noncomputable instance distribSMul [Semiring k] [DistribSMul R k] :
...
@[to_additive isCentralScalar]
noncomputable instance isCentralScalar [Semiring k] [SMulZeroClass R k] [SMulZeroClass Rᵐᵒᵖ k]
...
@[to_additive]
noncomputable instance semiring : Semiring (MonoidAlgebra k G) :=
...
@[to_additive]
noncomputable instance nonUnitalCommSemiring [CommSemiring k] [CommSemigroup G] :
...
@[to_additive]
noncomputable instance commSemiring [CommSemiring k] [CommMonoid G] :
...
@[to_additive]
noncomputable instance addCommGroup [Ring k] : AddCommGroup (MonoidAlgebra k G) :=
  Finsupp.instAddCommGroup

@[to_additive]
noncomputable instance nonUnitalNonAssocRing [Ring k] [Mul G] :
    NonUnitalNonAssocRing (MonoidAlgebra k G) :=
  { MonoidAlgebra.addCommGroup, MonoidAlgebra.nonUnitalNonAssocSemiring with }

@[to_additive]
noncomputable instance nonUnitalRing [Ring k] [Semigroup G] : NonUnitalRing (MonoidAlgebra k G) :=
  { MonoidAlgebra.addCommGroup, MonoidAlgebra.nonUnitalSemiring with }

@[to_additive]
noncomputable instance nonAssocRing [Ring k] [MulOneClass G] : NonAssocRing (MonoidAlgebra k G) :=
...
@[to_additive]
noncomputable instance ring [Monoid G] : Ring (MonoidAlgebra R G) :=
...
noncomputable abbrev singleAddHom [Semiring k] (a : G) : k →+ MonoidAlgebra k G :=
  Finsupp.singleAddHom a
...
open Finset in
@[to_additive mul_apply_antidiagonal]
...
@[to_additive (attr := simp) single_mul_single]
...
@[to_additive single_commute_single]
...
-- The additive form should be a `@[simp]` lemma, but the simp_nf linter times out if we add this.
-- Probably the correct fix is to make a `[Add]MonoidAlgebra.single` with the correct type,
-- instead of relying on `Finsupp.single`.
@[to_additive (attr := simp) single_pow]
...
@[to_additive mul_single_apply_aux]
...
@[to_additive mul_single_zero_apply]
...
@[to_additive mul_single_apply_of_not_exists_add]
...
@[to_additive single_mul_apply_aux]
...
@[to_additive single_zero_mul_apply]
...
@[to_additive single_mul_apply_of_not_exists_add]
...
@[to_additive (attr := simps) /-- `Finsupp.single 0` as a `RingHom` -/]
noncomputable def singleOneRingHom [Semiring k] [MulOneClass G] : k →+* MonoidAlgebra k G :=
  { singleAddHom 1 with
    toFun := single 1
    map_one' := rfl
...
@[to_additive /-- If two ring homomorphisms from `k[G]` are equal on all `single a 1`
