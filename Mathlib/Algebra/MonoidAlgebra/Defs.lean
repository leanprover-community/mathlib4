...
instance nonAssocRing [Ring k] [MulOneClass G] : NonAssocRing (MonoidAlgebra k G) :=
  { MonoidAlgebra.addCommGroup,
    ...
    intCast_negSucc := fun n => by simp only [Int.cast_negSucc, Nat.cast_add, Nat.cast_one,
      neg_add_rev, Finsupp.single_add, single_neg]; rfl }
...
instance nonUnitalNonAssocSemiring : NonUnitalNonAssocSemiring k[G] :=
  { Finsupp.instAddCommMonoid with
    ...
    nsmul_zero := by
      ...
      rw [nsmul_apply, zero_nsmul, zero_apply]
    nsmul_succ := by
      intros
      refine Finsupp.ext fun _ => ?_
      rw [nsmul_apply, succ_nsmul, add_apply, nsmul_apply] }
