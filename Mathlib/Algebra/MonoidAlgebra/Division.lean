...
@[simp]
lemma divOf_apply (g : G) (x : k[G]) (g' : G) : (x /ᵒᶠ g) g' = x (g + g') :=
  rfl
#align add_monoid_algebra.div_of_apply AddMonoidAlgebra.divOf_apply

@[simp]
lemma support_divOf (g : G) (x : k[G]) :
    (x /ᵒᶠ g).support =
...
lemma of'_mul_divOf (a : G) (x : k[G]) : of' k G a * x /ᵒᶠ a = x := by
  refine Finsupp.ext fun _ => ?_  -- Porting note: `ext` doesn't work
...
lemma mul_of'_divOf (x : k[G]) (a : G) : x * of' k G a /ᵒᶠ a = x := by
  refine Finsupp.ext fun _ => ?_  -- Porting note: `ext` doesn't work
...
@[simp]
lemma modOf_apply_of_not_exists_add (x : k[G]) (g : G) (g' : G)
...
@[simp]
lemma modOf_apply_of_exists_add (x : k[G]) (g : G) (g' : G)
...
@[simp]
lemma modOf_apply_add_self (x : k[G]) (g : G) (d : G) : (x %ᵒᶠ g) (d + g) = 0 :=
...
lemma modOf_apply_self_add (x : k[G]) (g : G) (d : G) : (x %ᵒᶠ g) (g + d) = 0 :=
...
lemma of'_mul_modOf (g : G) (x : k[G]) : of' k G g * x %ᵒᶠ g = 0 := by
...
lemma mul_of'_modOf (x : k[G]) (g : G) : x * of' k G g %ᵒᶠ g = 0 := by
  refine Finsupp.ext fun g' => ?_  -- Porting note: `ext g'` doesn't work
  rw [Finsupp.zero_apply]
  obtain ⟨d, rfl⟩ | h := em (∃ d, g' = g + d)
  · rw [modOf_apply_self_add]
...
lemma divOf_add_modOf (x : k[G]) (g : G) :
