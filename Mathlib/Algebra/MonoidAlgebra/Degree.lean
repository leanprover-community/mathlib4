...
/-- Let `R` be a semiring, let `B` be a SemilatticeSup with Bot,
...
For example, `B` could be `WithBot A` with `D = WithBot.some`.
...
@[reducible] def supDegree [SemilatticeSup B] [OrderBot B] (f : R[A]) : B := f.support.sup D
