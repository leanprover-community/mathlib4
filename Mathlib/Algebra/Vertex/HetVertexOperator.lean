...
here allows us to consider composites and multiply by multivariable Laurent series.
## Definitions
* HetVertexOperator : An `R`-linear map from an `R`-module `V` to `HahnModule Γ W`.
...
* Composition of heterogeneous vertex operators - values are Hahn series on lex order product
  (needs PR#10781).
* `HahnSeries Γ R`-module structure on `HetVertexOperator Γ R V W` (needs PR#10846).  This means we
