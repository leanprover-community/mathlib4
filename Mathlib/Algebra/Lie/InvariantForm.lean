...
lemma _root_.LinearMap.BilinForm.lieInvariant_iff [LieAlgebra R L] [LieModule R L M] :
  ...
  · ext y z
    rw [LieHom.lie_apply, _root_.sub_apply, Module.Dual.lie_apply, _root_.zero_apply,
      _root_.zero_apply, h, sub_self]
  · replace h := LinearMap.congr_fun₂ (h x) y z
    simp only [LieHom.lie_apply, _root_.sub_apply, Module.Dual.lie_apply,
      _root_.zero_apply, sub_eq_zero] at h
    simp [← h]
...
theorem isSemisimple_of_nondegenerate : IsSemisimple K L := by
  ...
  intro z hz
  rw [← neg_eq_zero, ← hΦ_inv]
  suffices ⁅(x : L), z⁆ = 0 by simp only [this, map_zero, _root_.zero_apply]
