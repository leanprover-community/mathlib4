...
lemma genWeightSpace_ne_bot (χ : Weight R L M) : genWeightSpace M χ ≠ ⊥ := by
  obtain ⟨m, hm⟩ := χ.exists_lie_eq_smul'
  simp only [ne_eq, LieSubmodule.eq_bot_iff, not_forall, _root_.not_imp, mem_genWeightSpace]
  refine ⟨m, ?_, hm.1⟩
  intro x
  use 1
  simpa [sub_eq_zero] using hm.2 x
