...
lemma mem_sl2SubalgebraOfRoot_iff {α : Weight K H L} (hα : α.IsNonZero) :
    ∃ h e f : L, IsSl2Triple h e f ∧ e ∈ rootSpace H α ∧ f ∈ rootSpace H (- α) ∧ ∀ x : L,
  ...
  obtain ⟨ce, hce⟩ : ∃ c : K, he.choose = c • e := by
    obtain ⟨c, hc⟩ := (finrank_eq_one_iff_of_nonzero' ⟨e, ht.1⟩ (by simpa using t.e_ne_zero)).mp
  ...
  obtain ⟨cf, hcf⟩ : ∃ c : K, hf.choose = c • f := by
    obtain ⟨c, hc⟩ := (finrank_eq_one_iff_of_nonzero' ⟨f, ht.2⟩ (by simpa using t.f_ne_zero)).mp
  ...
  simp_rw [hcf, hce]
  refine ⟨h, e, f, t, ht.1, ht.2, fun x ↦ ⟨fun ⟨c₁, c₂, c₃, hx⟩ ↦
    ⟨c₁ * ce, c₂ * cf, c₃ * cf * ce, ?_⟩, fun ⟨c₁, c₂, c₃, hx⟩ ↦
    ⟨c₁ * ce⁻¹, c₂ * cf⁻¹, c₃ * ce⁻¹ * cf⁻¹, ?_⟩⟩⟩
  · simp [hx, mul_smul]
