...
lemma perfect_of_isAtom_of_nonabelian {R L : Type*} [CommRing R] [LieRing L] [LieAlgebra R L]
    (I : LieIdeal R L) (hI : IsAtom I) (h : ¬IsLieAbelian I) :
    ⁅I, I⁆ = I := by
  rw [← hI.le_iff_eq]
  · exact LieSubmodule.lie_le_right I I
  · intro contra
    rw [LieSubmodule.lie_eq_bot_iff] at contra
    apply h
    constructor
    simpa only [LieIdeal.coe_bracket_of_module, Subtype.ext_iff, LieSubmodule.coe_bracket,
      ZeroMemClass.coe_zero, Subtype.forall, LieSubmodule.mem_coeSubmodule] using contra
