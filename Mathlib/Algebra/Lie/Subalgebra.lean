...
@[simp]
lemma coe_inclusion (x : K) : (inclusion h x : L) = x :=
  rfl
#align lie_subalgebra.coe_hom_of_le LieSubalgebra.coe_inclusion

lemma inclusion_apply (x : K) : inclusion h x = ⟨x.1, h x.2⟩ :=
  rfl
...
@[simp]
lemma coe_ofLe : (ofLe h : Submodule R K') = LinearMap.range (Submodule.inclusion h) :=
  rfl
...
@[simp]
lemma equivOfLe_apply (x : K) : equivOfLe h x = ⟨inclusion h x, (inclusion h).mem_range_self x⟩ :=
  rfl
