...
lemma eventually_iInf_lowerCentralSeries_eq [IsArtinian R M] :
...
lemma map_lowerCentralSeries_le (f : M →ₗ⁅R,L⁆ M₂) :
...
lemma isNilpotent_quotient_iff :
...
lemma iInf_lcs_le_of_isNilpotent_quot (h : IsNilpotent R L (M ⧸ N)) :
...
@[simp]
lemma nilpotencyLength_eq_one_iff [Nontrivial M] :
    nilpotencyLength R L M = 1 ↔ IsTrivial L M := by
  rw [nilpotencyLength_eq_succ_iff, ← trivial_iff_lower_central_eq_bot]
  simp

lemma isTrivial_of_nilpotencyLength_le_one [IsNilpotent R L M] (h : nilpotencyLength R L M ≤ 1) :
    IsTrivial L M := by
  nontriviality M
...
lemma lowerCentralSeriesLast_le_of_not_isTrivial [IsNilpotent R L M] (h : ¬ IsTrivial L M) :
...
lemma ucs_bot_one : (⊥ : LieSubmodule R L M).ucs 1 = LieModule.maxTrivSubmodule R L M := by
