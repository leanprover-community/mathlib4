...
open FiniteDimensional LieAlgebra

/- We define the Submodules generated by repeatedly applying a linear map `f: V →ₗ[F] V`
to a vector `v`-/

namespace LieModule.LieTheorem

variable {R M : Type*} [Semiring R] [AddCommMonoid M] [Module R M]
variable (x : M) (f : Module.End R M)

/-- The submodule generated by `v`, `f v`, `f f v`, ..., `f^[n-1] v`.-/
abbrev U (n : ℕ) : Submodule R M := .span R {(f^a) x | a < n}

lemma U_mono {a b : ℕ} (h : a ≤ b) : U x f a ≤ U x f b :=
  Submodule.span_mono (fun _ ⟨c, hc, hw⟩ ↦ ⟨c, lt_of_lt_of_le hc h, hw⟩)

lemma map_U_le (n : ℕ) : Submodule.map f (U x f n) ≤ U x f (n + 1) := by
  rw [Submodule.map_span]
  apply Submodule.span_mono
  suffices ∀ a < n, ∃ b < n + 1, (f ^ b) x = (f ^ (a + 1)) x by simpa [pow_succ']
  aesop

/-- The union of `U` for all `n : ℕ`.-/
abbrev iSupU : Submodule R M := ⨆ k : ℕ, U x f k

lemma map_iSupU_le_iSup : Submodule.map f (iSupU x f) ≤ iSupU x f := by
  rw [Submodule.map_iSup, iSup_le_iff]
  exact fun i ↦ (map_U_le x f i).trans (le_iSup _ _)

-- the lie action of `L` on `V`
local notation "ρ" => LieModule.toEnd k
local notation "π" => ρ L V
...
lemma T_apply_succ (n : ℕ) :
    Submodule.map (T A χ w) (U v (π z) (n + 1)) ≤ U v (π z) n := by
  ...
  · next n hn =>
    ...
    · exact U_mono v _ (Nat.le_succ n) (hn w m hm)
    ...
    · exact U_mono v _ n.le_succ (hn wz n n.lt_succ_self)
    · exact Submodule.smul_mem _ _ (Submodule.subset_span ⟨n, n.lt_succ_self, rfl⟩)
    · exact map_U_le _ _ _ <| Submodule.mem_map_of_mem <| hn w n n.lt_succ_self

lemma T_map_iSup_U' : Submodule.map (T A χ w) (iSupU v (π z)) ≤ iSupU v (π z) := by
  rw [Submodule.map_iSup, iSup_le_iff]
  rintro (_|i)
  · simp [Submodule.map_span]
  · exact (T_apply_succ A χ z w hv i).trans (le_iSup _ _)

lemma T_map_iSup_U (x : V) (hx : x ∈ iSupU v (π z)) : (T A χ w) x ∈ iSupU v (π z) :=
  T_map_iSup_U' A χ z w hv <| Submodule.mem_map_of_mem hx

def iSupIR : LieSubmodule k A V where
  toSubmodule := iSupU v (π z)
  lie_mem {w} x hx := by
    simp only [AddSubsemigroup.mem_carrier, AddSubmonoid.mem_toSubsemigroup,
      Submodule.mem_toAddSubmonoid]
    have hx' : ⁅w, x⁆ = (T A χ w) x + χ w • x := by simp
    rw [hx']
    exact add_mem (T_map_iSup_U A χ z w hv x hx)
      (Submodule.smul_mem (iSupU v (π z)) _ hx)

lemma T_map_U_nilpotent (N : ℕ) : ∀ x ∈ (U v (π z)) N, (T A χ w ^ N) x = 0 := by
...
theorem T_res_nilpotent : _root_.IsNilpotent ((T A χ w).restrict (T_map_iSup_U A χ z w hv)) := by
  suffices iSupU v (π z) ≤ Module.End.maxGenEigenspace (T A χ w) 0 by
  ...
  use i
  exact T_map_U_nilpotent A χ z w hv i x hx

lemma trace_T_res_zero :
    LinearMap.trace k (iSupU v (π z)) ((T A χ w).restrict (T_map_iSup_U A χ z w hv)) = 0 := by
  apply IsNilpotent.eq_zero
  exact LinearMap.isNilpotent_trace_of_isNilpotent (T_res_nilpotent A χ z w hv)

lemma trace_πza (a : A) :
    LinearMap.trace k (iSupIR A χ z hv) (ρ A _ ⁅z, a⁆) =
...
theorem trace_πza_zero (a : A) : LinearMap.trace k (iSupIR A χ z hv) (ρ A _ ⁅z, a⁆) = 0 := by
  set U := iSupIR A χ z hv
  have hzU : ∀ x ∈ U, (π z) x ∈ U :=
    fun _ hx ↦ (map_iSupU_le_iSup v (π z)) (Submodule.mem_map_of_mem hx)
  have hres : ρ A U ⁅z, a⁆ = ⁅(π z).restrict hzU, ρ A U a⁆ := by
    ext ⟨x, hx⟩
...
lemma chi_za_zero (a : A) (hv' : v ≠ 0) : χ ⁅z, a⁆ = 0 := by
  ...
  have iSup_U_nontrivial : Nontrivial (iSupIR A χ z hv) :=
...
theorem extend_weight [LieModule.IsTriangularizable k L V] (A : LieIdeal k L) (z : L) (hz : z ∉ A)
  ...
  obtain ⟨c, hc⟩ : ∃ c, Module.End.HasEigenvalue (ρ _ Vχ' z) c := by
  ...
  · intro x
    ...
    have hzv' : ⁅z, v'⁆ = (ρ _ Vχ' z) ⟨v', hv'⟩ := rfl
...
theorem _root_.LieModule.exists_forall_lie_eq_smul_of_Solvable
