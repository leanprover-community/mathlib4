...
/-- A `LieRing` can be regarded as a `NonUnitalNonAssocSemiring` by turning its
`Bracket` (denoted `⁅, ⁆`) into a `Mul` (denoted `*`). -/
instance (priority := 10000) : NonUnitalNonAssocSemiring (CommutatorRing L) :=
...
instance (priority := 10000) (L : Type v) [Nonempty L] : Nonempty (CommutatorRing L) := ‹Nonempty L›

instance (priority := 10000) (L : Type v) [Inhabited L] : Inhabited (CommutatorRing L) := ‹Inhabited L›

instance (priority := 10000) : LieRing (CommutatorRing L) := show LieRing L by infer_instance

instance (priority := 10000) : LieAlgebra R (CommutatorRing L) := show LieAlgebra R L by infer_instance
...
instance (priority := 10000) isScalarTower : IsScalarTower R (CommutatorRing L) (CommutatorRing L) := ⟨smul_lie⟩
#align lie_algebra.is_scalar_tower LieAlgebra.isScalarTower

/-- Regarding the `LieRing` of a `LieAlgebra` as a `NonUnitalNonAssocSemiring`, we can
reinterpret the `lie_smul` law as an `SMulCommClass`. -/
instance (priority := 10000) smulCommClass : SMulCommClass R (CommutatorRing L) (CommutatorRing L) :=
