...
noncomputable instance {S : Type*} [Monoid S] [DistribMulAction S R] [IsScalarTower S R R] :
...
noncomputable instance : Zero (FreeLieAlgebra R X) where zero := Quot.mk _ 0

noncomputable instance : Add (FreeLieAlgebra R X) where
  add := Quot.map₂ (· + ·) (fun _ _ _ => Rel.addLeft _) fun _ _ _ => Rel.add_right _

noncomputable instance : Neg (FreeLieAlgebra R X) where neg := Quot.map Neg.neg fun _ _ => Rel.neg

noncomputable instance : Sub (FreeLieAlgebra R X) where
  sub := Quot.map₂ Sub.sub (fun _ _ _ => Rel.subLeft _) fun _ _ _ => Rel.subRight _

noncomputable instance : AddGroup (FreeLieAlgebra R X) :=
  Function.Surjective.addGroup (Quot.mk _) Quot.mk_surjective rfl (fun _ _ => rfl)
    (fun _ => rfl) (fun _ _ => rfl) (fun _ _ => rfl) fun _ _ => rfl

noncomputable instance : AddCommSemigroup (FreeLieAlgebra R X) :=
  Function.Surjective.addCommSemigroup (Quot.mk _) Quot.mk_surjective fun _ _ => rfl

noncomputable instance : AddCommGroup (FreeLieAlgebra R X) :=
  { (inferInstance : AddGroup (FreeLieAlgebra R X)),
    (inferInstance : AddCommSemigroup (FreeLieAlgebra R X)) with }

noncomputable instance {S : Type*} [Semiring S] [Module S R] [IsScalarTower S R R] :
...
noncomputable instance : LieRing (FreeLieAlgebra R X) where
...
noncomputable instance : LieAlgebra R (FreeLieAlgebra R X) where
  lie_smul := by
    rintro t ⟨a⟩ ⟨c⟩
...
`R`. -/
noncomputable def of : X → FreeLieAlgebra R X := fun x => Quot.mk _ (lib.of R x)

variable {L : Type w} [LieRing L] [LieAlgebra R L]

/-- An auxiliary definition used to construct the equivalence `lift` below. -/
noncomputable def liftAux (f : X → CommutatorRing L) :=
  lib.lift R f
...
noncomputable def lift : (X → L) ≃ (FreeLieAlgebra R X →ₗ⁅R⁆ L) where
  toFun f :=
...
algebra. -/
@[simps!]
noncomputable def universalEnvelopingEquivFreeAlgebra :
