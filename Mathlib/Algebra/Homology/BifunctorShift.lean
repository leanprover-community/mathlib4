...
variable {F G : C₁ ⥤ D} [Preadditive D]

@[simp]
lemma zsmul_app (a : ℤ) (φ : F ⟶ G) (X : C₁) : (a • φ).app X = a • φ.app X := by
  sorry

@[simp]
lemma units_zsmul_app (a : ℤˣ) (φ : F ⟶ G) (X : C₁) : (a • φ).app X = a • φ.app X := by
  apply zsmul_app
...
variable {I : Type*} {c : ComplexShape I} [Preadditive D]
  {K L : HomologicalComplex D c}

@[simp]
lemma units_zsmul_f (a : ℤˣ) (φ : K ⟶ L) (i : I) : (a • φ).f i = a • φ.f i := rfl
