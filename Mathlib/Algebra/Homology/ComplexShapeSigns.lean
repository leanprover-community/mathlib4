...
instance : TensorSigns (ComplexShape.down ℕ) where
  ...
  rel_add p q r (hpq : q + 1 = p) := by dsimp; cutsat
  add_rel p q r (hpq : q + 1 = p) := by dsimp; cutsat
  ε'_succ := by
    rintro _ q rfl
    dsimp
...
instance : TensorSigns (ComplexShape.up ℤ) where
  ...
  rel_add p q r (hpq : p + 1 = q) := by dsimp; cutsat
  add_rel p q r (hpq : p + 1 = q) := by dsimp; cutsat
  ε'_succ := by
    rintro p _ rfl
    dsimp
