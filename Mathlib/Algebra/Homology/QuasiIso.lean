...
lemma quasiIso_of_comp_left (f : C ⟶ D) [QuasiIso f] (g : D ⟶ E) [QuasiIso (f ≫ g)] :
    QuasiIso g :=
...
lemma quasiIso_of_comp_right (f : C ⟶ D) (g : D ⟶ E) [QuasiIso g] [QuasiIso (f ≫ g)] :
    QuasiIso f :=
...
lemma toQuasiIso {C D : HomologicalComplex W c} (e : HomotopyEquiv C D) : QuasiIso e.hom :=
  ⟨fun i => by
...
lemma toQuasiIso_inv {C D : HomologicalComplex W c} (e : HomotopyEquiv C D) (i : ι) :
...
lemma toSingle₀CokernelAtZeroIso_hom_eq [hf : QuasiIso f] :
...
lemma to_single₀_epi_at_zero [hf : QuasiIso f] : Epi (f.f 0) := by
  constructor
  intro Z g h Hgh
...
lemma to_single₀_exact_d_f_at_zero [hf : QuasiIso f] : Exact (X.d 1 0) (f.f 0) := by
...
lemma to_single₀_exact_at_succ [hf : QuasiIso f] (n : ℕ) :
...
lemma fromSingle₀KernelAtZeroIso_inv_eq [hf : QuasiIso f] :
...
lemma from_single₀_mono_at_zero [hf : QuasiIso f] : Mono (f.f 0) := by
  constructor
  intro Z g h Hgh
...
lemma from_single₀_exact_f_d_at_zero [hf : QuasiIso f] : Exact (f.f 0) (X.d 0 1) := by
  rw [Preadditive.exact_iff_homology_zero]
...
lemma from_single₀_exact_at_succ [hf : QuasiIso f] (n : ℕ) :
...
lemma CategoryTheory.Functor.quasiIso_of_map_quasiIso {C D : HomologicalComplex A c} (f : C ⟶ D)
