...
noncomputable def inl : Cochain F (mappingCone φ) (-1) :=
  Cochain.mk (fun p q hpq => homotopyCofiber.inlX φ p q (by dsimp; grind))
...
noncomputable def fst : Cocycle (mappingCone φ) F 1 :=
  Cocycle.mk (Cochain.mk (fun p q hpq => homotopyCofiber.fstX φ p q hpq)) 2 (by grind) (by
    ext p _ rfl
    simp [δ_v 1 2 (by grind) _ p (p + 2) (by grind) (p + 1) (p + 1) (by grind) rfl,
...
lemma inl_snd :
  ...
  ext p q hpq
  simp [Cochain.comp_v _ _ (add_zero (-1)) p q q (by grind) (by grind)]

@[simp]
lemma inr_fst :
    (Cochain.ofHom (inr φ)).comp (fst φ).1 (zero_add 1) = 0 := by
  ext p q hpq
  simp [Cochain.comp_v _ _ (zero_add 1) p p q (by grind) (by grind)]
...
lemma δ_descCochain (n' : ℤ) (hn' : n + 1 = n') :
  ...
  simp only [δ_add, Cochain.comp_add, δ_comp (fst φ).1 α _ 2 n n' hn' (by grind) (by grind),
...
noncomputable def liftCochain (α : Cochain K F m) (β : Cochain K G n) (h : n + 1 = m) :
  ...
  α.comp (inl φ) (by grind) + β.comp (Cochain.ofHom (inr φ)) (add_zero n)
...
noncomputable def mapHomologicalComplexXIso' (n m : ℤ) (hnm : n + 1 = m) :
  ...
  hom := H.map ((fst φ).1.v n m (by grind)) ≫
      (inl ((H.mapHomologicalComplex (ComplexShape.up ℤ)).map φ)).v m n (by grind) +
      H.map ((snd φ).v n n (add_zero n)) ≫
        (inr ((H.mapHomologicalComplex (ComplexShape.up ℤ)).map φ)).f n
  inv := (fst ((H.mapHomologicalComplex (ComplexShape.up ℤ)).map φ)).1.v n m (by grind) ≫
      H.map ((inl φ).v m n (by grind)) +
...
noncomputable def mapHomologicalComplexIso :
  ...
  HomologicalComplex.Hom.isoOfComponents (mapHomologicalComplexXIso φ H) (by
    rintro n _ rfl
    rw [ext_to_iff _ _ (n + 2) (by grind), assoc, assoc, d_fst_v _ _ _ _ rfl,
      assoc, assoc, d_snd_v _ _ _ rfl]
    simp only [mapHomologicalComplexXIso_eq φ H n (n + 1) rfl,
      mapHomologicalComplexXIso_eq φ H (n + 1) (n + 2) (by grind),
    ...
    · dsimp
      simp only [Functor.mapHomologicalComplex_obj_X, Functor.mapHomologicalComplex_obj_d,
        ...
        d_fst_v φ n (n + 1) (n + 2) rfl (by grind), Functor.map_neg]
    · dsimp
