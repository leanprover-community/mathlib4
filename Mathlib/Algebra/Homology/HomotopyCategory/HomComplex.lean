...
def comp {n₁ n₂ n₁₂ : ℤ} (z₁ : Cochain F G n₁) (z₂ : Cochain G K n₂) (h : n₁ + n₂ = n₁₂) :
    Cochain F K n₁₂ :=
  Cochain.mk (fun p q hpq => z₁.v p (p + n₁) rfl ≫ z₂.v (p + n₁) q (by grind))
...
lemma δ_zero_cochain_v (z : Cochain F G 0) (p q : ℤ) (hpq : p + 1 = q) :
  ...
  simp only [δ_v 0 1 (zero_add 1) z p q hpq p q (by grind) hpq, zero_add,
...
def equivHomotopy (φ₁ φ₂ : F ⟶ G) :
  ...
  invFun z :=
    { hom := fun i j => if hij : i + (-1) = j then z.1.v i j hij else 0
      zero := fun i j (hij : j + 1 ≠ i) => dif_neg (fun _ => hij (by grind))
      comm := fun p => by
  ...
  left_inv := fun ho => by
    ...
    split_ifs with h
    · rfl
    · rw [ho.zero i j (fun h' => h (by dsimp at h'; grind))]
  right_inv := fun z => by
    ext p q hpq
