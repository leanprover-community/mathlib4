...
def cocycleOfDegreewiseSplit : Cocycle S.X₃ S.X₁ 1 :=
  Cocycle.mk
    (Cochain.mk (fun p q _ => (σ p).s ≫ S.X₂.d p q ≫ (σ q).r)) 2 (by linarith) (by
      ...
      dsimp [ShortComplex.map] at this r_f s_g ⊢
...
noncomputable def mappingConeHomOfDegreewiseSplitIso :
  ...
  Hom.isoOfComponents (fun p => mappingConeHomOfDegreewiseSplitXIso S σ p _ rfl) (by
    ...
    have s_g := (σ (p + 1)).s_g
    dsimp [ShortComplex.map] at r_f s_g ⊢
    simp only [mappingConeHomOfDegreewiseSplitXIso, shiftFunctor_obj_X', Cochain.ofHom_v,
      XIsoOfEq_rfl, Iso.refl_hom, Units.neg_smul, one_smul, comp_neg, sub_comp, assoc, Hom.comm,
      neg_sub, comp_sub, mappingCone.ext_from_iff _ _ _ rfl, mappingCone.inl_v_snd_v_assoc,
      zero_comp, mappingCone.inl_v_fst_v_assoc, zero_sub,
      ...
      sub_zero, Hom.comm_assoc, shiftFunctor_obj_d', Int.negOnePow_one, neg_comp, sub_neg_eq_add,
      zero_add, and_true]
...
noncomputable def triangleRotateShortComplexSplitting (n : ℤ) :
  ...
  f_r := by
    rw [ShortComplex.map_f] -- why does not dsimp or simp applies this automatically?
    simp
  s_g := by
