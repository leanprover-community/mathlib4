...
-- The autogenerated `right_H` lemma above would create issues in `PageInfinity.lean`
@[simp]
lemma spectralSequenceHomologyData_right_H'
    (hn₁ : n₀ + 1 = n₁ := by lia) (hn₂ : n₁ + 1 = n₂ := by lia) :
    (X.spectralSequenceHomologyData data r r' hrr' hr pq pq' pq'' hpq hpq'
      i₀' i₀ i₁ i₂ i₃ i₃' hi₀' hi₀ hi₁ hi₂ hi₃ hi₃'  n₀ n₁ n₂ hn₁' hn₁ hn₂).right.H =
    X.E (homOfLE ((data.i₀_le' hrr' hr pq' hi₀' hi₀).trans (data.le₀₁' r hr pq' hi₀ hi₁)))
    (homOfLE (data.le₁₂' pq' hi₁ hi₂))
    (homOfLE ((data.le₂₃' r hr pq' hi₂ hi₃).trans (data.le₃₃' hrr' hr pq' hi₃ hi₃')))
    n₀ n₁ n₂ hn₁ hn₂ := rfl
