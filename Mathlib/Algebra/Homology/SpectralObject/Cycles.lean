...
in `ι` and for integers which satisfy `n₀ + 1 = n₁`.
In the documentation, we shall denote `δ : H^n₀(g) ⟶ H^n₁(f)`
this operator, and the kernel `X.cycles n₀ n₁ hn₁ f g`
shall be shortened as `Z^n₀(f, g)` and the cokernel
`X.opcycles n₀ n₁ hn₁ f g` as `opZ^n₁(f, g)`.
...
if `cokernelSequenceCycles_exact` and the exactness of
