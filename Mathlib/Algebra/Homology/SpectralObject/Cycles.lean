...
lemma cyclesMap_comp (α : mk₂ f g ⟶ mk₂ f' g') (α' : mk₂ f' g' ⟶ mk₂ f'' g'')
  ...
  subst h
  rw [← cancel_mono (X.iCycles f'' g'' n), Category.assoc, X.cyclesMap_i f' g' f'' g'' α' _ n rfl,
    X.cyclesMap_i_assoc f g f' g' α _ n rfl, ← Functor.map_comp]
...
lemma opcyclesMap_id (n : ℤ) :
  ...
  rw [← cancel_epi (X.pOpcycles f g n), X.p_opcyclesMap f g f g (𝟙 _) (𝟙 _), Functor.map_id,
...
lemma opcyclesMap_comp (α : mk₂ f g ⟶ mk₂ f' g') (α' : mk₂ f' g' ⟶ mk₂ f'' g'')
  ...
  subst h
  rw [← cancel_epi (X.pOpcycles f g n), X.p_opcyclesMap_assoc f g f' g' α _,
    X.p_opcyclesMap f' g' f'' g'' α' _, ← Functor.map_comp_assoc]
...
lemma δ_toCycles (hn₁ : n₀ + 1 = n₁ := by lia) :
  ...
  rw [← cancel_mono (X.iCycles f₁ f₂ n₁), Category.assoc, toCycles_i, δToCycles_iCycles,
    ← X.δ_naturality f₁₂ f₃ f₂ f₃ (twoδ₁Toδ₀ f₁ f₂ f₁₂ h₁₂) (𝟙 _) n₀ n₁, Functor.map_id,
    Category.id_comp]
...
lemma fromOpcyles_δ (hn₁ : n₀ + 1 = n₁ := by lia) :
  ...
  rw [← cancel_epi (X.pOpcycles f₂ f₃ n₀), p_fromOpcycles_assoc, pOpcycles_δFromOpcycles,
    X.δ_naturality f₁ f₂ f₁ f₂₃ (𝟙 _) (twoδ₂Toδ₁ f₂ f₃ f₂₃ h₂₃) n₀ n₁, Functor.map_id,
    Category.comp_id]
