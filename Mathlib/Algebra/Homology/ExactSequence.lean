...
structure IsComplex : Prop where
  ...
  zero (i : ℕ) (hi : i + 2 ≤ n := by clean_up; omega) :
...
@[reassoc]
lemma IsComplex.zero' (hS : S.IsComplex) (i j k : ℕ) (hij : i + 1 = j := by clean_up; omega)
    (hjk : j + 1 = k := by clean_up; omega) (hk : k ≤ n := by clean_up; omega) :
...
lemma isComplex₁ (S : ComposableArrows C 1) : S.IsComplex where
  zero i hi := by exfalso; clean_up; omega
...
@[reducible]
def sc' (hS : S.IsComplex) (i j k : ℕ) (hij : i + 1 = j := by clean_up; omega)
    (hjk : j + 1 = k := by clean_up; omega) (hk : k ≤ n := by clean_up; omega) :
    ShortComplex C :=
...
abbrev sc (hS : S.IsComplex) (i : ℕ) (hi : i + 2 ≤ n := by clean_up; omega) :
    ShortComplex C :=
  S.sc' hS i (i + 1) (i + 2)
...
structure Exact extends S.IsComplex : Prop where
  exact (i : ℕ) (hi : i + 2 ≤ n := by clean_up; omega) : (S.sc toIsComplex i).Exact

variable {S}

lemma Exact.exact' (hS : S.Exact) (i j k : ℕ) (hij : i + 1 = j := by clean_up; omega)
    (hjk : j + 1 = k := by clean_up; omega) (hk : k ≤ n := by clean_up; omega) :
...
def sc'Map {S₁ S₂ : ComposableArrows C n} (φ : S₁ ⟶ S₂) (h₁ : S₁.IsComplex) (h₂ : S₂.IsComplex)
    (i j k : ℕ) (hij : i + 1 = j := by clean_up; omega)
    (hjk : j + 1 = k := by clean_up; omega) (hk : k ≤ n := by clean_up; omega) :
...
def scMap {S₁ S₂ : ComposableArrows C n} (φ : S₁ ⟶ S₂) (h₁ : S₁.IsComplex) (h₂ : S₂.IsComplex)
    (i : ℕ) (hi : i + 2 ≤ n := by clean_up; omega) :
    S₁.sc h₁ i ⟶ S₂.sc h₂ i :=
...
def sc'MapIso {S₁ S₂ : ComposableArrows C n} (e : S₁ ≅ S₂)
    (h₁ : S₁.IsComplex) (h₂ : S₂.IsComplex) (i j k : ℕ) (hij : i + 1 = j := by clean_up; omega)
    (hjk : j + 1 = k := by clean_up; omega) (hk : k ≤ n := by clean_up; omega) :
    S₁.sc' h₁ i j k ≅ S₂.sc' h₂ i j k where
...
lemma exact₂_iff (S : ComposableArrows C 2) (hS : S.IsComplex) :
  ...
  constructor
  · intro h
    exact h.exact 0 (by clean_up; omega)
  · intro h
...
lemma exact_iff_δlast {n : ℕ} (S : ComposableArrows C (n + 2)) :
  ...
  · intro h
    ...
    · rw [exact₂_iff]; swap
      ...
      exact h.exact n (by clean_up; omega)
  · rintro ⟨h, h'⟩
    refine' Exact.mk (IsComplex.mk (fun i hi => _)) (fun i hi => _)
    · obtain hi | rfl := LE.le.lt_or_eq (show i ≤ n by clean_up; omega)
      · exact h.toIsComplex.zero i
      · exact h'.toIsComplex.zero 0
    · obtain hi | rfl := LE.le.lt_or_eq (show i ≤ n by clean_up; omega)
      · exact h.exact i
