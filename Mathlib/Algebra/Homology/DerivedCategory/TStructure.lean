...
def TStructure.t : TStructure (DerivedCategory C) where
  ...
  le_zero_le := by
    rintro X ⟨K, e, _⟩
    exact ⟨K, e, K.isStrictlyLE_of_le 0 1 (by cutsat)⟩
  ge_one_le := by
    rintro X ⟨K, e, _⟩
    exact ⟨K, e, K.isStrictlyGE_of_ge 0 1 (by cutsat)⟩
  exists_triangle_zero_one X := by
    ...
    refine ⟨Q.obj (K.truncLE 0), Q.obj (K.truncGE 1),
      ...
      inv (Q.map (K.shortComplexTruncLEX₃ToTruncGE 0 1 (by cutsat))) ≫ (triangleOfSES h).mor₃,
      isomorphic_distinguished _ (triangleOfSES_distinguished h) _ (Iso.symm ?_)⟩
    refine Triangle.isoMk _ _ (Iso.refl _) e₂
      (asIso (Q.map (K.shortComplexTruncLEX₃ToTruncGE 0 1 (by cutsat)))) ?_ ?_ (by simp)
    · dsimp
      rw [id_comp]
      rfl
    · dsimp
