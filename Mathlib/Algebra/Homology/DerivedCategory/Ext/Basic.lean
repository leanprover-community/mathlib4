...
lemma add_comp (α₁ α₂ : Ext X Y n) {m : ℕ} (β : Ext Y Z m) {p : ℕ} (h : n + m = p) :
  ...
  letI := HasDerivedCategory.standard C; ext; simp +zetaDelta [add_hom']
...
lemma neg_comp (α : Ext X Y n) {m : ℕ} (β : Ext Y Z m) {p : ℕ} (h : n + m = p) :
  ...
  letI := HasDerivedCategory.standard C; ext; simp +zetaDelta [neg_hom']
...
lemma zero_comp {m : ℕ} (β : Ext Y Z m) (p : ℕ) (h : n + m = p) :
  ...
  letI := HasDerivedCategory.standard C; ext; simp +zetaDelta [zero_hom']
