...
lemma δ_apply' (x₃ : (forget₂ C Ab).obj D.L₀.X₃)
  ...
  refine (congr($(e.hom.naturality D.δ) x₃).trans ?_).trans
    congr($((e.hom.naturality D.v₂₃.τ₁).symm) x₁)
  exact D.δ_apply _ _ _
    ((congr($(e.hom.naturality D.L₁.g) x₂).symm.trans (by simp_all)).trans
      congr($(e.hom.naturality D.v₀₁.τ₃) x₃))
    ((congr($(e.hom.naturality D.L₂.f) x₁).symm.trans (by simp_all)).trans
      congr($(e.hom.naturality D.v₁₂.τ₂) x₂))
