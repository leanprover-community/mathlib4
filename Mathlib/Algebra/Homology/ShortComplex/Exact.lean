...
instance (priority := 10000) : F.PreservesMonomorphisms where
  preserves {X Y} f hf := by
    let S := ShortComplex.mk (0 : X ⟶ X) f zero_comp
    exact ((S.map F).exact_iff_mono (by simp)).1
      (((S.exact_iff_mono rfl).2 hf).map F)

instance (priority := 10000) [Faithful F] [CategoryWithHomology C] : F.ReflectsMonomorphisms where
...
instance (priority := 10000) : F.PreservesEpimorphisms where
  preserves {X Y} f hf := by
    let S := ShortComplex.mk f (0 : Y ⟶ Y) comp_zero
    exact ((S.map F).exact_iff_epi (by simp)).1
      (((S.exact_iff_epi rfl).2 hf).map F)

instance (priority := 10000) [Faithful F] [CategoryWithHomology C] : F.ReflectsEpimorphisms where
  reflects {X Y} f hf := by
    let S := ShortComplex.mk f (0 : Y ⟶ Y) comp_zero
