...
def LinearMap.frobenius [Algebra R S] : S →ₛₗ[_root_.frobenius R p] S where
...
def LinearMap.iterateFrobenius [Algebra R S] : S →ₛₗ[_root_.iterateFrobenius R p n] S where
