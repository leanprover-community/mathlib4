...
structure StarLinearMap {R S : Type*} [Semiring R] [Semiring S] (σ : R →+* S) (M : Type*)
  (M₂ : Type*) [AddCommMonoid M] [AddCommMonoid M₂] [Module R M] [Module S M₂] [Star M] [Star M₂]
  extends LinearMap σ M M₂ where
  /-- By definition, a non-unital ⋆-algebra homomorphism preserves the `star` operation. -/
  map_star' : ∀ a : M, toFun (star a) = star (toFun a)

@[inherit_doc] notation:25 M " →ₗ†[" σ:25 "] " M₂:0 => StarLinearMap σ M M₂

/-
**TODO** Should (RingHom.id R) be (starRingEnd R)?
-/

@[inherit_doc] notation:25 A " →ₗ†[" R "] " B => StarLinearMap (RingHom.id R) A B
...
add_decl_doc StarLinearMap.toLinearMap

/-
**TODO** since the other Classes have been deprecated, presumably we don't want to introduce
`StarLinearMapClass`?
-/
...
class StarLinearMapClass (F : Type*) (R A B : outParam Type*)
...
namespace StarHomClass

variable {F R A B : Type*} [Semiring R]
variable [AddCommMonoid A] [Module R A] [Star A]
variable [AddCommMonoid B] [Module R B] [Star B]
variable [FunLike F A B] [LinearMapClass F R A B] [StarHomClass F A B]

/-
**TODO** should `map_star'` be `map_star`
-/
...
`A →⋆ₙₐ[R] B`. -/
@[coe]
def toStarLinearMap [StarHomClass F A B] (f : F) : A →ₗ†[R] B :=
  { (f : A →ₗ[R] B) with
    map_star' := map_star f }

instance [StarHomClass F A B] : CoeTC F (A →ₗ†[R] B) :=
  ⟨toStarLinearMap⟩
...
variable [AddCommMonoid A] [Module R A] [Star A]
variable [AddCommMonoid B] [Module R B] [Star B]
variable [AddCommMonoid C] [Module R C] [Star C]
variable [AddCommMonoid D] [Module R D] [Star D]

instance : FunLike (A →ₗ†[R] B) A B where
  coe f := f.toFun
  coe_injective' := by rintro ⟨⟨⟨f, _⟩, _⟩, _⟩ ⟨⟨⟨g, _⟩, _⟩, _⟩ h; congr

instance : LinearMapClass (A →ₗ†[R] B) R A B where
  map_smulₛₗ f := f.map_smul'
  map_add f := f.map_add'

instance : StarHomClass (A →ₗ†[R] B) A B where
  map_star f := f.map_star'

/-- See Note [custom simps projection] -/
def Simps.apply (f : A →ₗ†[R] B) : A → B := f

/-
**TODO** Should this work?
initialize_simps_projections StarLinearMap
  (toFun → apply)
-/

@[simp]
protected theorem coe_coe {F : Type*} [FunLike F A B] [LinearMapClass F R A B]
    [StarHomClass F A B] (f : F) :
    ⇑(f : A →ₗ†[R] B) = f := rfl

@[simp]
theorem coe_toLinearMap {f : A →ₗ†[R] B} : (f.toLinearMap : A → B) = f :=
  rfl

@[ext]
theorem ext {f g : A →ₗ†[R] B} (h : ∀ x, f x = g x) : f = g :=
  DFunLike.ext _ _ h

/-- Copy of a `NonUnitalStarAlgHom` with a new `toFun` equal to the old one. Useful
to fix definitional equalities. -/
protected def copy (f : A →ₗ†[R] B) (f' : A → B) (h : f' = f) : A →ₗ†[R] B where
  toFun := f'
...
@[simp]
theorem coe_copy (f : A →ₗ†[R] B) (f' : A → B) (h : f' = f) : ⇑(f.copy f' h) = f' :=
  rfl

theorem copy_eq (f : A →ₗ†[R] B) (f' : A → B) (h : f' = f) : f.copy f' h = f :=
  DFunLike.ext' h

@[simp]
theorem coe_mk (f : A → B) (h₁ h₂ h₃) :
    ((⟨⟨⟨f, h₁⟩, h₂⟩, h₃⟩ : A →ₗ†[R] B) : A → B) = f :=
  rfl

-- this is probably the more useful lemma for Lean 4 and should likely replace `coe_mk` above
@[simp]
theorem coe_mk' (f : A →ₗ[R] B) (h) :
    ((⟨f, h⟩ : A →ₗ†[R] B) : A → B) = f :=
  rfl

@[simp]
theorem mk_coe (f : A →ₗ†[R] B) (h₁ h₂ h₃) :
    (⟨⟨⟨f, h₁⟩, h₂⟩, h₃⟩ : A →ₗ†[R] B) = f := by
  ext
  rfl
...
protected def id : A →ₗ†[R] A :=
  { (1 : A →ₗ[R] A) with map_star' := fun _ => rfl }

@[simp]
theorem coe_id : ⇑(StarLinearMap.id R A) = id :=
  rfl
