...
abbrev convStar : Star (E →ₗ[R] F) where
  star f :=
  { toFun x := star (f (star x))
    map_add' _ _ := by simp
    map_smul' _ _ := by simp }

scoped[ConvolutionStar] attribute [instance] LinearMap.convStar

open scoped ConvolutionStar

@[simp] theorem convStar_apply (f : E →ₗ[R] F) (x : E) : (star f) x = star (f (star x)) := rfl

/-- The involutive convolution star structure on linear maps. -/
abbrev convInvolutiveStar : InvolutiveStar (E →ₗ[R] F) where
  star_involutive x := by ext; simp

scoped[ConvolutionStar] attribute [instance] LinearMap.convInvolutiveStar

/-- The convolution star additive monoid structure on linear maps. -/
abbrev convStarAddMonoid : StarAddMonoid (E →ₗ[R] F) where
...
theorem convStar_comp (f : E →ₗ[R] F) (g : G →ₗ[R] E) :
    star (f ∘ₗ g) = star f ∘ₗ star g := by ext; simp

@[simp] theorem convStar_id : star (LinearMap.id (R := R) (M := E)) = LinearMap.id := by ext; simp
@[simp] theorem convStar_zero : star (0 : E →ₗ[R] F) = 0 := by ext; simp
...
theorem convStar_mulLeft (x : E) : star (mulLeft R x) = mulRight R (star x) := by ext; simp

theorem convStar_mulRight (x : E) : star (mulRight R x) = mulLeft R (star x) := by
  rw [star_eq_iff_star_eq, convStar_mulLeft, star_star]
...
lemma convStarModule : StarModule R (E →ₗ[R] F) where
  star_smul _ _ := by ext; simp

scoped[ConvolutionStar] attribute [instance] LinearMap.convStarModule
