...
lemma map_adjoin [StarModule R B] (f : A →⋆ₐ[R] B) (s : Set A) :
...
lemma ext_adjoin {s : Set A} [StarAlgHomClass F R (adjoin R s) B] {f g : F}
...
lemma ext_adjoin_singleton {a : A} [StarAlgHomClass F R (adjoin R ({a} : Set A)) B] {f g : F}
