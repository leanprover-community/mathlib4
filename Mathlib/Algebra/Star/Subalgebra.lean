...
instance (priority := 10000) setLike : SetLike (StarSubalgebra R A) A where
  coe S := S.carrier
  coe_injective' p q h := by obtain ⟨⟨⟨⟨⟨_, _⟩, _⟩, _⟩, _⟩, _⟩ := p; cases q; congr

instance (priority := 10000) starMemClass : StarMemClass (StarSubalgebra R A) A where
  star_mem {s} := s.star_mem'


instance (priority := 10000) subsemiringClass : SubsemiringClass (StarSubalgebra R A) A where
  add_mem {s} := s.add_mem'
  mul_mem {s} := s.mul_mem'
  one_mem {s} := s.one_mem'
  zero_mem {s} := s.zero_mem'

instance (priority := 10000) smulMemClass : SMulMemClass (StarSubalgebra R A) R A where
  smul_mem {s} r a (ha : a ∈ s.toSubalgebra) :=
    (SMulMemClass.smul_mem r ha : r • a ∈ s.toSubalgebra)

instance (priority := 10000) subringClass {R A} [CommRing R] [StarRing R] [Ring A] [StarRing A] [Algebra R A]
...
instance (priority := 10000) starRing (s : StarSubalgebra R A) : StarRing s :=
...
instance (priority := 10000) algebra (s : StarSubalgebra R A) : Algebra R s :=
  s.toSubalgebra.algebra'

instance (priority := 10000) starModule (s : StarSubalgebra R A) : StarModule R s where
...
instance (priority := 10000) involutiveStar : InvolutiveStar (Subalgebra R A) where
  star S :=
...
if `x` is normal. -/
instance (priority := 10000) adjoinCommSemiringOfIsStarNormal (x : A) [IsStarNormal x] :
...
/-- The star subalgebra `StarSubalgebra.adjoin R {x}` generated by a single `x : A` is commutative
if `x` is normal. -/
instance (priority := 10000) adjoinCommRingOfIsStarNormal (R : Type u) {A : Type v} [CommRing R] [StarRing R] [Ring A]
...
instance (priority := 10000) completeLattice : CompleteLattice (StarSubalgebra R A) where
...
instance (priority := 10000) inhabited : Inhabited (StarSubalgebra R A) :=
  ⟨⊤⟩
