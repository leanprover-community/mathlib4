...
@[simp]
lemma star_mul' [CommSemigroup R] [StarMul R] (x y : R) : star (x * y) = star x * star y :=
...
@[simp]
lemma star_div [CommGroup R] [StarMul R] (x y : R) : star (x / y) = star x / star y :=
...
lemma star_id_of_comm {R : Type*} [CommSemiring R] {x : R} : star x = x :=
  rfl
...
lemma starRingEnd_apply [CommSemiring R] [StarRing R] {x : R} : starRingEnd R x = star x :=
  rfl
...
If that's not the case try reordering lemmas or adding @[priority].
 -/
-- @[simp]
lemma starRingEnd_self_apply [CommSemiring R] [StarRing R] (x : R) :
...
lemma RingHom.star_def {S : Type*} [NonAssocSemiring S] [CommSemiring R] [StarRing R]
    (f : S →+* R) : Star.star f = RingHom.comp (starRingEnd R) f :=
  rfl
#align ring_hom.star_def RingHom.star_def

lemma RingHom.star_apply {S : Type*} [NonAssocSemiring S] [CommSemiring R] [StarRing R]
...
/-- When multiplication is commutative, `star` preserves division. -/
@[simp]
lemma star_div' [Semifield R] [StarRing R] (x y : R) : star (x / y) = star x / star y := by
  apply op_injective
...
lemma IsUnit.star [Monoid R] [StarMul R] {a : R} : IsUnit a → IsUnit (star a)
