...
lemma mul_inv_mem_unitary {G : Type*} [Group G] [StarMul G] (a b : G) :
...
lemma inv_mul_mem_unitary {G : Type*} [Group G] [StarMul G] (a b : G) :
    a⁻¹ * b ∈ unitary G ↔ a * star a = b * star b := by
  simpa [← mul_inv_rev] using mul_inv_mem_unitary a⁻¹ b⁻¹
...
protected lemma _root_.Units.mul_inv_mem_unitary (a b : Rˣ) :
  ...
  simp [← mul_inv_mem_unitary, ← Units.val_mul, Units.unitary_eq]
...
protected lemma _root_.Units.inv_mul_mem_unitary (a b : Rˣ) :
  ...
  simp [← inv_mul_mem_unitary, ← Units.val_mul, Units.unitary_eq]
