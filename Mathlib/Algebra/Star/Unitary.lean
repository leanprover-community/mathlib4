...
lemma toUnits_injective : Function.Injective (toUnits : unitary R → Rˣ) := fun _ _ h =>
  Subtype.ext <| Units.ext_iff.mp h
#align unitary.to_units_injective unitary.toUnits_injective

lemma _root_.IsUnit.mem_unitary_of_star_mul_self  {u : R} (hu : IsUnit u)
...
lemma _root_.IsUnit.mem_unitary_of_mul_star_self {u : R} (hu : IsUnit u)
