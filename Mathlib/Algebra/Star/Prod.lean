...
@[simp]
lemma fst_star [Star R] [Star S] (x : R × S) : (star x).1 = star x.1 :=
  rfl
#align prod.fst_star Prod.fst_star

@[simp]
lemma snd_star [Star R] [Star S] (x : R × S) : (star x).2 = star x.2 :=
  rfl
#align prod.snd_star Prod.snd_star

lemma star_def [Star R] [Star S] (x : R × S) : star x = (star x.1, star x.2) :=
  rfl
...
lemma Units.embed_product_star [Monoid R] [StarMul R] (u : Rˣ) :
