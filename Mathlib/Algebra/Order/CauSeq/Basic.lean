...
@[ext]
lemma ext {f g : CauSeq β abv} (h : ∀ i, f i = g i) : f = g := Subtype.eq (funext h)
...
lemma bounded (f : CauSeq β abv) : ∃ r, ∀ i, abv (f i) < r := f.2.bounded
#align cau_seq.bounded CauSeq.bounded

lemma bounded' (f : CauSeq β abv) (x : α) : ∃ r > x, ∀ i, abv (f i) < r := f.2.bounded' x
#align cau_seq.bounded' CauSeq.bounded'
...
lemma smul_equiv_smul {G : Type*} [SMul G β] [IsScalarTower G β β] {f1 f2 : CauSeq β abv} (c : G)
...
@[simp]
protected lemma sup_idem (a : CauSeq α abs) : a ⊔ a = a := Subtype.ext (sup_idem _)
#align cau_seq.sup_idem CauSeq.sup_idem

@[simp]
protected lemma inf_idem (a : CauSeq α abs) : a ⊓ a = a := Subtype.ext (inf_idem _)
