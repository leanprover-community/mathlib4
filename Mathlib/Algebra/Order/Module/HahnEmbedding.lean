...
Archimedean grades is a family of linearly independent submodules,
where each is the complement of the ball under the corresponding closed ball.
...
structure HahnEmbeddingSeed where
  ...
  strictMono (c : ArchimedeanClass M) : StrictMono (gradeMap c)
...
theorem grade_eq_bot_iff (c : ArchimedeanClass M) : seed.grade c = ⊥ ↔ c = ⊤ := by
  ...
  · rintro h
    rw [h] at hsup
    replace hsup : ball K c = closedBall K c := by simpa using hsup
    contrapose! hsup with h
    apply Submodule.ext_iff.not.mpr
...
omit [IsOrderedAddMonoid R] [Archimedean R] in
theorem archimedean_grade (c : ArchimedeanClass M) : Archimedean (seed.grade c) := by
...
`PartialHahnEmbedding`. These embedding share nice properties, including being strictly monotone,
transferring `ArchimedeanClass` to `HahnEmbedding.orderTop`, and being "truncation-closed"
(see `IsPartialHahnEmbedding.truncLT_mem_range`).
-/
...
theorem baseEmbedding_pos {x : seed.baseEmbedding.domain} (hx : 0 < x) :
  ...
  have hmono : StrictMonoOn (fun x ↦ ArchimedeanClass.mk (f x).val) f.support := by
    intro c hc d hd h
    simp only
    rw [seed.archimedeanClassMk_eq (f c).prop (by simpa using hc)]
    rw [seed.archimedeanClassMk_eq (f d).prop (by simpa using hd)]
    exact h
  rw [DFinsupp.sum, mk_sum hsupport hmono]
  rw [seed.archimedeanClassMk_eq (f _).prop (by simpa using f.support.min'_mem hsupport)]
  by_cases hsupport' : (f.support.erase (f.support.min' hsupport)).Nonempty
  · rw [mk_sum hsupport' (hmono.mono (by simp))]
    rw [seed.archimedeanClassMk_eq (f _).prop (by
...
theorem baseEmbedding_strictMono : StrictMono seed.baseEmbedding := by
  intro x y h
  apply lt_of_sub_pos
...
instance : Inhabited (PartialHahnEmbedding seed) where
  default := ⟨seed.baseEmbedding, seed.isPartialHahnEmbedding_baseEmbedding⟩
...
theorem apply_of_mem_grade {x : f.val.domain} {c : FiniteArchimedeanClass M}
  ...
  simp [HahnSeries.coeff_single_of_ne hdc, DFinsupp.single_eq_of_ne hcd]

omit [Archimedean R] in
/-- Archimedean equivalence is preserved by `f`. -/
theorem archimedeanClassMk_eq_iff (x y : f.val.domain) :
...
theorem orderTop_eq_iff (x y : f.val.domain) :
  ...
  have : Nontrivial R := (seed.strictMono c).injective.nontrivial
...
theorem orderTop_eq_archimedeanClass (x : f.val.domain) :
...
theorem orderTop_eq_finiteArchimedeanClassMk {x : f.val.domain} (hx0 : x.val ≠ 0) :
...
theorem coeff_eq_zero_of_mem {c : ArchimedeanClass M} {x : f.val.domain} (hx : x.val ∈ ball K c)
    {d : FiniteArchimedeanClass M} (hC : d.val ≤ c) : (ofLex (f.val x)).coeff d = 0 := by
  by_cases hc : c = ⊤
  · rw [hc] at hx
...
theorem coeff_ne_zero {x : f.val.domain} (hx0 : x.val ≠ 0) :
...
theorem coeff_eq_of_mem (x : M) {y z : f.val.domain} {c : ArchimedeanClass M}
    (hy : y.val - x ∈ ball K c) (hz : z.val - x ∈ ball K c)
    {d : FiniteArchimedeanClass M} (hC : d ≤ c) :
...
theorem evalCoeff_eq {x : M} {c : FiniteArchimedeanClass M} {y : f.val.domain}
    (hy : y.val - x ∈ ball K c) : evalCoeff f x c = (ofLex (f.val y)).coeff c := by
...
theorem isWF_support_evalCoeff (x : M) : (evalCoeff f x).support.IsWF := by
...
noncomputable
def eval (x : M) : Lex (HahnSeries (FiniteArchimedeanClass M) R) :=
...
theorem eval_smul (k : K) (x : M) : f.eval (k • x) = k • f.eval x := by
  by_cases hk : k = 0
  · simp [hk]
  unfold eval
...
theorem archimedeanClassMk_le_of_eval_eq {x : M} {y : f.val.domain} (h : f.eval x = f.val y)
    (z : f.val.domain) : mk (x - z.val) ≤ mk (x - y.val) := by
...
theorem eval_ne {x : M} (hx : x ∉ f.val.domain) (y : f.val.domain) : f.eval x ≠ f.val y := by
...
theorem eval_eq_truncLT {x : M} {c : FiniteArchimedeanClass M} {y : f.val.domain}
...
theorem exists_sub_mem_ball {x : M} (hx : x ∉ f.val.domain) (y : f.val.domain) :
    ∃ z : f.val.domain, z.val - x ∈ ball K (mk (y.val - x)) := by
  have : y.val - x ≠ 0 := by
    contrapose! hx
...
theorem eval_lt {x : M} (hx : x ∉ f.val.domain) (y : f.val.domain) (h : x < y.val) :
    f.eval x < f.val y := by
...
noncomputable
def extendFun {x : M} (hx : x ∉ f.val.domain) :
    M →ₗ.[K] Lex (HahnSeries (FiniteArchimedeanClass M) R) :=
  LinearPMap.supSpanSingleton f.val x (eval f x) hx

theorem extendFun_strictMono {x : M} (hx : x ∉ f.val.domain) : StrictMono (f.extendFun hx) := by
...
theorem baseEmbedding_le_extendFun {x : M} (hx : x ∉ f.val.domain) :
    seed.baseEmbedding ≤ (f.extendFun hx) := by
  rw [extendFun]
  exact le_trans f.prop.baseEmbedding_le <| LinearPMap.left_le_sup _ _ _

theorem truncLT_eval_mem_range_extendFun {x : M} (hx : x ∉ f.val.domain)
    (c : FiniteArchimedeanClass M) : toLex (HahnSeries.truncLTLinearMap K c (ofLex (f.eval x))) ∈
...
theorem truncLT_mem_range_extendFun {x : M} (hx : x ∉ f.val.domain) (y : (f.extendFun hx).domain)
    (c : FiniteArchimedeanClass M) :
...
omit [IsOrderedAddMonoid R] [Archimedean R] in
theorem baseEmbedding_le_sSupFun {c : Set (PartialHahnEmbedding seed)}
...
omit [IsOrderedAddMonoid R] [Archimedean R] in
theorem truncLT_mem_range_sSupFun {c : Set (PartialHahnEmbedding seed)}
...
there exists a strictly monotone `M →ₗ[K] Lex (HahnSeries (FiniteArchimedeanClass M) R)` that maps
`ArchimedeanClass M` to `HahnSeries.orderTop` in the expected way, as long as
`HahnEmbeddingSeed K M R` is nonempty. The `PartialHahnEmbedding` with maximal domain is the
desired embedding. -/
theorem exists_linearMap_hahnSeries_strictMono_and_archimedeanClassMk_eq
    [h : Nonempty (HahnEmbeddingSeed K M R)] :
  ...
  obtain ⟨e, hdomain⟩ := PartialHahnEmbedding.exists_domain_eq_top h.some
