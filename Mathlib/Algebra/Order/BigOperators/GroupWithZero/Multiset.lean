...
theorem prod_map_le_pow_card {F L : Type*}[MulPosMono R] [AddCommGroup L] [FunLike F L R]
    [NonnegHomClass F L R] {f : F} {y : L} {t : Multiset L}
    (hf : ∀ x : R, x ∈ Multiset.map f t → x ≤ f y) : (map f t).prod ≤ f y ^ card t := by
