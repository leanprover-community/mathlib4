...
theorem card_le_card_biUnion_add_card_fiber {s : Finset ι} {f : ι → Finset α}
  ...
  gcongr
  exact (card_le_card_biUnion (hs.subset <| filter_subset _ _) fun i hi ↦
    nonempty_of_ne_empty (mem_filter.1 hi).2).trans <| card_le_card <|
      biUnion_subset_biUnion_of_subset_left _ <| filter_subset _ _

theorem card_le_card_biUnion_add_one {s : Finset ι} {f : ι → Finset α} (hf : Injective f)
    (hs : (s : Set ι).PairwiseDisjoint f) : #s ≤ #(s.biUnion f) + 1 :=
  (card_le_card_biUnion_add_card_fiber hs).trans <| by
    gcongr
    exact card_le_one.2 fun _ hi _ hj ↦ hf <| (mem_filter.1 hi).2.trans (mem_filter.1 hj).2.symm
