...
@[simp] theorem sub_eq_top {a b : WithTop α} : a - b = ⊤ ↔ a = ⊤ ∧ b ≠ ⊤ := by
...
lemma sub_ne_top {a b : WithTop α} : a - b ≠ ⊤ ↔ a ≠ ⊤ ∨ b = ⊤ := by simp [or_iff_not_imp_left]
...
variable [CanonicallyOrderedAddCommMonoid α] [Sub α] [OrderedSub α] {a : WithTop α}
