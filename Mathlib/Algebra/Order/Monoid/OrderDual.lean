...
@[to_additive]
instance isOrderedMonoid [CommSemigroup α] [Preorder α] [IsOrderedMonoid α] :
    IsOrderedMonoid αᵒᵈ where
  mul_le_mul_left _ _ h c := mul_le_mul_left h c

@[to_additive]
instance isOrderedCancelMonoid [CommSemigroup α] [Preorder α] [IsOrderedCancelMonoid α] :
