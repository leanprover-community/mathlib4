...
def Function.Injective.orderedCommMonoid [OrderedCommMonoid α] {β : Type*} [One β] [Mul β]
    ...
    OrderedCommMonoid β :=
  reduceProj% zeta%
  { toCommMonoid := delta% hf.commMonoid f one mul npow
    toPartialOrder := PartialOrder.lift f hf
    mul_le_mul_left := fun a b ab c ↦ show f (c * a) ≤ f (c * b) by
...
def Function.Injective.orderedCancelCommMonoid [OrderedCancelCommMonoid α] [One β] [Mul β] [Pow β ℕ]
  ...
  reduceProj% zeta%
  { toOrderedCommMonoid := delta% hf.orderedCommMonoid f one mul npow
...
def Function.Injective.linearOrderedCancelCommMonoid [LinearOrderedCancelCommMonoid α] [One β]
  ...
  reduceProj% zeta%
  { toOrderedCancelCommMonoid := delta% hf.orderedCancelCommMonoid f one mul npow
    __ := delta% hf.linearOrderedCommMonoid f one mul npow hsup hinf }
