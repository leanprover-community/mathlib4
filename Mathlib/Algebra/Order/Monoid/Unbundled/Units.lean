...
theorem Units.mul_le_mul_left [MulLeftMono M] (a : Mˣ) {b c : M} : a * b ≤ a * c ↔ b ≤ c :=
  a.mulLECancellable_val.mul_le_mul_iff_left

theorem IsUnit.mulLECancellable [MulLeftMono M] {a : M} (ha : IsUnit a) : MulLECancellable a :=
...
theorem Units.mul_le_mul_right [MulRightMono M] (a : Mˣ) {b c : M} : b * a ≤ c * a ↔ b ≤ c :=
