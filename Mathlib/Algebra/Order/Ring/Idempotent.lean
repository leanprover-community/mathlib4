...
instance : BooleanAlgebra {a : R × R // a.1 * a.2 = 0 ∧ a.1 + a.2 = 1} where
  min a b := (aᶜ ⊔ bᶜ)ᶜ
  inf_le_left a b := by simp_rw [(· ≤ ·), (· ⊔ ·), (·ᶜ), mul_right_comm,
    (IsIdempotentElem.of_mul_add a.2.1 a.2.2).1.eq]
  inf_le_right a b := by simp_rw [(· ≤ ·), (· ⊔ ·), (·ᶜ), mul_assoc,
    (IsIdempotentElem.of_mul_add b.2.1 b.2.2).1.eq]
  le_inf a b c hab hac := by
    simp_rw [(· ≤ ·), (· ⊔ ·), (·ᶜ), ← mul_assoc]; rw [hab, hac]
  le_sup_inf a b c := Eq.le <| mul_eq_zero_add_eq_one_ext_right <| by
    simp_rw [(· ⊔ ·), (·ᶜ), add_mul, mul_add, mul_mul_mul_comm _ b.1.1,
  ...
  inf_compl_le_bot a := Eq.le <| mul_eq_zero_add_eq_one_ext_right <| by
    simp_rw [(· ⊔ ·), (·ᶜ), (IsIdempotentElem.of_mul_add a.2.1 a.2.2).1.eq, add_comm, a.2.2]
...
instance : BooleanAlgebra {a : R // IsIdempotentElem a} where
  ...
  himp_eq a b := Subtype.ext <| by simp_rw [(· ⊔ ·), add_comm b.1, add_sub_assoc, mul_sub, mul_one,
