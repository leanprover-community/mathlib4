...
`SMulWithZero k M`.-/
lemma smul_neg_iff_of_pos (hc : 0 < c) : c • a < 0 ↔ a < 0 := by
...
lemma smul_lt_smul_of_neg (h : a < b) (hc : c < 0) : c • b < c • a := by
...
lemma smul_le_smul_of_nonpos (h : a ≤ b) (hc : c ≤ 0) : c • b ≤ c • a := by
...
lemma eq_of_smul_eq_smul_of_neg_of_le (hab : c • a = c • b) (hc : c < 0) (h : a ≤ b) : a = b := by
...
lemma lt_of_smul_lt_smul_of_nonpos (h : c • a < c • b) (hc : c ≤ 0) : b < a := by
...
lemma smul_pos_iff_of_neg (hc : c < 0) : 0 < c • a ↔ a < 0 := by
...
lemma antitone_smul_left (hc : c ≤ 0) : Antitone (SMul.smul c : M → M) := fun _ _ h =>
  smul_le_smul_of_nonpos h hc
#align antitone_smul_left antitone_smul_left

lemma strict_anti_smul_left (hc : c < 0) : StrictAnti (SMul.smul c : M → M) := fun _ _ h =>
...
lemma smul_lowerBounds_subset_upperBounds_smul (hc : c ≤ 0) :
...
lemma smul_upperBounds_subset_lowerBounds_smul (hc : c ≤ 0) :
...
lemma BddBelow.smul_of_nonpos (hc : c ≤ 0) (hs : BddBelow s) : BddAbove (c • s) :=
  (antitone_smul_left hc).map_bddBelow hs
#align bdd_below.smul_of_nonpos BddBelow.smul_of_nonpos

lemma BddAbove.smul_of_nonpos (hc : c ≤ 0) (hs : BddAbove s) : BddBelow (c • s) :=
