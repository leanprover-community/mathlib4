...
lemma zpow_le_of_le (ha : 1 ≤ a) (h : m ≤ n) : a ^ m ≤ a ^ n := by
...
lemma zpow_strictMono (hx : 1 < a) : StrictMono ((· ^ ·) a : ℤ → α) :=
...
lemma zpow_strictAnti (h₀ : 0 < a) (h₁ : a < 1) : StrictAnti ((· ^ ·) a : ℤ → α) :=
...
@[simp]
lemma zpow_inj (h₀ : 0 < a) (h₁ : a ≠ 1) : a ^ m = a ^ n ↔ m = n :=
...
lemma zpow_two_nonneg (a : α) : 0 ≤ a ^ (2 : ℤ) := by
  convert zpow_bit0_nonneg a 1
#align zpow_two_nonneg zpow_two_nonneg

lemma zpow_neg_two_nonneg (a : α) : 0 ≤ a ^ (-2 : ℤ) :=
...
lemma zpow_two_pos_of_ne_zero (h : a ≠ 0) : 0 < a ^ (2 : ℤ) := by
  convert zpow_bit0_pos h 1
#align zpow_two_pos_of_ne_zero zpow_two_pos_of_ne_zero

@[simp]
lemma zpow_bit0_pos_iff (hn : n ≠ 0) : 0 < a ^ bit0 n ↔ a ≠ 0 :=
  ⟨by
    rintro h rfl
...
@[simp]
lemma zpow_bit0_abs (a : α) (p : ℤ) : |a| ^ bit0 p = a ^ bit0 p :=
...
lemma Nat.cast_le_pow_div_sub (H : 1 < a) (n : ℕ) : (n : α) ≤ a ^ n / (a - 1) :=
