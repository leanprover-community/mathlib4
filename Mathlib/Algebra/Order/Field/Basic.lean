...
private lemma exists_lt_mul_left {a b c : α} (hc : c < a * b) : ∃ a' < a, c < a' * b := by
  rcases le_or_lt b 0 with hb | hb
  · obtain ⟨a', ha'⟩ := exists_lt a
    exact ⟨a', ha', hc.trans_le (antitone_mul_right hb ha'.le)⟩
  · obtain ⟨a', hc', ha'⟩ := exists_between ((div_lt_iff₀ hb).2 hc)
    exact ⟨a', ha', (div_lt_iff₀ hb).1 hc'⟩

private lemma exists_lt_mul_right {a b c : α} (hc : c < a * b) : ∃ b' < b, c < a * b' := by
...
/-- See `mul_le_of_forall_lt` for multiplicative groups.-/
lemma mul_le_of_forall_lt' {a b c : α} (h : ∀ a' < a, ∀ b' < b, a' * b' ≤ c) : a * b ≤ c := by
