...
@[to_additive]
instance orderedCancelCommMonoid [∀ i, CommMonoid (α i)] [∀ i, OrderedCancelCommMonoid (α i)] :
...
@[to_additive]
instance orderedCommGroup [∀ i, CommGroup (α i)] [∀ i, OrderedCommGroup (α i)] :
    OrderedCommGroup (Lex (∀ i, α i)) where
...
noncomputable instance linearOrderedCancelCommMonoid [IsWellOrder ι (· < ·)]
    [∀ i, CommMonoid (α i)] [∀ i, LinearOrderedCancelCommMonoid (α i)] :
...
noncomputable instance linearOrderedCommGroup [IsWellOrder ι (· < ·)]
    [∀ i, CommGroup (α i)] [∀ i, LinearOrderedCommGroup (α i)] :
