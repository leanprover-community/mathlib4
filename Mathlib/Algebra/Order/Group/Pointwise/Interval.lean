...
theorem inv_Ioo_0_right {a : α} (ha : a < 0) : (Ioo a 0)⁻¹ = Iio a⁻¹ := by
  ext x
  refine ⟨fun h ↦ (lt_inv_of_neg (inv_lt_zero.1 h.2) ha).2 h.1, fun h ↦ ?_⟩
  have h' := (h.trans (inv_lt_zero.2 ha))
  exact ⟨(lt_inv_of_neg ha h').2 h, inv_lt_zero.2 h'⟩

theorem inv_Iio₀ {a : α} (ha : a < 0) : (Iio a)⁻¹ = Ioo a⁻¹ 0 := by
  rw [inv_eq_iff_eq_inv, inv_Ioo_0_right (inv_lt_zero.2 ha), inv_inv]
