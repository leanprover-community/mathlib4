...
lemma max_one_div_max_inv_one_eq_self (a : α) : max a 1 / max a⁻¹ 1 = a := by
...
lemma min_div_div_right' (a b c : α) : min (a / c) (b / c) = min a b / c := by
...
lemma max_div_div_right' (a b c : α) : max (a / c) (b / c) = max a b / c := by
...
lemma min_div_div_left' (a b c : α) : min (a / b) (a / c) = a / max b c := by
...
lemma max_div_div_left' (a b c : α) : max (a / b) (a / c) = a / min b c := by
...
lemma max_sub_max_le_max (a b c d : α) : max a b - max c d ≤ max (a - c) (b - d) := by
...
lemma abs_max_sub_max_le_max (a b c d : α) : |max a b - max c d| ≤ max |a - c| |b - d| := by
...
lemma abs_min_sub_min_le_max (a b c d : α) : |min a b - min c d| ≤ max |a - c| |b - d| := by
...
lemma abs_max_sub_max_le_abs (a b c : α) : |max a c - max b c| ≤ |a - b| := by
