...
@[to_additive] lemma oneLePart_eq_ite_lt [DecidableLT α] : a⁺ᵐ = if 1 < a then a else 1 := by
...
@[to_additive] lemma leOnePart_eq_ite_lt [DecidableLT α] : a⁻ᵐ = if a < 1 then a⁻¹ else 1 := by
