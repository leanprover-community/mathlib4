...
@[to_additive (attr := simp)]
lemma inv_le_self_iff : a⁻¹ ≤ a ↔ 1 ≤ a := by simp [inv_le_iff_one_le_mul']
#align neg_le_self_iff neg_le_self_iff

@[to_additive (attr := simp)]
lemma inv_lt_self_iff : a⁻¹ < a ↔ 1 < a := by simp [inv_lt_iff_one_lt_mul]
#align neg_lt_self_iff neg_lt_self_iff

@[to_additive (attr := simp)]
lemma le_inv_self_iff : a ≤ a⁻¹ ↔ a ≤ 1 := by simp [← not_iff_not]
#align le_neg_self_iff le_neg_self_iff

@[to_additive (attr := simp)]
lemma lt_inv_self_iff : a < a⁻¹ ↔ a < 1 := by simp [← not_iff_not]
