...
theorem exists_prime_mul_pow_lt_factorial [Ring K] [LinearOrder K] [IsStrictOrderedRing K]
    [FloorRing K] (n : ℕ) (a c : K) :
    ∃ p > n, p.Prime ∧ a * c ^ p < (p - 1)! :=
  ((Filter.frequently_atTop.mpr Nat.exists_infinite_primes).and_eventually
...
theorem exists_prime_mul_pow_div_factorial_lt_one [Field K] [LinearOrder K] [IsStrictOrderedRing K]
    [FloorRing K]
    (n : ℕ) (a c : K) :
