...
theorem exists_prime_mul_pow_lt_factorial [Ring K] [LinearOrderedRing K] [FloorRing K]
    (n : ℕ) (a c : K) : ∃ p > n, p.Prime ∧ a * c ^ p < (p - 1)! := by
...
theorem exists_prime_mul_pow_div_factorial_lt_one [Field K] [LinearOrderedField K] [FloorRing K]
    (n : ℕ) (a c : K) :
