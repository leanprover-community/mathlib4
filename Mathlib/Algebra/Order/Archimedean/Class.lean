...
@[to_additive (attr := simp)]
theorem lift_orderHom_mk {α : Type*} [PartialOrder α]
    (f : {a : M // a ≠ 1} → α) (h : ∀ (a b : {a : M // a ≠ 1}), mk a.prop ≤ mk b.prop → f a ≤ f b)
    {a : M} (ha : a ≠ 1) : lift_orderHom f h (mk ha) = f ⟨a, ha⟩ :=
  lift_mk f (fun a b heq ↦ le_antisymm (h a b heq.le) (h b a heq.ge)) ha

variable (M) in
/-- Adding top to non-top classes turns it back to full classes. -/
@[to_additive "Adding top to non-top classes turns it back to full classes."]
noncomputable
