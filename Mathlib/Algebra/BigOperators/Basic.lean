...
theorem map_prod [CommMonoid β] [CommMonoid γ] {G : Type*} [MonoidHomClass G β γ] (g : G)
    (f : α → β) (s : Finset α) : g (∏ x ∈ s, f x) = ∏ x ∈ s, g (f x) := by
...
protected theorem RingHom.map_sum [NonAssocSemiring β] [NonAssocSemiring γ] (g : β →+* γ)
    (f : α → β) (s : Finset α) : g (∑ x ∈ s, f x) = ∑ x ∈ s, g (f x) :=
  map_sum g f s
...
@[to_additive (attr := simp)]
theorem prod_singleton : ∏ x ∈ singleton a, f x = f a :=
...
theorem prod_bij {s : Finset α} {t : Finset γ} {f : α → β} {g : γ → β} (i : ∀ a ∈ s, γ)
    ...
    (i_surj : ∀ b ∈ t, ∃ a ha, b = i a ha) : ∏ x ∈ s, f x = ∏ x ∈ t, g x :=
...
theorem prod_bij' {s : Finset α} {t : Finset γ} {f : α → β} {g : γ → β} (i : ∀ a ∈ s, γ)
    ...
    (right_inv : ∀ a ha, i (j a ha) (hj a ha) = a) : ∏ x ∈ s, f x = ∏ x ∈ t, g x := by
...
theorem Equiv.prod_comp_finset {ι'} [DecidableEq ι] (e : ι ≃ ι') (f : ι' → β) {s' : Finset ι'}
    {s : Finset ι} (h : s = s'.image e.symm) : ∏ i' ∈ s', f i' = ∏ i ∈ s, f (e i) := by
  rw [h]
  refine'
...
theorem prod_fiberwise_of_maps_to [DecidableEq γ] {s : Finset α} {t : Finset γ} {g : α → γ}
    (h : ∀ x ∈ s, g x ∈ t) (f : α → β) :
    (∏ y ∈ t, ∏ x ∈ s.filter fun x => g x = y, f x) = ∏ x ∈ s, f x := by
...
@[to_additive]
theorem prod_attach {f : α → β} : ∏ x ∈ s.attach, f x = ∏ x ∈ s, f x :=
  haveI := Classical.decEq α
  calc
    ∏ x ∈ s.attach, f x.val = ∏ x ∈ s.attach.image Subtype.val, f x := by
  ...
  calc
    (∏ x ∈ s, h (if hx : p x then f x hx else g x hx)) =
        (∏ x ∈ s.filter p, h (if hx : p x then f x hx else g x hx)) *
          ∏ x ∈ s.filter fun x => ¬p x, h (if hx : p x then f x hx else g x hx) :=
      (prod_filter_mul_prod_filter_not s p _).symm
    _ = (∏ x ∈ (s.filter p).attach, h (if hx : p x.1 then f x.1 hx else g x.1 hx)) *
          ∏ x ∈ (s.filter fun x => ¬p x).attach, h (if hx : p x.1 then f x.1 hx else g x.1 hx) :=
...
theorem prod_dite_of_false {p : α → Prop} {hp : DecidablePred p} (h : ∀ x ∈ s, ¬p x)
    ...
    ∏ x ∈ s, (if hx : p x then f x hx else g x hx) = ∏ x : s, g x.val (h x.val x.property) :=
...
theorem prod_dite_of_true {p : α → Prop} {hp : DecidablePred p} (h : ∀ x ∈ s, p x)
    (f : ∀ x : α, p x → β) (g : ∀ x : α, ¬p x → β) :
    ∏ x ∈ s, (if hx : p x then f x hx else g x hx) = ∏ x : s, f x.val (h x.val x.property) :=
  prod_bij (fun x hx => ⟨x, hx⟩) (fun x hx => by simp)
    (fun a ha => by
      dsimp
      rw [dif_pos])
...
theorem eventually_constant_prod {u : ℕ → β} {N : ℕ} (hu : ∀ n ≥ N, u n = 1) {n : ℕ} (hn : N ≤ n) :
    (∏ k ∈ range (n + 1), u k) = ∏ k ∈ range (N + 1), u k := by
...
theorem prod_mem_multiset [DecidableEq α] (m : Multiset α) (f : { x // x ∈ m } → β) (g : α → β)
    (hfg : ∀ x, f x = g x) : ∏ x : { x // x ∈ m }, f x = ∏ x ∈ m.toFinset, g x :=
...
theorem eq_prod_range_div {M : Type*} [CommGroup M] (f : ℕ → M) (n : ℕ) :
    f n = f 0 * ∏ i ∈ range n, f (i + 1) / f i := by rw [prod_range_div, mul_div_cancel'_right]
...
theorem prod_comp [DecidableEq γ] (f : γ → β) (g : α → γ) :
    (∏ a ∈ s, f (g a)) = ∏ b ∈ s.image g, f b ^ (s.filter fun a => g a = b).card :=
  calc
    (∏ a ∈ s, f (g a)) =
        ∏ x ∈ (s.image g).sigma fun b : γ => s.filter fun a => g a = b, f (g x.2) :=
      prod_bij (fun a _ha => ⟨g a, a⟩) (by simp; tauto) (fun _ _ => rfl) (by simp)
        (by -- `(by finish)` closes this
          ...
          tauto)
    _ = ∏ b ∈ s.image g, ∏ a ∈ s.filter fun a => g a = b, f (g a) := prod_sigma _ _ _
    _ = ∏ b ∈ s.image g, ∏ _a ∈ s.filter fun a => g a = b, f b :=
      prod_congr rfl fun b _hb => prod_congr rfl (by simp (config := { contextual := true }))
    _ = ∏ b ∈ s.image g, f b ^ (s.filter fun a => g a = b).card :=
...
theorem prod_ite_one {f : α → Prop} [DecidablePred f] (hf : (s : Set α).PairwiseDisjoint f)
    (a : β) : (∏ i ∈ s, ite (f i) a 1) = ite (∃ i ∈ s, f i) a 1 := by
  split_ifs with h
  · obtain ⟨i, hi, hfi⟩ := h
...
theorem sum_boole {s : Finset α} {p : α → Prop} [NonAssocSemiring β] {hp : DecidablePred p} :
    (∑ x ∈ s, if p x then (1 : β) else (0 : β)) = (s.filter p).card := by
