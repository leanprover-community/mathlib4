...
lemma prod_univ_get [CommMonoid α] (l : List α) : ∏ i, l.get i = l.prod := by
  simp [Finset.prod_eq_multiset_prod]

@[to_additive (attr := simp)]
lemma prod_univ_get' [CommMonoid β] (l : List α) (f : α → β) :
...
@[to_additive]
lemma prod_univ_two' [CommMonoid β] (f : α → β) (a b : α) :
    ∏ i, f (![a, b] i) = f a * f b :=
  prod_univ_two _
...
@[to_additive]
lemma prod_ofFn {n : ℕ} {f : Fin n → α} : (ofFn f).prod = ∏ i, f i :=
  Fin.prod_ofFn f
