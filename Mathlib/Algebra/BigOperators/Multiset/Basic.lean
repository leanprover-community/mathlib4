...
lemma prod_coe (l : List α) : prod ↑l = l.prod :=
  prod_eq_foldl _
...
@[to_additive]
lemma prod_eq_pow_single [DecidableEq α] (a : α) (h : ∀ a' ≠ a, a' ∈ s → a' = 1) :
...
@[to_additive]
lemma prod_hom [CommMonoid β] (s : Multiset α) {F : Type*} [FunLike F α β]
...
@[to_additive]
lemma prod_hom' [CommMonoid β] (s : Multiset ι) {F : Type*} [FunLike F α β]
    [MonoidHomClass F α β] (f : F)
...
@[to_additive]
lemma map_multiset_prod [CommMonoid α] [CommMonoid β] {F : Type*} [FunLike F α β]
    [MonoidHomClass F α β] (f : F)
