...
@[to_additive]
lemma prod_mapRange_index {f : M → M'} {hf : f 0 = 0} {g : α →₀ M} {h : α → M' → N}
...
@[to_additive (attr := simp)]
lemma prod_zero_index {h : α → M → N} : (0 : α →₀ M).prod h = 1 :=
  rfl
...
@[to_additive (attr := simp)]
lemma prod_ite_eq [DecidableEq α] (f : α →₀ M) (a : α) (b : α → M → N) :
...
-/
-- @[simp]
lemma sum_ite_self_eq [DecidableEq α] {N : Type*} [AddCommMonoid N] (f : α →₀ N) (a : α) :
...
@[simp]
lemma sum_ite_self_eq_aux [DecidableEq α] {N : Type*} [AddCommMonoid N] (f : α →₀ N) (a : α) :
...
@[simp]
lemma prod_pow [Fintype α] (f : α →₀ ℕ) (g : α → N) :
...
lemma onFinset_prod {s : Finset α} {f : α → M} {g : α → M → N} (hf : ∀ a, f a ≠ 0 → a ∈ s)
...
@[to_additive]
lemma _root_.SubmonoidClass.finsupp_prod_mem {S : Type*} [SetLike S N] [SubmonoidClass S N]
...
@[to_additive]
lemma prod_congr {f : α →₀ M} {g1 g2 : α → M → N} (h : ∀ x ∈ f.support, g1 x (f x) = g2 x (f x)) :
    f.prod g1 = f.prod g2 :=
...
@[to_additive]
lemma map_finsupp_prod [Zero M] [CommMonoid N] [CommMonoid P] {H : Type*} [MonoidHomClass H N P]
...
protected lemma MulEquiv.map_finsupp_prod [Zero M] [CommMonoid N] [CommMonoid P] (h : N ≃* P)
...
@[to_additive (attr := deprecated)
  "Deprecated, use `_root_.map_finsupp_sum` instead."]
protected lemma MonoidHom.map_finsupp_prod [Zero M] [CommMonoid N] [CommMonoid P] (h : N →* P)
    (f : α →₀ M) (g : α → M → N) : h (f.prod g) = f.prod fun a b => h (g a b) :=
  map_finsupp_prod h f g
...
protected lemma RingHom.map_finsupp_sum [Zero M] [Semiring R] [Semiring S] (h : R →+* S)
...
protected lemma RingHom.map_finsupp_prod [Zero M] [CommSemiring R] [CommSemiring S] (h : R →+* S)
...
@[to_additive]
lemma MonoidHom.coe_finsupp_prod [Zero β] [Monoid N] [CommMonoid P] (f : α →₀ β)
...
@[to_additive (attr := simp)]
lemma MonoidHom.finsupp_prod_apply [Zero β] [Monoid N] [CommMonoid P] (f : α →₀ β)
...
lemma single_multiset_sum [AddCommMonoid M] (s : Multiset M) (a : α) :
...
lemma single_finset_sum [AddCommMonoid M] (s : Finset ι) (f : ι → M) (a : α) :
...
lemma single_sum [Zero M] [AddCommMonoid N] (s : ι →₀ M) (f : ι → M → N) (a : α) :
    single a (s.sum f) = s.sum fun d c => single a (f d c) :=
  single_finset_sum _ _ _
#align finsupp.single_sum Finsupp.single_sum

@[to_additive]
lemma prod_neg_index [AddGroup G] [CommMonoid M] {g : α →₀ G} {h : α → G → M}
...
lemma support_finset_sum [DecidableEq β] [AddCommMonoid M] {s : Finset α} {f : α → β →₀ M} :
...
@[simp]
lemma sum_zero [Zero M] [AddCommMonoid N] {f : α →₀ M} : (f.sum fun _ _ => (0 : N)) = 0 :=
  Finset.sum_const_zero
#align finsupp.sum_zero Finsupp.sum_zero

@[to_additive (attr := simp)]
lemma prod_mul [Zero M] [CommMonoid N] {f : α →₀ M} {h₁ h₂ : α → M → N} :
...
@[to_additive (attr := simp)]
lemma prod_inv [Zero M] [CommGroup G] {f : α →₀ M} {h : α → M → G} :
...
@[simp]
lemma sum_sub [Zero M] [AddCommGroup G] {f : α →₀ M} {h₁ h₂ : α → M → G} :
...
lemma prod_add_index [DecidableEq α] [AddZeroClass M] [CommMonoid N] {f g : α →₀ M}
...
@[to_additive
      ...
      hypotheses."]
lemma prod_add_index' [AddZeroClass M] [CommMonoid N] {f g : α →₀ M} {h : α → M → N}
...
@[simp]
lemma sum_hom_add_index [AddZeroClass M] [AddCommMonoid N] {f g : α →₀ M} (h : α → M →+ N) :
...
@[simp]
lemma prod_hom_add_index [AddZeroClass M] [CommMonoid N] {f g : α →₀ M}
...
@[simp]
lemma liftAddHom_singleAddHom [AddCommMonoid M] :
...
@[simp]
lemma sum_single [AddCommMonoid M] (f : α →₀ M) : f.sum single = f :=
  FunLike.congr_fun liftAddHom_singleAddHom f
#align finsupp.sum_single Finsupp.sum_single

@[simp]
lemma sum_univ_single [AddCommMonoid M] [Fintype α] (i : α) (m : M) :
...
-- Porting note: simp can prove this
-- @[simp]
lemma liftAddHom_apply_single [AddCommMonoid M] [AddCommMonoid N] (f : α → M →+ N) (a : α)
...
@[simp]
lemma liftAddHom_comp_single [AddCommMonoid M] [AddCommMonoid N] (f : α → M →+ N) (a : α) :
...
lemma comp_liftAddHom [AddCommMonoid M] [AddCommMonoid N] [AddCommMonoid P] (g : N →+ P)
    (f : α → M →+ N) :
...
lemma sum_sub_index [AddCommGroup β] [AddCommGroup γ] {f g : α →₀ β} {h : α → β → γ}
...
@[to_additive]
lemma prod_embDomain [Zero M] [CommMonoid N] {v : α →₀ M} {f : α ↪ β} {g : β → M → N} :
...
@[to_additive]
lemma prod_finset_sum_index [AddCommMonoid M] [CommMonoid N] {s : Finset ι} {g : ι → α →₀ M}
...
@[to_additive]
lemma prod_sum_index [AddCommMonoid M] [AddCommMonoid N] [CommMonoid P] {f : α →₀ M}
...
lemma multiset_sum_sum_index [AddCommMonoid M] [AddCommMonoid N] (f : Multiset (α →₀ M))
    (h : α → M → N) (h₀ : ∀ a, h a 0 = 0)
...
lemma support_sum_eq_biUnion {α : Type*} {ι : Type*} {M : Type*} [DecidableEq α]
...
lemma multiset_map_sum [Zero M] {f : α →₀ M} {m : β → γ} {h : α → M → Multiset β} :
...
lemma multiset_sum_sum [Zero M] [AddCommMonoid N] {f : α →₀ M} {h : α → M → Multiset N} :
...
lemma prod_add_index_of_disjoint [AddCommMonoid M] {f1 f2 : α →₀ M}
...
lemma prod_dvd_prod_of_subset_of_dvd [AddCommMonoid M] [CommMonoid N] {f1 f2 : α →₀ M}
...
lemma Finset.sum_apply' : (∑ k in s, f k) i = ∑ k in s, f k i :=
  (Finsupp.applyAddHom i : (ι →₀ A) →+ A).map_sum f s
#align finset.sum_apply' Finset.sum_apply'

lemma Finsupp.sum_apply' : g.sum k x = g.sum fun i b => k i b x :=
...
lemma Finsupp.sum_sum_index' : (∑ x in s, f x).sum t = ∑ x in s, (f x).sum t :=
...
lemma prod_pow_pos_of_zero_not_mem_support {f : ℕ →₀ ℕ} (hf : 0 ∉ f.support) :
    0 < f.prod (· ^ ·) :=
