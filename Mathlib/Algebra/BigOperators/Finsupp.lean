...
@[to_additive]
lemma prod_eq_single {f : α →₀ M} (a : α) {g : α → M → N}
...
@[to_additive]
lemma map_finsupp_prod [Zero M] [CommMonoid N] [CommMonoid P] {H : Type*}
...
@[simp]
lemma univ_sum_single_apply [AddCommMonoid M] [Fintype α] (i : α) (m : M) :
...
lemma equivFunOnFinite_symm_eq_sum [Fintype α] [AddCommMonoid M] (f : α → M) :
