...
lemma centerToCentroid_toMulHom_toFun_apply (z : NonUnitalSubsemiring.center α) (a : α) :
...
def centerIsoCentroid : Subsemiring.center α ≃+* CentroidHom α :=
  { centerToCentroid with
    ...
    left_inv := fun z ↦ Subtype.ext <| by simp [centerToCentroid_toMulHom_toFun_apply, ← map_mul_right]
    right_inv := fun T ↦ CentroidHom.ext <| by
      simp [centerToCentroid_toMulHom_toFun_apply, ← map_mul_right] }
