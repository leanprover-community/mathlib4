...
instance (priority := 10000) : Zero { p : M₀ // IsIdempotentElem p } where zero := ⟨0, zero⟩
...
instance (priority := 10000) : One { p : M₁ // IsIdempotentElem p } where one := ⟨1, one⟩
...
instance (priority := 10000) : HasCompl { p : R // IsIdempotentElem p } :=
