...
lemma of_isIdempotent [IsIdempotent M (· * ·)] (a : M) : IsIdempotentElem a :=
  IsIdempotent.idempotent a
#align is_idempotent_elem.of_is_idempotent IsIdempotentElem.of_isIdempotent

lemma eq {p : M} (h : IsIdempotentElem p) : p * p = p :=
  h
#align is_idempotent_elem.eq IsIdempotentElem.eq

lemma mul_of_commute {p q : S} (h : Commute p q) (h₁ : IsIdempotentElem p)
...
lemma zero : IsIdempotentElem (0 : M₀) :=
  mul_zero _
#align is_idempotent_elem.zero IsIdempotentElem.zero

lemma one : IsIdempotentElem (1 : M₁) :=
  mul_one _
#align is_idempotent_elem.one IsIdempotentElem.one

lemma one_sub {p : R} (h : IsIdempotentElem p) : IsIdempotentElem (1 - p) := by
...
@[simp]
lemma one_sub_iff {p : R} : IsIdempotentElem (1 - p) ↔ IsIdempotentElem p :=
...
lemma pow {p : N} (n : ℕ) (h : IsIdempotentElem p) : IsIdempotentElem (p ^ n) :=
...
lemma pow_succ_eq {p : N} (n : ℕ) (h : IsIdempotentElem p) : p ^ (n + 1) = p :=
...
@[simp]
lemma iff_eq_one {p : G} : IsIdempotentElem p ↔ p = 1 :=
...
@[simp]
lemma iff_eq_zero_or_one {p : G₀} : IsIdempotentElem p ↔ p = 0 ∨ p = 1 := by
  refine'
...
@[simp]
lemma coe_zero : ↑(0 : { p : M₀ // IsIdempotentElem p }) = (0 : M₀) :=
  rfl
...
@[simp]
lemma coe_one : ↑(1 : { p : M₁ // IsIdempotentElem p }) = (1 : M₁) :=
  rfl
...
@[simp]
lemma zero_compl : (0 : { p : R // IsIdempotentElem p })ᶜ = 1 :=
  Subtype.ext <| sub_zero _
#align is_idempotent_elem.zero_compl IsIdempotentElem.zero_compl

@[simp]
lemma one_compl : (1 : { p : R // IsIdempotentElem p })ᶜ = 0 :=
