...
@[simp]
theorem ofClass_addMonoidHom_mk (f : α → β) (h₁ h₂ h₃) :
...
theorem ofClass_addMonoidHom_injective : Injective fun f : α →ₙ+* β => (f : α →+ β) :=
...
@[simp]
theorem ofClass_addMonoidHom_id : (NonUnitalRingHom.id α : α →+ α) = AddMonoidHom.id α :=
  rfl
...
@[simp]
theorem ofClass_addMonoidHom_comp (g : β →ₙ+* γ) (f : α →ₙ+* β) :
...
@[simp]
theorem ofClass_monoidHom_mk (f : α →* β) (h₁ h₂) : ((⟨f, h₁, h₂⟩ : α →+* β) : α →* β) = f :=
  rfl
...
@[simp]
theorem ofClass_addMonoidHom_mk (f : α → β) (h₁ h₂ h₃ h₄) :
...
theorem ofClass_addMonoidHom_injective : Injective (fun f : α →+* β => (f : α →+ β)) := fun _ _ h =>
  ext <| DFunLike.congr_fun (F := α →+ β) h

theorem ofClass_monoidHom_injective : Injective (fun f : α →+* β => (f : α →* β)) :=
  Injective.of_comp (f := DFunLike.coe) DFunLike.coe_injective
...
@[simp]
theorem ofClass_addMonoidHom_id : (id α : α →+ α) = AddMonoidHom.id α :=
  rfl

@[simp]
theorem ofClass_monoidHom_id : (id α : α →* α) = MonoidHom.id α :=
  rfl
...
@[simp]
theorem ofClass_addMonoidHom_mkRingHomOfMulSelfOfTwoNeZero (h h_two h_one) :
