...
/-- Predicate-based variant of `AddSubmonoid.closure_isSquare`. -/
theorem AddSubmonoid.isSumSq_eq_mem_closure [AddCommMonoid R] [Mul R] :
    (· ∈ closure {r : R | IsSquare r}) = IsSumSq := by simp [closure_isSquare]

theorem foo [AddCommMonoid R] [Mul R] {ι : Type*} {I : Finset ι}
    {x : ι → R} {i : ι} {hi : i ∈ I} : x i * x i ∈ AddSubmonoid.closure {x | IsSquare x} :=
  AddSubmonoid.subset_closure <| (by simp : (∀ i ∈ I, x i * x i ∈ {x : R | IsSquare x})) _ hi
...
theorem IsSumSq.sum_mul_self [AddCommMonoid R] [Mul R]
  ...
  rw [AddSubmonoid.isSumSq_eq_mem_closure]
  exact sum_mem (foo · ·)
...
theorem isSumSq_iff_exists_sum [AddCommMonoid R] [Mul R] (a : R) :
  ...
  refine ⟨?_, by rintro ⟨ι, I, y, rfl⟩; exact IsSumSq.sum_mul_self⟩
  exact fun hyp => by
    rw [isSumSq_eq_mem_closure] at hyp
    obtain ⟨ι, I, y, y_cl, rfl⟩ := AddSubmonoid.exists_finset_sum_of_mem_closure hyp
...
theorem Subsemiring.closure_isSquare [CommSemiring R] :
  ...
  simpa [hy] using sum_mem
    (subset_closure <| (by simp : (∀ i ∈ I, y i * y i ∈ {x | IsSquare x})) · ·)
