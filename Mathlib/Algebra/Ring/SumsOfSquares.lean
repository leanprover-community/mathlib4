...
@[mk_iff]
inductive isSumSq {R : Type*} [Add R] [Zero R] [Mul R] : R → Prop
  | zero                              : isSumSq 0
...
theorem isSumSq.add {R : Type*} [AddMonoid R] [Mul R] {S1 S2 : R} (p1 : isSumSq S1)
...
def SumSqIn (R : Type*) [AddMonoid R] [Mul R] : AddSubmonoid R where
