...
/-- In an additive unital magma with multiplication, squares in `R` are sums of squares.
...
-/
def AddSubmonoid.sumSqIn [AddMonoid R] [Mul R] : AddSubmonoid R where
...
-/
def Subsemiring.sumSqIn [CommSemiring R] : Subsemiring R where
  __ := AddSubmonoid.sumSqIn R
  mul_mem' := IsSumSq.mul
