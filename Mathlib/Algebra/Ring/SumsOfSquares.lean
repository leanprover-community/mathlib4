...
/--
The set of squares in a type `R` endowed with a `Mul R` instance. By definition, a square in `R` is a term `x : R` such that `x = y * y` for some `y : R` (see Mathlib.Algebra.Group.Even).
-/
