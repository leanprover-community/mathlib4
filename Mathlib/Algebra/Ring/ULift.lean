...
instance (priority := 10000) mulZeroClass [MulZeroClass α] : MulZeroClass (ULift α) :=
...
instance (priority := 10000) distrib [Distrib α] : Distrib (ULift α) :=
  { add := (· + ·), mul := (· * ·),
...
instance (priority := 10000) nonUnitalNonAssocSemiring [NonUnitalNonAssocSemiring α] :
...
instance (priority := 10000) nonAssocSemiring [NonAssocSemiring α] : NonAssocSemiring (ULift α) :=
...
instance (priority := 10000) nonUnitalSemiring [NonUnitalSemiring α] : NonUnitalSemiring (ULift α) :=
  { zero := (0 : ULift α), add := (· + ·), mul := (· * ·), nsmul := AddMonoid.nsmul,
...
instance (priority := 10000) semiring [Semiring α] : Semiring (ULift α) :=
  { ULift.addMonoidWithOne with
...
instance (priority := 10000) nonUnitalCommSemiring [NonUnitalCommSemiring α] : NonUnitalCommSemiring (ULift α) :=
...
instance (priority := 10000) commSemiring [CommSemiring α] : CommSemiring (ULift α) :=
  { ULift.semiring with
      nsmul := AddMonoid.nsmul, natCast := fun n => ULift.up n, npow := Monoid.npow, mul_comm }
#align ulift.comm_semiring ULift.commSemiring

instance (priority := 10000) nonUnitalNonAssocRing [NonUnitalNonAssocRing α] : NonUnitalNonAssocRing (ULift α) :=
...
instance (priority := 10000) nonUnitalRing [NonUnitalRing α] : NonUnitalRing (ULift α) :=
  { zero := (0 : ULift α), add := (· + ·), mul := (· * ·), sub := Sub.sub, neg := Neg.neg,
...
instance (priority := 10000) nonAssocRing [NonAssocRing α] : NonAssocRing (ULift α) :=
...
instance (priority := 10000) ring [Ring α] : Ring (ULift α) :=
  { zero := (0 : ULift α), one := (1 : ULift α), add := (· + ·), mul := (· * ·), sub := Sub.sub,
...
instance (priority := 10000) nonUnitalCommRing [NonUnitalCommRing α] : NonUnitalCommRing (ULift α) :=
  { zero := (0 : ULift α), add := (· + ·), mul := (· * ·), sub := Sub.sub, neg := Neg.neg,
...
instance (priority := 10000) commRing [CommRing α] : CommRing (ULift α) :=
