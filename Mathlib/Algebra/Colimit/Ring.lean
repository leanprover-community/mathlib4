...
theorem exists_of [Nonempty ι] [IsDirectedLE ι] (z : DirectLimit G f) :
    ∃ i x, of G f i x = z := by
...
nonrec theorem Polynomial.exists_of [Nonempty ι] [IsDirectedLE ι]
...
@[elab_as_elim]
theorem induction_on [Nonempty ι] [IsDirectedLE ι] {C : DirectLimit G f → Prop}
...
lemma lift_injective [Nonempty ι] [IsDirectedLE ι]
...
variable (f' : ∀ i j, i ≤ j → G i →+* G j)
variable [DirectedSystem G fun i j h ↦ f' i j h] [IsDirectedLE ι]
variable (G f)
...
theorem of_injective [IsDirectedLE ι] [DirectedSystem G fun i j h ↦ f' i j h]
...
variable [Nonempty ι] [IsDirectedLE ι] [∀ i, Field (G i)]
variable (f : ∀ i j, i ≤ j → G i → G j)
variable (f' : ∀ i j, i ≤ j → G i →+* G j)
