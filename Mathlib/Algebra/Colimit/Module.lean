...
theorem exists_of₂ [Nonempty ι] [IsDirectedLE ι] (z w : DirectLimit G f) :
...
@[elab_as_elim]
protected theorem induction_on [Nonempty ι] [IsDirectedLE ι] {C : DirectLimit G f → Prop}
...
lemma lift_injective [IsDirectedLE ι]
...
variable [Nonempty ι] [IsDirectedLE ι] [DirectedSystem G (f · · ·)]
...
variable {G f} [DirectedSystem G (f · · ·)] [IsDirectedLE ι]
...
/-- A component that corresponds to zero in the direct limit is already zero in some
bigger module in the directed system. -/
theorem of.zero_exact [IsDirectedLE ι] [DirectedSystem G fun i j h ↦ f i j h] (i x)
