...
protected lemma SemiconjBy.map [MulHomClass F M N] (h : SemiconjBy a x y) (f : F) :
...
@[to_additive (attr := simp)]
protected lemma Commute.map [MulHomClass F M N] (h : Commute x y) (f : F) : Commute (f x) (f y) :=
  SemiconjBy.map h f
#align commute.map Commute.map
#align add_commute.map AddCommute.map

@[to_additive (attr := simp)]
protected lemma SemiconjBy.of_map [MulHomClass F M N] (f : F) (hf : Function.Injective f)
    (h : SemiconjBy (f a) (f x) (f y)) : SemiconjBy a x y :=
  hf (by simpa only [SemiconjBy, map_mul] using h)

@[to_additive (attr := simp)]
lemma Commute.of_map [MulHomClass F M N] {f : F} (hf : Function.Injective f)
