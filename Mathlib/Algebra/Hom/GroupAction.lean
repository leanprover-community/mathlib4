...
* `A →ₑ+[φ] B` is `DistribSMulHom φ A B`.
...
/-
/-- Equivariant functions -/
abbrev SMulHom (M : Type _) (X Y : Type _) [SMul M X] [SMul M Y] := SMulHom (@id M) X Y
-/
...
export SMulSemiHomClass (map_smulₛₗ)


/-- `SMulHomClass F M X Y` states that `F` is a type of
morphisms which are equivariant with respect to actions of `M`
This is an abbreviation of `SMulSemiHomClass`. -/
abbrev SMulHomClass (F : Type _) (M : outParam (Type _))
(X Y : outParam (Type _)) [SMul M X] [SMul M Y] :=
  SMulSemiHomClass F (@id M) X Y
...
instance : SMulSemiHomClass (X →ₑ[φ] Y) φ X Y
  ...
  map_smulₛₗ := SMulHom.map_smul'

initialize_simps_projections SMulHom (toFun → apply)
...
/-- Turn an element of a type `F` satisfying `SMulSemiHomClass F φ X Y`
  into an actual `SMulHom`.
  This is declared as the default coercion from `F` to `SMulSemiHom φ X Y`. -/
@[coe]
def _root_.SMulSemiHomClass.toSMulHom [SMulSemiHomClass F φ X Y] (f : F) :
  X →ₑ[φ] Y where
  toFun := FunLike.coe f
  map_smul' := map_smulₛₗ f

/-- Any type satisfying `SMulSemiHomClass` can be cast into `SMulHom` via
  `SMulHomSemiClass.toSMulHom`. -/
instance [SMulSemiHomClass F φ X Y] : CoeTC F (X →ₑ[φ] Y) :=
  ⟨SMulSemiHomClass.toSMulHom⟩

protected theorem map_smulₑ [SMulSemiHomClass F φ X Y] (f : F) (m : M) (x : X) :
  f (m • x) = (φ m) • f x :=
  map_smulₛₗ f m x
#align mul_action_hom.map_smul SMulHom.map_smulₑ

@[ext]
theorem ext [SMulSemiHomClass F φ X Y] {f g : F} :
  (∀ x, f x = g x) → f = g :=
  FunLike.ext f g
#align mul_action_hom.ext SMulHom.ext

theorem ext_iff [SMulSemiHomClass F φ X Y] {f g : F} : f = g ↔ ∀ x, f x = g x :=
  FunLike.ext_iff
#align mul_action_hom.ext_iff SMulHom.ext_iff

protected theorem congr_fun [SMulSemiHomClass F φ X Y] {f g : F} (h : f = g) (x : X) :
  f x = g x :=
...
@[simp]
theorem comp_id (f : X →ₑ[φ] Y) : f.comp (SMulHom.id M) = f := by simp
...
theorem comp_inv {f : X →ₑ[φ] Y } {g : Y → X}
  {k₁ : Function.LeftInverse φ' φ} {k₂ : Function.RightInverse φ' φ}
  {h₁ : Function.LeftInverse g f} {h₂ : Function.RightInverse g f} :
  (inverse f g k₂ h₁ h₂).comp' f (CompTriple.comp_inv k₁) = SMulHom.id M := by
  rw [ext_iff]
  intro x
...
theorem inv_comp {f : X →ₑ[φ] Y } {g : Y → X}
  ...
  {h₁ : Function.LeftInverse g f} {h₂ : Function.RightInverse g f} :
  f.comp' (inverse f g k₂ h₁ h₂) (CompTriple.comp_inv k₂) = SMulHom.id N := by
  rw [ext_iff]
  intro x
  simp only [comp'_apply, inverse_apply, id_apply]
  exact h₂ x
...
@[simps]
def _root_.SMulCommClass.toSMulHom {M} (N α : Type _)
...
structure DistribSMulSemiHom extends A →ₑ[φ] B, A →+ B
#align distrib_mul_action_hom DistribSMulSemiHom

/-- Equivariant additive monoid homomorphisms. -/
abbrev DistribSMulHom (M : Type _) [Monoid M]
  (A : Type _) [AddMonoid A] [DistribMulAction M A]
  (B : Type _) [AddMonoid B] [DistribMulAction M B] :=
  DistribSMulSemiHom (@id M) A B


/-- Reinterpret an equivariant additive monoid homomorphism as an additive monoid homomorphism. -/
add_decl_doc DistribSMulSemiHom.toAddMonoidHom
#align distrib_mul_action_hom.to_add_monoid_hom DistribSMulSemiHom.toAddMonoidHom

/-- Reinterpret an equivariant additive monoid homomorphism as an equivariant function. -/
add_decl_doc DistribSMulSemiHom.toSMulHom
#align distrib_mul_action_hom.to_mul_action_hom DistribSMulSemiHom.toSMulHom

/- Porting note: local notation given a name, conflict with Algebra.Hom.Freiman
 see https://github.com/leanprover/lean4/issues/2000 -/
@[inherit_doc]
notation:25 (name := «DistribSMulSemiHomLocal≺»)
  A " →ₑ+[" φ:25 "] " B:0 => DistribSMulSemiHom φ A B

@[inherit_doc]
notation:25 (name := «DistribSMulHomLocal≺»)
  A " →+[" M:25 "] " B:0 => DistribSMulHom M A B

-- QUESTION/TODO : Impose that `φ` is a morphism of monoids?

/-- `DistribSMulSemiHomClass F φ A B` states that `F` is a type of morphisms
  preserving the additive monoid structure and equivariant with respect to `φ`.
    You should extend this class when you extend `DistribSMulHom`. -/
class DistribSMulSemiHomClass (F : Type _)
  ...
  extends SMulSemiHomClass F φ A B,
  AddMonoidHomClass F A B
#align distrib_mul_action_hom_class DistribSMulSemiHomClass

/-- `DistribSMulHomClass F M A B` states that `F` is a type of morphisms preserving
  the additive monoid structure and equivariant with respect to the action of `M`.
    It is an abbreviation to `DistribSMulHomClass F (@id M) A B` -/
abbrev DistribSMulHomClass (F : Type _)
  (M : outParam (Type _)) (A B : outParam (Type _))
  [Monoid M] [Monoid N] [AddMonoid A] [AddMonoid B] [DistribMulAction M A] [DistribMulAction M B] :=
  DistribSMulSemiHomClass F (@id M) A B

/- porting note: Removed a @[nolint dangerousInstance] for
DistribSMulHomClass.toAddMonoidHomClass not dangerous due to `outParam`s -/
...
--   ⟨toAddMonoidHom⟩
-- #align distrib_mul_action_hom.has_coe DistribSMulHom.coe
...
instance : DistribSMulSemiHomClass (A →ₑ+[φ] B) φ A B
    where
  coe m := m.toFun
  ...
  map_zero := DistribSMulSemiHom.map_zero'
  map_add := DistribSMulSemiHom.map_add'
...
`SMulHom`. This is declared as the default coercion from `F` to `SMulHom M X Y`. -/
@[coe]
def _root_.DistribSMulSemiHomClass.toDistribSMulSemiHom
    [DistribSMulSemiHomClass F φ A B] (f : F) : A →ₑ+[φ] B :=
  { (f : A →+ B),  (f : A →ₑ[φ] B) with }

/-- Any type satisfying `SMulHomClass` can be cast into `SMulHom` via
  `SMulHomClass.toSMulHom`. -/
instance [DistribSMulSemiHomClass F φ A B] :
  CoeTC F (A →ₑ+[φ] B) :=
  ⟨DistribSMulSemiHomClass.toDistribSMulSemiHom⟩

/-- If `DistribMulAction` of `M` and `N` on `A` commute,
  then for each `c : M`, `(c • ·)` is an `N`-action additive homomorphism. -/
@[simps]
def _root_.SMulCommClass.toDistribSMulSemiHom {M} (N A : Type _) [Monoid N] [AddMonoid A]
...
#align distrib_mul_action_hom.to_fun_eq_coe DistribSMulSemiHom.toFun_eq_coe

@[norm_cast]
theorem coe_fn_coe (f : A →ₑ+[φ] B) : ⇑(f : A →+ B) = f :=
  rfl
#align distrib_mul_action_hom.coe_fn_coe DistribSMulSemiHom.coe_fn_coe
...
theorem ext {f g : A →ₑ+[φ] B} : (∀ x, f x = g x) → f = g :=
  FunLike.ext f g
#align distrib_mul_action_hom.ext DistribSMulSemiHom.ext

theorem ext_iff {f g : A →ₑ+[φ] B} : f = g ↔ ∀ x, f x = g x :=
  FunLike.ext_iff
#align distrib_mul_action_hom.ext_iff DistribSMulSemiHom.ext_iff

protected theorem congr_fun {f g : A →ₑ+[φ] B} (h : f = g) (x : A) : f x = g x :=
  FunLike.congr_fun h _
#align distrib_mul_action_hom.congr_fun DistribSMulSemiHom.congr_fun

theorem toSMulHom_injective {f g : A →ₑ+[φ] B} (h : (f : A →ₑ[φ] B) = (g : A →ₑ[φ] B)) :
    f = g := by
  ext a
  exact SMulHom.congr_fun h a
#align distrib_mul_action_hom.to_mul_action_hom_injective DistribSMulSemiHom.toSMulHom_injective

theorem toAddMonoidHom_injective {f g : A →ₑ+[φ] B} (h : (f : A →+ B) = (g : A →+ B)) : f = g := by
  ext a
  exact FunLike.congr_fun h a
#align distrib_mul_action_hom.to_add_monoid_hom_injective DistribSMulSemiHom.toAddMonoidHom_injective

protected theorem map_zero (f : A →ₑ+[φ] B) : f 0 = 0 :=
  map_zero f
#align distrib_mul_action_hom.map_zero DistribSMulSemiHom.map_zero

protected theorem map_add (f : A →ₑ+[φ] B) (x y : A) : f (x + y) = f x + f y :=
  map_add f x y
#align distrib_mul_action_hom.map_add DistribSMulSemiHom.map_add

protected theorem map_neg (f : A' →ₑ+[φ] B') (x : A') : f (-x) = -f x :=
  map_neg f x
#align distrib_mul_action_hom.map_neg DistribSMulSemiHom.map_neg

protected theorem map_sub (f : A' →ₑ+[φ] B') (x y : A') : f (x - y) = f x - f y :=
  map_sub f x y
#align distrib_mul_action_hom.map_sub DistribSMulSemiHom.map_sub

protected theorem map_smulₑ (f : A →ₑ+[φ] B) (m : M) (x : A) : f (m • x) = (φ m) • f x :=
  map_smulₛₗ f m x
#align distrib_mul_action_hom.map_smul DistribSMulSemiHom.map_smulₑ

variable (M)

/-- The identity map as an equivariant additive monoid homomorphism. -/
protected def id : A →+[M] A :=
  ⟨SMulHom.id _, rfl, fun _ _ => rfl⟩
#align distrib_mul_action_hom.id DistribSMulSemiHom.id

@[simp]
theorem id_apply (x : A) : DistribSMulSemiHom.id M x = x := by
  rfl
#align distrib_mul_action_hom.id_apply DistribSMulSemiHom.id_apply
...
instance : One (A →+[M] A) :=
  ⟨DistribSMulSemiHom.id M⟩

@[simp]
theorem coe_zero : ⇑(0 : A →ₑ+[φ] B) = 0 :=
  rfl
#align distrib_mul_action_hom.coe_zero DistribSMulSemiHom.coe_zero

@[simp]
theorem coe_one : ⇑(1 : A →+[M] A) = id :=
  rfl
#align distrib_mul_action_hom.coe_one DistribSMulSemiHom.coe_one

theorem zero_apply (a : A) : (0 : A →ₑ+[φ] B) a = 0 :=
  rfl
#align distrib_mul_action_hom.zero_apply DistribSMulSemiHom.zero_apply

theorem one_apply (a : A) : (1 : A →+[M] A) a = a :=
  rfl
#align distrib_mul_action_hom.one_apply DistribSMulSemiHom.one_apply
...
def comp (g : B →ₑ+[ψ] C) (f : A →ₑ+[φ] B) : A →ₑ+[ψ ∘ φ] C :=
  { SMulHom.comp (g : B →ₑ[ψ] C) (f : A →ₑ[φ] B) ,
    AddMonoidHom.comp (g : B →+ C) (f : A →+ B) with }
#align distrib_mul_action_hom.comp DistribSMulSemiHom.comp
...
theorem comp_apply (g : B →ₑ+[ψ] C) (f : A →ₑ+[φ] B) (x : A) :
  g.comp f x = g (f x) :=
  rfl
#align distrib_mul_action_hom.comp_apply DistribSMulSemiHom.comp_apply
...
theorem id_comp (f : A →ₑ+[φ] B) :
  comp (DistribSMulSemiHom.id N) f = f :=
  ext fun x => by rw [comp_apply, id_apply]
#align distrib_mul_action_hom.id_comp DistribSMulSemiHom.id_comp

@[simp]
theorem id_comp' (f : A →ₑ+[φ] B) :
  comp' (DistribSMulSemiHom.id N) f (CompTriple.id_comp) = f :=
  ext fun x => by rw [comp'_apply, id_apply]

@[simp]
theorem comp_id (f : A →ₑ+[φ] B) :
  comp f (DistribSMulSemiHom.id M) = f :=
  ext fun x => by rw [comp_apply, id_apply]
#align distrib_mul_action_hom.comp_id DistribSMulSemiHom.comp_id

@[simp]
theorem comp'_id (f : A →ₑ+[φ] B) :
  f.comp' (DistribSMulSemiHom.id M) (CompTriple.comp_id)= f :=
  ext fun x => by rw [comp'_apply, id_apply]

/-- The inverse of a bijective `DistribSMulSemiHom` is a `DistribSMulSemiHom`. -/
@[simps]
...
#align distrib_mul_action_hom.inverse DistribSMulSemiHom.inverse
...
#align distrib_mul_action_hom.ext_ring DistribSMulSemiHom.ext_ring

theorem ext_ring_iff {f g : R →ₑ+[σ] N'} : f = g ↔ f 1 = g 1 :=
  ⟨fun h => h ▸ rfl, ext_ring⟩
#align distrib_mul_action_hom.ext_ring_iff DistribSMulSemiHom.ext_ring_iff
...
add_decl_doc MulSemiringActionHom.toDistribSMulSemiHom
#align mul_semiring_action_hom.to_distrib_mul_action_hom MulSemiringActionHom.toDistribSMulSemiHom
...
class MulSemiringActionSemiHomClass (F : Type _)
  ...
  DistribSMulSemiHomClass F φ R S, RingHomClass F R S
...
protected def id : R →+*[M] R :=
  ⟨DistribSMulSemiHom.id _, rfl, (fun _ _ => rfl)⟩
...
def comp' (g : S →ₑ+*[ψ] T) (f : R →ₑ+*[φ] S)  (κ : CompTriple φ ψ χ) : R →ₑ+*[χ] T :=
  { DistribSMulSemiHom.comp' (g : S →ₑ+[ψ] T) (f : R →ₑ+[φ] S) κ,
