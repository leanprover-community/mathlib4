...
@[to_additive (attr := coe) "Turn an element of a type `F` satisfying `AddEquivClass F α β` into an actual
...
@[to_additive (attr := simp)] theorem piSubsingleton_apply {ι : Type _} (M : ι → Type _) [∀ j, Mul (M j)]
...
@[to_additive (attr := simp)] theorem piSubsingleton_symmApply {ι : Type _} (M : ι → Type _) [∀ j, Mul (M j)]
...
@[to_additive (attr := simp)] theorem MulHom.toMulEquiv_apply [Mul M] [Mul N] (f : M →ₙ* N) (g : N →ₙ* M)
...
@[to_additive (attr := simp)] theorem MulHom.toMulEquiv_symmApply [Mul M] [Mul N] (f : M →ₙ* N) (g : N →ₙ* M)
    (h₁ : g.comp f = MulHom.id _) (h₂ : f.comp g = MulHom.id _) :
...
@[to_additive (attr := simp)] theorem MonoidHom.toMulEquiv_apply [MulOneClass M] [MulOneClass N] (f : M →* N)
...
@[to_additive (attr := simp)] theorem MonoidHom.toMulEquiv_symmApply [MulOneClass M] [MulOneClass N]
