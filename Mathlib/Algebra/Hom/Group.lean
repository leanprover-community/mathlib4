...
lemma of_map {R M} [Zero R] [Zero M] [ZeroHomClass F R M]
  (f : F) {r : R} [neZero : NeZero (f r)] : NeZero r :=
  ⟨fun h => ne (f r) <| by rw [h]; exact ZeroHomClass.map_zero f⟩
#align ne_zero.of_map NeZero.of_map

lemma of_injective {R M} [Zero R] {r : R} [NeZero r] [Zero M] [ZeroHomClass F R M] {f : F}
...
lemma map_one [OneHomClass F M N] (f : F) : f 1 = 1 :=
...
@[to_additive]
lemma Subsingleton.of_oneHomClass [Subsingleton M] [OneHomClass F M N] :
...
@[to_additive]
lemma map_eq_one_iff [OneHomClass F M N] (f : F) (hf : Function.Injective f) {x : M} :
  f x = 1 ↔ x = 1 := hf.eq_iff' (map_one f)
#align map_eq_one_iff map_eq_one_iff
#align map_eq_zero_iff map_eq_zero_iff

@[to_additive]
lemma map_ne_one_iff {R S F : Type*} [One R] [One S] [OneHomClass F R S] (f : F)
...
@[to_additive]
lemma ne_one_of_map {R S F : Type*} [One R] [One S] [OneHomClass F R S] {f : F} {x : R}
...
@[to_additive (attr := simp)]
lemma OneHom.coe_coe [OneHomClass F M N] (f : F) : ((f : OneHom M N) : M → N) = f := rfl
...
@[to_additive (attr := simp)]
lemma map_mul [MulHomClass F M N] (f : F) (x y : M) : f (x * y) = f x * f y :=
...
@[to_additive (attr := simp)]
lemma MulHom.coe_coe [MulHomClass F M N] (f : F) : ((f : MulHom M N) : M → N) = f := rfl
...
@[to_additive (attr := simp)]
lemma MonoidHom.coe_coe [MonoidHomClass F M N] (f : F) : ((f : M →* N) : M → N) = f := rfl
#align monoid_hom.coe_coe MonoidHom.coe_coe
#align add_monoid_hom.coe_coe AddMonoidHom.coe_coe

@[to_additive]
lemma map_mul_eq_one [MonoidHomClass F M N] (f : F) {a b : M} (h : a * b = 1) : f a * f b = 1 :=
  by rw [← map_mul, h, map_one]
#align map_mul_eq_one map_mul_eq_one
#align map_add_eq_zero map_add_eq_zero

@[to_additive]
lemma map_div' [DivInvMonoid G] [DivInvMonoid H] [MonoidHomClass F G H]
...
lemma map_inv [Group G] [DivisionMonoid H] [MonoidHomClass F G H]
  (f : F) (a : G) : f a⁻¹ = (f a)⁻¹ :=
...
lemma map_mul_inv [Group G] [DivisionMonoid H] [MonoidHomClass F G H] (f : F) (a b : G) :
...
lemma map_div [Group G] [DivisionMonoid H] [MonoidHomClass F G H] (f : F) :
  ∀ a b, f (a / b) = f a / f b := map_div' _ <| map_inv f
#align map_div map_div
#align map_sub map_sub

@[to_additive (attr := simp) (reorder := 8 9)]
lemma map_pow [Monoid G] [Monoid H] [MonoidHomClass F G H] (f : F) (a : G) :
  ∀ n : ℕ, f (a ^ n) = f a ^ n
...
@[to_additive]
lemma map_zpow' [DivInvMonoid G] [DivInvMonoid H] [MonoidHomClass F G H]
...
lemma map_zpow [Group G] [DivisionMonoid H] [MonoidHomClass F G H]
...
@[simp]
lemma MonoidWithZeroHom.coe_coe [MonoidWithZeroHomClass F M N] (f : F) :
  ((f : M →*₀ N) : M → N) = f := rfl
...
@[to_additive (attr := simp)]
lemma MonoidHom.toMulHom_coe [MulOneClass M] [MulOneClass N] (f : M →* N) :
...
lemma OneHom.congr_fun [One M] [One N] {f g : OneHom M N} (h : f = g) (x : M) : f x = g x :=
...
/-- Deprecated: use `FunLike.congr_fun` instead. -/
@[to_additive (attr := deprecated) "Deprecated: use `FunLike.congr_fun` instead."]
lemma MulHom.congr_fun [Mul M] [Mul N] {f g : M →ₙ* N} (h : f = g) (x : M) : f x = g x :=
  FunLike.congr_fun h x
...
/-- Deprecated: use `FunLike.congr_fun` instead. -/
@[to_additive (attr := deprecated) "Deprecated: use `FunLike.congr_fun` instead."]
lemma MonoidHom.congr_fun [MulOneClass M] [MulOneClass N] {f g : M →* N} (h : f = g) (x : M) :
...
/-- Deprecated: use `FunLike.congr_fun` instead. -/
@[deprecated]
lemma MonoidWithZeroHom.congr_fun [MulZeroOneClass M] [MulZeroOneClass N] {f g : M →*₀ N}
...
lemma OneHom.congr_arg [One M] [One N] (f : OneHom M N) {x y : M} (h : x = y) : f x = f y :=
...
/-- Deprecated: use `FunLike.congr_arg` instead. -/
@[to_additive (attr := deprecated) "Deprecated: use `FunLike.congr_arg` instead."]
lemma MulHom.congr_arg [Mul M] [Mul N] (f : M →ₙ* N) {x y : M} (h : x = y) : f x = f y :=
  FunLike.congr_arg f h
...
/-- Deprecated: use `FunLike.congr_arg` instead. -/
@[to_additive (attr := deprecated) "Deprecated: use `FunLike.congr_arg` instead."]
lemma MonoidHom.congr_arg [MulOneClass M] [MulOneClass N] (f : M →* N) {x y : M} (h : x = y) :
...
/-- Deprecated: use `FunLike.congr_arg` instead. -/
@[deprecated]
lemma MonoidWithZeroHom.congr_arg [MulZeroOneClass M] [MulZeroOneClass N] (f : M →*₀ N) {x y : M}
...
lemma OneHom.coe_inj [One M] [One N] ⦃f g : OneHom M N⦄ (h : (f : M → N) = g) : f = g :=
...
/-- Deprecated: use `FunLike.coe_injective` instead. -/
@[to_additive (attr := deprecated) "Deprecated: use `FunLike.coe_injective` instead."]
lemma MulHom.coe_inj [Mul M] [Mul N] ⦃f g : M →ₙ* N⦄ (h : (f : M → N) = g) : f = g :=
  FunLike.coe_injective h
...
/-- Deprecated: use `FunLike.coe_injective` instead. -/
@[to_additive (attr := deprecated) "Deprecated: use `FunLike.coe_injective` instead."]
lemma MonoidHom.coe_inj [MulOneClass M] [MulOneClass N] ⦃f g : M →* N⦄ (h : (f : M → N) = g) :
...
/-- Deprecated: use `FunLike.coe_injective` instead. -/
@[deprecated]
lemma MonoidWithZeroHom.coe_inj [MulZeroOneClass M] [MulZeroOneClass N] ⦃f g : M →*₀ N⦄
...
lemma OneHom.ext_iff [One M] [One N] {f g : OneHom M N} : f = g ↔ ∀ x, f x = g x :=
  FunLike.ext_iff
...
/-- Deprecated: use `FunLike.ext_iff` instead. -/
@[to_additive (attr := deprecated) "Deprecated: use `FunLike.ext_iff` instead."]
lemma MulHom.ext_iff [Mul M] [Mul N] {f g : M →ₙ* N} : f = g ↔ ∀ x, f x = g x :=
  FunLike.ext_iff
...
/-- Deprecated: use `FunLike.ext_iff` instead. -/
@[to_additive (attr := deprecated) "Deprecated: use `FunLike.ext_iff` instead."]
lemma MonoidHom.ext_iff [MulOneClass M] [MulOneClass N] {f g : M →* N} : f = g ↔ ∀ x, f x = g x :=
  FunLike.ext_iff
...
/-- Deprecated: use `FunLike.ext_iff` instead. -/
@[deprecated]
lemma MonoidWithZeroHom.ext_iff [MulZeroOneClass M] [MulZeroOneClass N] {f g : M →*₀ N} :
...
@[to_additive (attr := simp)]
lemma OneHom.mk_coe [One M] [One N] (f : OneHom M N) (h1) : OneHom.mk f h1 = f :=
  OneHom.ext fun _ => rfl
#align one_hom.mk_coe OneHom.mk_coe
#align zero_hom.mk_coe ZeroHom.mk_coe

@[to_additive (attr := simp)]
lemma MulHom.mk_coe [Mul M] [Mul N] (f : M →ₙ* N) (hmul) : MulHom.mk f hmul = f :=
  MulHom.ext fun _ => rfl
#align mul_hom.mk_coe MulHom.mk_coe
#align add_hom.mk_coe AddHom.mk_coe

@[to_additive (attr := simp)]
lemma MonoidHom.mk_coe [MulOneClass M] [MulOneClass N] (f : M →* N) (hmul) :
...
@[simp]
lemma MonoidWithZeroHom.mk_coe [MulZeroOneClass M] [MulZeroOneClass N] (f : M →*₀ N) (h1 hmul) :
...
@[to_additive (attr := simp)]
lemma OneHom.coe_copy {_ : One M} {_ : One N} (f : OneHom M N) (f' : M → N) (h : f' = f) :
    (f.copy f' h) = f' :=
  rfl
...
@[to_additive]
lemma OneHom.coe_copy_eq {_ : One M} {_ : One N} (f : OneHom M N) (f' : M → N) (h : f' = f) :
    f.copy f' h = f :=
  FunLike.ext' h
...
@[to_additive (attr := simp)]
lemma MulHom.coe_copy {_ : Mul M} {_ : Mul N} (f : M →ₙ* N) (f' : M → N) (h : f' = f) :
    (f.copy f' h) = f' :=
  rfl
...
@[to_additive]
lemma MulHom.coe_copy_eq {_ : Mul M} {_ : Mul N} (f : M →ₙ* N) (f' : M → N) (h : f' = f) :
    f.copy f' h = f :=
  FunLike.ext' h
...
@[to_additive (attr := simp)]
lemma MonoidHom.coe_copy {_ : MulOneClass M} {_ : MulOneClass N} (f : M →* N) (f' : M → N)
    (h : f' = f) : (f.copy f' h) = f' :=
  rfl
...
@[to_additive]
lemma MonoidHom.copy_eq {_ : MulOneClass M} {_ : MulOneClass N} (f : M →* N) (f' : M → N)
    (h : f' = f) : f.copy f' h = f :=
  FunLike.ext' h
...
@[simp]
lemma MonoidWithZeroHom.coe_copy {_ : MulZeroOneClass M} {_ : MulZeroOneClass N} (f : M →*₀ N)
    (f' : M → N) (h : f' = f) : (f.copy f' h) = f' :=
  rfl
#align monoid_with_zero_hom.coe_copy MonoidWithZeroHom.coe_copy

lemma MonoidWithZeroHom.copy_eq {_ : MulZeroOneClass M} {_ : MulZeroOneClass N} (f : M →*₀ N)
    (f' : M → N) (h : f' = f) : f.copy f' h = f :=
  FunLike.ext' h
#align monoid_with_zero_hom.copy_eq MonoidWithZeroHom.copy_eq

@[to_additive]
protected lemma OneHom.map_one [One M] [One N] (f : OneHom M N) : f 1 = 1 :=
  f.map_one'
...
@[to_additive]
protected lemma MonoidHom.map_one [MulOneClass M] [MulOneClass N] (f : M →* N) : f 1 = 1 :=
  f.map_one'
#align monoid_hom.map_one MonoidHom.map_one
#align add_monoid_hom.map_zero AddMonoidHom.map_zero

protected lemma MonoidWithZeroHom.map_one [MulZeroOneClass M] [MulZeroOneClass N] (f : M →*₀ N) :
  f 1 = 1 := f.map_one'
...
protected lemma MonoidWithZeroHom.map_zero [MulZeroOneClass M] [MulZeroOneClass N] (f : M →*₀ N) :
  f 0 = 0 := f.map_zero'
#align monoid_with_zero_hom.map_zero MonoidWithZeroHom.map_zero

@[to_additive]
protected lemma MulHom.map_mul [Mul M] [Mul N] (f : M →ₙ* N) (a b : M) : f (a * b) = f a * f b :=
  f.map_mul' a b
...
@[to_additive]
protected lemma MonoidHom.map_mul [MulOneClass M] [MulOneClass N] (f : M →* N) (a b : M) :
  f (a * b) = f a * f b := f.map_mul' a b
#align monoid_hom.map_mul MonoidHom.map_mul
#align add_monoid_hom.map_add AddMonoidHom.map_add

protected lemma MonoidWithZeroHom.map_mul [MulZeroOneClass M] [MulZeroOneClass N] (f : M →*₀ N)
...
@[to_additive (attr := simp)]
lemma OneHom.coe_comp [One M] [One N] [One P] (g : OneHom N P) (f : OneHom M N) :
  ↑(g.comp f) = g ∘ f := rfl
#align one_hom.coe_comp OneHom.coe_comp
#align zero_hom.coe_comp ZeroHom.coe_comp

@[to_additive (attr := simp)]
lemma MulHom.coe_comp [Mul M] [Mul N] [Mul P] (g : N →ₙ* P) (f : M →ₙ* N) :
  ↑(g.comp f) = g ∘ f := rfl
#align mul_hom.coe_comp MulHom.coe_comp
#align add_hom.coe_comp AddHom.coe_comp

@[to_additive (attr := simp)]
lemma MonoidHom.coe_comp [MulOneClass M] [MulOneClass N] [MulOneClass P]
...
@[simp]
lemma MonoidWithZeroHom.coe_comp [MulZeroOneClass M] [MulZeroOneClass N] [MulZeroOneClass P]
  (g : N →*₀ P) (f : M →*₀ N) : ↑(g.comp f) = g ∘ f := rfl
#align monoid_with_zero_hom.coe_comp MonoidWithZeroHom.coe_comp

@[to_additive]
lemma OneHom.comp_apply [One M] [One N] [One P] (g : OneHom N P) (f : OneHom M N) (x : M) :
  g.comp f x = g (f x) := rfl
#align one_hom.comp_apply OneHom.comp_apply
#align zero_hom.comp_apply ZeroHom.comp_apply

@[to_additive]
lemma MulHom.comp_apply [Mul M] [Mul N] [Mul P] (g : N →ₙ* P) (f : M →ₙ* N) (x : M) :
  g.comp f x = g (f x) := rfl
#align mul_hom.comp_apply MulHom.comp_apply
#align add_hom.comp_apply AddHom.comp_apply

@[to_additive]
lemma MonoidHom.comp_apply [MulOneClass M] [MulOneClass N] [MulOneClass P]
...
lemma MonoidWithZeroHom.comp_apply [MulZeroOneClass M] [MulZeroOneClass N] [MulZeroOneClass P]
  (g : N →*₀ P) (f : M →*₀ N) (x : M) : g.comp f x = g (f x) := rfl
...
lemma OneHom.comp_assoc {Q : Type*} [One M] [One N] [One P] [One Q]
...
@[to_additive]
lemma MulHom.comp_assoc {Q : Type*} [Mul M] [Mul N] [Mul P] [Mul Q]
...
@[to_additive]
lemma MonoidHom.comp_assoc {Q : Type*} [MulOneClass M] [MulOneClass N] [MulOneClass P]
...
lemma MonoidWithZeroHom.comp_assoc {Q : Type*} [MulZeroOneClass M] [MulZeroOneClass N]
  [MulZeroOneClass P] [MulZeroOneClass Q] (f : M →*₀ N) (g : N →*₀ P) (h : P →*₀ Q) :
  (h.comp g).comp f = h.comp (g.comp f) := rfl
#align monoid_with_zero_hom.comp_assoc MonoidWithZeroHom.comp_assoc

@[to_additive]
lemma OneHom.cancel_right [One M] [One N] [One P] {g₁ g₂ : OneHom N P} {f : OneHom M N}
...
@[to_additive]
lemma MulHom.cancel_right [Mul M] [Mul N] [Mul P] {g₁ g₂ : N →ₙ* P} {f : M →ₙ* N}
  (hf : Function.Surjective f) : g₁.comp f = g₂.comp f ↔ g₁ = g₂ :=
...
@[to_additive]
lemma MonoidHom.cancel_right [MulOneClass M] [MulOneClass N] [MulOneClass P]
...
lemma MonoidWithZeroHom.cancel_right [MulZeroOneClass M] [MulZeroOneClass N] [MulZeroOneClass P]
  {g₁ g₂ : N →*₀ P} {f : M →*₀ N} (hf : Function.Surjective f) :
  g₁.comp f = g₂.comp f ↔ g₁ = g₂ :=
...
@[to_additive]
lemma OneHom.cancel_left [One M] [One N] [One P] {g : OneHom N P} {f₁ f₂ : OneHom M N}
...
@[to_additive]
lemma MulHom.cancel_left [Mul M] [Mul N] [Mul P] {g : N →ₙ* P} {f₁ f₂ : M →ₙ* N}
  (hg : Function.Injective g) : g.comp f₁ = g.comp f₂ ↔ f₁ = f₂ :=
...
@[to_additive]
lemma MonoidHom.cancel_left [MulOneClass M] [MulOneClass N] [MulOneClass P]
...
lemma MonoidWithZeroHom.cancel_left [MulZeroOneClass M] [MulZeroOneClass N] [MulZeroOneClass P]
...
@[to_additive]
lemma MonoidHom.toOneHom_injective [MulOneClass M] [MulOneClass N] :
...
@[to_additive]
lemma MonoidHom.toMulHom_injective [MulOneClass M] [MulOneClass N] :
...
lemma MonoidWithZeroHom.toMonoidHom_injective [MulZeroOneClass M] [MulZeroOneClass N] :
...
lemma MonoidWithZeroHom.toZeroHom_injective [MulZeroOneClass M] [MulZeroOneClass N] :
...
@[to_additive (attr := simp)]
lemma OneHom.comp_id [One M] [One N] (f : OneHom M N) : f.comp (OneHom.id M) = f :=
  OneHom.ext fun _ => rfl
#align one_hom.comp_id OneHom.comp_id
#align zero_hom.comp_id ZeroHom.comp_id

@[to_additive (attr := simp)]
lemma MulHom.comp_id [Mul M] [Mul N] (f : M →ₙ* N) : f.comp (MulHom.id M) = f :=
  MulHom.ext fun _ => rfl
#align mul_hom.comp_id MulHom.comp_id
#align add_hom.comp_id AddHom.comp_id

@[to_additive (attr := simp)]
lemma MonoidHom.comp_id [MulOneClass M] [MulOneClass N] (f : M →* N) :
...
@[simp]
lemma MonoidWithZeroHom.comp_id [MulZeroOneClass M] [MulZeroOneClass N] (f : M →*₀ N) :
...
@[to_additive (attr := simp)]
lemma OneHom.id_comp [One M] [One N] (f : OneHom M N) : (OneHom.id N).comp f = f :=
  OneHom.ext fun _ => rfl
#align one_hom.id_comp OneHom.id_comp
#align zero_hom.id_comp ZeroHom.id_comp

@[to_additive (attr := simp)]
lemma MulHom.id_comp [Mul M] [Mul N] (f : M →ₙ* N) : (MulHom.id N).comp f = f :=
  MulHom.ext fun _ => rfl
#align mul_hom.id_comp MulHom.id_comp
#align add_hom.id_comp AddHom.id_comp

@[to_additive (attr := simp)]
lemma MonoidHom.id_comp [MulOneClass M] [MulOneClass N] (f : M →* N) :
...
@[simp]
lemma MonoidWithZeroHom.id_comp [MulZeroOneClass M] [MulZeroOneClass N] (f : M →*₀ N) :
...
@[to_additive]
protected lemma MonoidHom.map_pow [Monoid M] [Monoid N] (f : M →* N) (a : M) (n : ℕ) :
  f (a ^ n) = f a ^ n := map_pow f a n
#align monoid_hom.map_pow MonoidHom.map_pow
#align add_monoid_hom.map_nsmul AddMonoidHom.map_nsmul

@[to_additive]
protected lemma MonoidHom.map_zpow' [DivInvMonoid M] [DivInvMonoid N] (f : M →* N)
...
@[simp]
lemma coe_one : ((1 : Monoid.End M) : M → M) = id := rfl
...
@[simp]
lemma coe_one : ((1 : AddMonoid.End A) : A → A) = id := rfl
...
@[to_additive]
lemma mul_comp [MulOneClass M] [MulOneClass N] [CommMonoid P] (g₁ g₂ : N →* P) (f : M →* N) :
  (g₁ * g₂).comp f = g₁.comp f * g₂.comp f := rfl
#align monoid_hom.mul_comp MonoidHom.mul_comp
#align add_monoid_hom.add_comp AddMonoidHom.add_comp

@[to_additive]
lemma comp_mul [MulOneClass M] [CommMonoid N] [CommMonoid P] (g : N →* P) (f₁ f₂ : M →* N) :
  g.comp (f₁ * f₂) = g.comp f₁ * g.comp f₂ := by
  ext
  simp only [mul_apply, Function.comp_apply, map_mul, coe_comp]
...
protected lemma map_inv [Group α] [DivisionMonoid β] (f : α →* β) (a : α) :
  f a⁻¹ = (f a)⁻¹ := map_inv f _
...
protected lemma map_zpow [Group α] [DivisionMonoid β] (f : α →* β) (g : α) (n : ℤ) :
...
/-- Group homomorphisms preserve division. -/
@[to_additive "Additive group homomorphisms preserve subtraction."]
protected lemma map_div [Group α] [DivisionMonoid β] (f : α →* β) (g h : α) :
...
/-- Group homomorphisms preserve division. -/
@[to_additive "Additive group homomorphisms preserve subtraction."]
protected lemma map_mul_inv [Group α] [DivisionMonoid β] (f : α →* β) (g h : α) :
...
lemma _root_.injective_iff_map_eq_one {G H} [Group G] [MulOneClass H] [MonoidHomClass F G H]
...
@[to_additive (attr := simp)]
lemma inv_apply {M G} [MulOneClass M] [CommGroup G] (f : M →* G) (x : M) :
  f⁻¹ x = (f x)⁻¹ := rfl
#align monoid_hom.inv_apply MonoidHom.inv_apply
#align add_monoid_hom.neg_apply AddMonoidHom.neg_apply

@[to_additive (attr := simp)]
lemma inv_comp {M N A} [MulOneClass M] [MulOneClass N] [CommGroup A]
...
@[to_additive (attr := simp)]
lemma comp_inv {M A B} [MulOneClass M] [CommGroup A] [CommGroup B]
...
@[to_additive (attr := simp)]
lemma div_apply {M G} [MulOneClass M] [CommGroup G] (f g : M →* G) (x : M) :
  (f / g) x = f x / g x := rfl
