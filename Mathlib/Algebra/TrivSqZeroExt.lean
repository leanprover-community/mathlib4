...
theorem inl_mul_eq_smul [Semiring R] [AddCommMonoid M] [DistribMulAction R M] [Module Rᵐᵒᵖ M]
    (r : R) (x : tsze R M) :
    inl r * x = r •> x :=
  ext rfl (by dsimp; rw [smul_zero, add_zero])

theorem mul_inl_eq_op_smul [Semiring R] [AddCommMonoid M] [DistribMulAction R M] [Module Rᵐᵒᵖ M]
    (x : tsze R M) (r : R) :
    x * inl r = x <• r :=
...
theorem snd_list_prod [Semiring R] [AddCommMonoid M] [DistribMulAction R M] [Module Rᵐᵒᵖ M]
