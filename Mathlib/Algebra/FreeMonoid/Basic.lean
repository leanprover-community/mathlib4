...
@[to_additive (attr := simp)]
lemma toList_ofList (l : List α) : toList (ofList l) = l := rfl
...
@[to_additive (attr := simp)]
lemma ofList_toList (xs : FreeMonoid α) : ofList (toList xs) = xs := rfl
...
@[to_additive (attr := simp)]
lemma toList_mul (xs ys : FreeMonoid α) : toList (xs * ys) = toList xs ++ toList ys := rfl
#align free_monoid.to_list_mul FreeMonoid.toList_mul
#align free_add_monoid.to_list_add FreeAddMonoid.toList_add

@[to_additive (attr := simp)]
lemma ofList_append (xs ys : List α) : ofList (xs ++ ys) = ofList xs * ofList ys := rfl
...
@[to_additive (attr := simp)]
lemma toList_prod (xs : List (FreeMonoid α)) : toList xs.prod = (xs.map toList).join := by
...
@[to_additive (attr := simp)]
lemma ofList_join (xs : List (List α)) : ofList xs.join = (xs.map ofList).prod :=
...
@[to_additive (attr := simp)]
lemma ofList_cons (x : α) (xs : List α) : ofList (x :: xs) = of x * ofList xs := rfl
#align free_monoid.of_list_cons FreeMonoid.ofList_cons
#align free_add_monoid.of_list_cons FreeAddMonoid.ofList_cons

@[to_additive]
lemma toList_of_mul (x : α) (xs : FreeMonoid α) : toList (of x * xs) = x :: toList xs := rfl
...
@[to_additive (attr := simp)]
lemma lift_ofList (f : α → M) (l : List α) : lift f (ofList l) = (l.map f).prod :=
  prodAux_eq _

@[to_additive (attr := simp)]
lemma lift_symm_apply (f : FreeMonoid α →* M) : lift.symm f = f ∘ of := rfl
...
@[to_additive (attr := simp)]
lemma lift_restrict (f : FreeMonoid α →* M) : lift (f ∘ of) = f := lift.apply_symm_apply f
#align free_monoid.lift_restrict FreeMonoid.lift_restrict
#align free_add_monoid.lift_restrict FreeAddMonoid.lift_restrict

@[to_additive]
lemma comp_lift (g : M →* N) (f : α → M) : g.comp (lift f) = lift (g ∘ f) := by
  ext
  simp
...
@[to_additive]
lemma hom_map_lift (g : M →* N) (f : α → M) (x : FreeMonoid α) : g (lift f x) = lift (g ∘ f) x :=
...
@[to_additive]
lemma smul_def (f : α → β → β) (l : FreeMonoid α) (b : β) :
    haveI := mkMulAction f
...
@[to_additive]
lemma ofList_smul (f : α → β → β) (l : List α) (b : β) :
    haveI := mkMulAction f
...
@[to_additive]
lemma map_comp (g : β → γ) (f : α → β) : map (g ∘ f) = (map g).comp (map f) := hom_eq fun _ ↦ rfl
