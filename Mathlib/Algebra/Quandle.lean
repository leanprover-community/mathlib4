...
inductive PreEnvelGroupRel' (R : Type u) [Rack R] : PreEnvelGroup R → PreEnvelGroup R → Type u
  ...
  | inv_mul_self (a : PreEnvelGroup R) : PreEnvelGroupRel' R (mul (inv a) a) unit
  | act_incl (x y : R) :
...
instance (R : Type*) [Rack R] : Group (EnvelGroup R) :=
  { inv_mul_self := fun a =>
      Quotient.inductionOn a fun a => Quotient.sound (PreEnvelGroupRel'.inv_mul_self a).rel }
...
theorem well_def {R : Type*} [Rack R] {G : Type*} [Group G] (f : R →◃ Quandle.Conj G) :
  ...
  | _, _, PreEnvelGroupRel'.inv_mul_self a => by simp [toEnvelGroup.mapAux]
