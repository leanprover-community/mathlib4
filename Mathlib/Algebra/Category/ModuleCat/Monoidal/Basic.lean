...
instance (priority := high) monoidalCategory :
    MonoidalCategory (ModuleCat.{u} R) := MonoidalCategory.ofTensorHom
  (tensor_id := fun M N ↦ tensor_id M N)
  (tensor_comp := fun f g h ↦ MonoidalCategory.tensor_comp f g h)
  (associator_naturality := fun f g h ↦ MonoidalCategory.associator_naturality f g h)
  (leftUnitor_naturality := fun f ↦ MonoidalCategory.leftUnitor_naturality f)
...
variable (R) in
@[simps]
def uliftFunctor : ModuleCat.{u} R ⥤ ModuleCatMax.{v, u} R where
...
@[simps (config := .lemmasOnly)]
instance : MonoidalCategoryStruct (ModuleCatMax.{v, u} R) where
  tensorObj := tensorObj
  whiskerLeft := whiskerLeft
  whiskerRight := whiskerRight
  tensorHom := tensorHom
  tensorUnit := (uliftFunctor R).obj (ModuleCat.of R R)
  associator := associator
...
@[ext]
theorem _root_.ULift.ext_linearMap {R : Type u} [Semiring R]
    {M : Type v} {N : Type w} [AddCommMonoid M] [AddCommMonoid N]
    [Module R M] [Module R N] (f g : ULift.{x} M →ₗ[R] N)
    (h : f ∘ₗ (ULift.moduleEquiv (R := R) (M := M)).symm
      = g ∘ₗ ((ULift.moduleEquiv (R := R) (M := M)).symm : M →ₗ[R] _)) : f = g :=
  LinearMap.ext fun ⟨x⟩ => congr($h x)

--3.9
set_option trace.profiler true in
def hmm : Monoidal.InducingFunctorData (uliftFunctor.{v} R) where
  μIso X Y := (TensorProduct.congr ULift.moduleEquiv ULift.moduleEquiv
    ≪≫ₗ ULift.moduleEquiv.symm).toModuleIso
  whiskerLeft_eq X Y Z f := by
    apply ULift.ext_linearMap
    apply TensorProduct.ext
    rfl
  whiskerRight_eq X f := by
    apply ULift.ext_linearMap
    apply TensorProduct.ext
    rfl
  tensorHom_eq f g := by
    ...
    apply TensorProduct.ext
    rfl
  εIso := ULift.moduleEquiv.symm.toModuleIso
...
instance (priority := low) : MonoidalCategory (ModuleCatMax.{v, u} R) :=
  Monoidal.fromInduced (uliftFunctor R) hmm
end ModuleCat
