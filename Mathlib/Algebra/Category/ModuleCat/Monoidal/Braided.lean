...
-/
class BraidedCategory (C : Type u) [Category.{v} C] [SemigroupalCategory.{v} C] where
...
variable {C : Type u} [Category.{v} C] [SemigroupalCategory.{v} C] [BraidedCategory.{v} C]
...
-/
def braidedCategoryOfFaithful {C D : Type*} [Category C] [Category D] [SemigroupalCategory C]
    [SemigroupalCategory D] (F : SemigroupalFunctor C D) [F.Faithful] [BraidedCategory D]
    (β : ∀ X Y : C, X ⊗ Y ≅ Y ⊗ X)
  ...
  braiding_naturality_left := by
    ...
    rw [Functor.map_comp, ← LaxSemigroupalFunctor.μ_natural_left_assoc, w, Functor.map_comp,
      reassoc_of% w, braiding_naturality_left_assoc, LaxSemigroupalFunctor.μ_natural_right]
  braiding_naturality_right := by
    intros
    apply F.map_injective
    refine (cancel_epi (F.μ ?_ ?_)).1 ?_
    rw [Functor.map_comp, ← LaxSemigroupalFunctor.μ_natural_right_assoc, w, Functor.map_comp,
      reassoc_of% w, braiding_naturality_right_assoc, LaxSemigroupalFunctor.μ_natural_left]
  hexagon_forward := by
    intros
    apply F.map_injective
    refine (cancel_epi (F.μ _ _)).1 ?_
    ...
    rw [Functor.map_comp, Functor.map_comp, Functor.map_comp, Functor.map_comp, ←
      LaxSemigroupalFunctor.μ_natural_left_assoc, ← comp_whiskerRight_assoc, w,
      comp_whiskerRight_assoc, LaxSemigroupalFunctor.associativity_assoc,
      LaxSemigroupalFunctor.associativity_assoc, ← LaxSemigroupalFunctor.μ_natural_right, ←
      SemigroupalCategory.whiskerLeft_comp_assoc, w, SemigroupalCategory.whiskerLeft_comp_assoc,
      reassoc_of% w, braiding_naturality_right_assoc,
      LaxSemigroupalFunctor.associativity, hexagon_forward_assoc]
  hexagon_reverse := by
    intros
    ...
    rw [Functor.map_comp, Functor.map_comp, Functor.map_comp, Functor.map_comp, ←
      LaxSemigroupalFunctor.μ_natural_right_assoc, ← SemigroupalCategory.whiskerLeft_comp_assoc, w,
      SemigroupalCategory.whiskerLeft_comp_assoc, LaxSemigroupalFunctor.associativity_inv_assoc,
      LaxSemigroupalFunctor.associativity_inv_assoc, ← LaxSemigroupalFunctor.μ_natural_left,
      ← comp_whiskerRight_assoc, w, comp_whiskerRight_assoc, reassoc_of% w,
      braiding_naturality_left_assoc, LaxSemigroupalFunctor.associativity_inv, hexagon_reverse_assoc]

/-- Pull back a braiding along a fully faithful monoidal functor. -/
noncomputable def braidedCategoryOfFullyFaithful {C D : Type*} [Category C] [Category D]
    [SemigroupalCategory C] [SemigroupalCategory D] (F : SemigroupalFunctor C D) [F.Full]
...
variable (C : Type u₁) [Category.{v₁} C] [SemigroupalCategory C] [BraidedCategory C]
...
theorem braiding_rightUnitor (X : C) : (β_ (𝟙_ C) X).hom ≫ (ρ_ X).hom = (λ_ X).hom := by
  rw [← whiskerLeft_iff, SemigroupalCategory.whiskerLeft_comp, braiding_rightUnitor_aux₂]
...
-/
class SymmetricCategory (C : Type u) [Category.{v} C] [SemigroupalCategory.{v} C] extends
...
lemma SymmetricCategory.braiding_swap_eq_inv_braiding {C : Type u₁}
    [Category.{v₁} C] [SemigroupalCategory C] [SymmetricCategory C] (X Y : C) :
    (β_ Y X).hom = (β_ X Y).inv := Iso.inv_ext' (symmetry X Y)

variable (C : Type u₁) [Category.{v₁} C] [SemigroupalCategory C] [BraidedCategory C]
variable (D : Type u₂) [Category.{v₂} D] [SemigroupalCategory D] [BraidedCategory D]
variable (E : Type u₃) [Category.{v₃} E] [SemigroupalCategory E] [BraidedCategory E]

/-- A lax braided functor between braided monoidal categories is a lax monoidal functor
which preserves the braiding.
-/
structure LaxBraidedFunctor extends LaxSemigroupalFunctor C D where
...
def comp (F : LaxBraidedFunctor C D) (G : LaxBraidedFunctor D E) : LaxBraidedFunctor C E :=
  { LaxSemigroupalFunctor.comp F.toLaxSemigroupalFunctor G.toLaxSemigroupalFunctor with
    braided := fun X Y => by
      dsimp
      slice_lhs 2 3 =>
...
instance categoryLaxBraidedFunctor : Category (LaxBraidedFunctor C D) :=
  InducedCategory.category LaxBraidedFunctor.toLaxSemigroupalFunctor
...
-/
@[simps]
def mkIso {F G : LaxBraidedFunctor C D} (i : F.toLaxSemigroupalFunctor ≅ G.toLaxSemigroupalFunctor) :
    F ≅ G :=
  { i with }
...
which preserves the braiding.
-/
structure BraidedFunctor extends SemigroupalFunctor C D where
...
-/
def symmetricCategoryOfFaithful {C D : Type*} [Category C] [Category D] [SemigroupalCategory C]
    [SemigroupalCategory D] [BraidedCategory C] [SymmetricCategory D] (F : BraidedFunctor C D)
...
@[simps toLaxSemigroupalFunctor]
def toLaxBraidedFunctor (F : BraidedFunctor C D) : LaxBraidedFunctor C D :=
  { toLaxSemigroupalFunctor := F.toLaxSemigroupalFunctor
...
def comp (F : BraidedFunctor C D) (G : BraidedFunctor D E) : BraidedFunctor C E :=
  { SemigroupalFunctor.comp F.toSemigroupalFunctor G.toSemigroupalFunctor with }

instance categoryBraidedFunctor : Category (BraidedFunctor C D) :=
  InducedCategory.category BraidedFunctor.toSemigroupalFunctor
...
-/
@[simps]
def mkIso {F G : BraidedFunctor C D} (i : F.toSemigroupalFunctor ≅ G.toSemigroupalFunctor) : F ≅ G :=
  { i with }
...
theorem tensor_left_unitality (X₁ X₂ : C) :
  ...
  clear this
  slice_rhs 1 2 => rw [← SemigroupalCategory.whiskerLeft_comp, ← comp_whiskerRight,
...
theorem tensor_right_unitality (X₁ X₂ : C) :
  ...
  slice_rhs 1 3 => rw [this]
  clear this
  slice_rhs 2 3 => rw [← SemigroupalCategory.whiskerLeft_comp, ← comp_whiskerRight,
...
theorem tensor_associativity (X₁ X₂ Y₁ Y₂ Z₁ Z₂ : C) :
  ...
  dsimp only [tensor_obj, prodSemigroupal_tensorObj, tensor_μ]
  simp only [whiskerRight_tensor, comp_whiskerRight, whisker_assoc, assoc, Iso.inv_hom_id_assoc,
    tensor_whiskerLeft, braiding_tensor_left, SemigroupalCategory.whiskerLeft_comp,
...
@[simps!]
def tensorSemigroupal : SemigroupalFunctor (C × C) C :=
  { tensor C with
    ε := (λ_ (𝟙_ C)).inv
    μ := tensor_μ C
...
theorem leftUnitor_monoidal (X₁ X₂ : C) :
  ...
  dsimp only [tensor_obj, prodSemigroupal_tensorObj]
  coherence
...
section SemigroupalOppositeLemmas
...
lemma SymmetricCategory.reverseBraiding_eq (C : Type u₁) [Category.{v₁} C]
    [SemigroupalCategory C] [i : SymmetricCategory C] :
...
def SymmetricCategory.equivReverseBraiding (C : Type u₁) [Category.{v₁} C]
    [SemigroupalCategory C] [SymmetricCategory C] :=
