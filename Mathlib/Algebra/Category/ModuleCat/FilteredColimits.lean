...
def colimitDesc (t : Cocone F) : colimit F ⟶ t.pt :=
  ...
  have hf {j : J} (x : F.obj j) : f (M.mk _ ⟨j, x⟩) = t.ι.app j x := by
    exact congr_fun ((forget _).congr_map (h.fac ((forget₂ _ _).mapCocone t) j)) _
  ofHom
    { f with
      map_smul' := fun r x => by
        obtain ⟨j, x, rfl⟩ := M.mk_surjective F x
...
def colimitCoconeIsColimit : IsColimit (colimitCocone F) where
  ...
  uniq t _ h := by
    ext ⟨j, x⟩
    exact (congr_fun ((forget _).congr_map (h j)) _).trans
      (congr_fun ((forget _).congr_map (ι_colimitDesc F t j)) _).symm
