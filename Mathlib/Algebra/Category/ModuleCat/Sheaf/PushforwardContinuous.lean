...
lemma pushforwardNatTrans_id :
    pushforwardNatTrans φ (𝟙 G) = (pushforwardCongr (by cat_disch)).hom := by
  apply NatTrans.ext'; funext M
  apply SheafOfModules.hom_ext
  apply PresheafOfModules.hom_ext; intro U
  apply ConcreteCategory.ext_apply; intro x
  simp only [pushforwardNatTrans_app_val_app, NatTrans.id_app, op_id,
    PresheafOfModules.map_id, pushforwardCongr_hom_app_val_app,
...
lemma pushforwardNatTrans_comp (α : F ⟶ G) (β : G ⟶ H)
    ...
    pushforwardNatTrans φ (α ≫ β) = pushforwardNatTrans φ β ≫ pushforwardNatTrans _ α ≫
      (pushforwardCongr (by cat_disch)).hom := by
  apply NatTrans.ext'; funext M
  apply SheafOfModules.hom_ext
  apply PresheafOfModules.hom_ext; intro U
  apply ConcreteCategory.ext_apply; intro x
  simp only [pushforwardNatTrans_app_val_app, NatTrans.comp_app, op_comp,
    SheafOfModules.comp_val, PresheafOfModules.comp_app, pushforwardCongr_hom_app_val_app,
    CategoryTheory.comp_apply]
  exact PresheafOfModules.map_comp_apply M.val _ _ x
