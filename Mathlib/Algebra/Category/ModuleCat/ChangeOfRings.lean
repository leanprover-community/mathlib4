  ...
  { toFun := fun y : Y =>
      { toFun := fun s : S => g <| (s • y : Y)
        ...
        map_smul' := fun r (s : S) => by
          dsimp
          rw [ModuleCat.restrictScalars.smul_def f r (M := ModuleCat.of S S), ← LinearMap.map_smul]
          simp only [smul_eq_mul, mul_smul, restrictScalars.smul_def] }
    map_add' := fun y1 y2 : Y =>
...
def HomEquiv.toRestriction {X Y} (g : Y ⟶ (coextendScalars f).obj X) :
  ...
  { toFun := fun y : Y => (g y) (1 : S)
    ...
    map_smul' := fun r (y : Y) => by
      dsimp
      rw [← map_smul, ModuleCat.restrictScalars.smul_def f r (M := ModuleCat.of S S),
        map_smul, CoextendScalars.smul_apply, one_mul]
