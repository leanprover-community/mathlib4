...
-- TODO: this instance gives diamonds if `f : S →+* S`...
instance {R : Type u₁} {S : Type u₂} [Ring R] [Ring S] {f : R →+* S}
  ...
  naturality Y Y' g :=
    hom_ext <| LinearMap.ext fun y : Y => LinearMap.ext fun s : S => by
      ...
      simp only [ModuleCat.hom_comp, Functor.id_map, Functor.id_obj, Functor.comp_obj,
        Functor.comp_map]
      rw [LinearMap.coe_comp, LinearMap.coe_comp, Function.comp, Function.comp]
  ...
  Adjunction.mk' {
    homEquiv := fun X Y ↦
      { toFun := RestrictionCoextensionAdj.HomEquiv.fromRestriction.{u₁,u₂,v} f
        ...
        left_inv := fun g => hom_ext <| LinearMap.ext fun x : X => by
          -- Porting note (https://github.com/leanprover-community/mathlib4/pull/10745): once just simp
          rw [RestrictionCoextensionAdj.HomEquiv.toRestriction_hom_apply, AddHom.toFun_eq_coe,
            LinearMap.coe_toAddHom]
          -- This needs to be `erw` because `coextendScalars.obj Y` is unfolded in
          -- `fromRestriction_hom_apply_apply`. Changing `fromRestriction` allows the whole proof
          -- to be `simp` but causes defeq trouble in the definition itself...)
          erw [RestrictionCoextensionAdj.HomEquiv.fromRestriction_hom_apply_apply]
          simp
...
def HomEquiv.toRestrictScalars {X Y} (g : (extendScalars f).obj X ⟶ Y) :
  ...
  asHom (X := X) (Y := (restrictScalars f).obj Y)
...
def Unit.map {X} : X ⟶ (extendScalars f ⋙ restrictScalars f).obj X :=
  asHom (X := X) (Y := (extendScalars f ⋙ restrictScalars f).obj X)
...
attribute [nolint simpNF] Counit.map_hom_apply
