...
rings and `α : R₀ ⟶ R.obj` is locally bijective, and `A` is the sheafification
of the underlying presheaf of abelian groups of `M₀`, i.e. we have a locally bijective
map `φ : M₀.presheaf ⟶ A.obj`, then we endow `A` with the structure of a
...
`r : R.obj.obj X` and `m : A.obj.obj X`, this structure contains the data
of `x : A.obj.obj X` along with the property which makes `x` a good candidate
