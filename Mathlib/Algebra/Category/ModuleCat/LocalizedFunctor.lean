...
instance : HasForget₂ CommRingCat.{u} MonCat.{u} where
  forget₂ := Functor.comp (forget₂ CommRingCat.{u} CommMonCat.{u})
    (forget₂ CommMonCat.{u} MonCat.{u})
  forget_comp := rfl

variable {C : Type u₁} [Category.{v₁} C] {R : C ⥤ CommRingCat.{u}}
  (S : SubmonoidFunctor (R ⋙ (forget₂ CommRingCat.{u} MonCat.{u})))
...
lemma SubmonoidFunctor.ringMap {X Y : C} (i : X ⟶ Y) (s : ringObj S X) :
...
@[mk_iff] class SubmonoidFunctor.isLocalization {R' : C ⥤ CommRingCat} (p : R ⟶ R') : Prop where
  map_units (p) (U : C) (y : S.ringObj U) : IsUnit ((p.app U).hom y)
  surj (p) (U : C) (z : R'.obj U) : ∃ x : (R.obj U) × (S.ringObj U), z * (p.app U).hom x.2 =
    (p.app U).hom x.1
  exists_of_eq {U : C} {x y} : (p.app U).hom x = (p.app U).hom y → ∃ c : S.ringObj U, c * x = c * y

attribute [nolint docBlame] SubmonoidFunctor.isLocalization.map_units
  SubmonoidFunctor.isLocalization.surj SubmonoidFunctor.isLocalization.exists_of_eq

lemma isLocalizationMap_of_isLocalization {R' : C ⥤ CommRingCat} (p : R ⟶ R') [S.isLocalization p]
    {U : C} : (S.ringObj U).IsLocalizationMap (p.app U) :=
  ⟨SubmonoidFunctor.isLocalization.map_units _ _, SubmonoidFunctor.isLocalization.surj _ _ ,
    SubmonoidFunctor.isLocalization.exists_of_eq⟩
...
lemma IsLocalization.eq_iff_exists {R' : C ⥤ CommRingCat} (p : R ⟶ R') [S.isLocalization p]
  ...
  Iff.intro SubmonoidFunctor.isLocalization.exists_of_eq fun ⟨c, h⟩ ↦ by
    apply_fun (p.app U).hom at h
    rename_i h₁
    simp_all only [Subtype.exists, exists_prop]
    simp_rw [map_mul] at h
    obtain h₂ := (SubmonoidFunctor.isLocalization.map_units p U c)
    apply IsUnit.mul_left_cancel h₂
    apply h
...
instance : S.isLocalization (ι S) where
  map_units U x :=  _root_.IsLocalization.map_units (Localization (S.ringObj U)) x
  surj U z := _root_.IsLocalization.surj (S.ringObj U) z
  exists_of_eq :=  _root_.IsLocalization.exists_of_eq
...
noncomputable def fromLocalization :
  ...
  lift' S p (by intro U x; apply (SubmonoidFunctor.isLocalization.map_units p U x))
...
lemma fromLocalization.surj {U : C} : Function.Surjective ((fromLocalization S p).app U).hom :=
  fun z => let ⟨⟨m, (s : S.ringObj U)⟩, eq1⟩ := SubmonoidFunctor.isLocalization.surj p U z
...
theorem iso_inv_apply_aux {U : C} (r : R'.obj U) : ((iso S p).inv.app U) r = Localization.mk
  ...
  rw [mk_eq_mk, IsLocalization.lift_mk', Units.eq_mul_inv_iff_mul_eq, IsUnit.coe_liftRight,
    ...
    (SubmonoidFunctor.isLocalization.surj _ _ _).choose_spec]
