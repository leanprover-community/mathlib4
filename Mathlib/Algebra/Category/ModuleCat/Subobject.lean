  ...
  OrderIso.symm
    { invFun := fun S => LinearMap.range S.arrow.hom
      ...
      left_inv := fun N => by
        ...
        · have :
            ...
            (underlyingIso (↾N.subtype : of R _ ⟶ M)).inv =
              asHom (underlyingIso (↾N.subtype : of R _ ⟶ M)).symm.toLinearEquiv.toLinearMap := by
              ext x
              rfl
      ...
      map_rel_iff' := fun {S T} => by
        ...
        convert LinearMap.range_comp_le_range (ofMkLEMk _ _ h).hom (asHomRight T.subtype).hom
