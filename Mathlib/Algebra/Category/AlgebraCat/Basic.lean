...
instance : ConcreteCategory.{v} (AlgebraCat.{v} R) (fun X Y => X →ₐ[R] Y) where
  hom := Hom.hom'
  ofHom f := ⟨f⟩

variable {R} in
/-- Turn a morphism in `AlgebraCat` back into an `AlgebraHom`. -/
abbrev Hom.hom {A B : AlgebraCat.{v} R} (f : Hom A B) :=
  ConcreteCategory.hom (C := AlgebraCat R) f

variable {R} in
/-- Typecheck an `AlgebraHom` as a morphism in `AlgebraCat`. -/
abbrev ofHom {A B : Type v} [Ring A] [Algebra R A] [Ring B] [Algebra R B] (f : A →ₐ[R] B) :
    of R A ⟶ of R B :=
