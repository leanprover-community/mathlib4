...
instance forget₂_addCommGroup_full : (forget₂ (ModuleCat.{u} ℤ) AddCommGrpCat.{u}).Full where
  map_surjective {A B}
...
instance forget₂_addCommGrp_essSurj : (forget₂ (ModuleCat.{u} ℤ) AddCommGrpCat.{u}).EssSurj where
  mem_essImage A :=
    ⟨ModuleCat.of ℤ A,
      ⟨{  hom := 𝟙 A
          inv := 𝟙 A }⟩⟩
...
noncomputable instance forget₂AddCommGroupIsEquivalence :
    (forget₂ (ModuleCat.{u} ℤ) AddCommGrpCat.{u}).IsEquivalence where
