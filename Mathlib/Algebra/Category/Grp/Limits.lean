...
/-- A functor `F : J ⥤ Grp.{u}` has a limit iff `F ⋙ forget Grp` has a limit. -/
@[to_additive "A functor `F : J ⥤ AddGrp.{u}` has a limit iff
`F ⋙ forget AddGrp` has a limit."]
lemma hasLimit_iff_of_forget :
    HasLimit F ↔ HasLimit (F ⋙ forget Grp) := by
  rw [hasLimit_iff_small_sections, Types.hasLimit_iff_small_sections]
...
noncomputable instance forget_createsLimit :
  ...
  set e : forget Grp.{u} ≅ forget₂ Grp.{u} MonCat.{u} ⋙ forget MonCat.{u} :=
...
/-- A functor `F : J ⥤ CommGrp.{u}` has a limit iff `F ⋙ forget Grp` has a limit. -/
@[to_additive "A functor `F : J ⥤ AddCommGrp.{u}` has a limit iff
`F ⋙ forget AddGrp` has a limit."]
lemma hasLimit_iff_of_forget :
    HasLimit F ↔ HasLimit (F ⋙ forget CommGrp) := by
  rw [hasLimit_iff_small_sections, Types.hasLimit_iff_small_sections]
...
noncomputable instance forget_createsLimit :
  ...
  set e : forget CommGrp.{u} ≅ forget₂ CommGrp.{u} Grp.{u} ⋙ forget Grp.{u} :=
    NatIso.ofComponents (fun _ ↦ Iso.refl _) (fun _ ↦ rfl)
  exact createsLimitOfNatIso e.symm
