...
@[to_additive]
instance (priority := 10000) bundledHom : BundledHom @MulHom :=
...
instance (priority := 10000) instConcreteCategory : ConcreteCategory MagmaCat := BundledHom.concreteCategory MulHom

attribute [to_additive] instMagmaCatLargeCategory instConcreteCategory

@[to_additive]
instance (priority := 10000) : CoeSort MagmaCat (Type*) where
  coe X := X.α
...
@[to_additive]
instance (priority := 10000) (X : MagmaCat) : Mul X := X.str

@[to_additive]
instance (priority := 10000) instFunLike (X Y : MagmaCat) : FunLike (X ⟶ Y) X Y :=
  inferInstanceAs <| FunLike (X →ₙ* Y) X Y

@[to_additive]
instance (priority := 10000) instMulHomClass (X Y : MagmaCat) : MulHomClass (X ⟶ Y) X Y :=
...
@[to_additive]
instance (priority := 10000) : Inhabited MagmaCat :=
  ⟨MagmaCat.of PEmpty⟩
...
@[to_additive]
instance (priority := 10000) : BundledHom.ParentProjection @Semigroup.toMul := ⟨⟩
...
-- "default handlers have not been implemented yet"
-- https://github.com/leanprover-community/mathlib4/issues/5020
instance (priority := 10000) instConcreteCategory : ConcreteCategory SemigroupCat :=
...
@[to_additive]
instance (priority := 10000) : CoeSort SemigroupCat (Type*) where
  coe X := X.α
...
@[to_additive]
instance (priority := 10000) (X : SemigroupCat) : Semigroup X := X.str

@[to_additive]
instance (priority := 10000) instFunLike (X Y : SemigroupCat) : FunLike (X ⟶ Y) X Y :=
  inferInstanceAs <| FunLike (X →ₙ* Y) X Y

@[to_additive]
instance (priority := 10000) instMulHomClass (X Y : SemigroupCat) : MulHomClass (X ⟶ Y) X Y :=
  inferInstanceAs <| MulHomClass (X →ₙ* Y) X Y
...
@[to_additive]
instance (priority := 10000) : Inhabited SemigroupCat :=
  ⟨SemigroupCat.of PEmpty⟩

@[to_additive]
instance (priority := 10000) hasForgetToMagmaCat : HasForget₂ SemigroupCat MagmaCat :=
...
@[to_additive]
instance (priority := 10000) MagmaCat.forgetReflectsIsos : ReflectsIsomorphisms (forget MagmaCat.{u}) where
  reflects {X Y} f _ := by
...
@[to_additive]
instance (priority := 10000) SemigroupCat.forgetReflectsIsos : ReflectsIsomorphisms (forget SemigroupCat.{u}) where
  reflects {X Y} f _ := by
...
@[to_additive]
instance (priority := 10000) SemigroupCat.forget₂Full : Full (forget₂ SemigroupCat MagmaCat) where preimage f := f
