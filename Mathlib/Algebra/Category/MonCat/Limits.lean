...
noncomputable instance limitMonoid :
  ...
  inferInstaceAs% (Monoid (Shrink (F ⋙ forget MonCat.{u}).sections))
...
instance commMonoidObj (j) : CommMonoid ((F ⋙ forget CommMonCat.{u}).obj j) :=
  inferInstaceAs% (CommMonoid (F.obj j))
...
noncomputable instance limitCommMonoid :
  ...
  inferInstaceAs% (CommMonoid (Shrink (F ⋙ forget CommMonCat.{u}).sections))

@[to_additive]
instance : Small.{u} (Functor.sections ((F ⋙ forget₂ CommMonCat MonCat) ⋙ forget MonCat)) :=
  inferInstaceAs% (Small.{u} (Functor.sections (F ⋙ forget CommMonCat)))
