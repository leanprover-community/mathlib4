...
@[to_additive]
instance forget_preservesLimits : PreservesLimits (forget MonCat.{u}) :=
...
@[to_additive]
instance forget_createsLimits : CreatesLimits (forget MonCat.{u}) :=
...
and then reuse the existing limit. -/]
instance forget₂CreatesLimit : CreatesLimit F (forget₂ CommMonCat MonCat.{u}) :=
