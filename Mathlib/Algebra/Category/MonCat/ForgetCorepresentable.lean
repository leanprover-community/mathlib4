...
The forget functor `AddCommMonCat.{u} ⥤ Type u` is corepresentable
...
/-- The forget functor `MonCat.{u} ⥤ Type u` is corepresentable. -/
...
/-- The forget functor `CommMonCat.{u} ⥤ Type u` is corepresentable. -/
def CommMonCat.coyonedaObjIsoForget :
    coyoneda.obj (op (of (ULift.{u} (Multiplicative ℕ)))) ≅ forget CommMonCat.{u} :=
  (NatIso.ofComponents (fun M => (MonoidHom.fromULiftMultiplicativeNatEquiv M.α).toIso))

/-- The forget functor `AddMonCat.{u} ⥤ Type u` is corepresentable. -/
