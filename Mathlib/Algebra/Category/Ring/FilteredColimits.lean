...
def colimitCocone : Cocone F where
  pt := colimit.{v, u} F
  ι := let src := (SemiRingCat.FilteredColimits.colimitCocone
          (F ⋙ forget₂ CommSemiRingCat SemiRingCat.{max v u})).ι
...
def colimitCocone : Cocone F where
  pt := colimit.{v, u} F
  ι := let src :=  (SemiRingCat.FilteredColimits.colimitCocone
          (F ⋙ forget₂ RingCat SemiRingCat.{max v u})).ι
    { app := src.app, naturality := src.naturality }
set_option linter.uppercaseLean3 false in
...
def colimitCocone : Cocone F where
  pt := colimit.{v, u} F
  ι := let src := (RingCat.FilteredColimits.colimitCocone
    (F ⋙ forget₂ CommRingCat RingCat.{max v u})).ι
    { app := src.app, naturality := src.naturality }
set_option linter.uppercaseLean3 false in
