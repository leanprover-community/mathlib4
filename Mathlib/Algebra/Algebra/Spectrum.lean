...
lemma isUnit_resolvent {r : R} {a : A} : r ∈ resolventSet R a ↔ IsUnit (resolvent a r) :=
...
@[simp]
lemma zero_eq [Nontrivial A] : σ (0 : A) = {0} := by
...
lemma smul_eq_smul [Nontrivial A] (k : 𝕜) (a : A) (ha : (σ a).Nonempty) :
    σ (k • a) = k • σ a := by
