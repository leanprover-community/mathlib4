...
@[simp]
lemma mem_toNonUnitalSubring {S : NonUnitalSubalgebra R A} {x} :
    x ∈ S.toNonUnitalSubring ↔ x ∈ S :=
  Iff.rfl

@[simp]
lemma coe_toNonUnitalSubring (S : NonUnitalSubalgebra R A) :
    (↑S.toNonUnitalSubring : Set A) = S :=
  rfl

lemma toNonUnitalSubring_injective :
...
lemma toNonUnitalSubring_inj {S U : NonUnitalSubalgebra R A} :
...
@[simp]
lemma top_toSubring {R A : Type*} [CommRing R] [NonUnitalNonAssocRing A] [Module R A]
