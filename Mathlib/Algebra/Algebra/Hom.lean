...
@[simp]
protected lemma coe_coe {F : Type*} [FunLike F A B] [AlgHomClass F R A B] (f : F) :
    ⇑(f : A →ₐ[R] B) = f :=
  rfl
...
@[simp]
lemma coe_mk {f : A →+* B} (h) : ((⟨f, h⟩ : A →ₐ[R] B) : A → B) = f :=
  rfl

@[norm_cast]
lemma coe_mks {f : A → B} (h₁ h₂ h₃ h₄ h₅) : ⇑(⟨⟨⟨⟨f, h₁⟩, h₂⟩, h₃, h₄⟩, h₅⟩ : A →ₐ[R] B) = f :=
  rfl
#align alg_hom.coe_mk AlgHom.coe_mks

-- Porting note: This theorem is new.
@[simp, norm_cast]
lemma coe_ringHom_mk {f : A →+* B} (h) : ((⟨f, h⟩ : A →ₐ[R] B) : A →+* B) = f :=
  rfl
...
lemma coe_fn_injective : @Function.Injective (A →ₐ[R] B) (A → B) (↑) :=
  DFunLike.coe_injective
#align alg_hom.coe_fn_injective AlgHom.coe_fn_injective

lemma coe_fn_inj {φ₁ φ₂ : A →ₐ[R] B} : (φ₁ : A → B) = φ₂ ↔ φ₁ = φ₂ :=
  DFunLike.coe_fn_eq
#align alg_hom.coe_fn_inj AlgHom.coe_fn_inj

lemma coe_ringHom_injective : Function.Injective ((↑) : (A →ₐ[R] B) → A →+* B) := fun φ₁ φ₂ H =>
...
lemma coe_monoidHom_injective : Function.Injective ((↑) : (A →ₐ[R] B) → A →* B) :=
...
lemma coe_addMonoidHom_injective : Function.Injective ((↑) : (A →ₐ[R] B) → A →+ B) :=
...
@[ext]
lemma ext {φ₁ φ₂ : A →ₐ[R] B} (H : ∀ x, φ₁ x = φ₂ x) : φ₁ = φ₂ :=
  DFunLike.ext _ _ H
#align alg_hom.ext AlgHom.ext

lemma ext_iff {φ₁ φ₂ : A →ₐ[R] B} : φ₁ = φ₂ ↔ ∀ x, φ₁ x = φ₂ x :=
  DFunLike.ext_iff
#align alg_hom.ext_iff AlgHom.ext_iff

@[simp]
lemma mk_coe {f : A →ₐ[R] B} (h₁ h₂ h₃ h₄ h₅) : (⟨⟨⟨⟨f, h₁⟩, h₂⟩, h₃, h₄⟩, h₅⟩ : A →ₐ[R] B) = f :=
  ext fun _ => rfl
...
lemma comp_algebraMap : (φ : A →+* B).comp (algebraMap R A) = algebraMap R B :=
...
protected lemma map_zero : φ 0 = 0 :=
  map_zero _
...
protected lemma map_finsupp_sum {α : Type*} [Zero α] {ι : Type*} (f : ι →₀ α) (g : ι → α → A) :
...
@[simp]
lemma coe_id : ⇑(AlgHom.id R A) = id :=
  rfl
#align alg_hom.coe_id AlgHom.coe_id

@[simp]
lemma id_toRingHom : (AlgHom.id R A : A →+* A) = RingHom.id _ :=
  rfl
...
@[simp]
lemma comp_id : φ.comp (AlgHom.id R A) = φ :=
  ext fun _x => rfl
#align alg_hom.comp_id AlgHom.comp_id

@[simp]
lemma id_comp : (AlgHom.id R B).comp φ = φ :=
  ext fun _x => rfl
...
@[simp]
lemma toLinearMap_id : toLinearMap (AlgHom.id R A) = LinearMap.id :=
...
nonrec lemma map_list_prod (s : List A) : φ s.prod = (s.map φ).prod :=
  map_list_prod φ s
...
protected lemma map_finsupp_prod {α : Type*} [Zero α] {ι : Type*} (f : ι →₀ α) (g : ι → α → A) :
...
@[simp]
lemma toRatAlgHom_toRingHom [Ring R] [Ring S] [Algebra ℚ R] [Algebra ℚ S] (f : R →+* S) :
    ↑f.toRatAlgHom = f :=
...
@[simp]
lemma AlgHom.toRingHom_toRatAlgHom [Ring R] [Ring S] [Algebra ℚ R] [Algebra ℚ S]
...
@[simp]
lemma smul_units_def (f : A →ₐ[R] A) (x : Aˣ) :
...
lemma toAlgHom_injective [FaithfulSMul M A] :
