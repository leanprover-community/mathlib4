...
theorem End_algebraMap_isUnit_inv_apply_eq_iff {x : R}
  ...
  mp H := ((congr_arg h.unit H).symm.trans (End_isUnit_apply_inv_apply_of_isUnit h _)).symm
  mpr H :=
    H.symm ▸ by
...
theorem End_algebraMap_isUnit_inv_apply_eq_iff' {x : R}
  ...
  mp H := ((congr_arg h.unit H).trans (End_isUnit_apply_inv_apply_of_isUnit h _)).symm
  mpr H :=
    H.symm ▸ by
