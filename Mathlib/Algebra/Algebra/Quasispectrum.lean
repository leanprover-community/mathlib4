...
(quasi)spectrum. -/
class NonnegSpectrumClass (𝕜 A : Type*)
    [CommSemiring 𝕜] [OrderedCommSemiring 𝕜] [NonUnitalRing A] [PartialOrder A] [Module 𝕜 A] :
    Prop where
...
lemma iff_spectrum_nonneg {𝕜 A : Type*}
    [Semifield 𝕜] [LinearOrderedSemifield 𝕜] [Ring A] [PartialOrder A] [Algebra 𝕜 A] :
...
lemma spectrum_nonneg_of_nonneg {𝕜 A : Type*}
    [CommSemiring 𝕜] [OrderedCommSemiring 𝕜] [Ring A] [PartialOrder A] [Algebra 𝕜 A]
    [NonnegSpectrumClass 𝕜 A] ⦃a : A⦄ (ha : 0 ≤ a) ⦃x : 𝕜⦄ (hx : x ∈ spectrum 𝕜 a) :
    0 ≤ x :=
