...
scoped instance (x : α) : CoeAdjoinAux x {x} := ⟨mem_singleton_self x⟩
...
theorem mem_adjoin_of_map_mul {s} {x : A} {f : A →ₗ[R] B} (hf : ∀ a₁ a₂, f (a₁ * a₂) = f a₁ * f a₂)
  ...
  | algebraMap r =>
    have : f 1 ∈ adjoin R (f '' (s ∪ {1})) :=
      subset_adjoin ⟨1, ⟨Set.subset_union_right <| mem_singleton_self 1, rfl⟩⟩
...
theorem self_mem_adjoin_singleton (x : A) : x ∈ R[x] :=
  Algebra.subset_adjoin (mem_singleton)
