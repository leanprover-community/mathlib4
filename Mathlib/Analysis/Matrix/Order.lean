...
`‖x‖ = sqrt (x * M * xᴴ).trace`. -/
@[implicit_reducible]
noncomputable def toMatrixNormPseudoMetric (M : Matrix n n 𝕜) (hM : M.PosSemidef) :
    NormPseudoMetric (Matrix n n 𝕜) :=
  @InnerProductSpace.Core.toNormPseudoMetric _ _ _ _ _ hM.matrixPreInnerProductSpace

lemma toMatrixIsNormedAddGroup (M : Matrix n n 𝕜) (hM : M.PosSemidef) :
    letI := M.toMatrixNormPseudoMetric hM
    IsNormedAddGroup (Matrix n n 𝕜) :=
  @InnerProductSpace.Core.toIsNormedAddGroup _ _ _ _ _ hM.matrixPreInnerProductSpace

set_option linter.deprecated false in
/-- A positive definite matrix `M` induces a norm on `Matrix n n 𝕜`
`‖x‖ = sqrt (x * M * xᴴ).trace`. -/
@[deprecated toMatrixIsNormedAddGroup (since := "2026-05-17"), implicit_reducible]
noncomputable def toMatrixSeminormedAddCommGroup (M : Matrix n n 𝕜) (hM : M.PosSemidef) :
    SeminormedAddCommGroup (Matrix n n 𝕜) where
  toNormPseudoMetric := M.toMatrixNormPseudoMetric hM
  toIsNormedAddGroup := M.toMatrixIsNormedAddGroup hM
...
`‖x‖ = sqrt (x * M * xᴴ).trace`. -/
@[implicit_reducible]
noncomputable def toMatrixNormMetric (M : Matrix n n 𝕜) (hM : M.PosDef) :
    NormMetric (Matrix n n 𝕜) :=
...
noncomputable def toMatrixNormedAddCommGroup (M : Matrix n n 𝕜) (hM : M.PosDef) :
    NormedAddCommGroup (Matrix n n 𝕜) where
  toNormMetric := M.toMatrixNormMetric hM
  toIsNormedAddGroup := M.toMatrixIsNormedAddGroup hM.posSemidef
...
noncomputable def toMatrixInnerProductSpace (M : Matrix n n 𝕜) (hM : M.PosSemidef) :
    letI : NormPseudoMetric (Matrix n n 𝕜) := M.toMatrixNormPseudoMetric hM
    haveI : IsNormedAddGroup (Matrix n n 𝕜) := M.toMatrixIsNormedAddGroup hM
