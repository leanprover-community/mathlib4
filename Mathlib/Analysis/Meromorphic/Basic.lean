...
@[fun_prop]
lemma fun_add {f g : 𝕜 → E} (hf : MeromorphicAt f x) (hg : MeromorphicAt g x) :
...
@[fun_prop]
lemma fun_sub {f g : 𝕜 → E} (hf : MeromorphicAt f x) (hg : MeromorphicAt g x) :
    MeromorphicAt (fun z ↦ f z - g z) x :=
  hf.sub hg
...
@[simp]
lemma update_iff [DecidableEq 𝕜] {f : 𝕜 → E} {w : 𝕜} {e : E} :
    MeromorphicAt (Function.update f w e) x ↔ MeromorphicAt f x :=
  meromorphicAt_congr (Function.update_eventuallyEq_nhdsNE f w x e)

@[fun_prop]
lemma update [DecidableEq 𝕜] {f : 𝕜 → E} (hf : MeromorphicAt f x) (w e) :
    MeromorphicAt (Function.update f w e) x :=
  update_iff.mpr hf
...
@[fun_prop]
lemma fun_inv {f : 𝕜 → 𝕜} (hf : MeromorphicAt f x) :
    MeromorphicAt (fun z ↦ (f z)⁻¹) x := hf.inv
...
@[fun_prop]
lemma fun_div {f g : 𝕜 → 𝕜} (hf : MeromorphicAt f x) (hg : MeromorphicAt g x) :
    MeromorphicAt (fun z ↦ f z / g z) x :=
  hf.div hg
...
@[fun_prop]
lemma pow {f : 𝕜 → 𝕜} (hf : MeromorphicAt f x) (n : ℕ) :
    MeromorphicAt (f ^ n) x := by
  induction n with
  | zero => simpa only [pow_zero] using MeromorphicAt.const 1 x
  | succ m hm => simpa only [pow_succ] using hm.mul hf

@[fun_prop]
lemma fun_pow {f : 𝕜 → 𝕜} (hf : MeromorphicAt f x) (n : ℕ) :
...
@[fun_prop]
lemma zpow {f : 𝕜 → 𝕜} (hf : MeromorphicAt f x) (n : ℤ) :
    MeromorphicAt (f ^ n) x := by
  cases n with
...
theorem eventually_continuousAt {f : 𝕜 → E} (h : MeromorphicAt f x) :
...
theorem eventually_analyticAt [CompleteSpace E] {f : 𝕜 → E} (h : MeromorphicAt f x) :
...
lemma iff_eventuallyEq_zpow_smul_analyticAt {f : 𝕜 → E} :
    MeromorphicAt f x ↔
...
variable
  {ι : Type*} {I : Finset ι} {F : ι → 𝕜 → 𝕜} {G : ι → 𝕜 → E}
  {s t : 𝕜 → 𝕜} {f g : 𝕜 → E} {U : Set 𝕜}
...
/-- Finite products of meromorphic functions are meromorphic. -/
lemma prod (h : ∀ σ, MeromorphicOn (F σ) U) :
    MeromorphicOn (∏ i ∈ I, F i) U :=
  fun z hz ↦ MeromorphicAt.prod (fun σ ↦ h σ z hz)

/-- Finite products of meromorphic functions are meromorphic. -/
lemma fun_prod (h : ∀ σ, MeromorphicOn (F σ) U) :
    MeromorphicOn (fun z ↦ ∏ i ∈ I, F i z) U :=
  fun z hz ↦ MeromorphicAt.fun_prod (fun σ ↦ h σ z hz)

/-- Finite sums of meromorphic functions are meromorphic. -/
lemma sum (h : ∀ σ, MeromorphicOn (G σ) U) :
    MeromorphicOn (∑ i ∈ I, G i) U :=
  fun z hz ↦ MeromorphicAt.sum (fun σ ↦ h σ z hz)

/-- Finite sums of meromorphic functions are meromorphic. -/
lemma fun_sum (h : ∀ σ, MeromorphicOn (G σ) U) :
    MeromorphicOn (fun z ↦ ∑ i ∈ I, G i z) U :=
