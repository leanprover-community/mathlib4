...
@[simp]
lemma dslope_same (f : 𝕜 → E) (a : 𝕜) : dslope f a a = deriv f a :=
  update_same _ _ _
...
lemma ContinuousLinearMap.dslope_comp {F : Type*} [NormedAddCommGroup F] [NormedSpace 𝕜 F]
...
lemma eqOn_dslope_slope (f : 𝕜 → E) (a : 𝕜) : EqOn (dslope f a) (slope f a) {a}ᶜ := fun _ =>
  dslope_of_ne f
#align eq_on_dslope_slope eqOn_dslope_slope

lemma dslope_eventuallyEq_slope_of_ne (f : 𝕜 → E) (h : b ≠ a) : dslope f a =ᶠ[𝓝 b] slope f a :=
...
lemma dslope_eventuallyEq_slope_punctured_nhds (f : 𝕜 → E) : dslope f a =ᶠ[𝓝[≠] a] slope f a :=
...
@[simp]
lemma sub_smul_dslope (f : 𝕜 → E) (a b : 𝕜) : (b - a) • dslope f a b = f b - f a := by
  rcases eq_or_ne b a with (rfl | hne) <;> simp [dslope_of_ne, *]
#align sub_smul_dslope sub_smul_dslope

lemma dslope_sub_smul_of_ne (f : 𝕜 → E) (h : b ≠ a) :
...
@[simp]
lemma continuousAt_dslope_same : ContinuousAt (dslope f a) a ↔ DifferentiableAt 𝕜 f a := by
...
lemma ContinuousWithinAt.of_dslope (h : ContinuousWithinAt (dslope f a) s b) :
...
lemma ContinuousAt.of_dslope (h : ContinuousAt (dslope f a) b) : ContinuousAt f b :=
  (continuousWithinAt_univ _ _).1 h.continuousWithinAt.of_dslope
#align continuous_at.of_dslope ContinuousAt.of_dslope

lemma ContinuousOn.of_dslope (h : ContinuousOn (dslope f a) s) : ContinuousOn f s := fun x hx =>
  (h x hx).of_dslope
#align continuous_on.of_dslope ContinuousOn.of_dslope

lemma continuousWithinAt_dslope_of_ne (h : b ≠ a) :
...
lemma continuousAt_dslope_of_ne (h : b ≠ a) : ContinuousAt (dslope f a) b ↔ ContinuousAt f b := by
...
lemma continuousOn_dslope (h : s ∈ 𝓝 a) :
...
lemma DifferentiableWithinAt.of_dslope (h : DifferentiableWithinAt 𝕜 (dslope f a) s b) :
...
lemma DifferentiableAt.of_dslope (h : DifferentiableAt 𝕜 (dslope f a) b) :
...
lemma DifferentiableOn.of_dslope (h : DifferentiableOn 𝕜 (dslope f a) s) :
    DifferentiableOn 𝕜 f s := fun x hx => (h x hx).of_dslope
#align differentiable_on.of_dslope DifferentiableOn.of_dslope

lemma differentiableWithinAt_dslope_of_ne (h : b ≠ a) :
...
lemma differentiableOn_dslope_of_nmem (h : a ∉ s) :
...
lemma differentiableAt_dslope_of_ne (h : b ≠ a) :
