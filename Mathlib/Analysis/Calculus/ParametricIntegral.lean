...
theorem integrable_of_dominated_loc_of_lip' {F' : α → H →L[𝕜] E} (ε_pos : 0 < ε)
...
theorem hasFDerivAt_integral_of_dominated_loc_of_lip' {F' : α → H →L[𝕜] E} (ε_pos : 0 < ε)
    ...
    (bound_integrable : Integrable (bound : α → ℝ) μ)
    (h_diff : ∀ᵐ a ∂μ, HasFDerivAt (F · a) (F' a) x₀) :
    HasFDerivAt (fun x ↦ ∫ a, F x a ∂μ) (∫ a, F' a ∂μ) x₀ := by
  ...
  have hF'_int : Integrable F' μ :=
    integrable_of_dominated_loc_of_lip' (F := F) ε_pos hF'_meas h_lipsch b_int h_diff
...
theorem integrable_of_dominated_loc_of_lip {F' : α → H →L[𝕜] E}
  ...
  exact integrable_of_dominated_loc_of_lip' δ_pos hF'_meas h_lip bound_integrable h_diff
...
-- interval version of integrability lemma
theorem integrable_of_dominated_loc_of_lip_interval [NormedSpace ℝ H] {μ : Measure ℝ}
  ...
  let H₁ := integrable_of_dominated_loc_of_lip ε_pos hF_meas.1 hF'_meas.1 h_lip.1
    bound_integrable.1 h_diff.1
  let H₂ := integrable_of_dominated_loc_of_lip ε_pos hF_meas.2 hF'_meas.2 h_lip.2
...
-- Version of `integrable_of_dominated_loc_of_lip` for `f` defined on `ℝ` or `ℂ`.
-- TODO: extend this docstring
theorem integrable_of_dominated_loc_of_lip'' {F' : α → E} (ε_pos : 0 < ε)
    (hF_meas : ∀ᶠ x in 𝓝 x₀, AEStronglyMeasurable (F x) μ) (hF'_meas : AEStronglyMeasurable F' μ)
  ...
  have r := integrable_of_dominated_loc_of_lip ε_pos hF_meas hm h_lipsch bound_integrable h_diff
  rw [← integrable_norm_iff hm] at r
  simpa only [(· ∘ ·), integrable_norm_iff, hF'_meas, one_mul, norm_one,
    ...
    ContinuousLinearMap.norm_restrictScalars, ContinuousLinearMap.norm_smulRightL_apply] using r
...
theorem hasDerivAt_integral_of_dominated_loc_of_lip {F' : α → E} (ε_pos : 0 < ε)
    ...
    (bound_integrable : Integrable (bound : α → ℝ) μ)
    (h_diff : ∀ᵐ a ∂μ, HasDerivAt (F · a) (F' a) x₀) :
    HasDerivAt (fun x ↦ ∫ a, F x a ∂μ) (∫ a, F' a ∂μ) x₀ := by
  set L : E →L[𝕜] 𝕜 →L[𝕜] E := ContinuousLinearMap.smulRightL 𝕜 𝕜 E 1
  ...
  have hF'_int : Integrable F' μ := by
    ...
    simpa only [(· ∘ ·), integrable_norm_iff, hF'_meas, one_mul, norm_one,
      ContinuousLinearMap.comp_apply, ContinuousLinearMap.coe_restrict_scalarsL',
      ContinuousLinearMap.norm_restrictScalars, ContinuousLinearMap.norm_smulRightL_apply] using r
  let key := hasFDerivAt_integral_of_dominated_loc_of_lip ε_pos hF_meas hF_int hm h_lipsch
...
theorem integrable_of_dominated_loc_of_deriv_le (ε_pos : 0 < ε)
  ...
  have : ∀ᵐ a ∂μ, LipschitzOnWith (Real.nnabs (bound a)) (fun x : 𝕜 ↦ F x a) (ball x₀ ε) := by
    ...
    rw [← NNReal.coe_le_coe, coe_nnnorm, Real.coe_nnabs]
    exact (ha_bound x x_in).trans (le_abs_self _)
  exact integrable_of_dominated_loc_of_lip'' ε_pos hF_meas hF'_meas this bound_integrable diff_x₀
...
theorem hasDerivAt_integral_of_dominated_loc_of_deriv_le (ε_pos : 0 < ε)
  ...
  have : ∀ᵐ a ∂μ, LipschitzOnWith (Real.nnabs (bound a)) (fun x : 𝕜 ↦ F x a) (ball x₀ ε) := by
    ...
    rw [← NNReal.coe_le_coe, coe_nnnorm, Real.coe_nnabs]
    exact (ha_bound x x_in).trans (le_abs_self _)
  exact hasDerivAt_integral_of_dominated_loc_of_lip ε_pos hF_meas hF_int hF'_meas this
    bound_integrable diff_x₀
