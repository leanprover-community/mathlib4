...
theorem hasFDerivWithinAt_integral_of_dominated_loc_of_lip' {F' : α → H →L[𝕜] E} {s : Set H}
    ...
    (h_lipsch : ∀ᵐ a ∂μ, ∀ x ∈ ball x₀ ε, x ∈ s → ‖F x a - F x₀ a‖ ≤ bound a * ‖x - x₀‖)
    (bound_integrable : Integrable (bound : α → ℝ) μ)
    (h_diff : ∀ᵐ a ∂μ, HasFDerivWithinAt (F · a) (F' a) s x₀) :
    Integrable F' μ ∧ HasFDerivWithinAt (fun x ↦ ∫ a, F x a ∂μ) (∫ a, F' a ∂μ) s x₀ := by
  ...
  replace h_lipsch : ∀ᵐ a ∂μ, ∀ x ∈ ball x₀ ε, x ∈ s → ‖F x a - F x₀ a‖ ≤ b a * ‖x - x₀‖ :=
    h_lipsch.mono fun a ha x hx hxs ↦
      (ha x hx hxs).trans <| mul_le_mul_of_nonneg_right (le_abs_self _) (norm_nonneg _)
  have hF_int' : ∀ x ∈ ball x₀ ε, x ∈ s → Integrable (F x) μ := fun x x_in hxs ↦ by
    have : ∀ᵐ a ∂μ, ‖F x₀ a - F x a‖ ≤ ε * b a := by
      simp only [norm_sub_rev (F x₀ _)]
      refine h_lipsch.mono fun a ha ↦ (ha x x_in hxs).trans ?_
      rw [mul_comm ε]
      rw [mem_ball, dist_eq_norm] at x_in
      exact mul_le_mul_of_nonneg_left x_in.le (b_nonneg _)
    exact integrable_of_norm_sub_le (hF_meas x x_in hxs) hF_int
  ...
  · rcases subsingleton_or_nontrivial H with hH|hH
    · have : Subsingleton (H →L[𝕜] E) := inferInstance
      convert (hasFDerivAt_of_subsingleton _ x₀).hasFDerivWithinAt
  ...
  have h_ball : s ∩ ball x₀ ε ∈ 𝓝[s] x₀ := inter_mem_nhdsWithin _ (ball_mem_nhds x₀ ε_pos)
  have : ∀ᶠ x in 𝓝[s] x₀,
      ‖x - x₀‖⁻¹ * ‖((∫ a, F x a ∂μ) - ∫ a, F x₀ a ∂μ) - (∫ a, F' a ∂μ) (x - x₀)‖ =
      ‖∫ a, ‖x - x₀‖⁻¹ • (F x a - F x₀ a - F' a (x - x₀)) ∂μ‖ := by
    apply mem_of_superset h_ball
    rintro x ⟨hxs, x_in⟩; simp only
    ...
    exacts [hF_int' x x_in hxs, hF_int, (hF_int' x x_in hxs).sub hF_int,
      hF'_int.apply_continuousLinearMap _]
  rw [hasFDerivWithinAt_iff_tendsto, tendsto_congr' this, ← tendsto_zero_iff_norm_tendsto_zero, ←
    show (∫ a : α, ‖x₀ - x₀‖⁻¹ • (F x₀ a - F x₀ a - (F' a) (x₀ - x₀)) ∂μ) = 0 by simp]
  apply tendsto_integral_filter_of_dominated_convergence
  · filter_upwards [h_ball] with _ ⟨hxs, x_in⟩
    apply AEStronglyMeasurable.const_smul
    exact ((hF_meas _ x_in hxs).sub hF_int.1).sub (hF'_meas.apply_continuousLinearMap _)
  · refine mem_of_superset h_ball fun x ⟨hxs, hx⟩ ↦ ?_
    apply (h_diff.and h_lipsch).mono
    on_goal 1 => rintro a ⟨-, ha_bound⟩
    show ‖‖x - x₀‖⁻¹ • (F x a - F x₀ a - F' a (x - x₀))‖ ≤ b a + ‖F' a‖
    replace ha_bound : ‖F x a - F x₀ a‖ ≤ b a * ‖x - x₀‖ := ha_bound x hx hxs
    calc
  ...
  · apply h_diff.mono
    intro a ha
    suffices Tendsto (fun x ↦ ‖x - x₀‖⁻¹ • (F x a - F x₀ a - F' a (x - x₀))) (𝓝[s] x₀) (𝓝 0) by
      simpa
...
theorem hasFDerivAt_integral_of_dominated_loc_of_lip_interval {μ : Measure ℝ}
    {F : H → ℝ → E} {F' : ℝ → H →L[𝕜] E} {a b : ℝ} {bound : ℝ → ℝ} (ε_pos : 0 < ε)
...
derivative norm uniformly bounded by an integrable function (the ball radius is independent of `a`),
and `F x` is ae-measurable for `x` in a possibly smaller neighborhood of `x₀`. -/
theorem hasFDerivAt_integral_of_dominated_of_fderiv_le'' {μ : Measure ℝ}
    {F : H → ℝ → E} {F' : H → ℝ → H →L[𝕜] E} {a b : ℝ} {bound : ℝ → ℝ} (ε_pos : 0 < ε)
    (hF_meas : ∀ᶠ x in 𝓝 x₀, AEStronglyMeasurable (F x) <| μ.restrict (Ι a b))
    (hF_int : IntervalIntegrable (F x₀) μ a b)
