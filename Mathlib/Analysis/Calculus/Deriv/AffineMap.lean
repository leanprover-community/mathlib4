...
lemma hasStrictDerivAt : HasStrictDerivAt f (f.linear 1) x := by
  rw [f.decomp]
  exact f.linear.hasStrictDerivAt.add_const (f 0)

lemma hasDerivAtFilter : HasDerivAtFilter f (f.linear 1) x L := by
  rw [f.decomp]
...
@[simp] protected lemma deriv : deriv f x = f.linear 1 := f.hasDerivAt.deriv

protected lemma differentiableAt : DifferentiableAt 𝕜 f x := f.hasDerivAt.differentiableAt
protected lemma differentiable : Differentiable 𝕜 f := fun _ ↦ f.differentiableAt

protected lemma differentiableWithinAt : DifferentiableWithinAt 𝕜 f s x :=
  f.differentiableAt.differentiableWithinAt

protected lemma differentiableOn : DifferentiableOn 𝕜 f s := fun _ _ ↦ f.differentiableWithinAt
...
lemma hasStrictDerivAt_lineMap : HasStrictDerivAt (lineMap a b) (b - a) x := by
  simpa using (lineMap a b : 𝕜 →ᵃ[𝕜] E).hasStrictDerivAt

lemma hasDerivAt_lineMap :  HasDerivAt (lineMap a b) (b - a) x :=
  hasStrictDerivAt_lineMap.hasDerivAt

lemma hasDerivWithinAt_lineMap : HasDerivWithinAt (lineMap a b) (b - a) s x :=
