...
protected nonrec lemma HasDerivAtFilter.star (h : HasDerivAtFilter f f' x L) :
...
protected nonrec lemma HasDerivWithinAt.star (h : HasDerivWithinAt f f' s x) :
    HasDerivWithinAt (fun x => star (f x)) (star f') s x :=
  h.star
#align has_deriv_within_at.star HasDerivWithinAt.star

protected nonrec lemma HasDerivAt.star (h : HasDerivAt f f' x) :
    HasDerivAt (fun x => star (f x)) (star f') x :=
  h.star
#align has_deriv_at.star HasDerivAt.star

protected nonrec lemma HasStrictDerivAt.star (h : HasStrictDerivAt f f' x) :
...
protected lemma derivWithin.star (hxs : UniqueDiffWithinAt 𝕜 s x) :
