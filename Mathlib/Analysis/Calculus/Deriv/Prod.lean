...
lemma derivWithin_pi (h : ∀ i, DifferentiableWithinAt 𝕜 (fun x => φ x i) s x)
...
lemma deriv_pi (h : ∀ i, DifferentiableAt 𝕜 (fun x => φ x i) x) :
