...
theorem HasDerivWithinAt.eventually_ne (h : HasDerivWithinAt f f' s x) (hf' : f' ≠ 0) (c : F) :
    ∀ᶠ z in 𝓝[s \ {x}] x, f z ≠ c :=
  h.hasFDerivWithinAt.eventually_ne'
    ⟨‖f'‖⁻¹, fun z => by field_simp [norm_smul, mt norm_eq_zero.1 hf']⟩ c

theorem HasDerivAt.eventually_ne' (h : HasDerivAt f f' x) (hf' : f' ≠ 0) (c : F) :
    ∀ᶠ z in 𝓝[≠] x, f z ≠ c := by
  simpa only [compl_eq_univ_diff] using (hasDerivWithinAt_univ.2 h).eventually_ne hf' c

theorem HasDerivAt.eventually_ne_self (h : HasDerivAt f f' x) (hf' : f' ≠ 0) :
    ∀ᶠ z in 𝓝[≠] x, f z ≠ f x :=
  h.hasFDerivAt.eventually_ne_self
    ⟨‖f'‖⁻¹, fun z => by field_simp [norm_smul, mt norm_eq_zero.1 hf']⟩

@[deprecated (since := "2025-04-20")]
alias HasDerivAt.eventually_ne := HasDerivAt.eventually_ne_self
...
theorem derivWithin_zero_of_frequently_const {c} (h : ∃ᶠ y in 𝓝[s \ {x}] x, f y = c) :
  ...
  · contrapose h
    rw [not_frequently]
    exact hf.hasDerivWithinAt.eventually_ne h c
