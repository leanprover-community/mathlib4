...
lemma contDiff_normed {n : ℕ∞} : ContDiff ℝ n (f.normed μ) :=
...
lemma support_normed_eq : Function.support (f.normed μ) = Metric.ball c f.rOut := by
...
lemma tsupport_normed_eq : tsupport (f.normed μ) = Metric.closedBall c f.rOut := by
...
lemma hasCompactSupport_normed : HasCompactSupport (f.normed μ) := by
...
lemma tendsto_support_normed_smallSets {ι} {φ : ι → ContDiffBump c} {l : Filter ι}
...
lemma integral_normed_smul {X} [NormedAddCommGroup X] [NormedSpace ℝ X]
...
lemma measure_closedBall_le_integral : (μ (closedBall c f.rIn)).toReal ≤ ∫ x, f x ∂μ := by calc
...
lemma integral_le_measure_closedBall : ∫ x, f x ∂μ ≤ (μ (closedBall c f.rOut)).toReal := by calc
...
lemma measure_closedBall_div_le_integral [IsAddHaarMeasure μ] (K : ℝ) (h : f.rOut ≤ K * f.rIn) :
...
lemma normed_le_div_measure_closedBall_rOut [IsAddHaarMeasure μ] (K : ℝ) (h : f.rOut ≤ K * f.rIn)
    (x : E) :
