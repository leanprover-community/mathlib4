...
theorem ae_lineDeriv_sum_eq
  ...
  suffices S2 : ∫ x, (∑ i ∈ s, a i * fderiv ℝ g x (v i)) * f x ∂μ =
    ...
    obtain ⟨D, g_lip⟩ : ∃ D, LipschitzWith D g :=
      ContDiff.lipschitzWith_of_hasCompactSupport g_comp g_smooth (by exact_mod_cast le_top)
    simp_rw [integral_lineDeriv_mul_eq hf g_lip g_comp]
    simp_rw [(g_smooth.differentiable (by exact_mod_cast le_top))
  ...
  · exact ((g_smooth.continuous_fderiv (by exact_mod_cast le_top)).clm_apply
