...
def constFormalMultilinearSeries (𝕜 : Type*) [NontriviallyNormedField 𝕜] (E : Type*)
    [AddCommGroup E] [NormedAddCommGroup E] [NormedSpace 𝕜 E] [ContinuousConstSMul 𝕜 E]
    [IsTopologicalAddGroup E] {F : Type*} [AddCommGroup F] [NormedAddCommGroup F]
    [IsTopologicalAddGroup F] [NormedSpace 𝕜 F] [ContinuousConstSMul 𝕜 F] (c : F) :
    FormalMultilinearSeries 𝕜 E F
  | 0 => ContinuousMultilinearMap.uncurry0 _ _ c
  | _ => 0

@[simp]
theorem constFormalMultilinearSeries_apply_zero [NontriviallyNormedField 𝕜] [AddCommGroup E]
    [NormedAddCommGroup E] [AddCommGroup F] [NormedAddCommGroup F] [NormedSpace 𝕜 E]
    [NormedSpace 𝕜 F] {c : F} :
    constFormalMultilinearSeries 𝕜 E c 0 = ContinuousMultilinearMap.uncurry0 _ _ c :=
  rfl

@[simp]
theorem constFormalMultilinearSeries_apply_succ [NontriviallyNormedField 𝕜] [AddCommGroup E]
    [NormedAddCommGroup E] [AddCommGroup F] [NormedAddCommGroup F] [NormedSpace 𝕜 E]
    [NormedSpace 𝕜 F] {c : F} {n : ℕ} :
    constFormalMultilinearSeries 𝕜 E c (n + 1) = 0 :=
  rfl

theorem constFormalMultilinearSeries_apply_of_nonzero [NontriviallyNormedField 𝕜] [AddCommGroup E]
    [NormedAddCommGroup E] [AddCommGroup F] [NormedAddCommGroup F] [NormedSpace 𝕜 E]
    [NormedSpace 𝕜 F] {c : F} {n : ℕ} (hn : n ≠ 0) : constFormalMultilinearSeries 𝕜 E c n = 0 :=
  Nat.casesOn n (fun hn => (hn rfl).elim) (fun _ _ => rfl) hn

@[simp]
lemma constFormalMultilinearSeries_zero [NontriviallyNormedField 𝕜] [AddCommGroup E]
    [NormedAddCommGroup E] [AddCommGroup F] [NormedAddCommGroup F] [NormedSpace 𝕜 E]
    [NormedSpace 𝕜 F] :
