...
def implicitFunctionDataOfProdDomain
    (dfu : HasStrictFDerivAt f f'u u) (if₂u : (f'u ∘ᶠ .inr 𝕜 E₁ E₂).IsInvertible) :
  ...
  range_leftDeriv := by
    have : (f'u ∘ᶠ .inr 𝕜 E₁ E₂).range ≤ f'u.range := LinearMap.range_comp_le_range ..
...
theorem hasStrictFDerivAt_implicitFunctionOfProdDomain
    ...
    HasStrictFDerivAt (dfu.implicitFunctionOfProdDomain if₂u)
      (-(f'u ∘ᶠ .inr 𝕜 E₁ E₂).inverse ∘ᶠ (f'u ∘ᶠ .inl 𝕜 E₁ E₂)) u.1 := by
  suffices f'u ∘ᶠ (.prod (.id ..) (-(f'u ∘ᶠ .inr ..).inverse ∘ᶠ (f'u ∘ᶠ .inl ..))) = 0 from
