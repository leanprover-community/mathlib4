...
theorem isLittleO_sub_sub_fderiv
    ...
    (t : Set E := .univ) (seg : ∀ᶠ χ in l, [z χ -[ℝ] y χ] ⊆ t := by simp)
    (hy : Tendsto y l (𝓝 x)) (hz : Tendsto z l (𝓝 x)) (cf' : Tendsto ↿f' (l ×ˢ 𝓝[t] x) (𝓝 φ))
    (df' : ∀ᶠ p in l ×ˢ 𝓝[t] x, HasFDerivWithinAt (f p.1) (f' p.1 p.2) t p.2) :
  ...
  replace df' : ∀ᶠ χ in l, ∀ v ∈ [z χ -[ℝ] y χ], HasFDerivWithinAt (f χ) (f' χ v) t v :=
