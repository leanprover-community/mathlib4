...
variable {ι : Type*} {E : ι → Type*} [∀ i, AddCommGroup (E i)] [∀ i, NormedAddCommGroup (E i)]
  [∀ i, NormedSpace 𝕜 (E i)] [Fintype ι] (f : ContinuousMultilinearMap 𝕜 E F)
...
theorem _root_.HasStrictFDerivAt.continuousMultilinearMap_apply {G : Type*} [AddCommGroup G]
...
theorem _root_.HasFDerivWithinAt.continuousMultilinearMap_apply {G : Type*} [AddCommGroup G]
...
theorem _root_.HasFDerivAt.continuousMultilinearMap_apply {G : Type*} [AddCommGroup G]
    [NormedAddCommGroup G] [NormedSpace 𝕜 G] [DecidableEq ι] {x : G}
    {f : G → ContinuousMultilinearMap 𝕜 E F} {g : ∀ i, G → E i}
    {f' : G →L[𝕜] ContinuousMultilinearMap 𝕜 E F} {g' : ∀ i, G →L[𝕜] E i}
...
private theorem factorial_smul' {n : ℕ} : ∀ {F : Type max u v} [AddCommGroup F]
    [NormedAddCommGroup F] [NormedSpace 𝕜 F] [CompleteSpace F] {p : FormalMultilinearSeries 𝕜 E F}
    {f : E → F}, HasFPowerSeriesOnBall f p x r →
    n ! • p n (fun _ ↦ y) = iteratedFDeriv 𝕜 n f x (fun _ ↦ y) := by
  induction n with | zero => _ | succ n ih => _ <;> intro F _ _ _ _ p f h
...
variable {ι : Type*} {G : ι → Type*} [∀ i, AddCommGroup (G i)] [∀ i, NormedAddCommGroup (G i)]
  [∀ i, NormedSpace 𝕜 (G i)] [Fintype ι] {H : Type*} [AddCommGroup H] [NormedAddCommGroup H]
  [NormedSpace 𝕜 H]
