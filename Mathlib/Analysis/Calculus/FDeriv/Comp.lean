...
@[fun_prop]
theorem DifferentiableAt.fun_comp' {f : E → F} {g : F → G} (hg : DifferentiableAt K g (f x))
...
@[fun_prop]
theorem DifferentiableOn.fun_comp {g : F → G} {t : Set F} (hg : DifferentiableOn K g t)
...
@[fun_prop]
theorem Differentiable.fun_comp {g : F → G} (hg : Differentiable K g) (hf : Differentiable K f) :
