...
lemma HasFDerivAtFilter.comp {g : F → G} {g' : F →L[𝕜] G} {L' : Filter F}
...
lemma HasFDerivWithinAt.comp {g : F → G} {g' : F →L[𝕜] G} {t : Set F}
...
lemma HasFDerivAt.comp_hasFDerivWithinAt {g : F → G} {g' : F →L[𝕜] G}
...
lemma HasFDerivWithinAt.comp_of_mem {g : F → G} {g' : F →L[𝕜] G} {t : Set F}
...
/-- The chain rule. -/
lemma HasFDerivAt.comp {g : F → G} {g' : F →L[𝕜] G} (hg : HasFDerivAt g g' (f x))
...
lemma DifferentiableWithinAt.comp {g : F → G} {t : Set F}
...
lemma DifferentiableAt.comp {g : F → G} (hg : DifferentiableAt 𝕜 g (f x))
...
lemma DifferentiableAt.comp_differentiableWithinAt {g : F → G} (hg : DifferentiableAt 𝕜 g (f x))
...
lemma fderivWithin.comp {g : F → G} {t : Set F} (hg : DifferentiableWithinAt 𝕜 g t (f x))
...
lemma fderivWithin_fderivWithin {g : F → G} {f : E → F} {x : E} {y : F} {s : Set E} {t : Set F}
...
lemma fderivWithin.comp₃ {g' : G → G'} {g : F → G} {t : Set F} {u : Set G} {y : F} {y' : G}
...
lemma fderiv.comp {g : F → G} (hg : DifferentiableAt 𝕜 g (f x)) (hf : DifferentiableAt 𝕜 f x) :
    fderiv 𝕜 (g ∘ f) x = (fderiv 𝕜 g (f x)).comp (fderiv 𝕜 f x) :=
  (hg.hasFDerivAt.comp x hf.hasFDerivAt).fderiv
#align fderiv.comp fderiv.comp

lemma fderiv.comp_fderivWithin {g : F → G} (hg : DifferentiableAt 𝕜 g (f x))
...
lemma DifferentiableOn.comp {g : F → G} {t : Set F} (hg : DifferentiableOn 𝕜 g t)
...
lemma Differentiable.comp_differentiableOn {g : F → G} (hg : Differentiable 𝕜 g)
...
protected lemma HasStrictFDerivAt.comp {g : F → G} {g' : F →L[𝕜] G}
...
protected lemma DifferentiableOn.iterate {f : E → E} (hf : DifferentiableOn 𝕜 f s)
...
protected lemma HasFDerivAtFilter.iterate {f : E → E} {f' : E →L[𝕜] E}
...
protected lemma HasFDerivAt.iterate {f : E → E} {f' : E →L[𝕜] E} (hf : HasFDerivAt f f' x)
...
protected lemma HasFDerivWithinAt.iterate {f : E → E} {f' : E →L[𝕜] E}
...
protected lemma HasStrictFDerivAt.iterate {f : E → E} {f' : E →L[𝕜] E}
...
protected lemma DifferentiableAt.iterate {f : E → E} (hf : DifferentiableAt 𝕜 f x) (hx : f x = x)
...
protected lemma DifferentiableWithinAt.iterate {f : E → E} (hf : DifferentiableWithinAt 𝕜 f s x)
