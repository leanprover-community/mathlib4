...
protected lemma hasStrictFDerivAt : HasStrictFDerivAt iso (iso : E →L[𝕜] F) x :=
...
protected lemma hasFDerivWithinAt : HasFDerivWithinAt iso (iso : E →L[𝕜] F) s x :=
...
protected lemma differentiable : Differentiable 𝕜 iso := fun _ => iso.differentiableAt
#align continuous_linear_equiv.differentiable ContinuousLinearEquiv.differentiable

protected lemma differentiableOn : DifferentiableOn 𝕜 iso s :=
...
lemma comp_differentiableWithinAt_iff {f : G → E} {s : Set G} {x : G} :
...
lemma comp_differentiableAt_iff {f : G → E} {x : G} :
...
lemma comp_differentiableOn_iff {f : G → E} {s : Set G} :
...
lemma comp_differentiable_iff {f : G → E} : Differentiable 𝕜 (iso ∘ f) ↔ Differentiable 𝕜 f := by
...
lemma comp_hasFDerivWithinAt_iff {f : G → E} {s : Set G} {x : G} {f' : G →L[𝕜] E} :
...
lemma comp_hasStrictFDerivAt_iff {f : G → E} {x : G} {f' : G →L[𝕜] E} :
...
lemma comp_hasFDerivAt_iff {f : G → E} {x : G} {f' : G →L[𝕜] E} :
...
lemma comp_hasFDerivWithinAt_iff' {f : G → E} {s : Set G} {x : G} {f' : G →L[𝕜] F} :
...
lemma comp_hasFDerivAt_iff' {f : G → E} {x : G} {f' : G →L[𝕜] F} :
    HasFDerivAt (iso ∘ f) f' x ↔ HasFDerivAt f ((iso.symm : F →L[𝕜] E).comp f') x := by
  simp_rw [← hasFDerivWithinAt_univ, iso.comp_hasFDerivWithinAt_iff']
#align continuous_linear_equiv.comp_has_fderiv_at_iff' ContinuousLinearEquiv.comp_hasFDerivAt_iff'

lemma comp_fderivWithin {f : G → E} {s : Set G} {x : G} (hxs : UniqueDiffWithinAt 𝕜 s x) :
...
lemma comp_right_differentiableWithinAt_iff {f : F → G} {s : Set F} {x : E} :
...
lemma comp_right_differentiableAt_iff {f : F → G} {x : E} :
...
lemma comp_right_differentiableOn_iff {f : F → G} {s : Set F} :
...
lemma comp_right_hasFDerivWithinAt_iff {f : F → G} {s : Set F} {x : E} {f' : F →L[𝕜] G} :
...
lemma comp_right_hasFDerivAt_iff {f : F → G} {x : E} {f' : F →L[𝕜] G} :
...
lemma comp_right_hasFDerivWithinAt_iff' {f : F → G} {s : Set F} {x : E} {f' : E →L[𝕜] G} :
...
lemma comp_right_hasFDerivAt_iff' {f : F → G} {x : E} {f' : E →L[𝕜] G} :
...
lemma comp_right_fderivWithin {f : F → G} {s : Set F} {x : E}
...
lemma comp_differentiable_iff {f : G → E} : Differentiable 𝕜 (iso ∘ f) ↔ Differentiable 𝕜 f :=
...
inverse function. -/
lemma HasStrictFDerivAt.of_local_left_inverse {f : E → F} {f' : E ≃L[𝕜] F} {g : F → E} {a : F}
...
an inverse function. -/
lemma HasFDerivAt.of_local_left_inverse {f : E → F} {f' : E ≃L[𝕜] F} {g : F → E} {a : F}
...
lemma has_fderiv_at_filter_real_equiv {L : Filter E} :
...
the image. -/
lemma HasFDerivWithinAt.mapsTo_tangent_cone {x : E} (h : HasFDerivWithinAt f f' s x) :
...
-/
lemma HasFDerivWithinAt.uniqueDiffWithinAt {x : E} (h : HasFDerivWithinAt f f' s x)
...
lemma UniqueDiffOn.image {f' : E → E →L[𝕜] F} (hs : UniqueDiffOn 𝕜 s)
...
lemma HasFDerivWithinAt.uniqueDiffWithinAt_of_continuousLinearEquiv {x : E} (e' : E ≃L[𝕜] F)
