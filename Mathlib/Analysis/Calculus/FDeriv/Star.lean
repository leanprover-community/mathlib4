...
@[fun_prop]
lemma HasStrictFDerivAt.star (h : HasStrictFDerivAt f f' x) :
...
lemma HasFDerivAtFilter.star (h : HasFDerivAtFilter f f' x L) :
...
@[fun_prop]
lemma DifferentiableAt.star (h : DifferentiableAt 𝕜 f x) :
...
@[fun_prop]
lemma DifferentiableOn.star (h : DifferentiableOn 𝕜 f s) :
...
@[fun_prop]
lemma Differentiable.star (h : Differentiable 𝕜 f) : Differentiable 𝕜 fun y => star (f y) :=
  fun x => (h x).star
...
lemma fderivWithin_star (hxs : UniqueDiffWithinAt 𝕜 s x) :
