...
theorem DifferentiableWithinAt.fderivWithin_restrictScalars (hf : DifferentiableWithinAt 𝕜' f s x)
    (hs : UniqueDiffWithinAt 𝕜 s x) :
    fderivWithin 𝕜 f s x = (fderivWithin 𝕜' f s x).restrictScalars 𝕜 :=
  (hf.hasFDerivWithinAt.restrictScalars 𝕜).fderivWithin hs
