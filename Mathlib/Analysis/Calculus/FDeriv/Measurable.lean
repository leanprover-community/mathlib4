  ...
  calc
    ...
    _ ≤ ε * r + ε * r := by
      ...
      · apply le_of_mem_A h₂
        ...
        · simp only [dist_eq_norm, add_sub_eq_right, mem_closedBall, ylt.le]
      · apply le_of_mem_A h₁
        · simp only [le_of_lt (half_pos hr), mem_closedBall, dist_self]
        · simp only [dist_eq_norm, add_sub_eq_right, mem_closedBall, ylt.le]
    _ = 2 * ε * r := by ring
  ...
  have : HasFDerivAt f f' x := by
    ...
    have J1 : ‖f (x + y) - f x - L e (n e) m (x + y - x)‖ ≤ (1 / 2) ^ e * (1 / 2) ^ m := by
      ...
      · simp only [mem_closedBall, dist_self]
        positivity
      · simpa only [dist_eq_norm, add_sub_eq_right, mem_closedBall, pow_succ', mul_one_div] using
          h'k
    have J2 : ‖f (x + y) - f x - L e (n e) m y‖ ≤ 4 * (1 / 2) ^ e * ‖y‖ :=
      calc
        ‖f (x + y) - f x - L e (n e) m y‖ ≤ (1 / 2) ^ e * (1 / 2) ^ m := by
          simpa only [add_sub_eq_right] using J1
