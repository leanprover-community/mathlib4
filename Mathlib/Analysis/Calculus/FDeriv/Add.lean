...
@[fun_prop]
lemma HasStrictFDerivAt.const_smul (h : HasStrictFDerivAt f f' x) (c : R) :
...
lemma HasFDerivAtFilter.const_smul (h : HasFDerivAtFilter f f' x L) (c : R) :
...
@[fun_prop]
nonrec lemma HasFDerivWithinAt.const_smul (h : HasFDerivWithinAt f f' s x) (c : R) :
    HasFDerivWithinAt (fun x => c • f x) (c • f') s x :=
  h.const_smul c
#align has_fderiv_within_at.const_smul HasFDerivWithinAt.const_smul

@[fun_prop]
nonrec lemma HasFDerivAt.const_smul (h : HasFDerivAt f f' x) (c : R) :
    HasFDerivAt (fun x => c • f x) (c • f') x :=
  h.const_smul c
#align has_fderiv_at.const_smul HasFDerivAt.const_smul

@[fun_prop]
lemma DifferentiableWithinAt.const_smul (h : DifferentiableWithinAt 𝕜 f s x) (c : R) :
...
@[fun_prop]
lemma DifferentiableAt.const_smul (h : DifferentiableAt 𝕜 f x) (c : R) :
...
@[fun_prop]
lemma DifferentiableOn.const_smul (h : DifferentiableOn 𝕜 f s) (c : R) :
...
lemma fderivWithin_const_smul (hxs : UniqueDiffWithinAt 𝕜 s x)
...
lemma fderiv_const_smul (h : DifferentiableAt 𝕜 f x) (c : R) :
...
@[fun_prop]
nonrec lemma HasStrictFDerivAt.add (hf : HasStrictFDerivAt f f' x)
...
lemma HasFDerivAtFilter.add (hf : HasFDerivAtFilter f f' x L)
...
@[fun_prop]
nonrec lemma HasFDerivWithinAt.add (hf : HasFDerivWithinAt f f' s x)
    (hg : HasFDerivWithinAt g g' s x) : HasFDerivWithinAt (fun y => f y + g y) (f' + g') s x :=
  hf.add hg
#align has_fderiv_within_at.add HasFDerivWithinAt.add

@[fun_prop]
nonrec lemma HasFDerivAt.add (hf : HasFDerivAt f f' x) (hg : HasFDerivAt g g' x) :
    HasFDerivAt (fun x => f x + g x) (f' + g') x :=
  hf.add hg
#align has_fderiv_at.add HasFDerivAt.add

@[fun_prop]
lemma DifferentiableWithinAt.add (hf : DifferentiableWithinAt 𝕜 f s x)
...
@[simp, fun_prop]
lemma DifferentiableAt.add (hf : DifferentiableAt 𝕜 f x) (hg : DifferentiableAt 𝕜 g x) :
...
@[fun_prop]
lemma DifferentiableOn.add (hf : DifferentiableOn 𝕜 f s) (hg : DifferentiableOn 𝕜 g s) :
    DifferentiableOn 𝕜 (fun y => f y + g y) s := fun x hx => (hf x hx).add (hg x hx)
#align differentiable_on.add DifferentiableOn.add

@[simp, fun_prop]
lemma Differentiable.add (hf : Differentiable 𝕜 f) (hg : Differentiable 𝕜 g) :
    Differentiable 𝕜 fun y => f y + g y := fun x => (hf x).add (hg x)
#align differentiable.add Differentiable.add

lemma fderivWithin_add (hxs : UniqueDiffWithinAt 𝕜 s x) (hf : DifferentiableWithinAt 𝕜 f s x)
...
lemma fderiv_add (hf : DifferentiableAt 𝕜 f x) (hg : DifferentiableAt 𝕜 g x) :
    fderiv 𝕜 (fun y => f y + g y) x = fderiv 𝕜 f x + fderiv 𝕜 g x :=
  (hf.hasFDerivAt.add hg.hasFDerivAt).fderiv
#align fderiv_add fderiv_add

@[fun_prop]
lemma HasStrictFDerivAt.add_const (hf : HasStrictFDerivAt f f' x) (c : F) :
...
lemma HasFDerivAtFilter.add_const (hf : HasFDerivAtFilter f f' x L) (c : F) :
...
@[fun_prop]
nonrec lemma HasFDerivWithinAt.add_const (hf : HasFDerivWithinAt f f' s x) (c : F) :
    HasFDerivWithinAt (fun y => f y + c) f' s x :=
  hf.add_const c
#align has_fderiv_within_at.add_const HasFDerivWithinAt.add_const

@[fun_prop]
nonrec lemma HasFDerivAt.add_const (hf : HasFDerivAt f f' x) (c : F) :
    HasFDerivAt (fun x => f x + c) f' x :=
  hf.add_const c
#align has_fderiv_at.add_const HasFDerivAt.add_const

@[fun_prop]
lemma DifferentiableWithinAt.add_const (hf : DifferentiableWithinAt 𝕜 f s x) (c : F) :
...
@[simp]
lemma differentiableWithinAt_add_const_iff (c : F) :
...
@[fun_prop]
lemma DifferentiableAt.add_const (hf : DifferentiableAt 𝕜 f x) (c : F) :
...
@[simp]
lemma differentiableAt_add_const_iff (c : F) :
...
@[fun_prop]
lemma DifferentiableOn.add_const (hf : DifferentiableOn 𝕜 f s) (c : F) :
...
@[simp]
lemma differentiableOn_add_const_iff (c : F) :
...
lemma fderiv_add_const (c : F) : fderiv 𝕜 (fun y => f y + c) x = fderiv 𝕜 f x := by
  simp only [← fderivWithin_univ, fderivWithin_add_const uniqueDiffWithinAt_univ]
#align fderiv_add_const fderiv_add_const

@[fun_prop]
lemma HasStrictFDerivAt.const_add (hf : HasStrictFDerivAt f f' x) (c : F) :
...
lemma HasFDerivAtFilter.const_add (hf : HasFDerivAtFilter f f' x L) (c : F) :
...
@[fun_prop]
nonrec lemma HasFDerivWithinAt.const_add (hf : HasFDerivWithinAt f f' s x) (c : F) :
    HasFDerivWithinAt (fun y => c + f y) f' s x :=
  hf.const_add c
#align has_fderiv_within_at.const_add HasFDerivWithinAt.const_add

@[fun_prop]
nonrec lemma HasFDerivAt.const_add (hf : HasFDerivAt f f' x) (c : F) :
    HasFDerivAt (fun x => c + f x) f' x :=
  hf.const_add c
#align has_fderiv_at.const_add HasFDerivAt.const_add

@[fun_prop]
lemma DifferentiableWithinAt.const_add (hf : DifferentiableWithinAt 𝕜 f s x) (c : F) :
    DifferentiableWithinAt 𝕜 (fun y => c + f y) s x :=
...
@[simp]
lemma differentiableWithinAt_const_add_iff (c : F) :
...
@[fun_prop]
lemma DifferentiableOn.const_add (hf : DifferentiableOn 𝕜 f s) (c : F) :
...
@[simp]
lemma differentiableOn_const_add_iff (c : F) :
...
lemma fderiv_const_add (c : F) : fderiv 𝕜 (fun y => c + f y) x = fderiv 𝕜 f x := by
...
@[fun_prop]
lemma HasStrictFDerivAt.sum (h : ∀ i ∈ u, HasStrictFDerivAt (A i) (A' i) x) :
...
lemma HasFDerivAtFilter.sum (h : ∀ i ∈ u, HasFDerivAtFilter (A i) (A' i) x L) :
...
@[fun_prop]
lemma HasFDerivWithinAt.sum (h : ∀ i ∈ u, HasFDerivWithinAt (A i) (A' i) s x) :
...
@[fun_prop]
lemma HasFDerivAt.sum (h : ∀ i ∈ u, HasFDerivAt (A i) (A' i) x) :
    HasFDerivAt (fun y => ∑ i in u, A i y) (∑ i in u, A' i) x :=
  HasFDerivAtFilter.sum h
#align has_fderiv_at.sum HasFDerivAt.sum

@[fun_prop]
lemma DifferentiableWithinAt.sum (h : ∀ i ∈ u, DifferentiableWithinAt 𝕜 (A i) s x) :
...
@[simp, fun_prop]
lemma DifferentiableAt.sum (h : ∀ i ∈ u, DifferentiableAt 𝕜 (A i) x) :
...
@[fun_prop]
lemma DifferentiableOn.sum (h : ∀ i ∈ u, DifferentiableOn 𝕜 (A i) s) :
...
@[simp, fun_prop]
lemma Differentiable.sum (h : ∀ i ∈ u, Differentiable 𝕜 (A i)) :
...
lemma fderivWithin_sum (hxs : UniqueDiffWithinAt 𝕜 s x)
...
lemma fderiv_sum (h : ∀ i ∈ u, DifferentiableAt 𝕜 (A i) x) :
...
@[fun_prop]
nonrec lemma HasFDerivWithinAt.neg (h : HasFDerivWithinAt f f' s x) :
    HasFDerivWithinAt (fun x => -f x) (-f') s x :=
  h.neg
#align has_fderiv_within_at.neg HasFDerivWithinAt.neg

@[fun_prop]
nonrec lemma HasFDerivAt.neg (h : HasFDerivAt f f' x) : HasFDerivAt (fun x => -f x) (-f') x :=
  h.neg
#align has_fderiv_at.neg HasFDerivAt.neg

@[fun_prop]
lemma DifferentiableWithinAt.neg (h : DifferentiableWithinAt 𝕜 f s x) :
...
@[fun_prop]
lemma DifferentiableAt.neg (h : DifferentiableAt 𝕜 f x) : DifferentiableAt 𝕜 (fun y => -f y) x :=
...
@[fun_prop]
lemma DifferentiableOn.neg (h : DifferentiableOn 𝕜 f s) : DifferentiableOn 𝕜 (fun y => -f y) s :=
  fun x hx => (h x hx).neg
...
@[fun_prop]
lemma Differentiable.neg (h : Differentiable 𝕜 f) : Differentiable 𝕜 fun y => -f y := fun x =>
  (h x).neg
...
lemma fderivWithin_neg (hxs : UniqueDiffWithinAt 𝕜 s x) :
...
@[fun_prop]
lemma HasStrictFDerivAt.sub (hf : HasStrictFDerivAt f f' x) (hg : HasStrictFDerivAt g g' x) :
    HasStrictFDerivAt (fun x => f x - g x) (f' - g') x := by
  simpa only [sub_eq_add_neg] using hf.add hg.neg
#align has_strict_fderiv_at.sub HasStrictFDerivAt.sub

lemma HasFDerivAtFilter.sub (hf : HasFDerivAtFilter f f' x L) (hg : HasFDerivAtFilter g g' x L) :
    HasFDerivAtFilter (fun x => f x - g x) (f' - g') x L := by
  simpa only [sub_eq_add_neg] using hf.add hg.neg
#align has_fderiv_at_filter.sub HasFDerivAtFilter.sub

@[fun_prop]
nonrec lemma HasFDerivWithinAt.sub (hf : HasFDerivWithinAt f f' s x)
    (hg : HasFDerivWithinAt g g' s x) : HasFDerivWithinAt (fun x => f x - g x) (f' - g') s x :=
  hf.sub hg
#align has_fderiv_within_at.sub HasFDerivWithinAt.sub

@[fun_prop]
nonrec lemma HasFDerivAt.sub (hf : HasFDerivAt f f' x) (hg : HasFDerivAt g g' x) :
    HasFDerivAt (fun x => f x - g x) (f' - g') x :=
  hf.sub hg
#align has_fderiv_at.sub HasFDerivAt.sub

@[fun_prop]
lemma DifferentiableWithinAt.sub (hf : DifferentiableWithinAt 𝕜 f s x)
    (hg : DifferentiableWithinAt 𝕜 g s x) : DifferentiableWithinAt 𝕜 (fun y => f y - g y) s x :=
...
@[simp]
lemma differentiableWithinAt_sub_const_iff (c : F) :
...
@[fun_prop]
lemma DifferentiableOn.sub_const (hf : DifferentiableOn 𝕜 f s) (c : F) :
...
@[simp]
lemma differentiableOn_sub_const_iff (c : F) :
...
@[simp]
lemma differentiable_sub_const_iff (c : F) :
...
lemma fderivWithin_sub_const (hxs : UniqueDiffWithinAt 𝕜 s x) (c : F) :
...
@[simp]
lemma differentiableWithinAt_const_sub_iff (c : F) :
...
@[fun_prop]
lemma DifferentiableOn.const_sub (hf : DifferentiableOn 𝕜 f s) (c : F) :
...
@[simp]
lemma differentiableOn_const_sub_iff (c : F) :
...
@[simp]
lemma differentiable_const_sub_iff (c : F) :
...
lemma fderivWithin_const_sub (hxs : UniqueDiffWithinAt 𝕜 s x) (c : F) :
    fderivWithin 𝕜 (fun y => c - f y) s x = -fderivWithin 𝕜 f s x := by
  simp only [sub_eq_add_neg, fderivWithin_const_add, fderivWithin_neg, hxs]
#align fderiv_within_const_sub fderivWithin_const_sub

lemma fderiv_const_sub (c : F) : fderiv 𝕜 (fun y => c - f y) x = -fderiv 𝕜 f x := by
