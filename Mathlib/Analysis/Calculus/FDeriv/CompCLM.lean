...
variable {ι : Type*} [Fintype ι] {c : E → E [⋀^ι]→L[𝕜] F} {c' : E →L[𝕜] (E [⋀^ι]→L[𝕜] F)}

@[fun_prop]
theorem HasStrictFDerivAt.continuousAlternatingMap_apply_const (hc : HasStrictFDerivAt c c' x)
    (u : ι → E) : HasStrictFDerivAt (c · u) (c'.flipAlternating u) x :=
  (ContinuousAlternatingMap.apply 𝕜 E F u).hasStrictFDerivAt.comp x hc
...
theorem HasFDerivWithinAt.continuousAlternatingMap_apply_const (hc : HasFDerivWithinAt c c' s x)
  ...
  (ContinuousAlternatingMap.apply 𝕜 E F u).hasFDerivAt.comp_hasFDerivWithinAt x hc

@[fun_prop]
theorem HasFDerivAt.continuousAlternatingMap_apply_const (hc : HasFDerivAt c c' x) (u : ι → E) :
    HasFDerivAt (fun y ↦ (c y) u) (c'.flipAlternating u) x :=
  (ContinuousAlternatingMap.apply 𝕜 E F u).hasFDerivAt.comp x hc

@[fun_prop]
theorem DifferentiableWithinAt.continuousAlternatingMap_apply_const
    (hc : DifferentiableWithinAt 𝕜 c s x) (u : ι → E) :
    DifferentiableWithinAt 𝕜 (fun y ↦ (c y) u) s x :=
...
theorem DifferentiableOn.continuousAlternatingMap_apply_const (hc : DifferentiableOn 𝕜 c s)
    (u : ι → E) : DifferentiableOn 𝕜 (fun y ↦ (c y) u) s :=
  fun x hx ↦ (hc x hx).continuousAlternatingMap_apply_const u

@[fun_prop]
theorem Differentiable.continuousAlternatingMap_apply_const (hc : Differentiable 𝕜 c) (u : ι → E) :
...
theorem fderiv_continuousAlternatingMap_apply_const (hc : DifferentiableAt 𝕜 c x) (u : ι → E) :
...
theorem fderivWithin_continuousAlternatingMap_apply_const_apply (hxs : UniqueDiffWithinAt 𝕜 s x)
    (hc : DifferentiableWithinAt 𝕜 c s x) (u : ι → E) (m : E) :
    (fderivWithin 𝕜 (fun y ↦ (c y) u) s x) m = (fderivWithin 𝕜 c s x) m u := by
