    ...
    calc
      ...
      _ ≤ ε * ‖h • v + (t * h) • w‖ * ‖h • w‖ := by
        ...
        have H : x + h • v + (t * h) • w ∈ Metric.ball x δ ∩ interior s := by
          ...
          exact I.trans_lt hδ
        simpa only [mem_setOf_eq, add_assoc x, add_sub_eq_right] using sδ H
      _ ≤ ε * (‖h • v‖ + ‖h • w‖) * ‖h • w‖ := by
        gcongr
...
theorem Convex.isLittleO_alternate_sum_square {v w : E} (h4v : x + (4 : ℝ) • v ∈ interior s)
  ...
  have h2v : x + (2 : ℝ) • v ∈ interior s := by
    ...
    simp only [smul_smul, one_div, add_sub_eq_right, add_right_inj]
    norm_num
  ...
  have hvw : x + (v + w) ∈ interior s := by
    ...
    simp only [smul_smul, one_div, add_sub_eq_right, add_right_inj, smul_add, smul_sub]
    norm_num
    abel
  ...
  have hvww : x + (v + w) + w ∈ interior s := by
    ...
    rw [one_div, add_sub_add_right_eq_sub, add_sub_eq_right, inv_smul_smul₀ two_ne_zero, two_smul]
    abel
