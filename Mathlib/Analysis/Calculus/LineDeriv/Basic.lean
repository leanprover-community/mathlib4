...
theorem hasLineDerivWithinAt_congr_set (h : s.toPred =ᶠ[𝓝 x] t.toPred) :
  ...
  have : s.toPred =ᶠ[𝓝 (F 0)] t.toPred := by convert h; simp [F]
  exact B.preimage_mem_nhds this

theorem lineDifferentiableWithinAt_congr_set (h : s.toPred =ᶠ[𝓝 x] t.toPred) :
...
theorem lineDerivWithin_congr_set (h : s.toPred =ᶠ[𝓝 x] t.toPred) :
