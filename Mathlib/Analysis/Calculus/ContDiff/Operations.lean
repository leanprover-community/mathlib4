...
theorem contDiffAt_map_inverse [CompleteSpace E] (e : E ≃L[𝕜] F) :
  ...
  nontriviality E
  -- first, we use the lemma `to_ringInverse` to rewrite in terms of `Ring.inverse` in the ring
  -- `E →L[𝕜] E`
  let O₁ : (E →L[𝕜] E) → F →L[𝕜] E := fun f => f.comp (e.symm : F →L[𝕜] E)
  let O₂ : (E →L[𝕜] F) → E →L[𝕜] E := fun f => (e.symm : F →L[𝕜] E).comp f
  have : ContinuousLinearMap.inverse = O₁ ∘ Ring.inverse ∘ O₂ := funext (to_ringInverse e)
  rw [this]
