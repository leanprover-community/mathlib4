...
theorem isLocalMin_of_deriv_deriv_pos
  ...
  obtain ⟨b,hb⟩ := hp.2
  apply isLocalMin_of_deriv_Ioo hζ₀ hζ₁ hc
  · exact fun x hx => DifferentiableAt.differentiableWithinAt <| hb.2.symm.subset
      ⟨hlu.2 ⟨lt_trans (by linarith[min_le_left δ ε, min_le_left (x₀ - l) (u - x₀)]) hx.1,
        lt_trans hx.2 hlu.1.2⟩, hb.1 <| ne_of_lt hx.2⟩
  · exact fun x hx => DifferentiableAt.differentiableWithinAt <| hb.2.symm.subset
      ⟨hlu.2 ⟨lt_trans hlu.1.1 hx.1, lt_trans hx.2 (by
        linarith[min_le_left δ ε, min_le_right (x₀ - l) (u - x₀)])⟩, hb.1 <| ne_of_gt hx.1⟩
  · exact (fun x hx => le_of_lt <| hε.2.1 _ ⟨by simp only [mem_Ioo] at hx;linarith, hx.2⟩)
  · exact (fun x hx => le_of_lt <| hε.2.2 _ ⟨hx.1, by simp only [mem_Ioo] at hx;linarith⟩)
