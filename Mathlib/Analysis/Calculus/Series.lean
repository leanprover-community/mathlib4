...
theorem tendstoUniformlyOn_tsum {f : α → β → F} (hu : Summable u) {s : Set β}
    ...
    TendstoUniformlyOn (fun t : Finset α => fun x => ∑ n in t, f n x)
...
theorem tendstoUniformlyOn_tsum_nat {f : ℕ → β → F} {u : ℕ → ℝ} (hu : Summable u) {s : Set β}
    (hfu : ∀ n x, x ∈ s → ‖f n x‖ ≤ u n) :
    TendstoUniformlyOn (fun N => fun x => ∑ n in Finset.range N, f n x)
...
theorem tendstoUniformly_tsum_nat {f : ℕ → β → F} {u : ℕ → ℝ} (hu : Summable u)
    (hfu : ∀ n x, ‖f n x‖ ≤ u n) : TendstoUniformly (fun N => fun x => ∑ n in Finset.range N, f n x)
      (fun x => ∑' n, f n x) atTop :=
...
theorem summable_of_summable_hasFDerivAt_of_isPreconnected (hu : Summable u) (hs : IsOpen s)
  ...
  apply cauchy_map_of_uniformCauchySeqOn_fderiv hs h's A (fun t y hy => ?_) hx₀ hx (by simpa only)
...
theorem hasFDerivAt_tsum_of_isPreconnected (hu : Summable u) (hs : IsOpen s)
  ...
  classical
    ...
    apply hasFDerivAt_of_tendstoUniformlyOn hs
      (tendstoUniformlyOn_tsum hu hf') (fun t y hy => ?_) A _ hx
...
theorem summable_of_summable_hasFDerivAt (hu : Summable u)
  ...
  apply summable_of_summable_hasFDerivAt_of_isPreconnected hu isOpen_univ
    isConnected_univ.isPreconnected (fun n x _ => hf n x) (fun n x _ => hf' n x) (mem_univ _)
    hf0 (mem_univ _)
...
theorem hasFDerivAt_tsum (hu : Summable u) (hf : ∀ n x, HasFDerivAt (f n) (f' n x) x)
  ...
  let : NormedSpace ℝ E; exact NormedSpace.restrictScalars ℝ 𝕜 _
  exact hasFDerivAt_tsum_of_isPreconnected hu isOpen_univ isConnected_univ.isPreconnected
...
theorem fderiv_tsum (hu : Summable u) (hf : ∀ n, Differentiable 𝕜 (f n))
  ...
  ext1 x; exact fderiv_tsum_apply hu hf hf' hf0 x
...
theorem iteratedFDeriv_tsum (hf : ∀ i, ContDiff 𝕜 N (f i))
  ...
  · have h'k : (k : ℕ∞) < N := lt_of_lt_of_le (WithTop.coe_lt_coe.2 (Nat.lt_succ_self _)) hk
    ...
    · intro n x
      simpa [iteratedFDeriv_succ_eq_comp_left, LinearIsometryEquiv.norm_map] using
        h'f k.succ n x hk
...
theorem contDiff_tsum_of_eventually (hf : ∀ i, ContDiff 𝕜 N (f i))
    (hv : ∀ k : ℕ, (k : ℕ∞) ≤ N → Summable (v k)) (h'f : ∀ k : ℕ, (k : ℕ∞) ≤ N →
  ...
  classical
    ...
    have : (fun x => ∑' i, f i x) =
        (fun x => ∑ i in T, f i x) + fun x => ∑' i : { i // i ∉ T }, f i x := by
      ext1 x
