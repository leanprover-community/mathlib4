...
theorem probability (n : ℕ) (x : I) : (∑ k : Fin (n + 1), bernstein n k x) = 1 := by
  ...
  simp? [AlgHom.map_sum, Finset.sum_range] at this says simp only [Finset.sum_range, map_sum, Polynomial.coe_aeval_eq_eval, map_one] at this
  exact this
...
theorem variance {n : ℕ} (h : 0 < (n : ℝ)) (x : I) :
  ...
  apply_fun fun p => Polynomial.aeval (x : ℝ) p at this
  simp? [AlgHom.map_sum, Finset.sum_range, ← Polynomial.nat_cast_mul] at this says simp only [nsmul_eq_mul, Finset.sum_range, map_sum, map_mul, map_pow, map_sub, map_natCast,
