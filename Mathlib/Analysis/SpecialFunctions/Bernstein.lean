...
theorem variance {n : ℕ} (h : 0 < (n : ℝ)) (x : I) :
  ...
  apply_fun fun p => Polynomial.aeval (x : ℝ) p at this
  simp only [nsmul_eq_mul, Finset.sum_range, map_sum, MonoidHom.coe_mk, OneHom.coe_mk, map_mul,
    map_pow, map_sub, map_natCast, Polynomial.aeval_X, Polynomial.coe_aeval_eq_eval, map_one] at this
