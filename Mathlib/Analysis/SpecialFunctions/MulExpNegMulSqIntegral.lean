...
theorem integrable_mulExpNegMulSq_comp (f : C(E, ℝ)) {ε : ℝ} (hε : ε > 0) :
    Integrable (mulExpNegMulSq ε ∘ f) P := by
    apply integrable P
      ⟨⟨mulExpNegMulSq ε ∘ f, continuous_mulExpNegMulSq.comp f.continuous⟩, ⟨2 * (sqrt ε)⁻¹, _⟩⟩
...
theorem integrable_mulExpNegMulSq_comp_restrict_compact {K : Set E} (hK : IsCompact K)
    (hKmeas : MeasurableSet K) (g : C(E, ℝ)) {ε : ℝ} :
...
-/
theorem tendsto_integral_one_plus_inv_mul (g : E →ᵇ ℝ)
    {ε : ℝ} (hε : ε > 0) :
...
theorem integral_mulExpNegMulSq_comp_eq {P' : Measure E} [IsFiniteMeasure P']
    ...
    (heq : ∀ g ∈ A, ∫ x, (g : E → ℝ) x ∂P = ∫ x, (g : E → ℝ) x ∂P') :
    ∀ (g : A), ∫ x, (mulExpNegMulSq ε ∘ g) x ∂P
  ...
  apply tendsto_nhds_unique (tendsto_integral_one_plus_inv_mul P (mkOfBound g C h) hε) limP
...
theorem abs_integral_sub_setIntegral_mulExpNegMulSq_comp_lt (f : C(E, ℝ))
  ...
  apply lt_of_le_of_lt (norm_integral_sub_setIntegral_le
    ...
    (integrable_mulExpNegMulSq_comp P f hε))
...
theorem abs_setIntegral_mulExpNegMulSq_comp_sub_le_mul_measure {K : Set E} (hK : IsCompact K)
    ...
    |∫ x in K, (mulExpNegMulSq ε ∘ g) x ∂P - ∫ x in K, (mulExpNegMulSq ε ∘ f) x ∂P|
      ≤ δ * (P K).toReal := by
  rw [← (integral_sub (integrable_mulExpNegMulSq_comp_restrict_compact P hK hKmeas g)
      (integrable_mulExpNegMulSq_comp_restrict_compact P hK hKmeas f))]
