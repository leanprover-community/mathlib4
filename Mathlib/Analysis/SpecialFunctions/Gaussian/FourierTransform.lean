...
theorem integrable_cexp_neg_mul_sq_norm_add_of_euclideanSpace
  ...
  have := EuclideanSpace.measurePreserving_measurableOfLp ι
  ...
  simp only [EuclideanSpace.measurableOfLp, MeasurableEquiv.symm_mk, MeasurableEquiv.coe_mk,
    EuclideanSpace.norm_eq, PiLp.toLp_apply, Real.norm_eq_abs, sq_abs, PiLp.inner_apply,
...
theorem integral_cexp_neg_mul_sq_norm_add_of_euclideanSpace
  ...
  have := (EuclideanSpace.measurePreserving_measurableOfLp ι).symm
  ...
  · simp only [EuclideanSpace.measurableOfLp, MeasurableEquiv.symm_mk, MeasurableEquiv.coe_mk,
    ...
    rw [sq_sqrt]
    exact Finset.sum_nonneg (fun i _hi ↦ by positivity)
  · simp [PiLp.inner_apply, EuclideanSpace.measurableOfLp, Finset.mul_sum, mul_assoc]
    simp_rw [mul_comm]
