...
lemma toCircle_apply_mk (x : ℝ) : @toCircle T x = expMapCircle (2 * π / T * x) :=
  rfl
...
lemma homeomorphCircle'_apply_mk (x : ℝ) : homeomorphCircle' x = expMapCircle x :=
  rfl
...
lemma homeomorphCircle_apply (hT : T ≠ 0) (x : AddCircle T) :
