...
`a`. See `norm_circleMap_zero_sub_sq'` for the half-angle form. -/
...
/-- Law of cosines for the chord from a point of the circle `circleMap 0 r` to an arbitrary point
`a`, half-angle form. For `r = 1` and `a = 1`, see also `Complex.norm_exp_I_mul_ofReal_sub_one`. -/
theorem norm_circleMap_zero_sub_sq' (r θ : ℝ) (a : ℂ) :
    ‖circleMap 0 r θ - a‖ ^ 2
...
lemma eq_of_circleMap_eq {a b R : ℝ} {c : ℂ} (h_R : R ≠ 0) (h_dist : |a - b| < 2 * π)
  ...
  simp only [hn, Int.cast_mul, Int.cast_ofNat, mul_assoc, add_sub_cancel_left, abs_mul,
    Nat.abs_ofNat, abs_pi] at h_dist
