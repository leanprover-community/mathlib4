...
lemma tendsto_mul_log_one_add_of_tendsto {g : ℝ → ℝ} {t : ℝ}
  ...
  `IsOrderBornology.cobounded_eq` was marked `@[simp]`, so didn't have to be passed explicitly here.
  Now the `simpNF` linter complains about it. -/
  have hg0 := tendsto_zero_of_isBoundedUnder_smul_of_tendsto_cobounded
    hg.norm.isBoundedUnder_le (tendsto_id'.mpr (by simp [IsOrderBornology.cobounded_eq]))
