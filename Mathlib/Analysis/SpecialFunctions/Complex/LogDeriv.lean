...
lemma hasStrictDerivAt_log {x : ℂ} (h : x ∈ slitPlane) : HasStrictDerivAt log x⁻¹ x :=
...
lemma hasStrictFDerivAt_log_real {x : ℂ} (h : x ∈ slitPlane) :
...
lemma contDiffAt_log {x : ℂ} (h : x ∈ slitPlane) {n : ℕ∞} : ContDiffAt ℂ n log x :=
