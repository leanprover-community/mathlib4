...
* `Complex.sin_series_bound`, `Complex.cos_series_bound`, `Real.sin_series_bound` and
  ...
  they generalise.
* `Real.sin_cos_bound_of_pos` brackets `sin x` and `cos x` strictly between consecutive partial
  sums of their Taylor series, at every order and for every `x > 0`, with no bound on `x`.
...
/-- For `x > 0`, `sin x` and `cos x` lie strictly between consecutive partial sums of their
Taylor series, at every order.

Unlike the remainder bounds above this needs no bound on `x`: the alternating series brackets the
value for every positive argument. The low-order consequences are collected below. -/
theorem Real.sin_cos_bound_of_pos (x : ℝ) (hx : 0 < x) (n : ℕ) :
  ...
  have Hstep_cos_sin (n : ℕ) (ih : ∀ x > 0, cos x < cosSeries n x) (x : ℝ) (hx : 0 < x) :
    ...
    · simp only [sinSeries, Pi.sub_def]
      exact continuousOn_sin.sub <| continuousOn_finsetSum _ fun _ _ ↦ by fun_prop
...
theorem cos_lt_one_sub_sq_div_two_add_pow_four {x : ℝ} (hx : x ≠ 0) :
  ...
  have h := (sin_cos_bound_of_pos |x| (abs_pos.2 hx) 0).2.2.2
...
theorem sin_lt_sub_cube_add_pow_five {x : ℝ} (hx : 0 < x) :
  ...
  have h := (sin_cos_bound_of_pos x hx 1).2.1
