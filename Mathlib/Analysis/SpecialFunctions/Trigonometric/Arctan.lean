...
lemma tan_ne_zero_iff {θ : ℝ} : tan θ ≠ 0 ↔ ∀ k : ℤ, θ ≠ k * π / 2 := by
...
lemma tan_eq_zero_iff {θ : ℝ} : tan θ = 0 ↔ ∃ k : ℤ, θ = k * π / 2 := by
...
lemma arccos_eq_arctan {x : ℝ} (h : 0 < x) : arccos x = arctan (sqrt (1 - x ^ 2) / x) := by
  rw [arccos, eq_comm]
...
@[simp]
lemma coe_tanLocalHomeomorph : ⇑tanLocalHomeomorph = tan :=
  rfl
#align real.coe_tan_local_homeomorph Real.coe_tanLocalHomeomorph

@[simp]
lemma coe_tanLocalHomeomorph_symm : ⇑tanLocalHomeomorph.symm = arctan :=
  rfl
