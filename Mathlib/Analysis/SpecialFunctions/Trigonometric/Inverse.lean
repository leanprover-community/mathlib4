...
@[simp]
lemma range_arcsin : range arcsin = Icc (-(π / 2)) (π / 2) := by
...
lemma sin_arcsin' {x : ℝ} (hx : x ∈ Icc (-1 : ℝ) 1) : sin (arcsin x) = x := by
...
lemma strictMonoOn_arcsin : StrictMonoOn arcsin (Icc (-1) 1) :=
...
lemma injOn_arcsin : InjOn arcsin (Icc (-1) 1) :=
...
lemma continuousAt_arcsin {x : ℝ} : ContinuousAt arcsin x :=
...
lemma arcsin_of_le_neg_one {x : ℝ} (hx : x ≤ -1) : arcsin x = -(π / 2) := by
...
@[simp]
lemma arcsin_lt_pi_div_two {x : ℝ} : arcsin x < π / 2 ↔ x < 1 :=
...
@[simp]
lemma neg_pi_div_two_lt_arcsin {x : ℝ} : -(π / 2) < arcsin x ↔ -1 < x :=
...
@[simp]
lemma arcsin_eq_pi_div_two {x : ℝ} : arcsin x = π / 2 ↔ 1 ≤ x :=
...
@[simp]
lemma pi_div_four_le_arcsin {x} : π / 4 ≤ arcsin x ↔ √2 / 2 ≤ x := by
  rw [← sin_pi_div_four, le_arcsin_iff_sin_le']
  have := pi_pos
  constructor <;> linarith
#align real.pi_div_four_le_arcsin Real.pi_div_four_le_arcsin

lemma mapsTo_sin_Ioo : MapsTo sin (Ioo (-(π / 2)) (π / 2)) (Ioo (-1) 1) := fun x h => by
...
lemma cos_arcsin (x : ℝ) : cos (arcsin x) = √(1 - x ^ 2) := by
...
-- The junk values for `arcsin` and `sqrt` make this true even outside `[-1, 1]`.
lemma tan_arcsin (x : ℝ) : tan (arcsin x) = x / √(1 - x ^ 2) := by
...
@[simp]
lemma arccos_pos {x : ℝ} : 0 < arccos x ↔ x < 1 := by simp [arccos]
#align real.arccos_pos Real.arccos_pos

lemma cos_arccos {x : ℝ} (hx₁ : -1 ≤ x) (hx₂ : x ≤ 1) : cos (arccos x) = x := by
  rw [arccos, cos_pi_div_two_sub, sin_arcsin hx₁ hx₂]
#align real.cos_arccos Real.cos_arccos

lemma arccos_cos {x : ℝ} (hx₁ : 0 ≤ x) (hx₂ : x ≤ π) : arccos (cos x) = x := by
...
lemma strictAntiOn_arccos : StrictAntiOn arccos (Icc (-1) 1) := fun _ hx _ hy h =>
  sub_lt_sub_left (strictMonoOn_arcsin hx hy h) _
#align real.strict_anti_on_arccos Real.strictAntiOn_arccos

lemma arccos_injOn : InjOn arccos (Icc (-1) 1) :=
...
@[simp]
lemma arccos_zero : arccos 0 = π / 2 := by simp [arccos]
#align real.arccos_zero Real.arccos_zero

@[simp]
lemma arccos_one : arccos 1 = 0 := by simp [arccos]
#align real.arccos_one Real.arccos_one

@[simp]
lemma arccos_neg_one : arccos (-1) = π := by simp [arccos, add_halves]
#align real.arccos_neg_one Real.arccos_neg_one

@[simp]
lemma arccos_eq_zero {x} : arccos x = 0 ↔ 1 ≤ x := by simp [arccos, sub_eq_zero]
#align real.arccos_eq_zero Real.arccos_eq_zero

@[simp]
lemma arccos_eq_pi_div_two {x} : arccos x = π / 2 ↔ x = 0 := by simp [arccos]
#align real.arccos_eq_pi_div_two Real.arccos_eq_pi_div_two

@[simp]
lemma arccos_eq_pi {x} : arccos x = π ↔ x ≤ -1 := by
...
lemma arccos_of_one_le {x : ℝ} (hx : 1 ≤ x) : arccos x = 0 := by
  rw [arccos, arcsin_of_one_le hx, sub_self]
#align real.arccos_of_one_le Real.arccos_of_one_le

lemma arccos_of_le_neg_one {x : ℝ} (hx : x ≤ -1) : arccos x = π := by
...
lemma sin_arccos (x : ℝ) : sin (arccos x) = √(1 - x ^ 2) := by
  by_cases hx₁ : -1 ≤ x; swap
  · rw [not_le] at hx₁
...
@[simp]
lemma arccos_le_pi_div_two {x} : arccos x ≤ π / 2 ↔ 0 ≤ x := by simp [arccos]
#align real.arccos_le_pi_div_two Real.arccos_le_pi_div_two

@[simp]
lemma arccos_lt_pi_div_two {x : ℝ} : arccos x < π / 2 ↔ 0 < x := by simp [arccos]
#align real.arccos_lt_pi_div_two Real.arccos_lt_pi_div_two

@[simp]
lemma arccos_le_pi_div_four {x} : arccos x ≤ π / 4 ↔ √2 / 2 ≤ x := by
...
lemma arcsin_eq_arccos {x : ℝ} (h : 0 ≤ x) : arcsin x = arccos (√(1 - x ^ 2)) := by
