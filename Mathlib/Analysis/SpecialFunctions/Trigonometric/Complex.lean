...
theorem tan_eq_zero_iff {θ : ℂ} : tan θ = 0 ↔ ∃ k : ℤ, k * π / 2 = θ := by
  ...
  simp [field, mul_comm, eq_comm, -eqComm]
    ...
    _ ↔ (∃ k : ℤ, y = 2 * k * π + x) ∨ ∃ k : ℤ, y = 2 * k * π - x := by
      apply or_congr <;>
        simp [field, sin_eq_zero_iff, eq_sub_iff_add_eq',
          sub_eq_iff_eq_add, mul_comm (2 : ℂ), mul_right_comm _ (2 : ℂ), -eqComm]
...
theorem sin_eq_sin_iff {x y : ℂ} :
  ...
  refine exists_congr fun k => or_congr ?_ ?_ <;> refine Eq.congr rfl ?_ <;>
...
theorem cos_eq_iff_quadratic {z w : ℂ} :
  ...
  rw [← sub_eq_zero]
  simpa [field, cos, exp_neg, -eqComm] using Eq.congr (by ring) rfl
