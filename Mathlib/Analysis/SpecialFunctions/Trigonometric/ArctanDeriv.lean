...
lemma hasStrictDerivAt_tan {x : ℝ} (h : cos x ≠ 0) : HasStrictDerivAt tan (1 / cos x ^ 2) x :=
...
lemma hasDerivAt_tan {x : ℝ} (h : cos x ≠ 0) : HasDerivAt tan (1 / cos x ^ 2) x :=
...
@[simp]
lemma deriv_tan (x : ℝ) : deriv tan x = 1 / cos x ^ 2 :=
...
lemma hasStrictDerivAt_arctan (x : ℝ) : HasStrictDerivAt arctan (1 / (1 + x ^ 2)) x := by
...
lemma hasDerivAt_arctan (x : ℝ) : HasDerivAt arctan (1 / (1 + x ^ 2)) x :=
  (hasStrictDerivAt_arctan x).hasDerivAt
#align real.has_deriv_at_arctan Real.hasDerivAt_arctan

lemma hasDerivAt_arctan' (x : ℝ) : HasDerivAt arctan (1 + x ^ 2)⁻¹ x :=
  one_div (1 + x ^ 2) ▸ hasDerivAt_arctan x

lemma differentiableAt_arctan (x : ℝ) : DifferentiableAt ℝ arctan x :=
...
lemma HasStrictDerivAt.arctan (hf : HasStrictDerivAt f f' x) :
...
lemma derivWithin_arctan (hf : DifferentiableWithinAt ℝ f s x) (hxs : UniqueDiffWithinAt ℝ s x) :
...
@[simp]
lemma deriv_arctan (hc : DifferentiableAt ℝ f x) :
...
lemma HasStrictFDerivAt.arctan (hf : HasStrictFDerivAt f f' x) :
...
lemma HasFDerivAt.arctan (hf : HasFDerivAt f f' x) :
...
lemma HasFDerivWithinAt.arctan (hf : HasFDerivWithinAt f f' s x) :
...
lemma fderivWithin_arctan (hf : DifferentiableWithinAt ℝ f s x) (hxs : UniqueDiffWithinAt ℝ s x) :
...
@[simp]
lemma fderiv_arctan (hc : DifferentiableAt ℝ f x) :
    fderiv ℝ (fun x => arctan (f x)) x = (1 / (1 + f x ^ 2)) • fderiv ℝ f x :=
  hc.hasFDerivAt.arctan.fderiv
#align fderiv_arctan fderiv_arctan

lemma DifferentiableWithinAt.arctan (hf : DifferentiableWithinAt ℝ f s x) :
...
@[simp]
lemma DifferentiableAt.arctan (hc : DifferentiableAt ℝ f x) :
...
lemma DifferentiableOn.arctan (hc : DifferentiableOn ℝ f s) :
    DifferentiableOn ℝ (fun x => arctan (f x)) s := fun x h => (hc x h).arctan
#align differentiable_on.arctan DifferentiableOn.arctan

@[simp]
lemma Differentiable.arctan (hc : Differentiable ℝ f) : Differentiable ℝ fun x => arctan (f x) :=
  fun x => (hc x).arctan
#align differentiable.arctan Differentiable.arctan

lemma ContDiffAt.arctan (h : ContDiffAt ℝ n f x) : ContDiffAt ℝ n (fun x => arctan (f x)) x :=
  contDiff_arctan.contDiffAt.comp x h
#align cont_diff_at.arctan ContDiffAt.arctan

lemma ContDiff.arctan (h : ContDiff ℝ n f) : ContDiff ℝ n fun x => arctan (f x) :=
  contDiff_arctan.comp h
#align cont_diff.arctan ContDiff.arctan

lemma ContDiffWithinAt.arctan (h : ContDiffWithinAt ℝ n f s x) :
...
lemma ContDiffOn.arctan (h : ContDiffOn ℝ n f s) : ContDiffOn ℝ n (fun x => arctan (f x)) s :=
