...
theorem lintegral_rpow_eq_lintegral_meas_le_mul_enorm
    {f : α → ℝ≥0∞} (hf : AEMeasurable f μ) {p : ℝ} (p_pos : 0 < p) :
    ∫⁻ ω, f ω ^ p ∂μ =
      ENNReal.ofReal p *
        ∫⁻ t in Ioi (0 : ℝ), μ {a : α | ENNReal.ofReal t ≤ f a} * ENNReal.ofReal (t ^ (p - 1)) := by
  ...
  sorry /- TODO: generalise the lemma below!
  have key := lintegral_comp_eq_lintegral_meas_le_mul μ hf g_intble g_nn
