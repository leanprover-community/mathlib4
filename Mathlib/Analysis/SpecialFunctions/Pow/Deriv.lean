...
theorem hasDerivAt_ofReal_cpow {x : ℝ} (hx : x ≠ 0) {r : ℂ} (hr : r ≠ -1) :
  ...
  · -- easy case : `0 < x`
    ...
    · exact (mul_div_eq_left₀ _ hr).symm
    · convert HasDerivAt.cpow_const ?_ ?_ using 1
      · rw [add_sub_eq_left, mul_comm]; exact (mul_one _).symm
  ...
  · -- harder case : `x < 0`
    ...
    suffices HasDerivAt (fun y : ℝ => (-↑y) ^ (r + 1) * exp (↑π * I * (r + 1)))
      ...
      conv_rhs => rw [mul_assoc, mul_comm, mul_div_eq_left₀ _ hr]
    ...
    convert HasDerivAt.cpow_const ?_ ?_ using 1
    · rw [add_sub_eq_left, add_sub_eq_left]; exact (mul_one _).symm
