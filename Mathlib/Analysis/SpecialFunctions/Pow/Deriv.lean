...
variable {E : Type*} [AddCommGroup E] [NormedAddCommGroup E] [NormedSpace ℂ E] {f g : E → ℂ}
  {f' g' : StrongDual ℂ E} {x : E} {s : Set E} {c : ℂ}
...
theorem hasDerivAt_ofReal_cpow_const' {x : ℝ} (hx : x ≠ 0) {r : ℂ} (hr : r ≠ -1) :
  ...
  · -- harder case : `x < 0`
    ...
    suffices HasDerivAt (fun y : ℝ => (y : ℂ) ^ (r + 1)) ((r + 1) * ↑(-x) ^ r) (-x) by
      convert! @HasDerivAt.scomp ℝ _ ℂ _ _ _ x ℝ _ _ _ _ _ _ _ _ this (hasDerivAt_neg x) using 1
...
variable {E : Type*} [AddCommGroup E] [NormedAddCommGroup E] [NormedSpace ℝ E] {f g : E → ℝ}
  {f' g' : StrongDual ℝ E} {x : E} {s : Set E} {c p : ℝ} {n : WithTop ℕ∞}
