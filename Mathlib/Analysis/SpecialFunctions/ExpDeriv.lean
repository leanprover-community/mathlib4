...
lemma hasDerivAt_exp (x : ℂ) : HasDerivAt exp (exp x) x := by
...
lemma hasStrictDerivAt_exp (x : ℂ) : HasStrictDerivAt exp (exp x) x :=
...
lemma hasStrictFDerivAt_exp_real (x : ℂ) : HasStrictFDerivAt exp (exp x • (1 : ℂ →L[ℝ] ℂ)) x :=
...
lemma HasStrictDerivAt.cexp (hf : HasStrictDerivAt f f' x) :
...
lemma HasDerivAt.cexp (hf : HasDerivAt f f' x) :
...
lemma HasDerivWithinAt.cexp (hf : HasDerivWithinAt f f' s x) :
...
lemma derivWithin_cexp (hf : DifferentiableWithinAt 𝕜 f s x) (hxs : UniqueDiffWithinAt 𝕜 s x) :
...
@[simp]
lemma deriv_cexp (hc : DifferentiableAt 𝕜 f x) :
...
lemma HasStrictFDerivAt.cexp (hf : HasStrictFDerivAt f f' x) :
...
lemma HasFDerivWithinAt.cexp (hf : HasFDerivWithinAt f f' s x) :
...
lemma HasFDerivAt.cexp (hf : HasFDerivAt f f' x) :
...
lemma DifferentiableWithinAt.cexp (hf : DifferentiableWithinAt 𝕜 f s x) :
...
@[simp]
lemma DifferentiableAt.cexp (hc : DifferentiableAt 𝕜 f x) :
...
lemma hasStrictDerivAt_exp (x : ℝ) : HasStrictDerivAt exp (exp x) x :=
  (Complex.hasStrictDerivAt_exp x).real_of_complex
#align real.has_strict_deriv_at_exp Real.hasStrictDerivAt_exp

lemma hasDerivAt_exp (x : ℝ) : HasDerivAt exp (exp x) x :=
...
lemma HasStrictDerivAt.exp (hf : HasStrictDerivAt f f' x) :
...
lemma derivWithin_exp (hf : DifferentiableWithinAt ℝ f s x) (hxs : UniqueDiffWithinAt ℝ s x) :
...
@[simp]
lemma deriv_exp (hc : DifferentiableAt ℝ f x) :
...
lemma HasFDerivWithinAt.exp (hf : HasFDerivWithinAt f f' s x) :
...
lemma HasFDerivAt.exp (hf : HasFDerivAt f f' x) :
...
lemma HasStrictFDerivAt.exp (hf : HasStrictFDerivAt f f' x) :
...
lemma DifferentiableWithinAt.exp (hf : DifferentiableWithinAt ℝ f s x) :
...
@[simp]
lemma DifferentiableAt.exp (hc : DifferentiableAt ℝ f x) :
...
lemma DifferentiableOn.exp (hc : DifferentiableOn ℝ f s) :
    DifferentiableOn ℝ (fun x => Real.exp (f x)) s := fun x h => (hc x h).exp
#align differentiable_on.exp DifferentiableOn.exp

@[simp]
lemma Differentiable.exp (hc : Differentiable ℝ f) : Differentiable ℝ fun x => Real.exp (f x) :=
  fun x => (hc x).exp
#align differentiable.exp Differentiable.exp

lemma fderivWithin_exp (hf : DifferentiableWithinAt ℝ f s x) (hxs : UniqueDiffWithinAt ℝ s x) :
...
@[simp]
lemma fderiv_exp (hc : DifferentiableAt ℝ f x) :
