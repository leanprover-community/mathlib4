...
/-
Heartbeat-heavy: the proof that `R x → 0` as `x → ∞` is a long automation-driven argument
(integrals + inequalities). We scope the increased heartbeat limit to that lemma, rather than to
the main theorem `log_Gamma_real_eq`.
-/
set_option maxHeartbeats 320000 in
-- The proof of `tendsto_R_atTop` is large and automation-heavy; we scope the increased heartbeat
-- limit to this lemma to keep `log_Gamma_real_eq` free of heartbeat overrides.
...
/-- The integral representation gives the bound `‖Complex.Gamma z‖ ≤ Real.Gamma z.re`
for `0 < z.re`.

Key input: `‖(t : ℂ) ^ (z - 1)‖ = t ^ (z.re - 1)` for `0 < t`. -/
...
/-- Combined bound: for `z.re ∈ Icc 1 2`, we have `‖Complex.Gamma z‖ ≤ 1`. -/
