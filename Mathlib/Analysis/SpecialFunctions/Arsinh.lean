...
lemma exp_arsinh (x : ℝ) : exp (arsinh x) = x + √(1 + x ^ 2) := by
  apply exp_log
...
@[simp]
lemma cosh_arsinh (x : ℝ) : cosh (arsinh x) = √(1 + x ^ 2) := by
...
lemma hasStrictDerivAt_arsinh (x : ℝ) : HasStrictDerivAt arsinh (√(1 + x ^ 2))⁻¹ x := by
...
lemma hasDerivAt_arsinh (x : ℝ) : HasDerivAt arsinh (√(1 + x ^ 2))⁻¹ x :=
