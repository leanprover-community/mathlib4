...
lemma exp_arsinh (x : ℝ) : exp (arsinh x) = x + sqrt (1 + x ^ 2) := by
  apply exp_log
...
@[simp]
lemma arsinh_neg (x : ℝ) : arsinh (-x) = -arsinh x := by
...
lemma hasStrictDerivAt_arsinh (x : ℝ) : HasStrictDerivAt arsinh (sqrt (1 + x ^ 2))⁻¹ x := by
...
lemma hasDerivAt_arsinh (x : ℝ) : HasDerivAt arsinh (sqrt (1 + x ^ 2))⁻¹ x :=
...
nonrec lemma ContinuousAt.arsinh (h : ContinuousAt f a) :
    ContinuousAt (fun x => arsinh (f x)) a :=
  h.arsinh
#align continuous_at.arsinh ContinuousAt.arsinh

nonrec lemma ContinuousWithinAt.arsinh (h : ContinuousWithinAt f s a) :
    ContinuousWithinAt (fun x => arsinh (f x)) s a :=
  h.arsinh
#align continuous_within_at.arsinh ContinuousWithinAt.arsinh

lemma ContinuousOn.arsinh (h : ContinuousOn f s) : ContinuousOn (fun x => arsinh (f x)) s :=
  fun x hx => (h x hx).arsinh
#align continuous_on.arsinh ContinuousOn.arsinh

lemma Continuous.arsinh (h : Continuous f) : Continuous fun x => arsinh (f x) :=
...
lemma HasStrictFDerivAt.arsinh (hf : HasStrictFDerivAt f f' a) :
...
lemma HasFDerivAt.arsinh (hf : HasFDerivAt f f' a) :
...
lemma HasFDerivWithinAt.arsinh (hf : HasFDerivWithinAt f f' s a) :
...
lemma DifferentiableAt.arsinh (h : DifferentiableAt ℝ f a) :
...
lemma DifferentiableWithinAt.arsinh (h : DifferentiableWithinAt ℝ f s a) :
...
lemma DifferentiableOn.arsinh (h : DifferentiableOn ℝ f s) :
    DifferentiableOn ℝ (fun x => arsinh (f x)) s := fun x hx => (h x hx).arsinh
#align differentiable_on.arsinh DifferentiableOn.arsinh

lemma Differentiable.arsinh (h : Differentiable ℝ f) : Differentiable ℝ fun x => arsinh (f x) :=
  differentiable_arsinh.comp h
#align differentiable.arsinh Differentiable.arsinh

lemma ContDiffAt.arsinh (h : ContDiffAt ℝ n f a) : ContDiffAt ℝ n (fun x => arsinh (f x)) a :=
  contDiff_arsinh.contDiffAt.comp a h
#align cont_diff_at.arsinh ContDiffAt.arsinh

lemma ContDiffWithinAt.arsinh (h : ContDiffWithinAt ℝ n f s a) :
...
lemma ContDiff.arsinh (h : ContDiff ℝ n f) : ContDiff ℝ n fun x => arsinh (f x) :=
  contDiff_arsinh.comp h
#align cont_diff.arsinh ContDiff.arsinh

lemma ContDiffOn.arsinh (h : ContDiffOn ℝ n f s) : ContDiffOn ℝ n (fun x => arsinh (f x)) s :=
  fun x hx => (h x hx).arsinh
...
lemma HasStrictDerivAt.arsinh (hf : HasStrictDerivAt f f' a) :
