...
/-- Composing the exponential curve generated by `x` with a continuous linear map differentiates
by applying that map to `x * exp (t • x)`. -/
theorem ContinuousLinearMap.hasDerivAt_exp_smul_const {E : Type*} [NormedAddCommGroup E]
    [NormedSpace 𝕂 E] (L : 𝔸 →L[𝕂] E) (x : 𝔸) (t : 𝕂) :
    HasDerivAt (fun u : 𝕂 ↦ L (exp (u • x))) (L (x * exp (t • x))) t :=
  L.hasFDerivAt.comp_hasDerivAt t (hasDerivAt_exp_smul_const' x t)

/-- The exponential curve composed with a multiplicative map that is Fréchet differentiable at
`1` has derivative `L x * φ (exp (t • x))` at `t`. -/
theorem MonoidHom.hasDerivAt_exp_smul_const [NormedAlgebra ℚ 𝔸] (φ : 𝔸 →* 𝕂)
    (L : 𝔸 →L[𝕂] 𝕂) (hφ : HasFDerivAt φ L 1) (x : 𝔸) (t : 𝕂) :
    HasDerivAt (fun t ↦ φ (exp (t • x))) (L x * φ (exp (t • x))) t := by
  have : HasDerivAt (fun t ↦ φ (exp (t • x))) (L x) (t - t) := by
    simp only [sub_self]
    have := hφ.comp_hasDerivAt_of_eq 0 (hasDerivAt_exp_smul_const' x 0) (by simp)
    exact this.congr_deriv (by simp)
  convert! ((this.comp_sub_const t t).const_mul
    (φ (exp (t • x)))).congr_deriv (mul_comm _ _) using 2
  rw [← map_mul, ← NormedSpace.exp_add_of_commute ((Commute.smul_left rfl _).symm.smul_left _),
...
theorem MonoidHom.map_exp_of_hasFDerivAt_one [NormedAlgebra ℚ 𝔸] (φ : 𝔸 →* 𝕂)
    (L : 𝔸 →L[𝕂] 𝕂) (hφ : HasFDerivAt φ L 1) (x : 𝔸) : φ (exp x) = exp (L x) := by
  have hh (t : 𝕂) : HasDerivAt (fun s ↦ exp (-L x * s)) (exp (-L x * t) * -L x) t := by
    simpa [Function.comp_def] using hasDerivAt_exp.comp t ((hasDerivAt_id t).const_mul (-L x))
  apply (isUnit_exp (L x)).div_eq_one_iff_eq.mp
  set h : 𝕂 → 𝕂 := fun t ↦ φ (exp (t • x)) * exp (-L x * t)
  suffices h 1 = h 0 by simpa [h, NormedSpace.exp_neg, ← div_eq_mul_inv]
  suffices ∀ t, HasDerivAt h 0 t from
    is_const_of_deriv_eq_zero (fun t ↦ (this t).differentiableAt) (fun t ↦ (this t).deriv) 1 0
  exact fun t ↦ ((φ.hasDerivAt_exp_smul_const L hφ x t).mul (hh t)).congr_deriv (by ring)
