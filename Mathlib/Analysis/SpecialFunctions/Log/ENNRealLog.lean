...
theorem log_rpow {x : ℝ≥0∞} {y : ℝ} : log (x ^ y) = y * log x := by
  ...
  · rcases lt_trichotomy y 0 with (y_neg | rfl | y_pos)
    · simp [y_neg, EReal.coe_mul_bot_of_neg]
    · simp
    · simp [y_pos, EReal.coe_mul_bot_of_pos]
  · rcases lt_trichotomy y 0 with (y_neg | rfl | y_pos)
    · simp [y_neg, EReal.coe_mul_top_of_neg]
    · simp
    · simp [y_pos, EReal.coe_mul_top_of_pos]
  · rw [log_pos_real' (by rw [← ENNReal.toReal_rpow]; positivity), log_pos_real' x_real]
    norm_cast
    rw [← ENNReal.toReal_rpow]
    exact Real.log_rpow x_real y
