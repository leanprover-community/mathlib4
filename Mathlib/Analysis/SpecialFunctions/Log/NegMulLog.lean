...
lemma negMulLog_sum_eq_iff (h₀ : ∀ i ∈ s, 0 < w i) (h₁ : ∑ i in s, w i = 1)
    (hmem : ∀ i ∈ s, 0 ≤ p i) :
    negMulLog (∑ i in s, w i * p i) = ∑ i in s, w i * negMulLog (p i) ↔
      ∀ j ∈ s, p j = ∑ i in s, w i * p i :=
  strictConcaveOn_negMulLog.map_sum_eq_iff h₀ h₁ hmem
