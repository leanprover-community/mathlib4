...
lemma log_le_log_iff (h : 0 < x) (h₁ : 0 < y) : log x ≤ log y ↔ x ≤ y := by
...
@[gcongr]
lemma log_lt_log (hx : 0 < x) (h : x < y) : log x < log y := by
...
lemma log_natCast_nonneg (n : ℕ) : 0 ≤ log n := by
  if hn : n = 0 then
    simp [hn]
  else
...
lemma log_neg_natCast_nonneg (n : ℕ) : 0 ≤ log (-n) := by
  rw [← log_neg_eq_log, neg_neg]
  exact log_natCast_nonneg _

lemma log_intCast_nonneg (n : ℤ) : 0 ≤ log n := by
...
lemma log_sqrt {x : ℝ} (hx : 0 ≤ x) : log (√x) = log x / 2 := by
