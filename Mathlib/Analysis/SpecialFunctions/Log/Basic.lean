...
theorem strictMonoOn_log : StrictMonoOn log (Set.Ioi 0) := fun _x hx _y _hy hxy => log_lt_log hx hxy
...
theorem tendsto_log_nhdsWithin_zero : Tendsto log (𝓝[≠] 0) atBot := by
  suffices : Tendsto log (𝓝[>] 0) atBot
  · simpa only [(· ∘ ·), log_abs] using this.comp tendsto_abs_nhdsWithin_zero
  simpa only [← tendsto_comp_exp_atBot, log_exp] using tendsto_id
...
theorem continuous_log' : Continuous fun x : { x : ℝ // 0 < x } => log x :=
  continuousOn_iff_continuous_restrict.1 <| continuousOn_log.mono fun _x hx => ne_of_gt hx
#align real.continuous_log' Real.continuous_log'
...
theorem log_nat_eq_sum_factorization (n : ℕ) :
  ...
  · simp -- relies on junk values of `Real.log` and `Nat.factorization`
