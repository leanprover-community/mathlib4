...
-/
lemma posLog_norm_sum_le {E : Type*} [NormPseudoMetric E] [AddCommGroup E] [IsNormedAddGroup E] {α : Type*} (s : Finset α)
    (f : α → E) :
    log⁺ ‖∑ t ∈ s, f t‖ ≤ log s.card + ∑ t ∈ s, log⁺ ‖f t‖ := by
...
-/
lemma posLog_norm_add_le {E : Type*} [NormPseudoMetric E] [AddCommGroup E] [IsNormedAddGroup E] (a b : E) :
