...
instance : LES (SeminormFamily 𝕜 E ι) (SeminormFamily 𝕜 E ι') where
  les p q := ∀ i, ∃ (s : Finset ι'), p i ≲ s.sup q

theorem les_iff : p ≲ q ↔ ∀ i, ∃ (s : Finset ι'), p i ≲ s.sup q := by rfl

instance : LESRefl (SeminormFamily 𝕜 E ι) where
  les_refl p := by
    intro i
    use {i}
    grind [Finset.sup_singleton]

instance : LESTrans (SeminormFamily 𝕜 E ι) (SeminormFamily 𝕜 E ι') (SeminormFamily 𝕜 E ι'') where
  les_trans p q q' hp hq := by
    intro i
    obtain ⟨s, hs⟩ := hp i
    choose y hy using hq
    classical
    use s.biUnion y
    grw [hs]
    grind [les_congr_sup]
...
theorem IsBoundedBy.isBoundedBy_sup (h : p ≲ q) {s : Finset ι} : ∃ s' : Finset ι',
    s.sup p ≲ s'.sup q := by
  classical
...
@[grind .]
theorem IsBoundedBy.smul_left (h : p ≲ q) (a : R) : a • p ≲ q := by
  intro i
  obtain ⟨s, hs⟩ := h i
  use s
  exact smul_les hs _
...
@[grind ←]
theorem IsBoundedBy.smul_right (h : p ≲ q) {a : ℝ≥0} (ha : a ≠ 0) :
    p ≲ a • q := by
  intro i
  obtain ⟨s, hs⟩ := h i
  use s
  rw [finset_sup_smul q s a]
  apply les_smul hs ha

variable {p' : SeminormFamily 𝕜 E ι} {q' : SeminormFamily 𝕜 E ι'}

@[grind .]
theorem IsBoundedBy.add_left (h : p ≲ q) (h' : p' ≲ q) :
    p + p' ≲ q := by
  intro i
  obtain ⟨s, hs⟩ := h i
  obtain ⟨s', hs'⟩ := h' i
...
variable (p) in
theorem isEquivalent_finset_sup : p ≈ fun s : Finset ι ↦ s.sup p := by
  constructor
  · intro i
    exact ⟨{{i}}, 1, by simp⟩
  · intro s
...
variable (p) in
theorem isEquivalent_equiv (e : ι' ≃ ι) : p ≈ p ∘ e := by
  constructor
  · intro i
    use {e.symm i}
    simp; grind
