...
theorem hasIntegralIndicatorConst (l : IntegrationParams) (hl : l.bRiemann = false)
  ...
  -- ⊢ ∃ r, (∀ (c : ℝ≥0), IntegrationParams.RCond l (r c)) ∧ ∀ (c : ℝ≥0) (π : Tagge …
  ...
  -- ⊢ dist (integralSum (indicator s fun x => y) (BoxAdditiveMap.toSMul (Measure.t …
                                                                                       -- ⊢ dist (integralSum (indicator s fun x => y) (BoxAdditiveMap.toSMul (Measure.t …
  ...
  dsimp [integralSum]
  -- ⊢ dist (∑ J in π.boxes, ENNReal.toReal (↑↑μ ↑J) • indicator s (fun x => y) (Ta …
  ...
  -- ⊢ |ENNReal.toReal (↑↑μ (Prepartition.iUnion (Prepartition.filter π.toPrepartit …
  ...
  · have hμt : μ t ≠ ∞ := ((measure_mono (htU.trans (inter_subset_left _ _))).trans_lt hUt).ne
    ...
    -- ⊢ x ∈ closedBall (TaggedPrepartition.tag π J) ↑(rs' (TaggedPrepartition.tag π  …
...
theorem HasIntegral.of_aeEq_zero {l : IntegrationParams} {I : Box ι} {f : (ι → ℝ) → E}
  ...
  -- ⊢ dist (integralSum f (BoxAdditiveMap.toSMul (Measure.toBoxAdditive μ)) π) 0 ≤ …
  rw [dist_eq_norm, sub_zero, ← integralSum_fiberwise fun J => N (π.tag J)]
  -- ⊢ ‖∑ y in Finset.image (fun J => N (TaggedPrepartition.tag π J)) π.boxes, inte …
  ...
  rintro n -
  -- ⊢ ‖integralSum f (BoxAdditiveMap.toSMul (Measure.toBoxAdditive μ)) (TaggedPrep …
  dsimp [integralSum]
  -- ⊢ ‖∑ J in (TaggedPrepartition.filter π fun x => ⌈‖f (TaggedPrepartition.tag π  …
  ...
  -- ⊢ ∑ b in (TaggedPrepartition.filter π fun J => N (TaggedPrepartition.tag π J)  …
                                              -- ⊢ ∑ b in (TaggedPrepartition.filter π fun J => N (TaggedPrepartition.tag π J)  …
  rw [← sum_mul, ← Prepartition.measure_iUnion_toReal]
  -- ⊢ ENNReal.toReal (↑↑μ (Prepartition.iUnion (TaggedPrepartition.filter π fun J  …
...
theorem hasBoxIntegral (f : SimpleFunc (ι → ℝ) E) (μ : Measure (ι → ℝ)) [IsLocallyFiniteMeasure μ]
  ...
  -- ⊢ HasIntegral I l (↑(piecewise s hs (const (ι → ℝ) y) (const (ι → ℝ) 0))) (Box …
...
theorem IntegrableOn.hasBoxIntegral [CompleteSpace E] {f : (ι → ℝ) → E} {μ : Measure (ι → ℝ)}
  ...
  borelize E
  -- ⊢ HasIntegral I l f (BoxAdditiveMap.toSMul (Measure.toBoxAdditive μ)) (∫ (x :  …
  ...
  -- ⊢ HasIntegral I l f (BoxAdditiveMap.toSMul (Measure.toBoxAdditive μ)) (∫ (a :  …
                              -- ⊢ HasIntegral I l f (BoxAdditiveMap.toSMul (Measure.toBoxAdditive μ)) (∫ (a :  …
  refine' BoxIntegral.HasIntegral.congr_ae _ hfg.symm hl
  -- ⊢ HasIntegral I l g (BoxAdditiveMap.toSMul (Measure.toBoxAdditive μ)) (∫ (a :  …
  clear! f
  -- ⊢ HasIntegral I l g (BoxAdditiveMap.toSMul (Measure.toBoxAdditive μ)) (∫ (a :  …
  ...
  · /- Since each `f (Nx $ π.tag J)` is `ε`-close to `g (π.tag J)`, replacing the latter with
    ...
    -- ⊢ dist (∑ J in π.boxes, ↑(↑(BoxAdditiveMap.toSMul (Measure.toBoxAdditive μ)) J …
    refine' dist_sum_sum_le_of_le _ fun J _ => _; dsimp
    -- ⊢ dist (↑(↑(BoxAdditiveMap.toSMul (Measure.toBoxAdditive μ)) J) (g (TaggedPrep …
                                                  -- ⊢ dist (ENNReal.toReal (↑↑μ ↑J) • g (TaggedPrepartition.tag π J)) (ENNReal.toR …
    rw [dist_eq_norm, ← smul_sub, norm_smul, Real.norm_eq_abs, abs_of_nonneg ENNReal.toReal_nonneg]
    -- ⊢ ENNReal.toReal (↑↑μ ↑J) * ‖g (TaggedPrepartition.tag π J) - ↑(SimpleFunc.app …
    refine' mul_le_mul_of_nonneg_left _ ENNReal.toReal_nonneg
    -- ⊢ ‖g (TaggedPrepartition.tag π J) - ↑(SimpleFunc.approxOn g (_ : Measurable g) …
    rw [← dist_eq_norm']; exact hNxε _
    -- ⊢ dist (↑(SimpleFunc.approxOn g (_ : Measurable g) (Set.range g ∪ {0}) 0 (_ :  …
                          -- 🎉 no goals
  · /- We group the terms of both sums by the values of `Nx (π.tag J)`.
    ...
    -- ⊢ dist (∑ y in Finset.image (fun J => Nx (TaggedPrepartition.tag π J)) π.boxes …
    refine' le_trans _ (NNReal.coe_lt_coe.2 hcε).le
    -- ⊢ dist (∑ y in Finset.image (fun J => Nx (TaggedPrepartition.tag π J)) π.boxes …
    refine'
    ...
    -- ⊢ ∑ J in (Prepartition.filter π.toPrepartition fun J => Nx (TaggedPrepartition …
    · refine' sum_congr rfl fun J hJ => _
      -- ⊢ ENNReal.toReal (↑↑μ ↑J) • ↑(f (Nx (TaggedPrepartition.tag π J))) (TaggedPrep …
      simp [hNxn J hJ]
      -- 🎉 no goals
    · refine' sum_congr rfl fun J hJ => _
      -- ⊢ ∫ (x : ι → ℝ) in ↑J, ↑(f (Nx (TaggedPrepartition.tag π J))) x ∂μ = BoxIntegr …
  ...
  · /-  For the last jump, we use the fact that the distance between `f (Nx x) x` and `g x` is less
    ...
    -- ⊢ dist (∑ J in π.boxes, ∫ (x : ι → ℝ) in ↑J, ↑(f (Nx (TaggedPrepartition.tag π …
    ...
    -- ⊢ dist (∫ (x : ι → ℝ) in ↑J, ↑(f (Nx (TaggedPrepartition.tag π J))) x ∂μ) (∫ ( …
    rw [dist_eq_norm, ← integral_sub (hfi _ J hJ) (hgi J hJ)]
    -- ⊢ ‖∫ (a : ι → ℝ) in ↑J, ↑(f (Nx (TaggedPrepartition.tag π J))) a - g a ∂μ‖ ≤ ∫ …
