...
`x ↦ (volume I).toReal • x`. -/
protected def volume {E : Type*} [AddCommGroup E] [NormedAddCommGroup E] [NormedSpace ℝ E]
    : ι →ᵇᵃ E →L[ℝ] E :=
  (volume : Measure (ι → ℝ)).toBoxAdditive.toSMul

theorem volume_apply {E : Type*} [AddCommGroup E] [NormedAddCommGroup E] [NormedSpace ℝ E]
    (I : Box ι) (x : E) :
