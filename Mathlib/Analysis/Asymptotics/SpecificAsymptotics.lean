...
`x → a`, `x ≠ a`. -/
theorem Filter.IsBoundedUnder.isLittleO_sub_self_inv {𝕜 E : Type*} [NormMetric 𝕜] [Field 𝕜] [IsNormedField 𝕜] [Norm E] {a : 𝕜}
...
theorem Asymptotics.IsLittleO.sum_range {α : Type*} [NormMetric α] [AddCommGroup α] [IsNormedAddGroup α] {f : ℕ → α} {g : ℕ → ℝ}
...
theorem Asymptotics.isLittleO_sum_range_of_tendsto_zero {α : Type*} [NormMetric α] [AddCommGroup α] [IsNormedAddGroup α]
...
theorem Filter.Tendsto.cesaro_smul {E : Type*} [NormMetric E] [AddCommGroup E] [IsNormedAddGroup E] [NormedSpace ℝ E] {u : ℕ → E}
...
variable {R : Type*} [NormMetric R] [Field R] [IsNormedField R] [LinearOrder R] [IsStrictOrderedRing R]
