...
theorem isOWith_iff : IsOWith c l f g ↔ ∀ᶠ x in l, ‖f x‖ ≤ c * ‖g x‖ := by rw [IsOWith_def]
#align asymptotics.is_O_with_iff Asymptotics.isOWith_iff

alias isOWith_iff ↔ IsOWith.bound IsOWith.of_bound
...
irreducible. -/
theorem isO_iff_isOWith : f =O[l] g ↔ ∃ c : ℝ, IsOWith c l f g := by rw [IsO_def]
...
`is_o` to be irreducible at the end of this file. -/
theorem isOCat_iff_forall_isOWith : f =o[l] g ↔ ∀ ⦃c : ℝ⦄, 0 < c → IsOWith c l f g := by rw [IsOCat_def]
#align asymptotics.is_o_iff_forall_is_O_with Asymptotics.isOCat_iff_forall_isOWith

alias isOCat_iff_forall_isOWith ↔ IsOCat.forall_isOWith IsOCat.of_isOWith
...
theorem isOCat_iff : f =o[l] g ↔ ∀ ⦃c : ℝ⦄, 0 < c → ∀ᶠ x in l, ‖f x‖ ≤ c * ‖g x‖ := by
  simp only [IsOCat_def, IsOWith_def]
#align asymptotics.is_o_iff Asymptotics.isOCat_iff

alias isOCat_iff ↔ IsOCat.bound IsOCat.of_bound
...
theorem IsOWith.isO (h : IsOWith c l f g) : f =O[l] g := by rw [IsO_def]; exact ⟨c, h⟩
...
theorem IsOWith.exists_pos (h : IsOWith c l f g') : ∃ (c' : _) (_H : 0 < c'), IsOWith c' l f g' :=
...
theorem IsO.exists_pos (h : f =O[l] g') : ∃ (c : _) (_H : 0 < c), IsOWith c l f g' :=
  let ⟨_c, hc⟩ := h.isOWith
  hc.exists_pos
#align asymptotics.is_O.exists_pos Asymptotics.IsO.exists_pos

theorem IsOWith.exists_nonneg (h : IsOWith c l f g') :
    ∃ (c' : _) (_H : 0 ≤ c'), IsOWith c' l f g' :=
  let ⟨c, cpos, hc⟩ := h.exists_pos
  ⟨c, le_of_lt cpos, hc⟩
#align asymptotics.is_O_with.exists_nonneg Asymptotics.IsOWith.exists_nonneg

theorem IsO.exists_nonneg (h : f =O[l] g') : ∃ (c : _) (_H : 0 ≤ c), IsOWith c l f g' :=
  let ⟨_c, hc⟩ := h.isOWith
  hc.exists_nonneg
...
theorem isO_iff_eventually : f =O[l] g' ↔ ∀ᶠ c in atTop, ∀ᶠ x in l, ‖f x‖ ≤ c * ‖g' x‖ :=
  isO_iff_eventually_isOWith.trans <| by simp only [IsOWith_def]
...
theorem IsO.exists_mem_basis {ι} {p : ι → Prop} {s : ι → Set α} (h : f =O[l] g')
  ...
  flip Exists₂.imp h.exists_pos fun c _hc h => by
    simpa only [isOWith_iff, hb.eventually_iff, exists_prop] using h
...
theorem isOWith_inv (hc : 0 < c) : IsOWith c⁻¹ l f g ↔ ∀ᶠ x in l, c * ‖f x‖ ≤ ‖g x‖ := by
  simp only [IsOWith_def, ← div_eq_inv_mul, le_div_iff' hc]
...
theorem isOCat_iff_nat_mul_le_aux (h₀ : (∀ x, 0 ≤ ‖f x‖) ∨ ∀ x, 0 ≤ ‖g x‖) :
  ...
  · rintro H (_ | n)
    ...
    · have : (0 : ℝ) < n.succ := Nat.cast_pos.2 n.succ_pos
      exact (isOWith_inv this).1 (H.def' <| inv_pos.2 this)
  · refine' fun H => isOCat_iff.2 fun ε ε0 => _
    rcases exists_nat_gt ε⁻¹ with ⟨n, hn⟩
    have hn₀ : (0 : ℝ) < n := (inv_pos.2 ε0).trans hn
    refine' ((isOWith_inv hn₀).2 (H n)).bound.mono fun x hfg => _
...
theorem isOCat_iff_nat_mul_le : f =o[l] g' ↔ ∀ n : ℕ, ∀ᶠ x in l, ↑n * ‖f x‖ ≤ ‖g' x‖ :=
  isOCat_iff_nat_mul_le_aux (Or.inr fun _x => norm_nonneg _)
#align asymptotics.is_o_iff_nat_mul_le Asymptotics.isOCat_iff_nat_mul_le

theorem isOCat_iff_nat_mul_le' : f' =o[l] g ↔ ∀ n : ℕ, ∀ᶠ x in l, ↑n * ‖f' x‖ ≤ ‖g x‖ :=
  isOCat_iff_nat_mul_le_aux (Or.inl fun _x => norm_nonneg _)
#align asymptotics.is_o_iff_nat_mul_le' Asymptotics.isOCat_iff_nat_mul_le'
...
theorem isO_congr (hf : f₁ =ᶠ[l] f₂) (hg : g₁ =ᶠ[l] g₂) : f₁ =O[l] g₁ ↔ f₂ =O[l] g₂ := by
  simp only [IsO_def]
  exact exists_congr fun c => isOWith_congr rfl hf hg
...
theorem isOCat_congr (hf : f₁ =ᶠ[l] f₂) (hg : g₁ =ᶠ[l] g₂) : f₁ =o[l] g₁ ↔ f₂ =o[l] g₂ := by
  simp only [IsOCat_def]
  exact forall₂_congr fun c _hc => isOWith_congr (Eq.refl c) hf hg
...
@[trans]
theorem _root_.Filter.EventuallyEq.trans_isO {f₁ f₂ : α → E} {g : α → F} (hf : f₁ =ᶠ[l] f₂)
    (h : f₂ =O[l] g) : f₁ =O[l] g :=
...
instance : @Trans (α → E) (α → E) (α → F) (· =ᶠ[l] ·) (· =O[l] ·) (· =O[l] ·) where
  trans := Filter.EventuallyEq.trans_isO

@[trans]
theorem _root_.Filter.EventuallyEq.trans_isOCat {f₁ f₂ : α → E} {g : α → F} (hf : f₁ =ᶠ[l] f₂)
    (h : f₂ =o[l] g) : f₁ =o[l] g :=
  h.congr' hf.symm EventuallyEq.rfl
#align filter.eventually_eq.trans_is_o Filter.EventuallyEq.trans_isOCat

instance : @Trans (α → E) (α → E) (α → F) (· =ᶠ[l] ·) (· =o[l] ·) (· =o[l] ·) where
  trans := Filter.EventuallyEq.trans_isOCat
...
instance : @Trans (α → E) (α → F) (α → F) (· =o[l] ·) (· =ᶠ[l] ·) (· =o[l] ·) where
  trans := IsOCat.trans_eventuallyEq
...
theorem IsO.comp_tendsto (hfg : f =O[l] g) {k : β → α} {l' : Filter β} (hk : Tendsto k l' l) :
    (f ∘ k) =O[l'] (g ∘ k) :=
  isO_iff_isOWith.2 <| hfg.isOWith.imp fun _c h => h.comp_tendsto hk
...
theorem IsOCat.comp_tendsto (hfg : f =o[l] g) {k : β → α} {l' : Filter β} (hk : Tendsto k l' l) :
    (f ∘ k) =o[l'] (g ∘ k) :=
  IsOCat.of_isOWith fun _c cpos => (hfg.forall_isOWith cpos).comp_tendsto hk
...
theorem IsO.mono (h : f =O[l'] g) (hl : l ≤ l') : f =O[l] g :=
  isO_iff_isOWith.2 <| h.isOWith.imp fun _c h => h.mono hl
#align asymptotics.is_O.mono Asymptotics.IsO.mono

theorem IsOCat.mono (h : f =o[l'] g) (hl : l ≤ l') : f =o[l] g :=
  IsOCat.of_isOWith fun _c cpos => (h.forall_isOWith cpos).mono hl
...
theorem _root_.Filter.Eventually.trans_isO {f : α → E} {g : α → F'} {k : α → G}
...
theorem _root_.Filter.Eventually.isO {f : α → E} {g : α → ℝ} {l : Filter α}
    (hfg : ∀ᶠ x in l, ‖f x‖ ≤ g x) : f =O[l] g :=
  IsO.of_bound' <| hfg.mono fun _x hx => hx.trans <| Real.le_norm_self _
...
theorem isOCat_irrefl' (h : ∃ᶠ x in l, ‖f' x‖ ≠ 0) : ¬f' =o[l] f' := by
  intro ho
...
theorem isOCat_irrefl (h : ∃ᶠ x in l, f'' x ≠ 0) : ¬f'' =o[l] f'' :=
  isOCat_irrefl' <| h.mono fun _x => norm_ne_zero_iff.mpr
#align asymptotics.is_o_irrefl Asymptotics.isOCat_irrefl
...
theorem IsOCat.sup (h : f =o[l] g) (h' : f =o[l'] g) : f =o[l ⊔ l'] g :=
  IsOCat.of_isOWith fun _c cpos => (h.forall_isOWith cpos).sup (h'.forall_isOWith cpos)
...
theorem isOWith_insert [TopologicalSpace α] {x : α} {s : Set α} {C : ℝ} {g : α → E} {g' : α → F}
  ...
  simp_rw [IsOWith_def, nhdsWithin_insert, eventually_sup, eventually_pure, h, true_and_iff]
...
theorem isOWith_norm_right : (IsOWith c l f fun x => ‖g' x‖) ↔ IsOWith c l f g' := by
  simp only [IsOWith_def, norm_norm]
...
alias isOWith_norm_right ↔ IsOWith.of_norm_right IsOWith.norm_right
...
alias isOWith_abs_right ↔ IsOWith.of_abs_right IsOWith.abs_right
...
@[simp]
theorem isO_norm_right : (f =O[l] fun x => ‖g' x‖) ↔ f =O[l] g' := by
  simp only [IsO_def]
  exact exists_congr fun _ => isOWith_norm_right
...
alias isO_norm_right ↔ IsO.of_norm_right IsO.norm_right
...
alias isO_abs_right ↔ IsO.of_abs_right IsO.abs_right
...
@[simp]
theorem isOCat_norm_right : (f =o[l] fun x => ‖g' x‖) ↔ f =o[l] g' := by
  simp only [IsOCat_def]
  exact forall₂_congr fun _ _ => isOWith_norm_right
...
alias isOCat_norm_right ↔ IsOCat.of_norm_right IsOCat.norm_right
...
alias isOCat_abs_right ↔ IsOCat.of_abs_right IsOCat.abs_right
...
alias isOWith_norm_left ↔ IsOWith.of_norm_left IsOWith.norm_left
...
alias isOWith_abs_left ↔ IsOWith.of_abs_left IsOWith.abs_left
...
@[simp]
theorem isO_norm_left : (fun x => ‖f' x‖) =O[l] g ↔ f' =O[l] g := by
  simp only [IsO_def]
  exact exists_congr fun _ => isOWith_norm_left
...
alias isO_norm_left ↔ IsO.of_norm_left IsO.norm_left
...
alias isO_abs_left ↔ IsO.of_abs_left IsO.abs_left
#align asymptotics.is_O.of_abs_left Asymptotics.IsO.of_abs_left
#align asymptotics.is_O.abs_left Asymptotics.IsO.abs_left

@[simp]
theorem isOCat_norm_left : (fun x => ‖f' x‖) =o[l] g ↔ f' =o[l] g := by
  simp only [IsOCat_def]
  exact forall₂_congr fun _ _ => isOWith_norm_left
...
alias isOCat_norm_left ↔ IsOCat.of_norm_left IsOCat.norm_left
...
alias isOCat_abs_left ↔ IsOCat.of_abs_left IsOCat.abs_left
...
alias isOWith_norm_norm ↔ IsOWith.of_norm_norm IsOWith.norm_norm
...
alias isOWith_abs_abs ↔ IsOWith.of_abs_abs IsOWith.abs_abs
...
alias isO_norm_norm ↔ IsO.of_norm_norm IsO.norm_norm
...
alias isO_abs_abs ↔ IsO.of_abs_abs IsO.abs_abs
...
alias isOCat_norm_norm ↔ IsOCat.of_norm_norm IsOCat.norm_norm
...
alias isOCat_abs_abs ↔ IsOCat.of_abs_abs IsOCat.abs_abs
...
theorem isOWith_neg_right : (IsOWith c l f fun x => -g' x) ↔ IsOWith c l f g' := by
  simp only [IsOWith_def, norm_neg]
#align asymptotics.is_O_with_neg_right Asymptotics.isOWith_neg_right

alias isOWith_neg_right ↔ IsOWith.of_neg_right IsOWith.neg_right
...
@[simp]
theorem isO_neg_right : (f =O[l] fun x => -g' x) ↔ f =O[l] g' := by
  simp only [IsO_def]
  exact exists_congr fun _ => isOWith_neg_right
#align asymptotics.is_O_neg_right Asymptotics.isO_neg_right

alias isO_neg_right ↔ IsO.of_neg_right IsO.neg_right
...
@[simp]
theorem isOCat_neg_right : (f =o[l] fun x => -g' x) ↔ f =o[l] g' := by
  simp only [IsOCat_def]
  exact forall₂_congr fun _ _ => isOWith_neg_right
#align asymptotics.is_o_neg_right Asymptotics.isOCat_neg_right

alias isOCat_neg_right ↔ IsOCat.of_neg_right IsOCat.neg_right
...
alias isOWith_neg_left ↔ IsOWith.of_neg_left IsOWith.neg_left
...
@[simp]
theorem isO_neg_left : (fun x => -f' x) =O[l] g ↔ f' =O[l] g := by
  simp only [IsO_def]
  exact exists_congr fun _ => isOWith_neg_left
#align asymptotics.is_O_neg_left Asymptotics.isO_neg_left

alias isO_neg_left ↔ IsO.of_neg_left IsO.neg_left
#align asymptotics.is_O.of_neg_left Asymptotics.IsO.of_neg_left
#align asymptotics.is_O.neg_left Asymptotics.IsO.neg_left

@[simp]
theorem isOCat_neg_left : (fun x => -f' x) =o[l] g ↔ f' =o[l] g := by
  simp only [IsOCat_def]
  exact forall₂_congr fun _ _ => isOWith_neg_left
#align asymptotics.is_o_neg_left Asymptotics.isOCat_neg_left

alias isOCat_neg_left ↔ IsOCat.of_neg_left IsOCat.neg_left
#align asymptotics.is_o.of_neg_left Asymptotics.IsOCat.of_neg_left
...
theorem isO_fst_prod' {f' : α → E' × F'} : (fun x => (f' x).1) =O[l] f' := by
  simpa [IsO_def, IsOWith_def] using isO_fst_prod (E' := E') (F' := F')
#align asymptotics.is_O_fst_prod' Asymptotics.isO_fst_prod'

theorem isO_snd_prod' {f' : α → E' × F'} : (fun x => (f' x).2) =O[l] f' := by
  simpa [IsO_def, IsOWith_def] using isO_snd_prod (E' := E') (F' := F')
#align asymptotics.is_O_snd_prod' Asymptotics.isO_snd_prod'
...
theorem IsOCat.prod_rightl (h : f =o[l] g') : f =o[l] fun x => (g' x, k' x) :=
  IsOCat.of_isOWith fun _c cpos => (h.forall_isOWith cpos).prod_rightl k' cpos.le
...
theorem IsOCat.prod_rightr (h : f =o[l] g') : f =o[l] fun x => (f' x, g' x) :=
  IsOCat.of_isOWith fun _c cpos => (h.forall_isOWith cpos).prod_rightr f' cpos.le
...
theorem IsOWith.prod_left_same (hf : IsOWith c l f' k') (hg : IsOWith c l g' k') :
  ...
  rw [isOWith_iff] at *; filter_upwards [hf, hg]with x using max_le
...
theorem IsO.prod_left (hf : f' =O[l] k') (hg : g' =O[l] k') : (fun x => (f' x, g' x)) =O[l] k' :=
  let ⟨_c, hf⟩ := hf.isOWith
  let ⟨_c', hg⟩ := hg.isOWith
  (hf.prod_left hg).isO
...
theorem IsOCat.prod_left (hf : f' =o[l] k') (hg : g' =o[l] k') : (fun x => (f' x, g' x)) =o[l] k' :=
  IsOCat.of_isOWith fun _c hc => (hf.forall_isOWith hc).prod_left_same (hg.forall_isOWith hc)
...
theorem IsOWith.sub_isOCat (h₁ : IsOWith c₁ l f₁ g) (h₂ : f₂ =o[l] g) (hc : c₁ < c₂) :
  ...
  simpa only [sub_eq_add_neg] using h₁.add_isOCat h₂.neg_left hc
...
theorem isOCat_zero : (fun _x => (0 : E')) =o[l] g' :=
...
theorem isOWith_zero (hc : 0 ≤ c) : IsOWith c l (fun _x => (0 : E')) g' :=
...
theorem isOWith_zero' : IsOWith 0 l (fun _x => (0 : E')) g :=
  IsOWith.of_bound <| univ_mem' fun x => by simp
#align asymptotics.is_O_with_zero' Asymptotics.isOWith_zero'

theorem isO_zero : (fun _x => (0 : E')) =O[l] g :=
...
theorem isO_refl_left : (fun x => f' x - f' x) =O[l] g' :=
  (isO_zero g' l).congr_left fun _x => (sub_self _).symm
#align asymptotics.is_O_refl_left Asymptotics.isO_refl_left

theorem isOCat_refl_left : (fun x => f' x - f' x) =o[l] g' :=
  (isOCat_zero g' l).congr_left fun _x => (sub_self _).symm
#align asymptotics.is_o_refl_left Asymptotics.isOCat_refl_left

variable {g g' l}

@[simp]
theorem isOWith_zero_right_iff : (IsOWith c l f'' fun _x => (0 : F')) ↔ f'' =ᶠ[l] 0 := by
  simp only [IsOWith_def, exists_prop, true_and_iff, norm_zero, MulZeroClass.mul_zero,
    norm_le_zero_iff, EventuallyEq, Pi.zero_apply]
#align asymptotics.is_O_with_zero_right_iff Asymptotics.isOWith_zero_right_iff

@[simp]
theorem isO_zero_right_iff : (f'' =O[l] fun _x => (0 : F')) ↔ f'' =ᶠ[l] 0 :=
  ⟨fun h =>
    let ⟨_c, hc⟩ := h.isOWith
...
@[simp]
theorem isOCat_zero_right_iff : (f'' =o[l] fun _x => (0 : F')) ↔ f'' =ᶠ[l] 0 :=
  ⟨fun h => isO_zero_right_iff.1 h.isO, fun h =>
    IsOCat.of_isOWith fun _c _hc => isOWith_zero_right_iff.2 h⟩
...
theorem isOWith_const_const (c : E) {c' : F''} (hc' : c' ≠ 0) (l : Filter α) :
    IsOWith (‖c‖ / ‖c'‖) l (fun _x : α => c) fun _x => c' := by
  simp only [IsOWith_def]
  apply univ_mem'
  intro x
...
@[simp]
theorem isOWith_top : IsOWith c ⊤ f g ↔ ∀ x, ‖f x‖ ≤ c * ‖g x‖ := by rw [IsOWith_def]; rfl
#align asymptotics.is_O_with_top Asymptotics.isOWith_top

@[simp]
theorem isO_top : f =O[⊤] g ↔ ∃ C, ∀ x, ‖f x‖ ≤ C * ‖g x‖ := by rw [isO_iff]; rfl
#align asymptotics.is_O_top Asymptotics.isO_top

@[simp]
theorem isOCat_top : f'' =o[⊤] g'' ↔ ∀ x, f'' x = 0 := by
  refine' ⟨_, fun h => (isOCat_zero g'' ⊤).congr (fun x => (h x).symm) fun x => rfl⟩
  simp only [isOCat_iff, eventually_top]
...
theorem isOWith_const_one (c : E) (l : Filter α) :
    IsOWith ‖c‖ l (fun _x : α => c) fun _x => (1 : F) := by simp [isOWith_iff]
#align asymptotics.is_O_with_const_one Asymptotics.isOWith_const_one

theorem isO_const_one (c : E) (l : Filter α) : (fun _x : α => c) =O[l] fun _x => (1 : F) :=
...
@[simp]
theorem isOCat_one_iff : f' =o[l] (fun _x => 1 : α → F) ↔ Tendsto f' l (𝓝 0) := by
  simp only [isOCat_iff, norm_one, mul_one, Metric.nhds_basis_closedBall.tendsto_right_iff,
    Metric.mem_closedBall, dist_zero_right]
#align asymptotics.is_o_one_iff Asymptotics.isOCat_one_iff

@[simp]
theorem isO_one_iff : f =O[l] (fun _x => 1 : α → F) ↔ IsBoundedUnder (· ≤ ·) l fun x => ‖f x‖ := by
  simp only [isO_iff, norm_one, mul_one]
  rfl
#align asymptotics.is_O_one_iff Asymptotics.isO_one_iff

alias isO_one_iff ↔ _ _root_.Filter.IsBoundedUnder.isO_one
#align filter.is_bounded_under.is_O_one Filter.IsBoundedUnder.isO_one

@[simp]
theorem isOCat_one_left_iff : (fun _x => 1 : α → F) =o[l] f ↔ Tendsto (fun x => ‖f x‖) l atTop :=
  calc
    (fun _x => 1 : α → F) =o[l] f ↔ ∀ n : ℕ, ∀ᶠ x in l, ↑n * ‖(1 : F)‖ ≤ ‖f x‖ :=
      isOCat_iff_nat_mul_le_aux <| Or.inl fun _x => by simp only [norm_one, zero_le_one]
...
theorem _root_.Filter.Tendsto.isO_one {c : E'} (h : Tendsto f' l (𝓝 c)) : f' =O[l] (fun _x => 1 : α → F) :=
...
theorem IsO.isBoundedUnder_le {c : F} (h : f =O[l] fun _x => c) :
...
theorem isO_const_iff {c : F''} :
    (f'' =O[l] fun _x => c) ↔ (c = 0 → f'' =ᶠ[l] 0) ∧ IsBoundedUnder (· ≤ ·) l fun x => ‖f'' x‖ := by
  refine' ⟨fun h => ⟨fun hc => isO_zero_right_iff.1 (by rwa [← hc]), h.isBoundedUnder_le⟩, _⟩
  rintro ⟨hcf, hf⟩
  rcases eq_or_ne c 0 with (hc | hc)
  exacts[(hcf hc).trans_isO (isO_zero _ _), hf.isO_const hc]
...
theorem isO_iff_isBoundedUnder_le_div (h : ∀ᶠ x in l, g'' x ≠ 0) :
  ...
  simp only [isO_iff, IsBoundedUnder, IsBounded, eventually_map]
  exact
    exists_congr fun c =>
...
theorem isO_const_left_iff_pos_le_norm {c : E''} (hc : c ≠ 0) :
  ...
  · rintro ⟨b, hb₀, hb⟩
    refine' IsO.of_bound (‖c‖ / b) (hb.mono fun x hx => _)
...
theorem isOWith_const_mul_self (c : R) (f : α → R) (l : Filter α) :
  ...
  isOWith_of_le' _ fun _x => norm_mul_le _ _
...
theorem isOWith_self_const_mul' (u : Rˣ) (f : α → R) (l : Filter α) :
    IsOWith ‖(↑u⁻¹ : R)‖ l f fun x => ↑u * f x := by
  refine' (isOWith_const_mul_self ↑u⁻¹ _ l).congr_left _
...
theorem IsOWith.pow' {f : α → R} {g : α → 𝕜} (h : IsOWith c l f g) :
  ...
  | 0 => by simpa using isOWith_const_const (1 : R) (one_ne_zero' 𝕜) l
  | 1 => by simpa
  | n + 2 => by simpa [pow_succ] using h.mul (IsOWith.pow' h (n + 1))
...
theorem IsOCat.of_pow {f : α → 𝕜} {g : α → R} {n : ℕ} (h : (f ^ n) =o[l] (g ^ n)) (hn : n ≠ 0) :
    f =o[l] g :=
  IsOCat.of_isOWith fun _c hc => (h.def' <| pow_pos hc _).of_pow hn le_rfl hc.le
...
theorem IsOWith.inv_rev {f : α → 𝕜} {g : α → 𝕜'} (h : IsOWith c l f g)
  ...
  refine' IsOWith.of_bound (h.bound.mp (h₀.mono fun x h₀ hle => _))
...
theorem IsOCat.inv_rev {f : α → 𝕜} {g : α → 𝕜'} (h : f =o[l] g)
  ...
  IsOCat.of_isOWith fun _c hc => (h.def' hc).inv_rev h₀
...
theorem isO_const_smul_left {c : 𝕜} (hc : c ≠ 0) : (fun x => c • f' x) =O[l] g ↔ f' =O[l] g := by
  have cne0 : ‖c‖ ≠ 0 := mt norm_eq_zero.mp hc
  rw [← isO_norm_left]
  simp only [norm_smul]
  rw [isO_const_mul_left_iff cne0, isO_norm_left]
#align asymptotics.is_O_const_smul_left Asymptotics.isO_const_smul_left

theorem isOCat_const_smul_left {c : 𝕜} (hc : c ≠ 0) : (fun x => c • f' x) =o[l] g ↔ f' =o[l] g := by
  have cne0 : ‖c‖ ≠ 0 := mt norm_eq_zero.mp hc
  rw [← isOCat_norm_left]
  simp only [norm_smul]
  rw [isOCat_const_mul_left_iff cne0, isOCat_norm_left]
#align asymptotics.is_o_const_smul_left Asymptotics.isOCat_const_smul_left

theorem isO_const_smul_right {c : 𝕜} (hc : c ≠ 0) : (f =O[l] fun x => c • f' x) ↔ f =O[l] f' := by
  have cne0 : ‖c‖ ≠ 0 := mt norm_eq_zero.mp hc
  rw [← isO_norm_right]
  simp only [norm_smul]
  rw [isO_const_mul_right_iff cne0, isO_norm_right]
...
theorem isOCat_const_smul_right {c : 𝕜} (hc : c ≠ 0) : (f =o[l] fun x => c • f' x) ↔ f =o[l] f' :=
  by
  have cne0 : ‖c‖ ≠ 0 := mt norm_eq_zero.mp hc
  rw [← isOCat_norm_right]
  simp only [norm_smul]
  rw [isOCat_const_mul_right_iff cne0, isOCat_norm_right]
...
theorem IsO.smul_isOCat (h₁ : k₁ =O[l] k₂) (h₂ : f' =o[l] g') :
    (fun x => k₁ x • f' x) =o[l] fun x => k₂ x • g' x := by
  refine' ((h₁.norm_norm.mul_isOCat h₂.norm_norm).congr _ _).of_norm_norm <;>
    · intros; simp only [norm_smul]
#align asymptotics.is_O.smul_is_o Asymptotics.IsO.smul_isOCat

theorem IsOCat.smul_isO (h₁ : k₁ =o[l] k₂) (h₂ : f' =O[l] g') :
    (fun x => k₁ x • f' x) =o[l] fun x => k₂ x • g' x := by
  refine' ((h₁.norm_norm.mul_isO h₂.norm_norm).congr _ _).of_norm_norm <;>
    · intros; simp only [norm_smul]
...
theorem IsOWith.sum (h : ∀ i ∈ s, IsOWith (C i) l (A i) g) :
  ...
  · simp only [isOWith_zero', Finset.sum_empty, forall_true_iff]
...
theorem IsO.sum (h : ∀ i ∈ s, A i =O[l] g) : (fun x => ∑ i in s, A i x) =O[l] g := by
  simp only [IsO_def] at *
  choose! C hC using h
  exact ⟨_, IsOWith.sum hC⟩
#align asymptotics.is_O.sum Asymptotics.IsO.sum

theorem IsOCat.sum (h : ∀ i ∈ s, A i =o[l] g') : (fun x => ∑ i in s, A i x) =o[l] g' := by
  induction' s using Finset.induction_on with i s is IH
  · simp only [isOCat_zero, Finset.sum_empty, forall_true_iff]
  · simp only [is, Finset.sum_insert, not_false_iff]
    exact (h _ (Finset.mem_insert_self i s)).add (IH fun j hj => h _ (Finset.mem_insert_of_mem hj))
...
theorem IsOCat.tendsto_div_nhds_zero {f g : α → 𝕜} (h : f =o[l] g) :
  ...
  (isOCat_one_iff 𝕜).mp <| by
    calc
      (fun x => f x / g x) =o[l] fun x => g x / g x := by
        simpa only [div_eq_mul_inv] using h.mul_isO (isO_refl _ _)
      _ =O[l] fun _x => (1 : 𝕜) := isO_of_le _ fun x => by simp [div_self_le_one]
...
alias isOCat_iff_tendsto' ↔ _ isOCat_of_tendsto'
...
theorem isOCat_const_left_of_ne {c : E''} (hc : c ≠ 0) :
  ...
  simp only [← isOCat_one_left_iff ℝ]
  exact ⟨(isO_const_const (1 : ℝ) hc l).trans_isOCat, (isO_const_one ℝ c l).trans_isOCat⟩
...
theorem isOCat_const_left {c : E''} :
  ...
  rcases eq_or_ne c 0 with (rfl | hc)
  · simp only [isOCat_zero, eq_self_iff_true, true_or_iff]
...
theorem isOCat_const_const_iff [NeBot l] {d : E''} {c : F''} :
  ...
  simp only [isOCat_const_left, or_iff_left_iff_imp]
...
theorem isOCat_pure {x} : f'' =o[pure x] g'' ↔ f'' x = 0 :=
  calc
    f'' =o[pure x] g'' ↔ (fun _y : α => f'' x) =o[pure x] fun _ => g'' x := isOCat_congr rfl rfl
    _ ↔ f'' x = 0 := isOCat_const_const_iff

#align asymptotics.is_o_pure Asymptotics.isOCat_pure

theorem isOCat_const_id_comap_norm_atTop (c : F'') : (fun _x : E'' => c) =o[comap norm atTop] id :=
...
theorem isOCat_const_id_atTop (c : E'') : (fun _x : ℝ => c) =o[atTop] id :=
...
theorem isOCat_const_id_atBot (c : E'') : (fun _x : ℝ => c) =o[atBot] id :=
...
theorem isOWith_iff_exists_eq_mul (hc : 0 ≤ c) :
    IsOWith c l u v ↔ ∃ (φ : α → 𝕜)(_hφ : ∀ᶠ x in l, ‖φ x‖ ≤ c), u =ᶠ[l] φ * v := by
  constructor
  · intro h
    use fun x => u x / v x
  ...
  · rintro ⟨φ, hφ, h⟩
    exact isOWith_of_eq_mul φ hφ h
...
theorem isO_iff_exists_eq_mul :
  ...
  · rintro h
    ...
    exact ⟨φ, ⟨c, hφ⟩, huvφ⟩
  · rintro ⟨φ, ⟨c, hφ⟩, huvφ⟩
    exact isO_iff_isOWith.2 ⟨c, isOWith_of_eq_mul φ hφ huvφ⟩
#align asymptotics.is_O_iff_exists_eq_mul Asymptotics.isO_iff_exists_eq_mul

alias isO_iff_exists_eq_mul ↔ IsO.exists_eq_mul _
...
theorem isOCat_iff_exists_eq_mul :
  ...
  · simp only [IsOCat_def]
    ...
    exact isOWith_of_eq_mul _ ((hφ c hpos).mono fun x => le_of_lt) huvφ
#align asymptotics.is_o_iff_exists_eq_mul Asymptotics.isOCat_iff_exists_eq_mul

alias isOCat_iff_exists_eq_mul ↔ IsOCat.exists_eq_mul _
...
theorem isO_iff_div_isBoundedUnder {α : Type _} {l : Filter α} {f g : α → 𝕜}
    ...
    f =O[l] g ↔ IsBoundedUnder (· ≤ ·) l fun x => ‖f x / g x‖ := by
  refine' ⟨div_isBoundedUnder_of_isO, fun h => _⟩
  obtain ⟨c, hc⟩ := h
  simp only [eventually_map, norm_div] at hc
  refine' IsO.of_bound c (hc.mp <| hgf.mono fun x hx₁ hx₂ => _)
  by_cases hgx : g x = 0
  · simp [hx₁ hgx, hgx]
...
theorem IsOCat.tendsto_zero_of_tendsto {α E 𝕜 : Type _} [NormedAddCommGroup E] [NormedField 𝕜]
  ...
  · rwa [isOCat_one_iff] at h
  exact huv.trans_isO (hv.isO_one 𝕜)
#align asymptotics.is_o.tendsto_zero_of_tendsto Asymptotics.IsOCat.tendsto_zero_of_tendsto

theorem isOCat_pow_pow {m n : ℕ} (h : m < n) : (fun x : 𝕜 => x ^ n) =o[𝓝 0] fun x => x ^ m := by
  ...
  exact IsO.mul_isOCat (isO_refl _ _) (IsOCat.pow ((isOCat_one_iff _).2 tendsto_id) hp0)
...
theorem isOCat_pow_id {n : ℕ} (h : 1 < n) : (fun x : 𝕜 => x ^ n) =o[𝓝 0] fun x => x := by
  convert isOCat_pow_pow h (𝕜 := 𝕜)
  simp only [pow_one]
#align asymptotics.is_o_pow_id Asymptotics.isOCat_pow_id

theorem isOCat_norm_pow_id {n : ℕ} (h : 1 < n) : (fun x : E' => ‖x‖ ^ n) =o[𝓝 0] fun x => x := by
  have := @isOCat_norm_pow_norm_pow E' _ _ _ h
  simp only [pow_one] at this
  exact isOCat_norm_right.mp this
...
theorem isOCat_pow_sub_pow_sub (x₀ : E') {n m : ℕ} (h : n < m) :
  ...
  (isOCat_pow_pow h).comp_tendsto this
...
theorem isOCat_pow_sub_sub (x₀ : E') {m : ℕ} (h : 1 < m) :
  ...
  simpa only [isOCat_norm_right, pow_one] using isOCat_pow_sub_pow_sub x₀ h
...
theorem bound_of_isO_cofinite (h : f =O[cofinite] g'') :
  ...
  rw [IsOWith_def, eventually_cofinite] at hC
...
theorem isO_cofinite_iff (h : ∀ x, g'' x = 0 → f'' x = 0) :
  ...
  ⟨fun h' =>
    let ⟨C, _C₀, hC⟩ := bound_of_isO_cofinite h'
...
theorem isO_nat_atTop_iff {f : ℕ → E''} {g : ℕ → F''} (h : ∀ x, g x = 0 → f x = 0) :
  ...
  rw [← Nat.cofinite_eq_atTop, isO_cofinite_iff h]
...
theorem isOWith_pi {ι : Type _} [Fintype ι] {E' : ι → Type _} [∀ i, NormedAddCommGroup (E' i)]
  ...
  simp only [isOWith_iff, pi_norm_le_iff_of_nonneg (this _), eventually_all]
...
theorem isO_pi {ι : Type _} [Fintype ι] {E' : ι → Type _} [∀ i, NormedAddCommGroup (E' i)]
  ...
  simp only [isO_iff_eventually_isOWith, ← eventually_all]
  exact eventually_congr (eventually_atTop.2 ⟨0, fun c => isOWith_pi⟩)
#align asymptotics.is_O_pi Asymptotics.isO_pi

@[simp]
theorem isOCat_pi {ι : Type _} [Fintype ι] {E' : ι → Type _} [∀ i, NormedAddCommGroup (E' i)]
    {f : α → ∀ i, E' i} : f =o[l] g' ↔ ∀ i, (fun x => f x i) =o[l] g' := by
  simp (config := { contextual := true }) only [IsOCat_def, isOWith_pi, le_of_lt]
...
theorem isO_congr (e : LocalHomeomorph α β) {b : β} (hb : b ∈ e.target) {f : β → E} {g : β → F} :
  ...
  simp only [IsO_def]
  exact exists_congr fun C => e.isOWith_congr hb
set_option linter.uppercaseLean3 false in
...
theorem isOCat_congr (e : LocalHomeomorph α β) {b : β} (hb : b ∈ e.target) {f : β → E} {g : β → F} :
    f =o[𝓝 b] g ↔ (f ∘ e) =o[𝓝 (e.symm b)] (g ∘ e) := by
  simp only [IsOCat_def]
  exact forall₂_congr fun c _hc => e.isOWith_congr hb
set_option linter.uppercaseLean3 false in
...
theorem isO_congr (e : α ≃ₜ β) {b : β} {f : β → E} {g : β → F} :
    f =O[𝓝 b] g ↔ (f ∘ e) =O[𝓝 (e.symm b)] (g ∘ e) := by
  simp only [IsO_def]
  exact exists_congr fun C => e.isOWith_congr
set_option linter.uppercaseLean3 false in
...
theorem isOCat_congr (e : α ≃ₜ β) {b : β} {f : β → E} {g : β → F} :
    f =o[𝓝 b] g ↔ (f ∘ e) =o[𝓝 (e.symm b)] (g ∘ e) := by
  simp only [IsOCat_def]
  exact forall₂_congr fun c _hc => e.isOWith_congr
