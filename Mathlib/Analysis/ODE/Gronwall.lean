...
theorem norm_le_gronwallBound_of_norm_deriv_left_le {f f' : ℝ → E} {δ K ε : ℝ} {a b : ℝ}
  ...
  set F : ℝ → E := fun x => f (b - x) with F_def
  set F' : ℝ → E := fun x => f' (b - x) with F'_def
  have hF : ContinuousOn F (Icc 0 <| b - a) := by
    apply hf.comp (continuous_sub_left b).continuousOn
    simpa using Set.mapsTo_const_sub_Icc b 0 (b-a)
  have hF' (x : ℝ) (hx : x ∈ Ico 0 (b - a)) : HasDerivWithinAt F (- F' x) (Ici x) x := by
    have := HasDerivWithinAt.scomp x (hf' (b-x) ?_)
      ((hasDerivAt_id' x).const_sub b).hasDerivWithinAt (s := Set.Ici x) ?_
    · simpa using this
    · simpa using Set.mapsTo_const_sub_Ico b 0 (b-a) hx
    · simpa using Set.mapsTo_const_sub_Ici b x
  have bound (x : ℝ) (hx : x ∈ Ico 0 (b - a)) : ‖-F' x‖ ≤ K * ‖F x‖ + ε := by
    simp [F_def, F'_def]
    apply bound
    simpa using Set.mapsTo_const_sub_Ico b 0 (b-a) hx
  intro x hx
  have hy : b - x ∈ Set.Icc 0 (b-a) := by
    simpa using Set.mapsTo_const_sub_Icc b a b hx
  have := norm_le_gronwallBound_of_norm_deriv_right_le hF hF' (by simpa [F_def, sub_zero]) bound _ hy
  simpa [F_def, sub_sub_cancel, sub_zero] using this

/-- Let `f : [a, b] → E` be a differentiable function such that `f a = 0`
and `‖f'(x)‖ ≤ K ‖f(x)‖` for some constant `K`. Then `f = 0` on `[a, b]`. -/
theorem eq_zero_of_abs_deriv_le_mul_abs_self_of_eq_zero_left
...
theorem eq_zero_of_abs_deriv_le_mul_abs_self_of_eq_zero_right
    ...
    (hf' : ∀ x ∈ Set.Ioc a b, HasDerivWithinAt f (f' x) (Set.Iic x) x)
    (ha : f b = 0) (bound : ∀ x ∈ Set.Ioc a b, ‖f' x‖ ≤ K * ‖f x‖) (x : ℝ) :
    x ∈ Set.Icc a b → f x = 0 := by
  intro hx
  apply norm_eq_zero.mp <| le_antisymm _ (norm_nonneg (f x))
  calc ‖f x‖ ≤ gronwallBound 0 K 0 (b - x) :=
    norm_le_gronwallBound_of_norm_deriv_left_le hf hf' ?_ ?_ _ hx
    _ = 0 := by rw [gronwallBound_ε0_δ0]
  · simp [ha]
  · simpa using bound
