...
instance (priority := 10000) : CoeFun (PicardLindelof E) fun _ => ℝ → E → E :=
  ⟨toFun⟩

instance (priority := 10000) : Inhabited (PicardLindelof E) :=
...
instance (priority := 10000) : CoeFun (FunSpace v) fun _ => Icc v.tMin v.tMax → E :=
  ⟨toFun⟩

instance (priority := 10000) : Inhabited v.FunSpace :=
...
instance (priority := 10000) : MetricSpace v.FunSpace :=
...
instance (priority := 10000) [CompleteSpace E] : CompleteSpace v.FunSpace := by
