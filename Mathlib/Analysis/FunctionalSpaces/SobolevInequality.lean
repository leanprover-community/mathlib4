...
theorem lintegral_pow_le_pow_lintegral_fderiv {u : E → F}
  ...
  have h0C : C = (c * ‖(e.symm : (ι → ℝ) →L[ℝ] E)‖₊ ^ p) * (c ^ p)⁻¹ := by
    simp_rw +zetaDelta [lintegralPowLePowLIntegralFDerivConst]
...
theorem eLpNorm_le_eLpNorm_fderiv_of_eq_inner  {u : E → F'}
  ...
  · convert eLpNorm_le_eLpNorm_fderiv_one μ hu h2u hn using 2
    ...
    · norm_cast
      simp_rw +zetaDelta [eLpNormLESNormFDerivOfEqInnerConst]
      field_simp
  -- the case `p > 1`
  ...
  calc (∫⁻ x, ‖u x‖₊ ^ (p' : ℝ) ∂μ) ^ (1 / (n' : ℝ)) = eLpNorm v n' μ := by
    ...
    _ ≤ C * γ * ((∫⁻ x, ‖u x‖₊ ^ (p' : ℝ) ∂μ) ^ (1 / q) *
        ...
        · simp_rw +zetaDelta [← ENNReal.rpow_mul, ← h3γ]
        · borelize F'
          fun_prop
        · fun_prop
...
theorem eLpNorm_le_eLpNorm_fderiv_of_eq [FiniteDimensional ℝ F]
  ...
  calc eLpNorm u p' μ
    ...
    _ = SNormLESNormFDerivOfEqConst F μ p * eLpNorm (fderiv ℝ u) p μ := by
      simp_rw +zetaDelta [SNormLESNormFDerivOfEqConst]
      push_cast
      simp_rw [mul_assoc]
