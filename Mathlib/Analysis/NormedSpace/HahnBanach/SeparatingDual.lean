...
variable {R V : Type _} [CommRing R] [AddCommGroup V] [TopologicalSpace V]
...
variable {R V : Type _} [NormedField R] [TopologicalSpace V] [AddCommGroup V]
  [TopologicalAddGroup V] [Module R V] [SeparatingDual R V]

lemma exists_eq_one {x : V} (hx : x ≠ 0) :
    ∃ f : V →L[R] R, f x = 1 := by
  rcases exists_ne_zero (R := R) hx with ⟨f, hf⟩
  refine ⟨(f x)⁻¹ • f, ?_⟩
  simp only [ContinuousLinearMap.coe_smul', Pi.smul_apply, smul_eq_mul, ne_eq, inv_mul_cancel hf]
...
theorem exists_continuousLinearMap_apply_eq [ContinuousSMul R V]
    {x y : V} (hx : x ≠ 0) (hy : y ≠ 0) :
    ∃ A : V ≃L[R] V, A x = y := by
