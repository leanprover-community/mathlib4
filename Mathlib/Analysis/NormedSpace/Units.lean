...
lemma inverse_add_norm (x : Rˣ) : (fun t : R => inverse (↑x + t)) =O[𝓝 0] fun _t => (1 : ℝ) := by
...
lemma inverse_continuousAt (x : Rˣ) : ContinuousAt inverse (x : R) := by
...
lemma closure_ne_top (I : Ideal R) (hI : I ≠ ⊤) : I.closure ≠ ⊤ := by
