...
@[simp]
lemma equiv_zero : WithLp.equiv p V 0 = 0 :=
  rfl
#align pi_Lp.equiv_zero WithLp.equiv_zero

@[simp]
lemma equiv_symm_zero : (WithLp.equiv p V).symm 0 = 0 :=
  rfl
#align pi_Lp.equiv_symm_zero WithLp.equiv_symm_zero

@[simp]
lemma equiv_add : WithLp.equiv p V (x + y) = WithLp.equiv p V x + WithLp.equiv p V y :=
  rfl
...
@[simp]
lemma equiv_sub : WithLp.equiv p V (x - y) = WithLp.equiv p V x - WithLp.equiv p V y :=
  rfl
...
@[simp]
lemma equiv_neg : WithLp.equiv p V (-x) = -WithLp.equiv p V x :=
  rfl
#align pi_Lp.equiv_neg WithLp.equiv_neg

@[simp]
lemma equiv_symm_neg : (WithLp.equiv p V).symm (-x') = -(WithLp.equiv p V).symm x' :=
  rfl
#align pi_Lp.equiv_symm_neg WithLp.equiv_symm_neg

@[simp]
lemma equiv_smul : WithLp.equiv p V (c • x) = c • WithLp.equiv p V x :=
  rfl
#align pi_Lp.equiv_smul WithLp.equiv_smul

@[simp]
lemma equiv_symm_smul : (WithLp.equiv p V).symm (c • x') = c • (WithLp.equiv p V).symm x' :=
  rfl
