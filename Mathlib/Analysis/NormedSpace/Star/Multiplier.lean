...
instance instCstarRing : CstarRing 𝓜(𝕜, A) where
  norm_star_mul_self := @fun (a : 𝓜(𝕜, A)) => congr_arg ((↑) : ℝ≥0 → ℝ) <|
    ...
    argument where `DenselyNormedField 𝕜` is required (for `sSup_unit_closedBall_eq_nnnorm`). -/
      ...
      rw [← nnnorm_snd]
      simp only [mul_snd, ← sSup_unit_closedBall_eq_nnnorm, star_snd, mul_apply]
      simp only [← @op_nnnorm_mul_apply 𝕜 _ A]
      simp only [← sSup_unit_closedBall_eq_nnnorm, mul_apply']
      ...
      · simp only [Set.mem_image, Set.mem_setOf_eq, exists_prop, exists_exists_and_eq_and]
        ...
        simp_rw [← nnnorm_fst, ← sSup_unit_closedBall_eq_nnnorm] at hr'
