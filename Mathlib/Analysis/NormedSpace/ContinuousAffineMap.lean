...
noncomputable instance (priority := 10000) hasNorm : Norm (V →A[𝕜] W) :=
...
noncomputable instance (priority := 10000) : NormedAddCommGroup (V →A[𝕜] W) :=
...
instance (priority := 10000) : NormedSpace 𝕜 (V →A[𝕜] W) where
  norm_smul_le t f := by
