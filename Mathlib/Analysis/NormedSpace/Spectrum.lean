...
theorem exp_mem_exp [ROrCLike 𝕜] [NormedRing A] [NormedAlgebra 𝕜 A] [CompleteSpace A] (a : A) {z : 𝕜}
  ...
  have hb : Summable fun n : ℕ => ((n + 1).factorial⁻¹ : 𝕜) • (a - ↑ₐ z) ^ n := by
    ...
    rw [norm_smul, mul_comm, norm_inv, ROrCLike.norm_natCast, ← div_eq_mul_inv]
