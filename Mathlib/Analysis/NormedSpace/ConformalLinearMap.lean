...
lemma isConformalMap_id : IsConformalMap (id R M) :=
...
@[nontriviality]
lemma isConformalMap_of_subsingleton [Subsingleton M] (f' : M →L[R] N) : IsConformalMap f' :=
...
protected lemma injective {f : M' →L[R] N} (h : IsConformalMap f) : Function.Injective f := by
...
lemma ne_zero [Nontrivial M'] {f' : M' →L[R] N} (hf' : IsConformalMap f') : f' ≠ 0 := by
  rintro rfl
