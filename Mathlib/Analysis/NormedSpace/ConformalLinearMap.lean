...
/-- A continuous linear map `f'` is said to be conformal if it's
    a nonzero multiple of a linear isometry. -/
def IsConformalMap {R : Type*} {X Y : Type*} [NormedField R] [AddCommGroup X] [SeminormedAddGroup X]
...
variable {R M N G M' : Type*} [NormedField R] [AddCommGroup M] [SeminormedAddGroup M]
