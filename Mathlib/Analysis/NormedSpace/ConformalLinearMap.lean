...
/-- A continuous linear map `f'` is said to be conformal if it's
    a nonzero multiple of a linear isometry. -/
def IsConformalMap {R : Type*} {X Y : Type*}
    [Field R] [StrictNormedRing R] [AddCommGroup X] [SeminormedAddGroup X]
    [AddCommGroup Y] [SeminormedAddGroup Y] [NormedSpace R X] [NormedSpace R Y] (f' : X →L[R] Y) :=
  ∃ c ≠ (0 : R), ∃ li : X →ₗᵢ[R] Y, f' = c • li.toContinuousLinearMap

variable {R M N G M' : Type*} [Field R] [StrictNormedRing R] [AddCommGroup M] [SeminormedAddGroup M]
  [AddCommGroup N] [SeminormedAddGroup N] [AddCommGroup G] [SeminormedAddGroup G]
  [NormedSpace R M] [NormedSpace R N] [NormedSpace R G]
  [AddCommGroup M'] [NormedAddGroup M'] [NormedSpace R M'] {f : M →L[R] N} {g : N →L[R] G} {c : R}
