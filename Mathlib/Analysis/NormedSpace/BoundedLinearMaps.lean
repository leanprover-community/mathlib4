...
theorem isBoundedLinearMap_prod_multilinear {E : ι → Type*} [∀ i, NormedAddCommGroup (E i)]
  ...
  { map_add := fun p₁ p₂ => by
      ext1 m
      -- ⊢ ↑(ContinuousMultilinearMap.prod (p₁ + p₂).fst (p₁ + p₂).snd) m = ↑(Continuou …
      rfl
      -- 🎉 no goals
    map_smul := fun c p => by
      ext1 m
      -- ⊢ ↑(ContinuousMultilinearMap.prod (c • p).fst (c • p).snd) m = ↑(c • Continuou …
      rfl
      -- 🎉 no goals
    bound :=
...
theorem isBoundedLinearMap_continuousMultilinearMap_comp_linear (g : G →L[𝕜] E) :
  ...
  · apply_rules [mul_nonneg, pow_nonneg, norm_nonneg]
    -- 🎉 no goals
  intro m
  -- ⊢ ‖↑(ContinuousMultilinearMap.compContinuousLinearMap f fun x => g) m‖ ≤ ‖g‖ ^ …
  calc
