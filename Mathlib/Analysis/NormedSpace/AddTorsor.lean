...
lemma AffineSubspace.isClosed_direction_iff (s : AffineSubspace 𝕜 Q) :
...
@[simp]
lemma dist_center_homothety (p₁ p₂ : P) (c : 𝕜) :
...
@[simp]
lemma nndist_center_homothety (p₁ p₂ : P) (c : 𝕜) :
...
@[simp]
lemma dist_lineMap_lineMap (p₁ p₂ : P) (c₁ c₂ : 𝕜) :
...
@[simp]
lemma nndist_lineMap_lineMap (p₁ p₂ : P) (c₁ c₂ : 𝕜) :
...
lemma lipschitzWith_lineMap (p₁ p₂ : P) : LipschitzWith (nndist p₁ p₂) (lineMap p₁ p₂ : 𝕜 → P) :=
...
@[simp]
lemma dist_lineMap_left (p₁ p₂ : P) (c : 𝕜) : dist (lineMap p₁ p₂ c) p₁ = ‖c‖ * dist p₁ p₂ := by
  -- Porting note: was
...
@[simp]
lemma nndist_homothety_self (p₁ p₂ : P) (c : 𝕜) :
...
@[simp] lemma dist_right_pointReflection (p q : P) :
...
lemma eventually_homothety_image_subset_of_finite_subset_interior (x : Q) {s : Set Q} {t : Set Q}
