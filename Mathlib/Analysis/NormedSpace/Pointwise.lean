...
theorem eventually_singleton_add_smul_subset {x : E} {s : Set E} (hs : Bornology.IsBounded s)
  ...
  apply hε
  simpa only [this, dist_eq_norm, add_sub_eq_right, mem_closedBall] using I
...
theorem NormedSpace.sphere_nonempty [Nontrivial E] {x : E} {r : ℝ} :
  ...
  simp only [mem_sphere_iff_norm, add_sub_eq_left, norm_smul, Real.norm_eq_abs, norm_inv, norm_norm,
    ne_eq, norm_eq_zero]
