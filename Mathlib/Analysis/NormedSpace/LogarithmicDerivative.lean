...
theorem DifferentiableAt.product {α : Type _} {ι : Finset α} (F : α → 𝕜 → 𝕜) (s : 𝕜)
  ...
  · simp only [Finset.cons_eq_insert]
    rw [← Finset.prod_fn, Finset.prod_insert]
    · apply DifferentiableAt.mul
      ...
      apply ih
      intro r
      simp only [Subtype.forall, Finset.cons_eq_insert, Finset.mem_insert, forall_eq_or_imp] at hd
      apply hd.2 _ r.2
    · exact ha
...
theorem logDeriv_tendsto (f : ℕ → ℂ → ℂ) (g : ℂ → ℂ) {s : Set ℂ} (hs : IsOpen s) (x : s)
    ...
    Tendsto (fun n : ℕ => logDeriv (f n) x) atTop (𝓝 ((logDeriv g) x)) := by
  simp_rw [logDeriv]
