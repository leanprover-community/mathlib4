...
lemma ofReal_alg (x : ℝ) : (x : K) = x • (1 : K) :=
...
lemma real_smul_eq_coe_smul [AddCommGroup E] [Module K E] [Module ℝ E] [IsScalarTower ℝ K E]
...
@[simp, rclike_simps]
lemma I_im (z : K) : im z * im (I : K) = im z :=
  mul_im_I_ax z
set_option linter.uppercaseLean3 false in
#align is_R_or_C.I_im RCLike.I_im

@[simp, rclike_simps]
lemma I_im' (z : K) : im (I : K) * im z = im z := by rw [mul_comm, I_im]
set_option linter.uppercaseLean3 false in
#align is_R_or_C.I_im' RCLike.I_im'

@[rclike_simps] -- porting note (#10618): was `simp`
lemma I_mul_re (z : K) : re (I * z) = -im z := by
  simp only [I_re, zero_sub, I_im', zero_mul, mul_re]
set_option linter.uppercaseLean3 false in
#align is_R_or_C.I_mul_re RCLike.I_mul_re

lemma I_mul_I : (I : K) = 0 ∨ (I : K) * I = -1 :=
  I_mul_I_ax
set_option linter.uppercaseLean3 false in
...
@[simp, rclike_simps]
lemma conj_re (z : K) : re (conj z) = re z :=
  RCLike.conj_re_ax z
#align is_R_or_C.conj_re RCLike.conj_re

@[simp, rclike_simps]
lemma conj_im (z : K) : im (conj z) = -im z :=
  RCLike.conj_im_ax z
#align is_R_or_C.conj_im RCLike.conj_im

@[simp, rclike_simps]
lemma conj_I : conj (I : K) = -I :=
  RCLike.conj_I_ax
set_option linter.uppercaseLean3 false in
#align is_R_or_C.conj_I RCLike.conj_I

@[simp, rclike_simps]
lemma conj_ofReal (r : ℝ) : conj (r : K) = (r : K) := by
  rw [ext_iff]
...
lemma conj_nat_cast (n : ℕ) : conj (n : K) = n := map_natCast _ _

-- See note [no_index around OfNat.ofNat]
lemma conj_ofNat (n : ℕ) [n.AtLeastTwo] : conj (no_index (OfNat.ofNat n : K)) = OfNat.ofNat n :=
  map_ofNat _ _

@[rclike_simps] -- Porting note (#10618): was a `simp` but `simp` can prove it
lemma conj_neg_I : conj (-I) = (I : K) := by rw [map_neg, conj_I, neg_neg]
set_option linter.uppercaseLean3 false in
...
lemma sub_conj (z : K) : z - conj z = 2 * im z * I :=
  calc
...
@[rclike_simps]
lemma conj_smul (r : ℝ) (z : K) : conj (r • z) = r • conj z := by
...
@[simp, norm_cast, rclike_simps]
lemma ofReal_inv (r : ℝ) : ((r⁻¹ : ℝ) : K) = (r : K)⁻¹ :=
  map_inv₀ _ r
#align is_R_or_C.of_real_inv RCLike.ofReal_inv

lemma inv_def (z : K) : z⁻¹ = conj z * ((‖z‖ ^ 2)⁻¹ : ℝ) := by
...
@[simp, rclike_simps]
lemma div_I (z : K) : z / I = -(z * I) := by rw [div_eq_mul_inv, inv_I, mul_neg]
set_option linter.uppercaseLean3 false in
#align is_R_or_C.div_I RCLike.div_I

@[rclike_simps] -- porting note (#10618): was `simp`
lemma normSq_inv (z : K) : normSq z⁻¹ = (normSq z)⁻¹ :=
  map_inv₀ normSq z
#align is_R_or_C.norm_sq_inv RCLike.normSq_inv

@[rclike_simps] -- porting note (#10618): was `simp`
lemma normSq_div (z w : K) : normSq (z / w) = normSq z / normSq w :=
  map_div₀ normSq z w
#align is_R_or_C.norm_sq_div RCLike.normSq_div

@[rclike_simps] -- porting note (#10618): was `simp`
lemma norm_conj {z : K} : ‖conj z‖ = ‖z‖ := by simp only [← sqrt_normSq_eq_norm, normSq_conj]
...
@[simp, rclike_simps, norm_cast]
lemma ofReal_natCast (n : ℕ) : ((n : ℝ) : K) = n :=
...
lemma natCast_re (n : ℕ) : re (n : K) = n := by rw [← ofReal_natCast, ofReal_re]
#align is_R_or_C.nat_cast_re RCLike.natCast_re

@[simp, rclike_simps, norm_cast]
lemma natCast_im (n : ℕ) : im (n : K) = 0 := by rw [← ofReal_natCast, ofReal_im]
#align is_R_or_C.nat_cast_im RCLike.natCast_im

-- See note [no_index around OfNat.ofNat]
@[simp, rclike_simps]
lemma ofNat_re (n : ℕ) [n.AtLeastTwo] : re (no_index (OfNat.ofNat n) : K) = OfNat.ofNat n :=
  natCast_re n

-- See note [no_index around OfNat.ofNat]
@[simp, rclike_simps]
lemma ofNat_im (n : ℕ) [n.AtLeastTwo] : im (no_index (OfNat.ofNat n) : K) = 0 :=
  natCast_im n

-- See note [no_index around OfNat.ofNat]
@[simp, rclike_simps, norm_cast]
lemma ofReal_ofNat (n : ℕ) [n.AtLeastTwo] :
    ((no_index (OfNat.ofNat n) : ℝ) : K) = OfNat.ofNat n :=
  ofReal_natCast n

lemma ofNat_mul_re (n : ℕ) [n.AtLeastTwo] (z : K) :
    re (OfNat.ofNat n * z) = OfNat.ofNat n * re z := by
  rw [← ofReal_ofNat, re_ofReal_mul]

lemma ofNat_mul_im (n : ℕ) [n.AtLeastTwo] (z : K) :
    im (OfNat.ofNat n * z) = OfNat.ofNat n * im z := by
  rw [← ofReal_ofNat, im_ofReal_mul]

@[simp, rclike_simps, norm_cast]
lemma ofReal_intCast (n : ℤ) : ((n : ℝ) : K) = n :=
  map_intCast _ n
#align is_R_or_C.of_real_int_cast RCLike.ofReal_intCast

@[simp, rclike_simps] -- Porting note: removed `norm_cast`
lemma intCast_re (n : ℤ) : re (n : K) = n := by rw [← ofReal_intCast, ofReal_re]
#align is_R_or_C.int_cast_re RCLike.intCast_re

@[simp, rclike_simps, norm_cast]
lemma intCast_im (n : ℤ) : im (n : K) = 0 := by rw [← ofReal_intCast, ofReal_im]
#align is_R_or_C.int_cast_im RCLike.intCast_im

@[simp, rclike_simps, norm_cast]
lemma ofReal_ratCast (n : ℚ) : ((n : ℝ) : K) = n :=
  map_ratCast _ n
#align is_R_or_C.of_real_rat_cast RCLike.ofReal_ratCast

@[simp, rclike_simps] -- Porting note: removed `norm_cast`
lemma ratCast_re (q : ℚ) : re (q : K) = q := by rw [← ofReal_ratCast, ofReal_re]
#align is_R_or_C.rat_cast_re RCLike.ratCast_re

@[simp, rclike_simps, norm_cast]
lemma ratCast_im (q : ℚ) : im (q : K) = 0 := by rw [← ofReal_ratCast, ofReal_im]
...
@[simp, rclike_simps, norm_cast]
lemma norm_natCast (n : ℕ) : ‖(n : K)‖ = n := by
  rw [← ofReal_natCast]
  exact norm_of_nonneg (Nat.cast_nonneg n)
#align is_R_or_C.norm_nat_cast RCLike.norm_natCast

@[simp, rclike_simps]
lemma norm_ofNat (n : ℕ) [n.AtLeastTwo] : ‖(no_index (OfNat.ofNat n) : K)‖ = OfNat.ofNat n :=
  norm_natCast n
...
lemma mul_self_norm (z : K) : ‖z‖ * ‖z‖ = normSq z := by rw [normSq_eq_def', sq]
...
lemma abs_re_le_norm (z : K) : |re z| ≤ ‖z‖ := by
...
lemma abs_im_le_norm (z : K) : |im z| ≤ ‖z‖ := by
  rw [mul_self_le_mul_self_iff (abs_nonneg _) (norm_nonneg _), abs_mul_abs_self, mul_self_norm]
...
lemma abs_im_div_norm_le_one (z : K) : |im z / ‖z‖| ≤ 1 := by
  rw [abs_div, abs_norm]
...
lemma isCauSeq_re (f : CauSeq K norm) : IsCauSeq abs fun n => re (f n) := fun ε ε0 =>
...
lemma isCauSeq_im (f : CauSeq K norm) : IsCauSeq abs fun n => im (f n) := fun ε ε0 =>
  (f.cauchy ε0).imp fun i H j ij =>
...
lemma isCauSeq_norm {f : ℕ → K} (hf : IsCauSeq norm f) : IsCauSeq abs (norm ∘ f) := fun ε ε0 =>
  let ⟨i, hi⟩ := hf ε ε0
...
lemma lt_iff_re_im : z < w ↔ re z < re w ∧ im z = im w := by
...
lemma toOrderedSMul : OrderedSMul ℝ K :=
...
@[simp, rclike_simps, norm_cast]
lemma reCLM_coe : ((reCLM : K →L[ℝ] ℝ) : K →ₗ[ℝ] ℝ) = reLm :=
  rfl
#align is_R_or_C.re_clm_coe RCLike.reCLM_coe

@[simp, rclike_simps]
lemma reCLM_apply : ((reCLM : K →L[ℝ] ℝ) : K → ℝ) = re :=
  rfl
#align is_R_or_C.re_clm_apply RCLike.reCLM_apply

@[continuity, fun_prop]
lemma continuous_re : Continuous (re : K → ℝ) :=
  reCLM.continuous
...
@[simp, rclike_simps, norm_cast]
lemma imCLM_coe : ((imCLM : K →L[ℝ] ℝ) : K →ₗ[ℝ] ℝ) = imLm :=
  rfl
#align is_R_or_C.im_clm_coe RCLike.imCLM_coe

@[simp, rclike_simps]
lemma imCLM_apply : ((imCLM : K →L[ℝ] ℝ) : K → ℝ) = im :=
  rfl
#align is_R_or_C.im_clm_apply RCLike.imCLM_apply

@[continuity, fun_prop]
lemma continuous_im : Continuous (im : K → ℝ) :=
  imCLM.continuous
...
@[simp, rclike_simps]
lemma conjAe_coe : (conjAe : K → K) = conj :=
  rfl
...
@[simp, rclike_simps]
lemma conjLIE_apply : (conjLIE : K → K) = conj :=
  rfl
...
@[simp, rclike_simps]
lemma conjCLE_coe : (@conjCLE K _).toLinearEquiv = conjAe.toLinearEquiv :=
  rfl
#align is_R_or_C.conj_cle_coe RCLike.conjCLE_coe

@[simp, rclike_simps]
lemma conjCLE_apply : (conjCLE : K → K) = conj :=
  rfl
...
@[continuity]
lemma continuous_conj : Continuous (conj : K → K) :=
  continuous_star
...
@[simp, rclike_simps]
lemma ofRealAm_coe : (ofRealAm : ℝ → K) = ofReal :=
  rfl
...
@[simp, rclike_simps]
lemma ofRealLI_apply : (ofRealLI : ℝ → K) = ofReal :=
  rfl
...
@[simp, rclike_simps]
lemma ofRealCLM_coe : (@ofRealCLM K _ : ℝ →ₗ[ℝ] K) = ofRealAm.toLinearMap :=
  rfl
#align is_R_or_C.of_real_clm_coe RCLike.ofRealCLM_coe

@[simp, rclike_simps]
lemma ofRealCLM_apply : (ofRealCLM : ℝ → K) = ofReal :=
  rfl
#align is_R_or_C.of_real_clm_apply RCLike.ofRealCLM_apply

@[continuity, fun_prop]
lemma continuous_ofReal : Continuous (ofReal : ℝ → K) :=
  ofRealLI.continuous
#align is_R_or_C.continuous_of_real RCLike.continuous_ofReal

@[continuity]
lemma continuous_normSq : Continuous (normSq : K → ℝ) :=
