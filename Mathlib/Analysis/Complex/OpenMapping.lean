...
theorem isOpenQuotientMap_pow_units (n : ℕ) [NeZero n] : IsOpenQuotientMap (· ^ n : ℂˣ → ℂˣ) where
  surjective := _
  continuous := continuous_pow _
  isOpenMap := _

/-theorem isOpenQuotientMap_zpow (n : ℤ) : IsOpenQuotientMap (· ^ n : ℂ → ℂ) := by
  _ -/
