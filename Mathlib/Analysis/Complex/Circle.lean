...
Because later (in `Mathlib/Geometry/Manifold/Instances/Sphere.lean`) one wants to equip the circle
with a smooth manifold structure borrowed from `Metric.sphere`, the underlying set is
