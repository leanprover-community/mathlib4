...
lemma interior_reProdIm (s t : Set ℝ) : interior (s ×ℂ t) = interior s ×ℂ interior t := by
...
lemma frontier_reProdIm (s t : Set ℝ) :
...
lemma frontier_setOf_le_re_and_le_im (a b : ℝ) :
...
lemma frontier_setOf_le_re_and_im_le (a b : ℝ) :
...
lemma IsOpen.reProdIm (hs : IsOpen s) (ht : IsOpen t) : IsOpen (s ×ℂ t) :=
  (hs.preimage continuous_re).inter (ht.preimage continuous_im)
#align is_open.re_prod_im IsOpen.reProdIm

lemma IsClosed.reProdIm (hs : IsClosed s) (ht : IsClosed t) : IsClosed (s ×ℂ t) :=
  (hs.preimage continuous_re).inter (ht.preimage continuous_im)
#align is_closed.re_prod_im IsClosed.reProdIm

lemma Bornology.IsBounded.reProdIm (hs : IsBounded s) (ht : IsBounded t) : IsBounded (s ×ℂ t) :=
