...
theorem zero_form_isBoundedAtImInfty {α : Type*} [NormMetric α] [Field α] [IsNormedField α] :
    IsBoundedAtImInfty (0 : ℍ → α) :=
  const_boundedAtFilter atImInfty (0 : α)

/-- Module of functions that are zero at infinity. -/
def zeroAtImInftySubmodule (α : Type*) [NormMetric α] [Field α] [IsNormedField α] : Submodule α (ℍ → α) :=
  zeroAtFilterSubmodule _ atImInfty

/-- Subalgebra of functions that are bounded at infinity. -/
def boundedAtImInftySubalgebra (α : Type*) [NormMetric α] [Field α] [IsNormedField α] : Subalgebra α (ℍ → α) :=
...
theorem isZeroAtImInfty_iff {α : Type*} [NormPseudoMetric α] [AddGroup α] [IsNormedAddGroup α] {f : ℍ → α} :
    IsZeroAtImInfty f ↔ ∀ ε : ℝ, 0 < ε → ∃ A : ℝ, ∀ z : ℍ, A ≤ im z → ‖f z‖ ≤ ε :=
  (atImInfty_basis.tendsto_iff Metric.nhds_basis_closedBall).trans <| by simp

theorem IsZeroAtImInfty.isBoundedAtImInfty {α : Type*} [NormPseudoMetric α] [AddGroup α] [IsNormedAddGroup α] {f : ℍ → α}
