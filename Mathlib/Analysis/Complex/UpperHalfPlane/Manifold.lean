...
lemma contMDiff_num (g : GL(2, ℝ)) : ContMDiff 𝓘(ℂ) 𝓘(ℂ) n (fun τ : ℍ ↦ num g τ) :=
  (contMDiff_const.mul contMDiff_coe).add contMDiff_const

lemma contMDiff_denom (g : GL(2, ℝ)) : ContMDiff 𝓘(ℂ) 𝓘(ℂ) n (fun τ : ℍ ↦ denom g τ) :=
...
lemma contMDiff_inv_denom (g : GL(2, ℝ)) : ContMDiff 𝓘(ℂ) 𝓘(ℂ) n (fun τ : ℍ ↦ (denom g τ)⁻¹) := by
  simpa using contMDiff_denom_zpow g (-1)

/-- Each element of `GL(2, ℝ)⁺` defines a map of `C ^ n` manifolds `ℍ → ℍ`. -/
lemma contMDiff_smul {g : GL(2, ℝ)} (hg : 0 < g.det.val) :
...
lemma mdifferentiable_smul {g : GL(2, ℝ)} (hg : 0 < g.det.val) :
