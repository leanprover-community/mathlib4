...
/-! The former hand-written extensions `evalUpperHalfPlaneIm` and `evalUpperHalfPlaneCoe` are
replaced by `@[auto_positivity]`: the imaginary part of a point of the upper half plane is
positive, and its coercion to `ℂ` is nonzero. -/
