...
This file defines `UpperHalfPlane` to be the upper half plane in `ℂ`, and equip it with actions
of `ℝ` by addition and `ℝˣ` by multiplication. The action of the general linear group is defined
elsewhere.
