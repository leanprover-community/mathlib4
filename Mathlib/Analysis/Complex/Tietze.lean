...
- `ROrCLike.instTietzeExtensionTVS`: finite dimensional topological vector spaces over `ℝ` (or `ℂ`)
...
instance Set.instTietzeExtensionUnitBall {𝕜 : Type v} [ROrCLike 𝕜] {E : Type w}
...
instance Set.instTietzeExtensionUnitClosedBall {𝕜 : Type v} [ROrCLike 𝕜] {E : Type w}
    [NormedAddCommGroup E] [NormedSpace 𝕜 E] [FiniteDimensional 𝕜 E] :
  ...
  suffices this : Continuous (piecewise (Metric.closedBall 0 1) id g) by
    ...
    · by_cases hx : x ∈ Metric.closedBall 0 1
      ...
      · simp only [g, piecewise_eq_of_not_mem (hi := hx), ROrCLike.real_smul_eq_coe_smul (K := 𝕜)]
...
theorem Metric.instTietzeExtensionBall {𝕜 : Type v} [ROrCLike 𝕜] {E : Type w}
...
theorem Metric.instTietzeExtensionClosedBall (𝕜 : Type v) [ROrCLike 𝕜] {E : Type w}
