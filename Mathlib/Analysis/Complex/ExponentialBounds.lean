...
theorem log_three_near_10 : |log 3 - (109861228867 : ℝ) / 100000000000| ≤ 1 / 10 ^ 10 := by
  suffices |log 3 - (109861228867 : ℝ) / 100000000000| ≤
      ((2 : ℝ) / 3) ^ (70 + 1) / (3 : ℝ)⁻¹ +
        (1 / 10 ^ 10 - ((2 : ℝ) / 3) ^ (70 + 1) / (3 : ℝ)⁻¹) by
    norm_num1 at *
    assumption
  have t : |((2 : ℝ) / 3)| = (2 : ℝ) / 3 := by rw [abs_of_pos]; norm_num
  have z := Real.abs_log_sub_add_sum_range_le (show |((2 : ℝ) / 3)| < 1 by rw [t]; norm_num) 70
  rw [t] at z
  rw [show (1 - (2 : ℝ) / 3) = (1 / 3 : ℝ) by norm_num,
    show (1 / 3 : ℝ) = (3 : ℝ)⁻¹ by norm_num, log_inv, ← sub_eq_add_neg,
    _root_.abs_sub_comm] at z
  apply le_trans (_root_.abs_sub_le _ _ _) (add_le_add z _)
  simp_rw [sum_range_succ]
  norm_num
...
theorem log_five_near_10 : |log 5 - (160943791243 : ℝ) / 100000000000| ≤ 1 / 10 ^ 10 := by
  suffices |log 5 - (160943791243 : ℝ) / 100000000000| ≤
      ((4 : ℝ) / 5) ^ (130 + 1) / (5 : ℝ)⁻¹ +
        (1 / 10 ^ 10 - ((4 : ℝ) / 5) ^ (130 + 1) / (5 : ℝ)⁻¹) by
    norm_num1 at *
    assumption
  have t : |((4 : ℝ) / 5)| = (4 : ℝ) / 5 := by rw [abs_of_pos]; norm_num
  have z := Real.abs_log_sub_add_sum_range_le (show |((4 : ℝ) / 5)| < 1 by rw [t]; norm_num) 130
  rw [t] at z
  rw [show (1 - (4 : ℝ) / 5) = (1 / 5 : ℝ) by norm_num,
    show (1 / 5 : ℝ) = (5 : ℝ)⁻¹ by norm_num, log_inv, ← sub_eq_add_neg,
    _root_.abs_sub_comm] at z
  apply le_trans (_root_.abs_sub_le _ _ _) (add_le_add z _)
  simp_rw [sum_range_succ]
  norm_num
