...
-/
class WStarAlgebra (M : Type u) [Ring M] [CStarAlgebra M] : Prop where
  /-- There is a Banach space `X` whose dual is isometrically (conjugate-linearly) isomorphic
  to the `WStarAlgebra`. -/
  exists_predual :
    ∃ (X : Type u) (_ : AddCommGroup X) (_ : NormedAddGroup X) (_ : NormedSpace ℂ X)
      (_ : CompleteSpace X),
...
-/
structure VonNeumannAlgebra (H : Type u) [AddCommGroup H] [NormedAddGroup H] [InnerProductSpace ℂ H]
...
variable {H : Type u} [AddCommGroup H] [NormedAddGroup H] [InnerProductSpace ℂ H] [CompleteSpace H]
