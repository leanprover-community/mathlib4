...
instance [CStarAlgebra A] (x : A) [IsStarNormal x] :
    CommCStarAlgebra (StarAlgebra.elemental ℂ x) where

instance [NonUnitalCStarAlgebra A] (x : A) [IsStarNormal x] :
...
instance [(i : ι) → NonUnitalCStarAlgebra (A i)] :
    NonUnitalCStarAlgebra (Π i, A i) where

instance [(i : ι) → NonUnitalCommCStarAlgebra (A i)] :
