...
+ `CStarAlgebra.predirectedOn_nonneg_ball`: the set `{x : A | 0 ≤ x} ∩ Metric.ball 0 1` is directed.
...
lemma CStarAlgebra.predirectedOn_nonneg_ball :
    PredirectedOn (· ≤ ·) ({x : A | 0 ≤ x} ∩ Metric.ball 0 1) := by
...
lemma isBasis_nonneg_sections :
  ...
  nonempty := ⟨0, by simp⟩
  inter {x y} hx hy := by
    peel predirectedOn_nonneg_ball x (by simpa) y (by simpa) with z hz
