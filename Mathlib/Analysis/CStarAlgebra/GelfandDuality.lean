...
lemma IsStarNormal.norm_add_eq_max {a b : A} (ha : IsStarNormal a) (hb : IsStarNormal b)
    (hab₁ : Commute a b) (hab₂ : Commute a (star b)) (hab : a * b = 0) :
    ‖a + b‖ = max ‖a‖ ‖b‖ := by
  /- Since `a` and `b` are normal and commute and commute with the `star` of the other,
  the C⋆-subalgebra generated by `a` and `b` is commutative, and the result follows from the
  correpsonding result for commutative C⋆-algebras. -/
  ...
  let _ : NonUnitalCommRing (adjoin ℂ {a, b}) :=
    adjoinNonUnitalCommRingOfComm ℂ (by grind) (by grind [commute_star_comm])
...
lemma IsStarNormal.nnnorm_add_eq_max {a b : A} (ha : IsStarNormal a) (hb : IsStarNormal b)
    (hab₁ : Commute a b) (hab₂ : Commute a (star b)) (hab : a * b = 0) :
    ‖a + b‖₊ = max ‖a‖₊ ‖b‖₊ :=
  NNReal.eq <| ha.norm_add_eq_max hb hab₁ hab₂ hab

lemma IsStarNormal.norm_sub_eq_max {a b : A} (ha : IsStarNormal a) (hb : IsStarNormal b)
    (hab₁ : Commute a b) (hab₂ : Commute a (star b)) (hab : a * b = 0) :
    ‖a - b‖ = max ‖a‖ ‖b‖ := by
  simpa [sub_eq_add_neg] using
    ha.norm_add_eq_max hb.neg hab₁.neg_right (by simpa using hab₂) (by simpa)

lemma IsStarNormal.nnnorm_sub_eq_max {a b : A} (ha : IsStarNormal a) (hb : IsStarNormal b)
    (hab₁ : Commute a b) (hab₂ : Commute a (star b)) (hab : a * b = 0) :
    ‖a - b‖₊ = max ‖a‖₊ ‖b‖₊ :=
  NNReal.eq <| ha.norm_sub_eq_max hb hab₁ hab₂ hab

open NonUnitalStarAlgebra in
lemma IsSelfAdjoint.norm_add_eq_max {a b : A}
    (ha : IsSelfAdjoint a) (hb : IsSelfAdjoint b) (hab : a * b = 0) :
    ‖a + b‖ = max ‖a‖ ‖b‖ :=
...
lemma IsSelfAdjoint.nnnorm_add_eq_max {a b : A}
    (ha : IsSelfAdjoint a) (hb : IsSelfAdjoint b) (hab : a * b = 0) :
    ‖a + b‖₊ = max ‖a‖₊ ‖b‖₊ :=
  NNReal.eq <| ha.norm_add_eq_max hb hab

lemma IsSelfAdjoint.norm_sub_eq_max {a b : A}
    (ha : IsSelfAdjoint a) (hb : IsSelfAdjoint b) (hab : a * b = 0) :
    ‖a - b‖ = max ‖a‖ ‖b‖ := by
  simpa [sub_eq_add_neg] using ha.norm_add_eq_max hb.neg (by simpa)

lemma IsSelfAdjoint.nnnorm_sub_eq_max {a b : A}
    (ha : IsSelfAdjoint a) (hb : IsSelfAdjoint b) (hab : a * b = 0) :
    ‖a - b‖₊ = max ‖a‖₊ ‖b‖₊ :=
