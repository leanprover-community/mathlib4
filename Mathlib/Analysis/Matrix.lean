...
lemma linfty_opNorm_def (A : Matrix m n α) :
...
lemma linfty_opNNNorm_def (A : Matrix m n α) :
...
lemma linfty_opNNNorm_col (v : m → α) : ‖col v‖₊ = ‖v‖₊ := by
...
@[simp]
lemma linfty_opNorm_col (v : m → α) : ‖col v‖ = ‖v‖ :=
...
@[simp]
lemma linfty_opNNNorm_row (v : n → α) : ‖row v‖₊ = ∑ i, ‖v i‖₊ := by simp [linfty_opNNNorm_def]
...
@[simp]
lemma linfty_opNorm_row (v : n → α) : ‖row v‖ = ∑ i, ‖v i‖ :=
...
@[simp]
lemma linfty_opNNNorm_diagonal [DecidableEq m] (v : m → α) : ‖diagonal v‖₊ = ‖v‖₊ := by
  rw [linfty_opNNNorm_def, Pi.nnnorm_def]
  congr 1 with i : 1
...
@[simp]
lemma linfty_opNorm_diagonal [DecidableEq m] (v : m → α) : ‖diagonal v‖ = ‖v‖ :=
...
lemma linfty_opNNNorm_mul (A : Matrix l m α) (B : Matrix m n α) : ‖A * B‖₊ ≤ ‖A‖₊ * ‖B‖₊ := by
...
lemma linfty_opNorm_mul (A : Matrix l m α) (B : Matrix m n α) : ‖A * B‖ ≤ ‖A‖ * ‖B‖ :=
...
lemma linfty_opNNNorm_mulVec (A : Matrix l m α) (v : m → α) : ‖A *ᵥ v‖₊ ≤ ‖A‖₊ * ‖v‖₊ := by
...
lemma linfty_opNorm_mulVec (A : Matrix l m α) (v : m → α) : ‖A *ᵥ v‖ ≤ ‖A‖ * ‖v‖ :=
...
private lemma norm_unitOf (a : α) : ‖unitOf a‖₊ = 1 := by
  rw [unitOf]
  split_ifs with h
  · simp
...
private lemma mul_unitOf (a : α) : a * unitOf a = algebraMap _ _ (‖a‖₊ : ℝ)  := by
  simp [unitOf]
  split_ifs with h
  · simp [h]
