...
theorem fourier_integral_convergent_iff (he : Continuous e)
  ...
  have aux :
    ...
    have c : Continuous fun v => e (-L v x) := by
      refine' (he).comp (continuous_ofAdd.comp (Continuous.neg _))
  ...
  convert aux hf (-w)
  rw [← smul_assoc, smul_eq_mul, Submonoid.smul_def, ← AddChar.map_add_mul,
    LinearMap.map_neg, neg_neg, ← sub_eq_add_neg, sub_self, AddChar.map_zero_one,
