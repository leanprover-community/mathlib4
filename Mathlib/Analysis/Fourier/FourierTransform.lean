...
introduce the notation `𝐞` in the `FourierTransform` namespace).
...
theorem fourierIntegral_comp_add_right [MeasurableAdd V] (e : AddChar 𝕜 𝕊) (μ : Measure V)
    ...
    fourierIntegral e μ L (f ∘ fun v => v + v₀) =
      fun w ↦ (e (L v₀ w) : ℂ) • fourierIntegral e μ L f w := by
  ext1 w
...
theorem fourier_integral_convergent_iff (he : Continuous e)
    ...
    Integrable f μ ↔ Integrable (fun v ↦ (e (-L v w) : ℂ) • f v) μ := by
  -- first prove one-way implication
  have aux {g : V → E} (hg : Integrable g μ) (x : W) :
      Integrable (fun v ↦ (e (-L v x) : ℂ) • g v) μ := by
    have c : Continuous fun v ↦ (e (-L v x) : ℂ) := (continuous_induced_rng.mp he).comp
      (hL.comp (continuous_prod_mk.mpr ⟨continuous_id, continuous_const⟩)).neg
  ...
  have := aux hf (-w)
  simp_rw [← mul_smul, ← Submonoid.coe_mul, ← AddChar.map_add_mul, LinearMap.map_neg, neg_neg,
    add_neg_self, AddChar.map_zero_one, Submonoid.coe_one, one_smul] at this
  exact this
...
theorem integral_bilin_fourierIntegral_eq_flip
  ...
  calc
  ∫ ξ, M (fourierIntegral e μ L f ξ) (g ξ) ∂ν
    = ∫ ξ, M.flip (g ξ) (∫ x, (e (-L x ξ) : ℂ) • f x ∂μ) ∂ν := rfl
  _ = ∫ ξ, (∫ x, M.flip (g ξ) ((e (-L x ξ) : ℂ) • f x) ∂μ) ∂ν := by
    congr with ξ
    apply (ContinuousLinearMap.integral_comp_comm _ _).symm
    exact (fourier_integral_convergent_iff he hL _).1 hf
  _ = ∫ x, (∫ ξ, M.flip (g ξ) ((e (-L x ξ) : ℂ) • f x) ∂ν) ∂μ := by
    ...
    · have A : AEStronglyMeasurable (fun (p : W × V) ↦ e (-L p.2 p.1) • f p.2) (ν.prod μ) :=
        (he.comp (hL.comp continuous_swap).neg).aestronglyMeasurable.smul hf.1.snd
      have A' := hg.1.fst.prod_mk A
      show AEStronglyMeasurable (fun p : W × V ↦ M (e (-(L p.2) p.1) • f p.2) (g p.1)) (ν.prod μ)
      have : Continuous fun p : F × E ↦ M p.2 p.1 := M.flip.continuous₂
      have := this.comp_aestronglyMeasurable A'
      exact this
    · apply eventually_of_forall
      rintro ⟨ξ, x⟩
      simp only [SMulHomClass.map_smul, ContinuousLinearMap.flip_apply, Function.uncurry_apply_pair,
        norm_smul, norm_eq_of_mem_sphere, one_mul, norm_mul, norm_norm]
      exact (M.le_opNorm₂ (f x) (g ξ)).trans (le_of_eq (by ring))
  _ = ∫ x, (∫ ξ, M (f x) ((e (-L.flip ξ x) : ℂ) • g ξ) ∂ν) ∂μ := by
  ...
  _ = ∫ x, M (f x) (∫ ξ, (e (-L.flip ξ x) : ℂ) • g ξ ∂ν) ∂μ := by
    congr with x
...
def fourierChar : AddChar ℝ 𝕊 where
  ...
  map_add_mul' a b := by simp only [mul_add, expMapCircle_add]
  map_zero_one' := by simp only [mul_zero, expMapCircle_zero]
...
theorem fourierIntegral_real_eq (f : ℝ → E) (w : ℝ) :
    fourierIntegral f w = ∫ v : ℝ, (𝐞 (-(v * w)) : ℂ) • f v :=
  rfl
