...
noncomputable def haarAddCircle : Measure (AddCircle T) :=
  addHaarMeasure ⊤
...
noncomputable def fourier (n : ℤ) : C(AddCircle T, ℂ) where
...
noncomputable def fourierSubalgebra : StarSubalgebra ℂ C(AddCircle T, ℂ) where
...
noncomputable abbrev fourierLp (p : ℝ≥0∞) [Fact (1 ≤ p)] (n : ℤ) : Lp ℂ p (@haarAddCircle T hT) :=
...
noncomputable def fourierCoeff (f : AddCircle T → E) (n : ℤ) : E :=
...
noncomputable def fourierCoeffOn {a b : ℝ} (hab : a < b) (f : ℝ → E) (n : ℤ) : E :=
...
noncomputable def fourierBasis : HilbertBasis ℤ ℂ (Lp ℂ 2 <| @haarAddCircle T hT) :=
