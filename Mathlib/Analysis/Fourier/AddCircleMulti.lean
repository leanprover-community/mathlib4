...
noncomputable def mFourier : C(UnitAddTorus d, ℂ) where
...
noncomputable abbrev mFourierLp (p : ℝ≥0∞) [Fact (1 ≤ p)] (n : d → ℤ) :
...
to `ℓ²(ℤᵈ, ℂ)`. -/
noncomputable def mFourierBasis : HilbertBasis (d → ℤ) ℂ L²(UnitAddTorus d) :=
