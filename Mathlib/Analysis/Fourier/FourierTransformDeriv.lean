...
lemma hasFDerivAt_fourierChar_neg_bilinear_right (v : V) (w : W) :
  ...
  ext y
  simp only [neg_mul, ContinuousLinearMap.coe_comp', smul_apply,
    Function.comp_apply, ofRealCLM_apply, smul_eq_mul, ContinuousLinearMap.comp_neg,
    neg_apply, ContinuousLinearMap.toSpanSingleton_apply, real_smul, neg_inj]
  ring
...
lemma fderiv_fourierChar_neg_bilinear_right_apply (v : V) (w y : W) :
  ...
  simp only [(hasFDerivAt_fourierChar_neg_bilinear_right L v w).fderiv, neg_mul,
    ContinuousLinearMap.coe_comp', smul_apply,
...
theorem fourierIntegral_iteratedFDeriv [FiniteDimensional ℝ V]
  ...
  | succ n ih =>
    ...
    simp only [ih h'n.le, fourierSMulRight_apply, _root_.neg_apply,
      ...
      ContinuousMultilinearMap.smul_apply, fourierPowSMulRight_apply, ← coe_smul (E := E),
      smul_smul]
    congr 1
...
lemma hasDerivAt_fourier
  ...
  simp_rw [smul_apply, fourierSMulRight, smul_apply,
...
theorem fourier_iteratedDeriv {f : ℝ → E} {N : ℕ∞} {n : ℕ} (hf : ContDiff ℝ N f)
  ...
  simp [← Complex.coe_smul, smul_smul, ← mul_pow, innerSL_apply_apply ℝ]
