...
variable {𝕜 E A B : Type*} [Field 𝕜] [NontriviallyNormedField 𝕜]
...
variable [Ring B] [NormedRing B] [NormedAlgebra 𝕜 B] [Algebra A B] {f : E → B}
...
theorem AnalyticOnNhd.eval_polynomial {A} [CommRing A] [NormedRing A] [NormedAlgebra 𝕜 A]
    (p : A[X]) :
    AnalyticOnNhd 𝕜 (eval · p) Set.univ := analyticOnNhd_id.aeval_polynomial p

theorem AnalyticOn.eval_polynomial {A} [CommRing A] [NormedRing A] [NormedAlgebra 𝕜 A] (p : A[X]) :
...
variable [CommRing B] [NormedRing B] [NormedAlgebra 𝕜 B] [Algebra A B] {σ : Type*} {f : E → σ → B}
