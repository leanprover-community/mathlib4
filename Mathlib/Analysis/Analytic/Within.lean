...
lemma analyticOn_of_locally_analyticOn {f : E → F} {s : Set E}
  ...
  exact ⟨p, min (.ofReal r) t,
    { r_pos := lt_min (by positivity) fp.r_pos
      ...
      hasSum := by
        intro y ys yr
        simp only [Metric.mem_eball, lt_min_iff, edist_lt_ofReal, dist_zero_right] at yr
        apply fp.hasSum
        · simp only [mem_insert_iff, add_eq_left] at ys
          rcases ys with rfl | ys
          · simp
...
lemma IsOpen.analyticOn_iff_analyticOnNhd {f : E → F} {s : Set E} (hs : IsOpen s) :
  ...
  { r_pos := lt_min (by positivity) fp.r_pos
    ...
    hasSum := by
      intro y ym
      simp only [Metric.mem_eball, lt_min_iff, edist_lt_ofReal, dist_zero_right] at ym
...
lemma hasFPowerSeriesWithinOnBall_iff_exists_hasFPowerSeriesOnBall [CompleteSpace F] {f : E → F}
  ...
  · intro ⟨g, hfg, hg⟩
    ...
    · exact hg.hasSum lt
    · refine ⟨ys, ?_⟩
      simpa only [Metric.mem_eball, edist_eq_enorm_sub, add_sub_cancel_left, sub_zero] using lt
...
lemma hasFPowerSeriesWithinAt_iff_exists_hasFPowerSeriesAt [CompleteSpace F] {f : E → F}
  ...
  · intro ⟨r, h⟩
    ...
    exact inter_mem_nhdsWithin _ (Metric.eball_mem_nhds _ h.r_pos)
  · intro ⟨g, hfg, ⟨r, hg⟩⟩
    ...
    · intro y ⟨ys, xy⟩
      refine hfg ?_ ys
      simp only [Metric.mem_eball, lt_min_iff, edist_lt_ofReal] at xy
      exact xy.2
