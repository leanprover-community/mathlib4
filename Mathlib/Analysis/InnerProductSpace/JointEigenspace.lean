...
theorem orthogonalComplement_iSup_iInf_eigenspaces_eq_bot [Finite n]
  ...
  · have i := Classical.arbitrary m
    ...
    convert H with γ
    have Blah : ∀ j : {j // j ≠ i}, Commute (T j) (T i) := fun j ↦ hC (↑j) i
    have Blah1 := mapsTo_iInf_genEigenspace_of_forall_comm (M := E) Blah γ 1
    rw [@iInf_subtype] at Blah1
    rw [← iSup_eigenspace_restrict ⇑(T i) (hT i) (Blah1)]
    congr! with μ
