...
theorem real_inner_self_abs (x : F) : |⟪x, x⟫_ℝ| = ⟪x, x⟫_ℝ :=
  inner_self_ofReal_norm (𝕜 := ℝ) (E := F) x
...
theorem real_inner_mul_inner_self_le (x y : F) : ⟪x, y⟫_ℝ * ⟪x, y⟫_ℝ ≤ ⟪x, x⟫_ℝ * ⟪y, y⟫_ℝ :=
  calc
    ⟪x, y⟫_ℝ * ⟪x, y⟫_ℝ ≤ ‖⟪x, y⟫_ℝ‖ * ‖⟪y, x⟫_ℝ‖ := by
      ...
      exact le_abs_self _
    _ ≤ ⟪x, x⟫_ℝ * ⟪y, y⟫_ℝ := inner_mul_inner_self_le (𝕜 := ℝ) x y
...
theorem real_inner_self_eq_norm_mul_norm (x : F) : ⟪x, x⟫_ℝ = ‖x‖ * ‖x‖ := by
  have h := inner_self_eq_norm_mul_norm (𝕜 := ℝ) (E := F) x
  simpa using h
...
theorem abs_real_inner_div_norm_mul_norm_eq_one_iff (x y : F) :
  ...
  norm_inner_div_norm_mul_norm_eq_one_iff (𝕜 := ℝ) (E := F) x y
...
variable {G : Type*} [AddCommGroup E] [SeminormedAddCommGroup E] [InnerProductSpace 𝕜 E]
