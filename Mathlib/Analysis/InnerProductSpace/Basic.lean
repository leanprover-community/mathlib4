...
We define both the real and complex cases at the same time using the `ROrCLike` typeclass.
...
- We define the class `InnerProductSpace 𝕜 E` extending `NormedSpace 𝕜 E` with a number of basic
  ...
  or `ℂ`, through the `ROrCLike` typeclass.
...
-/
class InnerProductSpace (𝕜 : Type*) (E : Type*) [ROrCLike 𝕜] [NormedAddCommGroup E] extends
...
structure InnerProductSpace.Core (𝕜 : Type*) (F : Type*) [ROrCLike 𝕜] [AddCommGroup F]
...
local notation "normSqK" => @ROrCLike.normSq 𝕜 _

local notation "reK" => @ROrCLike.re 𝕜 _

local notation "ext_iff" => @ROrCLike.ext_iff 𝕜 _
...
theorem cauchy_schwarz_aux (x y : F) :
  ...
  rw [← mul_assoc, mul_conj, ROrCLike.conj_mul, mul_left_comm, ← inner_conj_symm y, mul_conj]
  push_cast
  ring
...
def toNormedSpace : NormedSpace 𝕜 F where
  norm_smul_le r x := by
    ...
    rw [ROrCLike.conj_mul, ← ofReal_pow, re_ofReal_mul, sqrt_mul, ← ofReal_normSq_eq_inner_self,
      ofReal_re]
    · simp [sqrt_normSq_eq_norm, ROrCLike.sqrt_normSq_eq_norm]
    · positivity
...
local notation "IK" => @ROrCLike.I 𝕜 _
...
theorem inner_self_ofReal_re (x : E) : (re ⟪x, x⟫ : 𝕜) = ⟪x, x⟫ :=
  ((ROrCLike.is_real_TFAE (⟪x, x⟫ : 𝕜)).out 2 3).2 (inner_self_im _)
set_option linter.uppercaseLean3 false in
...
such an instance by restriction-of-scalars, `InnerProductSpace.rorCLikeToReal 𝕜 E`, but this
...
/-- A field `𝕜` satisfying `ROrCLike` is itself a `𝕜`-inner product space. -/
instance ROrCLike.innerProductSpace : InnerProductSpace 𝕜 𝕜 where
  inner x y := conj x * y
...
#align is_R_or_C.inner_product_space ROrCLike.innerProductSpace

@[simp]
theorem ROrCLike.inner_apply (x y : 𝕜) : ⟪x, y⟫ = conj x * y :=
  rfl
#align is_R_or_C.inner_apply ROrCLike.inner_apply
...
def Inner.rorCLikeToReal : Inner ℝ E where inner x y := re ⟪x, y⟫
#align has_inner.is_R_or_C_to_real Inner.rorCLikeToReal

/-- A general inner product space structure implies a real inner product structure. This is not
registered as an instance since it creates problems with the case `𝕜 = ℝ`, but in can be used in a
proof to obtain a real inner product space structure from a given `𝕜`-inner product space
structure. -/
def InnerProductSpace.rorCLikeToReal : InnerProductSpace ℝ E :=
  { Inner.rorCLikeToReal 𝕜 E,
...
#align inner_product_space.is_R_or_C_to_real InnerProductSpace.rorCLikeToReal

variable {E}

theorem real_inner_eq_re_inner (x y : E) :
    @Inner.inner ℝ E (Inner.rorCLikeToReal 𝕜 E) x y = re ⟪x, y⟫ :=
  rfl
#align real_inner_eq_re_inner real_inner_eq_re_inner

theorem real_inner_I_smul_self (x : E) :
    @Inner.inner ℝ E (Inner.rorCLikeToReal 𝕜 E) x ((I : 𝕜) • x) = 0 := by
...
instance InnerProductSpace.complexToReal [NormedAddCommGroup G] [InnerProductSpace ℂ G] :
  ...
  InnerProductSpace.rorCLikeToReal ℂ G
