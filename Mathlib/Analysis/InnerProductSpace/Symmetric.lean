...
variable {𝕜 E E' F G : Type*} [ROrCLike 𝕜]
...
theorem IsSymmetric.restrictScalars {T : E →ₗ[𝕜] E} (hT : T.IsSymmetric) :
    @LinearMap.IsSymmetric ℝ E _ _ (InnerProductSpace.rorCLikeToReal 𝕜 E)
      (@LinearMap.restrictScalars ℝ 𝕜 _ _ _ _ _ _ (InnerProductSpace.rorCLikeToReal 𝕜 E).toModule
        (InnerProductSpace.rorCLikeToReal 𝕜 E).toModule _ _ _ T) :=
...
theorem IsSymmetric.inner_map_polarization {T : E →ₗ[𝕜] E} (hT : T.IsSymmetric) (x y : E) :
  ...
  · simp_rw [map_add, map_sub, inner_add_left, inner_add_right, inner_sub_left, inner_sub_right,
      LinearMap.map_smul, inner_smul_left, inner_smul_right, ROrCLike.conj_I, mul_add, mul_sub,
