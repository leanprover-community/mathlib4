...
* `HilbertBasis.toUnconditionalSchauderBasis`: Every Hilbert basis of `E` is an unconditional
  Schauder basis (`UnconditionalSchauderBasis`), with coordinate functionals `x ↦ ⟪b i, x⟫`.

* `HilbertBasis.toSchauderBasis`: Every Hilbert basis of `E` indexed by `ℕ` is a classical
...
/-- Every Hilbert basis of `E` is an unconditional Schauder basis (`UnconditionalSchauderBasis`),
...
/-- Every Hilbert basis of `E` indexed by `ℕ` is a classical Schauder basis (`SchauderBasis`), with
coordinate functionals `x ↦ ⟪b i, x⟫`. The expansion `x = ∑ i, ⟪b i, x⟫ • b i` converges as the
sequence of partial sums over `Finset.range n`; this restricts the unconditional convergence of
`HilbertBasis.toUnconditionalSchauderBasis` along `Finset.range`. -/
