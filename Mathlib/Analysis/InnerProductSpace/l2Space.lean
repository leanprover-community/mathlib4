...
* `lp.innerProductSpace`: Construction of the inner product space instance on the Hilbert sum
...
instance innerProductSpace : InnerProductSpace 𝕜 (lp G 2) :=
  { lp.normedSpace with
    inner := fun f g => ∑' i, ⟪f i, g i⟫
    norm_sq_eq_inner := fun f => by
      -- Porting note: all but the last step of the `calc` block moved into a `have`
      -- so as to not fail
      have : ‖f‖ ^ 2 = re (∑' i, ⟪f i, f i⟫) := by
        calc
          ‖f‖ ^ 2 = ‖f‖ ^ (2 : ℝ≥0∞).toReal := by norm_cast
          _ = ∑' i, ‖f i‖ ^ (2 : ℝ≥0∞).toReal := (lp.norm_rpow_eq_tsum ?_ f)
          _ = ∑' i, ‖f i‖ ^ 2 := by norm_cast
          _ = ∑' i, re ⟪f i, f i⟫ := by simp only [Real.rpow_two, @norm_sq_eq_inner 𝕜]
...
protected theorem hasSum_repr_symm (b : HilbertBasis ι 𝕜 E) (f : ℓ²(ι, 𝕜)) :
  ...
  letI : NormedSpace 𝕜 (lp (fun _ : ι => 𝕜) 2) := by infer_instance
