...
lemma range_outerProduct (x : E) {y : F} (hy : y ≠ 0) :
    LinearMap.range (outerProduct 𝕜 x y) = 𝕜 ∙ x :=
  range_smulRight_apply (map_eq_zero_iff _
    (InnerProductSpace.toDualMap 𝕜 F).injective |>.not.mpr hy) _

lemma rank_range_outerProduct {x : E} {y : F} (hx : x ≠ 0) (hy : y ≠ 0) :
    Module.rank 𝕜 (LinearMap.range (outerProduct 𝕜 x y)) = 1 := by
  rw [range_outerProduct x hy, Module.rank_eq_one_iff_finrank_eq_one]
