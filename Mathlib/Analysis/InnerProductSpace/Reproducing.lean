...
variable (H) in
/-- Helper function only used in profs. -/
...
theorem RKHSEquivOfKernel_apply_symm_apply (g : OfKernel (kernel H)) :
    RKHSEquivOfKernel H ((RKHSEquivOfKernel H).symm g) = g :=
...
lemma RKHSEquivOfKernel_kerFun_eq_coe'_single (x : X) (v : V) : RKHSEquivOfKernel H (kerFun H x v) =
    UniformSpace.Completion.coe' (α := H₀ (kernel H)) (Finsupp.single (x, v) 1) := by
  apply Eq.symm
  apply ((RKHSEquivOfKernel H).symm_apply_eq).mp
...
lemma RKHSEquivOfKernel_kerFun_eq (x : X) (v : V) : RKHSEquivOfKernel H (kerFun H x v) =
...
lemma RKHSEquivOfKernel_symm_kerFun_eq (x : X) (v : V) :
    (RKHSEquivOfKernel H).symm (kerFun (OfKernel (kernel H)) x v) = (kerFun H x v) := by
