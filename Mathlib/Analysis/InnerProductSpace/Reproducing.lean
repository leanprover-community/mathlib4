...
/--
A reproducing kernel Hilbert space is a Hilbert space with
an injection to functions to another Hilbert space such that point evaluation is continuous.
-/
...
--TODO: make private
variable (H) in
/-- The kernel functions of a reproducing kernel Hilbert space -/
def kerFun (x : X) : V →L[𝕜] H := (.proj x ∘L coeCLM 𝕜).adjoint

variable (H) in
/-- The kernel of a reproducing kernel Hilbert space -/
...
private abbrev kerFunAux' (x : X) : V →ₗ[𝕜] SeparationQuotient (H₀ K) where
  toFun v : SeparationQuotient (H₀ K) := .mk (.single ⟨x, v⟩ 1)
  map_add' _ _ := by
    rw [ext_iff_inner_left 𝕜, SeparationQuotient.surjective_mk.forall]
    simp [← SeparationQuotient.mk_add, inner_H₀_def, ← Finsupp.sum_add, mul_assoc,
    ← mul_add, inner_add_right]
  map_smul' _ _ := by
    rw [ext_iff_inner_left 𝕜, SeparationQuotient.surjective_mk.forall]
    simp [← SeparationQuotient.mk_smul, inner_H₀_def, inner_smul_right, mul_assoc]
...
variable (K) in
/-- Explicit formula for the kernel functions of `OfKernel`, use `RKHS.kerFun` instead. -/
private def kerFun (x : X) : V →L[𝕜] OfKernel K := UniformSpace.Completion.toComplL ∘L kerFunAux x
...
instance instRKHS : RKHS 𝕜 (OfKernel K) X V where
  ...
  coeCLM_injective := by
    ...
    refine UniformSpace.Completion.denseRange_coe.eq_zero_of_inner_right 𝕜 ?_
    refine SeparationQuotient.surjective_mk.forall.mpr fun ff ↦ ?_
    induction ff using Finsupp.induction with
    | zero =>
      have : @UniformSpace.Completion.coe' (SeparationQuotient (H₀ K))
        SeparationQuotient.instUniformSpace 0 = 0 := rfl
      simp [this]
    | single_add i a =>
    simp only [SeparationQuotient.mk_add, UniformSpace.Completion.coe_add, inner_add_left, *,
      add_zero]
    rw [← SeparationQuotient.mkCLM_apply 𝕜, ← UniformSpace.Completion.coe_toComplL (𝕜 := 𝕜)]
