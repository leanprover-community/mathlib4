...
variable (H) in
/-- Helper function that maps the kernel function of `H` into the RKHS `H` isometrically. -/
...
variable (H) in
/-- A reproducing kernel Hilbert space is isometric to the Hilbert space generated by its kernel. -/
def EquivOfKernel : H ≃ₗᵢ[𝕜] OfKernel (kernel H) := by
  have h_iso := Isometry.completion_extension (ofH₀ H).isometry
  have h_lin := UniformSpace.Completion.isLinearMap_extension (ofH₀ H).isometry.uniformContinuous
  ...
  have h_surj : Function.Surjective ofOfKernel := by
    ...
    have h_sub : Set.range (ofH₀ H) ⊆ Set.range ⇑ofOfKernel := by
      ...
      exact ⟨(f : OfKernel (kernel H)),
        UniformSpace.Completion.extension_coe (ofH₀ H).isometry.uniformContinuous f⟩
