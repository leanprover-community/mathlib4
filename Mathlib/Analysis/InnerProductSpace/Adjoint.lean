...
lemma _root_.InnerProductSpace.adjoint_outerProduct (x : E) (y : F) :
    (outerProduct 𝕜 x y).adjoint = outerProduct 𝕜 y x := by
  simp [outerProduct_def, adjoint_comp, ← adjoint_innerSL_apply]

lemma _root_.InnerProductSpace.isSelfAdjoint_outerProduct_self (x : E) :
    IsSelfAdjoint (outerProduct 𝕜 x x) :=
  adjoint_outerProduct x x

omit [CompleteSpace E] in
lemma _root_.InnerProductSpace.outerProduct_comp {G : Type*}
    [NormedAddCommGroup G] [InnerProductSpace 𝕜 G] [CompleteSpace G]
    (x : E) (y : F) (f : G →L[𝕜] F) :
    outerProduct 𝕜 x y ∘L f = outerProduct 𝕜 x (adjoint f y) := by
  simp_rw [outerProduct_def, comp_assoc, innerSL_apply_comp]

lemma _root_.InnerProductSpace.isSelfAdjoint_outerProduct_add (x y : E) :
    IsSelfAdjoint (outerProduct 𝕜 x y + outerProduct 𝕜 y x) :=
  (adjoint_outerProduct (𝕜 := 𝕜) y x) ▸ IsSelfAdjoint.star_add_self _
