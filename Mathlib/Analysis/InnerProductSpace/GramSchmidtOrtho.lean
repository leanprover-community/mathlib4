...
theorem gramSchmidt_def'' (f : ι → E) (n : ι) :
  ...
  rw [orthogonalProjection_singleton, ROrCLike.ofReal_pow]
#align gram_schmidt_def'' gramSchmidt_def''
...
theorem gramSchmidt_orthogonal (f : ι → E) {a b : ι} (h₀ : a ≠ b) :
  ...
  · by_cases h : gramSchmidt 𝕜 f a = 0
    ...
    · rw [ROrCLike.ofReal_pow, ← inner_self_eq_norm_sq_to_K, div_mul_cancel, sub_self]
...
theorem gramSchmidt_orthonormal {f : ι → E} (h₀ : LinearIndependent 𝕜 f) :
  ...
  · intro i j hij
    simp only [gramSchmidtNormed, inner_smul_left, inner_smul_right, ROrCLike.conj_inv,
      ROrCLike.conj_ofReal, mul_eq_zero, inv_eq_zero, ROrCLike.ofReal_eq_zero, norm_eq_zero]
    repeat' right
