...
variable {𝕜 : Type*} [ROrCLike 𝕜] (E : Type*) [NormedAddCommGroup E]
...
theorem inner_.norm_sq (x : E) : ‖x‖ ^ 2 = re (inner_ 𝕜 x x) := by
  simp only [inner_]
  have h₁ : ROrCLike.normSq (4 : 𝕜) = 16 := by
    have : ((4 : ℝ) : 𝕜) = (4 : 𝕜) := by norm_cast
    rw [← this, normSq_eq_def', ROrCLike.norm_of_nonneg (by norm_num : (0 : ℝ) ≤ 4)]
    norm_num
  have h₂ : ‖x + x‖ = 2 * ‖x‖ := by rw [← two_smul 𝕜, norm_smul, ROrCLike.norm_two]
...
private theorem rat_prop (r : ℚ) : innerProp' E (r : 𝕜) := by
  intro x y
  have : (r.den : 𝕜) ≠ 0 := by
    haveI : CharZero 𝕜 := ROrCLike.charZero_rorCLike
...
private theorem real_prop (r : ℝ) : innerProp' E (r : 𝕜) := by
  ...
  · simp only [Function.comp_apply, ROrCLike.ofReal_ratCast, rat_prop _ _]
