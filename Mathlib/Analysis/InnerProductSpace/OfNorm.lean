...
noncomputable def InnerProductSpaceable.inner_ (x y : E) : 𝕜 :=
...
/-- The property that `inner_` is compatible with scalar multiplication by `r` on the left, that is
`inner_ 𝕜 (r • x) y = conj r * inner_ 𝕜 x y` for all `x y`. -/
def innerProp' (r : 𝕜) : Prop :=
...
theorem _root_.Continuous.inner_ {f g : ℝ → E} (hf : Continuous f) (hg : Continuous g) :
  ...
  unfold InnerProductSpaceable.inner_
  fun_prop
