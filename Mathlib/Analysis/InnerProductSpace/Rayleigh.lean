...
theorem rayleigh_smul (x : E) {c : 𝕜} (hc : c ≠ 0) :
  ...
  -- ⊢ ‖c‖ ^ 2 * reApplyInnerSelf T x * ‖x‖ ^ 2 = reApplyInnerSelf T x * (‖c‖ * ‖x‖ …
  ring
  -- 🎉 no goals
...
theorem _root_.LinearMap.IsSymmetric.hasStrictFDerivAt_reApplyInnerSelf {T : F →L[ℝ] F}
  ...
  -- ⊢ 2 • ↑(innerSL ℝ) (↑T x₀) = ContinuousLinearMap.comp (fderivInnerClm ℝ (↑T x₀ …
  ext y
  -- ⊢ ↑(2 • ↑(innerSL ℝ) (↑T x₀)) y = ↑(ContinuousLinearMap.comp (fderivInnerClm ℝ …
...
theorem linearly_dependent_of_isLocalExtrOn (hT : IsSelfAdjoint T) {x₀ : F}
  ...
  -- ⊢ ↑(InnerProductSpace.toDualMap ℝ F) (a • x₀ + b • ↑T x₀) = ↑(InnerProductSpac …
  simp only [LinearIsometry.map_add, LinearIsometry.map_smul, LinearIsometry.map_zero]
  -- ⊢ ↑(InnerProductSpace.toDualMap ℝ F) (a • x₀) + ↑(InnerProductSpace.toDualMap  …
  simp only [map_smulₛₗ, IsROrC.conj_to_real]
  -- ⊢ a • ↑(InnerProductSpace.toDualMap ℝ F) x₀ + b • ↑(InnerProductSpace.toDualMa …
  ...
  -- ⊢ (fun x x_1 => x • x_1) 2 (a • ↑(innerSL ℝ) x₀ + b • ↑(innerSL ℝ) (↑T x₀)) =  …
...
theorem hasEigenvector_of_isMaxOn (hT : IsSelfAdjoint T) {x₀ : E} (hx₀ : x₀ ≠ 0)
  ...
  -- ⊢ ⨆ (x : { x // x ≠ 0 }), ContinuousLinearMap.rayleighQuotient T ↑x = Continuo …
  have hx₀' : 0 < ‖x₀‖ := by simp [hx₀]
  -- ⊢ ⨆ (x : { x // x ≠ 0 }), ContinuousLinearMap.rayleighQuotient T ↑x = Continuo …
  have hx₀'' : x₀ ∈ sphere (0 : E) ‖x₀‖ := by simp
  -- ⊢ ⨆ (x : { x // x ≠ 0 }), ContinuousLinearMap.rayleighQuotient T ↑x = Continuo …
  rw [T.iSup_rayleigh_eq_iSup_rayleigh_sphere hx₀']
  -- ⊢ ⨆ (x : ↑(sphere 0 ‖x₀‖)), ContinuousLinearMap.rayleighQuotient T ↑x = Contin …
  refine' IsMaxOn.iSup_eq hx₀'' _
  -- ⊢ IsMaxOn (ContinuousLinearMap.rayleighQuotient T) (sphere 0 ‖x₀‖) x₀
  intro x hx
  -- ⊢ x ∈ {x | (fun x => ContinuousLinearMap.rayleighQuotient T x ≤ ContinuousLine …
  dsimp
  -- ⊢ ContinuousLinearMap.rayleighQuotient T x ≤ ContinuousLinearMap.rayleighQuoti …
  have : ‖x‖ = ‖x₀‖ := by simpa using hx
  -- ⊢ ContinuousLinearMap.rayleighQuotient T x ≤ ContinuousLinearMap.rayleighQuoti …
  simp only [ContinuousLinearMap.rayleighQuotient]
  -- ⊢ ContinuousLinearMap.reApplyInnerSelf T x / ‖x‖ ^ 2 ≤ ContinuousLinearMap.reA …
  rw [this]
  -- ⊢ ContinuousLinearMap.reApplyInnerSelf T x / ‖x₀‖ ^ 2 ≤ ContinuousLinearMap.re …
  gcongr
  -- ⊢ ContinuousLinearMap.reApplyInnerSelf T x ≤ ContinuousLinearMap.reApplyInnerS …
  exact hextr hx
  -- 🎉 no goals
...
theorem hasEigenvector_of_isMinOn (hT : IsSelfAdjoint T) {x₀ : E} (hx₀ : x₀ ≠ 0)
  ...
  intro x hx
  -- ⊢ x ∈ {x | (fun x => ContinuousLinearMap.rayleighQuotient T x₀ ≤ ContinuousLin …
  dsimp
  -- ⊢ ContinuousLinearMap.rayleighQuotient T x₀ ≤ ContinuousLinearMap.rayleighQuot …
  have : ‖x‖ = ‖x₀‖ := by simpa using hx
  -- ⊢ ContinuousLinearMap.rayleighQuotient T x₀ ≤ ContinuousLinearMap.rayleighQuot …
  simp only [ContinuousLinearMap.rayleighQuotient]
  -- ⊢ ContinuousLinearMap.reApplyInnerSelf T x₀ / ‖x₀‖ ^ 2 ≤ ContinuousLinearMap.r …
  rw [this]
  -- ⊢ ContinuousLinearMap.reApplyInnerSelf T x₀ / ‖x₀‖ ^ 2 ≤ ContinuousLinearMap.r …
  gcongr
  -- ⊢ ContinuousLinearMap.reApplyInnerSelf T x₀ ≤ ContinuousLinearMap.reApplyInner …
  exact hextr hx
  -- 🎉 no goals
...
theorem hasEigenvalue_iSup_of_finiteDimensional (hT : T.IsSymmetric) :
  ...
  -- ⊢ HasEigenvalue T ↑(⨆ (x : { x // x ≠ 0 }), ↑IsROrC.re (inner (↑T ↑x) ↑x) / ‖↑ …
