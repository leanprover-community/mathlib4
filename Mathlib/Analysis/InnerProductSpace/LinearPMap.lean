...
lemma IsFormalAdjoint.le_adjoint (h : T.IsFormalAdjoint S) : S ≤ T† :=
...
lemma toPMap_adjoint_eq_adjoint_toPMap_of_dense (hp : Dense (p : Set E)) :
...
lemma _root_.IsSelfAdjoint.dense_domain (hA : IsSelfAdjoint A) : Dense (A.domain : Set E) := by
  by_contra h
