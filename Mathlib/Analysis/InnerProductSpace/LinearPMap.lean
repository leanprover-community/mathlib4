...
def adjointDomain : Submodule 𝕜 F where
  ...
  zero_mem' := by
    rw [Set.mem_ofPred, LinearMap.map_zero, LinearMap.zero_comp]
    exact continuous_zero
  add_mem' hx hy := by rw [Set.mem_ofPred, LinearMap.map_add] at *; exact hx.add hy
  smul_mem' a x hx := by
    rw [Set.mem_ofPred, LinearMap.map_smulₛₗ] at *
