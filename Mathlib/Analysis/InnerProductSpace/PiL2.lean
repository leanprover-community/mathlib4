...
noncomputable def LinearIsometry.extend (L : S →ₗᵢ[𝕜] V) : V →ₗᵢ[𝕜] V := by
  ...
  have M_norm_map : ∀ x : V, ‖M x‖ = ‖x‖ := by
    ...
    simp only [p1, p2, LinearIsometry.norm_map, add_left_inj, mul_eq_mul_left_iff, norm_eq_zero,
      true_or_iff, eq_self_iff_true, ContinuousLinearMap.coe_coe, Submodule.coe_norm,
