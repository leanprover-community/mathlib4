...
instance PiLp.innerProductSpace {ι : Type*} [Fintype ι] (f : ι → Type*)
    [∀ i, NormMetric (f i)] [∀ i, AddCommGroup (f i)] [∀ i, IsNormedAddGroup (f i)] [∀ i, InnerProductSpace 𝕜 (f i)] :
...
theorem PiLp.inner_apply {ι : Type*} [Fintype ι] {f : ι → Type*} [∀ i, NormMetric (f i)] [∀ i, AddCommGroup (f i)] [∀ i, IsNormedAddGroup (f i)]
...
theorem sum_sq_inner_right {E : Type*} [NormMetric E] [AddCommGroup E] [IsNormedAddGroup E]
...
open scoped RealInnerProductSpace in
theorem sum_sq_inner_left {ι E : Type*} [NormMetric E] [AddCommGroup E] [IsNormedAddGroup E]
...
protected def map {G : Type*} [NormMetric G] [AddCommGroup G] [IsNormedAddGroup G] [InnerProductSpace 𝕜 G]
    (b : OrthonormalBasis ι 𝕜 E) (L : E ≃ₗᵢ[𝕜] G) : OrthonormalBasis ι 𝕜 G where
  repr := L.symm.trans b.repr

@[simp]
protected theorem map_apply {G : Type*} [NormMetric G] [AddCommGroup G] [IsNormedAddGroup G] [InnerProductSpace 𝕜 G]
    (b : OrthonormalBasis ι 𝕜 E) (L : E ≃ₗᵢ[𝕜] G) (i : ι) : b.map L i = L (b i) :=
  rfl

@[simp]
protected theorem toBasis_map {G : Type*} [NormMetric G] [AddCommGroup G] [IsNormedAddGroup G] [InnerProductSpace 𝕜 G]
...
protected def _root_.Pi.orthonormalBasis {η : Type*} [Fintype η] {ι : η → Type*}
    [∀ i, Fintype (ι i)] {𝕜 : Type*} [RCLike 𝕜] {E : η → Type*} [∀ i, NormMetric (E i)] [∀ i, AddCommGroup (E i)] [∀ i, IsNormedAddGroup (E i)]
...
variable {E' : Type*} [Fintype ι'] [NormMetric E'] [AddCommGroup E'] [IsNormedAddGroup E'] [InnerProductSpace 𝕜 E']
...
variable {V : Type*} [NormMetric V] [AddCommGroup V] [IsNormedAddGroup V] [InnerProductSpace 𝕜 V] [FiniteDimensional 𝕜 V]
...
theorem InnerProductSpace.toMatrix_rankOne {𝕜 E F ι ι' : Type*} [RCLike 𝕜]
    [NormPseudoMetric E] [AddCommGroup E] [IsNormedAddGroup E] [NormedSpace 𝕜 E] [NormMetric F] [AddCommGroup F] [IsNormedAddGroup F] [InnerProductSpace 𝕜 F]
