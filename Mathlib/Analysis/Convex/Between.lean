...
theorem Wbtw.trans_expand_left {w x y z : P} (h₁ : Wbtw R w x y) (h₂ : Wbtw R x y z)
  ...
  by_cases h_denom : 1 - t₁ + t₁ * t₂ = 0
  · have h1: t₁ = 1 := by
      have h_eq : t₁ = 1 + t₁ * t₂ := by
        rw [sub_add_eq_add_sub, sub_eq_zero] at h_denom
        exact h_denom.symm
      have h_ge_one : 1 ≤ t₁ := by
        rw [h_eq]
        apply le_add_of_nonneg_right
        exact mul_nonneg ht₁.1 ht₂.1
      exact le_antisymm ht₁.2 h_ge_one
    rw [← hx]
    rw [← hx, h1] at h_ne
    simp only [lineMap_apply_one, ne_eq, not_true_eq_false] at h_ne
  · refine ⟨t₁ * t₂ / (1 - t₁ + t₁ * t₂), ?_, ?_⟩
    · constructor
      · apply div_nonneg (mul_nonneg ht₁.1 ht₂.1)
        nlinarith [ht₁.1, ht₁.2, ht₂.1, ht₂.2]
      · rw [div_le_one₀]
        · grind => instantiate only [= Set.mem_Icc]
        · have h_nonneg : 0 ≤ 1 - t₁ + t₁ * t₂ := by
            nlinarith [ht₁.1, ht₁.2, ht₂.1, ht₂.2]
          apply lt_of_le_of_ne h_nonneg
          exact Ne.symm h_denom
    · rw [← hy, lineMap_apply, lineMap_apply, eq_comm, eq_vadd_iff_vsub_eq] at hx
      ...
      apply (eq_inv_smul_iff₀ h_denom).mpr
      rw [add_smul, sub_smul, one_smul]
      nth_rw 1 [hx]
