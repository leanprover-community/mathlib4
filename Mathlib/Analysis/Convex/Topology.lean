...
lemma Real.convex_iff_isPreconnected {s : Set ℝ} : Convex ℝ s ↔ IsPreconnected s :=
...
lemma stdSimplex_subset_closedBall : stdSimplex ℝ ι ⊆ Metric.closedBall 0 1 := by
  intro f hf
...
lemma bounded_stdSimplex : Metric.Bounded (stdSimplex ℝ ι) :=
...
lemma isClosed_stdSimplex : IsClosed (stdSimplex ℝ ι) :=
...
lemma isCompact_stdSimplex : IsCompact (stdSimplex ℝ ι) :=
...
lemma segment_subset_closure_openSegment : [x -[𝕜] y] ⊆ closure (openSegment 𝕜 x y) := by
...
lemma Convex.combo_self_interior_mem_interior {s : Set E} (hs : Convex 𝕜 s) {x y : E} (hx : x ∈ s)
...
lemma Convex.openSegment_interior_closure_subset_interior {s : Set E} (hs : Convex 𝕜 s) {x y : E}
...
lemma Convex.openSegment_interior_self_subset_interior {s : Set E} (hs : Convex 𝕜 s) {x y : E}
...
lemma Convex.openSegment_closure_interior_subset_interior {s : Set E} (hs : Convex 𝕜 s) {x y : E}
...
lemma Convex.openSegment_self_interior_subset_interior {s : Set E} (hs : Convex 𝕜 s) {x y : E}
...
-/
lemma Convex.add_smul_sub_mem_interior' {s : Set E} (hs : Convex 𝕜 s) {x y : E}
...
lemma Convex.add_smul_sub_mem_interior {s : Set E} (hs : Convex 𝕜 s) {x y : E} (hx : x ∈ s)
...
lemma Convex.add_smul_mem_interior' {s : Set E} (hs : Convex 𝕜 s) {x y : E} (hx : x ∈ closure s)
...
lemma Convex.add_smul_mem_interior {s : Set E} (hs : Convex 𝕜 s) {x y : E} (hx : x ∈ s)
...
`interior s`. -/
protected lemma Convex.strictConvex' {s : Set E} (hs : Convex 𝕜 s)
...
lemma Set.Finite.isCompact_convexHull {s : Set E} (hs : s.Finite) :
...
lemma Set.Finite.isClosed_convexHull [T2Space E] {s : Set E} (hs : s.Finite) :
...
TODO Generalise this from convex sets to sets that are balanced / star-shaped about `x`. -/
lemma Convex.subset_interior_image_homothety_of_one_lt {s : Set E} (hs : Convex ℝ s) {x : E}
...
lemma JoinedIn_of_segment_subset {E : Type*} [AddCommGroup E] [Module ℝ E]
...
protected lemma Convex.isPathConnected {s : Set E} (hconv : Convex ℝ s) (hne : s.Nonempty) :
...
protected lemma Convex.isConnected {s : Set E} (h : Convex ℝ s) (hne : s.Nonempty) :
    IsConnected s :=
  (h.isPathConnected hne).isConnected
#align convex.is_connected Convex.isConnected

/-- A convex set is preconnected. -/
protected lemma Convex.isPreconnected {s : Set E} (h : Convex ℝ s) : IsPreconnected s :=
...
-/
protected lemma TopologicalAddGroup.pathConnectedSpace : PathConnectedSpace E :=
