...
omit [Field 𝕜] [IsStrictOrderedRing 𝕜] in
private lemma diff_singleton_eventually_mem_nhds_right {s : Set 𝕜} {a : 𝕜}
  ...
  · exact mem_nhds_iff.2 ⟨Ioo a b, subset_diff_singleton this (left_notMem_Ioo a b), isOpen_Ioo, hx⟩
...
theorem Convex.diff_singleton_eventually_mem_nhds {s : Set 𝕜} (hs : Convex 𝕜 s) (a : 𝕜) :
  ...
  exact ⟨diff_singleton_eventually_mem_nhds_left fun x hx ↦ hs.Ioo_subset_of_mem_closure hx has,
    diff_singleton_eventually_mem_nhds_right fun x hx ↦ hs.Ioo_subset_of_mem_closure has hx⟩
