...
noncomputable def incSumSet (k : ℕ) (x : n → R) :=
    if hk : k ≤ card n then
      Classical.choose (exists_min'_image (powersetCard k (univ : Finset n))
        (fun s => ∑ i ∈ s, x i) (by grind [= powersetCard_nonempty]))
    else ∅
...
lemma le_of_mem_incSumSet [DecidableEq n] [IsStrictOrderedRing R] (k : ℕ) (x : n → R) (hk : k ≤ card n) (i : n)
    (hi : i ∈ incSumSet k x) (j : n) (hj : j ∈ (incSumSet k x)ᶜ) : x i ≤ x j := by
  by_contra hcontra
  let s := incSumSet k x
...
lemma ge_of_mem_decSumSet [DecidableEq n] [IsStrictOrderedRing R] (k : ℕ) (x : n → R) (hk : k ≤ card n) (i : n)
    (hi : i ∈ decSumSet k x) (j : n) (hj : j ∈ (decSumSet k x)ᶜ) : x j ≤ x i := by
  by_contra hcontra
  let s := decSumSet k x
  let t : Finset n := insert j (s.erase i)
  have hs : s = insert i (s.erase i) := by grind
...
lemma exists_eq_incSum (k : ℕ) (x : n → R) (hk : k ≤ card n) :
  ...
  obtain ⟨s, hs₁, hs₂⟩ := exists_min'_image (powersetCard k (univ : Finset n)) (fun s => ∑ i ∈ s, x i) (by grind)
  refine ⟨s, ?_, hs₂, ?_⟩
...
lemma exists_eq_decSum (k : ℕ) (x : n → R) (hk : k ≤ card n) :
  ...
  obtain ⟨s, hs₁, hs₂⟩ := exists_max'_image (powersetCard k (univ : Finset n)) (fun s => ∑ i ∈ s, x i) (by grind)
  refine ⟨s, ?_, hs₂, ?_⟩
  · exact mem_powersetCard_univ.mp hs₁
  · intro t ht
    rw [← hs₂]
...
lemma decSum_of_antitone [IsOrderedRing R] {n : ℕ} (k : ℕ) (x : Fin n → R) (hx : Antitone x)
  ...
  · intro b hb
    ...
    have hcard : (a \ s).card = (s \ a).card := by rw [card_sdiff_eq_card_sdiff_iff]; grind
    rw [ha, sum_union (by grind)]
    conv_rhs => rw [hs, sum_union (by grind)]
    gcongr ?_ + ?_
    let K := ((a \ s).image x).max' <| by grind
