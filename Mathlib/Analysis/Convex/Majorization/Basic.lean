...
@[grind =]
lemma decSumSet_nonempty_iff {k : ℕ} (x : n → R) (hk : k ≤ card n) :
    (decSumSet k x).Nonempty ↔ 0 < k := by grind [← Finset.card_ne_zero]
...
lemma exists_eq_incSum (k : ℕ) (x : n → R) (hk : k ≤ card n) :
  ...
  exact ⟨incSumSet k x, card_incSumSet hk, incSum_eq_sum_incSumSet hk,
...
lemma exists_eq_incSum_one (x : n → R) (hcard : 1 ≤ card n) :
  ...
  exact ⟨i, by simp [hs₂, hi], fun j => by simpa [hi] using hs₃ {j} (by simp)⟩
...
lemma mulVec_isSubmajorizedBy_of_mem_doublyStochastic [DecidableEq n] [IsStrictOrderedRing R]
  ...
  · let s := decSumSet k x
    ...
    let M := (s.image x).min' <| by grind [Finset.image_nonempty]
    have hM : ∀ i ∈ s, M ≤ x i := by grind only [Finset.min'_le, = Finset.mem_image]
    have hM' : ∀ i ∈ sᶜ, x i ≤ M := by
      have h₁ := Finset.min'_mem (s.image x) (by grind [Finset.image_nonempty])
    ...
    grind => linarith
  · rw [decSum_of_ge_card (k := k) (x := x) (by grind),
        decSum_of_ge_card (k := k) (x := (A *ᵥ x)) (by grind)]
    exact le_of_eq <| sum_mulVec_of_mem_doublyStochastic hA
