...
theorem extremePoints_singleton : ({x} : Set E).extremePoints 𝕜 = {x} :=
  extremePoints_subset.antisymm <| singleton_subset_iff.2
    ⟨mem_singleton_self x, fun _ hx₁ _ _ _ ↦ hx₁⟩
...
theorem Convex.mem_extremePoints_iff_convex_sdiff (hA : Convex 𝕜 A) :
  ...
  by_contra! h
  exact (hAx ⟨hx₁, fun hx₁ ↦ h.1 (mem_singleton.2 hx₁)⟩
      ⟨hx₂, fun hx₂ ↦ h.2 (mem_singleton.2 hx₂)⟩ hx).2 rfl
