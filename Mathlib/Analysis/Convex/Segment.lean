...
variable {𝕜 E F G ι : Type*} {X : ι → Type*}
...
variable [OrderedSemiring 𝕜] [∀ i, AddCommMonoid (X i)] [∀ i, Module 𝕜 (X i)] {s : Set ι}

theorem segment_subset (x y : ∀ i, X i) : segment 𝕜 x y ⊆ s.pi fun i => segment 𝕜 (x i) (y i) := by
  rintro z ⟨a, b, ha, hb, hab, hz⟩ i -
  exact ⟨a, b, ha, hb, hab, congr_fun hz i⟩

theorem openSegment_subset (x y : ∀ i, X i) :
