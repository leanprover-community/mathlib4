...
-- @[simp]
-- theorem coe_succ_nsmul (x : S) (n : ℕ) : (n + 1) • x = (n + 1 : 𝕜) • (x : E) := rfl

-- -- @[simp]
-- -- theorem coe_nsmul' (x : S) (n : ℕ) : ((n : 𝕜) • (x)) = n • (x : E) := by
-- --   sorry

@[simp]
theorem coe_nsmul (x : S) (n : ℕ) : (n • x : E) = n • (x : E) := sorry
--   match n with
--   | 0 => by rw [zero_smul] ; rfl
--   | n + 1 => by
--       rw [coe_succ_nsmul]
--       norm_cast
--       sorry

instance : AddCommMonoid S := by
