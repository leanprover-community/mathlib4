...
theorem ConvexCone.pointed_of_nonempty_of_isClosed (K : ConvexCone ℝ E) (ne : (K : Set E).Nonempty)
    (hc : IsClosed (K : Set E)) : K.Pointed := by
  obtain ⟨x, hx⟩ := ne
  let f : ℝ → E := (· • x)
  -- f (0, ∞) is a subset of K
  have fI : f '' Set.Ioi 0 ⊆ (K : Set E) := by
    rintro _ ⟨_, h, rfl⟩
    exact K.smul_mem (Set.mem_Ioi.1 h) hx
  -- closure of f (0, ∞) is a subset of K
  have clf : closure (f '' Set.Ioi 0) ⊆ (K : Set E) := hc.closure_subset_iff.2 fI
