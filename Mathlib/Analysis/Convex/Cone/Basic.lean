...
@[ext]
lemma ext {S T : ConvexCone 𝕜 E} (h : ∀ x, x ∈ S ↔ x ∈ T) : S = T :=
  SetLike.ext h
...
lemma Flat.pointed {S : ConvexCone 𝕜 E} (hS : S.Flat) : S.Pointed := by
...
lemma Blunt.salient {S : ConvexCone 𝕜 E} : S.Blunt → S.Salient := by
...
lemma salient_positive : Salient (positive 𝕜 E) := fun x xs hx hx' =>
  lt_irrefl (0 : E)
    (calc
...
lemma pointed_positive : Pointed (positive 𝕜 E) :=
  le_refl 0
...
lemma salient_strictlyPositive : Salient (strictlyPositive 𝕜 E) :=
...
lemma blunt_strictlyPositive : Blunt (strictlyPositive 𝕜 E) :=
  lt_irrefl 0
...
lemma toCone_isLeast : IsLeast { t : ConvexCone 𝕜 E | s ⊆ t } (hs.toCone s) := by
