...
instance (priority := 10000) : Membership (Finset E) (SimplicialComplex 𝕜 E) :=
  ⟨fun s K => s ∈ K.faces⟩
...
instance (priority := 10000) : Inf (SimplicialComplex 𝕜 E) :=
  ⟨fun K L =>
...
instance (priority := 10000) : SemilatticeInf (SimplicialComplex 𝕜 E) :=
...
instance (priority := 10000) hasBot : Bot (SimplicialComplex 𝕜 E) :=
  ⟨{  faces := ∅
...
instance (priority := 10000) : OrderBot (SimplicialComplex 𝕜 E) :=
  { SimplicialComplex.hasBot 𝕜 E with bot_le := fun _ => Set.empty_subset _ }

instance (priority := 10000) : Inhabited (SimplicialComplex 𝕜 E) :=
  ⟨⊥⟩
