...
theorem lineDerivCLM_ofFun (f : E → F) {μ : Measure E} [μ.IsAddHaarMeasure]
    (hf : ContDiff ℝ 1 f) (v : E) (hkn : k + 1 ≤ n) :
    Distribution.lineDerivCLM v (ofFun Ω f μ k)
      = ofFun Ω (fun x ↦ lineDeriv ℝ f x v) μ n := by
  ext φ
  ...
  rw [lineDerivCLM_apply, ofFun_apply hf_loc, ofFun_apply hf'_loc]
