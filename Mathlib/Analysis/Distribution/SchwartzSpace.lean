  ...
  ⟨fun c f =>
    { toFun := c • (f : E → F)
      ...
      decay' := fun k n => by
        ...
        grw [← f.le_seminormAux _ _ x, decay_smul_aux]
        rw [← mul_comm _ ‖c‖, mul_assoc, mul_assoc, mul_comm ‖c‖]
        gcongr
        linarith }⟩
...
protected def evalCLM (m : E) : 𝓢(E, E →L[ℝ] F) →L[𝕜] 𝓢(E, F) :=
  mkCLM (fun f x => f x m) (fun _ _ _ => rfl) (fun _ _ _ => rfl)
    ...
    (by
      ...
      intro f x
      grw [norm_iteratedFDeriv_clm_apply_const (f.smooth _) le_top]
...
def bilinLeftCLM (B : E →L[ℝ] F →L[ℝ] G) {g : D → F} (hg : g.HasTemperateGrowth) :
  ...
  -- Todo (after port): generalize to `B : E →L[𝕜] F →L[𝕜] G` and `𝕜`-linear
    ...
    (by
      -- TODO: write as a calc block or find some other way to automate the algebraic steps
      rintro ⟨k, n⟩
      ...
      use
        ...
        by positivity
      intro f x
      grw [ContinuousLinearMap.norm_iteratedFDeriv_le_of_bilinear B (f.smooth _) hg.1 x le_top]
    ...
    (by
      ...
      have hgrowth' : ∀ N : ℕ, 1 ≤ N → N ≤ n →
        ...
        · refine' le_self_pow (one_le_pow_of_one_le _ l) hN₁'
          simp only [le_add_iff_nonneg_right, norm_nonneg]
      have hxk : ‖x‖ ≤ 1 + ‖x‖ := by simp only [zero_le_one, le_add_iff_nonneg_left]
      dsimp only
      grw [←Function.comp_def f g, hxk,
        norm_iteratedFDeriv_comp_le (f.smooth _) hg.1 le_top x hbound hgrowth']
      have rearrange :
        (1 + ‖x‖) ^ k *
