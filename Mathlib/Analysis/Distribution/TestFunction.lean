...
theorem differentiable_withOrder (f : 𝓓^{n}(Ω, F)) (hn : n ≠ 0) : Differentiable ℝ f :=
...
noncomputable def fderivLM [SMulCommClass ℝ 𝕜 F] :
  ...
  map_add' f g := by
    ext
    simp [fderiv_add (f.contDiff.differentiable (by simp)).differentiableAt
                     (g.contDiff.differentiable (by simp)).differentiableAt]
  map_smul' c f := by
    ext
    simp [fderiv_const_smul (f.contDiff.differentiable (by simp)).differentiableAt]
