...
`‖x y‖ = ‖x‖ ‖y‖`. -/
class IsNormedField (α : Type*) [NormMetric α] [DivisionRing α] extends
    IsNormedAddGroup α, NormMulClass α where

/-- A normed division ring is a division ring endowed with a seminorm which satisfies the equality
`‖x y‖ = ‖x‖ ‖y‖`. -/
@[deprecated IsNormedField (since := "2026-05-25")]
structure NormedDivisionRing (α : Type*) extends NormMetric α, DivisionRing α where
...
instance (priority := 100) IsNormedField.toIsNormedRing [NormMetric α] [DivisionRing α]
    [IsNormedField α] :
    IsNormedRing α where
  norm_mul_le a b := (norm_mul a b).le

attribute [instance 100] IsNormedField.toNormMulClass

section NormedDivisionRing

variable [NormMetric α] [DivisionRing α] [IsNormedField α] {a b : α}
...
variable {𝕜 : Type*} [NormMetric 𝕜] [DivisionRing 𝕜] [IsNormedField 𝕜] [DiscreteTopology 𝕜]
...
@[deprecated IsNormedField (since := "2026-05-25")]
structure NormedField (α : Type*) extends NormMetric α, Field α where
  /-- The distance is induced by the norm. -/
  dist_eq : ∀ x y, dist x y = norm (-x + y)
  /-- The norm is multiplicative. -/
  protected norm_mul : ∀ a b, norm (a * b) = norm a * norm b
...
class IsNontriviallyNormedField (α : Type*) [NormMetric α] [Field α] extends
    IsNormedField α where
...
class IsDenselyNormedField (α : Type*) [NormMetric α] [Field α] extends
    IsNormedField α where
...
See note [lower instance priority]. -/
instance (priority := 100) IsDenselyNormedField.toIsNontriviallyNormedField
    [NormMetric α] [Field α] [IsDenselyNormedField α] :
    IsNontriviallyNormedField α where
  non_trivial :=
    let ⟨a, h, _⟩ := IsDenselyNormedField.lt_norm_lt 1 2 zero_le_one one_lt_two
    ⟨a, h⟩
...
variable (α) [NormMetric α] [Field α] [IsNontriviallyNormedField α]

theorem exists_one_lt_norm : ∃ x : α, 1 < ‖x‖ :=
  ‹IsNontriviallyNormedField α›.non_trivial
...
theorem exists_lt_norm_lt {r₁ r₂ : ℝ} (h₀ : 0 ≤ r₁) (h : r₁ < r₂) : ∃ x : α, r₁ < ‖x‖ ∧ ‖x‖ < r₂ :=
  IsDenselyNormedField.lt_norm_lt r₁ r₂ h₀ h
...
instance Real.isDenselyNormedField : IsDenselyNormedField ℝ where
  lt_norm_lt _ _ h₀ hr :=
...
`IsNormedField` structure on the domain.

See note [reducible non-instances] -/
lemma IsNormedField.induced [DivisionRing R] [NormMetric S] [DivisionRing S] [IsNormedField S]
    [NonUnitalRingHomClass F R S] (f : F) (hf : Function.Injective f) :
    letI := NormMetric.induced R S f hf
    IsNormedField R :=
  letI := NormMetric.induced R S f hf
  { IsNormedAddGroup.induced R S f with
...
field structure.
-/
instance toIsNormedField [NormMetric F] [Field F] [IsNormedField F] [SubfieldClass S F] (s : S) :
    IsNormedField s :=
  IsNormedField.induced s F (SubringClass.subtype s) Subtype.val_injective
...
lemma toIsNormedField {K : Type*} [Field K] (v : AbsoluteValue K ℝ) :
    letI := v.toNormMetric
    IsNormedField K :=
  letI := v.toNormMetric
  { norm_mul := v.map_mul }
