...
@[simp]
theorem norm_coe {E} [AddCommGroup E] [SeminormedAddGroup E] (x : E) : ‖(x : Completion E)‖ = ‖x‖ :=
  Completion.extension_coe uniformContinuous_norm x

instance [AddCommGroup E] [SeminormedAddGroup E] : NormedAddGroup (Completion E) where
  dist_eq x y := by
...
@[simp]
theorem nnnorm_coe {E} [AddCommGroup E] [SeminormedAddGroup E] (x : E) :
    ‖(x : Completion E)‖₊ = ‖x‖₊ := by
  simp [nnnorm]

@[simp]
lemma enorm_coe {E} [AddCommGroup E] [SeminormedAddGroup E] (x : E) :
