...
lemma norm_mul_le_max (x y : S) :
    ‖x * y‖ ≤ max ‖x‖ ‖y‖ := by
  simpa only [le_max_iff, dist_eq_norm_inv_mul, div_inv_eq_mul, div_one, one_mul] using
...
lemma isUltrametricDist_of_forall_norm_mul_le_max_norm
  ...
  dist_triangle_max x y z := by
    simpa only [dist_eq_norm_inv_mul, le_max_iff, div_mul_div_cancel] using h (x / y) (y / z)
...
lemma norm_mul_eq_max_of_norm_ne_norm
  ...
  rw [← div_inv_eq_mul, ← dist_eq_norm_inv_mul, dist_eq_max_of_dist_ne_dist _ 1 _ (by simp [h])]
...
def ball_openSubgroup {r : ℝ} (hr : 0 < r) : OpenSubgroup S where
  ...
  mul_mem' {x} {y} hx hy := by
    simp only [Metric.mem_ball, dist_eq_norm_inv_mul, div_one] at hx hy ⊢
...
def closedBall_openSubgroup {r : ℝ} (hr : 0 < r) : OpenSubgroup S where
  ...
  mul_mem' {x} {y} hx hy := by
    simp only [Metric.mem_closedBall, dist_eq_norm_inv_mul, div_one] at hx hy ⊢
