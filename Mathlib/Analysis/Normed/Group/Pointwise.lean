...
@[to_additive]
lemma Bornology.IsBounded.mul (hs : IsBounded s) (ht : IsBounded t) : IsBounded (s * t) := by
...
@[to_additive]
lemma Bornology.IsBounded.of_mul (hst : IsBounded (s * t)) : IsBounded s ∨ IsBounded t :=
...
@[to_additive]
lemma Bornology.IsBounded.inv : IsBounded s → IsBounded s⁻¹ := by
...
@[to_additive]
lemma Bornology.IsBounded.div (hs : IsBounded s) (ht : IsBounded t) : IsBounded (s / t) :=
...
@[to_additive]
lemma ediam_mul_le (x y : Set E) : EMetric.diam (x * y) ≤ EMetric.diam x + EMetric.diam y :=
...
@[to_additive (attr := simp)]
lemma inv_thickening : (thickening δ s)⁻¹ = thickening δ s⁻¹ := by
...
@[to_additive (attr := simp)]
lemma inv_cthickening : (cthickening δ s)⁻¹ = cthickening δ s⁻¹ := by
...
@[to_additive (attr := simp)]
lemma inv_ball : (ball x δ)⁻¹ = ball x⁻¹ δ := (IsometryEquiv.inv E).preimage_ball x δ
#align inv_ball inv_ball
#align neg_ball neg_ball

@[to_additive (attr := simp)]
lemma inv_closedBall : (closedBall x δ)⁻¹ = closedBall x⁻¹ δ :=
  (IsometryEquiv.inv E).preimage_closedBall x δ
#align inv_closed_ball inv_closedBall
#align neg_closed_ball neg_closedBall

@[to_additive]
lemma singleton_mul_ball : {x} * ball y δ = ball (x * y) δ := by
...
@[to_additive]
lemma singleton_div_ball : {x} / ball y δ = ball (x / y) δ := by
...
@[to_additive]
lemma ball_mul_singleton : ball x δ * {y} = ball (x * y) δ := by
...
@[to_additive]
lemma ball_div_singleton : ball x δ / {y} = ball (x / y) δ := by
...
@[to_additive]
lemma singleton_mul_ball_one : {x} * ball 1 δ = ball x δ := by simp
#align singleton_mul_ball_one singleton_mul_ball_one
#align singleton_add_ball_zero singleton_add_ball_zero

@[to_additive]
lemma singleton_div_ball_one : {x} / ball 1 δ = ball x δ := by
...
@[to_additive]
lemma ball_one_mul_singleton : ball 1 δ * {x} = ball x δ := by simp [ball_mul_singleton]
#align ball_one_mul_singleton ball_one_mul_singleton
#align ball_zero_add_singleton ball_zero_add_singleton

@[to_additive]
lemma ball_one_div_singleton : ball 1 δ / {x} = ball x⁻¹ δ := by
...
@[to_additive]
lemma smul_ball_one : x • ball (1 : E) δ = ball x δ := by
  rw [smul_ball, smul_eq_mul, mul_one]
#align smul_ball_one smul_ball_one
#align vadd_ball_zero vadd_ball_zero

@[to_additive (attr := simp 1100)]
lemma singleton_mul_closedBall : {x} * closedBall y δ = closedBall (x * y) δ := by
...
@[to_additive (attr := simp 1100)]
lemma singleton_div_closedBall : {x} / closedBall y δ = closedBall (x / y) δ := by
...
@[to_additive (attr := simp 1100)]
lemma closedBall_mul_singleton : closedBall x δ * {y} = closedBall (x * y) δ := by
...
@[to_additive (attr := simp 1100)]
lemma closedBall_div_singleton : closedBall x δ / {y} = closedBall (x / y) δ := by
  simp [div_eq_mul_inv]
#align closed_ball_div_singleton closedBall_div_singleton
#align closed_ball_sub_singleton closedBall_sub_singleton

@[to_additive]
lemma singleton_mul_closedBall_one : {x} * closedBall 1 δ = closedBall x δ := by simp
...
@[to_additive]
lemma singleton_div_closedBall_one : {x} / closedBall 1 δ = closedBall x δ := by
...
@[to_additive]
lemma closedBall_one_mul_singleton : closedBall 1 δ * {x} = closedBall x δ := by simp
...
@[to_additive]
lemma closedBall_one_div_singleton : closedBall 1 δ / {x} = closedBall x⁻¹ δ := by simp
...
@[to_additive (attr := simp 1100)]
lemma smul_closedBall_one : x • closedBall (1 : E) δ = closedBall x δ := by simp
#align smul_closed_ball_one smul_closedBall_one
#align vadd_closed_ball_zero vadd_closedBall_zero

@[to_additive]
lemma mul_ball_one : s * ball 1 δ = thickening δ s := by
...
@[to_additive (attr := simp)]
lemma mul_ball : s * ball x δ = x • thickening δ s := by
  rw [← smul_ball_one, mul_smul_comm, mul_ball_one]
#align mul_ball mul_ball
#align add_ball add_ball

@[to_additive (attr := simp)]
lemma div_ball : s / ball x δ = x⁻¹ • thickening δ s := by simp [div_eq_mul_inv]
#align div_ball div_ball
#align sub_ball sub_ball

@[to_additive (attr := simp)]
lemma ball_mul : ball x δ * s = x • thickening δ s := by rw [mul_comm, mul_ball]
#align ball_mul ball_mul
#align ball_add ball_add

@[to_additive (attr := simp)]
lemma ball_div : ball x δ / s = x • thickening δ s⁻¹ := by simp [div_eq_mul_inv]
