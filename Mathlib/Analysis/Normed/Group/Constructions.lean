...
section SeminormedGroup
variable [∀ i, SeminormedGroup (X i)] [SeminormedGroup E] (f : ∀ i, X i) {x : ∀ i, X i} {r : ℝ}

/-- Finite product of seminormed groups, using the sup norm. -/
@[to_additive "Finite product of seminormed groups, using the sup norm."]
instance Pi.seminormedGroup : SeminormedGroup (∀ i, X i) where
...
/-- Finite product of seminormed groups, using the sup norm. -/
@[to_additive "Finite product of seminormed groups, using the sup norm."]
instance Pi.seminormedCommGroup [∀ i, SeminormedCommGroup (X i)] : SeminormedCommGroup (∀ i, X i) :=
  { Pi.seminormedGroup with
    mul_comm := mul_comm }

/-- Finite product of normed groups, using the sup norm. -/
@[to_additive "Finite product of seminormed groups, using the sup norm."]
instance Pi.normedGroup [∀ i, NormedGroup (X i)] : NormedGroup (∀ i, X i) :=
  { Pi.seminormedGroup with
    eq_of_dist_eq_zero := eq_of_dist_eq_zero }

/-- Finite product of normed groups, using the sup norm. -/
@[to_additive "Finite product of seminormed groups, using the sup norm."]
instance Pi.normedCommGroup [∀ i, NormedCommGroup (X i)] : NormedCommGroup (∀ i, X i) :=
  { Pi.seminormedGroup with
    mul_comm := mul_comm
    eq_of_dist_eq_zero := eq_of_dist_eq_zero }

theorem Pi.nnnorm_single [DecidableEq ι] [∀ i, NormedAddCommGroup (X i)] {i : ι} (y : X i) :
    ‖Pi.single i y‖₊ = ‖y‖₊ := by
  have H : ∀ b, ‖single i y b‖₊ = single (f := fun _ ↦ ℝ≥0) i ‖y‖₊ b := by
    intro b
    refine Pi.apply_single (fun i (x : X i) ↦ ‖x‖₊) ?_ i y b
    simp
  simp [Pi.nnnorm_def, H, Pi.single_apply, Finset.sup_ite, Finset.filter_eq']

lemma Pi.enorm_single [DecidableEq ι] [∀ i, NormedAddCommGroup (X i)] {i : ι} (y : X i) :
    ‖Pi.single i y‖ₑ = ‖y‖ₑ := by simp [enorm, Pi.nnnorm_single]

theorem Pi.norm_single [DecidableEq ι] [∀ i, NormedAddCommGroup (X i)] {i : ι} (y : X i) :
    ‖Pi.single i y‖ = ‖y‖ :=
