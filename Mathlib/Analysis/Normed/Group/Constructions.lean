...
instance normedAddCommGroup : WithNormedAddGroup PUnit where
...
@[to_additive]
instance seminormedGroup [SeminormedGroup E] : WithSeminormedGroup (ULift E) :=
...
@[to_additive]
instance normedGroup [NormedGroup E] : WithNormedGroup (ULift E) :=
...
instance Additive.seminormedAddGroup [SeminormedGroup E] : WithSeminormedAddGroup (Additive E) where
  dist_eq x y := dist_eq_norm_div x.toMul y.toMul


instance Multiplicative.seminormedGroup [SeminormedAddGroup E] :
    WithSeminormedGroup (Multiplicative E) where
  dist_eq x y := dist_eq_norm_sub x.toAdd y.toAdd

instance Additive.normedAddGroup [NormedGroup E] : WithNormedAddGroup (Additive E) :=
  { Additive.seminormedAddGroup with
    eq_of_dist_eq_zero := eq_of_dist_eq_zero }

instance Multiplicative.normedGroup [NormedAddGroup E] : WithNormedGroup (Multiplicative E) :=
...
instance Prod.seminormedGroup : WithSeminormedGroup (E × F) where
  dist_eq x y := by
...
instance normedGroup [NormedGroup E] [NormedGroup F] : WithNormedGroup (E × F) :=
...
instance Pi.seminormedGroup : WithSeminormedGroup (∀ i, G i) where
...
@[to_additive /-- Finite product of seminormed groups, using the sup norm. -/]
instance Pi.normedGroup [∀ i, NormedGroup (G i)] : WithNormedGroup (∀ i, G i) :=
...
-/
instance instSeminormedAddGroup [SeminormedAddGroup E] : WithSeminormedAddGroup Eᵐᵒᵖ where
...
instance instNormedAddGroup [NormedAddGroup E] : WithNormedAddGroup Eᵐᵒᵖ where
  __ := instMetricSpace
