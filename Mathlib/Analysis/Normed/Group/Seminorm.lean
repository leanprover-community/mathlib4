...
@[to_additive]
instance (priority := 10000) funLike : FunLike (GroupSeminorm E) E ℝ
    where
  coe f := f.toFun
  coe_injective' f g h := by cases f; cases g; congr

@[to_additive]
instance (priority := 10000) groupSeminormClass : GroupSeminormClass (GroupSeminorm E) E ℝ
    where
...
instance (priority := 10000) : CoeFun (GroupSeminorm E) fun _ => E → ℝ :=
  ⟨DFunLike.coe⟩
...
@[to_additive]
instance (priority := 10000) : PartialOrder (GroupSeminorm E) :=
...
@[to_additive]
instance (priority := 10000) instZeroGroupSeminorm : Zero (GroupSeminorm E) :=
  ⟨{  toFun := 0
...
@[to_additive]
instance (priority := 10000) : Inhabited (GroupSeminorm E) :=
  ⟨0⟩

@[to_additive]
instance (priority := 10000) : Add (GroupSeminorm E) :=
  ⟨fun p q =>
    { toFun := fun x => p x + q x
...
@[to_additive]
instance (priority := 10000) : Sup (GroupSeminorm E) :=
  ⟨fun p q =>
    { toFun := p ⊔ q
      map_one' := by
...
@[to_additive]
instance (priority := 10000) semilatticeSup : SemilatticeSup (GroupSeminorm E) :=
...
@[to_additive]
noncomputable instance (priority := 10000) : Inf (GroupSeminorm E) :=
  ⟨fun p q =>
...
@[to_additive]
noncomputable instance (priority := 10000) : Lattice (GroupSeminorm E) :=
...
instance (priority := 10000) toOne [DecidableEq E] : One (AddGroupSeminorm E) :=
...
instance (priority := 10000) toSMul : SMul R (AddGroupSeminorm E) :=
  ⟨fun r p =>
    { toFun := fun x => r • p x
      map_zero' := by
...
instance (priority := 10000) isScalarTower [SMul R' ℝ] [SMul R' ℝ≥0] [IsScalarTower R' ℝ≥0 ℝ] [SMul R R']
...
instance (priority := 10000) funLike : FunLike (NonarchAddGroupSeminorm E) E ℝ
    where
  coe f := f.toFun
  coe_injective' f g h := by obtain ⟨⟨_, _⟩, _, _⟩ := f; cases g; congr

instance (priority := 10000) nonarchAddGroupSeminormClass : NonarchAddGroupSeminormClass (NonarchAddGroupSeminorm E) E
    where
...
/-- Helper instance for when there's too many metavariables to apply `DFunLike.hasCoeToFun`. -/
instance (priority := 10000) : CoeFun (NonarchAddGroupSeminorm E) fun _ => E → ℝ :=
  ⟨DFunLike.coe⟩
...
noncomputable instance (priority := 10000) : PartialOrder (NonarchAddGroupSeminorm E) :=
  PartialOrder.lift _ DFunLike.coe_injective
...
instance (priority := 10000) : Zero (NonarchAddGroupSeminorm E) :=
  ⟨{  toFun := 0
...
instance (priority := 10000) : Inhabited (NonarchAddGroupSeminorm E) :=
  ⟨0⟩

-- TODO: define `SupSet` too, from the skeleton at
-- https://github.com/leanprover-community/mathlib/pull/11329#issuecomment-1008915345
instance (priority := 10000) : Sup (NonarchAddGroupSeminorm E) :=
  ⟨fun p q =>
    { toFun := p ⊔ q
...
noncomputable instance (priority := 10000) : SemilatticeSup (NonarchAddGroupSeminorm E) :=
  DFunLike.coe_injective.semilatticeSup _ coe_sup
...
instance (priority := 10000) toOne [DecidableEq E] : One (GroupSeminorm E) :=
...
instance (priority := 10000) : SMul R (GroupSeminorm E) :=
  ⟨fun r p =>
    { toFun := fun x => r • p x
      map_one' := by
...
instance (priority := 10000) [SMul R' ℝ] [SMul R' ℝ≥0] [IsScalarTower R' ℝ≥0 ℝ] [SMul R R'] [IsScalarTower R R' ℝ] :
...
instance (priority := 10000) [DecidableEq E] : One (NonarchAddGroupSeminorm E) :=
  ⟨{  toFun := fun x => if x = 0 then 0 else 1
      map_zero' := if_pos rfl
...
instance (priority := 10000) : SMul R (NonarchAddGroupSeminorm E) :=
  ⟨fun r p =>
    { toFun := fun x => r • p x
      map_zero' := by
...
@[to_additive]
instance (priority := 10000) funLike : FunLike (GroupNorm E) E ℝ
    where
  coe f := f.toFun
  coe_injective' f g h := by obtain ⟨⟨_, _, _, _⟩, _⟩ := f; cases g; congr

@[to_additive]
instance (priority := 10000) groupNormClass : GroupNormClass (GroupNorm E) E ℝ
    where
  map_one_eq_zero f := f.map_one'
  map_mul_le_add f := f.mul_le'
  map_inv_eq_map f := f.inv'
...
instance (priority := 10000) : CoeFun (GroupNorm E) fun _ => E → ℝ :=
  DFunLike.hasCoeToFun
...
@[to_additive]
instance (priority := 10000) : PartialOrder (GroupNorm E) :=
  PartialOrder.lift _ DFunLike.coe_injective
...
@[to_additive]
instance (priority := 10000) : Add (GroupNorm E) :=
  ⟨fun p q =>
...
-- TODO: define `SupSet`
@[to_additive]
instance (priority := 10000) : Sup (GroupNorm E) :=
  ⟨fun p q =>
    { p.toGroupSeminorm ⊔ q.toGroupSeminorm with
      eq_one_of_map_eq_zero' := fun _x hx =>
...
@[to_additive]
instance (priority := 10000) : SemilatticeSup (GroupNorm E) :=
  DFunLike.coe_injective.semilatticeSup _ coe_sup
...
instance (priority := 10000) : One (AddGroupNorm E) :=
...
instance (priority := 10000) : Inhabited (AddGroupNorm E) :=
  ⟨1⟩
...
instance (priority := 10000) _root_.AddGroupNorm.toOne [AddGroup E] [DecidableEq E] : One (AddGroupNorm E) :=
  ⟨{ (1 : AddGroupSeminorm E) with
...
instance (priority := 10000) toOne : One (GroupNorm E) :=
...
instance (priority := 10000) : Inhabited (GroupNorm E) :=
  ⟨1⟩
...
instance (priority := 10000) funLike : FunLike (NonarchAddGroupNorm E) E ℝ
    where
  coe f := f.toFun
  coe_injective' f g h := by obtain ⟨⟨⟨_, _⟩, _, _⟩, _⟩ := f; cases g; congr

instance (priority := 10000) nonarchAddGroupNormClass : NonarchAddGroupNormClass (NonarchAddGroupNorm E) E
    where
  map_add_le_max f := f.add_le_max'
  map_zero f := f.map_zero'
  map_neg_eq_map' f := f.neg'
...
/-- Helper instance for when there's too many metavariables to apply `DFunLike.hasCoeToFun`. -/
noncomputable instance (priority := 10000) : CoeFun (NonarchAddGroupNorm E) fun _ => E → ℝ :=
  DFunLike.hasCoeToFun
...
noncomputable instance (priority := 10000) : PartialOrder (NonarchAddGroupNorm E) :=
  PartialOrder.lift _ DFunLike.coe_injective
...
instance (priority := 10000) : Sup (NonarchAddGroupNorm E) :=
  ⟨fun p q =>
...
noncomputable instance (priority := 10000) : SemilatticeSup (NonarchAddGroupNorm E) :=
  DFunLike.coe_injective.semilatticeSup _ coe_sup

instance (priority := 10000) [DecidableEq E] : One (NonarchAddGroupNorm E) :=
...
instance (priority := 10000) [DecidableEq E] : Inhabited (NonarchAddGroupNorm E) :=
  ⟨1⟩
