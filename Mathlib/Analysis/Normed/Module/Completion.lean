...
instance [NormedField A] [CompletableTopField A] :
  ...
  dist_eq x y := by
    refine Completion.induction_on₂ x y ?_ ?_ <;> clear x y
    · exact isClosed_eq (Completion.uniformContinuous_extension₂ _).continuous
        (Continuous.comp Completion.continuous_extension continuous_sub)
    · intro x y
      rw [← Completion.coe_sub, norm_coe, Completion.dist_eq, dist_eq_norm]
  norm_mul' x y := by
    refine Completion.induction_on₂ x y ?_ ?_ <;> clear x y
    · exact isClosed_eq (Continuous.comp continuous_norm continuous_mul)
        (Continuous.comp _root_.continuous_mul
          (Continuous.prod_map continuous_norm continuous_norm))
    · intro x y
      simp only [← coe_mul, norm_coe]
