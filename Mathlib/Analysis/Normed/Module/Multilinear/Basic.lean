...
lemma norm_map_coord_zero (f : E →ₗₘ[𝕜] G) (hf : Continuous f)
...
theorem bound_of_shell_of_norm_map_coord_zero (f : E →ₗₘ[𝕜] G)
    (hf₀ : ∀ {m i}, ‖m i‖ = 0 → ‖f m‖ = 0)
...
theorem bound_of_shell_of_continuous (f : E →ₗₘ[𝕜] G) (hfc : Continuous f)
    {ε : ι → ℝ} {C : ℝ} (hε : ∀ i, 0 < ε i) {c : ι → 𝕜} (hc : ∀ i, 1 < ‖c i‖)
    (hf : ∀ m : ∀ i, E i, (∀ i, ε i / ‖c i‖ ≤ ‖m i‖) → (∀ i, ‖m i‖ < ε i) → ‖f m‖ ≤ C * ∏ i, ‖m i‖)
    (m : ∀ i, E i) : ‖f m‖ ≤ C * ∏ i, ‖m i‖ :=
...
positive. -/
theorem exists_bound_of_continuous (f : E →ₗₘ[𝕜] G) (hf : Continuous f) :
...
theorem norm_image_sub_le_of_bound' [DecidableEq ι] (f : E →ₗₘ[𝕜] G) {C : ℝ} (hC : 0 ≤ C)
...
theorem norm_image_sub_le_of_bound (f : E →ₗₘ[𝕜] G)
...
continuous. -/
theorem continuous_of_bound (f : E →ₗₘ[𝕜] G) (C : ℝ) (H : ∀ m, ‖f m‖ ≤ C * ∏ i, ‖m i‖) :
    Continuous f := by
  let D := max C 1
...
condition. -/
def mkContinuous (f : E →ₗₘ[𝕜] G) (C : ℝ) (H : ∀ m, ‖f m‖ ≤ C * ∏ i, ‖m i‖) :
    ContinuousMultilinearMap 𝕜 E G :=
  { f with cont := f.continuous_of_bound C H }

@[simp]
theorem coe_mkContinuous (f : E →ₗₘ[𝕜] G) (C : ℝ) (H : ∀ m, ‖f m‖ ≤ C * ∏ i, ‖m i‖) :
...
nonnegative. -/
theorem MultilinearMap.mkContinuous_norm_le (f : E →ₗₘ[𝕜] G) {C : ℝ} (hC : 0 ≤ C)
...
`mkContinuous`, then its norm is bounded by the bound given to the constructor if it is
nonnegative. -/
theorem MultilinearMap.mkContinuous_norm_le' (f : E →ₗₘ[𝕜] G) {C : ℝ}
...
which is a linear map from `ContinuousMultilinearMap 𝕜 E G` to `E' →ₗₘ[𝕜] G'`. -/
def mkContinuousLinear (f : G →ₗ[𝕜] E →ₗₘ[𝕜] G') (C : ℝ)
...
theorem mkContinuousLinear_norm_le' (f : G →ₗ[𝕜] E →ₗₘ[𝕜] G') (C : ℝ)
...
theorem mkContinuousLinear_norm_le (f : G →ₗ[𝕜] E →ₗₘ[𝕜] G') {C : ℝ} (hC : 0 ≤ C)
...
theorem bound_of_shell (f : E →ₗₘ[𝕜] G) {ε : ι → ℝ} {C : ℝ} {c : ι → 𝕜}
