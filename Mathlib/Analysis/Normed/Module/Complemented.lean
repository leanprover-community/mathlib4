...
lemma ker_id_sub_eq_range {P : E →ₗ[𝕜] E} (h : IsIdempotentElem P) : ker (1 - P) = range P :=
  (Submodule.toAddSubgroup_inj (ker (1 - P)) (range P)).mp (by
    rw [range_toAddSubgroup, ker_toAddSubgroup, ← toAddMonoidHom'_apply,
      ← toAddMonoidEnd_apply, ← toAddMonoidHom'_apply, ← toAddMonoidEnd_apply, RingHom.map_sub,
      RingHom.map_one, AddMonoid.End.ker_id_sub_eq_range (h.map _)])
