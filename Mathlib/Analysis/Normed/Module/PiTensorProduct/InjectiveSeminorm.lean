...
noncomputable irreducible_def injectiveSeminorm : Seminorm 𝕜 (⨂[𝕜] i, E i) :=
  sSup {p | ∃ (G : Type (max uι u𝕜 uE)) (_ : AddCommGroup G) (_ : SeminormedAddCommGroup G)
...
lemma dualSeminorms_bounded : BddAbove {p | ∃ (G : Type (max uι u𝕜 uE)) (_ : AddCommGroup G)
...
theorem injectiveSeminorm_apply (x : ⨂[𝕜] i, E i) :
    injectiveSeminorm x = ⨆ p : {p | ∃ (G : Type (max uι u𝕜 uE)) (_ : AddCommGroup G)
...
variable [∀ i, AddCommGroup (E'' i)] [∀ i, SeminormedAddCommGroup (E'' i)]
  [∀ i, NormedSpace 𝕜 (E'' i)]
...
private theorem mapL_add_smul_aux {ι : Type uι}
    {E : ι → Type uE} [∀ i, AddCommGroup (E i)] [(i : ι) → SeminormedAddCommGroup (E i)]
    [(i : ι) → NormedSpace 𝕜 (E i)] {E' : ι → Type u_1} [∀ i, AddCommGroup (E' i)]
    [(i : ι) → SeminormedAddCommGroup (E' i)] [(i : ι) → NormedSpace 𝕜 (E' i)]
