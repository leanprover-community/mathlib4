...
def algNormFromConst {x : L} (hx : x ≠ 0) :
  ...
  { normFromConst hx' spectralAlgNorm'_isPowMul with
    smul' k y := by
      have h_mul : ∀ y : L, spectralNorm' K L (algebraMap K L k * y) =
          spectralNorm' K L (algebraMap K L k) * spectralNorm' K L y := fun y ↦ by
        rw [spectralNorm'_extends, ← Algebra.smul_def, ← spectralAlgNorm'_def,
          map_smul_eq_mul _ _ _, spectralAlgNorm'_def]
      have h : spectralNorm' K L (algebraMap K L k) =
        seminormFromConst' x (spectralAlgNorm' K L).toRingSeminorm (algebraMap K L k) := by
          rw [seminormFromConst_apply_of_isMul hx' spectralAlgNorm'_isPowMul h_mul]; rfl
      rw [← @spectralNorm'_extends K _ L _ _ k, Algebra.smul_def, h]
      exact seminormFromConst_isMul_of_isMul hx' spectralAlgNorm'_isPowMul h_mul y }
