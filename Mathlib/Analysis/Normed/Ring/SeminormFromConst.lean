...
theorem seminormFromConst_seq_zero (hf : f 0 = 0) : seminormFromConst_seq c f 0 = 0 := by
  ...
  ext n
  rw [MulZeroClass.zero_mul, hf, zero_div, Pi.zero_apply]
...
def seminormFromConst' (x : R) : ℝ :=
  Classical.choose (Real.tendsto_of_bddBelow_antitone (seminormFromConst_bddBelow c f x)
...
theorem seminormFromConst_isLimit (x : R) :
  ...
  Classical.choose_spec (Real.tendsto_of_bddBelow_antitone (seminormFromConst_bddBelow c f x)
      (seminormFromConst_seq_antitone hf1 hc hpm x))

/-- `seminormFromConst' hf1 hc hpm 0 = 0`. -/
theorem seminormFromConst_zero : seminormFromConst' hf1 hc hpm 0 = 0 :=
...
theorem seminormFromConst_is_norm_one_class : seminormFromConst' hf1 hc hpm 1 = 1 := by
...
theorem seminormFromConst_mul (x y : R) : seminormFromConst' hf1 hc hpm (x * y) ≤
  ...
  refine le_of_tendsto_of_tendsto' hlim
    (Tendsto.mul (seminormFromConst_isLimit hf1 hc hpm x) (seminormFromConst_isLimit hf1 hc hpm y))
    (fun n ↦ ?_)
  simp only [seminormFromConst_seq]
  rw [div_mul_div_comm, ← pow_add, two_mul,
    div_le_div_right (pow_pos (lt_of_le_of_ne (apply_nonneg f _) hc.symm) _), pow_add, ← mul_assoc,
    mul_comm (x * y), ← mul_assoc, mul_assoc, mul_comm (c ^ n)]
  exact map_mul_le_mul f (x * c ^ n) (y * c ^ n)

/-- `seminormFromConst' hf1 hc hpm` is invariant under negation of `x`. -/
theorem seminormFromConst_neg (x : R) :
...
def seminormFromConst : RingSeminorm R where
  ...
  map_zero' := seminormFromConst_zero hf1 hc hpm
  add_le'   := seminormFromConst_add hf1 hc hpm
  neg'      := seminormFromConst_neg hf1 hc hpm
  mul_le'   := seminormFromConst_mul hf1 hc hpm
...
theorem seminormFromConst_is_norm_le_one_class : seminormFromConst' hf1 hc hpm 1 ≤ 1 :=
  le_of_eq (seminormFromConst_is_norm_one_class hf1 hc hpm)
...
theorem seminormFromConst_isNonarchimedean (hna : IsNonarchimedean f) :
  ...
  apply le_of_tendsto_of_tendsto' (seminormFromConst_isLimit hf1 hc hpm (x + y))
    (Tendsto.max (seminormFromConst_isLimit hf1 hc hpm x) (seminormFromConst_isLimit hf1 hc hpm y))
  intro n
...
theorem seminormFromConst_isPowMul : IsPowMul (seminormFromConst' hf1 hc hpm) := fun x m hm ↦ by
  ...
  have hlim : Tendsto (fun n => seminormFromConst_seq c f (x ^ m) (m * n)) atTop
    ...
    apply Tendsto.comp (seminormFromConst_isLimit hf1 hc hpm (x ^ m))
  ...
  convert Tendsto.pow (seminormFromConst_isLimit hf1 hc hpm x) m using 1
  ext n
...
theorem seminormFromConst_is_hMul_of_is_hMul {x : R} (hx : ∀ y : R, f (x * y) = f x * f y) (y : R) :
  ...
  have hlim : Tendsto (seminormFromConst_seq c f (x * y)) atTop
    ...
    simpa [hseq] using Tendsto.const_mul _ (seminormFromConst_isLimit hf1 hc hpm y)
...
theorem seminormFromConst_const_mul (x : R) : seminormFromConst' hf1 hc hpm (c * x) =
  ...
  have hlim : Tendsto (fun n => seminormFromConst_seq c f x (n + 1)) atTop
    ...
    apply Tendsto.comp (seminormFromConst_isLimit hf1 hc hpm x)
  ...
  simpa [hterm] using Tendsto.mul tendsto_const_nhds hlim
