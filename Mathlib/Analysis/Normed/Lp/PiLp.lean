...
instance [∀ i, PseudoEMetricSpace (β i)] : PseudoEMetricSpace (PiLp p β) :=
  (pseudoEmetricAux p β).replaceUniformity (private_decl% (uniformity_aux p β)).symm
...
instance [∀ i, PseudoMetricSpace (β i)] : PseudoMetricSpace (PiLp p β) :=
  ((pseudoMetricAux p β).replaceUniformity
    (private_decl% (uniformity_aux p β)).symm).replaceBornology
    fun s => Filter.ext_iff.1 (private_decl% (cobounded_aux p β)).symm sᶜ
