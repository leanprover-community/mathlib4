...
theorem paley_zygmund [IsProbabilityMeasure μ] {Z : Ω → ℝ} (hZ_nn : 0 ≤ᵐ[μ] Z) (hZ2 : MemLp Z 2 μ)
  ...
  * (μ {ω | θ * ∫ ω', Z ω' ∂μ < Z ω}).toReal := by
  ...
  have h_split: ∫ ω, Z ω ∂μ = ∫ ω in S, Z ω ∂μ + ∫ ω in Sᶜ, Z ω ∂μ :=
    ...
    hZ2.aemeasurable) (hZ2.integrable one_le_two)).symm
  have h_lower: (1 - θ) * (∫ ω, Z ω ∂μ) ≤ ∫ ω in S, Z ω ∂μ:= by
    have h_bound_comp: ∫ ω in Sᶜ, Z ω ∂μ ≤ θ * ∫ ω, Z ω ∂μ := by
      calc
        ∫ ω in Sᶜ, Z ω ∂μ ≤ ∫ ω in Sᶜ, θ * (∫ ω', Z ω' ∂μ) ∂μ:=by
          exact setIntegral_mono_on₀
            (hf := (hZ2.integrable one_le_two).integrableOn)
        ...
        _≤ θ * ∫ ω, Z ω ∂μ := by
          ...
          apply mul_le_of_le_one_left (mul_nonneg hθ0 (integral_nonneg_of_ae hZ_nn))
            measureReal_le_one
  ...
  have h_cs: (∫ ω in S, Z ω ∂μ) ^ 2 ≤
    ...
    · have h_jensen := ConvexOn.map_set_average_le
        ...
        (measure_ne_top μ S)
        (by simp)
        (hZ2.integrable one_le_two).integrableOn
        hZ2.integrable_sq.integrableOn
      have hμS_pos : 0 < μ.real S := by
        rw [measureReal_def]
        exact ENNReal.toReal_pos hS (measure_ne_top μ S)
      rw [setAverage_eq, setAverage_eq, smul_eq_mul, smul_eq_mul] at h_jensen
      rw [mul_pow] at h_jensen
      rw [sq ((μ.real S)⁻¹), mul_assoc] at h_jensen
      rw [mul_le_mul_iff_of_pos_left (inv_pos.mpr hμS_pos)] at h_jensen
      rw [mul_comm, ← div_eq_mul_inv, div_le_iff₀ hμS_pos] at h_jensen
      rw [measureReal_def] at h_jensen
      calc
