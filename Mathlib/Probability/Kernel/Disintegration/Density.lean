...
the σ-algebra of `γ`.
For `x : γ`, let `partitionSet n x` be the set in the partition such that `x ∈ partitionSet n x`.

For a given `n`, the function `densityProcess κ ν n : α → γ → Set β → ℝ` defined by
`fun a x s ↦ (κ a (partitionSet n x ×ˢ s) / ν a (partitionSet n x)).toReal` has the desired
property that `∫ x in A, densityProcess κ ν n a x s ∂(ν a) = (κ a (A ×ˢ s)).toReal` for all `A`
in the σ-algebra generated by the partition at scale `n` and is measurable in `(a, x)`.
...
def densityProcess (κ : kernel α (γ × β)) (ν : kernel α γ) (n : ℕ) (a : α) (x : γ) (s : Set β) :
    ℝ :=
  (κ a (partitionSet n x ×ˢ s) / ν a (partitionSet n x)).toReal
...
lemma measurable_densityProcess_partitionFiltration_aux (κ : kernel α (γ × β)) (ν : kernel α γ)
    ...
    Measurable[mα.prod (partitionFiltration γ n)] (fun (p : α × γ) ↦
      κ p.1 (partitionSet n p.2 ×ˢ s) / ν p.1 (partitionSet n p.2)) := by
  change Measurable[mα.prod (partitionFiltration γ n)]
  ...
  change @Measurable (α × γ) (countablePartition γ n) (mα.prod (partitionFiltration γ n)) ⊤
...
lemma measurable_partitionFiltration_densityProcess (κ : kernel α (γ × β)) (ν : kernel α γ) (n : ℕ)
    ...
    Measurable[partitionFiltration γ n] (fun x ↦ densityProcess κ ν n a x s) := by
...
lemma stronglyMeasurable_partitionFiltration_densityProcess (κ : kernel α (γ × β)) (ν : kernel α γ)
    ...
    StronglyMeasurable[partitionFiltration γ n] (fun x ↦ densityProcess κ ν n a x s) :=
...
lemma adapted_densityProcess (κ : kernel α (γ × β)) (ν : kernel α γ) (a : α)
    {s : Set β} (hs : MeasurableSet s) :
    Adapted (partitionFiltration γ) (fun n x ↦ densityProcess κ ν n a x s) :=
...
lemma meas_partitionSet_le_of_fst_le (hκν : fst κ ≤ ν) (n : ℕ) (a : α) (x : γ) (s : Set β) :
    κ a (partitionSet n x ×ˢ s) ≤ ν a (partitionSet n x) := by
  calc κ a (partitionSet n x ×ˢ s)
    ≤ fst κ a (partitionSet n x) := by
        rw [fst_apply' _ _ (measurableSet_partitionSet _ _)]
        refine measure_mono (fun x ↦ ?_)
        simp only [mem_prod, mem_setOf_eq, and_imp]
        exact fun h _ ↦ h
  _ ≤ ν a (partitionSet n x) := hκν a _
...
lemma densityProcess_le_one (hκν : fst κ ≤ ν) (n : ℕ) (a : α) (x : γ) (s : Set β) :
  ...
  exact meas_partitionSet_le_of_fst_le hκν n a x s
...
lemma set_integral_densityProcess_I (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
  ...
  · refine Measurable.aemeasurable ?_
    change Measurable ((fun (p : α × _) ↦ κ p.1 (partitionSet n p.2 ×ˢ s)
      / ν p.1 (partitionSet n p.2)) ∘ (fun x ↦ (a, x)))
    exact (measurable_densityProcess_aux κ ν n hs).comp measurable_prod_mk_left
  · refine ae_of_all _ (fun x ↦ ?_)
    by_cases h0 : ν a (partitionSet n x) = 0
    · suffices κ a (partitionSet n x ×ˢ s) = 0 by simp [h0, this]
      have h0' : fst κ a (partitionSet n x) = 0 :=
  ...
  congr
  have : ∫⁻ x in u, κ a (partitionSet n x ×ˢ s) / ν a (partitionSet n x) ∂(ν a)
...
lemma condexp_densityProcess (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
    ...
    (ν a)[fun x ↦ densityProcess κ ν j a x s | partitionFiltration γ i]
...
lemma martingale_densityProcess (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
    (a : α) {s : Set β} (hs : MeasurableSet s) :
    Martingale (fun n x ↦ densityProcess κ ν n a x s) (partitionFiltration γ) (ν a) :=
...
lemma densityProcess_mono_set (hκν : fst κ ≤ ν) (n : ℕ) (a : α) (x : γ)
  ...
  · rw [h0, ENNReal.toReal_div, ENNReal.toReal_div]
    simp
  have h_ne_top : ∀ s, κ a (partitionSet n x ×ˢ s) / ν a (partitionSet n x) ≠ ⊤ := by
    intro s
...
lemma densityProcess_mono_kernel_left {κ' : kernel α (γ × β)} (hκκ' : κ ≤ κ')
  ...
  · rw [h0, ENNReal.toReal_div, ENNReal.toReal_div]
    simp
  have h_le : κ' a (partitionSet n x ×ˢ s) ≤ ν a (partitionSet n x) :=
    meas_partitionSet_le_of_fst_le hκ'ν n a x s
...
lemma densityProcess_antitone_kernel_right {ν' : kernel α γ}
  ...
  unfold densityProcess
  have h_le : κ a (partitionSet n x ×ˢ s) ≤ ν a (partitionSet n x) :=
    meas_partitionSet_le_of_fst_le hκν n a x s
  by_cases h0 : ν a (partitionSet n x) = 0
  · simp [le_antisymm (h_le.trans h0.le) zero_le', h0]
  have h0' : ν' a (partitionSet n x) ≠ 0 :=
...
lemma tendsto_densityProcess_atTop_empty_of_antitone (κ : kernel α (γ × β)) (ν : kernel α γ)
    ...
    Tendsto (fun m ↦ densityProcess κ ν n a x (s m)) atTop (𝓝 (densityProcess κ ν n a x ∅)) := by
  simp_rw [densityProcess]
  by_cases h0 : ν a (partitionSet n x) = 0
  ...
  · have h := tendsto_measure_iInter (μ := κ a) (s := fun m ↦ partitionSet n x ×ˢ s m) ?_ ?_ ?_
    · convert h
...
lemma tendsto_densityProcess_atTop_of_antitone (κ : kernel α (γ × β)) (ν : kernel α γ)
    ...
    (s : ℕ → Set β) (hs : Antitone s) (hs_iInter : ⋂ i, s i = ∅)
    (hs_meas : ∀ n, MeasurableSet (s n)) :
    Tendsto (fun m ↦ densityProcess κ ν n a x (s m)) atTop (𝓝 0) := by
  rw [← densityProcess_empty κ ν n a x]
  exact tendsto_densityProcess_atTop_empty_of_antitone κ ν n a x s hs hs_iInter hs_meas
...
lemma tendsto_snorm_one_densityProcess_limitProcess (hκν : fst κ ≤ ν) [IsFiniteKernel κ]
    ...
    Tendsto (fun n ↦ snorm ((fun x ↦ densityProcess κ ν n a x s)
      - (partitionFiltration γ).limitProcess (fun n x ↦ densityProcess κ ν n a x s) (ν a))
      1 (ν a)) atTop (𝓝 0) := by
...
lemma set_lintegral_density (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
    (a : α) {s : Set β} (hs : MeasurableSet s) {A : Set γ} (hA : MeasurableSet A) :
...
lemma lintegral_density (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
    (a : α) {s : Set β} (hs : MeasurableSet s) :
...
lemma tendsto_integral_density_of_monotone (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
    ...
    (hs_meas : ∀ n, MeasurableSet (s n)) :
    Tendsto (fun m ↦ ∫ x, density κ ν a x (s m) ∂(ν a)) atTop (𝓝 (κ a univ).toReal) := by
...
lemma tendsto_integral_density_of_antitone (hκν : fst κ ≤ ν) [IsFiniteKernel κ]
    ...
    (hs_meas : ∀ n, MeasurableSet (s n)) :
    Tendsto (fun m ↦ ∫ x, density κ ν a x (s m) ∂(ν a)) atTop (𝓝 0) := by
  simp_rw [integral_density hκν a (hs_meas _)]
...
lemma tendsto_density_atTop_ae_of_antitone (hκν : fst κ ≤ ν) [IsFiniteKernel κ]
    [IsFiniteKernel ν] (a : α) (s : ℕ → Set β) (hs : Antitone s) (hs_iInter : ⋂ i, s i = ∅)
    (hs_meas : ∀ n, MeasurableSet (s n)) :
    ∀ᵐ x ∂(ν a), Tendsto (fun m ↦ density κ ν a x (s m)) atTop (𝓝 0) := by
  have h_anti : ∀ x, Antitone (fun m ↦ density κ ν a x (s m)) :=
    fun x n m hnm ↦ density_mono_set hκν a x (hs hnm)
  have h_le_one : ∀ m x, density κ ν a x (s m) ≤ 1 := fun m x ↦ density_le_one hκν a x (s m)
  -- for all `x`, `fun m ↦ density κ a (s m) x` has a limit
  have h_exists : ∀ x, ∃ l, Tendsto (fun m ↦ density κ ν a x (s m)) atTop (𝓝 l) := by
    intro x
    have h_tendsto : Tendsto (fun m ↦ density κ ν a x (s m)) atTop atBot ∨
        ∃ l, Tendsto (fun m ↦ density κ ν a x (s m)) atTop (𝓝 l) :=
      tendsto_of_antitone (h_anti x)
    cases' h_tendsto with h_absurd h_tendsto
    · rw [tendsto_atTop_atBot_iff_of_antitone (h_anti x)] at h_absurd
      obtain ⟨r, hr⟩ := h_absurd (-1)
      have h_nonneg := density_nonneg hκν a x (s r)
      linarith
    · exact h_tendsto
  -- let `F` be the pointwise limit of `fun m ↦ density κ a (s m) t` for all `t`
  let F : γ → ℝ := fun x ↦ (h_exists x).choose
  have hF_tendsto : ∀ x, Tendsto (fun m ↦ density κ ν a x (s m)) atTop (𝓝 (F x)) :=
    fun x ↦ (h_exists x).choose_spec
  have hF_nonneg : ∀ x, 0 ≤ F x :=
    fun x ↦ ge_of_tendsto' (hF_tendsto x) (fun m ↦ density_nonneg hκν a x (s m))
  have hF_le_one : ∀ x, F x ≤ 1 := fun x ↦ le_of_tendsto' (hF_tendsto x) (fun m ↦ h_le_one m x)
  ...
  suffices F=ᵐ[ν a] 0 by
    filter_upwards [this] with x hx_eq
    simp only [Pi.zero_apply] at hx_eq
    rw [← hx_eq]
    exact hF_tendsto x
  ...
  have h_integral : Tendsto (fun m : ℕ ↦ ∫ x, density κ ν a x (s m) ∂(ν a)) atTop
      (𝓝 (∫ x, F x ∂(ν a))) := by
...
lemma densityProcess_univ [IsFiniteKernel κ] (n : ℕ) (a : α) (x : γ) :
    densityProcess κ (fst κ) n a x univ = if fst κ a (partitionSet n x) = 0 then 0 else 1 := by
  rw [densityProcess]
  by_cases h : fst κ a (partitionSet n x) = 0
  · simp [h]
    by_cases h' : κ a (partitionSet n x ×ˢ univ) = 0
    · simp [h']
    · rw [ENNReal.div_zero h']
      simp
  · simp only [h, ite_false]
    rw [fst_apply' _ _ (measurableSet_partitionSet _ _)]
    have : partitionSet n x ×ˢ univ = {p : γ × β | p.1 ∈ partitionSet n x} := by
      ext x
      simp
...
lemma densityProcess_univ_ae (κ : kernel α (γ × β)) [IsFiniteKernel κ] (n : ℕ) (a : α) :
  ...
  have : {x | ¬ densityProcess κ (fst κ) n a x univ = 1}
      ⊆ {x | fst κ a (partitionSet n x) = 0} := by
    intro x hx
    simp only [mem_setOf_eq] at hx ⊢
    rw [densityProcess_univ] at hx
    simpa using hx
  refine measure_mono_null this ?_
  have : {x | fst κ a (partitionSet n x) = 0}
...
lemma tendsto_densityProcess_atTop_univ_of_monotone (κ : kernel α (γ × β))
  ...
  · rw [ne_eq, ENNReal.div_eq_top]
    ...
    · refine fun h_top ↦ eq_top_mono (measure_mono (fun x ↦ ?_)) h_top
      simp only [mem_prod, mem_setOf_eq, and_imp]
      exact fun h _ ↦ h
  by_cases h0 : fst κ a (partitionSet n x) = 0
  · rw [fst_apply' _ _ (measurableSet_partitionSet _ _)] at h0 ⊢
    suffices ∀ m, κ a (partitionSet n x ×ˢ s m) = 0 by
      simp only [this, h0, ENNReal.zero_div, tendsto_const_nhds_iff]
      suffices κ a (partitionSet n x ×ˢ univ) = 0 by
  ...
  · have h := tendsto_measure_iUnion (μ := κ a)
      (s := fun m ↦ partitionSet n x ×ˢ s m) ?_
    swap
    · intro m m' hmm'
      simp only [le_eq_subset, prod_subset_prod_iff, subset_rfl, true_and]
      exact Or.inl <| hs hmm'
...
lemma tendsto_density_atTop_ae_of_monotone [IsFiniteKernel κ]
  ...
  let ν := fst κ
  have h_mono : ∀ x, Monotone (fun m ↦ density κ (fst κ) a x (s m)) :=
    fun x n m hnm ↦ density_mono_set le_rfl a x (hs hnm)
  have h_le_one : ∀ m x, density κ ν a x (s m) ≤ 1 := fun m x ↦ density_le_one le_rfl a x (s m)
  -- for all `x`, `fun m ↦ density κ a (s m) x` has a limit
  have h_exists : ∀ x, ∃ l, Tendsto (fun m ↦ density κ ν a x (s m)) atTop (𝓝 l) := by
    intro x
    have h_tendsto : Tendsto (fun m ↦ density κ ν a x (s m)) atTop atTop ∨
        ∃ l, Tendsto (fun m ↦ density κ ν a x (s m)) atTop (𝓝 l) :=
      tendsto_of_monotone (h_mono x)
    cases' h_tendsto with h_absurd h_tendsto
    · rw [tendsto_atTop_atTop_iff_of_monotone (h_mono x)] at h_absurd
      obtain ⟨r, hr⟩ := h_absurd 2
      exact absurd (hr.trans (h_le_one r x)) one_lt_two.not_le
    · exact h_tendsto
  -- let `F` be the pointwise limit of `fun m ↦ density κ a (s m) x` for all `x`
  let F : γ → ℝ := fun x ↦ (h_exists x).choose
  have hF_tendsto : ∀ x, Tendsto (fun m ↦ density κ ν a x (s m)) atTop (𝓝 (F x)) :=
    fun x ↦ (h_exists x).choose_spec
  have hF_nonneg : ∀ x, 0 ≤ F x :=
    fun x ↦ ge_of_tendsto' (hF_tendsto x) (fun m ↦ density_nonneg le_rfl a x (s m))
  have hF_le_one : ∀ x, F x ≤ 1 := fun x ↦ le_of_tendsto' (hF_tendsto x) (fun m ↦ h_le_one m x)
  have hF_int : Integrable F (ν a) := by
    rw [← memℒp_one_iff_integrable]
    constructor
