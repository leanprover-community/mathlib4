...
lemma meas_partitionSet_le_of_fst_le (hκν : fst κ ≤ ν) (n : ℕ) (a : α) (t : γ) (s : Set β) :
...
lemma densityProcess_le_one (hκν : fst κ ≤ ν) (n : ℕ) (a : α) (t : γ) (s : Set β) :
  ...
  exact meas_partitionSet_le_of_fst_le hκν n a t s
...
lemma condexp_densityProcess (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
  ...
  · exact fun t _ _ ↦ (integrable_densityProcess hκν _ _ hs).integrableOn
  · intro t ht _
...
lemma densityProcess_mono_kernel_left {κ' : kernel α (γ × β)} (hκκ' : κ ≤ κ')
  ...
  · rw [h0, ENNReal.toReal_div, ENNReal.toReal_div]
    simp
  have h_le : κ' a (partitionSet n t ×ˢ s) ≤ ν a (partitionSet n t) :=
    meas_partitionSet_le_of_fst_le hκ'ν n a t s
...
lemma densityProcess_antitone_kernel_right {ν' : kernel α γ}
  ...
  unfold densityProcess
  have h_le : κ a (partitionSet n t ×ˢ s) ≤ ν a (partitionSet n t) :=
    meas_partitionSet_le_of_fst_le hκν n a t s
  by_cases h0 : ν a (partitionSet n t) = 0
  · simp [le_antisymm (h_le.trans h0.le) zero_le', h0]
  have h0' : ν' a (partitionSet n t) ≠ 0 :=
...
lemma set_integral_density_of_measurableSet (hκν : fst κ ≤ ν) [IsFiniteKernel κ]
  ...
  suffices ∫ t in A, density κ ν a t s ∂(ν a) = ∫ t in A, densityProcess κ ν n a t s ∂(ν a) by
...
lemma set_lintegral_density (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
    (a : α) {s : Set β} (hs : MeasurableSet s) {A : Set γ} (hA : MeasurableSet A) :
    ∫⁻ t in A, ENNReal.ofReal (density κ ν a t s) ∂(ν a) = κ a (A ×ˢ s) := by
...
lemma lintegral_density (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
    (a : α) {s : Set β} (hs : MeasurableSet s) :
    ∫⁻ t, ENNReal.ofReal (density κ ν a t s) ∂(ν a) = κ a (univ ×ˢ s) := by
