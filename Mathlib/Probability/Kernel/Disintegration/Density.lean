...
```
def f (κ : kernel α (γ × β)) (ν : kernel a γ) (a : α) (t : γ) (s : Set β) : ℝ :=
...
is countably generated. For each `n : ℕ`, we define a finite partition of `γ`, such that those
partitions are finer as `n` grows, and the σ-algebra generated by the union of all partitions is
the σ-algebra of `γ`.
For `t : γ`, let `partitionSet n t` be the set in the partition such that `t ∈ partitionSet n t`.

For a given `n`, the function `densityProcess κ ν n : α → γ → Set β → ℝ` defined by
`fun a t s ↦ (κ a (partitionSet n t ×ˢ s) / ν a (partitionSet n t)).toReal` has the desired
property that `∫ t in A, densityProcess κ ν n a t s ∂(ν a) = (κ a (A ×ˢ s)).toReal` for all `A`
in the σ-algebra generated by the partition at scale `n` and is measurable in `(a, t)`.

Let `partitionFiltration γ` be the filtration of those σ-algebras for all `n : ℕ`.
The functions `densityProcess κ ν n` described here are a bounded `ν`-martingale for the filtration
`partitionFiltration γ`. By Doob's martingale L1 convergence theorem, that martingale converges to
a limit, which has a product-measurable version and satisfies the integral equality for all `A` in
`⨆ n, partitionFiltration γ n`. Finally, the partitions were chosen such that that supremum is equal
to the σ-algebra on `γ`, hence the equality holds for all measurable sets.
We have obtained the desired density function.

## Main definitions

* `ProbabilityTheory.kernel.density`: for `k : kernel α (γ × β)` and `ν : kernel α γ` two finite
...
* `ProbabilityTheory.kernel.set_integral_density`: for all measurable sets `A : Set γ` and
  `s : Set β`, `∫ t in A, kernel.density κ ν a t s ∂(ν a) = (κ a (A ×ˢ s)).toReal`.
...
variable {α β γ : Type*} {mα : MeasurableSpace α} {mγ : MeasurableSpace γ} [CountablyGenerated γ]

section partitionFiltration

/-- A filtration built from the measurable spaces generated by `countablePartition γ n` for
all `n : ℕ`. -/
def partitionFiltration (γ : Type*) [m : MeasurableSpace γ] [CountablyGenerated γ] :
    Filtration ℕ m where
  seq := fun n ↦ generateFrom <| countablePartition γ n
  mono' := monotone_nat_of_le_succ (generateFrom_countablePartition_le_succ _)
  le' := generateFrom_countablePartition_le γ

lemma measurableSet_partitionFiltration_countablePartition (n : ℕ) {s : Set γ}
    (hs : s ∈ countablePartition γ n) :
    MeasurableSet[partitionFiltration γ n] s :=
  measurableSet_generateFrom hs

lemma measurableSet_partitionFiltration_partitionSet (n : ℕ) (t : γ) :
    MeasurableSet[partitionFiltration γ n] (partitionSet n t) :=
  measurableSet_partitionFiltration_countablePartition n (partitionSet_mem n t)

lemma measurable_partitionSet_aux (n : ℕ) (m : MeasurableSpace (countablePartition γ n)) :
    @Measurable γ (countablePartition γ n) (partitionFiltration γ n) m
      (fun c : γ ↦ ⟨partitionSet n c, partitionSet_mem n c⟩) := by
  refine @measurable_to_countable' (countablePartition γ n) γ m _
    (partitionFiltration γ n) _ (fun t ↦ ?_)
  rcases t with ⟨t, ht⟩
  suffices MeasurableSet[partitionFiltration γ n] {x | partitionSet n x = t} by
    convert this
    ext x
    simp
  ...
  rw [this]
  exact measurableSet_partitionFiltration_countablePartition _ ht

lemma measurable_partitionSet (n : ℕ) : Measurable[partitionFiltration γ n] (partitionSet n) :=
  measurable_subtype_coe.comp (measurable_partitionSet_aux _ _)

lemma iSup_partitionFiltration (β : Type*) [m : MeasurableSpace β] [CountablyGenerated β] :
    ⨆ n, partitionFiltration β n = m := by
  conv_rhs => rw [← generateFrom_iUnion_countablePartition β]
...
variable [MeasurableSpace β] {κ : kernel α (γ × β)} {ν : kernel α γ}
...
lemma densityProcess_def (κ : kernel α (γ × β)) (ν : kernel α γ) (n : ℕ) (a : α) (s : Set β) :
    (fun t ↦ densityProcess κ ν n a t s)
      = fun t ↦ (κ a (partitionSet n t ×ˢ s) / ν a (partitionSet n t)).toReal := rfl

lemma measurable_densityProcess_partitionFiltration_aux (κ : kernel α (γ × β)) (ν : kernel α γ)
    (n : ℕ) {s : Set β} (hs : MeasurableSet s) :
    Measurable[MeasurableSpace.prod mα (partitionFiltration γ n)] (fun (p : α × γ) ↦
      κ p.1 (partitionSet n p.2 ×ˢ s) / ν p.1 (partitionSet n p.2)) := by
  change Measurable[MeasurableSpace.prod mα (partitionFiltration γ n)]
  ...
  change @Measurable (α × γ) (countablePartition γ n)
    (MeasurableSpace.prod mα (partitionFiltration γ n)) ⊤
    ((fun c : γ ↦ ⟨partitionSet n c, partitionSet_mem n c⟩) ∘ (fun p : α × γ ↦ p.2))
  exact (measurable_partitionSet_aux n ⊤).comp measurable_snd

lemma measurable_densityProcess_aux (κ : kernel α (γ × β)) (ν : kernel α γ) (n : ℕ)
    {s : Set β} (hs : MeasurableSet s) :
    Measurable (fun (p : α × γ) ↦
      κ p.1 (partitionSet n p.2 ×ˢ s) / ν p.1 (partitionSet n p.2)) := by
  refine Measurable.mono (measurable_densityProcess_partitionFiltration_aux κ ν n hs) ?_ le_rfl
  exact sup_le_sup le_rfl (comap_mono ((partitionFiltration γ).le _))
...
lemma measurable_partitionFiltration_densityProcess (κ : kernel α (γ × β)) (ν : kernel α γ) (n : ℕ)
    (a : α) {s : Set β} (hs : MeasurableSet s) :
    Measurable[partitionFiltration γ n] (fun t ↦ densityProcess κ ν n a t s) := by
  refine @Measurable.ennreal_toReal _ (partitionFiltration γ n) _ ?_
  exact (measurable_densityProcess_partitionFiltration_aux κ ν n hs).comp measurable_prod_mk_left

lemma stronglyMeasurable_partitionFiltration_densityProcess (κ : kernel α (γ × β)) (ν : kernel α γ)
    (n : ℕ) (a : α) {s : Set β} (hs : MeasurableSet s) :
    StronglyMeasurable[partitionFiltration γ n] (fun t ↦ densityProcess κ ν n a t s) :=
  (measurable_partitionFiltration_densityProcess κ ν n a hs).stronglyMeasurable

lemma adapted_densityProcess (κ : kernel α (γ × β)) (ν : kernel α γ) (a : α)
    {s : Set β} (hs : MeasurableSet s) :
    Adapted (partitionFiltration γ) (fun n t ↦ densityProcess κ ν n a t s) :=
  fun n ↦ stronglyMeasurable_partitionFiltration_densityProcess κ ν n a hs
...
lemma apply_partitionSet_le_of_fst_le (hκν : fst κ ≤ ν) (n : ℕ) (a : α) (t : γ) (s : Set β) :
    κ a (partitionSet n t ×ˢ s) ≤ ν a (partitionSet n t) := by
  calc κ a (partitionSet n t ×ˢ s)
    ≤ fst κ a (partitionSet n t) := by
        rw [fst_apply' _ _ (measurableSet_partitionSet _ _)]
        refine measure_mono (fun x ↦ ?_)
        simp only [mem_prod, mem_setOf_eq, and_imp]
        exact fun h _ ↦ h
  _ ≤ ν a (partitionSet n t) := hκν a _

lemma densityProcess_le_one (hκν : fst κ ≤ ν) (n : ℕ) (a : α) (t : γ) (s : Set β) :
...
lemma set_integral_densityProcess_I (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
  ...
  · refine ae_of_all _ (fun t ↦ ?_)
    ...
    · suffices κ a (partitionSet n t ×ˢ s) = 0 by simp [h0, this]
      have h0' : fst κ a (partitionSet n t) = 0 :=
        le_antisymm ((hκν a _).trans h0.le) zero_le'
      rw [fst_apply' _ _ (measurableSet_partitionSet _ _)] at h0'
...
lemma integral_densityProcess (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
    (n : ℕ) (a : α) {s : Set β} (hs : MeasurableSet s) :
  ...
  · intro i j hij
    ...
    refine Or.inl (disjoint_countablePartition n i.prop j.prop ?_)
  ...
  · intro i j hij
    refine disjoint_countablePartition n i.prop j.prop ?_
    rw [ne_eq, Subtype.coe_inj]
    exact hij
...
lemma set_integral_densityProcess (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
    (n : ℕ) (a : α) {s : Set β} (hs : MeasurableSet s) {A : Set γ}
    (hA : MeasurableSet[partitionFiltration γ n] A) :
    ∫ t in A, densityProcess κ ν n a t s ∂(ν a) = (κ a (A ×ˢ s)).toReal := by
  refine induction_on_inter (m := partitionFiltration γ n) (s := countablePartition γ n)
  ...
  · rintro s hs t ht hst
    ...
    exact hst <| disjoint_countablePartition n hs ht h_ne
  · simp
  · rintro u hu
    rw [set_integral_densityProcess_I hκν _ _ hs hu]
  · intro A hA hA_eq
    have hA' : MeasurableSet A := (partitionFiltration γ).le _ _ hA
  ...
  · intro f hf_disj hf h_eq
    rw [integral_iUnion (fun i ↦ (partitionFiltration γ).le n _ (hf i)) hf_disj
      (integrable_densityProcess hκν _ _ hs).integrableOn]
    simp_rw [h_eq]
    rw [iUnion_prod_const,
      measure_iUnion _ (fun i ↦ ((partitionFiltration γ).le n _ (hf i)).prod hs)]
...
lemma set_integral_densityProcess_of_le (hκν : fst κ ≤ ν) [IsFiniteKernel κ]
    [IsFiniteKernel ν] {n m : ℕ} (hnm : n ≤ m) (a : α) {s : Set β} (hs : MeasurableSet s)
    {A : Set γ} (hA : MeasurableSet[partitionFiltration γ n] A) :
    ∫ t in A, densityProcess κ ν m a t s ∂(ν a) = (κ a (A ×ˢ s)).toReal :=
  set_integral_densityProcess hκν m a hs ((partitionFiltration γ).mono hnm A hA)

lemma condexp_densityProcess (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
    {i j : ℕ} (hij : i ≤ j) (a : α) {s : Set β} (hs : MeasurableSet s) :
    (ν a)[fun t ↦ densityProcess κ ν j a t s | partitionFiltration γ i]
  ...
  · exact StronglyMeasurable.aeStronglyMeasurable'
      (stronglyMeasurable_partitionFiltration_densityProcess κ ν i a hs)

lemma martingale_densityProcess (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
    (a : α) {s : Set β} (hs : MeasurableSet s) :
    Martingale (fun n t ↦ densityProcess κ ν n a t s) (partitionFiltration γ) (ν a) :=
  ⟨adapted_densityProcess κ ν a hs, fun _ _ h ↦ condexp_densityProcess hκν h a hs⟩

lemma densityProcess_mono_set (hκν : fst κ ≤ ν) (n : ℕ) (a : α) (t : γ)
    {s s' : Set β} (h : s ⊆ s') :
...
lemma densityProcess_mono_kernel_left {κ' : kernel α (γ × β)} (hκκ' : κ ≤ κ')
    (hκ'ν : fst κ' ≤ ν) (n : ℕ) (a : α) (t : γ) (s : Set β) :
    densityProcess κ ν n a t s ≤ densityProcess κ' ν n a t s := by
  unfold densityProcess
  by_cases h0 : ν a (partitionSet n t) = 0
  · rw [h0, ENNReal.toReal_div, ENNReal.toReal_div]
...
lemma densityProcess_antitone_kernel_right {ν' : kernel α γ}
    (hνν' : ν ≤ ν') (hκν : fst κ ≤ ν) (n : ℕ) (a : α) (t : γ) (s : Set β) :
    densityProcess κ ν' n a t s ≤ densityProcess κ ν n a t s := by
  unfold densityProcess
  have h_le : κ a (partitionSet n t ×ˢ s)
...
lemma tendsto_densityProcess_limitProcess (hκν : fst κ ≤ ν) [IsFiniteKernel κ]
    [IsFiniteKernel ν] (a : α) {s : Set β} (hs : MeasurableSet s) :
    ∀ᵐ t ∂(ν a), Tendsto (fun n ↦ densityProcess κ ν n a t s) atTop
      (𝓝 ((partitionFiltration γ).limitProcess
      (fun n t ↦ densityProcess κ ν n a t s) (ν a) t)) := by
...
lemma limitProcess_densityProcess_mem_L1 (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
    (a : α) {s : Set β} (hs : MeasurableSet s) :
    Memℒp ((partitionFiltration γ).limitProcess
      (fun n t ↦ densityProcess κ ν n a t s) (ν a)) 1 (ν a) := by
...
lemma tendsto_snorm_one_densityProcess_limitProcess (hκν : fst κ ≤ ν) [IsFiniteKernel κ]
    [IsFiniteKernel ν] (a : α) {s : Set β} (hs : MeasurableSet s) :
...
`∫ t in A, density κ ν a t s ∂(ν a) = (κ a (A ×ˢ s)).toReal` for all measurable `A : Set γ`. -/
noncomputable
def density (κ : kernel α (γ × β)) (ν : kernel α γ) (a : α) (t : γ) (s : Set β) : ℝ :=
  limsup (fun n ↦ densityProcess κ ν n a t s) atTop

lemma density_ae_eq_limitProcess (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
    (a : α) {s : Set β} (hs : MeasurableSet s) :
    (fun t ↦ density κ ν a t s)
      =ᵐ[ν a] (partitionFiltration γ).limitProcess
        (fun n t ↦ densityProcess κ ν n a t s) (ν a) := by
  filter_upwards [tendsto_densityProcess_limitProcess hκν a hs] with t ht using ht.limsup_eq

lemma tendsto_m_density (hκν : fst κ ≤ ν) (a : α) [IsFiniteKernel κ] [IsFiniteKernel ν]
    {s : Set β} (hs : MeasurableSet s) :
    ∀ᵐ t ∂(ν a),
      Tendsto (fun n ↦ densityProcess κ ν n a t s) atTop (𝓝 (density κ ν a t s)) := by
...
lemma measurable_density_left (κ : kernel α (γ × β)) (ν : kernel α γ) (t : γ)
    {s : Set β} (hs : MeasurableSet s) :
    Measurable (fun a ↦ density κ ν a t s) := by
  change Measurable ((fun (p : α × γ) ↦ density κ ν p.1 p.2 s) ∘ (fun a ↦ (a, t)))
...
lemma measurable_density_right (κ : kernel α (γ × β)) (ν : kernel α γ)
    ...
    Measurable (fun t ↦ density κ ν a t s) := by
  change Measurable ((fun (p : α × γ) ↦ density κ ν p.1 p.2 s) ∘ (fun t ↦ (a, t)))
  exact (measurable_density κ ν hs).comp measurable_prod_mk_left

lemma density_mono_set (hκν : fst κ ≤ ν) (a : α) (t : γ) {s s' : Set β} (h : s ⊆ s') :
    density κ ν a t s ≤ density κ ν a t s' := by
...
lemma density_nonneg (hκν : fst κ ≤ ν) (a : α) (t : γ) (s : Set β) :
    0 ≤ density κ ν a t s := by
  refine le_limsup_of_frequently_le ?_ ?_
  · exact frequently_of_forall (fun n ↦ densityProcess_nonneg _ _ _ _ _ _)
  · exact isBoundedUnder_of ⟨1, fun n ↦ densityProcess_le_one hκν _ _ _ _⟩

lemma density_le_one (hκν : fst κ ≤ ν) (a : α) (t : γ) (s : Set β) :
    density κ ν a t s ≤ 1 := by
...
lemma snorm_density_le (hκν : fst κ ≤ ν) (a : α) (s : Set β) :
    snorm (fun t ↦ density κ ν a t s) 1 (ν a) ≤ ν a univ := by
...
lemma integrable_density (hκν : fst κ ≤ ν) [IsFiniteKernel ν]
    (a : α) {s : Set β} (hs : MeasurableSet s) :
    Integrable (fun t ↦ density κ ν a t s) (ν a) := by
  rw [← memℒp_one_iff_integrable]
  refine ⟨Measurable.aestronglyMeasurable ?_, ?_⟩
  · exact measurable_density_right κ ν hs a
  · exact (snorm_density_le hκν a s).trans_lt (measure_lt_top _ _)

lemma tendsto_set_integral_densityProcess (hκν : fst κ ≤ ν) [IsFiniteKernel κ]
  ...
  refine tendsto_set_integral_of_L1' (μ := ν a) (fun t ↦ density κ ν a t s)
    (integrable_density hκν a hs) (F := fun i t ↦ densityProcess κ ν i a t s) (l := atTop)
...
lemma set_integral_density_of_measurableSet (hκν : fst κ ≤ ν) [IsFiniteKernel κ]
    [IsFiniteKernel ν] (n : ℕ) (a : α) {s : Set β} (hs : MeasurableSet s) {A : Set γ}
    (hA : MeasurableSet[partitionFiltration γ n] A) :
    ∫ t in A, density κ ν a t s ∂(ν a) = (κ a (A ×ˢ s)).toReal := by
  suffices ∫ t in A, density κ ν a t s ∂(ν a)
      = ∫ t in A, densityProcess κ ν n a t s ∂(ν a) by
    rw [this]
...
lemma integral_density (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
    (a : α) {s : Set β} (hs : MeasurableSet s) :
    ∫ t, density κ ν a t s ∂(ν a) = (κ a (univ ×ˢ s)).toReal := by
...
lemma set_integral_density (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
    ...
    ∫ t in A, density κ ν a t s ∂(ν a) = (κ a (A ×ˢ s)).toReal := by
  have hA' : MeasurableSet[⨆ n, partitionFiltration γ n] A := by rwa [iSup_partitionFiltration]
  refine induction_on_inter (m := ⨆ n, partitionFiltration γ n)
    (C := fun A ↦ ∫ t in A, density κ ν a t s ∂(ν a) = (κ a (A ×ˢ s)).toReal)
    (measurableSpace_iSup_eq (partitionFiltration γ)) ?_ ?_ ?_ ?_ ?_ hA'
  · rintro s ⟨n, hs⟩ t ⟨m, ht⟩ _
    exact ⟨max n m, ((partitionFiltration γ).mono (le_max_left n m) _ hs).inter
      ((partitionFiltration γ).mono (le_max_right n m) _ ht)⟩
  · simp
  · intro A ⟨n, hA⟩
    exact set_integral_density_of_measurableSet hκν n a hs hA
  · intro A hA hA_eq
    rw [iSup_partitionFiltration] at hA
  ...
  · intro f hf_disj hf h_eq
    ...
    · simp_rw [h_eq]
      ...
      · intro i j hij
        rw [Function.onFun, Set.disjoint_prod]
        exact Or.inl (hf_disj hij)
      · rw [iSup_partitionFiltration] at hf
        exact fun i ↦ (hf i).prod hs
    · rwa [iSup_partitionFiltration] at hf

lemma tendsto_integral_density_of_monotone (hκν : fst κ ≤ ν) [IsFiniteKernel κ] [IsFiniteKernel ν]
    (a : α) (s : ℕ → Set β) (hs : Monotone s) (hs_iUnion : ⋃ i, s i = univ)
    (hs_meas : ∀ n, MeasurableSet (s n)) :
    Tendsto (fun m ↦ ∫ t, density κ ν a t (s m) ∂(ν a)) atTop (𝓝 (κ a univ).toReal) := by
  simp_rw [integral_density hκν a (hs_meas _)]
...
lemma tendsto_integral_density_of_antitone (hκν : fst κ ≤ ν) [IsFiniteKernel κ]
    [IsFiniteKernel ν] (a : α) (s : ℕ → Set β) (hs : Antitone s) (hs_iInter : ⋂ i, s i = ∅)
    (hs_meas : ∀ n, MeasurableSet (s n)) :
    Tendsto (fun m ↦ ∫ t, density κ ν a t (s m) ∂(ν a)) atTop (𝓝 0) := by
  simp_rw [integral_density hκν a (hs_meas _)]
...
lemma tendsto_density_atTop_ae_of_antitone (hκν : fst κ ≤ ν) [IsFiniteKernel κ]
    [IsFiniteKernel ν] (a : α) (s : ℕ → Set β) (hs : Antitone s) (hs_iInter : ⋂ i, s i = ∅)
    (hs_meas : ∀ n, MeasurableSet (s n)) :
...
lemma densityProcess_univ [IsFiniteKernel κ] (n : ℕ) (a : α) (t : γ) :
    densityProcess κ (fst κ) n a t univ = if fst κ a (partitionSet n t) = 0 then 0 else 1 := by
  rw [densityProcess]
  by_cases h : fst κ a (partitionSet n t) = 0
  · simp [h]
  ...
  · simp only [h, ite_false]
    ...
    · simp
    · rwa [fst_apply' _ _ (measurableSet_partitionSet _ _)] at h
    · exact measure_ne_top _ _

lemma densityProcess_univ_ae (κ : kernel α (γ × β)) [IsFiniteKernel κ] (n : ℕ) (a : α) :
    ∀ᵐ t ∂(fst κ a), densityProcess κ (fst κ) n a t univ = 1 := by
  rw [ae_iff]
  have : {t | ¬ densityProcess κ (fst κ) n a t univ = 1}
      ⊆ {t | fst κ a (partitionSet n t) = 0} := by
    intro t ht
    simp only [mem_setOf_eq] at ht ⊢
    rw [densityProcess_univ] at ht
    simpa using ht
  refine measure_mono_null this ?_
  have : {t | fst κ a (partitionSet n t) = 0}
  ...
  · intro s hs t ht hst
    ...
    exact fun _ _ ↦ disjoint_countablePartition n hs ht hst
  · intro s hs
...
lemma tendsto_densityProcess_atTop_univ_of_monotone (κ : kernel α (γ × β))
    ...
    Tendsto (fun m ↦ densityProcess κ (fst κ) n a t (s m)) atTop
      (𝓝 (densityProcess κ (fst κ) n a t univ)) := by
  simp_rw [densityProcess]
  refine (ENNReal.tendsto_toReal ?_).comp ?_
  · rw [ne_eq, ENNReal.div_eq_top]
    push_neg
    simp_rw [fst_apply' _ _ (measurableSet_partitionSet _ _)]
    constructor
    ...
    · refine fun h_top ↦ eq_top_mono (measure_mono (fun x ↦ ?_)) h_top
      simp only [mem_prod, mem_setOf_eq, and_imp]
      exact fun h _ ↦ h
  by_cases h0 : fst κ a (partitionSet n t) = 0
  · rw [fst_apply' _ _ (measurableSet_partitionSet _ _)] at h0 ⊢
...
lemma tendsto_densityProcess_atTop_ae_of_monotone (κ : kernel α (γ × β)) [IsFiniteKernel κ]
    ...
    ∀ᵐ t ∂(fst κ a), Tendsto (fun m ↦ densityProcess κ (fst κ) n a t (s m)) atTop (𝓝 1) := by
...
lemma density_univ (κ : kernel α (γ × β)) [IsFiniteKernel κ] (a : α) :
    ∀ᵐ t ∂(fst κ a), density κ (fst κ) a t univ = 1 := by
