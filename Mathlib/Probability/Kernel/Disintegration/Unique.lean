...
/-- A s-finite kernel which satisfy the disintegration property of a kernel `ρ` is almost
everywhere equal to the disintegration kernel of `ρ` when evaluated on a measurable set.

This theorem in the case of finite kernels is weaker than `eq_condKernel_of_kernel_eq_compProd`
which asserts that the kernels are equal almost everywhere and not just on a given measurable
set. -/
theorem eq_condKernel_of_kernel_eq_compProd' {κ : kernel (α × β) Ω} [IsSFiniteKernel κ]
...
/-- Auxiliary lemma for `eq_condKernel_of_kernel_eq_compProd`.
Uniqueness of the disintegration kernel on ℝ. -/
lemma eq_condKernel_of_kernel_eq_compProd_real {ρ : kernel α (β × ℝ)} [IsFiniteKernel ρ]
  ...
  have huniv : ∀ᵐ x ∂(kernel.fst ρ a), κ (a, x) Set.univ = kernel.condKernel ρ (a, x) Set.univ :=
    eq_condKernel_of_kernel_eq_compProd' hκ MeasurableSet.univ a
  ...
  · simp only [iUnion_singleton_eq_range, mem_range, forall_exists_index, forall_apply_eq_imp_iff]
    exact ae_all_iff.2 fun q => eq_condKernel_of_kernel_eq_compProd' hκ measurableSet_Iic a
...
theorem eq_condKernel_of_kernel_eq_compProd (κ : kernel (α × β) Ω) [IsFiniteKernel κ]
    ...
    ∀ᵐ x ∂(kernel.fst ρ a), κ (a, x) = kernel.condKernel ρ (a, x) := by
-- The idea is to transport the question to `ℝ` from `Ω` using `embeddingReal Ω`
-- and then construct a measure on `α × ℝ`
  ...
  have hρ'' : ∀ᵐ x ∂(kernel.fst ρ a),
    ...
    rw [← hρ']
    refine eq_condKernel_of_kernel_eq_compProd_real ?_ a
    ext a s hs
  ...
  suffices ρ' = (kernel.fst ρ ⊗ₖ (kernel.map (kernel.condKernel ρ) f hf.measurable)) by
    rw [← hρ'] at this
    have heq := eq_condKernel_of_kernel_eq_compProd_real this a
    rw [hρ'] at heq
...
lemma kernel.condKernel_apply_eq_condKernel [CountableOrCountablyGenerated α β]
    ...
    (fun b ↦ kernel.condKernel κ (a, b)) =ᵐ[kernel.fst κ a] (κ a).condKernel := by
  have : κ a = (κ a).fst
