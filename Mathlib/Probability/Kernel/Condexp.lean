...
theorem _root_.MeasureTheory.Integrable.comp_snd_map_prod_id [AddCommGroup F] [NormedAddCommGroup F]
...
variable [AddCommGroup F] [NormedAddCommGroup F] {f : Ω → F}
...
theorem condExp_ae_eq_integral_condExpKernel' [AddCommGroup F] [NormedAddCommGroup F] {f : Ω → F}
...
theorem condExp_ae_eq_trim_integral_condExpKernel_of_stronglyMeasurable [AddCommGroup F]
...
theorem condExp_ae_eq_trim_integral_condExpKernel [AddCommGroup F] [NormedAddCommGroup F]
    {f : Ω → F} [NormedSpace ℝ F] [CompleteSpace F] (hm : m ≤ mΩ) (hf_int : Integrable f μ) :
    μ[f | m] =ᵐ[μ.trim hm] fun ω ↦ ∫ y, f y ∂condExpKernel μ m ω := by
...
variable {s t : Set Ω} [AddCommGroup F] [NormedAddCommGroup F] [NormedSpace ℝ F] [CompleteSpace F]
