...
theorem _root_.MeasureTheory.Integrable.comp_snd_map_prod_id [NormMetric F] [AddCommGroup F] [IsNormedAddGroup F]
...
variable [NormMetric F] [AddCommGroup F] [IsNormedAddGroup F] {f : Ω → F}
...
theorem condExp_ae_eq_integral_condExpKernel' [NormMetric F] [AddCommGroup F] [IsNormedAddGroup F] {f : Ω → F}
...
theorem condExp_ae_eq_trim_integral_condExpKernel_of_stronglyMeasurable
    [NormMetric F] [AddCommGroup F] [IsNormedAddGroup F] {f : Ω → F} [NormedSpace ℝ F] [CompleteSpace F]
...
theorem condExp_ae_eq_trim_integral_condExpKernel [NormMetric F] [AddCommGroup F] [IsNormedAddGroup F] {f : Ω → F}
    [NormedSpace ℝ F] [CompleteSpace F] (hm : m ≤ mΩ) (hf_int : Integrable f μ) :
    μ[f | m] =ᵐ[μ.trim hm] fun ω ↦ ∫ y, f y ∂condExpKernel μ m ω := by
...
variable {s t : Set Ω} [NormMetric F] [AddCommGroup F] [IsNormedAddGroup F] [NormedSpace ℝ F] [CompleteSpace F]
