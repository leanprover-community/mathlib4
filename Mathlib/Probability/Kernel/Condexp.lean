/-
Copyright (c) 2023 Rémy Degenne. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Rémy Degenne
-/
module

public import Mathlib.MeasureTheory.Constructions.Pi
public import Mathlib.Probability.ConditionalProbability
public import Mathlib.Probability.Kernel.CompProdEqIff
public import Mathlib.Probability.Kernel.Composition.Lemmas
public import Mathlib.Probability.Kernel.Composition.MeasureComp
public import Mathlib.Probability.Kernel.CondDistrib

/-!
# Kernel associated with a conditional expectation

We define `condExpKernel μ m`, a kernel from `Ω` to `Ω` such that for all integrable functions `f`,
`μ[f | m] =ᵐ[μ] fun ω => ∫ y, f y ∂(condExpKernel μ m ω)`.

This kernel is defined if `Ω` is a standard Borel space. In general, `μ⟦s | m⟧` maps a measurable
set `s` to a function `Ω → ℝ≥0∞`, and for all `s` that map is unique up to a `μ`-null set. For all
`a`, the map from sets to `ℝ≥0∞` that we obtain that way verifies some of the properties of a
measure, but the fact that the `μ`-null set depends on `s` can prevent us from finding versions of
the conditional expectation that combine into a true measure. The standard Borel space assumption
on `Ω` allows us to do so.

## Main definitions

* `condExpKernel μ m`: kernel such that `μ[f | m] =ᵐ[μ] fun ω => ∫ y, f y ∂(condExpKernel μ m ω)`.

## Main statements

* `condExp_ae_eq_integral_condExpKernel`: `μ[f | m] =ᵐ[μ] fun ω => ∫ y, f y ∂(condExpKernel μ m ω)`.

-/

@[expose] public section


open MeasureTheory Set Filter TopologicalSpace

open scoped ENNReal MeasureTheory ProbabilityTheory

namespace MeasurableSpace

/-- Restrict a finite vector to the prefix subtype used by `piPrefix`. -/
def piPrefixRestrict (Ω : Type*) {n : ℕ} (k : Fin (n + 1))
    (s : Fin n → Ω) : ({i : Fin n // (i : ℕ) < (k : ℕ)} → Ω) :=
  fun i ↦ s i.1

/-- The sub-σ-algebra of `MeasurableSpace.pi` on `Fin n → Ω` generated by the first `k`
coordinate projections. -/
@[reducible]
def piPrefix (Ω : Type*) [MeasurableSpace Ω] {n : ℕ} (k : Fin (n + 1)) :
    MeasurableSpace (Fin n → Ω) :=
  MeasurableSpace.comap (piPrefixRestrict Ω k)
    (inferInstance : MeasurableSpace ({i : Fin n // (i : ℕ) < (k : ℕ)} → Ω))

lemma piPrefix_le_pi (Ω : Type*) [MeasurableSpace Ω] {n : ℕ} (k : Fin (n + 1)) :
    piPrefix Ω k ≤ (inferInstance : MeasurableSpace (Fin n → Ω)) := by
  exact (measurable_pi_iff.mpr fun i ↦ measurable_pi_apply i.1).comap_le

lemma measurable_piPrefix_eval (Ω : Type*) [MeasurableSpace Ω] {n : ℕ}
    (k : Fin (n + 1)) (i : Fin n) (hi : (i : ℕ) < (k : ℕ)) :
    @Measurable (Fin n → Ω) Ω (piPrefix Ω k) _ (fun s ↦ s i) := by
  have h_eval :
      Measurable fun f : ({i : Fin n // (i : ℕ) < (k : ℕ)} → Ω) ↦
        f (⟨i, hi⟩ : {i : Fin n // (i : ℕ) < (k : ℕ)}) :=
    measurable_pi_apply (X := fun _ : {i : Fin n // (i : ℕ) < (k : ℕ)} ↦ Ω)
      (⟨i, hi⟩ : {i : Fin n // (i : ℕ) < (k : ℕ)})
  exact h_eval.comp (Measurable.of_comap_le le_rfl)

lemma measurable_piPrefixRestrict (Ω : Type*) [MeasurableSpace Ω] {n : ℕ}
    (k : Fin (n + 1)) :
    @Measurable (Fin n → Ω) ({i : Fin n // (i : ℕ) < (k : ℕ)} → Ω)
      (piPrefix Ω k) _ (piPrefixRestrict Ω k) := by
  exact Measurable.of_comap_le le_rfl

lemma piPrefix_eq_comap_piPrefixRestrict (Ω : Type*) [MeasurableSpace Ω] {n : ℕ}
    (k : Fin (n + 1)) :
    piPrefix Ω k =
      MeasurableSpace.comap (piPrefixRestrict Ω k)
        (inferInstance : MeasurableSpace ({i : Fin n // (i : ℕ) < (k : ℕ)} → Ω)) := by
  rfl

end MeasurableSpace

namespace ProbabilityTheory

section AuxLemmas

variable {Ω F : Type*} {m mΩ : MeasurableSpace Ω} {μ : Measure Ω} {f : Ω → F}

theorem _root_.MeasureTheory.AEStronglyMeasurable.comp_snd_map_prod_id [TopologicalSpace F]
    (hm : m ≤ mΩ) (hf : AEStronglyMeasurable f μ) :
    AEStronglyMeasurable[m.prod mΩ] (fun x : Ω × Ω => f x.2)
      (@Measure.map Ω (Ω × Ω) mΩ (m.prod mΩ) (fun ω => (id ω, id ω)) μ) := by
  simpa using (aestronglyMeasurable_comp_snd_map_prodMk_iff hm).mpr hf

theorem _root_.MeasureTheory.Integrable.comp_snd_map_prod_id [NormedAddCommGroup F]
    (hf : Integrable f μ) : Integrable (fun x : Ω × Ω => f x.2)
      (@Measure.map Ω (Ω × Ω) mΩ (m.prod mΩ) (fun ω => (id ω, id ω)) μ) := by
  simpa using Integrable.comp_snd_map_prodMk id hf

end AuxLemmas

variable {Ω F : Type*} {m : MeasurableSpace Ω} [mΩ : MeasurableSpace Ω]
  [StandardBorelSpace Ω] {μ : Measure Ω} [IsFiniteMeasure μ]

open Classical in
/-- Kernel associated with the conditional expectation with respect to a σ-algebra. It satisfies
`μ[f | m] =ᵐ[μ] fun ω => ∫ y, f y ∂(condExpKernel μ m ω)`.
It is defined as the conditional distribution of the identity given the identity, where the second
identity is understood as a map from `Ω` with the σ-algebra `mΩ` to `Ω` with σ-algebra `m ⊓ mΩ`.
We use `m ⊓ mΩ` instead of `m` to ensure that it is a sub-σ-algebra of `mΩ`. We then use
`Kernel.comap` to get a kernel from `m` to `mΩ` instead of from `m ⊓ mΩ` to `mΩ`. -/
noncomputable irreducible_def condExpKernel (μ : Measure Ω) [IsFiniteMeasure μ]
    (m : MeasurableSpace Ω) : @Kernel Ω Ω m mΩ :=
  if _h : Nonempty Ω then
    Kernel.comap (@condDistrib Ω Ω Ω mΩ _ _ mΩ (m ⊓ mΩ) id id μ _) id
      (measurable_id'' (inf_le_left : m ⊓ mΩ ≤ m))
  else 0

lemma condExpKernel_eq (μ : Measure Ω) [IsFiniteMeasure μ] [h : Nonempty Ω]
    (m : MeasurableSpace Ω) :
    condExpKernel (mΩ := mΩ) μ m = Kernel.comap (@condDistrib Ω Ω Ω mΩ _ _ mΩ (m ⊓ mΩ) id id μ _) id
      (measurable_id'' (inf_le_left : m ⊓ mΩ ≤ m)) := by
  simp [condExpKernel, h]

lemma condExpKernel_apply_eq_condDistrib [Nonempty Ω] {ω : Ω} :
    condExpKernel μ m ω = @condDistrib Ω Ω Ω mΩ _ _ mΩ (m ⊓ mΩ) id id μ _ (id ω) := by
  simp [condExpKernel_eq, Kernel.comap_apply]

instance : IsMarkovKernel (condExpKernel μ m) := by
  rcases isEmpty_or_nonempty Ω with h | h
  · exact ⟨fun a ↦ (IsEmpty.false a).elim⟩
  · simpa [condExpKernel, h] using by infer_instance

lemma compProd_trim_condExpKernel (hm : m ≤ mΩ) :
    (μ.trim hm) ⊗ₘ condExpKernel μ m
      = @Measure.map Ω (Ω × Ω) mΩ (m.prod mΩ) (fun ω ↦ (id ω, id ω)) μ := by
  rcases isEmpty_or_nonempty Ω with h | h
  · simp [Measure.eq_zero_of_isEmpty μ]
  rw [condExpKernel_eq]
  have : m ⊓ mΩ = m := inf_of_le_left hm
  have h := compProd_map_condDistrib (mβ := m) (μ := μ) (X := id) measurable_id.aemeasurable
  rw [← h, trim_eq_map hm]
  congr 1
  ext a s hs
  simp only [Kernel.coe_comap, Function.comp_apply, id_eq]
  congr

lemma condExpKernel_comp_trim (hm : m ≤ mΩ) : condExpKernel μ m ∘ₘ μ.trim hm = μ := by
  rw [← Measure.snd_compProd, compProd_trim_condExpKernel, @Measure.snd_map_prodMk, Measure.map_id]
  exact measurable_id'' hm

/-- If an observable is measurable with respect to the conditioning σ-algebra, then the
conditional-expectation kernel is almost surely supported on points with the same value of that
observable. -/
theorem condExpKernel_ae_eq_of_measurable {β : Type*} [MeasurableSpace β] [MeasurableEq β]
    (hm : m ≤ mΩ) {f : Ω → β} (hf : Measurable[m] f) :
    ∀ᵐ x ∂(μ.trim hm), ∀ᵐ y ∂(condExpKernel μ m x), f y = f x := by
  have hf_full : Measurable f := hf.mono hm le_rfl
  letI : MeasurableSpace (Ω × Ω) := m.prod mΩ
  have hmeas : MeasurableSet {p : Ω × Ω | f p.2 = f p.1} := by
    have hfst : Measurable fun p : Ω × Ω ↦ f p.1 := hf.comp measurable_fst
    have hsnd : Measurable fun p : Ω × Ω ↦ f p.2 := hf_full.comp measurable_snd
    simpa using (hsnd.eq hfst) (measurableSet_singleton True)
  have hdiag : ∀ᵐ p ∂Measure.map (fun x : Ω ↦ (x, x)) μ, f p.2 = f p.1 := by
    have hmap : Measurable (fun x : Ω ↦ (x, x)) :=
      Measurable.prodMk (measurable_id'' hm) measurable_id
    rw [ae_map_iff hmap.aemeasurable hmeas]
    exact Filter.Eventually.of_forall fun _ ↦ rfl
  have hjoint : ∀ᵐ p ∂((μ.trim hm) ⊗ₘ condExpKernel μ m), f p.2 = f p.1 := by
    rw [compProd_trim_condExpKernel hm]
    exact hdiag
  simpa using Measure.ae_ae_of_ae_compProd hjoint

section PiPrefix

variable {n : ℕ} {Ω' : Type*} [MeasurableSpace Ω']

namespace Kernel

/-- The explicit finite-product kernel associated with conditioning on the prefix coordinates:
on an actual prefix value, sample the complementary coordinates from the corresponding product
measure, then reassemble the full vector. The product split is expressed using mathlib's native
subtype form of `MeasurableEquiv.piEquivPiSubtypeProd`. -/
noncomputable def piPrefixTailFromPrefix (Ω' : Type*) [MeasurableSpace Ω'] {n : ℕ}
    (μ : Fin n → Measure Ω') (k : Fin (n + 1)) :
    Kernel ({i : Fin n // (i : ℕ) < (k : ℕ)} → Ω') (Fin n → Ω') :=
  let p : Fin n → Prop := fun i ↦ (i : ℕ) < (k : ℕ)
  let e := MeasurableEquiv.piEquivPiSubtypeProd (fun _ : Fin n ↦ Ω') p
  (Kernel.id ×ₖ
    Kernel.const ({i : Fin n // p i} → Ω') (Measure.pi fun i : {i // ¬p i} ↦ μ i.1)).map
      e.symm

/-- The explicit finite-product kernel associated with conditioning on the prefix coordinates:
keep the prefix coordinates at their conditioning values and sample the complementary coordinates
from the corresponding product measure. -/
noncomputable def piPrefixTail (Ω' : Type*) [MeasurableSpace Ω'] {n : ℕ}
    (μ : Fin n → Measure Ω') (k : Fin (n + 1)) :
    @Kernel (Fin n → Ω') (Fin n → Ω')
      (MeasurableSpace.piPrefix Ω' k) MeasurableSpace.pi :=
  (piPrefixTailFromPrefix Ω' μ k).comap (MeasurableSpace.piPrefixRestrict Ω' k)
    (MeasurableSpace.measurable_piPrefixRestrict Ω' k)

end Kernel

private lemma map_compProd_trim_comap {α β γ : Type*} {mα : MeasurableSpace α}
    {mβ : MeasurableSpace β} {mγ : MeasurableSpace γ} {μ : Measure α}
    [SFinite μ] {κ : Kernel β γ} [IsSFiniteKernel κ] {f : α → β} (hf : Measurable f) :
    @Measure.map (α × γ) (β × γ) ((mβ.comap f).prod mγ) (mβ.prod mγ)
        (Prod.map f id)
        ((μ.trim hf.comap_le) ⊗ₘ
          (κ.comap f (Measurable.of_comap_le le_rfl))) =
      (μ.map f) ⊗ₘ κ := by
  let hf_comap : @Measurable α β (mβ.comap f) mβ f := Measurable.of_comap_le le_rfl
  have hmap : @Measurable (α × γ) (β × γ) ((mβ.comap f).prod mγ) (mβ.prod mγ)
      (Prod.map f id) := hf_comap.prodMap measurable_id
  haveI : SFinite (μ.trim hf.comap_le) := by
    rw [trim_eq_map hf.comap_le]
    infer_instance
  ext s hs
  rw [Measure.map_apply hmap hs]
  rw [Measure.compProd_apply (hmap hs), Measure.compProd_apply hs]
  simp only [Kernel.comap_apply]
  have hmeas : Measurable[mβ.comap f]
      (fun a : α ↦ κ (f a) (Prod.mk (f a) ⁻¹' s)) := by
    exact (Kernel.measurable_kernel_prodMk_left hs).comp hf_comap
  change (∫⁻ a : α, κ (f a) (Prod.mk (f a) ⁻¹' s) ∂μ.trim hf.comap_le) =
    ∫⁻ a : β, κ a (Prod.mk a ⁻¹' s) ∂μ.map f
  rw [lintegral_trim hf.comap_le hmeas]
  rw [lintegral_map (Kernel.measurable_kernel_prodMk_left hs) hf]

private lemma compProd_id_prod_const_map {α β γ : Type*} [MeasurableSpace α]
    [MeasurableSpace β] [MeasurableSpace γ] {μ : Measure α} [SFinite μ]
    {ν : Measure β} [SFinite ν] {f : α × β → γ} (hf : Measurable f) :
    μ ⊗ₘ ((Kernel.id ×ₖ Kernel.const α ν).map f) =
      (μ.prod ν).map (fun x ↦ (x.1, f x)) := by
  ext s hs
  rw [Measure.compProd_apply hs]
  rw [Measure.map_apply (measurable_fst.prodMk hf) hs]
  rw [Measure.prod_apply ((measurable_fst.prodMk hf) hs)]
  congr with a
  rw [Kernel.map_apply' _ hf _ (measurable_prodMk_left hs)]
  rw [Kernel.id, Kernel.deterministic_prod_apply' measurable_id (Kernel.const α ν) a
    (hf (measurable_prodMk_left hs))]
  rfl

private lemma measure_eq_of_map_eq_of_comap {α β : Type*} [mα : MeasurableSpace α]
    [mβ : MeasurableSpace β] {μ ν : Measure α} {f : α → β}
    (hm : mα = mβ.comap f)
    (h : @Measure.map α β mα mβ f μ = @Measure.map α β mα mβ f ν) :
    μ = ν := by
  have hf : @Measurable α β mα mβ f := by
    rw [hm]
    exact Measurable.of_comap_le le_rfl
  refine Measure.ext fun s hs ↦ ?_
  rw [hm] at hs
  rcases (MeasurableSpace.measurableSet_comap (f := f) (m := mβ)).mp hs with
    ⟨t, ht, rfl⟩
  have ht_eq := congrArg (fun ρ : Measure β ↦ ρ t) h
  simpa [Measure.map_apply hf ht] using ht_eq

private lemma compProd_piPrefixTailFromPrefix
    {μ : Fin n → Measure Ω'} [∀ i, IsProbabilityMeasure (μ i)] (k : Fin (n + 1)) :
    (Measure.pi μ).map (MeasurableSpace.piPrefixRestrict Ω' k) ⊗ₘ
        Kernel.piPrefixTailFromPrefix Ω' μ k =
      (Measure.pi μ).map (fun S ↦ (MeasurableSpace.piPrefixRestrict Ω' k S, S)) := by
  let p : Fin n → Prop := fun i ↦ (i : ℕ) < (k : ℕ)
  let P := {i : Fin n // p i} → Ω'
  let Q := {i : Fin n // ¬p i} → Ω'
  let e := MeasurableEquiv.piEquivPiSubtypeProd (fun _ : Fin n ↦ Ω') p
  let μP : Measure P := Measure.pi fun i : {i : Fin n // p i} ↦ μ i.1
  let μQ : Measure Q := Measure.pi fun i : {i : Fin n // ¬p i} ↦ μ i.1
  have hsplit : Measure.map e (Measure.pi μ) = μP.prod μQ :=
    (MeasureTheory.measurePreserving_piEquivPiSubtypeProd (μ := μ)
      (α := fun _ : Fin n ↦ Ω') p).map_eq
  have hprefix : (Measure.pi μ).map (MeasurableSpace.piPrefixRestrict Ω' k) = μP := by
    calc (Measure.pi μ).map (MeasurableSpace.piPrefixRestrict Ω' k)
      _ = Measure.map Prod.fst (Measure.map e (Measure.pi μ)) := by
        rw [Measure.map_map measurable_fst e.measurable]
        rfl
      _ = Measure.map Prod.fst (μP.prod μQ) := by rw [hsplit]
      _ = μP := by simp [μQ]
  rw [Kernel.piPrefixTailFromPrefix]
  rw [hprefix]
  rw [compProd_id_prod_const_map e.symm.measurable]
  calc (μP.prod μQ).map (fun x ↦ (x.1, e.symm x))
    _ = Measure.map (fun S : Fin n → Ω' ↦ (MeasurableSpace.piPrefixRestrict Ω' k S, S))
        (Measure.pi μ) := by
      rw [← hsplit]
      rw [Measure.map_map (measurable_fst.prodMk e.symm.measurable) e.measurable]
      refine Measure.map_congr (Filter.Eventually.of_forall fun S ↦ ?_)
      ext i <;> simp [e, MeasurableSpace.piPrefixRestrict]

private lemma compProd_trim_piPrefixTail
    {μ : Fin n → Measure Ω'} [∀ i, IsProbabilityMeasure (μ i)] (k : Fin (n + 1)) :
    (Measure.pi μ).trim (MeasurableSpace.piPrefix_le_pi Ω' k) ⊗ₘ
        Kernel.piPrefixTail Ω' μ k =
      @Measure.map (Fin n → Ω') ((Fin n → Ω') × (Fin n → Ω')) MeasurableSpace.pi
        ((MeasurableSpace.piPrefix Ω' k).prod MeasurableSpace.pi)
        (fun S : Fin n → Ω' ↦ (S, S)) (Measure.pi μ) := by
  let Φ : (Fin n → Ω') × (Fin n → Ω') →
      ({i : Fin n // (i : ℕ) < (k : ℕ)} → Ω') × (Fin n → Ω') :=
    Prod.map (MeasurableSpace.piPrefixRestrict Ω' k) id
  have hf_full : Measurable (MeasurableSpace.piPrefixRestrict Ω' k) :=
    (MeasurableSpace.measurable_piPrefixRestrict Ω' k).mono
      (MeasurableSpace.piPrefix_le_pi Ω' k) le_rfl
  have hspace : ((MeasurableSpace.piPrefix Ω' k).prod MeasurableSpace.pi :
      MeasurableSpace ((Fin n → Ω') × (Fin n → Ω'))) =
      (((inferInstance : MeasurableSpace ({i : Fin n // (i : ℕ) < (k : ℕ)} → Ω')).prod
        (inferInstance : MeasurableSpace (Fin n → Ω'))).comap Φ) := by
    dsimp [Φ]
    calc ((MeasurableSpace.piPrefix Ω' k).prod MeasurableSpace.pi :
        MeasurableSpace ((Fin n → Ω') × (Fin n → Ω')))
      _ = (MeasurableSpace.comap (MeasurableSpace.piPrefixRestrict Ω' k)
            (inferInstance : MeasurableSpace ({i : Fin n // (i : ℕ) < (k : ℕ)} → Ω'))).prod
          (MeasurableSpace.comap id (inferInstance : MeasurableSpace (Fin n → Ω'))) := by
            rw [MeasurableSpace.piPrefix_eq_comap_piPrefixRestrict, MeasurableSpace.comap_id]
      _ = (((inferInstance : MeasurableSpace ({i : Fin n // (i : ℕ) < (k : ℕ)} → Ω')).prod
            (inferInstance : MeasurableSpace (Fin n → Ω'))).comap
          (Prod.map (MeasurableSpace.piPrefixRestrict Ω' k) id)) := by
            rw [← MeasurableSpace.comap_prodMap]
  haveI : IsSFiniteKernel (Kernel.piPrefixTailFromPrefix Ω' μ k) := by
    rw [Kernel.piPrefixTailFromPrefix]
    infer_instance
  refine @measure_eq_of_map_eq_of_comap ((Fin n → Ω') × (Fin n → Ω'))
    ((({i : Fin n // (i : ℕ) < (k : ℕ)} → Ω') × (Fin n → Ω')))
    ((MeasurableSpace.piPrefix Ω' k).prod MeasurableSpace.pi) _ _ _ Φ hspace ?_
  have hmap_left : @Measure.map ((Fin n → Ω') × (Fin n → Ω'))
      ((({i : Fin n // (i : ℕ) < (k : ℕ)} → Ω') × (Fin n → Ω')))
      ((MeasurableSpace.piPrefix Ω' k).prod MeasurableSpace.pi) _ Φ
      ((Measure.pi μ).trim (MeasurableSpace.piPrefix_le_pi Ω' k) ⊗ₘ
        Kernel.piPrefixTail Ω' μ k) =
      (Measure.pi μ).map (MeasurableSpace.piPrefixRestrict Ω' k) ⊗ₘ
        Kernel.piPrefixTailFromPrefix Ω' μ k := by
    rw [Kernel.piPrefixTail]
    dsimp [Φ]
    convert map_compProd_trim_comap (μ := Measure.pi μ)
      (κ := Kernel.piPrefixTailFromPrefix Ω' μ k) hf_full using 1
  have hmap_right : @Measure.map ((Fin n → Ω') × (Fin n → Ω'))
      ((({i : Fin n // (i : ℕ) < (k : ℕ)} → Ω') × (Fin n → Ω')))
      ((MeasurableSpace.piPrefix Ω' k).prod MeasurableSpace.pi) _ Φ
      (@Measure.map (Fin n → Ω') ((Fin n → Ω') × (Fin n → Ω')) MeasurableSpace.pi
        ((MeasurableSpace.piPrefix Ω' k).prod MeasurableSpace.pi)
        (fun S : Fin n → Ω' ↦ (S, S)) (Measure.pi μ)) =
      (Measure.pi μ).map (fun S ↦ (MeasurableSpace.piPrefixRestrict Ω' k S, S)) := by
    have hdiag : @Measurable (Fin n → Ω') ((Fin n → Ω') × (Fin n → Ω'))
        MeasurableSpace.pi ((MeasurableSpace.piPrefix Ω' k).prod MeasurableSpace.pi)
        (fun S : Fin n → Ω' ↦ (S, S)) :=
      Measurable.prodMk (measurable_id'' (MeasurableSpace.piPrefix_le_pi Ω' k))
        measurable_id
    have hΦ : @Measurable ((Fin n → Ω') × (Fin n → Ω'))
        ((({i : Fin n // (i : ℕ) < (k : ℕ)} → Ω') × (Fin n → Ω')))
        ((MeasurableSpace.piPrefix Ω' k).prod MeasurableSpace.pi) _ Φ := by
      dsimp [Φ]
      exact (MeasurableSpace.measurable_piPrefixRestrict Ω' k).prodMap measurable_id
    rw [Measure.map_map hΦ hdiag]
    refine Measure.map_congr (Filter.Eventually.of_forall fun S ↦ ?_)
    rfl
  rw [hmap_left, hmap_right]
  exact compProd_piPrefixTailFromPrefix k

/-- For a finite product of probability measures, the conditional-expectation kernel with respect
to the prefix-coordinate σ-algebra is the kernel that keeps the prefix coordinates fixed and samples
the complementary coordinates independently from their product measure. -/
theorem condExpKernel_piPrefix_eq_dirac_prod_pi [MeasurableEq Ω']
    [StandardBorelSpace (Fin n → Ω')] {μ : Fin n → Measure Ω'}
    [∀ i, IsProbabilityMeasure (μ i)] (k : Fin (n + 1)) :
    condExpKernel (Measure.pi μ) (MeasurableSpace.piPrefix Ω' k)
      =ᵐ[(Measure.pi μ).trim (MeasurableSpace.piPrefix_le_pi Ω' k)]
        Kernel.piPrefixTail Ω' μ k := by
  haveI : IsFiniteKernel (Kernel.piPrefixTail Ω' μ k) := by
    rw [Kernel.piPrefixTail, Kernel.piPrefixTailFromPrefix]
    infer_instance
  refine Kernel.ae_eq_of_compProd_eq ?_
  rw [compProd_trim_condExpKernel (MeasurableSpace.piPrefix_le_pi Ω' k),
    compProd_trim_piPrefixTail k]
  refine Measure.map_congr (Filter.Eventually.of_forall fun S ↦ ?_)
  rfl

/-- For a finite product measure, the conditional-expectation kernel with respect to the
prefix-coordinate σ-algebra is almost surely supported on samples that agree with the conditioning
sample on all revealed prefix coordinates. -/
theorem condExpKernel_piPrefix_ae_eq [MeasurableEq Ω'] [StandardBorelSpace (Fin n → Ω')]
    {μ : Fin n → Measure Ω'} [∀ i, IsFiniteMeasure (μ i)] (k : Fin (n + 1)) :
    ∀ᵐ S ∂((Measure.pi μ).trim (MeasurableSpace.piPrefix_le_pi Ω' k)),
      ∀ᵐ T ∂(condExpKernel (Measure.pi μ) (MeasurableSpace.piPrefix Ω' k) S),
        ∀ i : Fin n, (i : ℕ) < (k : ℕ) → T i = S i := by
  letI : MeasurableSpace ((Fin n → Ω') × (Fin n → Ω')) :=
    (MeasurableSpace.piPrefix Ω' k).prod MeasurableSpace.pi
  have hmeas : MeasurableSet
      {p : (Fin n → Ω') × (Fin n → Ω') |
        ∀ i : Fin n, (i : ℕ) < (k : ℕ) → p.2 i = p.1 i} := by
    rw [Set.setOf_forall]
    refine MeasurableSet.iInter fun i : Fin n ↦ ?_
    by_cases hi : (i : ℕ) < (k : ℕ)
    · have hfst : Measurable fun p : (Fin n → Ω') × (Fin n → Ω') ↦ p.1 i :=
        (MeasurableSpace.measurable_piPrefix_eval Ω' k i hi).comp measurable_fst
      have hsnd : Measurable fun p : (Fin n → Ω') × (Fin n → Ω') ↦ p.2 i :=
        (measurable_pi_apply i).comp measurable_snd
      simpa [hi] using hsnd.eq hfst
    · simp [hi]
  have hdiag : ∀ᵐ p ∂Measure.map (fun S : Fin n → Ω' ↦ (S, S)) (Measure.pi μ),
      ∀ i : Fin n, (i : ℕ) < (k : ℕ) → p.2 i = p.1 i := by
    have hmap : Measurable (fun S : Fin n → Ω' ↦ (S, S)) :=
      Measurable.prodMk (measurable_id'' (MeasurableSpace.piPrefix_le_pi Ω' k)) measurable_id
    rw [ae_map_iff hmap.aemeasurable hmeas]
    exact Filter.Eventually.of_forall fun _ _ _ ↦ rfl
  have hjoint : ∀ᵐ p ∂((Measure.pi μ).trim (MeasurableSpace.piPrefix_le_pi Ω' k) ⊗ₘ
      condExpKernel (Measure.pi μ) (MeasurableSpace.piPrefix Ω' k)),
      ∀ i : Fin n, (i : ℕ) < (k : ℕ) → p.2 i = p.1 i := by
    rw [compProd_trim_condExpKernel (MeasurableSpace.piPrefix_le_pi Ω' k)]
    exact hdiag
  simpa using Measure.ae_ae_of_ae_compProd hjoint

end PiPrefix

section Measurability

variable [NormedAddCommGroup F] {f : Ω → F}

theorem measurable_condExpKernel {s : Set Ω} (hs : MeasurableSet s) :
    Measurable[m] fun ω => condExpKernel μ m ω s :=
  (condExpKernel μ m).measurable_coe hs

theorem stronglyMeasurable_condExpKernel {s : Set Ω} (hs : MeasurableSet s) :
    StronglyMeasurable[m] fun ω => condExpKernel μ m ω s :=
  Measurable.stronglyMeasurable (measurable_condExpKernel hs)

theorem _root_.MeasureTheory.StronglyMeasurable.integral_condExpKernel' [NormedSpace ℝ F]
    (hf : StronglyMeasurable f) :
    StronglyMeasurable[m ⊓ mΩ] (fun ω ↦ ∫ y, f y ∂condExpKernel μ m ω) := by
  nontriviality Ω
  simp_rw [condExpKernel_apply_eq_condDistrib]
  exact (hf.comp_measurable measurable_snd).integral_condDistrib

theorem _root_.MeasureTheory.StronglyMeasurable.integral_condExpKernel [NormedSpace ℝ F]
    (hf : StronglyMeasurable f) :
    StronglyMeasurable[m] (fun ω ↦ ∫ y, f y ∂condExpKernel μ m ω) :=
  hf.integral_condExpKernel'.mono inf_le_left

theorem _root_.MeasureTheory.AEStronglyMeasurable.integral_condExpKernel [NormedSpace ℝ F]
    (hf : AEStronglyMeasurable f μ) :
    AEStronglyMeasurable (fun ω => ∫ y, f y ∂condExpKernel μ m ω) μ := by
  nontriviality Ω
  simp_rw [condExpKernel_apply_eq_condDistrib]
  exact AEStronglyMeasurable.integral_condDistrib
    (aemeasurable_id'' μ (inf_le_right : m ⊓ mΩ ≤ mΩ)) aemeasurable_id
    (hf.comp_snd_map_prod_id inf_le_right)

theorem aestronglyMeasurable_integral_condExpKernel [NormedSpace ℝ F]
    (hf : AEStronglyMeasurable f μ) :
    AEStronglyMeasurable[m] (fun ω => ∫ y, f y ∂condExpKernel μ m ω) μ := by
  nontriviality Ω
  rw [condExpKernel_eq]
  have h := aestronglyMeasurable_integral_condDistrib
    (aemeasurable_id'' μ (inf_le_right : m ⊓ mΩ ≤ mΩ)) aemeasurable_id
    (hf.comp_snd_map_prod_id (inf_le_right : m ⊓ mΩ ≤ mΩ))
  rw [MeasurableSpace.comap_id] at h
  exact h.mono inf_le_left

lemma aestronglyMeasurable_trim_condExpKernel (hm : m ≤ mΩ) (hf : AEStronglyMeasurable f μ) :
    ∀ᵐ ω ∂(μ.trim hm), f =ᵐ[condExpKernel μ m ω] hf.mk f := by
  refine Measure.ae_ae_of_ae_comp ?_
  rw [condExpKernel_comp_trim hm]
  exact hf.ae_eq_mk

end Measurability

section Integrability

variable [NormedAddCommGroup F] {f : Ω → F}

theorem _root_.MeasureTheory.Integrable.condExpKernel_ae (hf_int : Integrable f μ) :
    ∀ᵐ ω ∂μ, Integrable f (condExpKernel μ m ω) := by
  nontriviality Ω
  rw [condExpKernel_eq]
  convert!
    Integrable.condDistrib_ae (aemeasurable_id'' μ (inf_le_right : m ⊓ mΩ ≤ mΩ)) aemeasurable_id
      hf_int.comp_snd_map_prod_id using 1

theorem _root_.MeasureTheory.Integrable.integral_norm_condExpKernel (hf_int : Integrable f μ) :
    Integrable (fun ω => ∫ y, ‖f y‖ ∂condExpKernel μ m ω) μ := by
  nontriviality Ω
  rw [condExpKernel_eq]
  convert!
    Integrable.integral_norm_condDistrib (aemeasurable_id'' μ (inf_le_right : m ⊓ mΩ ≤ mΩ))
      aemeasurable_id hf_int.comp_snd_map_prod_id using 1

theorem _root_.MeasureTheory.Integrable.norm_integral_condExpKernel [NormedSpace ℝ F]
    (hf_int : Integrable f μ) :
    Integrable (fun ω => ‖∫ y, f y ∂condExpKernel μ m ω‖) μ := by
  nontriviality Ω
  rw [condExpKernel_eq]
  convert!
    Integrable.norm_integral_condDistrib (aemeasurable_id'' μ (inf_le_right : m ⊓ mΩ ≤ mΩ))
      aemeasurable_id hf_int.comp_snd_map_prod_id using 1

theorem _root_.MeasureTheory.Integrable.integral_condExpKernel [NormedSpace ℝ F]
    (hf_int : Integrable f μ) :
    Integrable (fun ω => ∫ y, f y ∂condExpKernel μ m ω) μ := by
  nontriviality Ω
  rw [condExpKernel_eq]
  convert!
    Integrable.integral_condDistrib (aemeasurable_id'' μ (inf_le_right : m ⊓ mΩ ≤ mΩ))
      aemeasurable_id hf_int.comp_snd_map_prod_id using 1

theorem integrable_toReal_condExpKernel {s : Set Ω} (hs : MeasurableSet s) :
    Integrable (fun ω => (condExpKernel μ m ω).real s) μ := by
  nontriviality Ω
  rw [condExpKernel_eq]
  exact integrable_toReal_condDistrib (aemeasurable_id'' μ (inf_le_right : m ⊓ mΩ ≤ mΩ)) hs

end Integrability

lemma condExpKernel_ae_eq_condExp' {s : Set Ω} (hs : MeasurableSet s) :
    (fun ω ↦ (condExpKernel μ m ω).real s) =ᵐ[μ] μ⟦s | m ⊓ mΩ⟧ := by
  rcases isEmpty_or_nonempty Ω with h | h
  · have : μ = 0 := Measure.eq_zero_of_isEmpty μ
    simpa [this] using! trivial
  have h := condDistrib_ae_eq_condExp (μ := μ)
    (measurable_id'' (inf_le_right : m ⊓ mΩ ≤ mΩ)) measurable_id hs
  simp only [id_eq, MeasurableSpace.comap_id, preimage_id_eq] at h
  simp_rw [condExpKernel_apply_eq_condDistrib]
  exact h

lemma condExpKernel_ae_eq_condExp
    (hm : m ≤ mΩ) {s : Set Ω} (hs : MeasurableSet s) :
    (fun ω ↦ (condExpKernel μ m ω).real s) =ᵐ[μ] μ⟦s | m⟧ :=
  (condExpKernel_ae_eq_condExp' hs).trans (by rw [inf_of_le_left hm])

lemma condExpKernel_ae_eq_trim_condExp
    (hm : m ≤ mΩ) {s : Set Ω} (hs : MeasurableSet s) :
    (fun ω ↦ (condExpKernel μ m ω).real s) =ᵐ[μ.trim hm] μ⟦s | m⟧ := by
  simp_rw [measureReal_def]
  rw [(measurable_condExpKernel hs).ennreal_toReal.stronglyMeasurable.ae_eq_trim_iff hm
    stronglyMeasurable_condExp]
  exact condExpKernel_ae_eq_condExp hm hs

lemma condDistrib_apply_ae_eq_condExpKernel_map {β γ : Type*} {mβ : MeasurableSpace β}
    {mγ : MeasurableSpace γ} [StandardBorelSpace β] [Nonempty β] {X : Ω → β} {Y : Ω → γ}
    (hX : Measurable X) (hY : Measurable Y) {s : Set β} (hs : MeasurableSet s) :
    (fun a ↦ condDistrib X Y μ (Y a) s)
      =ᵐ[μ] fun a ↦ (condExpKernel μ (mγ.comap Y)).map X a s := by
  simp_rw [Kernel.map_apply' _ hX _ hs]
  filter_upwards [condDistrib_ae_eq_condExp hY hX (μ := μ) hs,
    condExpKernel_ae_eq_condExp hY.comap_le (μ := μ) (hX hs)] with a ha₁ ha₂
  rw [← measureReal_eq_measureReal_iff, ha₁, ha₂]

theorem condExp_ae_eq_integral_condExpKernel' [NormedAddCommGroup F] {f : Ω → F}
    [NormedSpace ℝ F] [CompleteSpace F] (hf_int : Integrable f μ) :
    μ[f | m ⊓ mΩ] =ᵐ[μ] fun ω => ∫ y, f y ∂condExpKernel μ m ω := by
  rcases isEmpty_or_nonempty Ω with h | h
  · have : μ = 0 := Measure.eq_zero_of_isEmpty μ
    simpa [this] using! trivial
  have hX : @Measurable Ω Ω mΩ (m ⊓ mΩ) id := measurable_id.mono le_rfl (inf_le_right : m ⊓ mΩ ≤ mΩ)
  simp_rw [condExpKernel_apply_eq_condDistrib]
  have h := condExp_ae_eq_integral_condDistrib_id hX hf_int
  simpa only [MeasurableSpace.comap_id, id_eq] using! h

/-- The conditional expectation of `f` with respect to a σ-algebra `m` is almost everywhere equal to
the integral `∫ y, f y ∂(condExpKernel μ m ω)`. -/
theorem condExp_ae_eq_integral_condExpKernel [NormedAddCommGroup F] {f : Ω → F}
    [NormedSpace ℝ F] [CompleteSpace F] (hm : m ≤ mΩ) (hf_int : Integrable f μ) :
    μ[f | m] =ᵐ[μ] fun ω => ∫ y, f y ∂condExpKernel μ m ω :=
  ((condExp_ae_eq_integral_condExpKernel' hf_int).symm.trans (by rw [inf_of_le_left hm])).symm

/-- Auxiliary lemma for `condExp_ae_eq_trim_integral_condExpKernel`. -/
theorem condExp_ae_eq_trim_integral_condExpKernel_of_stronglyMeasurable
    [NormedAddCommGroup F] {f : Ω → F} [NormedSpace ℝ F] [CompleteSpace F]
    (hm : m ≤ mΩ) (hf : StronglyMeasurable f) (hf_int : Integrable f μ) :
    μ[f | m] =ᵐ[μ.trim hm] fun ω ↦ ∫ y, f y ∂condExpKernel μ m ω := by
  refine StronglyMeasurable.ae_eq_trim_of_stronglyMeasurable hm ?_ ?_ ?_
  · exact stronglyMeasurable_condExp
  · exact hf.integral_condExpKernel
  · exact condExp_ae_eq_integral_condExpKernel hm hf_int

/-- The conditional expectation of `f` with respect to a σ-algebra `m` is
(`μ.trim hm`)-almost everywhere equal to the integral `∫ y, f y ∂(condExpKernel μ m ω)`. -/
theorem condExp_ae_eq_trim_integral_condExpKernel [NormedAddCommGroup F] {f : Ω → F}
    [NormedSpace ℝ F] [CompleteSpace F] (hm : m ≤ mΩ) (hf_int : Integrable f μ) :
    μ[f | m] =ᵐ[μ.trim hm] fun ω ↦ ∫ y, f y ∂condExpKernel μ m ω := by
  refine (condExp_congr_ae_trim hm hf_int.1.ae_eq_mk).trans ?_
  refine (condExp_ae_eq_trim_integral_condExpKernel_of_stronglyMeasurable hm
    hf_int.1.stronglyMeasurable_mk ?_).trans ?_
  · rwa [integrable_congr hf_int.1.ae_eq_mk.symm]
  filter_upwards [aestronglyMeasurable_trim_condExpKernel hm hf_int.1] with ω hω
  rw [integral_congr_ae hω]

section Cond

/-! ### Relation between conditional expectation, conditional kernel and the conditional measure. -/

open MeasurableSpace

variable {s t : Set Ω} [NormedAddCommGroup F] [NormedSpace ℝ F] [CompleteSpace F]

omit [StandardBorelSpace Ω]

lemma condExp_generateFrom_singleton (hs : MeasurableSet s) {f : Ω → F} (hf : Integrable f μ) :
    μ[f | generateFrom {s}] =ᵐ[μ.restrict s] fun _ ↦ ∫ x, f x ∂μ[|s] := by
  by_cases hμs : μ s = 0
  · rw [Measure.restrict_eq_zero.2 hμs]
    rfl
  refine ae_eq_trans (condExp_restrict_ae_eq_restrict
    (generateFrom_singleton_le hs)
    (measurableSet_generateFrom rfl) hf).symm ?_
  · refine (ae_eq_condExp_of_forall_setIntegral_eq (generateFrom_singleton_le hs) hf.restrict ?_ ?_
      stronglyMeasurable_const.aestronglyMeasurable).symm
    · rintro t - -
      rw [integrableOn_const_iff]
      exact Or.inr <| measure_lt_top (μ.restrict s) t
    · rintro t ht -
      obtain (h | h | h | h) := measurableSet_generateFrom_singleton_iff.1 ht
      · simp [h]
      · simp only [h, cond, integral_smul_measure, ENNReal.toReal_inv, integral_const,
        MeasurableSet.univ, measureReal_restrict_apply, univ_inter, measureReal_restrict_apply_self,
        ← measureReal_def]
        rw [smul_inv_smul₀, Measure.restrict_restrict hs, inter_self]
        exact ENNReal.toReal_ne_zero.2 ⟨hμs, measure_ne_top _ _⟩
      · simp only [h, integral_const, MeasurableSet.univ, measureReal_restrict_apply, univ_inter,
          measureReal_restrict_apply hs.compl, compl_inter_self, measureReal_empty, zero_smul,
          ((Measure.restrict_apply_eq_zero hs.compl).2 <| compl_inter_self s ▸ measure_empty),
          setIntegral_measure_zero]
      · simp only [h, Measure.restrict_univ, cond, integral_smul_measure, ENNReal.toReal_inv, ←
        measureReal_def, integral_const, MeasurableSet.univ, measureReal_restrict_apply, univ_inter]
        rw [smul_inv_smul₀]
        exact (measureReal_ne_zero_iff (by finiteness)).2 hμs

lemma condExp_set_generateFrom_singleton (hs : MeasurableSet s) (ht : MeasurableSet t) :
    μ⟦t | generateFrom {s}⟧ =ᵐ[μ.restrict s] fun _ ↦ μ[|s].real t := by
  rw [← integral_indicator_one ht]
  exact condExp_generateFrom_singleton hs <| Integrable.indicator (integrable_const 1) ht

lemma condExpKernel_singleton_ae_eq_cond [StandardBorelSpace Ω] (hs : MeasurableSet s)
    (ht : MeasurableSet t) :
    ∀ᵐ ω ∂μ.restrict s,
      condExpKernel μ (generateFrom {s}) ω t = μ[t | s] := by
  have : (fun ω ↦ (condExpKernel μ (generateFrom {s}) ω).real t) =ᵐ[μ.restrict s]
      μ⟦t | generateFrom {s}⟧ :=
    ae_restrict_le <| condExpKernel_ae_eq_condExp
      (generateFrom_singleton_le hs) ht
  filter_upwards [condExp_set_generateFrom_singleton hs ht, this] with ω hω₁ hω₂
  rwa [hω₁, measureReal_def, measureReal_def,
    ENNReal.toReal_eq_toReal_iff' (measure_ne_top _ t) (measure_ne_top _ t)] at hω₂

end Cond

end ProbabilityTheory
