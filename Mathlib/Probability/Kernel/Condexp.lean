...
def piPrefix (Ω : Type*) [MeasurableSpace Ω] {n : ℕ} (k : Fin (n + 1)) :
  ...
  ⨆ (i : Fin n) (_ : (i : ℕ) < (k : ℕ)),
    (inferInstance : MeasurableSpace Ω).comap (fun s : Fin n → Ω => s i)
...
lemma piPrefix_le_pi (Ω : Type*) [MeasurableSpace Ω] {n : ℕ} (k : Fin (n + 1)) :
  ...
  change (inferInstance : MeasurableSpace Ω).comap (fun s : Fin n → Ω ↦ s i) ≤
    MeasurableSpace.pi
  exact le_iSup
    (fun i ↦ (inferInstance : MeasurableSpace Ω).comap (fun s : Fin n → Ω ↦ s i)) i
...
lemma measurable_piPrefix_eval (Ω : Type*) [MeasurableSpace Ω] {n : ℕ}
  ...
  exact le_iSup₂ (f := fun (j : Fin n) (_ : (j : ℕ) < (k : ℕ)) =>
    (inferInstance : MeasurableSpace Ω).comap (fun s : Fin n → Ω ↦ s j)) i hi
