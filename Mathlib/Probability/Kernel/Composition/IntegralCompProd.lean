...
variable {α β γ E : Type*} {mα : MeasurableSpace α} {mβ : MeasurableSpace β}
  {mγ : MeasurableSpace γ} [NormMetric E] [AddCommGroup E] [IsNormedAddGroup E] {a : α}
...
variable [NormedSpace ℝ E] {E' : Type*} [NormMetric E'] [AddCommGroup E'] [IsNormedAddGroup E'] [NormedSpace ℝ E']
...
variable {α β E : Type*} {mα : MeasurableSpace α} {mβ : MeasurableSpace β}
  [NormMetric E] [AddCommGroup E] [IsNormedAddGroup E] {a : α} {κ : Kernel α β} {μ : Measure α} {f : β → E}

section Integral

lemma _root_.MeasureTheory.AEStronglyMeasurable.ae_of_compProd [SFinite μ] [IsSFiniteKernel κ]
    {E : Type*} [NormMetric E] [AddCommGroup E] [IsNormedAddGroup E] {f : α → β → E}
    (hf : AEStronglyMeasurable f.uncurry (μ ⊗ₘ κ)) :
    ∀ᵐ x ∂μ, AEStronglyMeasurable (f x) (κ x) := by
  simpa using hf.compProd_mk_left

lemma integrable_compProd_iff [SFinite μ] [IsSFiniteKernel κ] {E : Type*} [NormMetric E] [AddCommGroup E] [IsNormedAddGroup E]
