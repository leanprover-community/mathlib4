...
lemma cexp_bound_exact : ∀ (u v : ℝ), dist (Complex.exp (↑u * I)) (Complex.exp (↑v * I)) ≤ 2 :=
  fun u v =>
    calc
      ...
      _ = ‖Complex.exp (↑u * I)‖ + ‖Complex.exp (↑v * I)‖ := by
        rw [complex_dist_zero_eq_abs, complex_dist_zero_eq_abs']
...
lemma charFun_tendsto_if_inner_tendsto (hX : Measurable X) (hXn : ∀ n, Measurable (Xn n)) :
  (∀ t : EuclideanSpace ℝ (Fin d), Tendsto (fun n : ℕ => P.map (aemeasurable_dotProduct (hXn n) t))
    atTop (𝓝 (Q.map (aemeasurable_dotProduct' hX t)))) → (∀ t : EuclideanSpace ℝ (Fin d),
  ...
  by
    ...
    calc
      ...
      _ ≤ ‖ψ u‖ + ‖ψ v‖ :=
      by
        simp only [Real.norm_eq_abs, ψ_im]
    ...
    let ψ_decomp (x : ℝ) : ψ x = ψ_re_bcf x + (ψ_im_bcf x) * Complex.I :=
      by
        simp only [mkOfBound_coe, ContinuousMap.coe_mk, ψ_re_bcf, ψ_im_bcf]
        rw [Complex.re_add_im]
    let h_lim (f : ℝ →ᵇ ℝ) : 0 ≤ f → atTop.limsup (fun n =>
    ∫ x, f x ∂ (↑(P.map (aemeasurable_dotProduct (hXn n) t))))
    ≤ ∫ x, f x ∂ (↑(Q.map (aemeasurable_dotProduct' hX t))) :=
      by
        intro _
        rw [Tendsto.limsup_eq]
        haveI := @tendsto_iff_forall_integral_tendsto ℝ _ _ _ ℕ atTop (fun n =>
          P.map (aemeasurable_dotProduct (hXn n) t))
          (Q.map (aemeasurable_dotProduct' hX t) : ProbabilityMeasure ℝ)
        haveI := Iff.mp this (hconv t)
        apply this f
    have integralConv_re2 :
      Tendsto (fun n => ∫ (x : ℝ), ψ_re_bcf x ∂ (↑(P.map (aemeasurable_dotProduct (hXn n) t))))
      atTop (𝓝 (∫ (x : ℝ), ψ_re_bcf x ∂ (↑(Q.map (aemeasurable_dotProduct' hX t))))) := by
        haveI := @tendsto_iff_forall_integral_tendsto ℝ _ _ _ ℕ atTop (fun n =>
          P.map (aemeasurable_dotProduct (hXn n) t))
          (Q.map (aemeasurable_dotProduct' hX t) : ProbabilityMeasure ℝ)
        haveI := Iff.mp this (hconv t)
    ...
    have integralConv_re :
      Tendsto (fun n => ∫ (x : ℝ), ψ_re_bcf x ∂ (↑(P.map (aemeasurable_dotProduct (hXn n) t))))
      atTop (𝓝 (∫ (x : ℝ), ψ_re_bcf x ∂ (↑(Q.map (aemeasurable_dotProduct' hX t))))) :=
      BoundedContinuousFunction.tendsto_integral_of_forall_limsup_integral_le_integral
        h_lim ψ_re_bcf
    have integralConv_im :
      Tendsto (fun n => ∫ (x : ℝ), ψ_im_bcf x ∂ (↑(P.map (aemeasurable_dotProduct (hXn n) t))))
      atTop (𝓝 (∫ (x : ℝ), ψ_im_bcf x ∂ (↑(Q.map (aemeasurable_dotProduct' hX t)))) ) :=
      BoundedContinuousFunction.tendsto_integral_of_forall_limsup_integral_le_integral
        h_lim ψ_im_bcf
    have h_ψ : ∀ (ν : ProbabilityMeasure ℝ), ∫ x, ψ x ∂ ν = ∫ x, ψ_re_bcf x ∂ ν
      + (∫ x, ψ_im_bcf x ∂ ν) * Complex.I :=
      by
        intro ν
        simp_rw [ψ_decomp]
    ...
    have integralConv :
      Tendsto (fun n => ∫ (x : ℝ), ψ x ∂ (↑(P.map (aemeasurable_dotProduct (hXn n) t))))
      atTop (𝓝 (∫ (x : ℝ), ψ x ∂ (↑(Q.map (aemeasurable_dotProduct' hX t))))) :=
        by
          rw [h_ψ]
    ...
    · rw [←MeasureTheory.integral_map (aemeasurable_dotProduct' hX t)
      (continuous_exp_ofReal_mul_I.stronglyMeasurable.aestronglyMeasurable)]
      apply Tendsto.congr (fun n => (MeasureTheory.integral_map (hXn n).aemeasurable
        φ.continuous.stronglyMeasurable.aestronglyMeasurable).symm)
      apply Tendsto.congr (fun n => (MeasureTheory.integral_map (aemeasurable_dotProduct (hXn n) t)
      (continuous_exp_ofReal_mul_I.stronglyMeasurable.aestronglyMeasurable)))
      exact integralConv
...
lemma rv_tendsto_if_charFun_tendsto (hX : Measurable X) (hXn : ∀ n, Measurable (Xn n)) :
  (∀ t : EuclideanSpace ℝ (Fin d), Tendsto (fun n ↦ charFun (P.map (hXn n).aemeasurable) t) atTop
  (𝓝 (charFun (Q.map hX.aemeasurable) t))) → Tendsto (fun n ↦ P.map (hXn n).aemeasurable) atTop
  (𝓝 (Q.map hX.aemeasurable)) :=
  by
    intro h
