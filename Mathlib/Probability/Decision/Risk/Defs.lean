...
equal to `∫⁻ y, ℓ θ y ∂((κ ∘ₖ P) θ)`. We don't introduce a definition for that risk.

* `bayesianRisk ℓ P κ π`: the bayesian risk of the estimator, average of the risk with respect to
  the prior `π : Measure Θ`.
* `bayesRiskPrior ℓ P π`: the Bayes risk with respect to the prior `π`, minimum of the Bayesian
  risks over all estimators, that is over all Markov kernels `κ : Kernel 𝓧 𝓨`.
* `bayesRisk ℓ P`: the Bayes risk, supremum of the Bayes risks over all priors
...
noncomputable
def bayesianRisk {m𝓨 : MeasurableSpace 𝓨}
...
estimators. -/
noncomputable
def bayesRiskPrior [MeasurableSpace 𝓨] (ℓ : Θ → 𝓨 → ℝ≥0∞) (P : Kernel Θ 𝓧) (π : Measure Θ) : ℝ≥0∞ :=
...
the prior. -/
noncomputable
def bayesRisk [MeasurableSpace 𝓨] (ℓ : Θ → 𝓨 → ℝ≥0∞) (P : Kernel Θ 𝓧) : ℝ≥0∞ :=
...
@[simp]
lemma bayesRiskPrior_zero_left [Nonempty 𝓨] (ℓ : Θ → 𝓨 → ℝ≥0∞) (π : Measure Θ) :
    bayesRiskPrior ℓ (0 : Kernel Θ 𝓧) π = 0 := by simp [bayesRiskPrior, iInf_subtype']

@[simp]
lemma bayesRiskPrior_zero_right [Nonempty 𝓨] (ℓ : Θ → 𝓨 → ℝ≥0∞) (P : Kernel Θ 𝓧) :
    bayesRiskPrior ℓ P (0 : Measure Θ) = 0 := by simp [bayesRiskPrior, iInf_subtype']

@[simp]
lemma bayesRisk_zero [Nonempty 𝓨] (ℓ : Θ → 𝓨 → ℝ≥0∞) :
