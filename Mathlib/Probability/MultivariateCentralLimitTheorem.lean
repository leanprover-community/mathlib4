    ...
    have h_char_μ :
      ...
      have h_map_rhs :
        ProbabilityMeasure.map μ hmInner.aemeasurable
          = ProbabilityMeasure.map (⟨gaussianReal 0 1, inferInstance⟩ : ProbabilityMeasure ℝ)
            hhf.aemeasurable := by
        ...
        rw [integral_map (MeasurableEquiv.toLp 2 (Fin d → ℝ)).measurable.aemeasurable
          h_cont_LHS.aestronglyMeasurable]
        ...
        have h_prod_int :
          ∫ (x : Fin d → ℝ), ∏ i, Complex.exp (↑(u * (t.ofLp i * x i)) * Complex.I) ∂Measure.pi
      ...
      have hcf :
          charFun (ProbabilityMeasure.map μ hmInner.aemeasurable) (1 : ℝ)
            = charFun (ProbabilityMeasure.map (⟨gaussianReal 0 1, inferInstance⟩
              : ProbabilityMeasure ℝ) hhf.aemeasurable) (1 : ℝ) := by
        rw [h_map_rhs]
      -- 1. The LHS Bridge: Unfold the characteristic functions and use integral_map
      have h_LHS : charFun μ t = charFun (ProbabilityMeasure.map μ hmInner.aemeasurable) (1 : ℝ) :=
      by
        rw [charFun_apply, ProbabilityMeasure.toMeasure_map, charFun_apply_real]
        have h_cont : Continuous (fun (y : ℝ) ↦ Complex.exp (↑(1 : ℝ) * ↑y * Complex.I)) :=
          by fun_prop
      ...
      have h_RHS : charFun (ProbabilityMeasure.map (⟨gaussianReal 0 1, inferInstance⟩
      : ProbabilityMeasure ℝ) hhf.aemeasurable) (1 : ℝ) = Complex.exp (-(↑‖t‖ ^ 2 / 2)) := by
...
theorem tendstoInDistribution_sqrt_inv_mul_sum {Y : Ω → (EuclideanSpace ℝ (Fin d))}
    (hY : HasLaw Y standardGaussianRealMultivariate P) (h0 : P[X 0] = 0)
    (h1 : ∀ i j, P[(fun ω ↦ (X 0 ω i) * (X 0 ω j))] = if i = j then 1 else 0)
