...
lemma lintegral_gammaPDF_of_nonpos {x a r : ℝ} (hx : x ≤ 0) :
  ...
  · intro a (_ : a < _)
    rw [gammaPDF_eq, ENNReal.ofReal_eq_zero, if_neg (by linarith)]
...
lemma lintegral_gammaPDF_eq_one {a r : ℝ} (ha : 0 < a) (hr : 0 < r) :
  ...
  have leftSide : ∫⁻ x in Iio 0, gammaPDF a r x = 0 := by
    rw [setLIntegral_congr_fun measurableSet_Iio fun x (hx : x < 0) ↦ gammaPDF_of_neg hx,
      lintegral_zero]
