...
lemma aemeasurable {X : Ω → E} {s : Set E} (hns : μ s ≠ 0) (hnt : μ s ≠ ∞)
...
lemma absolutelyContinuous {X : Ω → E} {s : Set E} (hu : IsUniform X s ℙ μ) : map X ℙ ≪ μ := by
  rw [hu]; exact ProbabilityTheory.cond_absolutelyContinuous

lemma measure_preimage {X : Ω → E} {s : Set E} (hns : μ s ≠ 0) (hnt : μ s ≠ ∞)
...
lemma isProbabilityMeasure {X : Ω → E} {s : Set E} (hns : μ s ≠ 0) (hnt : μ s ≠ ∞)
...
lemma pdf_eq_zero_of_measure_eq_zero_or_top {X : Ω → E} {s : Set E}
...
lemma pdf_eq {X : Ω → E} {s : Set E} (hms : MeasurableSet s)
...
lemma pdf_toReal_ae_eq {X : Ω → E} {s : Set E} (hms : MeasurableSet s)
...
lemma mul_pdf_integrable (hcs : IsCompact s) (huX : IsUniform X s ℙ) :
...
@[simp]
lemma support_uniformOfFinset : (uniformOfFinset s hs).support = s :=
  Set.ext
    (by
      let ⟨a, ha⟩ := hs
...
@[simp]
lemma toOuterMeasure_uniformOfFinset_apply :
...
@[simp]
lemma toMeasure_uniformOfFinset_apply [MeasurableSpace α] (ht : MeasurableSet t) :
...
lemma toOuterMeasure_uniformOfFintype_apply :
...
lemma toMeasure_uniformOfFintype_apply [MeasurableSpace α] (hs : MeasurableSet s) :
...
@[simp]
lemma support_ofMultiset : (ofMultiset s hs).support = s.toFinset :=
...
lemma ofMultiset_apply_of_not_mem {a : α} (ha : a ∉ s) : ofMultiset s hs a = 0 := by
...
@[simp]
lemma toOuterMeasure_ofMultiset_apply :
...
@[simp]
lemma toMeasure_ofMultiset_apply [MeasurableSpace α] (ht : MeasurableSet t) :
