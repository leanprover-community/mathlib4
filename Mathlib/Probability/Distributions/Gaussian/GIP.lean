...
lemma covCoord_gaussianLin
  ...
  have hX :
      AEStronglyMeasurable (fun x : E n => x i)
        ((gaussianStd n).map (fun z : E n => A.mulVec z)) := by
...
theorem gaussianLin_ibp_coord
  ...
  | succ n =>
      ...
      have hibp_comp :
        ...
        let gR : E (n + 1) → ℝ :=
          fun x => partialDeriv (n + 1) k (fun z : E (n + 1) => f (A.mulVec z)) x
        obtain ⟨Cf, hCf⟩ := hbound_f
        have hg_as : AEStronglyMeasurable (fun x : E (n + 1) => f (A.mulVec x))
            (gaussianStd (n + 1)) := by
        ...
        have hgR_int : Integrable gR (gaussianStd (n + 1)) := by
          ...
          have hterm_int :
            ...
            have hassm : AEStronglyMeasurable
                (fun x : E (n + 1) => partialDeriv (n + 1) j f (A.mulVec x))
        ...
        have hL_rewrite' :
          ...
          simpa [hLpair, gL] using
            (hmp.symm.integral_comp' (g := gL)).symm
        have hR_rewrite' :
            (∫ x, partialDeriv (n + 1) k (fun z : E (n + 1) => f (A.mulVec z)) x
                ∂gaussianStd (n + 1)) =
              ∫ p, hRpair p ∂(γ.prod μrest) := by
          simpa [hRpair, gR] using
            (hmp.symm.integral_comp' (g := gR)).symm
        ...
        have hmul_update : ∀ t : ℝ,
          ...
          calc
            A.mulVec (Function.update x0 k t)
                = A.mulVec (x0 + t • e (n + 1) k) := by simp [ht]
      ...
      have hmul_i : (fun z : E (n + 1) => (A.mulVec z) i)
          = fun z : E (n + 1) => ∑ k : Fin (n + 1), A i k * z k := by
        funext z
        simp [Matrix.mulVec, dotProduct]
      have hmul_i_val :
      ...
      have hterm_int :
          ∀ k : Fin (n + 1),
            Integrable (fun z : E (n + 1) => (A i k * z k) * f (A.mulVec z)) μ := by
        intro k
      ...
      have hchain_int :
        ...
        have hterm_int' : ∀ j : Fin (n + 1),
          ...
          have hmeas : Measurable (fun z : E (n + 1) => partialDeriv (n + 1) j f (A.mulVec z)) :=
            (hcont_partial j).measurable.comp hmeasA
          have hassm : AEStronglyMeasurable
              (fun z : E (n + 1) => partialDeriv (n + 1) j f (A.mulVec z)) μ :=
            (hmeas.aemeasurable).aestronglyMeasurable
