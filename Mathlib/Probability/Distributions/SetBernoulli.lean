...
lemma setBernoulli_of_not_subset {s : Set ι} (hs : ¬ s ⊆ u) : setBer(u, p) {s} = 0 :=
  Measure.mono_null (by simpa) setBernoulli_ae_subset

lemma setBernoulli_apply_eq_inter_subset (S : Set (Set ι)) :
...
lemma setBernoulli_empty : setBer((∅ : Set ι), p) = .dirac ∅ := by
  ext s hs
  rw [setBernoulli_apply_eq_inter_subset]
  by_cases h : ∅ ∈ s
  · have : {t | t ∈ s ∧ t ⊆ ∅} = {∅} := by ext; grind
    rw [this, setBernoulli_singleton] <;> simp
    simp_all
  · have : {t | t ∈ s ∧ t ⊆ ∅} = ∅ := by ext; grind
    rw [this]
    simp_all
