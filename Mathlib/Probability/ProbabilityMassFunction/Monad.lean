...
@[deprecated "Use Measure.bind_comm in the Measure.Prod file." (since := "2026-08-17")]
...
theorem toOuterMeasure_bind_apply (p : PMF α) (f : α → PMF β) (s : Set β) :
  ...
  calc
    (p.bind f).toOuterMeasure s = ∑' b, if b ∈ s then ∑' a, p a * f a b else 0 := by
      simp [toOuterMeasure_apply, Set.indicator_apply, bind_apply]
...
@[deprecated "Use Measure.sum_eq_zero and Measure.compProd_eq_zero_iff." (since := "2026-08-17")]
theorem bindOnSupport_eq_zero_iff {p : PMF α} (f : ∀ a ∈ p.support, PMF β) (b : β) :
...
@[deprecated "Use Measure.bind_comm in Measure.Prod." (since := "2026-08-17")]
