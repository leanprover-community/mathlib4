...
@[simp]
theorem pure_apply : (pure a a' : ℝ≥0∞) = if a' = a then 1 else 0 :=
  coe_toNNReal ((pure a).val_apply_ne_top _)
...
theorem pure_apply_self : (pure a a : ℝ≥0∞) = 1 :=
  by simp only [pure_apply, ite_true]
#align pmf.pure_apply_self Pmf.pure_apply_self

theorem pure_apply_of_ne (h : a' ≠ a) : (pure a a' : ℝ≥0∞) = 0 :=
  by simp only [pure_apply, h, ite_false]
...
@[simp]
theorem bind_apply (b : β) : p.bind f b = ∑' a, (p a : ℝ≥0∞) * f a b :=
  coe_toNNReal ((p.bind f).val_apply_ne_top _)
...
theorem pure_bind (a : α) (f : α → Pmf β) : (pure a).bind f = f a := by
  have : ∀ b a', ite (a' = a) (f a' b : ℝ≥0∞) 0 = ite (a' = a) (f a b : ℝ≥0∞) 0 := fun b a' => by
    split_ifs with h <;> simp; subst h; simp
  refine  Pmf.ext fun x => ENNReal.coe_injective ?_
  simp [this]
...
theorem bind_pure : p.bind pure = p :=
  Pmf.ext fun x => ENNReal.coe_injective ((bind_apply _ _ _).trans (_root_.trans
...
theorem bind_const (p : Pmf α) (q : Pmf β) : (p.bind fun _ => q) = q :=
  Pmf.ext fun _ => ENNReal.coe_injective <|
    by rw [bind_apply, ENNReal.tsum_mul_right, tsum_coe, one_mul]
#align pmf.bind_const Pmf.bind_const

@[simp]
theorem bind_bind : (p.bind f).bind g = p.bind fun a => (f a).bind g :=
  Pmf.ext fun _ => ENNReal.coe_injective <| by
...
theorem toOuterMeasure_bind_apply :
    (p.bind f).toOuterMeasure s = ∑' a, (p a : ℝ≥0∞) * (f a).toOuterMeasure s :=
  calc
    (p.bind f).toOuterMeasure s = ∑' b, if b ∈ s then ∑' a, (p a : ℝ≥0∞) * f a b else 0 := by
      simp [toOuterMeasure_apply, Set.indicator_apply]
    _ = ∑' (b) (a), (p a : ℝ≥0∞) * if b ∈ s then (f a b : ℝ≥0∞) else 0 :=
      (tsum_congr fun b => by split_ifs <;> simp)
    _ = ∑' (a) (b), (p a : ℝ≥0∞) * if b ∈ s then (f a b : ℝ≥0∞) else 0 :=
      (tsum_comm' ENNReal.summable (fun _ => ENNReal.summable) fun _ => ENNReal.summable)
    _ = ∑' a, p a * ∑' b, if b ∈ s then (f a b : ℝ≥0∞) else 0 :=
      (tsum_congr fun a => ENNReal.tsum_mul_left)
    _ = ∑' a, p a * ∑' b, if b ∈ s then (f a b : ℝ≥0∞) else 0 :=
      (tsum_congr fun a => (congr_arg fun x =>
        (p a : ℝ≥0∞) * x) <| tsum_congr fun b => by split_ifs <;> rfl)
    ...
    ((toOuterMeasure_bind_apply p f s).trans
      (tsum_congr fun a =>
        congr_arg (fun x => (p a : ℝ≥0∞) * x)
          (toMeasure_apply_eq_toOuterMeasure_apply (f a) s hs).symm))
...
def bindOnSupport (p : Pmf α) (f : ∀ a ∈ p.support, Pmf β) : Pmf β :=
  ⟨fun b => ∑' a, p a * if h : (p a : ℝ≥0∞) = 0 then 0 else (f a h b : ℝ≥0∞),
...
theorem bindOnSupport_apply (b : β) :
    p.bindOnSupport f b = ∑' a, p a * if h : (p a : ℝ≥0∞) = 0 then 0 else (f a h b : ℝ≥0∞) :=
  coe_toNNReal ((p.bindOnSupport f).val_apply_ne_top _)
...
theorem support_bindOnSupport :
  ...
  exact
    ⟨fun hb =>
      let ⟨a, ⟨ha, ha'⟩⟩ := hb
      ⟨a, ha, by simpa only [ha] using ha'⟩,
      fun hb =>
      let ⟨a, ha, ha'⟩ := hb
      ⟨a, ⟨ha, by simpa only [(mem_support_iff _ a).1 ha] using ha'⟩⟩⟩
...
theorem bindOnSupport_eq_bind (p : Pmf α) (f : α → Pmf β) :
  ...
  Pmf.ext fun b => ENNReal.coe_injective <| by
    have : ∀ a, ite ((p a : ℝ≥0∞) = 0) 0 (p a * (f a b : ℝ≥0∞)) = p a * (f a b : ℝ≥0∞) :=
      fun a => ite_eq_right_iff.2 fun h => h.symm ▸ symm (zero_mul <| (f a b : ℝ≥0∞))
...
theorem bindOnSupport_eq_zero_iff (b : β) :
    (p.bindOnSupport f b : ℝ≥0∞) = 0 ↔ ∀ (a) (ha : (p a : ℝ≥0∞) ≠ 0), (f a ha b : ℝ≥0∞) = 0 := by
...
theorem bindOnSupport_bindOnSupport (p : Pmf α) (f : ∀ a ∈ p.support, Pmf β)
  ...
  Pmf.ext fun b => ENNReal.coe_injective <| by
    ...
    any_goals ring1
    · have := h_1 a'
      simp only [h, dite_false, mul_eq_zero, false_or] at this
      contradiction
    · simp [h_2]
...
theorem toOuterMeasure_bindOnSupport_apply :
    (p.bindOnSupport f).toOuterMeasure s =
      ∑' a, p a * if h : (p a : ℝ≥0∞) = 0 then 0 else (f a h).toOuterMeasure s := by
  simp only [toOuterMeasure_apply, Set.indicator_apply, bindOnSupport_apply]
  calc
    (∑' b, ite (b ∈ s) (∑' a,
      (p a : ℝ≥0∞) * dite ((p a : ℝ≥0∞) = 0) (fun h => 0) fun h => (f a h b : ℝ≥0∞)) 0)
      = ∑' (b) (a),
        ite (b ∈ s) (p a * dite ((p a : ℝ≥0∞) = 0) (fun h => 0) fun h => (f a h b : ℝ≥0∞)) 0 :=
      tsum_congr fun b => by split_ifs with hbs <;> simp only [eq_self_iff_true, tsum_zero]
    _ = ∑' (a) (b),
        ite (b ∈ s) (p a * dite ((p a : ℝ≥0∞) = 0) (fun h => 0) fun h => (f a h b : ℝ≥0∞)) 0 :=
      ENNReal.tsum_comm
    _ = ∑' a, p a * ∑' b,
        ite (b ∈ s) (dite ((p a : ℝ≥0∞) = 0) (fun h => 0) fun h => (f a h b : ℝ≥0∞)) 0 :=
      (tsum_congr fun a => by simp only [← ENNReal.tsum_mul_left, mul_ite, mul_zero])
    _ = ∑' a, p a * dite ((p a : ℝ≥0∞) = 0) (fun h => 0) fun h => ∑' b,
        ite (b ∈ s) (f a h b : ℝ≥0∞) 0 :=
...
theorem toMeasure_bindOnSupport_apply [MeasurableSpace β] (hs : MeasurableSet s) :
    (p.bindOnSupport f).toMeasure s =
      ∑' a, p a * if h : (p a : ℝ≥0∞) = 0 then 0 else (f a h).toMeasure s := by
