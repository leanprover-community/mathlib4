...
noncomputable
def binomial (p : ℝ≥0) (h : p ≤ 1) (n : ℕ) : Pmf (Fin (n + 1)) := .ofFintype
  (fun i => ((p^(i : ℕ) * (1-p)^((Fin.last n - i) : ℕ) * (n.choose i : ℕ)) : NNReal)) (by
    rw [ ← ENNReal.coe_finset_sum , ENNReal.coe_eq_one]
...
theorem binomial_one_eq_bernoulli (p : ℝ≥0) (h : p ≤ 1) :
  ...
  Pmf.ext fun i => ENNReal.coe_injective $ by
