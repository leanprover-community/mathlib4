...
theorem condExp_stopping_time_ae_le_restrict_eq_const [Preorder E] (h : Submartingale f ℱ μ)
    (hτ : IsStoppingTime ℱ τ) [SigmaFinite (μ.trim hτ.measurableSpace_le)] (hin : i ≤ n) :
    f i ≤ᵐ[μ.restrict {x | τ x = i}] μ[f n|hτ.measurableSpace] := by
  refine Filter.EventuallyLE.trans_eq (ae_restrict_of_ae (h.ae_le_condExp hin)) ?_
  symm
...
theorem condExp_stopping_time_ae_le_restrict_eq_const_of_le_const [Preorder E]
...
theorem stoppedValue_ae_le_restrict_eq [Preorder E] (h : Submartingale f ℱ μ)
  ...
  refine Filter.EventuallyLE.trans ?_
    (condExp_stopping_time_ae_le_restrict_eq_const_of_le_const h hτ hτ_le i)
  rw [Filter.EventuallyLE, ae_restrict_iff' (ℱ.le _ _ (hτ.measurableSet_eq i))]
  refine Filter.Eventually.of_forall fun x hx => ?_
  rw [Set.mem_setOf_eq] at hx
  simp [stoppedValue, hx]

theorem ae_le_restrict_biUnion_iff {α ι δ : Type*} [Preorder δ] {m0 : MeasurableSpace α}
    {μ : Measure α} (s : ι → Set α) {t : Set ι} (ht : t.Countable) (f g : α → δ) :
    f ≤ᵐ[μ.restrict (⋃ i ∈ t, s i)] g ↔ ∀ i ∈ t, f ≤ᵐ[μ.restrict (s i)] g := by
  simp_rw [ae_restrict_biUnion_eq s ht, Filter.EventuallyLE, Filter.eventually_iSup]
...
theorem stoppedValue_ae_le_condExp_of_le_const_of_countable_range
    [Preorder E] (h : Submartingale f ℱ μ)
  ...
  rw [this, ae_le_restrict_biUnion_iff _ h_countable_range]
  intro i hi
  ...
  exact stoppedValue_ae_le_restrict_eq h _ hτ_le i

/-- The value of a martingale `f` at a stopping time `τ` bounded by `n` is the conditional
expectation of `f n` with respect to the σ-algebra generated by `τ`. -/
theorem stoppedValue_ae_le_condExp_of_le_const [Preorder E] [Countable ι] (h : Submartingale f ℱ μ)
    (hτ : IsStoppingTime ℱ τ) (hτ_le : ∀ x, τ x ≤ n)
    [SigmaFinite (μ.trim (hτ.measurableSpace_le_of_le hτ_le))] :
    stoppedValue f τ ≤ᵐ[μ] μ[f n|hτ.measurableSpace] :=
  h.stoppedValue_ae_le_condExp_of_le_const_of_countable_range hτ hτ_le (Set.to_countable _)
