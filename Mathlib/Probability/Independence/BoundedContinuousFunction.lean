...
prove the result when `X` and `Y` are product spaces without assuming second countability, and
...
Then we build on that two show that a `σ`-algebra `m` is independent from a stochastic process `X`
