...
lemma IndepSets.indep {m1 m2 m : MeasurableSpace Ω} {κ : kernel α Ω} {μ : Measure α}
...
/-- The measurable space structures generated by independent pi-systems are independent. -/
lemma iIndepSets.iIndep [IsMarkovKernel κ] (m : ι → MeasurableSpace Ω)
...
@[symm]
nonrec lemma IndepFun.symm {_ : MeasurableSpace β} {f g : Ω → β} (hfg : IndepFun f g κ μ) :
    IndepFun g f κ μ := hfg.symm

lemma IndepFun.ae_eq {mβ : MeasurableSpace β} {f g f' g' : Ω → β} (hfg : IndepFun f g κ μ)
...
lemma iIndepFun.indepFun_finset [IsMarkovKernel κ] {ι : Type*} {β : ι → Type*}
...
lemma iIndepFun.indepFun_prod [IsMarkovKernel κ] {ι : Type*} {β : ι → Type*}
...
@[to_additive]
lemma iIndepFun.mul [IsMarkovKernel κ] {ι : Type*} {β : Type*} {m : MeasurableSpace β}
...
@[to_additive]
lemma iIndepFun.indepFun_finset_prod_of_not_mem [IsMarkovKernel κ] {ι : Type*} {β : Type*}
...
@[to_additive]
lemma iIndepFun.indepFun_prod_range_succ [IsMarkovKernel κ] {β : Type*}
