...
lemma measurableSet_predictable_singleton_prod
  ...
  · ext m
    simp only [Set.mem_singleton, Set.mem_Ioc]
    lia
...
lemma measurableSet_prodMk_add_one_of_predictable {𝓕 : Filtration ℕ m} {s : Set (ℕ × Ω)}
  ...
  · rw [MeasurableSpace.map_def,
    ...
    · simp
    · ext p
      simp only [Nat.bot_eq_zero, Set.mem_preimage, Set.mem_prod, Set.mem_singleton,
  ...
  · rw [MeasurableSpace.map_def]
    ...
    · rw [(_ : (fun (p : Set.singleton (n + 1) × Ω) ↦ ((p.1 : ℕ), p.2)) ⁻¹' (Set.Ioi i ×ˢ A)
      ...
      · ext p
        simp only [Set.mem_preimage, Set.mem_prod, Set.mem_Ioi, Set.mem_singleton,
          and_congr_left_iff]
        intro hp2
        rw [p.1.2]
