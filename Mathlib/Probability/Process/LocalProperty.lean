...
lemma Locally.isLocalizingSequence_localSeq [Zero E] (hX : Locally p 𝓕 X P) :
    IsLocalizingSequence 𝓕 (hX.localSeq) P :=
  hX.choose_spec.1

lemma Locally.stoppedProcess_localSeq [Zero E] (hX : Locally p 𝓕 X P) (n : ℕ) :
...
lemma IsStable.locally_and_iff (hp : IsStable 𝓕 p) (hq : IsStable 𝓕 q) :
  ...
  · convert hp _ (hpX.stoppedProcess n) _ <| hqX.IsLocalizingSequence.isStoppingTime n using 1
    ...
    rw [(_ : {ω | ⊥ < hpX.localSeq n ω ∧ ⊥ < hqX.localSeq n ω}
...
lemma measure_iInter_of_ae_antitone {ι : Type*}
  ...
  have hst (i : ι) : s i =ᵐ[P] t i := by
    ...
    exact ⟨fun (h : s i ω) j hj ↦ hω hj h, fun h ↦ h i le_rfl⟩
  ...
  · obtain ⟨i, hi⟩ := hfin
    exact ⟨i, by rwa [measure_congr (hst i).symm]⟩
...
lemma IsStable.locally_of_isPreLocalizingSequence [Zero E] {τ : ℕ → Ω → WithTop ι}
