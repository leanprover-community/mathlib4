...
theorem memLp_limitProcess_of_eLpNorm_bdd {p : ℝ≥0∞} {F : Type*} [NormedAddCommGroup F]
    {ℱ : Filtration ℕ m} {f : ℕ → Ω → F} (hfm : ∀ n, AEStronglyMeasurable (f n) μ)
    (hbdd : ⨆ n, eLpNorm (f n) p μ < ∞) : MemLp (limitProcess f ℱ μ) p μ := by
  rw [limitProcess]
  split_ifs with h
  · have hlim : AEStronglyMeasurable (Classical.choose h) μ :=
    ...
    rw [memLp_iff]
    apply (Lp.eLpNorm_lim_le_liminf_eLpNorm hfm _ hlim (Classical.choose_spec h).2).trans_lt
    refine lt_of_le_of_lt ?_ hbdd
    simp_rw [liminf_eq, eventually_atTop]
    exact sSup_le fun b ⟨a, ha⟩ ↦ (ha a le_rfl).trans (le_iSup (fun b ↦ eLpNorm (f b) p  μ) a)
  · exact MemLp.zero
