...
protected lemma isLUB_sSup (s : Set (Filtration ι m)) :
    IsLUB s (sSup s) :=
  .of_image (f := seq) .rfl (by simpa only [isLUB_pi, Set.image_image] using fun _ ↦ isLUB_sSup _)

protected lemma isGLB_sInf (s : Set (Filtration ι m)) :
    IsGLB s (sInf s) := by
  unfold sInf
  dsimp only [instInfSet]
  split_ifs with hn
...
noncomputable instance instCompleteLattice : CompleteLattice (Filtration ι m) where
  ...
  isLUB_sSup_of_exists_isLUB _ _ := Filtration.isLUB_sSup _
  isGLB_sInf_of_exists_isGLB _ _ := Filtration.isGLB_sInf _
  exists_isLUB _ := ⟨_, Filtration.isLUB_sSup _⟩
  exists_isGLB _ := ⟨_, Filtration.isGLB_sInf _⟩
  le_top f i := f.le' i
  bot_le _ _ := bot_le
