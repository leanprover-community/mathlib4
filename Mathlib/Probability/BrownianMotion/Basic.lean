...
local notation "𝓖" => (natural B (fun t ↦ Measurable.stronglyMeasurable (mB t)))₊ 0

/-- **Blumenthal's zero-one law**: Let `𝓕` be the canonical filtration associated to a Brownian
motion. Then the `σ`-algebra `⨅ s > 0, 𝓕 s` is trivial. -/
lemma IsBrownianReal.measure_eq_zero_or_one_of_measurableSet_rightCont_zero
    (hB : IsBrownianReal B P) (cB : ∀ ω, Continuous (B · ω)) {A : Set Ω} (hA : MeasurableSet[𝓖] A) :
    P A = 0 ∨ P A = 1 := by
  have 𝓖_eq : 𝓖 = ⨅ s > (0 : ℝ≥0), (.comap (fun ω (t : Set.Iic s) ↦ B t ω) inferInstance) := by
...
-- We easily have that `𝓖 ≤ m1 ≤ mΩ`.
  ...
  have : m1 ≤ m2 := by
    ...
    convert self_mem_nhdsWithin
    ext; simp [pos_iff_ne_zero]
  -- We prove the result by showing that `𝓖` is independent of itself.
  refine measure_eq_zero_or_one_of_indep_self ?_ hA
  -- To do so, we show that for all `A ∈ 𝓖`, all finite sets `I ⊆ (0, +∞)` and all
  -- bounded continuous functions `f : (I → ℝ) → ℝ`,
  -- `∫ ω in A, f (fun t ↦ B t) ∂P = P.real A * ∫ ω, f (fun t ↦ B t) ∂P`.
  refine indep_of_indep_of_le_right ?_ (h𝓖.trans this)
  refine indep_comap_process_of_bcf h𝓖' (fun _ ↦ (mB _).aemeasurable) fun A hA I f ↦ ?_
  ...
  -- This follows from the fact that, because `A ∈ 𝓖` in particular `A` is measurable
  ...
  have key1 (ε : ℝ≥0) (hε1 : 0 < ε) (hε2 : ε ≤ I.min' hI) :
    ...
    rw [Indep.setIntegral_eq_mul h𝓖' _ (by fun_prop) hA (by fun_prop)]
