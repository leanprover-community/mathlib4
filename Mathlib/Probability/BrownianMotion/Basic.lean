...
* `IsBrownian.measure_eq_zero_or_one_of_measurableSet_rightCont_zero`:
...
/-- **Blumenthal's zero-one law**: Let `𝓕` be the canonical filtration associated to a Brownian
motion. Then the `σ`-algebra `⨅ s > 0, 𝓕 s` is trivial. -/
lemma IsBrownian.measure_eq_zero_or_one_of_measurableSet_rightCont_zero (hB : IsBrownian B P)
    (cB : ∀ ω, Continuous (B · ω)) {A : Set Ω} (hA : MeasurableSet[𝓕] A) :
    P A = 0 ∨ P A = 1 := by
