...
variable {Ω 𝓧 𝓨 𝓩 : Type*} {mΩ : MeasurableSpace Ω}
  ...
  {P : Measure Ω} [IsFiniteMeasure P] {X : Ω → 𝓧} {Y : Ω → 𝓨} {κ : Kernel 𝓧 𝓨}
...
def HasCondDistrib (Y : Ω → 𝓨) (X : Ω → 𝓧) (κ : Kernel 𝓧 𝓨)
    (P : Measure Ω) [IsFiniteMeasure P] : Prop :=
...
lemma HasCondDistrib.comp [IsSFiniteKernel κ]
...
lemma HasLaw.prod_of_hasCondDistrib {Q : Measure 𝓧} [IsSFiniteKernel κ]
  ...
  congr!
  exact h1.map_eq.symm

lemma HasCondDistrib.comp_right [IsSFiniteKernel κ] {f : 𝓩 → 𝓧}
...
lemma HasCondDistrib.of_compProd [IsFiniteKernel κ] {Z : Ω → 𝓩}
