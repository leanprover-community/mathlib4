...
lemma condCount_singleton (ω : Ω) (t : Set Ω) [Decidable (ω ∈ t)] :
...
lemma condCount_inter_self (hs : s.Finite) : condCount s (s ∩ t) = condCount s t := by
...
lemma condCount_self (hs : s.Finite) (hs' : s.Nonempty) : condCount s s = 1 := by
...
lemma condCount_eq_one_of (hs : s.Finite) (hs' : s.Nonempty) (ht : s ⊆ t) :
    condCount s t = 1 := by
...
lemma pred_true_of_condCount_eq_one (h : condCount s t = 1) : s ⊆ t := by
...
lemma condCount_eq_zero_iff (hs : s.Finite) : condCount s t = 0 ↔ s ∩ t = ∅ := by
...
lemma condCount_union (hs : s.Finite) (htu : Disjoint t u) :
...
lemma condCount_compl (t : Set Ω) (hs : s.Finite) (hs' : s.Nonempty) :
...
lemma condCount_disjoint_union (hs : s.Finite) (ht : t.Finite) (hst : Disjoint s t) :
...
lemma condCount_add_compl_eq (u t : Set Ω) (hs : s.Finite) :
