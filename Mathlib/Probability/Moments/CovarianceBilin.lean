...
We define the covariance of a measure in a separable Banach space, as a continous bilinear form.

## Main definitions

Let `μ` be a finite measure on a separable Banach space `E` with the Borel σ-algebra, such that
`∫ x, ‖x‖^2 ∂μ < ∞`.
