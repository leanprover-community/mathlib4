...
theorem kerNat_proj (κ : (k : ℕ) → kernel ((x : Iic k) → X x) (X (k + 1)))
  ...
  · ext x s ms
    rw [kernel.map_apply' _ _ _ ms, ← compProd_kerNat κ hab hbc,
      compProd_apply' _ _ hab hbc _ (measurable_projection _ ms), ← one_mul (kerNat κ a b x s),
...
time `a`, `partialKernel κ a b` gives the distribution of the trajectory up to time `b`. -/
