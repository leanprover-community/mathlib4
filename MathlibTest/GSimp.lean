...
import Mathlib.Tactic.GSimp.Elab
...
example (h₁ : a ≤ b) (h₂ : b ≤ c) : a + 5 ≤ c + 6 := by
  ...
  gsimp [show (5 : α) < 6 by norm_num]

example (h₁ : a ≤ b) (h₂ : b ≤ c) : c + 6 > a + 5 := by
  gsimp [h₁, h₂] -- TODO: support `>`
...
example {n : ℤ} (hn : n ≡ 1 [ZMOD 3]) : n ^ 3 + 7 * n ≡ 2 [ZMOD 3] := by gsimp [hn]; decide
...
/-! Rewriting at a wildcard `*`, i.e. `gsimp [h] at *`, will sometimes include a rewrite at `h` itself
...
Having `gsimp [h] at *` rewrite at `h` itself is a bit weird, but is consistent with the behaviour of
...
example {a b : ℤ} (h1 : a ≡ 3 [ZMOD 5]) (h2 : b ≡ a ^ 2 + 1 [ZMOD 5]) :
    a ^ 2 + b ^ 2 ≡ 4 [ZMOD 5] := by
  gsimp [h1] at *
  guard_hyp h1 :ₛ 3 ≡ 3 [ZMOD 5] -- `gsimp [h1] at *` rewrites at `h1`
  guard_hyp h2 :ₛ b ≡ 3 ^ 2 + 1 [ZMOD 5]
  guard_target =ₛ 3 ^ 2 + b ^ 2 ≡ 4 [ZMOD 5]
  gsimp [h2]
  decide
...
example {x y a b : ℚ} (h : x < y) (h1 : a ≤ 3 * x) : 2 * x ≤ b := by
  gsimp [h] at *
  guard_hyp h :ₛ x < y -- `gsimp [h] at *` does not rewrite at `h`
  guard_hyp h1 :ₛ a ≤ 3 * y
...
example {x y a b : ℤ} (h1 : |x| ≤ a) (h2 : |y| ≤ b) :
  ...
  have : 0 ≤ a := by gsimp [← h1]; positivity
  gsimp [abs_add_le, abs_mul, abs_mul, abs_pow, h1, h2, abs_of_nonneg]
  norm_num

example {a b : ℚ} {P : Prop} (hP : P) (h : P → a < b) : False := by
  have : 2 * a ≤ 2 * b := by gsimp [h]; exact hP
  exact test_sorry

example {a b : ℚ} {P Q : Prop} (hP : P) (hQ : Q) (h : P → Q → a < b) : False := by
  have : 2 * a ≤ 2 * b := by gsimp [h ?_ hQ]; exact hP
  exact test_sorry
...
-- previously, `gsimp` failed to rewrite in expressions with syntheticOpaque metavariables
example : ∃ n, n < 2 := by
  refine ⟨?_, ?_⟩
  on_goal 2 => gsimp [← one_lt_two]
  exact 0
  refine zero_lt_one
...
-- Test that `gsimp` works even in the presence of metadata.
...
-- `rw`/`gsimp` index based on the head constant, so the following fails.
/--
