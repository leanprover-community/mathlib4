...
of this type. -/
structure WellOrder : Type (u + 1) extends LinOrd where
  /-- The proposition that the linear order is a well order. -/
  wo : IsWellOrder carrier str.le

attribute [instance] WellOrder.wo


example : Inhabited WellOrder where
  default := by
    refine ⟨LinOrd.of PUnit,?_⟩
    convert IsWellOrder.mk
    · simp [IsTrichotomous.mk]
    · simp [IsTrans.mk]
    · sorry --dont know if this is true
...
instance inhabited : Inhabited WellOrder where
  default := by
    refine ⟨LinOrd.of PEmpty,inferInstance⟩


/-- Equivalence relation on well orders on arbitrary types in universe `u`, given by order
isomorphism. -/
instance Ordinal.isEquivalent : Setoid WellOrder where
  r := fun r s => Nonempty (r.str.le ≃r s.str.le)
  iseqv := ⟨fun _ => ⟨RelIso.refl _⟩, fun ⟨e⟩ => ⟨e.symm⟩, fun ⟨e₁⟩ ⟨e₂⟩ => ⟨e₁.trans e₂⟩⟩

/-- Equivalence relation on well orders on arbitrary types in universe `u`, given by order
isomorphism., analogous to `Ordinal.isEquivalent`. -/
instance instSetoidLinord : Setoid LinOrd where
...
/-- `OrderType.{u}` is the type linear orders in `Type u`, up to order isomorphism. -/
@[pp_with_univ]
def OrderType : Type (u + 1) :=
  Quotient instSetoidLinord
...
-- def Ordinal.toType (o : OrderType) : Type u :=
--   o.out.carrier

def OrderType.toType (o : OrderType) : Type u :=
  o.out.carrier

instance : CoeSort OrderType (Type _) where
 coe o := o.toType

def OrderType.toLinearOrder (o : OrderType) := o.out.str

def OrderType.toRel (o: OrderType) : o.toType → o.toType → Prop :=
  o.toLinearOrder.le

def Ordinal.toType (o : Ordinal) : Type u :=
  o.out.carrier

def Ordinal.toRel (o : Ordinal) : LinOrd :=
  o.out.toLinOrd

instance hasWellFounded_toType (o : Ordinal ) : WellFoundedRelation (o.toType) :=
  ⟨o.out.str.le, o.out.wo.wf⟩

instance linearOrder_toType (o : Ordinal) : LinearOrder o.toType :=
  @IsWellOrder.linearOrder _ o.out.str.le o.out.wo
...
def type' (o : OrderType.{u}) [wo : IsWellOrder o.toType o.toRel] : Ordinal :=
  ⟦⟨o.out, wo⟩⟧

def type (r : LinearOrder α) [wo : IsWellOrder α r.le] : Ordinal :=
  ⟦⟨@LinOrd.mk α r,wo⟩⟧

instance : Coe Ordinal OrderType where
 coe o := ⟦o.toRel⟧
...
instance : Zero OrderType where
  zero := ⟦LinOrd.of PEmpty⟧

instance : Zero Ordinal where
  zero := ⟦⟨LinOrd.of PEmpty,inferInstance⟩⟧
...
instance one : One Ordinal where
  one := sorry
