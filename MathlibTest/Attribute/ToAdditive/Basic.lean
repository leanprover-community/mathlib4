...
run_meta guard <| translations.find? (← getEnv) `Test.foo21
  matches some ⟨`Test.bar21, ⟨[], ⟨[], #[]⟩⟩, false, .arg 1⟩
...
run_meta guard <|
  translations.find? (← getEnv) ``monoidAlgebraFoo
    matches some ⟨``addMonoidAlgebraFoo, ⟨[], ⟨[], #[]⟩⟩, false, .arg 1⟩
...
/--
error: `@[to_additive]` failed to add declaration `add_rfl`.
...
but is expected to have type
  FakeMul α
in the application
  @inferInstance (FakeMul α) inst✝

For help, see the docstring of `to_additive`, section `Troubleshooting`.
-/
#guard_msgs in
@[to_additive]
theorem mul_rfl [Mul α] (a b : α) : haveI : FakeMul α := inferInstance; a * b = a * b := Eq.refl (a * b)
...
#guard_msgs in
@[to_additive]
local instance [Group α] : FakeMul α := ⟨(· * ·)⟩
