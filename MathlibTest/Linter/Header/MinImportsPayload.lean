...
checks accept this file, so the `#guard_msgs` above expects no message. The `#exit` command
below then triggers the end-of-file report of the `minImports` linter, which positions the
"unneeded import" warning with the payload. -/
