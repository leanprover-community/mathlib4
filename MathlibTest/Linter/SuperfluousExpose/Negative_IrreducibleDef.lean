...
/-! Negative case: only `@[irreducible] def`s. The attribute stops the
elaborator from unfolding the def on its own, and downstream code can still
write `rw [opaqueZero]` or `unfold opaqueZero`, which read the body. The
