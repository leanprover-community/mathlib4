...
def downloadFiles (hashMap : IO.NameHashMap) (forceDownload : Bool) (parallel : Bool) :
    IO Unit := do
...
def getFiles (hashMap : IO.NameHashMap) (forceDownload forceUnpack parallel decompress : Bool) :
    IO.CacheM Unit := do
