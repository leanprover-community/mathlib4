...
`Upload.dest` (`Cache/Upload.lean`) builds it for the flat and the
developer-cache upload; the backends transfer to it. The prefixes build on
`fileDirPath` and `markerDirPath` (`Cache/Infra.lean`, `Cache/Marker.lean`),
the same policies the reads use, so every upload path follows the read-side
path contract.
-/
