...
def runCurl (args : Array String) (throwFailure := true) : IO String := do
  runCmdWithInput (← getCurl) args (throwFailure := throwFailure)

def validateCurl : IO Bool := do
  if (← CURLBIN.pathExists) then return true
  match (← runCmdWithInput "curl" #["--version"]).splitOn " " with
  | "curl" :: v :: _ => match v.splitOn "." with
    | maj :: min :: _ =>
      ...
      let arch ← (·.trim) <$> runCmdWithInput "uname" #["-m"] (throwFailure := false)
      let kernel ← (·.trim) <$> runCmdWithInput "uname" #["-s"] (throwFailure := false)
      if kernel == "Linux" && arch ∈ ["x86_64", "aarch64"] then
        IO.println s!"curl is too old; downloading more recent version"
        IO.FS.createDirAll IO.CACHEDIR
        let _ ← runCmdWithInput "curl" #[
        ...
        let _ ← runCmdWithInput "chmod" #["u+x", CURLBIN.toString]
        return true
...
def validateLeanTar : IO Unit := do
  ...
  if let some version ← some <$> runCmdWithInput "leantar" #["--version"] <|> pure none then
  ...
  let target ← if win then
    ...
    let mut arch ← (·.trim) <$> runCmdWithInput "uname" #["-m"] (throwFailure := false)
  ...
  let _ ← runCmdWithInput "tar" #["-xf", s!"{LEANTARBIN}.{ext}",
      ...
      if overwrite || !(← zipPath.pathExists) then
        tasks := tasks.push <| ← IO.asTask do
          ...
          let trace :: args := (← buildPaths.filterM (·.1.pathExists)) |>.map (·.1.toString)
            | unreachable!
          runCmdWithInput (← getLeanTar) <| #[zipPath.toString, trace] ++
...
def lookup (hashMap : HashMap) (paths : List FilePath) : IO Unit := do
  ...
  for path in paths do
    ...
    for line in (← runCmdWithInput (← getLeanTar) #["-k", ltar.toString]).splitOn "\n" |>.dropLast do
