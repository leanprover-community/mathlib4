...
-/
def mkBuildPaths (path : FilePath) : IO $ FilePath × List (FilePath × Bool) := do
...
def packCache (hashMap : HashMap) (overwrite : Bool) (comment : Option String := none) :
  ...
  for (path, hash) in hashMap.toList do
    ...
    let (root, buildPaths) ← mkBuildPaths path
      ...
      if overwrite || !(← zipPath.pathExists) then
        tasks := tasks.push <| ← IO.asTask do
          ...
          let trace :: args := (← buildPaths.filterM (·.1.pathExists)) |>.map (·.1.toString)
            | unreachable!
          runCmd (← getLeanTar) $ #["-C", s!"{root}", zipPath.toString, trace] ++
...
def unpackCache (hashMap : HashMap) (force : Bool) : IO Unit := do
  ...
  if size > 0 then
    ...
    let config : Array Lean.Json ← hashMap.foldM (init := #[]) fun config path hash =>
