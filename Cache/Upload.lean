...
def runPut [Monad m] [MonadLiftT IO m] (container? : Option Container)
  ...
  let dest ← uploadLocation backend container? (repo?.getD MATHLIBREPO) scope?
  match backend with
  | .azure =>
    let token ← getAzureAuth
    let fileNames ← getFileNames
    azurePutStaged dest token srcDir fileNames overwrite
  | .s3 =>
    let creds ← getS3Auth
    let region ← getS3Region
    let fileNames ← getFileNames
    s3PutStaged dest creds region srcDir fileNames overwrite
