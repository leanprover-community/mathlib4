...
=======
/-! ## Three definitions of the Langer graph are equivalent

The Langer graph has three independent definitions:
1. **Algebraic** — `langerSimpleGraph`, via the Zorn product on Q(6,2)
2. **Geometric** — `tutte12Distance2Graph`, distance-2 in the Tutte 12-cage
3. **Group-theoretic** — `langerCosetGraph`, the Sabidussi coset graph Sab(G₂(2), H₁₉₂, D)

The equivalence (1) ≃g (3) is `langerSabidussiIso` above.
The equivalence (2) ≃g (3) is `dist2SabidussiIso` below.
Composing gives (1) ≃g (2): `langer_iso_tutte12_distance2`.

The key: both graphs have the **same connection set D** at vertex 0
(`graphs_agree_at_zero`), so both map to the same coset graph. -/

/-- The **connection set** D at vertex 0, defined from the Langer graph.
This is the set of group elements mapping 0 to a Langer-neighbor. -/
noncomputable abbrev langerD : Set langerG :=
  connectionSet langerG langerSimpleGraph (0 : Fin 63)

/-- The **Sabidussi coset graph** Sab(G₂(2), H₁₉₂, D). -/
noncomputable abbrev langerCosetGraph :
    SimpleGraph (langerG ⧸ MulAction.stabilizer langerG (0 : Fin 63)) :=
  SimpleGraph.cosetGraph _ langerD (connectionSet.isConnectionSet 0)

/-- The distance-2 graph of the Tutte 12-cage restricted to points. -/
-- tutte12Distance2Bool iterates over 63 line vertices per pair, expensive for native_decide
set_option maxHeartbeats 800000 in
def tutte12Distance2Graph : SimpleGraph (Fin 63) where
  Adj p q := tutte12Distance2Bool p q
  symm p q := by unfold tutte12Distance2Bool; revert p q; native_decide
  loopless := ⟨fun p => by unfold tutte12Distance2Bool; revert p; native_decide⟩

instance tutte12Distance2DecAdj : DecidableRel tutte12Distance2Graph.Adj :=
  fun p q => inferInstanceAs (Decidable (tutte12Distance2Bool p q))

/-- G₂(2) preserves distance-2 adjacency. -/
noncomputable instance dist2GraphAction :
    GraphAction langerG (Fin 63) tutte12Distance2Graph where
  adj_smul := by
    intro ⟨σ, hσ⟩ u v hadj
    change tutte12Distance2Graph.Adj (σ u) (σ v)
    revert u v; change ∀ u v, tutte12Distance2Graph.Adj u v →
      tutte12Distance2Graph.Adj (σ u) (σ v)
    refine Subgroup.closure_induction
      (p := fun σ _ => ∀ u v, tutte12Distance2Graph.Adj u v →
        tutte12Distance2Graph.Adj (σ u) (σ v))
      ?_ ?_ ?_ ?_ hσ
    · intro x ⟨i, hi⟩; subst hi; fin_cases i <;> intro u v h
      · exact (show tutte12Distance2Bool u v → tutte12Distance2Bool (g2gen1 u) (g2gen1 v) from
          fun h => by rw [← g2gen1_dist2_inv]; exact h) h
      · exact (show tutte12Distance2Bool u v → tutte12Distance2Bool (g2gen2 u) (g2gen2 v) from
          fun h => by rw [← g2gen2_dist2_inv]; exact h) h
    · intro u v h; simpa
    · intro x y _ _ hx hy u v hadj
      simp only [Equiv.Perm.mul_apply]; exact hx _ _ (hy u v hadj)
    · intro x _ hx u v hadj
      let f : { p : Fin 63 × Fin 63 // tutte12Distance2Graph.Adj p.1 p.2 } →
              { p : Fin 63 × Fin 63 // tutte12Distance2Graph.Adj p.1 p.2 } :=
...
/-- The distance-2 graph is isomorphic to `langerCosetGraph` = Sab(G₂(2), H₁₉₂, D),
the **same** coset graph as the Langer graph.

We construct this directly using `sabidussiEquiv` (the orbit-stabilizer bijection,
which depends only on the group action) and prove `map_rel_iff'` by bridging through
`graphs_agree_at_zero`: membership in `langerD` ↔ Langer adjacency at 0 ↔ distance-2
adjacency at 0, then transport by `adj_smul_iff`. -/
noncomputable def dist2SabidussiIso :
    tutte12Distance2Graph ≃g langerCosetGraph where
  toEquiv := sabidussiEquiv (G := langerG) (0 : Fin 63)
  map_rel_iff' := by
    intro u w
    set g₁ := (MulAction.exists_smul_eq langerG (0 : Fin 63) u).choose
    set g₂ := (MulAction.exists_smul_eq langerG (0 : Fin 63) w).choose
    have hu : (g₁ : Equiv.Perm (Fin 63)) • (0 : Fin 63) = u :=
      (MulAction.exists_smul_eq langerG (0 : Fin 63) u).choose_spec
    have hw : (g₂ : Equiv.Perm (Fin 63)) • (0 : Fin 63) = w :=
      (MulAction.exists_smul_eq langerG (0 : Fin 63) w).choose_spec
    change (↑g₁⁻¹ * ↑g₂) ∈ langerD ↔ tutte12Distance2Graph.Adj u w
    rw [← hu, ← hw]
    simp only [langerD, connectionSet, Set.mem_setOf_eq, mul_smul]
    constructor
    · -- langerSimpleGraph.Adj 0 (g₁⁻¹g₂ • 0) → dist2.Adj (g₁•0) (g₂•0)
      intro h
      have h' : tutte12Distance2Graph.Adj (0 : Fin 63) ((↑g₁⁻¹ * ↑g₂) • (0 : Fin 63)) :=
        (graphs_agree_at_zero _).mp h
      rw [mul_smul] at h'
      exact (@GraphAction.adj_smul_iff langerG (Fin 63) _ _ tutte12Distance2Graph _
        g₁⁻¹ (g₁ • (0 : Fin 63)) (g₂ • (0 : Fin 63))).mp (by rwa [inv_smul_smul])
    · -- dist2.Adj (g₁•0) (g₂•0) → langerSimpleGraph.Adj 0 (g₁⁻¹g₂ • 0)
      intro h
      have h' := (@GraphAction.adj_smul_iff langerG (Fin 63) _ _ tutte12Distance2Graph _
        g₁⁻¹ (g₁ • (0 : Fin 63)) (g₂ • (0 : Fin 63))).mpr h
      rw [inv_smul_smul] at h'
      have h'' : langerSimpleGraph.Adj (0 : Fin 63) ((↑g₁⁻¹ * ↑g₂) • (0 : Fin 63)) :=
        (graphs_agree_at_zero _).mpr h'
      rwa [mul_smul] at h''

/-- **The three definitions of the Langer graph are equivalent.**

The algebraic definition (Zorn product) and the geometric definition
(distance-2 in the Tutte 12-cage) are isomorphic, proved via the Sabidussi
representation theorem. Both are isomorphic to the same coset graph
Sab(G₂(2), H₁₉₂, D), because they share the same connection set at vertex 0. -/
noncomputable def langer_iso_tutte12_distance2 :
    langerSimpleGraph ≃g tutte12Distance2Graph :=
  langerSabidussiIso.trans dist2SabidussiIso.symm
