...
lemma BAP_eq : ∠ s.B s.A s.P = π / 6 := by
  ...
  rw [← s.AP_bisect_BAC, ← two_mul, s.BAC_eq] at this; linarith [this]
...
lemma ABC_eq : ∠ s.A s.B s.C = 2 * s.x := by
  ...
  rw [s.ABQ_eq, s.QBC_eq, ← two_mul] at this; exact this.symm
...
lemma ACB_eq : ∠ s.A s.C s.B = 2 * π / 3 - 2 * s.x := by
  ...
  rw [angle_comm, s.ABC_eq, s.BAC_eq] at this; linarith
...
lemma x_eq : s.x = 2 * π / 9 := by
  ...
  rw [add_div, sub_div s.x, ← Real.two_mul_sin_mul_sin, show π / 3 / 2 = π / 6 by linarith,
  ...
  rw [one_div_mul_eq_div, div_eq_iff_mul_eq two_ne_zero, ← mul_rotate, Real.two_mul_sin_mul_sin,
    show s.x / 2 - (5 * π / 6 - 2 * s.x) = 5 * (s.x / 2) + π / 6 - π by linarith, Real.cos_sub_pi,
    show s.x / 2 + (5 * π / 6 - 2 * s.x) = π - (3 * (s.x / 2) + π / 6) by linarith, Real.cos_pi_sub,
    neg_sub_neg] at work
  ...
  have notleft : Real.cos (2 * y) * 2 ≠ 1 := by
    ...
    apply_fun Real.arccos at h
    rw [Real.arccos_cos s.x_pos.le (by bx),
      Real.arccos_cos (by positivity) (by linarith [Real.pi_pos])] at h
    exact s.x_lt_pi_div_three.ne h
  replace iden := iden.resolve_left notleft
  rw [← Real.cos_pi_div_two] at iden; apply_fun Real.arccos at iden
  rw [Real.arccos_cos (by unfold y; bx) (by unfold y; bx),
    Real.arccos_cos (by positivity) (by linarith [Real.pi_pos])] at iden
  linarith

end Setup

theorem result (s : Setup X) : ∠ s.A s.B s.C = 4 * π / 9 ∧ ∠ s.A s.C s.B = 2 * π / 9 := by
  rw [s.ABC_eq, s.ACB_eq, s.x_eq]; constructor <;> linarith
