...
theorem exists_between_and_le_dist (S : Finset ℝ) (n : Nat)
    (hS : #(S.filter (· ∈ Set.Ioo 0 1)) < n) :
    ∃ x : ℝ, 0 < x ∧ x < 1 ∧ ∀ y ∈ S, 1 / (2 * n) ≤ |x - y| := by
  have : n > 0 := by omega
  -- make `n` defEq to `_ + 1`
  ...
  let interval (i : Fin n) : Set ℝ :=
    Set.Ioo ((i : ℝ) / n) (((i : ℝ) + 1) / n)

  let rel (y : ℝ) (i : Fin n) : Prop :=
    y ∈ interval i

  by_cases h : ∀ i ∈ Finset.univ, ∃ y ∈ S.filter (· ∈ Set.Ioo 0 1), rel y i
...
theorem exists_affine_between_and_le_dist {S : Finset P} (n : Nat) (a b c d : P)
    (hS : #((S.filter (project a b · ∈ Set.Ioo 0 1))) < n)
    (hc : project a b c ≤ 0) (hd : 1 ≤ project a b d) (hab : a ≠ b) :
    ∃ l : AffineSubspace ℝ P, finrank ℝ l.direction = k ∧ l.SOppSide c d ∧
    ∀ p ∈ S, dist a b / (2 * n) ≤ Metric.infDist p l := by

  replace hS : #(filter (· ∈ Set.Ioo 0 1) (image (project a b) S)) < n := by
  ...
  obtain ⟨x, lt_x, x_lt, hx⟩ := exists_between_and_le_dist (image (project a b) S) n hS
  ...
  constructor
  · -- The subspace is strongly between `a` and `b`.
    let f := fun x : ℝ => project a b <| AffineMap.lineMap c d x
    have cont : Continuous f:= by
      unfold f project
      refine Continuous.div₀ ?_ ?_ fun _ => by positivity
      · refine Continuous.comp' ?_ ?_
        · fun_prop
        · refine Continuous.vsub ?_ ?_
          · fun_prop
          · apply AffineMap.continuous_of_finiteDimensional
      · fun_prop
    have : x ∈ Set.Ioo (f 0) (f 1) := by
      simp [f]
      constructor
      · linarith
      · linarith

    obtain ⟨v, hv, hf⟩ := intermediate_value_Ioo zero_le_one (f := f) cont.continuousOn this
    unfold f project at hf
    set w := (AffineMap.lineMap c d) v
    replace hf : w ∈
        AffineSubspace.mk' (AffineMap.lineMap a b x) (LinearMap.ker (innerₛₗ ℝ (a -ᵥ b))) := by
      rw [div_eq_iff (by positivity)] at hf; simp at hf ⊢
      rw [← add_right_inj (inner (a -ᵥ b) (a -ᵥ w))]
      simp [← inner_add_right, inner_smul_right, real_inner_self_eq_norm_sq]
      exact hf.symm
    refine Sbtw.sOppSide_of_not_mem_of_mem ?_ ?_ hf
    · simp [w, hv.1, hv.2]
      rintro rfl; linarith only [hc, hd]
    · unfold project at hc; rw [div_le_iff₀ (by positivity)] at hc
      simp at hc ⊢
      intro h
      replace h := add_le_add hc (le_of_eq h)
      simp [← inner_add_right, inner_smul_right, real_inner_self_eq_norm_sq] at h
      absurd h; push_neg
      positivity
  ...
  rw [← mul_le_mul_left this, mul_div, ← pow_two]
  trans; swap; · exact h
...
theorem result : ∃ c : ℝ, 0 < c ∧ ∀ {n : ℕ} (hn : 1 < n) {S : Finset P} (hSn : #S = n)
  ...
  · -- If there are points with distance at least `n^(2/3)`, then we can solve the problem by
    ...
    -- rw [dist_pos] at this
    -- obtain ⟨l, rank, sOpp, h⟩ := exists_affine_between_and_le_dist 1 (n*2) a b a b
    --   (lt_of_le_of_lt (card_filter_le S _) (by omega))
    --   (le_of_eq (by simp)) (le_of_eq (by simp (disch := assumption)))
    --   this
