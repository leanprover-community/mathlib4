...
\[
  \sqrt{\frac{x^2 + f(y)^2}{2}} \geq \frac{f(x) + y}{2}
    \geq \sqrt{x f(y)}
\]
for all `x, y ∈ ℝ₊`.
...
Write `d x = f x - x` for the displacement of `x`. Squaring the two inequalities in the
problem gives the key estimate
\[
  |d(x)-d(y)|\bigl(f(x)+y+x+f(y)\bigr) \leq (x-f(y))^2.
\]

Substituting `x = f y` into the original inequalities shows that `d (f y) = d y`. Consequently,
the iterates of `f` form an arithmetic progression:
\[
  f^{n}(x)=x+n d(x).
\]
Since all these iterates are positive, `d(x)` cannot be negative.

Next suppose that `a = d(x)` and `b = d(y)` are both positive but unequal. Choose a sufficiently
large iterate of `y`, and then choose an iterate of `x` immediately below its image using the
floor function. The left-hand side of the key estimate is then greater than `a^2`, while the
right-hand side is less than `a^2`, a contradiction. Thus all positive displacements have the
same value.
...
/-- The two inequalities in the problem control the difference between the displacements at two
positive inputs. -/
lemma displacement_control (hf : ∀ x > 0, 0 < f x) (h : Condition f) (hx : 0 < x) (hy : 0 < y) :
...
`f x - x`. -/
lemma iterate_eq_add_mul_displacement (hf : ∀ x > 0, 0 < f x) (h : Condition f)
...
lemma displacement_nonneg (hf : ∀ x > 0, 0 < f x) (h : Condition f) (hx : 0 < x) : 0 ≤ f x - x := by
  by_contra! hneg
...
lemma displacement_iterate_eq (hf : ∀ x > 0, 0 < f x) (h : Condition f)
...
lemma positive_displacements_eq (hf : ∀ x > 0, 0 < f x) (h : Condition f)
...
displacement. -/
lemma pos_displacement_le_dist_of_zero_displacement (hf : ∀ x > 0, 0 < f x) (h : Condition f)
...
lemma displacement_eq (hf : ∀ x > 0, 0 < f x) (h : Condition f) (hx : 0 < x) (hy : 0 < y) :
    f x - x = f y - y := by
  ...
  · obtain ⟨z, hz, hfz⟩ := hex
    have hzero_or_eq {t : ℝ} (ht : 0 < t) :
      ...
      grind [positive_displacements_eq hf h]
    set d : Set.Ioi (0 : ℝ) → ℝ := fun t ↦ f t - t
    have hnear (p q : Set.Ioi (0 : ℝ)) (hpq : dist p q < f z - z) : d p = d q := by
      have hpq_sep :=
        pos_displacement_le_dist_of_zero_displacement hf h p.property q.property hfz
      have hqp_sep :=
        pos_displacement_le_dist_of_zero_displacement hf h q.property p.property hfz
...
theorem imo2026_q5 (hf : ∀ x > 0, 0 < f x) : Condition f ↔ ∃ c ≥ 0, ∀ x > 0, f x = x + c := by
  constructor
  · intro hcond
