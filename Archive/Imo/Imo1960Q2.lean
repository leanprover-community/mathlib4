...
theorem isGood_iff {x} : IsGood x ↔ x ∈ Ico (-1/2) (45/8) \ {0} := by
  ...
  | inr hx2 =>
    ...
    have H : 1 - √(2 * x + 1) ≠ 0 := by
      ...
      simpa
    calc
      -- Note that the fraction in the LHS is equal to `(1 + √(2 * x + 1)) ^ 2`
      IsGood x ↔ (1 + √(2 * x + 1)) ^ 2 < 2 * x + 9 := by
        have : 4 * x ^ 2 = (1 + √(2 * x + 1)) ^ 2 * (1 - √(2 * x + 1)) ^ 2 := by
