...
lemma four_pow_four_pos : (0 : ℝ) < 4 ^ 4 := by norm_num
#align imo2006_q3.four_pow_four_pos Imo2006Q3.four_pow_four_pos

lemma mid_ineq {s t : ℝ} : s * t ^ 3 ≤ (3 * t + s) ^ 4 / 4 ^ 4 := by
...
lemma rhs_ineq {x y : ℝ} : 3 * (x + y) ^ 2 ≤ 2 * (x ^ 2 + y ^ 2 + (x + y) ^ 2) := by
...
lemma subst_wlog {x y z s : ℝ} (hxy : 0 ≤ x * y) (hxyz : x + y + z = 0) :
