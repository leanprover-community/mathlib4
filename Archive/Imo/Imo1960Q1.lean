...
-/
lemma not_zero {n : ℕ} (h1 : ProblemPredicate n) : n ≠ 0 :=
...
lemma ge_100 {n : ℕ} (h1 : ProblemPredicate n) : 100 ≤ n := by
...
lemma lt_1000 {n : ℕ} (h1 : ProblemPredicate n) : n < 1000 := by
  have h2 : n < 10 ^ 3 := by
    rw [← h1.left]
...
lemma searchUpTo_start : SearchUpTo 9 99 :=
  ⟨rfl, fun n h p => by linarith [ge_100 p]⟩
#align imo1960_q1.search_up_to_start Imo1960Q1.searchUpTo_start

lemma searchUpTo_step {c n} (H : SearchUpTo c n) {c' n'} (ec : c + 1 = c') (en : n + 11 = n') {l}
...
lemma searchUpTo_end {c} (H : SearchUpTo c 1001) {n : ℕ} (ppn : ProblemPredicate n) :
    SolutionPredicate n :=
  H.2 _ (by linarith [lt_1000 ppn]) ppn
#align imo1960_q1.search_up_to_end Imo1960Q1.searchUpTo_end

lemma right_direction {n : ℕ} : ProblemPredicate n → SolutionPredicate n := by
