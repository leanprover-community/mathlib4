...
/-- The primes in `(k, x]`.
-/
noncomputable def p (x k : ℕ) :=
  {p ∈ range (x + 1) | k < p ∧ Nat.Prime p}
...
noncomputable def u (x k : ℕ) :=
  Finset.biUnion (p x k) fun p => {e ∈ range x | p ∣ e + 1}
...
noncomputable def m (x k : ℕ) :=
  {e ∈ range x | ∀ p : ℕ, Nat.Prime p ∧ p ∣ e + 1 → p ≤ k}
...
theorem sum_lt_half_of_not_tendsto
  ...
  refine' (h _ _).2
  rw [disjoint_iff_ne]
  simp_intro a ha b hb only [mem_filter]
...
#align theorems_100.range_sdiff_eq_bUnion Theorems100.range_sdiff_eq_bUnion
...
theorem card_le_two_pow {x k : ℕ} : card ({e ∈ m x k | Squarefree (e + 1)}) ≤ 2 ^ k := by
  ...
  have h : M₁ ⊆ image f K := by
    ...
    · suffices p ∈ (m + 1).factors → ∃ a : ℕ, a < k ∧ a.succ = p by simpa
      simp_intro hp only [Nat.mem_factors m.succ_ne_zero]
      exact
...
theorem card_le_two_pow_mul_sqrt {x k : ℕ} : card (m x k) ≤ 2 ^ k * Nat.sqrt x := by
  ...
  have h1 : M x k ⊆ image f K := by
    ...
    refine'
      ⟨a, b, ⟨⟨⟨_, fun p hp => _⟩, hab₂⟩, ⟨_, fun p hp => _⟩⟩, by simp_rw [f, hab₁, m.succ_sub_one]⟩
