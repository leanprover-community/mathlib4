 ...
 * `galActionHom_bijective_of_prime_degree'` in `field_theory/polynomial_galois_group` :
...
theorem degree_Phi : (Φ R a b).degree = ((5 : ℕ) : WithBot ℕ) := by
  suffices degree (X ^ 5 - C (a : R) * X) = ((5 : ℕ) : WithBot ℕ) by
    ...
    convert (degree_C_le (R := R)).trans_lt (WithBot.coe_lt_coe.mpr (Nat.zero_lt_bit1 2))
  rw [degree_sub_eq_left_of_degree_lt] <;> rw [degree_X_pow]
  exact (degree_C_mul_X_le (a : R)).trans_lt (WithBot.coe_lt_coe.mpr (Nat.one_lt_bit1 two_ne_zero))
...
theorem irreducible_Phi (p : ℕ) (hp : p.Prime) (hpa : p ∣ a) (hpb : p ∣ b) (hp2b : ¬p ^ 2 ∣ b) :
  ...
  · intro n hn
    ...
    rw [degree_Phi, Nat.cast_withBot, Nat.cast_withBot, WithBot.coe_lt_coe] at hn
    interval_cases hn : n <;>
...
theorem real_roots_Phi_le : Fintype.card ((Φ ℚ a b).rootSet ℝ) ≤ 3 := by
  ...
  refine'
    (card_rootSet_le_derivative _).trans
      (Nat.succ_le_succ ((card_rootSet_le_derivative _).trans (Nat.succ_le_succ _)))
  let _x := rootSet (C ((algebraMap ℤ ℚ) 20) * X ^ 3 : ℚ[X]) ℝ
  suffices (rootSet (C ((algebraMap ℤ ℚ) 20) * X ^ 3 : ℚ[X]) ℝ).Subsingleton by
    -- Porting note: `norm_num` ignores arguments
    simp [Fintype.card_le_one_iff_subsingleton, ← mul_assoc, *] at *
    norm_num
    exact this
...
theorem real_roots_Phi_ge_aux (hab : b < a) :
  ...
  have hf0 : 0 ≤ f 0 := by rw [hf]; norm_num
  by_cases hb : (1 : ℝ) - a + b < 0
  · have hf1 : f 1 < 0 := by simp [hf, hb]
    have hfa : 0 ≤ f a := by
      -- Porting note: was part of `simp_rw`
      rw [hf]
      simp_rw [← sq]
...
theorem exists_not_solvable_by_rad : ∃ x : ℂ, IsAlgebraic ℚ x ∧ ¬IsSolvableByRad ℚ x := by
  obtain ⟨x, hx⟩ :=
    exists_root_of_splits (algebraMap ℚ ℂ) (IsAlgClosed.splits_codomain (Φ ℚ 4 2))
      (ne_of_eq_of_ne (degree_Phi 4 2) (mt WithBot.coe_eq_coe.mp (Nat.bit1_ne_zero 2)))
