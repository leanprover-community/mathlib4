...
theorem mem_countedSequence_iff_perm {p q l} :
  ...
  · simp only [countedSequence, List.subset_def, mem_ofPred, List.mem_cons (b := (1 : ℤ)),
      List.mem_singleton]
  · norm_num1
...
theorem first_vote_pos :
  ...
  | p + 1, q + 1, _ => by
    ...
    · have : List.cons (-1) '' countedSequence (p + 1) q ∩ {l : List ℤ | l.headI = 1} = ∅ := by
        ext
        simp only [mem_inter_iff, mem_image, mem_ofPred, mem_empty_iff_false, iff_false,
...
theorem countedSequence_int_pos_counted_succ_succ (p q : ℕ) :
  ...
  rw [counted_succ_succ, union_inter_distrib_right,
    ...
    · ext
      simp only [mem_inter_iff, mem_image, mem_ofPred, and_iff_left_iff_imp, mem_empty_iff_false,
