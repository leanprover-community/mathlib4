...
#align theorems_100.«82».Ico_lemma Theorems100.Ico_lemma
...
structure Cube (n : ℕ) : Type where
  ...
  w : ℝ -- width
  hw : 0 < w
#align theorems_100.«82».cube Theorems100.Cube

namespace Cube

theorem hw' (c : Cube n) : 0 ≤ c.w :=
  le_of_lt c.hw
#align theorems_100.«82».cube.hw' Theorems100.Cube.hw'

/-- The j-th side of a cube is the half-open interval `[b j, b j + w)` -/
def side (c : Cube n) (j : Fin n) : Set ℝ :=
  Ico (c.b j) (c.b j + c.w)
#align theorems_100.«82».cube.side Theorems100.Cube.side

@[simp]
theorem b_mem_side (c : Cube n) (j : Fin n) : c.b j ∈ c.side j := by simp [side, Cube.hw, le_refl]
#align theorems_100.«82».cube.b_mem_side Theorems100.Cube.b_mem_side

def toSet (c : Cube n) : Set (Fin n → ℝ) :=
  {x | ∀ j, x j ∈ side c j}
#align theorems_100.«82».cube.to_set Theorems100.Cube.toSet

theorem side_nonempty (c : Cube n) (i : Fin n) : (side c i).Nonempty := by simp [side, c.hw]
#align theorems_100.«82».cube.side_nonempty Theorems100.Cube.side_nonempty

theorem univ_pi_side (c : Cube n) : pi univ (side c) = c.toSet :=
  ext fun _ => mem_univ_pi
#align theorems_100.«82».cube.univ_pi_side Theorems100.Cube.univ_pi_side
...
theorem toSet_subset {c c' : Cube n} : c.toSet ⊆ c'.toSet ↔ ∀ j, c.side j ⊆ c'.side j := by
  simp only [← univ_pi_side, univ_pi_subset_univ_pi_iff, (c.side_nonempty _).ne_empty, exists_false,
    or_false_iff]
#align theorems_100.«82».cube.to_set_subset Theorems100.Cube.toSet_subset
...
#align theorems_100.«82».cube.to_set_disjoint Theorems100.Cube.toSet_disjoint

theorem b_mem_toSet (c : Cube n) : c.b ∈ c.toSet := by simp [toSet]
#align theorems_100.«82».cube.b_mem_to_set Theorems100.Cube.b_mem_toSet

protected def tail (c : Cube (n + 1)) : Cube n :=
  ⟨tail c.b, c.w, c.hw⟩
#align theorems_100.«82».cube.tail Theorems100.Cube.tail

theorem side_tail (c : Cube (n + 1)) (j : Fin n) : c.tail.side j = c.side j.succ :=
  rfl
#align theorems_100.«82».cube.side_tail Theorems100.Cube.side_tail

def bottom (c : Cube (n + 1)) : Set (Fin (n + 1) → ℝ) :=
  {x | x 0 = c.b 0 ∧ tail x ∈ c.tail.toSet}
#align theorems_100.«82».cube.bottom Theorems100.Cube.bottom

theorem b_mem_bottom (c : Cube (n + 1)) : c.b ∈ c.bottom := by
  simp [bottom, toSet, side, Cube.hw, le_refl, Cube.tail]
#align theorems_100.«82».cube.b_mem_bottom Theorems100.Cube.b_mem_bottom

def xm (c : Cube (n + 1)) : ℝ :=
  c.b 0 + c.w
#align theorems_100.«82».cube.xm Theorems100.Cube.xm

theorem b_lt_xm (c : Cube (n + 1)) : c.b 0 < c.xm := by simp [xm, hw]
#align theorems_100.«82».cube.b_lt_xm Theorems100.Cube.b_lt_xm

theorem b_ne_xm (c : Cube (n + 1)) : c.b 0 ≠ c.xm :=
  ne_of_lt c.b_lt_xm
#align theorems_100.«82».cube.b_ne_xm Theorems100.Cube.b_ne_xm

def shiftUp (c : Cube (n + 1)) : Cube (n + 1) :=
  ⟨cons c.xm <| tail c.b, c.w, c.hw⟩
#align theorems_100.«82».cube.shift_up Theorems100.Cube.shiftUp

@[simp]
theorem tail_shiftUp (c : Cube (n + 1)) : c.shiftUp.tail = c.tail := by simp [shiftUp, Cube.tail]
#align theorems_100.«82».cube.tail_shift_up Theorems100.Cube.tail_shiftUp

@[simp]
theorem head_shiftUp (c : Cube (n + 1)) : c.shiftUp.b 0 = c.xm :=
  rfl
#align theorems_100.«82».cube.head_shift_up Theorems100.Cube.head_shiftUp

def unitCube : Cube n :=
  ⟨fun _ => 0, 1, by norm_num⟩
#align theorems_100.«82».cube.unit_cube Theorems100.Cube.unitCube

@[simp]
theorem side_unitCube {j : Fin n} : unitCube.side j = Ico 0 1 := by
  norm_num [unitCube, side]
#align theorems_100.«82».cube.side_unit_cube Theorems100.Cube.side_unitCube
...
structure Correct (cs : ι → Cube n) : Prop where
  ...
  three_le : 3 ≤ n
#align theorems_100.«82».correct Theorems100.Correct
...
#align theorems_100.«82».correct.to_set_subset_unit_cube Theorems100.Correct.toSet_subset_unitCube

theorem side_subset {i j} : (cs i).side j ⊆ Ico 0 1 := by
  simpa only [side_unitCube] using toSet_subset.1 h.toSet_subset_unitCube j
#align theorems_100.«82».correct.side_subset Theorems100.Correct.side_subset

theorem zero_le_of_mem_side {i j x} (hx : x ∈ (cs i).side j) : 0 ≤ x :=
  (side_subset h hx).1
#align theorems_100.«82».correct.zero_le_of_mem_side Theorems100.Correct.zero_le_of_mem_side

theorem zero_le_of_mem {i p} (hp : p ∈ (cs i).toSet) (j) : 0 ≤ p j :=
  zero_le_of_mem_side h (hp j)
#align theorems_100.«82».correct.zero_le_of_mem Theorems100.Correct.zero_le_of_mem

theorem zero_le_b {i j} : 0 ≤ (cs i).b j :=
  zero_le_of_mem h (cs i).b_mem_toSet j
#align theorems_100.«82».correct.zero_le_b Theorems100.Correct.zero_le_b
...
theorem b_add_w_le_one {j} : (cs i).b j + (cs i).w ≤ 1 := by
  ...
  convert this.2
  simp [hw]
#align theorems_100.«82».correct.b_add_w_le_one Theorems100.Correct.b_add_w_le_one

theorem nontrivial_fin : Nontrivial (Fin n) :=
  Fin.nontrivial_iff_two_le.2 (Nat.le_of_succ_le_succ h.three_le)
#align theorems_100.«82».correct.nontrivial_fin Theorems100.Correct.nontrivial_fin
...
#align theorems_100.«82».correct.w_ne_one Theorems100.Correct.w_ne_one
...
#align theorems_100.«82».correct.shift_up_bottom_subset_bottoms Theorems100.Correct.shiftUp_bottom_subset_bottoms
...
#align theorems_100.«82».valley Theorems100.Valley
...
#align theorems_100.«82».valley_unit_cube Theorems100.valley_unitCube
...
#align theorems_100.«82».bcubes Theorems100.bcubes
...
theorem t_le_t (hi : i ∈ bcubes cs c) (j : Fin n) :
  ...
  exact h'.2; simp [hw]
#align theorems_100.«82».t_le_t Theorems100.t_le_t
...
#align theorems_100.«82».w_lt_w Theorems100.w_lt_w
...
theorem nontrivial_bcubes : (bcubes cs c).Nontrivial := by
  ...
  · exact (hi.2 _).1
  rfl
#align theorems_100.«82».nontrivial_bcubes Theorems100.nontrivial_bcubes

/-- There is a cube in the valley -/
theorem nonempty_bcubes : (bcubes cs c).Nonempty :=
  (nontrivial_bcubes h v).nonempty
#align theorems_100.«82».nonempty_bcubes Theorems100.nonempty_bcubes
...
#align theorems_100.«82».mi_xm_ne_one Theorems100.mi_xm_ne_one
...
#align theorems_100.«82».smallest_on_boundary Theorems100.smallest_onBoundary
...
#align theorems_100.«82».mi_not_on_boundary Theorems100.mi_not_onBoundary
...
#align theorems_100.«82».valley_mi Theorems100.valley_mi
...
noncomputable def sequenceOfCubes : ℕ → { i : ι // Valley cs (cs i).shiftUp }
  ...
  | k + 1 =>
    ...
    ⟨mi h v, valley_mi⟩
#align theorems_100.«82».sequence_of_cubes Theorems100.sequenceOfCubes

def decreasingSequence (k : ℕ) : ℝ :=
  (cs (sequenceOfCubes h k).1).w
#align theorems_100.«82».decreasing_sequence Theorems100.decreasingSequence
...
#align theorems_100.«82».strict_anti_sequence_of_cubes Theorems100.strictAnti_sequenceOfCubes
...
#align theorems_100.«82».injective_sequence_of_cubes Theorems100.injective_sequenceOfCubes
...
#align theorems_100.«82».not_correct Theorems100.not_correct
...
#align theorems_100.«82».cannot_cube_a_cube Theorems100.cannot_cube_a_cube
