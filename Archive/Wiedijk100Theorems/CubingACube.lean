...
theorem Ico_lemma {α} [LinearOrder α] {x₁ x₂ y₁ y₂ z₁ z₂ w : α} (h₁ : x₁ < y₁) (hy : y₁ < y₂)
  ...
  · simp +contextual [hw, lt_irrefl, not_ge_of_lt h₁]
...
theorem shiftUp_bottom_subset_bottoms (hc : (cs i).xm ≠ 1) :
  ...
  have : i ≠ i' := by rintro rfl; apply not_ge_of_lt (hi' 0).2; rw [hp0]; rfl
  have := h.1 this
...
theorem nontrivial_bcubes : (bcubes cs c).Nontrivial := by
  ...
  rintro rfl
  apply not_ge_of_lt (hi'.2 ⟨1, Nat.le_of_succ_le_succ h.three_le⟩).2
