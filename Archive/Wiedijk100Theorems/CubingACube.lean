...
theorem shiftUp_bottom_subset_bottoms (hc : (cs i).xm ≠ 1) :
  ...
  have : p ∈ (unitCube : Cube (n + 1)).toSet := by
    simp only [toSet, forall_iff_succ, hp0, side_unitCube, mem_ofPred, mem_Ico, head_shiftUp]
    refine ⟨⟨?_, ?_⟩, ?_⟩
...
theorem valley_unitCube [Nontrivial ι] (h : Correct cs) : Valley cs unitCube := by
  refine ⟨?_, ?_, ?_⟩
  · intro v
    simp only [bottom, and_imp, mem_iUnion, mem_ofPred]
    intro h0 hv
    have : v ∈ (unitCube : Cube (n + 1)).toSet := by
      dsimp only [toSet, unitCube, mem_ofPred]
...
theorem mi_not_onBoundary (j : Fin n) : ¬OnBoundary (mi_mem_bcubes : mi h v ∈ _) j := by
  ...
  have i'_i'' : i' ≠ i'' := by
    ...
    have : (cs i).b ∈ (cs i').toSet := by
      simp only [toSet, forall_iff_succ, hi.1, bottom_mem_side h2i', true_and, mem_ofPred]
...
theorem valley_mi : Valley cs (cs (mi h v)).shiftUp := by
  ...
  · rintro i' hi' ⟨p2, hp2, h2p2⟩; simp only [head_shiftUp] at hi'
    ...
    have : ∃ p3, p3 ∈ (cs i').tail.toSet ∧ p3 ∉ (cs i).tail.toSet ∧ p3 ∈ c.tail.toSet := by
      simp only [toSet, not_forall, mem_ofPred] at h2p1; obtain ⟨j, hj⟩ := h2p1
      ...
      · simp only [toSet, not_forall, mem_ofPred]; use j; rw [ite_eq_left rfl]; convert! h2w
      · intro j'; by_cases h : j' = j
    ...
    have h2p' : p' ∈ (cs i'').toSet := by
      simp only [p', toSet, forall_iff_succ, cons_succ, cons_zero, mem_ofPred]
