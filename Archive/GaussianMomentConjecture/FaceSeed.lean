...
theorem chargesStraddleZero_subtype
    (F : Finset (Fin 2 →₀ ℕ))
    (hF : GMC2LowestFaceExistence.ChargesStraddleZero F GMC2.chargeQ) :
    GMC2LowestFaceExistence.ChargesStraddleZero
...
theorem exists_nonzero_lowest_face_seed
  ...
  have hfaceTilted : ∀ s ∈ F,
      GMC2FrobeniusFace.tiltedHeight
        GMC2FaceDictionary.exponentA GMC2FaceDictionary.exponentB lambda s = delta := by
    intro s hs
    rw [GMC2FaceDictionary.tiltedHeight_eq]
    exact (hface s).mp hs |>.2
  have hchargeInjective :
      Function.Injective (fun s : ↥F => GMC2.charge s) := by
    simpa only [GMC2FaceDictionary.charge_eq] using
      (GMC2FrobeniusFace.charge_injective_on_face
  ...
  obtain ⟨m0, hm0, hseed⟩ :=
    GMC2DvdKInterface.exists_nonzero_face_seed hDvdK ↥F
