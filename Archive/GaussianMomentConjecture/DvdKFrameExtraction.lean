...
`w = Rl·x⁻ᴹ` (`x⁻ᴹ = HahnSeries.single (−M) 1`).  Built from `Φ = C(xᴹ)·(1 − C(w)·X)` and `PowerSeries.inverse_one_sub_C_mul_X`. -/
...
/-- **The frame generating function is the moment series.**  Composing `xCoeff0_xM_div_PhiFrame` (`xᴹ/Φ = 1 + t·(R/Φ)`) with leg (c), the frame `F := xCoeff0(xᴹ/Φ)` equals
