...
namespace GMC2.FaceHeightFloor
...
def weightedTiltedHeightQ
  ...
  ∑ i ∈ F, (r i : ℚ) * GMC2.FrobeniusFace.tiltedHeight a b lambda i

/-- Rational mass is the cast of the natural channel mass. -/
theorem massQ_eq_natCast_channelMass
    (F : Finset ι) (r : ι → ℕ) :
    GMC2.FrobeniusFace.massQ F r = (channelMass F r : ℚ) := by
...
theorem weightedTiltedHeightQ_eq_radial_sub_charge [DecidableEq ι]
    ...
    weightedTiltedHeightQ a b lambda F r =
      GMC2.FrobeniusFace.radialHeightQ a F r -
        lambda * GMC2.FrobeniusFace.totalChargeQ a b F r := by
  calc
    weightedTiltedHeightQ a b lambda F r =
        ∑ i ∈ F,
          ((r i : ℚ) * (a i : ℚ) -
            lambda * ((r i : ℚ) * (GMC2.FrobeniusFace.charge a b i : ℚ))) := by
      apply Finset.sum_congr rfl
      intro i hi
      simp only [GMC2.FrobeniusFace.tiltedHeight]
      ring
    _ = GMC2.FrobeniusFace.radialHeightQ a F r -
        lambda * GMC2.FrobeniusFace.totalChargeQ a b F r := by
      simp [GMC2.FrobeniusFace.radialHeightQ,
        GMC2.FrobeniusFace.totalChargeQ, Finset.sum_sub_distrib,
        Finset.mul_sum]
...
theorem balanced_radialHeight_floor [DecidableEq ι]
    ...
    (hbalanced : GMC2.FrobeniusFace.totalChargeQ a b F r = 0) :
    delta * GMC2.FrobeniusFace.massQ F r ≤
      GMC2.FrobeniusFace.radialHeightQ a F r := by
  calc
    delta * GMC2.FrobeniusFace.massQ F r =
        ∑ i ∈ F, delta * (r i : ℚ) := by
      simp [GMC2.FrobeniusFace.massQ, Finset.mul_sum]
    ...
    _ = GMC2.FrobeniusFace.radialHeightQ a F r -
        lambda * GMC2.FrobeniusFace.totalChargeQ a b F r :=
      weightedTiltedHeightQ_eq_radial_sub_charge a b lambda F r
    _ = GMC2.FrobeniusFace.radialHeightQ a F r := by
      rw [hbalanced]
      ring
...
theorem balanced_radialHeight_floor_of_mass [DecidableEq ι]
    ...
    (m : ℚ) * delta ≤ GMC2.FrobeniusFace.radialHeightQ a F r := by
...
theorem balanced_radialHeight_eq_of_supported_on_face [DecidableEq ι]
    ...
    (hbalanced : GMC2.FrobeniusFace.totalChargeQ a b F r = 0) :
    GMC2.FrobeniusFace.radialHeightQ a F r =
      delta * GMC2.FrobeniusFace.massQ F r := by
    ...
    calc
      weightedTiltedHeightQ a b lambda F r =
        ...
        · rw [hsupport i hi hri]
          ring
      _ = delta * GMC2.FrobeniusFace.massQ F r := by
        simp [GMC2.FrobeniusFace.massQ, Finset.mul_sum]
  calc
    GMC2.FrobeniusFace.radialHeightQ a F r =
        weightedTiltedHeightQ a b lambda F r +
...
theorem balanced_radialHeight_eq_floor_of_mass [DecidableEq ι]
    ...
    (hbalanced : GMC2.FrobeniusFace.totalChargeQ a b F r = 0)
    (hmass : channelMass F r = m) :
    GMC2.FrobeniusFace.radialHeightQ a F r = (m : ℚ) * delta := by
...
theorem balanced_natural_height_floor_of_reference [DecidableEq ι]
    ...
    (hrefSupport : ∀ i ∈ F, r0 i ≠ 0 →
      GMC2.FrobeniusFace.tiltedHeight a b lambda i = delta)
    (hrefBalanced : GMC2.FrobeniusFace.totalChargeQ a b F r0 = 0)
    (hrefMass : channelMass F r0 = m0)
    (hrefHeight : GMC2.FrobeniusFace.radialHeightQ a F r0 = (A0 : ℚ))
    (hbalanced : GMC2.FrobeniusFace.totalChargeQ a b F r = 0)
    (hmass : channelMass F r = p * m0)
    (hheight : GMC2.FrobeniusFace.radialHeightQ a F r = (A : ℚ)) :
    p * A0 ≤ A := by
...
theorem off_face_base_channel_natural_height_gap [DecidableEq ι]
    ...
    (hstrict : delta < GMC2.FrobeniusFace.tiltedHeight a b lambda j) :
    A0 + 1 ≤ A := by
  have href := balanced_radialHeight_eq_floor_of_mass
    a b lambda delta F r0 m0 hrefSupport hrefBalanced hrefMass
  have hoff := GMC2.FrobeniusFace.balanced_height_strict_of_off_face
    a b lambda delta F r hlower hbalanced j hj hrj hstrict
  have hstrictQ : (A0 : ℚ) < (A : ℚ) := by
    calc
      (A0 : ℚ) = GMC2.FrobeniusFace.radialHeightQ a F r0 := hrefHeight.symm
      _ = (m0 : ℚ) * delta := href
      _ = delta * GMC2.FrobeniusFace.massQ F r := by
        rw [massQ_eq_natCast_channelMass, hmass]
        ring
      _ < GMC2.FrobeniusFace.radialHeightQ a F r := hoff
      _ = (A : ℚ) := hheight
