...
`Pfr, hfr`.  Through transpose `phi = map(ofPowerSeries) ∘ tauHom`, the Weierstrass unit's
...
* `GMC2DvdKFrameHSide.xCoeff0_logDeriv_map_ofPowerSeries` (the `h`-side, leg (a)), and
* `GMC2DvdKFrameExtraction.xCoeff0_xM_div_PhiFrame_eq_one_of_vanish` (the `F = D_m` / `F = 1` extraction),
...
the analytic lane.  When those land, `hderiv` (and hence GMC(2)) closes with no new analysis.
-/
...
theorem hderiv_via_transpose (Rl : LaurentSeries F) (M : ℕ)
    ...
    (hPu : IsUnit Pfr)
    (hc : GMC2DvdKFrame.xCoeff0 (PowerSeries.logDeriv Pfr) = 0)
    (hg : IsUnit (GMC2DvdKFrame.xCoeff0 (GMC2DvdKTranspose.phi Wu)))
    (hvanish : ∀ m : ℕ, 1 ≤ m → (Rl ^ m).coeff ((M : ℤ) * m) = 0) :
    PowerSeries.derivative _ (GMC2DvdKFrame.xCoeff0 (GMC2DvdKTranspose.phi Wu)) = 0 := by
...
frame's constant-in-`x` coefficient equals Weierstrass `h(0,t)`), `hderiv_via_transpose` becomes
...
/-- **`hfact` for the concrete Weierstrass objects** — discharged by composing connector
