...
* `PowerSeries.map_logDeriv` (general, reusable — Mathlib gap): `map ψ (logDeriv u) = logDeriv (map ψ u)` for any ring
  hom `ψ` and unit `u`, from `map_derivative` + ring homs preserving unit inverses.
...
series, e.g. `1 + t(x+x⁻¹)`).  The Weierstrass unit lands here, so the h-side is `PowerSeries.map_logDeriv` twice. -/
...
theorem xCoeff0_logDeriv_map_ofPowerSeries {H : PowerSeries (PowerSeries F)} (hH : IsUnit H) :
    GMC2DvdKFrame.xCoeff0 (PowerSeries.logDeriv (PowerSeries.map (HahnSeries.ofPowerSeries ℤ F) H))
      = derivative _ (GMC2DvdKFrame.xCoeff0 (PowerSeries.map (HahnSeries.ofPowerSeries ℤ F) H))
