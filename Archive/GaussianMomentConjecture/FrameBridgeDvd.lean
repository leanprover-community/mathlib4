...
mapped to the algebraic closure of `LaurentSeries F`, the small-root packet is a sub-multiset of
`Φ`'s roots). This is *not* the trivial power-series divisibility over the fraction field (where the
distinguished factor becomes a unit); it is the genuine polynomial factorization, obtained from
Mathlib's Weierstrass **division uniqueness** (`IsWeierstrassDivisorAt.eq_of_mul_add_eq_mul_add`)
applied to the two divisions of `Φ` by the monic `smallRootFactor`: the Weierstrass factorization
`Φ = ↑P·h` (remainder `0`) and the ordinary polynomial division (remainder `Φ %ₘ P`). Uniqueness
forces `Φ %ₘ P = 0`. No valuation on the algebraic closure.
-/

open Polynomial

namespace GMC2.FrameBridgeDvd

variable {F : Type*} [Field F]

/-- The polynomial (over `PowerSeries F`) whose coercion into `(PowerSeries F)⟦X⟧` is Weierstrass
`Φ = xᴹ − t·R`. -/
...
theorem coe_PhiPoly (R : Polynomial F) (M : ℕ) :
    ((PhiPoly R M : Polynomial (PowerSeries F)) : PowerSeries (PowerSeries F))
      = GMC2.DvdKWeierstrass.Phi R M := by
  rw [PhiPoly, GMC2.DvdKWeierstrass.Phi]
  push_cast
  ring

/-- **The bridge divisibility.** The Weierstrass distinguished factor `smallRootFactor R M` divides
`Φ = PhiPoly R M` in the *polynomial* ring `(PowerSeries F)[X]`. Proof: the Weierstrass
factorization `↑Φ = ↑P·h` and the ordinary monic division `Φ = P·(Φ/ₘP) + (Φ%ₘP)` are two
Weierstrass divisions of `↑Φ` by `↑P`; uniqueness
(`IsWeierstrassDivisorAt.eq_of_mul_add_eq_mul_add`, `F[[t]]` Hausdorff) forces `Φ %ₘ P = 0`. -/
theorem smallRootFactor_dvd_PhiPoly (R : Polynomial F) (M : ℕ) :
    GMC2.DvdKWeierstrass.smallRootFactor R M ∣ PhiPoly R M := by
  set P := GMC2.DvdKWeierstrass.smallRootFactor R M with hPdef
  have hPmonic : P.Monic := GMC2.DvdKWeierstrass.smallRootFactor_monic R M
  have hPnat : P.natDegree = M := GMC2.DvdKWeierstrass.smallRootFactor_natDegree R M
  have hPdist : P.IsDistinguishedAt (IsLocalRing.maximalIdeal (PowerSeries F)) :=
    PowerSeries.isDistinguishedAt_weierstrassDistinguished
      (GMC2.DvdKWeierstrass.phi_residue_ne_zero R M)
  ...
  obtain ⟨h, -, hfact⟩ := GMC2.DvdKWeierstrass.phi_eq_smallRootFactor_mul R M
