...
where (the small-root product identity) the small-root product is `Π = c·t` (`c ≠ 0`) and (Vieta) `C_Φ = (−1)^d r_0/r_d ∈ F`
...
the small-root product identity (that `Π = c·t`), the unramified-Hensel gap.
-/
