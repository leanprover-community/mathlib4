...
To instantiate the abstract orbit-product wrapper (`OrbitProductWrapper`) at an actual polynomial,
we need the Galois group `L ≃ₐ[K] L` acting on the roots of an irreducible `Φ ∈ K[X]` (splitting in
a normal `L`), **transitively**, with the root inclusion **equivariant**. Using the *direct* action
