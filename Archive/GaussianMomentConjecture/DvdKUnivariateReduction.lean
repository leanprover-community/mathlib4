...
`GMC2.DvdKZeroCharge.DvdK1BothSigns` (a statement about the universal constant-term relations of an
...
/-- **The single-polynomial crux (`= the small-root product identity = h(0,t) = 1`).** For
`Φ = Phi R M = Xᴹ − t·R` with `1 ≤ M < deg R` and `R(0) ≠ 0`, if every central power coefficient
`Dₘ = (Rᵐ).coeff (M·m)` (`m ≥ 1`) vanishes, then the small-root packet product is `c·t` for some
`c ≠ 0`. This is the sole deep analytic input — the Weierstrass small-root/log-derivative identity,
the analytic lane. -/
def SinglePolyCrux : Prop :=
  ∀ (R : Polynomial ℂ) (M : ℕ), 1 ≤ M → M < R.natDegree → R.coeff 0 ≠ 0 →
      ...
      ∃ (S : Finset ((GMC2.PhiVieta.Phi R M).rootSet (GMC2.PhiVieta.Phi R M).SplittingField))
        (_x0 : (GMC2.PhiVieta.Phi R M).rootSet (GMC2.PhiVieta.Phi R M).SplittingField)
        (cc : ℂ), cc ≠ 0 ∧
        (∏ β ∈ S, (β : (GMC2.PhiVieta.Phi R M).SplittingField))
          = algebraMap (RatFunc ℂ) (GMC2.PhiVieta.Phi R M).SplittingField
...
of every central power coefficient `Dₘ = (Rᵐ).coeff (M·m)`, `m ≥ 1`, produces a small-root packet
whose product is `c·t`), the one-variable Duistermaat–van der Kallen input holds on every both-signs
support.
...
theorem dvdK1_bothSigns_of_crux (crux : SinglePolyCrux) :
  ...
  have hcon' : ∀ m : ℕ, 1 ≤ m →
      MvPolynomial.aeval c (GMC2.ConstantTermRelations.constantTermRelation q m) = 0 := by
    intro m hm
    by_contra hmne
...
theorem dvdK1_of_crux (crux : SinglePolyCrux) : GMC2.DvdKInterface.DvdK1 :=
  GMC2.DvdKZeroCharge.dvdK1_of_bothSigns (dvdK1_bothSigns_of_crux crux)

/-- **NC2 from the single-polynomial crux.**  Composing with `GMC2.NC2.nc2_of_dvdK1` (the height
...
theorem nc2_of_crux (crux : SinglePolyCrux) : GMC2.NC2 :=
  GMC2.NC2.nc2_of_dvdK1 (dvdK1_of_crux crux)

/-- **GMC(2) from the single-polynomial crux — the capstone.** Composing with
`GMC2.NC2.gmc2_of_dvdK1`: the full GMC(2) two-variable nullcone conclusion — for `P, Q` over
`ℂ[x,y]` with `E(Pᵐ) = 0` for all `m ≥ 1`, eventually `E(Q·Pᵐ) = 0` — holds modulo **exactly** the
single deep analytic input `SinglePolyCrux` (`= h(0,t) = 1`). Every other step is kernel-pure. -/
