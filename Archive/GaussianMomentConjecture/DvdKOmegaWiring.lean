...
The `Algebra (RatFunc ℂ) (LaurentSeries ℂ)` instance is non-synthesizable, so it is provided locally
via `rfToL`; then `AlgebraicClosure.instAlgebra` lifts it to `Ω` and `IsAlgClosed.lift` supplies the
embedding `ψ`.
-/

open Polynomial

namespace GMC2.DvdKOmegaWiring

/-- **`SinglePolyCrux` holds** — the sole remaining hypothesis of `gmc2_of_crux`. -/
theorem singlePolyCrux_holds : GMC2.DvdKUnivariateReduction.SinglePolyCrux := by
  classical
  intro R M hM hMd hR0 hvanish
  set Φ := GMC2.PhiVieta.Phi R M with hΦdef
  have hΦ0 : Φ ≠ 0 := (GMC2.DvdKAssembly.irreducible_Phi R M hM hR0).ne_zero
  -- the non-synthesizable base algebra, via `rfToL`
  letI algRL : Algebra (RatFunc ℂ) (LaurentSeries ℂ) := (GMC2.DvdKPhiCoincide.rfToL).toAlgebra
  ...
  -- the divisor over Ω
  set Pω : Polynomial Ω :=
    ((GMC2.DvdKWeierstrass.smallRootFactor R M).map (HahnSeries.ofPowerSeries ℤ ℂ)).map
      (algebraMap (LaurentSeries ℂ) Ω) with hPωdef
  have halg : (algebraMap (LaurentSeries ℂ) Ω).comp (GMC2.DvdKPhiCoincide.rfToL)
      = algebraMap (RatFunc ℂ) Ω := rfl
  -- monic
  have hmonic : Pω.Monic :=
    ((GMC2.DvdKWeierstrass.smallRootFactor_monic R M).map _).map _
  ...
  have hdvd : Pω ∣ Φ.map (algebraMap (RatFunc ℂ) Ω) := by
    ...
    exact (Polynomial.map_dvd_map' (algebraMap (LaurentSeries ℂ) Ω)).mpr
      (GMC2.DvdKPhiCoincide.smallRootFactor_map_dvd_phiVieta_map R M)
  -- Φ.map is separable ⇒ its divisor Pω is squarefree ⇒ nodup roots
  have hsepΦ : (Φ.map (algebraMap (RatFunc ℂ) Ω)).Separable :=
    ((PerfectField.separable_of_irreducible (GMC2.DvdKAssembly.irreducible_Phi R M hM hR0))).map
  ...
  have hval : Pω.coeff 0 = algebraMap (RatFunc ℂ) Ω v := by
    rw [hPωdef, Polynomial.coeff_map, Polynomial.coeff_map,
      GMC2.DvdKTransposeAssembly.smallRootFactor_coeff0_of_vanish R M hM hvanish]
    ...
    have hcC : (HahnSeries.ofPowerSeries ℤ ℂ) ((algebraMap ℂ (PowerSeries ℂ)) (R.coeff 0))
        = GMC2.DvdKPhiCoincide.rfToL (RatFunc.C (R.coeff 0)) := by
      rw [← RingHom.comp_apply, GMC2.DvdKPhiCoincide.ofPowerSeries_comp_C,
        ← RatFunc.algebraMap_eq_C, ← RingHom.comp_apply, GMC2.DvdKPhiCoincide.rfToL_comp_algebraMap]
    rw [map_mul, map_neg, map_mul, map_neg, HahnSeries.ofPowerSeries_X,
      GMC2.DvdKPhiCoincide.rfToL_X, hcC]
    ring
  -- assemble via bridge
  obtain ⟨S, hS⟩ :=
    GMC2.FrameBridgeAssembly.hS_of_dvd_value Φ hΦ0 ψ Pω hmonic hPωsplit hPωnd hdvd v hval
  -- a root of Φ for the (unused) `x0` witness
  have hdeg : 0 < Φ.natDegree := by rw [hΦdef, GMC2.PhiVieta.natDegree_Phi R M hMd]; omega
  obtain ⟨x0, hx0⟩ : ∃ x : Φ.SplittingField, x ∈ Φ.rootSet Φ.SplittingField := by
    ...
    have hnd : (Φ.map (algebraMap (RatFunc ℂ) Φ.SplittingField)).natDegree ≠ 0 := by
      rw [Polynomial.natDegree_map_eq_of_injective (FaithfulSMul.algebraMap_injective _ _),
        hΦdef, GMC2.PhiVieta.natDegree_Phi R M hMd]; omega
  ...
  have hPωdeg : Pω.natDegree = M := by
    rw [hPωdef, Polynomial.natDegree_map_eq_of_injective (FaithfulSMul.algebraMap_injective _ _),
      ...
      GMC2.DvdKWeierstrass.smallRootFactor_natDegree]
