...
namespace GMC2SupportFaceBridge
...
@[simp] theorem faceEquivSupportFace_supportValue
    ...
    (faceEquivSupportFace P F hsubset s).1 =
      GMC2FaceSeedDescent.faceToSupport hsubset s := rfl
...
@[simp] theorem coefficient_faceEquivSupportFace
    ...
    coefficient (faceEquivSupportFace P F hsubset s).1 =
      coefficient (GMC2FaceSeedDescent.faceToSupport hsubset s) := rfl
...
@[simp] theorem tiltedHeight_faceEquivSupportFace
    ...
    GMC2FrobeniusFace.tiltedHeight
        (fun t : ↥(supportFace P F) ↦
          GMC2FaceDictionary.exponentA (t.1 : ↥P.support))
        (fun t : ↥(supportFace P F) ↦
          GMC2FaceDictionary.exponentB (t.1 : ↥P.support))
        lambda (faceEquivSupportFace P F hsubset s) =
      GMC2FrobeniusFace.tiltedHeight
        GMC2FaceDictionary.exponentA GMC2FaceDictionary.exponentB lambda s := rfl
...
theorem totalCharge_reindex
    (e : α ≃ β) (q : β → ℤ) (r : α → ℕ) :
    GMC2ConstantTermRelations.totalCharge q (reindexChannelEquiv e r) =
      GMC2ConstantTermRelations.totalCharge (q ∘ e) r := by
  unfold GMC2ConstantTermRelations.totalCharge
...
theorem aeval_constantTermRelationInt_equiv
    ...
    MvPolynomial.aeval (coefficient ∘ e)
        (GMC2IntegralRelations.constantTermRelationInt (q ∘ e) m) =
      MvPolynomial.aeval coefficient
        (GMC2IntegralRelations.constantTermRelationInt q m) := by
  rw [GMC2IntegralRelations.aeval_constantTermRelationInt,
    GMC2IntegralRelations.aeval_constantTermRelationInt]
  ...
  · intro r hr
    ...
    by_cases hcharge :
        GMC2ConstantTermRelations.totalCharge (q ∘ e) r = 0
...
theorem aeval_liftedFaceSeedInt_eq_supportFace
    ...
    MvPolynomial.aeval coefficient
        (GMC2IntegralFaceSeedDescent.liftedFaceSeedInt P F hsubset m0) =
  ...
  rw [GMC2IntegralFaceSeedDescent.aeval_liftedFaceSeedInt]
...
theorem isBalanced_iff_totalCharge_zero
  ...
  have hcharge :
      GMC2ConstantTermRelations.totalCharge
        ...
        ((GMC2MomentRelations.channelExponent exponent r) 0 : ℤ) -
          ((GMC2MomentRelations.channelExponent exponent r) 1 : ℤ) := by
    unfold GMC2ConstantTermRelations.totalCharge
    calc
      ...
      _ = ((GMC2MomentRelations.channelExponent exponent r) 0 : ℤ) -
          ((GMC2MomentRelations.channelExponent exponent r) 1 : ℤ) := by
        simp [GMC2MomentRelations.channelExponent]
...
theorem channelMass_eq_of_mem_piAntidiag
    ...
    GMC2FaceHeightFloor.channelMass Finset.univ r = m := by
...
theorem totalCharge_extendByZero
    ...
    GMC2ConstantTermRelations.totalCharge
        ...
        (GMC2ChannelDilation.extendByZero face s) =
      GMC2ConstantTermRelations.totalCharge
        (fun i : ↥face ↦ GMC2.charge (exponent i)) s := by
  unfold GMC2ConstantTermRelations.totalCharge
  simpa using
    (GMC2ChannelDilation.sum_extendByZero face s
...
theorem multinomial_extendByZero
  ...
  have hsum :
      (∑ i : ι, GMC2ChannelDilation.extendByZero face s i) =
        ∑ i : ↥face, s i := by
    simpa using
      (GMC2ChannelDilation.sum_extendByZero face s
        (fun _ n ↦ n) (by simp))
  have hprod :
      (∏ i : ι, Nat.factorial
          (GMC2ChannelDilation.extendByZero face s i)) =
        ∏ i : ↥face, Nat.factorial (s i) := by
    simpa using
      (GMC2ChannelDilation.prod_extendByZero face s
...
theorem aeval_constantTermRelationInt_eq_faceConstantTerm
  ...
  rw [GMC2IntegralRelations.aeval_constantTermRelationInt]
  rw [GMC2NormalizedResidue.faceConstantTerm,
    GMC2NormalizedResidue.balancedFaceChannels, Finset.sum_filter]
  apply Finset.sum_congr rfl
  intro s hs
  have hbalanced :=
  ...
  · have hb := hbalanced.mpr hcharge
    ...
    rfl
  · have hb : ¬ GMC2NormalizedMoment.IsBalanced exponent
        (GMC2ChannelDilation.extendByZero face s) :=
...
theorem support_totalChargeQ_eq_zero_of_isBalanced
    ...
    (hbalanced : GMC2NormalizedMoment.IsBalanced
      (fun s : ↥P.support ↦ (s : Fin 2 →₀ ℕ)) r) :
    GMC2FrobeniusFace.totalChargeQ
        (fun s : ↥P.support ↦ GMC2FaceDictionary.exponentA s)
        (fun s : ↥P.support ↦ GMC2FaceDictionary.exponentB s)
        Finset.univ r = 0 := by
  rw [GMC2FaceReferenceChannel.face_totalChargeQ_eq_cast_totalCharge]
...
theorem support_tiltedHeight_lower
    ...
    ∀ i ∈ (Finset.univ : Finset ↥P.support),
      delta ≤ GMC2FrobeniusFace.tiltedHeight
        (fun s : ↥P.support ↦ GMC2FaceDictionary.exponentA s)
        (fun s : ↥P.support ↦ GMC2FaceDictionary.exponentB s)
        lambda i := by
  intro i hi
  simpa only [GMC2FaceDictionary.tiltedHeight_eq] using hlower i i.property
...
theorem mem_supportFace_iff_tiltedHeight_eq
  ...
  constructor
  · intro hi
    simpa only [GMC2FaceDictionary.tiltedHeight_eq] using (hface i).1 hi |>.2
  · intro hi
    apply (hface i).2
    refine ⟨i.property, ?_⟩
    simpa only [GMC2FaceDictionary.tiltedHeight_eq] using hi
...
theorem channelExponent_dilate
    ...
    (exponent : ι → Fin 2 →₀ ℕ) :
    GMC2MomentRelations.channelExponent exponent
        (GMC2ChannelDilation.dilate p face s) =
      p • GMC2MomentRelations.channelExponent
        (fun i : ↥face ↦ exponent i) s := by
  simpa only [GMC2MomentRelations.channelExponent] using
    GMC2ChannelDilation.accumulatedBidegree_dilate p face s exponent
...
theorem channelHeight_dilate
    ...
    GMC2NormalizedMoment.channelHeight exponent
        (GMC2ChannelDilation.dilate p face s) =
      p * GMC2NormalizedMoment.channelHeight
        (fun i : ↥face ↦ exponent i) s := by
  unfold GMC2NormalizedMoment.channelHeight
...
theorem normalized_height_obligations_of_face_reference
    ...
    (hface : ∀ s, s ∈ F ↔ s ∈ P.support ∧
      GMC2.radialExponentQ s - lambda * GMC2.chargeQ s = delta)
    (hrefBalanced : GMC2FrobeniusFace.totalChargeQ
      (fun s : ↥F ↦ GMC2FaceDictionary.exponentA s)
      (fun s : ↥F ↦ GMC2FaceDictionary.exponentB s)
      Finset.univ r0 = 0)
    (hrefMass : GMC2FaceHeightFloor.channelMass Finset.univ r0 = m0)
    (hrefHeight : GMC2FrobeniusFace.radialHeightQ
      (fun s : ↥F ↦ GMC2FaceDictionary.exponentA s)
      Finset.univ r0 = (A0 : ℚ)) :
    ReferenceHeightObligations P F m0 A0 := by
  let exponentP : ↥P.support → Fin 2 →₀ ℕ := fun s ↦ s
  let aP : ↥P.support → ℤ := fun s ↦ GMC2FaceDictionary.exponentA s
  let bP : ↥P.support → ℤ := fun s ↦ GMC2FaceDictionary.exponentB s
  have hrefSupport : ∀ i ∈ (Finset.univ : Finset ↥F), r0 i ≠ 0 →
    ...
    simpa only [GMC2FaceDictionary.tiltedHeight_eq] using hiFace.2
  have hrefFloor := GMC2FaceHeightFloor.balanced_radialHeight_eq_floor_of_mass
    (fun s : ↥F ↦ GMC2FaceDictionary.exponentA s)
    (fun s : ↥F ↦ GMC2FaceDictionary.exponentB s)
  ...
  have hlowerP : ∀ i ∈ (Finset.univ : Finset ↥P.support),
      delta ≤ GMC2FrobeniusFace.tiltedHeight aP bP lambda i := by
    simpa only [aP, bP] using support_tiltedHeight_lower P lambda delta hlower
  have hradial : ∀ r : ↥P.support → ℕ,
      GMC2FrobeniusFace.radialHeightQ aP Finset.univ r =
        (GMC2NormalizedMoment.channelHeight exponentP r : ℚ) := by
    intro r
    simpa only [aP, exponentP] using
      GMC2FaceReferenceChannel.face_radialHeightQ_eq_cast_channelHeight P.support r
  have hbalancedQ : ∀ r : ↥P.support → ℕ,
      GMC2NormalizedMoment.IsBalanced exponentP r →
      GMC2FrobeniusFace.totalChargeQ aP bP Finset.univ r = 0 := by
    intro r hr
  ...
  · intro p r hr hbal
    ...
    have hfloor := GMC2FaceHeightFloor.balanced_radialHeight_floor_of_mass
    ...
    have hcast : ((p * A0 : ℕ) : ℚ) ≤
      ...
      calc
        ...
        _ = ((p * m0 : ℕ) : ℚ) * delta := by
          push_cast
          ring
        _ ≤ (GMC2NormalizedMoment.channelHeight exponentP r : ℚ) := hfloor
  ...
  · intro r hr hbal hsupported
    ...
    have hsuppHeight : ∀ i ∈ (Finset.univ : Finset ↥P.support), r i ≠ 0 →
        GMC2FrobeniusFace.tiltedHeight aP bP lambda i = delta := by
      intro i hi hri
      have hiFace := hsupported i hri
      simpa only [aP, bP] using
        (mem_supportFace_iff_tiltedHeight_eq P F lambda delta hface i).1 hiFace
    have heq := GMC2FaceHeightFloor.balanced_radialHeight_eq_floor_of_mass
      aP bP lambda delta Finset.univ r m0 hsuppHeight
      (hbalancedQ r hbal) hmass
    rw [hradial r] at heq
    have hcast :
        (GMC2NormalizedMoment.channelHeight exponentP r : ℚ) = (A0 : ℚ) := by
      rw [heq, hrefQ]
    exact_mod_cast hcast
  · intro r hr hbal hoff
    obtain ⟨j, hrj, hjFace⟩ := hoff
    have hmass := channelMass_eq_of_mem_piAntidiag r m0 hr
    have hjNe : GMC2FrobeniusFace.tiltedHeight aP bP lambda j ≠ delta := by
      intro hjEq
      apply hjFace
      exact (mem_supportFace_iff_tiltedHeight_eq P F lambda delta hface j).2
        (by simpa only [aP, bP] using hjEq)
    have hjStrict : delta < GMC2FrobeniusFace.tiltedHeight aP bP lambda j :=
      lt_of_le_of_ne (hlowerP j (Finset.mem_univ j)) (Ne.symm hjNe)
    have hoffStrict := GMC2FrobeniusFace.balanced_height_strict_of_off_face
    ...
    have hstrictQ : (A0 : ℚ) <
      ...
      calc
        ...
        _ = delta * GMC2FrobeniusFace.massQ Finset.univ r := by
          rw [GMC2FaceHeightFloor.massQ_eq_natCast_channelMass, hmass]
          ring
        _ < GMC2FrobeniusFace.radialHeightQ aP Finset.univ r := hoffStrict
        _ = (GMC2NormalizedMoment.channelHeight exponentP r : ℚ) := hradial r
    have hstrictNat : A0 < GMC2NormalizedMoment.channelHeight exponentP r := by
      exact_mod_cast hstrictQ
    have : A0 + 1 ≤ GMC2NormalizedMoment.channelHeight exponentP r := by
      omega
    simpa only [exponentP] using this

end

end GMC2SupportFaceBridge
