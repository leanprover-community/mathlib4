...
/- Begin misc lemma: these should be moved somewhere else in separate PR -/
...
theorem uniformContinuous_weierstrass {a : ℝ} (ha : a ∈ Set.Ioo 0 1) (b : ℝ) :
  ...
  refine Eventually.of_forall fun _ ↦ Finset.uniformContinuous_sum _ fun n _ ↦ ?_
  exact (lipschitzWith_cos.uniformContinuous.comp (uniformContinuous_id.const_mul' _)).const_mul' _

/-!
### Non-differentiability

To show that Weierstrass function $f(x)$ is not differentiable at aany $x$, we choose a sequence
...
theorem tendsto_inv_xm_sub_x {b : ℝ} (hb : 1 < b) (x : ℝ) :
  ...
  refine tendsto_nhdsWithin_iff.mpr ⟨?_, Eventually.of_forall fun m ↦ by simpa using lt_xm hb0 x m⟩
