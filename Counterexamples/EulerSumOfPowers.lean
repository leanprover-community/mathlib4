...
abbrev sumOfPowersConjectureAux {α β γ : Type*}
    [Zero α] [Zero β] [Zero γ] [Add γ] [HPow α ℕ γ] [HPow β ℕ γ] :=
  ∀ (n : ℕ) (a : List α) (b : β),
  n ≠ 0 → a ≠ ∅ → 0 ∉ a → b ≠ 0 → (a.map (· ^ n)).sum = b ^ n → n ≤ a.length

abbrev sumOfPowersConjecture := @sumOfPowersConjectureAux ℕ ℕ ℕ _ _ _ _ _ _

abbrev sumOfPowersConjectureInt := @sumOfPowersConjectureAux ℤ ℤ ℤ _ _ _ _ _ _

theorem sum_of_powers_conjecture_implies_flt_with (R : Type*) [Semiring R] :
    @sumOfPowersConjectureAux R R R _ _ _ _ _ _ → ∀ n ≥ 3, FermatLastTheoremWith R n := by
  intro conj n hn a b c ha hb hc hsum
  have : n ≤ 2 := conj n [a, b] c
    (by omega)
    (by simp)
...
theorem sum_of_powers_conjecture_implies_flt : sumOfPowersConjecture → FermatLastTheorem :=
  sum_of_powers_conjecture_implies_flt_with ℕ
...
-/
theorem sum_of_powers_conjecture_false₀ : ¬sumOfPowersConjecture := by
  intro conj
  let n := 5
  let a := [27, 84, 110, 133]
  let b := 144
  have : 5 ≤ 4 := conj n a b (by simp) (by simp [a]) (by simp [a]) (by simp) (by decide)
  omega
...
theorem sum_of_powers_conjecture_false₁ : ¬sumOfPowersConjecture := by
  ...
  let b := 20_615_673
  have : 4 ≤ 3 := conj n a b (by simp) (by simp [a]) (by simp [a]) (by simp) (by decide)
  omega
...
-/
theorem sum_of_powers_conjecture_int_false₀ : ¬sumOfPowersConjectureInt := by
  intro conj
  let n := 5
