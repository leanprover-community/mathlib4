...
def kIdeal : Ideal ((ZMod 2)[X,Y,Z]) :=
  Ideal.span (Set.range fun i =>
    (MvPolynomial.X i * MvPolynomial.X i : (ZMod 2)[X,Y,Z]))

theorem mem_kIdeal_iff (x : (ZMod 2)[X,Y,Z]) :
...
theorem X_Y_Z_notMem_kIdeal : (X * Y * Z : (ZMod 2)[X,Y,Z]) ∉ kIdeal := by
  intro h
...
theorem mul_self_mem_kIdeal_of_X_Y_Z_mul_mem {x : (ZMod 2)[X,Y,Z]}
...
theorem comap_C_kIdeal : kIdeal.comap (C : ZMod 2 →+* (ZMod 2)[X,Y,Z]) = ⊥ := by
  refine bot_unique ?_
...
instance K.charP : CharP K 2 := by
  ...
  have : Nat.castRingHom ((ZMod 2)[X,Y,Z]) = C.comp (Nat.castRingHom _) := by
    ext1 r; rfl
