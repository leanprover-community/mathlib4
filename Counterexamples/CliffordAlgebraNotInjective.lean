...
noncomputable def kIdeal : Ideal (MvPolynomial (Fin 3) (ZMod 2)) :=
...
/-- The generators of `K`. -/
noncomputable def K.gen (i : Fin 3) : K := Ideal.Quotient.mk _ (MvPolynomial.X i)
...
-/
@[simps!]
noncomputable def lFunc : (Fin 3 → K) →ₗ[K] K :=
...
noncomputable def Q' : QuadraticForm K (Fin 3 → K) :=
  ∑ i, sq i
...
@[simps!]
noncomputable def Q : QuadraticForm K L :=
