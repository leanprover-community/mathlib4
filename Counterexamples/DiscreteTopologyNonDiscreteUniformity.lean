...
lemma Int.eq_of_pow_sub_le {d : ℕ} {m n : ℤ} (hd1 : 1 < d)
  ...
  replace h : |(1 : ℝ) - d ^ (n - m)| < (d : ℝ)⁻¹ := by
    rw [← mul_lt_mul_iff_of_pos_left (a := (d : ℝ) ^ (-n)) (zpow_pos _ _),
      ...
      ← zpow_add₀ <| Nat.cast_ne_zero.mpr hd0.ne', sub_eq_add_neg (b := m),
      neg_add_cancel_left, ← abs_neg, neg_sub,
      abs_of_nonneg (a := (d : ℝ) ^ (-n)) (zpow_pos _ _).le, ← zpow_neg_one,
      ← zpow_add₀ <| Nat.cast_ne_zero.mpr hd0.ne', ← sub_eq_add_neg]
    · exact h
  ...
  · obtain ⟨a, ha⟩ := Int.eq_ofNat_of_zero_le (sub_nonneg.mpr H)
    rw [ha, ← mul_lt_mul_iff_of_pos_left (a := (d : ℝ)) <| Nat.cast_pos'.mpr hd0,
      mul_inv_cancel₀ <| Nat.cast_ne_zero.mpr hd0.ne',
    ...
    simp only [Nat.cast_eq_zero, hd0.ne', Nat.abs_cast, Nat.cast_eq_one, hd1.ne',
  ...
  · have h1 : (d : ℝ) ^ (n - m) ≤ 1 - (d : ℝ)⁻¹ := calc
      (d : ℝ) ^ (n - m) ≤ (d : ℝ)⁻¹ := by
        ...
        linarith
      _ ≤ 1 - (d : ℝ)⁻¹ := by
        rw [inv_eq_one_div, one_sub_div <| Nat.cast_ne_zero.mpr hd0.ne',
